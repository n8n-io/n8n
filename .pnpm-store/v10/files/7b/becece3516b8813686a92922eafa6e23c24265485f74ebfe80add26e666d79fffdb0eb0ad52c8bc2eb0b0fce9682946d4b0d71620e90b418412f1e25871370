 ...
 */
function expectFormDataToEqual(actual, expected) {
    ...
    for (let i = 0; i < expectedEntries.length; i++) {
        ...
        if (typeof expectedVal === 'string') {
            ...
            expect(actualVal.size).toBe(expectedVal.size);
            expect(actualVal.type).toBe(expectedVal.type);
        }
    }
}
describe('ApiClient', () => {
    ...
    describe('push()', () => {
        ...
        it('should push to remote', async () => {
            ...
            expectedFormData.append('files[some-file.yaml]', new Blob([filesMock[0].stream]));
