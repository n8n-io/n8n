...
const typeorm = require_rolldown_runtime.__toESM(require("typeorm"));

//#region src/vectorstores/typeorm.ts
var typeorm_exports = {};
require_rolldown_runtime.__export(typeorm_exports, {
...
*/
var TypeORMVectorStoreDocument = class extends __langchain_core_documents.Document {
	embedding;
};
...
*/
var TypeORMVectorStore = class TypeORMVectorStore extends __langchain_core_vectorstores.VectorStore {
	tableName;
	schemaName;
	documentEntity;
	filter;
	...
	constructor(embeddings, fields) {
		...
		this._verbose = fields.verbose ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("LANGCHAIN_VERBOSE") === "true";
	}
	/**
	...
	async similaritySearchVectorWithScore(query, k, filter) {
      ...
      LIMIT $3;`;
		...
		for (const doc of documents) if (doc._distance != null && doc.pageContent != null) {
			const document = new __langchain_core_documents.Document(doc);
			document.id = doc.id;
