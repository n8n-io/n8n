...
const __azure_search_documents = require_rolldown_runtime.__toESM(require("@azure/search-documents"));

//#region src/vectorstores/azure_aisearch.ts
var azure_aisearch_exports = {};
require_rolldown_runtime.__export(azure_aisearch_exports, {
...
*/
var AzureAISearchVectorStore = class AzureAISearchVectorStore extends __langchain_core_vectorstores.VectorStore {
	get lc_secrets() {
		return {
	...
	constructor(embeddings, config) {
		...
		const endpoint = config.endpoint ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("AZURE_AISEARCH_ENDPOINT");
		const key = config.key ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("AZURE_AISEARCH_KEY");
		...
		if (!config.client) {
			credentials ??= new __azure_search_documents.AzureKeyCredential(key);
			this.client = new __azure_search_documents.SearchClient(endpoint, this.indexName, credentials, { userAgentOptions: { userAgentPrefix: USER_AGENT_PREFIX } });
			const indexClient = new __azure_search_documents.SearchIndexClient(endpoint, credentials, { userAgentOptions: { userAgentPrefix: USER_AGENT_PREFIX } });
	...
	*/
	async deleteMany(filter) {
		...
		const deleteResults = [];
		const bufferedClient = new __azure_search_documents.SearchIndexingBufferedSender(this.client, (entity) => entity.id);
