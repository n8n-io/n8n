...
* @author Armano
*/
var require_no_template_shadow = /* @__PURE__ */ require_rolldown_runtime.__commonJSMin(((exports, module) => {
			...
			return utils.compositingVisitors(utils.isScriptSetup(context) ? { Program() {
				...
				VElement(node) {
					...
					for (const variable of node.variables) {
						...
						if (scopeStack.nodes.some((node$1) => node$1.name === name) || jsVars.has(name)) context.report({
							node: varNode,
							loc: varNode.loc,
