...
*/
var RedisVectorStore = class RedisVectorStore extends VectorStore {
	redisClient;
	indexName;
	indexOptions;
	createIndexOptions;
	...
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		if (results.total) {
			for (const res of results.documents) if (res.value) {
				...
				if (document.vector_score) result.push([new Document({
	...
	*/
	async createIndex(dimensions = 1536) {
		...
		if (this.customSchema) for (const [fieldName, fieldConfig] of Object.entries(this.customSchema)) {
			const indexedFieldName = `${this.metadataKey}.${fieldName}`;
			if (fieldConfig.type === SchemaFieldTypes.TAG) schema[indexedFieldName] = {
				type: SchemaFieldTypes.TAG,
				SORTABLE: fieldConfig.SORTABLE ? true : void 0,
				SEPARATOR: fieldConfig.SEPARATOR || ","
			};
			else if (fieldConfig.type === SchemaFieldTypes.NUMERIC) schema[indexedFieldName] = {
				type: SchemaFieldTypes.NUMERIC,
				SORTABLE: fieldConfig.SORTABLE ? true : void 0
			};
			else if (fieldConfig.type === SchemaFieldTypes.TEXT) schema[indexedFieldName] = {
	...
	buildCustomQuery(query, k, metadataFilter) {
		...
		if (metadataFilter && this.customSchema) {
			...
			for (const [fieldName, value] of Object.entries(metadataFilter)) if (this.customSchema[fieldName]) {
				...
				const indexedFieldName = `${this.metadataKey}.${fieldName}`;
				if (fieldConfig.type === SchemaFieldTypes.NUMERIC) if (typeof value === "object" && value !== null) {
				...
				else if (fieldConfig.type === SchemaFieldTypes.TAG) if (Array.isArray(value)) {
				...
				else if (fieldConfig.type === SchemaFieldTypes.TEXT) filterClauses.push(`@${indexedFieldName}:(${value})`);
			}
