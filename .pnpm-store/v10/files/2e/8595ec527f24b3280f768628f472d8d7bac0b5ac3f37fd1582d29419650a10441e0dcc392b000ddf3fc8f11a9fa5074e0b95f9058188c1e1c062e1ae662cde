...
const defaultRunnableBound = /* @__PURE__ */ new _langchain_core_runnables.RunnablePassthrough();
var PregelNode = class PregelNode extends _langchain_core_runnables.RunnableBinding {
	...
	getNode() {
		...
		else if (this.bound === defaultRunnableBound) return new _langchain_core_runnables.RunnableSequence({
			first: writers[0],
			middle: writers.slice(1, writers.length - 1),
			last: writers[writers.length - 1],
			omitSequenceTags: true
		});
		else if (writers.length > 0) return new _langchain_core_runnables.RunnableSequence({
			first: this.bound,
	...
	pipe(coerceable) {
		if (require_write.ChannelWrite.isWriter(coerceable)) return new PregelNode({
			...
			mapper: this.mapper,
			writers: this.writers,
			bound: (0, _langchain_core_runnables._coerceToRunnable)(coerceable),
			config: this.config,
			kwargs: this.kwargs,
			retryPolicy: this.retryPolicy,
			cachePolicy: this.cachePolicy
