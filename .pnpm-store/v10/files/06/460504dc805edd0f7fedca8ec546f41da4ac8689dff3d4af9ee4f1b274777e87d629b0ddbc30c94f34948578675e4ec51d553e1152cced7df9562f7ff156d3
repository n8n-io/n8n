import { _makeMessageChunkFromAnthropicEvent, extractToken, extractToolCallChunk, extractToolCalls, extractToolUseContent, formatMessagesForAnthropic } from "./anthropic.js";
	...
	*/
	...
	static prepareMessagesOutput(provider, response, fields) {
		...
		if (provider === "anthropic") {
			...
			const chunk = _makeMessageChunkFromAnthropicEvent(response, { coerceContentToString: fields?.coerceContentToString });
			if (!chunk) return void 0;
			const newToolCallChunk = extractToolCallChunk(chunk);
			let toolUseContent;
			const extractedContent = extractToolUseContent(chunk, void 0);
		...
		else if (responseBody.event_type === "text-generation" && typeof responseBody?.text === "string") return new ChatGenerationChunk({
			message: new AIMessageChunk({ content: responseBody.text }),
			text: responseBody.text
		});
		else if (responseBody.event_type === "search-queries-generation") return parseMessageCohere(responseBody);
		else if (responseBody.event_type === "stream-end" && responseBody.response !== void 0 && responseBody["amazon-bedrock-invocationMetrics"] !== void 0) return new ChatGenerationChunk({
...
function parseMessage(responseBody, asChunk) {
	...
	if (asChunk) return new ChatGenerationChunk({
		message: new AIMessageChunk({
...
function parseMessageCohere(responseBody, asChunk) {
	...
	if (asChunk) return new ChatGenerationChunk({
		message: new AIMessageChunk({ content: parsedContent }),
		text: parsedContent,
		generationInfo
	});
	else return {
		message: new AIMessage({ content: parsedContent }),
		text: parsedContent,
		generationInfo
	};
}
...
//#endregion
export { BedrockLLMInputOutputAdapter };
