...
var ZeroShotAgent = class ZeroShotAgent extends Agent {
	...
	*/
	static createPrompt(tools, args) {
		...
		const formatInstructions = renderTemplate(FORMAT_INSTRUCTIONS, "f-string", { tool_names: toolNames });
		const template = [
			prefix,
			toolStrings,
			formatInstructions,
	...
	*/
	...
	static async deserialize(data) {
		...
		return deserializeHelper(llm, tools, rest, (llm$1, tools$1, args) => ZeroShotAgent.fromLLMAndTools(llm$1, tools$1, {
			prefix: args.prefix,
			suffix: args.suffix,
