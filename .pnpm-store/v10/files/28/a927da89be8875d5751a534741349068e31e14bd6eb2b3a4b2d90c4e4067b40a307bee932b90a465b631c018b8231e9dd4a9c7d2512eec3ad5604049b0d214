...
async function createOpenAPIChain(spec, options = {}) {
	...
	const { llm = options.llm, prompt = __langchain_core_prompts.ChatPromptTemplate.fromMessages([__langchain_core_prompts.HumanMessagePromptTemplate.fromTemplate("Use the provided API's to respond to this user query:\n\n{query}")]), requestChain = new SimpleRequestChain({ requestMethod: async (name, args) => defaultExecutionMethod(name, args, {
