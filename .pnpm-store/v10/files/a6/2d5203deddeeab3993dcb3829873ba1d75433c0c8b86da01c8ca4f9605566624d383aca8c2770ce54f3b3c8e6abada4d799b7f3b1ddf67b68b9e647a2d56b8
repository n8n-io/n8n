...
var libsql_exports = {};
require_rolldown_runtime.__export(libsql_exports, { LibSQLVectorStore: () => LibSQLVectorStore });
/**
* A vector store using LibSQL/Turso for storage and retrieval.
*/
var LibSQLVectorStore = class LibSQLVectorStore extends __langchain_core_vectorstores.VectorStore {
	db;
	table;
	column;
	/**
	...
	async similaritySearchVectorWithScore(query, k, filter) {
      ...
      JOIN ${this.table} ON top_k.rowid = ${this.table}.rowid`,
		...
		if (typeof filter === "string") sql.sql += ` AND ${filter}`;
			...
			const builder = new require_sqlite_where_builder.SqliteWhereBuilder(filter);
