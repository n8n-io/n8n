...
var upstash_exports = {};
require_rolldown_runtime.__export(upstash_exports, { UpstashVectorStore: () => UpstashVectorStore });
...
*/
var UpstashVectorStore = class extends __langchain_core_vectorstores.VectorStore {
	index;
	caller;
	...
	constructor(embeddings, args) {
		...
		if (embeddings instanceof __langchain_core_utils_testing.FakeEmbeddings) this.useUpstashEmbeddings = true;
		const { index, namespace,...asyncCallerArgs } = args;
		this.index = index;
		this.caller = new __langchain_core_utils_async_caller.AsyncCaller(asyncCallerArgs);
	...
	*/
	async addVectors(vectors, documents, options) {
		...
		const vectorChunks = (0, __langchain_core_utils_chunk_array.chunkArray)(upstashVectors, CONCURRENT_UPSERT_LIMIT);
	...
	*/
	async _addData(documents, options) {
		...
		const namespace = this.index.namespace(this.namespace ?? "");
		const vectorChunks = (0, __langchain_core_utils_chunk_array.chunkArray)(upstashVectorsWithData, CONCURRENT_UPSERT_LIMIT);
		const batchRequests = vectorChunks.map((chunk) => this.caller.call(async () => namespace.upsert(chunk)));
		await Promise.all(batchRequests);
		return documentIds;
	}
	...
	*/
	static async fromTexts(texts, metadatas, embeddings, dbConfig) {
		...
		for (let i = 0; i < texts.length; i += 1) {
			...
			const newDocument = new __langchain_core_documents.Document({
				pageContent: texts[i],
				metadata
			});
...
Object.defineProperty(exports, 'upstash_exports', {
  enumerable: true,
  get: function () {
    return upstash_exports;
  }
});
