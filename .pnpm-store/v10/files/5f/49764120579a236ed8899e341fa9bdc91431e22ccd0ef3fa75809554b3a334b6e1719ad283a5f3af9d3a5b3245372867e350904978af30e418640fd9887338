...
*/
var TencentHunyuanEmbeddings = class extends __langchain_core_embeddings.Embeddings {
	tencentSecretId;
	tencentSecretKey;
	...
	constructor(fields) {
		super(fields ?? {});
		this.tencentSecretId = fields?.tencentSecretId ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("TENCENT_SECRET_ID");
		if (!this.tencentSecretId) throw new Error("Tencent SecretID not found");
		this.tencentSecretKey = fields?.tencentSecretKey ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("TENCENT_SECRET_KEY");
