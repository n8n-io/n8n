...
*/
var GraphCypherQAChain = class GraphCypherQAChain extends __langchain_classic_chains.BaseChain {
	graph;
	...
	static fromLLM(props) {
		...
		const qaChain = new __langchain_classic_chains.LLMChain({
			llm: qaLLM || llm,
			prompt: qaPrompt
		});
		const cypherGenerationChain = new __langchain_classic_chains.LLMChain({
			llm: cypherLLM || llm,
			prompt: cypherPrompt
		});
