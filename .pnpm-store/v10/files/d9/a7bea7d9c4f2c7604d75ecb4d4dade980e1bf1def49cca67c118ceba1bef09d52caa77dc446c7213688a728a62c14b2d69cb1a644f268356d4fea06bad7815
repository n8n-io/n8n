...
const __aws_sdk_client_dynamodb = require_rolldown_runtime.__toESM(require("@aws-sdk/client-dynamodb"));

//#region src/stores/message/dynamodb.ts
var dynamodb_exports = {};
require_rolldown_runtime.__export(dynamodb_exports, { DynamoDBChatMessageHistory: () => DynamoDBChatMessageHistory });
/**
...
* class.
*/
var DynamoDBChatMessageHistory = class extends __langchain_core_chat_history.BaseListChatMessageHistory {
	lc_namespace = [
		"langchain",
		"stores",
		"message",
	...
	*/
	...
	constructor({ tableName, sessionId, partitionKey, sortKey, messageAttributeName, config, key = {} }) {
		...
		this.client = new __aws_sdk_client_dynamodb.DynamoDBClient(config ?? {});
	...
	*/
	async getMessages() {
		try {
			...
			const response = await this.client.send(new __aws_sdk_client_dynamodb.GetItemCommand(params));
	...
	*/
	async addMessage(message) {
		try {
			...
			await this.client.send(new __aws_sdk_client_dynamodb.UpdateItemCommand(params));
		} catch (error) {
	...
	*/
	async clear() {
		try {
			...
			await this.client.send(new __aws_sdk_client_dynamodb.DeleteItemCommand(params));
		} catch (error) {
...
Object.defineProperty(exports, 'dynamodb_exports', {
  enumerable: true,
  get: function () {
    return dynamodb_exports;
  }
});
