...
*/
var GenerativeAgentMemoryChain = class GenerativeAgentMemoryChain extends require_base.BaseChain {
	static lc_name() {
	...
	*/
	async scoreMemoryImportance(memoryContent, runManager) {
		const prompt = __langchain_core_prompts.PromptTemplate.fromTemplate("On the scale of 1 to 10, where 1 is purely mundane (e.g., brushing teeth, making bed) and 10 is extremely poignant (e.g., a break up, college acceptance), rate the likely poignancy of the following piece of memory. Respond with a single integer.\nMemory: {memory_content}\nRating: ");
	...
	*/
	async getInsightsOnTopic(topic, now, runManager) {
		const prompt = __langchain_core_prompts.PromptTemplate.fromTemplate("Statements about {topic}\n{related_statements}\n\nWhat 5 high-level insights can you infer from the above statements? (example format: insight (because of 1, 5, 3))");
...
* ```
*/
var GenerativeAgentMemory = class extends __langchain_core_memory.BaseMemory {
	llm;
	memoryRetriever;
	verbose;
	reflectionThreshold;
