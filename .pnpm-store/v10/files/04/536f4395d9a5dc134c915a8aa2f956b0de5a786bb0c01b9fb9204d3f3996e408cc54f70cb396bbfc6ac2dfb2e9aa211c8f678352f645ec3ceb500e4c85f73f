...
var prisma_exports = {};
__export(prisma_exports, { PrismaVectorStore: () => PrismaVectorStore });
...
*/
var PrismaVectorStore = class PrismaVectorStore extends VectorStore {
	tableName;
	vectorColumnName;
	selectColumns;
	filter;
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		const results = [];
		for (const article of articles) if (article._distance != null && article[this.contentColumn] != null) results.push([new Document({
...
//#endregion
export { PrismaVectorStore, prisma_exports };
