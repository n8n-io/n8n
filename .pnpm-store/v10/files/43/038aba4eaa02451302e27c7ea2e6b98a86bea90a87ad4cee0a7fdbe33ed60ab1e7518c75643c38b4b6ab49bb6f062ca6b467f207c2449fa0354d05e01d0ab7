...
const mysql2_promise = require_rolldown_runtime.__toESM(require("mysql2/promise"));

//#region src/vectorstores/singlestore.ts
var singlestore_exports = {};
require_rolldown_runtime.__export(singlestore_exports, { SingleStoreVectorStore: () => SingleStoreVectorStore });
...
* functions.
*/
var SingleStoreVectorStore = class SingleStoreVectorStore extends __langchain_core_vectorstores.VectorStore {
	connectionPool;
	tableName;
	idColumnName;
	contentColumnName;
	...
	constructor(embeddings, config) {
		...
		this.connectionPool = (0, mysql2_promise.createPool)(withConnectAttributes(config));
	...
	*/
	async addVectors(vectors, documents) {
		...
		await Promise.all(vectors.map(async (vector, idx) => {
			try {
				await this.connectionPool.query((0, mysql2.format)(`INSERT INTO ${tableName}(
	...
	async similaritySearchTextAndVectorWithScore(query, vector, k, filter) {
		...
		switch (this.searchConfig.searchStrategy) {
			...
			case "FILTER_BY_VECTOR":
				queryText = (0, mysql2.format)(`SELECT ${this.contentColumnName}, ${this.metadataColumnName},
          ...
          FROM ${this.tableName} ${whereClause} ORDER BY __score ${OrderingDirective[this.distanceMetric]} LIMIT ?;`, [
				...
				break;
			case "WEIGHTED_SUM":
				queryText = (0, mysql2.format)(`SELECT ${this.contentColumnName}, ${this.metadataColumnName}, __score1 * ? + __score2 * ? as __score
          FROM (
...
Object.defineProperty(exports, 'singlestore_exports', {
  enumerable: true,
  get: function () {
    return singlestore_exports;
  }
});
