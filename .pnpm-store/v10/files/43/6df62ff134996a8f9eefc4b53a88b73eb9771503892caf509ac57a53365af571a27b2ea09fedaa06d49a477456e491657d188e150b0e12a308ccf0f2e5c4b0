import { collectReportViolationAndSuggestionData, findPossibleVariableValues, getContextIdentifiers, getKeyName, getMessageIdNodes, getReportInfo, getRuleInfo, isVariableFromParameter } from "../utils.js";

//#region lib/rules/no-unused-message-ids.ts
const rule = {
	meta: {
		type: "problem",
		docs: {
		...
		fixable: void 0,
		schema: [],
		messages: { unusedMessage: "The messageId \"{{messageId}}\" is never used." }
	},
	create(context) {
		...
		return {
			...
			"Program:exit"() {
				if (hasSeenUnknownMessageId || !hasSeenViolationReport) return;
				const scope = sourceCode.getScope(sourceCode.ast);
				const messageIdNodesUnused = messageIdNodes.filter((node) => !messageIdsUsed.has(getKeyName(node, scope)));
				for (const messageIdNode of messageIdNodesUnused) context.report({
					node: messageIdNode,
					messageId: "unusedMessage",
					data: { messageId: getKeyName(messageIdNode, scope) }
				});
			},
			CallExpression(node) {
				if (node.callee.type === "MemberExpression" && contextIdentifiers.has(node.callee.object) && node.callee.property.type === "Identifier" && node.callee.property.name === "report") {
					...
					for (const messageId of reportMessagesAndDataArray.map((obj) => obj.messageId).filter((id) => !!id)) {
						...
						if (values.length === 0 || values.some((val) => val.type !== "Literal")) hasSeenUnknownMessageId = true;
			...
			Property(node) {
				if (node.key.type === "Identifier" && node.key.name === "messageId") {
					hasSeenViolationReport = true;
					const values = node.value.type === "Literal" ? [node.value] : findPossibleVariableValues(node.value, scopeManager);
					if (values.length === 0 || values.some((val) => val.type !== "Literal") || node.value.type === "Identifier" && isVariableFromParameter(node.value, scopeManager)) hasSeenUnknownMessageId = true;
