 ...
 */
export function getColumns(context, ruleNamesAndRules) {
    const { options } = context;
    const { ruleListColumns } = options;
    const columns = {
        // Alphabetical order.
        [COLUMN_TYPE.CONFIGS_ERROR]: getConfigsThatSetARule(context, SEVERITY_TYPE.error).length > 0,
        [COLUMN_TYPE.CONFIGS_OFF]: getConfigsThatSetARule(context, SEVERITY_TYPE.off).length > 0,
        [COLUMN_TYPE.CONFIGS_WARN]: getConfigsThatSetARule(context, SEVERITY_TYPE.warn).length > 0,
