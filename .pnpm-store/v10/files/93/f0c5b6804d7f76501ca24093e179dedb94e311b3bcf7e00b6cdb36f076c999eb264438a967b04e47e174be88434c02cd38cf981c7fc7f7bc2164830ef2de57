...
const apify_client = require_rolldown_runtime.__toESM(require("apify-client"));

//#region src/document_loaders/web/apify_dataset.ts
var apify_dataset_exports = {};
require_rolldown_runtime.__export(apify_dataset_exports, { ApifyDatasetLoader: () => ApifyDatasetLoader });
/**
...
* ```
*/
var ApifyDatasetLoader = class ApifyDatasetLoader extends __langchain_core_document_loaders_base.BaseDocumentLoader {
	apifyClient;
	datasetId;
	...
	constructor(datasetId, config) {
		...
		this.caller = new __langchain_core_utils_async_caller.AsyncCaller(asyncCallerParams);
	}
	/**
	...
	*/
	...
	static _getApifyApiToken(config) {
		return config?.token ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("APIFY_API_TOKEN");
	}
	/**
