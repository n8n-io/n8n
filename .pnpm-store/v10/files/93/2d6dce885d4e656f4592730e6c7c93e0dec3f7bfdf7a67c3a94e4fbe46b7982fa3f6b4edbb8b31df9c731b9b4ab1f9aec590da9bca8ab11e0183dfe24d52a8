...
//#region lib/rules/no-unused-properties.js
/**
* @fileoverview Disallow unused properties, data and computed properties.
* @author Learning Equality
*/
var require_no_unused_properties = /* @__PURE__ */ require_rolldown_runtime.__commonJSMin(((exports, module) => {
	const utils = require_index.default;
	const eslintUtils = require("@eslint-community/eslint-utils");
	const { isJSDocComment } = require_comments$1.default;
	const { getStyleVariablesContext } = require_index$1.default;
	const { definePropertyReferenceExtractor, mergePropertyReferences } = require_property_references$1.default;
	/**
	...
	*/
	function findJSDocComment(node, sourceCode) {
		...
		while (currentNode) {
			...
			if (!tokenBefore || !eslintUtils.isCommentToken(tokenBefore)) return null;
		...
		if (tokenBefore && isJSDocComment(tokenBefore)) return tokenBefore;
		return null;
	}
	module.exports = {
		meta: {
		...
		create(context) {
			...
			*/
			function reportUnusedProperties() {
				for (const container of vueComponentPropertiesContainerMap.values()) {
					const propertyReferences = mergePropertyReferences([...container.propertyReferences, ...templatePropertiesContainer.propertyReferences]);
					const propertyReferencesForProps = mergePropertyReferences(container.propertyReferencesForProps);
			...
			*/
			...
			const scriptVisitor = utils.compositingVisitors(utils.defineScriptSetupVisitor(context, {
				...
				onVueObjectEnter(node, vueNode) {
					...
					for (const watcherOrExpose of utils.iterateProperties(node, new Set([GROUP_WATCHER, GROUP_EXPOSE]))) if (watcherOrExpose.groupName === GROUP_WATCHER) {
						...
						if (watcher.type === "object") {
							...
							if (property.kind === "init") for (const handlerValueNode of utils.iterateWatchHandlerValues(property)) container.propertyReferences.push(propertyReferenceExtractor.extractFromNameLiteral(handlerValueNode));
						}
...
Object.defineProperty(exports, 'default', {
  enumerable: true,
  get: function () {
    return require_no_unused_properties();
  }
});