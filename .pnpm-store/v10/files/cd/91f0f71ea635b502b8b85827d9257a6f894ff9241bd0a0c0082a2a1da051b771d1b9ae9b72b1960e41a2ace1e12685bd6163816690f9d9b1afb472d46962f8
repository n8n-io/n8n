...
require_rolldown_runtime.__export(airtable_exports, { AirtableLoader: () => AirtableLoader });
var AirtableLoader = class AirtableLoader extends __langchain_core_document_loaders_base.BaseDocumentLoader {
	apiToken;
	tableId;
	baseId;
	kwargs;
	...
	*/
	constructor({ tableId, baseId, kwargs = {} }) {
		super();
		this.apiToken = (0, __langchain_core_utils_env.getEnvironmentVariable)("AIRTABLE_API_TOKEN") || "";
		this.tableId = tableId;
		this.baseId = baseId;
		this.kwargs = kwargs;
		if (!this.apiToken) throw new Error("Missing Airtable API token. Please set AIRTABLE_API_TOKEN environment variable.");
		this.asyncCaller = new __langchain_core_utils_async_caller.AsyncCaller({
			maxRetries: 3,
			maxConcurrency: 5
		});
	}
...
Object.defineProperty(exports, 'airtable_exports', {
  enumerable: true,
  get: function () {
    return airtable_exports;
  }
});
