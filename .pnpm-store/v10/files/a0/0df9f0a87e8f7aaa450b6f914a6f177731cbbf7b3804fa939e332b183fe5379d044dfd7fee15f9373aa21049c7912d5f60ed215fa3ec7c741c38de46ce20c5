...
var datadog_exports = {};
require_rolldown_runtime.__export(datadog_exports, { DatadogLLMObsTracer: () => DatadogLLMObsTracer });
var DatadogLLMObsTracer = class extends __langchain_core_tracers_base.BaseTracer {
	name = "datadog_tracer";
	ddLLMObsEndpoint;
	endpoint = (0, __langchain_core_utils_env.getEnvironmentVariable)("DD_LLMOBS_ENDPOINT") || "https://api.datadoghq.com/api/unstable/llm-obs/v1/trace/spans";
	...
	constructor(fields) {
		...
		const apiKey = ddApiKey || (0, __langchain_core_utils_env.getEnvironmentVariable)("DD_API_KEY");
	...
	transformOutput(outputs, spanKind) {
		...
		if (spanKind === "llm") return {
			output: { messages: outputs?.generations?.flatMap((generations) => generations.map(({ message, text }) => {
				if ((0, __langchain_core_messages.isAIMessage)(message) && message?.usage_metadata) {
...
Object.defineProperty(exports, 'datadog_exports', {
  enumerable: true,
  get: function () {
    return datadog_exports;
  }
});
