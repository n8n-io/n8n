...
const __langchain_core_utils_tiktoken = require_rolldown_runtime.__toESM(require("@langchain/core/utils/tiktoken"));

//#region src/text_splitter.ts
var TextSplitter = class extends __langchain_core_documents.BaseDocumentTransformer {
	lc_namespace = [
		"langchain",
...
var TokenTextSplitter = class extends TextSplitter {
	...
	async splitText(text) {
		if (!this.tokenizer) this.tokenizer = await (0, __langchain_core_utils_tiktoken.getEncoding)(this.encodingName);
		const splits = [];
