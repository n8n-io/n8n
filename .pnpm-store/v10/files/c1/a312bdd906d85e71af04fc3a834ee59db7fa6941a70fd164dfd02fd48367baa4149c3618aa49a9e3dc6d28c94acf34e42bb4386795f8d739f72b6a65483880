import { getMetaDocsProperty, getRuleInfo, isUndefinedIdentifier } from "../utils.js";
import { getStaticValue } from "@eslint-community/eslint-utils";

//#region lib/rules/require-meta-docs-recommended.ts
function insertRecommendedProperty(fixer, objectNode, recommendedValue) {
	if (objectNode.properties.length === 0) return fixer.replaceText(objectNode, `{ recommended: ${recommendedValue} }`);
	const lastProperty = objectNode.properties.at(-1);
	if (!lastProperty) return fixer.replaceText(objectNode, `{ recommended: ${recommendedValue} }`);
	return fixer.insertTextAfter(lastProperty, `, recommended: ${recommendedValue}`);
}
const rule = {
	meta: {
		type: "suggestion",
	...
	create(context) {
		...
		const { docsNode, metaNode, metaPropertyNode: descriptionNode } = getMetaDocsProperty("recommended", ruleInfo, scopeManager);
		if (!descriptionNode) {
		...
		if (context.options[0]?.allowNonBoolean) return {};
