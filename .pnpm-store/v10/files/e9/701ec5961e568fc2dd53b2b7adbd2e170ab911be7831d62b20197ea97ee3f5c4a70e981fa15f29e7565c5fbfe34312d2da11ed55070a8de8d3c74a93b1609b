...
function getPropertyFromRule(context, ruleName, property) {
...
function getConfigurationColumnValueForRule(context, ruleName, severityType) {
    // Collect the emojis/badges for the configs that set the rule to this severity level.
    const emojisAndBadges = getEmojisForConfigsSettingRuleToSeverity(context, ruleName, severityType);
...
function buildRuleRow(context, ruleName, rule, columnsEnabled, pathToFile) {
    const columns = {
        // Alphabetical order.
        [COLUMN_TYPE.CONFIGS_ERROR]: getConfigurationColumnValueForRule(context, ruleName, SEVERITY_TYPE.error),
        [COLUMN_TYPE.CONFIGS_OFF]: getConfigurationColumnValueForRule(context, ruleName, SEVERITY_TYPE.off),
        [COLUMN_TYPE.CONFIGS_WARN]: getConfigurationColumnValueForRule(context, ruleName, SEVERITY_TYPE.warn),
...
function generateRulesListMarkdown(context, ruleNamesAndRules, columns, pathToFile) {
    ...
    return markdownTable(sanitizeMarkdownTable(context, [
        listHeaderRow,
        ...ruleNamesAndRules.map(([name, rule]) => buildRuleRow(context, name, rule, columns, pathToFile)),
    ]), { align: 'l' });
}
function generateRuleListMarkdownForRulesAndHeaders(context, rulesAndHeaders, headerLevel, columns, pathToFile) {
...
function getRulesAndHeadersForSplit(context, ruleNamesAndRules, ruleListSplit) {
    ...
    for (const ruleListSplitItem of ruleListSplit) {
        ...
        const valuesForThisPropertyFromUnusedRules = [
            ...new Set(unusedRules.map(([name]) => getPropertyFromRule(context, name, ruleListSplitItem))).values(),
        ];
        const valuesForThisPropertyFromAllRules = [
            ...new Set(ruleNamesAndRules.map(([name]) => getPropertyFromRule(context, name, ruleListSplitItem))).values(),
        ];
...
export function updateRulesList(context, ruleNamesAndRules, markdown, pathToFile) {
    ...
    if (listStartIndex === -1 || listEndIndex === -1) {
        throw new Error(`${relative(getPluginRoot(path), pathToFile)} is missing rules list markers: ${BEGIN_RULE_LIST_MARKER}${END_RULE_LIST_MARKER}`);
    }
    ...
    if (typeof ruleListSplit === 'function') {
        ...
        rulesAndHeaders.push(...getRulesAndHeadersForSplit(context, ruleNamesAndRules, ruleListSplit));
    }
    else {
