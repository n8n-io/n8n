...
* text embeddings.
*/
var BaseGoogleEmbeddings = class extends _langchain_core_embeddings.Embeddings {
	model;
	dimensions;
	connection;
	constructor(fields) {
	...
	*/
	async embedDocuments(documents) {
		const instanceChunks = (0, _langchain_core_utils_chunk_array.chunkArray)(documents.map((document) => ({ content: document })), 1);
