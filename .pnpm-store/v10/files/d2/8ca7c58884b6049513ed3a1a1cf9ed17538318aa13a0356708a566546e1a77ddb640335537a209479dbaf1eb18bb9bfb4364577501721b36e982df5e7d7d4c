...
* ```
*/
var ConversationalRetrievalQAChain = class ConversationalRetrievalQAChain extends require_base.BaseChain {
	static lc_name() {
		...
		if (Array.isArray(chatHistory)) {
			if (Array.isArray(chatHistory[0]) && typeof chatHistory[0][0] === "string") {
				...
				historyMessages = chatHistory.flat().map((stringMessage, i) => {
					if (i % 2 === 0) return new __langchain_core_messages.HumanMessage(stringMessage);
					else return new __langchain_core_messages.AIMessage(stringMessage);
				});
	...
	*/
	static fromLLM(llm, retriever, options = {}) {
		const { questionGeneratorTemplate, qaTemplate, qaChainOptions = {
			type: "stuff",
			prompt: qaTemplate ? __langchain_core_prompts.PromptTemplate.fromTemplate(qaTemplate) : void 0
		}, questionGeneratorChainOptions, verbose,...rest } = options;
		const qaChain = require_load.loadQAChain(llm, qaChainOptions);
		const questionGeneratorChainPrompt = __langchain_core_prompts.PromptTemplate.fromTemplate(questionGeneratorChainOptions?.template ?? questionGeneratorTemplate ?? question_generator_template);
...
//# sourceMappingURL=conversational_retrieval_chain.cjs.map