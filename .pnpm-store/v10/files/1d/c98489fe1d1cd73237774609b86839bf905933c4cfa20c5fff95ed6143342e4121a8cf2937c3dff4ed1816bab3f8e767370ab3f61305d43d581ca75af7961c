...
import { reprStr, repr, setVariableValue, callFunction, addCellArray, structify, containsBody, prepareQueryString, prepareCookies, cookieString, paramsString, } from "./common.js";
function isSupportedByWebServices(request) {
    return (["get", "post", "put", "delete", "patch"].includes(request.urls[0].method.toLowerCase().toString()) &&
...
function parseWebOptions(request) {
    ...
    if (request.urls[0].auth) {
        ...
        if (username.length) {
            ...
            const authHeader = `['Basic ' matlab.net.base64encode(${repr(joinWords(request.urls[0].auth, ":"))})]`;
...
function prepareBasicURI(request) {
    ...
    if (request.urls[0].queryList) {
        response.push(setVariableValue("baseURI", repr(request.urls[0].urlWithoutQueryList)));
...
export function toWebServices(request, warnings) {
    let lines = [
