...
* ```
*/
var VectorStoreRetrieverMemory = class extends __langchain_core_memory.BaseMemory {
	...
	*/
	async loadMemoryVariables(values) {
		const query = (0, __langchain_core_memory.getInputValue)(values, this.inputKey);
	...
	*/
	async saveContext(inputValues, outputValues) {
		...
		await this.vectorStoreRetriever.addDocuments([new __langchain_core_documents.Document({
			pageContent: text,
			metadata
		})]);
	}
