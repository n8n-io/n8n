...
`Kendra` extends `KendraClient` and additionally supports all operations, waiters, and paginators as methods.
This style may be familiar to you from the AWS SDK for JavaScript v2.

If you are bundling the AWS SDK, we recommend using only the bare-bones client (`KendraClient`).
...
const client = new Kendra({ region: "REGION" });
