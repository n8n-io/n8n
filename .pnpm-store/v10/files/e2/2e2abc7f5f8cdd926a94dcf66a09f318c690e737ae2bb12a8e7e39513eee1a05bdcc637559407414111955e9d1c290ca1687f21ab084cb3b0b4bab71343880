...
require_rolldown_runtime.__export(replicate_exports, { Replicate: () => Replicate });
/**
...
* ```
*/
var Replicate = class Replicate extends __langchain_core_language_models_llms.LLM {
	static lc_name() {
		return "Replicate";
	}
	get lc_secrets() {
	...
	constructor(fields) {
		super(fields);
		const apiKey = fields?.apiKey ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("REPLICATE_API_KEY") ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("REPLICATE_API_TOKEN");
	...
	async *_streamResponseChunks(prompt, options, runManager) {
		...
		for await (const chunk of stream) {
			...
			if (chunk.event === "done") yield new __langchain_core_outputs.GenerationChunk({
				text: "",
...
Object.defineProperty(exports, 'replicate_exports', {
  enumerable: true,
  get: function () {
    return replicate_exports;
  }
});
//# sourceMappingURL=replicate.cjs.map