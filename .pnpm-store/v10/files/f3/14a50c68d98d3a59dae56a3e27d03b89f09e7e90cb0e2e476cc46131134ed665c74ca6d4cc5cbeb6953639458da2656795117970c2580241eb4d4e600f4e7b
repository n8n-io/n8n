...
var QueryTransformer = class {
	...
	*/
	matchFunctionName(funcName) {
		if (funcName in __langchain_core_structured_query.Comparators) if (this.allowedComparators.length > 0) if (this.allowedComparators.includes(funcName)) return funcName;
		else throw new Error("Received comparator not allowed");
		else return funcName;
		if (funcName in __langchain_core_structured_query.Operators) if (this.allowedOperators.length > 0) if (this.allowedOperators.includes(funcName)) return funcName;
		...
		const traverse = (node) => {
			switch (node.type) {
				case "call_expression": {
					...
					if (funcName in __langchain_core_structured_query.Operators) return new __langchain_core_structured_query.Operation(funcName, node.args?.map((arg) => traverse(arg)));
					if (funcName in __langchain_core_structured_query.Comparators) {
						if (node.args && node.args.length === 2) {
							const [attribute, value] = node.args;
							return new __langchain_core_structured_query.Comparison(funcName, traverse(attribute), traverse(value));
						}
	...
	*/
	async parse(expression) {
		const expressionParser = new require_output_parsers_expression.ExpressionParser();
