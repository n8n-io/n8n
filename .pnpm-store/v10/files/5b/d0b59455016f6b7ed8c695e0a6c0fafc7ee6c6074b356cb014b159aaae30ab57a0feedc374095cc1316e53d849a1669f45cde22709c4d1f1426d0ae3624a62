...
class GenericContainer {
    ...
    async createArchiveToCopyToContainer() {
        const tar = (0, archiver_1.default)("tar");
        const filesToCopyWithStats = await Promise.all(this.filesToCopy.map(async (fileToCopy) => ({
            ...fileToCopy,
            stats: await fs_1.promises.stat(fileToCopy.source),
        })));
    ...
    withSecurityOpt(...securityOptions) {
    ...
    withCopyToContainerOptions(copyToContainerOptions) {
