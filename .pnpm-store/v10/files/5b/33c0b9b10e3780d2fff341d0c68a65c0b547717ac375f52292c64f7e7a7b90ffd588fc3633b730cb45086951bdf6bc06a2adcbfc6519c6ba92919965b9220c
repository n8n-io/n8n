...
//#region lib/rules/require-meta-docs-description.ts
const DEFAULT_PATTERN = /* @__PURE__ */ new RegExp("^(enforce|require|disallow)");
const rule = {
	meta: {
		type: "suggestion",
		docs: {
	...
	create(context) {
		...
		return { Program(ast) {
			...
			const { docsNode, metaNode, metaPropertyNode: descriptionNode } = getMetaDocsProperty("description", ruleInfo, scopeManager);
			if (!descriptionNode) {
				context.report({
			...
			if (typeof staticValue.value !== "string" || staticValue.value === "") context.report({
				node: descriptionNode.value,
				messageId: "wrongType"
			});
			else if (staticValue.value !== staticValue.value.trim()) context.report({
				node: descriptionNode.value,
				messageId: "extraWhitespace"
			});
			else if (!pattern.test(staticValue.value)) context.report({
				node: descriptionNode.value,
				messageId: "mismatch",
