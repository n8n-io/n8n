...
const __rockset_client_dist_codegen_api_js = require_rolldown_runtime.__toESM(require("@rockset/client/dist/codegen/api.js"));

//#region src/vectorstores/rockset.ts
var rockset_exports = {};
require_rolldown_runtime.__export(rockset_exports, {
...
*/
var RocksetStore = class RocksetStore extends __langchain_core_vectorstores.VectorStore {
	client;
	collectionName;
	workspaceName;
	textKey;
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		} })).results?.map((rocksetDoc) => [new __langchain_core_documents.Document({
	...
	*/
	static async collectionReady(dbConfig) {
		return (await dbConfig.client.collections.getCollection(dbConfig.workspaceName ?? "commons", dbConfig.collectionName)).data?.status === __rockset_client_dist_codegen_api_js.Collection.StatusEnum.READY;
	}
	/**
...
Object.defineProperty(exports, 'rockset_exports', {
  enumerable: true,
  get: function () {
    return rockset_exports;
  }
});
//# sourceMappingURL=rockset.cjs.map