...
const mem0MemoryToMessages = (memories) => {
	...
	if (memoryContent) messages.push(new SystemMessage(memoryContent));
	memories.forEach((memory) => {
		if (memory.messages) memory.messages.forEach((msg) => {
			const content = typeof msg.content === "string" ? msg.content : JSON.stringify(msg.content);
			if (msg.role === "user") messages.push(new HumanMessage(content));
			else if (msg.role === "assistant") messages.push(new AIMessage(content));
			else if (content) messages.push(new ChatMessage(content, msg.role));
		});
	});
	return messages;
};
...
* ```
*/
var Mem0Memory = class extends chat_memory_exports.BaseChatMemory {
	memoryKey = "history";
	apiKey;
	sessionId;
	humanPrefix = "Human";
	...
	*/
	async loadMemoryVariables(values) {
		...
		return { [this.memoryKey]: this.separateMessages ? getBufferString(mem0MemoryToMessages(memories), this.humanPrefix, this.aiPrefix) : condenseMem0MemoryIntoHumanMessage(memories).content ?? "" };
	}
	/**
...
//#endregion
export { Mem0Memory, condenseMem0MemoryIntoHumanMessage, mem0MemoryContextToSystemPrompt, mem0MemoryToMessages, mem0_exports };
