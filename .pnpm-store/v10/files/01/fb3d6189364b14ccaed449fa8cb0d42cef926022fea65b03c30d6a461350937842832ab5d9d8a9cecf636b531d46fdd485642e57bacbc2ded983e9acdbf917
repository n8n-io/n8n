...
function ensureESLint9UnsupportedApi(input) {
    if (!isESLint9UnsupportedApi(input)) {
        throw new TypeError('ESLint 10 and later versions remove the FileEnumerator API, which is required by the "no-unused-modules" rule.');
    }
    return input;
}
...
function listFilesUsingFileEnumerator(src, extensions) {
    const { FileEnumerator, shouldUseFlatConfig } = ensureESLint9UnsupportedApi(eslintUnsupportedApi);
