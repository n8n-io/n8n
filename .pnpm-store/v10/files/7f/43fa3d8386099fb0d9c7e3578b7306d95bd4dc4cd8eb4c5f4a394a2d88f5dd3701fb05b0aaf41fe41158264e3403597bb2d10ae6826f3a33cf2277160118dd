...
class VideoRecorder {
  ...
  async _launch() {
    await (0, import_utils.mkdirIfNeeded)(this._options.outputFile);
  ...
  async stop() {
    ...
    if (this._isStopped)
      return;
    if (!this._lastFrame) {
      this._writeFrame(createWhiteImage(this._options.width, this._options.height), (0, import_utils.monotonicTime)());
    }
