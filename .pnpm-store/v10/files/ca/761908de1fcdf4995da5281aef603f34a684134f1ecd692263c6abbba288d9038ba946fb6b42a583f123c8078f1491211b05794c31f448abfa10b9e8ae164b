...
const __langchain_core_utils_ssrf = require_rolldown_runtime.__toESM(require("@langchain/core/utils/ssrf"));
...
var RecursiveUrlLoader = class extends __langchain_core_document_loaders_base.BaseDocumentLoader {
	...
	getChildLinks(html, baseUrl) {
		...
		for (const link of allLinks) {
			...
			if (link.startsWith("http")) {
				const isAllowed = !this.preventOutside || (0, __langchain_core_utils_ssrf.isSameOrigin)(link, baseUrl);
	...
	async getUrlAsDoc(url) {
		let res;
		try {
			(0, __langchain_core_utils_ssrf.validateSafeUrl)(url, { allowHttp: true });
	...
	async getChildUrlsRecursive(inputUrl, visited = /* @__PURE__ */ new Set(), depth = 0) {
		...
		let res;
		try {
			await (0, __langchain_core_utils_ssrf.validateSafeUrl)(url, { allowHttp: true });
			res = await this.fetchWithTimeout(url, { timeout: this.timeout });
			res = await res.text();
		} catch {
			return [];
