...
const _sfc_main$1 = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(__props, { emit: emits }) {
    ...
    const style = vue.toRef(() => attrs.style ?? {});
    ...
    return (_ctx, _cache) => {
      return !_ctx.hidden && _ctx.dimensions.width !== 0 && _ctx.dimensions.height !== 0 ? (vue.openBlock(), vue.createElementBlock(vue.Fragment, { key: 0 }, [
        vue.unref(miniMapSlots)[`node-${props.type}`] ? (vue.openBlock(), vue.createBlock(vue.resolveDynamicComponent(vue.unref(miniMapSlots)[`node-${props.type}`]), vue.normalizeProps(vue.mergeProps({ key: 0 }, { ...props, ..._ctx.$attrs })), null, 16)) : (vue.openBlock(), vue.createElementBlock("rect", vue.mergeProps({
          key: 1,
          id: _ctx.id
        }, _ctx.$attrs, {
...
const _sfc_main = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(__props, { emit }) {
    ...
    const bb = vue.computed(() => core.getRectOfNodes(getNodesInitialized.value.filter((node) => !node.hidden)));
      ...
      (onCleanup) => {
        if (el.value) {
          ...
          const zoomHandler = (event) => {
            ...
            const factor = event.sourceEvent.ctrlKey && core.isMacOs() ? 10 : 1;
          ...
          const zoomAndPanHandler = zoom().wheelDelta((event) => core.wheelDelta(event) * (__props.zoomStep / 10)).on("zoom", __props.pannable ? panHandler : () => {
