...
require_rolldown_runtime.__export(moonshot_exports, { ChatMoonshot: () => ChatMoonshot });
...
var ChatMoonshot = class extends __langchain_core_language_models_chat_models.BaseChatModel {
	static lc_name() {
		return "ChatMoonshot";
	}
	get callKeys() {
	...
	constructor(fields = {}) {
		super(fields);
		this.apiKey = fields?.apiKey ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("MOONSHOT_API_KEY");
	...
	*/
	...
	async _generate(messages, options, runManager) {
		...
		return {
			generations: [{
				text,
				message: new __langchain_core_messages.AIMessage(text)
			}],
