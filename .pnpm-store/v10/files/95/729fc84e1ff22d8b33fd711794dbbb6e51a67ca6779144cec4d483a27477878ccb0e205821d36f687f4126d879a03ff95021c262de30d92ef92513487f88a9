...
var AutoGPT = class AutoGPT {
	...
	constructor({ aiName, memory, chain, outputParser, tools, feedbackTool, maxIterations }) {
		...
		const chunkSize = (0, __langchain_core_language_models_base.getEmbeddingContextSize)("modelName" in memory.vectorStore.embeddings ? memory.vectorStore.embeddings.modelName : void 0);
		this.textSplitter = new require_text_splitter.text_splitter_exports.TokenTextSplitter({
			chunkSize,
	...
	*/
	static fromLLMAndTools(llm, tools, { aiName, aiRole, memory, maxIterations = 100, outputParser = new require_output_parser.AutoGPTOutputParser() }) {
		const prompt = new require_prompt.AutoGPTPrompt({
			aiName,
			aiRole,
			tools,
			tokenCounter: llm.getNumTokens.bind(llm),
			sendTokenLimit: (0, __langchain_core_language_models_base.getModelContextSize)("modelName" in llm ? llm.modelName : "gpt2")
		});
	...
	*/
	async run(goals) {
		...
		while (loopCount < this.maxIterations) {
			...
			this.fullMessageHistory.push(new __langchain_core_messages.HumanMessage(user_input));
			this.fullMessageHistory.push(new __langchain_core_messages.AIMessage(assistantReply));
			const action = await this.outputParser.parse(assistantReply);
			const tools = this.tools.reduce((acc, tool) => ({
				...acc,
				[tool.name]: tool
			}), {});
			if (action.name === require_schema.FINISH_NAME) return action.args.response;
			let result;
			...
			this.fullMessageHistory.push(new __langchain_core_messages.SystemMessage(result));
		}
		return void 0;
	}
};
