...
// src/index.js
var import_events3 = require("events"), import_utils = require("ioredis/built/utils/index"), import_command_list = require("../data/command-list.json");
...
function throwIfNotConnected(commandName, RedisMock2) {
  if (isNotConnected(RedisMock2) && commandName !== "connect" && commandName !== "defineCommand" && !RedisMock2.options.enableOfflineQueue)
    throw new Error(
...
// src/commands/bitcount.js
var POPCOUNT_TABLE = new Array(256);
for (let i = 0; i < 256; i++) {
  let count = 0, n = i;
  for (; n; )
    count += n & 1, n >>= 1;
...
function bitcount(key, start, end) {
  ...
  let value = this.data.get(key), buffer;
  Buffer.isBuffer(value) ? buffer = value : buffer = Buffer.from(value);
  let len = buffer.length;
  if (start === void 0 && end === void 0) {
    let count2 = 0;
    for (let i = 0; i < len; i++)
      count2 += POPCOUNT_TABLE[buffer[i]];
    return count2;
  }
  let startIdx = start !== void 0 ? parseInt(start, 10) : 0, endIdx = end !== void 0 ? parseInt(end, 10) : len - 1;
  if (Number.isNaN(startIdx) && (startIdx = 0), Number.isNaN(endIdx) && (endIdx = 0), startIdx < 0 && (startIdx = len + startIdx), endIdx < 0 && (endIdx = len + endIdx), startIdx < 0 && (startIdx = 0), endIdx < 0 && (endIdx = 0), startIdx > endIdx || startIdx >= len) return 0;
  endIdx >= len && (endIdx = len - 1);
  let count = 0;
  for (let i = startIdx; i <= endIdx; i++)
    count += POPCOUNT_TABLE[buffer[i]];
  return count;
}
var bitcountBuffer = bitcount;
...
function parseEvents(keyspaceEventsConfigString) {
  let result = { ...allEventsDisabled }, unAliasedString = keyspaceEventsConfigString.replace("A", "g$lshzxe");
...
function discard() {
  ...
  return this.dirty = !1, this.watching.clear(), this.batch = void 0, "OK";
}
...
function getCountAndMatch(args) {
  ...
  else throw test.startsWith("MATCH") || test.startsWith("COUNT") ? new Error("BAD Syntax") : new Error(`Uknown option ${args[0]}`);
  if (Number.isNaN(count))
    throw new Error("count must be integer");
  return [count, matchPattern];
}
function scanHelper(allKeysOrEntries, size, cursorStart, ...args) {
...
function hscan(key, cursor, ...args) {
  if (!this.data.has(key))
    return ["0", []];
  let entries = Object.entries(this.data.get(key)), [cur, scannedEntries] = scanHelper(entries, 1, cursor, ...args);
      ...
      case "[": {
        ...
        for (not && (p++, patternLen--); ; ) {
          if (pattern2[p] === "\\" && patternLen >= 2)
            ...
            } else nocase ? pattern2[p].toLowerCase() === string[s].toLowerCase() && (match = !0) : pattern2[p] === string[s] && (match = !0);
          }
          p++, patternLen--;
        }
        if (not && (match = !match), !match) return !1;
        s++, stringLen--;
        break;
      }
      case "\\":
      ...
      default:
        if (nocase) {
          if (pattern2[p].toLowerCase() !== string[s].toLowerCase())
            return !1;
        } else if (pattern2[p] !== string[s]) return !1;
        s++, stringLen--;
        break;
    }
...
// src/commands/lmpop.js
function lmpop(...args) {
  var _a, _b;
  if (args.length < 3)
    throw new Error("ERR wrong number of arguments for 'lmpop' command");
  let numKeys = parseInt(args[0], 10);
  if (Number.isNaN(numKeys) || numKeys < 1)
    throw new Error("ERR numkeys should be greater than 0");
  if (args.length < numKeys + 2)
    throw new Error("ERR wrong number of arguments for 'lmpop' command");
  let keys2 = args.slice(1, numKeys + 1), direction = (_a = args[numKeys + 1]) == null ? void 0 : _a.toLowerCase();
  if (direction !== "left" && direction !== "right")
    throw new Error("ERR syntax error");
  let count = 1, remainingArgs = args.slice(numKeys + 2);
  if (remainingArgs.length > 0)
    if (((_b = remainingArgs[0]) == null ? void 0 : _b.toLowerCase()) === "count") {
      if (remainingArgs.length < 2)
        throw new Error("ERR syntax error");
      let parsedCount = parseInt(remainingArgs[1], 10);
      if (Number.isNaN(parsedCount) || parsedCount < 1)
        throw new Error("ERR count should be greater than 0");
      if (count = parsedCount, remainingArgs.length > 2)
        throw new Error("ERR syntax error");
    } else
      throw new Error("ERR syntax error");
  for (let key of keys2) {
    ...
    if (list.length > 0) {
      let actualCount = Math.min(count, list.length), result = [];
      for (let i = 0; i < actualCount; i++)
        direction === "left" ? result.push(list.shift()) : result.push(list.pop());
      return list.length > 0 ? this.data.set(key, list) : this.data.delete(key), [key, result];
    }
  }
  return null;
}
function lmpopBuffer(...args) {
  let val = lmpop.apply(this, args);
  return convertStringToBuffer(val);
}
...
`).find((line) => line.indexOf("redis_version") !== -1), redisVersion = (0, import_semver.coerce)(redisRawVersion).version;
  ...
  return version == 7 ? version7(redisVersion) : version == 6 ? version6(redisVersion) : version == 5 ? version5(redisVersion) : version8();
}
...
function lpop(key, count) {
  ...
  if (list.length === 0)
    return null;
  if (count) {
    let items = list.slice(0, count), remainingItems = list.slice(count);
    return remainingItems.length > 0 ? this.data.set(key, remainingItems) : this.data.delete(key), items;
  }
  let item = list.shift();
  return list.length > 0 ? this.data.set(key, list) : this.data.delete(key), item;
}
function lpopBuffer(key, count) {
  let val = lpop.apply(this, [key, count]);
  return convertStringToBuffer(val);
}

// src/commands/lpos.js
function lpos(key, element, ...args) {
  if (this.data.has(key) && !(this.data.get(key) instanceof Array))
    throw new Error(`Key ${key} does not contain a list`);
  let list = this.data.get(key) || [], rank = 1, count = null, maxlen = list.length;
  for (let i = 0; i < args.length; i++) {
    let arg = String(args[i]).toUpperCase();
    if (arg === "RANK" && i + 1 < args.length) {
      if (rank = parseInt(args[i + 1], 10), rank === 0)
        throw new Error(
      ...
      if (count = parseInt(args[i + 1], 10), count < 0)
        throw new Error("ERR COUNT can not be negative");
      i++;
    } else if (arg === "MAXLEN" && i + 1 < args.length) {
      if (maxlen = parseInt(args[i + 1], 10), maxlen < 0)
        throw new Error("ERR MAXLEN can not be negative");
      i++;
    }
  }
  let matches = [], searchLength = Math.min(maxlen, list.length);
  for (let i = 0; i < searchLength; i++)
    list[i] === element && matches.push(i);
  if (matches.length === 0)
  ...
  if (rank > 0) {
    ...
    let absRank = Math.abs(rank);
    if (absRank > matches.length)
      return count !== null ? [] : null;
    selectedMatches = matches.slice(0, matches.length - absRank + 1).reverse();
  }
  return count !== null ? count === 0 ? selectedMatches : selectedMatches.slice(0, count) : selectedMatches.length > 0 ? selectedMatches[0] : null;
}
function lposBuffer(...args) {
  let val = lpos.apply(this, args);
  return convertStringToBuffer(val);
}
...
function encoding(key) {
  let val = this.data.get(key);
  return val instanceof Set ? [...val].every((n) => n === parseInt(n, 10).toString()) ? "intset" : "listpack" : val instanceof Map || Array.isArray(val) ? "listpack" : typeof val.valueOf() == "string" ? val === parseInt(val, 10).toString() ? "int" : "embstr" : "listpack";
}
...
function pubsub(subCommand, ...args) {
  var _a, _b;
  switch (subCommand) {
    case "CHANNELS": {
      let channels = [], pattern2 = args[0];
    ...
    case "NUMSUB": {
      let result = [];
      return args.forEach((channel) => {
        var _a2, _b2, _c;
        result.push(channel), result.push(((_c = (_b2 = (_a2 = this.channels) == null ? void 0 : _a2.instanceListeners) == null ? void 0 : _b2.get(channel)) == null ? void 0 : _c.size) || 0);
      }), result;
    }
    default:
...
function rpop(key, count) {
  ...
  if (list.length === 0)
    return null;
  if (count) {
    let itemCount = Math.min(count, list.length), items = [];
    for (let i = 0; i < itemCount; i++)
      items.push(list.pop());
    return list.length > 0 ? this.data.set(key, list) : this.data.delete(key), items;
  }
  let item = list.pop();
  return list.length > 0 ? this.data.set(key, list) : this.data.delete(key), item;
}
function rpopBuffer(key, count) {
  let val = rpop.apply(this, [key, count]);
  return convertStringToBuffer(val);
}
...
function rpoplpush(source, destination) {
  ...
  let newSource = this.data.get(source), item = newSource.pop(), newDest = newSource;
  return source !== destination && (newDest = this.data.get(destination)), newDest.unshift(item), this.data.set(source, newSource), newSource !== newDest && this.data.set(destination, newDest), item;
}
...
function set(key, value, ...options) {
  let nx = options.indexOf("NX") !== -1, xx = options.indexOf("XX") !== -1, get2 = options.indexOf("GET") !== -1, filteredOptions = options.filter(
    (option) => option !== "NX" && option !== "XX"
  );
  if (nx && xx) throw new Error("ERR syntax error");
  if (nx && this.data.has(key))
    return get2 ? this.data.get(key) : null;
  if (xx && !this.data.has(key)) return null;
  let result = "OK";
  get2 && (result = this.data.has(key) ? this.data.get(key) : null), this.data.set(key, value);
  let expireOptions = new Map(createGroupedArray(filteredOptions, 2));
  return expireOptions.has("EX") ? expire.call(this, key, expireOptions.get("EX")) : expireOptions.has("PX") ? pexpire.call(this, key, expireOptions.get("PX")) : expireOptions.has("EXAT") ? expireat.call(this, key, expireOptions.get("EXAT")) : expireOptions.has("PXAT") ? pexpireat.call(this, key, expireOptions.get("PXAT")) : this.expires.delete(key), result;
}
...
function setrange(key, offset, value) {
  ...
  return start > 0 && (newVal = oldVal.slice(0, start)), newVal += value, end < oldVal.length && (newVal += oldVal.slice(end, oldVal.length + 1)), newVal.length < end && (newVal = newVal.padStart(end, "\0")), this.data.set(key, newVal), newVal.length;
}
...
// src/commands/unwatch.js
function unwatch() {
  return this.dirty = !1, this.watching.clear(), "OK";
}
function unwatchBuffer() {
  let val = unwatch.call(this);
  return convertStringToBuffer(val);
}

// src/commands/watch.js
function watch(...keys2) {
  if (!this.dirty)
    for (let key of keys2)
      this.watching.add(key);
  return "OK";
}
function watchBuffer(...keys2) {
  let val = watch.apply(this, keys2);
  return convertStringToBuffer(val);
}
...
function xread(option, ...args) {
  ...
  let half = Math.ceil(rest.length / 2), toPoll = rest.slice(0, half).map((ele, index) => [ele, rest[half + index]]), pollStream = (stream, id, count = 1) => {
  ...
  return op === "BLOCK" ? new Promise((resolve) => {
    let timeElapsed = 0, f = () => setTimeout(() => {
      ...
      return events.find((event) => event[1] && event[1].length > 0) ? resolve(events) : (timeElapsed += 100, f());
    }, 100);
    f();
  }) : new Promise((resolve) => {
...
function normalizeCountToIndex(offset, count, array) {
  return count < 0 ? count === -1 ? array.length : -count > array.length ? 0 : array.length + count : offset + count;
}
...
function zdiff(numkeys, ...vals) {
  if (vals.length === 0)
    throw new Error("ERR wrong number of arguments for 'zdiff' command");
  let [keys2, withScores] = vals[vals.length - 1] === "WITHSCORES" ? [vals.slice(0, -1), !0] : [vals, !1];
  if ((Number.isInteger(numkeys) ? numkeys : parseInt(numkeys, 10)) !== keys2.length)
    throw new Error(
      `ERR numkeys must match the number of keys. numkeys===${numkeys} keys but got ${keys2.length} keys`
    );
  let firstKeyMembers = zrange.call(this, keys2[0], 0, -1), otherMembers = new Set(
    keys2.slice(1).map((key) => zrange.call(this, key, 0, -1)).flat()
  ), diffedMembers = firstKeyMembers.filter(
    (member) => !otherMembers.has(member)
  );
  return withScores ? diffedMembers.map((member) => [member, zscore.call(this, keys2[0], member)]).flat() : diffedMembers;
}
function zdiffBuffer(numkeys, ...vals) {
  let val = zdiff.apply(this, numkeys, vals);
  return convertStringToBuffer(val);
}
...
// src/commands/zmscore.js
function zmscore(key, ...members) {
  let map = this.data.get(key);
  return !map || !(map instanceof Map) ? Array(members.length).fill(null) : members.map((member) => {
    let entry = map.get(member);
    return entry ? entry.score.toString() : null;
  });
}
function zmscoreBuffer(...args) {
  let val = zmscore.apply(this, args);
  return convertStringToBuffer(val);
}
...
// src/commands-utils/emitDisconnectEvent.js
function emitConnectEvent2(redisMock) {
  process.nextTick(() => {
    redisMock.emit("end");
  });
}
...
function createSharedData(sharedExpires, modifiedKeyEvents = new import_events.EventEmitter()) {
  let raw = {};
  return Object.freeze({
    ...
    delete(key) {
      sharedExpires.has(key) && sharedExpires.delete(key), delete raw[key], modifiedKeyEvents.emit("modified", key);
    },
    get(key) {
    ...
    keys(prefix) {
      let validKeys = Object.keys(raw).filter((key) => sharedExpires.has(key) ? !sharedExpires.isExpired(key) : !0);
      return prefix ? validKeys.filter((key) => key.startsWith(prefix)) : validKeys;
    },
    set(key, val) {
      let item = val;
      Array.isArray(val) ? item = val.slice() : Buffer.isBuffer(val) ? item = Buffer.from(val) : val instanceof Set ? item = new Set(val) : val instanceof Map ? item = new Map(val) : typeof val == "object" && val && (item = { ...val }), raw[key] = item, modifiedKeyEvents.emit("modified", key);
    }
  });
}
...
function createContext(keyPrefix) {
  let expires = createSharedExpires(), modifiedKeyEvents = new import_events2.EventEmitter();
  return {
    channels: new import_events2.EventEmitter(),
    expires,
    data: createSharedData(expires, modifiedKeyEvents),
    patternChannels: new import_events2.EventEmitter(),
    keyPrefix,
    modifiedKeyEvents
  };
}
...
var Pipeline = class {
  ...
  _isDirty() {
    if (this.redis.dirty)
      return !0;
    for (let key of this.redis.watching)
  ...
  exec(callback) {
    if (this._isDirty())
      return this.redis.dirty = !1, this.redis.watching.clear(), this.batch = void 0, (0, import_as_callback2.default)(Promise.resolve(null), callback);
...
var defaultOptions = {
...
}, RedisMock = class _RedisMock extends import_events3.EventEmitter {
  constructor(...args) {
    ...
    if (this.keyData = `${optionsWithDefault.host}:${optionsWithDefault.port}/${optionsWithDefault.db}`, !context_default.get(this.keyData)) {
      let context = createContext(optionsWithDefault.keyPrefix);
      context_default.set(this.keyData, context);
    }
    this.context = context_default.get(this.keyData), this.expires = createExpires(
      ...
      optionsWithDefault.keyPrefix
    ), this.dirty = !1, this.watching = /* @__PURE__ */ new Set(), this._signalModifiedKey = this._signalModifiedKey.bind(this), this.context.modifiedKeyEvents.on("modified", this._signalModifiedKey), this._initCommands(), this.keyspaceEvents = parseKeyspaceEvents(
  ...
  disconnect() {
    ...
    removeFrom(this.channels), removeFrom(this.patternChannels), this.context.modifiedKeyEvents.off("modified", this._signalModifiedKey), this.connected = !1, emitConnectEvent2(this);
  }
  _initCommands() {
  ...
  _signalModifiedKey(key) {
    !this.dirty && this.watching.has(key) && (this.dirty = !0);
  }
};
...
RedisMock.Pipeline = pipeline_default;
