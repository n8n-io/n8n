...
var astradb_exports = {};
require_rolldown_runtime.__export(astradb_exports, { AstraDBChatMessageHistory: () => AstraDBChatMessageHistory });
/**
...
*/
var AstraDBChatMessageHistory = class AstraDBChatMessageHistory extends __langchain_core_chat_history.BaseListChatMessageHistory {
	lc_namespace = [
		"langchain",
		"stores",
		"message",
	...
	*/
	static async initialize({ token, endpoint, collectionName, namespace, sessionId }) {
		const client = new __datastax_astra_db_ts.DataAPIClient(token, { caller: ["langchainjs"] });
