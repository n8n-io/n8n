...
*/
var require_no_use_computed_property_like_method = /* @__PURE__ */ require_rolldown_runtime.__commonJSMin(((exports, module) => {
	...
	*/
	function resolvedExpressions(context, node) {
		...
		* @returns {Set<Expression>}
		*/
		function resolvedExpressionsInternal(node$1) {
			let resolvedSet = resolvedMap.get(node$1);
			if (!resolvedSet) {
				resolvedSet = /* @__PURE__ */ new Set();
				resolvedMap.set(node$1, resolvedSet);
				for (const e of extractResolvedExpressions(node$1)) resolvedSet.add(e);
			}
			if (resolvedSet.size === 0) resolvedSet.add(node$1);
			return resolvedSet;
		}
		/**
		* @param {Expression} node
		...
		*/
		function* extractResolvedExpressions(node$1) {
			switch (node$1.type) {
				case "Identifier": {
					const variable = utils.findVariableByIdentifier(context, node$1);
				...
				case "ConditionalExpression":
					yield* resolvedExpressionsInternal(node$1.consequent);
					yield* resolvedExpressionsInternal(node$1.alternate);
					break;
				case "LogicalExpression":
					yield* resolvedExpressionsInternal(node$1.left);
					yield* resolvedExpressionsInternal(node$1.right);
					break;
			}
		}
	}
