...
*/
var CallExpressionHandler = class extends NodeHandler {
	/**
	...
	*/
	async accepts(node) {
		if (ASTParser.isCallExpression(node)) return node;
		else return false;
	}
	/**
	...
	*/
	async handle(node) {
		...
		const { callee } = node;
		let funcCall;
		if (ASTParser.isIdentifier(callee)) funcCall = callee.name.replace(/^["'](.+(?=["']$))["']$/, "$1");
		else if (ASTParser.isMemberExpression(callee)) funcCall = await this.parentHandler.handle(callee);
...
//# sourceMappingURL=call_expression_handler.js.map