...
var _chunkORVFCRDGcjs = require('./chunk-ORVFCRDG.cjs');
...
async function resolveOptions(options) {
  const {
    ...
    transform,
    autoInstall = false,
    collectionsNodeResolvePath = _process2.default.cwd()
  } = options;
...
var QwikCompiler = async (svg, collection, icon, options) => {
  ...
  const res = toJsxComponent(svg, _chunkORVFCRDGcjs.__spreadProps.call(void 0, _chunkORVFCRDGcjs.__spreadValues.call(void 0, {}, mergedOptions), {
    defaultExport: true
  }));
  return res;
};
...
var SvelteCompiler = async (svg) => {
  if (svelteRunes == null) {
    try {
      const { VERSION } = await Promise.resolve().then(() => _interopRequireWildcard(require("svelte/compiler")));
...
var Vue2Compiler = async (svg, collection, icon) => {
  ...
  const transpileMod = await _localpkg.importModule.call(void 0, "vue-template-es2015-compiler");
...
export default markRaw({ name: '${collection}-${icon}', render${injectScripts ? `, data() {${injectScripts};return { idMap }}` : ""} })`;
  code += "\n/* vite-plugin-components disabled */";
  return code;
};
