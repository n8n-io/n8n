...
const __aws_sdk_client_kendra = require_rolldown_runtime.__toESM(require("@aws-sdk/client-kendra"));
...
* ```
*/
var AmazonKendraRetriever = class extends __langchain_core_retrievers.BaseRetriever {
	static lc_name() {
	...
	constructor({ indexId, topK = 10, clientOptions, attributeFilter, region }) {
		...
		this.topK = topK;
		this.kendraClient = new __aws_sdk_client_kendra.KendraClient({
			region,
			...clientOptions
		});
	...
	*/
	async queryKendra(query, topK, attributeFilter) {
		const retrieveCommand = new __aws_sdk_client_kendra.RetrieveCommand({
		...
		if (retriveLength === 0) {
			const queryCommand = new __aws_sdk_client_kendra.QueryCommand({
				IndexId: this.indexId,
				QueryText: query,
				PageSize: topK,
				AttributeFilter: attributeFilter
...
//# sourceMappingURL=kendra.cjs.map