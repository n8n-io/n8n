...
var MenuItemImpl_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ (0, vue.defineComponent)({
	...
	setup(__props) {
		...
		const isHighlighted = (0, vue.computed)(() => isFocused.value || contentContext.highlightedElement.value === currentElement.value);
		async function handlePointerMove(event) {
			if (event.defaultPrevented || !require_Menu_utils.isMouseEvent(event)) return;
			if (props.disabled) contentContext.onItemLeave(event);
				...
				if (!defaultPrevented) {
					...
					const isInputFocused = ["INPUT", "TEXTAREA"].includes(require_shared_getActiveElement.getActiveElement()?.tagName || "");
			...
			return (0, vue.openBlock)(), (0, vue.createBlock)((0, vue.unref)(CollectionItem), { value: { textValue: _ctx.textValue } }, {
				default: (0, vue.withCtx)(() => [(0, vue.createVNode)((0, vue.unref)(require_Primitive_Primitive.Primitive), (0, vue.mergeProps)({
					...
					onFocus: _cache[0] || (_cache[0] = async (event) => {
						...
						(0, vue.unref)(contentContext).highlightedElement.value = event.currentTarget;
					}),
