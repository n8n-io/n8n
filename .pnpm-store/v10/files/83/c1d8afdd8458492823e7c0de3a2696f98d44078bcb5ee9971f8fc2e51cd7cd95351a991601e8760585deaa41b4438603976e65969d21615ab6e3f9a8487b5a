...
const require_noop = require('../output_parsers/noop.cjs');
...
function _getLanguageModel(llmLike) {
	...
	else if ("bound" in llmLike && __langchain_core_runnables.Runnable.isRunnable(llmLike.bound)) return _getLanguageModel(llmLike.bound);
	else if ("runnable" in llmLike && "fallbacks" in llmLike && __langchain_core_runnables.Runnable.isRunnable(llmLike.runnable)) return _getLanguageModel(llmLike.runnable);
	else if ("default" in llmLike && __langchain_core_runnables.Runnable.isRunnable(llmLike.default)) return _getLanguageModel(llmLike.default);
...
* ```
*/
var LLMChain = class LLMChain extends require_base.BaseChain {
	static lc_name() {
		return "LLMChain";
	}
	...
	constructor(fields) {
		...
		this.outputParser = fields.outputParser ?? new require_noop.NoOpOutputParser();
	...
	*/
	...
	static async deserialize(data) {
		...
		return new LLMChain({
			llm: await __langchain_core_language_models_base.BaseLanguageModel.deserialize(llm),
			prompt: await __langchain_core_prompts.BasePromptTemplate.deserialize(prompt)
		});
	}
	/** @deprecated */
	serialize() {
...
//# sourceMappingURL=llm_chain.cjs.map