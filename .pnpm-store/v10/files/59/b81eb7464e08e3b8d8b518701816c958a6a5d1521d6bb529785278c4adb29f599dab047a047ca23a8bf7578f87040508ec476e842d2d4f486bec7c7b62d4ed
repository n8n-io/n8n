...
var convex_exports = {};
require_rolldown_runtime.__export(convex_exports, { ConvexVectorStore: () => ConvexVectorStore });
/**
...
*/
var ConvexVectorStore = class ConvexVectorStore extends __langchain_core_vectorstores.VectorStore {
	ctx;
	table;
	index;
	textField;
	...
	constructor(embeddings, config) {
		...
		this.get = config.get ?? (0, convex_server.makeFunctionReference)("langchain/db:get");
	}
	/**
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		return documents.map(({ [this.textField]: text, [this.embeddingField]: embedding, [this.metadataField]: metadata }, idx) => [new __langchain_core_documents.Document({
			pageContent: text,
			metadata: {
				...metadata,
	...
	*/
	static async fromTexts(texts, metadatas, embeddings, dbConfig) {
		const docs = texts.map((text, i) => new __langchain_core_documents.Document({
			pageContent: text,
