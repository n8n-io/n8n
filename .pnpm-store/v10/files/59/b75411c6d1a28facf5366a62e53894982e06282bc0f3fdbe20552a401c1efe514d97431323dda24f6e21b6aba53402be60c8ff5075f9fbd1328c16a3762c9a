...
*/
var BaseChatIflytekXinghuo = class extends __langchain_core_language_models_chat_models.BaseChatModel {
	static lc_name() {
	...
	constructor(fields) {
		super(fields ?? {});
		const iflytekAppid = fields?.iflytekAppid ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("IFLYTEK_APPID");
		if (!iflytekAppid) throw new Error("Iflytek APPID not found");
		else this.iflytekAppid = iflytekAppid;
		const iflytekApiKey = fields?.iflytekApiKey ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("IFLYTEK_API_KEY");
		if (!iflytekApiKey) throw new Error("Iflytek API key not found");
		else this.iflytekApiKey = iflytekApiKey;
		const iflytekApiSecret = fields?.iflytekApiSecret ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("IFLYTEK_API_SECRET");
	...
	*/
	...
	async completion(request, stream, signal) {
		...
		const streams = __langchain_core_utils_stream.IterableReadableStream.fromReadableStream(readable);
