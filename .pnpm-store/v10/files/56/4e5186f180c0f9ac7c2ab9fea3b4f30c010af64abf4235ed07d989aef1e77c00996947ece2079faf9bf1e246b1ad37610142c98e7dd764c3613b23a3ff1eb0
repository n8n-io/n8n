...
*/
var BaseChain = class extends __langchain_core_language_models_base.BaseLangChain {
	get lc_namespace() {
		return [
			"langchain",
			"chains",
	...
	*/
	async invoke(input, options) {
		...
		const callbackManager_ = await __langchain_core_callbacks_manager.CallbackManager.configure(config?.callbacks, this.callbacks, config?.tags, this.tags, config?.metadata, this.metadata, { verbose: this.verbose });
		...
		Object.defineProperty(outputValues, __langchain_core_outputs.RUN_KEY, {
	...
	*/
	async call(values, config, tags) {
		const parsedConfig = {
			tags,
			...(0, __langchain_core_callbacks_manager.parseCallbackConfigArg)(config)
		};
