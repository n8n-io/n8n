...
require_rolldown_runtime.__export(llama_cpp_exports, { ChatLlamaCpp: () => ChatLlamaCpp });
/**
...
* ```
*/
var ChatLlamaCpp = class ChatLlamaCpp extends __langchain_core_language_models_chat_models.SimpleChatModel {
	static inputs;
	maxTokens;
	temperature;
	topK;
	...
	*/
	...
	async *_streamResponseChunks(input, _options, runManager) {
		...
		for await (const chunk of stream) {
			yield new __langchain_core_outputs.ChatGenerationChunk({
				...
				message: new __langchain_core_messages.AIMessageChunk({ content: this._model.detokenize([chunk]) }),
				generationInfo: {}
			});
	...
	_buildPrompt(input) {
		const prompt = input.map((message) => {
			...
			else if (__langchain_core_messages.ChatMessage.isInstance(message)) messageText = `\n\n${message.role[0].toUpperCase()}${message.role.slice(1)}: ${message.content}`;
			else {
