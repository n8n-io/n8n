...
require_rolldown_runtime.__export(momento_vector_index_exports, { MomentoVectorIndex: () => MomentoVectorIndex });
/**
...
*/
var MomentoVectorIndex = class MomentoVectorIndex extends __langchain_core_vectorstores.VectorStore {
	client;
	indexName;
	textField;
	_ensureIndexExists;
	...
	*/
	async ensureIndexExists(numDimensions) {
		...
		if (response instanceof __gomomento_sdk_core.CreateVectorIndex.Success) return true;
		else if (response instanceof __gomomento_sdk_core.CreateVectorIndex.AlreadyExists) return false;
		else if (response instanceof __gomomento_sdk_core.CreateVectorIndex.Error) throw new Error(response.toString());
	...
	*/
	async addVectors(vectors, documents, documentProps) {
		...
		for (let i = 0; i < numBatches; i += 1) {
			...
			if (response instanceof __gomomento_sdk_core.VectorUpsertItemBatch.Success) continue;
			else if (response instanceof __gomomento_sdk_core.VectorUpsertItemBatch.Error) throw new Error(response.toString());
			else throw new Error(`Unknown response type: ${response.toString()}`);
		}
	}
	/**
	...
	*/
	async delete(params) {
		...
		if (response instanceof __gomomento_sdk_core.VectorDeleteItemBatch.Success) {} else if (response instanceof __gomomento_sdk_core.VectorDeleteItemBatch.Error) throw new Error(response.toString());
		else throw new Error(`Unknown response type: ${response.toString()}`);
	}
	/**
	...
	*/
	async similaritySearchVectorWithScore(query, k) {
		const response = await this.client.search(this.indexName, query, {
			topK: k,
			metadataFields: __gomomento_sdk_core.ALL_VECTOR_METADATA
		});
		if (response instanceof __gomomento_sdk_core.VectorSearch.Success) {
			if (response.hits === void 0) return [];
			return response.hits().map((hit) => [new __langchain_core_documents.Document({
		...
		} else if (response instanceof __gomomento_sdk_core.VectorSearch.Error) throw new Error(response.toString());
		else throw new Error(`Unknown response type: ${response.toString()}`);
	}
	/**
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		if (response instanceof __gomomento_sdk_core.VectorSearchAndFetchVectors.Success) {
			...
			return finalResult;
		} else if (response instanceof __gomomento_sdk_core.VectorSearchAndFetchVectors.Error) throw new Error(response.toString());
		else throw new Error(`Unknown response type: ${response.toString()}`);
	}
	/**
