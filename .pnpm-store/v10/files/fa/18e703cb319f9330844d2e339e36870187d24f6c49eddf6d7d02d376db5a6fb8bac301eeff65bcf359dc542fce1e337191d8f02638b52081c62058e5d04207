...
require_rolldown_runtime.__export(output_parser_exports, { ReActSingleInputOutputParser: () => ReActSingleInputOutputParser });
...
var ReActSingleInputOutputParser = class extends require_types.AgentActionOutputParser {
	...
	*/
	async parse(text) {
		...
		if (actionMatch) {
			if (includesAnswer) throw new __langchain_core_output_parsers.OutputParserException(`${FINAL_ANSWER_AND_PARSABLE_ACTION_ERROR_MESSAGE}: ${text}`);
		...
		throw new __langchain_core_output_parsers.OutputParserException(`Could not parse LLM output: ${text}`);
	}
	/**
