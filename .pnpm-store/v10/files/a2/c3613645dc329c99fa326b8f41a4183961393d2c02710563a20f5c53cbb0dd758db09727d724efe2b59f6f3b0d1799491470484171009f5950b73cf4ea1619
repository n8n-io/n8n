...
export class Import {
    ...
    static resolveModule(options) {
        ...
        const normalizedRootPath = (getRealPath || FileSystem.getRealPath)(baseFolderPath);
    ...
    static async resolveModuleAsync(options) {
        ...
        const normalizedRootPath = await (getRealPathAsync || getRealPath || FileSystem.getRealPathAsync)(baseFolderPath);
        if (modulePath.startsWith('.')) {
            // This looks like a conventional relative path
            return path.resolve(normalizedRootPath, modulePath);
        }
    ...
    static _getPackageName(rootPath) {
        const packageJsonPath = PackageJsonLookup.instance.tryGetPackageJsonFilePathFor(rootPath);
        if (packageJsonPath) {
