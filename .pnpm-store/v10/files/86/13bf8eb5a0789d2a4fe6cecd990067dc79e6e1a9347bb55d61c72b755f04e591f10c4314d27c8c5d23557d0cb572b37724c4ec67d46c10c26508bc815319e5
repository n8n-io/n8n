...
var BlobStoreGoogle = class extends require_experimental_utils_media_core.BlobStore {
	...
	async _get(key) {
		...
		if (data && metadata) return await require_experimental_utils_media_core.MediaBlob.fromBlob(data, {
			metadata,
			path: key
		});
		else return;
...
var BlobStoreAIStudioFileBase = class extends BlobStoreGoogle {
	...
	async _get(key) {
		const metadata = await this._getMetadata(key);
		if (metadata) return new require_experimental_utils_media_core.MediaBlob({
			path: key,
			data: {
				value: "",
