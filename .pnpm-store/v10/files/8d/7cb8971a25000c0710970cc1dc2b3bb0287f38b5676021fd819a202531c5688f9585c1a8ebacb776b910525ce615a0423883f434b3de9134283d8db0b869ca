...
class DockerComposeEnvironment {
    ...
    async up(services) {
        ...
        const startedContainerNameSet = new Set(startedContainers.map((startedContainer) => (0, container_runtime_1.parseComposeContainerName)(this.projectName, startedContainer.Names[0])));
    ...
    warnForUnusedWaitStrategies(startedContainerNames) {
        const unusedWaitStrategyContainerNames = Object.keys(this.waitStrategy).filter((configuredContainerName) => !startedContainerNames.has(configuredContainerName));
        if (unusedWaitStrategyContainerNames.length > 0) {
            common_1.log.warn(`No containers were started for the configured wait strategy names: "${unusedWaitStrategyContainerNames.join('", "')}". Wait strategies are matched against container names (for example "redis-1"), not service names.`);
        }
    }
}
