...
require_rolldown_runtime.__export(google_scholar_exports, { SERPGoogleScholarAPITool: () => SERPGoogleScholarAPITool });
/**
* Tool for querying Google Scholar using the SerpApi service.
*/
var SERPGoogleScholarAPITool = class extends __langchain_core_tools.Tool {
	/**
  ...
  articles and papers by keywords or authors. Input should be a search query string.`;
	...
	*/
	constructor(fields) {
		super(...arguments);
		const apiKey = fields?.apiKey ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("SERPAPI_API_KEY");
