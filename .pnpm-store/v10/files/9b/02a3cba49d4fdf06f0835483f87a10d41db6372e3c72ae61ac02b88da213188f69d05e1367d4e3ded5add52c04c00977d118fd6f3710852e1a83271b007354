...
const cheerio = require_rolldown_runtime.__toESM(require("cheerio"));

//#region src/tools/webbrowser.ts
var webbrowser_exports = {};
require_rolldown_runtime.__export(webbrowser_exports, {
...
* ```
*/
var WebBrowser = class extends __langchain_core_tools.Tool {
	static lc_name() {
		return "WebBrowser";
	}
	get lc_namespace() {
	...
	constructor({ model, headers, embeddings, requestConfig, textSplitter }) {
		...
		this.textSplitter = textSplitter ?? new __langchain_textsplitters.RecursiveCharacterTextSplitter({
			chunkSize: 2e3,
			chunkOverlap: 200
		});
	}
	...
	async _call(inputs, runManager) {
		...
		if (doSummary) context = texts.slice(0, 4).join("\n");
			...
			const docs = texts.map((pageContent) => new __langchain_core_documents.Document({
				pageContent,
				metadata: []
			}));
			const vectorStore = await require_vectorstores_memory.MemoryVectorStore.fromDocuments(docs, this.embeddings);
			const results = await vectorStore.similaritySearch(task, 4, void 0, runManager?.getChild("vectorstore"));
			context = require_util_document.formatDocumentsAsString(results);
		}
		...
		const chain = __langchain_core_runnables.RunnableSequence.from([this.model, new __langchain_core_output_parsers.StringOutputParser()]);
