...
__export(friendli_exports, { Friendli: () => Friendli });
/**
...
*/
var Friendli = class extends LLM {
	...
	*/
	...
	async *_streamResponseChunks(prompt, _options, runManager) {
		...
		for await (const chunk of stream) if (chunk.event !== "complete") {
			...
			const generationChunk = new GenerationChunk({ text: parsedChunk.text ?? "" });
...
//#endregion
export { Friendli, friendli_exports };
