...
*/
var BaseAgent = class extends __langchain_core_load_serializable.Serializable {
	get returnValues() {
		return ["output"];
	}
	get allowedTools() {
...
var AgentRunnableSequence = class extends __langchain_core_runnables.RunnableSequence {
	streamRunnable;
	singleAction;
	static fromRunnables([first, ...runnables], config) {
		const sequence = __langchain_core_runnables.RunnableSequence.from([first, ...runnables], config.name);
...
var RunnableSingleActionAgent = class extends BaseSingleActionAgent {
	...
	*/
	...
	async plan(steps, inputs, callbackManager, config) {
		...
		const combinedConfig = (0, __langchain_core_runnables.patchConfig)(config, {
