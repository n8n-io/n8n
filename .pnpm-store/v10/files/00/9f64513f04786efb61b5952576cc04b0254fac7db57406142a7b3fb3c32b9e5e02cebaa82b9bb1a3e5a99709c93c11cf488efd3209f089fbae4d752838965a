...
const youtubei_js = require_rolldown_runtime.__toESM(require("youtubei.js"));

//#region src/document_loaders/web/youtube.ts
var youtube_exports = {};
require_rolldown_runtime.__export(youtube_exports, { YoutubeLoader: () => YoutubeLoader });
/**
...
* ```
*/
var YoutubeLoader = class YoutubeLoader extends __langchain_core_document_loaders_base.BaseDocumentLoader {
	videoId;
	language;
	addVideoInfo;
	constructor(config) {
	...
	*/
	async load() {
		...
		try {
			const youtube = await youtubei_js.Innertube.create({
				lang: this.language,
