...
* <br />
*/
var PineconeStore = class PineconeStore extends __langchain_core_vectorstores.VectorStore {
	textKey;
	namespace;
	pineconeIndex;
	filter;
	...
	constructor(embeddings, params) {
		...
		else if (pineconeConfig) this.pineconeIndex = new __pinecone_database_pinecone.Index(pineconeConfig.indexName, {
	...
	*/
	async addVectors(vectors, documents, options) {
		...
		const documentIds = ids == null ? documents.map(() => (0, uuid.v4)()) : ids;
		...
		const chunkSize = 100;
		const chunkedVectors = (0, __langchain_core_utils_chunk_array.chunkArray)(pineconeVectors, chunkSize);
	...
	*/
	_formatMatches(matches = []) {
		...
		for (const record of matches) {
			...
			if (score) documentsWithScores.push([new __langchain_core_documents.Document({
				id,
