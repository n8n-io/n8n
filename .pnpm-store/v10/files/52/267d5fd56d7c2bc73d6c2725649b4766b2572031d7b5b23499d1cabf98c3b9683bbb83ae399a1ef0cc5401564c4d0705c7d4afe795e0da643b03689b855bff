...
const __opensearch_project_opensearch = require_rolldown_runtime.__toESM(require("@opensearch-project/opensearch"));

//#region src/vectorstores/opensearch.ts
var opensearch_exports = {};
require_rolldown_runtime.__export(opensearch_exports, { OpenSearchVectorStore: () => OpenSearchVectorStore });
/**
...
* OpenSearch index.
*/
var OpenSearchVectorStore = class OpenSearchVectorStore extends __langchain_core_vectorstores.VectorStore {
	client;
	indexName;
	isAoss;
	engine;
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		return body.hits.hits.map((hit) => [new __langchain_core_documents.Document({
	...
	*/
	async doesIndexExist() {
		try {
			...
			if (err instanceof __opensearch_project_opensearch.errors.ResponseError && err.statusCode === 404) return false;
			throw err;
		}
	}
	/**
