...
var MemberExpressionHandler = class extends NodeHandler {
	...
	*/
	async handle(node) {
		...
		let prop;
		if (ASTParser.isIdentifier(property)) prop = property.name.replace(/^["'](.+(?=["']$))["']$/, "$1");
		else if (ASTParser.isStringLiteral(property)) prop = `${property.value}`.replace(/^["'](.+(?=["']$))["']$/, "$1");
		else throw new Error("Invalid property key type");
		let identifier;
		if (ASTParser.isIdentifier(object)) identifier = object.name.replace(/^["'](.+(?=["']$))["']$/, "$1");
		else if (ASTParser.isStringLiteral(object)) identifier = `${object.value}`.replace(/^["'](.+(?=["']$))["']$/, "$1");
...
//#endregion
export { MemberExpressionHandler };
//# sourceMappingURL=member_expression_handler.js.map