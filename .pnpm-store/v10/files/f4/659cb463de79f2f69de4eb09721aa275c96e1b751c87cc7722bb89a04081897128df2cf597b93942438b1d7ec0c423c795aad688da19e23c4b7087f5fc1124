...
function getLegendForConfigColumnOfSeverity({ context, severityType, }) {
...
function getLegendsForIndividualConfigs({ context, }) {
    const { options, plugin } = context;
    const { urlConfigs } = options;
    /* istanbul ignore next -- this shouldn't happen */
    if (!plugin.configs || !plugin.rules) {
    ...
    return configNamesThatSetRuleToThisSeverity.flatMap((configName) => {
        ...
        return [
            `${emoji} Set in the \`${configNameToDisplay(context, configName)}\` ${configLinkOrWord}.`,
        ];
    });
}
export function generateLegend(context, columns) {
