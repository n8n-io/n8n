...
var web_exports = {};
require_rolldown_runtime.__export(web_exports, {
...
function convertOneMessageToText(message, humanPrompt, aiPrompt) {
	...
	else if (__langchain_core_messages.ChatMessage.isInstance(message)) return `\n\n${message.role[0].toUpperCase() + message.role.slice(1)}: {message.content}`;
	throw new Error(`Unknown role: ${message._getType()}`);
}
function convertMessagesToPromptAnthropic(messages, humanPrompt = "\n\nHuman:", aiPrompt = "\n\nAssistant:") {
	const messagesCopy = [...messages];
	if (messagesCopy.length === 0 || messagesCopy[messagesCopy.length - 1]._getType() !== "ai") messagesCopy.push(new __langchain_core_messages.AIMessage({ content: "" }));
...
function formatTools(tools) {
	...
	if (tools.every(__langchain_core_utils_function_calling.isLangChainTool)) return tools.map((tc) => ({
		name: tc.name,
		description: tc.description,
		input_schema: (0, __langchain_core_utils_types.isInteropZodSchema)(tc.schema) ? (0, __langchain_core_utils_json_schema.toJsonSchema)(tc.schema) : tc.schema
	}));
	if (tools.every(__langchain_core_language_models_base.isOpenAITool)) return tools.map((tc) => ({
	...
	if (tools.some(__langchain_core_utils_function_calling.isStructuredTool) || tools.some(__langchain_core_language_models_base.isOpenAITool) || tools.some(require_anthropic.isAnthropicTool)) throw new Error("All tools passed to BedrockChat must be of the same type.");
...
* </details>
*/
var BedrockChat = class extends __langchain_core_language_models_chat_models.BaseChatModel {
	...
	constructor(fields) {
		const awsAccessKeyId = fields?.awsAccessKeyId ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("AWS_ACCESS_KEY_ID");
		const awsSecretAccessKey = fields?.awsSecretAccessKey ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("AWS_SECRET_ACCESS_KEY");
		const awsSessionToken = fields?.awsSessionToken ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("AWS_SESSION_TOKEN");
		...
		if (provider === "anthropic" || provider === "cohere" || provider === "meta" || provider === "mistral") {
			...
			for await (const chunk of this._readChunks(reader)) {
				...
				if (body.bytes !== void 0) {
					...
					if (this.usesMessagesApi) {
						const chunk$1 = require_index.BedrockLLMInputOutputAdapter.prepareMessagesOutput(provider, chunkResult, { coerceContentToString: !toolsInParams });
						...
						if (finalMetrics != null && typeof finalMetrics === "object" && (0, __langchain_core_messages.isAIMessage)(chunk$1.message)) chunk$1.message.usage_metadata = {
						...
						const chunk$1 = new __langchain_core_outputs.ChatGenerationChunk({
							text,
