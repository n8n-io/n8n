...
function convertMessageToCohereMessage(message, toolResults) {
	...
	const getToolCall = (message$1) => {
		if ((0, __langchain_core_messages.isAIMessage)(message$1) && message$1.tool_calls) return message$1.tool_calls.map((toolCall) => ({
			name: toolCall.name,
...
function _formatToolsToCohere(tools) {
	...
	else if (tools.every(__langchain_core_language_models_base.isOpenAITool)) return tools.map((tool) => {
		return {
	...
	else if (tools.every(__langchain_core_utils_function_calling.isLangChainTool)) return tools.map((tool) => {
		const parameterDefinitionsFromZod = (0, __langchain_core_utils_types.isInteropZodSchema)(tool.schema) ? (0, __langchain_core_utils_json_schema.toJsonSchema)(tool.schema) : tool.schema;
		return {
			name: tool.name,
...
* <br />
*/
var ChatCohere = class extends __langchain_core_language_models_chat_models.BaseChatModel {
	static lc_name() {
		return "ChatCohere";
	}
	...
	_getChatRequest(messages, options) {
		...
		if (!params.forceSingleStep) {
			...
			for (let i = 0; i < messages.length - 1; i += 1) {
				const message = messages[i];
				if ((0, __langchain_core_messages.isAIMessage)(message) && message.tool_calls) continue;
				if (message._getType().toLowerCase() === "tool") {
					tempToolResults = tempToolResults.concat(this._messageToCohereToolResults(messages, i));
					if (i === messages.length - 1 || !(messages[i + 1]._getType().toLowerCase() === "tool")) {
	...
	_messagesToCohereToolResultsCurrChatTurn(messages) {
		...
		for (const message of currChatTurnMessages) if (isToolMessage(message)) {
			...
			const previousAiMsgs = currChatTurnMessages.filter((msg) => (0, __langchain_core_messages.isAIMessage)(msg) && msg.tool_calls !== void 0);
	...
	_messageToCohereToolResults(messages, toolMessageIndex) {
		...
		const previousAiMessage = messagesUntilTool.filter((message) => (0, __langchain_core_messages.isAIMessage)(message) && message.tool_calls).slice(-1)[0];
	...
	async *_streamResponseChunks(messages, options, runManager) {
		...
		for await (const chunk of stream) if (chunk.eventType === "text-generation") {
			yield new __langchain_core_outputs.ChatGenerationChunk({
				text: chunk.text,
				message: new __langchain_core_messages.AIMessageChunk({ content: chunk.text })
			});
			await runManager?.handleLLMNewToken(chunk.text);
		} else if (chunk.eventType !== "stream-end") yield new __langchain_core_outputs.ChatGenerationChunk({
			text: "",
