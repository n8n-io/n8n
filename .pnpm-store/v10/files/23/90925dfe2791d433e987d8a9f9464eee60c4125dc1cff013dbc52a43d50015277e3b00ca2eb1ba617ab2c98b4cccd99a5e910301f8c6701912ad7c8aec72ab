...
var ViolationOfExpectationsChain = class extends require_base.BaseChain {
	...
	constructor(fields) {
		...
		this.stringOutputParser = new __langchain_core_output_parsers.StringOutputParser();
	}
		...
		chatHistory.forEach((item, index) => {
			...
			if (item._getType() === "ai") {
				...
				newArray.push({
					...
					userResponse: userResponse ? new __langchain_core_messages.HumanMessage(userResponse) : void 0
				});
			}
		});
		return newArray;
	...
	*/
	async _call(values, runManager) {
		...
		const isEveryMessageBaseMessage = chatHistory.every((message) => (0, __langchain_core_messages.isBaseMessage)(message));
	...
	*/
	async predictNextUserMessage(chatHistory, runManager) {
		...
		const chain = require_violation_of_expectations_prompt.PREDICT_NEXT_USER_MESSAGE_PROMPT.pipe(llmWithFunctions).pipe(this.jsonOutputParser);
	...
	*/
	async generateRevisedPrediction({ originalPrediction, explainedPredictionErrors, userInsights, runManager }) {
		const revisedPredictionChain = require_violation_of_expectations_prompt.GENERATE_REVISED_PREDICTION_PROMPT.pipe(this.llm).pipe(this.stringOutputParser);
