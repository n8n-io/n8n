...
async function sendBlockEvent(crashedThreadId) {
  ...
  if (asyncState) {
    ...
    const scope = Object.assign(new core.Scope(), asyncState.scope).getScopeData();
    const isolationScope = Object.assign(new core.Scope(), asyncState.isolationScope).getScopeData();
