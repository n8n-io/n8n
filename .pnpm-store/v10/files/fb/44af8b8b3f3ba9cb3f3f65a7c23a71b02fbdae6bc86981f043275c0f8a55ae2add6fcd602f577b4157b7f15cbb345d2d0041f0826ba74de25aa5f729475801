...
exports.SearchIndexingBufferedSender = exports.DEFAULT_RETRY_COUNT = exports.DEFAULT_FLUSH_WINDOW = exports.DEFAULT_BATCH_SIZE = void 0;
const tslib_1 = require("tslib");
const core_util_1 = require("@azure/core-util");
const events_1 = tslib_1.__importDefault(require("events"));
const indexDocumentsBatch_js_1 = require("./indexDocumentsBatch.js");
const serviceUtils_js_1 = require("./serviceUtils.js");
const tracing_js_1 = require("./tracing.js");
/**
 * Default Batch Size
 */
exports.DEFAULT_BATCH_SIZE = 512;
/**
 * Default window flush interval
 */
exports.DEFAULT_FLUSH_WINDOW = 60000;
/**
...
class SearchIndexingBufferedSender {
    ...
    constructor(client, documentKeyRetriever, options = {}) {
        ...
        this.initialBatchActionCount = options.initialBatchActionCount ?? exports.DEFAULT_BATCH_SIZE;
        this.flushWindowInMs = options.flushWindowInMs ?? exports.DEFAULT_FLUSH_WINDOW;
        // Retry specific configuration properties
        this.throttlingDelayInMs = options.throttlingDelayInMs ?? exports.DEFAULT_FLUSH_WINDOW;
        this.maxRetriesPerAction = options.maxRetriesPerAction ?? exports.DEFAULT_RETRY_COUNT;
        this.maxThrottlingDelayInMs = options.maxThrottlingDelayInMs ?? DEFAULT_MAX_RETRY_DELAY;
        this.batchObject = new indexDocumentsBatch_js_1.IndexDocumentsBatch();
        if (this.autoFlush) {
    ...
    async uploadDocuments(documents, options = {}) {
        const { span, updatedOptions } = (0, tracing_js_1.createSpan)("SearchIndexingBufferedSender-uploadDocuments", options);
        try {
    ...
    async mergeDocuments(documents, options = {}) {
        const { span, updatedOptions } = (0, tracing_js_1.createSpan)("SearchIndexingBufferedSender-mergeDocuments", options);
        try {
    ...
    async mergeOrUploadDocuments(documents, options = {}) {
        const { span, updatedOptions } = (0, tracing_js_1.createSpan)("SearchIndexingBufferedSender-mergeOrUploadDocuments", options);
        try {
    ...
    async deleteDocuments(documents, options = {}) {
        const { span, updatedOptions } = (0, tracing_js_1.createSpan)("SearchIndexingBufferedSender-deleteDocuments", options);
        try {
     ...
     */
    async flush(options = {}) {
        const { span, updatedOptions } = (0, tracing_js_1.createSpan)("SearchIndexingBufferedSender-flush", options);
        try {
    ...
    async submitDocuments(actionsToSend, options, retryAttempt = 1) {
        try {
            ...
            const result = await this.client.indexDocuments(new indexDocumentsBatch_js_1.IndexDocumentsBatch(actionsToSend), options);
            // raise success event
            ...
            if (e.statusCode && e.statusCode === 413 && actionsToSend.length > 1) {
                ...
                const delayWithJitter = clampedExponentialDelay / 2 + (0, serviceUtils_js_1.getRandomIntegerInclusive)(0, clampedExponentialDelay / 2);
                await (0, core_util_1.delay)(delayWithJitter);
