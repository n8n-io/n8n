    ...
    const traceableFunc = (...args) => {
        ...
        const [currentContext, rawInputs] = (() => {
            ...
            if (isRunTree(prevRunFromStore)) {
                if (_LC_CHILD_RUN_END_PROMISES_KEY in prevRunFromStore &&
                    Array.isArray(prevRunFromStore[_LC_CHILD_RUN_END_PROMISES_KEY])) {
                    prevRunFromStore[_LC_CHILD_RUN_END_PROMISES_KEY].push(runEndedPromise);
                }
                else {
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    prevRunFromStore[_LC_CHILD_RUN_END_PROMISES_KEY] = [
                        runEndedPromise,
                    ];
                }
                ...
                if (lc_contextVars) {
                    (currentRunTree ?? {})[_LC_CONTEXT_VARIABLES_KEY] = lc_contextVars;
                }
