...
const __qdrant_js_client_rest = require_rolldown_runtime.__toESM(require("@qdrant/js-client-rest"));
...
*/
var QdrantVectorStore = class QdrantVectorStore extends __langchain_core_vectorstores.VectorStore {
	get lc_secrets() {
		return {
	...
	constructor(embeddings, args) {
		...
		const url = args.url ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("QDRANT_URL");
		const apiKey = args.apiKey ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("QDRANT_API_KEY");
		if (!args.client && !url) throw new Error("Qdrant client or url address must be set.");
		this.client = args.client || new __qdrant_js_client_rest.QdrantClient({
			url,
			apiKey
		});
	...
	*/
	async addVectors(vectors, documents, documentOptions) {
		...
		const points = vectors.map((embedding, idx) => ({
			id: documents[idx].id ?? documentOptions?.ids?.[idx] ?? (0, uuid.v4)(),
			vector: embedding,
			payload: {
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		const result = results.map((res) => [new __langchain_core_documents.Document({
			id: res.id,
