...
function summarizationMiddleware(options) {
 ...
 /**
			...
			*/
			...
			const model = typeof userOptions.model === "string" ? await require_chat_models_universal.initChatModel(userOptions.model) : userOptions.model;
...
function isSafeCutoffPoint(messages, cutoffIndex) {
	...
	*/
	...
	for (let i = searchStart; i < searchEnd; i++) {
		if (!require_utils.hasToolCalls(messages[i])) continue;
...
//#endregion
exports.contextSizeSchema = contextSizeSchema;
exports.getProfileLimits = getProfileLimits;
exports.keepSchema = keepSchema;
