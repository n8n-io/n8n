...
const require_extname = require('../../utils/extname.cjs');
...
const ignore = require_rolldown_runtime.__toESM(require("ignore"));
const binary_extensions = require_rolldown_runtime.__toESM(require("binary-extensions"));

//#region src/document_loaders/web/github.ts
var github_exports = {};
require_rolldown_runtime.__export(github_exports, { GithubRepoLoader: () => GithubRepoLoader });
const extensions = /* @__PURE__ */ new Set(binary_extensions.default);
/**
...
function isBinaryPath(name) {
	return extensions.has(require_extname.extname(name).slice(1).toLowerCase());
}
/**
...
*/
var GithubRepoLoader = class GithubRepoLoader extends __langchain_core_document_loaders_base.BaseDocumentLoader {
	baseUrl;
	apiUrl;
	owner;
	repo;
	...
	ignorePaths;
	submoduleInfos;
	constructor(githubUrl, { accessToken = (0, __langchain_core_utils_env.getEnvironmentVariable)("GITHUB_ACCESS_TOKEN"), baseUrl = "https://github.com", apiUrl = "https://api.github.com", branch = "main", recursive = true, processSubmodules = false, unknown = __langchain_classic_document_loaders_fs_directory.UnknownHandling.Warn, ignoreFiles = [], ignorePaths, verbose = false, maxConcurrency = 2, maxRetries = 2,...rest } = {}) {
		super();
		...
		if (ignorePaths) this.ignore = (0, ignore.default)().add(ignorePaths);
	...
	*/
	async load() {
		...
		const documents = (await this.processRepo()).map((fileResponse) => new __langchain_core_documents.Document({
	...
	*/
	handleError(message) {
		switch (this.unknown) {
			case __langchain_classic_document_loaders_fs_directory.UnknownHandling.Ignore: break;
			case __langchain_classic_document_loaders_fs_directory.UnknownHandling.Warn:
				console.warn(message);
				break;
			case __langchain_classic_document_loaders_fs_directory.UnknownHandling.Error: throw new Error(message);
