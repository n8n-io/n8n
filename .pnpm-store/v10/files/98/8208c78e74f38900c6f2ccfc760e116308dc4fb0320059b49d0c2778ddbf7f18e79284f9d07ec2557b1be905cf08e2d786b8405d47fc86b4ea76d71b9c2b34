...
var llm_exports = {};
require_rolldown_runtime.__export(llm_exports, {
...
`;
const DEFAULT_PROMPT = /* @__PURE__ */ __langchain_core_prompts.ChatPromptTemplate.fromMessages([["system", SYSTEM_PROMPT], ["human", "Tip: Make sure to answer in the correct format and do not include any explanations. Use the given format to extract information from the following input: {input}"]]);
...
function createOptionalEnumType({ enumValues = void 0, description = "", isRel = false }) {
	let schema;
	if (enumValues && enumValues.length) schema = zod_v3.z.enum(enumValues).describe(`${description} Available options are: ${enumValues.join(", ")}.`);
	else {
		...
		schema = zod_v3.z.string().describe(description + additionalInfo);
	}
	return schema;
}
function createNodeSchema(allowedNodes, nodeProperties) {
	const nodeSchema = zod_v3.z.object({
		id: zod_v3.z.string(),
	...
	return nodeProperties.length > 0 ? nodeSchema.extend({ properties: zod_v3.z.array(zod_v3.z.object({
		key: createOptionalEnumType({
			enumValues: nodeProperties,
			description: "Property key."
		}),
		value: zod_v3.z.string().describe("Extracted value.")
...
function createRelationshipSchema(allowedNodes, allowedRelationships, relationshipProperties) {
	const relationshipSchema = zod_v3.z.object({
		sourceNodeId: zod_v3.z.string(),
		...
		targetNodeId: zod_v3.z.string(),
		targetNodeType: createOptionalEnumType({
			enumValues: allowedNodes,
			description: "The target node of the relationship."
		})
	});
	return relationshipProperties.length > 0 ? relationshipSchema.extend({ properties: zod_v3.z.array(zod_v3.z.object({
		key: createOptionalEnumType({
...
function createSchema(allowedNodes, allowedRelationships, nodeProperties, relationshipProperties) {
	...
	const dynamicGraphSchema = zod_v3.z.object({
		nodes: zod_v3.z.array(nodeSchema).describe("List of nodes"),
		relationships: zod_v3.z.array(relationshipSchema).describe("List of relationships.")
	});
...
function mapToBaseNode(node) {
	return new require_graphs_document.Node({
		id: node.id,
...
function mapToBaseRelationship({ fallbackRelationshipType }) {
	return function(relationship) {
		return new require_graphs_document.Relationship({
			source: new require_graphs_document.Node({
				id: relationship.sourceNodeId,
				type: relationship.sourceNodeType ? toTitleCase(relationship.sourceNodeType) : ""
			}),
			target: new require_graphs_document.Node({
...
var LLMGraphTransformer = class {
	...
	constructor({ llm, allowedNodes = [], allowedRelationships = [], prompt = DEFAULT_PROMPT, strictMode = true, nodeProperties = [], relationshipProperties = [], fallbackRelationshipType = null }) {
		...
		const structuredLLM = llm.withStructuredOutput((0, __langchain_core_utils_json_schema.toJsonSchema)(schema));
	...
	*/
	async processResponse(document) {
		...
		return new require_graphs_document.GraphDocument({
			nodes,
			relationships,
			source: document
		});
