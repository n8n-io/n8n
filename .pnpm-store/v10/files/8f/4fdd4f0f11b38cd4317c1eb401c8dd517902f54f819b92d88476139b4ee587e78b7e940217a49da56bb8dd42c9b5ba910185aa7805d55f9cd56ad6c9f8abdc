...
function* mapOutputUpdates(outputChannels, tasks, cached) {
	...
	let updated;
	if (outputTasks.some(([task]) => task.writes.some(([chan, _]) => chan === "__return__"))) updated = outputTasks.flatMap(([task]) => task.writes.filter(([chan, _]) => chan === require_constants.RETURN).map(([_, value]) => [task.name, value]));
