...
const flat = require_rolldown_runtime.__toESM(require("flat"));

//#region src/vectorstores/googlevertexai.ts
var googlevertexai_exports = {};
require_rolldown_runtime.__export(googlevertexai_exports, {
...
*/
var IdDocument = class extends __langchain_core_documents.Document {
	id;
	constructor(fields) {
		super(fields);
		this.id = fields.id;
...
* instance.
*/
var MatchingEngine = class MatchingEngine extends __langchain_core_vectorstores.VectorStore {
	/**
	...
	*/
	...
	constructor(embeddings, args) {
		...
		this.caller = new __langchain_core_utils_async_caller.AsyncCaller(this.callerParams || {});
	...
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		const ret = await Promise.all(neighbors.map(async (neighbor) => {
			...
			try {
				...
				doc = new __langchain_core_documents.Document({ pageContent: `Missing document ${id}` });
			}
			doc.id ??= id;
			return [doc, distance];
		}));
