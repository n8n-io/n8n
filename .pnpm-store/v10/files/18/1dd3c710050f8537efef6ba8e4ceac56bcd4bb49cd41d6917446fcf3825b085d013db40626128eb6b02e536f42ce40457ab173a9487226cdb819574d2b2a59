...
var node_param_type_options_password_missing_exports = {};
__export(node_param_type_options_password_missing_exports, {
  default: () => node_param_type_options_password_missing_default
});
module.exports = __toCommonJS(node_param_type_options_password_missing_exports);
var import_utils = require("../ast/utils");
var import_getters = require("../ast/getters");
var import_constants = require("../constants");
const isFalsePositive = (paramName) => import_constants.FALSE_POSITIVE_NODE_SENSITIVE_PARAM_NAMES.includes(paramName);
const isSensitive = (paramName) => {
  if (isFalsePositive(paramName))
    return false;
  return import_constants.NODE_SENSITIVE_PARAM_NAMES.some(
    (sensitiveName) => paramName.toLowerCase().includes(sensitiveName.toLowerCase())
  );
};
const sensitiveStrings = import_constants.NODE_SENSITIVE_PARAM_NAMES.map((i) => `\`${i}\``).join(
  ","
);
var node_param_type_options_password_missing_default = import_utils.utils.createRule({
