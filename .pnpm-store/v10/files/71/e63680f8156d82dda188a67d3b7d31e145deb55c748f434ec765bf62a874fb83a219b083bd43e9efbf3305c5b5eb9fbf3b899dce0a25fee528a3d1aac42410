...
require_rolldown_runtime.__export(unstructured_exports, {
	UNSTRUCTURED_API_FILETYPES: () => UNSTRUCTURED_API_FILETYPES,
	UnknownHandling: () => __langchain_classic_document_loaders_fs_directory.UnknownHandling,
...
* as input.
*/
var UnstructuredLoader = class extends __langchain_core_document_loaders_base.BaseDocumentLoader {
	filePath;
	buffer;
	fileName;
	...
	constructor(filepathOrBufferOptions, unstructuredOptions = {}) {
		...
		if (!isLegacySyntax) {
			...
			this.apiKey = options.apiKey ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("UNSTRUCTURED_API_KEY");
			this.apiUrl = options.apiUrl ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("UNSTRUCTURED_API_URL") ?? this.apiUrl;
	...
	async load() {
		...
		for (const element of elements) {
			...
			if (typeof text === "string" && text !== "") documents.push(new __langchain_core_documents.Document({
				pageContent: text,
				metadata: {
					...metadata,
...
* ```
*/
var UnstructuredDirectoryLoader = class extends __langchain_classic_document_loaders_fs_directory.DirectoryLoader {
	constructor(directoryPathOrLegacyApiUrl, optionsOrLegacyDirectoryPath, legacyOptionRecursive = true, legacyOptionUnknown = __langchain_classic_document_loaders_fs_directory.UnknownHandling.Warn) {
		let directoryPath;
		let options;
...
exports.UnknownHandling = __langchain_classic_document_loaders_fs_directory.UnknownHandling;
