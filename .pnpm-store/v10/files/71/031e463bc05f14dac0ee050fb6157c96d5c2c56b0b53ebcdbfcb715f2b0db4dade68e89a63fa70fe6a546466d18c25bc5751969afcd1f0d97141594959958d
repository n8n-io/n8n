...
var AgentExecutorIterator = class extends __langchain_core_load_serializable.Serializable {
	lc_namespace = [
		"langchain",
		"agents",
		"executor_iterator"
	...
	*/
	async onFirstStep() {
		if (this.iterations === 0) {
			const callbackManager = await __langchain_core_callbacks_manager.CallbackManager.configure(this.callbacks ?? this.config?.callbacks, this.agentExecutor.callbacks, this.tags ?? this.config?.tags, this.agentExecutor.tags, this.metadata ?? this.config?.metadata, this.agentExecutor.metadata, { verbose: this.agentExecutor.verbose });
...
*/
var ExceptionTool = class extends __langchain_core_tools.Tool {
	name = "_Exception";
...
var AgentExecutor = class AgentExecutor extends require_base.BaseChain {
	...
	*/
	...
	constructor(input) {
		...
		if (__langchain_core_runnables.Runnable.isRunnable(input.agent)) {
	...
	async _call(inputs, runManager, config) {
		...
		while (this.shouldContinue(iterations)) {
			...
			const newSteps = await Promise.all(actions.map(async (action) => {
				...
				let observation;
				try {
					observation = tool ? await tool.invoke(action.toolInput, (0, __langchain_core_runnables.patchConfig)(config, { callbacks: runManager?.getChild() })) : `${action.tool} is not a valid tool, try another one.`;
					if (typeof observation !== "string") throw new Error("Received unsupported non-string response from tool call.");
				} catch (e) {
					if (e instanceof __langchain_core_tools.ToolInputParsingException) {
