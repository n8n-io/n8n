...
require_rolldown_runtime.__export(tavily_search_api_exports, { TavilySearchAPIRetriever: () => TavilySearchAPIRetriever });
/**
* A class for retrieving documents related to a given search term
* using the Tavily Search API.
*/
var TavilySearchAPIRetriever = class extends __langchain_core_retrievers.BaseRetriever {
	static lc_name() {
	...
	constructor(fields) {
		...
		this.apiKey = fields?.apiKey ?? (0, __langchain_core_utils_env.getEnvironmentVariable)("TAVILY_API_KEY");
	...
	async _getRelevantDocuments(query, _runManager) {
		...
		if (this.includeGeneratedAnswer) docs.push(new __langchain_core_documents.Document({
