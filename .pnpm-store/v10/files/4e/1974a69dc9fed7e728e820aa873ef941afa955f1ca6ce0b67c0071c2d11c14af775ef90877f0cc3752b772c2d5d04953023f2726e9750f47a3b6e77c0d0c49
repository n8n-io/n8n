...
* ```
*/
var ChatTencentHunyuan = class extends __langchain_core_language_models_chat_models.BaseChatModel {
	static lc_name() {
	...
	*/
	...
	async *_streamResponseChunks(messages, options, runManager) {
		...
		for await (const chunk of stream) {
			...
			yield new __langchain_core_outputs.ChatGenerationChunk({
				text: Content,
				message: new __langchain_core_messages.AIMessageChunk({ content: Content }),
	...
	async *createStream(request, signal) {
		...
		const stream = __langchain_core_utils_stream.IterableReadableStream.fromReadableStream(response.body);
		let extra = "";
		for await (const chunk of stream) {
	...
	async _generate(messages, options, runManager) {
		...
		if (params.Stream) {
			...
			for await (const chunk of stream) {
				...
				generations.push({
					text: text$1,
					message: new __langchain_core_messages.AIMessage(text$1)
				});
