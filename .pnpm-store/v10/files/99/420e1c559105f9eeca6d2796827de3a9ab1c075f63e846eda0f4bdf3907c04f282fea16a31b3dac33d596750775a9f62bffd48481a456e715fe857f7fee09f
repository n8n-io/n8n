...
const require_set = require('../util/set.cjs');
...
* ```
*/
var SequentialChain = class SequentialChain extends require_base.BaseChain {
	static lc_name() {
	...
	_validateChains() {
		...
		const keysIntersection = require_set.intersection(inputKeysSet, memoryKeysSet);
		...
		const availableKeys = require_set.union(inputKeysSet, memoryKeysSet);
		for (const chain of this.chains) {
			let missingKeys = require_set.difference(new Set(chain.inputKeys), availableKeys);
			if (chain.memory) missingKeys = require_set.difference(missingKeys, new Set(chain.memory.memoryKeys));
			...
			const overlappingOutputKeys = require_set.intersection(availableKeys, outputKeysSet);
		...
		if (this.outputVariables.length === 0) if (this.returnAll) {
			const outputKeys = require_set.difference(availableKeys, inputKeysSet);
			this.outputVariables = Array.from(outputKeys);
		} else this.outputVariables = this.chains[this.chains.length - 1].outputKeys;
		else {
			const missingKeys = require_set.difference(new Set(this.outputVariables), new Set(availableKeys));
	...
	static async deserialize(data) {
		...
		for (const serializedChain of serializedChains) {
			const deserializedChain = await require_base.BaseChain.deserialize(serializedChain);
...
* ```
*/
var SimpleSequentialChain = class SimpleSequentialChain extends require_base.BaseChain {
	static lc_name() {
...
//# sourceMappingURL=sequential_chain.cjs.map