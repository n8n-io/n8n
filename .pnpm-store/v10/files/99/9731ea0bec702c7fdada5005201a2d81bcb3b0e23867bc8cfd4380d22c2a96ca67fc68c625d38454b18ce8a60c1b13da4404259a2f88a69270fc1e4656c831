...
*/
var PropertyAssignmentHandler = class extends NodeHandler {
	/**
	...
	*/
	async accepts(node) {
		if (ASTParser.isPropertyAssignment(node)) return node;
		else return false;
	}
	/**
	...
	*/
	async handle(node) {
		...
		let name;
		if (ASTParser.isIdentifier(node.key)) name = node.key.name;
		else if (ASTParser.isStringLiteral(node.key)) name = node.key.value;
...
//#endregion
export { PropertyAssignmentHandler };
//# sourceMappingURL=property_assignment_handler.js.map