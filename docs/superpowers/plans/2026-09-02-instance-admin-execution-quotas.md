...
Create `packages/frontend/editor-ui/src/features/settings/security/ExecutionQuotaTable.vue`, modeled directly on `ApiKeyTable.vue`'s structure: columns for project name, limit, period, consumed/remaining, and an actions column with an "Edit" action that emits `edit` with the row's data (following the exact same emit-based pattern `ApiKeyTable.vue` uses — do not build inline cell editing, it's not this codebase's convention for admin tables). **Correction from Task 3's actual outcome:** `getAllProjectsExecutionQuota` lives in `packages/frontend/editor-ui/src/features/collaboration/projects/projects.api.ts`, not `@n8n/rest-api-client` (that package has no project-domain API files — the plan's original assumption was wrong, ruled and corrected during Task 3). Call it directly with `useRootStore().restApiContext` as the context argument, the same way `SecuritySettings.vue` calls `securitySettingsApi.getSecuritySettings(rootStore.restApiContext)` — same calling convention, different source file.
