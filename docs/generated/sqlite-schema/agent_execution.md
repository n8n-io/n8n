...
| - | CHECK | CHECK ("status" IN ('queued', 'running', 'success', 'error', 'cancelled', 'interrupted')) |
...
| IDX_agent_execution_threadId | CREATE UNIQUE INDEX "IDX_agent_execution_threadId" ON "agent_execution" ("threadId") WHERE "runContext" IS NOT NULL AND "status" = 'running' |
| IDX_agent_execution_threadId_enqueueSequence | CREATE UNIQUE INDEX "IDX_agent_execution_threadId_enqueueSequence" ON "agent_execution" ("threadId", "enqueueSequence")  |
