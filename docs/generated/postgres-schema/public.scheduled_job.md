...
| orphanedAt | timestamp(3) with time zone |  | true |  |  | When the reconciliation sweep last confirmed this job's owner was gone; NULL while it is alive. Quarantine marker: the job is disabled first and deleted only once this is older than the quarantine grace. |
