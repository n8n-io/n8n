...
description: >-
  ...
  .vue files in packages/frontend, authoring or reviewing design-system / Reka
  UI components, or for component architecture, styling, UI changes, or feature
  work.
---
...
**Tree is the canonical reference** for new design-system components — mirror its
patterns; do not invent alternate structures, prop handling, or composition
styles:
...
- **No type assertions** — Never use `as` or non-null `!`. Use type guards /
  predicates instead (tests may use `as`).
...
- ALWAYS use CSS variables for styles from
  `packages/frontend/@n8n/design-system/src/css/_tokens.scss` or
  `packages/frontend/@n8n/design-system/src/css/_primitives.scss`. Use hard-coded
  values only when no suitable tokens exist.
- ALWAYS prefer existing components from
  `packages/frontend/@n8n/design-system/src/components` (and `src/v2` where that
  is the home for the component). Prefer components that aren't marked
  `@deprecated`.
- Use `light-dark()` when alternating colors for light/dark mode
- When reviewing UI changes or adding new components, follow
  `rules/web-interface-guidelines.md`
- When reviewing product/editor animations, follow
  `rules/web-animation-guidelines.md` and prefer mixins from
...
- **Computed** — Avoid `computed` unless there is clear, non-trivial value
  ...
  simple methods.
- **Shared utils** — Prefer helpers from `@n8n/utils` over reimplementing common
  logic locally.
...
- **Prefer slots for customization** — Expose visual/content areas as named
  ...
  `#toggle`, etc.) that forward through the component hierarchy into the default
  renderer. Provide a sensible default slot fallback so the component works out
  of the box without consumers supplying slot content. See `Tree.vue` →
  `TreeNode.vue` → `TreeNodeDefault.vue`.
...
- **Tokens** — Prefer semantic tokens; never invent one-off hard-coded values when
  a token exists.
- **No `:global`** — Do not use `:global(...)` in design-system component CSS.
  Keep styles scoped to the component (CSS modules / scoped selectors). Style
  child parts via component classes, slots, or props — not by piercing into
...
- **Motion in design-system components** — Do not use Vue `<Transition>`,
  `<TransitionGroup>`, or CSS transitions/animations in design-system
  components (follow Tree). For editor-ui / product UI motion, use the motion
  mixins instead.
...
- "Add button with + icon to add new item" → Wrap `N8nButton` with `iconOnly`
  prop with `N8nTooltip` and wrap in `N8nTooltip`. Use `N8nIcon` and proper
  aria-label.
- "Add a destructive action button" → use `N8nButton` with
  `variant="destructive"`
- "Make background color white/black" → Use `var(--background--surface)` for
  white on light mode and "black" on dark mode
- "Animate the title in gracefully" → Use `fade-in-up` mixin from `motion.scss`
  with `var(--duration--base)` (editor-ui / product UI — not design-system
  components)
