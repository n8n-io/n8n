...
| `@ApiResponse(status, options)` / `@ApiResponse(status, Dto, options)` | `options: { description?, binaryMediaType?, headers? }`. `description` replaces the generated "Operation successful." wording. `binaryMediaType` (e.g. `'application/gzip'`) documents a non-JSON body the handler writes to `res` itself — never together with a DTO, never on `204`, never `application/json` (use a DTO instead); the registry 500s if the handler returns without sending. `headers` documents response headers the route sets, `{ [name]: { description } }`. |
...
| `@Query` / `@Body` / `@Param('name')` | Bind + validate via a `Z.class` DTO / path param. `@Body` defaults to JSON. |
| `@Body({ mediaType: 'multipart/form-data', uploadLimits })` | Parses the body with multer instead of JSON. `uploadLimits: () => MultipartUploadLimits` is read once at route activation. A file field's DTO shape is `publicApiUploadedFileSchema` (`@n8n/api-types`); the merged parse input is `{ ...req.body, ...files-by-fieldname }`, files winning over a same-named text field. |
