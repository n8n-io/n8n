...
| `@Licensed('feat')` | Gates the route on a single `BooleanLicenseFeature`; `PublicApiControllerRegistry` runs its own license middleware (after auth/`@ApiKeyScope`/`@ProjectScope`|`@GlobalScope`, before the handler) and 403s unlicensed requests. Only takes one feature — if the gate is an any-of/all-of combination (e.g. a `LicenseState.isXLicensed()` helper checking several flags), `@Licensed` can't express that; check manually in the handler instead (`Container.get(License).isLicensed(...)` or the `LicenseState` helper, throwing `ForbiddenError` on failure). |
