...
- Credential encryption weakened, or `N8N_ENCRYPTION_KEY` reused for token
  signing, OAuth state, or anything but credential encryption
- OAuth state/CSRF validation bypassed, or webhook requests keeping auth cookies
- Credential access ignoring scope boundaries (instance/project/user), or a
  credential resolved by name and type with no ownership filter
