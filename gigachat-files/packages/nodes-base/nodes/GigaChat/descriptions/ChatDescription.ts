import type { INodeExecutionData, INodeProperties, IExecuteSingleFunctions, IN8nHttpFullResponse } from 'n8n-workflow';
		...
		routing: {
			output: {
				postReceive: [
					...
					async function (this: IExecuteSingleFunctions, items: INodeExecutionData[], _response: IN8nHttpFullResponse): Promise<INodeExecutionData[]> {
		...
		options: [
			{
				...
				description: 'Контролирует случайность: более низкие значения дают менее случайные завершения',
				type: 'number',
				routing: {
					send: {
						type: 'body',
