...
async function getAuthHeaders(ctx, authentication, itemIndex = 0) {
  if (authentication === "none") {
    return {};
  }
  
  if (authentication === "predefinedCredentialType") {
    // Handle predefined credential types - this would need to be implemented based on the specific credential
    return {};
  }
  ...
  if (authentication === "genericCredentialType") {
    ...
    if (genericType === "httpBasicAuth" || genericType === "httpDigestAuth") {
      ...
      // For MCP, we only need headers, not the full auth object
      const authString = Buffer.from(`${basicAuth.user}:${basicAuth.password}`).toString('base64');
      return { headers: { Authorization: `Basic ${authString}` } };
    }
    
    if (genericType === "httpHeaderAuth") {
      const headerAuth = await ctx.getCredentials("httpHeaderAuth", itemIndex);
      return { headers: { [headerAuth.name]: headerAuth.value } };
    }
    
    if (genericType === "httpBearerAuth") {
      const bearerAuth = await ctx.getCredentials("httpBearerAuth", itemIndex);
      return { headers: { Authorization: `Bearer ${bearerAuth.token}` } };
    }
