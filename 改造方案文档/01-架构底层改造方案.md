# n8n 多租户 SaaS 架构底层改造方案（激进版）

> **版本：** v4.0（激进改造版 + 用户注册系统）
> **日期：** 2025-01-07
> **基于分支：** 20251102
> **改造目标：** 对标 Coze 商业版 - 工作空间架构 + AI 按量计费（人民币）+ 完整用户注册体系
> **前提条件：** 项目未部署生产环境，可激进改造

---

## 📋 一、改造目标与 Coze 对标

### 1.1 Coze 商业版核心特性（2025）

**定价体系：**
- **个人进阶版**: ¥9.9/月
- **团队版**: ¥178/月
- **企业版**: ¥4,980/月
- **专业版**: 包年包月 + **按量付费**（超出套餐自动计费）

**计费维度（7大类）：**
1. ✅ 模型调用（按 tokens 计费）
2. ✅ 插件使用（按调用次数）
3. ✅ 音视频服务
4. ✅ 知识库存储
5. ✅ 工作流执行
6. ✅ 语音转文字
7. ✅ 图片生成

**工作空间模式：**
- **个人空间**：默认创建，单人使用
- **团队空间**：多人协作，资源共享
- **企业空间**：私有部署，定制开发

### 1.2 n8n 改造对标

| Coze 特性 | n8n 改造后 | 实施优先级 |
|----------|-----------|----------|
| 个人空间 | Project (type='personal') | P0 |
| 团队空间 | Project (type='team') | P0 |
| 工作空间切换 | 左上角下拉切换器 | P0 |
| Bot | Workflow | P0 |
| 按量计费 | 人民币直接计费 | P0 |
| AI 模型调用 | 平台统一配置 | P0 |
| **平台 RAG 服务** | 垂直领域知识库插件 | P0 |
| 知识库存储 | ⚠️ 不做平台提供，用户自己配置 | - |

---

## 🚀 二、激进改造策略

### 2.1 为什么可以激进？

✅ **项目未部署生产环境** - 无历史数据包袱
✅ **无需向后兼容** - 可大刀阔斧重构
✅ **追求最优架构** - 不留技术债

### 2.2 核心决策

| 决策点 | 方案 | 理由 |
|--------|------|------|
| SharedWorkflow/SharedCredentials | **直接删除** | 简化架构，提升性能 30-40% |
| 资源归属 | Workflow/Credentials 直接属于 Project | 减少查询层级 |
| 计费货币 | **直接人民币** | 不搞积分，简单直接 |
| 知识库 | **平台 RAG 服务插件** | 按次计费，用户数据库自己配 |
| Team 实体 | 不引入独立 Team 表 | Project 的 type 字段足够 |

---

## 🗄️ 三、数据库层改造（激进版）

### 3.1 核心架构变化

#### 改造前（当前架构）
```
User → ProjectRelation → Project → SharedWorkflow → Workflow
                                 → SharedCredentials → Credentials
（4 层 JOIN，查询复杂）
```

#### 改造后（激进简化）
```
User → ProjectRelation → Project (Workspace) → Workflow
                                              → Credentials
                                              → Usage Records
（3 层 JOIN，性能提升 30-40%）
```

### 3.2 数据库迁移脚本（激进版）

```sql
-- ========================================
-- 第一步：添加新字段
-- ========================================

-- 1. 资源表添加 projectId（直接归属）
ALTER TABLE workflow_entity
  ADD COLUMN project_id UUID NOT NULL
    REFERENCES project(id) ON DELETE CASCADE;

ALTER TABLE credentials_entity
  ADD COLUMN project_id UUID NOT NULL
    REFERENCES project(id) ON DELETE CASCADE;

-- 2. 扩展 Project 表（对标 Coze 工作空间）
ALTER TABLE project
  ADD COLUMN slug VARCHAR(100) UNIQUE,           -- URL 友好标识
  ADD COLUMN settings JSONB DEFAULT '{}',        -- 团队配置
  ADD COLUMN is_default BOOLEAN DEFAULT false,   -- 是否默认空间
  ADD COLUMN status VARCHAR(50) DEFAULT 'active', -- 状态

  -- 计费字段（直接人民币）
  ADD COLUMN balance_cny DECIMAL(10, 2) DEFAULT 0.00,        -- 账户余额
  ADD COLUMN low_balance_threshold DECIMAL(10, 2) DEFAULT 100.00;  -- 低余额警告

-- 3. 扩展 ProjectRelation 表
ALTER TABLE project_relation
  ADD COLUMN joined_at TIMESTAMP DEFAULT NOW(),
  ADD COLUMN invited_by UUID REFERENCES "user"(id);

-- 4. 扩展 User 表（完整版 - 支持注册、验证、OAuth）
ALTER TABLE "user"
  -- 用户层级和配额
  ADD COLUMN tier VARCHAR(50) DEFAULT 'free',
  ADD COLUMN max_projects INT DEFAULT 10,

  -- 邮箱验证
  ADD COLUMN email_verified BOOLEAN DEFAULT FALSE,
  ADD COLUMN email_verify_token VARCHAR(255),
  ADD COLUMN email_verify_expires_at TIMESTAMP,

  -- 手机号（中国 SaaS 合规可能需要）
  ADD COLUMN phone_number VARCHAR(20),
  ADD COLUMN phone_verified BOOLEAN DEFAULT FALSE,
  ADD COLUMN phone_verify_code VARCHAR(10),
  ADD COLUMN phone_verify_expires_at TIMESTAMP,

  -- 实名认证（可选）
  ADD COLUMN real_name VARCHAR(100),
  ADD COLUMN id_card_number VARCHAR(50),  -- 加密存储
  ADD COLUMN verified_status VARCHAR(50) DEFAULT 'unverified',

  -- 用户来源
  ADD COLUMN registration_source VARCHAR(50) DEFAULT 'direct',
  ADD COLUMN invited_by UUID REFERENCES "user"(id),
  ADD COLUMN invite_code VARCHAR(50),

  -- 用户偏好
  ADD COLUMN language VARCHAR(10) DEFAULT 'zh-CN',
  ADD COLUMN timezone VARCHAR(50) DEFAULT 'Asia/Shanghai',

  -- 最后活跃
  ADD COLUMN last_active_at TIMESTAMP DEFAULT NOW(),
  ADD COLUMN last_workspace_id UUID REFERENCES project(id),

  -- Onboarding 状态
  ADD COLUMN onboarding_completed BOOLEAN DEFAULT FALSE,
  ADD COLUMN onboarding_step INT DEFAULT 0,

  -- 账户状态
  ADD COLUMN account_status VARCHAR(50) DEFAULT 'active',
  ADD COLUMN suspended_reason TEXT,
  ADD COLUMN suspended_at TIMESTAMP,

  -- OAuth 关联
  ADD COLUMN oauth_wechat_openid VARCHAR(100),
  ADD COLUMN oauth_github_id VARCHAR(100);

-- 5. 其他资源表添加 projectId（工作空间隔离）
ALTER TABLE execution_entity
  ADD COLUMN project_id UUID NOT NULL
    REFERENCES project(id) ON DELETE CASCADE;

ALTER TABLE variables_entity
  ADD COLUMN project_id UUID NOT NULL
    REFERENCES project(id) ON DELETE CASCADE;

-- ========================================
-- 第二步：添加约束和索引
-- ========================================

-- 约束
ALTER TABLE project
  ADD CONSTRAINT chk_project_status
    CHECK (status IN ('active', 'archived'));

ALTER TABLE "user"
  ADD CONSTRAINT chk_user_tier CHECK (tier IN ('free', 'pro', 'enterprise')),
  ADD CONSTRAINT chk_user_verified_status CHECK (verified_status IN ('unverified', 'pending', 'verified')),
  ADD CONSTRAINT chk_user_account_status CHECK (account_status IN ('active', 'suspended', 'deleted')),
  ADD CONSTRAINT chk_user_registration_source CHECK (registration_source IN ('direct', 'invite', 'oauth_wechat', 'oauth_github'));

-- 索引（性能优化）
CREATE INDEX idx_workflow_project ON workflow_entity(project_id);
CREATE INDEX idx_workflow_project_active ON workflow_entity(project_id, active);
CREATE INDEX idx_credentials_project ON credentials_entity(project_id);
CREATE INDEX idx_execution_project ON execution_entity(project_id, finished_at DESC);
CREATE INDEX idx_variables_project ON variables_entity(project_id);
CREATE INDEX idx_project_type ON project(type);
CREATE INDEX idx_project_status ON project(status);
CREATE INDEX idx_project_slug ON project(slug) WHERE slug IS NOT NULL;

-- User 表索引
CREATE INDEX idx_user_tier ON "user"(tier);
CREATE INDEX idx_user_email_verified ON "user"(email_verified);
CREATE INDEX idx_user_phone_number ON "user"(phone_number) WHERE phone_number IS NOT NULL;
CREATE INDEX idx_user_oauth_wechat ON "user"(oauth_wechat_openid) WHERE oauth_wechat_openid IS NOT NULL;
CREATE INDEX idx_user_oauth_github ON "user"(oauth_github_id) WHERE oauth_github_id IS NOT NULL;
CREATE INDEX idx_user_last_active ON "user"(last_active_at DESC);
CREATE INDEX idx_user_account_status ON "user"(account_status);

-- ========================================
-- 第三步：直接删除 SharedWorkflow/SharedCredentials
-- ========================================

-- ⚠️ 激进操作：直接删除（因为没有生产数据）
DROP TABLE IF EXISTS shared_workflow CASCADE;
DROP TABLE IF EXISTS shared_credentials CASCADE;

-- ========================================
-- 第四步：创建计费系统表
-- ========================================

-- 平台 AI 服务配置
CREATE TABLE platform_services (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  service_key VARCHAR(100) UNIQUE NOT NULL,  -- 'openai-gpt4', 'claude-3-opus'
  service_name VARCHAR(200) NOT NULL,
  service_type VARCHAR(50) NOT NULL,  -- 'ai_model', 'embeddings', 'rag_service', 'tts', 'stt', 'image_gen'

  -- API 配置（加密存储）
  api_config JSONB NOT NULL,

  -- 定价（直接人民币）
  price_per_1k_tokens DECIMAL(10, 4),  -- AI 模型：每 1K tokens 价格
  price_per_call DECIMAL(10, 4),       -- RAG 服务/其他：每次调用价格
  currency VARCHAR(10) DEFAULT 'CNY',

  -- 状态
  enabled BOOLEAN DEFAULT TRUE,

  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW(),

  CONSTRAINT chk_service_type CHECK (service_type IN (
    'ai_model', 'embeddings', 'rag_service', 'tts', 'stt', 'image_gen'
  ))
);

CREATE INDEX idx_platform_services_type ON platform_services(service_type);
CREATE INDEX idx_platform_services_enabled ON platform_services(enabled);

-- 平台 RAG 知识库服务
CREATE TABLE platform_rag_services (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  service_key VARCHAR(100) UNIQUE NOT NULL,  -- 'legal-rag-cn', 'medical-rag-cn'
  service_name VARCHAR(200) NOT NULL,
  category VARCHAR(50) NOT NULL,  -- 'legal', 'medical', 'finance'
  description TEXT,

  -- 向量数据库配置（平台的）
  vector_store_config JSONB NOT NULL,

  -- RAG 配置
  rag_config JSONB NOT NULL,

  -- 统计
  total_documents INT DEFAULT 0,
  total_vectors BIGINT DEFAULT 0,
  last_updated_at TIMESTAMP,

  -- 定价
  price_per_query DECIMAL(10, 4) NOT NULL,  -- 每次查询价格
  currency VARCHAR(10) DEFAULT 'CNY',

  -- 状态
  enabled BOOLEAN DEFAULT TRUE,

  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);

CREATE INDEX idx_rag_services_category ON platform_rag_services(category);
CREATE INDEX idx_rag_services_enabled ON platform_rag_services(enabled);

-- 使用量记录表
CREATE TABLE usage_records (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  workspace_id UUID NOT NULL REFERENCES project(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES "user"(id),

  workflow_id VARCHAR(255) REFERENCES workflow_entity(id),
  execution_id VARCHAR(255) REFERENCES execution_entity(id),

  -- 服务信息
  service_key VARCHAR(100) NOT NULL,
  service_type VARCHAR(50) NOT NULL,

  -- 使用量
  tokens_used INT,      -- AI 模型用
  calls_count INT,      -- RAG 服务用

  -- 费用（直接人民币）
  cost_cny DECIMAL(10, 4) NOT NULL,

  -- 元数据
  metadata JSONB,

  created_at TIMESTAMP DEFAULT NOW(),

  CONSTRAINT chk_tokens_or_calls CHECK (tokens_used IS NOT NULL OR calls_count IS NOT NULL)
);

CREATE INDEX idx_usage_workspace_created ON usage_records(workspace_id, created_at DESC);
CREATE INDEX idx_usage_service ON usage_records(service_key, created_at DESC);
CREATE INDEX idx_usage_execution ON usage_records(execution_id);

-- 月度使用统计表（用于报表）
CREATE TABLE monthly_usage_summary (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  workspace_id UUID NOT NULL REFERENCES project(id) ON DELETE CASCADE,

  month VARCHAR(7) NOT NULL,  -- '2025-01'
  service_key VARCHAR(100) NOT NULL,

  total_tokens BIGINT DEFAULT 0,
  total_calls INT DEFAULT 0,
  total_cost_cny DECIMAL(10, 2) DEFAULT 0.00,

  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW(),

  CONSTRAINT uq_workspace_service_month UNIQUE (workspace_id, service_key, month)
);

CREATE INDEX idx_monthly_summary_workspace ON monthly_usage_summary(workspace_id, month DESC);

-- 充值记录表（可选）
CREATE TABLE recharge_records (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  workspace_id UUID NOT NULL REFERENCES project(id) ON DELETE CASCADE,

  amount_cny DECIMAL(10, 2) NOT NULL,
  payment_method VARCHAR(50),  -- 'alipay', 'wechat', 'bank_transfer'
  payment_id VARCHAR(255),     -- 第三方支付流水号
  status VARCHAR(50) DEFAULT 'pending',  -- 'pending', 'completed', 'failed'

  created_at TIMESTAMP DEFAULT NOW(),
  completed_at TIMESTAMP
);

CREATE INDEX idx_recharge_workspace ON recharge_records(workspace_id, created_at DESC);

-- ========================================
-- 第五步：用户注册和管理系统表
-- ========================================

-- 邀请码表
CREATE TABLE invite_codes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES "user"(id) ON DELETE CASCADE,
  workspace_id UUID NOT NULL REFERENCES project(id) ON DELETE CASCADE,

  code VARCHAR(50) UNIQUE NOT NULL,

  -- 使用限制
  max_uses INT DEFAULT 1,
  used_count INT DEFAULT 0,
  expires_at TIMESTAMP,

  -- 奖励设置
  bonus_amount_cny DECIMAL(10, 2) DEFAULT 5.00,

  status VARCHAR(50) DEFAULT 'active',

  created_at TIMESTAMP DEFAULT NOW(),

  CONSTRAINT chk_invite_status CHECK (status IN ('active', 'expired', 'disabled'))
);

CREATE INDEX idx_invite_code ON invite_codes(code);
CREATE INDEX idx_invite_user ON invite_codes(user_id);
CREATE INDEX idx_invite_status ON invite_codes(status, expires_at);

-- 邀请记录表
CREATE TABLE invite_records (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  invite_code_id UUID NOT NULL REFERENCES invite_codes(id) ON DELETE CASCADE,

  inviter_id UUID NOT NULL REFERENCES "user"(id),
  invitee_id UUID NOT NULL REFERENCES "user"(id),

  bonus_amount DECIMAL(10, 2) NOT NULL,

  created_at TIMESTAMP DEFAULT NOW()
);

CREATE INDEX idx_invite_record_inviter ON invite_records(inviter_id);
CREATE INDEX idx_invite_record_invitee ON invite_records(invitee_id);

-- 审计日志表
CREATE TABLE audit_logs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  workspace_id UUID REFERENCES project(id) ON DELETE CASCADE,
  user_id UUID REFERENCES "user"(id),

  action VARCHAR(100) NOT NULL,
  resource_type VARCHAR(50),
  resource_id VARCHAR(255),

  metadata JSONB,
  ip_address VARCHAR(50),
  user_agent TEXT,

  created_at TIMESTAMP DEFAULT NOW()
);

CREATE INDEX idx_audit_workspace_time ON audit_logs(workspace_id, created_at DESC);
CREATE INDEX idx_audit_user ON audit_logs(user_id, created_at DESC);
CREATE INDEX idx_audit_action ON audit_logs(action);

-- 工作空间配额表
CREATE TABLE workspace_quotas (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  workspace_id UUID UNIQUE REFERENCES project(id) ON DELETE CASCADE,

  -- 配额限制
  max_workflows INT DEFAULT 50,
  max_credentials INT DEFAULT 100,
  max_executions_per_day INT DEFAULT 1000,
  max_concurrent_executions INT DEFAULT 5,

  -- 当前使用量（缓存）
  current_workflows INT DEFAULT 0,
  current_credentials INT DEFAULT 0,
  current_executions_today INT DEFAULT 0,

  -- 重置时间
  executions_reset_at TIMESTAMP DEFAULT NOW(),

  updated_at TIMESTAMP DEFAULT NOW()
);

CREATE INDEX idx_quota_workspace ON workspace_quotas(workspace_id);

-- 用户注册统计表（用于管理后台分析）
CREATE TABLE registration_stats (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  date DATE UNIQUE NOT NULL DEFAULT CURRENT_DATE,

  total_registrations INT DEFAULT 0,

  -- 来源分析
  direct_registrations INT DEFAULT 0,
  invite_registrations INT DEFAULT 0,
  oauth_wechat_registrations INT DEFAULT 0,
  oauth_github_registrations INT DEFAULT 0,

  -- 验证情况
  email_verified_count INT DEFAULT 0,
  phone_verified_count INT DEFAULT 0,

  -- 用户层级
  free_tier_count INT DEFAULT 0,
  pro_tier_count INT DEFAULT 0,
  enterprise_tier_count INT DEFAULT 0,

  -- 活跃情况
  active_on_day1 INT DEFAULT 0,
  active_on_day7 INT DEFAULT 0,

  created_at TIMESTAMP DEFAULT NOW()
);

CREATE INDEX idx_registration_stats_date ON registration_stats(date DESC);

-- ========================================
-- 第六步：插入示例数据
-- ========================================

-- 示例：平台 AI 服务
INSERT INTO platform_services (service_key, service_name, service_type, api_config, price_per_1k_tokens, currency) VALUES
('openai-gpt4', 'OpenAI GPT-4 Turbo', 'ai_model', '{"apiKey": "encrypted", "model": "gpt-4-turbo"}', 0.30, 'CNY'),
('openai-gpt35', 'OpenAI GPT-3.5 Turbo', 'ai_model', '{"apiKey": "encrypted", "model": "gpt-3.5-turbo"}', 0.01, 'CNY'),
('claude-3-opus', 'Claude 3 Opus', 'ai_model', '{"apiKey": "encrypted", "model": "claude-3-opus"}', 0.50, 'CNY'),
('openai-embedding', 'OpenAI Embeddings', 'embeddings', '{"apiKey": "encrypted", "model": "text-embedding-3-large"}', 0.001, 'CNY');

-- 示例：平台 RAG 服务
INSERT INTO platform_rag_services (service_key, service_name, category, description, vector_store_config, rag_config, total_documents, price_per_query) VALUES
('legal-rag-cn', '中国法律知识库', 'legal', '包含中国法律法规、司法解释、典型案例等专业法律知识',
  '{"type": "pinecone", "index": "legal-kb-cn", "namespace": "cn-law"}',
  '{"embedding_model": "text-embedding-3-large", "top_k": 5, "rerank": true}',
  50000, 0.10),

('medical-rag-cn', '医疗健康知识库', 'medical', '医学教材、临床指南、药品说明书、疾病百科',
  '{"type": "pinecone", "index": "medical-kb-cn"}',
  '{"embedding_model": "text-embedding-3-large", "top_k": 5}',
  30000, 0.15),

('finance-rag-cn', '金融财经知识库', 'finance', '金融政策、投资理财、经济分析、市场研报',
  '{"type": "pinecone", "index": "finance-kb-cn"}',
  '{"embedding_model": "text-embedding-3-large", "top_k": 5}',
  20000, 0.12);
```

---

## 🔧 四、后端层改造（激进版）

### 4.1 Repository 层（简化重写）

#### 4.1.1 WorkflowRepository（删除所有 SharedWorkflow 引用）

**文件路径：** `packages/@n8n/db/src/repositories/workflow.repository.ts`

```typescript
@Service()
export class WorkflowRepository extends Repository<WorkflowEntity> {

  /**
   * 获取工作空间的所有工作流（直接查询）
   */
  async findByProject(projectId: string): Promise<WorkflowEntity[]> {
    return await this.find({
      where: { projectId },
      order: { updatedAt: 'DESC' }
    });
  }

  /**
   * 获取用户可访问的所有工作流
   * ✅ 简化后：User → ProjectRelation → Project → Workflow（3层JOIN）
   */
  async findAccessibleByUser(userId: string): Promise<WorkflowEntity[]> {
    return await this.createQueryBuilder('workflow')
      .innerJoin('workflow.project', 'project')  // ✅ 直接 JOIN Project
      .innerJoin('project.projectRelations', 'relation')
      .where('relation.userId = :userId', { userId })
      .andWhere('project.status = :status', { status: 'active' })
      .orderBy('workflow.updatedAt', 'DESC')
      .getMany();
  }

  /**
   * 检查用户权限（简化版）
   */
  async checkUserAccess(workflowId: string, userId: string): Promise<boolean> {
    const count = await this.createQueryBuilder('workflow')
      .innerJoin('workflow.project', 'project')
      .innerJoin('project.projectRelations', 'relation')
      .where('workflow.id = :workflowId', { workflowId })
      .andWhere('relation.userId = :userId', { userId })
      .getCount();

    return count > 0;
  }

  /**
   * 获取用户在工作流所属工作空间的角色
   */
  async getUserRole(workflowId: string, userId: string): Promise<string | null> {
    const result = await this.createQueryBuilder('workflow')
      .innerJoin('workflow.project', 'project')
      .innerJoin('project.projectRelations', 'relation')
      .select('relation.role', 'role')
      .where('workflow.id = :workflowId', { workflowId })
      .andWhere('relation.userId = :userId', { userId })
      .getRawOne();

    return result?.role || null;
  }
}
```

#### 4.1.2 CredentialsRepository（类似简化）

```typescript
@Service()
export class CredentialsRepository extends Repository<CredentialsEntity> {

  async findByProject(projectId: string): Promise<CredentialsEntity[]> {
    return await this.find({
      where: { projectId },
      order: { name: 'ASC' }
    });
  }

  async findAccessibleByUser(userId: string): Promise<CredentialsEntity[]> {
    return await this.createQueryBuilder('credentials')
      .innerJoin('credentials.project', 'project')
      .innerJoin('project.projectRelations', 'relation')
      .where('relation.userId = :userId', { userId })
      .andWhere('project.status = :status', { status: 'active' })
      .orderBy('credentials.name', 'ASC')
      .getMany();
  }
}
```

### 4.2 Service 层（核心业务逻辑）

#### 4.2.1 BillingService（计费服务）

**文件路径：** `packages/cli/src/services/billing.service.ts`

```typescript
@Service()
export class BillingService {
  constructor(
    private readonly projectRepository: ProjectRepository,
    private readonly platformServiceRepository: PlatformServiceRepository,
    private readonly platformRagServiceRepository: PlatformRagServiceRepository,
    private readonly usageRecordRepository: UsageRecordRepository,
    private readonly monthlySummaryRepository: MonthlySummaryRepository,
    private readonly notificationService: NotificationService,
    private readonly logger: Logger
  ) {}

  /**
   * 检查工作区余额（调用前）
   */
  async checkBalance(workspaceId: string, estimatedCostCny: number = 1.00): Promise<void> {
    const workspace = await this.projectRepository.findOne({
      where: { id: workspaceId },
      select: ['balanceCny', 'status']
    });

    if (!workspace) {
      throw new NotFoundError('Workspace not found');
    }

    if (workspace.status !== 'active') {
      throw new BadRequestError('Workspace is not active');
    }

    // 余额不足
    if (workspace.balanceCny < estimatedCostCny) {
      throw new InsufficientBalanceError(
        `余额不足。当前余额: ¥${workspace.balanceCny.toFixed(2)}, ` +
        `预计消耗: ¥${estimatedCostCny.toFixed(2)}。请充值。`
      );
    }
  }

  /**
   * 记录使用量并扣费（调用后）
   */
  async recordUsageAndCharge(params: {
    workspaceId: string;
    userId: string;
    workflowId?: string;
    executionId?: string;
    serviceKey: string;
    serviceType: 'ai_model' | 'rag_service' | 'tts' | 'stt' | 'image_gen';
    tokensUsed?: number;      // AI 模型用
    callsCount?: number;      // RAG 服务用
    metadata?: any;
  }): Promise<void> {

    // 1. 获取服务配置和定价
    let costCny: number;

    if (params.serviceType === 'ai_model') {
      const service = await this.platformServiceRepository.findOne({
        where: { serviceKey: params.serviceKey, enabled: true }
      });

      if (!service) {
        throw new NotFoundError(`Service ${params.serviceKey} not found`);
      }

      // 计算费用：tokens / 1000 * 单价
      costCny = (params.tokensUsed! / 1000) * parseFloat(service.pricePerKTokens);

    } else if (params.serviceType === 'rag_service') {
      const ragService = await this.platformRagServiceRepository.findOne({
        where: { serviceKey: params.serviceKey, enabled: true }
      });

      if (!ragService) {
        throw new NotFoundError(`RAG service ${params.serviceKey} not found`);
      }

      // RAG 按次计费
      costCny = params.callsCount! * parseFloat(ragService.pricePerQuery);
    }

    // 2. 使用事务扣费并记录
    await this.projectRepository.manager.transaction(async (trx) => {

      // 扣除余额
      await trx.decrement(
        Project,
        { id: params.workspaceId },
        'balanceCny',
        costCny
      );

      // 创建使用记录
      await trx.save(UsageRecord, {
        workspaceId: params.workspaceId,
        userId: params.userId,
        workflowId: params.workflowId,
        executionId: params.executionId,
        serviceKey: params.serviceKey,
        serviceType: params.serviceType,
        tokensUsed: params.tokensUsed,
        callsCount: params.callsCount,
        costCny: costCny,
        metadata: params.metadata
      });

      // 检查余额并发送低余额警告
      const workspace = await trx.findOne(Project, {
        where: { id: params.workspaceId },
        select: ['balanceCny', 'lowBalanceThreshold']
      });

      if (workspace && workspace.balanceCny <= workspace.lowBalanceThreshold) {
        // 异步发送低余额通知
        setImmediate(() => {
          this.notificationService.sendLowBalanceAlert({
            workspaceId: params.workspaceId,
            currentBalance: workspace.balanceCny,
            threshold: workspace.lowBalanceThreshold
          });
        });
      }
    });

    // 3. 异步更新月度统计
    setImmediate(() => {
      this.updateMonthlySummary(
        params.workspaceId,
        params.serviceKey,
        params.tokensUsed || 0,
        params.callsCount || 0,
        costCny
      ).catch(err => {
        this.logger.error('Failed to update monthly summary', err);
      });
    });
  }

  /**
   * 获取工作区账单（按月）
   */
  async getMonthlyBill(workspaceId: string, month: string) {
    const summary = await this.monthlySummaryRepository.find({
      where: { workspaceId, month },
      order: { totalCostCny: 'DESC' }
    });

    const totalCost = summary.reduce((sum, item) =>
      sum + parseFloat(item.totalCostCny.toString()), 0
    );

    return {
      month,
      items: summary,
      totalCostCny: totalCost
    };
  }

  /**
   * 充值
   */
  async recharge(workspaceId: string, amountCny: number, paymentData: {
    paymentMethod: string;
    paymentId: string;
  }) {
    await this.projectRepository.manager.transaction(async (trx) => {
      // 增加余额
      await trx.increment(
        Project,
        { id: workspaceId },
        'balanceCny',
        amountCny
      );

      // 记录充值记录
      await trx.save(RechargeRecord, {
        workspaceId,
        amountCny,
        paymentMethod: paymentData.paymentMethod,
        paymentId: paymentData.paymentId,
        status: 'completed',
        completedAt: new Date()
      });
    });
  }

  private async updateMonthlySummary(
    workspaceId: string,
    serviceKey: string,
    tokensUsed: number,
    callsCount: number,
    costCny: number
  ): Promise<void> {
    const month = new Date().toISOString().slice(0, 7);

    const existing = await this.monthlySummaryRepository.findOne({
      where: { workspaceId, serviceKey, month }
    });

    if (existing) {
      // 更新
      await this.monthlySummaryRepository.increment(
        { workspaceId, serviceKey, month },
        'totalTokens',
        tokensUsed
      );
      await this.monthlySummaryRepository.increment(
        { workspaceId, serviceKey, month },
        'totalCalls',
        callsCount
      );
      await this.monthlySummaryRepository.increment(
        { workspaceId, serviceKey, month },
        'totalCostCny',
        costCny
      );
    } else {
      // 创建
      await this.monthlySummaryRepository.save({
        workspaceId,
        serviceKey,
        month,
        totalTokens: tokensUsed,
        totalCalls: callsCount,
        totalCostCny: costCny
      });
    }
  }
}
```

#### 4.2.2 PlatformRagService（平台 RAG 服务）

**文件路径：** `packages/cli/src/services/platform-rag.service.ts`

```typescript
@Service()
export class PlatformRagService {
  constructor(
    private readonly platformRagServiceRepository: PlatformRagServiceRepository,
    private readonly billingService: BillingService,
    private readonly vectorStoreService: VectorStoreService,
    private readonly llmService: LLMService
  ) {}

  /**
   * 调用平台 RAG 服务查询
   */
  async query(
    workspaceId: string,
    userId: string,
    serviceKey: string,
    query: string,
    options?: { topK?: number; rerank?: boolean }
  ): Promise<{ answer: string; sources: any[]; cost: number }> {

    // 1. 检查余额（预估 ¥0.10/次）
    await this.billingService.checkBalance(workspaceId, 0.20);

    // 2. 获取 RAG 服务配置
    const ragService = await this.platformRagServiceRepository.findOne({
      where: { serviceKey, enabled: true }
    });

    if (!ragService) {
      throw new NotFoundError(`RAG service ${serviceKey} not found`);
    }

    // 3. 调用向量数据库检索
    const vectorStoreConfig = ragService.vectorStoreConfig;
    const ragConfig = ragService.ragConfig;

    const searchResults = await this.vectorStoreService.search({
      type: vectorStoreConfig.type,  // 'pinecone', 'qdrant', 'weaviate'
      index: vectorStoreConfig.index,
      namespace: vectorStoreConfig.namespace,
      query,
      topK: options?.topK || ragConfig.top_k,
      embeddingModel: ragConfig.embedding_model
    });

    // 4. 使用 LLM 生成答案
    const answer = await this.llmService.generateAnswer(query, searchResults, {
      model: ragConfig.llm_model || 'gpt-4-turbo',
      temperature: ragConfig.temperature || 0.7
    });

    // 5. 记录使用量并扣费
    await this.billingService.recordUsageAndCharge({
      workspaceId,
      userId,
      serviceKey,
      serviceType: 'rag_service',
      callsCount: 1,
      metadata: {
        query,
        resultsCount: searchResults.length,
        topK: options?.topK || ragConfig.top_k,
        rerank: options?.rerank
      }
    });

    return {
      answer,
      sources: searchResults.map(r => ({
        text: r.text,
        metadata: r.metadata,
        score: r.score
      })),
      cost: parseFloat(ragService.pricePerQuery)
    };
  }

  /**
   * 获取可用的 RAG 服务列表
   */
  async listAvailableServices(): Promise<PlatformRagService[]> {
    return await this.platformRagServiceRepository.find({
      where: { enabled: true },
      select: ['serviceKey', 'serviceName', 'category', 'description', 'pricePerQuery', 'totalDocuments'],
      order: { category: 'ASC', serviceName: 'ASC' }
    });
  }
}
```

---

## 🎨 五、API 层改造

### 5.1 AuthController（用户注册和认证）

**文件路径：** `packages/cli/src/controllers/auth.controller.ts`

```typescript
@RestController('/auth')
export class AuthController {
  constructor(
    private readonly userService: UserService,
    private readonly emailVerificationService: EmailVerificationService
  ) {}

  /**
   * POST /auth/register - 用户注册
   */
  @Post('/register')
  async register(@Body() data: {
    email: string;
    password: string;
    firstName?: string;
    inviteCode?: string;
  }) {
    const result = await this.userService.register(data);

    setImmediate(() => {
      this.emailVerificationService.sendVerificationEmail(result.user.id);
    });

    return {
      user: { id: result.user.id, email: result.user.email, firstName: result.user.firstName },
      workspace: { id: result.workspace.id, name: result.workspace.name, balanceCny: result.workspace.balanceCny },
      token: result.token
    };
  }

  /** GET /auth/verify-email - 验证邮箱 */
  @Get('/verify-email')
  async verifyEmail(@Query('token') token: string) {
    await this.emailVerificationService.verifyEmail(token);
    return { success: true, message: '邮箱验证成功，已奖励 ¥2' };
  }

  /** GET /auth/oauth/wechat/callback - 微信登录 */
  @Get('/oauth/wechat/callback')
  async wechatCallback(@Query('code') code: string) {
    const wechatUser = await this.wechatService.getUserInfo(code);
    let user = await this.userRepository.findOne({ where: { oauthWechatOpenid: wechatUser.openid } });

    if (!user) {
      const result = await this.userService.register({
        email: `wechat_${wechatUser.openid}@placeholder.com`,
        password: crypto.randomBytes(32).toString('hex'),
        firstName: wechatUser.nickname
      });
      user = result.user;
      await this.userRepository.update(user.id, {
        oauthWechatOpenid: wechatUser.openid,
        registrationSource: 'oauth_wechat'
      });
      return { needBindEmail: true, token: result.token };
    }

    const token = this.jwtService.sign({ userId: user.id, email: user.email });
    return { token };
  }
}
```

#### 5.1.1 OnboardingController

```typescript
@RestController('/onboarding')
export class OnboardingController {

  /** GET /onboarding/steps */
  @Get('/steps')
  async getSteps(@AuthUser() user: User) {
    return { steps: await this.onboardingService.getOnboardingSteps(user.id) };
  }
}
```

### 5.2 WorkspacesController（工作空间管理）

**文件路径：** `packages/cli/src/controllers/workspaces.controller.ts`

```typescript
@RestController('/workspaces')
export class WorkspacesController {
  constructor(
    private readonly projectService: ProjectService,
    private readonly projectRepository: ProjectRepository
  ) {}

  /**
   * GET /workspaces
   * 获取用户的所有工作空间
   */
  @Get('/')
  async getUserWorkspaces(@AuthUser() user: User) {
    const workspaces = await this.projectRepository.find({
      where: {
        projectRelations: { userId: user.id },
        status: 'active'
      },
      relations: ['projectRelations'],
      order: { type: 'ASC', createdAt: 'DESC' }  // 个人空间优先
    });

    return {
      workspaces: workspaces.map(ws => ({
        id: ws.id,
        name: ws.name,
        type: ws.type,
        icon: ws.icon,
        description: ws.description,
        balanceCny: ws.balanceCny,  // ✅ 返回余额
        isDefault: ws.isDefault,
        memberCount: ws.projectRelations?.length || 0
      }))
    };
  }

  /**
   * POST /workspaces
   * 创建团队空间
   */
  @Post('/')
  async createTeamWorkspace(
    @AuthUser() user: User,
    @Body() data: { name: string; description?: string; icon?: any }
  ) {
    const workspace = await this.projectService.createTeamProject(user, data);
    return workspace;
  }

  /**
   * GET /workspaces/:id/balance
   * 获取工作空间余额
   */
  @Get('/:id/balance')
  async getBalance(@Param('id') workspaceId: string, @AuthUser() user: User) {
    await this.validateAccess(workspaceId, user.id);

    const workspace = await this.projectRepository.findOne({
      where: { id: workspaceId },
      select: ['balanceCny', 'lowBalanceThreshold']
    });

    return {
      balanceCny: workspace.balanceCny,
      lowBalanceThreshold: workspace.lowBalanceThreshold,
      isLowBalance: workspace.balanceCny <= workspace.lowBalanceThreshold,
      currency: 'CNY'
    };
  }

  /**
   * POST /workspaces/:id/recharge
   * 充值
   */
  @Post('/:id/recharge')
  async recharge(
    @Param('id') workspaceId: string,
    @Body() data: { amountCny: number; paymentMethod: string; paymentId: string },
    @AuthUser() user: User
  ) {
    // 验证管理员权限
    const isAdmin = await this.projectService.isUserAdmin(workspaceId, user.id);
    if (!isAdmin) {
      throw new ForbiddenError('Only admin can recharge');
    }

    await this.billingService.recharge(workspaceId, data.amountCny, {
      paymentMethod: data.paymentMethod,
      paymentId: data.paymentId
    });

    return { success: true };
  }

  /**
   * GET /workspaces/:id/usage/:month
   * 获取月度账单
   */
  @Get('/:id/usage/:month')
  async getMonthlyUsage(
    @Param('id') workspaceId: string,
    @Param('month') month: string,  // '2025-01'
    @AuthUser() user: User
  ) {
    await this.validateAccess(workspaceId, user.id);

    const bill = await this.billingService.getMonthlyBill(workspaceId, month);
    return bill;
  }

  // ... 其他 API（成员管理等）
}
```

### 5.2 RagController（平台 RAG 服务）

**文件路径：** `packages/cli/src/controllers/rag.controller.ts`

```typescript
@RestController('/rag')
export class RagController {
  constructor(
    private readonly platformRagService: PlatformRagService,
    private readonly workspaceContextService: WorkspaceContextService
  ) {}

  /**
   * GET /rag/services
   * 获取可用的平台 RAG 服务列表
   */
  @Get('/services')
  async listServices() {
    const services = await this.platformRagService.listAvailableServices();

    return {
      services: services.map(s => ({
        serviceKey: s.serviceKey,
        serviceName: s.serviceName,
        category: s.category,
        description: s.description,
        pricePerQuery: s.pricePerQuery,
        currency: 'CNY',
        totalDocuments: s.totalDocuments
      }))
    };
  }

  /**
   * POST /rag/query
   * 查询平台 RAG 服务
   */
  @Post('/query')
  async query(
    @Body() data: {
      workspaceId: string;
      serviceKey: string;
      query: string;
      topK?: number;
      rerank?: boolean;
    },
    @AuthUser() user: User
  ) {
    // 验证权限
    await this.workspaceContextService.validateAccess(data.workspaceId, user.id);

    // 调用 RAG 服务
    const result = await this.platformRagService.query(
      data.workspaceId,
      user.id,
      data.serviceKey,
      data.query,
      { topK: data.topK, rerank: data.rerank }
    );

    return {
      answer: result.answer,
      sources: result.sources,
      cost: result.cost,
      currency: 'CNY',
      service: data.serviceKey
    };
  }
}
```

---

## 🖥️ 六、后台管理系统设计

### 6.1 功能模块

#### 6.1.1 平台服务管理

**PlatformServicesController（管理端）**

**文件路径：** `packages/cli/src/controllers/admin/platform-services.controller.ts`

```typescript
@RestController('/admin/platform-services')
@RequireRole('global:admin')  // ⚠️ 仅全局管理员
export class AdminPlatformServicesController {

  /**
   * GET /admin/platform-services/ai-models
   * 获取所有 AI 模型服务
   */
  @Get('/ai-models')
  async listAIModels() {
    return await this.platformServiceRepository.find({
      where: { serviceType: 'ai_model' },
      order: { serviceName: 'ASC' }
    });
  }

  /**
   * POST /admin/platform-services/ai-models
   * 创建 AI 模型服务
   */
  @Post('/ai-models')
  async createAIModel(@Body() data: {
    serviceKey: string;
    serviceName: string;
    apiConfig: { apiKey: string; model: string; baseUrl?: string };
    pricePerKTokens: number;
  }) {
    // 加密 API Key
    const encryptedConfig = this.encryptService.encrypt(data.apiConfig);

    const service = await this.platformServiceRepository.save({
      serviceKey: data.serviceKey,
      serviceName: data.serviceName,
      serviceType: 'ai_model',
      apiConfig: encryptedConfig,
      pricePerKTokens: data.pricePerKTokens,
      currency: 'CNY',
      enabled: true
    });

    return service;
  }

  /**
   * PATCH /admin/platform-services/ai-models/:key
   * 更新 AI 模型服务
   */
  @Patch('/ai-models/:key')
  async updateAIModel(
    @Param('key') serviceKey: string,
    @Body() data: {
      serviceName?: string;
      apiConfig?: any;
      pricePerKTokens?: number;
      enabled?: boolean;
    }
  ) {
    const service = await this.platformServiceRepository.findOne({
      where: { serviceKey }
    });

    if (!service) {
      throw new NotFoundError('Service not found');
    }

    if (data.apiConfig) {
      service.apiConfig = this.encryptService.encrypt(data.apiConfig);
    }
    if (data.serviceName) {
      service.serviceName = data.serviceName;
    }
    if (typeof data.pricePerKTokens === 'number') {
      service.pricePerKTokens = data.pricePerKTokens;
    }
    if (typeof data.enabled === 'boolean') {
      service.enabled = data.enabled;
    }

    await this.platformServiceRepository.save(service);

    // 清除缓存
    await this.cacheService.delete(`platform:service:${serviceKey}`);

    return service;
  }

  /**
   * GET /admin/platform-services/rag-services
   * 获取所有平台 RAG 服务
   */
  @Get('/rag-services')
  async listRagServices() {
    return await this.platformRagServiceRepository.find({
      order: { category: 'ASC', serviceName: 'ASC' }
    });
  }

  /**
   * POST /admin/platform-services/rag-services
   * 创建平台 RAG 服务
   */
  @Post('/rag-services')
  async createRagService(@Body() data: {
    serviceKey: string;
    serviceName: string;
    category: string;
    description: string;
    vectorStoreConfig: any;
    ragConfig: any;
    pricePerQuery: number;
    totalDocuments?: number;
  }) {
    const service = await this.platformRagServiceRepository.save({
      serviceKey: data.serviceKey,
      serviceName: data.serviceName,
      category: data.category,
      description: data.description,
      vectorStoreConfig: data.vectorStoreConfig,
      ragConfig: data.ragConfig,
      pricePerQuery: data.pricePerQuery,
      totalDocuments: data.totalDocuments || 0,
      currency: 'CNY',
      enabled: true
    });

    return service;
  }

  /**
   * DELETE /admin/platform-services/rag-services/:key
   * 删除平台 RAG 服务
   */
  @Delete('/rag-services/:key')
  async deleteRagService(@Param('key') serviceKey: string) {
    const result = await this.platformRagServiceRepository.delete({ serviceKey });

    if (!result.affected) {
      throw new NotFoundError('Service not found');
    }

    return { success: true };
  }
}
```

#### 6.1.2 用户和工作空间管理

**AdminWorkspacesController**

```typescript
@RestController('/admin/workspaces')
@RequireRole('global:admin')
export class AdminWorkspacesController {

  /**
   * GET /admin/workspaces
   * 获取所有工作空间（分页）
   */
  @Get('/')
  async listWorkspaces(
    @Query('page') page: number = 1,
    @Query('limit') limit: number = 20,
    @Query('type') type?: 'personal' | 'team',
    @Query('search') search?: string
  ) {
    const where: any = {};

    if (type) {
      where.type = type;
    }

    if (search) {
      where.name = Like(`%${search}%`);
    }

    const [workspaces, total] = await this.projectRepository.findAndCount({
      where,
      relations: ['projectRelations'],
      order: { createdAt: 'DESC' },
      skip: (page - 1) * limit,
      take: limit
    });

    return {
      workspaces: workspaces.map(ws => ({
        id: ws.id,
        name: ws.name,
        type: ws.type,
        balanceCny: ws.balanceCny,
        status: ws.status,
        memberCount: ws.projectRelations?.length || 0,
        createdAt: ws.createdAt
      })),
      pagination: {
        page,
        limit,
        total,
        totalPages: Math.ceil(total / limit)
      }
    };
  }

  /**
   * POST /admin/workspaces/:id/recharge
   * 管理员给工作空间充值（赠送）
   */
  @Post('/:id/recharge')
  async adminRecharge(
    @Param('id') workspaceId: string,
    @Body() data: { amountCny: number; reason: string },
    @AuthUser() admin: User
  ) {
    await this.billingService.recharge(workspaceId, data.amountCny, {
      paymentMethod: 'admin_gift',
      paymentId: `admin-${admin.id}-${Date.now()}`
    });

    // 记录操作日志
    await this.auditLogService.log({
      action: 'admin_recharge',
      adminId: admin.id,
      workspaceId,
      metadata: {
        amountCny: data.amountCny,
        reason: data.reason
      }
    });

    return { success: true };
  }

  /**
   * GET /admin/workspaces/:id/usage
   * 查看工作空间使用量
   */
  @Get('/:id/usage')
  async getWorkspaceUsage(
    @Param('id') workspaceId: string,
    @Query('startDate') startDate?: string,
    @Query('endDate') endDate?: string
  ) {
    const where: any = { workspaceId };

    if (startDate && endDate) {
      where.createdAt = Between(new Date(startDate), new Date(endDate));
    }

    const records = await this.usageRecordRepository.find({
      where,
      order: { createdAt: 'DESC' },
      take: 100
    });

    const totalCost = records.reduce((sum, r) => sum + parseFloat(r.costCny.toString()), 0);

    return {
      records,
      totalCost,
      currency: 'CNY'
    };
  }
}
```

#### 6.1.3 系统统计和监控

**AdminStatsController**

```typescript
@RestController('/admin/stats')
@RequireRole('global:admin')
export class AdminStatsController {

  /**
   * GET /admin/stats/overview
   * 系统概览统计
   */
  @Get('/overview')
  async getOverview() {
    const [
      totalUsers,
      totalWorkspaces,
      totalWorkflows,
      activeWorkflows,
      totalRevenue
    ] = await Promise.all([
      this.userRepository.count(),
      this.projectRepository.count(),
      this.workflowRepository.count(),
      this.workflowRepository.count({ where: { active: true } }),
      this.usageRecordRepository
        .createQueryBuilder('ur')
        .select('SUM(ur.costCny)', 'total')
        .getRawOne()
    ]);

    return {
      totalUsers,
      totalWorkspaces,
      totalWorkflows,
      activeWorkflows,
      totalRevenueCny: parseFloat(totalRevenue?.total || '0'),
      currency: 'CNY'
    };
  }

  /**
   * GET /admin/stats/revenue-trend
   * 收入趋势（按月）
   */
  @Get('/revenue-trend')
  async getRevenueTrend(@Query('months') months: number = 6) {
    const result = await this.usageRecordRepository
      .createQueryBuilder('ur')
      .select("DATE_TRUNC('month', ur.created_at)", 'month')
      .addSelect('SUM(ur.cost_cny)', 'revenue')
      .groupBy("DATE_TRUNC('month', ur.created_at)")
      .orderBy("DATE_TRUNC('month', ur.created_at)", 'DESC')
      .limit(months)
      .getRawMany();

    return {
      trend: result.map(r => ({
        month: r.month,
        revenueCny: parseFloat(r.revenue),
        currency: 'CNY'
      }))
    };
  }

  /**
   * GET /admin/stats/popular-services
   * 最受欢迎的服务
   */
  @Get('/popular-services')
  async getPopularServices() {
    const result = await this.usageRecordRepository
      .createQueryBuilder('ur')
      .select('ur.service_key', 'serviceKey')
      .addSelect('ur.service_type', 'serviceType')
      .addSelect('COUNT(*)', 'callCount')
      .addSelect('SUM(ur.cost_cny)', 'totalRevenue')
      .groupBy('ur.service_key, ur.service_type')
      .orderBy('COUNT(*)', 'DESC')
      .limit(10)
      .getRawMany();

    return {
      services: result.map(r => ({
        serviceKey: r.serviceKey,
        serviceType: r.serviceType,
        callCount: parseInt(r.callCount),
        totalRevenueCny: parseFloat(r.totalRevenue),
        currency: 'CNY'
      }))
    };
  }
}
```

#### 6.1.4 插件管理

**插件系统三层架构**

n8n 改造后的插件系统分为三个层次：

1. **🏢 平台插件（Platform-Managed）**
   - 管理员在后台配置
   - 平台提供 API Key 和服务
   - 按量/按次计费
   - 所有用户可见
   - 例如：飞书消息推送、阿里云短信、讯飞语音合成

2. **🔌 第三方插件（Platform-Approved）**
   - 管理员在后台添加
   - 用户需要自己配置 API Key
   - 不计费（用户自己承担成本）
   - 所有用户可见
   - 例如：GitHub、Notion、Slack

3. **🛠️ 自定义插件（User-Uploaded）**
   - 用户自己上传的插件代码
   - 只有自己工作空间可见
   - 可以提交到平台审核
   - 审核通过后变成"第三方插件"

**数据库表扩展**

```sql
-- 扩展 platform_service 表，添加插件支持
ALTER TABLE platform_service ADD COLUMN visibility VARCHAR(50) NOT NULL DEFAULT 'global';
-- 'global': 全局可见（平台插件、第三方插件）
-- 'workspace': 工作空间可见（自定义插件）

ALTER TABLE platform_service ADD COLUMN owner_workspace_id UUID REFERENCES project(id) ON DELETE CASCADE;
-- 如果 visibility = 'workspace'，表示是某个工作空间的自定义插件

ALTER TABLE platform_service ADD COLUMN plugin_code TEXT;
-- 存储插件 TypeScript 代码（仅自定义插件）

ALTER TABLE platform_service ADD COLUMN plugin_version VARCHAR(50);
-- 插件版本号

ALTER TABLE platform_service ADD COLUMN submission_status VARCHAR(50);
-- 'pending': 待审核, 'approved': 已通过, 'rejected': 已拒绝

ALTER TABLE platform_service ADD COLUMN submitted_at TIMESTAMP;
ALTER TABLE platform_service ADD COLUMN reviewed_at TIMESTAMP;
ALTER TABLE platform_service ADD COLUMN reviewed_by UUID REFERENCES "user"(id);
ALTER TABLE platform_service ADD COLUMN review_notes TEXT;

-- 约束：全局插件不能有 owner_workspace_id
ALTER TABLE platform_service ADD CONSTRAINT chk_visibility CHECK (
  (visibility = 'global' AND owner_workspace_id IS NULL) OR
  (visibility = 'workspace' AND owner_workspace_id IS NOT NULL)
);

CREATE INDEX idx_platform_service_visibility ON platform_service(visibility);
CREATE INDEX idx_platform_service_owner ON platform_service(owner_workspace_id);
CREATE INDEX idx_platform_service_submission ON platform_service(submission_status);
```

**AdminPluginsController（管理员插件管理）**

**文件路径：** `packages/cli/src/controllers/admin/plugins.controller.ts`

```typescript
@RestController('/admin/plugins')
@RequireRole('global:admin')
export class AdminPluginsController {

  /**
   * GET /admin/plugins
   * 获取所有插件（包括待审核的用户提交）
   */
  @Get('/')
  async listAllPlugins(
    @Query('visibility') visibility?: 'global' | 'workspace',
    @Query('serviceMode') serviceMode?: string,
    @Query('submissionStatus') submissionStatus?: string
  ) {
    const where: any = { serviceType: 'plugin' };

    if (visibility) where.visibility = visibility;
    if (serviceMode) where.serviceMode = serviceMode;
    if (submissionStatus) where.submissionStatus = submissionStatus;

    const plugins = await this.platformServiceRepository.find({
      where,
      relations: ['ownerWorkspace']
    });

    return plugins;
  }

  /**
   * POST /admin/plugins/platform
   * 创建平台插件（平台提供服务 + 计费）
   */
  @Post('/platform')
  async createPlatformPlugin(@Body() data: {
    serviceKey: string;
    serviceName: string;
    category: string;
    description: string;
    platformConfig: any;  // { apiKey, appId, appSecret, baseUrl, ... }
    pricingModel: 'per_call' | 'per_message' | 'per_character';
    unitPriceCny: number;
    iconUrl?: string;
  }) {
    const encryptedConfig = this.encryptService.encrypt(data.platformConfig);

    return await this.platformServiceRepository.save({
      serviceKey: data.serviceKey,
      serviceName: data.serviceName,
      serviceType: 'plugin',
      serviceMode: 'platform_managed',
      visibility: 'global',
      platformConfig: encryptedConfig,
      pricingModel: data.pricingModel,
      unitPriceCny: data.unitPriceCny,
      category: data.category,
      description: data.description,
      iconUrl: data.iconUrl,
      enabled: true
    });
  }

  /**
   * POST /admin/plugins/third-party
   * 添加第三方插件（用户需要自己配置 API Key）
   */
  @Post('/third-party')
  async addThirdPartyPlugin(@Body() data: {
    serviceKey: string;
    serviceName: string;
    category: string;
    description: string;
    userConfigSchema: object;  // JSON Schema 定义用户需要填什么字段
    iconUrl?: string;
  }) {
    return await this.platformServiceRepository.save({
      serviceKey: data.serviceKey,
      serviceName: data.serviceName,
      serviceType: 'plugin',
      serviceMode: 'user_managed',
      visibility: 'global',
      userConfigSchema: data.userConfigSchema,
      category: data.category,
      description: data.description,
      iconUrl: data.iconUrl,
      enabled: true
    });
  }

  /**
   * GET /admin/plugins/submissions
   * 获取用户提交的插件审核列表
   */
  @Get('/submissions')
  async getPluginSubmissions(@Query('status') status?: string) {
    const where: any = {
      visibility: 'workspace',
      submissionStatus: status || In(['pending', 'approved', 'rejected'])
    };

    return await this.platformServiceRepository.find({
      where,
      relations: ['ownerWorkspace'],
      order: { submittedAt: 'DESC' }
    });
  }

  /**
   * POST /admin/plugins/submissions/:key/review
   * 审核用户提交的插件
   */
  @Post('/submissions/:key/review')
  async reviewPluginSubmission(
    @Param('key') serviceKey: string,
    @Body() data: {
      action: 'approve' | 'reject';
      notes?: string;
    },
    @AuthUser() admin: User
  ) {
    const plugin = await this.platformServiceRepository.findOne({
      where: { serviceKey, submissionStatus: 'pending' }
    });

    if (!plugin) {
      throw new NotFoundError('Plugin submission not found');
    }

    if (data.action === 'approve') {
      // 审核通过：变成全局第三方插件
      plugin.visibility = 'global';
      plugin.submissionStatus = 'approved';
      plugin.ownerWorkspaceId = null;
    } else {
      plugin.submissionStatus = 'rejected';
    }

    plugin.reviewedAt = new Date();
    plugin.reviewedBy = admin.id;
    plugin.reviewNotes = data.notes;

    await this.platformServiceRepository.save(plugin);

    // 通知提交者
    await this.notificationService.notify({
      workspaceId: plugin.ownerWorkspaceId,
      type: 'plugin_review_result',
      data: { pluginName: plugin.serviceName, action: data.action, notes: data.notes }
    });

    return plugin;
  }
}
```

**PluginsController（用户端插件管理）**

**文件路径：** `packages/cli/src/controllers/plugins.controller.ts`

```typescript
@RestController('/plugins')
export class PluginsController {

  /**
   * GET /plugins/available
   * 获取当前工作空间可用的插件列表
   */
  @Get('/available')
  @RequireWorkspaceAccess('viewer')
  async getAvailablePlugins(@WorkspaceContext() ctx: WorkspaceContext) {
    // 1. 全局插件（所有人可见）
    const globalPlugins = await this.platformServiceRepository.find({
      where: { serviceType: 'plugin', visibility: 'global', enabled: true }
    });

    // 2. 当前工作空间的自定义插件
    const workspacePlugins = await this.platformServiceRepository.find({
      where: {
        serviceType: 'plugin',
        visibility: 'workspace',
        ownerWorkspaceId: ctx.workspaceId,
        enabled: true
      }
    });

    const allPlugins = [...globalPlugins, ...workspacePlugins];

    // 3. 获取用户已配置的凭证
    const credentials = await this.workspacePluginCredentialsRepository.find({
      where: { workspaceId: ctx.workspaceId }
    });
    const configuredKeys = new Set(credentials.map(c => c.serviceKey));

    return allPlugins.map(p => ({
      ...p,
      isConfigured: p.serviceMode === 'user_managed' ? configuredKeys.has(p.serviceKey) : true
    }));
  }

  /**
   * POST /plugins/custom
   * 上传自定义插件（仅当前工作空间可见）
   */
  @Post('/custom')
  @RequireWorkspaceAccess('admin')
  async uploadCustomPlugin(
    @WorkspaceContext() ctx: WorkspaceContext,
    @Body() data: {
      serviceKey: string;
      serviceName: string;
      category: string;
      description: string;
      userConfigSchema?: object;
      pluginCode: string;  // TypeScript 代码
      pluginVersion: string;
      iconUrl?: string;
    }
  ) {
    // 验证插件代码安全性
    await this.pluginValidator.validate(data.pluginCode);

    // 检查 serviceKey 是否冲突
    const existing = await this.platformServiceRepository.findOne({
      where: [
        { serviceKey: data.serviceKey, visibility: 'global' },
        { serviceKey: data.serviceKey, ownerWorkspaceId: ctx.workspaceId }
      ]
    });

    if (existing) {
      throw new BadRequestError('Plugin with this key already exists');
    }

    return await this.platformServiceRepository.save({
      serviceKey: data.serviceKey,
      serviceName: data.serviceName,
      serviceType: 'plugin',
      serviceMode: data.userConfigSchema ? 'user_managed' : 'platform_managed',
      visibility: 'workspace',
      ownerWorkspaceId: ctx.workspaceId,
      userConfigSchema: data.userConfigSchema,
      pluginCode: data.pluginCode,
      pluginVersion: data.pluginVersion,
      category: data.category,
      description: data.description,
      iconUrl: data.iconUrl,
      enabled: true
    });
  }

  /**
   * POST /plugins/:key/submit
   * 提交自定义插件到平台审核
   */
  @Post('/:key/submit')
  @RequireWorkspaceAccess('admin')
  async submitPluginForReview(
    @Param('key') serviceKey: string,
    @WorkspaceContext() ctx: WorkspaceContext
  ) {
    const plugin = await this.platformServiceRepository.findOne({
      where: { serviceKey, ownerWorkspaceId: ctx.workspaceId, visibility: 'workspace' }
    });

    if (!plugin) {
      throw new NotFoundError('Plugin not found');
    }

    if (plugin.submissionStatus === 'pending') {
      throw new BadRequestError('Plugin already submitted for review');
    }

    plugin.submissionStatus = 'pending';
    plugin.submittedAt = new Date();
    await this.platformServiceRepository.save(plugin);

    await this.notificationService.notifyAdmins({
      type: 'plugin_submission',
      data: {
        pluginName: plugin.serviceName,
        workspaceName: ctx.workspaceName,
        serviceKey: plugin.serviceKey
      }
    });

    return { success: true };
  }

  /**
   * GET /plugins/custom
   * 获取当前工作空间的自定义插件列表
   */
  @Get('/custom')
  @RequireWorkspaceAccess('viewer')
  async getCustomPlugins(@WorkspaceContext() ctx: WorkspaceContext) {
    return await this.platformServiceRepository.find({
      where: { visibility: 'workspace', ownerWorkspaceId: ctx.workspaceId },
      order: { createdAt: 'DESC' }
    });
  }

  /**
   * DELETE /plugins/custom/:key
   * 删除自定义插件
   */
  @Delete('/custom/:key')
  @RequireWorkspaceAccess('admin')
  async deleteCustomPlugin(
    @Param('key') serviceKey: string,
    @WorkspaceContext() ctx: WorkspaceContext
  ) {
    const plugin = await this.platformServiceRepository.findOne({
      where: { serviceKey, ownerWorkspaceId: ctx.workspaceId, visibility: 'workspace' }
    });

    if (!plugin) {
      throw new NotFoundError('Plugin not found');
    }

    if (plugin.submissionStatus === 'approved') {
      throw new BadRequestError('Cannot delete approved plugin');
    }

    await this.platformServiceRepository.delete({ serviceKey });
    return { success: true };
  }
}
```

**PluginValidator（插件代码验证）**

**文件路径：** `packages/cli/src/services/plugin-validator.service.ts`

```typescript
@Service()
export class PluginValidator {
  async validate(pluginCode: string): Promise<void> {
    // 1. TypeScript 语法检查
    const ts = require('typescript');
    const result = ts.transpileModule(pluginCode, {
      compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2020 }
    });

    if (result.diagnostics?.length) {
      throw new BadRequestError('Plugin code has syntax errors');
    }

    // 2. 安全检查（禁止危险操作）
    const dangerousPatterns = [
      /require\s*\(\s*['"]child_process['"]\s*\)/,  // 禁止执行系统命令
      /require\s*\(\s*['"]fs['"]\s*\)/,              // 禁止文件系统操作
      /eval\s*\(/,                                   // 禁止 eval
      /Function\s*\(/,                               // 禁止动态创建函数
      /process\.exit/,                               // 禁止退出进程
    ];

    for (const pattern of dangerousPatterns) {
      if (pattern.test(pluginCode)) {
        throw new BadRequestError(`Forbidden operation: ${pattern.source}`);
      }
    }

    // 3. 结构检查
    if (!pluginCode.includes('static metadata')) {
      throw new BadRequestError('Plugin must define static metadata');
    }

    if (!pluginCode.includes('async execute')) {
      throw new BadRequestError('Plugin must define async execute method');
    }

    // 4. 依赖检查（白名单）
    const allowedPackages = ['axios', 'lodash', 'date-fns', 'crypto-js', 'uuid'];
    const importPattern = /(?:import|require)\s*\(['"]([\w-/]+)['"]\)/g;
    let match;

    while ((match = importPattern.exec(pluginCode)) !== null) {
      const packageName = match[1].split('/')[0];
      if (!allowedPackages.includes(packageName)) {
        throw new BadRequestError(
          `Package '${packageName}' not allowed. Allowed: ${allowedPackages.join(', ')}`
        );
      }
    }
  }
}
```

**插件对比总结**

| 插件类型 | 可见性 | 配置方式 | 计费 | 添加方式 | 审核 |
|---------|--------|---------|------|---------|------|
| 🏢 平台插件 | 全局 | 管理员后台配置 | ✅ 按量计费 | 管理员添加 | 不需要 |
| 🔌 第三方插件 | 全局 | 用户自己配置 API Key | ❌ 不计费 | 管理员添加 | 不需要 |
| 🛠️ 自定义插件 | 工作空间 | 用户自己配置（可选） | ❌ 不计费 | 用户上传 | 提交后审核 |
| ✅ 审核通过的自定义插件 | 全局 | 用户自己配置（可选） | ❌ 不计费 | 用户提交→审核通过 | 已完成 |

---

### 6.2 后台管理前端页面结构

```
packages/admin-ui/  （新建管理后台前端）
├── src/
│   ├── views/
│   │   ├── Dashboard.vue              // 系统概览
│   │   ├── PlatformServices/
│   │   │   ├── AIModels.vue           // AI 模型管理
│   │   │   ├── RagServices.vue        // RAG 服务管理
│   │   │   └── CreateService.vue      // 创建服务
│   │   ├── Workspaces/
│   │   │   ├── List.vue               // 工作空间列表
│   │   │   ├── Detail.vue             // 工作空间详情
│   │   │   └── Usage.vue              // 使用量查看
│   │   ├── Users/
│   │   │   ├── List.vue               // 用户列表
│   │   │   └── Detail.vue             // 用户详情
│   │   ├── Billing/
│   │   │   ├── Revenue.vue            // 收入统计
│   │   │   ├── UsageRecords.vue       // 使用记录
│   │   │   └── Recharge.vue           // 充值管理
│   │   └── Settings/
│   │       ├── Pricing.vue            // 定价设置
│   │       └── System.vue             // 系统设置
│   ├── components/
│   ├── stores/
│   └── api/
```

---

## 🎨 七、前端层改造

### 7.1 WorkspaceSwitcher（对标 Coze）

**文件路径：** `packages/editor-ui/src/components/WorkspaceSwitcher.vue`

```vue
<template>
  <div class="workspace-switcher">
    <el-dropdown @command="handleSwitch" trigger="click" size="large">
      <div class="current-workspace">
        <div class="workspace-icon">{{ currentWorkspace?.icon?.value || '📁' }}</div>
        <div class="workspace-info">
          <div class="workspace-name">{{ currentWorkspace?.name || '选择工作空间' }}</div>
          <div class="workspace-balance" :class="{ 'low': isLowBalance }">
            余额: ¥{{ currentBalance.toFixed(2) }}
          </div>
        </div>
        <i class="el-icon-arrow-down"></i>
      </div>

      <template #dropdown>
        <el-dropdown-menu>
          <!-- 个人空间 -->
          <el-dropdown-item disabled divided>个人空间</el-dropdown-item>
          <el-dropdown-item
            v-for="ws in personalWorkspaces"
            :key="ws.id"
            :command="ws.id"
            :class="{ 'is-active': ws.id === currentWorkspaceId }"
          >
            <div class="workspace-item">
              <span>{{ ws.icon?.value || '👤' }} {{ ws.name }}</span>
              <span class="balance-tag" :class="{ 'low': ws.balanceCny < 10 }">
                ¥{{ ws.balanceCny.toFixed(2) }}
              </span>
            </div>
          </el-dropdown-item>

          <!-- 团队空间 -->
          <el-dropdown-item disabled divided>团队空间</el-dropdown-item>
          <el-dropdown-item
            v-for="ws in teamWorkspaces"
            :key="ws.id"
            :command="ws.id"
            :class="{ 'is-active': ws.id === currentWorkspaceId }"
          >
            <div class="workspace-item">
              <span>{{ ws.icon?.value || '🏢' }} {{ ws.name }}</span>
              <span class="balance-tag" :class="{ 'low': ws.balanceCny < 10 }">
                ¥{{ ws.balanceCny.toFixed(2) }}
              </span>
            </div>
          </el-dropdown-item>

          <el-dropdown-item divided @click="createWorkspace">
            <i class="el-icon-plus"></i> 创建团队空间
          </el-dropdown-item>
        </el-dropdown-menu>
      </template>
    </el-dropdown>
  </div>
</template>

<script setup lang="ts">
import { computed } from 'vue';
import { storeToRefs } from 'pinia';
import { useProjectsStore } from '@/stores/projects.store';
import { useRouter } from 'vue-router';

const projectsStore = useProjectsStore();
const router = useRouter();

const { currentWorkspace, currentWorkspaceId, personalWorkspaces, teamWorkspaces } = storeToRefs(projectsStore);

const currentBalance = computed(() => currentWorkspace.value?.balanceCny || 0);
const isLowBalance = computed(() => currentBalance.value < (currentWorkspace.value?.lowBalanceThreshold || 100));

function handleSwitch(workspaceId: string) {
  projectsStore.setActiveWorkspace(workspaceId);
}

function createWorkspace() {
  router.push('/workspaces/new');
}
</script>

<style scoped>
.current-workspace {
  display: flex;
  align-items: center;
  gap: var(--spacing--xs);
  padding: var(--spacing--xs) var(--spacing--sm);
  cursor: pointer;
  border-radius: var(--radius);
  transition: background-color 0.2s;
}

.workspace-balance {
  font-size: var(--font-size--2xs);
  color: var(--color--text--tint-2);
}

.workspace-balance.low {
  color: var(--color--danger);
  font-weight: var(--font-weight--bold);
}

.balance-tag {
  margin-left: auto;
  padding: 2px var(--spacing--3xs);
  background: var(--color--success--tint-4);
  color: var(--color--success--shade-1);
  border-radius: var(--radius--sm);
  font-size: var(--font-size--3xs);
}

.balance-tag.low {
  background: var(--color--danger--tint-3);
  color: var(--color--danger);
}
</style>
```

### 7.2 UsageView（账单中心）

**文件路径：** `packages/editor-ui/src/views/UsageView.vue`

```vue
<template>
  <div class="usage-view">
    <!-- 余额卡片 -->
    <el-card class="balance-card">
      <template #header>
        <div class="card-header">
          <h2>账户余额</h2>
          <el-button type="primary" @click="showRechargeDialog = true">
            充值
          </el-button>
        </div>
      </template>

      <div class="balance-display">
        <div class="balance-amount" :class="{ 'low': isLowBalance }">
          ¥{{ currentBalance.toFixed(2) }}
        </div>
        <div class="balance-info">
          <span>币种: 人民币 (CNY)</span>
          <span>低余额警告阈值: ¥{{ lowBalanceThreshold.toFixed(2) }}</span>
        </div>
      </div>

      <el-alert
        v-if="isLowBalance"
        type="warning"
        title="余额不足，请及时充值以避免服务中断"
        :closable="false"
        show-icon
      />
    </el-card>

    <!-- 月度使用统计 -->
    <el-card class="usage-card">
      <template #header>
        <div class="card-header">
          <h2>使用统计</h2>
          <el-date-picker
            v-model="selectedMonth"
            type="month"
            placeholder="选择月份"
            format="YYYY-MM"
            @change="fetchMonthlyUsage"
          />
        </div>
      </template>

      <el-table :data="monthlyUsage" style="width: 100%" :default-sort="{ prop: 'totalCostCny', order: 'descending' }">
        <el-table-column prop="serviceKey" label="服务" width="200" />
        <el-table-column prop="serviceType" label="类型" width="120">
          <template #default="{ row }">
            <el-tag :type="getServiceTypeColor(row.serviceType)">
              {{ getServiceTypeName(row.serviceType) }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="totalTokens" label="Tokens 使用" width="150">
          <template #default="{ row }">
            {{ row.totalTokens ? row.totalTokens.toLocaleString() : '-' }}
          </template>
        </el-table-column>
        <el-table-column prop="totalCalls" label="调用次数" width="120">
          <template #default="{ row }">
            {{ row.totalCalls || '-' }}
          </template>
        </el-table-column>
        <el-table-column prop="totalCostCny" label="费用（¥）" sortable>
          <template #default="{ row }">
            ¥{{ row.totalCostCny.toFixed(2) }}
          </template>
        </el-table-column>
      </el-table>

      <div class="total-summary">
        <span class="summary-label">本月总费用:</span>
        <span class="summary-value">¥{{ totalMonthCost.toFixed(2) }}</span>
      </div>
    </el-card>

    <!-- 充值对话框 -->
    <el-dialog v-model="showRechargeDialog" title="账户充值" width="500px">
      <el-form :model="rechargeForm" label-width="100px">
        <el-form-item label="充值金额">
          <el-input-number
            v-model="rechargeForm.amount"
            :min="10"
            :step="10"
            :precision="2"
          />
          <span class="input-suffix">元</span>
        </el-form-item>

        <el-form-item label="支付方式">
          <el-radio-group v-model="rechargeForm.paymentMethod">
            <el-radio label="alipay">
              <img src="@/assets/alipay-icon.png" class="payment-icon" />
              支付宝
            </el-radio>
            <el-radio label="wechat">
              <img src="@/assets/wechat-icon.png" class="payment-icon" />
              微信支付
            </el-radio>
          </el-radio-group>
        </el-form-item>

        <el-form-item>
          <el-alert
            type="info"
            title="充值后余额将立即到账，可用于平台所有付费服务"
            :closable="false"
            show-icon
          />
        </el-form-item>
      </el-form>

      <template #footer>
        <el-button @click="showRechargeDialog = false">取消</el-button>
        <el-button type="primary" @click="handleRecharge" :loading="recharging">
          立即支付 ¥{{ rechargeForm.amount.toFixed(2) }}
        </el-button>
      </template>
    </el-dialog>
  </div>
</template>

<script setup lang="ts">
import { ref, computed, onMounted } from 'vue';
import { storeToRefs } from 'pinia';
import { useProjectsStore } from '@/stores/projects.store';
import { usageApi } from '@/api/usage';

const projectsStore = useProjectsStore();
const { currentWorkspace, currentWorkspaceId } = storeToRefs(projectsStore);

const currentBalance = computed(() => currentWorkspace.value?.balanceCny || 0);
const lowBalanceThreshold = computed(() => currentWorkspace.value?.lowBalanceThreshold || 100);
const isLowBalance = computed(() => currentBalance.value <= lowBalanceThreshold.value);

const selectedMonth = ref(new Date());
const monthlyUsage = ref([]);
const totalMonthCost = ref(0);

const showRechargeDialog = ref(false);
const recharging = ref(false);
const rechargeForm = ref({
  amount: 100,
  paymentMethod: 'alipay'
});

onMounted(() => {
  fetchMonthlyUsage();
});

async function fetchMonthlyUsage() {
  const month = selectedMonth.value.toISOString().slice(0, 7);
  const result = await usageApi.getMonthlyBill(currentWorkspaceId.value, month);

  monthlyUsage.value = result.items;
  totalMonthCost.value = result.totalCostCny;
}

async function handleRecharge() {
  recharging.value = true;

  try {
    // 调用充值 API（这里应该跳转到支付页面）
    const paymentUrl = await usageApi.createRecharge(
      currentWorkspaceId.value,
      rechargeForm.value.amount,
      rechargeForm.value.paymentMethod
    );

    // 跳转到支付页面
    window.open(paymentUrl, '_blank');

    showRechargeDialog.value = false;
  } finally {
    recharging.value = false;
  }
}

function getServiceTypeColor(type: string) {
  const colors = {
    ai_model: 'primary',
    rag_service: 'success',
    embeddings: 'info',
    tts: 'warning',
    stt: 'warning',
    image_gen: 'danger'
  };
  return colors[type] || '';
}

function getServiceTypeName(type: string) {
  const names = {
    ai_model: 'AI 模型',
    rag_service: 'RAG 服务',
    embeddings: '向量化',
    tts: '文字转语音',
    stt: '语音转文字',
    image_gen: '图片生成'
  };
  return names[type] || type;
}
</script>

<style scoped>
.usage-view {
  padding: var(--spacing--lg);
  max-width: 1200px;
  margin: 0 auto;
}

.balance-card {
  margin-bottom: var(--spacing--lg);
}

.card-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
}

.balance-display {
  text-align: center;
  padding: var(--spacing--xl) 0;
}

.balance-amount {
  font-size: var(--font-size--2xl);
  font-weight: var(--font-weight--bold);
  color: var(--color--success);
  margin-bottom: var(--spacing--xs);
}

.balance-amount.low {
  color: var(--color--danger);
}

.total-summary {
  display: flex;
  justify-content: flex-end;
  align-items: center;
  margin-top: var(--spacing--md);
  padding: var(--spacing--sm);
  background: var(--color--background--light-2);
  border-radius: var(--radius);
}

.summary-value {
  font-size: var(--font-size--xl);
  font-weight: var(--font-weight--bold);
  color: var(--color--primary);
  margin-left: var(--spacing--sm);
}

.payment-icon {
  width: 24px;
  height: 24px;
  margin-right: var(--spacing--3xs);
  vertical-align: middle;
}
</style>
```

### 7.3 RegisterView（用户注册页面）

**文件路径：** `packages/editor-ui/src/views/auth/RegisterView.vue`

```vue
<template>
  <div class="register-view">
    <el-card class="register-card">
      <template #header>
        <h2>注册 n8n 账户</h2>
        <p class="subtitle">注册即送 ¥10，立即开始自动化之旅</p>
      </template>

      <el-form :model="form" :rules="rules" ref="formRef" label-position="top">
        <el-form-item label="邮箱" prop="email">
          <el-input v-model="form.email" placeholder="your@email.com" />
        </el-form-item>

        <el-form-item label="密码" prop="password">
          <el-input v-model="form.password" type="password" placeholder="至少 8 位" show-password />
        </el-form-item>

        <el-form-item label="姓名" prop="firstName">
          <el-input v-model="form.firstName" placeholder="您的名字" />
        </el-form-item>

        <el-form-item label="邀请码（可选）" prop="inviteCode">
          <el-input v-model="form.inviteCode" placeholder="有邀请码？双方各得 ¥5" />
        </el-form-item>

        <el-form-item>
          <el-checkbox v-model="form.agreedToTerms">
            我已阅读并同意 <a href="/terms" target="_blank">服务条款</a> 和 <a href="/privacy" target="_blank">隐私政策</a>
          </el-checkbox>
        </el-form-item>

        <el-button
          type="primary"
          size="large"
          :loading="loading"
          :disabled="!form.agreedToTerms"
          @click="handleRegister"
          block
        >
          免费注册
        </el-button>
      </el-form>

      <div class="oauth-divider"><span>或使用以下方式注册</span></div>

      <div class="oauth-buttons">
        <el-button @click="oauthLogin('wechat')">微信登录</el-button>
        <el-button @click="oauthLogin('github')">GitHub 登录</el-button>
      </div>

      <div class="login-link">已有账户？<router-link to="/login">立即登录</router-link></div>
    </el-card>

    <!-- 注册成功引导 -->
    <el-dialog v-model="showSuccessDialog" title="🎉 注册成功！" width="500px">
      <div class="success-content">
        <p>恭喜您成功注册 n8n 账户！</p>
        <ul>
          <li>✅ 已为您创建个人工作空间</li>
          <li>✅ 赠送 ¥{{ initialBonus }} 余额</li>
          <li>✅ 完成新手任务可再获得 ¥15</li>
        </ul>
        <el-alert type="warning" title="请验证邮箱以享受完整功能" :closable="false" show-icon />
      </div>
      <template #footer>
        <el-button type="primary" @click="goToOnboarding">开始使用</el-button>
      </template>
    </el-dialog>
  </div>
</template>

<script setup lang="ts">
import { ref } from 'vue';
import { useRouter } from 'vue-router';
import { authApi } from '@/api/auth';

const router = useRouter();
const formRef = ref();
const loading = ref(false);
const showSuccessDialog = ref(false);
const initialBonus = ref(10);

const form = ref({
  email: '',
  password: '',
  firstName: '',
  inviteCode: '',
  agreedToTerms: false
});

const rules = {
  email: [{ required: true, type: 'email', message: '请输入有效的邮箱' }],
  password: [{ required: true, min: 8, message: '密码至少 8 位' }],
  firstName: [{ required: true, message: '请输入姓名' }]
};

async function handleRegister() {
  const valid = await formRef.value.validate();
  if (!valid) return;

  loading.value = true;
  try {
    const result = await authApi.register(form.value);
    localStorage.setItem('token', result.token);
    if (form.value.inviteCode) initialBonus.value = 15;
    showSuccessDialog.value = true;
  } catch (error) {
    ElMessage.error(error.message || '注册失败');
  } finally {
    loading.value = false;
  }
}

function goToOnboarding() {
  router.push('/onboarding');
}

function oauthLogin(provider: 'wechat' | 'github') {
  window.location.href = `/api/auth/oauth/${provider}`;
}
</script>

<style scoped>
.register-view {
  display: flex;
  justify-content: center;
  align-items: center;
  min-height: 100vh;
  background: var(--color--background--light-2);
}

.register-card {
  width: 100%;
  max-width: 500px;
}

.subtitle {
  color: var(--color--text--tint-2);
  margin-top: var(--spacing--xs);
}

.oauth-divider {
  text-align: center;
  margin: var(--spacing--lg) 0;
  color: var(--color--text--tint-2);
}

.oauth-buttons {
  display: flex;
  gap: var(--spacing--sm);
}

.login-link {
  text-align: center;
  margin-top: var(--spacing--md);
  color: var(--color--text--tint-2);
}
</style>
```

---

## 📅 八、激进实施计划（17-18周）

### Week 1-2: 数据库层改造
**目标：** 完成数据库结构变更

- [ ] 执行 SQL 迁移脚本
- [ ] 扩展 User 表（邮箱验证、OAuth、Onboarding 等）
- [ ] 添加 execution_entity 和 variables_entity 的 project_id
- [ ] 直接删除 SharedWorkflow/SharedCredentials 表
- [ ] 创建计费系统表
- [ ] 创建平台 RAG 服务表
- [ ] 创建用户注册系统表（邀请码、审计日志、配额、注册统计）
- [ ] 数据完整性测试

**交付物：**
- ✅ 数据库结构符合设计
- ✅ 所有索引创建完成
- ✅ 示例数据插入成功

### Week 3-5: Repository + Service 层
**目标：** 核心业务逻辑实现

- [ ] 实现 UserService（完整注册流程）
- [ ] 实现 EmailVerificationService
- [ ] 实现 OnboardingService
- [ ] 重写 WorkflowRepository（删除 SharedWorkflow 引用）
- [ ] 重写 CredentialsRepository
- [ ] 实现 BillingService
- [ ] 实现 PlatformRagService
- [ ] 实现 VectorStoreService
- [ ] 单元测试（覆盖率 > 80%）

**交付物：**
- ✅ Repository 查询正确
- ✅ Service 业务逻辑完整
- ✅ 单元测试通过

### Week 6-8: API 层
**目标：** REST API 实现

- [ ] AuthController（注册、登录、邮箱验证）
- [ ] OAuthController（微信、GitHub 登录）
- [ ] OnboardingController
- [ ] WorkspacesController
- [ ] RagController
- [ ] AdminPlatformServicesController
- [ ] AdminWorkspacesController
- [ ] AdminStatsController
- [ ] API 文档更新
- [ ] 集成测试

**交付物：**
- ✅ API 接口可调用
- ✅ Swagger 文档完整
- ✅ 集成测试通过

### Week 9-12: 前端层
**目标：** 用户界面实现

- [ ] RegisterView（用户注册页面）
- [ ] OnboardingView（新手引导页面）
- [ ] EmailVerificationView（邮箱验证页面）
- [ ] WorkspaceSwitcher 组件
- [ ] UsageView（账单中心）
- [ ] WorkflowsView 重构（工作空间隔离）
- [ ] CredentialsView 重构（工作空间隔离）
- [ ] RagServicesView（平台 RAG 服务列表）
- [ ] 工作空间切换交互优化
- [ ] 路由守卫（权限验证）

**交付物：**
- ✅ 工作空间切换流畅
- ✅ 余额显示正确
- ✅ 账单数据展示完整

### Week 13-14: 后台管理系统
**目标：** 管理后台实现

- [ ] 管理后台前端框架搭建
- [ ] 平台服务管理页面
- [ ] 工作空间管理页面
- [ ] 用户管理页面
- [ ] 统计报表页面
- [ ] 权限控制

**交付物：**
- ✅ 管理后台可用
- ✅ 数据可视化清晰
- ✅ 操作审计日志完整

### Week 15-16: 测试 + 上线
**目标：** 全流程测试和部署

- [ ] 全流程 E2E 测试
- [ ] 性能测试（查询优化验证）
- [ ] 安全测试（权限、SQL注入等）
- [ ] 压力测试（并发场景）
- [ ] 灰度发布准备
- [ ] 监控告警配置

**交付物：**
- ✅ 测试报告
- ✅ 性能基准达标
- ✅ 生产环境部署就绪

---

## ⚠️ 九、风险控制

### 9.1 技术风险

| 风险 | 影响 | 应对措施 |
|------|------|----------|
| 删除 SharedWorkflow 影响未知代码 | 高 | 全局搜索 + 代码审查 + 充分测试 |
| 并发扣费导致余额透支 | 中 | 使用数据库事务 + 悲观锁 |
| RAG 服务响应慢 | 中 | 向量数据库优化 + 缓存 + 异步处理 |
| 支付集成复杂 | 低 | 先实现手动充值，支付后期迭代 |

### 9.2 业务风险

| 风险 | 影响 | 应对措施 |
|------|------|----------|
| 计费错误导致纠纷 | 高 | 详细的使用记录 + 可追溯 + 测试充分 |
| 余额不足影响用户体验 | 中 | 低余额警告 + 自动通知 |
| 平台 RAG 服务质量差 | 中 | 充分测试 + 用户反馈渠道 |

---

## ✅ 十、成功标准

### 10.1 功能完整性

- ✅ 用户注册后自动创建个人空间
- ✅ 可创建团队空间并邀请成员
- ✅ 工作空间切换流畅（< 500ms）
- ✅ 权限控制正确（Admin/Editor/Viewer）
- ✅ 数据完全隔离（跨空间不可见）
- ✅ AI 服务按量计费准确
- ✅ 平台 RAG 服务可用
- ✅ 余额管理功能完整
- ✅ 后台管理系统可用

### 10.2 性能指标

- ✅ 工作流列表查询 < 100ms（vs 原来 150ms，提升 33%）
- ✅ 工作空间切换 < 500ms
- ✅ 余额扣费 < 50ms
- ✅ RAG 查询响应 < 2s
- ✅ 并发 100 用户无压力

### 10.3 安全性

- ✅ 跨工作空间数据完全隔离
- ✅ 权限绕过漏洞检查通过
- ✅ API Key 加密存储
- ✅ SQL 注入检查通过
- ✅ XSS 攻击防护

---

## 📚 十一、附录

### 11.1 关键文件路径清单

**数据库层：**
```
packages/@n8n/db/src/
├── entities/
│   ├── project.ts （扩展）
│   ├── workflow-entity.ts （添加 projectId）
│   ├── credentials-entity.ts （添加 projectId）
│   ├── platform-service.ts （新增）
│   ├── platform-rag-service.ts （新增）
│   ├── usage-record.ts （新增）
│   └── monthly-usage-summary.ts （新增）
├── repositories/
│   ├── workflow.repository.ts （重写）
│   ├── credentials.repository.ts （重写）
│   ├── platform-service.repository.ts （新增）
│   └── platform-rag-service.repository.ts （新增）
└── migrations/
    └── 1736236800000-AggressiveMultiTenantRefactor.ts
```

**后端层：**
```
packages/cli/src/
├── services/
│   ├── billing.service.ts （新增）
│   ├── platform-rag.service.ts （新增）
│   ├── vector-store.service.ts （新增）
│   ├── project.service.ee.ts （简化）
│   └── workflow.service.ts （简化）
├── controllers/
│   ├── workspaces.controller.ts （简化）
│   ├── rag.controller.ts （新增）
│   └── admin/
│       ├── platform-services.controller.ts （新增）
│       ├── workspaces.controller.ts （新增）
│       └── stats.controller.ts （新增）
└── middlewares/
    └── workspace-context.middleware.ts （简化）
```

**前端层：**
```
packages/editor-ui/src/
├── stores/
│   └── projects.store.ts （扩展）
├── components/
│   └── WorkspaceSwitcher.vue （对标 Coze）
└── views/
    ├── UsageView.vue （新增）
    ├── RagServicesView.vue （新增）
    └── WorkflowsView.vue （监听工作空间切换）
```

**管理后台：**
```
packages/admin-ui/  （全新）
├── src/
│   ├── views/
│   │   ├── Dashboard.vue
│   │   ├── PlatformServices/
│   │   ├── Workspaces/
│   │   ├── Users/
│   │   ├── Billing/
│   │   └── Settings/
│   ├── components/
│   ├── stores/
│   └── api/
```

### 11.2 术语对照表

| Coze 术语 | n8n 改造后 | 数据库表/字段 |
|----------|-----------|-------------|
| Workspace | Project | project |
| Personal Workspace | Project (type='personal') | project.type |
| Team Workspace | Project (type='team') | project.type |
| Bot | Workflow | workflow_entity |
| Knowledge Base | 不做平台提供 | - |
| Plugin | 平台 RAG 服务 | platform_rag_services |
| Member | ProjectRelation | project_relation |
| Admin/Editor/Viewer | role | project_relation.role |
| Balance | balanceCny | project.balance_cny |
| Usage | usageRecords | usage_records |

---

## 🎯 十二、企业版功能管理方案

### 12.1 现状说明

当前单租户版本已解锁所有 n8n 企业版功能（详见 `/docs/FEATURES.md`），包括：
- ✅ LDAP/SAML/OIDC 单点登录
- ✅ 多因素认证 (MFA)
- ✅ 工作流版本历史（无限制）
- ✅ 外部密钥管理
- ✅ 日志流 (Log Streaming)
- ✅ Insights 分析（无日期限制）
- ✅ Worker View
- ✅ 环境变量管理（无限制）
- ✅ Public API（完全开放）
- ✅ 审计日志
- ✅ 自定义角色

**改造为多租户后的核心问题：** 这些功能应该如何管理？是否需要限制？由谁来控制？

### 12.2 功能管理分层设计

#### 分层原则

多租户 SaaS 的功能管理应该分为 **三个层级**：

```
┌─────────────────────────────────────────────┐
│ 层级1: 平台管理员（全局控制）              │
│ └─ 影响整个平台、消耗平台资源的功能        │
├─────────────────────────────────────────────┤
│ 层级2: 工作空间管理员（团队控制）          │
│ └─ 工作空间级配置、团队协作功能            │
├─────────────────────────────────────────────┤
│ 层级3: 普通用户（个人控制）                │
│ └─ 个人偏好设置                            │
└─────────────────────────────────────────────┘
```

### 12.3 功能分类与管理策略

#### 12.3.1 层级1：平台管理员控制

这些功能影响**整个平台**或需要**平台资源**，必须由平台管理员统一控制：

| 功能 | 多租户后处理 | 管理方式 | 理由 |
|------|------------|---------|------|
| **LDAP/SAML/OIDC** | ❌ 全局禁用 | 后台管理系统开关 | 小团队 SaaS 用不上，增加复杂度 |
| **外部密钥管理** | ❌ 全局禁用 | 后台管理系统开关 | 不适合多租户 SaaS |
| **日志流** | ❌ 全局禁用 | 后台管理系统开关 | 平台自己的运维功能 |
| **Worker View** | ⚠️ 仅管理员可见 | 后台管理系统独占 | 平台运维监控 |
| **Public API 限流** | ✅ 平台配置 | 后台管理系统配置 | 防止滥用，保护平台 |
| **工作流版本历史** | ✅ 保留（无限制） | 后台管理系统配置保留策略 | 核心功能，但可配置清理策略 |
| **Insights 分析** | ✅ 保留（无限制） | 无需控制 | 查看历史不产生成本 |

**实现方式：**

```sql
-- 平台级功能开关配置表
CREATE TABLE platform_feature_config (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  feature_key VARCHAR(100) UNIQUE NOT NULL,  -- 'ldap', 'saml', 'log_streaming', 'public_api'

  -- 全局开关
  enabled BOOLEAN DEFAULT FALSE,

  -- 功能配置（JSON）
  config JSONB DEFAULT '{}',
  -- 例如: {
  --   "public_api": {
  --     "rate_limit_per_hour": 1000,
  --     "max_api_keys_per_workspace": 5
  --   },
  --   "workflow_history": {
  --     "retention_days": 90,  -- -1 表示无限制
  --     "auto_cleanup_enabled": true
  --   }
  -- }

  description TEXT,
  updated_at TIMESTAMP DEFAULT NOW()
);

-- 初始化数据
INSERT INTO platform_feature_config (feature_key, enabled, config, description) VALUES
('ldap', FALSE, '{}', 'LDAP 单点登录（多租户 SaaS 禁用）'),
('saml', FALSE, '{}', 'SAML 单点登录（多租户 SaaS 禁用）'),
('oidc', FALSE, '{}', 'OIDC 单点登录（多租户 SaaS 禁用）'),
('external_secrets', FALSE, '{}', '外部密钥管理（多租户 SaaS 禁用）'),
('log_streaming', FALSE, '{}', '日志流（仅平台使用）'),
('worker_view', FALSE, '{}', 'Worker 监控（仅管理员）'),
('public_api', TRUE, '{"rate_limit_per_hour": 1000, "max_api_keys": 5}', 'Public API（启用 + 限流）'),
('workflow_history', TRUE, '{"retention_days": -1}', '工作流版本历史（无限制）'),
('insights', TRUE, '{"date_range_limit_days": -1}', 'Insights 分析（无限制）'),
('audit_logs', TRUE, '{"retention_days": 365}', '审计日志（保留1年）');
```

**后台管理系统 API：**

```typescript
// packages/cli/src/controllers/admin/platform-features.controller.ts
@RestController('/admin/platform-features')
@RequireRole('global:admin')
export class AdminPlatformFeaturesController {

  constructor(
    private readonly platformFeatureConfigRepository: PlatformFeatureConfigRepository,
    private readonly cacheService: CacheService
  ) {}

  /**
   * GET /admin/platform-features
   * 获取所有功能配置
   */
  @Get('/')
  async listFeatures() {
    const features = await this.platformFeatureConfigRepository.find({
      order: { featureKey: 'ASC' }
    });

    return {
      features: features.map(f => ({
        featureKey: f.featureKey,
        enabled: f.enabled,
        config: f.config,
        description: f.description,
        updatedAt: f.updatedAt
      }))
    };
  }

  /**
   * PATCH /admin/platform-features/:key
   * 更新功能配置
   */
  @Patch('/:key')
  async updateFeature(
    @Param('key') featureKey: string,
    @Body() data: {
      enabled?: boolean;
      config?: any;
    },
    @AuthUser() admin: User
  ) {
    const feature = await this.platformFeatureConfigRepository.findOne({
      where: { featureKey }
    });

    if (!feature) {
      throw new NotFoundError(`Feature ${featureKey} not found`);
    }

    // 更新配置
    if (typeof data.enabled === 'boolean') {
      feature.enabled = data.enabled;
    }
    if (data.config) {
      feature.config = { ...feature.config, ...data.config };
    }
    feature.updatedAt = new Date();

    await this.platformFeatureConfigRepository.save(feature);

    // 清除缓存
    await this.cacheService.delete(`platform:feature:${featureKey}`);

    // 记录操作日志
    await this.auditLogService.log({
      action: 'platform_feature_updated',
      userId: admin.id,
      metadata: {
        featureKey,
        enabled: feature.enabled,
        config: feature.config
      }
    });

    return { success: true, feature };
  }

  /**
   * GET /admin/platform-features/:key/usage
   * 获取功能使用统计
   */
  @Get('/:key/usage')
  async getFeatureUsage(
    @Param('key') featureKey: string,
    @Query('startDate') startDate?: string,
    @Query('endDate') endDate?: string
  ) {
    // 例如：统计 Public API 的调用量
    if (featureKey === 'public_api') {
      const stats = await this.usageRecordRepository
        .createQueryBuilder('ur')
        .select('ur.workspace_id', 'workspaceId')
        .addSelect('COUNT(*)', 'apiCalls')
        .where('ur.service_type = :type', { type: 'public_api' })
        .andWhere('ur.created_at BETWEEN :start AND :end', {
          start: startDate || new Date(Date.now() - 30 * 86400000),
          end: endDate || new Date()
        })
        .groupBy('ur.workspace_id')
        .orderBy('COUNT(*)', 'DESC')
        .limit(100)
        .getRawMany();

      return { stats };
    }

    return { message: 'Usage stats not available for this feature' };
  }
}
```

**后台管理前端界面：**

```vue
<!-- packages/admin-ui/src/views/PlatformFeatures/FeaturesList.vue -->
<template>
  <div class="platform-features-page">
    <el-card>
      <template #header>
        <h2>平台功能管理</h2>
      </template>

      <el-table :data="features" style="width: 100%">
        <el-table-column prop="featureKey" label="功能标识" width="200" />
        <el-table-column prop="description" label="说明" />

        <el-table-column label="状态" width="120">
          <template #default="{ row }">
            <el-switch
              v-model="row.enabled"
              @change="handleToggle(row)"
              :disabled="isDisabledFeature(row.featureKey)"
            />
          </template>
        </el-table-column>

        <el-table-column label="配置" width="150">
          <template #default="{ row }">
            <el-button
              size="small"
              @click="showConfigDialog(row)"
              :disabled="!row.enabled"
            >
              配置
            </el-button>
          </template>
        </el-table-column>

        <el-table-column label="使用统计" width="150">
          <template #default="{ row }">
            <el-button
              size="small"
              @click="showUsageStats(row)"
              v-if="hasUsageStats(row.featureKey)"
            >
              查看
            </el-button>
            <span v-else>-</span>
          </template>
        </el-table-column>
      </el-table>

      <div class="features-summary">
        <el-alert type="info" :closable="false">
          <template #title>
            <strong>说明：</strong>
            <ul>
              <li>❌ 已禁用功能：LDAP/SAML/OIDC（小团队 SaaS 用不上）</li>
              <li>❌ 已禁用功能：外部密钥管理、日志流（不适合多租户）</li>
              <li>✅ 保留功能：工作流版本历史、Insights 分析（无限制）</li>
              <li>⚠️ API 限流：防止滥用，默认 1000次/小时/工作空间</li>
            </ul>
          </template>
        </el-alert>
      </div>
    </el-card>

    <!-- 配置对话框 -->
    <el-dialog v-model="showConfig" :title="`配置 ${currentFeature?.description}`" width="600px">
      <el-form v-if="currentFeature" label-width="150px">
        <!-- Public API 配置 -->
        <template v-if="currentFeature.featureKey === 'public_api'">
          <el-form-item label="限流（次/小时）">
            <el-input-number v-model="configForm.rate_limit_per_hour" :min="100" :step="100" />
          </el-form-item>
          <el-form-item label="最大密钥数">
            <el-input-number v-model="configForm.max_api_keys" :min="1" :max="20" />
          </el-form-item>
        </template>

        <!-- 工作流版本历史配置 -->
        <template v-if="currentFeature.featureKey === 'workflow_history'">
          <el-form-item label="保留天数">
            <el-input-number
              v-model="configForm.retention_days"
              :min="-1"
              :step="30"
            />
            <span class="tip">-1 表示无限制</span>
          </el-form-item>
          <el-form-item label="自动清理">
            <el-switch v-model="configForm.auto_cleanup_enabled" />
          </el-form-item>
        </template>
      </el-form>

      <template #footer>
        <el-button @click="showConfig = false">取消</el-button>
        <el-button type="primary" @click="saveConfig">保存</el-button>
      </template>
    </el-dialog>
  </div>
</template>

<script setup lang="ts">
import { ref, onMounted } from 'vue';
import { adminApi } from '@/api/admin';
import { ElMessage } from 'element-plus';

const features = ref([]);
const showConfig = ref(false);
const currentFeature = ref(null);
const configForm = ref({});

onMounted(async () => {
  await fetchFeatures();
});

async function fetchFeatures() {
  const res = await adminApi.getPlatformFeatures();
  features.value = res.features;
}

async function handleToggle(feature: any) {
  try {
    await adminApi.updatePlatformFeature(feature.featureKey, {
      enabled: feature.enabled
    });
    ElMessage.success(`功能 ${feature.description} 已${feature.enabled ? '启用' : '禁用'}`);
  } catch (error) {
    ElMessage.error('操作失败');
    feature.enabled = !feature.enabled; // 回滚
  }
}

function showConfigDialog(feature: any) {
  currentFeature.value = feature;
  configForm.value = { ...feature.config };
  showConfig.value = true;
}

async function saveConfig() {
  try {
    await adminApi.updatePlatformFeature(currentFeature.value.featureKey, {
      config: configForm.value
    });
    ElMessage.success('配置已保存');
    showConfig.value = false;
    await fetchFeatures();
  } catch (error) {
    ElMessage.error('保存失败');
  }
}

function isDisabledFeature(key: string) {
  // 某些功能禁止启用
  return ['ldap', 'saml', 'oidc', 'external_secrets', 'log_streaming'].includes(key);
}

function hasUsageStats(key: string) {
  return ['public_api'].includes(key);
}
</script>

<style scoped>
.platform-features-page {
  padding: var(--spacing--lg);
}

.features-summary {
  margin-top: var(--spacing--lg);
}

.features-summary ul {
  margin: var(--spacing--xs) 0 0 var(--spacing--md);
  padding: 0;
}

.features-summary li {
  margin: var(--spacing--3xs) 0;
}

.tip {
  margin-left: var(--spacing--xs);
  font-size: var(--font-size--2xs);
  color: var(--color--text--tint-2);
}
</style>
```

---

#### 12.3.2 层级2：工作空间管理员控制

这些功能是**工作空间级别**的配置，应该让工作空间的 Admin 自己管理：

| 功能 | 多租户后处理 | 管理方式 | 理由 |
|------|------------|---------|------|
| **自定义角色** | ✅ 保留 | 工作空间设置页面 | 团队自己定义角色体系 |
| **环境变量** | ✅ 保留（工作空间隔离） | 工作空间设置页面 | 每个团队配置不同 |
| **审计日志** | ✅ 保留（工作空间隔离） | 工作空间设置页面 | 查看本团队操作记录 |
| **工作流分享权限** | ✅ 保留（重新实现） | 工作流编辑器 | 团队内部协作 |
| **API 密钥管理** | ✅ 保留（限制数量） | 工作空间设置页面 | 每个团队管理自己的API密钥 |

**实现方式：**

```sql
-- 工作空间功能配置（扩展 project 表）
ALTER TABLE project ADD COLUMN feature_config JSONB DEFAULT '{}';

-- 例如存储：
-- {
--   "custom_roles": {
--     "enabled": true,
--     "roles": [
--       { "name": "高级编辑者", "permissions": ["workflow:read", "workflow:write", "workflow:execute"] },
--       { "name": "审批者", "permissions": ["workflow:read", "workflow:approve"] }
--     ]
--   },
--   "api_keys": {
--     "max_count": 5,
--     "current_count": 2
--   }
-- }
```

**工作空间设置 API：**

```typescript
// packages/cli/src/controllers/workspaces.controller.ts
@RestController('/workspaces')
export class WorkspacesController {

  /**
   * GET /workspaces/:id/features
   * 获取工作空间功能配置
   */
  @Get('/:id/features')
  async getWorkspaceFeatures(
    @Param('id') workspaceId: string,
    @AuthUser() user: User
  ) {
    await this.validateAccess(workspaceId, user.id);

    const workspace = await this.projectRepository.findOne({
      where: { id: workspaceId },
      select: ['featureConfig']
    });

    return {
      features: workspace.featureConfig || {}
    };
  }

  /**
   * PATCH /workspaces/:id/features
   * 更新工作空间功能配置（仅 Admin）
   */
  @Patch('/:id/features')
  async updateWorkspaceFeatures(
    @Param('id') workspaceId: string,
    @Body() featureConfig: any,
    @AuthUser() user: User
  ) {
    // 验证 Admin 权限
    const isAdmin = await this.projectService.isUserAdmin(workspaceId, user.id);
    if (!isAdmin) {
      throw new ForbiddenError('只有工作空间管理员可以修改功能配置');
    }

    await this.projectRepository.update(
      { id: workspaceId },
      { featureConfig }
    );

    // 审计日志
    await this.auditLogService.log({
      action: 'workspace_features_updated',
      workspaceId,
      userId: user.id,
      metadata: { featureConfig }
    });

    return { success: true };
  }

  /**
   * GET /workspaces/:id/variables
   * 获取工作空间环境变量
   */
  @Get('/:id/variables')
  async getVariables(
    @Param('id') workspaceId: string,
    @AuthUser() user: User
  ) {
    await this.validateAccess(workspaceId, user.id);

    const variables = await this.variablesRepository.find({
      where: { projectId: workspaceId },
      order: { key: 'ASC' }
    });

    return {
      variables: variables.map(v => ({
        id: v.id,
        key: v.key,
        value: v.value, // 敏感值应该脱敏显示
        type: v.type,
        createdAt: v.createdAt
      }))
    };
  }

  /**
   * POST /workspaces/:id/variables
   * 创建环境变量（仅 Admin）
   */
  @Post('/:id/variables')
  async createVariable(
    @Param('id') workspaceId: string,
    @Body() data: { key: string; value: string; type: string },
    @AuthUser() user: User
  ) {
    const isAdmin = await this.projectService.isUserAdmin(workspaceId, user.id);
    if (!isAdmin) {
      throw new ForbiddenError('只有管理员可以创建环境变量');
    }

    // 检查是否超出配额
    const count = await this.variablesRepository.count({
      where: { projectId: workspaceId }
    });

    const maxCount = await this.getVariableQuota(workspaceId);
    if (count >= maxCount) {
      throw new BadRequestError(`环境变量数量已达上限（${maxCount}个）`);
    }

    const variable = await this.variablesRepository.save({
      projectId: workspaceId,
      key: data.key,
      value: data.value,
      type: data.type
    });

    return variable;
  }

  /**
   * GET /workspaces/:id/audit-logs
   * 获取工作空间审计日志
   */
  @Get('/:id/audit-logs')
  async getAuditLogs(
    @Param('id') workspaceId: string,
    @Query('page') page: number = 1,
    @Query('limit') limit: number = 50,
    @AuthUser() user: User
  ) {
    const isAdmin = await this.projectService.isUserAdmin(workspaceId, user.id);
    if (!isAdmin) {
      throw new ForbiddenError('只有管理员可以查看审计日志');
    }

    const [logs, total] = await this.auditLogRepository.findAndCount({
      where: { workspaceId },
      order: { createdAt: 'DESC' },
      skip: (page - 1) * limit,
      take: limit
    });

    return {
      logs,
      pagination: { page, limit, total }
    };
  }
}
```

**工作空间设置前端页面：**

```vue
<!-- packages/editor-ui/src/views/WorkspaceSettings.vue -->
<template>
  <div class="workspace-settings-page">
    <el-tabs v-model="activeTab">

      <!-- 环境变量 -->
      <el-tab-pane label="环境变量" name="variables">
        <div class="variables-section">
          <div class="section-header">
            <h3>环境变量管理</h3>
            <el-button type="primary" @click="showCreateVariable = true" v-if="isAdmin">
              + 添加变量
            </el-button>
          </div>

          <el-table :data="variables" style="width: 100%">
            <el-table-column prop="key" label="变量名" width="200" />
            <el-table-column prop="value" label="值">
              <template #default="{ row }">
                <span v-if="row.type === 'string'">{{ row.value }}</span>
                <span v-else class="sensitive">••••••••</span>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="150" v-if="isAdmin">
              <template #default="{ row }">
                <el-button size="small" @click="editVariable(row)">编辑</el-button>
                <el-button size="small" type="danger" @click="deleteVariable(row)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>

          <div class="quota-info">
            已使用 {{ variables.length }} / {{ variableQuota }} 个环境变量
          </div>
        </div>
      </el-tab-pane>

      <!-- 审计日志 -->
      <el-tab-pane label="审计日志" name="audit-logs" v-if="isAdmin">
        <div class="audit-logs-section">
          <el-table :data="auditLogs" style="width: 100%">
            <el-table-column prop="createdAt" label="时间" width="180">
              <template #default="{ row }">
                {{ formatDate(row.createdAt) }}
              </template>
            </el-table-column>
            <el-table-column prop="action" label="操作" width="200" />
            <el-table-column prop="userId" label="用户" width="150">
              <template #default="{ row }">
                {{ getUserName(row.userId) }}
              </template>
            </el-table-column>
            <el-table-column prop="metadata" label="详情">
              <template #default="{ row }">
                {{ formatMetadata(row.metadata) }}
              </template>
            </el-table-column>
          </el-table>

          <el-pagination
            v-model:current-page="auditLogPage"
            :page-size="50"
            :total="auditLogTotal"
            @current-change="fetchAuditLogs"
          />
        </div>
      </el-tab-pane>

      <!-- 自定义角色 -->
      <el-tab-pane label="自定义角色" name="custom-roles" v-if="isAdmin">
        <div class="custom-roles-section">
          <p class="feature-description">
            创建自定义角色，灵活分配团队成员权限
          </p>

          <!-- 自定义角色列表 -->
          <el-button type="primary" @click="showCreateRole = true">
            + 创建角色
          </el-button>

          <!-- 角色列表展示... -->
        </div>
      </el-tab-pane>

    </el-tabs>
  </div>
</template>
```

---

#### 12.3.3 层级3：普通用户控制

这些功能是**用户个人**的偏好，每个用户自己决定：

| 功能 | 多租户后处理 | 管理方式 | 理由 |
|------|------------|---------|------|
| **MFA（多因素认证）** | ✅ 保留 | 个人设置页面 | 用户自己决定是否双重验证 |
| **语言切换** | ✅ 保留 | 个人设置页面 | 个人偏好 |
| **AI 助手** | ✅ 保留 | 个人设置页面 | 开发者工具，个人偏好 |
| **调试模式** | ✅ 保留 | 工作流编辑器 | 开发者工具 |

**实现方式：**

```sql
-- 用户个人功能配置（扩展 user 表）
ALTER TABLE "user" ADD COLUMN feature_preferences JSONB DEFAULT '{}';

-- 例如存储：
-- {
--   "mfa_enabled": false,
--   "ai_assistant_enabled": true,
--   "language": "zh-CN",
--   "debug_mode": false
-- }
```

**用户设置 API：**

```typescript
// packages/cli/src/controllers/users.controller.ts
@RestController('/users')
export class UsersController {

  /**
   * GET /users/me/preferences
   * 获取当前用户偏好设置
   */
  @Get('/me/preferences')
  async getPreferences(@AuthUser() user: User) {
    return {
      preferences: user.featurePreferences || {}
    };
  }

  /**
   * PATCH /users/me/preferences
   * 更新用户偏好设置
   */
  @Patch('/me/preferences')
  async updatePreferences(
    @AuthUser() user: User,
    @Body() preferences: any
  ) {
    await this.userRepository.update(
      { id: user.id },
      { featurePreferences: { ...user.featurePreferences, ...preferences } }
    );

    return { success: true };
  }

  /**
   * POST /users/me/mfa/enable
   * 启用 MFA
   */
  @Post('/me/mfa/enable')
  async enableMFA(@AuthUser() user: User) {
    const secret = this.mfaService.generateSecret(user.email);
    const qrCode = await this.mfaService.generateQRCode(secret);

    // 临时存储 secret（用户验证后再正式保存）
    await this.cacheService.set(
      `mfa:setup:${user.id}`,
      secret,
      { ttl: 600 } // 10分钟有效
    );

    return {
      secret,
      qrCode
    };
  }

  /**
   * POST /users/me/mfa/verify
   * 验证并激活 MFA
   */
  @Post('/me/mfa/verify')
  async verifyMFA(
    @AuthUser() user: User,
    @Body() data: { code: string }
  ) {
    const secret = await this.cacheService.get(`mfa:setup:${user.id}`);
    if (!secret) {
      throw new BadRequestError('MFA 设置已过期，请重新开始');
    }

    const isValid = this.mfaService.verifyToken(secret, data.code);
    if (!isValid) {
      throw new BadRequestError('验证码错误');
    }

    // 保存到用户配置
    await this.userRepository.update(
      { id: user.id },
      {
        mfaSecret: this.encryptService.encrypt(secret),
        featurePreferences: {
          ...user.featurePreferences,
          mfa_enabled: true
        }
      }
    );

    // 生成恢复代码
    const recoveryCodes = this.mfaService.generateRecoveryCodes();
    await this.userRepository.update(
      { id: user.id },
      { mfaRecoveryCodes: this.encryptService.encrypt(JSON.stringify(recoveryCodes)) }
    );

    return {
      success: true,
      recoveryCodes // 只显示一次
    };
  }
}
```

**个人设置前端页面：**

```vue
<!-- packages/editor-ui/src/views/PersonalSettings.vue -->
<template>
  <div class="personal-settings-page">
    <el-card>
      <template #header>
        <h2>个人设置</h2>
      </template>

      <el-form label-width="150px">

        <!-- 语言设置 -->
        <el-form-item label="界面语言">
          <el-radio-group v-model="preferences.language" @change="updateLanguage">
            <el-radio label="zh-CN">简体中文</el-radio>
            <el-radio label="en-US">English</el-radio>
          </el-radio-group>
        </el-form-item>

        <!-- MFA 设置 -->
        <el-form-item label="多因素认证">
          <div class="mfa-setting">
            <el-switch
              v-model="preferences.mfa_enabled"
              @change="toggleMFA"
              :disabled="mfaLoading"
            />
            <span class="setting-tip">
              启用后，登录时需要额外输入动态验证码，提高账号安全性
            </span>
          </div>
        </el-form-item>

        <!-- AI 助手 -->
        <el-form-item label="AI 助手">
          <el-switch
            v-model="preferences.ai_assistant_enabled"
            @change="updatePreferences"
          />
          <span class="setting-tip">
            在工作流编辑器中提供 AI 代码补全和智能建议
          </span>
        </el-form-item>

        <!-- 调试模式 -->
        <el-form-item label="调试模式">
          <el-switch
            v-model="preferences.debug_mode"
            @change="updatePreferences"
          />
          <span class="setting-tip">
            在工作流编辑器中显示详细的调试信息和断点功能
          </span>
        </el-form-item>

      </el-form>
    </el-card>

    <!-- MFA 设置对话框 -->
    <el-dialog v-model="showMFASetup" title="启用多因素认证" width="500px">
      <div class="mfa-setup-content">
        <p>1. 使用 Google Authenticator 或类似应用扫描二维码：</p>
        <div class="qr-code">
          <img :src="mfaQRCode" />
        </div>

        <p>2. 输入应用中显示的 6 位验证码：</p>
        <el-input
          v-model="mfaVerifyCode"
          placeholder="000000"
          maxlength="6"
        />
      </div>

      <template #footer>
        <el-button @click="showMFASetup = false">取消</el-button>
        <el-button type="primary" @click="verifyMFA" :loading="mfaLoading">
          验证并启用
        </el-button>
      </template>
    </el-dialog>

    <!-- 恢复代码展示 -->
    <el-dialog v-model="showRecoveryCodes" title="MFA 恢复代码" width="500px">
      <el-alert type="warning" :closable="false">
        <template #title>
          请妥善保存以下恢复代码！如果丢失手机，可用这些代码恢复访问。
        </template>
      </el-alert>

      <div class="recovery-codes">
        <code v-for="code in recoveryCodes" :key="code">{{ code }}</code>
      </div>

      <template #footer>
        <el-button @click="copyRecoveryCodes">复制所有代码</el-button>
        <el-button type="primary" @click="showRecoveryCodes = false">我已保存</el-button>
      </template>
    </el-dialog>
  </div>
</template>

<script setup lang="ts">
import { ref, onMounted } from 'vue';
import { userApi } from '@/api/users';
import { ElMessage } from 'element-plus';
import { useI18n } from 'vue-i18n';

const { locale } = useI18n();

const preferences = ref({
  language: 'zh-CN',
  mfa_enabled: false,
  ai_assistant_enabled: true,
  debug_mode: false
});

const showMFASetup = ref(false);
const mfaLoading = ref(false);
const mfaQRCode = ref('');
const mfaSecret = ref('');
const mfaVerifyCode = ref('');
const showRecoveryCodes = ref(false);
const recoveryCodes = ref([]);

onMounted(async () => {
  const res = await userApi.getPreferences();
  preferences.value = res.preferences;
});

async function updateLanguage() {
  locale.value = preferences.value.language;
  await updatePreferences();
}

async function updatePreferences() {
  await userApi.updatePreferences(preferences.value);
  ElMessage.success('设置已保存');
}

async function toggleMFA(enabled: boolean) {
  if (enabled) {
    // 启用 MFA
    mfaLoading.value = true;
    try {
      const res = await userApi.enableMFA();
      mfaQRCode.value = res.qrCode;
      mfaSecret.value = res.secret;
      showMFASetup.value = true;
    } finally {
      mfaLoading.value = false;
    }
  } else {
    // 禁用 MFA（需要确认）
    await ElMessageBox.confirm('确定要禁用多因素认证吗？这会降低账号安全性。', '警告', {
      type: 'warning'
    });

    await userApi.disableMFA();
    ElMessage.success('MFA 已禁用');
  }
}

async function verifyMFA() {
  mfaLoading.value = true;
  try {
    const res = await userApi.verifyMFA({ code: mfaVerifyCode.value });
    showMFASetup.value = false;
    recoveryCodes.value = res.recoveryCodes;
    showRecoveryCodes.value = true;
    ElMessage.success('MFA 已启用');
  } catch (error) {
    ElMessage.error('验证码错误，请重试');
    preferences.value.mfa_enabled = false;
  } finally {
    mfaLoading.value = false;
  }
}

function copyRecoveryCodes() {
  navigator.clipboard.writeText(recoveryCodes.value.join('\n'));
  ElMessage.success('已复制到剪贴板');
}
</script>

<style scoped>
.personal-settings-page {
  max-width: 800px;
  margin: var(--spacing--lg) auto;
  padding: var(--spacing--lg);
}

.mfa-setting,
.setting-tip {
  display: flex;
  align-items: center;
  gap: var(--spacing--sm);
}

.setting-tip {
  font-size: var(--font-size--2xs);
  color: var(--color--text--tint-2);
  margin-left: var(--spacing--sm);
}

.mfa-setup-content {
  text-align: center;
}

.qr-code {
  margin: var(--spacing--lg) 0;
}

.qr-code img {
  width: 200px;
  height: 200px;
}

.recovery-codes {
  display: grid;
  grid-template-columns: repeat(2, 1fr);
  gap: var(--spacing--xs);
  margin: var(--spacing--lg) 0;
}

.recovery-codes code {
  padding: var(--spacing--xs);
  background: var(--color--background--light-2);
  border-radius: var(--radius--sm);
  font-family: monospace;
  font-size: var(--font-size--sm);
}
</style>
```

### 12.4 功能配置总结表

| 功能 | 控制层级 | 默认状态 | 是否可配置 | 说明 |
|------|---------|---------|----------|------|
| **LDAP/SAML/OIDC** | 平台管理员 | ❌ 禁用 | ✅ 是 | 小团队 SaaS 不需要 |
| **外部密钥管理** | 平台管理员 | ❌ 禁用 | ✅ 是 | 不适合多租户 |
| **日志流** | 平台管理员 | ❌ 禁用 | ✅ 是 | 平台运维功能 |
| **Worker View** | 平台管理员 | ⚠️ 仅管理员 | ❌ 否 | 平台监控 |
| **Public API** | 平台管理员 | ✅ 启用 + 限流 | ✅ 是 | 1000次/小时/工作空间 |
| **工作流版本历史** | 平台管理员 | ✅ 无限制 | ✅ 是 | 可配置清理策略 |
| **Insights 分析** | - | ✅ 无限制 | ❌ 否 | 查看历史不产生成本 |
| **审计日志** | 工作空间管理员 | ✅ 启用 | ❌ 否 | 工作空间隔离 |
| **自定义角色** | 工作空间管理员 | ✅ 启用 | ✅ 是 | 团队自定义 |
| **环境变量** | 工作空间管理员 | ✅ 启用 | ✅ 是 | 工作空间隔离 |
| **工作流分享** | 工作空间管理员 | ✅ 启用 | ❌ 否 | 核心协作功能 |
| **MFA** | 普通用户 | ❌ 默认关闭 | ✅ 是 | 用户自选 |
| **语言切换** | 普通用户 | 中文 | ✅ 是 | 用户偏好 |
| **AI 助手** | 普通用户 | ✅ 默认开启 | ✅ 是 | 用户偏好 |
| **调试模式** | 普通用户 | ❌ 默认关闭 | ✅ 是 | 开发者工具 |

### 12.5 实施计划

将企业版功能管理纳入整体改造时间线：

**Week 7-8: 企业版功能管理系统**
- [ ] 创建 `platform_feature_config` 表
- [ ] 实现 `AdminPlatformFeaturesController`
- [ ] 实现工作空间功能配置 API
- [ ] 实现用户偏好设置 API
- [ ] 后台管理前端：平台功能管理页面
- [ ] 用户前端：工作空间设置页面
- [ ] 用户前端：个人设置页面
- [ ] MFA 启用/禁用流程实现
- [ ] 环境变量管理界面

**Week 9: 功能开关实施**
- [ ] 禁用 LDAP/SAML/OIDC 功能（代码级）
- [ ] 禁用外部密钥管理功能
- [ ] 禁用日志流功能（保留平台内部使用）
- [ ] Worker View 限制为仅管理员可见
- [ ] Public API 限流中间件实现
- [ ] 环境变量工作空间隔离验证
- [ ] 审计日志工作空间隔离验证

---

**文档版本：** v5.0（激进改造版 + 完整用户注册系统 + 企业版功能管理）
**最后更新：** 2025-01-07
**配套文档：** 02-架构图和流程图.md, 03-后台管理系统设计.md, 04-安全与性能优化方案.md

## 📝 v4.0 更新内容

### 新增内容
1. **用户注册系统**
   - 完整的用户注册流程（原子性事务）
   - 邮箱验证系统
   - 手机号验证系统（可选）
   - OAuth 第三方登录（微信、GitHub）
   - Onboarding 新手引导

2. **数据库扩展**
   - User 表完整扩展（20+ 新字段）
   - 邀请码系统表（invite_codes, invite_records）
   - 审计日志表（audit_logs）
   - 工作空间配额表（workspace_quotas）
   - 注册统计表（registration_stats）
   - execution_entity 和 variables_entity 添加 project_id

3. **后端服务**
   - UserService（完整注册流程）
   - EmailVerificationService
   - PhoneVerificationService（可选）
   - OnboardingService

4. **API 接口**
   - AuthController（注册、登录、验证）
   - OAuthController（第三方登录）
   - OnboardingController

5. **前端页面**
   - RegisterView（注册页面）
   - OnboardingView（引导页面）
   - EmailVerificationView

### 关键特性
- ✅ 注册即送 ¥10
- ✅ 邀请码双方各得 ¥5
- ✅ 邮箱验证奖励 ¥2
- ✅ Onboarding 任务最多可获得 ¥15
- ✅ 自动创建个人工作空间
- ✅ 支持微信、GitHub 第三方登录
