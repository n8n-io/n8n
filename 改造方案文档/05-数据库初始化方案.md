# n8n 多租户架构 - 数据库初始化方案（全新项目）

> **配套文档：** 01-架构底层改造方案.md (v3.0), 02-架构图和流程图.md (v2.0)
> **版本：** v1.0
> **日期：** 2025-11-07
> **状态：** ✅ 适用于未上线项目
...
既然是全新项目，我们采用**直接创建新架构**的方式，而不是复杂的数据迁移：
...
-- 删除 shared_workflow 表及其相关约束
DROP TABLE IF EXISTS shared_workflow CASCADE;

-- 删除 shared_credentials 表及其相关约束
...
### 3.1 方式一：使用 TypeORM Migration（推荐）

创建 TypeORM migration 文件自动处理数据库结构：
...
export class AddMultiTenantSupport1731000000000 implements ReversibleMigration {
  async up({ queryRunner, tablePrefix }: MigrationContext): Promise<void> {
    // 1. 为 workflow_entity 添加 project_id
    ...
    // 2. 为 credentials_entity 添加 project_id 和 is_managed
    await queryRunner.query(`
    ...
    // 3. 为 execution_entity 添加 workspace_id
    await queryRunner.query(`
    ...
    // 4. 为 variables_entity 添加 project_id
    await queryRunner.query(`
...
# 或者在开发环境使用 synchronize（谨慎！）
# 在 packages/cli/src/databases/config.ts 中设置 synchronize: true
```

---

### 3.2 方式二：直接 SQL 脚本（快速）

如果你想快速在开发环境初始化，直接执行完整 SQL：

```bash
# 执行初始化脚本
psql -U n8n -d n8n < 改造方案文档/scripts/init-multi-tenant-db.sql
```
...
VALUES
  ('legal-rag-cn', '中国法律知识库', 'legal', 0.50, true),
  ('medical-rag-cn', '医疗健康知识库', 'medical', 0.80, true),
  ('finance-rag-cn', '金融财务知识库', 'finance', 0.60, true);
```
...
FROM information_schema.tables
WHERE table_schema = 'public'
  AND table_name IN ('workspace_balance', 'usage_record', 'platform_service', 'recharge_record');
-- ✅ 预期：4行
...
# 测试3: 创建工作流（需要 X-Workspace-Id header）
...
### Q2: TypeORM synchronize 和 migration 哪个好？

| 方式 | 优点 | 缺点 | 推荐场景 |
|------|------|------|---------|
| **synchronize: true** | 自动同步，快速 | 不安全，可能丢数据 | 仅开发环境 |
| **migration** | 安全，可控，可回滚 | 需要手动编写 | 生产环境、团队协作 |

**建议：**
- 开发环境：synchronize: true（快速迭代）
- 生产环境：必须使用 migration

### Q3: 如何处理 Entity 定义和数据库不一致？
...
1. ✅ 更新 Entity 定义（Workflow, Credentials, Execution）
2. ✅ 删除 SharedWorkflow, SharedCredentials Entity
3. ✅ 更新 Repository 层查询逻辑
4. ✅ 更新 Service 层业务逻辑
5. ✅ 测试所有功能
...
1. ✅ **无需数据迁移** - 项目未上线，直接按新架构创建
2. ✅ **删除旧表** - 直接删除 shared_workflow 和 shared_credentials
3. ✅ **使用 TypeORM Migration** - 规范的数据库变更管理
4. ✅ **初始化种子数据** - 平台服务、测试账户

### 9.2 与"迁移方案"的区别

| 项目 | 数据迁移方案 | 数据库初始化方案（你们） |
|------|-------------|---------------------|
| **适用场景** | 已上线生产环境 | ✅ 未上线全新项目 |
| **数据备份** | ✅ 必须 | ❌ 不需要 |
| **迁移脚本** | ✅ 复杂 | ❌ 不需要 |
| **回滚方案** | ✅ 必须准备 | ❌ 不需要（重置即可） |
| **实施时间** | 2-4小时 | ✅ 10-20分钟 |
