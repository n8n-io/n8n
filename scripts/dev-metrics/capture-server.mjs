#!/usr/bin/env node
/**
 * Local PostHog capture stub for testing dev-metrics events.
 *
 * Stands in for https://ph.n8n.io so you can see exactly what track.mjs would
 * send, without touching the real PostHog project. It accepts PostHog capture
 * requests (`/capture/`, `/i/v0/e/`, `/batch/`), pretty-prints each event, and
 * replies with PostHog's `{"status":1}` so the tracker is happy.
 *
 * Usage:
...
function logEvent(url, e) {
	...
	console.log(`\n#${count} [${time}] POST ${url}  ${e.event ?? '(no event)'}  id=${e.distinct_id}`);
	console.log(
...
const server = createServer((req, res) => {
	...
	req.on('end', () => {
		try {
			...
			// PostHog sends either a single event or a `batch` array.
...
server.listen(port, () => {
	console.log(`PostHog capture stub listening on http://localhost:${port}`);
	console.log('Point the tracker at it (in the shell that runs pnpm):');
	console.log(`  export N8N_DEV_METRICS_POSTHOG_HOST=http://localhost:${port}`);
