...
export async function connectOpenCode(options, ensureCodespace) {
	...
	try {
		...
		if (version && health.version !== version) {
			throw new Error(
				`OpenCode versions differ: local ${version}, server ${health.version}. Install the matching version with \`pnpm add --global opencode-ai@${health.version}\`, or use --web or --legacy. The remote server is still running.`,
			);
		}
