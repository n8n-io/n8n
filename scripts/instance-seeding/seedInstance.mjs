...
const PROJECT_NAMES = [
	...UTILITY_PROJECT_NAMES,
	...COMMUNITIES.flatMap((c) => c.projects),
	...TRENCHCOAT_PROJECTS,
];
const communityOf = (projectName) =>
	COMMUNITIES.find((c) => c.projects.includes(projectName))?.name ?? null;
...
const CRED_RECIPES = [
	...
	},
	{
		type: 'slackApi',
		fields: () => ({
			accessToken: `xoxb-${rand(48)}`,
			signatureSecret: rand(32),
			allowedHttpRequestDomains: 'none',
		}),
...
async function main() {
	...
	const existingTeamNames = new Set(
		initialProjects.filter((p) => p.type === 'team').map((p) => p.name),
	);
	...
	const teamProjects = projectsResp.filter(
		(p) => p.type === 'team' && PROJECT_NAMES.includes(p.name),
	);
	const utilityProjects = UTILITY_PROJECT_NAMES.map((n) =>
		teamProjects.find((p) => p.name === n),
	).filter(Boolean);
	...
	log(
		`Team projects available: ${teamProjects.length} (utility projects: ${utilityProjects.map((p) => p.name).join(', ')})`,
	);
	...
	const eligibleForOptOut = communityProjects.filter((p) => !isTrenchcoat(p.name)).map((p) => p.id);
	...
	for (const proj of allTargetProjects) {
		...
		const themePool = trenchcoat
			? [...TRENCHCOAT_WORKFLOW_THEMES, ...WORKFLOW_THEMES]
			: WORKFLOW_THEMES;
		const groups = trenchcoat ? (TRENCHCOAT_GROUPS[proj.name] ?? []) : [];
		for (let i = 0; i < p.leafCount; i++) {
			const theme = pickWord(themePool);
			const credPick = p.creds.length
				? pick(p.creds, Math.min(p.creds.length, randInt([0, 2])))
				: [];
	...
	const pickTrenchcoatSubWf = (projectId, sourceGroup, count) => {
		...
		const otherGroupPool = sourceGroup
			? ownPool.filter((w) => w.group && w.group !== sourceGroup)
			: [];
		...
		for (let i = 0; i < count; i++) {
			...
			if (r < 0.75) pool = sameGroupPool.length ? sameGroupPool : ownPool;
				...
				pool = otherGroupPool.length
					? otherGroupPool
					: sameGroupPool.length
						? sameGroupPool
						: ownPool;
			else pool = lynchpinPool.length ? lynchpinPool : ownPool;
			if (pool.length === 0) continue;
	...
	for (const dt of chosenDts) {
		...
		const consumerCandidates = allTargetProjects.filter((p) => {
			if (p.id === ownerProj.id || p.id === personal.id || utilityProjectIds.has(p.id))
				return false;
