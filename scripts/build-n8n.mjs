...
// Everything from the FE trim through the SBOM step mutates package.json
// files (the trim) or can fail after that mutation (deploy, strips, SBOM).
// Wrapped in functions, and every internal failure THROWS rather than
// calling process.exit() directly, so the try/finally below can guarantee
// the restore always runs, on success or failure alike. process.exit()
// bypasses try/finally entirely; a raw throw does not.
async function deployProductionClosures() {
	// Run FE trim script
	...
	return packageDeployTime;
}

// Restore the package.json files the FE trim mutated. A missing backup is
// NOT silently skipped: leaving a trimmed package.json on disk unnoticed is
// exactly the failure that broke a later `pnpm` command's lockfile check in
// production — it saw whatever the trim left behind and reported the gap as
// an "outdated lockfile", with no error anywhere in this script.
...
let packageDeployTime;
try {
	packageDeployTime = await deployProductionClosures();
} finally {
