...
const alias = [
	...
	// Fix util/ trailing slash resolution issue in rolldown-vite worker builds
	{ find: /^util\/$/, replacement: 'util' },
	// Custom assert shim to avoid util/ trailing slash issue in worker builds
	{ find: /^assert$/, replacement: resolve(__dirname, 'vite/assert-shim.js') },
	...
	{
		// v-code-diff postinstall doesn't run in Docker, so point directly to v3 dist
		find: 'v-code-diff',
...
const plugins: UserConfig['plugins'] = [
	...
	nodePolyfills({
		...
		// Note: 'assert' excluded - uses custom shim due to util/ trailing slash issue in worker builds
	}),
	{
