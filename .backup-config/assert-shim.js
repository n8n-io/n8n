...
assert.throws = function(fn, expected, message) {
  ...
  try {
    ...
    if (expected && !(e instanceof expected)) {
      throw new AssertionError({ message: message || 'Wrong exception type', actual: e, expected, operator: 'throws' });
    }
  }
  if (!threw) {
...
assert.doesNotThrow = function(fn, message) {
  try {
    ...
    throw new AssertionError({ message: message || 'Function threw unexpectedly', actual: e, operator: 'doesNotThrow' });
  }
};
