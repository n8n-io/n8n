...
write_env() {
	...
	# instance-ai loads by default since n8n 2.35.0; older versions must opt in.
	modules_line=""
	minor="${INSTALL_VERSION#*.}"
	if [ "${INSTALL_VERSION%%.*}" -eq 2 ] && [ "${minor%%.*}" -lt 35 ]; then
		modules_line="N8N_ENABLED_MODULES=instance-ai"
	fi
	cat >"${N8N_DIR}/.env" <<EOF
