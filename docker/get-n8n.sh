		...
		case "$1" in
			--version)
				if [ $# -gt 1 ] && [ "${2#-}" = "$2" ]; then
					...
					say "get-n8n.sh v${SCRIPT_VERSION} (installs the latest stable n8n, currently $(resolve_n8n_version), with sandbox service ${SANDBOX_VERSION})"
					exit 0
				fi
				;;
			--upgrade) UPGRADE=1 ;;
...
# Versions to run. 'get-n8n.sh --upgrade' updates these two lines and nothing
...
# install time; after that the local copy belongs to the user, except for the
# sandbox image tags that --upgrade repoints (migrate_compose_sandbox_pins).
...
check_compose_freshness() {
	...
	say "from v${installed}. Beyond the sandbox image tags, get-n8n.sh never rewrites"
