...
4. If `pnpm dev:fe` was used, check the recent terminal output for "Failed to resolve import" — that means a workspace dep wasn't built before Vite started.
5. Confirm `node_modules` aligns with `package.json` (any new dependency requires `pnpm install`).
6. Confirm the route guard. Frontend module descriptors with `beforeEnter` may gate the route on a different flag than the user assumes.
