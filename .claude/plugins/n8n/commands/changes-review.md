...
- **Simplicity / over-engineering** — review the diff for over-engineering: speculative abstractions, needless config/flexibility, error handling for impossible cases, and code that could be inlined or deleted. Tag `[MINOR]`–`[MAJOR]` and fold into the synthesis.
