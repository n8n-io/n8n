...
- **Simplicity / over-engineering (ponytail)** — *if the `ponytail-review` skill is installed*, run `/ponytail-review` on the diff as this lens; *if it isn't available*, apply the same checklist inline: speculative abstractions, needless config/flexibility, error handling for impossible cases, and code that could be inlined or deleted. Tag `[MINOR]`–`[MAJOR]` and fold into the synthesis.
