...
async function getBaseUrl(
	...
	const myCache = new NodeCache(86400);
	let baseUrl = '';
	if (!skipCache) {
		baseUrl = myCache.get('baseUrl');
		if (baseUrl) {
			return baseUrl;
		}
	}
	...
	console.log(`${companyName}\\${userName}:${password}`);
	...
	try {
		...
		if (responseData.status !== 200) {
			...
			baseUrl = responseData.body.urls.base;
			myCache.set('baseUrl', baseUrl);
			return baseUrl;
		}
	} catch (error) {
...
export async function EloquaApiRequest(
	...
	const credentials = this.getCredentials('EloquaApi');
...
/**
 * Make an API request to paginated Eloqua endpoint
 ...
 * @returns {Promise<any>}
 */
export async function EloquaApiRequestAllItems(
	this: IHookFunctions | IExecuteFunctions | ILoadOptionsFunctions,
	method: string,
	endpoint: string,
	body: IDataObject,
...
export function EloquaDefaultGetAllProperties(
	resource: string,
	operation: string
): INodeProperties[] {
	return [
		...
		},
		{
			...
			default: true,
			description:
				'Return a simplified version of the response instead of the raw data.',
		},
	];
}
