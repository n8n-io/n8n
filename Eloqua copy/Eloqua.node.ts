	...
	/* methods = {
		loadOptions: {
			...
			async getContactCustomFields(
				...
				const { fields } = await EloquaApiRequest.call(
					this,
					'GET',
					'/api/3/fields',
					{},
			...
			async getAccountCustomFields(
				...
				const returnData: INodePropertyOptions[] = [];
				const { accountCustomFieldMeta: fields } = await EloquaApiRequest.call(
					this,
					'GET',
			...
			async getTags(
				...
				const returnData: INodePropertyOptions[] = [];
				const { tags } = await EloquaApiRequest.call(
					this,
					'GET',
					'/api/3/tags',
					{},
			...
			try {
				...
				if (resource === 'contact') {
					...
					if (operation === 'create') {
						...
						const accountContactId = this.getNodeParameter(
							'accountContactId',
							i
						) as number;
					...
					if (operation === 'add') {
						...
						const contactTagId = this.getNodeParameter(
							'contactTagId',
							i
						) as number;
				...
				let responseData;
				if (returnAll === true) {
					responseData = await EloquaApiRequestAllItems.call(
						this,
						requestMethod,
						endpoint,
						body,
