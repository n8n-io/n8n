...
describe('AddBinaryDataSizeBytesToExecutionEntity Migration', () => {
	...
	describe('up', () => {
		it('should add binaryDataSizeBytes as NOT NULL default 0', async () => {
			await runSingleMigration(MIGRATION_NAME);
			const context = createTestMigrationContext(dataSource);

			expectNotNullDefaultZero(await getColumnMeta(context, 'binaryDataSizeBytes'), context);

			await context.queryRunner.release();
		});

		it('should set a column comment on binaryDataSizeBytes (Postgres only)', async () => {
			await runSingleMigration(MIGRATION_NAME);
			const context = createTestMigrationContext(dataSource);
			...
			const rows = (await context.queryRunner.query(
				...
				[`${context.tablePrefix}execution_entity`, 'binaryDataSizeBytes'],
			)) as Array<{ description: string }>;

			expect(rows[0]?.description).toContain('offloaded');
	...
	describe('down', () => {
		it('should remove the column', async () => {
			...
			const context = createTestMigrationContext(dataSource);
			expect(await getColumnMeta(context, 'binaryDataSizeBytes')).toBeUndefined();
			await context.queryRunner.release();
		});
	});
});
