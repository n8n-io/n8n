...
const INDEX_NAME = 'IDX_activity_event_resource';
...
describe('AddActivityEventResourceIndex migration', () => {
	...
	async function resourceIndexColumns(): Promise<string[] | undefined> {
		return await withContext(async (context) => {
			const table = await context.queryRunner.getTable(`${context.tablePrefix}activity_event`);
			return table?.indices.find((index) => index.name === `${context.tablePrefix}${INDEX_NAME}`)
				?.columnNames;
		});
	}
