...
const MIGRATION_NAME = 'AddServiceAccountUserIdToAgents1785900000000';

describe('AddServiceAccountUserIdToAgents Migration', () => {
	...
	describe('up', () => {
		it('should add serviceAccountUserId as a nullable column', async () => {
			await runSingleMigration(MIGRATION_NAME);
			dataSource = Container.get(DataSource);

			await withContext(async (context) => {
				const col = await getColumnMeta(context, 'serviceAccountUserId');
		...
		it('should default serviceAccountUserId to null on existing agents', async () => {
			...
			await withContext(async (context) => {
				const rows = await context.runQuery<Array<{ serviceAccountUserId: string | null }>>(
					`SELECT "serviceAccountUserId" FROM ${context.escape.tableName('agents')} WHERE "id" = :id`,
					{ id: agentId },
				);
				expect(rows).toHaveLength(1);
				expect(rows[0].serviceAccountUserId).toBeNull();
			});
		});
	...
	describe('down', () => {
		it('should remove the column and preserve child rows', async () => {
			...
			await withContext(async (context) => {
				expect(await getColumnMeta(context, 'serviceAccountUserId')).toBeUndefined();
