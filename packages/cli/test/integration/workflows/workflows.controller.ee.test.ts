...
const activationErrorsService = mockInstance(ActivationErrorsService);
...
beforeEach(async () => {
	...
	activationErrorsService.deregister.mockReset();
...
describe('PUT /:workflowId/transfer', () => {
	...
	test('should handle workflow transfer with data table relations correctly', async () => {
		//
		// ARRANGE
		//
		const projectA = await createTeamProject('Project A', member);
		const projectB = await createTeamProject('Project B', member);

		// Create data table in Project A
		const dataTableA = await createDataTable(projectA, {
			name: 'table-a',
			columns: [{ name: 'value', type: 'string' }],
		});

		// Create workflow in Project A with data table reference
		const workflow = await createWorkflow(
			{
				nodes: [
					{
		...
		//
		// ACT & ASSERT - Transfer to Project B
		//
		// This should succeed even though data table doesn't exist in Project B
		// The workflow will have errors when activated, but transfer itself should work
		const transferResponse = await testServer
			.authAgentFor(member)
			.put(`/workflows/${workflow.id}/transfer`)
			.send({ destinationProjectId: projectB.id })
			.expect(200);

		expect(transferResponse.body).toEqual({});
		expect(activationErrorsService.deregister).toHaveBeenCalledWith(workflow.id);

		// Verify workflow is in Project B
		const workflowAfterTransfer = await workflowRepository.findOneOrFail({
			where: { id: workflow.id },
			relations: ['shared'],
		});
		const ownerSharing = workflowAfterTransfer.shared.find((s) => s.role === 'workflow:owner');
		expect(ownerSharing?.projectId).toBe(projectB.id);

		//
		// ACT & ASSERT - Transfer back to Project A
		//
		// Simulate error from Project B context
		await activationErrorsService.register(
			workflow.id,
			`Could not find the data table: '${dataTableA.id}'`,
		);

		const transferBackResponse = await testServer
			.authAgentFor(member)
			.put(`/workflows/${workflow.id}/transfer`)
			.send({ destinationProjectId: projectA.id })
			.expect(200);

		expect(transferBackResponse.body).toEqual({});
		expect(activationErrorsService.deregister).toHaveBeenCalledTimes(2); // Called for both transfers

		// Verify error is cleared and workflow is ready
		const errorAfterTransferBack = await activationErrorsService.get(workflow.id);
		expect(errorAfterTransferBack).toBeNull();

		// Verify workflow is back in Project A
		const workflowAfterTransferBack = await workflowRepository.findOneOrFail({
			where: { id: workflow.id },
			relations: ['shared'],
		});
		const ownerSharingAfterTransferBack = workflowAfterTransferBack.shared.find(
			(s) => s.role === 'workflow:owner',
		);
		expect(ownerSharingAfterTransferBack?.projectId).toBe(projectA.id);

		// Workflow should be in a clean state - no recreation needed
		// Data table relations will be automatically re-resolved when workflow is activated
		// because getDataTableProxy uses the workflow's current project context
	});
