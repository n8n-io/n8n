...
describe('router should switch based on flag', () => {
	let savedWorkflowId;
	beforeEach(async () => {
		const createWorkflowResponse = await authOwnerAgent
			.post('/workflows')
			.send((0, utils_1.makeWorkflow)());
...
describe('PUT /workflows/:workflowId/share', () => {
	test('should save sharing with new users', async () => {
		...
		const sharedWorkflows = await (0, backend_test_utils_1.getWorkflowSharing)(workflow);
	...
	test('should override sharing', async () => {
		...
		const secondSharedWorkflows = await (0, backend_test_utils_1.getWorkflowSharing)(workflow);
	...
	test('should not allow sharing by another shared member', async () => {
		const workflow = await (0, backend_test_utils_1.createWorkflow)({}, member);
		await (0, backend_test_utils_1.shareWorkflowWithUsers)(workflow, [anotherMember]);
	...
	test('should ignore sharing with owner project', async () => {
		...
		await authOwnerAgent
			...
			.expect(200);
		const sharedWorkflows = await di_1.Container.get(db_1.SharedWorkflowRepository).findBy({
	...
	test('should ignore sharing with project that already has it shared', async () => {
		...
		await (0, backend_test_utils_1.shareWorkflowWithProjects)(workflow, [{ project: project2 }]);
		await authOwnerAgent
			.put(`/workflows/${workflow.id}/share`)
...
describe('GET /workflows/new', () => {
	[true, false].forEach((sharingEnabled) => {
		test(`should return an auto-incremented name, even when sharing is ${sharingEnabled ? 'enabled' : 'disabled'}`, async () => {
			license.enable('feat:sharing');
			await (0, backend_test_utils_1.createWorkflow)({ name: 'My workflow' }, owner);
			await (0, backend_test_utils_1.createWorkflow)({ name: 'My workflow 7' }, owner);
...
describe('GET /workflows/:workflowId', () => {
	...
	test('project viewers can view workflows', async () => {
		...
		const responseWorkflow = response.body.data;
	...
	test('should return a workflow with owner', async () => {
		...
		expect(responseWorkflow.sharedWithProjects).toHaveLength(0);
		expect(responseWorkflow.shared).toBeUndefined();
	});
	...
	test('should return all sharees', async () => {
		const workflow = await (0, backend_test_utils_1.createWorkflow)({}, owner);
		await (0, backend_test_utils_1.shareWorkflowWithUsers)(workflow, [member, anotherMember]);
		const response = await authOwnerAgent.get(`/workflows/${workflow.id}`).expect(200);
		const responseWorkflow = response.body.data;
		expect(responseWorkflow.homeProject).toMatchObject({
			id: ownerPersonalProject.id,
	...
	test('should return workflow with credentials owned by user', async () => {
		...
		const workflowPayload = (0, utils_1.makeWorkflow)({
			withPinData: false,
			withCredential: { id: savedCredential.id, name: savedCredential.name },
		});
		const workflow = await (0, backend_test_utils_1.createWorkflow)(workflowPayload, owner);
		const response = await authOwnerAgent.get(`/workflows/${workflow.id}`).expect(200);
		const responseWorkflow = response.body.data;
		expect(response.statusCode).toBe(200);
	...
	test.each([
		...
		async (_description, getActor) => {
			...
			const workflow = await (0, backend_test_utils_1.createWorkflow)(workflowPayload, actor);
	...
	test('should return workflow with credentials for all users with or without access', async () => {
		...
		const workflow = await (0, backend_test_utils_1.createWorkflow)(workflowPayload, member);
		await (0, backend_test_utils_1.shareWorkflowWithUsers)(workflow, [anotherMember]);
		const responseMember1 = await authMemberAgent.get(`/workflows/${workflow.id}`).expect(200);
		const member1Workflow = responseMember1.body.data;
		...
		const responseMember2 = await authAnotherMemberAgent
			.get(`/workflows/${workflow.id}`)
			.expect(200);
		const member2Workflow = responseMember2.body.data;
...
describe('POST /workflows', () => {
	test('project viewers cannot create workflows', async () => {
		...
		const response = await authMemberAgent
			.post('/workflows')
			.send({ ...(0, utils_1.makeWorkflow)(), projectId: teamProject.id });
	...
	it('Should create a workflow that uses no credential', async () => {
		const workflow = (0, utils_1.makeWorkflow)({ withPinData: false });
...
describe('PATCH /workflows/:workflowId', () => {
		...
		it.each([
			[
				...
				async function createCredential() {
					return await saveCredential((0, backend_test_utils_1.randomCredentialPayload)(), {
						user: owner,
					});
				},
			],
			[
				...
				'the member',
				async function creteWorkflow() {
					const team = await (0, backend_test_utils_1.createTeamProject)('Team 1', member);
					return await (0, backend_test_utils_1.createWorkflow)({}, team);
				},
				async function createCredential() {
					return await saveCredential((0, backend_test_utils_1.randomCredentialPayload)(), {
						user: member,
			...
			[
				...
				'team 2',
				async function creteWorkflow() {
					const team1 = await (0, backend_test_utils_1.createTeamProject)('Team 1', member);
					return await (0, backend_test_utils_1.createWorkflow)({}, team1);
				},
				async function createCredential() {
					const team2 = await (0, backend_test_utils_1.createTeamProject)('Team 2', member);
					return await saveCredential((0, backend_test_utils_1.randomCredentialPayload)(), {
						project: team2,
					});
				},
			...
			[
				...
				'the owner',
				async function creteWorkflow() {
					return await (0, backend_test_utils_1.createWorkflow)({}, member);
				},
				async function createCredential() {
					return await saveCredential((0, backend_test_utils_1.randomCredentialPayload)(), {
						user: owner,
	...
	describe('workflow history', () => {
		test('Should create workflow history version when licensed', async () => {
			...
			expect(
				await di_1.Container.get(db_1.WorkflowHistoryRepository).count({
					where: { workflowId: id },
				}),
			).toBe(1);
			const historyVersion = await di_1.Container.get(db_1.WorkflowHistoryRepository).findOne({
...
describe('PUT /:workflowId/transfer', () => {
	test('cannot transfer into the same project', async () => {
		...
		const workflow = await (0, backend_test_utils_1.createWorkflow)({}, destinationProject);
		await testServer
			.authAgentFor(member)
	...
	test('cannot transfer somebody elses workflow', async () => {
		...
		const workflow = await (0, backend_test_utils_1.createWorkflow)({}, anotherMember);
		await testServer
			.authAgentFor(member)
			.put(`/workflows/${workflow.id}/transfer`)
			.send({ destinationProjectId: destinationProject.id })
	...
	test.each(['project:editor', 'project:viewer'])(
		...
		async (projectRole) => {
			...
			const workflow = await (0, backend_test_utils_1.createWorkflow)({}, sourceProject);
			const destinationProject = await (0, backend_test_utils_1.createTeamProject)();
	...
	test('removes and re-adds the workflow from the active workflow manager during the transfer', async () => {
		...
		const workflow = await (0, backend_test_utils_1.createWorkflow)({ active: true }, member);
		const response = await testServer
			.authAgentFor(member)
			.put(`/workflows/${workflow.id}/transfer`)
			.send({ destinationProjectId: destinationProject.id })
	...
	test('should move workflow to project root if `destinationParentFolderId` is not provided', async () => {
		...
		const folder = await (0, folders_1.createFolder)(destinationProject, { name: 'Test Folder' });
		const workflow = await (0, backend_test_utils_1.createWorkflow)(
	...
	test('should fail destination parent folder does not exist in project', async () => {
		...
		const anotherProject = await (0, backend_test_utils_1.createTeamProject)(
			'Another Project',
			member,
		);
		const folderInDestinationProject = await (0, folders_1.createFolder)(destinationProject, {
			name: 'Test Folder',
		});
		const anotherFolder = await (0, folders_1.createFolder)(destinationProject, {
	...
	test('deactivates the workflow if it cannot be added to the active workflow manager again and returns the WorkflowActivationError as data', async () => {
		...
		const workflow = await (0, backend_test_utils_1.createWorkflow)({ active: true }, member);
		activeWorkflowManager.add.mockRejectedValue(
			new n8n_workflow_1.WorkflowActivationError('Failed'),
		);
		const response = await testServer
			.authAgentFor(member)
			.put(`/workflows/${workflow.id}/transfer`)
	...
	test('owner transfers workflow from project they are not part of, e.g. test global cred sharing scope', async () => {
		...
		await testServer
			...
			.expect(200);
		const allWorkflowSharings = await (0, backend_test_utils_1.getWorkflowSharing)(workflow);
	...
	test('member transfers workflow from a team project as project admin to another team project in which they have editor role but cannot share all the credentials', async () => {
		...
		const ownersCredential = await saveCredential(
			(0, backend_test_utils_1.randomCredentialPayload)(),
			{ user: owner },
		);
		const destinationProject = await (0, backend_test_utils_1.createTeamProject)(
		...
		const ownerCredentialSharings = await (0, credentials_1.getCredentialSharings)(
			ownersCredential,
		);
...
//# sourceMappingURL=workflows.controller.ee.test.js.map
