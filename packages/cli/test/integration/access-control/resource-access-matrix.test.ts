...
/**
 * Comprehensive Resource Access Control Matrix Tests
 *
 * This test suite validates ALL HTTP operations against different custom project roles
 * for workflows and credentials. It provides a complete access control matrix showing
 * exactly which status codes are returned for each role vs endpoint combination.
 *
 * Test Matrix:
 * - 8 Custom Role Types (4 workflow, 4 credential)
 * - 22 HTTP Endpoints (13 workflow, 9 credential)
 * - Deterministic status code expectations for each combination
 */
