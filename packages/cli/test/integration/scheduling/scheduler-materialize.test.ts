...
describe('scheduler materialization', () => {
	...
	describe('rules of one node under the owner-wide coalesce policy', () => {
		...
		it('never groups self-owned jobs together, whatever their policy', async () => {
			// Owner-scoped form of the pin from the non-workflow-jobs work: a system
			// task is its own owner (`ownerId` is its name), so unrelated system jobs
			// cannot coalesce into one another's catch-up run even under the
			// owner-wide policy.
