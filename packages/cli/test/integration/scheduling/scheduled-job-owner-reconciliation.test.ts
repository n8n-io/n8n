...
/**
 * The reconciliation sweep against real rows: the safety net that replaces the
 * `scheduled_job.workflowId` foreign key's `ON DELETE CASCADE`.
 *
 * Every case here is one the synchronous deprovision path did not cover — a
 * workflow unpublished without its jobs being removed — plus the guardrails that
 * stop the sweep from destroying valid jobs when the answer it acts on is wrong
 * or missing.
 */
describe('scheduled job owner reconciliation', () => {
	...
	/**
	 * The resolver the test owner type delegates to. Swapped per test rather than
	 * re-registered: an owner type is claimed once, which is the contract.
	 */
	let testResolver: ScheduledJobOwnerResolver;

	/**
	 * The resolver the workflow owner type delegates to, the real one by default.
	 * Swapped where a test needs to interleave work with a pass's liveness read.
	 */
	...
	beforeAll(async () => {
		...
		// A registry of this suite's own, so the test owner type can carry a
		// resolver each case replaces without touching the container's registry.
	...
	describe('a pass racing a concurrent provision', () => {
		it('may re-quarantine jobs a provision revived on a stale answer, and the next pass lifts it', async () => {
			...
			// The stale answer wins the interleaving: the live jobs are quarantined
			// again, though the fresh stamp keeps them a whole grace away from deletion.
