 ...
 * `workflow_published_version` with `ON DELETE CASCADE`, so unpublishing a
 * workflow removed its jobs in the database. That key is gone: teardown is now
 * an explicit call every owner module owes, and this suite pins both halves —
 * that the cascade really is gone, and that the explicit call covers what it
 * used to.
 */
describe('scheduled job owner teardown', () => {
	...
	it('no longer removes a workflow’s jobs when its published version goes away', async () => {
		// The pin on the dropped cascade: this is exactly why every teardown path
		// has to deprovision for itself.
	...
	it('commits the teardown with the caller’s transaction, and rolls back with it', async () => {
		...
		// The rollback took the delete with it: the job's removal is not independent
		// of whatever write made the workflow stop owning it.
		expect(await jobRepo.findOneBy({ id: job.id })).not.toBeNull();
	...
	it('lifts a quarantine the sweep left behind when the owner provisions again', async () => {
		// The recovery path for a teardown that never ran: the sweep disabled the
		// jobs of a workflow it believed gone, then the workflow publishes again.
		// Without this, the trigger stays silently dead until the next sweep.
		const workflow = await publishWorkflow();
		const nodeId = uuid();
		const desired = [
			{
		...
		// Same schedule as before: nothing about the definition changed, only its
		// quarantine, so an unchanged plan has to be enough to bring it back.
		await provisionOnce();
