 ...
 * shape system tasks use: identified by a well-known unique `name`, matched to
 * its handler by `taskType` alone. Pins the engine's indifference to the owner
 * kind across materialize -> claim -> fire -> retention, and the misfire
 * behaviour a self-owned job gets. The mechanics themselves are covered by the
...
describe('system jobs (self-owned)', () => {
	...
	it('under coalesce_owner, self-owned jobs are not grouped: each keeps its own catch-up run', async () => {
		// Two system jobs overdue beyond their grace. Under a shared owner they
		// would coalesce into one catch-up run; self-owned, `coalesce_owner` must
		// leave each job its own late run instead of grouping unrelated system jobs.
		const misfire = {
