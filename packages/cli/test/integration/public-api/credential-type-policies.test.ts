...
const projectRoute = (projectId: string) => `/credential-type-policies/projects/${projectId}`;

const ALLOW_ALL = { rules: [], defaultAction: 'allow', version: 0 };
const DENY_RULE = { id: 'r1', action: 'deny', selector: { kind: 'name', value: 'slackApi' } };
const DELEGATE_RULE = {
	id: 'r2',
	action: 'delegate',
	selector: { kind: 'name', value: 'slackApi' },
};
...
beforeAll(async () => {
	...
	projectAdmin = await createMemberWithApiKey({ scopes: ['credentialTypePolicy:manage'] });
	projectEditor = await createMemberWithApiKey({ scopes: ['credentialTypePolicy:manage'] });
	otherProjectAdmin = await createMemberWithApiKey({ scopes: ['credentialTypePolicy:manage'] });
 ...
 */
describe('credential type policies public API authorization', () => {
	...
	test.each([
		...
		['GET', '/credential-type-policies/policies'],
		['POST', '/credential-type-policies/policies'],
		['GET', '/credential-type-policies/policies/does-not-exist'],
		['PUT', '/credential-type-policies/policies/does-not-exist'],
		['DELETE', '/credential-type-policies/policies/does-not-exist'],
		['PUT', '/credential-type-policies/scopes/does-not-exist/attachments'],
...
describe('credential type policies public API license gating', () => {
	afterEach(() => {
	...
	test('rejects an owner with 403 when the license feature is disabled', async () => {
		...
		const instance = await testServer
...
describe('credential type policies public API instance scope', () => {
	...
	test('a GET body round-trips as a PUT body', async () => {
		...
		const current = await agent.get('/credential-type-policies/instance');
		const response = await agent.put('/credential-type-policies/instance').send(current.body);
	...
	test('PUT with a stale version returns 409', async () => {
		const agent = testServer.publicApiAgentFor(owner);
		expect((await agent.put('/credential-type-policies/instance').send(ALLOW_ALL)).statusCode).toBe(
			200,
		);
	...
	test('PUT applies the same validation as the internal controller', async () => {
		...
		const duplicateIds = await agent.put('/credential-type-policies/instance').send({
...
describe('credential type policies public API project scope', () => {
	...
	test('PUT is rejected with 409 when the project document is also attached to the instance scope', async () => {
		...
		const documents = await ownerAgent.get('/credential-type-policies/policies');
		expect(documents.body.data).toHaveLength(1);
		const documentId = documents.body.data[0].id as string;

		const instancePut = await ownerAgent.put('/credential-type-policies/instance').send(ALLOW_ALL);
...
describe('credential type policies public API policy documents', () => {
	...
	test('GET list paginates with an opaque cursor', async () => {
		...
		for (let i = 0; i < 3; i++) {
			expect(
				(await agent.post('/credential-type-policies/policies').send({ rules: [] })).statusCode,
			).toBe(201);
		}

		const firstPage = await agent.get('/credential-type-policies/policies?limit=2');
		...
		const secondPage = await agent.get(
			`/credential-type-policies/policies?cursor=${firstPage.body.nextCursor}`,
		);
	...
	test('GET list rejects an invalid cursor with 400', async () => {
		const response = await testServer
			.publicApiAgentFor(owner)
			.get('/credential-type-policies/policies?cursor=not-a-cursor');
	...
	test('GET, PUT, and DELETE of one document', async () => {
		const agent = testServer.publicApiAgentFor(owner);
		const created = await agent.post('/credential-type-policies/policies').send({ rules: [] });
		const policyId = created.body.policy.id as string;

		const fetched = await agent.get(`/credential-type-policies/policies/${policyId}`);
		expect(fetched.statusCode).toBe(200);
		expect(Object.keys(fetched.body).sort()).toEqual([...DOCUMENT_KEYS].sort());
		expect(fetched.body.id).toBe(policyId);

		const updated = await agent
			.put(`/credential-type-policies/policies/${policyId}`)
		...
		const deleted = await agent.delete(`/credential-type-policies/policies/${policyId}`);
		expect(deleted.statusCode).toBe(204);
		expect(deleted.body).toEqual({});

		const missing = await agent.get(`/credential-type-policies/policies/${policyId}`);
		expect(missing.statusCode).toBe(404);

		const deleteMissing = await agent.delete(`/credential-type-policies/policies/${policyId}`);
	...
	test('a document of another kind is not reachable by id, and survives untouched', async () => {
		...
		const fetched = await agent.get(`/credential-type-policies/policies/${other.id}`);
		expect(fetched.statusCode).toBe(404);

		const updated = await agent
			.put(`/credential-type-policies/policies/${other.id}`)
			.send({ rules: [DENY_RULE], version: other.version });
		expect(updated.statusCode).toBe(404);

		const deleted = await agent.delete(`/credential-type-policies/policies/${other.id}`);
	...
	test('a document of the node type policy kind is not reachable by id, and survives untouched', async () => {
		const policyRepo = Container.get(TypeAvailabilityPolicyRepository);
		const nodeTypePolicy = await policyRepo.createPolicy(
			{ kind: 'node-types', rules: [OTHER_KIND_RULE], updatedBy: owner.id },
			{},
		);
		const agent = testServer.publicApiAgentFor(owner);

		const fetched = await agent.get(`/credential-type-policies/policies/${nodeTypePolicy.id}`);
		expect(fetched.statusCode).toBe(404);

		const list = await agent.get('/credential-type-policies/policies');
		expect(list.body.data).toEqual([]);

		expect(await policyRepo.findByIdAndKind(nodeTypePolicy.id, 'node-types', {})).toMatchObject({
			kind: 'node-types',
			rules: [OTHER_KIND_RULE],
			version: nodeTypePolicy.version,
		});
	});

	test('PUT on an attached document bumps the instance scope version', async () => {
		const agent = testServer.publicApiAgentFor(owner);
		const instancePut = await agent.put('/credential-type-policies/instance').send(ALLOW_ALL);
		const versionBefore = instancePut.body.version as number;

		const documents = await agent.get('/credential-type-policies/policies');
		const document = documents.body.data[0];

		const updated = await agent
			.put(`/credential-type-policies/policies/${document.id}`)
			.send({ rules: [DENY_RULE], version: document.version });
		expect(updated.statusCode).toBe(200);

		const instance = await agent.get('/credential-type-policies/instance');
	...
	test('DELETE of an attached document returns 409', async () => {
		...
		const response = await agent.delete(`/credential-type-policies/policies/${policyId}`);

		expect(response.statusCode).toBe(409);
	});
});

describe('credential type policies public API attachments', () => {
