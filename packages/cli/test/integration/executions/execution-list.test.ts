...
beforeEach(async () => {
	...
	provider.searchExecutions.mockResolvedValue({ items: [], hasMore: false, total: 0 });
...
describe('editor execution visibility', () => {
	...
	it('resolves archive and workflow setting filters in the CP', async () => {
		...
		const sharingOptions =
			await Container.get(ExecutionService).buildSharingOptions('workflow:read');
...
describe('editor v1 source cursor', () => {
	...
	it('walks equal timestamps and a null start time in numeric ID order', async () => {
		...
		for (let i = 0; i < 3; i++) {
			...
			query.range.before = { timestamp: time, id: items.at(-1)!.id };
		}
