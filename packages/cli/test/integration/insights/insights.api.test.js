...
const db_utils_1 = require('@/modules/insights/database/entities/__tests__/db-utils');
...
describe('GET /insights routes return 200 if date range inside license limits', () => {
	...
	test.each(['day', 'week', '2weeks', 'month', 'quarter', '6months', 'year'])(
		'Call should work for date range %s',
		async (dateRange) => {
			const authAgent = agents.admin;
...
describe('GET /insights/by-workflow', () => {
	...
	describe('sorting order verification', () => {
		...
		test('should return workflows sorted by total:desc', async () => {
			const project = await (0, backend_test_utils_1.createTeamProject)('Test Project 1');
			const testData = [
			...
			const workflows = await Promise.all(
				testData.map(
					async (data) =>
						await (0, backend_test_utils_1.createWorkflow)({ name: data.name }, project),
				),
			);
			const periodStart = luxon_1.DateTime.utc().startOf('day');
			await Promise.all(
				workflows.map(
			...
			response.body.data.data.forEach((item, index) => {
				expect(item.total).toBe(expectedOrder[index]);
			});
		});
		test('should return workflows sorted by workflowName:asc', async () => {
			const project = await (0, backend_test_utils_1.createTeamProject)('Test Project A');
			const workflowNames = ['Zebra Workflow', 'Alpha Workflow', 'Beta Workflow'];
			const workflows = await Promise.all(
				workflowNames.map(
					async (name) => await (0, backend_test_utils_1.createWorkflow)({ name }, project),
				),
			);
			const periodStart = luxon_1.DateTime.utc().startOf('day');
			await Promise.all(
		...
		test('should return workflows sorted by failureRate:asc', async () => {
			const project = await (0, backend_test_utils_1.createTeamProject)('Another Test Project');
			const testData = [
				{ name: 'Low Failure', success: 9, failure: 1 },
				{ name: 'High Failure', success: 2, failure: 8 },
				{ name: 'Medium Failure', success: 5, failure: 5 },
			];
			const workflows = await Promise.all(
				testData.map(
					async (data) =>
			...
			expect(response.body.data.count).toBe(3);
			expect(response.body.data.data).toHaveLength(3);
			const expectedOrder = [0.1, 0.5, 0.8];
			response.body.data.data.forEach((item, index) => {
...
//# sourceMappingURL=insights.api.test.js.map
