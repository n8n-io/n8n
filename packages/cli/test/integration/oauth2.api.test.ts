...
test('GET /oauth2-credential/auth should return authentication URL', async () => {
	...
	const mockCredentialOverride = {
			...
			scope: 'chat:write',
			authentication: 'body',
			authQueryParameters: "user_scope=channels:read channels:write chat:write files:read files:write groups:read im:read mpim:read reactions:read reactions:write stars:read stars:write usergroups:write usergroups:read users.profile:read users.profile:write users:read",
			oauthTokenData: {},
			user_scope: '',
	}

	jest.spyOn(CredentialsHelper.prototype, 'applyDefaultsAndOverwrites').mockImplementation(() => mockCredentialOverride);
	...
	const response = await authAgent(owner).get('/oauth2-credential/auth').query({ id, clientId: newCredentialsData.data.clientId  });

	const { body: { data } }: { body: { data: string } } = response;
