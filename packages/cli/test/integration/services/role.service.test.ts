...
describe('RoleService', () => {
	...
	describe('combineResourceScopes', () => {
		it('should combine scopes for workflow resources', async () => {
			//
			// ARRANGE
			//
			const user = await createMember();
			...
			const userProjectRelations = [
				{
					projectId: 'project-1',
					role: {
						scopes: [{ slug: 'workflow:read' }, { slug: 'workflow:write' }],
					},
				},
			] as any[];
			...
			// ACT
			//
			const result = roleService.combineResourceScopes(
				'workflow',
				user,
				mockShared,
			...
			expect(result).toEqual(expect.arrayContaining([])); // Should return sorted array of scopes
		});

		it('should combine scopes for credential resources', async () => {
			//
			// ARRANGE
			//
			const user = await createMember();
			...
			const userProjectRelations = [
				{
					projectId: 'project-1',
					role: {
						scopes: [{ slug: 'credential:read' }, { slug: 'credential:write' }],
					},
				},
			] as any[];
		...
		it('should handle empty shared array', async () => {
			...
			//
			expect(Array.isArray(result)).toBe(true);
			expect(result.length).toBeGreaterThanOrEqual(0); // May have global scopes
		});
	});
});
