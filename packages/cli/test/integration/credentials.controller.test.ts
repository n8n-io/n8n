...
describe('GET /credentials', () => {
	describe('should return', () => {
		test('all credentials for owner', async () => {
			const { id: id1 } = await saveCredential(payload(), { user: owner, role: 'owner' });
			const { id: id2 } = await saveCredential(payload(), { user: member, role: 'owner' });
		...
		test('only own credentials for member', async () => {
			...
			const c1 = await saveCredential(payload(), { user: firstMember, role: 'owner' });
			const c2 = await saveCredential(payload(), { user: secondMember, role: 'owner' });
	...
	describe('filter', () => {
		test('should filter credentials by field: name - full match', async () => {
			const savedCred = await saveCredential(payload(), { user: owner, role: 'owner' });
	...
	describe('select', () => {
		test('should select credential field: id', async () => {
			await saveCredential(payload(), { user: owner, role: 'owner' });
			await saveCredential(payload(), { user: owner, role: 'owner' });
