...
describe('GET /credentials', () => {
	describe('should return', () => {
		test('all credentials for owner', async () => {
			const { id: id1 } = await createCredential(payload(), {
				user: owner,
				role: 'credential:owner',
			});
			const { id: id2 } = await createCredential(payload(), {
				user: member,
				role: 'credential:owner',
			});
		...
		test('only own credentials for member', async () => {
			...
			const c1 = await createCredential(payload(), { user: firstMember, role: 'credential:owner' });
			const c2 = await createCredential(payload(), {
				user: secondMember,
				role: 'credential:owner',
			});
	...
	describe('filter', () => {
		test('should filter credentials by field: name - full match', async () => {
			const savedCred = await createCredential(payload(), {
				user: owner,
				role: 'credential:owner',
			});
	...
	describe('select', () => {
		test('should select credential field: id', async () => {
			await createCredential(payload(), { user: owner, role: 'credential:owner' });
			await createCredential(payload(), { user: owner, role: 'credential:owner' });
