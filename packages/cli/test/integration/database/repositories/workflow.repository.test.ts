...
describe('WorkflowRepository', () => {
	...
	describe('findOwnerProjectAndArchivedState', () => {
		it('should return owner project and a real boolean archived state per id', async () => {
			// ARRANGE
			const { createOwner } = await import('../../shared/db/users.js');
			const { getPersonalProject } = await import('@n8n/backend-test-utils');
			...
			const liveWorkflow = await createWorkflow({ name: 'Live' });
			const archivedWorkflow = await createWorkflow({ name: 'Archived', isArchived: true });
			await shareWorkflowsToProject(
				[liveWorkflow, archivedWorkflow],
				personalProject.id,
				'workflow:owner',
			);

			// ACT
			const result = await workflowRepository.findOwnerProjectAndArchivedState([
				liveWorkflow.id,
				archivedWorkflow.id,
			]);

			// ASSERT - archived state comes back as a proper boolean regardless of driver
		...
		it('should omit ids that have no workflow row', async () => {
			// ARRANGE
			const { createOwner } = await import('../../shared/db/users.js');
			const { getPersonalProject } = await import('@n8n/backend-test-utils');
			...
			const workflow = await createWorkflow({ name: 'Exists' });
			await shareWorkflowsToProject([workflow], personalProject.id, 'workflow:owner');
		...
		it('should return a null project when no owner share exists', async () => {
			// ARRANGE
			const workflowRepository = Container.get(WorkflowRepository);
			const workflow = await createWorkflow({ name: 'Unowned' });

			// ACT
			const result = await workflowRepository.findOwnerProjectAndArchivedState([workflow.id]);

			// ASSERT
			expect(result).toEqual([{ id: workflow.id, projectId: null, isArchived: false }]);
		});
	});
});
