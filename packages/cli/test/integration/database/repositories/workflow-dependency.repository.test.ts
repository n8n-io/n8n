	...
	describe('WorkflowDependencyRepository', () => {
		...
		describe('updateDependenciesForWorkflow()', () => {
			it('should insert new dependencies for a workflow with no existing dependencies', async () => {
				...
				const dependencies = new WorkflowDependencies(workflow.id, 1, 'draft');
				dependencies.add({
			...
			it('should replace existing dependencies with newer version', async () => {
				...
				const initialDeps = new WorkflowDependencies(workflow.id, 1, 'draft');
				initialDeps.add({
					dependencyType: 'credentialId',
				...
				const updatedDeps = new WorkflowDependencies(workflow.id, 2, 'draft');
				updatedDeps.add({
					dependencyType: 'credentialId',
			...
			it('should not update when incoming version is older than existing version', async () => {
				...
				const newerDeps = new WorkflowDependencies(workflow.id, 2, 'draft');
				newerDeps.add({
					dependencyType: 'credentialId',
					dependencyKey: 'cred-new',
					dependencyInfo: null,
				...
				const olderDeps = new WorkflowDependencies(workflow.id, 1, 'draft');
				olderDeps.add({
					dependencyType: 'credentialId',
					dependencyKey: 'cred-old',
					dependencyInfo: null,
			...
			it('should prevent races between concurrent updates', async () => {
				...
				const depsVersion1 = new WorkflowDependencies(workflow.id, 1, 'draft');
				depsVersion1.add({
					dependencyType: 'credentialId',
				...
				const depsVersion2 = new WorkflowDependencies(workflow.id, 2, 'draft');
				depsVersion2.add({
					dependencyType: 'credentialId',
