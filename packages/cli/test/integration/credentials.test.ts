...
test('POST /credentials should create cred', async () => {
	...
	const response = await utils.createAuthAgent(app, ownerShell).post('/credentials').send(payload);
...
test('DELETE /credentials/:id should delete owned cred for owner', async () => {
	...
	const response = await utils
		.createAuthAgent(app, ownerShell)
		.delete(`/credentials/${savedCredential.id}`);
...
test('DELETE /credentials/:id should fail if cred not found', async () => {
	...
	const response = await utils.createAuthAgent(app, ownerShell).delete('/credentials/123');
