...
describe('AI Helpers Integration Tests', () => {
	...
	describe('Error Handling', () => {
		it('should handle service errors gracefully', async () => {
			const errorService = new ai_helpers_service_1.AiHelpersService(logger, nodeTypes);
			jest.spyOn(errorService, 'suggestNodes').mockRejectedValue(new Error('Service error'));
			const errorController = new ai_helpers_controller_1.AiHelpersController(logger, errorService);
			app.post('/ai-helpers/suggest-nodes-error', async (req, res) => {
				try {
					await errorController.suggestNodes(req, res);
				} catch (error) {
					res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
				}
			});
...
//# sourceMappingURL=ai-helpers.integration.test.js.map
