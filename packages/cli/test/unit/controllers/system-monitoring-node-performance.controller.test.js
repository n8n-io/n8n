...
const system_monitoring_controller_1 = require('@/controllers/system-monitoring.controller');
...
describe('SystemMonitoringController - Node Performance', () => {
	...
	const mockNodePerformanceMetrics = {
		nodeId: mockNodeId,
		nodeType: 'webhook',
	...
	const mockWorkflowNodeBreakdown = {
	...
	const mockNodePerformanceHistory = {
		nodeId: mockNodeId,
		timeRange: '24h',
		metrics: {
	...
	describe('getNodePerformanceMetrics', () => {
		...
		it('should throw BadRequestError for empty workflow ID', async () => {
			await expect(
				controller.getNodePerformanceMetrics(req, res, '', mockNodeId, req.query),
			).rejects.toThrow(bad_request_error_1.BadRequestError);
		});
		it('should throw BadRequestError for empty node ID', async () => {
			await expect(
				controller.getNodePerformanceMetrics(req, res, mockWorkflowId, '', req.query),
			).rejects.toThrow(bad_request_error_1.BadRequestError);
		});
		it('should throw InternalServerError when service fails', async () => {
			...
			await expect(
				controller.getNodePerformanceMetrics(req, res, mockWorkflowId, mockNodeId, req.query),
	...
	describe('getWorkflowNodeBreakdown', () => {
		...
		it('should return workflow node breakdown successfully', async () => {
			...
			const result = await controller.getWorkflowNodeBreakdown(req, res, mockWorkflowId, req.query);
		...
		it('should throw BadRequestError for empty workflow ID', async () => {
			await expect(controller.getWorkflowNodeBreakdown(req, res, '', req.query)).rejects.toThrow(
		...
		it('should throw InternalServerError when service fails', async () => {
			...
			await expect(
				controller.getWorkflowNodeBreakdown(req, res, mockWorkflowId, req.query),
			).rejects.toThrow(internal_server_error_1.InternalServerError);
		});
	});
	describe('getNodeTypePerformance', () => {
		...
		it('should return node type performance successfully', async () => {
			...
			const result = await controller.getNodeTypePerformance(req, res, req.query);
		...
		it('should validate limit parameter', async () => {
			req.query = { limit: '250' };
			await expect(controller.getNodeTypePerformance(req, res, req.query)).rejects.toThrow(
				bad_request_error_1.BadRequestError,
			);
		});
		it('should throw InternalServerError when service fails', async () => {
	...
	describe('getLiveNodeExecution', () => {
		...
		it('should throw BadRequestError for empty execution ID', async () => {
			await expect(controller.getLiveNodeExecution(req, res, '')).rejects.toThrow(
				bad_request_error_1.BadRequestError,
			);
		});
...
//# sourceMappingURL=system-monitoring-node-performance.controller.test.js.map
