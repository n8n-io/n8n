...
describe('SystemMonitoringController', () => {
	...
	describe('getSystemResources', () => {
		...
		it('should return enhanced system resources successfully', async () => {
			systemMonitoringService.getEnhancedSystemResources.mockResolvedValue(mockSystemResources);
			const result = await controller.getSystemResources(req, res, req.query);
		...
		it('should handle default query parameters', async () => {
			req.query = {};
			systemMonitoringService.getEnhancedSystemResources.mockResolvedValue(mockSystemResources);
			await controller.getSystemResources(req, res, req.query);
			expect(systemMonitoringService.getEnhancedSystemResources).toHaveBeenCalledWith({
		...
		it('should throw InternalServerError when service fails', async () => {
			...
			await expect(controller.getSystemResources(req, res, req.query)).rejects.toThrow(
	...
	describe('getSystemHealth', () => {
		it('should return system health status successfully', async () => {
			systemMonitoringService.checkSystemHealth.mockResolvedValue(mockHealthStatus);
		...
		it('should throw InternalServerError when service fails', async () => {
			...
			await expect(controller.getSystemHealth(req, res)).rejects.toThrow(
				internal_server_error_1.InternalServerError,
			);
		});
	});
	describe('getAlerts', () => {
		...
		it('should return alerts successfully', async () => {
			systemMonitoringService.getAlerts.mockReturnValue(mockAlertsResponse);
			const result = await controller.getAlerts(req, res, req.query);
		...
		it('should validate date range', async () => {
			...
			await expect(controller.getAlerts(req, res, req.query)).rejects.toThrow(
	...
	describe('getMetricsHistory', () => {
		...
		it('should return metrics history successfully', async () => {
			...
			systemMonitoringService.getMetricsHistory.mockReturnValue(mockMetricsHistory);
	...
	describe('getWorkflowMetrics', () => {
		...
		it('should return workflow metrics successfully', async () => {
			req.query = { timeRange: '24h' };
			const result = await controller.getWorkflowMetrics(req, res, 'workflow-123', req.query);
		...
		it('should throw BadRequestError for empty workflow ID', async () => {
			req.query = { timeRange: '24h' };
			await expect(controller.getWorkflowMetrics(req, res, '', req.query)).rejects.toThrow(
				bad_request_error_1.BadRequestError,
			);
		});
		it('should validate limit parameter', async () => {
			...
			await expect(
				controller.getWorkflowMetrics(req, res, 'workflow-123', req.query),
	...
	describe('getAlertRules', () => {
		...
		it('should return alert rules successfully', async () => {
			systemMonitoringService.getAlertRules.mockReturnValue(mockAlertRulesResponse);
		...
		it('should throw InternalServerError when service fails', async () => {
			...
			await expect(controller.getAlertRules(req, res)).rejects.toThrow(
				internal_server_error_1.InternalServerError,
			);
		});
	});
	describe('createAlertRule', () => {
		...
		it('should create alert rule successfully', async () => {
			systemMonitoringService.createAlertRule.mockResolvedValue(mockAlertRule);
		...
		it('should validate required fields', async () => {
			...
			await expect(controller.createAlertRule(req, res)).rejects.toThrow(
				bad_request_error_1.BadRequestError,
			);
		});
	...
	describe('updateAlertRule', () => {
		...
		it('should update alert rule successfully', async () => {
			systemMonitoringService.updateAlertRule.mockResolvedValue(mockAlertRule);
		...
		it('should throw BadRequestError for empty rule ID', async () => {
			await expect(controller.updateAlertRule(req, res, '')).rejects.toThrow(
				bad_request_error_1.BadRequestError,
			);
		});
	...
	describe('deleteAlertRule', () => {
		...
		it('should throw BadRequestError for empty rule ID', async () => {
			await expect(controller.deleteAlertRule(req, res, '')).rejects.toThrow(
				bad_request_error_1.BadRequestError,
			);
		});
		it('should throw NotFoundError when rule not found', async () => {
			...
			await expect(controller.deleteAlertRule(req, res, 'rule-123')).rejects.toThrow(
				not_found_error_1.NotFoundError,
			);
		});
	});
	describe('resolveAlert', () => {
		...
		it('should throw BadRequestError for empty alert ID', async () => {
			await expect(controller.resolveAlert(req, res, '')).rejects.toThrow(
				bad_request_error_1.BadRequestError,
			);
		});
	});
	...
	describe('updateMonitoringConfig', () => {
		...
		it('should validate interval range', async () => {
			...
			await expect(controller.updateMonitoringConfig(req, res)).rejects.toThrow(
				bad_request_error_1.BadRequestError,
			);
		});
		it('should validate retention period', async () => {
			req.body = { retentionPeriod: 1000 };
			await expect(controller.updateMonitoringConfig(req, res)).rejects.toThrow(
				bad_request_error_1.BadRequestError,
			);
		});
