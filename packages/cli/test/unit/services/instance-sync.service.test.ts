/**
 * Instance Sync Service Tests
 *
 * Tests for the InstanceSyncService covering cross-instance communication,
 * authentication, data transfer, and error handling scenarios.
 */
...
describe('InstanceSyncService', () => {
	let instanceSyncService: InstanceSyncService;

	beforeEach(() => {
		jest.clearAllMocks();
		instanceSyncService = new InstanceSyncService(
	...
	describe('transferToInstance', () => {
		const mockTransferRequest: CrossInstanceTransferRequestDto = {
		...
		const mockTransferResponse: CrossInstanceTransferResponseDto = {
		...
		it('should successfully transfer data to target instance', async () => {
			// Mock target instance validation
			mockHttpClient.get.mockResolvedValueOnce({
				status: 200,
				data: { status: 'healthy', version: '1.0.0' },
			});

			// Mock import endpoint response
	...
	describe('validateConnection', () => {
		...
		it('should measure connection latency', async () => {
			...
			expect(result.latency).toBeGreaterThanOrEqual(90); // Allow for timing variance
	...
	describe('error handling and logging', () => {
		it('should format duration correctly', async () => {
			const mockTransferRequest: CrossInstanceTransferRequestDto = {
				targetInstanceUrl: 'https://target.example.com',
				targetAuthToken: 'secure-token-123',
				exportData: { metadata: {} as any, workflows: [], credentials: [] },
				conflictResolution: 'skip',
			};
			...
			// Check that duration is logged in a readable format
			expect(mockLogger.info).toHaveBeenCalledWith(
				'Cross-instance transfer completed',
				expect.objectContaining({
					duration: expect.any(Number),
		...
		it('should sanitize URLs in logs', async () => {
			...
			// URL should be sanitized in logs (no credentials, no path)
			expect(mockLogger.error).toHaveBeenCalledWith(
				'Cross-instance transfer failed',
				expect.objectContaining({
