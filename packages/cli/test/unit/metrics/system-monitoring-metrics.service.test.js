...
const system_monitoring_metrics_service_1 = require('@/metrics/system-monitoring-metrics.service');
...
describe('SystemMonitoringMetricsService', () => {
	...
	beforeEach(() => {
		systemMonitoringService = (0, jest_mock_extended_1.mock)();
		prometheusMetricsService = (0, jest_mock_extended_1.mock)();
		service = new system_monitoring_metrics_service_1.SystemMonitoringMetricsService(
	...
	describe('initialization', () => {
		it('should initialize metrics with correct configuration', () => {
			expect(prom_client_1.default.Gauge).toHaveBeenCalledWith(
			...
			expect(prom_client_1.default.Histogram).toHaveBeenCalledWith(
				expect.objectContaining({
		...
		it('should set up metrics with correct prefix', () => {
			const gaugeCall = jest
				.mocked(prom_client_1.default.Gauge)
				.mock.calls.find(
	...
	describe('workflow execution metrics', () => {
		it('should record workflow execution metrics correctly', () => {
			const mockMetrics = service.metrics;
			const executionData = {
				workflowId: 'workflow-123',
				workflowName: 'Test Workflow',
				duration: 5000,
			...
			expect(mockMetrics.workflowExecutionDuration.observe).toHaveBeenCalledWith(5);
	...
	describe('metrics snapshot', () => {
		it('should return metrics snapshot for debugging', async () => {
			...
			jest
				.mocked(prom_client_1.default.register.getMetricsAsJSON)
	...
	describe('metrics collection interval handling', () => {
		it('should handle collection errors during interval execution', async () => {
			systemMonitoringService.getEnhancedSystemResources
				.mockResolvedValueOnce(mockEnhancedResources)
				.mockRejectedValue(new Error('Collection failed'));
			await service.startMetricsCollection(100);
			await new Promise((resolve) => setTimeout(resolve, 150));
			expect(true).toBe(true);
		});
	});
});
//# sourceMappingURL=system-monitoring-metrics.service.test.js.map
