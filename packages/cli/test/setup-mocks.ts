...
// Global console spy to reduce test noise (can be enabled per test if needed)
...
// Enhanced Zod-class mocking to prevent DTO initialization failures
jest.mock('zod-class', () => {
		...
		Z: {
			...
			class: jest.fn((shape) => {
				if (!shape || typeof shape !== 'object') {
					// Return a mock class constructor if shape is invalid
...
// Enhanced Container mocking with proper GlobalConfig support
jest.mock('@n8n/di', () => {
	const actual = jest.requireActual('@n8n/di');

	// Create comprehensive mock config for tests
	const mockGlobalConfig = {
		database: {
			type: 'sqlite', // Default to SQLite for tests
	...
	const mockContainer = {
		get: jest.fn((token) => {
			// Handle GlobalConfig specifically - check both name and constructor
			if (token && (token.name === 'GlobalConfig' || token.constructor?.name === 'GlobalConfig')) {
				return mockGlobalConfig;
			}

			// Handle GlobalConfig token directly if passed as the GlobalConfig class/function
			if (typeof token === 'function' && token.name === 'GlobalConfig') {
				return mockGlobalConfig;
			}

			// Handle other services based on token name or type
			...
			// Return mock instances for function-based tokens
			if (typeof token === 'function') {
				// Check if this might be GlobalConfig by checking the token string
...
// Mock problematic modules that cause test initialization failures
...
// Mock file system operations more comprehensively
...
// Mock @n8n/typeorm to prevent database initialization issues
...
// Mock GlobalConfig to prevent initialization issues at module load time
jest.mock('@n8n/config', () => {
	...
	// Use Object.defineProperty to set the name
...
// Mock EventService to prevent event system initialization issues
...
// Mock License checking to prevent initialization issues
jest.mock('@/license', () => ({
	License: {
