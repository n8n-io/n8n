...
async function benchmarkSQLiteTransfer() {
	const runs = 10;
	const stringTimings = [];
	const objectTimings = [];
	...
	const dbPath = path.join(__dirname, 'ipc-benchmark.db');
	...
	// Fork child process with DB path
	const child = fork(resolve(__dirname, 'ipc-benchmark-sqlite-worker.js'), [dbPath]);
	...
	console.log(`\n=== Running ${runs} string transfer tests (via SQLite) ===`);
	for (let i = 0; i < runs; i++) {
		const start = Date.now();

		// Write to database
		const result = await dbRun('INSERT INTO transfers (type, data) VALUES (?, ?)', [
			'string',
			testString,
		]);
		const rowId = result.lastID;

		// Send row ID to child
		await sendAndWait({ type: 'string', rowId });
	...
	console.log(`\n=== Running ${runs} object transfer tests (via SQLite) ===`);
	for (let i = 0; i < runs; i++) {
		const start = Date.now();
		...
		// Send row ID to child
		await sendAndWait({ type: 'object', rowId });
...
benchmarkSQLiteTransfer().catch((err) => {
