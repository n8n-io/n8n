...
describe('ExternalHooks', () => {
	...
	const workflowData = (0, jest_mock_extended_1.mock)({ id: '123', name: 'Test Workflow' });
	const hookFn = jest.fn();
	let externalHooks;
	beforeEach(() => {
		jest.resetAllMocks();
		globalConfig.externalHooks.files = [];
		externalHooks = new external_hooks_1.ExternalHooks(
			logger,
			errorReporter,
			globalConfig,
			userRepository,
	...
	describe('init()', () => {
		...
		it('should throw an error if hook file cannot be loaded', async () => {
			...
			await expect(externalHooks.init()).rejects.toThrow(n8n_workflow_1.UnexpectedError);
		});
...
//# sourceMappingURL=external-hooks.test.js.map
