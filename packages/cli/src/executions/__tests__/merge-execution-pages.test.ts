...
import { encodeExecutionCursor, parseExecutionCursor } from '../execution-cursor';
...
describe('execution cursor', () => {
	it('round trips positions without authorizing workflows', () => {
		const cursor = {
			version: 1 as const,
	...
	it.each([
		...
		Buffer.from(JSON.stringify({ version: 1, v1: { id: uuid, timestamp: time } })).toString(
			'base64url',
		),
