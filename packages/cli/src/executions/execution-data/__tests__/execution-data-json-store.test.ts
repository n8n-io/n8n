...
/**
 * `ExecutionDataJsonStore` is a config-only subclass of `JsonStore`, whose mechanics are
 * covered by `blob-storage/__tests__/json-store.test.ts`. These tests pin down only the
 * domain configuration: the key layout, the wired error types, and the registered locations.
 */
describe('ExecutionDataJsonStore', () => {
	...
	/**
	 * The key layout and bundle shape are a compatibility contract: every bundle already on
	 * disk or in a bucket lives at this exact key with this exact shape. Changing either
	 * silently orphans all existing data.
	 */
	it('should write the version-1 bundle at the stable execution-data key', async () => {
		...
		const [, body] = fsByteStore.write.mock.calls[0];
