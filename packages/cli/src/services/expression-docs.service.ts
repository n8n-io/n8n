...
@Service()
export class ExpressionDocsService {
	...
	getFunctionDocumentation(
		functionName?: string,
	): FunctionDocumentation[] | FunctionDocumentation | null {
	...
	getVariableDocumentation(
		variableName?: string,
	): VariableDocumentation[] | VariableDocumentation | null {
	...
	/**
	 * Get expression syntax documentation
	 */
	getExpressionSyntaxDocs(topic?: string): ExpressionSyntaxDoc[] | ExpressionSyntaxDoc | null {
	...
	/**
	 * Get contextual documentation based on node type
	 */
	getContextualDocumentation(
		nodeType: string,
		context?: 'input' | 'output' | 'parameters',
	): {
		relevantFunctions: FunctionDocumentation[];
		relevantVariables: VariableDocumentation[];
		contextSpecificTips: string[];
	} {
		const functions = this.getAllFunctionDocs();
		const variables = this.getAllVariableDocs();

		// Filter based on node type and context
		...
		const relevantVariables = variables.filter((variable) => {
			if (!variable.availability?.nodeTypes) return true; // Available everywhere
	...
	searchDocumentation(
		query: string,
		type?: 'functions' | 'variables' | 'syntax',
	): {
		functions: FunctionDocumentation[];
		variables: VariableDocumentation[];
		syntax: ExpressionSyntaxDoc[];
	} {
		const searchTerm = query.toLowerCase();
		const results = {
			functions: [] as FunctionDocumentation[],
			variables: [] as VariableDocumentation[],
			syntax: [] as ExpressionSyntaxDoc[],
		};
	 ...
	 */
	private getAllFunctionDocs(): FunctionDocumentation[] {
		return [
			// Math functions
			{
				name: '$min',
	...
	/**
	 * Get all variable documentation
	 */
	private getAllVariableDocs(): VariableDocumentation[] {
		return [
			{
				name: '$json',
				type: 'object',
	...
	/**
	 * Get all syntax documentation
	 */
	private getAllSyntaxDocs(): ExpressionSyntaxDoc[] {
		return [
			{
