...
- Only return names that appear in that list. When several entries could match one mention, return the single best match — the closest and most complete overlap with what the query said — rather than omitting or returning every candidate. The chosen workflow is shown back to the user in the filter UI, so a near-miss is visible and easy to correct, whereas omitting looks like the filter did nothing at all. Drop a mention only when nothing in the list is a plausible match for it, and omit "workflowNames" entirely if the query names no workflow.
