	...
	tools: [
		new DynamicStructuredTool({
			...
			schema: z.object({
				steps: z
					.array(
						z.string().describe('A clear, action-oriented description of a single workflow step'),
					)
					.min(1)
					.describe(
...
export const connectionComposerAgent = createReactAgent({
	llm: anthropicClaude35Sonnet,
	tools: [
		new DynamicStructuredTool({
...
export const nodesComposerChain = nodesComposerPrompt
	.pipe(
		anthropicClaude35Sonnet.bindTools([generateNodeConfigTool], {
