...
describe('AnalyticsService', () => {
	...
	beforeEach(() => {
		...
		mockErrorAnalyticsService = (0, jest_mock_extended_1.mock)();
	...
	describe('getNodeTypeAnalytics', () => {
		it('should return detailed analytics for a specific node type', async () => {
			...
			mockWorkflowRepository.createQueryBuilder.mockReturnValue(mockWorkflowQueryBuilder);
			mockExecutionRepository.createQueryBuilder.mockReturnValue(mockExecutionQueryBuilder);
	...
	describe('getAllNodeTypes', () => {
		it('should return all node types with basic statistics', async () => {
			...
			mockCacheService.get.mockResolvedValue(null);
			mockCacheService.set.mockResolvedValue();
			const extractNodeTypesSpy = jest.spyOn(service, 'extractNodeTypesFromWorkflows');
			extractNodeTypesSpy.mockResolvedValue(['HTTP Request', 'Code', 'Set']);
			const getExecutionsForNodeTypeSpy = jest.spyOn(service, 'getExecutionsForNodeType');
		...
		it('should calculate error rates correctly', async () => {
			...
			expect(result[0].errorRate).toBe(50.0);
		});
		it('should return cached data when available', async () => {
			const cachedData = [
				{
	...
	describe('performance metrics calculation', () => {
		it('should calculate percentiles correctly', async () => {
			...
			const metrics = await service.calculateNodePerformanceMetrics(executions);
			expect(metrics.p50ExecutionTime).toBe(180000);
			expect(metrics.p95ExecutionTime).toBe(300000);
			expect(metrics.p99ExecutionTime).toBe(300000);
		});
		it('should handle empty execution data', async () => {
			const metrics = await service.calculateNodePerformanceMetrics([]);
	...
	describe('trend calculation', () => {
		...
		it('should calculate stable trend for small changes', () => {
			const trend = service.calculateTrend(102, 100);
			expect(trend).toBe('stable');
		});
		it('should handle inverted trends for error metrics', () => {
			const trend = service.calculateTrend(120, 100, true);
	...
	describe('node scoring', () => {
		it('should calculate node score correctly', () => {
			...
			const score = service.calculateNodeScore(analytics);
		...
		it('should penalize high error rates', () => {
			...
			const highErrorScore = service.calculateNodeScore(highErrorAnalytics);
			const lowErrorScore = service.calculateNodeScore(lowErrorAnalytics);
	...
	describe('default time range', () => {
		it('should use 7 days as default time range', () => {
			const timeRange = service.getDefaultTimeRange();
