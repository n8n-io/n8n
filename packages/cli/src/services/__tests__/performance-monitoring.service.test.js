...
describe('PerformanceMonitoringService', () => {
	...
	beforeEach(() => {
		...
		service = new performance_monitoring_service_1.PerformanceMonitoringService(
			executionService,
			activeExecutions,
		);
	});
	describe('getExecutionProfile', () => {
		it('should generate execution profile with all options enabled', async () => {
			...
			expect(result).toEqual({
				...
				workflowId: 'workflow-456',
				status: api_types_1.ExecutionStatus.SUCCESS,
				timing: {
					...
					nodeExecutions: [
						{
							...
							inputItems: 1,
							outputItems: 1,
							status: api_types_1.NodeExecutionStatus.SUCCESS,
						},
						{
							nodeId: 'node-2',
		...
		it('should handle execution with error status', async () => {
			...
			activeExecutions.has.mockReturnValue(false);
			const result = await service.getExecutionProfile('exec-123');
			expect(result.status).toBe(api_types_1.ExecutionStatus.ERROR);
			expect(result.performance.nodeExecutions[0].status).toBe(
				api_types_1.NodeExecutionStatus.ERROR,
			);
		});
		...
		it('should handle execution without finished timestamp', async () => {
			...
			const result = await service.getExecutionProfile('exec-123');
			expect(result.status).toBe(api_types_1.ExecutionStatus.RUNNING);
		...
		it('should detect bottlenecks for slow execution', async () => {
							...
							'slow-node': [
								{
									...
									executionTime: 35000,
									data: {
										main: [[{ json: { data: 'x'.repeat(1000000) } }]],
									},
									node: {
										type: 'n8n-nodes-base.httpRequest',
									},
			...
			expect(result.bottlenecks[0]).toEqual({
				nodeId: 'slow-node',
				issue: api_types_1.BottleneckIssue.SLOW_EXECUTION,
				severity: api_types_1.BottleneckSeverity.MEDIUM,
		...
		it('should detect multiple bottleneck types', async () => {
							...
							'problematic-node': [
								{
									...
									data: {
										main: [[{ json: { data: 'x'.repeat(15000000) } }]],
									},
									node: {
										type: 'n8n-nodes-base.httpRequest',
									},
			...
			const slowExecutionBottleneck = result.bottlenecks.find(
				(b) => b.issue === api_types_1.BottleneckIssue.SLOW_EXECUTION,
			);
			expect(slowExecutionBottleneck?.severity).toBe(api_types_1.BottleneckSeverity.HIGH);
			const largeDataBottleneck = result.bottlenecks.find(
				(b) => b.issue === api_types_1.BottleneckIssue.LARGE_DATASET,
			);
			expect(largeDataBottleneck?.severity).toBe(api_types_1.BottleneckSeverity.MEDIUM);
		});
	});
	describe('getPerformanceMetrics', () => {
		...
		it('should calculate performance metrics for time range', async () => {
			...
			expect(result.metrics.timing.averageDuration).toBe(105000);
		...
		it('should calculate percentile durations correctly', async () => {
			...
			expect(result.metrics.timing.medianDuration).toBe(50000);
			expect(result.metrics.timing.p95Duration).toBe(95000);
			expect(result.metrics.timing.p99Duration).toBe(99000);
		});
	...
	describe('mapExecutionStatus', () => {
		it('should map all execution status values correctly', () => {
			...
			expect(serviceAny.mapExecutionStatus('new')).toBe(api_types_1.ExecutionStatus.NEW);
			expect(serviceAny.mapExecutionStatus('running')).toBe(api_types_1.ExecutionStatus.RUNNING);
			expect(serviceAny.mapExecutionStatus('success')).toBe(api_types_1.ExecutionStatus.SUCCESS);
			expect(serviceAny.mapExecutionStatus('error')).toBe(api_types_1.ExecutionStatus.ERROR);
			expect(serviceAny.mapExecutionStatus('canceled')).toBe(api_types_1.ExecutionStatus.CANCELED);
			expect(serviceAny.mapExecutionStatus('crashed')).toBe(api_types_1.ExecutionStatus.CRASHED);
			expect(serviceAny.mapExecutionStatus('waiting')).toBe(api_types_1.ExecutionStatus.WAITING);
			expect(serviceAny.mapExecutionStatus('unknown')).toBe(api_types_1.ExecutionStatus.ERROR);
		});
	});
...
//# sourceMappingURL=performance-monitoring.service.test.js.map
