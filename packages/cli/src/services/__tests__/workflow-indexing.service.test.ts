...
describe('WorkflowIndexingService', () => {
	let service: WorkflowIndexingService;
	let mockLogger: jest.Mocked<Logger>;
	let mockSearchEngineService: jest.Mocked<SearchEngineService>;
	...
	beforeEach(() => {
		...
		service = new WorkflowIndexingService(
			mockLogger,
			mockSearchEngineService,
			mockWorkflowRepository,
		);
	...
	describe('Index Initialization', () => {
		...
		it('should handle initialization errors', async () => {
			...
			await expect(service.initializeIndex()).rejects.toThrow(ApplicationError);
	...
	describe('Single Workflow Indexing', () => {
		...
		it('should handle indexing errors', async () => {
			...
			await expect(service.indexWorkflow(mockWorkflow)).rejects.toThrow(ApplicationError);
	...
	describe('Bulk Workflow Indexing', () => {
		...
		it('should process workflows in batches', async () => {
			...
			expect(mockSearchEngineService.bulkIndex).toHaveBeenCalledTimes(3); // 3 batches: 100, 100, 50
		});
	...
	describe('Workflow Removal', () => {
		...
		it('should handle removal errors', async () => {
			...
			await expect(service.removeWorkflow('workflow-1')).rejects.toThrow(ApplicationError);
	...
	describe('Full Reindexing', () => {
		...
		it('should handle reindexing errors', async () => {
			...
			await expect(service.reindexAllWorkflows()).rejects.toThrow(ApplicationError);
	...
	describe('Document Transformation', () => {
		...
		it('should calculate workflow complexity correctly', async () => {
			...
			// Verify complexity calculation includes nodes + connections + conditional/loop node bonuses
			const call = mockSearchEngineService.indexDocument.mock.calls[0][1];
			expect(call.complexity).toBeGreaterThan(4); // Base nodes + bonuses for if/switch/split
		});
