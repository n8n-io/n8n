 ...
 * only the replica that opened the wait is subscribed to that run.
 */
export class PendingWebhookResponse {
	/** Resolves when the run ends, or when the wait runs out — whichever is first. */
	...
	/** Held so an answered run does not leave a timer behind for the whole wait. */
