...
export class TestRunnerService {
	...
	async startTestRun(
		...
		// waiting for cases to finish. The IIFE body executes synchronously
		// until the first `await`, which guarantees `abortControllers` is
		...
		// need to observe completion (tests) await the returned `finished`
		// promise; the controller discards it.
							...
							await limit(async (): Promise<MetricContribution[]> => {
								...
								try {
									try {
										...
										const outputs = this.getEvaluationData(
											testCaseExecution,
											workflow,
											'setOutputs',
										);
