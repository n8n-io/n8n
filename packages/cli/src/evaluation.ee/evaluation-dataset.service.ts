...
export class EvaluationDatasetService {
	...
	async getCandidate(
		...
		const inputs = this.extractInputFields(workflowData, runData, config.startNodeName);
		const outputs = this.extractOutputFields(runData, config.endNodeName);
	...
	async addRow(user: User, workflowId: string, configId: string, dto: AddDatasetRowDto) {
		...
		for (const column of columns) {
			...
			row[column.name] = toDataTableCellValue(value as JsonValue);
		}
...
/** Coerce an execution value into a data table cell value (non-primitives → JSON string). */
function toDataTableCellValue(value: JsonValue): DataTableColumnJsType {
	if (
		typeof value === 'string' ||
		typeof value === 'number' ||
