 ...
 * unknown-scale values are dropped. Booleans coerce to 0/1. `scaleByMetric`
 * comes from the eval config; a metric absent from it falls back to the
 * name-based heuristic. Shared by the run (aggregate) and per-case scoring so
 * the two can't disagree on which metrics count or how they're scaled.
 */
