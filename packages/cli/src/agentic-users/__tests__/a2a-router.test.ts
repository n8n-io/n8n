 ...
 *
 * We mock Container.get to control PublicApiKeyService and AgenticUsersService,
...
const mockAgentsService: Partial<AgenticUsersService> = {
	getAgentCard: jest.fn(),
	executeAgentTask: jest.fn(),
};

// Re-established before every test because the repo's jest config sets
// `restoreMocks: true`, which would otherwise wipe a module-level spy.
beforeEach(() => {
	jest.spyOn(Container, 'get').mockImplementation((token: unknown) => {
		...
		if (name === 'AgenticUsersService') return mockAgentsService;
		return {};
	});
});
...
describe('A2A Router - resolveAgentFromApiKey', () => {
	...
	it('should return 403 when API key lacks required scope', async () => {
		...
		expect(jsonFn).toHaveBeenCalledWith({
			message: 'API key missing agenticUser:receive scope',
		});
	});
