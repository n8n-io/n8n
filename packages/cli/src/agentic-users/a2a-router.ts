...
import type { A2ASendMessageRequest } from '@/agentic-users/a2a-adapter';
...
type ApiKeyScope = 'agenticUser:receive' | 'agenticUser:execute';
...
export function createA2ARouter(): express.Router {
	...
	router.get(
		...
		async (req: express.Request, res: express.Response) => {
			const user = await resolveAgentFromApiKey(req, res, 'agenticUser:receive');
			if (!user) return;
	...
	router.post(
		...
		express.json(),
		async (req: express.Request, res: express.Response) => {
			const user = await resolveAgentFromApiKey(req, res, 'agenticUser:execute');
			if (!user) return;
