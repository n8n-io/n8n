...
export class OpenAiOAuth2DeviceController {
	...
	/** Renders the page itself, so it skips the `send()` wrapper. */
	@Get('/device-auth', { usesTemplates: true })
	async renderDeviceAuthPage(req: OpenAiDeviceAuthRequest, res: ExpressResponse) {
		const credentialId = String(req.query.id ?? '');
		await this.getOpenAiCredential(req, credentialId);

		const challenge = await this.requestDeviceChallenge();
		return res
			.type('html')
			.send(this.renderDevicePage(credentialId, challenge, res.locals.cspNonce));
	}
	...
	async completeDeviceAuth(
		...
		const credential = await this.getOpenAiCredential(req, id);
	...
	private async getOpenAiCredential(req: AuthenticatedRequest, credentialId: string) {
		// `getCredentialForAuthFlow` reads only `user` and `query.id`, and owns the missing-id,
		// scope and not-found handling every other OAuth flow goes through. `/device-complete`
		// carries the credential id in the body, hence the synthesized query.
		const credential = await this.oauthService.getCredentialForAuthFlow({
			user: req.user,
			query: { id: credentialId },
		} as OAuthRequest.OAuth2Credential.Auth);
...
<html>
	...
	<body>
		...
		<script nonce="${cspNonce}">
			...
			let pollTimeout;
			// Bridge to the opener for embed setups where the editor and the n8n backend
			// live on different origins (BroadcastChannel is same-origin only).
			// Not closed afterwards: the "Continue" button can re-run the poll after an
			// error, and posting on a closed channel throws.
			...
			function getBrowserId() {
				const storageKey = '${BROWSER_ID_STORAGE_KEY}';
