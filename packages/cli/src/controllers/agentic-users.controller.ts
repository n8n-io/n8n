import {
	CreateAgenticUserDto,
	DiscoverAgentDto,
	DispatchTaskDto,
	ExternalTaskDto,
	RegisterExternalAgentDto,
	UpdateAgenticUserDto,
...
} from '@/agentic-users/a2a-adapter';
import { validateExternalAgentUrl } from '@/agentic-users/validate-agent-url';
import { BadRequestError } from '@/errors/response-errors/bad-request.error';
import { sendErrorResponse } from '@/response-helper';
import { AgenticUsersService } from '@/services/agentic-users/agents.service';
import type { ExternalAgentConfig } from '@/services/agentic-users/agents.types';
import {
	MAX_ITERATIONS,
	executeTaskOverSse,
	sseWrite,
} from '@/services/agentic-users/agents.types';

@RestController('/agentic-users')
export class AgenticUsersController {
	constructor(private readonly agentsService: AgenticUsersService) {}

	@Post('/')
	@GlobalScope('agenticUser:create')
	async createAgent(
		_req: AuthenticatedRequest,
		_res: Response,
		@Body payload: CreateAgenticUserDto,
	) {
		return await this.agentsService.createAgent(payload);
	}

	@Patch('/:agentId')
	@GlobalScope('agenticUser:update')
	async updateAgent(
		_req: AuthenticatedRequest,
		_res: Response,
		@Param('agentId') agentId: string,
		@Body payload: UpdateAgenticUserDto,
	) {
		return await this.agentsService.updateAgent(agentId, payload);
	}

	@Delete('/:agentId')
	@GlobalScope('agenticUser:delete')
