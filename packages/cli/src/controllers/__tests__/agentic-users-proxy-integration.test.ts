...
import { AgenticUsersController } from '../agentic-users.controller';
import type { AgenticUsersService } from '@/services/agentic-users/agents.service';

// Mock SSRF validation — integration tests use localhost
jest.mock('@/agentic-users/validate-agent-url', () => ({
...
describe('proxyExternalTask integration', () => {
	const agentsService = mock<AgenticUsersService>();
	let controller: AgenticUsersController;

	beforeEach(() => {
		jest.clearAllMocks();
		controller = new AgenticUsersController(agentsService);
