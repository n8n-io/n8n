...
export = {
	installPackage: [
		...
		async (
			...
			try {
				const installedPackage = await lifecycle.installWithSideEffects(
	...
	getInstalledPackages: [
		...
		async (
			...
			const lifecycle = Container.get(CommunityPackagesLifecycleService);
			const packages = await lifecycle.listInstalledPackagesHydrated();
	...
	updatePackage: [
		...
		async (
			...
			try {
				const updated = await lifecycle.updateWithSideEffects(
					{
	...
	uninstallPackage: [
		...
		async (
			...
			try {
				await lifecycle.uninstallWithSideEffects(req.params.packageName, req.user, 'notFound');
