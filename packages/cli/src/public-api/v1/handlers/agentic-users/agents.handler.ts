...
import { MAX_ITERATIONS, executeTaskOverSse } from '@/services/agentic-users/agents.types';

import { publicApiScope } from '../../shared/middlewares/global.middleware';

export = {
	getAgentCard: [
		publicApiScope('agenticUser:read'),
		async (
			req: AuthenticatedRequest<{ id: string }>,
			res: express.Response,
		): Promise<express.Response> => {
			const agentsService = Container.get(AgenticUsersService);
	...
	dispatchAgentTask: [
		publicApiScope('agenticUser:execute'),
