...
import {
	paginateSourceControlledFiles,
	sortSourceControlledFiles,
} from '../source-controlled-file-pagination.service';
...
describe('sortSourceControlledFiles', () => {
	it('sorts by type first', () => {
		...
		const sorted = sortSourceControlledFiles(files);

		expect(sorted.map((f) => f.type)).toEqual(['credential', 'workflow']);
	});
	...
	it('sorts by file within the same type', () => {
		...
		expect(sorted.map((f) => f.file)).toEqual(['a.json', 'b.json']);
	});
	...
	it('does not mutate the input array', () => {
		...
		sortSourceControlledFiles(files);
	...
	it('produces the same order regardless of input order (stable across pages)', () => {
		...
		expect(sortSourceControlledFiles(files)).toEqual(sortSourceControlledFiles(shuffled));
	});
});
...
describe('paginateSourceControlledFiles', () => {
	...
	it('paging through with the same page size covers every item exactly once', () => {
		...
		expect([...page1.data, ...page2.data]).toEqual(sortSourceControlledFiles(files));
	});
});
