...
describe('PublicApiControllerRegistry', () => {
	...
	describe('query parameters', () => {
		...
		it('rejects an undeclared key on a route with a query DTO', async () => {
			...
			const response = await request(activate()).get('/api/v1/widgets?bogus=1').expect(400);
		...
		it('rejects an undeclared key next to a declared one', async () => {
			...
			const response = await request(activate()).get('/api/v1/widgets?limit=1&bogus=1').expect(400);
		...
		it('rejects any key on a route without a query DTO', async () => {
			...
			const response = await request(activate()).get('/api/v1/widgets/abc?bogus=1').expect(400);
		...
		it('passes a declared key to the handler parsed', async () => {
			...
			const response = await request(activate()).get('/api/v1/widgets?limit=5').expect(200);
		...
		it('accepts an empty query on a route with a query DTO', async () => {
			...
			await request(activate()).get('/api/v1/widgets').expect(200);
		});
