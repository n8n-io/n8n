...
const helpers = __importStar(require('@/ldap.ee/helpers.ee'));
...
describe('Ldap/helpers', () => {
	describe('updateLdapUserOnLocalDb', () => {
		test('does not use `Repository.update`, but `Repository.save` instead', async () => {
			const user = Object.assign(new db_1.User(), { id: (0, db_1.generateNanoId)() });
			const authIdentity = Object.assign(new db_1.AuthIdentity(), {
				user: { id: user.id },
			});
			const data = { firstName: 'Nathan', lastName: 'Nathaniel' };
