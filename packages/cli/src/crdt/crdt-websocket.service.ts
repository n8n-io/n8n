...
export class CRDTWebSocketService {
	...
	private async handleConnection(req: CRDTWebSocketRequest, res: ServerResponse) {
		...
		this.sendUpdate(ws, initialState);
	...
	private handleMessage(connection: CRDTConnection, data: ArrayBuffer | Buffer) {
		...
		this.state.applyUpdateBytes(docId, update);
		this.broadcastUpdate(docId, update, connection);
	}

	/**
	 * Send raw CRDT update bytes to a client.
	 */
	private sendUpdate(ws: WebSocket, data: Uint8Array) {
		if (ws.readyState === ws.OPEN) {
			ws.send(data);
		}
	}

	private broadcastUpdate(docId: string, update: Uint8Array, sender: CRDTConnection) {
		...
		for (const conn of docConnections) {
			if (conn !== sender && conn.ws.readyState === conn.ws.OPEN) {
				this.sendUpdate(conn.ws, update);
			}
		}
	}
