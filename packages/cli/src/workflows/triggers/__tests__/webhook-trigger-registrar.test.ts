...
describe('WebhookTriggerRegistrar', () => {
	...
	describe('registerForNode', () => {
		const buildRegistrar = (webhookService: MockProxy<WebhookService>) => {
			webhookService.createWebhook.mockImplementation(
				(data) => ({ webhookPath: data.webhookPath, node: data.node }) as WebhookEntity,
			);
		...
		const webhookFor = (path: string) =>
			mock<IWebhookData>({ node: 'Webhook', workflowId: 'wf-1', httpMethod: 'GET', path });

		const workflow = () => createWorkflow([node('webhook-node', 'webhook', { name: 'Webhook' })]);

		test('registers each webhook of the node', async () => {
			const webhookService = mock<WebhookService>();
			const registrar = buildRegistrar(webhookService);

			await registrar.registerForNode({
				workflow: workflow(),
				webhooks: [webhookFor('a'), webhookFor('b')],
				mode: 'trigger',
				activation: 'update',
			});
		...
		test('propagates a registration failure to the caller', async () => {
			const webhookService = mock<WebhookService>();
			webhookService.storeWebhook.mockRejectedValue(new Error('registration failed'));
			const registrar = buildRegistrar(webhookService);

			await expect(
				registrar.registerForNode({
					workflow: workflow(),
					webhooks: [webhookFor('a')],
					mode: 'trigger',
					activation: 'update',
				}),
			).rejects.toThrow('registration failed');

			// No retry here; that is the activator's concern.
			expect(webhookService.storeWebhook).toHaveBeenCalledTimes(1);
		});

		test('throws WebhookPathTakenError on a deterministic path conflict', async () => {
			const webhookService = mock<WebhookService>();
			const dbError = new Error('duplicate');
			dbError.name = 'QueryFailedError';
		...
		test('leaves earlier webhooks registered when a later one fails', async () => {
			const webhookService = mock<WebhookService>();
			const dbError = new Error('duplicate');
			dbError.name = 'QueryFailedError';
			// First webhook stores; second hits a deterministic conflict.
			webhookService.storeWebhook.mockResolvedValueOnce().mockRejectedValueOnce(dbError);
			const registrar = buildRegistrar(webhookService);
			...
			// The first webhook is stored and not torn down by the second's failure.
			expect(webhookService.storeWebhook).toHaveBeenCalledTimes(2);
			expect(webhookService.deleteWebhook).not.toHaveBeenCalled();
		});
	});
