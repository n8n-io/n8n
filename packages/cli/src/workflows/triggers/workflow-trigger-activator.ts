...
export class WorkflowTriggerActivator {
	...
	async updateTriggerCount(dbWorkflow: WorkflowEntity, version: WorkflowTriggerVersion) {
		const startedAt = Date.now();
		try {
			await this.updateTriggerCountInternal(dbWorkflow, version);
			this.emitTriggerOperation('update_count', 'success', startedAt);
		} catch (error) {
			this.emitTriggerOperation('update_count', 'failure', startedAt);
			throw error;
		}
	}

	private async updateTriggerCountInternal(
		dbWorkflow: WorkflowEntity,
		version: WorkflowTriggerVersion,
	) {
		await this.tracing.startSpan(
