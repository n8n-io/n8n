...
describe('WorkflowHistoryController', () => {
	...
	describe('updateVersion', () => {
		it('should call historyService.updateVersionWithUser with correct parameters', async () => {
			// Arrange
			const req = mock<WorkflowHistoryRequest.GetVersion>({
				user: mockUser,
				params: { workflowId: 'workflow123', versionId: 'version456' },
			});
			const body = { name: 'New Version Name', description: 'New description' };
			...
			historyService.updateVersionWithUser.mockResolvedValue(mockUpdatedVersion as any);

			// Act
			const result = await controller.updateVersion(req, mock(), body);

			// Assert
			expect(historyService.updateVersionWithUser).toHaveBeenCalledWith(
				mockUser,
				'workflow123',
				'version456',
				{ name: 'New Version Name', description: 'New description' },
			);
			expect(result).toEqual(mockUpdatedVersion);
		});

		it('should call historyService.updateVersionWithUser with only name', async () => {
			// Arrange
			const req = mock<WorkflowHistoryRequest.GetVersion>({
				user: mockUser,
				params: { workflowId: 'workflow123', versionId: 'version456' },
			});
			const body = { name: 'New Version Name' };
			const mockUpdatedVersion = {
				versionId: 'version456',
				workflowId: 'workflow123',
				name: 'New Version Name',
		...
		it('should throw NotFoundError when SharedWorkflowNotFoundError is caught', async () => {
			...
			historyService.updateVersionWithUser.mockRejectedValue(new SharedWorkflowNotFoundError(''));

			// Act & Assert
			await expect(controller.updateVersion(req, mock(), body)).rejects.toThrow(NotFoundError);
			await expect(controller.updateVersion(req, mock(), body)).rejects.toThrow(
				'Could not find workflow',
			);
		});
		...
		it('should throw NotFoundError when WorkflowHistoryVersionNotFoundError is caught', async () => {
			...
			historyService.updateVersionWithUser.mockRejectedValue(
		...
		it('should re-throw other errors', async () => {
			...
			historyService.updateVersionWithUser.mockRejectedValue(customError);

			// Act & Assert
			await expect(controller.updateVersion(req, mock(), body)).rejects.toThrow(customError);
		});
	});
});
