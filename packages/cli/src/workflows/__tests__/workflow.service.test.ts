...
describe('WorkflowService', () => {
	...
	describe('unpublish via outbox', () => {
		...
		function makeWorkflowEntity(overrides: Partial<WorkflowEntity> = {}): WorkflowEntity {
			...
			workflow.name = 'My workflow';
			workflow.isArchived = false;
			workflow.versionId = ACTIVE_VERSION_ID;
		...
		function enablePublicationService(enabled: boolean) {
			globalConfigMock.workflows.useWorkflowPublicationService = enabled;
		}
		...
		describe('deactivateWorkflow', () => {
			test('with the flag enabled, nulls the version and enqueues without touching the active workflow manager', async () => {
				enablePublicationService(true);
				const workflow = makeWorkflowEntity();
				workflowFinderServiceMock.findWorkflowForUser.mockResolvedValue(workflow);

				await workflowService.deactivateWorkflow(mock<User>({ id: 'user-1' }), WORKFLOW_ID);
				...
				expect(workflowPublishHistoryRepositoryMock.addRecord).toHaveBeenCalledWith(
					expect.objectContaining({ event: 'deactivated', versionId: ACTIVE_VERSION_ID }),
					trx,
				);
				expect(activeWorkflowManagerMock.remove).not.toHaveBeenCalled();
			});

			test('with the flag disabled, keeps the synchronous teardown and does not enqueue', async () => {
				enablePublicationService(false);
				const workflow = makeWorkflowEntity();
				workflowFinderServiceMock.findWorkflowForUser.mockResolvedValue(workflow);
		...
		describe('archive', () => {
			test('with the flag enabled, enqueues unpublish instead of synchronous teardown', async () => {
				enablePublicationService(true);
				const workflow = makeWorkflowEntity();
				workflowFinderServiceMock.findWorkflowForUser.mockResolvedValue(workflow);

				await workflowService.archive(mock<User>({ id: 'user-1' }), WORKFLOW_ID);
				...
				expect(activeWorkflowManagerMock.remove).not.toHaveBeenCalled();
				// the workflow is still archived
				expect(workflowRepositoryMock.update).toHaveBeenCalledWith(
					WORKFLOW_ID,
		...
		describe('delete', () => {
			test('keeps synchronous teardown and never enqueues, even with the flag enabled', async () => {
				enablePublicationService(true);
				const workflow = makeWorkflowEntity({ isArchived: true });
				workflowFinderServiceMock.findWorkflowForUser.mockResolvedValue(workflow);

				await workflowService.delete(mock<User>({ id: 'user-1' }), WORKFLOW_ID);
