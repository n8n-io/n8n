...
class App {
	...
	async config(): Promise<void> {
		...
		this.app.post(`/${this.restEndpoint}/credentials`, ResponseHelper.send(async (req: express.Request, res: express.Response): Promise<ICredentialsResponse> => {
			...
			const numberOfCredentials = await Db.collections.Credentials!.count();
			const credentialsLimit = config.get('credentials.limit') as number;

			if (credentialsLimit > 0 && numberOfCredentials >= credentialsLimit) {
				throw new ResponseHelper.ResponseError(`Credentials limit reached.`, undefined, 400);
			}
