...
class App {
	...
	async config(): Promise<void> {
		...
		this.app.get([`/${this.restEndpoint}/credential-icon/:credentialType`, `/${this.restEndpoint}/credential-icon/:scope/:credentialType`], async (req: express.Request, res: express.Response): Promise<void> => {
			try {
				const credentialName = `${req.params.scope ? `${req.params.scope}/` : ''}${req.params.credentialType}`;
