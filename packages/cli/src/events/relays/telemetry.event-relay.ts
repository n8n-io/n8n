...
export class TelemetryEventRelay extends EventRelay {
	...
	async init() {
		...
		// Original code below, but never executed
		...
		this.telemetry.identify({
			...
			n8n_binary_data_mode: config.getEnv('binaryDataManager.mode'),
			smtp_set_up: this.globalConfig.userManagement.emails.mode === 'smtp',
			ldap_allowed: config.getEnv('userManagement.authenticationMethod') === 'ldap',
			saml_enabled: config.getEnv('userManagement.authenticationMethod') === 'saml',
			license_plan_name: this.license.getPlanName(),
			license_tenant_id: this.globalConfig.license.tenantId,
			binary_data_s3: config.getEnv('binaryDataManager.availableModes').includes('s3'),
		...
		this.telemetry.track('Instance started', {
			earliest_workflow_created: await this.workflowRepository.findOne({
				select: ['createdAt'],
				order: { createdAt: 'ASC' },
				where: {},
			}),
