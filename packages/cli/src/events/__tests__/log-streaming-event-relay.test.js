...
describe('LogStreamingEventRelay', () => {
	const eventBus = (0, jest_mock_extended_1.mock)();
	...
	new log_streaming_event_relay_1.LogStreamingEventRelay(
		eventService,
		eventBus,
		instanceSettings,
	).init();
	...
	describe('workflow events', () => {
		...
		it('should log on `workflow-saved` event', () => {
			const event = {
				...
				workflow: (0, jest_mock_extended_1.mock)({ id: 'wf101', name: 'Updated Workflow' }),
				publicApi: false,
			};
		...
		it('should log on `workflow-post-execute` for successful execution', () => {
			const payload = (0, jest_mock_extended_1.mock)({
				executionId: 'some-id',
				userId: 'some-id',
				workflow: (0, jest_mock_extended_1.mock)({ id: 'some-id', name: 'some-name' }),
				runData: (0, jest_mock_extended_1.mock)({
					finished: true,
					status: 'success',
					mode: 'manual',
					data: { resultData: {} },
		...
		it('should log job completion on `workflow-post-execute` for successful job', () => {
			...
			const event = {
				...
				userId: 'user-456',
				workflow: (0, jest_mock_extended_1.mock)({ id: 'wf-789', name: 'Test Workflow' }),
				runData,
			};
			...
			const runData = (0, jest_mock_extended_1.mock)({
				...
				data: {
					resultData: {
						...
						error: {
							node: (0, jest_mock_extended_1.mock)({ type: 'some-type' }),
		...
		it('should log job failure on `workflow-post-execute` for failed job', () => {
			...
			const event = {
				...
				userId: 'user-789',
				workflow: (0, jest_mock_extended_1.mock)({ id: 'wf-101', name: 'Failed Workflow' }),
				runData,
			};
			eventService.emit('workflow-post-execute', event);
			expect(eventBus.sendQueueEvent).toHaveBeenCalledWith({
...
//# sourceMappingURL=log-streaming-event-relay.test.js.map
