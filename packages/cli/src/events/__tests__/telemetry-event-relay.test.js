...
describe('TelemetryEventRelay', () => {
	...
	const projectRelationRepository = (0, jest_mock_extended_1.mock)();
	const credentialsRepository = (0, jest_mock_extended_1.mock)();
	const eventService = new event_service_1.EventService();
	let telemetryEventRelay;
	beforeAll(async () => {
		telemetryEventRelay = new telemetry_event_relay_1.TelemetryEventRelay(
			eventService,
			telemetry,
			license,
			globalConfig,
	...
	describe('init', () => {
		it('with diagnostics enabled, should init telemetry and register listeners', async () => {
			globalConfig.diagnostics.enabled = true;
			const telemetryEventRelay = new telemetry_event_relay_1.TelemetryEventRelay(
				eventService,
				telemetry,
				license,
				globalConfig,
	...
	describe('lifecycle events', () => {
		it('should track on `server-started` event', async () => {
			const firstWorkflow = (0, jest_mock_extended_1.mock)({ createdAt: new Date() });
	...
	describe('workflow post execute events', () => {
		...
		const mockWorkflowBase = (0, jest_mock_extended_1.mock)({
			id: 'workflow123',
			name: 'Test Workflow',
			active: true,
			nodes: [
		...
		it('should call telemetry.track when manual node execution finished', async () => {
			...
			credentialsRepository.findOneBy.mockResolvedValue(
				(0, jest_mock_extended_1.mock)({ type: 'openAiApi', isManaged: false }),
			);
			const runData = {
				status: 'error',
				mode: 'manual',
				data: {
					...
					resultData: {
						...
						error: new n8n_workflow_1.NodeApiError(
							{
								id: '1',
								typeVersion: 1,
								name: 'Jira',
			...
			jest
				.spyOn(n8n_workflow_1.TelemetryHelpers, 'generateNodesGraph')
				.mockImplementation(() => nodeGraph);
			jest
				.spyOn(n8n_workflow_1.TelemetryHelpers, 'getNodeTypeForName')
				.mockImplementation(() => ({ type: 'n8n-nodes-base.jira', version: 1, name: 'Jira' }));
			const event = {
				workflow: mockWorkflowBase,
				executionId: 'execution123',
				userId: 'user123',
		...
		it('should call telemetry.track when manual node execution finished with is_managed and credential_type properties', async () => {
			sharedWorkflowRepository.findSharingRole.mockResolvedValue('workflow:editor');
			credentialsRepository.findOneBy.mockResolvedValue(
				(0, jest_mock_extended_1.mock)({ type: 'openAiApi', isManaged: true }),
			);
			const runData = {
				status: 'error',
				mode: 'manual',
		...
		it('should call telemetry.track when user ran out of free AI credits', async () => {
			...
			jest
				.spyOn(n8n_workflow_1.TelemetryHelpers, 'userInInstanceRanOutOfFreeAiCredits')
...
//# sourceMappingURL=telemetry-event-relay.test.js.map
