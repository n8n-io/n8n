...
let CredentialsService = class CredentialsService {
	...
	async validateCredentialFields(credentialType, credentialData) {
		const issues = [];
		try {
	...
	async analyzeCredentialFields(credentialType, credentialData) {
	...
	async diagnoseConnection(userId, credentials) {
		const suggestions = [];
		const details = {};
		try {
	...
	async generateCredentialRecommendations(credentialType, credentialData) {
		const recommendations = [];
		try {
			const credType = this.credentialTypes.getByName(credentialType);
			recommendations.push({
	...
	async generateTroubleshootingSteps(credentialType, errorMessage) {
		const steps = [];
		try {
			steps.push({
				step: 1,
