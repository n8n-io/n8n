...
describe('CredentialsService', () => {
	...
	describe('redact', () => {
		...
		describe('httpCustomAuth redactJsonLeaves (via service.redact)', () => {
			...
			const makeCredType = () => ({ properties: [] }) as unknown as ICredentialType;
			...
			it('should fall back to CREDENTIAL_BLANKING_VALUE for non-parseable JSON', () => {
				credentialTypes.getByName.calledWith('httpCustomAuth').mockReturnValueOnce(makeCredType());

				const result = service.redact({ json: 'not-valid-json' }, makeHttpCustomAuthCredential());

				expect(result.json).toEqual(CREDENTIAL_BLANKING_VALUE);
			});
	...
	describe('unredact (httpCustomAuth JSON shape merging)', () => {
		...
		it('should not apply JSON leaf merge when unredactJsonLeaves is false', () => {
			const result = service.unredact(
				{ json: s({ token: '***' }) },
				{ json: s({ token: 'saved' }) },
				false,
		...
		it('should not apply JSON leaf merge when no credentialType is given', () => {
			const result = service.unredact(
				{ json: s({ token: '***' }) },
				{ json: s({ token: 'saved' }) },
			);
