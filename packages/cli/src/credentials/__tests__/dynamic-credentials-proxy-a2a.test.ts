...
describe('DynamicCredentialsProxy — A2A inline resolution', () => {
	...
	it('should merge BYOK credentials with static data for A2A context', async () => {
		...
		expect(result).toEqual({
			data: { baseUrl: 'https://slack.com', token: 'consumer-slack-token' },
			isDynamic: false,
		});
	...
	it('should return BYOK credentials even when resolvingProvider is NOT set (no EE module)', async () => {
		...
		expect(result).toEqual({
			data: { baseUrl: 'https://slack.com', token: 'byok-token' },
			isDynamic: false,
		});
	});
	...
	it('should delegate to resolvingProvider for non-A2A context', async () => {
		...
		const mockProvider: jest.Mocked<ICredentialResolutionProvider> = {
			resolveIfNeeded: jest
				.fn()
				.mockResolvedValue({ data: { token: 'ee-resolved' }, isDynamic: true }),
		...
		expect(result).toEqual({ data: { token: 'ee-resolved' }, isDynamic: true });
	...
	it('should resolve different credential types independently from same A2A context', async () => {
		...
		expect(slackResult).toEqual({
			data: { baseUrl: 'https://slack.com', token: 'slack-token' },
			isDynamic: false,
		});
		expect(notionResult).toEqual({
			data: { baseUrl: 'https://notion.so', apiKey: 'notion-key' },
			isDynamic: false,
		});
	});
