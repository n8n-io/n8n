jest.mock('@/modules/dynamic-credentials.ee/dynamic-credentials.config', () => ({
	DYNAMIC_CREDENTIALS_ALLOWED_NODE_TYPES: ['n8n-nodes-base.webhook', 'webhook'],
}));
...
describe('DynamicCredentialsProxy', () => {
	...
	describe('shouldBeResolved', () => {
		const mockCredentialsEntity: ICredentialsDb = {
			id: 'cred-123',
			name: 'Test Credential',
			type: 'oAuth2Api',
			isResolvable: false,
		...
		it('should return false when credential is not resolvable', () => {
			const result = DynamicCredentialsProxy.shouldBeResolved(mockCredentialsEntity);

			expect(result).toBe(false);
		});

		it('should return false when executionContext is undefined', () => {
			const resolvableCredential = { ...mockCredentialsEntity, isResolvable: true };

			const result = DynamicCredentialsProxy.shouldBeResolved(resolvableCredential, undefined);

			expect(result).toBe(false);
		});

		it('should return false when triggerNode is undefined', () => {
			const resolvableCredential = { ...mockCredentialsEntity, isResolvable: true };
			const executionContext: IExecutionContext = {
				version: 1,
				establishedAt: Date.now(),
			...
			const result = DynamicCredentialsProxy.shouldBeResolved(
				resolvableCredential,
				executionContext,
			);
		...
		it('should return false when triggerNode.type is undefined', () => {
			const resolvableCredential = { ...mockCredentialsEntity, isResolvable: true };
			const executionContext: IExecutionContext = {
				version: 1,
				establishedAt: Date.now(),
		...
		it('should return false when triggerNode.type is not in allowed node types', () => {
			const resolvableCredential = { ...mockCredentialsEntity, isResolvable: true };
			const executionContext: IExecutionContext = {
				version: 1,
				establishedAt: Date.now(),
		...
		it('should return true when credential is resolvable and triggerNode.type is webhook', () => {
			const resolvableCredential = { ...mockCredentialsEntity, isResolvable: true };
			const executionContext: IExecutionContext = {
				version: 1,
				establishedAt: Date.now(),
		...
		it('should return true when credential is resolvable and triggerNode.type is n8n-nodes-base.webhook', () => {
			const resolvableCredential = { ...mockCredentialsEntity, isResolvable: true };
			const executionContext: IExecutionContext = {
				version: 1,
				establishedAt: Date.now(),
