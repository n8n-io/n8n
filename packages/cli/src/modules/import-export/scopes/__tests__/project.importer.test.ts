...
describe('ProjectImporter', () => {
	...
	it('should reuse an existing project with the same name without overwriting metadata', async () => {
		const entry = makeEntry();
		...
		// Reuse path must not clobber icon/description set on the target instance
	...
	it('should run the import pipeline with entity entries', async () => {
		...
		expect(mockImportPipeline.importEntities).toHaveBeenCalledWith(
			...
			expect.objectContaining({
				...
				subWorkflowBindings: undefined,
			}),
		);
	});

	it('should pass resolved bindings as the pipeline seed', async () => {
		const entry = makeEntry();
		const createdProject = { id: 'new-project-1', name: 'billing' } as Project;
