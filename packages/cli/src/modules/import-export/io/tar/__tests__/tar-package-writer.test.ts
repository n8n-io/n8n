...
describe('TarPackageWriter', () => {
	...
	it('should emit manifest as the first entry regardless of write order', async () => {
		const writer = new TarPackageWriter();
		// Write entries in "wrong" order — manifest comes last
		writer.writeDirectory('projects/a');
		writer.writeFile('projects/a/project.json', '{"id":"a"}');
		writer.writeFile('projects/a/extra.json', '{}');
