...
describe('FolderExporter', () => {
	...
	it('should return empty result when project has no folders', async () => {
		mockFolderRepository.find.mockResolvedValue([]);

		const result = await exporter.export(scope);

		expect(result.entries).toEqual([]);
		expect(result.folderPathMap.size).toBe(0);
	...
	it('should export a root folder', async () => {
		...
		expect(result.entries[0].id).toBe(folder.id);
		expect(result.entries[0].name).toBe('invoices');
		expect(result.entries[0].target).toBe('projects/billing-550e84/folders/invoices-aabb11');
	...
	it('should return folderPathMap in result', async () => {
		...
		expect(result.folderPathMap.get(folder.id)).toBe(
			'projects/billing-550e84/folders/invoices-aabb11',
		);
	});
	...
	it('should nest child folders under their parent', async () => {
		...
		const parentEntry = result.entries.find((e) => e.id === parentFolder.id)!;
		const childEntry = result.entries.find((e) => e.id === childFolder.id)!;
	...
	it('should handle deeply nested folders', async () => {
		...
		const leafEntry = result.entries.find((e) => e.id === leaf.id)!;
	...
	describe('folderIds scope', () => {
		...
		it('should fetch target folders and their descendants', async () => {
			...
			expect(result.entries).toHaveLength(2);
			expect(result.entries.map((e) => e.id).sort()).toEqual(['folder-1', 'folder-2']);
		});
