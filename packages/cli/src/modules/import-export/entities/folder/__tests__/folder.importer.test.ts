...
describe('FolderImporter', () => {
	...
	it('should do nothing when entries is empty', async () => {
		...
		expect(result.folderIdMap.size).toBe(0);
	});
	...
	it('should create a root folder when no existing match', async () => {
		...
		expect(result.folderIdMap.get('source-folder-1')).toBe('new-folder-1');
	});
	...
	it('should reuse an existing folder with the same name and parent', async () => {
		...
		expect(result.folderIdMap.get('source-folder-1')).toBe('existing-folder-1');
	});
	...
	it('should import nested folders in correct order (parent before child)', async () => {
		...
		expect(result.folderIdMap.get('parent')).toBe('new-parent');
		expect(result.folderIdMap.get('child')).toBe('new-child');
	});
	...
	describe('assignNewIds', () => {
		...
		it('should save folders with deterministic IDs', async () => {
			...
			expect(mockFolderRepository.save).toHaveBeenCalled();
			expect(result.folderIdMap.get('folder-1')).toBe('new-project-1-folder-1');
		});
	...
	it('should handle deeply nested folders', async () => {
		...
		expect(result.folderIdMap.get('root')).toBe('new-root');
		expect(result.folderIdMap.get('mid')).toBe('new-mid');
		expect(result.folderIdMap.get('leaf')).toBe('new-leaf');
	});
});
