...
export interface FolderImportResult {
	/** Source folder ID → created/matched folder ID. Consumed by WorkflowImporter. */
...
export class FolderImporter {
	 ...
	 */
	async import(scope: ImportScope, entries: ManifestEntry[]): Promise<FolderImportResult> {
		const folderIdMap = new Map<string, string>();
		if (entries.length === 0) return { folderIdMap };
