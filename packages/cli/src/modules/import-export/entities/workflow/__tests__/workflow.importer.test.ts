...
import { WorkflowImporter, type WorkflowImportDeps } from '../workflow.importer';
...
describe('WorkflowImporter', () => {
	...
	beforeEach(() => {
		...
		deps = {
			...
			tagsBySourceId: new Map(),
		};
	});

	it('should do nothing when entries is empty', async () => {
		await importer.import(scope, [], deps);

		expect(mockImportService.importWorkflows).not.toHaveBeenCalled();
	});

	it('should attach versionMetadata describing the package import', async () => {
		...
		await importer.import(scope, entries, deps);

		const imported = mockImportService.importWorkflows.mock.calls[0][0][0] as unknown as Record<
			string,
			unknown
		>;
		expect(imported.versionMetadata).toMatchObject({
			name: 'Imported from package',
			description: expect.stringContaining('source workflow id: wf-1'),
		});
	});
	...
	it('should produce same ID on repeated imports (idempotent)', async () => {
		...
		deps.subWorkflowBindings = new Map();
		await importer.import(scope, entries, deps);
	...
	it('should track old→new ID mapping in deps.subWorkflowBindings when assignNewIds is true', async () => {
		scope.assignNewIds = true;
		...
		expect(deps.subWorkflowBindings.get('wf-1')).toBe('new-project-1-wf-1');
	});

	it('should import workflows with remapped folder IDs', async () => {
		deps.folderIdMap.set('source-folder-1', 'new-folder-1');
	...
	it('should remap parentFolderId to deterministic folder IDs when assignNewIds is true', async () => {
		...
		deps.folderIdMap.set('folder-1', 'new-project-1-folder-1');
	...
	describe('credential remapping', () => {
		it('should remap credential IDs when bindings are provided', async () => {
			deps.credentialBindings = new Map([['source-cred-1', 'target-cred-1']]);
		...
		it('should leave unmapped credential IDs unchanged', async () => {
			deps.credentialBindings = new Map([['other-cred', 'target-cred']]);
	...
	describe('sub-workflow remapping', () => {
		it('should remap sub-workflow IDs using bindings (string format)', async () => {
			deps.subWorkflowBindings = new Map([['external-wf', 'resolved-external-wf']]);
		...
		it('should not remap executeWorkflow with source=parameter', async () => {
			deps.subWorkflowBindings = new Map([['source-wf-2', 'target-wf-2']]);
