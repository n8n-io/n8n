...
export class InstanceAiPendingConfirmationRepository extends Repository<InstanceAiPendingConfirmation> {
	...
	/**
	 * True when the run still has an actionable (not past-`expiresAt`)
	 * confirmation row — i.e. its HITL wait can still be claimed and resumed
	 * through the orphan path. Same liveness predicate as `claim` /
	 * `findLiveRequestIds`.
	 */
	async hasActionableForRun(runId: string, now: Date): Promise<boolean> {
