...
describe('parseStoredMessages', () => {
	...
	describe('assistant messages', () => {
		it('should parse assistant message with text only', () => {
			...
			// The agent tree comes only from a paired fold-derived snapshot; a
			// snapshot-less row renders as plain text with no tree.
			expect(assistant.agentTree).toBeUndefined();
		});

		it('should extract text but no tree from a snapshot-less row with unrecognized content parts', () => {
			const messages: StoredAgentMessage[] = [
				{
					id: 'msg-u',
					role: 'user',
