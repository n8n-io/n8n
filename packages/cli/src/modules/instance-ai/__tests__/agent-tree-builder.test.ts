...
import { buildAgentTreeFromEvents, findAgentNodeInTree } from '../agent-tree-builder';
	...
	it('should handle tool errors', () => {
		const events: InstanceAiEvent[] = [
			...
			},
			{
				...
				runId: 'run-1',
				agentId: 'agent-001',
				payload: { toolCallId: 'tc-1', toolName: 'plan', args: {} },
			},
			{
				type: 'tool-error',
				runId: 'run-1',
		...
		expect(tree.toolCalls[0].renderHint).toBe('plan');
	});
