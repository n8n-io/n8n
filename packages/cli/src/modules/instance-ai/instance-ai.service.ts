...
function buildPlannedTaskConversationContext(
	...
	const targetWorkflowId =
		task.kind === 'build-workflow' ? task.handoff.input.workflowId : undefined;
	if (targetWorkflowId) {
		parts.push(`Target workflow id: ${targetWorkflowId}`);
	}
...
export class InstanceAiService {
	...
	private buildPlannedTaskFollowUpMessage(
		...
		if (options.checkpoint?.kind === 'checkpoint') {
			const checkpoint = options.checkpoint;
			const depOutcomes = graph.tasks
				.filter((t) => checkpoint.deps.includes(t.id))
				.map((t) => ({
					id: t.id,
					title: t.title,
					kind: t.kind,
	...
	private createPlannedTaskContext(
		kind: Exclude<PlannedTaskRecord['kind'], 'checkpoint'>,
		context: OrchestrationContext,
	): OrchestrationContext {
