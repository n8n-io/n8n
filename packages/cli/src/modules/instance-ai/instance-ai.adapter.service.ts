...
export class InstanceAiAdapterService {
	...
	private createExecutionAdapter(user: User): InstanceAiExecutionService {
		...
		return {
			...
			async run(workflowId: string, inputData, options) {
				...
				// If we have input data and a trigger node, set up type-aware pin data
				if (inputData && triggerNode) {
					const pinData = getPinDataForTrigger(triggerNode, inputData);
	...
	private createWebResearchAdapter(): InstanceAiWebResearchService {
		const fetchCache = this.webResearchCache;
		const searchCacheRef = this.searchCache;
		const { braveSearchApiKey: apiKey, searxngUrl } = this;

		// Build the search method based on provider priority: Brave > SearXNG > disabled
		const searchMethod = this.buildSearchMethod(apiKey, searxngUrl, searchCacheRef);
