...
6. Use realistic but clearly fake values (e.g., "jane@example.com", "U_abc123"). For dates and timestamps, always use dates relative to TODAY (${new Date().toISOString().split('T')[0]}). For example, if a workflow filters "last 14 days", generate dates within that window. Never use hardcoded past dates that may fall outside time-based filters.
