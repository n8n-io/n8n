...
describe('EvalExecutionService', () => {
	...
	describe('parameter issue patching', () => {
		...
		it('normalizes list-mode sheetName when value is a tab title instead of sheetId', async () => {
			workflowFinderService.findWorkflowForUser.mockResolvedValue(
				makeWorkflowEntity({
					nodes: [
						makeStartNode(),
						{
							...
							parameters: {
								documentId: {
									__rl: true,
									mode: 'list',
									value: 'smartassist_bookings_sheet_id',
									cachedResultName: 'SmartAssist Bookings',
								},
								sheetName: {
									__rl: true,
			...
			await service.executeWithLlmMock('wf-1', makeUser());
			const runArg = workflowRunner.run.mock.calls[0][0];
			const sheetsNode = runArg.workflowData.nodes.find((node) => node.name === 'Append Booking');
