...
export class InstanceAiController {
	...
	// usesTemplates bypasses the send() wrapper so we can write NDJSON frames directly
	@Get('/stream/:threadId', { usesTemplates: true })
	async stream(
		...
		// A thread that doesn't exist yet is allowed — the frontend opens the
		// stream for new conversations before the first message creates the thread.
		...
		const { cleanup, ready } = this.instanceAiService.openThreadStream(req.user.id, threadId, res);
	...
	async cancelTask(
		...
		await this.assertThreadAccess(req.user.id, threadId);
		await this.instanceAiService.cancelBackgroundTask(threadId, taskId);
		return { ok: true };
	}
