...
export class InstanceAiController {
	...
	@Post('/eval/execute-and-evaluate/:workflowId')
	@GlobalScope('instanceAi:message')
	async executeAndEvaluate(
		req: AuthenticatedRequest,
		_res: Response,
		@Param('workflowId') workflowId: string,
		@Body payload: InstanceAiWorkflowEvalRequest,
	) {
		return await this.workflowEvalService.executeAndEvaluate(workflowId, req.user, payload);
	}
