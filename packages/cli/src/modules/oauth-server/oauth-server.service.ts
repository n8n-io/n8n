...
export class OAuthServerService implements OAuthServerProvider {
	 ...
	 * (form trigger, or a webhook trigger with the browser flow enabled). Public +
	 * PKCE, single redirect_uri = the trigger URL (which equals the client_id and
	 * the resource URL). The row is persisted lazily only to satisfy the FKs from
	 * auth codes / tokens; it is never a DCR client and is excluded from the
	 * registered-client cap.
	 */
	private async resolveVirtualClient(
		clientId: string,
	): Promise<OAuthClientInformationFull | undefined> {
		// First-party resources are triggers served under the (test) webhook base URL,
		// so a client_id that isn't can never resolve to one. Skip the resolver sweep +
		// lazy upsert for anything else, so the unauthenticated /authorize path can't be
		// used to fan out DB lookups on arbitrary client_ids.
		if (!this.isFirstPartyTriggerClientId(clientId)) {
			return undefined;
		}
	...
	/** Whether a client_id could be a first-party trigger resource URL (served under a webhook base URL). */
	private isFirstPartyTriggerClientId(clientId: string): boolean {
