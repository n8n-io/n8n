...
/**
 * A server the AI Gateway hosts: the remote points at the gateway's own MCP
 * endpoint rather than the vendor's, and `authType: 'gateway'` means the user
 * connects it with Gateway credits instead of their own credential. Withheld
 * from the listing entirely on instances without n8n Connect.
 */
export const firecrawlGatewayMockServer: McpRegistryServer = {
	...
	authType: 'gateway',
	remotes: [
		{ type: 'streamable-http', url: 'https://ai-assistant.n8n.io/v1/gateway/mcp/firecrawl' },
	],
	tools: [
		{ name: 'firecrawl_scrape', title: 'Firecrawl scrape', annotations: { readOnlyHint: true } },
		{ name: 'firecrawl_map', title: 'Firecrawl website map', annotations: { readOnlyHint: true } },
		{
