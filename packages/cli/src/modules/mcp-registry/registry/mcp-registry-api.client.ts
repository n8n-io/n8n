	...
	private withSupportedCredentials(server: McpRegistryServer): McpRegistryServer | null {
		if (server.authType === 'extendsCredential') {
			return server.extendsCredential &&
				this.isOAuth2CredentialType(server.extendsCredential.extends)
				? server
				: null;
		}
		...
		const usesCredentials = (server.usesCredentials ?? []).filter(({ credentialType }) =>
			this.isOAuth2CredentialType(credentialType),
		);
		return usesCredentials.length > 0 ? { ...server, usesCredentials } : null;
	}

	private isOAuth2CredentialType(credentialType: string): boolean {
		return (
			this.credentialTypes.recognizes(credentialType) &&
			(credentialType === 'oAuth2Api' ||
				this.credentialTypes.getParentTypes(credentialType).includes('oAuth2Api'))
		);
	}
}
