...
describe('mergeManifests', () => {
	...
	describe('dependencies', () => {
		...
		it('replaces a recreated variable that has a new id but the same name in the same scope', () => {
			const existing = makeManifest({
				projects: [acme],
				workflows: [entry('w1', 'W1', `${P}/workflows/w1`)],
				variables: [entry('v1', 'API_KEY', `${P}/variables/api-key-v1`)],
				requirements: { variables: [{ name: 'API_KEY', usedByWorkflows: ['w1'] }] },
			});
			const staging = makeManifest({
				projects: [acme],
				workflows: [entry('w1', 'W1', `${P}/workflows/w1`)],
				variables: [entry('v2', 'API_KEY', `${P}/variables/api-key-v2`)],
				requirements: { variables: [{ name: 'API_KEY', usedByWorkflows: ['w1'] }] },
			});

			const result = merge(existing, staging);

			expect(result.variables).toEqual([entry('v2', 'API_KEY', `${P}/variables/api-key-v2`)]);
			// Without the name key both would survive, because the name stays referenced.
