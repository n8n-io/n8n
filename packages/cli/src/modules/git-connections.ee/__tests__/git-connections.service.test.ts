...
describe('GitConnectionsService (credential state machine)', () => {
	...
	describe('selective push', () => {
		...
		const mockExport = (files: Record<string, string>) => {
			n8nPackagesService.exportPackageToDirectory.mockImplementation(
				async (_request, { targetDir }) => {
					...
					// The real exporter counts what it wrote, and a push returns those counts.
					return {
						manifest,
						counts: {
		...
		it('refuses a workflow that moved out of another project, before running the export', async () => {
