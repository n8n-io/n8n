...
export class GitConnectionsService {
	...
	async create(input: CreateGitConnectionDto) {
		...
		const connection = this.repository.create({
			...
			baseCommit: null,
			createBranchOnPromotion: input.createBranchOnPromotion ?? false,
		});
	...
	async update(id: string, input: UpdateGitConnectionDto) {
		...
		// createBranchOnPromotion only affects promote behaviour, not the working copy target,
		// so it is deliberately left out of `changesTarget`.
		if (input.createBranchOnPromotion !== undefined)
			updated.createBranchOnPromotion = input.createBranchOnPromotion;
	...
	private toPublic(connection: GitConnection): GitConnectionPublicDto {
		return {
			...
			createBranchOnPromotion: connection.createBranchOnPromotion,
