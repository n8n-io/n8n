...
export class McpAgentToolsService {
	...
	private callAgentTool(user: User): ToolDefinition<typeof callAgentInput> {
		return {
			...
			handler: async (input: CallAgentInput, { signal }) =>
				await this.run(user, MCP_CALL_AGENT_TOOL_NAME, { agentId: input.agentId }, async () => {
					const agent = await this.resolveAgent(user, input.agentId);
					await this.assertScope(user, agent.projectId, 'agent:execute');
					return await this.callAgent(user, agent, input.request, signal);
				}),
		};
	}
