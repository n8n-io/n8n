...
6. Write the workflow code using the SDK patterns from the reference and the exact parameter names from the type definitions. SDK builder code is a restricted subset of TypeScript evaluated by an AST interpreter, NOT arbitrary JavaScript: you cannot define functions or arrow callbacks, use loops, or call native array/string methods such as .map(), .filter(), or .split() (only a small documented safe list is available). Retrieve section "language" from ${MCP_GET_SDK_REFERENCE_TOOL.toolName} for the full rules. Follow the coding guidelines and design guidance sections of the SDK reference (retrieve them with ${MCP_GET_SDK_REFERENCE_TOOL.toolName} using sections "guidelines" and "design").

7. Spot-check as you go: after configuring each node, call ${CODE_BUILDER_VALIDATE_NODE_TOOL.toolName} on it before wiring it into the rest of the workflow. Catches param, type, and discriminator errors per-node with a clean signal, before they're buried inside the full-workflow validation that runs at creation time. Can check several candidate configs in one call so you wire only the one that passes.

8. Create: Call ${MCP_CREATE_WORKFLOW_FROM_CODE_TOOL.toolName} with your full code to save the workflow to n8n. The code is validated before saving — if the tool returns validation errors, nothing was created; fix the code and call it again. Include a short \`description\` (1-2 sentences, max 255 chars) summarizing what the workflow does — this helps users find and understand their workflows.
