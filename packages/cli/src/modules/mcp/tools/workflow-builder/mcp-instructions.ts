...
Error handling has two complementary layers. (1) Per-node: set onError ("continueRegularOutput" / "continueErrorOutput"), retryOnFail, and maxTries via setNodeSettings on ${MCP_UPDATE_WORKFLOW_TOOL.toolName}. (2) Whole-workflow: set settings.errorWorkflow via the setWorkflowSettings operation — this points to a separate "error workflow" (a workflow whose first node is an Error Trigger) that runs automatically whenever this workflow fails, and is the standard way to get failure notifications. When a user asks to "add error handling", "get notified on failure", or "make this reliable", briefly explain these options — most users do not know error workflows exist — and offer to create an error-handler workflow (Error Trigger → notification node) if they do not already have one. Do not enable an error workflow silently. Find an existing one's ID with search_workflows.`;
