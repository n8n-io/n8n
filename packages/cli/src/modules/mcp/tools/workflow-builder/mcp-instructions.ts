...
9. Create: Call ${MCP_CREATE_WORKFLOW_FROM_CODE_TOOL.toolName} with the validated code to save the workflow to n8n. Include a short \`description\` (1-2 sentences, max 255 chars) summarizing what the workflow does — this helps users find and understand their workflows. Always pass \`nodeDescriptions\` with an entry for every node, keyed by node name: a \`summary\` (1-2 plain sentences on what the node does in this workflow) and a \`rationale\` (one sentence on why that node or approach was chosen over the alternative, e.g. webhook vs polling). These power a guided tour shown to the user on the canvas — write plainly, no marketing tone.

10. Update: Call ${MCP_UPDATE_WORKFLOW_TOOL.toolName} with the workflow ID and a list of operations (addNode, removeNode, updateNodeParameters, setNodeParameter, renameNode, addConnection, removeConnection, setNodeCredential, setNodePosition, setNodeDisabled, setNodeSettings, setNodeDescription, setWorkflowMetadata, setWorkflowSettings). The whole batch is atomic: if any op fails the workflow is unchanged. To modify an existing node's configuration, use updateNodeParameters or setNodeParameter — do NOT use removeNode followed by addNode for the same node, as this disconnects any attached sub-nodes (LLM models, memory, tools) and they will not be re-attached automatically. Use setNodeSettings to change a node's execution behavior (onError, retryOnFail, maxTries, waitBetweenTries, alwaysOutputData, executeOnce); for sub-nodes (LLM model, memory, tools) this is the only way to set onError, because the canvas UI does not expose that setting for them. Use setNodeDescription to add or update a node's guided-tour text: a \`summary\` (1-2 plain sentences on what the node does in this workflow) and an optional \`rationale\` (one sentence on why that node or approach was chosen). Add one whenever you addNode in the same batch, and update it whenever a later edit changes what a node does or why.
