...
describe('getMcpInstructions', () => {
	...
	test('includes n8n Connect hint when builder is enabled and n8n Connect is available', () => {
		const instructions = getMcpInstructions(true, true);
		expect(instructions).toContain('Explore n8n Connect-compatible nodes');
	...
	test('omits n8n Connect hint when n8n Connect is not available', () => {
		const instructions = getMcpInstructions(true, false);
		expect(instructions).toContain('official MCP server for n8n');
		expect(instructions).not.toContain('n8n Connect');
		expect(instructions).not.toContain('n8nConnect');
		expect(instructions).not.toContain('list_n8n_connect_services');
	});

	test('omits n8n Connect hint by default', () => {
