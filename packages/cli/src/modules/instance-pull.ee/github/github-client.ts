...
/** Marker prepended to the bot comment body so we can find-or-update a single one. */
const BOT_COMMENT_MARKER = '<!-- n8n-instance-pull -->';
 ...
 * header. Only the handful of endpoints the demo needs are implemented.
 */
@Service()
export class GitHubClient {
	...
	/**
	 * Find the single bot comment on the PR (identified by {@link BOT_COMMENT_MARKER})
	 * and update it, or create it if absent. Keeps one rolling status comment.
	 */
	async upsertComment(prNumber: number, body: string): Promise<void> {
		const markedBody = `${BOT_COMMENT_MARKER}\n${body}`;
		const existing = await this.findBotComment(prNumber);

		if (existing) {
			await this.apiRequest('PATCH', this.repoPath(`/issues/comments/${existing.id}`), {
				body: markedBody,
			});
			return;
		}

		await this.apiRequest('POST', this.repoPath(`/issues/${prNumber}/comments`), {
			body: markedBody,
		});
	}

	/** Set a commit status on a SHA (obtain the head SHA via {@link getPullRequest}). */
	async setStatus(sha: string, state: CommitStatusState, description: string): Promise<void> {
		await this.apiRequest('POST', this.repoPath(`/statuses/${sha}`), {
			state,
			description: description.slice(0, 140), // GitHub caps description length
			context: 'n8n/instance-pull',
		});
	}
	...
	private async findBotComment(prNumber: number): Promise<GitHubIssueComment | null> {
		const comments = await this.apiRequest<GitHubIssueComment[]>(
			'GET',
			this.repoPath(`/issues/${prNumber}/comments?per_page=100`),
		);
		return comments.find((c) => c.body?.includes(BOT_COMMENT_MARKER)) ?? null;
	}
