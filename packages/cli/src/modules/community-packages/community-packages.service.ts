...
export class CommunityPackagesService {
	...
	private async runRegistryChecks(packageName: string, packageVersion: string, checksum?: string) {
		...
		if (checksum) {
			await verifyIntegrity(packageName, packageVersion, registry, checksum);
		}

		await checkIfVersionExistsOrThrow(packageName, packageVersion, registry);
	}
	...
	private async installOrUpdatePackage(
		...
		await this.runRegistryChecks(packageName, packageVersion, options.checksum);
	...
	async handleInstallEvent({
		...
		try {
			...
			// The payload only says that something needs converging. What gets installed comes
			// from the record the publishing instance persisted before it published.
			...
			// Integrity is recorded per version, so the payload checksum only applies when the
			// payload names the version that is about to be installed.
	...
	async handleUninstallEvent({ packageName }: { packageName: string }) {
		...
		try {
			...
			// The mirror of the install path: the removed record is what says the package is
			// meant to be gone, so a payload on its own never removes anything.
			const installedPackage = await this.findInstalledPackage(parsed.packageName);
			...
			await this.removeNpmPackage(parsed.packageName);
		} catch (error) {
			const details = { packageName, reason: ensureError(error).message };
