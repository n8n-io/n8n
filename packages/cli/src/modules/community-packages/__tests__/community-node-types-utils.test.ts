...
describe('community-node-types-utils', () => {
	...
	describe('getCommunityNodeTypes', () => {
		...
		it('should call paginatedRequest with correct URL for staging', async () => {
			...
			await getCommunityNodeTypes('staging', {}, AI_SDK_VERSION, NODES_API_VERSION);
		...
		it('should pass query string parameters to paginatedRequest', async () => {
			...
			await getCommunityNodeTypes('production', qs, AI_SDK_VERSION, NODES_API_VERSION);
		...
		it('should return data from paginatedRequest', async () => {
			...
			const result = await getCommunityNodeTypes(
				'production',
				{},
				AI_SDK_VERSION,
				NODES_API_VERSION,
	...
	describe('getCommunityNodesMetadata', () => {
		it('should call paginatedRequest with correct URL for production', async () => {
			...
			await getCommunityNodesMetadata('production', AI_SDK_VERSION, NODES_API_VERSION);
		...
		it('should call paginatedRequest with correct URL for staging', async () => {
			...
			await getCommunityNodesMetadata('staging', AI_SDK_VERSION, NODES_API_VERSION);
		...
		it('should return metadata from paginatedRequest', async () => {
			...
			const result = await getCommunityNodesMetadata(
				'production',
				AI_SDK_VERSION,
				NODES_API_VERSION,
			);
