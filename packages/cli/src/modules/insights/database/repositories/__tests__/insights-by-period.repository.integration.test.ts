...
import { DbLockService, SharedWorkflowRepository } from '@n8n/db';
...
describe('InsightsByPeriodRepository', () => {
	...
	describe('concurrent compaction across processes', () => {
		test('moves every source row once when two repository instances compact at the same time', async () => {
			// ARRANGE
