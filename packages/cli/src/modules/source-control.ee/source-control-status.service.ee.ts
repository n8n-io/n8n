...
const tracer = trace.getTracer('source-control-status');
...
export class SourceControlStatusService {
	...
	async getStatus(
		...
		return tracer.startActiveSpan(
			'SourceControlStatusService.getStatus',
			{
				attributes: {
					'sc.direction': options.direction ?? '',
					'sc.verbose': options.verbose ?? false,
				},
			},
			async (span) => {
				try {
					...
					const remoteFolders = await tracer.startActiveSpan(
						'SourceControlStatusService.getRemoteFoldersAndMappingsFromFile',
						async (s) => {
							try {
								return await this.sourceControlImportService.getRemoteFoldersAndMappingsFromFile(
									context,
								);
							} catch (e) {
					...
					span.setAttribute('sc.totalFiles', sourceControlledFiles.length);
	...
	private async resetWorkfolder(): Promise<void> {
		return tracer.startActiveSpan('SourceControlStatusService.resetWorkfolder', async (span) => {
			try {
	...
	private async getStatusWorkflows(
		...
		return tracer.startActiveSpan('SourceControlStatusService.getStatusWorkflows', async (span) => {
			try {
				const sourceControlledFiles: SourceControlledFile[] = [];
				...
				span.setAttribute('sc.workflows.fileCount', result.files.length);
				return result;
			} catch (e) {
				span.setStatus({ code: SpanStatusCode.ERROR, message: String(e) });
				throw e;
	...
	private async getStatusCredentials(
		...
		return tracer.startActiveSpan(
			'SourceControlStatusService.getStatusCredentials',
			async (span) => {
				try {
					const sourceControlledFiles: SourceControlledFile[] = [];
					const credRemoteIds =
					...
					const credLocalIds =
						await this.sourceControlImportService.getLocalCredentialsFromDb(context);
					...
					span.setAttribute('sc.credentials.fileCount', result.files.length);
					return result;
				} catch (e) {
					span.setStatus({ code: SpanStatusCode.ERROR, message: String(e) });
					throw e;
	...
	private async getStatusVariables(options: SourceControlGetStatus, collectVerbose: boolean) {
		return tracer.startActiveSpan('SourceControlStatusService.getStatusVariables', async (span) => {
			try {
				const sourceControlledFiles: SourceControlledFile[] = [];
				...
				span.setAttribute('sc.variables.fileCount', result.files.length);
				return result;
			} catch (e) {
				span.setStatus({ code: SpanStatusCode.ERROR, message: String(e) });
				throw e;
	...
	private async getStatusDataTables(options: SourceControlGetStatus, collectVerbose: boolean) {
		return tracer.startActiveSpan(
			'SourceControlStatusService.getStatusDataTables',
			async (span) => {
				try {
					const sourceControlledFiles: SourceControlledFile[] = [];
					...
					span.setAttribute('sc.dataTables.fileCount', result.files.length);
					return result;
				} catch (e) {
					span.setStatus({ code: SpanStatusCode.ERROR, message: String(e) });
					throw e;
	...
	private async getStatusTagsMappings(
		...
		return tracer.startActiveSpan(
			'SourceControlStatusService.getStatusTagsMappings',
			async (span) => {
				try {
					const sourceControlledFiles: SourceControlledFile[] = [];
					...
					const hasMappingChanges =
						mappingsMissingInLocalCount > 0 || mappingsMissingInRemoteCount > 0;
					const hasTagChanges =
						tagsMissingInLocalCount > 0 ||
						tagsMissingInRemoteCount > 0 ||
						tagsModifiedInEitherCount > 0;
					...
					span.setAttribute('sc.tags.fileCount', result.files.length);
					return result;
				} catch (e) {
					span.setStatus({ code: SpanStatusCode.ERROR, message: String(e) });
					throw e;
	...
	private async getStatusFoldersMapping(
		...
		return tracer.startActiveSpan(
			'SourceControlStatusService.getStatusFoldersMapping',
			async (span) => {
				try {
					const sourceControlledFiles: SourceControlledFile[] = [];
					...
					const allTeamProjects =
						await this.sourceControlImportService.getLocalTeamProjectsFromDb();

					const foldersModifiedInEither: ExportableFolder[] = [];

					for (const remoteFolder of foldersMappingsRemote.folders) {
						if (
							foldersMappingsLocal.folders.findIndex((local) => local.id === remoteFolder.id) === -1
						) {
							if (collectVerbose) {
					...
					for (const localFolder of foldersMappingsLocal.folders) {
						if (
							foldersMappingsRemote.folders.findIndex((remote) => remote.id === localFolder.id) ===
							-1
						) {
							if (collectVerbose) {
					...
					span.setAttribute('sc.folders.fileCount', result.files.length);
					return result;
				} catch (e) {
					span.setStatus({ code: SpanStatusCode.ERROR, message: String(e) });
					throw e;
	...
	private async getStatusProjects(
		...
		return tracer.startActiveSpan('SourceControlStatusService.getStatusProjects', async (span) => {
			try {
				const sourceControlledFiles: SourceControlledFile[] = [];
				const projectsRemote =
					await this.sourceControlImportService.getRemoteProjectsFromFiles(context);
				const projectsLocal =
					await this.sourceControlImportService.getLocalTeamProjectsFromDb(context);
				...
				for (const remoteProject of projectsRemote) {
					if (
						!projectLocalIds.has(remoteProject.id) &&
						!outOfScopeProjectIds.has(remoteProject.id)
					) {
						if (collectVerbose) {
				...
				span.setAttribute('sc.projects.fileCount', result.files.length);
				return result;
			} catch (e) {
				span.setStatus({ code: SpanStatusCode.ERROR, message: String(e) });
				throw e;
