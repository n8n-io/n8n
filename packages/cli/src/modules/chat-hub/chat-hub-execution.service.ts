...
import { EXECUTION_FINISHED_STATUSES, EXECUTION_POLL_INTERVAL } from './chat-hub.constants';
...
export class ChatHubExecutionService {
	...
	/**
	 * Delete a temporary chat workflow
	 */
	...
	private async executeChatWorkflow(
		...
		const SUPPORTED_RESPONSE_MODES = ['streaming', 'lastNode', 'responseNodes'] as const;

		if (
			!SUPPORTED_RESPONSE_MODES.includes(responseMode as (typeof SUPPORTED_RESPONSE_MODES)[number])
		) {
	 ...
	 * We need this to extract the streamed message from the Response they're being written to during execution,
	 * and to transfer it to the UI via Push instead (and also save to save them on DB).
	 */
