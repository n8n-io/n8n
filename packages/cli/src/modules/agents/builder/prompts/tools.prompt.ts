...
Use this guidance before calling \`add-integration\`, \`search_nodes\`,
\`search_mcp_servers\`, \`describe-nodes\`, \`build_custom_tool\`, or adding,
...
- Chat/trigger integration: call \`list_integration_types\`, then
  \`configure_channel\` with a returned type. Do not call \`add-integration\`
...
- Callable external service: the conversation or trigger happens elsewhere and
  ...
  service, call \`add-integration\` separately, using \`queries\` as
  ...
  - \`kind: "node"\`: load \`agent-builder-node-tools\`, use the returned node
    results, and continue with \`describe-nodes\`.
...
1. MCP servers selected by \`add-integration\`
2. Node tools returned by \`add-integration\`
3. Workflow tools (\`list-workflows\`)
...
- Call \`list-workflows\`; reference supported workflows by name with \`{ "type": "workflow", "workflow": "<name>" }\`. Always use the workflow name, never its id.

#### Node Tools

Load \`agent-builder-node-tools\` after \`add-integration\` returns
...
Load \`agent-builder-mcp\` after \`add-integration\` returns \`kind: "mcp"\`, or
...
- Chat/trigger integrations bypassed \`add-integration\` and were set up
  through \`configure_channel\`.
- Each non-chat callable service was resolved separately through
  \`add-integration\` unless the user explicitly requested an n8n node or
  custom MCP server.
