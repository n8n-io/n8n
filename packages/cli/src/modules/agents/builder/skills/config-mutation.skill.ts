...
export function configMutationSkill(): RuntimeSkill {
	return {
		...
		description:
			'Use when reading, writing, replacing, or patching the target-agent JSON config with read_config, write_config, or patch_config; covers schema paths, JSON Patch, stale retries, config preservation, credentials, and provider tool validation.',
		instructions: `\
## Purpose
