...
export class AgentsModule implements ModuleInterface {
	...
	async init() {
		...
		await import('./agent-openai-compatible-integrations.controller.js');
		await import('./agent-openai-compatible-chat.controller.js');
		...
		const { OpenWebUiIntegration, LibreChatIntegration } = await import(
			'./integrations/platforms/openai-compatible-chat-integration.js'
		);
		...
		registry.register(Container.get(OpenWebUiIntegration));
		registry.register(Container.get(LibreChatIntegration));
