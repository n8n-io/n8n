...
import {
	createModel,
	type EpisodicMemoryExtraction,
	type EpisodicMemoryExtractorInput,
	type EpisodicMemoryExtractFn,
	type EpisodicMemoryReflection,
	type EpisodicMemoryReflectorInput,
	type EpisodicMemoryReflectFn,
...
export interface CreateN8nEpisodicMemoryExtractFnOptions {
	extractionPrompt?: string;
}

export interface CreateN8nEpisodicMemoryReflectFnOptions {
	reflectionPrompt?: string;
}

export function buildN8nEpisodicMemoryExtractorPrompt(input: EpisodicMemoryExtractorInput): string {
	return [
		`Current timestamp: ${input.now.toISOString()}`,
		`Scope: agent:${input.scope.agentId}:resource:${input.scope.resourceId}`,
...
export function createN8nEpisodicMemoryExtractFn(
	model: ModelConfig,
	options: CreateN8nEpisodicMemoryExtractFnOptions = {},
	...
	return async (input): Promise<EpisodicMemoryExtraction> => {
		const { object } = await generateObject({
			...
			prompt: buildN8nEpisodicMemoryExtractorPrompt(input),
...
export function buildN8nEpisodicMemoryReflectorPrompt(input: EpisodicMemoryReflectorInput): string {
	return [
		`Current timestamp: ${input.now.toISOString()}`,
		`Scope: agent:${input.scope.agentId}:resource:${input.scope.resourceId}`,
...
export function createN8nEpisodicMemoryReflectFn(
	model: ModelConfig,
	options: CreateN8nEpisodicMemoryReflectFnOptions = {},
	...
	return async (input): Promise<EpisodicMemoryReflection> => {
		const { object } = await generateObject({
			...
			prompt: buildN8nEpisodicMemoryReflectorPrompt(input),
