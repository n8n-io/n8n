...
export function buildActionToolDescription(
	...
	return [
		...
		'If this turn was triggered by an inbound chat message, your final reply text is posted to that conversation automatically — do not duplicate it with a text-only respond. Call respond only with message.card, with email message.attachments, or after switching to a different target.',
		...
		...(descriptor.integration.type === 'email'
			? [
					'For email attachments, use message.attachments with filename, contentType, and either base64 content or a public/pre-signed url. Do not use message.card.',
				]
			: [
