...
vi.mock('cron', async () => {
	const actual = await vi.importActual<typeof import('cron')>('cron');
	return { ...actual, CronJob: vi.fn() };
});

const CronJobMock = CronJob as unknown as Mock;
...
function seedJob(service: AgentTaskService, taskId: string, agentId: string): { stop: Mock } {
	const job = { stop: vi.fn() };
	(service as unknown as { jobs: Map<string, { agentId: string; job: { stop: Mock } }> }).jobs.set(
		taskId,
		{ agentId, job },
	);
	return job;
...
describe('AgentTaskService', () => {
	...
	beforeEach(() => {
		vi.clearAllMocks();
		setMultiMain(false);
		CronJobMock.mockImplementation(function () {
	...
	describe('registerEnabledForAgent', () => {
		...
		it('skips a cron tick while the same task is already running', async () => {
			...
			(agentsService.executeForTaskPublished as Mock)
		...
		it('uses the shared lock to skip a tick after leader handoff while a previous leader run is active', async () => {
			...
			taskRunLockRepository.acquire
				...
				.mockResolvedValueOnce(null);
			(agentsService.executeForTaskPublished as Mock).mockReturnValueOnce(blockingStream());
	...
	describe('runTask', () => {
		...
		it('runs the published agent with the objective', async () => {
			...
			(taskSnapshotRepository.findByVersionAndTaskId as Mock).mockResolvedValue(makeSnapshot());
			(projectRelationRepository.findUserIdsByProjectId as Mock).mockResolvedValue(['user-1']);
			(agentsService.executeForTaskPublished as Mock).mockReturnValue(emptyStream());
		...
		it('logs when execution throws', async () => {
			...
			(taskSnapshotRepository.findByVersionAndTaskId as Mock).mockResolvedValue(makeSnapshot());
			(projectRelationRepository.findUserIdsByProjectId as Mock).mockResolvedValue(['user-1']);
			(agentsService.executeForTaskPublished as Mock).mockReturnValue(throwingStream());
		...
		it('renews the task run lock while a scheduled run is active', async () => {
			...
			try {
				...
				(taskSnapshotRepository.findByVersionAndTaskId as Mock).mockResolvedValue(makeSnapshot());
				(projectRelationRepository.findUserIdsByProjectId as Mock).mockResolvedValue(['user-1']);
				(agentsService.executeForTaskPublished as Mock).mockReturnValue(blockingStream());
	...
	describe('runNow', () => {
		it('runs the task immediately as the requesting user even when unpublished', async () => {
			...
			(agentRepository.findOne as Mock).mockResolvedValue(makeAgent({ activeVersionId: null }));
			(agentsService.executeForTaskNow as Mock).mockReturnValue(emptyStream());
