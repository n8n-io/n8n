...
describe('buildFromJson()', () => {
	...
	it('passes profile prompt overrides when memory is enabled', async () => {
		const mockMemory = makeMockMemoryBackend();
		const config = makeConfig({
			description: 'Helps users debug and review n8n code.',
			memory: {
				...
				storage: 'n8n',
				profiles: {
					prompts: { profileUpdate: 'Update the profiles.' },
				},
			},
		});
		...
		expect(getMemoryConfig(agent)?.profiles).toMatchObject({
			enabled: true,
			agentDescription: 'Helps users debug and review n8n code.',
			prompts: { profileUpdate: 'Update the profiles.' },
		});
	...
	it('enables profiles when memory is enabled unless raw JSON disables them', async () => {
		const agent = await buildFromJson(
			makeConfig({
				memory: {
					enabled: true,
	...
	it('can disable profiles while keeping session memory', async () => {
		const agent = await buildFromJson(
			makeConfig({
				memory: {
					enabled: true,
		...
		expect(getMemoryConfig(agent)?.profiles).toBeUndefined();
	});
