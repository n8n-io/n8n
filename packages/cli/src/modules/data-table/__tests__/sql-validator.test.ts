...
describe('SqlValidator', () => {
	...
	describe('allowed expressions', () => {
		...
		it('should accept table-qualified column references', () => {
			expect(() =>
				pgValidator.validate(
					'SELECT "orders"."amount", "orders"."status" FROM "orders"',
					tableSchemas,
				),
			).not.toThrow();
		});
	...
	describe('rejected constructs', () => {
		...
		it('should reject SELECT INTO TEMP TABLE', () => {
			expect(() =>
				pgValidator.validate('SELECT amount INTO TEMP TABLE audit_log FROM "orders"', tableSchemas),
			).toThrow(SqlValidationError);
		});

		it('should reject SELECT INTO UNLOGGED TABLE', () => {
			expect(() =>
				pgValidator.validate(
					'SELECT amount INTO UNLOGGED TABLE audit_log FROM "orders"',
					tableSchemas,
				),
			).toThrow(SqlValidationError);
		});
		...
		it('should reject unquoted column references', () => {
	...
	describe('closed-world identifier validation', () => {
		it('should reject unknown quoted identifiers (not a known column)', () => {
			expect(() => pgValidator.validate('SELECT "evil" FROM "orders"', tableSchemas)).toThrow(
				SqlValidationError,
			);
		});

		it('should reject unknown unquoted identifiers (not a table or function)', () => {
			expect(() => pgValidator.validate('SELECT evil FROM "orders"', tableSchemas)).toThrow(
				SqlValidationError,
			);
		});
		...
		it('should reject quoted columns that do not exist in the referenced table', () => {
			expect(() =>
				pgValidator.validate('SELECT "orders"."nonexistent" FROM "orders"', tableSchemas),
			).toThrow(SqlValidationError);
		});

		it('should accept quoted columns that exist in the referenced table', () => {
			expect(() =>
				pgValidator.validate('SELECT "orders"."amount" FROM "orders"', tableSchemas),
			).not.toThrow();
		});

		it('should accept standalone quoted columns that exist in any allowed table', () => {
			expect(() =>
				pgValidator.validate('SELECT "amount", "status" FROM "orders"', tableSchemas),
			).not.toThrow();
		});

		it('should reject standalone quoted columns not in any allowed table', () => {
			expect(() =>
				pgValidator.validate('SELECT "secret_column" FROM "orders"', tableSchemas),
			).toThrow(SqlValidationError);
		});
	});
	...
	describe('token-level validation', () => {
		it('should reject line comments (--)', () => {
			expect(() =>
				pgValidator.validate('SELECT * FROM "orders" -- WHERE 1=1', tableSchemas),
			).toThrow(SqlValidationError);
		});
		...
		it('should accept double-quoted column identifiers', () => {
			expect(() =>
				pgValidator.validate('SELECT "amount" FROM "orders"', tableSchemas),
			).not.toThrow();
		});

		it('should reject unterminated double-quoted identifiers', () => {
			expect(() => pgValidator.validate('SELECT "amount FROM "orders"', tableSchemas)).toThrow(
				SqlValidationError,
			);
		});
		...
		it('should reject :: cast syntax', () => {
			expect(() =>
				pgValidator.validate('SELECT "amount"::text FROM "orders"', tableSchemas),
			).toThrow(SqlValidationError);
		});
	...
	// ── Keyword-as-column-name bypass ──────────────────────────

	describe('keyword confusion: columns named after SQL keywords', () => {
		// Schemas where column names collide with dangerous SQL keywords.
		// The validator must reject dangerous clauses even when every token
		// resolves to a known column name. Quoted columns are safe; unquoted
		// SQL keywords (INTO, FOR, UPDATE) are rejected.
		const schemaWithDangerousColumns: TableColumnSchema[] = [
			{
				name: 'orders',
				columns: [
					'id',
		...
		it('should reject SELECT INTO even when "into" and target are column names', () => {
			expect(() =>
				pgValidator.validate(
					'SELECT "amount" INTO "audit_log" FROM "orders"',
					schemaWithDangerousColumns,
				),
			).toThrow(SqlValidationError);
		});

		it('should reject SELECT INTO TEMP TABLE even when all tokens are column names', () => {
			expect(() =>
				pgValidator.validate(
					'SELECT "amount" INTO TEMP TABLE "audit_log" FROM "orders"',
					schemaWithDangerousColumns,
				),
			).toThrow(SqlValidationError);
		});

		it('should reject SELECT INTO UNLOGGED TABLE even when all tokens are column names', () => {
			expect(() =>
				pgValidator.validate(
					'SELECT "amount" INTO UNLOGGED TABLE "audit_log" FROM "orders"',
					schemaWithDangerousColumns,
		...
		it('should reject FOR UPDATE even when "for" and "update" are column names', () => {
			expect(() =>
				pgValidator.validate(
		...
		it('should reject FOR SHARE even when "for" and "share" are column names', () => {
			expect(() =>
				pgValidator.validate('SELECT "amount" FROM "orders" FOR SHARE', schemaWithDangerousColumns),
			).toThrow(SqlValidationError);
		});

		it('should allow querying columns named after keywords when properly quoted', () => {
			expect(() =>
				pgValidator.validate(
					'SELECT "into", "for", "update", "table" FROM "orders"',
					schemaWithDangerousColumns,
				),
			).not.toThrow();
		});
