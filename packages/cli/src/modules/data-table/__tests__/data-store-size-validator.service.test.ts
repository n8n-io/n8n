...
describe('DataStoreSizeValidator', () => {
	...
	describe('basic functionality', () => {
		it('should fetch size on first call', async () => {
			fetchSizeFn.mockResolvedValue({ totalBytes: 50 * 1024 * 1024, tables: {} }); // 50MB
		...
		it('should pass validation when size is under limit', async () => {
			fetchSizeFn.mockResolvedValue({ totalBytes: 50 * 1024 * 1024, tables: {} });
		...
		it('should throw error when size exceeds limit', async () => {
			fetchSizeFn.mockResolvedValue({ totalBytes: 150 * 1024 * 1024, tables: {} });
		...
		it('should throw error when size equals limit', async () => {
			fetchSizeFn.mockResolvedValue({ totalBytes: 100 * 1024 * 1024, tables: {} });
	...
	describe('concurrent calls', () => {
		it('should handle concurrent calls correctly', async () => {
			let resolveCheck: (value: { totalBytes: number; tables: Record<string, number> }) => void;
			const checkPromise = new Promise<{ totalBytes: number; tables: Record<string, number> }>(
				(resolve) => {
			...
			resolveCheck!({ totalBytes: 50 * 1024 * 1024, tables: {} });
		...
		it('should share failure state among concurrent calls', async () => {
			...
			resolveCheck!({ totalBytes: 150 * 1024 * 1024, tables: {} });
	...
	describe('edge case: DB becomes full after initial check', () => {
		it('should correctly handle DB becoming full between cached checks', async () => {
			...
			fetchSizeFn.mockResolvedValueOnce({ totalBytes: 99 * 1024 * 1024, tables: {} });
			const time1 = new Date('2024-01-01T00:00:00Z');
			...
			fetchSizeFn.mockResolvedValueOnce({ totalBytes: 100 * 1024 * 1024, tables: {} });
