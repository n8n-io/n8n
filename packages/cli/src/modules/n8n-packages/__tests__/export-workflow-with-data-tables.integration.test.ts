...
import { buildWorkflowReferencingDataTable } from './utils/test-builders';
...
describe('workflow package export — with data tables', () => {
	...
	it('bundles a referenced table schema and lists it in requirements.dataTables', async () => {
		...
		const workflow = await buildWorkflowReferencingDataTable({
	...
	it('lists every column with its type for a table with all column types', async () => {
		...
		const dataTableFile = entries.find((e) => e.name === 'data-tables/everything/data-table.json');
	...
	it('dedupes a table referenced by two workflows in a single export', async () => {
		...
		const wfA = await buildWorkflowReferencingDataTable({
			name: 'Workflow A',
			project,
			dataTableId: dataTable.id,
		});
		const wfB = await buildWorkflowReferencingDataTable({
			name: 'Workflow B',
			project,
			dataTableId: dataTable.id,
		});
	...
	it('disambiguates same-named tables from two different projects with a suffix, instead of failing', async () => {
		...
		const tableA = await dataTableService.createDataTable(projectA.id, {
			name: 'Customers',
			columns: [{ name: 'email', type: 'string' }],
		});
		...
		const wfA = await buildWorkflowReferencingDataTable({
			name: 'Workflow A',
			project: projectA,
			dataTableId: tableA.id,
		});
		const wfB = await buildWorkflowReferencingDataTable({
			name: 'Workflow B',
			project: projectB,
			dataTableId: tableB.id,
		});
	...
	it('blocks the export when the caller can read the workflow but not the table', async () => {
		...
		const dataTable = await dataTableService.createDataTable(ownerProject.id, {
			name: 'Private table',
			columns: [{ name: 'email', type: 'string' }],
		});
		const workflow = await buildWorkflowReferencingDataTable({
