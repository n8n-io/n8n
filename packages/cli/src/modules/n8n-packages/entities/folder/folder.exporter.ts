...
export class FolderExporter {
	...
	async export(request: FolderExportRequest): Promise<FolderExportResult> {
		...
		this.writeLevel(roots, foldersDir, null, childrenByParent, request.writer, entries);
	...
	/**
	 * Writes one set of sibling folders into `parentDir`, then recurses into each
	 * folder's children. A fresh allocator per call scopes slug collisions to the
	 * parent directory; sub-folders nest directly (no repeated `folders/` segment).
	 */
	private writeLevel(
		siblings: Folder[],
		parentDir: string,
		...
		for (const folder of this.orderedByCreation(siblings)) {
			...
			// The workflows contained in this folder are written under
			// `${target}/workflows` by LIGO-684; an empty folder is just the shell.
