...
export abstract class BaseCommand<F = never> {
	...
	async init(): Promise<void> {
		...
		// Registered for every instance type: execution events fire on whichever process ran the
		// execution, which in scaling mode is the worker.
		Container.get(ActivityLogEventRelay).init();
