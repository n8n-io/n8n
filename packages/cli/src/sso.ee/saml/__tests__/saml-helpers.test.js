...
const helpers = __importStar(require('@/sso.ee/saml/saml-helpers'));
...
describe('sso/saml/samlHelpers', () => {
	describe('updateUserFromSamlAttributes', () => {
		test('does not user `Repository.update`, but `Repository.save` instead', async () => {
			const user = Object.assign(new db_1.User(), {
				id: (0, db_1.generateNanoId)(),
				authIdentities: [],
			});
			const samlUserAttributes = {
				firstName: 'Nathan',
				lastName: 'Nathaniel',
				email: 'n@8.n',
				userPrincipalName: 'Huh?',
			};
			userRepository.save.mockImplementationOnce(async (user) => user);
...
//# sourceMappingURL=saml-helpers.test.js.map
