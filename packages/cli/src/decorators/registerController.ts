...
export const registerController = (app: Application, config: Config, controller: object) => {
	...
	const controllerBasePath = Reflect.getMetadata(BASE_PATH, controllerClass) as string | undefined;
	if (!controllerBasePath)
		throw new Error(`${controllerClass.name} is missing the RestController decorator`);

	const routes = Reflect.getMetadata(ROUTES, controllerClass) as RouteMetadata[];
	if (routes.length > 0) {
