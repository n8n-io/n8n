 ...
 * @param errorMessage Text of the thrown error. Each caller passes its own
 * text, so that the error names the trigger type (schedule or poll) that
 * failed.
 * @returns The enabled node with this id.
 * @throws {UnexpectedError} If the node is missing or disabled. The error
 * carries `errorMessage` and the extras `taskId`, `jobId`, `workflowId` and
 * `nodeId`.
 */
