...
describe('SystemTaskScheduledJobOwner', () => {
	...
	describe('findStale', () => {
		it('lists the stored tasks this instance neither runs durably nor a newer version stamped', async () => {
			owner.declareDurable('prune-executions');
			jobs.findPayloadsByOwnerType.mockResolvedValue([
				row('prune-executions', { n8nVersion: N8N_VERSION }),
				row('compact-insights', { n8nVersion: NEWER_VERSION }),
				row('renew-license', { n8nVersion: OLDER_VERSION }),
				row('clean-jtis', {}),
			]);

			await expect(owner.findStale()).resolves.toEqual(['renew-license', 'clean-jtis']);
