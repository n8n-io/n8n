...
describe('Collaboration Message Parsing', () => {
	describe('parseWorkflowMessage', () => {
		it('should parse valid workflowEdit message', async () => {
			const message: WorkflowEditMessage = {
				type: 'workflowEdit',
		...
		it('should parse removeNode operation', async () => {
			...
			const result = await parseWorkflowMessage(message);
			expect(result.type).toBe('workflowEdit');
			expect((result as WorkflowEditMessage).operation.action).toBe('removeNode');
		});
		...
		it('should parse updateNode operation', async () => {
			...
			const result = await parseWorkflowMessage(message);
			expect(result.type).toBe('workflowEdit');
			expect((result as WorkflowEditMessage).operation.action).toBe('updateNode');
		});
		...
		it('should parse addConnection operation', async () => {
			...
			const result = await parseWorkflowMessage(message);
			expect(result.type).toBe('workflowEdit');
			expect((result as WorkflowEditMessage).operation.action).toBe('addConnection');
		});
		...
		it('should parse removeConnection operation', async () => {
			...
			const result = await parseWorkflowMessage(message);
			expect(result.type).toBe('workflowEdit');
			expect((result as WorkflowEditMessage).operation.action).toBe('removeConnection');
		});
		...
		it('should reject invalid message type', async () => {
			...
			await expect(parseWorkflowMessage(invalidMessage)).rejects.toThrow();
		});
