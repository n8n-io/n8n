...
export class Server extends AbstractServer {
	...
	async configure(): Promise<void> {
		...
		if (isApiEnabled()) {
			...
			const publicApiPath = publicApiEndpoint.startsWith('/') ? publicApiEndpoint.slice(1) : publicApiEndpoint;
		...
		const protectedTypeFiles = [
			...
			{ path: pathResolvingService.resolveTypesEndpoint('credentials.json'), file: 'types/credentials.json' },
		];
		...
		if (frontendService) {
			...
			this.app.use(`${pathResolvingService.resolveSchemasEndpoint()}/:node/:version{/:resource}{/:operation}.json`, serveSchemas);
