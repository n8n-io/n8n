...
describe('FolderRepository', () => {
	...
	afterEach(async () => {
		await backend_test_utils_1.testDb.truncate(['Folder', 'TagEntity']);
	});
	afterAll(async () => {
	...
	describe('getMany', () => {
		...
		beforeAll(async () => {
			...
			project = await (0, backend_test_utils_1.getPersonalProject)(owner);
		});
		describe('filters', () => {
			it('should return all folders if not filter is provided', async () => {
				const folder1 = await (0, folders_1.createFolder)(project, { name: 'folder1' });
				const folder2 = await (0, folders_1.createFolder)(project, { name: 'folder2' });
			...
			it('should filter folders by IDs', async () => {
				const anotherUser = await (0, users_1.createMember)();
				const anotherProject = await (0, backend_test_utils_1.getPersonalProject)(anotherUser);
				const folder1 = await (0, folders_1.createFolder)(project, { name: 'folder1' });
				await (0, folders_1.createFolder)(anotherProject, { name: 'folder2' });
				const [folders, count] = await folderRepository.getManyAndCount({
			...
			it('should filter folders by project ID', async () => {
				const anotherUser = await (0, users_1.createMember)();
				const anotherProject = await (0, backend_test_utils_1.getPersonalProject)(anotherUser);
				const folder1 = (0, folders_1.createFolder)(project, { name: 'folder1' });
				const folder2 = (0, folders_1.createFolder)(anotherProject, { name: 'folder2' });
				await Promise.all([folder1, folder2]);
				const [folders, count] = await folderRepository.getManyAndCount({
			...
			it('should filter folders by name case-insensitively', async () => {
				const folder1 = (0, folders_1.createFolder)(project, { name: 'Test Folder' });
				const folder2 = (0, folders_1.createFolder)(project, { name: 'Another Folder' });
				const folder3 = (0, folders_1.createFolder)(project, { name: 'test folder sub' });
			...
			it('should filter folders by parent folder ID', async () => {
				let folders;
				let count;
				const parentFolder = await (0, folders_1.createFolder)(project, { name: 'Parent' });
				...
				await (0, folders_1.createFolder)(project, { name: 'Unrelated' });
			...
			it('should filter folders by multiple tags (AND operator)', async () => {
				...
				const tag3 = await (0, tags_1.createTag)({ name: 'archived' });
				await (0, folders_1.createFolder)(project, {
					name: 'Folder 1',
					tags: [tag1, tag2],
				});
			...
			describe('workflowCount', () => {
				...
				beforeEach(async () => {
					const parentFolder = await (0, folders_1.createFolder)(project, { name: 'Parent' });
					testFolder = await (0, folders_1.createFolder)(project, {
						name: 'Test Folder',
						parentFolder,
					});
		...
		describe('select', () => {
			...
			beforeEach(async () => {
				...
				const tag = await (0, tags_1.createTag)({ name: 'test-tag' });
				testFolder = await (0, folders_1.createFolder)(project, {
					name: 'Test Folder',
					parentFolder,
					tags: [tag],
				});
				await (0, backend_test_utils_1.createWorkflow)({ parentFolder: testFolder });
			});
		...
		describe('pagination', () => {
			beforeEach(async () => {
				await (0, folders_1.createFolder)(project, {
					name: 'Folder 1',
					updatedAt: luxon_1.DateTime.now().minus({ minutes: 4 }).toJSDate(),
				});
				await (0, folders_1.createFolder)(project, {
					name: 'Folder 2',
					updatedAt: luxon_1.DateTime.now().minus({ minutes: 3 }).toJSDate(),
				});
				await (0, folders_1.createFolder)(project, {
					name: 'Folder 3',
					updatedAt: luxon_1.DateTime.now().minus({ minutes: 2 }).toJSDate(),
				});
				await (0, folders_1.createFolder)(project, {
					name: 'Folder 4',
					updatedAt: luxon_1.DateTime.now().minus({ minutes: 1 }).toJSDate(),
				});
				await (0, folders_1.createFolder)(project, {
					name: 'Folder 5',
		...
		describe('sorting', () => {
			beforeEach(async () => {
				...
				await (0, folders_1.createFolder)(project, {
					name: 'A Folder',
					createdAt: luxon_1.DateTime.now().plus({ minutes: 1 }).toJSDate(),
					updatedAt: luxon_1.DateTime.now().plus({ minutes: 1 }).toJSDate(),
				});
				await (0, folders_1.createFolder)(project, {
					name: 'D Folder',
					createdAt: luxon_1.DateTime.now().plus({ minutes: 2 }).toJSDate(),
					updatedAt: luxon_1.DateTime.now().plus({ minutes: 2 }).toJSDate(),
				});
				await (0, folders_1.createFolder)(project, {
					name: 'C Folder',
					createdAt: luxon_1.DateTime.now().plus({ minutes: 3 }).toJSDate(),
					updatedAt: luxon_1.DateTime.now().plus({ minutes: 3 }).toJSDate(),
				});
			});
...
//# sourceMappingURL=folder.repository.test.js.map
