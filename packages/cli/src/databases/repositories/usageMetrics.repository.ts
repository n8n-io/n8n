	...
	async getLicenseRenewalMetrics() {
		...
		const [
			...
			SELECT
				...
				(SELECT COUNT(*) FROM ${this.fullPrefix}workflow_statistics WHERE name IN ('production_success', 'production_error')) AS production_executions_count,
  			(SELECT COUNT(*) FROM ${this.fullPrefix}workflow_statistics WHERE name IN ('manual_success', 'manual_error')) AS manual_executions_count;
		`)) as Row[];
