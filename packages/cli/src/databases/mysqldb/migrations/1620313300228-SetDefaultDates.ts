...
export class SetDefaultDates1620313300228 implements MigrationInterface {
	...
	async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query("ALTER TABLE `" + tablePrefix + "`execution_entity` ADD `data` text NOT NULL");
		...
		await queryRunner.query("ALTER TABLE `" + tablePrefix + "`workflow_entity` CHANGE `nodes` `nodes` json NULL");
	...
	async down(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query("ALTER TABLE `" + tablePrefix + "`workflow_entity` CHANGE `nodes` `nodes` json NOT NULL");
		...
		await queryRunner.query("ALTER TABLE `" + tablePrefix + "`execution_entity` DROP COLUMN `data`");
		await queryRunner.query("ALTER TABLE `" + tablePrefix + "`execution_entity` ADD `data` mediumtext NOT NULL");
