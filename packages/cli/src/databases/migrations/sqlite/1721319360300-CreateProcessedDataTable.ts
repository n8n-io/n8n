...
export class CreateProcessedDataTable1721319360300 implements ReversibleMigration {
	async up({ queryRunner, tablePrefix }: MigrationContext) {
		await queryRunner.query(
			`CREATE TABLE "${tablePrefix}processed_data" ("value" varchar NOT NULL, "context" varchar NOT NULL, "workflowId" varchar NOT NULL, "createdAt" datetime(3), "updatedAt" datetime(3) NOT NULL DEFAULT (STRFTIME('%Y-%m-%d %H:%M:%f', 'NOW')), PRIMARY KEY ("value", "context", "workflowId"), FOREIGN KEY("workflowId") REFERENCES "${tablePrefix}workflow_entity"("id") ON DELETE CASCADE)`,
		);
		await queryRunner.query(
