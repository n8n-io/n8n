...
import { WEBHOOK_NODE_TYPE, WorkflowExpression } from 'n8n-workflow';
...
describe('LiveWebhooks', () => {
	...
	const expressionEngineConfig = mock<ExpressionEngineConfig>({ skipWebhookIsolate: true });
	...
	describe('webhook-phase isolate acquisition', () => {
		...
		// The real Webhook node's description shape: only plain `$parameter`
		// templates, plus a `resolve` entry standing in for the two fields whose
		// templates inline a function body.
		...
		const setupMocks = ({
			...
			webhookService.getWebhookMethods.mockResolvedValue([httpMethod]);
			// A plain literal, not `mock<INodeType>`: a mock proxy answers every
			// property access with a truthy stub, which makes `getNodeParameters`
			// see `displayOptions` on collection children and throw.
			nodeTypes.getByNameAndVersion.mockReturnValue({
				description: {
					displayName: 'Trigger',
					name: nodeType,
					...
					inputs: [],
					outputs: ['main'],
					// Declared properties matter: the `Workflow` constructor drops
					// parameters the node type does not declare, and the scan runs on
					// that same post-processed object.
		...
		it('skips acquisition for a Webhook node with no expression parameters', async () => {
			...
			// Released unconditionally: a no-op when nothing was acquired, and still
			// correct if something inside the phase acquired one.
		...
		it('still skips for a node that uses credentials', async () => {
			// Credentials live outside `node.parameters`, so they are not scanned.
			// Safe because `CredentialsHelper.getDecrypted` resolves credential
			// expressions on a workflow of its own and acquires its own isolate for
			// that (the live webhook path passes no `expressionResolveValues`).
		...
		it('acquires when the kill switch is off', async () => {
			expressionEngineConfig.skipWebhookIsolate = false;
			const request = setupMocks();

			await liveWebhooks.executeWebhook(request, mock<Response>());

			expect(acquireIsolate).toHaveBeenCalled();
			expressionEngineConfig.skipWebhookIsolate = true;
		});
	});
});
