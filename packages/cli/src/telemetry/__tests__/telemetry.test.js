...
describe('Telemetry', () => {
	let startPulseSpy;
	const spyTrack = jest.spyOn(telemetry_1.Telemetry.prototype, 'track').mockName('track');
	const mockRudderStack = (0, jest_mock_extended_1.mock)();
	let telemetry;
	...
	beforeAll(() => {
		startPulseSpy = jest
			.spyOn(telemetry_1.Telemetry.prototype, 'startPulse')
			.mockImplementation(() => {});
		jest.useFakeTimers();
		jest.setSystemTime(testDateTime);
		config_1.default.set('deployment.type', 'n8n-testing');
	});
	afterAll(async () => {
		jest.clearAllTimers();
		jest.useRealTimers();
	...
	beforeEach(async () => {
		spyTrack.mockClear();
		const postHog = new posthog_1.PostHogClient(instanceSettings, (0, jest_mock_extended_1.mock)());
		await postHog.init();
...
const fakeJestSystemTime = (dateTime) => {
