...
exports.SystemMonitoringMetricsService = void 0;
...
let SystemMonitoringMetricsService = class SystemMonitoringMetricsService extends events_1.EventEmitter {
	constructor(systemMonitoringService, _prometheusMetricsService) {
		super();
		this.systemMonitoringService = systemMonitoringService;
		this._prometheusMetricsService = _prometheusMetricsService;
	...
	initializeMetrics() {
		this.metrics = {
			...
			workflowExecutionMemoryUsage: new prom_client_1.default.Histogram({
			...
			workflowExecutionCpuUsage: new prom_client_1.default.Histogram({
			...
			activeWorkflowExecutions: new prom_client_1.default.Gauge({
				name: this.prefix + 'active_workflow_executions',
				help: 'Number of currently active workflow executions',
				labelNames: ['workflowId', 'workflowName'],
			}),
			workflowExecutionNodes: new prom_client_1.default.Histogram({
			...
			alertsTotal: new prom_client_1.default.Counter({
				name: this.prefix + 'alerts_total',
				help: 'Total number of alerts generated',
				labelNames: ['severity', 'type', 'workflowId'],
			}),
			alertsResolved: new prom_client_1.default.Counter({
			...
			nodeExecutionMemoryUsage: new prom_client_1.default.Histogram({
			...
			nodeExecutionCpuUsage: new prom_client_1.default.Histogram({
				name: this.prefix + 'node_execution_cpu_percent',
				help: 'Node execution CPU usage percentage',
				labelNames: ['workflowId', 'nodeId', 'nodeType'],
				buckets: [1, 5, 10, 25, 50, 75, 90, 100],
			}),
			nodeExecutionDataItems: new prom_client_1.default.Histogram({
			...
			nodeExecutionErrors: new prom_client_1.default.Counter({
			...
			nodeExecutionBottlenecks: new prom_client_1.default.Gauge({
				name: this.prefix + 'node_execution_bottleneck_score',
				help: 'Node bottleneck score (0-100)',
				labelNames: ['workflowId', 'nodeId', 'nodeType'],
			}),
			nodeExecutionActive: new prom_client_1.default.Gauge({
	...
	async startMetricsCollection(intervalMs = 30000) {
		try {
	...
	recordThresholdBreach(resource, threshold) {
		try {
			switch (resource) {
				case 'cpu':
	...
	async collectSystemMetrics() {
		try {
			...
			if (resources.processes.workers) {
				const workersByType = resources.processes.workers.reduce((acc, worker) => {
			...
			const alertCounts = resources.health.alerts.reduce((acc, alert) => {
...
exports.SystemMonitoringMetricsService = SystemMonitoringMetricsService;
exports.SystemMonitoringMetricsService = SystemMonitoringMetricsService = __decorate(
	[
		(0, di_1.Service)(),
	...
	SystemMonitoringMetricsService,
);
//# sourceMappingURL=system-monitoring-metrics.service.js.map
