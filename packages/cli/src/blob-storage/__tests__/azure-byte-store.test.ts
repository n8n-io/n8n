...
const notFoundByCode = () =>
	wrapProviderError(Object.assign(new Error('missing'), { code: 'BlobNotFound' }));

describe('AzureByteStore', () => {
	...
	describe('read', () => {
		...
		it('should return `null` when the blob is missing (BlobNotFound)', async () => {
			azureBlob.get.mockRejectedValueOnce(notFoundByCode());
