...
async function benchmarkChildProcessIPC() {
	const runs = 10;
	const stringTimings = [];
	const objectTimings = [];
	...
	// Fork child process
	const child = fork(resolve(__dirname, 'ipc-benchmark-child-worker.js'));
	...
	for (let i = 0; i < runs; i++) {
		...
		await sendAndWait({ type: 'string', data: testString });
	...
	for (let i = 0; i < runs; i++) {
		const start = Date.now();
		await sendAndWait({ type: 'object', data: testObject });
		const duration = Date.now() - start;
...
benchmarkChildProcessIPC().catch((err) => {
