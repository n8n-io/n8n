...
const InsightsPaywall = defineAsyncComponent(async () => await import('./InsightsPaywall.vue'));
...
const InsightsTableWorkflows = defineAsyncComponent(
	async () => await import('./tables/InsightsTableWorkflows.vue'),
);
...
// The project picker is shell-hosted: its list, permissions and search live in
// `features/collaboration/projects`, above this module. Unregistered (a module's
// own test run) means no picker, and the dashboard reads as "all projects".
const projectFilter = computed(() => componentRegistry.get('project-filter'));
...
const selectedProject = ref<SlotProjectSelection>(null);
