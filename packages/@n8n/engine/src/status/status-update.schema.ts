 ...
 */
export const MAX_STATUS_UPDATES_PER_BATCH = 500;
...
/**
 * Wire schema for one status update, and the single definition of its shape:
 * `StatusUpdate` is inferred from it, so the emitter's type and the receiver's
 * validation cannot drift.
 */
export const statusUpdateSchema = z.discriminatedUnion('type', [
	z.object({
		type: z.literal('execution:started'),
		...executionFields,
		// Kept in step with `ExecutionMode` by a type-level assertion in
		// `status-update.schema.test.ts`.
...
/** Body of the host's status endpoint. */
export const statusUpdateBatchSchema = z.object({
	updates: z.array(statusUpdateSchema).min(1).max(MAX_STATUS_UPDATES_PER_BATCH),
});

export type StatusUpdateBatch = z.infer<typeof statusUpdateBatchSchema>;
