...
export default defineWorkflow({
	...
	async run(ctx) {
		const emailData = await ctx.step(
			{ name: 'Parse Email Data', icon: 'mail', color: '#6b7280' },
			async () => {
		...
		const summary = await ctx.step(
			{ name: 'AI Summary', icon: 'bot', color: '#ec4899' },
			async () => {
		...
		const sent = await ctx.step(
			{ name: 'Send Summary to Telegram', icon: 'send', color: '#f97316' },
			async () => {
