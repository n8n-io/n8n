...
export default defineWorkflow({
	...
	async run(ctx) {
		const emails = await ctx.step(
			{ name: 'Fetch Emails Past 24 Hours', icon: 'mail', color: '#3b82f6' },
			async () => {
		...
		const organized = await ctx.step(
			{ name: 'Organize Email Data', icon: 'layers', color: '#8b5cf6' },
			async () => {
		...
		const summary = await ctx.step(
			{ name: 'Summarize with OpenAI', icon: 'bot', color: '#ec4899' },
			async () => {
		...
		const sent = await ctx.step(
			{ name: 'Send Summary Email', icon: 'mail', color: '#f97316' },
			async () => {
				const token = ctx.getSecret('GMAIL_ACCESS_TOKEN') ?? '';
				const htmlBody = `
				<h2>Email Summary</h2>
