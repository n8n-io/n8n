...
export default defineWorkflow({
	...
	async run(ctx) {
		const config = await ctx.step(
			{ name: 'Configuration', icon: 'cog', color: '#6b7280' },
			async () => {
				return {
		...
		const menuData = await ctx.step(
			{ name: 'GET Menu Item', icon: 'globe', color: '#3b82f6' },
			async () => {
		...
		const orderData = await ctx.step(
			{ name: 'GET with Query Params', icon: 'globe', color: '#3b82f6' },
			async () => {
		...
		// 3. POST with body
		const reviewResult = await ctx.step(
			{ name: 'POST with Body', icon: 'send', color: '#3b82f6' },
			async () => {
		...
		const secretDish = await ctx.step(
			{ name: 'GET with Headers/Auth', icon: 'globe', color: '#3b82f6' },
			async () => {
		...
		const slowResponse = await ctx.step(
			{ name: 'Request with Timeout', icon: 'clock', color: '#3b82f6' },
			async () => {
