...
export default defineWorkflow({
	...
	async run(ctx) {
		...
		if (input.hasVoice && input.voiceFileId) {
			...
			const downloaded = await ctx.step({ name: 'Download Voice File' }, async () => {
			...
			const transcription = await ctx.step({ name: 'Transcribe Audio' }, async () => {
		...
		const linkedInPost = await ctx.step({ name: 'LinkedIn Post Text' }, async () => {
			const apiKey = ctx.getSecret('OPENAI_API_KEY') ?? '';
			const res = await fetch('https://dummyjson.com/posts/add', {
				method: 'POST',
				headers: {
		...
		const imagePrompt = await ctx.step({ name: 'Generate Image Prompt' }, async () => {
			const apiKey = ctx.getSecret('OPENAI_API_KEY') ?? '';
			const res = await fetch('https://dummyjson.com/posts/add', {
				method: 'POST',
				headers: {
		...
		const image = await ctx.step({ name: 'Create Image' }, async () => {
			const apiKey = ctx.getSecret('OPENAI_API_KEY') ?? '';
			const res = await fetch('https://dummyjson.com/posts/add', {
				method: 'POST',
				headers: {
		...
		const posted = await ctx.step({ name: 'Create LinkedIn Post' }, async () => {
