...
export default defineWorkflow({
	...
	async run(ctx) {
		const formData = await ctx.step(
			{ name: 'Parse Form Data', icon: 'settings', color: '#6b7280' },
			async () => {
		...
		const copiedDoc = await ctx.step(
			{ name: 'Copy Template File', icon: 'file-text', color: '#3b82f6' },
			async () => {
		...
		const replacements = await ctx.step(
			{ name: 'Format Replacements', icon: 'zap', color: '#8b5cf6' },
			async () => {
		...
		const updated = await ctx.step(
			{ name: 'Replace Data in Google Doc', icon: 'upload', color: '#f97316' },
			async () => {
				const token = ctx.getSecret('GOOGLE_ACCESS_TOKEN') ?? '';
