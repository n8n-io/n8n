...
export default defineWorkflow({
	...
	async run(ctx) {
		const errorData = await ctx.step(
			{ name: 'Parse Error Data', icon: 'bug', color: '#ef4444' },
			async () => {
		...
		const logged = await ctx.step(
			{ name: 'Log Error', icon: 'database', color: '#3b82f6' },
			async () => {
		...
		const notified = await ctx.step(
			{ name: 'Notify in Channel', icon: 'message-circle', color: '#f97316' },
			async () => {
		...
		const emailed = await ctx.step(
			{ name: 'Send Email', icon: 'mail', color: '#f97316' },
			async () => {
