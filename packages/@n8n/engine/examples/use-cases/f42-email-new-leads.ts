...
export default defineWorkflow({
	...
	async run(ctx) {
		const leads = await ctx.step({ name: 'Get Leads from Sheet' }, async () => {
		...
		const uncontacted = await ctx.step({ name: 'Filter Uncontacted' }, async () => {
			const allLeads = leads.values ?? [];
			return allLeads.filter((lead: { Contacted: string }) => !lead.Contacted);
		});

		const updated = await ctx.step({ name: 'Update Sheet' }, async () => {
			const token = ctx.getSecret('GOOGLE_ACCESS_TOKEN') ?? '';
			const results = [];
		...
		const emailed = await ctx.step({ name: 'Send Outreach Emails' }, async () => {
			const results = [];
			for (const lead of uncontacted) {
