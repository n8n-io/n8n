...
export default defineWorkflow({
	...
	async run(ctx) {
		const loginData = await ctx.step(
			{ name: 'Set Login Data', icon: 'cog', color: '#6b7280' },
			async () => {
				return {
		...
		try {
			const connection = await ctx.step(
				{ name: 'SAP Connection', icon: 'globe', color: '#3b82f6' },
				async () => {
			...
			const success = await ctx.step(
				{ name: 'Success', icon: 'trophy', color: '#22c55e' },
				async () => {
			...
			const failed = await ctx.step({ name: 'Failed', icon: 'bug', color: '#ef4444' }, async () => {
				return {
					error: true,
