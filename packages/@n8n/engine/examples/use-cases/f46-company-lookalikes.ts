...
export default defineWorkflow({
	...
	async run(ctx) {
		const sourceList = await ctx.step(
			{ name: 'Read Source List', icon: 'database', color: '#3b82f6' },
			async () => {
		...
		const similarCompanies = await ctx.step(
			{ name: 'Fetch Similar Companies', icon: 'globe', color: '#3b82f6' },
			async () => {
		...
		const written = await ctx.step(
			{ name: 'Write Results', icon: 'upload', color: '#f97316' },
			async () => {
				const token = ctx.getSecret('GOOGLE_ACCESS_TOKEN') ?? '';
				const rows = [];
