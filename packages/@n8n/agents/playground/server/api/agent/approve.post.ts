...
/**
 * Approve a pending tool call and stream the resumed agent output.
 */
export default defineEventHandler(async (event) => {
	...
	try {
		const result = await agent.approveToolCall(body.runId, body.toolCallId);
		...
		sse.error(e instanceof Error ? e.message : 'Failed to approve');
	}
});
