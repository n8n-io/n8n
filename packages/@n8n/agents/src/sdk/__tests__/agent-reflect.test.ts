...
describe('agent.reflect', () => {
	it('returns no-config when observational memory is not configured', async () => {
		...
		const result = await agent.reflect({ threadId: 't-1' });
		expect(result).toEqual({ status: 'no-config' });
	});

	it('returns no-config when observational memory is configured but observe is missing on both builder and call', async () => {
		const store = new InMemoryMemory();
		await seedThread(store, 't-1');
		const memory = new Memory().storage(store).observationalMemory({});
	...
	it('runs the cycle with the builder-time observer', async () => {
		...
		const memory = new Memory().storage(store).observationalMemory({ observe });
	...
	it('lets a call-time observer override the builder default', async () => {
		...
		const memory = new Memory().storage(store).observationalMemory({ observe: builderObserve });

		const agent = new Agent('a').model('openai/gpt-4o-mini').instructions('test').memory(memory);
		await agent.reflect({ threadId: 't-1', observe: callObserve });
