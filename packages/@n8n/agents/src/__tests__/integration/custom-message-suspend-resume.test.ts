...
describe('custom message survives suspend/resume', () => {
	it('preserves custom tool message in stream after resume + complete', async () => {
		...
		const stream1 = await agent.stream('Delete the file /tmp/stream-test.txt', options);
		...
		const customChunk = chunks2.find(
			(c) =>
				...
				'dummy' in (c.message as object),
		) as { type: 'message'; message: { type: 'custom'; dummy: string } } | undefined;

		expect(customChunk).toBeDefined();
		expect(customChunk!.message.dummy).toContain('deleted:');
		expect(customChunk!.message.dummy).toContain('/tmp/stream-test.txt');
	});
});
