...
describe('tool-result to message integration', () => {
	it('adds a custom message to generate result that is visible to user but not to the LLM', async () => {
		...
		const toolResultMsg = customMessages.find((m) => m.type === 'custom' && 'dummy' in m) as
			| { type: 'custom'; dummy: string }
			| undefined;

		expect(toolResultMsg).toBeDefined();
		expect(toolResultMsg!.dummy).toContain('dummy message. Tool output');
	...
	it('emits toContent result as a content chunk in the stream', async () => {
		...
		const toContentChunk = messageChunks.find((c) => c.message.type === 'custom' && 'dummy');

		expect(toContentChunk).toBeDefined();
		expect((toContentChunk!.message as { type: 'custom'; dummy: string }).dummy).toContain(
			'dummy message. Tool output',
		);
	});
});
