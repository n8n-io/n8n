...
describe('episodic memory integration', () => {
	...
	it('processes a pending capture before a later run', async () => {
		const { recallAgent, memory } = createEpisodicAgents('episodic-pending-recovery');
		const resourceId = uniqueId('resource-recovery');
		const sourceThreadId = uniqueId('thread-recovery-source');
		const sourceMessageId = deterministicUuid(++uuidCounter);
		await memory.saveThread({ id: sourceThreadId, resourceId });
			...
			messages: [
				{
					...
					role: 'user',
					content: [{ type: 'text', text: 'Remember recovery codeword FIR-DELTA-28.' }],
				},
			],
		});
		...
		const result = await generateSuccessfully(
			recallAgent,
			'You must call recall_memory before answering. From previous memory, what recovery codeword did I ask you to remember?',
			{
				persistence: { threadId: uniqueId('thread-recovery-next'), resourceId },
			},
		);

		expect(toolNames(result.messages)).toContain('recall_memory');
		expect(JSON.stringify(resolvedToolOutput(result.messages, 'recall_memory'))).toContain(
			'FIR-DELTA-28',
		);
		...
		const entries = await searchEpisodicEntries(
			memory,
			resourceId,
			'recovery codeword FIR-DELTA-28',
		);
		expect(normalizedText(entries.map((entry) => entry.content).join('\n'))).toContain(
			normalizedText('FIR-DELTA-28'),
		);
	});

	it('keeps captured entries source-backed without creating observations', async () => {
