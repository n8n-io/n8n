...
const PLANNER_TODOS_DESCRIPTION =
	'Create or update a structured task list for a complex build. Use it to decompose the work, ' +
	'track progress, and record which tasks are blocked on user input. Do not use it for trivial ' +
	'single-step requests. This tool only updates the task list; it does not perform work or ask the user.';
...
const PLANNER_TODOS_SYSTEM_INSTRUCTION = [
	...
	'- The build has 3+ meaningful steps or multiple deliverables.',
	'- You need to track progress or record tasks blocked on user input.',
	'WHEN NOT TO USE write_todos:',
	'- The request is trivial, conversational, or a single simple change.',
	...
	'- When only blocked tasks remain, resolve everything missing in one trailing batch of interactive tool calls, then unblock and finish the tasks with the answers.',
