...
export const DEFAULT_EPISODIC_MEMORY_TOOL_INSTRUCTION =
	'Episodic memory is enabled through the memory tool. Use the record operation when the user explicitly asks you to remember something, asks you to carry information or a way of working into future tasks, or clearly marks an instruction or preference as important for future work. Allow for natural ambiguity and use judgment about clearly future-facing intent. Do not record incidental details, transient current-task context, or information merely mentioned without a reason to carry it forward. Only confirm that you remembered something after the record operation succeeds; if it fails, tell the user it was not saved. Use the recall operation when the user explicitly asks about prior conversations, earlier decisions, remembered details, previous sessions/work, similar historical situations, exact names, prior artifacts, or complete lists/inventories of what was established before. Use recall to find related prior entries; it does not answer from memory. Treat returned results as prior or historical candidate context, not current-thread truth. The current user message, current thread history, and current observations outrank recall results. Do not use recall for normal current-thread questions, thin current context, missing current information, or as a fallback for missing current context.';

/** @deprecated Use DEFAULT_EPISODIC_MEMORY_TOOL_INSTRUCTION. */
