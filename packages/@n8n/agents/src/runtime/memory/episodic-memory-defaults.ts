...
export const DEFAULT_EPISODIC_MEMORY_CAPTURE_TOOL_INSTRUCTION =
	'Episodic memory capture is enabled. flag_memory is the only record that survives this conversation; nothing you tell the user is noted or on file is stored anywhere else. Call flag_memory when the user asks you to remember something, and for preferences, decisions, facts, and corrections that a future conversation with this user or about this account would need: account or system identifiers and corrections to them, named contacts and their roles, the root cause and the fix that resolved a case, and requests or decisions that take effect later. Call it once per distinct item, in the same turn you acknowledge it. When unsure, flag it; a later pass removes low-value entries. Do not call it for ordinary chatter, unconfirmed inferences, secrets, raw tool output, or assistant proposals the user has not adopted. Set content to a concise durable statement that names the person, company, or account it is about. Set evidence to one short, contiguous exact quote from a user or assistant text message in this conversation that supports the statement. Copy its whitespace and punctuation without changes. Do not wrap it in quotation marks. A successful call only notes the candidate for later processing, so do not claim that a final memory entry was created.';
