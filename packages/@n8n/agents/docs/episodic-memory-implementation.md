...
matches, and renders retrieved entries into a `<memory>` block in retrieval
order.

Injected entries preserve retrieval/ranking order and include a relative-age
label such as `today`, `1 week ago`, or `2 months ago`.
...
should store a compact note when the transcript contains episodic context that could
...
Good entries preserve the situation -> mechanism/decision/finding/change ->
outcome/current state/next question progression. Diagnostic entries are a
subtype; the entry should usually be one to three sentences.
