...
function prepareParameters(
	...
	if (parametersInputType === 'keypair') {
		for (const entry of rawParameters) {
			if (entry.valueProvider.includes('model')) {
				...
				parameters.push.apply(parameters, extractParametersFromText(placeholders, entry.value, sendIn, entry.name));
	...
	if (parametersInputType === 'json' && jsonWithPlaceholders) {
		parameters.push.apply(parameters, extractParametersFromText(placeholders, jsonWithPlaceholders, sendIn, `${sendIn + 'Raw'}`));
	}
