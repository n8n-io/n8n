...
import type { IWebhookFunctions, INodeExecutionData, IWebhookResponseData } from 'n8n-workflow';
import pg from 'pg';
 ...
 */
export async function getChatModel(ctx: IWebhookFunctions): Promise<BaseChatModel> {
...
//  */
export async function langGraphAgentWebhook(
	this: IWebhookFunctions,
): Promise<IWebhookResponseData> {
	this.logger.debug('Executing LangGraph Agent Webhook');
	...
	const messagesOld = await prepareMessages(this, 0, {
		systemMessage: options.systemMessage,
		passthroughBinaryImages: options.passthroughBinaryImages ?? true,
		outputParser,
	});
	if (!model.bindTools) {
		throw new NodeOperationError(this.getNode(), 'The model does not support tools');
	}
	const modelWithTools = model.bindTools(tools);
	const prompt = preparePrompt(messagesOld);
	...
	const humanFeedback = (_state: typeof MessagesAnnotation.State) => {
	...
	if (!PG_CONNECTION_STRING) {
		throw new OperationalError('PG_CONNECTION_STRING environment variable is not set');
	}
	const pool = new Pool({
