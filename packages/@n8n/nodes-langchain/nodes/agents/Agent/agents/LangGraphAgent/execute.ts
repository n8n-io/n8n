...
//  */
export async function langGraphAgentExecute(
	this: IExecuteFunctions,
): Promise<INodeExecutionData[][]> {
	this.logger.debug('Executing LangGraph Agent');
	...
	for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
		try {
			...
			const messagesOld = await prepareMessages(this, itemIndex, {
			...
			const humanFeedback = async (_state: typeof MessagesAnnotation.State) => {
				console.log('--- humanFeedback ---');
				const lastMessage = _state.messages[_state.messages.length - 1];

				const lastToolContent = JSON.parse(lastMessage.content as string) as [
					{ __waiting: string },
				];
				const waiting = lastToolContent[0].__waiting;
				await this.putExecutionToWait(new Date(waiting));
