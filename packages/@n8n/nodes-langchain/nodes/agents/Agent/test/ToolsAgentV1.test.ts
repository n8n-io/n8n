...
describe('toolsAgentExecute', () => {
	...
	it('should process items sequentially when version < 1.10', async () => {
		const mockNode = mock<INode>();
		mockNode.typeVersion = 1.9;
		mockContext.getNode.mockReturnValue(mockNode);
	...
	it('should process items in parallel within batches when version >= 1.10 and batchSize > 1', async () => {
		const mockNode = mock<INode>();
