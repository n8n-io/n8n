...
describe('toolsAgentExecute', () => {
	...
	describe('streaming', () => {
		...
		it('should handle streaming when enableStreaming is true', async () => {
			...
			expect(mockContext.sendChunk).toHaveBeenCalledWith({ type: 'begin', itemIndex: 0 });
			...
			expect(mockContext.sendChunk).toHaveBeenCalledWith({ type: 'end', itemIndex: 0 });
		...
		it('should capture intermediate steps during streaming when returnIntermediateSteps is true', async () => {
			...
			expect(mockContext.sendChunk).toHaveBeenCalledWith({
				...
				metadata: {
					...
					toolName: 'TestTool',
					toolType: 'function',
					toolInput: JSON.stringify({ input: 'test data' }),
				},
			});
			expect(mockContext.sendChunk).toHaveBeenCalledWith({
				type: 'tool-call-end',
				metadata: {
					...
					toolName: 'TestTool',
					toolType: 'function',
					toolOutput: JSON.stringify('Tool execution result'),
				},
			});
		});
