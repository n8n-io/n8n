...
export const formatBuiltInTools = (builtInTools: BuiltInTools) => {
    ...
    if (builtInTools) {
        ...
        if (webSearchOptions) {
            ...
            // CAMBIO: Se usa any para evitar la referencia a OpenAIClient.Responses
            ...
            } as any); // Cast a any para que acepte 'web_search' que es un tipo extendido
        }
...
export const prepareAdditionalResponsesParams = (options: ModelOptions) => {
    const body: Partial<ChatResponseRequest> = {
        ...
        service_tier: options.serviceTier as any,
    ...
    if (options.promptConfig) {
        ...
        (body as any).prompt = removeEmptyProperties({
            id: prompt.promptId,
    ...
    if (options.textFormat) {
        ...
        // CAMBIO: Usamos any para el objeto de configuración de texto
        ...
        (body as any).text = textConfig;
    }

    if (options.reasoningEffort) {
        (body as any).reasoning = {
