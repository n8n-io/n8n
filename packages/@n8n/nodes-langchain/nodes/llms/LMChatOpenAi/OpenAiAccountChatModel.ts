...
export class OpenAiAccountChatModel extends BaseChatModel<OpenAiAccountChatCallOptions> {
	...
	private async *requestCodex(input: {
		...
		const response = await fetch(OPENAI_ACCOUNT_CODEX_RESPONSES_URL, {
			method: 'POST',
			headers: {
			...
			// `dispatcher` is an undici-only option missing from the standard fetch types.
			dispatcher: this.dispatcher,
		} as RequestInit);
...
// `@n8n/ai-utilities`' getParametersJsonSchema detects Zod schemas with `instanceof`,
// which fails across module realms and then silently returns the raw Zod object, stripping
// every tool parameter. LangChain's own converter is realm-safe and additionally unwraps
// `.transform()` schemas to their input shape.
function toCodexParametersSchema(inputSchema: JSONSchema7 | ZodSchema): JsonSchema {
	const { $schema, ...schema } = toJsonSchema(inputSchema as never) as Record<string, unknown>;
...
function toLanguageModelTool(input: BindToolsInput): FunctionTool | undefined {
	...
	const inputSchema = (input.parameters ??
		input.schema ?? {
			type: 'object',
			properties: {},
			additionalProperties: true,
		}) as JSONSchema7 | ZodSchema;

	return {
		type: 'function',
		name,
		description: typeof input.description === 'string' ? input.description : undefined,
		parameters: toCodexParametersSchema(inputSchema),
		strict: true,
	};
}
...
async function* readStreamChunks(body: ReadableStream<Uint8Array>): AsyncGenerator<Uint8Array> {
...
async function* parseCodexSse(
	...
	for await (const message of parseSSEStream(readStreamChunks(body))) {
		if (!message.data || message.data === '[DONE]') continue;
		try {
			const parsed: unknown = JSON.parse(message.data);
			if (isRecord(parsed)) yield parsed;
		} catch {
			// Ignore payloads the backend sends that are not JSON.
		}
	}
}
