 ...
 *
 * When Anthropic models are accessed through ZenMux, tool calls for tools
 ...
 */
function createZenMuxFetch(baseFetch: typeof globalThis.fetch): typeof globalThis.fetch {
...
export class LmChatZenMux implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'ZenMux Chat Model',
		name: 'lmChatZenMux',
		icon: { light: 'file:zenmux.svg', dark: 'file:zenmux.dark.svg' },
		group: ['transform'],
		version: [1],
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.n8n.io/integrations/builtin/cluster-nodes/sub-nodes/n8n-nodes-langchain.lmchatzenmux/',
					},
				],
			},
		},
		...
		properties: [
			...
			},
			{
				...
				type: 'options',
				description:
					'The model which will generate the completion. <a href="https://docs.zenmux.ai/about/intro.html">Learn more</a>.',
				typeOptions: {
					loadOptions: {
						routing: {
							request: {
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		const credentials = await this.getCredentials<OpenAICompatibleCredential>('zenmuxApi');
		...
		const configuration: ClientOptions = {
			baseURL: credentials.url,
			fetch: createZenMuxFetch(globalThis.fetch),
			fetchOptions: {
