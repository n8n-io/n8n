...
export class LmChatMiniMax implements INodeType {
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				type: 'options',
				description:
					'The model which will generate the completion. <a href="https://platform.minimaxi.com/document/Models">Learn more</a>.',
				options: [
					{
						name: 'MiniMax-M3',
						value: 'MiniMax-M3',
			...
			},
			{
				...
				options: [
					...
					},
					{
						...
						default: -1,
						description:
							'The maximum number of tokens to generate in the completion. MiniMax-M3 supports up to 128,000 output tokens.',
						type: 'number',
						typeOptions: {
							maxValue: 128000,
						},
					...
					},
					{
						...
						description:
							'Controls randomness: Lowering results in less random completions. MiniMax requires temperature to be between 0.01 and 1.',
						type: 'number',
					},
					{
						displayName: 'Timeout',
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		if (options.temperature !== undefined) {
			options.temperature = Math.max(0.01, Math.min(1, options.temperature));
		}
