...
describe('LmFakeTesting Node', () => {
	...
	describe('description', () => {
		it('should have the expected node properties', () => {
			...
			// Hidden property is set to true for testing purposes
	...
	describe('supplyData', () => {
		...
		it('should supply error-throwing FakeStreamingChatModel for error response type', async () => {
			mockSupplyDataFunction.getNodeParameter.mockImplementation(
				(param, _itemIndex, defaultValue) => {
					switch (param) {
						case 'responseType':
			...
			await expect((result.response as FakeStreamingChatModel).invoke('test')).rejects.toThrow(
