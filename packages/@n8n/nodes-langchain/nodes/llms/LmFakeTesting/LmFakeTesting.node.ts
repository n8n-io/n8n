...
export class LmFakeTesting implements INodeType {
	description: INodeTypeDescription = {
		...
		properties: [
			...
			{
				displayName: 'Loop Over Responses',
				name: 'loopResponses',
				type: 'boolean',
				default: false,
				description:
					'Whether to loop over responses if the Fake LLM gets called more times than the number of responses',
				displayOptions: {
					hide: {
						responseType: ['error'],
					},
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		const responseType = this.getNodeParameter('responseType', itemIndex) as 'sequence' | 'error';
		...
		const loopResponses = this.getNodeParameter('loopResponses', itemIndex, false) as boolean;
