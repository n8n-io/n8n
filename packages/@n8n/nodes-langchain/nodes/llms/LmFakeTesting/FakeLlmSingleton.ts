...
import type { FakeChatModel } from '@langchain/core/utils/testing';
import { FakeStreamingChatModel } from '@langchain/core/utils/testing';

import { SequentialFakeStreamingChatModel } from './SequentialFakeStreamingChatModel';
import type { FakeLlmConfig, FakeLlmResponse } from './types';
	...
	private recreateLlmInstance(callbacks?: any[]): void {
		...
		if (shouldThrowError) {
			...
			this.fakeLlmInstance = new FakeStreamingChatModel({
				callbacks: callbacks || [],
				thrownErrorString: errorMessage || 'Fake LLM error for testing',
			});
			...
			if (responseType === 'sequence') {
				...
				this.fakeLlmInstance = new SequentialFakeStreamingChatModel(
					chunks,
					callbacks,
					this.currentConfig.loopResponses,
				);
			} else {
