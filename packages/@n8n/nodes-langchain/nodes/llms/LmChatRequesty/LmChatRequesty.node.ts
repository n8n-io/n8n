 ...
 *
 * When Anthropic models are accessed through Requesty, tool calls for tools
 * with no parameters may return empty string arguments ("") instead of "{}".
 ...
 */
function createRequestyFetch(baseFetch: typeof globalThis.fetch): typeof globalThis.fetch {
...
export class LmChatRequesty implements INodeType {
	description: INodeTypeDescription = {
		...
		name: 'lmChatRequesty',
		icon: { light: 'file:requesty.svg', dark: 'file:requesty.dark.svg' },
		group: ['transform'],
		version: [1],
		...
		properties: [
			...
			},
			{
				...
				type: 'options',
				description:
					'The model which will generate the completion. <a href="https://docs.requesty.ai">Learn more</a>.',
				typeOptions: {
					loadOptions: {
						routing: {
							request: {
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		const credentials = await this.getCredentials<OpenAICompatibleCredential>('requestyApi');
		...
		const configuration: ClientOptions = {
			baseURL: credentials.url,
			fetch: createRequestyFetch(globalThis.fetch),
			fetchOptions: {
