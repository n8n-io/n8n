...
import { LmChatRequesty } from '../LmChatRequesty.node';
...
describe('LmChatRequesty', () => {
	let node: LmChatRequesty;

	const mockNodeDef: INode = {
		id: '1',
		name: 'Requesty Chat Model',
		typeVersion: 1,
		type: 'n8n-nodes-langchain.lmChatRequesty',
		position: [0, 0],
		parameters: {},
	};
	...
	describe('node description', () => {
		...
		it('should require requestyApi credentials', () => {
	...
	describe('supplyData', () => {
		it('should create ChatOpenAI with basic configuration', async () => {
			...
			expect(ctx.getCredentials).toHaveBeenCalledWith('requestyApi');
	...
	describe('fetch wrapper (empty tool call arguments fix)', () => {
		...
		it.each<{ input: unknown; expected: string; label: string }>([
			...
			const response = await wrappedFetch('https://router.requesty.ai/v1/chat/completions');
		...
		it('should pass through JSON responses without choices', async () => {
			...
			const wrappedFetch = await setupFetchWrapper(mockFetch);
			const response = await wrappedFetch('https://router.requesty.ai/v1/models');
