...
// only. The sheet has the columns row_id, employee_id, field, new_value, status, result.
// The next nodes read them.
const readChangeRequests = node({
...
// [spreadsheet] Google Sheets. Swap for Microsoft Excel 365 or Airtable: replace this node
// only. It reads the Validate Change fields.
const logResult = node({
	type: 'n8n-nodes-base.googleSheets',
	version: 4.7,
	config: {
		name: 'Log Result',
