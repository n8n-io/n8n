...
describe('resolveSandboxConfig', () => {
	describe('when sandbox is disabled by caller', () => {
		it('returns a disabled config regardless of env', () => {
			const env = baseEnv({
				DAYTONA_API_URL: 'https://app.daytona.io/api',
				DAYTONA_API_KEY: 'dtn_xxx',
			});
			const config = resolveSandboxConfig(env, { sandbox: false });
			expect(config.enabled).toBe(false);
		});
	});

	describe('when sandbox is enabled by caller', () => {
		...
		it('throws a clear error when DAYTONA_API_KEY is missing', () => {
			...
			expect(() => resolveSandboxConfig(env, { sandbox: true })).toThrow(/DAYTONA_API_KEY/);
		});

		it('throws a clear error when DAYTONA_API_URL is missing', () => {
			const env = baseEnv({ DAYTONA_API_KEY: 'dtn_xxx' });
			expect(() => resolveSandboxConfig(env, { sandbox: true })).toThrow(/DAYTONA_API_URL/);
		});
		...
		it('throws a clear error when provider=n8n-sandbox without serviceUrl', () => {
			...
			expect(() => resolveSandboxConfig(env, { sandbox: true })).toThrow(/N8N_SANDBOX_SERVICE_URL/);
		});

		it('rejects an unknown provider', () => {
			const env = baseEnv({ N8N_INSTANCE_AI_SANDBOX_PROVIDER: 'gvisor' });
			expect(() => resolveSandboxConfig(env, { sandbox: true })).toThrow(/provider/);
		});
	});
});
