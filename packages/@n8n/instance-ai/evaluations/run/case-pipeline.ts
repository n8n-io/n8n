	...
	const runScenarioRow = async (
		...
		if (agentRunnable && agentRef) {
			...
			return await attachExpectations({
				...
				...(agentResult.shadowJudge ? { shadowJudge: agentResult.shadowJudge } : {}),
				...(agentResult.judgeUsage ? { judgeUsage: agentResult.judgeUsage } : {}),
		...
		const runWorkflowScenario = async (): Promise<TargetOutput> => {
			...
			return await attachExpectations({
				...
				...(result.shadowJudge ? { shadowJudge: result.shadowJudge } : {}),
				...(result.judgeUsage ? { judgeUsage: result.judgeUsage } : {}),
