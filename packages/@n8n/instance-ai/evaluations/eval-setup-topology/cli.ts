...
function parseArgs(argv: string[]): CliArgs {
	const args: CliArgs = {
		...
		concurrency: 1,
		keepArtifacts: false,
		output: resolve(process.cwd(), 'eval-setup-topology-results.json'),
...
async function main(): Promise<void> {
	...
	const cases = loadEvalSetupTopologyCases({ filter: args.filter });
	if (cases.length === 0) {
		throw new Error('No eval setup topology cases found');
	}
	...
	const results = await Promise.all(
		cases.map((testCase) =>
			limit(async () => {
				...
				const result = await runEvalSetupTopologyCase({
					client,
					testCase,
					logger,
