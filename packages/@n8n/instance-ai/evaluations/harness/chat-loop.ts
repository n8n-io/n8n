...
async function waitForBackgroundTasks(config: WaitConfig, timeoutMs: number): Promise<void> {
	...
	if (!hasSpawnedAgents) {
		config.logger.verbose('No background agents spawned -- skipping background task wait');
		return;
	}

	config.logger.verbose('Background agent(s) detected -- waiting for background tasks...');
