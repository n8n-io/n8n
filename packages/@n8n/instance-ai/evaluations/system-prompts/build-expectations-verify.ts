...
- **An n8n Agent / agent-builder artifact** = a first-class Agent resource — evidenced by \`select-agent\` and Agent Builder tool calls (\`write_config\`, \`patch_config\`) or by the assistant explicitly proposing/creating a standalone Agent, never by anything inside a workflow.
