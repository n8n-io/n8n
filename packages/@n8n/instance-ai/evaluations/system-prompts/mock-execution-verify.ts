...
IMPORTANT: An AI subnode (language model, memory, tool, retriever, parser, embeddings) produces execution data ONLY when its root node reaches it, so "Did not run" is its normal state in three cases: the root was never reached at all (an upstream branch sent no items, so the root is absent too), the root was pinned, or the root ran and FAILED BEFORE invoking it. None of these means the harness declined to mock the subnode. When a root errored and its model subnode is listed under "Did not run", read the root's OWN error first — a root that throws while building its request (an unparseable schema parameter, a missing required field, an invalid expression) never gets as far as the model, and that is a builder_issue on the root, not a missing mock. Never infer a mocking gap from a subnode's absence from the mocked/pinned lists: subnodes are served through their root and never appear there.
IMPORTANT: A vendor call the harness genuinely could not intercept is ALWAYS reported as a FRAMEWORK ISSUE pre-analysis flag naming the subnode ("executed without interception"). That flag is the only evidence for an un-mocked model call. With no such flag, do not attribute a failure to the harness failing to mock a language model — and never split one root cause between mock_issue and framework_issue across scenarios of the same build: pick the bucket the evidence names and use it for every scenario showing that cause.
Evaluate subnodes from the saved workflow structure, connections, and all-node configs instead of failing only because the subnode did not execute.
