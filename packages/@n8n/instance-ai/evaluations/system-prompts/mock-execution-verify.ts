...
IMPORTANT: An AI subnode (language model, memory, tool, retriever, parser, embeddings) produces execution data ONLY when its root node reaches it, so "Did not run" is its normal state in two cases: the root was pinned, or the root ran and FAILED BEFORE invoking it. Neither means the harness declined to mock the subnode. When a root errored and its model subnode is listed under "Did not run", read the root's OWN error first — a root that throws while building its request (an unparseable schema parameter, a missing required field, an invalid expression) never gets as far as the model, and that is a builder_issue on the root, not a missing mock. Never infer a mocking gap from a subnode's absence from the mocked/pinned lists: subnodes are served through their root and never appear there.
