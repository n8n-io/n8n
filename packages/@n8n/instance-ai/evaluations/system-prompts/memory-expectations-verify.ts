...
2. **Raw message window**: the actual messages the model received on its last step. On an uncompressed thread this is where the facts live. Note what this is *not*: it is not the full conversation. Messages evicted from the window do not appear here — that is the point, and it is why you can trust it as a record of what the agent still had.
