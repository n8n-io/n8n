...
function parseArgs(argv: string[]): CliArgs {
	const args: CliArgs = {
		...
		concurrency: 1,
		keepArtifacts: false,
		output: resolve(process.cwd(), 'eval-proactive-offer-results.json'),
...
async function main(): Promise<void> {
	...
	const cases = loadEvalProactiveOfferCases({ filter: args.filter });
	if (cases.length === 0) {
		throw new Error('No eval proactive-offer cases found');
	}
	...
	const results = await Promise.all(
		cases.map((testCase) =>
			limit(async () => {
				logger.info(`RUN ${testCase.slug} (expectsOffer=${String(testCase.expectsOffer)})`);
				const result = await runEvalProactiveOfferCase({
					client,
					testCase,
					logger,
	...
	const runResult: EvalProactiveOfferRunResult = {
