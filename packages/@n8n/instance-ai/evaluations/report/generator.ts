...
function renderWorkflowsSection(result: InstanceAiResult, n8nBaseUrl: string): string {
	...
	for (let i = 0; i < result.outcome.workflowsCreated.length; i++) {
		...
		html += `<div style="margin:8px 0;padding:8px;background:#161b22;border:1px solid #30363d;border-radius:4px;font-size:12px">
			...
			<a href="${escapeHtml(workflowUrl)}" target="_blank" rel="noopener" style="margin-left:8px;color:#58a6ff;text-decoration:none;font-size:12px">Open in n8n &rarr;</a>
		</div>`;
		...
		<tr id="detail-${index}" class="detail-row" data-complexity="${result.complexity ?? 'medium'}" data-prompt-hash="${promptHash}" style="display:none">
			<td colspan="10">
				<div class="detail-content">
							...
							${result.checklist
								.map((item) => {
									...
									return `<li class="${passed ? 'pass' : 'fail'}">
										...
										<span class="badge badge-strategy-${item.strategy}">${item.strategy}</span>
					...
					${renderWorkflowsSection(result, n8nBaseUrl)}
...
<style>
	* { margin: 0; padding: 0; box-sizing: border-box; }
	...
	.badge-strategy-programmatic { background: #1f6feb33; color: #58a6ff; }
	.badge-strategy-llm { background: #8957e533; color: #d2a8ff; }
