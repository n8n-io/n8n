...
export async function gradeTaskCompleted(
	...
	const userMessage = HUMAN_TEMPLATE.replace('{category}', scenarioCategory)
		.replace('{userPrompt}', userPrompt)
		.replace('{criteria}', grader.criteria ?? '(none — judge against the user request alone)')
		.replace('{finalText}', trace.finalText.slice(0, MAX_FINAL_TEXT_CHARS))
		.replace('{toolCallSummary}', summarizeToolCalls(trace));
