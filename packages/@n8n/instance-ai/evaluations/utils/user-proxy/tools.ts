...
const sendFollowUpMessageDecisionSchema = z.object({
	...
	runWorkflowId: z
		...
		.optional()
		.describe(
			'Workflow ID to run before sending this message. Set only when the director says the user runs it. Use the intended workflow ID from a successful build result, not a prerequisite or cleanup workflow. This runs normally, without mocks, and supports credential-free manual or schedule workflows only.',
		),
});
