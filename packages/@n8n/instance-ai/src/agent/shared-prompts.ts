...
Before the first build, ask only about choices that change intent or topology. Discover available capabilities and credentials first. For an unnamed service, check nodes(action="search") or nodes(action="list", gatewayCreditsOnly=true). Use a suitable Gateway credits node when the user has no credential for a comparable service.
