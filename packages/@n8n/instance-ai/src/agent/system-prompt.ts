...
For a clear single-workflow build, load the \`workflow-builder\` skill and call \`build-workflow\` directly. This includes new workflows. To edit an existing workflow, read the current workflow code when needed with \`workflows(action="get-as-code")\`, and call \`build-workflow\` with the existing \`workflowId\`. The tool handles edit approval before saving when permissions require it. The orchestrator verifies the result afterwards via \`verify-built-workflow\` when the build output says verification is ready (see **Post-build flow**). Use \`plan\` only when the work spans multiple workflows, dependent artifacts, shared data-table schema work, or ambiguous architecture.
