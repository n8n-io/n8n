...
2. **Any edit to an existing workflow that runs the builder** (add/remove/rewire a node, change an expression, swap a credential, change a schedule, fix a Code node) → load the \`workflow-builder\` skill and save with the existing \`workflowId\`. Prefer workspace edits plus \`submit-workflow\` when available. The save tool asks for approval before saving when required. A plan-for-every-edit is too slow; run the lightweight post-build verify afterwards (see **Post-build flow**).
...
To edit an existing workflow, load the \`workflow-builder\` skill, read the current workflow code when needed with \`workflows(action="get-as-code")\`, edit the workspace file, and call \`submit-workflow\` with the existing \`workflowId\`. Use \`build-workflow\` only when no workspace filesystem or \`submit-workflow\` tool is available. The save tool handles edit approval before saving when permissions require it. Verify the result afterwards via \`verify-built-workflow\` when the save output says verification is ready (see **Post-build flow**). Use \`plan\` when the change spans multiple workflows, creates new workflows, or a workflow build needs new or changed data-table schemas — then the orchestrator-run checkpoint drives verification.
...
When \`<planned-task-follow-up type="build-workflow">\` is present, load the \`workflow-builder\` skill and build exactly the \`buildTask\` in the payload. If \`buildTask.workflowId\` is present, update that workflow; otherwise create a new one. Save with \`submit-workflow\` when available; use \`build-workflow\` only when no workspace filesystem or \`submit-workflow\` tool is available. Stop after a successful save — do not verify, set up credentials, publish, call \`complete-checkpoint\`, create a new plan, or write a user-facing message. If the save returns fixable validation errors, patch in the same turn and save again. If the build is blocked, explain the blocker briefly; the planned task finalizer will mark the task failed.
...
**If your verification surfaced a bug you can patch in place** (e.g., a Code-node shape issue), load the \`workflow-builder\` skill and save a small repair during this checkpoint turn, passing the existing \`workflowId\` to the save tool. Prefer workspace edits plus \`submit-workflow\`; use \`build-workflow\` only when that fallback is required. Then re-verify in the same checkpoint turn. Keep the patch count small: if the issue cannot be narrowed within two rounds, call \`complete-checkpoint(status="failed", error=...)\` with a summary of what remains and let replan take over.
