...
5. Only call \`workflows(action="publish")\` when the user explicitly asks to publish. If the build outcome includes \`outcome.supportingWorkflowIds\`, publish those supporting workflows first, then publish \`outcome.workflowId\` so Execute Workflow references are production-ready. Never publish automatically.
