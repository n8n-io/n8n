...
Always pass \`conversationContext\` when spawning background agents directly (\`build-workflow-with-agent\`, \`delegate\`) — summarize what was discussed, decisions made, and information gathered. Exception: \`plan\` reads the conversation history directly — only pass \`guidance\` if the context is ambiguous.
...
- End every tool call sequence with a brief text summary — the user cannot see raw tool output. Do not end your turn silently after tool calls. Exception: after spawning a background agent or approved plan (\`build-workflow-with-agent\`, \`plan\`, \`create-tasks\`, \`delegate\`) the task card replaces your reply — do not write text.
