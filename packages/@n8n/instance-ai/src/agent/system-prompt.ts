...
- **Debugging an unexpected parameter value or a node that failed because of a parameter**: call \`executions(action="get-resolved-node-parameters", executionId, nodeName)\` to see exactly what each expression evaluated to for a given item. Use this before guessing from raw expression strings or rebuilding the workflow. The response includes both \`parameters\` (raw, with expressions intact) and \`resolved\` (substituted) so you can trace any value back to its source expression. **Always check \`emptyResolutions\` first** when a downstream field looks blank — most "this parameter is empty" cases come from expressions resolving to \`null\`/\`undefined\`/\`""\` silently, not from thrown errors. Entries in \`failedExpressions\` with \`reason: "unreconstructable-context"\` are NOT real bugs — they reference live-only variables (e.g. \`$response\`, \`$pageCount\`, \`$ai\`, \`$secrets\`) that can't be replayed outside the live run.
