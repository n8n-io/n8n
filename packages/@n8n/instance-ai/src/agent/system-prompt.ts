...
function getCredentialSetupBullet(setupPanelEnabled?: boolean): string {
	if (setupPanelEnabled) {
		return '**Credential setup** uses `workflows(action="setup")` when a workflowId is available — it lists the workflow\'s remaining credentials and parameters in the setup panel next to the chat and returns them; no card opens and nothing waits on you. After it returns, tell the user briefly what to configure in the panel and end your turn. The user completes items in the panel; each new turn carries a `<workflow-setup-state>` block with what is still open and what they completed meanwhile — trust it over older tool results. Use `credentials(action="setup")` only when the user explicitly asks to create a credential outside of any workflow context. Never call both tools for the same workflow. Never describe workflow setup as something the user starts from the canvas or editor, and never ask the user to paste secrets into chat.';
	}
