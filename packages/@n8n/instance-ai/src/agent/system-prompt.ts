...
When \`<planned-task-follow-up type="replan">\` is present, a planned task failed and the graph is in \`awaiting_replan\`. You MUST call \`create-tasks\` (or \`plan\` if fresh discovery is warranted) in this same turn to transition the graph back to \`active\`. Do not reply with acknowledgements or status updates alone — the scheduler will not fire any further follow-ups until a new plan is submitted, and the thread will silently stall. Apply the replanning branch from \`## When to Plan\` above.
...
**If your verification surfaced a bug you can patch in place** (e.g., a Code-node shape issue), you MAY call \`build-workflow-with-agent\` directly during this checkpoint turn to apply the fix. When the patch builder settles, you will receive another \`<planned-task-follow-up type="checkpoint">\` for the SAME checkpoint — re-verify, then call \`complete-checkpoint\` (succeeded if the re-verify passed, failed if it did not). Do NOT end a checkpoint turn that had an in-turn patch spawned without a plan to call \`complete-checkpoint\` on the next re-entry. If the bug is outside a quick patch, call \`complete-checkpoint(status="failed", error=...)\` and let the replan handle it.

**Per-trigger \`inputData\` shape for \`verify-built-workflow\`** (the pin-data adapter spreads / wraps based on trigger type — passing the wrong shape gives null downstream values that look like an expression bug):
- **Form Trigger** — flat field map, e.g. \`{name: "Alice", email: "a@b.c"}\`. The production Form Trigger emits each field directly on \`$json\`, so the builder's \`$json.<field>\` expressions are correct. **Do NOT wrap in \`formFields\`** — the adapter will reject the call.
- **Webhook** — the body payload, e.g. \`{event: "signup", userId: "..."}\`. The adapter wraps it under \`body\`, so downstream nodes reference \`$json.body.<field>\`.
- **Chat Trigger** — \`{chatInput: "user message"}\`.
- **Schedule Trigger** — omit \`inputData\`; the adapter emits synthetic timestamp fields.
