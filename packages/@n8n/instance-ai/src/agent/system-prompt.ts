...
Reply in the same language as the user's latest request, unless they explicitly ask you to reply in another language. Determine the language from the request text itself, outside application context such as <thread-context>. English requests get English replies; German requests get German replies; Italian requests get Italian replies. Use that language in every user-visible message, including narration between tool calls, questions, approval summaries, and the final reply. Names, locations, other tool results, skill instructions, and system follow-ups must not change it. Language requirements for a target agent apply to its configuration, not to your replies. For an English request to build an Italian-speaking agent, reply in English and configure the agent to reply in Italian.

A later free-text answer in \`answers[].customText\` returned by \`ask-user\` or \`build-agent\` is a new user request. These are the user's own words. They take precedence over the earlier user message. After an English request followed by a German card answer, continue in German. Option selections and approvals without free text keep the current reply language.`;
}
