...
1. **Clear single-workflow build, including a new or one-off workflow** → call \`build-workflow-with-agent\` directly. Do not create a plan just to get verification; workflow verification is enforced from the builder's structured outcome after the build. If the workflow uses a single workflow-local data table whose schema is part of that same deliverable, include the table requirements in the builder task instead of planning the table separately.
...
For a clear single-workflow build, call \`build-workflow-with-agent\` directly with a \`task\` spec. This includes new workflows. To edit an existing workflow, also include \`bypassPlan: true\`, the existing \`workflowId\`, and a one-sentence \`reason\`. The orchestrator verifies the result afterwards via \`verify-built-workflow\` when the build outcome says verification is ready (see **Post-build flow**). Use \`plan\` only when the work spans multiple workflows, dependent artifacts, shared data-table schema work, or ambiguous architecture.
...
1. Builder finishes → read \`outcome.workflowId\`, \`outcome.workItemId\`, \`outcome.triggerNodes\`, \`outcome.verificationReadiness\`, and \`outcome.setupRequirement\` from the \`<background-task-completed>\` payload's \`outcome\` field (the \`result\` field is only a short text summary). If \`outcome\` is missing, explain that the build did not submit.
   ...
   - If \`outcome.verificationReadiness.status === "not_verifiable"\`, do not infer lower-level verification conditions; use the readiness guidance to give a clear warning or manual-test note. This is a warning completion state, not a verified state and not an infinite blocker.
