...
const activityInputSchema = z.object({
	...
	id: z
		...
		.int()
		.optional()
		.describe('For `expand`: the bracketed id of the entry, as shown in the activity list.'),
	category: z
		.string()
		.optional()
		.describe(
			'For `list`: restrict to one kind — "workflow", "execution", "eval" or "credential".',
		),
	resourceId: z
		.string()
		.optional()
	...
	return new Tool(DOMAIN_TOOL_IDS.ACTIVITY)
		.description(
			'Read the instance activity log — what has recently been created, changed, published, ' +
				'run, or failed. You are given a window of it at the start of a turn; use this to look ' +
				'further back, to filter to one kind of event or one resource, or to open a single ' +
				'entry with `expand`. Expanding an entry also returns everything else the log knows ' +
				"about the same resource, which is how you see a workflow's recent history at a " +
				'glance. Read-only, and it returns log entries rather than live records — use ' +
		...
		.handler(async (input) => {
			...
			if (!service) {
				throw new Error('The activity log is not enabled on this instance.');
			}
			...
			if (input.action === 'expand') {
				...
				// Entries are pruned, so a stale id is expected rather than exceptional — the agent
				// should carry on, not treat it as a failure.
