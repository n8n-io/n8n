...
import {
	...
	createPersistWorkflowTool,
} from '../persist-workflow.tool';
...
describe('createPersistWorkflowTool', () => {
	beforeEach(() => {
		vi.clearAllMocks();
	...
	it("offers no projectId — a build writes to the conversation's own project", () => {
		expect(persistWorkflowInputSchema.shape).not.toHaveProperty('projectId');
		...
		// wrote to.
		expect(() =>
			persistWorkflowInputSchema.parse({
	...
	it('describes itself as the internal persistence step behind build-workflow', () => {
		...
		const tool = createPersistWorkflowTool(context);
	...
	describe('publish state', () => {
		it('warns that a save to a published workflow is not live', async () => {
			...
			>(createPersistWorkflowTool(context), { filePath, workflowId: 'wf-1' });
	...
	describe('folder placement', () => {
		...
		it('does not advertise folderPath while folder exploration is off', () => {
			const { context, filePath } = makeContext({});
			const schema = getInputSchema(createPersistWorkflowTool(context));
		...
		it('advertises the folder output only while folder exploration is on', () => {
			...
			expect(outputShape(createPersistWorkflowTool(makeContext({}).context))).not.toHaveProperty(
				'folder',
			);
			expect(
				outputShape(
		...
		it('creates a new workflow inside the named folder and reports where it landed', async () => {
			...
			const result = await executeTool<BuildToolOutput & { folder?: unknown }>(
				createPersistWorkflowTool(context),
		...
		it('fails the build before saving when the folder does not resolve', async () => {
			...
			const result = await executeTool<BuildToolOutput>(createPersistWorkflowTool(context), {
				filePath,
	...
	it.each([true, false])(
		...
		async (approved) => {
			...
			const input = persistWorkflowInputSchema.parse({ filePath, workflowId: 'wf-existing' });
			const suspend = vi.fn();
			const result = await executeTool<BuildToolOutput>(createPersistWorkflowTool(context), input, {
				resumeData: { approved },
				suspend,
			});
	...
	it('shows the concrete summary before saving a workflow', async () => {
		...
		const input = persistWorkflowInputSchema.parse({
			filePath,
			workflowId: 'wf-existing',
			approvalSummary: 'Add a Slack notification after the check',
		});
		const suspend = vi.fn();
		await executeTool(createPersistWorkflowTool(context), input, { suspend });
	...
	it('preserves setup-applied placeholder values before updating an existing workflow', async () => {
		...
		await executeTool<BuildToolOutput>(createPersistWorkflowTool(context), {
			filePath,
			workflowId: 'wf-existing',
		});
	...
	it('rejects obsolete inline build payload fields', () => {
		expect(
			persistWorkflowInputSchema.safeParse({
	...
	it('rejects structurally invalid source paths at the schema layer', () => {
		expect(persistWorkflowInputSchema.safeParse({ filePath: '../main.workflow.ts' }).success).toBe(
			false,
		);
		expect(persistWorkflowInputSchema.safeParse({ filePath: '~/main.workflow.ts' }).success).toBe(
			false,
		...
		expect(
			persistWorkflowInputSchema.safeParse({ filePath: '/src/../../../etc/passwd' }).success,
		).toBe(false);
		expect(
			persistWorkflowInputSchema.safeParse({ filePath: 'src\\main.workflow.ts' }).success,
		).toBe(false);
	});
	...
	it('accepts absolute paths at the schema layer so the handler can resolve them', () => {
		...
		expect(
			persistWorkflowInputSchema.safeParse({ filePath: '/tmp/main.workflow.ts' }).success,
		).toBe(true);
	});
	...
	it('routes source-declared node outputs through the simulation plan', async () => {
		...
		await executeTool<BuildToolOutput>(createPersistWorkflowTool(context), { filePath });
