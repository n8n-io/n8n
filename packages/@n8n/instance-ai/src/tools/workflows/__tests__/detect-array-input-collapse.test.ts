...
describe('detectArrayInputCollapse', () => {
	const codes = async (w: WorkflowJSON) => (await detectArrayInputCollapse(w)).map((x) => x.code);

	it('flags the items[0].json + .slice pattern fed by HTTP (INS-662)', async () => {
		const js =
			'const storyIds = items[0].json;\nconst top = storyIds.slice(0, 3);\nreturn top.map(id => ({ json: { id } }));';
		const result = await detectArrayInputCollapse(
	...
	it('flags $input.first().json.map applied directly (Binance-style)', async () => {
		const js = 'return $input.first().json.map(row => ({ json: { close: row[4] } }));';
		expect(await codes(workflow('https://api.binance.com/api/v3/klines', js))).toEqual([
			'ARRAY_INPUT_COLLAPSED_TO_FIRST_ITEM',
		]);
	});

	it('flags Array.isArray($input.first().json) guard fed by HTTP', async () => {
		const js =
			'const ids = Array.isArray($input.first().json) ? $input.first().json : [];\nreturn ids.map(id => ({ json: { id } }));';
		expect(await codes(workflow('https://example.com/list.json', js))).toEqual([
			'ARRAY_INPUT_COLLAPSED_TO_FIRST_ITEM',
		]);
	});

	it('flags $input.item.json + .slice in runOnceForEachItem mode (HN build form)', async () => {
		const js =
			'const storyIds = $input.item.json;\nconst topThree = storyIds.slice(0, 3);\nreturn topThree.map(id => ({ json: { storyId: id } }));';
		expect(
			await codes(workflow('https://hacker-news.firebaseio.com/v0/topstories.json', js)),
		).toEqual(['ARRAY_INPUT_COLLAPSED_TO_FIRST_ITEM']);
	});

	it('does NOT flag the correct $input.all() pattern', async () => {
		const js =
			'const ids = $input.all().map(i => i.json);\nreturn ids.slice(0, 3).map(id => ({ json: { id } }));';
		expect(await codes(workflow('https://example.com/list.json', js))).toEqual([]);
	});

	it('does NOT flag an array op on a sub-field of the first item (single-object response)', async () => {
		const js =
			'const rows = $input.first().json.results.slice(0, 3);\nreturn rows.map(r => ({ json: r }));';
		expect(await codes(workflow('https://example.com/search.json', js))).toEqual([]);
	});

	it('does NOT flag the pattern when the upstream is not an HTTP Request node', async () => {
		const js = 'const ids = items[0].json;\nreturn ids.slice(0, 3).map(id => ({ json: { id } }));';
		expect(await codes(workflow('n/a', js, 'n8n-nodes-base.set'))).toEqual([]);
	});
});
