...
vi.mock('../persist-workflow.tool', () => ({
	confirmationSuspendSchema: { parse: (value: unknown) => value },
	createPersistWorkflowTool: () => ({ name: 'persist-workflow', handler: persistHandler }),
}));
...
function handlerOf(tool: ReturnType<typeof createBuildWorkflowTool>): Handler {
...
describe('build-workflow tool', () => {
	...
	it('edits an existing workflow and saves under its id', async () => {
		const context = makeContext();
		const created = await handlerOf(createBuildWorkflowTool(context))(
			{ action: 'create', request: REQUEST },
			{},
		);
		...
		const output = await handlerOf(createBuildWorkflowTool(context))(
			{
				action: 'edit',
				workflowId: 'wf-9',
	...
	it('debugs the latest failed execution when no executionId is given', async () => {
		const context = makeContext();
		await handlerOf(createBuildWorkflowTool(context))({ action: 'create', request: REQUEST }, {});
		const compiled = JSON.parse(persistHandler.mock.calls[0][0].sourceCode) as WorkflowJSON;
		vi.mocked(context.workflowService.getAsWorkflowJSON).mockResolvedValue(compiled);
	...
	it('reports a denied save as denied and a missing workflow as failed', async () => {
		...
		const denied = await handlerOf(createBuildWorkflowTool(context))(
			{ action: 'create', request: REQUEST },
			{},
		);
		...
		const missing = await handlerOf(createBuildWorkflowTool(context))(
