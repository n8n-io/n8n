...
export const persistWorkflowInputSchema = z
	.object({
		filePath: z
			.string()
			.min(1)
...
export const persistWorkflowInputSchemaWithFolderPlacement = persistWorkflowInputSchema
	.extend({
		folderPath: z.string().optional().describe(FOLDER_PATH_PLACEMENT_DESCRIPTION),
	})
	.strict();

function pickPersistWorkflowInputSchema(context: InstanceAiContext) {
	return context.folderExplorationEnabled === true
		? persistWorkflowInputSchemaWithFolderPlacement
		: persistWorkflowInputSchema;
}
...
const persistWorkflowOutputSchema = z.object({
	success: z.boolean(),
	filePath: z.string(),
...
function pickPersistWorkflowOutputSchema(context: InstanceAiContext) {
	return context.folderExplorationEnabled === true
		? persistWorkflowOutputSchema.extend({
		...
		: persistWorkflowOutputSchema;
}

export function createPersistWorkflowTool(context: InstanceAiContext) {
	...
	return new Tool(PERSIST_WORKFLOW_TOOL_ID)
		...
		.input(pickPersistWorkflowInputSchema(context))
		.output(pickPersistWorkflowOutputSchema(context))
		...
		.handler(async (input, ctx: BuildCtx) => {
			...
			try {
				if (input.sourceCode !== undefined && !context.workspace) {
					// Compiled JSON needs no sandbox: the compiler hands the source in directly.
