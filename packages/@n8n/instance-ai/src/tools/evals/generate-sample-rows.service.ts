...
const REAL_EXAMPLES_MAX_COUNT = 5;
const REAL_EXAMPLE_VALUE_MAX_CHARS = 200;
...
export async function runBatch(input: RunBatchInput): Promise<Array<Record<string, string>>> {
	...
	try {
		...
		const realExamplesBlock = buildRealExamplesBlock(input.realExamples, input.columns);
		...
		sections.push(
			[`Columns: ${input.columns.join(', ')}`, `Generate exactly ${input.rowCount} rows.`].join(
				'\n',
			),
		);
