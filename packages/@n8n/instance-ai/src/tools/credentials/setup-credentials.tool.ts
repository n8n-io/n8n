	...
	return createTool({
		...
		execute: async (input, ctx) => {
			...
			if (!resumeData.approved) {
				return { success: false, denied: true, reason: 'User declined credential setup' };
			}

			// State 3: User requested automatic browser-assisted setup
			...
			// State 4: Approved with credential selections
			return {
				success: true,
