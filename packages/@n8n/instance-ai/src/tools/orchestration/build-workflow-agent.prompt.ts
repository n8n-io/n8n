...
Sandbox mode uses **raw credential objects** (not \`newCredential()\`). Your briefing's \`<artifacts>\` block usually contains \`availableCredentials\` — a pre-fetched list captured at dispatch time. When present, it is authoritative: do NOT call \`credentials(action="list")\`. Each entry has \`id\`, \`name\`, and \`type\`. Wire them into nodes like this:
