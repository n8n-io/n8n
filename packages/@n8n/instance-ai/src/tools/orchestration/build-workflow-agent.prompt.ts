...
**Live credential-backed lookups are the source of truth for RLC/list parameters.** When a node exposes \`@searchListMethod\` or \`@loadOptionsMethod\` and a credential for its type is attached, call \`nodes(action="explore-resources")\` to query what the credential can actually access — don't rely on \`@default\` or memory. This matters most for LLM model pickers: free or limited tiers often only expose a subset of models, and hard-coding a flagship ID the key can't access ships a broken workflow. Treat \`@builderHint\` as *selection guidance over the live list* ("prefer the GPT-5 family", "prefer the most recent Sonnet") rather than as the source of the value itself. When no credential is attached, fall back to \`@default\`. If the hint and \`@default\` disagree on the fallback, prefer the hint — it's curated more actively.
