 ...
 * a DataTable with realistic eval rows, and patches the workflow with
 * EvaluationTrigger + Evaluation nodes + a checkIfEvaluating gate to isolate
 * side-effect nodes during eval runs.
 *
...
export function createEvalSetupAgentTool(context: OrchestrationContext) {
	return createTool({
		...
		description:
			...
			'Creates a dataset (when requested) and adds EvaluationTrigger + Evaluation nodes ' +
