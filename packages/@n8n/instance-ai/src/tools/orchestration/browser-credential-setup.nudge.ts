...
/**
 * Build the input for a nudge `subAgent.stream()` call. Each native agent
 * stream invocation starts a fresh conversation — passing only the
 ...
 */
export function buildNudgeStreamInput<M extends AgentMessage>(
	priorMessages: readonly M[],
): Array<M | AgentMessage> | string {
	if (priorMessages.length === 0) return NUDGE_PROMPT;
	return [...priorMessages, { role: 'user', content: [{ type: 'text', text: NUDGE_PROMPT }] }];
}
