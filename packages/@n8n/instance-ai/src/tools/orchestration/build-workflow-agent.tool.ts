...
/**
 * Wrap a plain `WorkflowBuildOutcome` in the typed `SubAgentOutcome` envelope
 * that `BackgroundTaskResult` and the planned-task scheduler consume. Keeps the
 * helpers above operating on the plain shape for testability while ensuring
 * the run-callback returns satisfy the envelope contract.
 */
function toBuilderSubAgentOutcome(
	taskKey: string,
	startTime: number,
	result: { text: string; outcome: WorkflowBuildOutcome },
): SubAgentOutcome {
...
export async function startBuildWorkflowAgentTask(
	...
	const baseWorkItemId =
		plannedHandoff?.input.workItemId ??
		input.workItemId ??
	...
	const briefing =
		reusedBuilderSession && workflowId
			...
			: await renderHandoff(handoff, context, builderRenderers);
								...
								if (submitTool && 'execute' in submitTool) {
									...
									if (resubmit.success && refreshedAttempt?.success) {
										...
										await compactSuccessfulBuilderMemory({
											...
											lastRequestedChange: builderInput.goal,
											finalText,
											shouldUseBuilderMemory,
										});
