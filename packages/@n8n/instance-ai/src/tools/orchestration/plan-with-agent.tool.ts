...
function toFormattedMessages(messages: PlannerHandoffInput['recentMessages']): FormattedMessage[] {
...
function renderPlannerTask(input: PlannerHandoffInput): string {
	return formatMessagesForBriefing(
		toFormattedMessages(input.recentMessages),
		input.guidance,
		input.timeZone,
...
function shouldAppendCurrentUserMessage(
	messages: PlannerHandoffInput['recentMessages'],
	...
	return lastUserMessage?.text.trim() !== current;
}
