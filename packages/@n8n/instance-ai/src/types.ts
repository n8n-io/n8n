...
export interface ExecutionResult {
	...
	error?: string;
	/** Nodes that errored even when the execution finished `success` (continueOnFail / onError). */
	erroredNodeNames?: string[];
	startedAt?: string;
	finishedAt?: string;
}
...
export interface PlannedTaskService {
	...
	/** Recover a guard-failed build task that has since produced a genuine save:
	 *  re-`succeeded`, re-arm cascade-cancelled dependents, reactivate the graph. */
	...
	/** Transition an `active`/`awaiting_replan` graph → `cancelled` on user Stop. No-op otherwise. */
	cancelActivePlan(threadId: string): Promise<PlannedTaskGraph | null>;
...
export interface WorkflowTaskService {
	...
	/** Look up the latest build outcome for a planned-task id (checkpoints know task ids, not work-item ids). */
	getBuildOutcomeByTask(taskId: string): Promise<WorkflowBuildOutcome | undefined>;
