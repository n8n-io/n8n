...
export type PlannedTaskKind = PlannedHandoffKind | 'checkpoint';
...
export interface CheckpointPlannedTask extends PlannedTaskBase {
	kind: 'checkpoint';
	/** Orchestrator-run verification instructions. Checkpoints do not spawn sub-agents. */
	spec: string;
}

export type PlannedTask = SubAgentPlannedTask | CheckpointPlannedTask;

export type PlannedTaskStatus = 'planned' | 'running' | 'succeeded' | 'failed' | 'cancelled';

interface PlannedTaskRecordBase {
	status: PlannedTaskStatus;
	agentId?: string;
	backgroundTaskId?: string;
	result?: string;
	error?: string;
	outcome?: SubAgentOutcome | Record<string, unknown>;
	startedAt?: number;
	finishedAt?: number;
}

export type PlannedTaskRecord = PlannedTask & PlannedTaskRecordBase;
