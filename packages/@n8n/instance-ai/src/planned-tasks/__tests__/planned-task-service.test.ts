import type { PlannedHandoff, PlannedHandoffKind } from '../../agent/handoff';
...
function makeHandoff(id: string, kind: PlannedHandoffKind = 'build-workflow'): PlannedHandoff {
	switch (kind) {
		case 'delegate':
			return {
				taskKey: id,
		...
		case 'manage-data-tables':
			return {
				taskKey: id,
				kind,
				input: { goal: 'Create a table' },
			};
		case 'research':
			return {
				taskKey: id,
				kind,
				input: { goal: 'Research this' },
			};
	}
}
...
function makeTask(overrides: Partial<PlannedTask> = {}): PlannedTask {
	...
	if (overrides.kind === 'checkpoint') {
		return {
			id,
			title: 'Test task',
			kind: 'checkpoint',
			spec: 'Verify the workflow',
			deps: [],
			...overrides,
		} as PlannedTask;
	}
