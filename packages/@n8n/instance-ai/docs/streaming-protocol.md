...
step-level facts are appended with a per-thread `seq` assigned by the writer's
drain, so cursors stay valid across restarts and across mains sharing one
database. The in-process bus keeps a bounded per-thread cache (500 events /
2 MB) for same-process reads only; it is never the replay authority.
