...
```typescript
{
  ...
  responseId?: string; // groups events from one provider response
...
```json
{"type":"tool-interrupted","runId":"run_abc123","agentId":"agent-001","payload":{"toolCallId":"tc_abc123","error":"Tool call was interrupted because its process stopped."}}
```
...
```json
{"type":"run-finish","runId":"run_abc123","agentId":"agent-001","payload":{"status":"interrupted"}}
```
