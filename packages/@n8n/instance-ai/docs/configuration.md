...
| `N8N_INSTANCE_AI_SANDBOX_TAG` | string | `''` | Operator-set value surfaced as a `tag` label on every Daytona sandbox. Use to flag sandboxes for grouping/filtering in the Daytona dashboard (e.g. distinguishing eval-driven sandboxes from production ones). Empty in production. |
