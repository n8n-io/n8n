...
| `N8N_INSTANCE_AI_OVERVIEW_MODEL` | string | `''` | Bare model name (no provider prefix) for workflow-overview generation, e.g. `claude-haiku-4-5`. Inherits the main model's provider and credentials. Empty = `claude-sonnet-4-6` when the main model is Anthropic, otherwise the main model. |
