...
- Coalesced step-level facts are appended to the `instance_ai_events` table
  (the durable replay source; per-thread `seq` ids survive restarts and are
  valid on any main), while token deltas stay memory-only. The in-process bus
  keeps a bounded per-thread cache (500 events / 2 MB) for same-process reads
