...
- **Orchestration tools** — `plan`, `delegate`, and background agent tools (`build-workflow-with-agent`, `manage-data-tables-with-agent`, `research-with-agent`)
- **Domain tools** — native tools across workflows, executions, credentials, nodes, data tables, web research, filesystem, and templates
- **Sub-agent factory** — creates stateless sub-agents with tool subsets
- **Workspace module** — sandbox provisioning (Daytona / local), filesystem abstraction, workspace setup for builder agents
