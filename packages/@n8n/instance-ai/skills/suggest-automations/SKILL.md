...
description: >-
  ...
  they choose. The system preloads it on onboarding threads, where the opening
  card already asked the user about themselves. Use it on other threads when
  the user asks what they could automate or wants ideas for a first workflow.
recommended_tools:
  - ask-user
  - build-workflow
---
...
1. From what you know about the user's team, pick the three automations
   people on it most often set up with the user's apps, for what the user
   said they want to automate. Each one is a trigger and what happens next,
   and runs on the user's apps. Decide in one step: do not run a command,
   read a file or write the reasoning out.
