...
"Calling n8n workflows"), and `settings` changes who may open the app (see
...
- The workflow runs as the app's project, with the project's credentials.
  For a `public` app, bound workflows are callable by anyone with the app
  URL: do not bind a workflow the user would not expose. Set `authMode` to
  `"n8n"` when only signed-in instance users should reach it.
...
the app's Details tab.

- `public`: anyone with the URL opens the app and can run its bound
  workflows. `result.principal` is `null`.
- `n8n`: the visitor signs in to this n8n instance first and needs access to
  the app's project (`app:read`). Every workflow run records who ran it:
  `result.principal` is `{ userId }`, and the execution's custom data has
  `appUserId`. Use it for internal tools built for the team.

Both modes hand the page a short-lived token that the SDK sends with every
call; the app never handles it. Honest limit: for a `public` app the token
