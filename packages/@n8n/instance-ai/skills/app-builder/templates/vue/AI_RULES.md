...
- Do not start a dev server; n8n runs one for the live preview. Your edits appear in the preview without a build. Run `apps` (`action: "build"`) to publish. Preview errors arrive as context on the user's next message; fix them before anything else.
