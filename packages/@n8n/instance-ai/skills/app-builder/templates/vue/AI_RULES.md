...
- `src/main.ts` boots the app, `src/router.ts` holds the routes, `src/pages/` holds one component per route. `src/components/ui/` holds shadcn-vue components added via `add-component` (only `button` and `switch` exist from `create`); shared app components go in `src/components/`. `src/style.css` holds the shadcn-vue theme (Tailwind + the `:root`/`.dark` CSS variables every component reads) — do not edit `src/theme-overrides.css` or `src/theme-mode.ts`, the app's Theme tab manages them.
