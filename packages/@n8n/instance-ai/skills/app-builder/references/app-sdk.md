...
`Bindings` interface. With it, `n8n.workflows.run` accepts only bound keys and
checks the input fields, and `n8n.tables.<key>` accepts only bound keys and
types the rows, at build time (`vue-tsc` runs in `npm run build`).
...
Before the first bind the file declares `workflows: {}` and `tables: {}`, so
any `run` or `n8n.tables.<key>` call fails the build. That is the intended
signal to bind first.
