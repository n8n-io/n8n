...
1. **Explicit artifact requests.** When the user names the deliverable —
   ...
   propose the alternative and obtain agreement before changing the named
   artifact. Explain the reason from the task's shape. The false-friends
...
4. For a standalone scheduled duty with a fixed trigger and delivery, apply
   Scheduled judgment work below before testing the general agent signals.
   Prefer a workflow unless the duty belongs to an existing or explicitly
   requested agent, or needs memory across runs.
5. Otherwise, choose **agent-anchored** for conversational, open-ended work
   whose control belongs to the agent. Choose **workflow-anchored** when a
   repeatable graph owns the process. A bounded LLM step can sit in that graph.
...
7. **Degenerate-shell check.** If a workflow-anchored design reduces to a
   ...
   **agent-anchored** and build an n8n Agent (an on-demand duty becomes the
   ...
   migrates into one embedded agent step, explain the alternative. Obtain
   agreement before changing an explicitly requested workflow into an agent.
...
**Agent-anchored signals** (apply after context, explicit intent, and the
scheduled-duty rule):
...
For an ambiguous request after applying the decision steps, load
`references/routing-examples.md` for boundary examples. Do not load examples
for a clear request or use an example to override the user's explicit choice.
