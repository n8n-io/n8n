...
- `subAgents.maxChildren` must be an integer from 1 to 20. When unset, it defaults to 10.
...
1. Call `agent_builder` (`action: "list_sub_agents"`) to discover published
   same-project agents that can be added. Do not write agent ids from memory,
   prose, or user-entered free text.
2. If published agents are available and the user has not named exact agents,
   use the `ask-user` tool (multi-select) to let them choose. Map each chosen
   option back to its `agentId` from the `list_sub_agents` result.
...
- Any `subAgents.maxChildren` value stays within 1 to 20.
