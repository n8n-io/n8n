...
describe('unresponsive IMAP connection', () => {
	...
	it('reports an error once the server stops responding', reportsSilenceAsAnError(fake));

	it('reports an error when the server never confirms the restarted IDLE', async () => {
		const connection = await fake.open({ socketTimeout: INACTIVITY_TIMEOUT });
		await connection.openBox('INBOX');
