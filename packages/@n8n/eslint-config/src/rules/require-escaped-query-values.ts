...
function isNeutralised(
	...
	switch (node.type) {
		...
		case 'CallExpression': {
			// Only a bare call to an imported helper counts. A member access such as
			...
			// Match the name the helper is exported under rather than its local
			// spelling, so an alias still counts and a same-named local does not.
			const specifier = binding.node;
			if (specifier.type !== 'ImportSpecifier' || specifier.imported.type !== 'Identifier') {
				return false;
			}

			return ESCAPERS.has(specifier.imported.name);
		}
