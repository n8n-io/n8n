...
query.push(\`mimeType = '\${FOLDER_MIME}'\`);`,
		...
		// An alias of a sanctioned helper is still that helper.
		{
			code: `import { escapeODataValue as esc } from '@utils/query-escaping';
qs.$filter = \`eq '\${esc(filter)}'\`;`,
		},
...
qs.$filter = \`eq '\${DEFAULT_NAME}'\`;`,
		...
		// The local spelling does not matter — this is `noop` under an approved name.
		{
			code: `import { noop as escapeODataValue } from './helpers';
