...
- The decision token (`_confirmation`) is generated by the daemon and
  carries the specific rule to apply (allow once, allow for session, etc.).
  The n8n server never interprets it — it passes through opaquely.
- If the user denies without providing a token, `resumeData.resourceDecisionToken`
