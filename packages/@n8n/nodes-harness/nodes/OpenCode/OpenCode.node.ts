...
export class OpenCode extends BaseHarnessNode {
	description: INodeTypeDescription = {
		...
		group: ['transform'],
		version: 1,
		subtitle: '={{$parameter["prompt"] ? "AI Harness" : "AI Harness"}}',
		description:
			'Execute the OpenCode AI coding agent in an isolated workspace and capture the diff of all changes',
		defaults: {
			name: 'OpenCode',
		},
		...
		properties: [
			...
			// --- Auto-approve ---
			{
				...
				default: true,
				description:
					'Whether to automatically approve all file edit and command permissions. Required for fully automated execution. Uses --dangerously-skip-permissions.',
			},
		],
	};
	...
	protected buildCommand(ctx: IExecuteFunctions, itemIndex: number) {
		...
		// Use -- separator to safely pass the prompt (prevents shell interpretation)
		args.push('--', prompt);
	...
	protected async buildEnv(ctx: IExecuteFunctions, _itemIndex: number) {
		...
		const env: Record<string, string> = {
			...
			// Explicit OpenCode isolation overrides
	...
	protected override parseEvents(stdout: string): OpenCodeEvent[] {
