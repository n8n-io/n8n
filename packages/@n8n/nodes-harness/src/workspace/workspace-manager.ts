 ...
 *
 * Lifecycle:
 * 1. create() - creates the workspace directory
 * 2. populateFromGit() or populateFromDirectory() - fills it with source files
 * 3. initBaseline() - creates a git baseline commit for diff tracking
 * 4. [harness CLI runs and modifies files]
 * 5. computeDiff() - computes what changed
 * 6. getChangedFiles() - reads changed file contents
...
export class WorkspaceManager {
	...
	async populateFromGit(workspace: Workspace, url: string, branch: string): Promise<void> {
		...
		// First remove the empty dir since git clone needs a non-existent target
	...
	/**
	 * Compute the diff of all changes since the baseline.
	 */
	...
	async getChangedFiles(workspace: Workspace): Promise<ChangedFile[]> {
		...
		for (const entry of nameStatus) {
			...
			if (status === 'D') {
				// Deleted files have no content to read
				changedFiles.push({
					path: entry.path,
					status: 'deleted',
					content: null,
