...
describe('SessionManagerService', () => {
	...
	describe('appendUserMessage', () => {
		it('should append a message to existing session', async () => {
			const mockStorage = mock<ISessionStorage>();
			const serviceWithStorage = new SessionManagerService(
				mockParsedNodeTypes,
				mockStorage,
				mockLogger,
			...
			const existingMessages = [new HumanMessage('first message')];
			mockStorage.getSession.mockResolvedValue({
			...
			await serviceWithStorage.appendUserMessage('thread-1', 'second message', 'msg-id-2');

			expect(mockStorage.saveSession).toHaveBeenCalledWith('thread-1', {
		...
		it('should create a new session when none exists', async () => {
			...
			await serviceWithStorage.appendUserMessage('thread-1', 'first message', 'msg-id-1');

			expect(mockStorage.saveSession).toHaveBeenCalledWith('thread-1', {
				messages: [expect.objectContaining({ content: 'first message' })],
				previousSummary: undefined,
				updatedAt: expect.any(Date),
			});
		});

		it('should include messageId in additional_kwargs', async () => {
			const mockStorage = mock<ISessionStorage>();
			const serviceWithStorage = new SessionManagerService(
				mockParsedNodeTypes,
				mockStorage,
			...
			await serviceWithStorage.appendUserMessage('thread-1', 'test', 'my-msg-id');

			expect(mockStorage.saveSession).toHaveBeenCalledWith('thread-1', {
				messages: [
					expect.objectContaining({
						additional_kwargs: { messageId: 'my-msg-id' },
					}),
				],
				previousSummary: undefined,
				updatedAt: expect.any(Date),
