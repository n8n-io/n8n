...
```typescript
import { buildSupervisorPrompt, SUPERVISOR_PROMPT_SUFFIX } from '@/prompts';

const systemPrompt = buildSupervisorPrompt();
// Use in ChatPromptTemplate...
```

### Using chain prompts

```typescript
import { promptCategorizationTemplate, formatTechniqueList } from '@/prompts';

const result = await promptCategorizationTemplate.invoke({
  userPrompt: 'Monitor Slack for mentions and notify via email',
  techniques: formatTechniqueList(),
});
```

### Using parameter updater
...
const systemPrompt = ParameterUpdatePromptBuilder.buildSystemPrompt({
  ...
  nodeDefinition: ifNodeDefinition,
  requestedChanges: ['check if amount > 100'],
  hasResourceLocatorParams: false,
});
```
...
These could be shared with other packages:
- `compactPromptTemplate` - Conversation summarization
- `workflowNamingPromptTemplate` - Name generation
- `EXPRESSION_RULES` - n8n expression syntax
- `OUTPUT_FORMAT` - JSON output specification
