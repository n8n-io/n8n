...
import { buildDiscoveryPrompt } from '@/prompts/agents';
...
export class DiscoveryGraph {
	...
	private shouldLoopPlanner(
		...
		if (state.planModifyCount >= DiscoveryGraph.MAX_PLAN_MODIFY_ITERATIONS) {
			this.logger?.warn(
				`[Discovery] Plan modify limit reached (${DiscoveryGraph.MAX_PLAN_MODIFY_ITERATIONS}), proceeding with last plan`,
			);
			return END;
		}
