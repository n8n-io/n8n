...
import type { CodeFormat } from '../../src/code-builder/types';

type FormatOption = 'sdk' | 'dataflow' | 'both';
	...
	for (let i = 0; i < args.length; i++) {
		switch (args[i]) {
			...
			case '--verbose':
				...
				break;
			case '--format':
				result.format = args[++i] as FormatOption;
				break;
		}
	}
...
function getFormatsToRun(format: FormatOption): CodeFormat[] {
	if (format === 'both') return ['sdk', 'dataflow'];
	return [format];
}

async function runFormatBatch(
	formatVariant: CodeFormat,
	prompts: PromptConfig[],
	checklistMap: Map<string, ChecklistItem[]>,
	runnerConfig: RunnerConfig,
	args: CliArgs,
): Promise<Run> {
	const variant: RunVariant = formatVariant;
	const run: Run = {
...
async function runEval() {
	...
	const prompts = filterPrompts(SYNTHETIC_PROMPTS, args);
	const formats = getFormatsToRun(args.format);
	...
	// Phase 1: Extract checklists (shared across all formats)
	...
	// Phase 2: Run examples for each format
	const allRuns: Run[] = [];

	for (const format of formats) {
		console.log(`\n=== Phase 2: Running ${prompts.length} examples [${format.toUpperCase()}] ===`);
		...
		const run = await runFormatBatch(format, prompts, checklistMap, runnerConfig, args);
		allRuns.push(run);
	}

	// Regenerate report with all runs
	console.log('\nGenerating report...');
	writeReport(listRuns());
	...
	for (const run of allRuns) {
		const variant = run.config.variant ?? 'sdk';
		console.log(`\n--- ${variant.toUpperCase()} ---`);
...
Options:
  ...
  --format <format>       Code format: sdk, dataflow, or both (default: both)
