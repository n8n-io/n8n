import { langsmithMetricKey } from '../feedback';
...
describe('langsmithMetricKey()', () => {
	it('should keep llm-judge metrics unprefixed (root and sub-metrics)', () => {
		const root: Feedback = { evaluator: 'llm-judge', metric: 'overallScore', score: 1 };
		const sub: Feedback = {
			evaluator: 'llm-judge',
	...
	it('should prefix programmatic metrics with evaluator name', () => {
		const fb: Feedback = { evaluator: 'programmatic', metric: 'trigger', score: 1 };
	...
	it('should keep pairwise v1 metrics unprefixed and namespace non-v1 details', () => {
		const v1: Feedback = { evaluator: 'pairwise', metric: 'pairwise_primary', score: 0 };
		const detail: Feedback = { evaluator: 'pairwise', metric: 'judge1', score: 0 };
	...
	it('should not produce collisions for the known evaluator contract', () => {
		const feedback: Feedback[] = [
			{ evaluator: 'llm-judge', metric: 'connections', score: 1 },
			{ evaluator: 'llm-judge', metric: 'overallScore', score: 1 },
			{ evaluator: 'programmatic', metric: 'connections', score: 1 },
			{ evaluator: 'programmatic', metric: 'overall', score: 1 },
			{ evaluator: 'pairwise', metric: 'pairwise_primary', score: 1 },
			{ evaluator: 'pairwise', metric: 'pairwise_total_violations', score: 1 },
			{ evaluator: 'pairwise', metric: 'judge1', score: 0 },
		];
