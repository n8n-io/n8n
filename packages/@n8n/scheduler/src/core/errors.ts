...
/**
 * Raised when a scheduler is composed with dependencies that cannot work
 * together (e.g. a reconciliation pass without the shared clock its windows
 * are computed on).
 */
...
/**
 * Raised when provisioning is attempted for an owner type no resolver has
 * claimed: without one, the reconciliation sweep could never tell that the
 * owner is gone, so its jobs would be uncleanable.
 */
