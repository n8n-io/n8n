...
by a flag. Each task owns one job, stamped with the n8n version that provisioned it. At
startup, once it has provisioned the tasks it runs durably, an instance deletes every
system-task job it does not run durably, unless the stamp is newer than its own
