...
export class N8nClient {
	...
	constructor(options: ClientOptions) {
		// Normalise: strip trailing slash, store raw instance URL
		this.instanceUrl = options.baseUrl.replace(/\/+$/, '');
	...
	// ─── Raw Request (non-API-v1 endpoints) ────────────────────────

	private async rawRequest<T>(
		method: string,
		path: string,
		options: { body?: unknown; query?: Record<string, string>; stream?: boolean } = {},
	): Promise<T | Response> {
		...
		try {
			...
			throw new ApiError(
				0,
				`Could not connect to n8n at ${this.instanceUrl}`,
				`Connection error: ${msg}. Check the URL and ensure the instance is running.`,
			);
		}
	...
	async sendAiMessage(
		threadId: string,
		message: string,
		opts?: { researchMode?: boolean },
	): Promise<{ runId: string }> {
		const body: Record<string, unknown> = { message };
		if (opts?.researchMode) body.researchMode = true;
		return (await this.rawRequest('POST', `/instance-ai/chat/${threadId}`, {
			body,
		})) as { runId: string };
	}

	async streamAiEvents(threadId: string, lastEventId?: string): Promise<Response> {
		const query: Record<string, string> = {};
		if (lastEventId) query.lastEventId = lastEventId;
		return await this.rawRequest<Response>('GET', `/instance-ai/events/${threadId}`, {
			query,
			stream: true,
		});
	}

	async listAiThreads(): Promise<{ threads: Array<Record<string, unknown>>; total: number }> {
		return (await this.rawRequest('GET', '/instance-ai/threads')) as {
			threads: Array<Record<string, unknown>>;
			total: number;
		};
	}

	async getAiThreadMessages(
		threadId: string,
		opts?: { limit?: number; page?: number },
	): Promise<{ messages: Array<Record<string, unknown>>; threadId: string }> {
		const query: Record<string, string> = {};
		if (opts?.limit) query.limit = String(opts.limit);
		if (opts?.page) query.page = String(opts.page);
		return (await this.rawRequest('GET', `/instance-ai/threads/${threadId}/messages`, {
			query,
		})) as { messages: Array<Record<string, unknown>>; threadId: string };
	}

	async cancelAiRun(threadId: string): Promise<{ ok: boolean }> {
		return (await this.rawRequest('POST', `/instance-ai/chat/${threadId}/cancel`)) as {
			ok: boolean;
		};
	}

	async confirmAiAction(requestId: string, approved: boolean): Promise<unknown> {
		return await this.rawRequest('POST', `/instance-ai/confirm/${requestId}`, {
			body: { approved },
		});
	}
}
