...
| `--data-table-schema-conflict-policy` | How strictly a matched data table's schema is compared. `keep-existing` (instance default) tolerates extra target columns; `fail` is the strict drift-detection choice and rejects any difference, including extra columns. A missing column or type mismatch always rejects. |
