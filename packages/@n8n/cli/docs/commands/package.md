...
| `--credential-matching-mode` | How credential references are matched on the target instance: `id-only` (default, match by id), `name-and-type` (match by exact name and type), or `type-only` (match by type). For `name-and-type` and `type-only`, project-owned credentials are preferred over global credentials; ties within the same scope use the most recently updated credential. |
