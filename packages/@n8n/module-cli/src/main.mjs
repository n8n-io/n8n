...
import { isModuleId, modulesRoot, repoRoot, ScaffoldError, substitutionsFor } from './scaffold.mjs';
...
const create = defineCommand({
	...
	async run({ args }) {
		...
		for (const edit of edits) consola.log(`  updated ${edit}`);
		...
		// `pnpm --filter <pkg> typecheck` fails on a cold tree — the package's dependencies have not
		// been built yet and nothing in that command builds them. Going through turbo does.
		consola.box(
			wants('frontend')
				? [
						'Next:',
