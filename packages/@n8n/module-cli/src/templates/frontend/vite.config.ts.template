...
export default mergeConfig(
	defineConfig({
		...
		// needs these two plugins. `src/__tests__/design-system-icons.test.ts` guards them.
		plugins: [
			vue(),
			// Off, so a test never reaches the network for a missing collection. The collection
			// comes from the `@iconify/json` devDependency instead.
