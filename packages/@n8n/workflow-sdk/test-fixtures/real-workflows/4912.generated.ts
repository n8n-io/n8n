return workflow('bCzsdWA9ELzYNPUy', 'Lead Workflow: Yelp & Trustpilot Scraping + OpenAI Analysis via BrightData', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.formTrigger', version: 2.2, config: { parameters: {
    ...
    }, position: [-640, -3320], name: 'Form Trigger - Get User Input' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2, config: { parameters: {
      ...
      options: {},
      promptType: 'define'
    }, subnodes: { model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatGoogleGemini', version: 1, config: { parameters: { options: {}, modelName: 'models/gemini-1.5-flash' }, credentials: {
          googlePalmApi: { id: 'credential-id', name: 'googlePalmApi Credential' }
        }, name: 'Gemini - Location AI Model' } }) }, position: [-340, -3460], name: 'AI Location Analyzer' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [160, -3520], name: 'Split Sub-locations' } }))
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: {} }, position: [460, -3520], name: 'Loop Yelp Locations' } }))
  .add(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [1760, -3540], name: 'Clean Unique Websites' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
    ...
    }, position: [2020, -3540], name: 'Read Yelp Sheet Websites' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [2200, -3540], name: 'Make Trustpilot URLs' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
      jsCode: 'const seen = new Set();\nconst output = [];\n\nfor (const item of items) {\n  const url = item.json["Trustpilot Website URL"];\n  \n  if (!seen.has(url)) {\n    seen.add(url);\n    output.push({ json: { "Trustpilot Website URL": url } });\n  }\n}\n\nreturn output;\n'
    }, position: [2440, -3180], name: 'Remove Duplicate TP URLs' } }))
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: {} }, position: [-420, -2740], name: 'Loop Trustpilot URLs' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [-60, -2940], name: 'Trigger Trustpilot Scraper' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [200, -2940], name: 'Check Trustpilot Scrape Progress' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.if', version: 2.2, config: { parameters: {
    ...
    }, position: [940, -2820], name: 'If Trustpilot Has Records' } }), node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { unit: 'minutes', amount: 1 }, position: [560, -2840], name: 'Wait (1 min) Trustpilot Completion' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [1180, -2840], name: 'Download Trustpilot Data' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
        value: {
    ...
    }, position: [1460, -2840], name: 'Save Trustpilot Data to Sheet' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      options: {},
      filtersUI: {
    ...
    }, position: [1680, -2840], name: 'Read Emails from Trustpilot Sheet' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
      jsCode: 'const seen = new Set();\nconst output = [];\n\nfor (const item of items) {\n  const email = item.json.Email;\n  if (!email || seen.has(email)) continue;\n\n  seen.add(email);\n\n  output.push({\n    json: {\n      email: email\n    }\n  });\n}\n\nreturn output;\n'
    }, position: [1880, -2840], name: 'Get Unique Emails' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2, config: { parameters: {
      ...
      promptType: 'define'
        ...
        }, name: 'Claude - Email AI Model' } }) }, position: [2080, -2840], name: 'AI Generate Email Content' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [2480, -2840], name: 'Parse Email JSON' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
    ...
    }, position: [2720, -2720], name: 'Send Outreach Email' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: 'https://api.brightdata.com/datasets/v3/trigger',
      method: 'POST',
      options: {},
    ...
    }, position: [560, -3360], name: 'Yelp Scraper' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: '=https://api.brightdata.com/datasets/v3/progress/{{ $json.snapshot_id }}',
      options: {},
      sendHeaders: true,
    ...
    }, position: [760, -3360], name: 'Check Yelp Scrape Progress' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { unit: 'minutes', amount: 1 }, position: [960, -3360], name: 'Wait (1 min) Yelp Completion' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.if', version: 2.2, config: { parameters: {
      options: {},
      conditions: {
        options: {
    ...
    }, position: [1420, -3380], name: 'If Yelp Has Records' } }), node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: '=https://api.brightdata.com/datasets/v3/progress/{{ $json.snapshot_id }}',
      options: {},
      sendHeaders: true,
      headerParameters: {
    ...
    }, position: [760, -3360], name: 'Check Yelp Scrape Progress' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [1660, -3400], name: 'Fetch Yelp Results' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
        value: {
          URL: '={{ $json.url }}',
