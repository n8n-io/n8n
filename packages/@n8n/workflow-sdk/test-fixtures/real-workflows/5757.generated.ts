const wf = workflow('1AeBTEXwiwxSvJ37', 'AI-Powered Email Automation', { executionOrder: 'v1' })
	.add(
		trigger({
			type: 'n8n-nodes-base.manualTrigger',
			version: 1,
			config: { position: [-140, -860], name: 'When clicking ‘Test workflow’' },
		}),
	)
	.then(
		node({
	...
	.add(
		sticky(
			...
			{ position: [-860, -860], width: 660, height: 360 },
		),
	)
	.add(
		sticky(
			...
			{ name: 'Sticky Note1', color: 6, position: [-860, -120], width: 760, height: 420 },
		),
	)
	.add(
		sticky(
			...
			{ name: 'Sticky Note2', color: 5, position: [-840, 580], width: 800, height: 340 },
		),
	);
