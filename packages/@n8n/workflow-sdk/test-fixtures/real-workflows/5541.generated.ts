return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-540, -520], name: 'When clicking ‘Test workflow’' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.8, config: { parameters: {
      ...
      promptType: 'define'
        ...
        }, name: 'OpenAI' } }) }, position: [-260, -520], name: 'AI Agent' } }))
  .then(node({ type: 'n8n-nodes-base.executeWorkflow', version: 1.2, config: { parameters: {
    ...
    }, position: [200, -520], name: 'Call sub-workflow' } }))
  .add(trigger({ type: 'n8n-nodes-base.executeWorkflowTrigger', version: 1.1, config: { parameters: { workflowInputs: { values: [{ name: 'execution_id' }] } }, position: [-540, 180], name: 'When Executed by Another Workflow' } }))
  .then(node({ type: 'n8n-nodes-base.n8n', version: 1, config: { parameters: {
      ...
      requestOptions: {}
    }, credentials: { n8nApi: { id: 'credential-id', name: 'n8nApi Credential' } }, position: [-320, 180], name: 'Get execution data' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-100, 180], name: 'Extract token usage data' } }))
  ...
  .then(node({ type: 'n8n-nodes-base.summarize', version: 1.1, config: { parameters: {
    ...
    }, position: [320, 180], name: 'Sum Token Totals - aggregate by model' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
    ...
    }, position: [540, 180], name: 'Record token usage' } }))
  .add(node({ type: '@n8n/n8n-nodes-langchain.lmChatGoogleGemini', version: 1, config: { parameters: { options: {}, modelName: 'models/gemini-2.5-flash' }, credentials: {
      googlePalmApi: { id: 'credential-id', name: 'googlePalmApi Credential' }
    }, position: [-400, -280], name: 'Gemini' } }))
