return workflow('', '')
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.mcpTrigger', version: 1, config: { parameters: { path: 'my-calendar' }, subnodes: { tools: [tool({ type: 'n8n-nodes-base.googleCalendarTool', version: 1.3, config: { parameters: {
        ...
        }, name: 'UpdateEvent' } })] }, position: [2060, -20], name: 'Google Calendar MCP' } }))
  .add(trigger({ type: 'n8n-nodes-base.executeWorkflowTrigger', version: 1.1, config: { parameters: {
    ...
    }, position: [1440, 880], name: 'When Executed by Another Workflow' } }))
  .then(switchCase([node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [2000, 520], name: 'Convert Text to Upper Case' } }), node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
          {
    ...
    }, position: [2000, 720], name: 'Convert Text to Lower Case' } }), node({ type: 'n8n-nodes-base.debugHelper', version: 1, config: { parameters: {
      category: 'randomData',
      randomDataCount: '={{ $json.payload.number }}'
    }, position: [2000, 1020], name: 'Random user data' } }), node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: '=https://official-joke-api.appspot.com/jokes/random/{{ $json.payload.number }}',
      options: {}
    }, position: [2000, 1220], name: 'Joke Request' } })], { version: 3.2, parameters: {
      rules: {
        values: [
          {
            outputKey: 'UPPERCASE',
  ...
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [2220, 1020], name: 'Return only some fields' } }))
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.chatTrigger', version: 1.1, config: { parameters: { options: {} }, position: [240, -180], name: 'When chat message received' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.8, config: { parameters: {
      options: {
        systemMessage: '=You are a helpful assistant.\nCurrent datetime is {{ $now.toString() }}'
          ...
          options: {}
        }, name: 'OpenAI 4o' } }), tools: [tool({ type: '@n8n/n8n-nodes-langchain.mcpClientTool', version: 1, config: { parameters: { sseEndpoint: 'https://n8n.yourdomain/mcp/my-calendar/sse' }, name: 'Calendar MCP' } }), tool({ type: '@n8n/n8n-nodes-langchain.mcpClientTool', version: 1, config: { parameters: { sseEndpoint: 'https://n8n.yourdomain/mcp/my-functions/sse' }, name: 'My Functions' } })], memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryBufferWindow', version: 1.3, config: { name: 'Simple Memory' } }) }, position: [440, -180], name: 'AI Agent' } }))
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.mcpTrigger', version: 1, config: { parameters: { path: 'my-functions' }, subnodes: { tools: [tool({ type: '@n8n/n8n-nodes-langchain.toolWorkflow', version: 2.1, config: { parameters: {
        ...
        }, name: 'Convert Text' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolWorkflow', version: 2.1, config: { parameters: {
          name: 'obtain_jokes',
          workflowId: { __rl: true, mode: 'id', value: '={{ $workflow.id }}' },
        ...
        }, name: 'Random Jokes' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolWorkflow', version: 2.1, config: { parameters: {
        ...
        }, name: 'Generate random user data' } })] }, position: [1440, -20], name: 'My Functions Server' } }))
  .add(sticky('## Activate the workflow to make the MCP Trigger work\nIn order to make the MCP server available, you need to activate the workflow.\n\nThen copy the Production URL of the MCP Trigger and paste it in the corresponding MCP Client tool.', { name: 'Sticky Note', color: 3, position: [1320, -180], width: 620, height: 520 }))
  .add(sticky('## MCP Clients\nFor every tool here you need to obtain he corresponding Production URL from the MCP Triggers on the right 👉', { name: 'Sticky Note1', color: 7, position: [820, -180], width: 440, height: 520 }))
  .add(sticky('# Try these example requests with the AI Agent\n\n### My Functions MCP\n1. Use your tools to convert this text to lower case: `EXAMPLE TeXt`\n\n2. Use your tools to convert this text to upper case: `example TeXt`\n\n3. Generate 5 random user data, please.\n\n4. Please obtain 3 jokes.\n\n\n\n\n### Calendar MCP\n5. What is my schedule for next week?\n\n6. I have a meeting with John tomorrow at 2pm. Please add it to my Calendar.\n\n7. Adjust the time of my meeting with John tomorrow from 2pm to 4pm, please.\n\n8. Cancel my meeting with John, tomorrow.', { name: 'Sticky Note2', color: 7, position: [-480, -180], width: 660, height: 640 }))
  .add(sticky('## The My Functions MCP calls this sub-workflow every time.\nA subworkflow is a separate workflow that can be called by other workflows and is able to receive parameters.\nLearn more about sub-workflows **[here](https://docs.n8n.io/flow-logic/subworkflows/)**', { name: 'Sticky Note3', color: 7, position: [1320, 360], width: 1260, height: 1060 }))
  .add(sticky('## Google Calendar tools require credentials\nIf you don\'t have your Google Credentials set up in n8n yet, watch [this](https://www.youtube.com/watch?v=3Ai1EPznlAc) video to learn how to do it.\n\nIf you are using n8n Cloud plans, it\'s very intuitive to setup and you may not even need the tutorial.', { name: 'Sticky Note4', color: 5, position: [1960, -180], width: 620, height: 520 }))
  .add(sticky('# Need help?\nFor getting help with this workflow, please create a topic on the community forums here:\nhttps://community.n8n.io/c/questions/', { name: 'Sticky Note6', position: [-480, 1160], width: 660, height: 180 }))
  .add(sticky('## Why model 4o? 👆\nAfter testing 4o-mini it had some difficulties handling the calendar requests, while the 4o model handled it with ease.\n\nDepending on your prompt and tools, 4o-mini might be able to work well too, but it requires further testing.', { name: 'Sticky Note7', color: 7, position: [240, 160], height: 240 }))
  .add(sticky('', { name: 'Sticky Note8', color: 4, position: [-480, -340], width: 3060, height: 140 }))
  .add(sticky('# Learn How to Build an MCP Server and Client', { name: 'Sticky Note9', color: 4, position: [640, -300], width: 800, height: 80 }))
  ...
  .add(sticky('### 💡 **Want to learn advanced n8n skills and earn money building workflows?**\n‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎Check out [Scrapes Academy](https://www.skool.com/scrapes/about?ref=21f10ad99f4d46ba9b8aaea8c9f58c34)', { name: 'Sticky Note16', color: 4, position: [-460, 1020], width: 620, height: 80 }))