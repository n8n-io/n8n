return workflow('', '')
  ...
  .then(node({ type: 'n8n-nodes-base.n8n', version: 1, config: { parameters: { filters: {}, requestOptions: {} }, credentials: { n8nApi: { id: 'credential-id', name: 'n8nApi Credential' } }, position: [2400, -1568], name: 'Fetch All Workflows' } }))
  .then(node({ type: 'n8n-nodes-base.aggregate', version: 1, config: { parameters: {
    ...
    }, position: [2624, -1568], name: 'Aggregate Workflow List' } }))
  .then(node({ type: 'n8n-nodes-base.form', version: 2.3, config: { parameters: {
      ...
      limitWaitTime: true
    }, position: [2848, -1568], name: 'Form Dynamic Workflow List' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [3072, -1568], name: 'Parse Selected Workflow Id' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [3296, -1568], name: 'Set Target Workflow Id' } }))
  .then(node({ type: 'n8n-nodes-base.n8n', version: 1, config: { parameters: {
      operation: 'get',
      workflowId: { __rl: true, mode: 'id', value: '={{ $json.workflow_id }}' },
      requestOptions: {}
    }, credentials: { n8nApi: { id: 'credential-id', name: 'n8nApi Credential' } }, position: [3520, -1568], name: 'Fetch Target Workflow JSON' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [3744, -1568], name: 'Extract Workflow Nodes Connections' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.7, config: { parameters: {
    ...
    }, subnodes: { model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenRouter', version: 1, config: { parameters: { model: 'openai/gpt-5.1-codex-mini', options: {} }, credentials: {
          openRouterApi: { id: 'credential-id', name: 'openRouterApi Credential' }
        }, name: 'OpenRouter Chat Model' } }), outputParser: outputParser({ type: '@n8n/n8n-nodes-langchain.outputParserStructured', version: 1.3, config: { parameters: {
          schemaType: 'manual',
          inputSchema: '{\n  "type": "array",\n  "items": {\n    "type": "object",\n    "properties": {\n      "old_name": {\n        "type": "string",\n        "description": "The exact match original node name"\n      },\n      "new_name": {\n        "type": "string",\n        "description": "The new node name"\n      }\n    },\n    "required": [\n      "old_name",\n      "new_name"\n    ]\n  }\n}'
        }, name: 'Parse LLM Mapping ToJson' } }) }, position: [2160, -1216], name: 'Generate Node Rename Mapping' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [2464, -1216], name: 'Prepare LLM Validation Data' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [2896, -1232], name: 'Apply Renamed Nodes Connections' } }), node({ type: 'n8n-nodes-base.stopAndError', version: 1, config: { parameters: { errorMessage: 'Validation Error' }, position: [2896, -1040], name: 'Stop' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [3120, -1232], name: 'Assemble New Workflow Object' } }))
  .then(node({ type: 'n8n-nodes-base.n8n', version: 1, config: { parameters: {
      operation: 'update',
      workflowId: {
        __rl: true,
    ...
    }, credentials: { n8nApi: { id: 'credential-id', name: 'n8nApi Credential' } }, position: [3344, -1232], name: 'Save Renamed Workflow Version' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [3552, -1232], name: 'Build Workflow Version Links' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.form', version: 2.3, config: { parameters: {
      options: {},
    ...
    }, position: [3968, -1328], name: 'Display Workflow Version Links' } }), node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: { options: {}, includeOtherFields: true }, position: [3968, -1104], name: 'Done' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [2176, -1888] } }))
  .then(node({ type: 'n8n-nodes-base.n8n', version: 1, config: { parameters: {
      operation: 'get',
      workflowId: {
        __rl: true,
