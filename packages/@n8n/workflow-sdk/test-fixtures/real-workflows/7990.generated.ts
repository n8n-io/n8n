return workflow('', '')
  ...
  .then(node({ type: 'n8n-nodes-base.mistralAi', version: 1, config: { parameters: { options: {} }, credentials: {
      mistralCloudApi: { id: 'credential-id', name: 'mistralCloudApi Credential' }
    }, position: [416, 0], name: 'Extract text' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.2, config: { parameters: {
      ...
      hasOutputParser: true
        ...
        }, name: 'Structured Output Parser' } }) }, position: [624, 0], name: 'AI Agent' } }))
