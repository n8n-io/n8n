return workflow('M4IIL41O16twgImq', 'Image Reader', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.telegramTrigger', version: 1.2, config: { parameters: { updates: ['message'], additionalFields: { download: true } }, credentials: {
  ...
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-180, -220], name: 'Clean Input Data' } }))
  .then(node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
      fileId: '={{ $json.Image.replace(/\\n/g, \'\') }}',
      resource: 'file'
    }, credentials: {
      telegramApi: { id: 'credential-id', name: 'telegramApi Credential' }
    }, position: [40, -220], name: 'get file' } }))
  .then(node({ type: 'n8n-nodes-tesseractjs.tesseractNode', version: 1, config: { parameters: { options: {} }, position: [260, -220], name: 'Tesseract OCR' } }))
