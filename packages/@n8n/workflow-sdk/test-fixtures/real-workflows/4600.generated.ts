return workflow('JdFtT41G2iqwGt1e', '🤖 AI content generation for Auto Service 🚘 Automate your social media📲!', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{ triggerAtHour: 9 }] } }, position: [1800, -220] } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.8, config: { parameters: {
      ...
      promptType: 'define'
        ...
        }, name: 'OPENAI WRITES POSTS' } }), tools: [tool({ type: '@tavily/n8n-nodes-tavily.tavilyTool', version: 1, config: { parameters: {
        ...
        }, name: 'Tavily Internet Search' } })] }, position: [2340, -180], name: 'GENERATE TEXT' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.8, config: { parameters: {
      ...
      promptType: 'define'
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OPENAI WRITES PROMPTS' } }) }, position: [2680, 460], name: 'GENERATE PROMPT' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
      ...
      openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
    }, position: [3280, -140], name: 'OPENAI GENERATES IMAGE' } }))
  .then(node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'choices[0].message.content' }, position: [3640, -140] } }))
  .add(node({ type: 'n8n-nodes-base.twitter', version: 2, config: { parameters: { additionalFields: {} }, position: [4180, 460], name: 'X' } }))
  .add(node({ type: 'n8n-nodes-base.facebookGraphApi', version: 1, config: { parameters: { options: {}, httpRequestMethod: 'POST' }, position: [4180, -60], name: 'Facebook' } }))
  .add(node({ type: 'n8n-nodes-base.linkedIn', version: 1, config: { parameters: {
      text: '={{ $json[\'choices[0].message.content\'] }}',
      person: '[CONFIGURE_YOUR_LINKEDIN_PERSON_ID]',
      authentication: 'communityManagement',
      additionalFields: {}
    }, position: [4180, 200], name: 'LinkedIn' } }))
  ...
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [1800, 60], name: 'When clicking Execute workflow' } }))
  ...
  .add(node({ type: '@n8n/n8n-nodes-langchain.lmChatMistralCloud', version: 1, config: { parameters: { model: 'pixtral-large-latest', options: {} }, credentials: {
      mistralCloudApi: { id: 'credential-id', name: 'mistralCloudApi Credential' }
    }, position: [4620, -220], name: 'Mistral Cloud Chat Model' } }))
  .add(node({ type: '@n8n/n8n-nodes-langchain.lmChatOpenRouter', version: 1, config: { parameters: { options: {} }, credentials: {
      openRouterApi: { id: 'credential-id', name: 'openRouterApi Credential' }
    }, position: [4880, -80], name: 'OpenRouter Chat Model' } }))
  .add(node({ type: '@n8n/n8n-nodes-langchain.lmChatAnthropic', version: 1.3, config: { parameters: {
      model: {
        __rl: true,
        mode: 'list',
      ...
      options: {}
    }, position: [4620, 40], name: 'Anthropic Chat Model' } }))
  .add(node({ type: '@n8n/n8n-nodes-langchain.lmChatGoogleGemini', version: 1, config: { parameters: { options: {} }, position: [4980, -220], name: 'Google Gemini Chat Model' } }))
  .add(node({ type: '@n8n/n8n-nodes-langchain.lmChatXAiGrok', version: 1, config: { parameters: { options: {} }, position: [4780, 40], name: 'xAI Grok Chat Model' } }))
  .add(node({ type: '@n8n/n8n-nodes-langchain.lmChatDeepSeek', version: 1, config: { parameters: { options: {} }, position: [4980, 40], name: 'DeepSeek Chat Model' } }))
  .add(node({ type: '@n8n/n8n-nodes-langchain.lmOpenHuggingFaceInference', version: 1, config: { parameters: {
      model: 'https://huggingface.co/black-forest-labs/FLUX.1-dev',
      options: {}
    }, credentials: {
      huggingFaceApi: { id: 'credential-id', name: 'huggingFaceApi Credential' }
    }, position: [5160, -220], name: 'Hugging Face Inference Model' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [4540, 400], name: 'Freepik API' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      sendBody: true,
      specifyBody: 'json'
    }, position: [4540, 580], name: 'Runware API' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [4760, 580], name: 'Clipdrop API' } }))
  .add(node({ type: '@n8n/n8n-nodes-langchain.lmChatOllama', version: 1, config: { parameters: { options: {} }, position: [4780, -220], name: 'Ollama Chat Model' } }))
  .add(node({ type: '@n8n/n8n-nodes-langchain.lmChatAzureOpenAi', version: 1, config: { parameters: { options: {} }, position: [5160, 40], name: 'Azure OpenAI Chat Model' } }))
  .add(node({ type: 'n8n-nodes-base.apiTemplateIo', version: 1, config: { credentials: {
      apiTemplateIoApi: { id: 'credential-id', name: 'apiTemplateIoApi Credential' }
    }, position: [5200, 580], name: 'APITemplate.io' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [4980, 580], name: 'Ideogram API' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [4760, 400], name: 'Replicate API' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [4980, 400], name: 'Imagen Google API' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [5200, 400], name: 'HuggingFace API' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [4980, 220], name: 'Runway Images' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [4540, 220], name: 'Minimax Images' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [4760, 220], name: 'Kling Images' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
  ...
  .add(sticky('', { name: 'Sticky Note9', color: 7, position: [4120, 160], width: 210, height: 200 }))
  .add(sticky('', { name: 'Sticky Note5', color: 7, position: [4120, -100], width: 210, height: 200 }))
  .add(sticky('', { name: 'Sticky Note7', color: 7, position: [4120, -360], width: 210, height: 200 }))
  .add(sticky('# START - Choose a Trigger![Guide](https://i.ibb.co/d41JsL8q/Screenshot-2025-05-30-122423-1.jpg#full-width#full-width)', { name: 'Sticky Note8', color: 7, position: [1600, -500], width: 500, height: 1260 }))
  ...
  .add(sticky('### Edit prompt and system message up for you, customize llm and search links, add your own prompts database ![](https://i.ibb.co/TxQrh405/erasebg-transformed-removebg-preview.png#full-width)', { name: 'Sticky Note13', color: 7, position: [2160, -360], width: 760, height: 460 }))
  .add(sticky('### Edit prompt and system message up for you, customize llm and add own prompts database ![](https://i.ibb.co/TxQrh405/erasebg-transformed-removebg-preview.png#full-width)', { name: 'Sticky Note14', color: 7, position: [2520, 300], width: 760, height: 420 }))
  .add(sticky('### Set up Ai model for generating images, customize prompt up for you ![](https://i.ibb.co/TxQrh405/erasebg-transformed-removebg-preview.png#full-width)', { name: 'Sticky Note15', color: 7, position: [3120, -300], width: 760, height: 380 }))
  .add(sticky('# The template is set up for Auto Service daily content uploads, but the underlying logic is universal. You can easily adapt it for any niche by editing prompts, adding nodes, and creating or uploading a variety of content to any platform. You can use any LLM and generative AI of your choice. Personally, I prefer the smooth and effective results from ChatGPT 4.1 combined with GPT Image 1.  Enjoy and [message me](https://community.n8n.io/u/n8ner/badges) with your reviews for future improvements! ![](https://i.ibb.co/qLxMHbd5/customize-ride1.jpg#full-width#full-width )', { name: 'Sticky Note1', color: 5, position: [4480, -520], width: 1620, height: 1300 }))