const wf = workflow('', '')
		...
		node({
			...
			config: {
				parameters: {
					...
					extractionValues: {
						values: [{ key: 'text', cssSelector: 'body', skipSelectors: 'svg, a' }],
					},
				},
				position: [1160, 700],
				name: 'Text Extractor',
	...
	.then(
		node({
			...
			version: 1.8,
			config: {
				parameters: { options: {}, resource: 'image' },
				position: [1400, 1160],
