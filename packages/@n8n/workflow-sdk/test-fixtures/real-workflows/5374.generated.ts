return workflow('eEVvcTInAYIB2SLi', 'Copycat SEO article (public version)', {
    timezone: 'Europe/Ljubljana',
    callerPolicy: 'workflowsFromSameOwner',
    executionOrder: 'v1'
  })
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{ field: 'hours', hoursInterval: 2 }] } }, position: [-320, 20] } }))
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.9, config: { parameters: {
      ...
      hasOutputParser: true
        ...
        }, name: 'Haiku ' } }) }, position: [100, 160], name: 'Page structure analiser' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.9, config: { parameters: {
      ...
      promptType: 'define',
      hasOutputParser: true
    }, subnodes: { tools: [tool({ type: 'n8n-nodes-base.webflowTool', version: 2, config: { parameters: {
        ...
        }, name: 'Get Articles' } })], model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatAnthropic', version: 1.3, config: { parameters: {
          model: {
            __rl: true,
            mode: 'list',
          ...
          anthropicApi: { id: 'credential-id', name: 'anthropicApi Credential' }
        }, name: 'Thinking Claude' } }), outputParser: outputParser({ type: '@n8n/n8n-nodes-langchain.outputParserAutofixing', version: 1, config: { parameters: {
          options: {
        ...
        }, name: 'OpenAI Mini model' } }), outputParser: outputParser({ type: '@n8n/n8n-nodes-langchain.outputParserStructured', version: 1.2, config: { parameters: {
          jsonSchemaExample: '{\n"article": "long article text",\n"summary": "summary of article",\n"author": "Marko Balažic"\n}'
        }, name: 'Structured Output Parser1' } }) }, name: 'Auto-fixing Output Parser' } }) }, position: [540, 100], name: 'Content writer' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.executeWorkflow', version: 1.2, config: { parameters: {
      options: {},
      workflowId: {
        __rl: true,
        mode: 'list',
    ...
    }, position: [980, -300] } }), null], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(ifBranch([node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.9, config: { parameters: {
      ...
      promptType: 'define'
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Mini model' } }) }, position: [1420, -340], name: 'Prompt engineer' } }), node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.9, config: { parameters: {
      text: '=You are an expert SEO writer working for www.productai.photo.\n\nRewrite the following article directly and return the full SEO-optimized version **in one complete response**, in **JSON** format with the following structure:\n\n{\n  "article": "HTML Markdown-formatted article goes here",\n  "summary": "1–2 sentence summary of the article here"\n}\n\nReturn only the requested output with no explanations, commentary, or additional text.\n\n## Guidelines:\n- Use the primary keyword:{{ $(\'Google Sheets\').item.json[\'Top keyword\'] }}  throughout the article, naturally and repeatedly.\n- Follow this exact structure based on the original HTML:\n  1. Header Navigation (SEO context)\n  2. Hero Section (main headline + paragraph)\n  3. Main Content Sections (H2s, H3s, internal links <a> tags)\n  4. Use Case Explanations\n  5. Step-by-Step Tutorial (numbered list with headings)\n  6. Call-to-Action Section (use link: create.productai.photo)\n  8. Technical SEO elements (image alt text, responsive, semantic)\n\n## Markdown Formatting:\n- Don\'t use H1 for the article title, start with H2.\n- Use H2 html tag for main sections and H3 html tag for subsections.\n- Short paragraphs, clear headings, internal links where relevant.\n- All CTA buttons or links use <a> tag must point to: **create.productai.photo**\n- Add **bold**, *italic*, and lists <li> where helpful.\n- User rich text elements HTML:\n* Paragraphs\n* Heading tags (H1-H6)\n* Images\n* Image captions\n* Image alt attributes\n* Custom code\n* Block quotes\n* Unordered lists (bulleted)\n* Ordered lists (numbered)\n* Videos (Youtube, Vimeo)\n* Rich media (Google maps, SoundCloud, Imgur, Giphy, Codepen, and more)\n\n## Output Rules:\n- DO NOT say “I will now rewrite...” or "I\'ll rewrite the article based on your requirements. " explain anything, just return the article.\n- DO NOT include planning or step-by-step generation.\n- Just return the **fully rewritten markdown article** in the required JSON structure.\n- Include a brief SEO-friendly summary.\n- Instead of a competition company name like (PixelCut, pixelcut, CreatorKit, Soona, Pebblely), ALWAYS use "ProductAI"\n\n## Source Article:\nRewrite the following article: {{ $(\'Google Sheets\').item.json.URL }} \n\n\n## Content Structure:\n{{ $json.output }} \n\n## Take internal links from webflow tool',
      options: {},
      promptType: 'define',
      hasOutputParser: true
          ...
          jsonSchemaExample: '{\n"article": "long article text",\n"summary": "summary of article",\n"author": "Marko Balažic"\n}'
        }, name: 'Structured Output Parser1' } }) }, name: 'Auto-fixing Output Parser' } }) }, position: [540, 100], name: 'Content writer' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [1440, -200], name: 'HTTP Request1' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 30 }, position: [1400, 20] } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: '={{ $json.urls.get }}',
      options: {},
      sendHeaders: true,
    ...
    }, position: [1600, 20], name: 'get_image_url' } }))
  ...
  .add(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
    ...
    }, position: [1880, 320], name: 'Google Sheets1' } }))
  .then(node({ type: 'n8n-nodes-base.noOp', version: 1, config: { position: [2080, 260], name: 'No Operation, do nothing' } }))
  .add(node({ type: 'n8n-nodes-base.webflow', version: 2, config: { parameters: {
      live: true,
      siteId: '648717e882e5860a12ab9d1c',
      fieldsUi: {
        fieldValues: [
  ...
  .add(sticky('## Content layout?', { name: 'Sticky Note4', color: 7, position: [40, -60], width: 400, height: 500 }))
  .add(sticky('## Content writer', { name: 'Sticky Note5', color: 7, position: [460, -40], width: 860, height: 560 }))