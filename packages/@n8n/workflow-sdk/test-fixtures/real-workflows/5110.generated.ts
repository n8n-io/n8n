const wf = workflow('LPUpZtHK7gGRA5wa', 'Automated AI YouTube Shorts Factory for ASMR (Seedance)', {
	executionOrder: 'v1',
})
	.add(
		trigger({
		...
		node({
			...
			config: {
				parameters: {
					...
					options: {
						systemMessage:
							'You are an AI that specializes in identifying viral trends on platforms like TikTok and YouTube Shorts. Your job is to brainstorm a single, simple, and satisfying ASMR video concept. You must only return the idea as a single line of plain text. Do not add any extra words, formatting, or explanation.',
					},
					promptType: 'define',
	...
	.then(
		node({
			...
			version: 1.1,
			config: { parameters: { amount: 120 }, position: [3480, 220], name: 'Wait for Clips' },
		}),
	)
	.then(
		node({
	...
	.then(
		node({
			type: 'n8n-nodes-base.wait',
			version: 1.1,
			config: { parameters: { amount: 60 }, position: [3480, 440], name: 'Wait for Sounds' },
		}),
	)
	.then(
		node({
	...
	.then(
		node({
			type: 'n8n-nodes-base.wait',
			version: 1.1,
			config: { parameters: { amount: 60 }, position: [3480, 640], name: 'Wait for Final Video' },
		}),
	)
	.then(
		node({
		...
		node({
			...
			config: {
				parameters: {
					...
					message:
						"=✅ Your new video is ready!  **Title:** {{ $('Update Final Video to Sheet').item.json.idea }}  Watch it here: https://www.youtube.com/watch?v={{ $('Upload to YouTube').item.json.id }}",
	...
	.add(
		sticky(
			...
			{ name: 'Note: Asset Generation', color: 3, position: [2560, 0], width: 600, height: 180 },
		),
	)
	.add(
		sticky('### Step 3: Final Assembly\nThe video clips are stitched together.', {
