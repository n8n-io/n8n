const wf = workflow('', '')
	...
	.then(
		node({
			...
			version: 3.4,
			config: {
				parameters: { mode: 'raw', options: {}, jsonOutput: '={{ $json.body }}' },
				position: [240, 64],
				name: 'Edit Fields',
			},
		}),
	...
	.then(
		node({
			...
			config: {
				parameters: {
					filters: {
						conditions: [{ keyName: 'id', keyValue: '={{ $json.id }}', condition: 'eq' }],
					},
					tableId: 'demo_contacts',
					operation: 'update',
					dataToSend: 'autoMapInputData',
	...
	.add(
		sticky(
			...
			{ name: 'Sticky Note10', color: 6, position: [1248, -32], width: 848, height: 171 },
		),
	)
	.add(
		sticky(
