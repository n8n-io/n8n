return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{}] } }, position: [-1240, -380], name: 'Scheduled Start: Check for New Posts' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
    ...
    }, position: [-920, -380], name: '1. Get Next Post Idea from Sheet' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-620, -380], name: '2. Prepare Input Variables (Topic, Audience, etc.)' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.5, config: { parameters: {
      ...
      hasOutputParser: true
        ...
        }, name: '(Parse Concept JSON)' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatGoogleGemini', version: 1, config: { parameters: { options: {}, modelName: 'models/gemini-2.0-flash-001' }, credentials: {
          googlePalmApi: { id: 'credential-id', name: 'googlePalmApi Credential' }
        }, name: '(LLM Model for Concept)' } }) }, position: [-160, -380], name: '3a. Generate Content Concept (Gemini)' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.5, config: { parameters: {
      text: '=',
      messages: {
        messageValues: [
      ...
      hasOutputParser: true
        ...
        }, name: '(Parse Prompts JSON)' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatGoogleGemini', version: 1, config: { parameters: { options: {}, modelName: 'models/gemini-2.0-flash-001' }, credentials: {
          googlePalmApi: { id: 'credential-id', name: 'googlePalmApi Credential' }
        }, name: '(LLM Model for Prompts)' } }) }, position: [580, -380], name: '3b. Generate Image Prompt Options (Gemini)' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.5, config: { parameters: {
      text: '=',
      messages: {
        messageValues: [
      ...
      hasOutputParser: true
        ...
        }, name: '(Parse Caption JSON)' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatGoogleGemini', version: 1, config: { parameters: { options: {}, modelName: 'models/gemini-2.0-flash' }, credentials: {
          googlePalmApi: { id: 'credential-id', name: 'googlePalmApi Credential' }
        }, name: '(LLM Model for Caption)' } }) }, position: [1540, -380], name: '3c. Generate Post Caption (Gemini)' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [2360, -380], name: '4. Generate Image using Prompt 1 (Replicate Flux)' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [2880, -380], name: '5. Prepare Data for Instagram API' } }))
  .then(node({ type: 'n8n-nodes-base.facebookGraphApi', version: 1, config: { parameters: {
    ...
    }, position: [3180, -380], name: '6a. Create Instagram Media Container' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { position: [3480, -380], name: '6b. Wait for Container Processing' } }))
  .then(node({ type: 'n8n-nodes-base.facebookGraphApi', version: 1, config: { parameters: {
      edge: 'media_publish',
      node: '17841473009917118',
      options: {
      ...
      facebookGraphApi: { id: 'credential-id', name: 'facebookGraphApi Credential' }
    }, position: [3780, -380], name: '6c. Publish Post to Instagram' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
      columns: {
        value: {
