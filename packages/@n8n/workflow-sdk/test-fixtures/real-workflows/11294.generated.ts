return workflow('iseNsMusqpCWXzue', 'Automate Inventory Reordering from Airtable using GPT-4o, Slack & Email', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-2144, 688], name: 'When clicking ‘Execute workflow’' } }))
  .then(node({ type: 'n8n-nodes-base.airtable', version: 2.1, config: { parameters: {
    ...
    }, position: [-1888, 688], name: 'Fetch Inventory Records from Airtable' } }))
  .then(ifBranch([node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.1, config: { parameters: {
    ...
    }, subnodes: { model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatAzureOpenAi', version: 1, config: { parameters: { model: 'gpt-4o', options: {} }, credentials: {
          azureOpenAiApi: { id: 'C3WzT18XqF8OdVM6', name: 'Azure Open AI account' }
        }, name: 'Configure GPT-4o — Inventory Optimization Model' } }) }, position: [-1184, 672], name: 'Generate Inventory Optimization Output (AI)' } }), node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
        value: {},
        schema: [
          {
    ...
    }, position: [-1520, 960], name: 'Log Invalid Inventory Rows to Google Sheet' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-768, 672], name: 'Merge AI Optimization Results ' } }))
  .add(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.1, config: { parameters: {
      ...
      promptType: 'define'
          ...
          azureOpenAiApi: { id: 'C3WzT18XqF8OdVM6', name: 'Azure Open AI account' }
        }, name: 'Configure GPT-4o Model' } }) }, position: [64, 1408], name: 'Generate Inventory Email Summary ' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
    ...
    }, position: [448, 1408], name: 'Email Inventory Summary to Manager' } }))
  .add(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.1, config: { parameters: {
      ...
      promptType: 'define'
          ...
          azureOpenAiApi: { id: 'C3WzT18XqF8OdVM6', name: 'Azure Open AI account' }
        }, name: 'Configure GPT-4o Model1' } }) }, position: [0, 608], name: 'Generate Inventory Slack Summary' } }))
  .then(node({ type: 'n8n-nodes-base.slack', version: 2.1, config: { parameters: {
      text: '={{ $json.output }}',
      user: {
        __rl: true,
        mode: 'list',
    ...
    }, position: [432, 608], name: 'Notify Operations Team on Slack' } }))
  .add(node({ type: 'n8n-nodes-base.asana', version: 1, config: { parameters: {
    ...
    }, position: [208, -80], name: 'Create Asana Task for Inventory Db' } }))
  .add(node({ type: 'n8n-nodes-base.notion', version: 2.2, config: { parameters: {
    ...
    }, position: [-64, -176], name: 'Log Trade Decision to Notion (inventory db)' } }))
  .add(trigger({ type: 'n8n-nodes-base.errorTrigger', version: 1, config: { position: [-1888, 1552], name: 'Workflow Error Handler' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.2, config: { parameters: {
