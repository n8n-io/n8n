return workflow('ZLvtiRLuKIdbuPvG', 'SEO Strategy Director', { executionOrder: 'v1' })
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.chatTrigger', version: 1.3, config: { parameters: {
      public: true,
      options: {
    ...
    }, position: [-160, -928], name: 'When chat message received' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.2, config: { parameters: {
      ...
      promptType: 'define'
    }, subnodes: { tools: [tool({ type: '@n8n/n8n-nodes-langchain.toolThink', version: 1.1, config: { name: 'Think' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolSerpApi', version: 1, config: { parameters: { options: {} }, credentials: {
          serpApi: { id: 'credential-id', name: 'serpApi Credential' }
        }, name: 'SerpAPI' } }), tool({ type: '@n8n/n8n-nodes-langchain.agentTool', version: 2.2, config: { parameters: {
          text: '=={{ $json.chatInput }}',
          options: {
        ...
        }, name: 'OpenAI Chat Model6' } }) }, name: 'SEO Content Writer' } }), tool({ type: '@n8n/n8n-nodes-langchain.agentTool', version: 2.2, config: { parameters: {
          text: '=={{ $json.chatInput }}',
          options: {
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Chat Model3' } }) }, name: 'Local SEO Specialist' } }), tool({ type: '@n8n/n8n-nodes-langchain.agentTool', version: 2.2, config: { parameters: {
          text: '=={{ $json.chatInput }}',
          options: {
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Chat Model4' } }) }, name: 'Link Building Strategist' } }), tool({ type: '@n8n/n8n-nodes-langchain.agentTool', version: 2.2, config: { parameters: {
          text: '=={{ $json.chatInput }}',
          options: {
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Chat Model5' } }) }, name: 'SEO Analytics Specialist' } }), tool({ type: '@n8n/n8n-nodes-langchain.agentTool', version: 2.2, config: { parameters: {
          text: '=={{ $json.chatInput }}',
          options: {
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Chat Model2' } }) }, name: 'Technical SEO Specialist' } }), tool({ type: '@n8n/n8n-nodes-langchain.agentTool', version: 2.2, config: { parameters: {
          text: '=={{ $json.chatInput }}',
          options: {
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Chat Model1' } }) }, name: 'Keyword Research Specialist' } })], memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryBufferWindow', version: 1.3, config: { parameters: { contextWindowLength: 25 }, name: 'Simple Memory' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
          model: { __rl: true, mode: 'list', value: 'gpt-4.1-mini' },
          options: {}
        }, credentials: {
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Chat Model' } }) }, position: [128, -752], name: 'SEO Director Agent' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.chat', version: 1, config: { parameters: {
      message: '={{ $json.output }}',
      options: {},
      waitUserReply: false
    }, position: [880, -880], name: 'Respond to Chat' } }))
  .add(sticky('## How it works\nThis workflow acts as an autonomous "AI SEO Agency." It is managed by a **Director Agent** (top center) that coordinates a team of 6 specialists.\n\n1. **Analysis:** When you chat, the Director checks **Memory** and runs live market research using **SerpApi (Google Search)**.\n2. **Delegation:** Based on real-time data, the Director assigns tasks to the 6 **Specialist Agents** below (Keyword, Technical, Link Building, Analytics, Local, and Content).\n3. **Reporting:** The Director compiles their work into a single, professional strategy report.\n\n## Setup steps\n1. **OpenAI Keys:** Add your OpenAI API key to **all 7** "OpenAI Chat Model" nodes (1 for the Director, 6 for the Specialists).\n2. **SerpApi Key:** Click the "SEO Director Agent" node. Under "Tools" > "SerpApi," add your key to enable live Google Search.\n3. **Run:** Click "Active" and start chatting.', { name: 'Sticky Note5', position: [-1184, -1296], width: 544, height: 448 }))
  .add(sticky('## The Manager Core\nThe **SEO Director Agent** is the brain. It receives your prompt, pulls context from **Memory**, and gathers live data via **SerpAPI** before deciding which specialists to hire for the job.', { name: 'Sticky Note', color: 7, position: [-208, -1088], width: 864, height: 528 }))
  .add(sticky('## The Specialist Team\nThese 6 nodes are configured as **Tools**. They don\'t run automatically; the Director calls them individually to perform specific tasks (like running a technical audit or researching keywords).\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n## Specialist LLMs\nThese models power the individual specialist agents. You need to add your OpenAI credential to each of these nodes (you can use the same key for all of them).', { name: 'Sticky Note1', color: 7, position: [-784, -544], width: 1952, height: 592 }))