return workflow('OvYZQiWH2KlJsFbK', 'Multi-Agent Evaluation (eval nodes)', { executionOrder: 'v1' })
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.chatTrigger', version: 1.1, config: { parameters: { options: {} }, position: [-760, 40], name: 'When chat message received' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.9, config: { parameters: {
      options: {
    ...
    }, subnodes: { tools: [tool({ type: '@n8n/n8n-nodes-langchain.vectorStoreQdrant', version: 1.3, config: { parameters: {
        ...
        }, name: 'Embeddings OpenAI' } }) }, name: 'Search_db' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolCalculator', version: 1, config: { name: 'Calculator' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolWorkflow', version: 2.2, config: { parameters: {
          workflowId: {
            __rl: true,
            mode: 'list',
        ...
        }, name: 'Summarizer' } }), tool({ type: 'n8n-nodes-base.httpRequestTool', version: 4.2, config: { parameters: {
        ...
        }, name: 'Web search' } })], model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenRouter', version: 1, config: { parameters: { model: 'openai/o3', options: {} }, credentials: {
          openRouterApi: { id: 'credential-id', name: 'openRouterApi Credential' }
        }, name: 'OpenRouter Chat Model' } }) }, position: [-380, -60], name: 'Search Agent' } }))
  .then(node({ type: 'n8n-nodes-base.evaluation', version: 4.6, config: { parameters: { operation: 'checkIfEvaluating' }, position: [320, -60], name: 'Evaluating?' } }))
  .add(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [540, -160], name: 'Check if tool called' } }))
  .then(node({ type: 'n8n-nodes-base.evaluation', version: 4.6, config: { parameters: {
      outputs: {
        values: [
          {
    ...
    }, position: [760, -160], name: 'Set Outputs' } }))
  .then(node({ type: 'n8n-nodes-base.evaluation', version: 4.6, config: { parameters: {
      metrics: {
        assignments: [
          {
  ...
  .add(node({ type: 'n8n-nodes-base.noOp', version: 1, config: { position: [536, 40], name: 'Return chat response' } }))
  .add(trigger({ type: 'n8n-nodes-base.evaluationTrigger', version: 4.6, config: { parameters: {
      sheetName: {
        __rl: true,
        mode: 'list',
        value: 969651976,
    ...
    }, position: [-980, -160], name: 'When fetching a dataset row' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-760, -160], name: 'Match chat format' } }))