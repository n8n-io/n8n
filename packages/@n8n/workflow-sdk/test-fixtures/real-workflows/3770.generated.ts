return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.executeWorkflowTrigger', version: 1.1, config: { parameters: {
    ...
    }, position: [-3060, 600], name: 'When Executed by Another Workflow' } }))
  .then(node({ type: 'n8n-nodes-base.redis', version: 1, config: { parameters: {
      key: 'mcp_n8n_tools',
      options: {},
      operation: 'get',
      propertyName: 'data'
    }, credentials: { redis: { id: 'credential-id', name: 'redis Credential' } }, position: [-2860, 600], name: 'Get Memory' } }))
  .then(switchCase([node({ type: 'n8n-nodes-base.n8n', version: 1, config: { parameters: { filters: { tags: 'mcp' }, requestOptions: {} }, credentials: { n8nApi: { id: 'credential-id', name: 'n8nApi Credential' } }, position: [-2400, 200], name: 'Get MCP-tagged Workflows' } }), node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
          {
    ...
    }, position: [-2400, 400], name: 'Convert to JSON' } }), node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
          {
    ...
    }, position: [-2400, 600], name: 'listTools Success' } }), node({ type: 'n8n-nodes-base.n8n', version: 1, config: { parameters: { filters: { tags: 'mcp' }, requestOptions: {} }, credentials: { n8nApi: { id: 'credential-id', name: 'n8nApi Credential' } }, position: [-2180, 600], name: 'Get MCP-tagged Workflows1' } }), node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
          {
    ...
    }, position: [-2360, 1120], name: 'Convert to JSON1' } })], { version: 3.2, parameters: {
      rules: {
        values: [
          {
            outputKey: 'Add',
  ...
  .then(node({ type: 'n8n-nodes-base.filter', version: 2.2, config: { parameters: {
    ...
    }, position: [-2180, 200], name: 'Filter Matching Ids' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-1740, 0], name: 'Simplify Workflows' } }), node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
          {
    ...
    }, position: [-1740, 200], name: 'AddTool Error' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.redis', version: 1, config: { parameters: {
      ...
      operation: 'set'
    }, credentials: { redis: { id: 'credential-id', name: 'redis Credential' } }, position: [-1520, 0], name: 'Store In Memory' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-1300, 0], name: 'AddTool Success' } }))
  .then(node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'data' }, position: [-2180, 400] } }))
  .then(node({ type: 'n8n-nodes-base.filter', version: 2.2, config: { parameters: {
      options: {},
      conditions: {
        options: {
    ...
    }, position: [-1960, 400], name: 'Filter Matching IDs' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.redis', version: 1, config: { parameters: { key: 'mcp_n8n_tools', operation: 'delete' }, credentials: { redis: { id: 'credential-id', name: 'redis Credential' } }, position: [-1520, 300], name: 'Delete Key' } }), node({ type: 'n8n-nodes-base.redis', version: 1, config: { parameters: {
      key: 'mcp_n8n_tools',
      value: '={{ $input.all().flatMap(item => item.json.data).compact() }}',
      operation: 'set'
    }, credentials: { redis: { id: 'credential-id', name: 'redis Credential' } }, position: [-1520, 500], name: 'Store In Memory1' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-1300, 400], name: 'Remove Tool Success' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [-1960, 600], name: 'Simplify Workflows1' } }))
  .then(node({ type: 'n8n-nodes-base.aggregate', version: 1, config: { parameters: {
      options: {},
      aggregate: 'aggregateAllItemData',
      destinationFieldName: 'response'
    }, position: [-1740, 600], name: 'listTools Success1' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      mode: 'raw',
      options: {},
      jsonOutput: '={{ $(\'When Executed by Another Workflow\').first().json.parameters }}'
    }, position: [-1920, 1020], name: 'Get Parameters' } }), node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
          {
    ...
    }, position: [-1920, 1220], name: 'ExecuteTool Error' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.executeWorkflow', version: 1.2, config: { parameters: {
    ...
    }, position: [-1660, 1020], name: 'Execute Workflow with PassThrough Variables' } }))
  .then(node({ type: 'n8n-nodes-base.aggregate', version: 1, config: { parameters: {
      options: {},
      aggregate: 'aggregateAllItemData',
      destinationFieldName: 'response'
    }, position: [-1440, 1020], name: 'executeTool Result' } }))
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.chatTrigger', version: 1.1, config: { parameters: { options: {} }, position: [-3840, 1040], name: 'When chat message received' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.8, config: { parameters: {
      options: {
    ...
    }, subnodes: { tools: [tool({ type: '@n8n/n8n-nodes-langchain.mcpClientTool', version: 1, config: { parameters: { sseEndpoint: '=<Production URL of MCP Server>' }, name: 'MCP Client' } })], memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryBufferWindow', version: 1.3, config: { parameters: { contextWindowLength: 30 }, name: 'Simple Memory' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
          model: {
            __rl: true,
            mode: 'list',
            value: 'gpt-4.1-mini',
        ...
        }, name: 'OpenAI Chat Model' } }) }, position: [-3600, 1040], name: 'AI Agent' } }))
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.mcpTrigger', version: 1, config: { parameters: { path: '4625bcf4-0dd9-4562-a70f-6fee41f6f12d' }, subnodes: { tools: [tool({ type: '@n8n/n8n-nodes-langchain.toolWorkflow', version: 2.1, config: { parameters: {
          name: 'addWorkflow',
        ...
        }, name: 'Add Workflow' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolWorkflow', version: 2.1, config: { parameters: {
          name: 'listTool',
          workflowId: { __rl: true, mode: 'id', value: '={{ $workflow.id }}' },
        ...
        }, name: 'List Workflows' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolWorkflow', version: 2.1, config: { parameters: {
          name: 'removeWorkflow',
          workflowId: { __rl: true, mode: 'id', value: '={{ $workflow.id }}' },
        ...
        }, name: 'RemoveWorkflow' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolWorkflow', version: 2.1, config: { parameters: {
          name: 'executeTool',
          workflowId: { __rl: true, mode: 'id', value: '={{ $workflow.id }}' },
        ...
        }, name: 'ExecuteWorkflow' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolWorkflow', version: 2.1, config: { parameters: {
          name: 'searchTool',
          workflowId: { __rl: true, mode: 'id', value: '={{ $workflow.id }}' },
