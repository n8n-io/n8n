const wf = workflow('WrK2POhl0j01MouD', 'Apify_lead_generation', { executionOrder: 'v1' })
	.add(
		trigger({
			type: 'n8n-nodes-base.manualTrigger',
			version: 1,
			config: { position: [-580, 460], name: 'When clicking ‘Execute workflow’' },
		}),
	)
	.then(
		node({
	...
	.then(
		node({
			...
			version: 4.2,
			config: {
				parameters: { url: 'enter (get dataset url)', options: {} },
				position: [100, 460],
			...
			config: {
				parameters: {
					...
					sheetName: {
						...
						value: 'gid=0',
						cachedResultUrl:
							'https://docs.google.com/spreadsheets/d/1EdMrBVfX_aHipnjdngetJ2fapohi6pLqULL5Onv-KpA/edit#gid=0',
	...
	.add(
		sticky(
			...
			{ color: 7, position: [-700, 240], width: 1560, height: 480 },
		),
	)
	.add(
		sticky('## Start the Apify Web Scraper Tool', {
			name: 'Sticky Note1',
			position: [-640, 380],
			width: 460,
			height: 260,
	...
	.add(
		sticky(
			'## Small amount lead generation\n**Use for a smaller amount of lead Scraping**\n- Apify HTTP request',
			{ name: 'Sticky Note3', color: 6, position: [-140, -60], width: 400, height: 280 },
		),
	)
	.add(
		sticky('## Add & Organize Results Into Google Sheet', {
			name: 'Sticky Note4',
			color: 5,
			position: [300, 380],
			width: 500,
