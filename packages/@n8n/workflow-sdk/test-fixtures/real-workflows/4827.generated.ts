const wf = workflow('', '')
	.add(
		trigger({
			...
			version: 1,
			config: { position: [1380, -560], name: 'When clicking "Execute Workflow"' },
		}),
	)
	.then(
		node({
	...
	.then(
		node({
			...
			version: 1.8,
			config: {
				parameters: { options: {}, resource: 'audio', operation: 'translate' },
				credentials: {
					openAiApi: { id: 'credential-id', name: 'openAiApi Credential' },
				},
				position: [2460, 160],
	...
	.add(
		sticky(
			...
			{ color: 5, position: [1000, -580] },
		),
	)
	.add(
		sticky(
			'This workflow listens for WhatsApp messages (text, audio, image, documents), converts them into embeddings, searches MongoDB, and uses GPT-4o-mini to provide context-aware answers with conversation memory.',
			{ name: 'Sticky Note1', color: 4, position: [1000, 360], height: 280 },
		),
	)
	.add(
		sticky(
