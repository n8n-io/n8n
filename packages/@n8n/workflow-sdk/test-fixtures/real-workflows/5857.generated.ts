const wf = workflow('SIDZ9gnOf5yEftvX', 'chat', { executionOrder: 'v1' })
	.add(
		trigger({
	...
	.add(
		sticky(
			...
			{ position: [-1520, -740], width: 340, height: 480 },
		),
	)
	.add(
		sticky(
			'This route runs when an image/file/pdf is attached. It saves what it sees in the chatmem',
			{ name: 'Sticky Note1', position: [-1100, -740] },
		),
	)
	.add(
		sticky('Use this node to set the conversation', {
			name: 'Sticky Note2',
			position: [-360, -640],
		}),
	)
	.add(
		sticky('This runs after the upload, it retrieves the initial content from the memory\n', {
			name: 'Sticky Note3',
			position: [-960, -160],
		}),
	)
	.add(
		sticky('This is the main chat, adjust the prompt to match your use case.', {
			name: 'Sticky Note4',
			position: [-360, -140],
		}),
	);
