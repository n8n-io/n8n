const wf = workflow('', '')
			...
			config: {
				parameters: {
					...
					folderToWatch: {
						...
						cachedResultUrl:
							'https://drive.google.com/drive/folders/1UHQhCrwZg_ZEzBIKv4LR_RWtAyHbxZsg',
	...
	.then(
		node({
			...
			version: 1,
			config: { position: [1320, 1500], name: 'Reformat' },
		}),
	)
	.then(
		node({
	...
	.add(
		sticky('# Upload New File into Knowledge Base\n\n', {
			color: 4,
			position: [500, 560],
			width: 820,
			height: 80,
	...
	.add(
		sticky('# Update File in Knowledge Base \n', {
			name: 'Sticky Note1',
			color: 4,
			position: [480, 1280],
			width: 640,
	...
	.add(
		sticky('## Convert Message to Text\n\n', {
			name: 'Sticky Note3',
			color: 5,
			position: [-1520, 800],
			width: 680,
