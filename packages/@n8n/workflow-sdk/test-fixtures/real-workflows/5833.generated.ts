return workflow('C4GyUqmPvAFtDBbG', '[IG] Carousel on IG', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{ field: 'minutes' }] } }, position: [-1020, 360] } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
    ...
    }, position: [-760, 360], name: 'Get Execution for Carousel' } }))
  .then(node({ type: 'n8n-nodes-base.googleDrive', version: 3, config: { parameters: {
    ...
    }, position: [-340, 360], name: 'Get image list from a Google Drive folder1' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [80, 360], name: 'Download Image from Google Drive (Carousel)' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [420, 360], name: 'Upload images to Cloudinary' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [780, 360], name: 'Setup for Instagram (access token, ig_business_id)' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [1100, 360], name: 'Create Media Container (Image)' } }))
  .then(node({ type: 'n8n-nodes-base.aggregate', version: 1, config: { parameters: {
    ...
    }, position: [1380, 360], name: 'Combine Instagram media containers' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [1700, 360], name: 'Create Media Container (Carousel)' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [2020, 360], name: 'Publish Instagram Carousel' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
        value: {
          Status: 'Processed',
