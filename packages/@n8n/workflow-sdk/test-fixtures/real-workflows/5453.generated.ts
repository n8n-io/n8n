return workflow('bKy4ngwZ5svUgPH6', 'hr screening system community1')
  .add(trigger({ type: 'n8n-nodes-base.formTrigger', version: 2.2, config: { parameters: {
    ...
    }, position: [-620, 500], name: 'On form submission' } }))
  .add(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
    ...
    }, position: [1560, 1020], name: 'job roles' } }))
  .then(merge([node({ type: '@n8n/n8n-nodes-langchain.chainSummarization', version: 2, config: { parameters: {
      options: {
        ...
        }, name: 'Google Gemini Chat Model' } }) }, position: [1460, 460], name: 'Summarization Chain' } }), node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      options: {},
      filtersUI: {
        values: [
          {
    ...
    }, position: [1560, 1020], name: 'job roles' } })], { version: 3.1, parameters: {
      mode: 'combine',
      options: {},
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.5, config: { parameters: {
      ...
      hasOutputParser: true
          ...
          schemaType: 'manual',
          inputSchema: '{\n	"type": "object",\n	"properties": {\n		"vote": {\n			"type": "string"\n		},\n		"consideration": {\n			"type": "string"\n		}\n	}\n}'
        }, name: 'Structured Output Parser' } }) }, position: [2120, 460], name: 'HR Expert' } }))
  ...
  .add(node({ type: 'n8n-nodes-base.googleDrive', version: 3, config: { parameters: {
    ...
    }, position: [400, 300], name: 'Upload CV' } }))
  .add(node({ type: 'n8n-nodes-base.extractFromFile', version: 1, config: { parameters: { options: {}, operation: 'pdf', binaryPropertyName: '=CV' }, position: [400, 520], name: 'Extract from File' } }))
  .then(merge([node({ type: '@n8n/n8n-nodes-langchain.informationExtractor', version: 1, config: { parameters: {
      text: '={{ $json.text }}',
      options: {
      ...
      inputSchema: '{\n	"type": "object",\n	"properties": {\n		"telephone": {\n			"type": "string"\n		},\n      "city": {\n			"type": "string"\n		},\n      "birthdate": {\n			"type": "string"\n		}\n	}\n}'
          ...
          googlePalmApi: { id: 'credential-id', name: 'googlePalmApi Credential' }
        }, name: 'Google Gemini Chat Model' } }) }, position: [720, 400], name: 'Personal Data' } }), node({ type: '@n8n/n8n-nodes-langchain.informationExtractor', version: 1, config: { parameters: {
      text: '={{ $json.text }}',
      options: {
        systemPromptTemplate: 'You are an expert extraction algorithm.\nOnly extract relevant information from the text.\nIf you do not know the value of an attribute asked to extract, you may omit the attribute\'s value.'
      },
      attributes: {
        attributes: [
          ...
          googlePalmApi: { id: 'credential-id', name: 'googlePalmApi Credential' }
        }, name: 'Google Gemini Chat Model' } }) }, position: [720, 580], name: 'Qualifications' } })], { version: 3, parameters: { mode: 'combine', options: {}, combineBy: 'combineAll' } }))
  .add(sticky('🎯 AI Resume Screener – Form-Based HR Automation\n\nThis workflow automates the entire resume screening process using AI.\nIt extracts key details from uploaded CVs, compares candidates to job-specific profiles stored in Google Sheets, and logs evaluation scores and summaries for easy HR review.\n\n🛠 Setup Required:\n\nGoogle Sheets with Role + Profile Wanted columns\n\nGoogle Drive account (for CV uploads)\n\nOpenAI or Gemini API Key\n\nActivate webhook for the form\n\n✅ What this workflow does:\n\nAccepts CV uploads via form\n\nExtracts candidate qualifications and personal data using AI\n\nSummarizes the candidate’s background\n\nLooks up the job profile from your Google Sheet\n\nScores and evaluates the match using AI\n\nSaves results and uploads CV to Drive\n\n🤝 Need Help Setting This Up?\n\nI’m happy to assist you in customizing or deploying this workflow for your team.\n\n📧 Email: tharwat.elsayed2000@gmail.com\n💬 WhatsApp: +20106 180 3236\n\n🧰 Optional Extensions:\n\nAdd email or Slack notifications\n\nSupport multiple languages with a translation step\n\nReplace the form trigger with Google Drive or Airtable\n\n', { name: 'Sticky Note', color: 3, position: [-1560, -220], width: 540, height: 920 }))
  .add(sticky('The CV is uploaded to Google Drive and converted so that it can be processed\n', { name: 'Sticky Note1', position: [320, 220], width: 300, height: 420 }))
  .add(sticky('The essential information for evaluating the candidate is collected in two different chains', { name: 'Sticky Note2', position: [680, 300], width: 360, height: 440 }))
  .add(sticky('Summary of relevant information useful for classifying the candidate', { name: 'Sticky Note3', position: [1420, 380], width: 320, height: 240 }))
  .add(sticky('Characteristics of the profile sought by the company that intends to hire the candidate', { name: 'Sticky Note4', position: [1800, 380], width: 220, height: 240 }))
  .add(sticky('Candidate evaluation with vote and considerations of the HR agent relating the profile sought with the candidate\'s skills', { name: 'Sticky Note5', position: [2060, 380], width: 360, height: 240 }))