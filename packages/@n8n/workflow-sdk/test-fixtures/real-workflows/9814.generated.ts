const wf = workflow('', '')
	.add(
		trigger({
			...
			version: 1,
			config: { position: [976, -144], name: 'When clicking ‘Execute workflow’' },
		}),
	)
	.then(
		node({
	...
	.then(
		node({
			type: 'n8n-nodes-base.wait',
			version: 1.1,
			config: { parameters: { unit: 'minutes' }, position: [1808, -96], name: 'Wait1' },
		}),
	)
	.then(
		node({
	...
	.then(
		node({
			type: 'n8n-nodes-base.wait',
			version: 1.1,
			config: { parameters: { unit: 'minutes' }, position: [1824, 144], name: 'Wait2' },
		}),
	)
	.then(
		node({
	...
	.add(
		sticky(
			'## Creative Outreach Agent - A dedicated agent designated for creativity and equipped with enriched data for a creative personalized email',
			{ color: 4, position: [560, 352], width: 1872, height: 528 },
		),
	)
	.add(
		sticky(
			'## Data Enrichment - Step 1; Fetch person details from local table, scrap data from multiple resources to enrich the user profile',
			{ name: 'Sticky Note1', position: [928, -448], width: 1504, height: 768 },
		),
	)
	.add(
		sticky(
	...
	.add(
		sticky('## Enrich personal LinkedIn data', {
			name: 'Sticky Note3',
			color: 6,
			position: [1568, -400],
			width: 800,
	...
	.add(
		sticky("## Enrich Company's LinkedIn data", {
			name: 'Sticky Note4',
			color: 6,
			position: [1568, -160],
			width: 800,
	...
	.add(
		sticky("## Enrich Company's Crunchbase data", {
			name: 'Sticky Note5',
			color: 6,
			position: [1568, 80],
			width: 800,
