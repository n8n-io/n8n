return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.formTrigger', version: 2.1, config: { parameters: {
    ...
    }, position: [-704, 272], name: 'Form: Upload Video' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [-496, 272], name: 'FFmpeg: Extract Audio' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 10 }, position: [-336, 272], name: 'Wait 10s (Audio)' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [-128, 272], name: 'Check Audio Job Status' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [208, 144], name: 'Download Audio' } }), node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { position: [208, 288], name: 'Wait 5s & Retry (Audio)' } })], { version: 2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 1,
  ...
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [384, 144], name: 'Whisper: Transcribe with Timestamps' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [576, 144], name: 'Parse Whisper Results' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.2, config: { parameters: {
      ...
      hasOutputParser: true
        ...
        }, name: 'Google Gemini Chat Model' } }) }, position: [784, 144], name: 'AI Agent - Select Viral Clips' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [1088, 144], name: 'Parse Gemini Analysis' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: 'https://api.upload-post.com/api/uploadposts/ffmpeg/jobs/upload',
      method: 'POST',
      options: {},
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [1264, 144], name: 'FFmpeg: Upload & Cut' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 10 }, position: [1472, 144], name: 'Wait 10s (Short)' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: '=https://api.upload-post.com/api/uploadposts/ffmpeg/jobs/{{ $json.job_id }}',
      options: {},
      authentication: 'genericCredentialType',
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [1680, 144], name: 'Check Short Job Status' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: '=https://api.upload-post.com/api/uploadposts/ffmpeg/jobs/{{ $json.job_id }}/download',
      options: { response: { response: {} } },
      authentication: 'genericCredentialType',
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [2160, 128], name: 'Download Short' } }), node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { position: [2160, 288], name: 'Wait 5s & Retry (Short)' } })], { version: 2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 1,
  ...
  .then(node({ type: 'n8n-nodes-upload-post.uploadPost', version: 1, config: { parameters: {
    ...
    }, position: [2432, 128], name: 'Schedule to TikTok, Instagram, and YouTube' } }))
  .add(sticky('**AUTO-SHORTS GENERATOR**\n\n**How it Works:**\n1.  Upload a long video via the form.\n2.  AI transcribes the audio (Whisper) and finds the most viral clips (Gemini).\n3.  The clips are auto-cut, cropped, and prepared for social media.\n4.  Shorts are scheduled to post daily on TikTok, Instagram, and YouTube.\n\n**Quick Setup:**\n1.  **OpenAI Whisper:** Add your API key to the `Whisper: Transcribe` node.\n2.  **Google Gemini:** Add your Google AI Studio API key to the `Google Gemini Chat Model` node.\n3.  **Upload-Post:**\n    *   Sign up at `app.upload-post.com` & connect your social accounts.\n    *   Generate an API token.\n    *   In n8n, create credentials for the `HTTP Request` nodes (Header Auth) and the `Schedule...` node (Upload-Post API) using that token.\n4.  **Scheduling:** Adjust the post time and timezone in the `Schedule to...` node.\n5.  **To Run:** Use the URL from the `Form: Upload Video` node to upload your video.', { name: 'Sticky Note', color: 7, position: [-752, -464], width: 480, height: 592 }))
  .add(sticky('**Intake & Form**\n- Form: Upload Video', { name: 'Section: Intake & Form', position: [-752, 128], width: 228, height: 352 }))
  .add(sticky('**Transcription & Parsing**\n- Whisper: Transcribe with Timestamps\n- Parse Whisper Results\n- Download Audio', { name: 'Section: Transcription & Parsing', position: [352, 16], width: 428, height: 320 }))
  .add(sticky('**AI Selection**\n- Google Gemini Chat Model\n- AI Agent – Select Viral Clips\n- Parse Gemini Analysis', { name: 'Section: AI Selection (Gemini)', position: [768, 16], width: 420, height: 480 }))
  .add(sticky('**FFmpeg Audio Job Loop**\n- FFmpeg: Extract Audio\n- Wait 10s (Audio) → Check Audio Job Status → Is Audio Job Completed?\n- Retry path: Wait 5s & Retry (Audio)', { name: 'Section: FFmpeg Audio Job Loop', position: [-528, 128], width: 880, height: 352 }))
  .add(sticky('**FFmpeg Short Job Loop**\n- FFmpeg: Upload & Cut\n- Wait 10s (Short) → Check Short Job Status → Is Short Job Completed?\n- Retry path: Wait 5s & Retry (Short)', { name: 'Section: FFmpeg Short Job Loop', position: [1184, 16], width: 1132, height: 480 }))
  .add(sticky('**Scheduling**\n- Download Short\n- Schedule to TikTok, Instagram, and YouTube', { name: 'Section: Scheduling', position: [2320, 16], width: 420, height: 480 }))