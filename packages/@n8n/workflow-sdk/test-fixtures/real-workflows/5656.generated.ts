return workflow('h7qrcXor10jWVpln', 'Voice2Propal – Smart WhatsApp Proposal Generator (Voice & Text)')
  .add(trigger({ type: 'n8n-nodes-base.whatsAppTrigger', version: 1, config: { parameters: { updates: ['messages'] }, credentials: {
      whatsAppTriggerApi: { id: 'credential-id', name: 'whatsAppTriggerApi Credential' }
    }, position: [-500, 500], name: 'WhatsApp Trigger' } }))
  .then(node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: '={{ $json.field }}' }, position: [-280, 500] } }))
  .then(switchCase([node({ type: 'n8n-nodes-base.whatsApp', version: 1, config: { parameters: {
      resource: 'media',
      operation: 'mediaUrlGet',
      mediaGetId: '={{ $json.audio.id }}'
    }, credentials: {
      whatsAppApi: { id: 'credential-id', name: 'whatsAppApi Credential' }
    }, position: [160, 420], name: 'WhatsApp Business Cloud' } }), node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
          {
    ...
    }, position: [600, 600], name: 'Edit Fields' } })], { version: 3.2, parameters: {
      rules: {
        values: [
          {
            outputKey: 'voice',
  ...
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [820, 500], name: 'Edit Fields1' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.8, config: { parameters: {
      ...
      promptType: 'define'
    }, subnodes: { tools: [tool({ type: 'n8n-nodes-base.airtableTool', version: 2.1, config: { parameters: {
          base: {
            __rl: true,
            mode: 'list',
        ...
        }, name: 'Get Info' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolCalculator', version: 1, config: { name: 'Calculator' } })], memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryBufferWindow', version: 1.3, config: { parameters: {
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Chat Model' } }) }, position: [1140, 500], name: 'AI Agent' } }))
  .then(node({ type: 'n8n-nodes-base.apiTemplateIo', version: 1, config: { parameters: {
      resource: 'pdf',
    ...
    }, position: [1600, 500], name: 'APITemplate.io' } }))
