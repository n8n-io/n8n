const wf = workflow('6BWdxf2GdoYg7sSo', 'Blog Writing', { executionOrder: 'v1' })
	.add(
		trigger({
	...
	.then(
		node({
			...
			version: 4.2,
			config: {
				parameters: { url: '={{$("Split Out").item.json.loc}}', options: {} },
				position: [-1792, 336],
				name: 'HTTP Request1',
			},
		}),
	...
	.then(
		node({
			type: 'n8n-nodes-base.xml',
			version: 1,
			config: { parameters: { options: {} }, position: [-1616, 336], name: 'XML1' },
		}),
	)
	.then(
		node({
	...
	.then(
		node({
			...
			config: {
				...
				position: [-976, 336],
				name: 'Set Airtable Fields for Agent',
			},
		}),
	)
	.then(
				...
				parameters: {
					...
					columns: {
						value: {
							...
							'Primary Keyword':
								"={{ $('Set Airtable Fields for Agents').item.json.Primary.Keyword }}",
						...
						assignments: [
							{
								...
								type: 'string',
								value:
									"=https://docs.google.com/document/d/{{ $('Add Final Article').item.json.documentId }}/edit",
							},
						],
					},
				},
	...
	.add(
		node({
			...
			config: {
				...
				position: [576, 528],
				name: 'Update Article Writer table1',
			},
		}),
	)
	.add(
	...
	.add(
		node({
			...
			config: {
				...
				position: [336, 528],
				name: 'Set KWs and Insights fields1',
			},
		}),
	)
	.add(
	...
	.add(
		sticky('## Create Main Body Prompt\n', {
			name: 'Sticky Note8',
			position: [-592, 736],
			width: 400,
			height: 540,
	...
	.add(
		sticky('## Assemble Entire Article\n', {
			name: 'Sticky Note11',
			position: [1296, 1632],
			width: 380,
			height: 540,
	...
	.add(
		sticky('## Perform Final Editor and Quality Check', {
			name: 'Sticky Note12',
			position: [1696, 1632],
			width: 560,
			height: 540,
	...
	.add(
		sticky('## Add Your Tavily API key', {
			name: 'Sticky Note13',
			color: 3,
			position: [-112, 496],
			height: 200,
