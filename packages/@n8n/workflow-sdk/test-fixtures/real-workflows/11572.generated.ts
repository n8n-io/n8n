const wf = workflow('aRW40LTNzG8bWwiu', 'TEMPLATE - R1 - News Aggregators', {
	...
	.then(
		node({
			...
			version: 1,
			config: {
				parameters: { options: {}, fieldToSplitOut: 'articles' },
				position: [336, -96],
				name: 'Split Out 1',
			},
		}),
	...
	.then(
		node({
			...
			version: 1.1,
			config: { position: [1680, 272], name: 'Wait1' },
		}),
	)
	.add(
		sticky(
			'## [Mediastack](https://www.mediastack.com)\n Free plan - 100 calls per month\n### Categories\n- General\n- Business\n- Entertainment\n- Health\n- Science\n- Sports\n- Technology',
			{ name: 'Sticky Note7', color: 4, position: [-320, -560], width: 2192, height: 320 },
		),
	)
	.add(
		sticky(
			...
			{ name: 'Sticky Note8', color: 3, position: [-320, -224], width: 2192, height: 336 },
		),
	)
	.add(
		sticky(
			'## [CurrentsAPI](https://currentsapi.services/en) - Latest News\nFree plan - 20 requests per day',
			{ name: 'Sticky Note10', color: 6, position: [-320, -864], width: 2192, height: 288 },
		),
	)
	.add(
		sticky(
			...
			{ name: 'Sticky Note12', color: 3, position: [-320, 128], width: 2192, height: 352 },
		),
	)
	.add(
		sticky(
			...
			{ color: 7, position: [-800, -864], width: 464, height: 1344 },
		),
	);
