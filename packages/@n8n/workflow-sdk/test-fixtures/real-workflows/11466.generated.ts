return workflow('V994vNilogCvoCug', 'News_Tech_EN', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.3, config: { parameters: { rule: { interval: [{}] } }, position: [-2032, 1248] } }))
  .add(node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: {
      url: 'https://feeds.feedburner.com/TheHackersNews',
      options: {}
    }, position: [-1040, -448], name: 'RSS_TheHackersNews' } }))
  .then(merge([node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: {
      url: 'https://feeds.feedburner.com/TheHackersNews',
      options: {}
    }, position: [-1040, -448], name: 'RSS_TheHackersNews' } }), node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: { url: 'https://cybersecuritynews.com/feed/', options: {} }, position: [-1040, -320], name: 'RSS_cybersecuritynews' } }), node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: { url: 'https://krebsonsecurity.com/feed/', options: {} }, position: [-1040, -192], name: 'RSS_krebsonsecurity' } }), node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: {
      url: 'https://feeds.feedburner.com/TheHackersNews',
      options: {}
    }, position: [-1040, -64], name: 'RSS_GrahamCluley' } }), node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: {
      url: 'https://cloudblog.withgoogle.com/topics/threat-intelligence/rss/',
      options: {}
    }, position: [-1040, 64], name: 'RSS_GoogleCloudBlog' } }), node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: { url: 'https://research.checkpoint.com/feed/', options: {} }, position: [-1040, 192], name: 'RSS_darkreading1' } }), node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: { url: 'https://www.darkreading.com/rss.xml', options: {} }, position: [-1040, 320], name: 'RSS_darkreading' } }), node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: { url: 'https://isc.sans.edu/rssfeed_full.xml', options: {} }, position: [-1040, 464], name: 'RSS_Sans' } })], { version: 3.2, parameters: { numberInputs: 8 }, name: 'Merge_Cyber1' }))
  .then(merge([node({ type: 'n8n-nodes-base.merge', version: 3.2, config: { parameters: { numberInputs: 8 }, position: [-736, -16], name: 'Merge_Cyber1' } }), node({ type: 'n8n-nodes-base.merge', version: 3.2, config: { position: [-736, 704], name: 'Merge_Cyber2' } }), node({ type: 'n8n-nodes-base.merge', version: 3.2, config: { parameters: { numberInputs: 5 }, position: [-736, 1104], name: 'Merge_Cyber3' } }), node({ type: 'n8n-nodes-base.merge', version: 3.2, config: { parameters: { numberInputs: 4 }, position: [-736, 1808], name: 'Merge_AI' } }), node({ type: 'n8n-nodes-base.merge', version: 3.2, config: { parameters: { numberInputs: 3 }, position: [-736, 2336], name: 'Merge_Nvidia' } })], { version: 3.2, parameters: { numberInputs: 5 }, name: 'Merge_All' }))
  ...
  .then(node({ type: 'n8n-nodes-base.sort', version: 1, config: { parameters: {
    ...
    }, position: [400, 1216], name: 'Sort - Articles by Date' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [608, 1216], name: 'Code in JavaScript' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.googleGemini', version: 1, config: { parameters: {
    ...
    }, position: [816, 1216], name: 'LLM - News Summarizer' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [1168, 1216], name: 'Build Final Newsletter HTML' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
    ...
    }, position: [1520, 1216], name: 'Send Final Digest Email' } }))
  .add(node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: {
      url: 'https://nvd.nist.gov/feeds/xml/cve/misc/nvd-rss.xml',
      options: {}
    }, position: [-1040, 624], name: 'RSS_cve1' } }))
  .add(node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: { url: 'https://cvefeed.io/rssfeed/latest.xml', options: {} }, position: [-1040, 752], name: 'RSS_cve2' } }))
  .add(node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: {
      url: 'https://www.ilsole24ore.com/rss/tecnologia--cybersicurezza.xml',
      options: {}
    }, position: [-1040, 912], name: 'RSS_ilsole24ore Cyber' } }))
  .add(node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: {
      url: 'https://www.ilsole24ore.com/rss/tecnologia.xml',
      options: {}
    }, position: [-1040, 1040], name: 'RSS_ilsole24ore Tech' } }))
  .add(node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: { url: 'https://www.cybersecurity360.it/feed/', options: {} }, position: [-1040, 1168], name: 'RSS_cybersecurity360' } }))
  .add(node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: { url: 'https://feeds.feedburner.com/eset/blog', options: {} }, position: [-1040, 1296], name: 'RSS Read' } }))
  .add(node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: { url: 'https://blog.talosintelligence.com/rss/', options: {} }, position: [-1040, 1440], name: 'RSS_CiscoTalos' } }))
  .add(node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: {
      url: 'https://www.artificialintelligence-news.com/feed/',
      options: {}
    }, position: [-1040, 1632], name: 'RSS_artificialintelligence-news.' } }))
  .add(node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: { url: 'https://research.google/blog/rss/', options: {} }, position: [-1040, 1760], name: 'RSS_GoogleResearch' } }))
  .add(node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: {
      url: 'https://news.mit.edu/rss/topic/artificial-intelligence2',
      options: {}
    }, position: [-1040, 1888], name: 'RSS_MIT' } }))
  .add(node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: { url: 'https://openai.com/news/rss.xml', options: {} }, position: [-1040, 2016], name: 'RSS_OpenAI' } }))
  .add(node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: {
      url: 'https://nvidianews.nvidia.com/releases.xml',
      options: {}
    }, position: [-1040, 2208], name: 'RSS_Nvidia1' } }))
  .add(node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: { url: 'https://developer.nvidia.com/blog/feed', options: {} }, position: [-1040, 2336], name: 'RSS_Nvidia2' } }))
  .add(node({ type: 'n8n-nodes-base.rssFeedRead', version: 1.2, config: { parameters: { url: 'https://feeds.feedburner.com/nvidiablog', options: {} }, position: [-1040, 2464], name: 'RSS_Nvidia3' } }))
  .add(sticky('### CyberSecurity', { name: 'Sticky Note', color: 7, position: [-1104, -512], height: 2096 }))
  .add(sticky('### AI', { name: 'Sticky Note1', color: 7, position: [-1104, 1600], height: 560 }))
  .add(sticky('### Nvidia', { name: 'Sticky Note2', color: 7, position: [-1104, 2176], height: 416 }))
  .add(sticky('## RSS Tech News to your inbox\n\nThis workflow automates the collection, normalization, and publication of content from a broad set of RSS feeds. The execution begins with a Schedule Trigger, which activates the entire pipeline at recurring intervals. Each feed is handled by its own individual RSS node, allowing full separation of sources and easier monitoring or troubleshooting. The collected items flow into a structured series of Merge nodes that consolidate all outputs into one unified data stream.\n\nAfter aggregation, the workflow performs filtering and transformation. A dedicated filtering step removes invalid or duplicate entries, while a custom Function node standardizes fields such as title, URL, timestamp, and raw content. This ensures that data coming from heterogeneous sources is normalized before further processing. The workflow also includes an AI-powered enrichment stage—via an OpenAI node—which generates summaries, tags, categories, or any other metadata needed to enhance the collected articles.\n\nThe pipeline concludes by delivering the processed content to a chosen destination (email, database, Notion, API endpoint, or social platform). The modular design allows new sources to be added effortlessly without altering the core logic, making the workflow scalable and easy to maintain.\nThis setup is ideal for automated newsletters, content curation, topic monitoring, or internal knowledge management systems.\n\nRead: [Full setup Guide](https://paoloronco.it/n8n-template-rss-tech-news-to-your-inbox/)\n\n#### Youtube Video: \n[![RSS Tech News](https://img.youtube.com/vi/Gck8nmvx1UA/0.jpg)](https://www.youtube.com/watch?v=Gck8nmvx1UA "RSS Tech News")', { name: 'Sticky Note3', position: [-224, -528], width: 592, height: 992 }))
  .add(sticky('📌 Schedule Trigger\nStarts the entire workflow automatically at defined intervals—no manual execution required.', { name: 'Sticky Note4', color: 7, position: [-2064, 1104], width: 192, height: 288 }))
  .add(sticky('📌 RSS Nodes (Multiple Sources)\n\nEach node fetches the latest entries from one feed. Using isolated nodes keeps the flow stable and simplifies debugging.', { name: 'Sticky Note5', color: 7, position: [-1408, -512], width: 192, height: 192 }))
  .add(sticky('📌 Merge Nodes\n\nCombine all feed outputs into one coherent dataset, preparing the workflow for global filtering and processing.', { name: 'Sticky Note6', color: 7, position: [-96, 992], width: 192, height: 416 }))
  .add(sticky('📌 Filter\n\nFilters out everything older than 24 hours by comparing each article’s isoDate to “now minus 24 hours”. Only fresh news is allowed through to keep the briefing truly daily and relevant.', { name: 'Sticky Note7', color: 7, position: [112, 992], width: 208, height: 352 }))
  .add(sticky('📌 Gemini Node (AI Enrichment)\n\nGenerates summaries, keywords, topics, or enhanced textual representations of each article.', { name: 'Sticky Note8', color: 7, position: [800, 992], width: 256, height: 352 }))
  .add(sticky('📌 Final Output Node (Email / Notion / API / etc.)\n\nSends the finished newsletter via Gmail.\nUses the subject from the Code node and the generated html as the email body, so you receive a curated Tech & AI briefing directly in your inbox with no manual work required.', { name: 'Sticky Note9', color: 7, position: [1472, 992], width: 256, height: 368 }))
  .add(sticky('📌Sort – Articles by Date\n\nSorts all remaining articles by isoDate in descending order. This ensures that the most recent and time-sensitive stories are processed first and shown higher in the final briefing.', { name: 'Sticky Note10', color: 7, position: [336, 992], width: 208, height: 352 }))
  .add(sticky('📌Sort – Code in JavaScript\n\nTakes all incoming RSS items and packs them into a single item with an articles array.\nFor each article it keeps: title, content or contentSnippet, link, isoDate\n', { name: 'Sticky Note11', color: 7, position: [560, 992], width: 224, height: 352 }))
  .add(sticky('📌 Build Final Newsletter HTML (Code)\n\nThis Code node takes the JSON returned by the LLM and prepares the final email.\nIt removes any ```json wrappers, parses the JSON safely, and checks that subject and html exist.\nThen it inserts both fields into a clean, responsive HTML email template.\nThe output is a single item containing the final subject and html, ready for the Gmail node.', { name: 'Sticky Note12', color: 7, position: [1072, 992], width: 368, height: 368 }))