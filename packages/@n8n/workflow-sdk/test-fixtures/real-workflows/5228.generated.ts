const wf = workflow('dLrtZMRZzusZDlpa', 'GoogleVertex_template', { executionOrder: 'v1' })
	.add(
		trigger({
			...
			config: {
				parameters: {
					...
					folderId: {
						...
						cachedResultUrl:
							'https://drive.google.com/drive/folders/1tzpmwAWiUGolnKciZvcCghB5obhPoXzL',
	...
	.add(
		sticky(
			...
			{ name: 'Sticky Note1', color: 4, position: [120, -160], width: 580, height: 320 },
		),
	)
	.add(
		sticky(
			'### Convert to Video file\n\nBase64 Input Field:\n```response.videos[0].bytesBase64Encoded```',
			{ name: 'Sticky Note2', color: 5, position: [720, -160], width: 200, height: 320 },
		),
	)
	.add(
		sticky('### Upload Video to Google Drive', {
			name: 'Sticky Note3',
			color: 6,
			position: [940, -160],
			width: 180,
	...
	.add(
		sticky('### Accepts a text prompt and a GCP access token via form.\n\n\n', {
			name: 'Sticky Note4',
			color: 7,
			position: [-380, -160],
			width: 230,
	...
	.add(
		sticky(
			...
			{ name: 'Sticky Note5', color: 7, position: [-140, 180], width: 660, height: 400 },
		),
	)
	.add(
		sticky(
			'### Output\n![Alt text](https://drive.google.com/thumbnail?id=1Biq2vhbzaFLya1ZsF8PhGL1RRta7XkMK&sz=w1000)',
			{ name: 'Sticky Note6', color: 7, position: [540, 180], width: 580, height: 400 },
		),
	)
	.add(
		sticky(
			'### How to get GCP Access Token\n\nUse this command in your VM/Cloud Shell:\n\n```bash\ngcloud auth print-access-token\n',
