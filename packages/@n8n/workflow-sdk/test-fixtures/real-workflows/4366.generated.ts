return workflow('leftLsw8mj6dIDBp', '[AOE]  Inbox & Calendar Management Agent', { executionOrder: 'v1' })
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.chatTrigger', version: 1.1, config: { parameters: { options: {} }, position: [120, 240], name: 'When chat message received' } }))
  .then(node({ type: 'n8n-nodes-base.noOp', version: 1, config: { position: [600, 240], name: 'sessionId-master' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.7, config: { parameters: {
      ...
      promptType: 'define'
        ...
        }, name: 'Delete an email' } }), tool({ type: 'n8n-nodes-base.gmailTool', version: 2.1, config: { parameters: {
          ...
          gmailOAuth2: { id: 'credential-id', name: 'gmailOAuth2 Credential' }
        }, name: 'Get last emails' } }), tool({ type: 'n8n-nodes-base.googleCalendarTool', version: 1.3, config: { parameters: {
        ...
        }, name: 'Get calendar events' } }), tool({ type: 'n8n-nodes-base.googleCalendarTool', version: 1.3, config: { parameters: {
        ...
        }, name: 'Add an calender entry' } }), tool({ type: 'n8n-nodes-base.gmailTool', version: 2.1, config: { parameters: {
          ...
          gmailOAuth2: { id: 'credential-id', name: 'gmailOAuth2 Credential' }
        }, name: 'Create an New Email Draft' } }), tool({ type: 'n8n-nodes-base.gmailTool', version: 2.1, config: { parameters: {
          ...
          gmailOAuth2: { id: 'credential-id', name: 'gmailOAuth2 Credential' }
        }, name: 'Get an email by MessageID' } }), tool({ type: 'n8n-nodes-base.dateTimeTool', version: 2, config: { parameters: {
        ...
        }, name: 'Determine the name of the day of the week' } }), tool({ type: 'n8n-nodes-base.gmailTool', version: 2.1, config: { parameters: {
          message: '={{ $fromAI(\'Message\', `The Text that should be send in reply`, \'string\') }}',
          options: {
            sendTo: '={{ $fromAI(\'To_Email\', `The email adress of the sender`, \'string\') }}',
          ...
          gmailOAuth2: { id: 'credential-id', name: 'gmailOAuth2 Credential' }
        }, name: 'Create an Email Draft as response to a thread' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolVectorStore', version: 1.1, config: { parameters: {
        ...
        }, name: 'OpenAI Chat Model1' } }), vectorStore: vectorStore({ type: '@n8n/n8n-nodes-langchain.vectorStoreInMemory', version: 1.1, config: { subnodes: { embedding: embedding({ type: '@n8n/n8n-nodes-langchain.embeddingsOpenAi', version: 1.2, config: { parameters: { options: {} }, credentials: {
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'Embeddings OpenAI1' } }) }, name: 'Threads History Vector Store' } }) }, name: 'Research context and infos in previous conversations' } })], model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
          model: {
            __rl: true,
            mode: 'list',
            value: 'gpt-4o',
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Chat Model' } }), memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryBufferWindow', version: 1.3, config: { parameters: {
          sessionKey: '={{ $(\'sessionId-master\').item.json.sessionId }}',
          sessionIdType: 'customKey',
          contextWindowLength: 10
        }, name: 'Window Buffer Memory' } }) }, position: [1060, 240], name: 'EMail Agent' } }))
  .add(trigger({ type: 'n8n-nodes-base.executeWorkflowTrigger', version: 1.1, config: { parameters: {
    ...
    }, position: [120, 80], name: 'When Executed by Another Workflow' } }))
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [1840, -760], name: 'When clicking ‘Test workflow’' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: { filters: {}, resource: 'thread' }, credentials: {
      gmailOAuth2: { id: 'credential-id', name: 'gmailOAuth2 Credential' }
    }, position: [2140, -560], name: 'Gmail - get recent Threads' } }))
  ...
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [2580, -560], name: 'Code - Summarize Email Thread as Text' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.vectorStoreInMemory', version: 1.1, config: { parameters: { mode: 'insert', clearStore: true }, subnodes: { embedding: embedding({ type: '@n8n/n8n-nodes-langchain.embeddingsOpenAi', version: 1.2, config: { parameters: { options: {} }, credentials: {
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        ...
        }, subnodes: { textSplitter: textSplitter({ type: '@n8n/n8n-nodes-langchain.textSplitterTokenSplitter', version: 1, config: { parameters: { chunkSize: 2000 }, name: 'Token Splitter' } }) }, name: 'Default Data Loader' } }) }, position: [2900, -560], name: 'Write - Threads History Vector Store' } }))
  ...
  .add(node({ type: '@n8n/n8n-nodes-langchain.textClassifier', version: 1, config: { parameters: {
      ...
      categories: {
        categories: [
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Chat Model2' } }) }, position: [560, -680], name: 'Classify Emails' } }))
  .add(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      ...
      gmailOAuth2: { id: 'credential-id', name: 'gmailOAuth2 Credential' }
    }, position: [1200, -800], name: 'Gmail - Label as Colleges' } }))
  .add(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      ...
      gmailOAuth2: { id: 'credential-id', name: 'gmailOAuth2 Credential' }
    }, position: [1200, -640], name: 'Gmail label as kunde' } }))
  ...
  .add(sticky('## Email Thread Knowledge adder', { name: 'Sticky Note4', color: 4, position: [1740, -900], width: 1680, height: 780 }))
  .add(sticky('## Main Inbox Assistance Agent\n\n### Before Using\n** Modify the classifier agent for your needs. Add the Labels in GMail before, if you want to assign labels.\n** Add proper credentials\n** Modify the Prompts (e.g. give more context about your role and company)\n\n### More on professional Agents:\n** From [AOE AI Lab](https://ai-radar.aoe.com/)', { name: 'Sticky Note7', color: 3, position: [-460, 0], width: 480, height: 440 }))