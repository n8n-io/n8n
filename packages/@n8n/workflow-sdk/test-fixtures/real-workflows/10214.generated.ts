return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.whatsAppTrigger', version: 1, config: { parameters: { options: {}, updates: ['messages'] }, credentials: {
    ...
    }, position: [80, -16], name: 'WhatsApp Trigger' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
      jsCode: 'const msg = $json.messages?.[0]?.text;\n\nif (!msg) {\n  // Exit early if no text message\n  return [];\n}\n\nreturn items;\n'
    }, position: [304, -16], name: 'Check Message' } }))
  .then(node({ type: 'n8n-nodes-base.redis', version: 1, config: { parameters: {
      key: '=llm-user:{{ $json.contacts[0].wa_id }}',
      options: {},
      operation: 'get'
    }, credentials: { redis: { id: 'credential-id', name: 'redis Credential' } }, position: [528, -16], name: 'Check User Number' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [752, -16], name: 'Model Decider' } }))
  .then(node({ type: 'n8n-nodes-base.redis', version: 1, config: { parameters: {
      ...
      expire: true,
      operation: 'set'
    }, credentials: { redis: { id: 'credential-id', name: 'redis Credential' } }, position: [976, -16], name: 'Store User Number' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2, config: { parameters: {
      ...
      promptType: 'define'
        ...
        }, name: 'Pricing' } }), tool({ type: 'n8n-nodes-base.mySqlTool', version: 2.4, config: { parameters: {
          query: '{{ $(\'AI Agent\').item.json.query }}',
          options: {},
          operation: 'executeQuery'
        }, credentials: { mySql: { id: 'credential-id', name: 'mySql Credential' } }, name: 'Execute a SQL query in MySQL' } })], model: languageModel({ type: '@n8n/n8n-nodes-langchain.modelSelector', version: 1, config: { parameters: {
          rules: {
            rule: [
              {
                conditions: {
        ...
        }, name: 'Google Gemini Chat Model' } }), languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatGoogleGemini', version: 1, config: { parameters: { options: {} }, credentials: {
          googlePalmApi: { id: 'credential-id', name: 'googlePalmApi Credential' }
        }, name: 'Google Gemini Chat Model1' } })] }, name: 'Choose Model' } }), memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryBufferWindow', version: 1.3, config: { parameters: {
          sessionKey: '={{ $(\'WhatsApp Trigger\').item.json.contacts[0].wa_id }}',
          sessionIdType: 'customKey'
        }, name: 'Simple Memory' } }) }, position: [1456, -16], name: 'AI Agent' } }))
