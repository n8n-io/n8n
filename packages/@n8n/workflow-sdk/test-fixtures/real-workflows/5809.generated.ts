return workflow('vMvm0kfL6V7u4bPO', 'Client Responder IG/TG', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.webhook', version: 2, config: { parameters: {
      path: '/shoe-orders',
      options: {},
      httpMethod: 'POST',
      responseMode: 'responseNode'
    }, position: [-600, 0], name: 'Receive Request' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.7, config: { parameters: {
      ...
      hasOutputParser: true
        ...
        }, name: 'OpenAI Chat Model' } }) }, position: [-360, 0], name: 'Parse Request AI' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
      jsCode: 'const input = $input.first().json;\nconst parsedData = JSON.parse(input.text);\n\nreturn [{\n  json: {\n    brand: parsedData.brand || "",\n    model: parsedData.model || "",\n    size: parsedData.size || "",\n    color: parsedData.color || ""\n  }\n}];'
    }, position: [80, 0], name: 'Extract Data' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
    ...
    }, position: [300, 0], name: 'Product Database' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [540, 0], name: 'Filter Products' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.7, config: { parameters: {
      text: '={{ $(\'Receive Request\').first().json.body.message }}',
      batching: {},
      messages: {
      ...
      promptType: 'define'
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Chat Model1' } }) }, position: [740, 0], name: 'AI Manager' } }))
  .then(node({ type: 'n8n-nodes-base.respondToWebhook', version: 1.3, config: { parameters: {
      options: {},
      respondWith: 'text',
