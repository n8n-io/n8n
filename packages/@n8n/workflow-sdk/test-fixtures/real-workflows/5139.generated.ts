const wf = workflow('REDACTED_WORKFLOW_ID', 'insta Reel publish', {
	timezone: 'UTC',
	...
	.then(
		node({
			...
			version: 1,
			config: {
				parameters: { options: {}, fileName: '/tmp/video.mp4', operation: 'write' },
				position: [-352, -304],
	...
	.then(
		node({
			type: 'n8n-nodes-base.wait',
			version: 1.1,
			config: { parameters: { amount: 20 }, position: [128, -144], name: 'Wait1' },
		}),
	)
	.then(
		node({
			...
			config: {
				parameters: {
					fileId: {
						...
						mode: 'url',
						value:
							"={{ $('Post File Download in N8N (Google Drive Node)').item.json.webViewLink }}",
					},
					options: {},
						...
						parameters: [
							{
								name: 'q',
								value:
									"'REDACTED_DRIVE_FOLDER_ID' in parents and mimeType='video/mp4' and trashed=false",
							},
	...
	.then(
		node({
			type: 'n8n-nodes-base.wait',
			version: 1.1,
			config: { parameters: { amount: 20 }, position: [-848, -64], name: 'Wait2' },
		}),
	)
	.add(
		trigger({
	...
	.add(
		sticky('## wait 20 seconds then trigger the insta posting workflow', {
			color: 5,
			position: [-976, -384],
			width: 400,
			height: 720,
	...
	.add(
		sticky(
			...
			{ name: 'Sticky Note1', position: [-1408, -800], width: 420, height: 1140 },
		),
	)
	.add(
		sticky(
			'## manual upload trigger\n\n**it will trigger the workflow whenever the video file as upload manually on the folder',
			{ name: 'Sticky Note2', color: 3, position: [-976, -800], width: 400, height: 420 },
		),
	)
	.add(
		sticky('## insta uploader', {
			name: 'Sticky Note4',
			color: 4,
			position: [-560, -800],
			width: 1880,
	...
	.add(
		sticky('## Instagram automation workflow.....', {
			name: 'Sticky Note3',
			color: 6,
			position: [-1520, -880],
			width: 2920,
	...
	.add(
		sticky(
			'## **Webhook to serve the file** [it act as a public url for facebook Api](REDACTED_WEBHOOK_URL)',
			{ name: 'Sticky Note5', position: [560, -128], width: 608, height: 288 },
		),
	);
