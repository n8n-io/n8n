return workflow('', '')
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.chatTrigger', version: 1.1, config: { parameters: { options: {} }, position: [160, 0], name: 'When chat message received' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.1, config: { parameters: {
      options: {
    ...
    }, subnodes: { tools: [tool({ type: '@n8n/n8n-nodes-langchain.toolThink', version: 1, config: { parameters: {
          description: 'Use the tool to think about the user query and the actual data extracted.'
        }, name: 'Think' } }), tool({ type: 'n8n-nodes-base.postgresTool', version: 2.6, config: { parameters: {
          table: {
            __rl: true,
            mode: 'name',
        ...
        }, name: 'structured data' } }), tool({ type: '@n8n/n8n-nodes-langchain.vectorStoreSupabase', version: 1.3, config: { parameters: {
        ...
        }, subnodes: { embedding: embedding({ type: '@n8n/n8n-nodes-langchain.embeddingsOpenAi', version: 1.2, config: { parameters: { options: { dimensions: 1536 } }, credentials: {
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'Embeddings OpenAI' } }) }, name: 'General knowledge' } }), tool({ type: 'n8n-nodes-base.perplexityTool', version: 1, config: { parameters: {
          options: {},
          messages: {
            message: [
              {
        ...
        }, name: 'Message a model in Perplexity' } }), tool({ type: '@n8n/n8n-nodes-langchain.mcpClientTool', version: 1, config: { parameters: { sseEndpoint: 'https://your instancesse' }, name: 'search  about any doc in google drive' } })], model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatAnthropic', version: 1.3, config: { parameters: {
          model: {
            __rl: true,
            mode: 'list',
        ...
        }, name: 'Anthropic Chat Model' } }), memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryPostgresChat', version: 1.3, config: { credentials: {
          postgres: { id: 'credential-id', name: 'postgres Credential' }
        }, name: 'Postgres Chat Memory' } }) }, position: [464, 0], name: 'Knowledge Agent' } }))
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [144, -688], name: 'When clicking ‘Execute workflow’' } }))
  .then(node({ type: 'n8n-nodes-base.googleDrive', version: 3, config: { parameters: {
    ...
    }, position: [352, -688], name: 'Download file' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.vectorStoreSupabase', version: 1.3, config: { parameters: {
      ...
      supabaseApi: { id: 'credential-id', name: 'supabaseApi Credential' }
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'Embeddings OpenAI1' } }), documentLoader: documentLoader({ type: '@n8n/n8n-nodes-langchain.documentDefaultDataLoader', version: 1.1, config: { parameters: {
          options: {},
          dataType: 'binary',
          textSplittingMode: 'custom'
        }, subnodes: { textSplitter: textSplitter({ type: '@n8n/n8n-nodes-langchain.textSplitterRecursiveCharacterTextSplitter', version: 1, config: { parameters: { options: {} }, name: 'Recursive Character Text Splitter1' } }) }, name: 'Default Data Loader1' } }) }, position: [592, -688], name: 'Add to Supabase Vector DB' } }))
  .add(trigger({ type: 'n8n-nodes-base.executeWorkflowTrigger', version: 1.1, config: { parameters: {
    ...
    }, position: [2112, 448], name: 'When Executed by Another Workflow' } }))
  .then(switchCase([node({ type: 'n8n-nodes-base.googleDrive', version: 3, config: { parameters: {
      ...
      operation: 'download'
    }, position: [2464, 448], name: 'Download File1' } })], { version: 3.2, parameters: {
      rules: {
        values: [
          {
            outputKey: 'ReadFile',
  ...
  .then(switchCase([node({ type: 'n8n-nodes-base.extractFromFile', version: 1, config: { parameters: { options: {}, operation: 'pdf' }, position: [2928, 160], name: 'Extract from PDF' } }), node({ type: 'n8n-nodes-base.extractFromFile', version: 1, config: { parameters: {
      options: {
        encoding: 'utf-8',
        headerRow: false,
        relaxQuotes: true,
    ...
    }, position: [2928, 352], name: 'Extract from CSV' } }), node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
      modelId: {
        __rl: true,
        mode: 'list',
        value: 'gpt-4o-mini',
      ...
      inputType: 'base64',
      operation: 'analyze'
    }, position: [2928, 528], name: 'Analyse Image' } }), node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: { options: {}, resource: 'audio', operation: 'transcribe' }, position: [2928, 704], name: 'Transcribe Audio' } })], { version: 3.2, parameters: {
      rules: {
        values: [
          {
            outputKey: 'pdf',
  ...
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [3088, 160], name: 'Get PDF Response' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [3088, 352], name: 'Get CSV Response' } }))
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.mcpTrigger', version: 1, config: { parameters: { path: 'a289c719-fb71-4b08-97c6-79d12645dc7e' }, subnodes: { tools: [tool({ type: '@n8n/n8n-nodes-langchain.toolWorkflow', version: 2.1, config: { parameters: {
          name: 'ReadFile',
        ...
        }, name: 'Read File From GDrive' } }), tool({ type: 'n8n-nodes-base.googleDriveTool', version: 3, config: { parameters: {
          limit: 10,
          filter: {
        ...
        }, name: 'Search Files from Gdrive' } })] }, position: [1712, 64], name: 'Google Drive MCP Server' } }))
