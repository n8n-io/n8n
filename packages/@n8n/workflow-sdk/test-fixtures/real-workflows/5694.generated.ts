return workflow('TEMPLATE_WORKFLOW_ID', 'Newsletter', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-40, -180], name: 'When clicking \'Test workflow\'' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
    ...
    }, position: [180, -180], name: 'Get topic from newsletter sheet' } }))
  .then(ifBranch([node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
    ...
    }, position: [620, -180], name: 'Create HTML for Newsletter' } }), null], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [996, -180], name: 'Prepare Data to create word doc' } }))
  .then(node({ type: 'n8n-nodes-base.googleDrive', version: 3, config: { parameters: {
    ...
    }, position: [1216, -180], name: 'Upload doc to google drive' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
    ...
    }, position: [1436, -180], name: 'Send an email to admin' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
      columns: {
        value: {
    ...
    }, position: [1656, -180], name: 'Update Status as Generated' } }))
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{}] } }, position: [-40, 230] } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
      options: {},
      sheetName: {
        __rl: true,
    ...
    }, position: [180, 230], name: 'Pick records to send email to client' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
    ...
    }, position: [620, 230], name: 'Get Client email address' } }), null], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: {} }, position: [840, 230], name: 'Loop Over Items' } }))
  .add(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
    ...
    }, position: [1060, 80], name: 'Update status as Sent' } }))
  .add(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      ...
      gmailOAuth2: { id: 'credential-id', name: 'gmailOAuth2 Credential' }
    }, position: [1060, 280], name: 'Send email to client' } }))