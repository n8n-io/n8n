return workflow('L87pnGIo6Q1Bmxwe', 'Automated Client Onboarding and Task Assignment System', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.webhook', version: 2.1, config: { parameters: {
      path: 'client-intake',
      options: {},
      httpMethod: 'POST',
      responseMode: 'lastNode'
    }, position: [128, 128], name: 'New Client Intake Form' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [352, 128], name: 'Workflow Configuration' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
      ...
      includeOtherFields: true
    }, position: [576, 128], name: 'Normalize Client Data' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 2.1, config: { parameters: {
    ...
    }, position: [800, 128], name: 'Generate Onboarding Checklist' } }))
  .then(switchCase([node({ type: 'n8n-nodes-base.googleSheets', version: 4.7, config: { parameters: {
      columns: {
        value: null,
        schema: [],
    ...
    }, position: [1376, -64], name: 'Log to Google Sheets - Strategy' } }), node({ type: 'n8n-nodes-base.googleSheets', version: 4.7, config: { parameters: {
      columns: {
        value: null,
        schema: [],
        mappingMode: 'autoMapInputData',
      ...
      googleSheetsOAuth2Api: { id: '5b6oIAuqcO9HRb2A', name: 'Google Sheets account 3' }
    }, position: [1376, 128], name: 'Log to Google Sheets - Management' } }), node({ type: 'n8n-nodes-base.googleSheets', version: 4.7, config: { parameters: {
      columns: {
        value: null,
        schema: [],
        mappingMode: 'autoMapInputData',
      ...
      googleSheetsOAuth2Api: { id: '5b6oIAuqcO9HRb2A', name: 'Google Sheets account 3' }
    }, position: [1376, 320], name: 'Log to Google Sheets - IT' } })], { version: 3.4, parameters: {
      rules: {
        values: [
          {
            outputKey: 'Strategy',
  ...
  .then(node({ type: 'n8n-nodes-base.slack', version: 2.4, config: { parameters: {
      ...
      otherOptions: {}
    }, position: [1600, -64], name: 'Notify Strategy Consultant' } }))
  .then(node({ type: 'n8n-nodes-base.emailSend', version: 2.1, config: { parameters: {
    ...
    }, position: [1824, 128], name: 'Send Welcome Email' } }))
  .then(node({ type: 'n8n-nodes-base.googleCalendar', version: 1.3, config: { parameters: {
    ...
    }, position: [2048, 128], name: 'Schedule Kickoff Meeting' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.3, config: { parameters: {
    ...
    }, position: [2272, 128], name: 'Sync to CRM' } }))
  .then(node({ type: 'n8n-nodes-base.slack', version: 2.4, config: { parameters: {
      ...
      otherOptions: {}
    }, position: [1600, 128], name: 'Notify Management Consultant' } }))
  .then(node({ type: 'n8n-nodes-base.slack', version: 2.4, config: { parameters: {
