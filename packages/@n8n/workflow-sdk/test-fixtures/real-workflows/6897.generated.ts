return workflow('[WORKFLOW_ID_REMOVED]', 'HR CVs Filter', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.telegramTrigger', version: 1.2, config: { parameters: { updates: ['message'], additionalFields: {} }, credentials: {
    ...
    }, position: [-1232, 352], name: 'Message Trigger' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      options: {}
    }, position: [-624, 256], name: 'Download CV File' } }), node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
      text: 'Please send your CV in PDF format only',
      chatId: '={{ $json.message.chat.id }}',
      additionalFields: {}
    }, credentials: {
      telegramApi: { id: 'credential-id', name: 'telegramApi Credential' }
    }, position: [-928, 688], name: 'PDF Request' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      options: {}
    }, position: [-384, 256], name: 'Download Actual File' } }))
  ...
  .then(merge([node({ type: 'n8n-nodes-base.googleDrive', version: 3, config: { parameters: {
    ...
    }, name: 'Store CV' } }), node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: '=https://api.telegram.org/file/bot[YOUR_BOT_TOKEN]/{{ $json.result.file_path }}',
      options: {}
    }, position: [-384, 256], name: 'Download Actual File' } })], { version: 3.2, parameters: { mode: 'chooseBranch', useDataOfInput: 2 } }))
  .then(node({ type: 'n8n-nodes-base.extractFromFile', version: 1, config: { parameters: { options: {}, operation: 'pdf' }, position: [0, 256], name: 'Extract cv content' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.1, config: { parameters: {
      ...
      promptType: 'define'
        ...
        }, name: 'Google Gemini Chat Model' } }) }, position: [192, 256], name: 'Qualify CV Agent' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [608, 256], name: 'Clean & Map Extracted Data' } }))
  ...
  .add(sticky(' Sends a message asking the user to resend the CV in PDF format if validation fails.', { name: 'Sticky Note4', color: 5, position: [-960, 576], width: 176, height: 256 }))
  .add(sticky('Uploads the downloaded PDF file to Google Drive.', { name: 'Sticky Note5', color: 5, position: [-48, -64], width: 192, height: 208 }))
  .add(sticky('Combines the file storage and download paths to pass data forward to extraction.', { name: 'Sticky Note6', color: 5, position: [-48, 448], width: 192, height: 240 }))
  .add(sticky('Extracts text content from the uploaded PDF file for analysis.', { name: 'Sticky Note7', color: 4, position: [-48, 192], width: 176, height: 224 }))
  .add(sticky('Analyzes CV text and extracts structured data like name, phone number, email, experience, and job title.', { name: 'Sticky Note8', color: 6, position: [208, 160], height: 240 }))
  .add(sticky(' Cleans and maps the structured JSON AI output into individual fields for use in Google Sheets', { name: 'Sticky Note9', color: 5, position: [544, 160], width: 192, height: 256 }))
  .add(sticky('Saves the extracted candidate information and CV link into a Google Sheet, updating existing entries if needed.', { name: 'Sticky Note10', color: 4, position: [768, 144], width: 224, height: 272 }))