return workflow('OQuVjQzqswmDM86W', 'WhatsApp Chatbot – Restaurant Info Assistant (Bookings, Menu, Timing)', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.whatsAppTrigger', version: 1, config: { parameters: { options: {}, updates: ['messages'] }, credentials: {
    ...
    }, position: [-400, -240], name: 'Receive WhatsApp Message' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.9, config: { parameters: {
      ...
      promptType: 'define'
    }, subnodes: { model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOllama', version: 1, config: { parameters: { model: 'llama3.2-16000:latest', options: {} }, credentials: {
          ollamaApi: { id: 'credential-id', name: 'ollamaApi Credential' }
        }, name: 'Generate Reply with AI' } }) }, position: [-180, -240], name: 'Extract Customer Query' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { position: [196, -240], name: 'Wait For Response' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.postgres', version: 2.6, config: { parameters: {
    ...
    }, position: [636, -340], name: 'Create New Table Booking' } }), node({ type: 'n8n-nodes-base.whatsApp', version: 1, config: { parameters: {
    ...
    }, position: [636, -140], name: 'Send Reply to Customer' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
