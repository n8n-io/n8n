return workflow('', 'LinkedIn AI Content Automation - Agentic Vibe', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{ field: 'hours', hoursInterval: 6 }] } }, position: [-840, 640] } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2, config: { parameters: {
      ...
      hasOutputParser: true
        ...
        }, name: 'Structured Output Parser' } }) }, position: [-500, 640], name: 'Content topic generator2' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.7, config: { parameters: {
      ...
      hasOutputParser: true
        ...
        }, name: 'Structured Output Parser1' } }) }, position: [-80, 640], name: 'Content creator' } }))
  .then(merge([node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
      ...
      openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
    }, position: [420, 460], name: 'OpenAI' } }), node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2, config: { parameters: {
      ...
      hasOutputParser: true
        ...
        }, name: 'Structured Output Parser2' } }) }, position: [420, 900], name: 'Hashtag generator /SEO' } })], { version: 3.2 }))
  .then(node({ type: 'n8n-nodes-base.linkedIn', version: 1, config: { parameters: {
    ...
    }, position: [1060, 620], name: 'LinkedIn' } }))