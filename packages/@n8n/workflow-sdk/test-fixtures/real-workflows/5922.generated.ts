return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.formTrigger', version: 2.2, config: { parameters: {
    ...
    }, position: [-180, -400], name: 'Trigger: Food Emoji Form Submission' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [60, -400], name: 'Prepare Style‑JSON Prompt' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
    ...
    }, position: [260, -400], name: 'LLM: Generate Style‑JSON' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
      model: 'gpt-image-1',
      ...
      openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
    }, position: [600, -400], name: 'Image‑Gen: Render Food Emoji Icon' } }))
