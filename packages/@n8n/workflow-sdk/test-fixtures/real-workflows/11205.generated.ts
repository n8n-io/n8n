const wf = workflow('', '')
	.add(
		trigger({
			...
			version: 1,
			config: {
				parameters: { path: 'banner-v7158-2025', options: {}, httpMethod: 'POST' },
				position: [-992, 608],
				name: 'LINE Webhook1',
			},
		}),
	...
	.then(
		node({
			...
			version: 1.1,
			config: { parameters: { amount: 10 }, position: [112, 608], name: 'Wait1' },
		}),
	)
	.then(
		node({
	...
	.then(
		node({
			...
			config: {
				...
				credentials: { aws: { id: 'credential-id', name: 'aws Credential' } },
				position: [1120, 608],
				name: 'Upload to S',
			},
		}),
	...
	.add(
		sticky(
			...
			{ name: 'Main Sticky Note', position: [-1504, 480], width: 350, height: 596 },
		),
	)
	.add(
		sticky(
