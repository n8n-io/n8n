return workflow('WczQxQgtvjjk1HC1', 'LinkedIn Profile Enrichment With Error Handling', { executionOrder: 'v1', saveManualExecutions: true })
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [160, 80] } }))
  .then(node({ type: 'n8n-nodes-base.nocoDb', version: 2, config: { parameters: {
      limit: 15,
    ...
    }, position: [380, 180], name: 'Get Guests with LinkedIn' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [680, 180], name: 'Run Apify LinkedIn Scraper' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpQueryAuth: { id: 'credential-id', name: 'httpQueryAuth Credential' }
    }, position: [900, 180], name: 'Wait for Completion' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [1340, 80], name: 'Get Scraper Results' } }), node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
      mode: 'runOnceForEachItem',
    ...
    }, position: [1640, 620], name: 'Handle Scraper Error' } })], { version: 2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 1,
  ...
  .add(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [1640, -100], name: 'Transform Data' } }))
  .then(node({ type: 'n8n-nodes-base.nocoDb', version: 2, config: { parameters: {
      id: '={{$json.Id}}',
      table: 'NOCODB_TABLEID',
      operation: 'update',
      ...
      nocoDbApiToken: { id: 'credential-id', name: 'nocoDbApiToken Credential' }
    }, position: [1860, -100], name: 'Update Guest Success' } }))
  .add(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
      mode: 'runOnceForEachItem',
    ...
    }, position: [1660, 260], name: 'Clear Broken LinkedIn URL' } }))
  .then(node({ type: 'n8n-nodes-base.nocoDb', version: 2, config: { parameters: {
      id: '={{$json.Id}}',
      table: 'NOCODB_TABLEID',
      operation: 'update',
      ...
      nocoDbApiToken: { id: 'credential-id', name: 'nocoDbApiToken Credential' }
    }, position: [1880, 260], name: 'Update Guest - Clear URL' } }))
  .then(node({ type: 'n8n-nodes-base.nocoDb', version: 2, config: { parameters: {
      id: '={{$json.Id}}',
      table: 'NOCODB_TABLEID',
      operation: 'update',
