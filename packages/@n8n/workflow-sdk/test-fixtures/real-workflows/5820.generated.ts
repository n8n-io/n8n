return workflow('', 'AI Gmail: Prioritize What You Should Read')
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{ field: 'hours' }] } }, position: [-1264, 0] } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      ...
      operation: 'getAll',
      returnAll: true
    }, position: [-1040, 0], name: 'Get All Unread Messages' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-816, 0], name: 'Edit Fields' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2, config: { parameters: {
      ...
      promptType: 'define'
          ...
          options: {}
        }, name: 'OpenAI Chat Model' } }), languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
          model: {
            __rl: true,
            mode: 'list',
            value: 'gpt-3.5-turbo',
          ...
          options: {}
        }, name: 'OpenAI Fall Back Model' } })], outputParser: outputParser({ type: '@n8n/n8n-nodes-langchain.outputParserStructured', version: 1.3, config: { parameters: {
          schemaType: 'manual',
          inputSchema: '{\n  "type": "object",\n  "properties": {\n    "label": { "type": "string" },\n    "summary": { "type": "string" },\n    "id": { "type": "string" },\n    "threadId": { "type": "string" }\n  },\n  "required": ["label", "id"]\n}\n'
        }, name: 'Structured Output Parser' } }) }, position: [-592, 0], name: 'AI Agent' } }))
  .then(switchCase([node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      labelIds: ['Label_4190586288433010009', 'IMPORTANT'],
      messageId: '={{ $json.output.id }}',
      operation: 'addLabels'
    }, position: [96, -288], name: 'Add label "Action" to email' } }), node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      labelIds: ['Label_4104611383523107189'],
      messageId: '={{ $json.output.id }}',
      operation: 'addLabels'
    }, position: [96, -96], name: 'Add label "Informational" to email' } }), node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      labelIds: ['Label_3361902760602362460'],
      messageId: '={{ $json.output.id }}',
      operation: 'addLabels'
    }, position: [96, 96], name: 'Add label "Receipt" to email' } }), node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      resource: 'thread',
      threadId: '={{ $json.output.threadId }}',
      operation: 'delete'
    }, position: [96, 288], name: 'Delete email' } })], { version: 3.2, parameters: {
      rules: {
        values: [
          {
            outputKey: 'Action',
