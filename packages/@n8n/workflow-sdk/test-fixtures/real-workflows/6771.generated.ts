return workflow('InxpOwOAX7qfXIvX', 'WhatsApp RAG Chatbot with Supabase, Gemini 2.5 Flash, and OpenAI Embeddings', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.whatsAppTrigger', version: 1, config: { parameters: { options: {}, updates: ['messages'] }, credentials: {
    ...
    }, position: [-304, 560], name: 'New WhatsApp Message' } }))
  .then(switchCase([node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.1, config: { parameters: {
      ...
      promptType: 'define'
        ...
        }, name: 'Google Gemini LLM' } }), tools: [tool({ type: '@n8n/n8n-nodes-langchain.vectorStoreSupabase', version: 1.3, config: { parameters: {
        ...
        }, name: 'Generate OpenAI Embeddings' } }) }, name: 'Retrieve Context from Supabase' } })] }, position: [368, 272], name: 'RAG Query Agent' } }), node({ type: 'n8n-nodes-base.whatsApp', version: 1, config: { parameters: {
      resource: 'media',
      operation: 'mediaUrlGet',
      mediaGetId: '={{ $json.messages[0].document.id }}'
    }, credentials: {
      whatsAppApi: { id: 'LV22R0NzX9vZLEp9', name: 'whatsapp_reply' }
    }, position: [288, 928], name: 'Get Document URL' } })], { version: 3.2, parameters: {
      rules: {
        values: [
          {
            outputKey: 'query',
  ...
  .then(node({ type: 'n8n-nodes-base.whatsApp', version: 1, config: { parameters: {
      ...
      whatsAppApi: { id: 'LV22R0NzX9vZLEp9', name: 'whatsapp_reply' }
    }, position: [912, 272], name: 'Send WhatsApp Reply' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [560, 928], name: 'Download WhatsApp Document' } }))
