return workflow('lZbh3p9grft1d8dr', 'Learn Customer Onboarding Automation with n8n', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.webhook', version: 1, config: { parameters: {
      path: 'customer-onboarding-start',
      options: {},
      httpMethod: 'POST'
    }, position: [-700, 380], name: 'New Customer Webhook' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.hubspot', version: 1, config: { parameters: {
      resource: 'contact',
      operation: 'create',
      authentication: 'appToken'
    }, credentials: {
      hubspotAppToken: { id: 'credential-id', name: 'hubspotAppToken Credential' }
    }, position: [-220, 340], name: 'Create HubSpot Contact' } }), node({ type: 'n8n-nodes-base.telegram', version: 1, config: { parameters: { operation: 'send' }, position: [-380, 700], name: 'Send Validation Error Alert' } })], { version: 2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 1,
