return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.webhook', version: 2, config: { parameters: { path: 'ETF', options: {} }, position: [464, 464], name: 'When called by Excel Macro' } }))
  .then(node({ type: 'n8n-nodes-base.microsoftExcel', version: 2.1, config: { parameters: {
      table: {
        __rl: true,
        mode: 'list',
    ...
    }, position: [656, 368], name: 'Logs the date & time' } }))
  .then(node({ type: 'n8n-nodes-base.microsoftExcel', version: 2.1, config: { parameters: {
      table: {
        __rl: true,
        mode: 'list',
    ...
    }, position: [864, 368], name: 'Gets rows from table' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: '=https://www.justetf.com/fr/etf-profile.html?isin={{ $json.ISIN }}',
      options: {}
    }, position: [1040, 368], name: 'Forge a Get request with ISIN Values' } }))
  .then(node({ type: 'n8n-nodes-base.html', version: 1.2, config: { parameters: {
    ...
    }, position: [1456, 368], name: 'Extracts defined values with css selector' } }))
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: {} }, position: [1616, 368], name: 'Loop Over Items' } }))
  .add(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [1824, 240], name: 'Edit Fields' } }))
  .then(node({ type: 'n8n-nodes-base.microsoftExcel', version: 2.1, config: { parameters: {
      options: {},
      fieldsUi: {
        values: [
    ...
    }, position: [2016, 240], name: 'Updates my table' } }))
