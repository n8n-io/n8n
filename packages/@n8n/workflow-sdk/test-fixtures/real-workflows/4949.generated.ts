return workflow('IyhsZQh7TFAGsDqL', 'Automate WhatsApp bookings with an AI assistant and smart SMS reminders (24/7)', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.webhook', version: 2, config: { parameters: {
      path: '6438cd95-74cb-4f40-a1a5-853706fe96f6',
      options: {},
      httpMethod: 'POST',
      responseMode: 'responseNode'
    }, position: [-960, -80], name: 'Webhook Trigger (WhatsApp Input)' } }))
  .then(switchCase([node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.8, config: { parameters: {
      ...
      promptType: 'define'
        ...
        }, name: 'AI Conversation Memory' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
          model: {
            __rl: true,
            mode: 'list',
            value: 'gpt-4o',
        ...
        }, name: 'LLM: GPT-4o Chat Model' } }), tools: [tool({ type: '@n8n/n8n-nodes-langchain.toolHttpRequest', version: 1.1, config: { parameters: {
        ...
        }, name: 'Fetch Available Time Slots' } }), tool({ type: 'n8n-nodes-base.googleSheetsTool', version: 4.5, config: { parameters: {
          columns: {
            value: {
        ...
        }, name: 'Create New Prospect in Google Sheet' } }), tool({ type: 'n8n-nodes-base.googleSheetsTool', version: 4.5, config: { parameters: {
          columns: {
            value: {
              'ID du contact': '={{ $json.body.contactId }}',
        ...
        }, name: 'Update Prospect with Booking Details' } })] }, position: [40, -460], name: 'AI Booking Assistant (Dr Firas)' } }), node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
      options: {},
      filtersUI: {
        values: [
          {
    ...
    }, position: [-420, 140], name: 'Retrieve Prospect Details' } })], { version: 3.2, parameters: {
      rules: {
        values: [
          {
  ...
  .then(node({ type: 'n8n-nodes-base.respondToWebhook', version: 1.1, config: { parameters: { options: {}, respondWith: 'allIncomingItems' }, position: [700, -460], name: 'Send Response to WhatsApp' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-200, 140], name: 'Normalize Booking Time (UTC Format)' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [20, 140], name: 'Send Booking' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [240, 140], name: 'Format Date for Readability (Display)' } }))
  .then(switchCase([node({ type: 'n8n-nodes-base.respondToWebhook', version: 1.1, config: { parameters: {
      options: {},
      respondWith: 'json',
    ...
    }, position: [700, -20], name: 'Webhook Response: Booking Confirmed' } }), node({ type: 'n8n-nodes-base.respondToWebhook', version: 1.1, config: { parameters: {
      options: {},
      respondWith: 'json',
      responseBody: '[\n{\n"output": "Il semble que le créneau horaire que vous avez choisi ne soit plus disponible. Veuillez en sélectionner un autre."\n}\n]'
    }, position: [700, 280], name: 'Webhook Response: Booking Failed' } })], { version: 3.2, parameters: {
      rules: {
        values: [
          {
            outputKey: 'succeeded',
  ...
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
    ...
    }, position: [920, 140], name: 'Mark Booking as Confirmed in Sheet' } }))
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{ field: 'hours' }] } }, position: [-420, 580], name: 'Trigger: Check Appointments Every Hour' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
      options: {},
      sheetName: {
        __rl: true,
    ...
    }, position: [-200, 580], name: 'Read Upcoming Appointments from Sheet' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [20, 580], name: 'Filter Appointments Within Next 2 Hours' } }))
  .then(node({ type: 'n8n-nodes-base.sms77', version: 1, config: { parameters: {
    ...
    }, position: [240, 580], name: 'Send SMS Reminder' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
      columns: {
        value: {
