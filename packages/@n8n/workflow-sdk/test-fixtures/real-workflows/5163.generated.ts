return workflow('Tq4g5UK8c3GZ8tm7', 'Ade_Technical_Analyst', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.telegramTrigger', version: 1, config: { parameters: { updates: ['*', 'message'], additionalFields: {} }, position: [-740, 180], name: 'Telegram Trigger1' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 2, config: { parameters: {
    ...
    }, position: [-580, 180], name: 'PreProcessing' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 2, config: { parameters: {
      values: {
        number: [
      ...
      options: {}
    }, position: [-420, 180], name: 'Settings' } }))
  .add(node({ type: 'n8n-nodes-base.telegram', version: 1, config: { parameters: {
      action: '={{ $json.bot_typing }}',
      chatId: '={{ $json.message.from.id }}',
      operation: 'sendChatAction'
    }, position: [-240, 320], name: 'Send Typing action' } }))
  .then(merge([node({ type: 'n8n-nodes-base.set', version: 2, config: { parameters: {
      values: {
        number: [
          { name: 'model_temperature', value: 0.8 },
          { name: 'token_length', value: 500 }
      ...
      options: {}
    }, position: [-420, 180], name: 'Settings' } }), node({ type: 'n8n-nodes-base.telegram', version: 1, config: { parameters: {
      action: '={{ $json.bot_typing }}',
      chatId: '={{ $json.message.from.id }}',
      operation: 'sendChatAction'
    }, position: [-240, 320], name: 'Send Typing action' } })], { version: 2.1, parameters: { mode: 'chooseBranch' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.7, config: { parameters: {
      ...
      promptType: 'define'
        ...
        }, name: 'Get Chart' } })], memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryBufferWindow', version: 1.3, config: { parameters: {
          sessionKey: '={{ $json?.message?.text || "" }}',
          sessionIdType: 'customKey'
        }, name: 'Simple Memory' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenRouter', version: 1, config: { parameters: { model: 'anthropic/claude-3.5-sonnet', options: {} }, name: 'OpenRouter Chat Model' } }) }, position: [180, 120], name: 'AI Agent' } }))
  .then(node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
    ...
    }, position: [500, 240], name: 'Send Analysis' } }))
  .add(trigger({ type: 'n8n-nodes-base.executeWorkflowTrigger', version: 1.1, config: { parameters: { inputSource: 'passthrough' }, position: [-620, 900], name: 'Workflow Input Trigger' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-400, 900], name: 'Set Stock Ticker' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [-180, 900], name: 'Get Chart1' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: { url: '={{ $json.url }}', options: {} }, position: [40, 900], name: 'Download Chart' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
      ...
      inputType: 'base64',
      operation: 'analyze'
    }, position: [240, 900], name: 'Technical Analysis' } }))
  .then(node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
      file: '={{ $(\'Get Chart1\').item.json.url }}',
      chatId: '123456789',
      operation: 'sendPhoto',
      additionalFields: {}
    }, position: [440, 900], name: 'Send Chart' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
