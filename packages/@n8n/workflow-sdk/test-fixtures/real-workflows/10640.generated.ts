return workflow('UkKQ9WAMrgGbWCJw', '💥 Automate Scrape Google Maps Business Leads (Email, Phone, Website) using Apify -vide II')
  .add(trigger({ type: 'n8n-nodes-base.telegramTrigger', version: 1.2, config: { parameters: { updates: ['message'], additionalFields: {} }, position: [-2144, 4032] } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-1920, 4032], name: 'Extract Input Data' } }))
  .then(node({ type: '@apify/n8n-nodes-apify.apify', version: 1, config: { parameters: {
    ...
    }, position: [-1680, 4032], name: 'Run Google Maps Scraper' } }))
  .then(node({ type: '@apify/n8n-nodes-apify.apify', version: 1, config: { parameters: {
      limit: {},
      offset: {},
      resource: 'Datasets',
      datasetId: '={{ $json.defaultDatasetId }}'
    }, position: [-1440, 4032], name: 'Get dataset items' } }))
  .then(node({ type: 'n8n-nodes-base.removeDuplicates', version: 2, config: { parameters: {
      compare: 'selectedFields',
      options: {},
      fieldsToCompare: 'title'
    }, position: [-1200, 4032] } }))
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: {} }, position: [-800, 4032], name: 'Loop Over Items' } }))
  .add(node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
      text: 'DONE',
      chatId: '={{ $(\'Telegram Trigger\').first().json.message.chat.id }}',
      additionalFields: {}
    }, position: [-784, 4272], name: 'Notification message' } }))
  .add(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
      ...
      jsonOutput: true
    }, position: [-544, 4048], name: 'AI Company Description Generator' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.7, config: { parameters: {
    ...
    }, position: [-160, 4048], name: 'Google maps database' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [128, 4048], name: 'Extract Only Website URLs' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: { url: '={{ $json[\'Site internet\'] }}', options: {} }, position: [384, 4048], name: 'Fetch Raw HTML Content from Business Website' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
    ...
    }, position: [608, 4048], name: 'Extract Business Email from Website HTML (GPT-4)' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.7, config: { parameters: {
      columns: {
        value: {
