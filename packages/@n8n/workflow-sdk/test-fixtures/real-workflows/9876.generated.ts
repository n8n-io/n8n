return workflow('sKPt2XLNb9anT2fW', 'Multi-Channel DHL Status Bot with n8n, Gmail, and Webhooks', {
  ...
  .add(trigger({ type: 'n8n-nodes-base.webhook', version: 2, config: { parameters: {
    ...
    }, position: [304, 112], name: 'Webhook Form Trigger' } }))
  .then(merge([trigger({ type: 'n8n-nodes-base.webhook', version: 2, config: { parameters: {
      path: 'dhl-tracking-inquiry',
      options: {},
      httpMethod: 'POST',
      responseMode: 'responseNode'
    }, position: [304, 112], name: 'Webhook Form Trigger' } }), trigger({ type: 'n8n-nodes-base.gmailTrigger', version: 1.1, config: { parameters: {
      simple: false,
    ...
    }, position: [304, 304], name: 'Gmail Email Trigger' } })], { version: 3, parameters: { mode: 'combine', options: {} }, name: 'Merge Triggers' }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [800, 208], name: 'Extract Tracking Number' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [1008, 208], name: 'Get DHL Tracking Status' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [1200, 208], name: 'Format Response Message' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.respondToWebhook', version: 1.1, config: { parameters: {
      options: {
        responseCode: 200,
        responseHeaders: {
    ...
    }, position: [1600, 112], name: 'Webhook Response' } }), node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      ...
      gmailOAuth2: { id: 'credential-id', name: 'gmailOAuth2 Credential' }
    }, position: [1600, 304], name: 'Send Gmail Response' } })], { version: 2, parameters: {
      options: {},
      conditions: {
        options: {
          leftValue: '',
  ...
  .add(trigger({ type: 'n8n-nodes-base.gmailTrigger', version: 1.1, config: { parameters: {
      simple: false,
      filters: { sender: [], readStatus: 'unread' },
      options: {},
      pollTimes: { item: [{ mode: 'everyMinute' }] }
      ...
      gmailOAuth2: { id: 'credential-id', name: 'gmailOAuth2 Credential' }
    }, position: [304, 304], name: 'Gmail Email Trigger' } }))
  .add(sticky('## Multi-Channel DHL Status Bot\n\nThis workflow automatically answers customer inquiries about DHL shipment status from two different channels: web forms and email.\n\n**Channels Handled:**\n- **Webhooks:** For \'Track My Order\' forms on your website.\n- **Gmail:** For direct email inquiries.\n\n**Process Flow:**\n1. Receives a tracking number from either source.\n2. Fetches the latest status from the DHL API.\n3. Sends a formatted response back to the original channel.', { name: 'Workflow Overview', position: [-112, 80], width: 320, height: 410 }))
  .add(sticky('## ⚙️ Trigger Configuration\n\n**1. Gmail Trigger:**\n- Select the \'Gmail Email Trigger\' node.\n- Connect your Gmail OAuth2 credentials.\n- **Action:** This allows the workflow to monitor an inbox for new tracking inquiries.\n\n**2. Webhook Trigger:**\n- Copy the Production URL from the \'Webhook Form Trigger\' node.\n- **Action:** Paste this URL into your website\'s form settings (e.g., WordPress, Webflow, custom HTML form).', { name: 'Trigger Setup', color: 3, position: [304, 480], width: 280, height: 308 }))
  .add(sticky('## Data Normalization\n\nThis Code node unifies data from both triggers.\n\n**It extracts:**\n- Tracking Number (from form body or email text)\n- Customer Email\n- Customer Name\n\nThis ensures the rest of the workflow can process the data in a consistent format, regardless of the source.', { name: 'Extraction Logic', color: 5, position: [688, 384], width: 250, height: 236 }))
  .add(sticky('## ⚙️ DHL API Configuration\n\n**Action Required:** You must add your DHL API key to proceed.\n\n1. **Get API Key:** Register and get a key from the [DHL Developer Portal](https://developer.dhl.com/).\n2. **Add Key to Node:**\n   - Select the \'Get DHL Tracking Status\' node.\n   - Go to `Headers` > `Header Parameters`.\n   - Replace `YOUR_DHL_API_KEY` with your actual key.', { name: 'DHL API Configuration', color: 4, position: [1008, 384], width: 280, height: 240 }))
  .add(sticky('## Response Routing\n\nThis IF node intelligently routes the response based on the original inquiry source.\n\n- **If `isWebhook` is true:** The workflow sends a JSON response back to the web form.\n- **If `isWebhook` is false:** The workflow sends a formatted reply via Gmail.', { name: 'Response Routing', color: 6, position: [1344, 352], width: 220, height: 232 }))
  .add(sticky('## ✉️ Email Customization\n\nCustomize the automated email reply.\n\n**Suggestions:**\n- **Set `Reply-To` in \'Send Gmail Response\' node:** Add a support address like `support@yourcompany.com`.\n- **Refine Message:** Edit the email body in the "Format Response Message" node to add your company signature or branding.', { name: 'Email Setup', color: 2, position: [1600, 464], height: 252 }))