return workflow('zjl4xwZIDr6Ks1Zv', 'Turn YouTube Transcripts into Newsletter Drafts using Dumpling AI + GPT-4o', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-80, -20], name: ' Manual Trigger: Start Workflow' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
    ...
    }, position: [140, -20], name: 'Read YouTube Links from Google Sheets' } }))
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: { reset: false } }, position: [360, -20], name: 'Loop: Process Each YouTube Link' } }))
  .add(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
    ...
    }, position: [580, -220], name: ' Send Email Notification When Draft is Ready' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [580, -20], name: ' Dumpling AI: Get YouTube Transcript' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
    ...
    }, position: [800, -20], name: 'GPT-4o: Write Newsletter Draft from Transcript' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
        value: {
