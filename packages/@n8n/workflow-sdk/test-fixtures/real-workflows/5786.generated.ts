return workflow('995Zs4albP6ZWzOD', 'The Recap AI - Email Scraper', { executionOrder: 'v1' })
  ...
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [240, 0], name: 'map_website' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [500, 0], name: 'start_batch_scrape' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { position: [760, 0], name: 'rate_limit_wait' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [1020, 0], name: 'fetch_scrape_results' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [1900, -20], name: 'set_result' } }), node({ type: 'n8n-nodes-base.if', version: 2.2, config: { parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
    ...
    }, position: [1580, 160], name: 'check_retry_count' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .add(node({ type: 'n8n-nodes-base.stopAndError', version: 1, config: { parameters: {
      errorMessage: 'Too many retries when attempting to scrape website.'
    }, position: [1900, 240], name: 'too_many_attempts_error' } }))
  .add(sticky('## Scrape Public Email Addresses\n\n- Takes a website\'s home page url as input to the automation\n- Uses Firecrawl\'s `/map` and `/scrape/batch` endpoints to scrape and extract email addresses that exist on the website\'s HTML\n- Formats the results in array', { name: 'Sticky Note1', color: 4, position: [-60, -260], width: 2180, height: 700 }))