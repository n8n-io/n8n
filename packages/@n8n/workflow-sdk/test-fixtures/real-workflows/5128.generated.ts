return workflow('jwYT7Sha0ieBnEvJ', 'Auto-Publish Web Articles as Social Posts for X, LinkedIn, Reddit & Threads with Gemini AI', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-180, 20] } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [40, 20], name: 'Set Input Data' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.6, config: { parameters: {
      ...
      hasOutputParser: true
          ...
          options: {}
        }, name: 'HTTP Request1' } })], model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatGoogleGemini', version: 1, config: { parameters: {
          options: {},
        ...
        }, name: 'Structured Output Parser' } }) }, position: [260, 20], name: 'Social Media Agent' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: {
      resume: 'form',
      options: {},
    ...
    }, position: [560, 20], name: 'Wait to approve by user' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-upload-post.uploadPost', version: 1, config: { parameters: {
    ...
    }, position: [1060, -160], name: 'Upload Post X' } }), null], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .add(node({ type: 'n8n-nodes-upload-post.uploadPost', version: 1, config: { parameters: {
      user: '={{ $(\'Set Input Data\').item.json[\'upload-post_user\'] }}',
      ...
      uploadPostApi: { id: 'credential-id', name: 'uploadPostApi Credential' }
    }, position: [1060, 20], name: 'Upload Post Threads' } }))
  ...
  .add(node({ type: 'n8n-nodes-upload-post.uploadPost', version: 1, config: { parameters: {
      ...
      uploadPostApi: { id: 'credential-id', name: 'uploadPostApi Credential' }
    }, position: [1060, 200], name: 'Upload Post Linkedin' } }))
  ...
  .add(node({ type: 'n8n-nodes-base.reddit', version: 1, config: { parameters: {
    ...
    }, position: [1060, 400], name: 'Upload Post Reddit' } }))