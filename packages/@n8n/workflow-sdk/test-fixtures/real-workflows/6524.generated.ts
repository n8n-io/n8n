const wf = workflow(
	'E4pFNNNIatazgVPI',
	'Automated AI News Video Creation and Social Media Publishing With Postiz',
	{ executionOrder: 'v1' },
)
	.add(
		trigger({
			...
			version: 1,
			config: { position: [-624, 784], name: "When clicking 'Test workflow'" },
		}),
	)
	.then(
		node({
			...
			version: 1,
			config: {
				parameters: { url: 'http://rss.cnn.com/rss/edition.rss', options: {} },
				position: [-400, 784],
				name: 'RSS Read',
			},
		}),
	...
	.then(
		node({
			...
			version: 1,
			config: { position: [16, 784], name: 'Limit1' },
		}),
	)
	.then(
		node({
	...
	.then(
		node({
			...
			version: 4,
			config: {
				parameters: { url: '={{ $json.data.video_url }}', options: {} },
				position: [-592, 1536],
				name: 'Download Video',
			},
		}),
				...
				parameters: {
					columns: {
						value: {
							...
							'Heygen video url':
								"={{ $('Get Avatar Video Status (HeyGen)').item.json.data.video_url }}",
	...
	.add(
		sticky(
			...
			{ position: [-864, 880], width: 176, height: 128 },
		),
	)
	.add(
		sticky(
	...
	.add(
		sticky(
			...
			{ name: 'Sticky Note11', position: [32, 1184] },
		),
	)
	.add(
		sticky(
