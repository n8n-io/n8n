return workflow('1RHsJldA8GlWFp1E', 'Smart Email Auto-Responder', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.gmailTrigger', version: 1.2, config: { parameters: {
      simple: false,
  ...
  .then(ifBranch([null, node({ type: 'n8n-nodes-base.if', version: 2.2, config: { parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
    ...
    }, position: [1640, 460], name: 'Reply' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.textClassifier', version: 1, config: { parameters: {
      options: {},
      ...
      categories: {
        ...
        }, name: 'Google Gemini Chat Model' } }) }, position: [1820, 400] } }))
  .add(node({ type: 'n8n-nodes-base.googleCalendar', version: 1.3, config: { parameters: {
      limit: 3,
      options: {
  ...
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: {} }, position: [2280, 160], name: 'Loop Over Items' } }))
  ...
  .then(node({ type: 'n8n-nodes-base.emailSend', version: 2.1, config: { parameters: {
    ...
    }, credentials: { smtp: { id: 'credential-id', name: 'smtp Credential' } }, position: [2720, 160], name: 'QuestionsReply' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      messageId: '={{ $(\'Gmail Trigger\').all()[0].json.id }}\n',
      operation: 'markAsRead'
    }, credentials: {
      gmailOAuth2: { id: 'credential-id', name: 'gmailOAuth2 Credential' }
    }, position: [2780, 460], name: 'Mark as Read' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      ...
      gmailOAuth2: { id: 'credential-id', name: 'gmailOAuth2 Credential' }
    }, position: [2960, 460], name: 'Apply Label' } }))
  .add(node({ type: 'n8n-nodes-base.googleCalendar', version: 1.3, config: { parameters: {
      eventId: '={{$json.id}}',
      calendar: {
        __rl: true,
    ...
    }, position: [2520, 300], name: 'Google Calendar1' } }))
  .add(node({ type: 'n8n-nodes-base.emailSend', version: 2.1, config: { parameters: {
      ...
      fromEmail: 'user@example.com'
    }, credentials: { smtp: { id: 'credential-id', name: 'smtp Credential' } }, position: [2260, 580], name: 'Youtube Video Inquiry' } }))
  .add(node({ type: 'n8n-nodes-base.emailSend', version: 2.1, config: { parameters: {
      ...
      toEmail: '={{ $json.from.value[0].name }} <{{ $json.from.value[0].address }}>',
      fromEmail: 'user@example.com'
    }, credentials: { smtp: { id: 'credential-id', name: 'smtp Credential' } }, position: [2260, 760], name: 'Send Email' } }))