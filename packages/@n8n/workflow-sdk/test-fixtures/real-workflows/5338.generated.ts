const wf = workflow(
	...
	.then(
		node({
			...
			version: 2,
			config: {
				parameters: { mediaUrl: '={{ $json.final_output }}', resource: 'media' },
				credentials: {
	...
	.then(
		merge(
				...
				node({
					...
					config: {
						parameters: {
							...
							postContentText:
								"={{ $('Save Idea & Metadata to Google Sheets').first().json.caption }}",
					...
					config: {
						parameters: {
							...
							postCreateYoutubeOptionTitle:
								"={{ $('Save Idea & Metadata to Google Sheets').first().json.idea }}",
			...
			{ version: 3.2, parameters: { mode: 'chooseBranch', numberInputs: 9 } },
		),
	)
	.then(
		node({
	...
	.add(
		sticky('## | Step 1: Generate Clips (Seedance)\n', {
			name: 'Sticky Note10',
			color: 2,
			position: [592, 464],
			width: 880,
	...
	.add(
		sticky('## | Step 2: Generate Sounds (Fal AI)\n', {
			name: 'Sticky Note13',
			color: 2,
			position: [1504, 464],
			width: 640,
	...
	.add(
		sticky('## | INPUT: Starting Idea Section', {
			name: 'Sticky Note14',
			color: 7,
			position: [592, -96],
			width: 1560,
	...
	.add(
		sticky('## | Step 3: Stitch Video (Fal AI)', {
			color: 3,
			position: [592, 752],
			width: 1560,
			height: 260,
	...
	.add(
		sticky('## | Step 4 — Publish Video to Social Media\n', {
			name: 'Sticky Note2',
			color: 4,
			position: [592, 1040],
			width: 1560,
