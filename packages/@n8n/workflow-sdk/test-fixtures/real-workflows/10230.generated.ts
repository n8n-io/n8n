return workflow('', '')
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.chatTrigger', version: 1.3, config: { parameters: { options: { responseMode: 'responseNodes' } }, position: [-48, -240] } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.2, config: { parameters: {
      ...
      hasOutputParser: true
    }, subnodes: { memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryBufferWindow', version: 1.3, config: { name: 'Chat Memory' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Chat Model1' } }) }, name: 'Structured Output Parser' } }) }, position: [288, 0], name: 'Project Manager Agent' } }))
  .then(ifBranch([node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.2, config: { parameters: {
      ...
      hasOutputParser: true
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Chat Model2' } }), outputParser: outputParser({ type: '@n8n/n8n-nodes-langchain.outputParserStructured', version: 1.3, config: { parameters: {
          jsonSchemaExample: '{\n	"task": "task",\n	"description": "description",\n  "status": "status"\n}'
        }, name: 'Structured Output Parser1' } }) }, position: [1264, -288], name: 'Write Json' } }), node({ type: '@n8n/n8n-nodes-langchain.chat', version: 1, config: { parameters: {
      message: '={{ $json.output.response }}',
      options: {},
      waitUserReply: false
    }, position: [992, 128], name: 'Get More Info' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
