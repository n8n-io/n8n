return workflow('P6LeLqxzbkO3FiPE', '10 Track Email Campaign Performance', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{ triggerAtHour: 9 }] } }, position: [-60, 0], name: '⏰ Daily Campaign Check Trigger' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [160, 0], name: '✏️ Set Campaign Input Fields' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2, config: { parameters: { options: {}, promptType: 'define', hasOutputParser: true }, subnodes: { outputParser: outputParser({ type: '@n8n/n8n-nodes-langchain.outputParserAutofixing', version: 1, config: { parameters: { options: {} }, subnodes: { model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
        ...
        }, name: 'Structured Output Parser' } }) }, name: 'Auto-fixing Output Parser' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
          model: { __rl: true, mode: 'list', value: 'gpt-4o-mini' },
          options: {}
        }, credentials: {
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: '🧠 LLM: Summarize & Format' } }), tools: [tool({ type: 'n8n-nodes-mcp.mcpClientTool', version: 1, config: { parameters: {
          toolName: 'scrape_as_markdown',
          operation: 'executeTool',
          toolParameters: '={{ /*n8n-auto-generated-fromAI-override*/ $fromAI(\'Tool_Parameters\', ``, \'json\') }}'
        }, credentials: {
          mcpClientApi: { id: 'credential-id', name: 'mcpClientApi Credential' }
        }, name: '🌐 Bright Data MCP: Scrape Report' } })] }, position: [520, 0], name: '🤖 Agent: Scrape & Analyze Campaign Performance' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
    ...
    }, position: [1340, -100], name: '📧 Send Follow-Up Engagement Email' } }), node({ type: 'n8n-nodes-base.noOp', version: 1, config: { position: [1340, 100], name: '🚫 Skip — No Action Needed' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
