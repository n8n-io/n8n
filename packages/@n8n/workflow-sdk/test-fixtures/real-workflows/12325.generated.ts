return workflow('IgeWI32LKSmgs8Qj', 'Create & track LinkedIn posts with Google Sheets, GPT-5.1, Unsplash, and Sona', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{}] } }, position: [-224, 240] } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      ...
      sheetName: {
        ...
        mode: 'list',
        value: 1500824165,
        cachedResultUrl: 'https://docs.google.com/spreadsheets/d/1qUir1c-_ScMnoYVoQ0W41nsv5IpLW6rjK8HUNqvNnAg/edit#gid=1500824165',
    ...
    }, position: [-48, 240], name: 'Get Post Idea' } }))
  .then(node({ type: 'n8n-nodes-base.limit', version: 1, config: { position: [128, 240], name: 'Limit to One Post' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
        value: {
          Status: '=In Progress',
      ...
      googleSheetsOAuth2Api: { id: 'oURwy13SlM7ZHMDw', name: 'Dummy' }
    }, position: [320, 240], name: 'Update Post Status' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.1, config: { parameters: {
      ...
      hasOutputParser: true
          ...
          options: {}
        }, credentials: { openAiApi: { id: 'rxfg6jEjEjPWmANV', name: 'Dummy' } }, name: 'OpenAI Chat Model' } }), outputParser: outputParser({ type: '@n8n/n8n-nodes-langchain.outputParserStructured', version: 1.3, config: { parameters: {
        ...
        }, name: 'Structured Output Parser' } }) }, position: [560, 176], name: 'AI Content Generation' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [1024, 176], name: 'Post Formatter' } }))
  .then(ifBranch([node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.1, config: { parameters: {
      ...
      promptType: 'define'
          ...
          options: {}
        }, credentials: { openAiApi: { id: 'rxfg6jEjEjPWmANV', name: 'Dummy' } }, name: 'OpenAI Chat Model1' } }) }, position: [-224, 768], name: 'Format Content to HTML' } }), node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.1, config: { parameters: {
      text: '=You are a LinkedIn content expert. Create an engaging LinkedIn post based on these inputs:\n\nTopic: {{ $(\'Limit to One Post\').item.json[\'Topic/Subject\'] }}\nContent Type: {{ $(\'Limit to One Post\').item.json[\'Content Type\'] }}\nTone: {{ $(\'Limit to One Post\').item.json[\'Tone\'] }}\nTarget Audience: {{ $(\'Limit to One Post\').item.json[\'Target Audience\'] }}\nAdditional Notes: {{ $(\'Limit to One Post\').item.json[\'Additional Notes\'] }}\n\nRequirements:\n- Write a compelling LinkedIn post (150–300 words optimal)\n- Use short paragraphs (2–3 lines max) for readability\n- Include a strong hook in the first line\n- Add 3–5 relevant hashtags\n- Include a clear call-to-action that encourages comments\n- Match the specified tone exactly\n- Format specifically for LinkedIn (line breaks, no markdown)\n- Write in a natural, conversational human voice\n- Avoid corporate jargon, buzzwords, and overly formal language\n- Use contractions (I\'m, don\'t, we\'re) and casual phrasing where appropriate\n- Vary sentence length; avoid repetitive sentence structures\n- Include light personal perspective or storytelling when relevant\n- Avoid clichés such as "game-changer," "leverage," "synergy," "circle back," or "deep dive"\n- Do not use the character "—" anywhere\n- Write one word image queries separated by commas. \n\nOutput Instructions:\nReturn your response as valid JSON in the following structure only:\n\n{\n  "post_content": "The full LinkedIn post text goes here...",\n  "hashtags": ["#Marketing", "#Leadership", "#AI"],\n  "character_count": 285,\n  "engagement_tip": "Best time to post: Tuesday–Thursday, 8–10 AM",\n  "call_to_action": "What\'s your experience with this? Share in the comments.",\n  "image_query": "AI, Computer, IT"\n}\n',
      options: {},
      promptType: 'define',
      hasOutputParser: true
          ...
          jsonSchemaExample: '{\n  "post_content": "The main LinkedIn post text goes here with emojis naturally integrated...",\n  "hashtags": ["#Marketing", "#Leadership", "#AI"],\n  "character_count": 285,\n  "engagement_tip": "Best time to post: Tuesday-Thursday, 8-10 AM",\n  "call_to_action": "What\'s your experience with this? Share in the comments!",\n  "image_prompt": "A detailed DALL-E prompt for generating a professional LinkedIn post image"\n}'
        }, name: 'Structured Output Parser' } }) }, position: [560, 176], name: 'AI Content Generation' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
    ...
    }, credentials: { gmailOAuth2: { id: 'nRdBnd2Ua3MFAbal', name: 'Dummy' } }, position: [112, 752], name: 'Send Post Preview' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      sendTo: 'example_email@gmail.com',
      message: '=Your LinkedIn post preview has been sent to your inbox.\n\nPlease review the formatted preview email, then:\n- Click "Approve" to publish the post to LinkedIn\n- Click "Decline" to cancel\n\nTopic: {{ $(\'Limit to One Post\').first().json[\'Topic/Subject\'] }}',
      options: {},
      subject: 'Approve LinkedIn Post for Publishing',
      operation: 'sendAndWait',
      approvalOptions: { values: { approvalType: 'double' } }
    }, credentials: { gmailOAuth2: { id: 'nRdBnd2Ua3MFAbal', name: 'Dummy' } }, position: [112, 960], name: 'Send email for confirmation' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.if', version: 2.2, config: { parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
    ...
    }, position: [592, 1024], name: 'Check Image Preference' } }), node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
        value: {
          Status: '=Rejected',
      ...
      googleSheetsOAuth2Api: { id: 'oURwy13SlM7ZHMDw', name: 'Dummy' }
    }, position: [320, 1120], name: 'Update Post Status to Rejected' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.linkedIn', version: 1, config: { parameters: {
    ...
    }, position: [2256, 784], name: 'Create a post with image' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
        value: {
          Status: '=Posted',
      ...
      googleSheetsOAuth2Api: { id: 'oURwy13SlM7ZHMDw', name: 'Dummy' }
    }, position: [2576, 896], name: 'Update Post Status to Posted' } }))
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.1, config: { parameters: {
      ...
      hasOutputParser: true
          ...
          options: {}
        }, credentials: { openAiApi: { id: 'rxfg6jEjEjPWmANV', name: 'Dummy' } }, name: 'GPT-4.1-mini' } }), outputParser: outputParser({ type: '@n8n/n8n-nodes-langchain.outputParserStructured', version: 1.3, config: { parameters: {
          jsonSchemaExample: '{\n  "download_url": "the download URL"\n}'
        }, name: 'Structured Output Parser1' } }) }, position: [1536, 816], name: 'Choose Image' } }))
