return workflow('kZ3wJtLi5OI0WzFF', 'HTX AI Agent v1.02', { executionOrder: 'v1' })
  ...
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-688, 240], name: 'User Authentication (Replace Telegram ID)' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-416, 240], name: 'Adds "SessionId"' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.8, config: { parameters: {
      ...
      promptType: 'define'
    }, subnodes: { tools: [tool({ type: '@n8n/n8n-nodes-langchain.toolThink', version: 1.1, config: { parameters: {
        ...
        }, name: 'Think' } }), tool({ type: 'n8n-nodes-base.httpRequestTool', version: 4.2, config: { parameters: {
        ...
        }, name: '24h Stats' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolCalculator', version: 1, config: { name: 'Calculator' } }), tool({ type: 'n8n-nodes-base.httpRequestTool', version: 4.2, config: { parameters: {
        ...
        }, name: 'Best Bid/Ask' } }), tool({ type: 'n8n-nodes-base.httpRequestTool', version: 4.2, config: { parameters: {
          url: 'https://api.huobi.pro/market/detail/merged',
          options: {},
          sendQuery: true,
          queryParameters: {
        ...
        }, name: 'Average Price' } }), tool({ type: 'n8n-nodes-base.httpRequestTool', version: 4.2, config: { parameters: {
        ...
        }, name: 'Recent Trades' } }), tool({ type: 'n8n-nodes-base.httpRequestTool', version: 4.2, config: { parameters: {
          url: 'https://api.huobi.pro/market/detail/merged',
          options: {},
          sendQuery: true,
          queryParameters: {
        ...
        }, name: 'Price (Latest)' } }), tool({ type: 'n8n-nodes-base.httpRequestTool', version: 4.2, config: { parameters: {
        ...
        }, name: 'Klines (Candles)' } }), tool({ type: 'n8n-nodes-base.httpRequestTool', version: 4.2, config: { parameters: {
        ...
        }, name: 'Order Book Depth' } })], memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryBufferWindow', version: 1.3, config: { name: 'Simple Memory' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
          model: {
            __rl: true,
            mode: 'list',
            value: 'gpt-4.1-mini',
        ...
        }, name: 'OpenAI Chat Model' } }) }, position: [-96, 240], name: 'HTX AI Agent' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
      jsCode: '// Input: assumes incoming message in `item.json.message`\nconst input = $json.output;\nconst chunkSize = 4000;\n\n// Function to split text\nfunction splitMessage(text, size) {\n  const result = [];\n  for (let i = 0; i < text.length; i += size) {\n    result.push(text.substring(i, i + size));\n  }\n  return result;\n}\n\n// Logic\nif (input.length <= chunkSize) {\n  return [{ json: { message: input } }];\n} else {\n  const chunks = splitMessage(input, chunkSize);\n  return chunks.map(chunk => ({ json: { message: chunk } }));\n}'
    }, position: [464, 240], name: 'Splits message is more than 4000 characters' } }))
  ...
  .add(sticky('\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n ### **Price (Latest)**\n\n**Endpoint:** `GET /market/detail/merged`\n\n**What it does:** Returns the **latest trade price**, best bid/ask, and other snapshot details for a symbol.\n\n**Params:**\n- `symbol` (STRING, required) → e.g., `btcusdt` (lowercase, no dash)\n\n**Returns:** `tick.close` = last trade price, plus bid/ask/volumes.\n\n**n8n query mapping:**\n```txt\nsymbol = $fromAI(\'parameters0_Value\', \'btcusdt\', \'string\')\n```\n\n**Notes:** Use lowercase symbols without `-` or `/` (e.g., `btcusdt`).', { name: 'Sticky Note11', color: 6, position: [-192, 512], height: 820 }))
  .add(sticky('\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n### **Klines (Candles)**\n\n**Endpoint:** `GET /market/history/kline`\n\n**What it does:** Returns candlestick data for a symbol/interval.\n\n**Params:**\n- `symbol` (STRING, required) → lowercase (e.g., `btcusdt`)\n- `period` (ENUM, required — e.g., `1min,5min,15min,30min,60min,4hour,1day,1mon,1week,1year`)\n- `size` (INT, optional, default 20, max 2000)\n\n**Returns (array per candle):** `[id, open, close, low, high, amount, vol, count]`\n\n**n8n query mapping:**\n```txt\nsymbol = $fromAI(\'parameters0_Value\', \'btcusdt\', \'string\')\nperiod = $fromAI(\'parameters1_Value\', \'15min\', \'string\')\nsize   = $fromAI(\'parameters2_Value\', 20, \'number\')\n```\n\n**Notes:**\n- Use lowercase symbols without `-` (e.g., `btcusdt`).\n- If no `size` provided, defaults to 20 latest candles.', { name: 'Sticky Note12', color: 6, position: [464, 512], height: 1060 }))
  .add(sticky('\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n### **Merged Ticker (Bid/Ask/Last)**\n\n**Endpoint:** `GET /market/detail/merged`\n**What it does:** Returns best bid/ask and last price for a symbol. Use Calculator to derive a midpoint `(bid+ask)/2` if you need an average.\n\n**Params:** `symbol` (STRING, required) → lowercase, no dash (e.g., `btcusdt`)\n\n**Returns (key fields):** `tick.bid`, `tick.ask`, `tick.close`, `tick.high`, `tick.low`, `tick.amount`, `tick.vol`, `ts`\n\n**n8n query mapping:**\n```txt\nsymbol = $fromAI(\'parameters0_Value\', \'btcusdt\', \'string\')\n```\n\n**Notes:** HTX has no direct avg-price endpoint; this is the canonical ticker to pair with Calculator for midpoint.', { name: 'Sticky Note13', color: 6, position: [800, 512], height: 900 }))
  .add(sticky('\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n### **Recent Trades**\n\n**Endpoint:** `GET /market/history/trade`\n**What it does:** Returns the most recent trades for a given symbol.\n\n**Params:**\n* `symbol` (STRING, required) → lowercase, no dash (e.g., `btcusdt`)\n* `size` (INT, default 100, max 2000 — we set default 100)\n\n**Returns:** array of trade batches → each batch has `data: [{id, price, amount, direction, ts}]`\n\n**n8n query mapping:**\n```txt\nsymbol = $fromAI(\'parameters0_Value\', \'btcusdt\', \'string\')\nsize   = $fromAI(\'parameters1_Value\', 100, \'number\')\n```\n\n**Notes:** Unlike Binance, HTX nests trades inside `data` arrays per batch.', { name: 'Sticky Note14', color: 6, position: [1136, 512], height: 868 }))
