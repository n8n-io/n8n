return workflow('g1hmK8jw38TOYX7D', 'Create Video with Google Veo3 and Upload to Youtube', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-220, 840], name: 'When clicking ‘Test workflow’' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
    ...
    }, position: [0, 840], name: 'Get new video' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [220, 840], name: 'Set data' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [440, 840], name: 'Create Video' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 60 }, position: [660, 840], name: 'Wait 60 sec.' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [840, 840], name: 'Get status' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [-220, 1220], name: 'Get Url Video' } }), node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 60 }, position: [660, 840], name: 'Wait 60 sec.' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
    ...
    }, position: [20, 1220], name: 'Generate title' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: '={{ $(\'Get Url Video\').item.json.video.url }}',
      options: {}
    }, position: [400, 1220], name: 'Get File Video' } }))
  .add(node({ type: 'n8n-nodes-base.googleDrive', version: 3, config: { parameters: {
    ...
    }, position: [680, 1100], name: 'Upload Video' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
      columns: {
        value: {
    ...
    }, position: [860, 1100], name: 'Update result' } }))
  ...
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
    ...
    }, position: [860, 1320], name: 'Update Youtube URL' } }))
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{ field: 'minutes' }] } }, position: [-220, 700] } }))
  .add(sticky('# Generate AI Videos with Google Veo3, Save to Google Drive and Upload to YouTube\n\nThis workflow allows users to **generate AI videos** using **Google Veo3**, save them to **Google Drive**, generate optimized YouTube titles with GPT-4o, and **automatically upload them to YouTube** . The entire process is triggered from a Google Sheet that acts as the central interface for input and output.\n\nIT automates video creation, uploading, and tracking, ensuring seamless integration between Google Sheets, Google Drive, Google Veo3, and YouTube.\n\n\n\n\n', { name: 'Sticky Note3', color: 3, position: [-220, -460], width: 740, height: 280 }))
  .add(sticky('## STEP 1 - GOOGLE SHEET\nCreate a [Google Sheet like this](https://docs.google.com/spreadsheets/d/1pcoY9N_vQp44NtSRR5eskkL5Qd0N0BGq7Jh_4m-7VEQ/edit?usp=sharing).\n\nPlease insert:\n- in the "PROMPT" column the accurate description of the video you want to create\n- in the "DURATION" column the lenght of the video you want to create\n\nLeave the "VIDEO" column unfilled. It will be inserted by the system once the video has been created', { name: 'Sticky Note4', position: [-220, -140], width: 740, height: 200 }))
  .add(sticky('## STEP 4 - MAIN FLOW\nStart the workflow manually or periodically by hooking the "Schedule Trigger" node. It is recommended to set it at 5 minute intervals.', { name: 'Sticky Note5', position: [-220, 520], width: 740, height: 100 }))
  .add(sticky('## STEP 2 - GET API KEY (YOURAPIKEY)\nCreate an account [here](https://fal.ai/) and obtain API KEY.\nIn the node "Create Image" set "Header Auth" and set:\n- Name: "Authorization"\n- Value: "Key YOURAPIKEY"', { name: 'Sticky Note6', position: [-220, 100], width: 740, height: 140 }))
  .add(sticky('Set API Key created in Step 2', { name: 'Sticky Note7', position: [400, 780], width: 180, height: 200 }))
  .add(sticky('Set YOUR_USERNAME in Step 3', { name: 'Sticky Note', position: [660, 1260], width: 180, height: 200 }))
  .add(sticky('## STEP 3 - Upload video on Youtube\n- Find your API key in your [Upload-Post Manage Api Keys](https://www.upload-post.com/?linkId=lp_144414&sourceId=n3witalia&tenantId=upload-post-app) 10 FREE uploads per month\n- Set the the "Auth Header":\n-- Name: Authorization\n-- Value: Apikey YOUR_API_KEY_HERE\n- Create profiles to manage your social media accounts. The "Profile" you choose will be used in the field YOUR_USRNAME (eg. test1 or test2).  ', { name: 'Sticky Note8', position: [-220, 280], width: 740, height: 200 }))