return workflow('cbeBbWowy2c7CXli', 'From Google Drive to Instagram, TikTok & YouTube with Airtable Tracking', { executionOrder: 'v1' })
  ...
  .then(node({ type: 'n8n-nodes-base.googleDrive', version: 1, config: { parameters: {
      fileId: {
        __rl: true,
        mode: '',
  ...
  .then(node({ type: 'n8n-nodes-base.writeBinaryFile', version: 1, config: { parameters: {
      options: {},
      fileName: '={{ $json.originalFilename.replaceAll(" ", "_") }}'
    }, position: [-2380, 420], name: 'Read video from Google Drive' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1, config: { parameters: { options: {}, resource: 'audio', operation: 'transcribe' }, credentials: {
      openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
    }, position: [-1960, 420], name: 'Get Audio from Video' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.4, config: { parameters: {
      modelId: {
        __rl: true,
        mode: 'list',
        value: 'gpt-4o',
      ...
      openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
    }, position: [-1740, 420], name: 'Generate Description for Videos' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-1360, 420], name: 'Set Variables' } }))
  .then(node({ type: 'n8n-nodes-base.airtable', version: 1, config: { parameters: {
      table: {
        __rl: true,
        mode: 'id',
    ...
    }, position: [-1160, 420], name: 'Create Airtable Record' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [-960, 420], name: 'Edit Airtable Fields1' } }))
  .then(node({ type: 'n8n-nodes-base.airtable', version: 1, config: { parameters: {
      ...
      airtableTokenApi: { id: 'credential-id', name: 'airtableTokenApi Credential' }
    }, position: [-780, 420], name: 'Update Airtable with Description' } }))
  .add(node({ type: 'n8n-nodes-base.readBinaryFile', version: 1, config: { parameters: {
      filePath: '={{ $(\'Read video from Google Drive\').item.json.originalFilename.replaceAll(" ", "_") }}',
      dataPropertyName: 'datavideo'
    }, position: [-260, 180], name: 'Read Video for TikTok' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [-20, 180], name: 'Upload Video to TikTok' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [220, 180], name: 'Edit Airtable Fields' } }))
  .then(node({ type: 'n8n-nodes-base.airtable', version: 1, config: { parameters: {
      id: '={{ $(\'Create Airtable Record\').item.json.id }}',
      table: {
        __rl: true,
      ...
      airtableTokenApi: { id: 'credential-id', name: 'airtableTokenApi Credential' }
    }, position: [440, 180], name: 'Update TikTok Status' } }))
  .add(node({ type: 'n8n-nodes-base.readBinaryFile', version: 1, config: { parameters: {
      filePath: '={{ $(\'Read video from Google Drive\').item.json.originalFilename.replaceAll(" ", "_") }}',
      dataPropertyName: 'datavideo'
    }, position: [-280, 480], name: 'Read Video for Instagram' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: 'https://api.upload-post.com/api/upload',
      method: 'POST',
      options: {},
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [-20, 480], name: 'Upload Video to Instagram' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [220, 480], name: 'Edit Airtable Fields 2' } }))
  .then(node({ type: 'n8n-nodes-base.airtable', version: 1, config: { parameters: {
      id: '={{ $(\'Create Airtable Record\').item.json.id }}',
      table: {
        __rl: true,
      ...
      airtableTokenApi: { id: 'credential-id', name: 'airtableTokenApi Credential' }
    }, position: [440, 480], name: 'Update Instagram Status' } }))
  .add(node({ type: 'n8n-nodes-base.readBinaryFile', version: 1, config: { parameters: {
      filePath: '={{ $(\'Read video from Google Drive\').item.json.originalFilename.replaceAll(" ", "_") }}',
      dataPropertyName: 'datavideo'
    }, position: [-280, 740], name: 'Read Video for YouTube' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: 'https://api.upload-post.com/api/upload',
      method: 'POST',
      options: {},
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [-20, 740], name: 'Upload Video to YouTube' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [220, 740], name: 'Edit Airtable Fields 3' } }))
  .then(node({ type: 'n8n-nodes-base.airtable', version: 1, config: { parameters: {
      id: '={{ $(\'Create Airtable Record\').item.json.id }}',
      table: {
        __rl: true,
      ...
      airtableTokenApi: { id: 'credential-id', name: 'airtableTokenApi Credential' }
    }, position: [440, 740], name: 'Update YouTube Status - Success' } }))
  .add(trigger({ type: 'n8n-nodes-base.errorTrigger', version: 1, config: { position: [-2260, 760] } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
    ...
    }, position: [-1920, 740] } }), null], { version: 2.1, parameters: {
      options: {},
      conditions: {
        options: {
          version: 1,
