return workflow('JoBrqIuKn4cnNOJ0', 'Personal Appointment Booking', { executionOrder: 'v1' })
  ...
  .then(ifBranch([node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, credentials: { calApi: { id: 'credential-id', name: 'calApi Credential' } }, position: [1040, 520], name: 'Check available slot in Cal.com' } }), node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      authentication: 'predefinedCredentialType',
      nodeCredentialType: 'calApi'
    }, credentials: { calApi: { id: 'credential-id', name: 'calApi Credential' } }, position: [1040, 720], name: 'Book an Appointment' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.respondToWebhook', version: 1.2, config: { parameters: { options: {} }, position: [1320, 620], name: 'Respond to Webhook' } }))