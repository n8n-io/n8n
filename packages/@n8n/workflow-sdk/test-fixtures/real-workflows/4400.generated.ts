const wf = workflow('IwtOfHq5pZQNDAF0', 'Complete RAG from PDF with Mistral OCR', {
	executionOrder: 'v1',
})
	.add(
		trigger({
			...
			version: 1,
			config: { position: [-480, -300], name: 'When clicking ‘Test workflow’' },
		}),
	)
	.then(
		node({
				...
				parameters: {
					filter: {
						folderId: {
							...
							cachedResultUrl:
								'https://drive.google.com/drive/folders/1LWVo3yn_1bWQJsLskBIbWTGwlfObvtUK',
				...
				parameters: {
					options: {
						summarizationMethodAndPrompts: {
							values: {
								prompt:
									'Write a concise summary of the following (in italiano):\n\n\n"{text}"\n\n\nCONCISE SUMMARY:',
								combineMapPrompt:
									'Write a concise summary of the following (in italiano):\n\n\n"{text}"\n\n\nCONCISE SUMMARY:',
							},
						},
