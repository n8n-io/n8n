const wf = workflow(
	'W9fdydWJuXntSK9c',
	'💥 Automate video ads with NanoBanana, Seedream 4, ChatGPT Image and Veo 3 - VIDE',
	{ executionOrder: 'v1' },
)
	.add(
		trigger({
	...
	.then(
		node({
			...
			version: 1,
			config: {
				parameters: { options: {}, fieldToSplitOut: 'output.image_prompt' },
				position: [2528, 272],
				...
				parameters: {
					columns: {
						value: {
							...
							'URL IMAGE SOURCE':
								"={{ $('Google Drive: Upload Image').first().json.webContentLink }}",
							'URL IMAGE CHATGPT':
								"={{ $('ChatGPT Image – fetch generated image').first().json.data.response.resultUrls[0] }}",
							'URL IMAGE SENDREAM':
								"={{ $('Set – select Seedream image URL').first().json.url_video }}",
							'URL IMAGE NANOBANANA':
								"={{ $('NanoBanana – fetch edited image').item.json.images[0].url }}",
						},
						schema: [
							{
								id: 'ID IMAGE',
	...
	.then(
		node({
			type: 'n8n-nodes-base.wait',
			version: 1.1,
			config: { parameters: { amount: 2 }, position: [3408, 768], name: 'Wait: Merge Process' },
		}),
	)
	.then(
		node({
	...
	.add(
		node({
			...
			version: 1.1,
			config: { position: [2160, 496], name: 'Think' },
		}),
	)
	.add(
		node({
	...
	.add(
		node({
			type: '@n8n/n8n-nodes-langchain.toolThink',
			version: 1,
			config: { position: [976, 960], name: 'Think1' },
		}),
	)
	.add(
		node({
	...
	.add(
		sticky('# 5️⃣  → Step 5 — Publish the final ad to multiple social platforms with Blotato', {
			name: 'Sticky Note9',
			position: [560, 1120],
			width: 3804,
			height: 608,
	...
	.add(
		sticky(
			'# 2️⃣  → Step 2 — Create product images with : 🌊 Seedream 4.0 +  🍌 NanoBanana + 🤖 ChatGPT image',
			{ name: 'Sticky Note11', color: 3, position: [2480, 0], width: 1884, height: 624 },
		),
	)
	.add(
		sticky('# 3️⃣  → Step 3 — Produce video ads with Veo 3', {
			name: 'Sticky Note1',
			color: 7,
			position: [560, 656],
			width: 1920,
