return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{ field: 'hours', hoursInterval: 24 }] } }, position: [-960, 240], name: 'Schedule Daily Check' } }))
  .then(node({ type: 'n8n-nodes-base.googleAds', version: 1, config: { parameters: { requestOptions: {}, additionalOptions: {} }, position: [-736, 240], name: 'Fetch Google Ads Data' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-512, 240], name: 'AI Performance Analysis' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.googleSheets', version: 4.4, config: { parameters: {
      columns: {
        value: {
    ...
    }, position: [160, 48], name: 'Update Campaign Dashboard' } }), node({ type: 'n8n-nodes-base.googleSheets', version: 4.4, config: { parameters: {
      columns: {
        value: {
          CPC: '${{ $json.metrics.avgCpc }}',
          CTR: '={{ $json.metrics.ctr }}%',
    ...
    }, position: [160, 48], name: 'Update Campaign Dashboard' } })], { version: 2, parameters: {
      options: {},
      conditions: {
        options: {
          leftValue: '',
  ...
  .then(node({ type: 'n8n-nodes-base.set', version: 3.3, config: { parameters: {
    ...
    }, position: [384, 144], name: 'Generate Daily Summary' } }))
  // Disconnected: Log All Campaigns
  .add(node({ type: 'n8n-nodes-base.googleSheets', version: 4.4, config: { parameters: {
      columns: {
        value: {
          Cost: '${{ $json.metrics.cost }}',
          Tier: '={{ $json.performanceTier }}',
    ...
    }, position: [-64, 432], name: 'Log All Campaigns' } }))
  // Disconnected: Alert: Scale Opportunity
  .add(node({ type: 'n8n-nodes-base.slack', version: 2.1, config: { parameters: {
      ...
      otherOptions: {}
    }, position: [-64, -96], name: 'Alert: Scale Opportunity' } }))
  ...
  .add(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-64, 240], name: 'Generate Action Plan' } }))
  // Disconnected: Email Performance Report
  .add(node({ type: 'n8n-nodes-base.emailSend', version: 2.1, config: { parameters: {
      options: { replyTo: 'user@example.com', allowUnauthorizedCerts: false },
      subject: '={{ $json.emailSubject }}',
      toEmail: 'user@example.com',
      fromEmail: 'user@example.com'
    }, position: [160, 240], name: 'Email Performance Report' } }))
  // Disconnected: Alert: Issues Detected
  .add(node({ type: 'n8n-nodes-base.slack', version: 2.1, config: { parameters: {
      text: '⚠️ *PERFORMANCE ALERT*\n\n*Campaign:* {{ $json.campaignName }}\n*Alert Level:* {{ $json.alertLevel }}\n*Performance Score:* {{ $json.performanceScore }}/100\n*Tier:* {{ $json.performanceTier }}\n\n*Current Metrics:*\n• Cost: ${{ $json.metrics.cost }}\n• Conversions: {{ $json.metrics.conversions }}\n• Cost/Conversion: ${{ $json.metrics.costPerConversion }}\n• Conversion Rate: {{ $json.metrics.conversionRate }}%\n\n*⚠️ Issues Identified:*\n{{ $json.insights.join(\'\\n\') }}\n\n_Action Required: Review and optimize within 24 hours_',
      otherOptions: {}
    }, position: [-64, 624], name: 'Alert: Issues Detected' } }))
  .add(sticky('# 📊 PPC Campaign Intelligence System\n\nAutomatically monitors Google Ads campaigns daily, analyzes performance with AI scoring, and alerts your team about scaling opportunities or issues requiring attention.\n\n**AI Analysis:** Evaluates CTR, conversion rate, cost efficiency, and traffic volume to assign performance scores (0-100).\n\n**Setup:**\n- Connect Google Ads, Sheets, Slack, SMTP\n- Update spreadsheet IDs and Slack channels\n- Customize scoring thresholds in code\n- Adjust email recipients\n\n**Benefits:**\n- Never miss scaling opportunities\n- Catch underperforming campaigns early\n- Data-driven optimization decisions\n- Automated daily reporting\n\n*Built by Daniel Shashko*\n[Connect on LinkedIn](https://www.linkedin.com/in/daniel-shashko/)', { name: 'Sticky Note', position: [-1552, 64], width: 500, height: 572 }))
  .add(sticky('Triggers workflow automatically every morning at 9 AM daily', { name: 'Sticky Note1', position: [-992, 112], width: 160, height: 96 }))
  .add(sticky('Pulls all campaign metrics from Google Ads API', { name: 'Sticky Note2', position: [-768, 112], width: 160, height: 96 }))
  .add(sticky('Scores campaigns based on CTR, conversions, and cost efficiency\n', { name: 'Sticky Note3', position: [-544, 112], width: 160, height: 96 }))
  .add(sticky('Splits campaigns into high-performers versus worst ones', { name: 'Sticky Note4', position: [-320, 112], width: 160, height: 96 }))
  .add(sticky('Records all campaign data to historical log spreadsheet\n', { name: 'Sticky Note5', position: [-256, 432], width: 160, height: 96 }))
  .add(sticky('Logs daily metrics to Google Sheets performance tracking tab', { name: 'Sticky Note6', position: [128, -80], width: 160, height: 96 }))
  .add(sticky('Sends Slack notification for excellent performing campaigns ready to scale', { name: 'Sticky Note7', position: [-272, -96], width: 160, height: 128 }))
  .add(sticky('Sends Slack warning for underperforming campaigns requiring immediate action\n', { name: 'Sticky Note8', position: [-256, 624], width: 160, height: 128 }))
  .add(sticky('Creates customized email reports based on campaign performance tier\n', { name: 'Sticky Note9', position: [-96, 112], width: 160, height: 96 }))
  .add(sticky('Sends personalized action plan emails to PPC team members\n', { name: 'Sticky Note10', position: [128, 416], width: 160, height: 96 }))
  .add(sticky('Calculates total spend, conversions, and campaign distribution summary', { name: 'Sticky Note11', position: [352, 16], width: 160, height: 96 }))