return workflow('0eQvHzKKVrYZMyyZ', 'Invoice Workflow', { executionOrder: 'v1' })
  ...
  .then(node({ type: 'n8n-nodes-base.extractFromFile', version: 1, config: { parameters: { options: {}, operation: 'pdf' }, position: [480, 0], name: 'Extract from File' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.informationExtractor', version: 1, config: { parameters: {
      text: '={{ $json.text }}',
      options: {
    ...
    }, subnodes: { model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.1, config: { parameters: { options: {} }, name: 'OpenAI Chat Model' } }) }, position: [740, 0] } }))
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.7, config: { parameters: {
      ...
      promptType: 'define'
          ...
          options: {}
        }, name: 'Anthropic Chat Model' } }) }, position: [1300, 0], name: 'Anthropic Agent' } }))
