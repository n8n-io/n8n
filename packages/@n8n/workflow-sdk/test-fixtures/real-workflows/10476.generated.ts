const wf = workflow('', '')
	...
	.then(
		node({
			...
			version: 1,
			config: {
				parameters: { cwd: '', command: './update-all-docker-compose.sh' },
				position: [624, 848],
				name: 'Update Docker',
			},
		}),
	...
	.add(
		sticky(
			...
			{ name: 'Sticky Note2', position: [384, -496], width: 1488, height: 320 },
		),
	)
	.add(
		sticky('## Docker Restart Service\nAutomatically restart a given docker container', {
			name: 'Sticky Note3',
			position: [368, -112],
			width: 1488,
			height: 352,
	...
	.add(
		sticky('## Allow the user to get all current deployed docker container\n', {
			name: 'Sticky Note4',
			position: [368, 320],
			width: 752,
			height: 352,
	...
	.add(
		sticky('## Automatically update all docker images on the server', {
			name: 'Sticky Note5',
			position: [368, 720],
			width: 1232,
			height: 352,
