return workflow('5mGRqFpu73QguZPC', 'ocr Telegram - SAP', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.telegramTrigger', version: 1.2, config: { parameters: { updates: ['callback_query'], additionalFields: {} }, credentials: {
      telegramApi: { id: 'credential-id', name: 'telegramApi Credential' }
    }, position: [-40, 640], name: 'Callback Waiting Answer' } }))
  .then(switchCase([node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      sendBody: true,
      specifyBody: 'json'
    }, position: [760, 520], name: 'Connect to SAP' } }), node({ type: 'n8n-nodes-base.noOp', version: 1, config: { position: [760, 820], name: 'No Operation, do nothing' } })], { version: 3.2, parameters: {
      rules: {
        values: [
          {
            outputKey: 'SI',
  ...
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
    ...
    }, position: [1160, 520], name: 'Get Header' } }))
  .add(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
    ...
    }, position: [1560, 680], name: 'Get Row Details' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
      jsCode: 'const items = $input.all();  // capturamos todos los items que entran\nconst DocumentLines = [];\n\nfor (let i = 0; i < items.length; i++) {\n  const item = items[i].json;\n\n  DocumentLines.push({\n    ItemCode: item.código,\n    Quantity: item.cantidad,\n    UnitPrice: item.precio\n  });\n}\n\nreturn [{ DocumentLines }];\n'
    }, position: [1880, 680], name: 'Generate DocumentLines' } }))
  .then(merge([node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      sheetName: {
        __rl: true,
        mode: 'list',
        value: '',
    ...
    }, position: [1160, 520], name: 'Get Header' } }), node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
      jsCode: 'const items = $input.all();  // capturamos todos los items que entran\nconst DocumentLines = [];\n\nfor (let i = 0; i < items.length; i++) {\n  const item = items[i].json;\n\n  DocumentLines.push({\n    ItemCode: item.código,\n    Quantity: item.cantidad,\n    UnitPrice: item.precio\n  });\n}\n\nreturn [{ DocumentLines }];\n'
    }, position: [1880, 680], name: 'Generate DocumentLines' } })], { version: 3.2, parameters: {
      mode: 'combine',
      options: {},
  ...
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [2580, 540], name: 'Create JSON' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [2980, 540], name: 'POST PurchaseInvoices' } }))
  .then(node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
      ...
      telegramApi: { id: 'credential-id', name: 'telegramApi Credential' }
    }, position: [3360, 540], name: 'PurchaseInvoices created' } }))
  .add(trigger({ type: 'n8n-nodes-base.telegramTrigger', version: 1.2, config: { parameters: { updates: ['message'], additionalFields: {} }, name: 'Trigger Receive Message' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [260, 0], name: 'Capture Telegram Data' } }))
  .then(node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
      ...
      telegramApi: { id: 'credential-id', name: 'telegramApi Credential' }
    }, position: [500, 0], name: 'File Received' } }))
  .then(node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
      fileId: '={{ $(\'Capture Telegram Data\').item.json.telegram.file_id }}',
      resource: 'file'
    }, credentials: {
      telegramApi: { id: 'credential-id', name: 'telegramApi Credential' }
    }, position: [720, 0], name: 'Download File' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [940, 0], name: 'Upload File LlamaIndex' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [1180, 0], name: 'Validate Status LlamaIndex' } }))
  .then(switchCase([node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [1620, -100], name: 'Get Results LlamaIndex' } }), node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 3 }, position: [1620, 100] } })], { version: 3.2, parameters: {
      rules: {
        values: [
          {
            outputKey: 'SUCCESS',
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.7, config: { parameters: {
      ...
      hasOutputParser: true
          ...
          jsonSchemaExample: '{\n  "nombre_proveedor": "Blockies Corporation",\n  "ruc_proveedor": "78787878-7",\n  "direccion_proveedor": "AV. DE LAS ARTES NORTE NRO. 310 (ESPALDA RAMBLA DE JAVIER PRADO) LIMA - LIMA - SAN BORJA",\n  "numero_factura": "00003",\n  "fecha_emision": "2025-04-15",\n  "detalle_productos": [\n    {\n      "codigo": "srv001",\n      "descripcion": "Servicio de Alquiler de Montacargas",\n      "cantidad": 1,\n      "precio": 35.00,\n      "subtotal": 35.00\n    }\n  ],\n  "subtotal_factura": 35.00,\n  "descuento_factura": 0.00,\n  "total_neto": 35.00\n}'
        }, name: 'Structured Output Parser (Example)' } }) }, position: [1840, -100], name: 'Basic LLM Chain' } }))
  .then(node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'output.detalle_productos' }, position: [2280, -100] } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
        value: {
