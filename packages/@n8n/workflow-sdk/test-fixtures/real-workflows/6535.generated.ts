return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.calTrigger', version: 2, config: { parameters: {
      events: ['BOOKING_CREATED'],
      options: { eventTypeId: 2098128 }
    }, credentials: { calApi: { id: 'credential-id', name: 'calApi Credential' } }, position: [-500, 140], name: 'Cal.com Trigger' } }))
  ...
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [340, 140], name: 'Scrape Profiles + Posts - Relevance AI' } }))
  ...
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [740, 140], name: 'Experiences' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [900, 140], name: 'Education' } }))
  ...
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [1300, 140], name: 'Citations' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
    ...
    }, position: [400, 560], name: 'Person + Company Profile' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
      modelId: {
        __rl: true,
        mode: 'list',
      ...
      openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
    }, position: [780, 560], name: 'Pain Points + Solutions' } }))
  .then(node({ type: 'n8n-nodes-base.html', version: 1.2, config: { parameters: {
    ...
    }, position: [1220, 560], name: 'Create HTML Report' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
    ...
    }, position: [1540, 560], name: 'Email Report' } }))
  .add(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      options: {},
      sheetName: {
        __rl: true,
    ...
    }, position: [-220, 280], name: 'Google Sheets1' } }))
  .add(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      options: {},
      sheetName: {
        __rl: true,
    ...
    }, position: [-220, 480], name: 'Google Sheets2' } }))
  .add(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      options: {},
      sheetName: {
        __rl: true,
