return workflow('STAFgThRSXspfU6H', 'Team of AI Copywriters', { executionOrder: 'v1' })
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.chatTrigger', version: 1.1, config: { parameters: { options: {} }, position: [-180, -20], name: 'When chat message received' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2, config: { parameters: {
      options: {
        ...
        }, name: 'OpenAI Chat Model' } }), tools: [tool({ type: '@n8n/n8n-nodes-langchain.toolWorkflow', version: 2.2, config: { parameters: {
          workflowId: {
            __rl: true,
            mode: 'list',
        ...
        }, name: 'Sales Letter Tool' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolWorkflow', version: 2.2, config: { parameters: {
          workflowId: {
            __rl: true,
            mode: 'list',
        ...
        }, name: 'Cold Email Writer Tool' } })] }, position: [120, -20], name: 'Copy Assistant' } }))
  .then(node({ type: 'n8n-nodes-base.googleDocs', version: 2, config: { parameters: {
    ...
    }, position: [480, -20], name: 'Update a document' } }))