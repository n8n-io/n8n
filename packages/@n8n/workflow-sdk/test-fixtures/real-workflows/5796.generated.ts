return workflow('I0Tzx8whIWYdQJVn', 'Generate Avatar Videos from Latest AI News using Dumpling AI and HeyGen', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{}] } }, position: [-840, -95] } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [-620, -95], name: 'Dumpling AI: Search AI News' } }))
  .then(node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'news' }, position: [-400, -95], name: 'Split: Individual News Items' } }))
  .then(node({ type: 'n8n-nodes-base.limit', version: 1, config: { parameters: { maxItems: 4 }, position: [-180, -95], name: 'Limit: Top 4 News Results' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [40, -95], name: 'Dumpling AI: Scrape Article Content' } }))
  .then(node({ type: 'n8n-nodes-base.aggregate', version: 1, config: { parameters: {
    ...
    }, position: [260, -95], name: ' Combine: Merge Scraped News Content' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.9, config: { parameters: {
      ...
      promptType: 'define'
        ...
        }, name: ' GPT-4o Model' } }) }, position: [480, -95], name: 'GPT-4o Agent: Write Video Script' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [856, -95], name: ' HeyGen: Generate Avatar Video' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { position: [1076, -95], name: 'Wait: For HeyGen to Process' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [1296, -95], name: 'HeyGen: Check Video Status' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
    ...
    }, position: [1736, -220], name: 'Google Sheets: Log Video URL' } }), node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 20 }, position: [1736, 5], name: ' Wait: Retry if Not Complete' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
