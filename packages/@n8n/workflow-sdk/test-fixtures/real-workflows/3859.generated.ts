return workflow('J2D0BssoDmn4BC6D', 'AI Customer-Support Assistant · WhatsApp Ready · Works for Any Business', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.whatsAppTrigger', version: 1, config: { parameters: { updates: ['messages'] }, credentials: {
    ...
    }, position: [-260, 140], name: 'WhatsApp Trigger' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.7, config: { parameters: {
      ...
      hasOutputParser: true
    }, subnodes: { tools: [tool({ type: '@n8n/n8n-nodes-langchain.toolHttpRequest', version: 1.1, config: { parameters: {
        ...
        }, name: 'get_page' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolHttpRequest', version: 1.1, config: { parameters: {
        ...
        }, name: 'list_links' } })], model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
        ...
        }, name: 'Postgres Users Memory' } }) }, position: [120, 140], name: 'AI Agent' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [520, 140], name: '24-hour window check' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [1040, 120], name: 'cleanAnswer' } }), node({ type: 'n8n-nodes-base.whatsApp', version: 1, config: { parameters: {
    ...
    }, position: [1060, 360], name: 'Send Pre-approved Template Message to Reopen the Conversation' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
