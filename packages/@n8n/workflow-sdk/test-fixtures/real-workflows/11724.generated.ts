return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.3, config: { parameters: { rule: { interval: [{ triggerAtHour: 19 }] } }, position: [-704, 272], name: 'Trice once a day - evenings' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-368, 272], name: 'Config Variables' } }))
  ...
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [528, 272], name: 'Clean data and assign fields' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [1664, 256], name: 'Assign Default Image' } }), node({ type: 'n8n-nodes-base.httpRequest', version: 4.3, config: { parameters: {
      url: '=https://your-site.com/wp-json/wp/v2/media/{{ $json.photo_id }}',
      options: {},
      authentication: 'predefinedCredentialType',
      nodeCredentialType: 'wordpressApi'
    }, credentials: { wordpressApi: { id: '', name: '' } }, position: [1200, 448], name: 'Get Image' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.3, config: { parameters: {
    ...
    }, credentials: { httpHeaderAuth: { id: '', name: '' } }, position: [2688, 272], name: 'Shotstack - Submit Request' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 30 }, position: [2944, 272], name: 'Wait - Shotstack Works' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.3, config: { parameters: {
      url: '=https://api.shotstack.io/stage/render/{{ $json.response.id }}',
      options: {},
      authentication: 'genericCredentialType',
      genericAuthType: 'httpHeaderAuth'
    }, credentials: { httpHeaderAuth: { id: '', name: '' } }, position: [3184, 272], name: 'Check Status' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.httpRequest', version: 4.3, config: { parameters: { url: '={{ $json.response.url }}', options: {} }, position: [3664, 256], name: 'Download video' } }), node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 30 }, position: [2944, 272], name: 'Wait - Shotstack Works' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.youTube', version: 1, config: { parameters: {
      title: '={{$(\'Config Variables\').first().json.daily_digest_text}} - {{ \n  (function() {\n    const d = new Date();\n    const day = d.getDate();\n    const weekday = ["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"][d.getDay()];\n    const year = d.getFullYear();\n    return `${day} ${weekday}, ${year}`;\n  })()\n}}\n',
      options: {
        tags: 'dailydigest',
        license: 'creativeCommon',
        embeddable: true,
        description: '={{$(\'Config Variables\').first().json.daily_digest_text}} - {{ \n  (function() {\n    const d = new Date();\n    const day = d.getDate();\n    const weekday = ["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"][d.getDay()];\n    const year = d.getFullYear();\n    return `${day} ${weekday}, ${year}`;\n  })()\n}}\n',
        defaultLanguage: 'ro',
      ...
      categoryId: '25',
      regionCode: 'MD'
    }, credentials: { youTubeOAuth2Api: { id: '', name: '' } }, position: [4144, 256], name: 'Upload video to youtube as' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
  ...
  .add(sticky('', { name: 'Sticky Note4', color: 4, position: [2416, 208], width: 1392, height: 304 }))
  .add(sticky('## Generate video\n\nThis section prepares the JSON payload for Shotstack based on their API, submits the rendering request, and checks every 30 seconds until the video is ready, then retrieves the download URL.', { name: 'Sticky Note5', position: [2416, 64], width: 1392, height: 128 }))
  .add(sticky('', { name: 'Sticky Note7', color: 4, position: [4032, 208], width: 336, height: 304 }))
  .add(sticky('## Submits Short\n\nThis node uploads the generated Short to YouTube.', { name: 'Sticky Note8', position: [4032, 64], width: 336, height: 128 }))