return workflow('', 'UGC Nano Banana', { executionOrder: 'v1' })
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.7, config: { parameters: {
    ...
    }, subnodes: { model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatMistralCloud', version: 1, config: { parameters: { model: 'mistral-large-latest', options: {} }, credentials: {
          mistralCloudApi: { id: 'credential-id', name: 'mistralCloudApi Credential' }
        }, name: 'Mistral Cloud Chat Model' } }), outputParser: outputParser({ type: '@n8n/n8n-nodes-langchain.outputParserStructured', version: 1.3, config: { parameters: {
          jsonSchemaExample: '{\n      "ugc_prompts": [\n        {\n          "prompt_number": "integer",\n          "prompt": "string"\n        }\n      ]\n}\n'
        }, name: 'Structured Output Parser' } }) }, position: [-176, -16], name: 'Generate Prompts' } }))
  .then(node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'output.ugc_prompts' }, position: [144, -16] } }))
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: {} }, position: [352, -16], name: 'Loop Over Items' } }))
  .add(node({ type: 'n8n-nodes-base.noOp', version: 1, config: { position: [560, -32], name: 'No Operation, do nothing' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [496, 128], name: 'Generate Image' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: { url: '={{ $json.images[0].url }}', options: {} }, position: [672, 128], name: 'Retrieve Image' } }))
