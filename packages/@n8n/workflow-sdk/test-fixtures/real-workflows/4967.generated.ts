return workflow('', '🎬 Instantly Turn Ideas into Viral Instagram Reel Scenarios 🤖 (Telegram, AI Agent)', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.telegramTrigger', version: 1.1, config: { parameters: { updates: ['message'], additionalFields: {} }, credentials: {
    ...
    }, position: [320, -80], name: 'Start: Receive Message on Telegram' } }))
  .then(switchCase([node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
      fileId: '={{ $json.message.voice.file_id }}',
      resource: 'file'
    }, credentials: {
      telegramApi: { id: 'credential-id', name: 'telegramApi Credential' }
    }, position: [760, -280], name: 'Get Voice Message' } }), node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
          {
    ...
    }, position: [980, -80], name: 'Set User Input' } }), node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
          {
    ...
    }, position: [760, 120], name: 'Set Error Message' } })], { version: 3.2, parameters: {
      rules: {
        values: [
          {
            outputKey: 'Audio',
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: { options: {}, resource: 'audio', operation: 'transcribe' }, credentials: {
    ...
    }, position: [980, -280], name: 'Transcribe Voice to Text' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.8, config: { parameters: {
      ...
      promptType: 'define'
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'AI Model: GPT-4o' } }), memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryBufferWindow', version: 1.3, config: { parameters: {
          sessionKey: '={{ $(\'Start: Receive Message on Telegram\').item.json.message.chat.id }}',
          sessionIdType: 'customKey',
          contextWindowLength: 10
        }, name: 'Memory for Chat Context' } }), tools: [tool({ type: 'n8n-nodes-base.googleSheetsTool', version: 4.5, config: { parameters: {
          columns: {
            value: {
        ...
        }, name: 'Optional: Log Ideas to Google Sheets' } })] }, position: [1232, -180], name: 'Generate Reels Scenario with AI' } }))
  .then(node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
      ...
      telegramApi: { id: 'credential-id', name: 'telegramApi Credential' }
    }, position: [1640, -180], name: 'Send Scenario to Telegram' } }))
  .then(node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
      text: '={{ $json.output }}',
      chatId: '={{ $(\'Start: Receive Message on Telegram\').item.json.message.chat.id }}',
      additionalFields: { appendAttribution: false }
