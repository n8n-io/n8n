return workflow('L9nteAq0NLYqIGxH', 'RAG Pipeline', { executionOrder: 'v1' })
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.vectorStoreQdrant', version: 1.2, config: { parameters: {
      mode: 'insert',
      options: {},
      ...
      qdrantApi: { id: 'credential-id', name: 'qdrantApi Credential' }
        ...
        } } }), documentLoader: documentLoader({ type: '@n8n/n8n-nodes-langchain.documentDefaultDataLoader', version: 1, config: { parameters: { options: {}, dataType: 'binary' }, subnodes: { textSplitter: textSplitter({ type: '@n8n/n8n-nodes-langchain.textSplitterRecursiveCharacterTextSplitter', version: 1, config: { parameters: { options: {}, chunkSize: 200, chunkOverlap: 50 }, name: 'Recursive Character Text Splitter' } }) }, name: 'Default Data Loader' } }) }, position: [220, 0], name: 'Qdrant Vector Store' } }))
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.chatTrigger', version: 1.1, config: { parameters: { options: {} }, position: [940, -20], name: 'When chat message received' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2, config: { parameters: {
    ...
    }, subnodes: { memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryBufferWindow', version: 1.3, config: { name: 'Simple Memory' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOllama', version: 1, config: { parameters: { options: {} }, credentials: {
          ollamaApi: { id: 'credential-id', name: 'ollamaApi Credential' }
        }, name: 'Ollama Chat Model' } }), tools: [tool({ type: '@n8n/n8n-nodes-langchain.vectorStoreQdrant', version: 1.2, config: { parameters: {
