return workflow('IIQhSqMWv4u6e0Jh', 'Create Video with Sora2 Pro and Upload to Youtube', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-528, 976], name: 'When clicking ‘Test workflow’' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
    ...
    }, position: [-304, 976], name: 'Get new video' } }))
  ...
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [144, 976], name: 'Create Video' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 60 }, position: [352, 976], name: 'Wait 60 sec.' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [544, 976], name: 'Get status' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [-528, 1344], name: 'Get Url Video' } }), node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 60 }, position: [352, 976], name: 'Wait 60 sec.' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
    ...
    }, position: [-288, 1344], name: 'Generate title' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: '={{ $(\'Get Url Video\').item.json.video.url }}',
      options: {}
    }, position: [96, 1344], name: 'Get File Video' } }))
  .add(node({ type: 'n8n-nodes-base.googleDrive', version: 3, config: { parameters: {
    ...
    }, position: [384, 1232], name: 'Upload Video' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
      columns: {
        value: {
    ...
    }, position: [560, 1232], name: 'Update result' } }))
  ...
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
    ...
    }, position: [560, 1456], name: 'Update Youtube URL' } }))
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{ field: 'minutes' }] } }, position: [-528, 832] } }))
  .add(sticky('# Generate AI Videos with Sora2 Pro, Save to Google Drive and Upload to YouTube\n\nThis workflow allows users to **generate AI videos** using **OpenAI Sora2 Pro**, save them to **Google Drive**, generate optimized YouTube titles with GPT-5, and **automatically upload them to YouTube** . The entire process is triggered from a Google Sheet that acts as the central interface for input and output.\n\nIT automates video creation, uploading, and tracking, ensuring seamless integration between Google Sheets, Google Drive, OpenAI Sora2 Pro, and YouTube.\n\n\n\n\n', { name: 'Sticky Note3', color: 3, position: [-528, -336], width: 740, height: 280 }))
  .add(sticky('## STEP 1 - GOOGLE SHEET\nCreate a [Google Sheet like this](https://docs.google.com/spreadsheets/d/1pcoY9N_vQp44NtSRR5eskkL5Qd0N0BGq7Jh_4m-7VEQ/edit?usp=sharing).\n\nPlease insert:\n- in the "PROMPT" column the accurate description of the video you want to create\n- in the "DURATION" column the lenght of the video you want to create\n\nLeave the "VIDEO" column unfilled. It will be inserted by the system once the video has been created', { name: 'Sticky Note4', position: [-528, -16], width: 740, height: 200 }))
  .add(sticky('## STEP 4 - MAIN FLOW\nStart the workflow manually or periodically by hooking the "Schedule Trigger" node. It is recommended to set it at 5 minute intervals.', { name: 'Sticky Note5', position: [-528, 656], width: 740, height: 100 }))
  .add(sticky('## STEP 2 - GET API KEY (YOURAPIKEY)\nCreate an account [here](https://fal.ai/) and obtain API KEY.\nIn the node "Create Image" set "Header Auth" and set:\n- Name: "Authorization"\n- Value: "Key YOURAPIKEY"', { name: 'Sticky Note6', position: [-528, 224], width: 740, height: 140 }))
  .add(sticky('Set API Key created in Step 2', { name: 'Sticky Note7', position: [96, 912], width: 180, height: 200 }))
  .add(sticky('Set YOUR_USERNAME in Step 3', { name: 'Sticky Note', position: [352, 1392], width: 180, height: 200 }))
  .add(sticky('## STEP 3 - Upload video on Youtube\n- Find your API key in your [Upload-Post Manage Api Keys](https://www.upload-post.com/?linkId=lp_144414&sourceId=n3witalia&tenantId=upload-post-app) 10 FREE uploads per month\n- Set the the "Auth Header":\n-- Name: Authorization\n-- Value: Apikey YOUR_API_KEY_HERE\n- Create profiles to manage your social media accounts. The "Profile" you choose will be used in the field YOUR_USRNAME (eg. test1 or test2).  ', { name: 'Sticky Note8', position: [-528, 416], width: 740, height: 200 }))