const wf = workflow(
		...
		node({
			...
			config: {
				parameters: {
					...
					options: {},
					jsonBody:
						'={{ { "image_url": $(\'Build Public Image URL\').first().json.data.url.replace(/^http:\\/\\/tmpfiles\\.org\\/(\\d+)\\/(.*)$/i, \'https://tmpfiles.org/dl/$1/$2\'), "audio_url": $json.data.url.replace(/^http:\\/\\/tmpfiles\\.org\\/(\\d+)\\/(.*)$/i, \'https://tmpfiles.org/dl/$1/$2\'), "resolution": "480p" } }}',
					sendBody: true,
					sendHeaders: true,
					specifyBody: 'json',
					headerParameters: {
	...
	.add(
		sticky(
			...
			{ name: 'Step 3 - AI Processing', color: 5, position: [-3264, 48], width: 1836, height: 484 },
		),
	)
	.add(
		sticky(
	...
	.add(
		sticky(
			...
			{ name: 'Step 5 - Publishing', color: 5, position: [-3264, 528], width: 2124, height: 660 },
		),
	)
	.add(
		sticky(
			...
			{ name: 'How It Works', color: 4, position: [-3696, 640], width: 396, height: 546 },
		),
	);
