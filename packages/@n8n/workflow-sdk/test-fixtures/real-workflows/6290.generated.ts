return workflow('wnDz3dZ455lkg5la', 'Website Chatbot Agent', {
    timezone: 'America/Chicago',
    callerPolicy: 'workflowsFromSameOwner',
    executionOrder: 'v1'
  })
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.chatTrigger', version: 1.1, config: { parameters: { mode: 'webhook', public: true, options: {} }, position: [60, 80], name: 'When chat message received' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.7, config: { parameters: {
      options: {
        ...
        }, name: 'RAGagent' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolWorkflow', version: 2, config: { parameters: {
          name: 'ticketAgent',
          workflowId: {
            __rl: true,
            mode: 'list',
        ...
        }, name: 'ticketAgent' } }), tool({ type: '@n8n/n8n-nodes-langchain.toolWorkflow', version: 2, config: { parameters: {
          name: 'calendarAgent',
          workflowId: {
            __rl: true,
            mode: 'list',
        ...
        }, name: 'calendarAgent' } })], memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryBufferWindow', version: 1.3, config: { parameters: { contextWindowLength: 10 }, name: 'Simple Memory' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
          model: {
            __rl: true,
            mode: 'list',
            value: 'gpt-4o-mini',
