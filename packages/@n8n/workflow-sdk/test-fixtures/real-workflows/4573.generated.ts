return workflow('95RHN758KyIlB84T', 'Google Maps business scraper with contact extraction via Apify and Firecrawl', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.1, config: { parameters: {
      rule: { interval: [{ field: 'minutes', minutesInterval: 30 }] }
    }, position: [0, -125] } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.2, config: { parameters: {
      options: {},
    ...
    }, position: [220, -125], name: 'Read Pending Queries' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [440, -125], name: 'Start Apify Scraping Job' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { position: [660, -125], name: 'Wait for Job Succeed' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpQueryAuth: { id: 'credential-id', name: 'httpQueryAuth Credential' },
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [880, -200], name: 'Check Scraping Status' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { position: [660, -125], name: 'Wait for Job Succeed' } }), node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpQueryAuth: { id: 'credential-id', name: 'httpQueryAuth Credential' },
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [1320, -125], name: 'Fetch Scraped Results' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
    ...
    }, position: [1540, -125], name: 'Save Business Data' } }))
  .then(node({ type: 'n8n-nodes-base.filter', version: 2.2, config: { parameters: {
    ...
    }, position: [1760, -125], name: 'Filter Businesses with Websites' } }))
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: {} }, position: [1980, -125], name: 'Batch Processing Logic' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [2260, -220], name: 'Scrape Website Content' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [2480, -220], name: 'Extract Contact Information' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
        value: {
          emails: '={{ $json.emails }}',
    ...
    }, position: [2700, -220], name: 'Save Contact Details' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
    ...
    }, position: [2920, -160], name: 'Mark as Processed' } }))
  .add(sticky('## 🚀 INITIALIZATION PHASE\n- Triggers every 30 minutes\n- Reads unprocessed records from [Google Sheet](https://docs.google.com/spreadsheets/d/1DHezdcetT0c3Ie1xB3z3jDc5WElsLN87K4J9EQDef9g/edit?usp=sharing)\n- Starts Google Places scraper for restaurants\n- Waits for completion', { name: 'Sticky Note', color: 4, position: [20, -360], width: 640, height: 180 }))
  .add(sticky('## 📊 DATA COLLECTION PHASE\n- Monitors scraper job status\n- Loops until job completes\n- Fetches scraped restaurant data\n- Saves to "Data" sheet in [Google Sheets](https://docs.google.com/spreadsheets/d/1DHezdcetT0c3Ie1xB3z3jDc5WElsLN87K4J9EQDef9g/edit?usp=sharing)', { name: 'Sticky Note1', position: [940, -420], width: 660, height: 180 }))
  .add(sticky('## 🌐 WEBSITE PROCESSING PHASE\n- Filters restaurants with valid websites\n- Loops through each website\n- Scrapes website content with Firecrawl\n- Extracts contact information (emails, social media)\n- Saves to "Details" sheet and marks as processed', { name: 'Sticky Note2', color: 6, position: [1940, -460], width: 740, height: 200 }))
  .add(sticky('## 🔄 LOOPS\n**Status Check Loop**: Continues until scraper job is complete\n**Website Processing Loop**: Processes each restaurant website individually', { name: 'Sticky Note3', position: [2260, 80], width: 580, height: 120 }))