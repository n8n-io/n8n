return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-400, 672], name: 'Start' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [32, 672], name: 'Sample Data' } }))
  .then(node({ type: 'n8n-nodes-base.aggregate', version: 1, config: { parameters: { options: {}, aggregate: 'aggregateAllItemData' }, position: [368, 672], name: 'Process Data' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [592, 1024], name: 'DataVisualization' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [800, 1024], name: 'Process Visualization Data' } }))
  .then(node({ type: 'n8n-nodes-base.convertToFile', version: 1.1, config: { parameters: {
    ...
    }, position: [1008, 1024], name: 'Convert to HTML File' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
    ...
    }, position: [1488, 1024], name: 'Send DataVisualization' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      genericAuthType: 'httpHeaderAuth'
    }, position: [592, 672], name: 'DataAnalysis' } }))
  .then(node({ type: 'n8n-nodes-base.markdown', version: 1, config: { parameters: {
      ...
      destinationKey: 'html'
    }, position: [1008, 672], name: 'Convert markdown to HTML' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      message: '={{ $json.html }}',
      options: {},
      subject: 'n8n-email'
      ...
      gmailOAuth2: { id: 'credential-id', name: 'gmailOAuth2 Credential' }
    }, position: [1488, 672], name: 'Send DataAnalysis message' } }))
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      genericAuthType: 'httpHeaderAuth'
    }, position: [592, 848], name: 'DataInterpretation' } }))
  .then(node({ type: 'n8n-nodes-base.markdown', version: 1, config: { parameters: {
      mode: 'markdownToHtml',
      options: {
        tables: true,
      ...
      markdown: '={{ $json.data.parseJson().data }}',
      destinationKey: 'html'
    }, position: [1008, 848], name: 'Convert markdown to HTML 2' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      message: '={{ $json.html }}',
      options: {},
      subject: 'n8ntest'
      ...
      gmailOAuth2: { id: 'credential-id', name: 'gmailOAuth2 Credential' }
    }, position: [1488, 848], name: 'Send DataInterpretation message' } }))
  .add(node({ type: 'n8n-nodes-base.mySql', version: 2.5, config: { parameters: {
      query: 'select * from orders limit 100',
      options: {},
      operation: 'executeQuery'
    }, position: [32, 864], name: 'Get data from database' } }))
