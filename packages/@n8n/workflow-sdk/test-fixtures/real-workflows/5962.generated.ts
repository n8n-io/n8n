return workflow('FJU3f2ANp6c9kw2k', '19 Monitor Keyword Rankings', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{ triggerAtHour: 9 }] } }, name: '🕒 Trigger: Run Daily/Weekly' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [200, 0], name: '📝 Input: Keyword & Domain' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2, config: { parameters: {
      ...
      hasOutputParser: true
    }, subnodes: { tools: [tool({ type: 'n8n-nodes-mcp.mcpClientTool', version: 1, config: { parameters: {
          toolName: 'search_engine',
          operation: 'executeTool',
          toolParameters: '={{ /*n8n-auto-generated-fromAI-override*/ $fromAI(\'Tool_Parameters\', ``, \'json\') }}'
        }, credentials: {
          mcpClientApi: { id: 'credential-id', name: 'mcpClientApi Credential' }
        }, name: 'MCP Client' } })], model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
          model: { __rl: true, mode: 'list', value: 'gpt-4o-mini' },
          options: {}
        }, credentials: {
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Chat Model' } }), outputParser: outputParser({ type: '@n8n/n8n-nodes-langchain.outputParserAutofixing', version: 1, config: { parameters: { options: {} }, subnodes: { model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
          model: { __rl: true, mode: 'list', value: 'gpt-4o-mini' },
          options: {}
        }, credentials: {
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        ...
        }, name: 'Structured Output Parser1' } }) }, name: 'Auto-fixing Output Parser' } }) }, position: [480, 0], name: '🤖 SERP Scraper Agent (MCP)' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [960, 0], name: '🧠 Format SERP Results' } }))
