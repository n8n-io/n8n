return workflow('OcdRRO25OfTnrILJ', 'Blog Post : Tendencias Ecosistema', { executionOrder: 'v1' })
  ...
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [-1260, 610], name: 'Research Topic- Perplexity' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-1040, 610], name: 'Get Title, Content, and Image FileName' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
    ...
    }, position: [-820, 610], name: 'Message a model' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [-460, 610], name: 'Leonardo: Create Post Image' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpBearerAuth: { id: 'credential-id', name: 'httpBearerAuth Credential' }
    }, position: [-240, 610], name: 'Get Leonardo Image Status' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      options: {}
    }, position: [200, 760], name: 'Get Leonardo Image' } }), node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { position: [200, 560] } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [420, 760], name: 'Upload Image to Wordpress' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpBasicAuth: { id: 'credential-id', name: 'httpBasicAuth Credential' }
    }, position: [640, 760], name: 'Agregar ALT a la Imagen' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpBasicAuth: { id: 'credential-id', name: 'httpBasicAuth Credential' }
    }, position: [860, 760], name: 'Crear Post en Wordpress' } }))
  ...
  .add(sticky('## Publicamos\nSe genera la publicación incluyendo la imagen y la categoría.', { name: 'Sticky Note4', position: [800, 560] }))
  .add(sticky('## Journal\nGeneramos una lista de las publicaciones que hemos hecho a través de este flujo', { name: 'Sticky Note5', position: [1060, 960] }))