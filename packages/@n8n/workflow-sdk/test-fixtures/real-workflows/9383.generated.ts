return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.jotFormTrigger', version: 1, config: { parameters: { form: '252801824783057' }, credentials: {
      jotFormApi: { id: 'credential-id', name: 'jotFormApi Credential' }
    }, position: [-1024, 256], name: 'JotForm Trigger' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.7, config: { parameters: {
    ...
    }, position: [-800, 256], name: 'Append or update row in sheet' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.2, config: { parameters: {
      ...
      promptType: 'define'
        ...
        }, name: 'Google Gemini Chat Model' } }) }, position: [-528, 256], name: 'AI Agent' } }))
  .then(node({ type: 'n8n-nodes-base.whatsApp', version: 1, config: { parameters: {
    ...
    }, position: [-144, 256], name: 'Send message' } }))
  .add(trigger({ type: 'n8n-nodes-base.whatsAppTrigger', version: 1, config: { parameters: {
      options: { messageStatusUpdates: ['delivered'] },
      updates: ['messages']
    }, credentials: {
      whatsAppTriggerApi: { id: 'credential-id', name: 'whatsAppTriggerApi Credential' }
    }, position: [240, 320], name: 'WhatsApp Trigger' } }))
  .then(ifBranch([null, node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.2, config: { parameters: {
      ...
      options: {},
      promptType: 'define'
    }, subnodes: { tools: [tool({ type: 'n8n-nodes-base.googleCalendarTool', version: 1.3, config: { parameters: {
        ...
        }, name: 'Create an event' } }), tool({ type: 'n8n-nodes-base.googleSheetsTool', version: 4.7, config: { parameters: {
          options: {},
          ...
          googleApi: { id: 'credential-id', name: 'googleApi Credential' }
        }, name: 'Know about the user enquiry' } }), tool({ type: 'n8n-nodes-base.googleCalendarTool', version: 1.3, config: { parameters: {
          options: {},
        ...
        }, name: 'GET MANY EVENTS OF DAY THE USER ASKED' } })], memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryPostgresChat', version: 1.3, config: { parameters: {
          sessionKey: '={{ $(\'If\').item.json.contacts[0].wa_id }}',
          sessionIdType: 'customKey'
        }, credentials: {
          postgres: { id: 'credential-id', name: 'postgres Credential' }
        }, name: 'Postgres Chat Memory' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatGoogleGemini', version: 1, config: { parameters: { options: {} }, credentials: {
          googlePalmApi: { id: 'credential-id', name: 'googlePalmApi Credential' }
        }, name: 'Google Gemini Chat Model1' } }) }, position: [912, 336], name: 'AI Agent1' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
