const wf = workflow('', '')
						...
						values: [
							{
								...
								placeholder:
									'https://www.google.com.br/maps/@-27.0969036,-52.6273034,14z?hl=pt-BR&entry=ttu&g_ep=EgoyMDI1MDcwOS4wIKXMDSoASAFQAw%3D%3D',
								requiredField: true,
							},
							{
	...
	.then(
		node({
			...
			version: 1,
			config: { position: [-1840, 384], name: 'No Operation, do nothing4' },
		}),
	)
	.then(
		node({
	...
	.then(
		node({
			...
			version: 1,
			config: { parameters: { maxItems: 15 }, position: [-464, 384], name: 'Set Limit' },
		}),
	)
	.then(
		node({
	...
	.then(
		node({
			type: 'n8n-nodes-base.wait',
			version: 1.1,
			config: { parameters: { amount: 60 }, position: [-1584, 640], name: 'Wait5' },
		}),
	)
	.then(
		node({
	...
	.add(
		trigger({
			...
			version: 1,
			config: { position: [-2720, -96], name: 'When clicking ‘Execute workflow’' },
		}),
	)
	.then(
		node({
			...
			config: {
				parameters: {
					query:
						'CREATE TABLE business_scraping_result (\n    place_id text PRIMARY KEY,\n    page_summary TEXT,\n    name TEXT,\n    category TEXT,\n    address TEXT,\n    description TEXT,\n    google_url TEXT,\n    website TEXT,\n    phone_number TEXT,\n    reviews_count INTEGER,\n    rating NUMERIC(2,1),\n    services_provided TEXT[],\n    top_reviews JSONB,\n    sales_helper TEXT\n);\n',
					options: {},
	...
	.add(
		sticky(
			...
			{ color: 7, position: [-2096, 272], width: 848, height: 592 },
		),
	)
	.add(
		sticky('### ☝️ Bright Data MCP\nPlace your token here', {
			name: 'Sticky Note1',
			color: 3,
			position: [480, 352],
			width: 192,
	...
	.add(
		sticky('## Run this to create your Supabase table', {
			name: 'Sticky Note2',
			color: 4,
			position: [-2768, -336],
			width: 528,
	...
	.add(
		sticky(
			'The `zoom_level` controls the radius of your search. A lower zoom means a higher radius.\n\nThe `dataset_id` refers to the [Google Maps scraper](https://brightdata.com/cp/scrapers/api/gd_m8ebnr0q2qlklc02fz/location/) from BrightData',
			{ name: 'Sticky Note3', color: 7, position: [-2384, 224], width: 256, height: 336 },
		),
	)
	.add(
		sticky("Limit the amount of items to save in your database, if you'd like.", {
			name: 'Sticky Note4',
			color: 7,
			position: [-528, 304],
			width: 224,
	...
	.add(
		sticky(
			'A loop is used here to ensure that even if an error occurs with one of the items, all previous items will still be saved in the database.',
			{ name: 'Sticky Note6', color: 7, position: [-288, 240], width: 208, height: 288 },
		),
	)
	.add(
		sticky(
			'To connect to Supabase using Postgres credentials, just click the `Connect` button in Supabase header and use the `Transaction pooler` parameters.\n\n[Official documentation](https://supabase.com/docs/guides/database/connecting-to-postgres)',
			{ name: 'Sticky Note7', color: 7, position: [1376, 224], width: 272, height: 320 },
		),
	)
	.add(
		sticky(
			'To connect to Supabase using Postgres credentials, just click the `Connect` button in Supabase header and use the `Transaction pooler` parameters.\n\n[Official documentation](https://supabase.com/docs/guides/database/connecting-to-postgres)',
			{ name: 'Sticky Note8', color: 7, position: [-2544, -256], width: 272, height: 320 },
		),
	)
	.add(
		sticky(
			...
			{ name: 'Sticky Note5', position: [-3216, -336], width: 416, height: 848 },
		),
	)
	.add(
		sticky(
	...
	.add(
		sticky(
			...
			{ name: 'Sticky Note16', color: 4, position: [1696, 448], width: 620, height: 80 },
		),
	);
