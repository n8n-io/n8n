const wf = workflow('', '')
	...
	.add(
		trigger({
			...
			config: {
				parameters: {
					workflowInputs: {
						values: [{ name: 'start_date' }, { name: 'end_date' }, { name: 'status' }],
					},
				},
				position: [280, 640],
					...
					queryParameters: {
						parameters: [
							{ name: 'sheet', value: 'Sheet1' },
							{
								name: 'tq',
								value:
									'=SELECT * WHERE A >= DATE "{{ $json.start_date }}" AND A <= DATE "{{ $json.end_date }}"',
							},
						],
					},
	...
	.add(
		sticky(
			'To send all the items back to the AI, we need to finish with everything aggregated into one single item.\n\nOtherwise it will respond with one item at a time, and the AI will only get the first item that arrives.',
			{ color: 7, position: [1220, 400], width: 220, height: 400 },
		),
	)
	.add(
		sticky(
			'This node sends a custom HTTP Request to the Google Sheets API.\n\nFiltering by date range in the Google Sheets API is very complicated.\n\nThis node solves that problem.\n\nBut doing the same in a database is much simpler. A tool could do it without needing a sub-workflow.',
			{ name: 'Sticky Note1', color: 7, position: [460, 400], width: 300, height: 400 },
		),
	)
	.add(
		sticky(
			'The output from this complex request is also messy.\n\nSo we use some code generated by ChatGPT to transform the data into JSON objects.',
			{ name: 'Sticky Note2', color: 7, position: [780, 400], width: 220, height: 400 },
		),
	)
	.add(
		sticky(
			...
			{ name: 'Sticky Note3', color: 4, position: [-360, -340], width: 320, height: 340 },
		),
	)
	.add(
		sticky(
			'## Copy this Sheets file to your Google Drive\nhttps://docs.google.com/spreadsheets/d/18A4d7KYrk8-uEMbu7shoQe_UIzmbTLV1FMN43bjA7qc/edit?gid=0#gid=0',
			{ name: 'Sticky Note4', color: 4, position: [-780, -340], width: 400 },
		),
	)
	.add(
		sticky(
			'### 👈\nThe Calculator is a tool that allows an agent to run mathematical calculations.',
			{ name: 'Sticky Note5', color: 7, position: [940, 60], width: 200, height: 140 },
		),
	)
	.add(
		sticky(
			...
			{ name: 'Sticky Note6', color: 4, position: [-780, -160], width: 400 },
		),
	)
	.add(
		sticky(
			...
			{ name: 'Sticky Note7', color: 7, position: [120, 20], width: 170, height: 260 },
		),
	)
	.add(
		sticky(
			'## 👆\nThis is a short term memory. It will remember the 5 previous interactions during the chat',
			{ name: 'Sticky Note8', color: 7, position: [340, 20], width: 150, height: 260 },
		),
	)
	.add(
		sticky(
			"The **AI Tools Agent** has access to all the tools at the same time. It uses the name and description to decide when to use each tool.\n\nNotice I'm using `$fromAI` function in all of them.\n\nSee documentations **[here](https://docs.n8n.io/advanced-ai/examples/using-the-fromai-function/)**",
			{ name: 'Sticky Note9', color: 7, position: [1160, -320], width: 340, height: 180 },
		),
	)
	.add(
		sticky(
			...
			{ name: 'Sticky Note11', color: 7, position: [1160, -120], width: 340, height: 320 },
		),
	)
	.add(
		sticky(
			...
			{ name: 'Sticky Note12', color: 7, position: [120, 340], width: 1380, height: 520 },
		),
	)
	.add(
		sticky('## Change the URL of the Sheets file in all the Sheets tools 👇', {
			name: 'Sticky Note13',
			color: 4,
			position: [820, -540],
			width: 300,
	...
	.add(
		sticky('## 👆 Change the URL of the Sheets file', {
			name: 'Sticky Note14',
			color: 4,
			position: [500, 820],
			width: 260,
	...
	.add(
		sticky(
			...
			{ name: 'Sticky Note15', position: [-780, 680], width: 740, height: 180 },
		),
	)
	.add(
		sticky(
			...
			{ name: 'Sticky Note16', color: 4, position: [-760, 560], width: 700, height: 80 },
		),
	);
