return workflow('CWailec0QgZyOq0o', 'Create Structured Ebooks in Minutes with Google Gemini Flash 2.0 & n8n', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-720, 0], name: 'When clicking ‘Execute workflow’' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-500, 0], name: 'Set the Input Fields' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.7, config: { parameters: {
      text: '={{ $json.Title }}\n\nMake sure to output as JSON',
      batching: {},
      promptType: 'define',
      hasOutputParser: true
    }, subnodes: { outputParser: outputParser({ type: '@n8n/n8n-nodes-langchain.outputParserStructured', version: 1.3, config: { parameters: {
          schemaType: 'manual',
        ...
        }, name: 'Structured Output Parser' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatGoogleGemini', version: 1, config: { parameters: { options: {}, modelName: 'models/gemini-2.0-flash-exp' }, credentials: {
          googlePalmApi: { id: 'credential-id', name: 'googlePalmApi Credential' }
        }, name: 'Google Gemini Chat Model for Ebook thought creation' } }) }, position: [-320, 0], name: 'Ebook Thought Creation' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
      jsCode: 'return $input.first().json.output.structure.chapters'
    }, position: [40, 0] } }))
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: {} }, position: [360, 0], name: 'Loop Over Items' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [700, 20], name: 'Set the title, objective' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.7, config: { parameters: {
      ...
      promptType: 'define'
          ...
          googlePalmApi: { id: 'credential-id', name: 'googlePalmApi Credential' }
        }, name: 'Google Gemini Chat Model' } }) }, position: [920, 20], name: 'Generate Detailed Chapter Content' } }))
  ...
  .add(sticky('## Ebook Detailed Chapter Creation', { name: 'Sticky Note4', color: 5, position: [200, -140], width: 1060, height: 520 }))
