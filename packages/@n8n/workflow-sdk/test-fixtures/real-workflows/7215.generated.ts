return workflow('EmfC3k9BlwxWhpVL', 'Own Data Store in Google Sheet', { executionOrder: 'v1' })
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.1, config: { parameters: {
      ...
      promptType: 'define'
        ...
        }, name: 'Google Gemini Chat Model' } }), tools: [tool({ type: 'n8n-nodes-base.googleSheetsTool', version: 4.6, config: { parameters: {
          options: {},
          sheetName: {
            __rl: true,
            mode: 'list',
        ...
        }, name: 'Get row(s) in sheet in Google Sheets' } })] }, position: [-368, -480], name: 'AI Agent' } }))
  .then(node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
      ...
      telegramApi: { id: 'credential-id', name: 'telegramApi Credential' }
    }, position: [-16, -480], name: 'Send a text message' } }))
  ...
  .then(ifBranch([node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      sendBody: true,
      specifyBody: 'json'
    }, position: [144, 48] } }), node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: { url: '={{ $json[\'URL \'] }}', options: {} }, position: [144, 288], name: 'HTTP Request1' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.informationExtractor', version: 1.2, config: { parameters: {
      ...
      attributes: {
        attributes: [
          ...
          googlePalmApi: { id: 'credential-id', name: 'googlePalmApi Credential' }
        }, name: 'Google Gemini Chat Model2' } }) }, position: [352, 48], name: 'Information Extractor1' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
    ...
    }, position: [736, 48], name: 'Append row in sheet' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
        value: {
    ...
    }, position: [944, 48], name: 'Append or update row in sheet2' } }))
  .then(node({ type: 'n8n-nodes-base.markdown', version: 1, config: { parameters: { html: '={{ $json.data }}', options: {} }, position: [352, 288] } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.informationExtractor', version: 1.2, config: { parameters: {
      ...
      attributes: {
        attributes: [
          ...
          googlePalmApi: { id: 'credential-id', name: 'googlePalmApi Credential' }
        }, name: 'Google Gemini Chat Model1' } }) }, position: [560, 288] } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
        value: {
          Data: '={{ $json.output[\'Article Refined Data\'] }}',
    ...
    }, position: [928, 288], name: 'Append row in sheet1' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
        value: {
          'URL ': '={{ $(\'Google Sheets Trigger\').item.json[\'URL \'] }}',
    ...
    }, position: [1136, 288], name: 'Append or update row in sheet' } }))
  .add(sticky('# Requesting Data', { name: 'Sticky Note', color: 6, position: [-656, -576], width: 896, height: 432 }))
  .add(sticky('# Storing Data through YouTube Video URL / Article Link', { name: 'Sticky Note1', color: 6, position: [-656, -32], width: 2080, height: 592 }))
