return workflow('Hi3KUTF6mlwuZPFR', 'Physician Profile Enricher', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [1600, 176], name: 'On-Demand Execution' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [1808, 176], name: 'Define Location' } }))
  .then(node({ type: 'n8n-nodes-browseract.browserAct', version: 1, config: { parameters: {
      type: 'WORKFLOW',
      timeout: 7200,
    ...
    }, position: [2032, 176], name: 'Run BrowserAct Workflow' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [2368, 176], name: 'Splitting Items' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.7, config: { parameters: {
    ...
    }, position: [2576, 176], name: 'Update Leads' } }))
  .then(node({ type: 'n8n-nodes-base.slack', version: 2.4, config: { parameters: {
    ...
    }, position: [2800, 176], name: 'Alert Team' } }))
  .add(sticky('## ⚡ Workflow Overview & Setup\n\n**Summary:** Automate the extraction of physician profiles (Name, Address, Specialty) from **Healow** based on location, archiving data to Google Sheets and notifying via Slack.\n\n### Requirements\n* **Credentials:** BrowserAct, Google Sheets, Slack.\n* **Mandatory:** BrowserAct API (Template: **Physician Profile Enricher**)\n\n### How to Use\n1. **Credentials:** Configure your API keys for BrowserAct, Google Sheets, and Slack.\n2. **BrowserAct Template:** Ensure you have the **Healow** template saved in your BrowserAct account.\n3. **Google Sheets:** Create a sheet named "Physician Profile" with headers: `Name`, `Address`, `Specialty`.\n4. **Configuration:** Update the **Define Location** node with your target City and State.\n\n### Need Help?\n[How to Find Your BrowseAct API Key & Workflow ID](https://docs.browseract.com)\n[How to Connect n8n to Browseract](https://docs.browseract.com)\n[How to Use & Customize BrowserAct Templates](https://docs.browseract.com)', { name: 'Documentation', position: [1200, -336], width: 400, height: 464 }))
  .add(sticky('### 🏥 Step 1: Target & Scrape\n\nSets the search parameters (City and State) and executes the **Healow** BrowserAct automation to extract physician data from the target website.', { name: 'Step 1 Explanation', color: 7, position: [1616, 16], width: 536, height: 124 }))
  .add(sticky('### 💾 Step 2: Process & Archive\n\nParses the raw scraped string into structured JSON objects, updates the Google Sheet database with new records, and sends a Slack notification upon success.', { name: 'Step 2 Explanation', color: 7, position: [2368, 368], width: 540, height: 124 }))
  .add(sticky('### Google Sheet Headers\nTo use this workflow, create a Google Sheet with the following headers:\n* Name\n* Specialty\n* Address', { name: 'Sticky Note', color: 3, position: [2496, 48], width: 256, height: 288 }))
  .add(sticky('@[youtube](DZ_Jq_b2-Ww)', { name: 'Sticky Note1', color: 6, position: [1616, -336], width: 528, height: 304 }))