const wf = workflow(
	'qgOiQsPSKD08eiB3',
	'Extract Structured Data from HackerNews, Export to Google Docs with Google Gemini',
	{ executionOrder: 'v1' },
)
	.add(
		trigger({
			...
			version: 1,
			config: { position: [-380, 0], name: 'When clicking ‘Execute workflow’' },
		}),
	)
	.then(
		node({
	...
	.add(
		sticky(
			...
			{ name: 'Sticky Note1', position: [460, -320] },
		),
	)
	.add(
		sticky(
			'## Extract Structured Data from Hacker News, Export to Google Docs with Google Gemini\n\nHacker news data extraction with Google Gemini. Export the clean data to Google Document.',
	...
	.add(
		sticky('## Hacker News Data Extraction', {
			name: 'Sticky Note4',
			color: 5,
			position: [200, -140],
			width: 1060,
