return workflow('abSdfsaYgkssXX7g', 'Dynamically Selects Models Based on Input Type', { executionOrder: 'v1' })
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.chatTrigger', version: 1.1, config: { parameters: { options: {} }, position: [-528, -112], name: 'When chat message received' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.7, config: { parameters: {
      ...
      hasOutputParser: true
          ...
          schemaType: 'manual',
          inputSchema: '{\n	"type": "object",\n	"properties": {\n		"request_type": {\n			"type": "string"\n		}\n	}\n}'
        }, name: 'Structured Output Parser' } }) }, position: [-288, -112], name: 'Request Type' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.1, config: { parameters: {
      ...
      promptType: 'define'
        ...
        }, name: 'Simple Memory' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.modelSelector', version: 1, config: { parameters: {
          rules: {
            rule: [
              {
                conditions: {
        ...
        }, name: 'Opus 4' } }), languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenRouter', version: 1, config: { parameters: { model: 'perplexity/sonar', options: {} }, credentials: {
          openRouterApi: { id: 'credential-id', name: 'openRouterApi Credential' }
        }, name: 'Perplexity' } }), languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
          model: { __rl: true, mode: 'list', value: 'gpt-4.1-mini' },
          options: {}
        }, credentials: {
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'GPT 4.1 mini' } }), languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatGoogleGemini', version: 1, config: { parameters: {
          options: {},
          modelName: 'models/gemini-2.0-flash-thinking-exp'
        }, credentials: {
          googlePalmApi: { id: 'credential-id', name: 'googlePalmApi Credential' }
        }, name: 'Gemini Thinking Pro' } })] } } }) }, position: [160, -112], name: 'AI Agent' } }))
  .add(sticky('## AI Orchestrator: dynamically Selects Models Based on Input Type\n\nThis workflow is designed to intelligently **route user queries to the most suitable large language model (LLM)** based on the type of request received in a chat environment. It uses structured classification and model selection to optimize both performance and cost-efficiency in AI-driven conversations.\n\nIt dynamically routes requests to specialized AI models based on content type, optimizing response quality and efficiency.', { name: 'Sticky Note', position: [-528, -416], width: 624, height: 256 }))