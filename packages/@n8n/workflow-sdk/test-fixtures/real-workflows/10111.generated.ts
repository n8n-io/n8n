return workflow('uOMGyPBvAP5ytb64', 'AI-Powered Invoice Reminder & Payment Tracker for Finance & Accounting', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: {
      rule: {
        interval: [{ field: 'cronExpression', expression: '0 9 * * *' }]
      }
    }, position: [0, -304], name: 'Schedule Daily Check' } }))
  .then(node({ type: 'n8n-nodes-base.postgres', version: 2.4, config: { parameters: {
    ...
    }, position: [224, -304], name: 'Fetch Pending Invoices' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [672, -304], name: 'Calculate Reminder Logic' } }), null], { version: 2, parameters: {
      options: {},
      conditions: {
        options: {
          leftValue: '',
  ...
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [896, -304], name: 'Prepare AI Prompt' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.2, config: { parameters: { options: {} }, subnodes: { model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1, config: { parameters: {
          model: '=gpt-4o-mini',
          options: { maxTokens: 500, temperature: 0.7 }
        }, credentials: {
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'Generate Email' } }) }, position: [1120, -304], name: 'AI Agent For Generate Email Content' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [1472, -304], name: 'Format Email' } }))
  .then(node({ type: 'n8n-nodes-base.emailSend', version: 2.1, config: { parameters: {
    ...
    }, credentials: { smtp: { id: 'credential-id', name: 'smtp Credential' } }, position: [1696, -304], name: 'Send Email Reminder' } }))
  .then(node({ type: 'n8n-nodes-base.postgres', version: 2.4, config: { parameters: {
      ...
      postgres: { id: 'credential-id', name: 'postgres Credential' }
    }, position: [1920, -304], name: 'Update Reminder Status' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [2144, -304], name: 'Create Activity Log' } }))
  .then(node({ type: 'n8n-nodes-base.postgres', version: 2.4, config: { parameters: {
      ...
      postgres: { id: 'credential-id', name: 'postgres Credential' }
    }, position: [2368, -304], name: 'Save to Activity Log' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [2592, -304], name: 'Generate Daily Summary' } }))
  .then(node({ type: 'n8n-nodes-base.emailSend', version: 2.1, config: { parameters: {
      options: {},
      subject: '=Daily Invoice Reminder Report - {{ $now.toFormat(\'yyyy-MM-dd\') }}',
      toEmail: 'user@example.com',
      fromEmail: 'user@example.com'
    }, credentials: { smtp: { id: 'credential-id', name: 'smtp Credential' } }, position: [2816, -304], name: 'Send Summary to Finance Team' } }))
  .add(trigger({ type: 'n8n-nodes-base.webhook', version: 2, config: { parameters: {
      path: 'invoice-paid',
      options: {},
      httpMethod: 'POST',
      responseMode: 'responseNode'
    }, position: [-48, 368], name: 'Webhook: Payment Received' } }))
  .then(node({ type: 'n8n-nodes-base.postgres', version: 2.4, config: { parameters: {
      ...
      postgres: { id: 'credential-id', name: 'postgres Credential' }
    }, position: [208, 368], name: 'Update Payment Status' } }))
  .add(node({ type: 'n8n-nodes-base.respondToWebhook', version: 1.1, config: { parameters: {
      options: {},
      respondWith: 'json',
      responseBody: '={\n  "success": true,\n  "message": "Payment recorded successfully",\n  "invoice_number": "{{ $json.invoice_number }}",\n  "amount_paid": {{ $json.payment_amount }},\n  "status": "{{ $json.payment_status }}"\n}'
    }, position: [464, 144], name: 'Webhook Response' } }))
  ...
  .add(sticky('Records reminder activity for audit.\n', { name: 'Sticky Note5', position: [2096, -416], width: 192, height: 288 }))
  .add(sticky('Marks reminder as sent in the database.\n', { name: 'Sticky Note6', position: [1872, -416], width: 192, height: 288 }))
  .add(sticky('Sends the email via Gmail or SMTP.\n', { name: 'Sticky Note7', position: [1648, -416], width: 192, height: 288 }))
  .add(sticky('Converts AI text to a professional HTML email.\n', { name: 'Sticky Note8', position: [1424, -416], width: 192, height: 288 }))
  .add(sticky('Uses AI to draft reminder emails.\n', { name: 'Sticky Note9', position: [1104, -416], width: 272, height: 288 }))
  .add(sticky('Prepares a report of reminders sent.\n', { name: 'Sticky Note10', position: [2544, -416], width: 192, height: 288 }))
  .add(sticky('Emails summary to the finance team.\n', { name: 'Sticky Note11', position: [2768, -416], width: 192, height: 288 }))
  .add(sticky('Stores raw workflow data for review.\n', { name: 'Sticky Note12', position: [2320, -416], width: 192, height: 288 }))
  .add(sticky('Generates thank-you massage via AI.\n', { name: 'Sticky Note13', position: [432, 0], width: 192, height: 288 }))
  .add(sticky('Captures payment notifications.\n', { name: 'Sticky Note14', position: [-96, 240], width: 192, height: 288 }))
  .add(sticky('Updates invoice as “paid.”\n', { name: 'Sticky Note15', position: [160, 240], width: 192, height: 288 }))
  .add(sticky('Sends payment receipt to client.', { name: 'Sticky Note16', position: [448, 416], width: 192, height: 288 }))