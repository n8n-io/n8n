const wf = workflow('', '')
	.add(
		trigger({
			...
			version: 1,
			config: { position: [0, -520], name: 'When clicking "Execute Workflow"' },
		}),
	)
	.then(
		node({
			...
			config: {
				parameters: {
					operation: 'get',
					documentURL:
						'https://docs.google.com/document/d/1gvgp71e9edob8WLqFIYCdzC7kUq3pLO37VKb-a-vVW4/edit?tab=t.0',
				},
				credentials: {
					...
					values: {
						string: [
							...
							},
							{
								name: 'text',
								value:
									"=Prompt: {{ $('Receive Message on Telegram').item.json.message.text }}. Completion: {{ $('Knowledge Base Agent').item.json.output }}",
							},
	...
	.add(
		sticky(
			'Run this workflow manually to import and index Google Docs product documentation into MongoDB with vector embeddings for fast search.',
			{ color: 5, position: [-420, -500] },
		),
	)
	.add(
		sticky(
			'This workflow uses retrieval-augmented generation (RAG) to answer user questions by searching the MongoDB vector store and generating AI responses enhanced with Telegram-based RLHF feedback.',
			{ name: 'Sticky Note1', color: 4, position: [-420, 40], height: 180 },
		),
	)
	.add(
		sticky(
