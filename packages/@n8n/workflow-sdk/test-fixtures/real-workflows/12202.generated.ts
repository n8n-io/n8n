return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-656, 1872], name: 'When clicking ‘Execute workflow’' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-448, 1872], name: 'Set Text to Video Parameters' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, credentials: { httpBearerAuth: { id: 'vX52jw1dTyyouVZr', name: 'KIA.AI' } }, position: [-256, 1872], name: 'Submit Video Generation Request' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1, config: { parameters: { unit: 'seconds', amount: 5 }, position: [-16, 1872], name: 'Wait for Video Generation' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, credentials: { httpBearerAuth: { id: 'vX52jw1dTyyouVZr', name: 'KIA.AI' } }, position: [144, 1872], name: 'Check Video Generation Status' } }))
  .then(switchCase([node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      authentication: 'genericCredentialType',
      genericAuthType: 'httpBearerAuth'
    }, credentials: { httpBearerAuth: { id: 'vX52jw1dTyyouVZr', name: 'KIA.AI' } }, position: [-256, 1872], name: 'Submit Video Generation Request' } }), node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [608, 1840], name: 'Extract Video URL' } }), node({ type: 'n8n-nodes-base.wait', version: 1, config: { parameters: { unit: 'seconds', amount: 5 }, position: [-16, 1872], name: 'Wait for Video Generation' } }), node({ type: 'n8n-nodes-base.wait', version: 1, config: { parameters: { unit: 'seconds', amount: 5 }, position: [-16, 1872], name: 'Wait for Video Generation' } }), node({ type: 'n8n-nodes-base.wait', version: 1, config: { parameters: { unit: 'seconds', amount: 5 }, position: [-16, 1872], name: 'Wait for Video Generation' } })], { version: 3.2, parameters: {
      rules: {
        values: [
          {
            outputKey: 'fail',
  ...
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.3, config: { parameters: { url: '={{ $json.resultUrls[0] }}', options: {} }, position: [784, 1840], name: 'Download Video File' } }))
  .add(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-432, 2384], name: 'Set Prompt & Image Url' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: 'https://api.kie.ai/api/v1/jobs/createTask',
      method: 'POST',
      options: {},
      ...
      authentication: 'genericCredentialType',
      genericAuthType: 'httpBearerAuth'
    }, credentials: { httpBearerAuth: { id: 'vX52jw1dTyyouVZr', name: 'KIA.AI' } }, position: [-208, 2384], name: 'Submit Video Generation1' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1, config: { parameters: { unit: 'seconds', amount: 5 }, position: [16, 2384], name: 'Wait for Image-to-Video Generation' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: 'https://api.kie.ai/api/v1/jobs/recordInfo',
      options: {},
      sendQuery: true,
    ...
    }, credentials: { httpBearerAuth: { id: 'vX52jw1dTyyouVZr', name: 'KIA.AI' } }, position: [176, 2384], name: 'Check Video Status' } }))
  .then(switchCase([node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: 'https://api.kie.ai/api/v1/jobs/createTask',
      method: 'POST',
      options: {},
      ...
      authentication: 'genericCredentialType',
      genericAuthType: 'httpBearerAuth'
    }, credentials: { httpBearerAuth: { id: 'vX52jw1dTyyouVZr', name: 'KIA.AI' } }, position: [-208, 2384], name: 'Submit Video Generation1' } }), node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
      jsCode: '// n8n Function node\nconst items = $input.all();\nconst out = [];\n\nfor (const item of items) {\n  // Your HTTP node sometimes returns an array at top-level\n  const payload = item.json;\n  const first = Array.isArray(payload) ? payload[0] : payload;\n\n  // Get the stringified resultJson safely\n  const resultJsonStr = first?.data?.resultJson ?? \'\';\n\n  // Parse and extract\n  let resultUrls = [];\n  try {\n    const parsed = JSON.parse(resultJsonStr);\n    resultUrls = parsed?.resultUrls ?? [];\n  } catch (e) {\n    // leave resultUrls as []\n  }\n\n  out.push({ json: { resultUrls } });\n}\n\nreturn out;\n'
    }, position: [640, 2352], name: 'Video URL' } }), node({ type: 'n8n-nodes-base.wait', version: 1, config: { parameters: { unit: 'seconds', amount: 5 }, position: [16, 2384], name: 'Wait for Image-to-Video Generation' } }), node({ type: 'n8n-nodes-base.wait', version: 1, config: { parameters: { unit: 'seconds', amount: 5 }, position: [16, 2384], name: 'Wait for Image-to-Video Generation' } }), node({ type: 'n8n-nodes-base.wait', version: 1, config: { parameters: { unit: 'seconds', amount: 5 }, position: [16, 2384], name: 'Wait for Image-to-Video Generation' } })], { version: 3.2, parameters: {
      rules: {
        values: [
          {
            outputKey: 'fail',
  ...
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.3, config: { parameters: { url: '={{ $json.resultUrls[0] }}', options: {} }, position: [800, 2352], name: 'Download Video1' } }))
  ...
  .add(sticky('**Text-to-Video Workflow**\n\nGenerates videos from text prompts using Kling 2.6. Set your parameters, submit the request, and the workflow polls for completion before downloading.', { name: 'Text-to-Video Section', position: [-720, 1664], width: 1728, height: 512 }))
  .add(sticky('**Image-to-Video Workflow**\n\nAnimates existing images into videos using Kling 2.6. Provide an image URL and prompt, then the workflow handles generation and download.', { name: 'Image-to-Video Section', position: [-720, 2224], width: 1728, height: 512 }))
