return workflow('e0BX3fhHvcBuQTBU', 'whats app agent community', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.whatsAppTrigger', version: 1, config: { parameters: { options: {}, updates: ['messages'] }, credentials: {
    ...
    }, position: [380, 440], name: 'when message received' } }))
  .then(node({ type: 'n8n-nodes-base.googleDocs', version: 2, config: { parameters: {
    ...
    }, position: [600, 440], name: 'company\'s knowledge' } }))
  .then(node({ type: 'n8n-nodes-base.aiTransform', version: 1, config: { parameters: {
    ...
    }, position: [820, 440], name: 'Prepare Prompt' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.7, config: { parameters: {
      ...
      hasOutputParser: true
        ...
        }, name: 'Google Gemini Chat Model' } }) }, position: [1040, 440], name: 'AI Agent' } }))
  .then(node({ type: 'n8n-nodes-base.dateTime', version: 2, config: { parameters: { options: {} }, position: [1420, 440], name: 'Date & Time' } }))
  ...
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [1860, 440], name: '24-hour window check' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [2300, 340], name: 'cleanAnswer' } }), node({ type: 'n8n-nodes-base.whatsApp', version: 1, config: { parameters: {
    ...
    }, position: [2300, 540], name: 'Send Pre-approved Template Message to Reopen the Conversation' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
