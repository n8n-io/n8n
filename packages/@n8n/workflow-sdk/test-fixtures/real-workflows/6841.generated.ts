return workflow('8RiTOY4w1qA2cIHH', 'Automate weekly Hollywood film briefing via Tavily and Gemini', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: {
    ...
    }, position: [656, 400], name: 'Weekly Thursday Trigger' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.1, config: { parameters: {
      ...
      hasOutputParser: true
        ...
        }, name: 'Format Output for Email' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatGoogleGemini', version: 1, config: { parameters: { options: {} }, name: 'Google Gemini 2.5 Flash' } }), tools: [tool({ type: 'n8n-nodes-base.httpRequestTool', version: 4.2, config: { parameters: {
        ...
        }, name: 'Fetch Hollywood News (Tavily)' } }), tool({ type: 'n8n-nodes-base.httpRequestTool', version: 4.2, config: { parameters: {
          url: 'https://api.tavily.com/search',
          method: 'POST',
          options: {},
        ...
        }, name: 'Fetch Weekly Releases (Tavily)' } }), tool({ type: 'n8n-nodes-base.httpRequestTool', version: 4.2, config: { parameters: {
          url: 'https://api.tavily.com/search',
          method: 'POST',
          options: {},
        ...
        }, name: 'Fetch Must-Watch Movies (Tavily)' } }), tool({ type: 'n8n-nodes-base.httpRequestTool', version: 4.2, config: { parameters: {
          url: 'https://api.tavily.com/search',
          method: 'POST',
          options: {},
        ...
        }, name: 'Fetch Weekly Box Office (Tavily)' } })] }, position: [1248, 400], name: 'Hollywood News Research Agent' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      sendTo: 'user@example.com',
      message: '={{ $json.output.body }}',
      options: {},
      subject: '={{ $json.output.subject }}'
    }, position: [2016, 400], name: 'Send a message' } }))
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: {
      rule: {
