return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.telegramTrigger', version: 1, config: { parameters: { updates: ['message'], additionalFields: {} }, credentials: {
    ...
    }, position: [880, 208], name: 'Listen for incoming events' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.2, config: { parameters: {
      fields: {
        values: [
          {
            name: 'text',
      ...
      options: {}
    }, position: [1104, 208], name: 'Voice or Text' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.telegram', version: 1.1, config: { parameters: {
      fileId: '={{ $(\'Listen for incoming events\').item.json.message.voice.file_id }}',
      resource: 'file',
      additionalFields: {}
    }, credentials: {
      telegramApi: { id: 'credential-id', name: 'telegramApi Credential' }
    }, position: [1552, 128], name: 'Get Voice File' } }), node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.6, config: { parameters: {
      text: '={{ $json.text }}',
      options: {
      ...
      promptType: 'define'
          ...
          gmailOAuth2: { id: 'credential-id', name: 'gmailOAuth2 Credential' }
        }, name: 'Send Email' } }), tool({ type: 'n8n-nodes-base.googleCalendarTool', version: 1.1, config: { parameters: {
          options: {
        ...
        }, name: 'Google Calendar' } }), tool({ type: 'n8n-nodes-base.googleTasksTool', version: 1, config: { parameters: {
        ...
        }, name: 'Create a task in Google Tasks' } }), tool({ type: 'n8n-nodes-base.googleTasksTool', version: 1, config: { parameters: {
          task: 'MTY1MTc5NzMxMzA5NDc5MTQ5NzQ6MDow',
          operation: 'getAll',
          additionalFields: {}
        }, credentials: {
        ...
        }, name: 'Get many tasks in Google Tasks' } })], model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenRouter', version: 1, config: { parameters: { options: {} }, credentials: {
          openRouterApi: { id: 'credential-id', name: 'openRouterApi Credential' }
        }, name: 'OpenRouter' } }), memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryBufferWindow', version: 1.2, config: { parameters: {
          sessionKey: '={{ $(\'Listen for incoming events\').first().json.message.from.id }}',
          sessionIdType: 'customKey'
        }, name: 'Window Buffer Memory' } }) }, position: [2224, 192], name: 'Jackie, AI Assistant 👩🏻‍🏫' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: { options: {}, resource: 'audio', operation: 'transcribe' }, credentials: {
    ...
    }, position: [1872, 128], name: 'Transcribe a recording' } }))
  ...
  .add(sticky('\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\nSend message back to Telegram\n', { name: 'Sticky Note4', color: 7, position: [2592, 96], width: 304, height: 288 }))
  .add(sticky('## [Video Tutorial](https://youtu.be/ROgf5dVqYPQ)\n@[youtube](ROgf5dVqYPQ)', { name: 'Sticky Note2', color: 7, position: [2944, -16], width: 544, height: 400 }))