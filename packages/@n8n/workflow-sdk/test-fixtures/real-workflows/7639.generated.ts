return workflow('', '')
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.chatTrigger', version: 1.3, config: { parameters: { options: {} }, position: [192, 848], name: 'Chat with Your Data' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.2, config: { parameters: {
      ...
      hasOutputParser: true
    }, subnodes: { memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryBufferWindow', version: 1.3, config: { name: 'Memory' } }), tools: [tool({ type: 'n8n-nodes-base.googleSheetsTool', version: 4.7, config: { parameters: {
          options: {},
          sheetName: {
            __rl: true,
            mode: 'list',
        ...
        }, name: 'Analyze Data' } })], model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
          model: {
            __rl: true,
            mode: 'list',
            value: 'gpt-4.1-nano',
