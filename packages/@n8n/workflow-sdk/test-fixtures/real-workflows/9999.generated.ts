return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.jotFormTrigger', version: 1, config: { parameters: { form: '252934846764067' }, credentials: {
      jotFormApi: { id: 'credential-id', name: 'jotFormApi Credential' }
    }, position: [-288, 0], name: 'JotForm Trigger' } }))
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.2, config: { parameters: {
      ...
      hasOutputParser: true
        ...
        }, name: 'Google Gemini Chat Model' } }), outputParser: outputParser({ type: '@n8n/n8n-nodes-langchain.outputParserStructured', version: 1.3, config: { parameters: {
        ...
        }, name: 'Structured Output Parser' } }) }, position: [1056, 0], name: 'AI Feedback Triage' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.trello', version: 1, config: { parameters: {
    ...
    }, position: [2016, -352], name: 'Create Trello Card' } }), node({ type: 'n8n-nodes-base.airtable', version: 2.1, config: { parameters: {
      base: {
        __rl: true,
        mode: 'list',
    ...
    }, position: [2016, 128], name: 'Log General Feedback to Airtable' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(ifBranch([node({ type: 'n8n-nodes-base.slack', version: 2.3, config: { parameters: {
    ...
    }, position: [2848, -544], name: 'Alert Dev Team' } }), node({ type: 'n8n-nodes-base.noOp', version: 1, config: { position: [2864, -224], name: 'No Alert Needed' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .add(node({ type: 'n8n-nodes-base.if', version: 2.2, config: { parameters: {
    ...
    }, position: [224, 432], name: 'Email Provided?' } }))
