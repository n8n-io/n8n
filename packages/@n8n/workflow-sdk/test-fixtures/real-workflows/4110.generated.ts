const wf = workflow(
	...
	.then(
		node({
			...
			version: 4.2,
			config: {
				parameters: { url: '={{ $json.data.origin_cover }}', options: {} },
				position: [-180, -280],
				...
				parameters: {
					columns: {
						value: {
							...
							'Lien de la vidéo':
								"={{ $('Trigger: Get TikTok URL via Telegram').item.json.message.text }}",
							'Modèle de script vidéo':
								"={{ $('Transcribe Audio to Script (GPT)').item.json.text }}",
							'Modèle de texte superposé':
								"={{ $('Download TikTok Audio').item.json.message.content }}",
						},
						schema: [
							{
								id: 'ID du modèle',
					...
					sheetName: {
						...
						value: 'gid=0',
						cachedResultUrl:
							'https://docs.google.com/spreadsheets/d/1SoYJvZbVNn4L1FNfk9eFqx_vGlCsMxgw8bx0DVFgBLY/edit#gid=0',
				...
				parameters: {
					columns: {
						value: {
							...
							'Texte superposé':
								"={{ $('Split Rewritten Content into Sections').item.json.textOverlay }}",
						},
						schema: [
							{
								id: 'ID du modèle',
			...
			config: {
				parameters: {
					columns: {
						value: {
							'ID de la vidéo':
								"={{ $('Save Rewritten Video to Google Sheets').item.json['ID de la vidéo'] }}",
					...
					sheetName: {
						...
						value: 52679157,
						cachedResultUrl:
							'https://docs.google.com/spreadsheets/d/1SoYJvZbVNn4L1FNfk9eFqx_vGlCsMxgw8bx0DVFgBLY/edit#gid=52679157',
	...
	.add(
		sticky('# 🟫 STEP 1 — Clone a viral TikTok video', {
			position: [-740, -380],
			width: 2460,
			height: 300,
		}),
	...
	.add(
		sticky('# 🟦 STEP 2 — Suggest new content idea\n', {
			name: 'Sticky Note1',
			color: 4,
			position: [-740, -20],
			width: 1920,
	...
	.add(
		sticky('# 🟪 STEP 3 — Create the new video with your avatar\n\n', {
			name: 'Sticky Note2',
			position: [-740, 340],
			width: 1920,
			height: 300,
	...
	.add(
		sticky('# 🟥 STEP 4 — Publish to 9 platforms\n', {
			name: 'Sticky Note3',
			color: 3,
			position: [-740, 660],
			width: 1920,
