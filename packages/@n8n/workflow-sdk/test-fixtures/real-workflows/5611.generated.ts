const wf = workflow('', '')
					...
					assignments: {
						assignments: [
							...
							},
							{
								...
								type: 'string',
								value:
									'={{ $json.organization.primary_phone && $json.organization.primary_phone.sanitized_number && $json.organization.primary_phone.sanitized_number ? $json.organization.primary_phone.sanitized_number : null }}',
							},
							{
	...
	.then(
		node({
			...
			version: 2.3,
			config: {
				parameters: { options: {}, mergeByFields: { values: [{}] } },
				position: [-448, -240],
					...
					fieldsUi: {
						fieldValues: [
							...
							},
							{
								...
								fieldValue:
									"={{ $json.businessIndustry }}{{ $('Apollo Scraper').item.json.organization.industry }}",
							},
						...
						assignments: [
							{
								...
								type: 'string',
								value:
									'={{ $input.all().length }} new contacts have been added to the Google Sheet!',
							},
						],
					},
				},
	...
	.add(
		sticky('# First step: recieve the message via audio or text', {
			color: 3,
			position: [-2880, -528],
			width: 860,
			height: 600,
	...
	.add(
		sticky('# Create the json for the url', {
			name: 'Sticky Note1',
			position: [-2000, -528],
			width: 580,
			height: 600,
	...
	.add(
		sticky('## Scrape the leads from apify actor', {
			name: 'Sticky Note3',
			color: 5,
			position: [-1376, -288],
			width: 880,
	...
	.add(
		sticky('## Insert the new leads to your database\n\n*can be airtable/sheets/supabase', {
			name: 'Sticky Note4',
			position: [-208, 48],
			height: 240,
		}),
	...
	.add(
		sticky(
			...
			{ name: 'Sticky Note5', color: 3, position: [-1296, 64], width: 576, height: 560 },
		),
	)
	.add(
		sticky(
