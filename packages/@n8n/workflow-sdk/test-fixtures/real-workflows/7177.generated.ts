const wf = workflow(
	'YP981T0Pa3Ab6Dwg',
	'Automated Pharmacy Inventory Alerts for Low Stock & Expiry Dates with Google Sheets',
	{ executionOrder: 'v1' },
)
	.add(
		node({
	...
	.then(
		node({
			...
			version: 1.1,
			config: { position: [-560, 440], name: 'Wait For All Data' },
		}),
	)
	.then(
		node({
		...
		node({
			...
			config: {
				parameters: {
					...
					options: {},
					subject:
						"Pharmacy Inventory Alert - Low Stock & Expiry Notice - {{ $now.format('YYYY-MM-DD') }}",
