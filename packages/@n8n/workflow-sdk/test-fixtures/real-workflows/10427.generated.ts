return workflow('e1a3hFH38UDCqquQ', '01 Analyze Facebook Ad Performance with an AI Media Buyer and Send Insights to Google Sheets', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-3184, 768], name: 'When clicking ‘Test workflow’' } }))
  .then(node({ type: 'n8n-nodes-base.nocoDb', version: 3, config: { parameters: {
    ...
    }, position: [-2928, 768], name: 'Getting Long-Term Token' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-2704, 768], name: 'Does it Need A Token Refresh?' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [-2240, 704], name: 'Getting Long-Lived Access Token1' } }), node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
          {
    ...
    }, position: [-1872, 976], name: 'Getting Long-Term Token1' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-2016, 704], name: 'Calculating End Date of Token1' } }))
  .then(node({ type: 'n8n-nodes-base.nocoDb', version: 3, config: { parameters: {
      table: 'mlkymmbtoa2iz72',
      fieldsUi: {
        fieldValues: [
      ...
      nocoDbApiToken: { id: 'credential-id', name: 'nocoDbApiToken Credential' }
    }, position: [-1824, 704], name: 'Updating Token' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-1600, 704], name: 'Extracting Long Term Token' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [-1360, 800], name: 'Extracting Access Token' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [-1120, 528], name: 'Getting Data For the Past 28 Days Segemented Per Campaign Adset and Ad' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-896, 528], name: 'Splitting Out In Table Format' } }))
  .then(node({ type: 'n8n-nodes-base.filter', version: 2.2, config: { parameters: {
    ...
    }, position: [-704, 528], name: 'Filtering Only For Sales Campaigns' } }))
  .add(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-496, 448], name: 'Aggregate Metrics by Ad Creative' } }))
  .then(node({ type: 'n8n-nodes-base.sort', version: 1, config: { parameters: {
      options: {},
      sortFieldsUi: {
        sortField: [{ order: 'descending', fieldName: 'total_spend' }]
      }
    }, position: [-304, 448], name: 'Sorting Based on Spends' } }))
  .add(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
    ...
    }, position: [272, -16], name: 'Sending Raw Data To A Google Sheet' } }))
  .add(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-96, 448], name: 'Stringify Everything' } }))
  .then(merge([node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
      jsCode: '// --- START OF CODE NODE 4 SCRIPT (Stringify Output) ---\n// Mode: Run Once for All Items\n// Input: The array of aggregated ad creative performance objects\n\nconst allAggregatedItems = $input.all(); // This is an array of n8n items\n\n// Extract the .json part from each n8n item to get the actual data objects\nconst dataToConvert = allAggregatedItems.map(item => item.json);\n\n// Stringify the array of data objects\n// The \'null, 2\' arguments pretty-print the JSON string with an indent of 2 spaces,\n// which can be helpful for readability if you\'re inspecting it, but not strictly necessary for an LLM.\n// If the LLM prefers a more compact JSON, you can omit \'null, 2\'.\nconst jsonString = JSON.stringify(dataToConvert, null, 2);\n\n// Output a single item containing this string\nreturn [{ json: { all_ads_data_string: jsonString } }];\n// --- END OF CODE NODE 4 SCRIPT ---'
    }, position: [-96, 448], name: 'Stringify Everything' } }), node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
      jsCode: '// --- START OF CODE NODE 4 SCRIPT (Stringify Output) ---\n// Mode: Run Once for All Items\n// Input: The array of aggregated ad creative performance objects\n\nconst allAggregatedItems = $input.all(); // This is an array of n8n items\n\n// Extract the .json part from each n8n item to get the actual data objects\nconst dataToConvert = allAggregatedItems.map(item => item.json);\n\n// Stringify the array of data objects\n// The \'null, 2\' arguments pretty-print the JSON string with an indent of 2 spaces,\n// which can be helpful for readability if you\'re inspecting it, but not strictly necessary for an LLM.\n// If the LLM prefers a more compact JSON, you can omit \'null, 2\'.\nconst jsonString = JSON.stringify(dataToConvert, null, 2);\n\n// Output a single item containing this string\nreturn [{ json: { benchmarkData: jsonString } }];\n// --- END OF CODE NODE 4 SCRIPT ---'
    }, position: [-48, 784], name: 'Stringify Benchmark Data' } })], { version: 3.1, name: 'Combining Ad Data and Benchmarking Data' }))
  .then(node({ type: 'n8n-nodes-base.aggregate', version: 1, config: { parameters: { options: {}, aggregate: 'aggregateAllItemData' }, position: [400, 512], name: 'Combine Ad & Benchmark Data for LLM' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.6, config: { parameters: {
      ...
      hasOutputParser: true
        ...
        }, name: 'Structured Output Parser' } }) }, position: [608, 528], name: 'Senior Facebook Ads Media Buyer' } }))
  .then(node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'output' }, position: [944, 528] } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
    ...
    }, position: [1184, 528], name: 'Updating Ad Insights Into Google Sheets' } }))
  .add(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
