const wf = workflow('', 'Apply to Jobs from Excel and Track Application Status', {
	executionOrder: 'v1',
})
	.add(
		node({
						...
						assignments: [
							{
								...
								type: 'string',
								value:
									"={{ $json.Job_URL.includes('linkedin.com') ? 'linkedin' : $json.Job_URL.includes('indeed.com') ? 'indeed' : 'generic' }}",
							},
							{
								...
								type: 'string',
								value:
									"={{ $('Configuration').first().json.coverLetterTemplate.replace('{{position}}', $json.Position).replace('{{company}}', $json.Company).replace('{{skills}}', 'relevant technical skills') }}",
							},
							{
								id: 'application-date',
						...
						assignments: [
							{
								...
								type: 'string',
								value:
									"={{ $json.statusCode >= 200 && $json.statusCode < 300 ? 'Applied' : 'Failed' }}",
							},
							{
								id: 'app-id',
								name: 'applicationId',
								type: 'string',
								value:
									"={{ $json.body?.applicationId || $json.body?.id || 'AUTO_' + $now.format('yyyyMMddHHmmss') }}",
							},
							{
								id: 'status-notes',
								name: 'statusNotes',
								type: 'string',
								value:
									"={{ $json.statusCode >= 200 && $json.statusCode < 300 ? 'Application submitted successfully via ' + $('Prepare Application Data').first().json.platform : 'Application failed: ' + $json.statusCode }}",
							},
							{
								id: 'job-data',
		...
		node({
			...
			config: {
				parameters: {
					...
					subject:
						'Job Application: {{ $json.originalJobData.Company }} - {{ $json.originalJobData.Position }}',
					operation: 'sendEmail',
					emailFormat: 'html',
				},
				position: [2440, 480],
						...
						assignments: [
							{
								...
								type: 'string',
								value:
									"={{ ['Applied', 'Under Review', 'Interview Scheduled', 'Rejected'][Math.floor(Math.random() * 4)] }}",
							},
							{
								id: 'check-date',
								name: 'checkDate',
	...
	.add(
		sticky(
			...
			{ name: '📋 Template Overview', color: 4, position: [380, 160], width: 389, height: 464 },
		),
	)
	.add(
		sticky(
			...
			{ name: '📊 Sheet Structure', color: 7, position: [820, 160], width: 295, height: 284 },
		),
	)
	.add(
		sticky(
			...
			{ name: '⚙️ Setup Notes', color: 6, position: [1180, 160], width: 295, height: 224 },
		),
	)
	.add(
		sticky(
			...
			{ name: '📊 Status Tracking Info', color: 7, position: [380, 740], width: 329, height: 204 },
		),
	);
