return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-4752, 3136], name: 'When clicking ‘Execute workflow’' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-4528, 3136], name: 'Prepare Data' } }))
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: {} }, position: [-4304, 3136], name: 'Loop Over Items1' } }))
  .add(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-4080, 2816], name: 'Collect Media IDs' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 20 }, position: [-3856, 2816], name: 'Wait before Carousel' } }))
  .then(node({ type: 'n8n-nodes-base.facebookGraphApi', version: 1, config: { parameters: {
    ...
    }, position: [-3632, 2816], name: 'Edit Carousel' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 15 }, position: [-3408, 2816], name: 'Wait before Upload to Instagram' } }))
  .then(node({ type: 'n8n-nodes-base.facebookGraphApi', version: 1, config: { parameters: {
      edge: 'media_publish',
      ...
      facebookGraphApi: { id: 'credential-id', name: 'facebookGraphApi Credential' }
    }, position: [-3184, 2816], name: 'Publish Carousel to Instagram' } }))
  .then(node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
    ...
    }, position: [-2960, 2816], name: 'Send Update Message' } }))
  .add(node({ type: 'n8n-nodes-base.googleDrive', version: 3, config: { parameters: {
    ...
    }, position: [-4016, 3136], name: 'Download file1' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [-3680, 3136], name: 'Upload images to Cloudinary' } }))
  .then(node({ type: 'n8n-nodes-base.facebookGraphApi', version: 1, config: { parameters: {
      edge: 'media',
      ...
      facebookGraphApi: { id: 'credential-id', name: 'facebookGraphApi Credential' }
    }, position: [-3488, 3168], name: 'Create each Carousel Picture' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
