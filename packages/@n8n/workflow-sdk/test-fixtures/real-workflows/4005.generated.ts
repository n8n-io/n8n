return workflow('w9YVsuUtlNgXOEAQ', 'LinkedIn Post Generation & Approval Automation', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{}] } }, position: [-260, 2595] } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
    ...
    }, position: [-40, 2595], name: 'Get Data from Sheets' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.6, config: { parameters: {
      ...
      promptType: 'define'
        ...
        }, name: 'OpenAI Chat Model' } }) }, position: [180, 2595], name: 'Generate Post Content' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [556, 2595], name: 'Data Formatting 1' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
    ...
    }, position: [776, 2495], name: 'Send Content Confirmation' } }))
  .then(switchCase([node({ type: 'n8n-nodes-base.if', version: 2.2, config: { parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
    ...
    }, position: [1294, 2220], name: 'If Image Provided' } }), node({ type: '@n8n/n8n-nodes-langchain.chainLlm', version: 1.6, config: { parameters: {
      ...
      promptType: 'define'
          ...
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Chat Model' } }) }, position: [1216, 2670], name: 'Regenerate Post Content' } }), node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
      columns: {
        value: {
          Output: '={{ $json.urn }}',
          Status: '=Completed',
    ...
    }, position: [2032, 2320], name: 'Update Google Sheet' } })], { version: 3.2, parameters: {
      rules: {
        values: [
          {
            outputKey: 'Yes',
  ...
  .add(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      options: {}
    }, position: [1592, 2120], name: 'Get Image' } }))
  .then(node({ type: 'n8n-nodes-base.linkedIn', version: 1, config: { parameters: {
    ...
    }, position: [1812, 2120], name: 'Post With Image' } }))
  .add(node({ type: 'n8n-nodes-base.linkedIn', version: 1, config: { parameters: {
      ...
      linkedInOAuth2Api: { id: 'credential-id', name: 'linkedInOAuth2Api Credential' }
    }, position: [1812, 2320], name: 'Post Without Image' } }))
  .add(sticky('## 1. Schedule & Sheet Data Retrieval\n\nThis workflow starts automatically on a defined schedule (e.g., daily or hourly).\n\nIt retrieves the next Google Sheet row marked as ‘Pending’.\n\nThe sheet should include columns like:\n1. Post Description\n2. Instructions\n3. Image\n4. Status\n5. row_number (required for updates)\n\nEnsure your Google Sheets credentials are correctly configured.', { name: 'Sticky Note', position: [-780, 2480], width: 420, height: 380 }))
  .add(sticky('## 2. AI-Powered Post Generation & Formatting\n\nUses OpenAI GPT to generate a LinkedIn post based on the sheet’s Post Description and Instructions.\n\nYou can modify the prompt if needed.\nThe generated post is then formatted along with relevant data for easy reference and consistency.', { name: 'Sticky Note1', position: [-220, 3040], width: 420, height: 240 }))
  .add(sticky('## 3. Gmail Approval Workflow\n\nSends the formatted post to an approver via Gmail.\n\nThe approver can respond with:\n✅ Yes – Approve\n✏️ No – Request changes\n❌ Cancel – Abort the post\n\nSet Gmail credentials and recipient email in the node.', { name: 'Sticky Note2', position: [440, 3040], width: 440, height: 240 }))
  .add(sticky('## 4. Approval Handling & Regeneration\n\nHandles all approval responses:\n- If Yes, proceed to post.\n- If No, regenerate content based on the feedback and resend.\n- If Cancel, update the Google Sheet as Cancelled.\n\nThis ensures a complete review cycle before posting.', { name: 'Sticky Note3', position: [1120, 3040], width: 440, height: 240 }))
  .add(sticky('## 5. Image Check, Posting & Sheet Update\n\nChecks if an image URL is provided.\n- If present: Downloads the image and posts with it.\n- If not: Posts content without an image.\n\nAfter posting, it updates the Google Sheet with:\n- Status = Completed or Cancelled\n- LinkedIn post link/output\n\nUses row_number for precise sheet updates.', { name: 'Sticky Note4', position: [1800, 3040], width: 460, height: 320 }))