return workflow('zJfKfgI70InCGXxb', 'Ai Product Writer (SEO Titles & Description) for Shopify, Google Merchant Center)', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: { rule: { interval: [{}] } }, position: [1040, 400] } }))
  ...
  .then(ifBranch([null, node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: {} }, position: [1600, 480], name: 'Loop Over Items' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.editImage', version: 1, config: { parameters: {
      width: 40,
      height: 40,
      options: {},
      operation: 'resize',
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.8, config: { parameters: {
      ...
      hasOutputParser: true
        ...
        }, name: 'Structured Output Parser1' } }) }, position: [2420, 520], name: 'shopify' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.8, config: { parameters: {
      ...
      hasOutputParser: true
        ...
        }, name: 'Structured Output Parser' } }) }, position: [2760, 520], name: 'GMC' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
      columns: {
        value: {
    ...
    }, position: [3140, 760], name: 'Google Sheets2' } }))
