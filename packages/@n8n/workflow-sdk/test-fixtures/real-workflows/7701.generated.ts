return workflow('tMez4xptNdwuoUMa', 'Real-Time Stock Price Monitor & Smart Alerts for Indian & US Markets', { executionOrder: 'v1' })
  .add(node({ type: 'n8n-nodes-base.cron', version: 1, config: { position: [-352, 176], name: 'Market Hours Trigger' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4, config: { parameters: {
    ...
    }, position: [-128, 176], name: 'Read Stock Watchlist' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [96, 176], name: 'Parse Watchlist Data' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.1, config: { parameters: {
    ...
    }, position: [320, 176], name: 'Fetch Live Stock Price' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [544, 176], name: 'Smart Alert Logic' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.emailSend', version: 2.1, config: { parameters: {
      ...
      emailFormat: 'text'
    }, credentials: { smtp: { id: 'credential-id', name: 'smtp Credential' } }, position: [992, -16], name: 'Send Email Alert' } }), null], { version: 2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 1,
  ...
  .add(node({ type: 'n8n-nodes-base.telegram', version: 1, config: { parameters: {
    ...
    }, position: [992, 368], name: 'Send Telegram Alert' } }))
  // Disconnected: Update Alert History
  .add(node({ type: 'n8n-nodes-base.googleSheets', version: 4, config: { parameters: {
      columns: {
        value: {},
        schema: [],
      ...
      googleApi: { id: 'credential-id', name: 'googleApi Credential' }
    }, position: [992, 176], name: 'Update Alert History' } }))
  ...
  .add(node({ type: 'n8n-nodes-base.if', version: 2, config: { parameters: {
    ...
    }, position: [1216, 176], name: 'Alert Status Check' } }))
  ...
  .add(node({ type: 'n8n-nodes-base.emailSend', version: 2.1, config: { parameters: {
      ...
      toEmail: 'user@example.com',
      fromEmail: 'user@example.com'
    }, credentials: { smtp: { id: 'credential-id', name: 'smtp Credential' } }, position: [1440, 80], name: 'Success Notification' } }))
