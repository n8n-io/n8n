const wf = workflow(
		...
		trigger({
			...
			config: {
				parameters: {
					...
					formDescription:
						'Generate AI Videos (with audio), using OpenAI Sora 2 and Upload to TikTok',
				},
				position: [-112, 576],
	...
	.then(
		node({
			...
			version: 1.1,
			config: { parameters: { amount: 60 }, position: [592, 576], name: 'Wait 60 sec.' },
		}),
	)
	.then(
		node({
	...
	.add(
		sticky(
			...
			{ name: 'Sticky Note5', position: [640, 128], width: 740, height: 116 },
		),
	)
	.add(
		sticky(
			'## STEP 1 - GET API KEY (YOURAPIKEY)\nCreate an account [here](https://fal.ai/) and obtain API KEY.\nIn the node "Create Image" set "Header Auth" and set:\n- Name: "Authorization"\n- Value: "Key YOURAPIKEY"',
			{ name: 'Sticky Note6', position: [-144, 288], width: 740, height: 172 },
		),
	)
	.add(
	...
	.add(sticky('Set ChannelId Step 3', { position: [1104, 784], width: 180, height: 200 }))
	.add(
		sticky(
			...
			{ name: 'Sticky Note8', position: [640, -80], width: 740, height: 184 },
		),
	);
