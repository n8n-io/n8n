return workflow('O7BiL9gvzMRGnzco', 'Development Done - Product Ads via Veo3', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.formTrigger', version: 2.2, config: { parameters: {
    ...
    }, position: [-272, -64], name: 'Prompt your Idea' } }))
  ...
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [400, -64], name: 'Converting Image file for Storing' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [624, -64], name: 'Uploading Image in Airtable' } }))
  .then(node({ type: 'n8n-nodes-base.airtable', version: 2.1, config: { parameters: {
      id: '={{ $json.id }}',
      base: {
        __rl: true,
      ...
      airtableTokenApi: { id: 'credential-id', name: 'airtableTokenApi Credential' }
    }, position: [848, -64], name: 'Get a record' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.googleGemini', version: 1, config: { parameters: {
    ...
    }, position: [1056, -64], name: 'Analyze image' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [1296, -64], name: 'Parse Request' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [1520, -64], name: 'Generate Video Veo 3' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { unit: 'minutes', amount: 1 }, position: [1744, -64] } }))
  .then(node({ type: 'n8n-nodes-base.airtable', version: 2.1, config: { parameters: {
      base: {
        __rl: true,
        mode: 'list',
      ...
      airtableTokenApi: { id: 'credential-id', name: 'airtableTokenApi Credential' }
    }, position: [1968, -64], name: 'Update record' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      googleOAuth2Api: { id: 'credential-id', name: 'googleOAuth2Api Credential' }
    }, position: [2192, -64], name: 'Get the Video' } }))
  .then(node({ type: 'n8n-nodes-base.convertToFile', version: 1.1, config: { parameters: {
    ...
    }, position: [2416, -64], name: 'Downloadable Video' } }))
  .add(sticky('## Airtable Configuration\n* **Airtable Credentials**: In the "Create a record", "Get a record", and "Update record" nodes, you must select your own Airtable credentials.\n* **Base and Table IDs**: Update the Base ID and Table ID in all four Airtable-related nodes to match your own setup: "Create a record", "Uploading Image in Airtable" (in the URL), "Get a record", and "Update record".', { name: 'Sticky Note', color: 5, position: [128, -336], width: 528, height: 192 }))
  .add(sticky('## Google AI Configuration\n* **Gemini Node** ("Analyze image"): Select your Google AI (Gemini) API credentials.\n**Veo Nodes ("Parse Request" & HTTP Requests):** \n* **Project ID**: In the URLs, replace with your Google Cloud Project ID.\n* **Credentials**: In both HTTP request nodes ("Generate Video Veo 3" & "Get the Video"), select your Google OAuth2 credentials.\n* **API Info**: For help with endpoints, see the [Google Cloud Vertex AI Studio](https://www.google.com/search?q=https://console.cloud.google.com/vertex-ai/studio/media/generate)', { name: 'Sticky Note1', color: 4, position: [1728, -368], width: 544, height: 240 }))
  .add(sticky('## Customization\n**Video Generation Parameters**: In the "Parse Request" node, you can modify the JavaScript code to change video settings like *aspectRatio*, *durationSeconds*, and *resolution*.', { name: 'Sticky Note2', color: 6, position: [1216, -352], width: 304, height: 192 }))