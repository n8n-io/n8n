return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-1080, 1440], name: 'Run Workflow' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-920, 1620], name: 'Set Topic to Search' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2, config: { parameters: {
      ...
      hasOutputParser: true
          ...
          jsonSchemaExample: '{\n  "column1": [\n    "2025-08-01",\n    "2025-08-02",\n    "2025-08-03"\n  ],\n  "column2": [\n    "Instagram",\n    "LinkedIn",\n    "Twitter"\n  ],\n  "column3": [\n    "Image Post",\n    "Blog Link",\n    "Video Snippet"\n  ],\n  "column4": [\n    "Workflow Automation",\n    "AI Agent Demo",\n    "Case Study"\n  ],\n  "column5": [\n    "Alice",\n    "Bob",\n    "Charlie"\n  ]\n}\n'
        }, name: 'Structured Output Parser' } }), tools: [tool({ type: '@n8n/n8n-nodes-langchain.toolThink', version: 1, config: { name: 'Tool: Inject Creativity1' } })] }, position: [-520, 1020], name: 'Generate Random Data' } }))
  .add(node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'output.column1' }, position: [1100, 1020], name: 'Split Column 1' } }))
  .then(merge([node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'output.column1' }, position: [1100, 1020], name: 'Split Column 1' } }), node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'output.column2' }, position: [920, 1120], name: 'Split Column 2' } }), node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'output.column3' }, position: [1120, 1240], name: 'Split Column 3' } }), node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'output.column4' }, position: [880, 1320], name: 'Split Column 4' } }), node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'output.column5' }, position: [1100, 1480], name: 'Split Column 5' } })], { version: 3.2, parameters: {
      mode: 'combine',
      options: {},
  ...
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [1620, 1300], name: 'Rename Columns' } }))
  .then(merge([node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [600, 1660], name: 'Pivot Column Names' } }), node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
          {
    ...
    }, position: [1620, 1300], name: 'Rename Columns' } })], { version: 3.2, name: 'Append Column Names' }))
  .add(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-120, 1260], name: 'Outpt all Data to One Field' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2, config: { parameters: {
