return workflow('83Ad0ngUzuJQHqb6', 'AI Voice Assistant | Shareable', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.webhook', version: 2.1, config: { parameters: {
      path: 'REPLACE ME',
      options: {},
      httpMethod: 'POST',
      responseMode: 'responseNode'
    }, position: [1008, 400], name: 'Webhook: Receive User Request (ElevenLabs)' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.2, config: { parameters: {
      ...
      options: {},
      promptType: 'define'
    }, subnodes: { memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryRedisChat', version: 1.5, config: { parameters: {
          sessionKey: '={{ $json.body.sessionId }}',
          sessionIdType: 'customKey',
          contextWindowLength: 20
        }, credentials: { redis: { id: 'credential-id', name: 'redis Credential' } }, name: 'Redis Chat Memory' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatAnthropic', version: 1.3, config: { parameters: {
          model: {
            __rl: true,
            mode: 'list',
        ...
        }, name: 'Anthropic Chat Model' } }), tools: [tool({ type: '@n8n/n8n-nodes-langchain.toolThink', version: 1.1, config: { name: 'Reasoning Tool (LangChain)' } }), tool({ type: 'n8n-nodes-base.googleCalendarTool', version: 1.3, config: { parameters: {
          options: {},
        ...
        }, name: 'Calendar: Create Appointment' } }), tool({ type: 'n8n-nodes-base.googleCalendarTool', version: 1.3, config: { parameters: {
