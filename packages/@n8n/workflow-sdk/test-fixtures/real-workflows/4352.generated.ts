return workflow('vJw8YoQhdA5CXGeB', 'Social_media_post _automation_from_google_trends_and _perplexity copy', { executionOrder: 'v1' })
  ...
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [-3200, -200], name: 'Google Trends' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [-2980, -200], name: '2 Most Trending' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-2760, -200], name: 'High search volume keywords' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
    ...
    }, position: [-2540, -200], name: 'Choosing Topic' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [-2160, -200], name: 'Research Topic- Perplexity' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 10 }, position: [-1940, -200] } }))
  .then(node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'choices[0].message.content' }, position: [-1720, -200] } }))
  .add(node({ type: 'n8n-nodes-base.twitter', version: 2, config: { parameters: { additionalFields: {} }, credentials: {
      twitterOAuth2Api: { id: 'credential-id', name: 'twitterOAuth2Api Credential' }
    }, position: [-1500, 0], name: 'X1' } }))
  .add(node({ type: 'n8n-nodes-base.facebookGraphApi', version: 1, config: { parameters: { options: {}, httpRequestMethod: 'POST' }, position: [-1500, -400], name: 'Facebook Graph API1' } }))
  .add(node({ type: 'n8n-nodes-base.linkedIn', version: 1, config: { parameters: {
    ...
    }, position: [-1480, -200], name: 'LinkedIn1' } }))
  ...
  .add(sticky('## Multi Social Media Posting', { name: 'Sticky Note4', position: [-1560, -540], height: 800 }))
