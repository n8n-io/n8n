return workflow('x4DcB7sVAeVmIrMD', 'Invoice-Parser-Lite', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [320, -80], name: 'When clicking ‘Test workflow’' } }))
  .add(node({ type: 'n8n-nodes-base.googleDrive', version: 3, config: { parameters: {
    ...
    }, position: [880, 260], name: 'Load files from Google Drive folder' } }))
  .then(merge([node({ type: 'n8n-nodes-base.googleSheets', version: 4.3, config: { parameters: {
      options: {},
      sheetName: {
        __rl: true,
        mode: 'list',
    ...
    }, position: [1160, -80], name: 'Get already processed rows from Sheets' } }), node({ type: 'n8n-nodes-base.googleDrive', version: 3, config: { parameters: {
      filter: {
        folderId: {
          __rl: true,
          mode: 'list',
    ...
    }, position: [880, 260], name: 'Load files from Google Drive folder' } })], { version: 3, parameters: {
      mode: 'combine',
      options: {},
  ...
  .then(node({ type: 'n8n-nodes-base.googleDrive', version: 3, config: { parameters: {
    ...
    }, position: [1560, 140], name: 'Download file for OCR' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [2080, 340], name: 'Mistral Upload1' } }), node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: 'https://api.mistral.ai/v1/files',
      method: 'POST',
      options: {},
      sendBody: true,
      ...
      mistralCloudApi: { id: 'credential-id', name: 'mistralCloudApi Credential' }
    }, position: [2060, 120], name: 'Mistral Upload' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      mistralCloudApi: { id: 'credential-id', name: 'mistralCloudApi Credential' }
    }, position: [2260, 340], name: 'Mistral Signed URL1' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      mistralCloudApi: { id: 'credential-id', name: 'mistralCloudApi Credential' }
    }, position: [2460, 320], name: 'Mistral IMAGE OCR' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [2840, 100], name: 'Join OCR Pages and remove id field' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.8, config: { parameters: {
      ...
      promptType: 'define'
        ...
        }, name: 'Google Gemini Chat Model' } }) }, position: [3020, 60], name: 'Field Extractor' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [3380, 60], name: 'Add google drive Id back' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
    ...
    }, position: [3640, 60], name: 'Save data to Google Sheets' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: '=https://api.mistral.ai/v1/files/{{ $json.id }}/url',
      options: {},
      sendQuery: true,
      ...
      mistralCloudApi: { id: 'credential-id', name: 'mistralCloudApi Credential' }
    }, position: [2260, 120], name: 'Mistral Signed URL' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: 'https://api.mistral.ai/v1/ocr',
      method: 'POST',
      options: {},
      ...
      mistralCloudApi: { id: 'credential-id', name: 'mistralCloudApi Credential' }
    }, position: [2460, 100], name: 'Mistral DOC OCR' } }))
  .add(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
    ...
    }, position: [680, -80], name: 'Get Fields Schema' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [920, -80], name: 'extract schema' } }))
  ...
  .add(sticky('### Select sheet', { name: 'Sticky Note4', color: 4, position: [1120, -180], width: 200, height: 80 }))
  .add(sticky('### Select sheet to save data (same as at the start)', { name: 'Sticky Note5', color: 4, position: [3600, -40], width: 200, height: 80 }))
  .add(sticky('>> ### First column header in cell A1 MUST BE "Id". Id tracks the google drive Id and ensures the workflow is not processing previously processed files', { name: 'Sticky Note6', color: 3, position: [580, -220], width: 300, height: 120 }))
  .add(sticky('Uses drive ID - no need to modify', { name: 'Sticky Note7', color: 2, position: [1540, 40], width: 160, height: 80 }))
  .add(sticky('Requires Mistral API Key for La Platforme\nhttps://mistral.ai/products/la-plateforme', { name: 'Sticky Note8', color: 2, position: [2040, -20], width: 520, height: 80 }))