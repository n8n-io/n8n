return workflow('c1RYY5JDniAWe4Lm', '💥 Create AI Viral Videos using NanoBanana 2 PRO & VEO3.1 and Publish via Blotato -vide', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.telegramTrigger', version: 1.2, config: { parameters: { updates: ['message'], additionalFields: {} }, credentials: {
    ...
    }, position: [784, 192], name: 'Telegram Trigger: Receive Video Idea' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [992, 192], name: 'Set: Bot Token (Placeholder)' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      options: {}
    }, position: [1216, 192], name: 'Telegram API: Get File URL' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.openAi', version: 1.8, config: { parameters: {
    ...
    }, position: [1424, 192], name: 'OpenAI Vision: Analyze Reference Image' } }))
  ...
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [1984, 192], name: 'NanoBanana: Create Image' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 2 }, position: [2192, 192], name: 'Wait for Image Edit' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: { url: '={{ $json.response_url }}', options: {} }, position: [2400, 192], name: 'Download Edited Image' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [800, 640], name: 'Set Master Prompt' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2, config: { parameters: {
      ...
      promptType: 'define',
      hasOutputParser: true
    }, subnodes: { tools: [tool({ type: '@n8n/n8n-nodes-langchain.toolThink', version: 1, config: { name: 'Think' } })], model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenAi', version: 1.2, config: { parameters: {
          model: { __rl: true, mode: 'list', value: 'gpt-4.1-mini' },
          options: {}
        }, credentials: {
          openAiApi: { id: 'credential-id', name: 'openAiApi Credential' }
        }, name: 'OpenAI Chat Model' } }), outputParser: outputParser({ type: '@n8n/n8n-nodes-langchain.outputParserStructured', version: 1.3, config: { parameters: {
          jsonSchemaExample: '{\n  "prompt": "string",\n  "caption": "string",\n  "title": "string", \n  "hashtags": "string"\n}\n'
        }, name: 'Structured Output Parser' } }) }, position: [1008, 640], name: 'AI Agent: Generate Video Script' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [1360, 640], name: 'Parse GPT Response' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [1568, 640], name: 'Optimize Prompt for Veo' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [1776, 640], name: 'Prepare Veo Request Body' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [1984, 640], name: 'Veo Generation' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 2 }, position: [2192, 640] } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      url: '={{ $json.video.url }}',
      options: { response: { response: { responseFormat: 'file' } } }
    }, position: [2400, 640], name: 'Download Video' } }))
  .then(node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
      ...
      telegramApi: { id: 'credential-id', name: 'telegramApi Credential' }
    }, position: [800, 1152], name: 'Send Video to Telegram' } }))
  .then(node({ type: '@blotato/n8n-nodes-blotato.blotato', version: 2, config: { parameters: {
    ...
    }, position: [1136, 1152], name: 'Upload Video to BLOTATO' } }))
  .then(merge([node({ type: '@blotato/n8n-nodes-blotato.blotato', version: 2, config: { parameters: {
      ...
      blotatoApi: { id: 'credential-id', name: 'blotatoApi Credential' }
    }, position: [1552, 1056], name: 'Tiktok' } }), node({ type: '@blotato/n8n-nodes-blotato.blotato', version: 2, config: { parameters: {
      options: {},
      platform: 'linkedin',
      accountId: {
        __rl: true,
      ...
      blotatoApi: { id: 'credential-id', name: 'blotatoApi Credential' }
    }, position: [1712, 1056], name: 'Linkedin' } }), node({ type: '@blotato/n8n-nodes-blotato.blotato', version: 2, config: { parameters: {
      options: {},
      platform: 'facebook',
      accountId: {
        __rl: true,
      ...
      blotatoApi: { id: 'credential-id', name: 'blotatoApi Credential' }
    }, position: [1872, 1056], name: 'Facebook' } }), node({ type: '@blotato/n8n-nodes-blotato.blotato', version: 2, config: { parameters: {
      options: {},
      accountId: {
        __rl: true,
        mode: 'list',
      ...
      blotatoApi: { id: 'credential-id', name: 'blotatoApi Credential' }
    }, position: [1552, 1216], name: 'Instagram' } }), node({ type: '@blotato/n8n-nodes-blotato.blotato', version: 2, config: { parameters: {
      options: {},
      platform: 'twitter',
      accountId: {
        __rl: true,
      ...
      blotatoApi: { id: 'credential-id', name: 'blotatoApi Credential' }
    }, position: [1712, 1216], name: 'Twitter (X)' } }), node({ type: '@blotato/n8n-nodes-blotato.blotato', version: 2, config: { parameters: {
      options: {},
      platform: 'youtube',
      accountId: {
        __rl: true,
      ...
      blotatoApi: { id: 'credential-id', name: 'blotatoApi Credential' }
    }, position: [1872, 1216], name: 'Youtube' } })], { version: 3.2, parameters: { mode: 'chooseBranch', numberInputs: 6 }, name: 'Merge1' }))
  .then(node({ type: 'n8n-nodes-base.telegram', version: 1.2, config: { parameters: {
      text: 'Published',
      chatId: '={{ $(\'Telegram Trigger: Receive Video Idea\').first().json.message.chat.id }}',
      additionalFields: {}
