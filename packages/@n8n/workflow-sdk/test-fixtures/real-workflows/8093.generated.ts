return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-2048, 7776], name: 'When clicking ‘Execute workflow’' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.7, config: { parameters: {
    ...
    }, position: [-1760, 7776], name: 'Get Student Data1' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.2, config: { parameters: {
      ...
      hasOutputParser: true
          ...
          jsonSchemaExample: '{\n  "StudentID": "S001",\n  "Program": "Example Program",\n  "Missing": [\n    "CS-201 | Computer Systems | 3",\n    "CS-220 | Databases | 3",\n    "GEN-107 | Introduction to Sociology | 3"\n  ],\n  "Summary": "Student still needs two core Computer Science courses and one general education requirement. Should prioritize 200-level major courses next term."\n}\n'
        }, name: 'Structured Output Parser1' } }) }, position: [-1120, 7552], name: 'Degree Audit Agent' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.7, config: { parameters: {
      columns: {
        value: {
