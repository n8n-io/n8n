const wf = workflow('', '')
							...
							},
							{
								...
								fieldOptions: {
									values: [
										{
											option:
												"=I understand higher depth and breath values I've selected may incur longer wait times and higher costs. I acknowledging this and wish to proceed with the research request.",
										},
									],
								},
								requiredField: true,
	...
	.then(
		node({
			...
			version: 1,
			config: {
				parameters: { options: {}, fieldToSplitOut: 'output.questions' },
				position: [-720, -460],
						...
						assignments: [
							{
								...
								type: 'string',
								value:
									"=Initial query: {{ $('Set Variables').first().json.prompt }}\nFollow-up Questions and Answers:\n{{\n$input.all()\n  .map(item => {\n    const q = Object.keys(item.json)[0];\n    const a = item.json[q];\n    return `question: ${q}\\nanswer: ${a}`;\n  })\n  .join('\\n')\n}}",
							},
						],
					},
				},
	...
	.then(
		node({
			...
			version: 1,
			config: {
				parameters: { options: {}, fieldToSplitOut: 'output.queries' },
				position: [-700, 820],
				name: 'SERP to Items',
			},
		}),
	...
	.then(
		node({
			...
			version: 1,
			config: { position: [-220, 1020], name: 'Item Ref' },
		}),
	)
	.then(
		node({
						...
						assignments: [
							{
								...
								type: 'array',
								value:
									'={{ $json.data.match(/<table[\\s\\S]*?<\\/table>|<ul[\\s\\S]*?<\\/ul>|<[^>]+>[^<]*<\\/[^>]+>/g) }}',
							},
						],
					},
				},
	...
	.then(
		node({
			...
			version: 1,
			config: {
				parameters: { options: {}, fieldToSplitOut: 'tag' },
				position: [-60, 1600],
				name: 'Tags to Items',
			},
		}),
	...
	.add(
		sticky(
			...
			{ name: 'Sticky Note14', color: 5, position: [-1880, 540], width: 460, height: 240 },
		),
	)
	.add(
		sticky(
			...
			{ name: 'Sticky Note15', color: 5, position: [720, -60], width: 340, height: 200 },
		),
	)
	.add(
		sticky(
			...
			{ name: 'Sticky Note16', position: [-2420, -920], width: 520, height: 1060 },
		),
	)
	.add(
		sticky(
			'![](https://res.cloudinary.com/daglih2g8/image/upload/f_auto,q_auto/v1/n8n-workflows/o4wqztloz3j6okfxpeyw#full-width)',
			{ name: 'Sticky Note17', color: 7, position: [-2420, -1180], width: 520, height: 240 },
		),
	)
	.add(
