return workflow('xiabGWCXUOe469Nm', 'Google Maps Email Scraper with HTTP Requests & JavaScript', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [140, 0], name: 'When clicking ‘Test workflow’' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [340, 0], name: 'Scrape Google Maps' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
      jsCode: 'const input = $input.first().json.data\nconst regex = /https?:\\/\\/[^\\/\\s"\'>]+/g\nconst websites = input.match(regex)\nreturn websites.map(website => ({json:{website}}))'
    }, position: [540, 0], name: 'Extract URLs' } }))
  .then(node({ type: 'n8n-nodes-base.filter', version: 2.2, config: { parameters: {
    ...
    }, position: [740, 0], name: 'Filter Google URLs' } }))
  .then(node({ type: 'n8n-nodes-base.removeDuplicates', version: 2, config: { parameters: { options: {} }, position: [940, 0] } }))
  .then(node({ type: 'n8n-nodes-base.limit', version: 1, config: { parameters: { maxItems: 10 }, position: [1120, 0] } }))
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: {} }, position: [1340, 0], name: 'Loop Over Items' } }))
  .add(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { position: [1520, -60], name: 'Wait1' } }))
  .then(node({ type: 'n8n-nodes-base.filter', version: 2.2, config: { parameters: {
      options: {},
      conditions: {
        options: {
    ...
    }, position: [1700, -60], name: 'Filter Out Empties' } }))
  .then(node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'emails' }, position: [1880, -60] } }))
  .then(node({ type: 'n8n-nodes-base.removeDuplicates', version: 2, config: { parameters: { options: {} }, position: [2080, -60], name: 'Remove Duplicates (2)' } }))
