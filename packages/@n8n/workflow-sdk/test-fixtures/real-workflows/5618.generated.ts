return workflow('BbEROzwxqcxp1yQZ', 'Scrape LinkedIn Profiles & Save to Google Sheets with Apify', { executionOrder: 'v1' })
  .add(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
    ...
    }, position: [-840, 840], name: 'Read LinkedIn URLs1' } }))
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: {} }, position: [-660, 840], name: 'Schedule Scraper Trigger 1' } }))
  .add(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      options: {},
      sheetName: {
        __rl: true,
      ...
      googleApi: { id: 'credential-id', name: 'googleApi Credential' }
    }, position: [-500, 640], name: 'Execute Apify LinkedIn Scraper1' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
    ...
    }, position: [-280, 640], name: 'Send Success Notification1' } }))
  .add(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [-460, 860], name: 'Data Formatting1' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [-280, 860], name: 'Fetch Profile Data1' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [-100, 860], name: 'Run Scraper Actor1' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
      ...
      httpHeaderAuth: { id: 'credential-id', name: 'httpHeaderAuth Credential' }
    }, position: [120, 860], name: 'Get Scraped Results1' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      ...
      googleApi: { id: 'credential-id', name: 'googleApi Credential' }
    }, position: [340, 860], name: 'Save to Google Sheets1' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
        value: {
      ...
      googleApi: { id: 'credential-id', name: 'googleApi Credential' }
    }, position: [560, 860], name: 'Update Progress Tracker1' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 10 }, position: [740, 860], name: 'Process Complete Wait 1' } }))
  .add(sticky('## Workflow Initialization\n\n### Starts the scraping and checks for required LinkedIn profile URLs or search parameters', { name: 'Sticky Note4', position: [-1160, 600], width: 440, height: 480 }))
  .add(sticky('\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n## DATA PROCESSING & EXTRACTION\n\n### Handles the actual scraping and data retrieval from Apify', { name: 'Sticky Note5', color: 3, position: [-520, 840], width: 780, height: 380 }))
  .add(sticky('## DATA STORAGE\n### Saves results in Google Sheet', { name: 'Sticky Note6', color: 4, position: [300, 740], width: 400, height: 320 }))
  .add(sticky('## COMPLETION\n### Completes the workflow and Send notification', { name: 'Sticky Note7', color: 5, position: [-540, 500], width: 400, height: 320 }))