return workflow('', '')
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-200, 405], name: 'When clicking ‘Test workflow’' } }))
  .then(node({ type: 'n8n-nodes-base.readWriteFile', version: 1, config: { parameters: {
      options: {},
    ...
    }, position: [20, 405], name: 'Read/Write Files from Disk' } }))
  .then(node({ type: 'n8n-nodes-base.extractFromFile', version: 1, config: { parameters: { options: {}, operation: 'pdf' }, position: [240, 405], name: 'Extract from File' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.8, config: { parameters: {
      ...
      promptType: 'define'
    }, subnodes: { model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatOpenRouter', version: 1, config: { parameters: { model: 'deepseek/deepseek-chat-v3-0324', options: {} }, credentials: {
          openRouterApi: { id: 'credential-id', name: 'openRouterApi Credential' }
        }, name: 'OpenRouter Chat Model' } }) }, position: [460, 420], name: 'Text Extractor' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [880, 380], name: 'Post-Processing' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [1340, 520], name: 'Send Raw Text Again' } }), node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'line_items' }, position: [1276, 280] } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [1496, 280], name: 'Generate Unique Key' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
    ...
    }, position: [1716, 280], name: 'Send Invoice Data' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
      options: {},
      sheetName: {
        __rl: true,
    ...
    }, position: [1980, 280], name: 'Fetch Master Data' } }))
  .then(node({ type: 'n8n-nodes-base.code', version: 2, config: { parameters: {
    ...
    }, position: [2156, 280], name: 'Validation' } }))
  .then(node({ type: 'n8n-nodes-base.googleSheets', version: 4.5, config: { parameters: {
      columns: {
        value: {
  ...
  .add(sticky('## Extracting Line Items', { name: 'Sticky Note4', color: 7, position: [1240, 220], width: 400, height: 220 }))
  .add(sticky('## Sending Data To G-Sheet', { name: 'Sticky Note5', color: 7, position: [1660, 200], width: 220, height: 240 }))
  .add(sticky('## Fetch Master Data & Validating Invoice Extracted Data', { name: 'Sticky Note6', color: 7, position: [1900, 200], width: 440, height: 240 }))
  .add(sticky('## Update Validation Result', { name: 'Sticky Note7', color: 7, position: [2360, 200], width: 300, height: 240 }))