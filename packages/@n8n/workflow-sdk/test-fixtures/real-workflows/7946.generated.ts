return workflow('h81ddl7uooV3eLBq', 'Hybrid Search with Qdrant & n8n, Legal AI: Retrieval', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-256, 400], name: 'Index Dataset from HuggingFace' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [-32, 400], name: 'Get Dataset Splits' } }))
  .then(node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'splits' }, position: [176, 400], name: 'Split Them All Out' } }))
  .then(node({ type: 'n8n-nodes-base.filter', version: 2.2, config: { parameters: {
    ...
    }, position: [384, 400], name: 'Keep Test Split' } }))
  .then(node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {
    ...
    }, position: [592, 400], name: 'Get Test Queries' } }))
  .then(node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'rows' }, position: [816, 400], name: 'Divide Per Row' } }))
  .then(node({ type: 'n8n-nodes-base.filter', version: 2.2, config: { parameters: {
      options: {},
      conditions: {
        options: {
    ...
    }, position: [1056, 400], name: 'Keep Questions with Answers in the Dataset' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [1280, 400], name: 'Keep Questions & IDs' } }))
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: { reset: false } }, position: [1776, 400], name: 'Loop Over Items' } }))
  .add(node({ type: 'n8n-nodes-base.aggregate', version: 1, config: { parameters: {
      options: {},
      aggregate: 'aggregateAllItemData',
      destinationFieldName: 'eval'
    }, position: [2032, 224], name: 'Aggregate Evals' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [2256, 224], name: 'Percentage of isHits in Evals' } }))
  .add(node({ type: 'n8n-nodes-base.merge', version: 3.2, config: { parameters: { mode: 'combine', options: {}, combineBy: 'combineAll' }, position: [2320, 608] } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      include: 'selected',
      options: {},
      assignments: {
    ...
    }, position: [2512, 608], name: 'isHit = If we Found the Correct Answer' } }))
