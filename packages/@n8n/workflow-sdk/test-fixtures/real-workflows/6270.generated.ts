return workflow('', '')
  .add(trigger({ type: '@n8n/n8n-nodes-langchain.chatTrigger', version: 1.1, config: { parameters: {
    ...
    }, position: [-176, -64], name: 'Example Chat' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 2.2, config: { parameters: {
      options: {
    ...
    }, subnodes: { tools: [tool({ type: 'n8n-nodes-base.rssFeedReadTool', version: 1.2, config: { parameters: {
        ...
        }, name: 'Get News' } }), tool({ type: 'n8n-nodes-base.httpRequestTool', version: 4.2, config: { parameters: {
