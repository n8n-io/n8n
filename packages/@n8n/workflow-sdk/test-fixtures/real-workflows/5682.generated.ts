return workflow('8pG71svvcOTxFAYE', 'AI Arena - Debate of AI Agents to Optimize Answers and Simulate Scenarios', {
  ...
  .add(trigger({ type: 'n8n-nodes-base.scheduleTrigger', version: 1.2, config: { parameters: {
    ...
    }, position: [-280, 220], name: 'Schedule' } }))
  .then(node({ type: 'n8n-nodes-globals.globalConstants', version: 1, config: { credentials: {
      globalConstantsApi: { id: 'credential-id', name: 'globalConstantsApi Credential' }
    }, position: [0, 340], name: 'Configure Workflow Args' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [300, 340], name: 'Prepare Input' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
    ...
    }, position: [780, 340], name: 'Guarantee Input' } }))
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: { reset: true }, batchSize: '=1' }, position: [1100, 220], name: 'Round Loop' } }))
  .then(node({ type: 'n8n-nodes-base.splitOut', version: 1, config: { parameters: { options: {}, fieldToSplitOut: 'constants.ai_agents' }, position: [1320, 400], name: 'Split Out AI Agents' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { position: [1520, 500], name: 'Wait Before Sending Agents' } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.6, config: { parameters: {
      ...
      hasOutputParser: true
    }, subnodes: { memory: memory({ type: '@n8n/n8n-nodes-langchain.memoryBufferWindow', version: 1.3, config: { name: 'Simple Memory' } }), outputParser: outputParser({ type: '@n8n/n8n-nodes-langchain.outputParserStructured', version: 1.2, config: { parameters: {
          schemaType: 'manual',
        ...
        }, name: 'JSON Output Parser' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatMistralCloud', version: 1, config: { parameters: {
          model: 'mistral-small-latest',
          options: { safeMode: true, maxRetries: 2 }
        }, credentials: {
          mistralCloudApi: { id: 'credential-id', name: 'mistralCloudApi Credential' }
        }, name: 'Mistral Cloud Chat Model' } }) }, position: [1740, 380], name: 'Debate Actor Abstraction' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 1 }, position: [2220, 300], name: 'Wait 1' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 1 }, position: [2480, 180], name: 'Wait 2' } }))
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: {
    ...
    }, position: [2720, 40], name: 'Debate Loop' } }))
  .then(node({ type: 'n8n-nodes-base.aggregate', version: 1, config: { parameters: { options: {}, aggregate: 'aggregateAllItemData' }, position: [3000, 40] } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.agent', version: 1.6, config: { parameters: {
      ...
      hasOutputParser: true
        ...
        }, name: 'Simple Memory 2' } }), outputParser: outputParser({ type: '@n8n/n8n-nodes-langchain.outputParserStructured', version: 1.2, config: { parameters: {
          schemaType: 'manual',
        ...
        }, name: 'JSON Output Parser 2' } }), model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatMistralCloud', version: 1, config: { parameters: {
          model: 'mistral-small-latest',
          options: { safeMode: true, maxRetries: 2 }
        }, credentials: {
          mistralCloudApi: { id: 'credential-id', name: 'mistralCloudApi Credential' }
        }, name: 'Mistral Cloud Chat Model 2' } }) }, position: [3220, -60], name: 'Debate Environment' } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.noOp', version: 1, config: { position: [4060, 200], name: 'End of Debate' } }), node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
      options: {},
      assignments: {
        assignments: [
          {
    ...
    }, position: [2900, 740], name: 'Update Input' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .add(trigger({ type: 'n8n-nodes-base.manualTrigger', version: 1, config: { position: [-280, 380], name: 'When clicking ‘Execute workflow’' } }))
  .add(node({ type: 'n8n-nodes-base.emailReadImap', version: 2, config: { parameters: {
      options: {
        forceReconnect: 120,
