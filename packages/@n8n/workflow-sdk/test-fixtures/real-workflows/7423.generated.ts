return workflow('c8SrVtTq7YyxoLlO', 'AI Lead Machine Agent', { executionOrder: 'v1' })
  .add(trigger({ type: 'n8n-nodes-base.formTrigger', version: 2.2, config: { parameters: {
    ...
    }, position: [64, 96], name: 'On form submission' } }))
  ...
  .then(node({ type: '@n8n/n8n-nodes-langchain.informationExtractor', version: 1.2, config: { parameters: {
    ...
    }, subnodes: { model: languageModel({ type: '@n8n/n8n-nodes-langchain.lmChatGoogleGemini', version: 1, config: { parameters: { options: {} }, name: 'Google Gemini Chat Model' } }) }, position: [-64, 448] } }))
  .then(ifBranch([node({ type: 'n8n-nodes-base.googleSheets', version: 4.6, config: { parameters: {
      columns: {
        value: {
    ...
    }, position: [640, 432], name: 'Append row in sheet' } }), node({ type: 'n8n-nodes-base.noOp', version: 1, config: { position: [640, 608], name: 'No Operation, do nothing' } })], { version: 2.2, parameters: {
      options: {},
      conditions: {
        options: {
          version: 2,
  ...
  .then(node({ type: 'n8n-nodes-base.splitInBatches', version: 3, config: { parameters: { options: {} }, position: [-400, 848], name: 'Loop Over Items' } }))
  .then(node({ type: 'n8n-nodes-base.wait', version: 1.1, config: { parameters: { amount: 1 }, position: [32, 864] } }))
  .then(node({ type: '@n8n/n8n-nodes-langchain.informationExtractor', version: 1.2, config: { parameters: {
      ...
      attributes: {
        attributes: [
          ...
          options: {}
        }, name: 'OpenAI Chat Model' } }) }, position: [352, 864], name: 'Information Extractor1' } }))
  .then(node({ type: 'n8n-nodes-base.set', version: 3.4, config: { parameters: {
    ...
    }, position: [768, 864], name: 'Edit Fields1' } }))
  .then(node({ type: 'n8n-nodes-base.gmail', version: 2.1, config: { parameters: {
      ...
      emailType: 'text'
    }, position: [992, 864], name: 'Send a message' } }))
