/**
 * Generates a single TypeScript types file with comprehensive JSDoc documentation
 * for LLM consumption. This file can be passed to an LLM to help it generate
 * n8n workflows using the @n8n/workflow-sdk.
 */

import * as fs from 'node:fs';
import * as path from 'node:path';

const OUTPUT_FILE = path.join(__dirname, '..', 'llm-types.d.ts');
...
export interface WorkflowJSON {
  ...
  connections: Record<string, {
    ...
    [connectionType: string]: Array<Array<{ node: string; type: string; index: number }>> | undefined;
  }>;
...
function main(): void {
	console.log('Generating LLM types file...');
	console.log(`Output: ${OUTPUT_FILE}`);

	fs.writeFileSync(OUTPUT_FILE, LLM_TYPES_CONTENT, 'utf-8');

	console.log('Done! Generated llm-types.d.ts');
