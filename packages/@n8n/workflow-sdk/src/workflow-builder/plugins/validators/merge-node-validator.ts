	...
	validateNode(
		...
		if (inputCount < 2) {
			issues.push({
				...
				message: `'${mapKey}' has only ${inputCount} input connection(s). Merge nodes require at least 2 inputs. Wire each upstream branch to a distinct input via \`.to(${mapKey}.input(0))\`, \`.to(${mapKey}.input(1))\`, etc.`,
				severity: 'warning',
