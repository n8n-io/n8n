/**
 * Source-level lint for workflow SDK TypeScript files.
 *
 * Graph validators cannot see statements after `export default`, repeated
 * branch overwrites, restricted-subset constructs, or mis-wrapped
 * `placeholder()` — this pass walks a best-effort AST of the source file.
 */
...
/**
 * Lint a workflow SDK source string.
 */
export function lintWorkflowSource(source: string): SourceLintIssue[] {
	...
	walk(ast, (node, parent) => {
		...
		if (node.type === 'Identifier' && DANGEROUS_GLOBALS.has(node.name)) {
			// Only bare globals — skip property names (obj.Math) and object keys.
	...
	for (const [key, info] of branchCounts) {
		...
		const [root, method] = key.split('.');
		issues.push({
			code: 'SDK_REPEATED_BRANCH_WIRING',
			message:
				`Repeated \`.${method}()\` on '${root}' (${info.count} times) — each call overwrites the previous target. ` +
