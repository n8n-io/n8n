const t0 = trigger({
	...
	version: 1.2,
	config: { parameters: { rule: { interval: [{ field: 'hours', hoursInterval: 1 }] } } },
});
...
const if1 = ifElse({
	...
	config: {
		name: 'IF 1',
		parameters: { conditions: { options: { leftValue: 'analysis.is_invoice' } } },
		executeOnce: true,
	},
}).onTrue(http3);

export default workflow('compiled', 'Compiled Workflow').add(t0.to(http1).to(ai2).to(if1));
