...
const sw1 = switchCase({ version: 3.2, config: { name: 'Switch 1', parameters: { mode: 'rules', rules: {"values":[{"conditions":{"conditions":[{"leftValue":"={{ $('Webhook').first().json.action }}","rightValue":"approve","operator":{"type":"string","operation":"equals"}}],"combinator":"and"}},{"conditions":{"conditions":[{"leftValue":"={{ $('Webhook').first().json.action }}","rightValue":"reject","operator":{"type":"string","operation":"equals"}}],"combinator":"and"}},{"conditions":{"conditions":[{"leftValue":"={{ $('Webhook').first().json.action }}","rightValue":"escalate","operator":{"type":"string","operation":"equals"}}],"combinator":"and"}}]}, options: {} }, executeOnce: true } })
  .onCase(0, http1)
  .onCase(1, http2)
  .onCase(2, http3);
