const t0 = trigger({
	...
	version: 2,
	config: { parameters: { httpMethod: 'POST', path: '/moderate', responseMode: 'responseNode' } },
});
...
const sw1 = switchCase({
	version: 3.2,
	config: { name: 'Switch 1', parameters: { value: 'body.action' }, executeOnce: true },
})
	.onCase(0, http1)
	.onCase(1, http2)
	.onCase(2, http3);
...
export default workflow('compiled', 'Compiled Workflow').add(t0.to(sw1).to(respond1));
