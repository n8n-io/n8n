...
2. **Trust empty item lists — don't synthesize fake items**
   ...
   - \`alwaysOutputData: true\` is only correct when a specific downstream node must still run on the "empty" case: either (a) a dedicated empty-case branch — pair it with an \`IF\` that routes the empty case; or (b) a node that runs once per trigger and aggregates its whole input, or is a terminal effect that must happen every run (so it must execute even with 0 input items). Set it on every upstream node that can emit 0 items, and make the downstream node treat the synthetic \`{}\` item as empty (don't blindly read fields). Never use it as a blanket default.
