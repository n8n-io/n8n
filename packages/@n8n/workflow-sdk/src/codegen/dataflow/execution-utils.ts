 ...
 * Ported from sdk-v3's simplified-compiler/execution-utils.ts.
 * Adapted for the data-flow compiler pipeline.
 */
...
export async function resolveImports(): Promise<ResolvedImports> {
	...
	>;
	let langchainKnown: Record<string, { className: string; sourcePath: string }> = {};
	try {
		// eslint-disable-next-line @typescript-eslint/no-require-imports
		langchainKnown = require(path.join(langchainPath, 'dist', 'known', 'nodes.json')) as Record<
			string,
			{ className: string; sourcePath: string }
		>;
	} catch {
		// langchain package not built — skip (only needed for AI node fixtures)
	}
...
export interface SubWorkflowRunEntry {
	name: string;
	run: IRun;
}
