...
export function generateSimplifiedCode(
	...
	const ctx: SimplifiedGenContext = {
		...
		deferredMergeDownstreams: tree.deferredMergeDownstreams ?? [],
	};
...
function formatJsonBody(jsonStr: string, ctx: SimplifiedGenContext): string {
	...
	// Handle expression-object format: ={{ { "key": expr, ... } }}
	// This is generated by the compiler when JSON body values contain node references.
	// Uses placeholder-based parsing: replace $('...').chain expressions with JSON-safe
	// placeholders, JSON.parse the result, then restore placeholders as ={{ expr }} strings.
	const exprObjMatch = /^={{ (\{[\s\S]*\}) }}$/.exec(jsonStr);
	if (exprObjMatch) {
		const inner = exprObjMatch[1];
		const placeholders: string[] = [];
		// Replace $('NodeName').chain.prop[0] expressions with JSON-safe placeholders
		const exprPattern = /\$\('[^']+'\)(?:\.[a-zA-Z_][a-zA-Z0-9_]*(?:\([^)]*\))?|\[\d+\])*/g;
		const withPlaceholders = inner.replace(exprPattern, (match) => {
			const idx = placeholders.length;
			placeholders.push(match);
			return `"__EXPR_${idx}__"`;
		});
		try {
			const parsed = JSON.parse(withPlaceholders) as Record<string, unknown>;
			const restored = restoreExprPlaceholders(parsed, placeholders);
			return formatObjectWithExpressions(restored as Record<string, unknown>, ctx);
		} catch {
			// Fall through to plain JSON parse below
		}
	}
...
/** Recursively restore __EXPR_N__ placeholders to ={{ expr }} strings in a parsed JSON value. */
function restoreExprPlaceholders(value: unknown, placeholders: string[]): unknown {
	if (typeof value === 'string') {
		const m = /^__EXPR_(\d+)__$/.exec(value);
		if (m) return `={{ ${placeholders[parseInt(m[1], 10)]} }}`;
		return value;
	}
	if (Array.isArray(value)) {
		return value.map((v) => restoreExprPlaceholders(v, placeholders));
	}
	if (typeof value === 'object' && value !== null) {
		const result: Record<string, unknown> = {};
		for (const [k, v] of Object.entries(value)) {
			result[k] = restoreExprPlaceholders(v, placeholders);
		}
		return result;
	}
	return value;
 ...
 * 1. Direct: branches converge at a Code node with `// @parallel-collect:` marker
 * 2. Via Merge: branches → Merge node → Code node with `// @parallel-collect:` marker
 *    (Merge node appears in deferredMergeDownstreams)
 *
 * Each branch has: HTTP node → aggregate Code node → (Merge or collect)
 */
function detectParallelCollectPattern(
	...
	// Strategy 1: Check fan-out targets for @parallel-collect marker directly
	for (const target of node.targets) {
	...
	// Strategy 2: Check deferredMergeDownstreams for @parallel-collect marker
	// This handles the pattern: branches → Merge → Collect Code → downstream
	if (!collectNodeName && ctx.deferredMergeDownstreams.length > 0) {
		for (const dmd of ctx.deferredMergeDownstreams) {
			if (dmd.downstreamChain) {
				const found = findParallelCollectNode(dmd.downstreamChain, ctx);
				if (found) {
					collectNodeName = found.nodeName;
					varNames = found.varNames;
					// Extract downstream nodes from the chain (after the collect node)
					if (dmd.downstreamChain.kind === 'chain') {
						const chainNodes = (dmd.downstreamChain as ChainNode).nodes;
						let pastCollect = false;
						for (const cn of chainNodes) {
							if (pastCollect) {
								downstream.push(cn);
							} else if (cn.kind === 'leaf' && (cn as LeafNode).node.name === collectNodeName) {
								pastCollect = true;
							}
						}
					}
	...
	// Extract HTTP nodes from branches
