import { TEST_WEBHOOK_TIMEOUT } from '../stubs/constants.stub';
...
import { WorkflowMissingIdError } from '../stubs/errors.stub';
import { NodeTypes } from '../stubs/node-types.stub';
import { Push } from '../stubs/push.stub';
import { removeTrailingSlash } from '../utils';
import type { TestWebhookRegistration } from './test-webhook-registrations.service';
import { TestWebhookRegistrationsService } from './test-webhook-registrations.service';
import * as WebhookHelpers from './webhook-helpers';
import * as WorkflowExecuteAdditionalData from '../stubs/execution.stub';
import type { TriggerToStartFrom } from '../types';
		...
		for (const httpMethod of ['GET', 'POST', 'PUT', 'DELETE', 'PATCH'] as IHttpRequestMethods[]) {
			...
			if (!webhook) {
				...
				if (registrations?.[key]) {
					webhook = this.getActiveWebhookFromRegistration(
						segments.join('/'),
						registrations?.[key],
					)!;
				}
