...
import { executeErrorWorkflow } from './stubs/execution.stub';
import { ExecutionService } from './stubs/execution.stub';
import { ExternalHooks } from './stubs/internal-api.stub';
import { NodeTypes } from './stubs/node-types.stub';
import { Push } from './stubs/push.stub';
...
import { WorkflowExecutionService } from './stubs/execution.stub';
import {
	WorkflowStaticDataService,
	WorkflowRepository,
	formatWorkflow,
} from './stubs/internal-api.stub';
...
export class ActiveWorkflowManager {
	...
	async addWebhooks(
		...
		for (const webhookData of webhooks) {
			...
			try {
				...
				} else if (error instanceof Error && 'detail' in error) {
	...
	private async activateWorkflow(
		...
		try {
			...
			this.executeErrorWorkflow(error as any, workflowForError, 'internal');

			// do not keep trying to activate on authorization error
			// eslint-disable-next-line @typescript-eslint/no-unsafe-call
			if ((error as any).message.includes('Authorization')) return;
	...
	async remove(workflowId: WorkflowId) {
		if (this.instanceSettings.isMultiMain) {
			try {
				...
				this.errorReporter.error(error);
				this.logger.error(
					`Could not remove webhooks of workflow "${workflowId}" because of error: "${(error as any).message}"`,
				);
			}
