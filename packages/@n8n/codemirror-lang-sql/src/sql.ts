...
export const OracleDB = SQLDialect.define({
	...
	keywords:
		SQLKeywords +
		' BEGIN END IF ELSE ELSIF LOOP FOR WHILE DECLARE EXCEPTION COMMIT ROLLBACK SAVEPOINT PACKAGE TYPE GOTO EXIT CONTINUE NULL THEN WHEN CASE ELSEIF',
	types:
		SQLTypes +
	...
	builtin:
		'ACCESS ADD ALL ALTER AND ANY AS ASC AUDIT BETWEEN BY CHAR CHECK CLUSTER COLUMN COMMENT COMPRESS CONNECT CREATE CURRENT DATE DECIMAL DEFAULT DELETE DESC DISTINCT DROP ELSE END EXCEPTION EXCLUSIVE EXISTS FILE FLOAT FOR FROM GRANT GROUP HAVING IDENTIFIED IMMEDIATE IN INCREMENT INDEX INITIAL INSERT INTEGER INTERSECT INTO IS LEVEL LIKE LOCK LONG MAXEXTENTS MINUS MLSLABEL MODE MODIFY NO NOT NOWAIT NULL NUMBER OF OFF ON OPTION OR ORDER PCTFREE PRIOR PRIVILEGE PUBLIC RAW RENAME RESOURCE REVOKE ROW ROWID ROWNUM ROWS SELECT SESSION SET SHARE SIZE SMALLINT START SUCCESSFUL SYNONYM SYSDATE SYSTIMESTAMP TABLE THEN TO TRIGGER UID UNION UNIQUE UPDATE USER USING VALUES VARCHAR VARCHAR2 VIEW WHENEVER WHERE WITH NVL NVL2 NULLIF COALESCE DECODE GREATEST LEAST TO_CHAR TO_DATE TO_NUMBER TO_TIMESTAMP TO_TIMESTAMP_TZ TO_CLOB TO_NCLOB CAST ROUND TRUNC MOD ABS CEIL FLOOR POWER SQRT EXP LN SIGN LENGTH SUBSTR INSTR LOWER UPPER INITCAP LTRIM RTRIM TRIM RPAD LPAD REPLACE TRANSLATE CONCAT CHR ASCII REGEXP_SUBSTR REGEXP_REPLACE REGEXP_INSTR COUNT SUM AVG MIN MAX STDDEV VARIANCE MEDIAN LISTAGG ROW_NUMBER RANK DENSE_RANK NTILE LEAD LAG FIRST_VALUE LAST_VALUE JSON_VALUE JSON_EXISTS JSON_QUERY JSON_TABLE EXTRACT XMLQUERY XMLTABLE VECTOR_DISTANCE VECTOR_DOT_PRODUCT VECTOR_NORMALIZE SQL%ROWCOUNT SQL%FOUND SQL%NOTFOUND SQL%ISOPEN LEVEL ROWNUM ROWID USER CURRENT_DATE CURRENT_TIMESTAMP LOCALTIMESTAMP SESSIONTIMEZONE DBTIMEZONE GOTO EXIT CONTINUE NULL THEN WHEN CASE',
	hashComments: false, // Oracle does not use # for comment
	slashComments: false, // Oracle does not use // for comment
