...
import { searchCodeBuilderNodes } from '../../node-catalog/search';
...
describe('searchCodeBuilderNodes', () => {
	describe('builder hints in search results', () => {
		it('should include builder hint for Form Trigger node', () => {
			const nodeTypeParser = new NodeTypeParser([mockFormTriggerNode, mockFormNode]);

			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['form trigger']);

			expect(results).toContain('n8n-nodes-base.formTrigger');
			expect(results).toContain('@builderHint');
			expect(results).toContain('n8n-nodes-base.form');
			expect(results).toContain('full form experience');
		});

		it('should automatically include related nodes in search results', () => {
			...
			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['respond']);

			// Should include Webhook node via @relatedNodes section
			expect(results).toContain('@relatedNodes');
			expect(results).toContain('n8n-nodes-base.webhook');
		});

		it('should include builder hint for Respond to Webhook node', () => {
			const nodeTypeParser = new NodeTypeParser([mockRespondToWebhookNode, mockWebhookNode]);

			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['respond webhook']);

			expect(results).toContain('n8n-nodes-base.respondToWebhook');
			expect(results).toContain('@builderHint');
			expect(results).toContain('responseMode');
			expect(results).toContain('responseNode');
		});

		it('should include builder hint for Form node', () => {
			const nodeTypeParser = new NodeTypeParser([mockFormNode, mockFormTriggerNode]);

			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['form']);

			expect(results).toContain('n8n-nodes-base.form');
			expect(results).toContain('@builderHint');
			expect(results).toContain('formTrigger');
		});

		it('should include builder hint for AI Agent node with outputParserStructured', () => {
			const nodeTypeParser = new NodeTypeParser([mockAgentNode, mockOutputParserStructuredNode]);

			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['agent']);

			expect(results).toContain('@n8n/n8n-nodes-langchain.agent');
			expect(results).toContain('@builderHint');
			expect(results).toContain('outputParserStructured');
			expect(results).toContain('structured JSON output');
		});

		it('should include outputParserStructured as related node when searching for AI Agent only', () => {
			...
			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['AI Agent']);
		...
		it('should show outputParserStructured in @relatedNodes when both nodes available but only agent matches search', () => {
			const nodeTypeParser = new NodeTypeParser([mockAgentNode, mockOutputParserStructuredNode]);
			...
			expect(results).toContain('@relatedNodes');
			expect(results).toContain('@n8n/n8n-nodes-langchain.outputParserStructured');
			expect(results).toContain('Structured JSON output');
		});

		it('should display related nodes inline via @relatedNodes (nodeA → nodeB)', () => {
			...
			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['Chain Node A']);

			// Node A should be found directly
			expect(results).toContain('test.chainA');

			// Node B should appear in @relatedNodes section (direct relation from A)
			expect(results).toContain('@relatedNodes');
			expect(results).toContain('test.chainB');
		});

		it('should prevent infinite recursion with circular related nodes', () => {
			...
			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['Node A']);

			// Node A should be found
			expect(results).toContain('test.nodeA');

			// Node B should appear in @relatedNodes section (only once, not infinitely)
			expect(results).toContain('@relatedNodes');
			expect(results).toContain('test.nodeB');
		});

		it('should NOT duplicate related nodes if already in search results', () => {
			const nodeTypeParser = new NodeTypeParser([mockFormTriggerNode, mockFormNode]);
			...
			const formMatches = results.match(/n8n-nodes-base\.form[^T]/g) ?? [];
			// May appear as: search result, hint text, @relatedNodes section
			expect(formMatches.length).toBeLessThanOrEqual(3);
		});

		it('should NOT include builder hint for nodes without hints', () => {
			const nodeTypeParser = new NodeTypeParser([mockHttpRequestNode]);

			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['http']);

			expect(results).toContain('n8n-nodes-base.httpRequest');
			expect(results).not.toContain('@builderHint');
		});
	});

	describe('relatedNodes with relationHint', () => {
		it('should display relatedNodes with their relationHint under @relatedNodes section', () => {
			...
			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['AI Agent V2']);

			// Should find the agent node
			expect(results).toContain('@n8n/n8n-nodes-langchain.agentV2');

			// Should have @relatedNodes section
			expect(results).toContain('@relatedNodes');

			// Should show related nodes with their hints
			expect(results).toContain('@n8n/n8n-nodes-langchain.memoryBufferWindow');
			expect(results).toContain('Maintains conversation history for context');
			expect(results).toContain('@n8n/n8n-nodes-langchain.lmChatOpenAi');
			expect(results).toContain('Connect a chat model for responses');
		});

		it('should format relatedNodes as nodeType: "relationHint"', () => {
			const nodeTypeParser = new NodeTypeParser([
				mockAgentNodeWithRelationHints,
				mockMemoryNode,
				mockChatModelNode,
		...
		it('should NOT expand relatedNodes as full search results when using new format', () => {
			const nodeTypeParser = new NodeTypeParser([
				mockAgentNodeWithRelationHints,
				mockMemoryNode,
				mockChatModelNode,
			...
			expect(results).not.toContain('[RELATED]');

			// Should NOT show "(+ X related)" count in header
			expect(results).not.toMatch(/\(\+ \d+ related\)/);
		});
	});

	describe('discriminator information in search results', () => {
		it('should include resource/operation info for nodes with that pattern', () => {
			const nodeTypeParser = new NodeTypeParser([
			...
			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['freshservice']);

			// Should include discriminator section
			expect(results).toContain('Discriminators:');

			// Should show resource info
			expect(results).toContain('resource:');
			expect(results).toContain('ticket');
			...
			expect(results).toContain('get_node_types');
			expect(results).toContain('resource');
			expect(results).toContain('operation');
		});

		it('should include mode discriminator for Code node', () => {
			const nodeTypeParser = new NodeTypeParser([
				mockFreshserviceNode,
				mockCodeNode,
				mockHttpRequestNode,
			...
			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['code']);
			...
			expect(results).toContain('runOnceForAllItems');
			expect(results).toContain('runOnceForEachItem');
		...
		it('should show "none" discriminators for nodes without patterns', () => {
			const nodeTypeParser = new NodeTypeParser([
				mockFreshserviceNode,
				mockCodeNode,
				mockHttpRequestNode,
			...
			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['http request']);

			// HTTP Request node has no discriminators, so it should show "none"
			expect(results).toContain('n8n-nodes-base.httpRequest');
			expect(results).toContain('HTTP Request');

			// Should show "Discriminators: none" for HTTP Request
			expect(results).toContain('Discriminators: none');
		});

		it('should provide correct usage example in the output', () => {
			const nodeTypeParser = new NodeTypeParser([mockFreshserviceNode]);

			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['freshservice']);

			// Should show how to call get_node_types with discriminators
			expect(results).toMatch(/get_node_types.*nodeId.*freshservice.*resource.*operation/s);
		});

		it('should include operation discriminator for operation-only nodes', () => {
			const nodeTypeParser = new NodeTypeParser([mockRemoveDuplicatesNode]);

			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['remove duplicates']);
			...
			expect(results).toContain('removeDuplicateInputItems');
			expect(results).toContain('Remove duplicates from incoming items');
			expect(results).toContain('removeItemsSeenInPreviousExecutions');
			expect(results).toContain('Deduplicate items already seen in previous executions');
			expect(results).toContain('clearDeduplicationHistory');
			expect(results).toContain('Wipe the store of previous items');

			// Should NOT show the subordinate mode discriminator
			expect(results).not.toContain('cleanDatabase');

			// Should include usage hint
			expect(results).toContain('get_node_types');
			expect(results).toMatch(/operation.*removeDuplicateInputItems/s);
		});

		it('should include related nodes in @relatedNodes section', () => {
			...
			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['Code Runner']);

			// Code Runner should be found directly
			expect(results).toContain('n8n-nodes-base.codeRunner');

			// Code node should appear in @relatedNodes section
			expect(results).toContain('@relatedNodes');
			expect(results).toContain('n8n-nodes-base.code');
		});
	});

	describe('multi-query behaviour', () => {
		it('should return results for multiple queries', () => {
			const nodeTypeParser = new NodeTypeParser([
				mockFreshserviceNode,
				mockCodeNode,
				mockHttpRequestNode,
			...
			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['freshservice', 'code']);

			// Should contain results for both queries
			expect(results).toContain('"freshservice"');
		...
		it('should handle queries with no results', () => {
			...
			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['nonexistent-node-xyz']);

			expect(results).toContain('No nodes found');
			expect(results).toContain('Try a different search term');
		});
	...
	describe('mode discriminator with outputConnectionType and SDK function mapping', () => {
		it('should show display name and SDK function for modes with outputConnectionType', () => {
			const nodeTypeParser = new NodeTypeParser([mockVectorStoreNode]);

			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['pinecone']);
			...
			expect(results).toContain('retrieve');
			expect(results).toContain('retrieve-as-tool');

			// Should show OVERRIDDEN display name for retrieve (without confusing "/Tool" text)
			expect(results).toContain('Retrieve Documents (As Vector Store for Chain)');
			expect(results).not.toContain('Retrieve Documents (As Vector Store for Chain/Tool)');
			expect(results).toContain('Retrieve Documents (As Tool for AI Agent)');
			...
			expect(results).toMatch(/retrieve.*vectorStore\(\{ mode: 'retrieve' \}\)/s);

			// retrieve-as-tool → tool({ mode: 'retrieve-as-tool' }) for subnodes.tools
			expect(results).toMatch(/retrieve-as-tool.*tool\(\{ mode: 'retrieve-as-tool' \}\)/s);
		});

		it('should show node() for modes without outputConnectionType', () => {
			const nodeTypeParser = new NodeTypeParser([mockVectorStoreNode]);

			const { results } = searchCodeBuilderNodes(nodeTypeParser, ['pinecone']);

			// Modes without outputConnectionType should be shown as standalone
			expect(results).toContain('load');
			expect(results).toContain('Get Many');
			expect(results).toContain('insert');
			expect(results).toContain('Insert Documents');
			expect(results).toContain('update');
			expect(results).toContain('Update Documents');

			// These should indicate they use node() function (no mode parameter needed for node())
			expect(results).toMatch(/load.*Get Many.*→ use node\(\)/is);
		});

		it('should NOT show SDK function for nodes where all modes use node()', () => {
			...
			// Should show mode values with display names
			expect(results).toContain('runOnceForAllItems');
			expect(results).toContain('Run Once for All Items');
			expect(results).toContain('runOnceForEachItem');
			expect(results).toContain('Run Once for Each Item');

			// Code node modes all use node(), so should NOT show SDK function mapping
			expect(results).not.toContain('node()');
			expect(results).not.toContain('→ use');
		});
	});
});
