...
import type { ListQueryWorkflow, ResourceType, WorkflowFolderUnionFull } from '../types';
...
export class WorkflowRepository extends Repository<WorkflowEntity> {
	...
	private buildBaseUnionQuery(workflowIds: string[], options: ListQueryOptions = {}) {
		const subQueryParameters: ListQueryOptions = {
			select: {
				createdAt: true,
				updatedAt: true,
				id: true,
	...
	async getWorkflowsAndFoldersUnion(workflowIds: string[], options: ListQueryOptions = {}) {
	...
	async getWorkflowsAndFoldersCount(workflowIds: string[], options: ListQueryOptions = {}) {
	...
	async getWorkflowsAndFoldersWithCount(workflowIds: string[], options: ListQueryOptions = {}) {
		if (
	...
	private enrichDataWithExtras(
		...
		extraData: {
			workflows: ListQueryWorkflow.WithSharing[] | ListQueryWorkflow.Plain[];
			folders: Folder[];
		},
	...
	async getMany(workflowIds: string[], options: ListQueryOptions = {}) {
	...
	async getManyAndCount(sharedWorkflowIds: string[], options: ListQueryOptions = {}) {
		...
		const [workflows, count] = (await query.getManyAndCount()) as [
			ListQueryWorkflow.Plain[] | ListQueryWorkflow.WithSharing[],
			number,
		];
	...
	getManyQuery(workflowIds: string[], options: ListQueryOptions = {}) {
	...
	private applyFilters(
		...
		filter?: ListQueryOptions['filter'],
	): void {
	...
	private applyPagination(qb: SelectQueryBuilder<WorkflowEntity>, options: ListQueryOptions): void {
		if (options?.take) {
