...
export class ScheduledJobRepository extends Repository<ScheduledJob> {
	...
	/**
	 * The owner kinds that currently own at least one job. A tiny result (one row
	 * per kind), so the reconciliation sweep can walk one kind at a time instead
	 * of holding every owner in memory.
	 */
	...
	/**
	 * One page of the distinct owners of a given kind, in `ownerId` order, keyset
	 * paginated so a sweep over many owners stays on the
	 * `(ownerType, ownerId, ownerMemberId)` index.
	 *
	 * Only owners with a job older than `settledBefore` are returned: a job
	 * inserted moments ago may belong to an owner whose own row is not committed
	 * yet, and reporting it would let the sweep quarantine a brand-new job.
	 *
	 ...
	 * cleared (so the materializer's partial index drops it), stamped
	 * `orphanedAt` (which starts the grace period before deletion), and its
	 * already-queued occurrences withdrawn so nothing materialized before the
	 * quarantine still fires.
	 *
	 * Both writes commit together: a job left disabled with live occurrences
	 * would fire for an owner that is gone, and one whose occurrences were
	 * withdrawn while it stayed enabled would simply queue them again.
	 *
	 * `settledBefore` keeps a job written between the caller's liveness check and
	 * this write out of it, the same bound {@link findOwnerIds} applies.
	 *
	...
	async findQuarantinedByOwnerIds(ownerType: string, ownerIds: string[]): Promise<ScheduledJob[]> {
		if (ownerIds.length === 0) {
			return [];
		}
	...
	/**
	 * Lift the quarantine on one job: re-enabled, `orphanedAt` cleared and its
	 * clock restarted from `nextRunAt` (`null` for a rule with nothing left to fire).
	 * Acts only while the job is still quarantined, so a concurrent lift or
	 * delete makes this a no-op instead of clobbering the clock they set.
	 *
...
/**
 * Matches every job an owner holds, whichever member provisioned it. Spelled
 * out rather than spread, so a caller handing over a full {@link ScheduledJobOwner}
 * cannot narrow an owner-wide delete to one member by accident.
 */
...
/**
 * Matches exactly one owner member. Spelled out rather than passed as a plain
 * object so a `null` member becomes `IS NULL` instead of `= NULL`, which
 * matches nothing.
 */
