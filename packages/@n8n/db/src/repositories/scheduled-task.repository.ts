...
export class ScheduledTaskRepository extends Repository<ScheduledTask> {
	...
	/**
	 * Whether the row's job was already running as many occurrences as its limit
	 * allows, as a SQL predicate for the given task alias. False for a job without
	 * a limit, which is what the claim treats as uncapped.
	 */
	private atConcurrencyLimitSql(alias: string, jobAlias: string): string {
		return `${jobAlias}"concurrencyLimit" IS NOT NULL
			AND ${jobAlias}"concurrencyLimit" <= (
				SELECT COUNT(*) FROM ${this.tableName} running
				 WHERE running."jobId" = ${alias}"jobId"
				   AND running."status" = '${ScheduledTaskStatus.Running}')`;
	}
