...
import type { AnnotationVote, ExecutionSummary, IRunExecutionData } from 'n8n-workflow';
...
import { AnnotationTagMapping } from '../entities/annotation-tag-mapping.ee';
import { ExecutionAnnotation } from '../entities/execution-annotation.ee';
...
import { PostgresLiveRowsRetrievalError } from '../errors';
import type { IExecutionResponse } from '../types';
...
export class ExecutionRepository extends Repository<ExecutionEntity> {
	...
	async findManyByRangeQuery(query: RangeQuery): Promise<ExecutionSummary[]> {
	...
	async fetchCount(query: CountQuery) {
	...
	private toQueryBuilder(query: Query) {
		const {
	 ...
	 */
	private toQueryBuilderWithAnnotations(query: Query) {
