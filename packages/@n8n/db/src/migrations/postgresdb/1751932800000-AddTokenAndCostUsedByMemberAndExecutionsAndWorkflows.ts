	...
	implements ReversibleMigration
{
	...
	public async up({ queryRunner, tablePrefix }: MigrationContext): Promise<void> {
		...
		await queryRunner.query(
			`ALTER TABLE "${tablePrefix}user" DROP COLUMN IF EXISTS "tokensConsumed"`,
		);
		await queryRunner.query(
			`ALTER TABLE "${tablePrefix}user" DROP COLUMN IF EXISTS "costIncurred"`,
		);
		await queryRunner.query(
			`ALTER TABLE "${tablePrefix}execution_entity" DROP COLUMN IF EXISTS "tokensConsumed"`,
		);
		await queryRunner.query(
			`ALTER TABLE "${tablePrefix}execution_entity" DROP COLUMN IF EXISTS "costIncurred"`,
		);
		await queryRunner.query(
			`ALTER TABLE "${tablePrefix}workflow_entity" DROP COLUMN IF EXISTS "tokensConsumed"`,
		);
		await queryRunner.query(
			`ALTER TABLE "${tablePrefix}workflow_entity" DROP COLUMN IF EXISTS "costIncurred"`,
		);
