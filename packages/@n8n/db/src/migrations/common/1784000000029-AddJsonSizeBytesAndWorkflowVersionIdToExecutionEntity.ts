	...
	implements ReversibleMigration
{
	async up({ escape, runQuery, isPostgres }: MigrationContext) {
		...
		await runQuery(`ALTER TABLE ${tableName} ADD COLUMN ${jsonSizeBytes} BIGINT DEFAULT NULL`);
		await runQuery(
		...
		if (isPostgres) {
			await runQuery(
				`COMMENT ON COLUMN ${tableName}.${jsonSizeBytes} IS 'Logical byte size of the JSON execution data bundle (run data, workflow snapshot, version id); excludes binary data.'`,
			);
			await runQuery(
