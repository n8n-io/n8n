 ...
 *
 * - `agents_observations`: append-only observation log keyed by
 *   `(scopeKind, scopeId, seq)`. Consumers define `kind`; payload is JSON.
 *   `compactedAt` is a soft-flag set by the compactor.
 ...
 *
 * `scopeKind` is polymorphic (`thread` / `resource` / `agent`); v1 only writes
 * `thread` rows, but the columns are in place so future scopes are a
 * behavioural change, not a schema migration. No FK on `scopeId` for that
 * reason — it can reference different parent tables depending on `scopeKind`.
 */
export class CreateAgentObservationTables1784000000000 implements ReversibleMigration {
	async up({ schemaBuilder: { createTable, column } }: MigrationContext) {
		await createTable('agents_observations')
			...
			.withUniqueConstraintOn(['scopeKind', 'scopeId', 'seq'])
		...
		await createTable('agents_observation_cursors').withColumns(
			...
			column('lastObservedSeq').bigint.notNull,
		).withTimestamps;
