...
const VALUES_AFTER = [...VALUES_BEFORE, 'teams'];

export class AllowTeamsAgentChatSubscriptions1790231058977 implements ReversibleMigration {
	...
	async down({ escape, runQuery, schemaBuilder }: MigrationContext) {
		await runQuery(
			`DELETE FROM ${escape.tableName(TABLE)} WHERE ${escape.columnName(COLUMN)} = 'teams'`,
		);
		await schemaBuilder.dropEnumCheck(TABLE, COLUMN, { recreatesOnSqlite: true });
