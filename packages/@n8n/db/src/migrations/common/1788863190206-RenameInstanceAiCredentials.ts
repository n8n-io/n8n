 ...
 * carry the new name.
 *
 * Both directions establish ownership through the Instance AI credential
 * assignment, not the name alone: an admin can create instance credentials with
 * any name through the API, and those must keep theirs. A connection that an
 * admin replaces after the upgrade is therefore not renamed back on rollback.
 */
export class RenameInstanceAiCredentials1788863190206 implements ReversibleMigration {
	...
	async down(context: MigrationContext) {
		for (const { from, to, credentialUseIds } of RENAMES) {
			await this.renameAssigned(context, to, from, credentialUseIds);
		}
	}
