...
export class CreateUsageTable1751932900000 implements ReversibleMigration {
	...
	public async up({ queryRunner, tablePrefix }: MigrationContext): Promise<void> {
		...
		await queryRunner.query(
			`CREATE INDEX IF NOT EXISTS "IDX_${tablePrefix}usage_entity_userId_executionDate" ON "${tablePrefix}usage_entity" ("userId", "executionDate")`,
		);

		await queryRunner.query(
			`CREATE UNIQUE INDEX IF NOT EXISTS "IDX_${tablePrefix}usage_entity_workflowId_userId_executionDate" ON "${tablePrefix}usage_entity" ("workflowId", "userId", "executionDate")`,
		);
	}
