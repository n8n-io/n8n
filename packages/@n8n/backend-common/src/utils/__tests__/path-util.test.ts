...
// node:fs/promises stays real (these tests build real temp dirs/symlinks); only `lstat` is a
// spy delegating to the real impl, so a single test can force an in-realm error (see below).
jest.mock('node:fs/promises', () => {
	const actual = jest.requireActual<typeof fsPromises>('node:fs/promises');
	return { ...actual, lstat: jest.fn(actual.lstat) };
});
...
describe('containsSymlinkedComponent', () => {
	...
	beforeEach(async () => {
		(fsPromises.lstat as jest.Mock).mockImplementation(
			jest.requireActual<typeof fsPromises>('node:fs/promises').lstat,
		);
	...
	it('ignores missing components', async () => {
		// Jest's VM realm makes native fs errors fail `error instanceof Error`, so drive the
		// missing-component case through an in-realm ENOENT to exercise the skip logic.
		const enoent = Object.assign(new Error('ENOENT'), { code: 'ENOENT' });
		(fsPromises.lstat as jest.Mock).mockRejectedValue(enoent);
