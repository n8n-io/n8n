...
// Task runner cannot use `getSandboxContext` because it requires `this` to be
// `IExecuteFunctions | ISupplyDataFunctions` and because `getNodeParameter` is
// not available
...
export abstract class CodeSandbox extends EventEmitter {
	...
	// 	if (json === undefined || !isObjectLiteral(json)) {
	// 		throw new ValidationError({
	...
	// 	if (binary !== undefined && !isObjectLiteral(binary)) {
	// 		throw new ValidationError({
