...
import { Type, Keyword } from './grammar.terms';
...
function getAliases(doc: Text, at: SyntaxNode) {
	...
	let aliases: null | Record<string, string[]> = null;
	for (
		...
		if (!sawFrom) {
			...
			if (next && plainID(next)) alias = idName(doc, next);
...
class CompletionLevel {
	...
	child(name: string) {
		const children: { [name: string]: CompletionLevel } =
...
export function completeFromSchema(
	...
	return (context: CompletionContext) => {
		...
		let options: readonly Completion[] = level.list;
