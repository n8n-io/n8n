...
function readNumber(input: InputStream, sawDot: boolean) {
	...
	if (input.next === Ch.E || input.next === Ch.e) {
		input.advance();
		// advance changes next, so we need to recast the variable to avoid type errors
...
export function dialect(
	...
	for (const prop in defaults)
		(dialect as unknown as Record<string, unknown>)[prop] = (
			(Object.prototype.hasOwnProperty.call(spec, prop) ? spec : defaults) as Record<
				string,
				unknown
			>
		)[prop];
