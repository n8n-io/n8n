 ...
 *
 * `projectId` is required when `scope` is `project` and refused otherwise. `userId`
 * is allowed only when `scope` is `user`: it names another user, which needs the
 * global `aiPreference` scopes, and defaults to the caller. Those rules live in the
 * service, because a refinement would stop the schema from being a plain object,
 * which the controller registry needs.
 */
