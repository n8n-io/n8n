...
/**
 * Strength of the claim one verification run supports, generated by the
 * backend from run evidence. `verified` means every planned node ran for
 * real; anything less must not be presented as verified.
 */
export const instanceAiVerificationClaimLevelSchema = z.enum([
	'verified',
	'partial',
	'unproven',
	'failed',
]);
export type InstanceAiVerificationClaimLevel = z.infer<
	typeof instanceAiVerificationClaimLevelSchema
>;

/**
 * The verification verdict, carried as structure rather than as rendered
 * prose so the client owns the wording. The event log is append-only, so a
 * persisted sentence could never be reworded or translated afterwards.
 */
export const instanceAiVerificationClaimSchema = z.object({
	level: instanceAiVerificationClaimLevelSchema,
	...
	/** Nodes the change was about that the run never proved. */
	unprovenTargets: z.array(z.string()),
	publishReady: z.boolean(),
	liveTestRecommended: z.boolean(),
});
export type InstanceAiVerificationClaim = z.infer<typeof instanceAiVerificationClaimSchema>;

export const verificationVerdictPayloadSchema = z.object({
	claim: instanceAiVerificationClaimSchema,
	workflowId: z.string().optional(),
});
...
export const instanceAiEventSchema = z.discriminatedUnion('type', [
	...
	// Structural (absent from INSTANCE_AI_EPHEMERAL_EVENT_TYPES), so it is
	// persisted with a seq and replayed — the verdict survives a refresh.
	z.object({
		type: z.literal('verification-verdict'),
		...eventBase,
		payload: verificationVerdictPayloadSchema,
	}),
]);
