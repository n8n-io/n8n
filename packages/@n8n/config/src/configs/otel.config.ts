...
export class OtelConfig {
	...
	@Env('EXPERIMENTAL_OTEL_ENABLED')
	enabled: boolean = false;

	/** OTLP exporter endpoint. */
	@Env('EXPERIMENTAL_OTEL_EXPORTER_OTLP_ENDPOINT')
	otlpEndpoint: string = 'http://localhost:4318';

	/** OTLP exporter protocol. */
	@Env('EXPERIMENTAL_OTEL_EXPORTER_OTLP_PROTOCOL', protocolSchema)
	otlpProtocol: 'http/protobuf' | 'grpc' = 'http/protobuf';

	/** Service name reported to the OTLP backend. */
	@Env('EXPERIMENTAL_OTEL_SERVICE_NAME')
	serviceName: string = 'n8n';

	/** Trace sampling rate (0.0 to 1.0). */
	@Env('EXPERIMENTAL_OTEL_TRACES_SAMPLER_ARG', sampleRateSchema)
	sampleRate: number = 1;
}
