...
def parse_allowlist(allowlist_str: str) -> Set[str]:
    ...
    return set(
        module.strip() 
        for module in allowlist_str.split(',') 
        if module.strip()
    )
...
def parse_env_vars() -> TaskRunnerOpts:
    ...
    return TaskRunnerOpts(
        ...
        max_concurrency=int(os.getenv(ENV_MAX_CONCURRENCY, str(DEFAULT_MAX_CONCURRENCY))),
        max_payload_size=int(os.getenv(ENV_MAX_PAYLOAD_SIZE, str(DEFAULT_MAX_PAYLOAD_SIZE))),
