 ...
 * compromised or buggy MCP host cannot trick the app into asking the host
 ...
 * expected to validate as well — but the app should not blindly forward
 * arbitrary strings.
 *
 * Returns `true` when the value parses as a `http(s)` URL with a non-empty
 * host. We deliberately do not enforce a specific origin: the app has no
