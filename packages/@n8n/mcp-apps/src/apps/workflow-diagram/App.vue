...
props.mcp.ontoolresult = (result) => {
	const structuredContent = result.structuredContent;
	workflow.value = isPreviewWorkflowOutput(structuredContent) ? structuredContent : null;
};

async function openWorkflow(event: MouseEvent) {
	const workflowUrl = workflow.value?.workflowUrl;
	...
	try {
		const result = await props.mcp.openLink({ url: workflowUrl });
		if (result.isError) {
			console.warn('Host rejected request to open workflow URL');
		}
	} catch (error) {
		console.warn('Failed to request host to open workflow URL', error);
	}
}
...
function isPreviewWorkflowOutput(value: unknown): value is PreviewWorkflowOutput {
	...
	return (
		...
		(value.workflowUrl === undefined || typeof value.workflowUrl === 'string') &&
		typeof value.name === 'string' &&
		(value.execution === undefined || isPreviewWorkflowExecution(value.execution)) &&
...
function isPreviewWorkflowNode(value: unknown): value is PreviewWorkflowOutput['nodes'][number] {
	...
	return (
		...
		isPreviewWorkflowNodeIcon(value.icon) &&
		(value.display === undefined || isPreviewWorkflowNodeDisplay(value.display)) &&
...
function isPreviewWorkflowExecution(
	value: unknown,
): value is NonNullable<PreviewWorkflowOutput['execution']> {
	if (!isRecord(value)) return false;
	...
	return (
		...
		(value.waitTill === undefined || value.waitTill === null || typeof value.waitTill === 'string')
	);
}

function isPreviewWorkflowNodeIcon(
	value: unknown,
): value is PreviewWorkflowOutput['nodes'][number]['icon'] {
...
function isPreviewWorkflowNodeDisplay(
	value: unknown,
): value is PreviewWorkflowOutput['nodes'][number]['display'] {
	if (!isRecord(value) || typeof value.variant !== 'string') return false;

	if (value.variant === 'node') return true;

	return (
		value.variant === 'stickyNote' &&
		typeof value.width === 'number' &&
		typeof value.height === 'number' &&
		(value.content === undefined || typeof value.content === 'string') &&
		(value.color === undefined || typeof value.color === 'number')
	);
}
...
<template>
	<main class="app-shell">
		<section v-if="workflow" class="workflow-preview" aria-live="polite">
			<a
				v-if="workflow.workflowUrl"
				class="open-workflow-link"
		...
		<section v-else class="loading-state" aria-live="polite" aria-busy="true">
			...
			<p class="loading-copy">
				Loading workflow preview<span aria-hidden="true" class="loading-dots">
					<span></span>
					<span></span>
					<span></span>
				</span>
