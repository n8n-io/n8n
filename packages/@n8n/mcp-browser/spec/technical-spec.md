...
- **MCP Server** (`@n8n/mcp-browser`) — hosts MCP tools, runs the CDP relay,
  and invokes the **agent-browser** CLI (one session per MCP connection).
- **CDP Relay** — WebSocket server bridging the automation client's CDP traffic
...
graph LR
    ...
    MCP -- "subprocess + session" --> AB
    AB -- "CDP over WebSocket\n(/cdp/{uuid})" --> RELAY
...
| `AgentBrowserAdapter` | `adapters/agent-browser.ts` | All browser operations via the `agent-browser` CLI over the relay CDP WebSocket |
...
src/
├── adapters/
│   └── agent-browser.ts     # AgentBrowserAdapter — agent-browser CLI + CDP relay
├── tools/
...
sequenceDiagram
    ...
    participant ABA as AgentBrowserAdapter
    participant RELAY as CDPRelayServer
    participant EXT as Browser Bridge Extension

    AI->>CONN: browser_connect tool
    CONN->>ABA: new AgentBrowserAdapter(config)
    ABA->>RELAY: new CDPRelayServer()
    RELAY->>RELAY: listen() on random port
    RELAY->>RELAY: waitForExtension() (15s timeout)
    EXT->>RELAY: WebSocket connect to /extension/{uuid}
    RELAY-->>ABA: extension connected
    Note over ABA: First agent-browser subprocess uses --cdp relay.cdpEndpoint()
    RELAY->>EXT: listRegisteredTabs (via first CDP client activity)
    EXT-->>RELAY: { tabs: [{ id, title, url }, ...] }
    RELAY->>RELAY: cache tab metadata (lazy — no debugger per tab yet)
    ABA-->>CONN: { pages, activePageId }
...
2. `AgentBrowserAdapter.close()` stops the relay (no separate browser teardown — user's Chrome stays open)
...
- `/cdp/{uuid}` — CDP automation client connects here (`agent-browser` daemon)
...
The relay resolves the CDP client session ID to a Chrome tab ID for routing.

### Session ID Mapping

The relay uses CDP `targetId` strings (e.g. `"B4FE7A8D1C3E…"`) as CDP session
identifiers — same as Chrome's targets. The relay maintains:
...
- `activatedTabs: Set<targetId>` — tabs whose debugger has been attached and
  `Target.attachedToTarget` sent to the CDP automation client
- `primaryTabId: string | undefined` — the first-seen tab ID

When forwarding CDP commands, the relay passes the client `sessionId`
directly as the extension's `id` parameter, since they are the same targetId
string.

Separately, `AgentBrowserAdapter` maps MCP `pageId` (extension `targetId`) to
agent-browser tab ids (`t1`, …) by pairing relay tab lists with `agent-browser tab`
output.
...
createConnectedTool(connection, name, description, inputSchema, async (state, input, pageId) => {
  // state.adapter.* — agent-browser-backed operations
...
### Tool → agent-browser → CDP → Extension flow

Tools call `AgentBrowserAdapter` methods, which run **`agent-browser`**
subcommands with `--cdp` pointing at `relay.cdpEndpoint()` and a per-connection
`--session`. agent-browser emits CDP over the relay WebSocket; the relay forwards
commands to the extension, which executes them via `chrome.debugger.sendCommand()`.

Tools never speak CDP directly. The abstraction layers are:

```
Tool code → AgentBrowserAdapter → agent-browser CLI → CDP → CDPRelayServer → Extension → chrome.debugger
```
...
When the CDP client sends `Target.setAutoAttach` on the root session, the relay
...
2. Sends `Target.attachedToTarget` to the CDP client so it can attach a session for that tab
...
The relay maps these events to `Target.attachedToTarget` and
`Target.detachedFromTarget` CDP notifications for the automation client where applicable.
