 ...
 *
 * All tab IDs are stable session IDs generated by the extension (e.g. "n8n-tab-abc123").
 */
...
export class CDPRelayServer {
	...
	/** Cached tab metadata: sessionId → { title, url }. Source of truth is extension. */
	...
	/** The primary session ID (first seen). */
	private primaryTabId: string | undefined;
	/** The most recently created session ID (for adapter.newPage() to pick up). */
	...
	private async forwardToExtension(
		...
		// sessionId IS the stable session ID — pass it directly to the extension
	...
	private async createTab(url?: string): Promise<{ id: string; title: string; url: string }> {
		...
		log.debug('createTab: result sessionId =', result.id, 'url =', result.url);
	...
	private setupExtensionEventHandlers(): void {
		...
		this.extensionConn.onmessage = <M extends keyof ExtensionEvents>(
			...
			if (method === 'forwardCDPEvent') {
				...
				// Use the extension's stable session ID as Playwright's sessionId
