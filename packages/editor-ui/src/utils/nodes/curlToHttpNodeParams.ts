...
type FlattenedObject<T extends NestedObject> = {
	[K in keyof T as `${Extract<K, string>}`]: T[K] extends NestedObject
		? FlattenedObject<T[K]>
		: T[K];
};
...
export const toHttpNodeParameters = (curlCommand: string): HttpNodeParameters => {
	const curlJson = jsonParse<CurlJson>(toJsonString(curlCommand));
	...
	httpNodeParameters.options.redirect = { redirect: {} };
