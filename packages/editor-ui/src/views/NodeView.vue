	...
	.extend({
		...
		methods: {
			...
			async keyDown (e: KeyboardEvent) {
				...
				if (e.key === 'd') {
					this.callDebounced('deactivateSelectedNode', { debounceTime: 350, trailing: false });

				} else if (e.key === 'Delete' || e.key === 'Backspace') {
					e.stopPropagation();
					e.preventDefault();

					this.callDebounced('deleteSelectedNodes', { debounceTime: 500, trailing: false });
					...
					if (lastSelectedNode !== null) {
						this.callDebounced('renameNodePrompt', { debounceTime: 1500, trailing: false }, lastSelectedNode.name);
					}
					...
					this.callDebounced('selectAllNodes', { debounceTime: 1000, trailing: false });
				} else if ((e.key === 'c') && (this.isCtrlKeyPressed(e) === true)) {
					this.callDebounced('copySelectedNodes', { debounceTime: 1000, trailing: false });
				} else if ((e.key === 'x') && (this.isCtrlKeyPressed(e) === true)) {
					// Cut nodes
					e.stopPropagation();
					e.preventDefault();

					this.callDebounced('cutSelectedNodes', { debounceTime: 1000, trailing: false });
					...
					this.callDebounced('onSaveKeyboardShortcut', { debounceTime: 1000, trailing: false });
				} else if (e.key === 'Enter') {
					...
					this.callDebounced('selectDownstreamNodes', { debounceTime: 1000, trailing: false });
					...
					this.callDebounced('nodeSelectedByName', { debounceTime: 100, trailing: false }, connections.main[0][0].node, false, true);
				} else if (e.key === 'ArrowLeft' && e.shiftKey === true) {
					// Select all downstream nodes
					e.stopPropagation();
					e.preventDefault();

					this.callDebounced('selectUpstreamNodes', { debounceTime: 1000, trailing: false });
					...
					if (nextSelectNode !== null) {
						this.callDebounced('nodeSelectedByName', { debounceTime: 100, trailing: false }, nextSelectNode, false, true);
					}
				}
			},
