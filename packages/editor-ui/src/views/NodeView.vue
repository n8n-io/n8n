	...
	.extend({
		...
		methods: {
			...
			async injectNode (nodeTypeName: string, options: AddNodeOptions = {}) {
				const nodeTypeData: INodeTypeDescription | null = this.$store.getters['nodeTypes/nodeType'](nodeTypeName);
				...
				if (options.position) {
					...
					if (this.newNodeInsertPosition) {
						...
						if (lastSelectedConnection) {
							const sourceNodeType = this.$store.getters['nodeTypes/nodeType'](lastSelectedNode.type, lastSelectedNode.typeVersion) as INodeTypeDescription | null;
			...
			async duplicateNode (nodeName: string) {
				...
				const nodeTypeData: INodeTypeDescription | null= this.$store.getters['nodeTypes/nodeType'](node.type, node.typeVersion);
			...
			removeNode (nodeName: string) {
				...
				const nodeType: INodeTypeDescription | null = this.$store.getters['nodeTypes/nodeType'](node.type, node.typeVersion);
			...
			async addNodes (nodes: INodeUi[], connections?: IConnections) {
				...
				nodes.forEach((node) => {
					nodeType = this.$store.getters['nodeTypes/nodeType'](node.type, node.typeVersion) as INodeTypeDescription | null;
