...
const hideContent = computed(() => {
	const hide = triggerPanel.value?.hideContent;
	...
	if (node.value) {
		const hideContentValue = workflowHelpers
			.getCurrentWorkflow()
			.expression.getSimpleParameterValue(node.value, hide, 'internal', {});
...
const serviceName = computed(() =>
	nodeType.value ? getTriggerNodeServiceName(nodeType.value) : '',
);

const displayChatButton = computed(() =>
	Boolean(node.value?.type === CHAT_TRIGGER_NODE_TYPE && node.value?.parameters.mode !== 'webhook'),
);

const isWebhookNode = computed(() => Boolean(node.value?.type === WEBHOOK_NODE_TYPE));
...
const isWebhookBasedNode = computed(() => Boolean(nodeType.value?.webhooks?.length));

const isPollingNode = computed(() => Boolean(nodeType.value?.polling));
...
const isWorkflowActive = computed(() => workflowsStore.isWorkflowActive);
...
const help = ref<ComponentPublicInstance | null>(null);
...
const onNodeExecute = () => emit('execute');
</script>
