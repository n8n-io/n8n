...
export const nodeBase = mixins(
	...
	methods: {
		...
		__addNode (node: INodeUi) {
			let nodeTypeData = this.$store.getters['nodeTypes/nodeType'](node.type, node.typeVersion) as INodeTypeDescription | null;
			if (!nodeTypeData) {
				// If node type is not know use by default the base.noOp data to display it
				nodeTypeData = this.$store.getters['nodeTypes/nodeType'](NO_OP_NODE_TYPE) as INodeTypeDescription;
			}
