	...
	computed: {
		...
		isSingleActiveTriggerNode (): boolean {
			const nodes = this.$store.getters.workflowTriggerNodes.filter((node: INodeUi) => {
				const nodeType =  this.$store.getters['nodeTypes/nodeType'](node.type, node.typeVersion) as INodeTypeDescription | null;
		...
		nodeType (): INodeTypeDescription | null {
			return this.data && this.$store.getters['nodeTypes/nodeType'](this.data.type, this.data.typeVersion);
		},
