	...
	<RunData
		...
		<template v-slot:node-not-run>
			<div :class="$style.noOutputData" v-if="parentNodes.length">
				...
				<NodeExecuteButton v-if="!readOnly" type="secondary" :transparent="true" :nodeName="currentNodeName" :label="$locale.baseText('ndv.input.noOutputData.executePrevious')" @execute="onNodeExecute" telemetrySource="inputs" />
