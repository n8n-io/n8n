<template>
	...
	>
		...
		>
			...
			<n8n-option
				v-for="(nodeType, i) in options"
				:key="nodeType.displayName + '_' + i"
				ref="tagRefs"
				:value="nodeType.name"
			...
			>
				<div class="danny">
					<NodeIcon class="pr-s" :node-type="nodeType" />
					<span>
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		// const options = computed<INodeUi[]>(() => {
		// 	return allNodes.value.filter((nodeType: string) => nodeType && tag.name.includes(filter.value));
		// });
		...
		onMounted(() => {
			...
			void workflowsStore.fetchAllWorkflows();
