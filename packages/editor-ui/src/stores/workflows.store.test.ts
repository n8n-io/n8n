...
describe('useWorkflowsStore', () => {
	...
	describe('executingNode', () => {
		it('should always have at least one executing node during execution', () => {
			workflowsStore.addExecutingNode('node1');

			expect(workflowsStore.executingNode).toEqual(['node1']);
			expect(workflowsStore.executingNodeCompletionQueue).toEqual([]);

			workflowsStore.addExecutingNode('node2');

			expect(workflowsStore.executingNode).toEqual(['node1', 'node2']);
			expect(workflowsStore.executingNodeCompletionQueue).toEqual([]);

			workflowsStore.addExecutingNode('node3');

			expect(workflowsStore.executingNode).toEqual(['node1', 'node2', 'node3']);
			expect(workflowsStore.executingNodeCompletionQueue).toEqual([]);

			workflowsStore.removeExecutingNode('node1');

			expect(workflowsStore.executingNode).toEqual(['node2', 'node3']);
			expect(workflowsStore.executingNodeCompletionQueue).toEqual([]);

			workflowsStore.removeExecutingNode('node2');

			expect(workflowsStore.executingNode).toEqual(['node3']);
			expect(workflowsStore.executingNodeCompletionQueue).toEqual([]);

			workflowsStore.removeExecutingNode('node3');

			expect(workflowsStore.executingNode).toEqual(['node3']);
			expect(workflowsStore.executingNodeCompletionQueue).toEqual(['node3']);

			workflowsStore.addExecutingNode('node4');

			expect(workflowsStore.executingNode).toEqual(['node4']);
			expect(workflowsStore.executingNodeCompletionQueue).toEqual([]);
		});
	});
	...
	describe('clearNodeExecutionQueue', () => {
		it('should clear all nodes from the execution queue', () => {
			workflowsStore.executingNode = ['node1', 'node2'];
			workflowsStore.executingNodeCompletionQueue = ['node1', 'node2'];

			workflowsStore.clearNodeExecutionQueue();

			expect(workflowsStore.executingNode).toEqual([]);
			expect(workflowsStore.executingNodeCompletionQueue).toEqual([]);
		});

		it('should keep the last executing node if keepLastInQueue is true and only one node is executing', () => {
			workflowsStore.executingNode = ['node1'];
			workflowsStore.executingNodeCompletionQueue = ['node1'];

			workflowsStore.clearNodeExecutionQueue(true);

			expect(workflowsStore.executingNode).toEqual(['node1']);
			expect(workflowsStore.executingNodeCompletionQueue).toEqual(['node1']);
		});
		...
		it('should remove all nodes except the last one if keepLastInQueue is true and more than one node is executing', () => {
			...
			expect(workflowsStore.executingNode).toEqual(['node2']);
			expect(workflowsStore.executingNodeCompletionQueue).toEqual(['node2']);
		});
		...
		it('should clear all nodes if keepLastInQueue is false', () => {
			...
			workflowsStore.clearNodeExecutionQueue(false);
		...
		it('should handle empty execution queue gracefully', () => {
			...
			workflowsStore.executingNodeCompletionQueue = [];
