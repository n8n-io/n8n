...
export async function getTags(context: IRestApiContext, withUsageCount = false): Promise<ITag[]> {
	return await makeRestApiRequest(context, 'GET', '/annotation-tags', { withUsageCount });
}

export async function createTag(context: IRestApiContext, params: { name: string }): Promise<ITag> {
	return await makeRestApiRequest(context, 'POST', '/annotation-tags', params);
}
...
export async function updateTag(
	...
	return await makeRestApiRequest(context, 'PATCH', `/annotation-tags/${id}`, params);
}

export async function deleteTag(context: IRestApiContext, id: string): Promise<boolean> {
	return await makeRestApiRequest(context, 'DELETE', `/annotation-tags/${id}`);
}
