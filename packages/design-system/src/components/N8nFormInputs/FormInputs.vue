<template>
	<fragment>
		<div
			...
			>
			<div
				v-for="(input, j) in filterCols(row)"
				:key="input.name"
...
<script lang="ts">
import { IFormInputsCol, IFormInputsRow } from '@/Interface';
import Vue from 'vue';
...
export default Vue.extend({
	...
	methods: {
		...
		filterCols(inputs: IFormInputsRow) {
			return inputs.filter((input: IFormInputsCol) => {
				if (!input.hasOwnProperty('visible')) {
					return true;
				}

				return input.visible(this.values);
			});
		},
	},
});
