...
// This component is visually similar to the SplitDropdownToggle component
...
// It can be used in different parts of editor UI while SplitDropdownToggle
...
import type { SplitDropdownButtonItem } from '../../types';
...
const props = withDefaults(defineProps<SplitDropdownProps>(), {
	placement: 'bottom',
...
const getItemClasses = (item: SplitDropdownButtonItem): Record<string, boolean> => {
	return {
...
const onSelect = (SplitDropdown: string) => emit('select', SplitDropdown);
...
<template>
	<div :class="['SplitDropdown-dropdown-container', $style.SplitDropdownContainer]">
		<ElDropdown
			ref="elementDropdown"
			split-button
			:placement="placement"
		...
		>
			<template #dropdown>
				<ElDropdownMenu :class="$style.userSplitDropdownsMenu">
					<ElDropdownItem
						v-for="item in items"
						:key="item.id"
						:command="item.id"
