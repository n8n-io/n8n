...
} from '../../src/node-parameters/parameter-path-template';
...
describe('resolveNativeParameterTemplate', () => {
	it('resolves a matching template on a static node', () => {
		expect(resolveNativeParameterTemplate(node({ path: 'p' }), '={{$parameter["path"]}}')).toEqual({
			resolved: true,
			value: 'p',
		});
	});
