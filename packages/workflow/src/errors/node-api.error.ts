...
export class NodeApiError extends NodeError {
	...
	constructor(
		...
		super(node, apiResponseError);

		// only for request library error
		if (apiResponseError.error) {
			removeCircularRefs(apiResponseError.error as JsonObject);
		}
		...
		if (
			!description &&
			(apiResponseError.description || (apiResponseError?.reason as IDataObject)?.description)
		) {
			// eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
			this.description = (apiResponseError.description ||
				(apiResponseError?.reason as IDataObject)?.description) as string;
		}
		...
		if (
			...
			(apiResponseError.message ||
				(apiResponseError?.reason as IDataObject)?.message ||
				description)
		) {
			// eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
			this.message = (apiResponseError.message ||
				// eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
				(apiResponseError?.reason as IDataObject)?.message ||
		...
		if (apiResponseError.reason) {
			const reason: IDataObject = apiResponseError.reason as unknown as IDataObject;

			if (reason.isAxiosError && reason.response) {
				apiResponseError = reason.response as JsonObject;
			}
		}
		...
		if (httpCode) {
			...
			this.httpCode =
				this.findProperty(apiResponseError, ERROR_STATUS_PROPERTIES, ERROR_NESTING_PROPERTIES) ??
				null;
		}
		...
		if (!this.description) {
			if (parseXml) {
				this.setDescriptionFromXml(apiResponseError.error as string);
			} else {
		...
		[this.message, this.description] = this.setDescriptiveErrorMessage(
			...
			// eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
			this.httpCode ||
				(apiResponseError?.code as string) ||
				((apiResponseError?.reason as JsonObject)?.code as string) ||
				undefined,
			messageMapping,
		);
