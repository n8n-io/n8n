...
export class Workflow {
	...
	/**
	 * Validates the workflow structure to detect problematic patterns such as cycles
	 * and specifically SplitInBatches "done" outputs connecting back to inputs.
	 *
	 * This method performs a depth-first search traversal of the workflow graph
	 * starting from trigger nodes to detect cycles that could cause infinite loops.
	 *
	 * @example
	 * ```typescript
	 * const workflow = new Workflow(workflowData);
	 * const result = workflow.validateWorkflowStructure();
	 *
	 * if (!result.isValid) {
	 *   result.warnings.forEach(warning => {
	 *     console.log(`Warning: ${warning.message}`);
	 *     if (warning.type === 'splitInBatchesDoneLoop') {
	 *       // Handle SplitInBatches specific issue
	 *     }
	 *   });
	 * }
	 * ```
	 *
	 * @returns Validation result with warnings about detected issues and validity status
	 */
	validateWorkflowStructure(): IWorkflowValidationResult {
		const warnings: IWorkflowValidationWarning[] = [];

		try {
			// Check if validation is enabled in workflow settings
			const shouldValidate = this.settings.validateWorkflowStructure !== false; // true by default
			if (!shouldValidate) {
			...
			if (nodeCount > maxNodes) {
				warnings.push({
					node: Object.keys(this.nodes)[0],
					type: 'workflowTooLarge',
					message: `Workflow validation skipped: too many nodes (${nodeCount} > ${maxNodes})`,
				});
				return { warnings, isValid: false };
			}

			// Optimized starting node discovery - single pass through all nodes
			const allNodes = Object.values(this.nodes);
			const startingNodes: INode[] = [];
			const triggerNodes: INode[] = [];

			for (const node of allNodes) {
				if (node.disabled === true) continue;

				// Check for trigger nodes first
				if (this.nodeTypes.getByNameAndVersion(node.type, node.typeVersion)?.trigger) {
					triggerNodes.push(node);
				} else if (STARTING_NODE_TYPES.includes(node.type)) {
					startingNodes.push(node);
				}
			}

			// Prefer trigger nodes, fallback to starting nodes, then all enabled nodes
			const nodesToCheck =
				triggerNodes.length > 0
					? triggerNodes
					: startingNodes.length > 0
						? startingNodes
						: allNodes.filter((node) => !node.disabled);

			// Track visited nodes for cycle detection using DFS
			const visitedNodes = new Set<string>();
			const visitingNodes = new Set<string>(); // Nodes currently in the traversal path
			const nodeStack: string[] = []; // Track the current path for cycle reporting

			// Perform DFS traversal from each starting node with depth limits
			for (const startNode of nodesToCheck) {
				if (visitedNodes.has(startNode.name)) {
			...
			// Graceful error handling - return warning instead of throwing
			warnings.push({
				node: 'unknown',
	...
	/**
	 * Performs depth-first search to detect cycles in the workflow graph.
	 *
	 * Uses the standard DFS cycle detection algorithm with visited and visiting sets
	 * to identify back edges that indicate cycles. Includes safety limits to prevent
	 * stack overflow on very deep or malformed workflows.
	 *
	 * @param nodeName - Current node being visited
	 * @param visited - Set of completely processed nodes
	 * @param visiting - Set of nodes currently in the traversal path (gray nodes)
	 * @param nodeStack - Current path stack for cycle reconstruction
	 * @param warnings - Array to collect validation warnings
	 * @param maxDepth - Maximum traversal depth to prevent stack overflow
	...
	private detectCyclesDFS(
		...
		if (currentDepth > maxDepth) {
			warnings.push({
				node: nodeName,
				type: 'validationError',
				message: `Maximum validation depth exceeded at node "${nodeName}" (depth: ${currentDepth})`,
			});
			return;
		}
		if (visiting.has(nodeName)) {
			// Found a cycle - trace back through the stack to find the cycle path
			const cycleStartIndex = nodeStack.indexOf(nodeName);
			const cyclePath = nodeStack.slice(cycleStartIndex).concat([nodeName]);

			// Check if this is a SplitInBatches "done" -> input cycle
			const isSplitInBatchesCycle = this.checkSplitInBatchesDoneLoop(cyclePath);

			warnings.push({
				node: nodeName,
				type: isSplitInBatchesCycle ? 'splitInBatchesDoneLoop' : 'cycle',
				message: isSplitInBatchesCycle
					? `SplitInBatches node "${nodeName}" has its "done" output connected back to its input, creating a problematic loop`
					: `Circular dependency detected in workflow: ${cyclePath.join(' → ')}`,
				details: {
					nodePath: cyclePath,
				},
			});
		...
		// Get the node to check if it exists and is not disabled
		...
		if (this.connectionsBySourceNode[nodeName]) {
			const nodeConnections = this.connectionsBySourceNode[nodeName];

			// Check all connection types (main, etc.)
			for (const connectionType of Object.keys(nodeConnections)) {
				const connectionsOfType = nodeConnections[connectionType as NodeConnectionType];

				if (connectionsOfType) {
					// Check each output index
					for (let outputIndex = 0; outputIndex < connectionsOfType.length; outputIndex++) {
						const connections = connectionsOfType[outputIndex];

						if (connections) {
							for (const connection of connections) {
								// Recursively check connected node
								this.detectCyclesDFS(
									connection.node,
									visited,
									visiting,
	...
	/**
	 * Checks if a detected cycle represents a SplitInBatches "done" output loop.
	 *
	 * This method examines a cycle path to determine if it contains a SplitInBatches
	 * node where the "done" output (index 0) connects back to create the cycle.
	 * This is the specific problematic pattern identified in PAY-2940.
	 *
	 * @param cyclePath - Array of node names representing the detected cycle
	 * @returns true if this is a SplitInBatches "done" output loop, false otherwise
	 * @private
	 */
	private checkSplitInBatchesDoneLoop(cyclePath: string[]): boolean {
		if (cyclePath.length < 2) {
			return false;
		}

		// Look for SplitInBatches nodes in the cycle
		for (let i = 0; i < cyclePath.length - 1; i++) {
			const currentNodeName = cyclePath[i];
			const nextNodeName = cyclePath[i + 1];

			const currentNode = this.getNode(currentNodeName);
			if (!currentNode || currentNode.type !== SPLIT_IN_BATCHES_NODE_TYPE) {
				continue;
			}

			// Check if there's a connection from this SplitInBatches node to the next node
			const connections = this.connectionsBySourceNode[currentNodeName];
			if (!connections || !connections.main) {
				continue;
			}

			// Check if the connection is from the "done" output (index 0)
			const doneConnections = connections.main[0]; // "done" output is at index 0
			if (doneConnections) {
				const hasConnectionToNext = doneConnections.some((conn) => conn.node === nextNodeName);
				if (hasConnectionToNext) {
					// This is a SplitInBatches "done" output connecting in the cycle
					return true;
				}
			}
		}
