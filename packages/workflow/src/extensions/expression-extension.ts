...
import { EXTENSION_OBJECTS } from '@n8n/expression-runtime';
...
export { extend, extendOptional, EXTENSION_OBJECTS } from '@n8n/expression-runtime';

const EXPRESSION_EXTENDER = 'extend';
const EXPRESSION_EXTENDER_OPTIONAL = 'extendOptional';

// Generic extensions (isEmpty/isNotEmpty) available on all types
const genericExtensionNames = ['isEmpty', 'isNotEmpty'];

const EXPRESSION_EXTENSION_METHODS = Array.from(
	new Set([
		...EXTENSION_OBJECTS.flatMap((ext) => Object.keys(ext.functions)),
		...genericExtensionNames,
	]),
);
