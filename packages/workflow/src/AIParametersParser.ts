...
export type AIParametersParserOptions = {
	node: INode;
};
...
export class AIParametersParser {
	 ...
	 */
	constructor(private readonly options: AIParametersParserOptions) {}
			...
			if (parenthesesCount === 0) {
				try {
					...
					throw new NodeOperationError(
						this.options.node,
						`Failed to parse $fromAI arguments: ${argsString}: ${String(error)}`,
					);
				}
			} else {
