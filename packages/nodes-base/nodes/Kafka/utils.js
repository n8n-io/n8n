 ...
 */
export function resolveRetryDelay(value, logger) {
    if (value === undefined)
        return DEFAULT_ERROR_RETRY_DELAY_MS;
    if (typeof value !== 'number' || !isUsableDelay(value)) {
        logger?.warn(`Kafka "Retry Delay on Error" of ${String(value)} cannot be used as a delay, falling back to ${DEFAULT_ERROR_RETRY_DELAY_MS}ms`);
        return DEFAULT_ERROR_RETRY_DELAY_MS;
    }
    return value;
}
 ...
 */
export function formatAndValidatePem(value, fieldName) {
 ...
 */
export function resolveKafkaSsl(credentials) {
    if (!credentials.ssl)
        return false;
...
export async function createConfig(ctx) {
    ...
    if (credentials.authentication) {
        if (!(credentials.username && credentials.password)) {
            throw new NodeOperationError(ctx.getNode(), 'Username and password are required for authentication');
        }
        config.sasl = {
 ...
 */
export function createConsumerConfig(ctx, options, nodeVersion) {
 ...
 */
export function configureMessageParser(options, logger, registry, prepareBinaryData) {
 ...
 */
export function toUserFacingConsumerError(node, error) {
 ...
 */
export function connectEventListeners(consumer, logger, onFatalCrash) {
 ...
 */
export function disconnectEventListeners(listeners) {
...
export function sanitizeRegistryError(error) {
 ...
 */
export async function getSchemaRegistryOptions(ctx, fallbackUrl) {
    const emptyConfigError = () => new NodeOperationError(ctx.getNode(), 'Select a Schema Registry credential or enter a Schema Registry URL');
    ...
    if (credentials.authentication === 'basicAuth') {
        if (!(credentials.username && credentials.password)) {
            throw new NodeOperationError(ctx.getNode(), 'Username and password are required for Schema Registry Basic Auth');
        }
 ...
 * @param fallbackUrl - The `schemaRegistryUrl` node parameter, used when no credential is selected
 */
export async function createSchemaRegistry(ctx, fallbackUrl) {
 ...
 */
export async function setSchemaRegistry(ctx) {
 ...
 */
export function configureDataEmitter(ctx, options, nodeVersion, closeSignal) {
    ...
    const abortPromise = new Promise((_, reject) => {
        closeSignal.addEventListener('abort', () => reject(new OperationalError('Trigger closed before the execution finished, offsets not resolved.')), { once: true });
    });
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => {
                timeoutId = setTimeout(() => {
                    reject(new NodeOperationError(ctx.getNode(), `Execution took longer than the configured workflow timeout of ${executionTimeoutInSeconds} seconds to complete, offsets not resolved.`));
 ...
 */
export function getAutoCommitSettings(options) {
 ...
 */
export async function stopAndDisconnectConsumer(consumer, logger, timeoutMs) {
    let teardownError;
 ...
 */
export async function runWithHeartbeat(task, heartbeat, intervalMs = 3000) {
    let timer;
    try {
