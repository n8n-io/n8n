...
import { setSchemaRegistry } from '../utils';
...
export class KafkaTriggerV2 {
    ...
    async trigger() {
        ...
        const parseMessage = createMessageParser(settings.parser, this.logger, registry, this.helpers.prepareBinaryData);
        ...
        const startConsumerOnce = async () => {
            ...
            let reportFatal;
            const startupFailure = new Promise((_, reject) => (reportFatal = reject));
            ...
            try {
                ...
                const consumer = await createKafkaConsumer(credentials, settings.consumer, {
                    ...
                    onFatalError: (error) => {
                        if (closeController.signal.aborted)
                            return;
                        reportFatal(explainManualRunGroupDenial(error, settings.configuredGroupId, settings.isManualRun));
                    },
                });
                ...
                // Startup failed.
                reportFatal = (fatal) => this.logger.error('Kafka consumer reported a fatal error after startup had failed', {
                    error: fatal,
                });
