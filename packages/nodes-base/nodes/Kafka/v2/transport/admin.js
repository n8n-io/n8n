 ...
 */
export async function assertTopicExists(credentials, topic, logger) {
    if (topic.startsWith('^'))
        return;
    ...
    try {
        ...
        if (hasErrorCode(error, ErrorCodes.ERR_UNKNOWN_TOPIC_OR_PART)) {
            ...
            throw new UserError(`Kafka topic "${topic}" does not exist. Create the topic on the broker, or correct the Topic field, then publish the workflow again.`, {
                level: 'warning',
                description: 'Publishing anyway would leave the workflow showing as published while consuming nothing, because a topic created later is only picked up at the next broker metadata refresh, minutes away.',
        ...
        if (hasErrorCode(error, ErrorCodes.ERR_TOPIC_EXCEPTION)) {
            throw new UserError(`Kafka topic "${topic}" is not a valid Kafka topic name. Correct the Topic field, then publish the workflow again.`, { level: 'warning', cause: error instanceof Error ? error : undefined });
        }
