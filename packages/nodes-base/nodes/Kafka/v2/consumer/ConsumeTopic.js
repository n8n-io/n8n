 ...
 */
export async function consumeTopic(consumer, options) {
    ...
    try {
        ...
        try {
            ...
            await withTimeout(consumer.disconnect(), CLOSE_TIMEOUT_MS, 'Kafka consumer did not disconnect in time');
        }
        catch {
 ...
 */
async function waitForGroupJoin(consumer, signal, closed, startupFailure) {
...
function resolveSettings(options) {
    ...
    return {
        ...
        partitionsConsumedConcurrently: positiveCount(options.partitionsConsumedConcurrently, DEFAULT_PARTITIONS_CONSUMED_CONCURRENTLY, 'Parallel Processing', logger),
