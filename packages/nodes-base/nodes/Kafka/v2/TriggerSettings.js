 ...
 */
function heartbeatWithinSession(heartbeatInterval, sessionTimeout, logger) {
    ...
    logger?.warn('Kafka Heartbeat Interval lowered to stay under a third of the Session Timeout, so the consumer is not dropped from its group', { supplied: heartbeatInterval, applied: largest, sessionTimeout });
    return largest;
}
/**
 ...
 */
function millisecondsInRange(value, max, label, logger) {
    if (value === undefined)
        return undefined;
...
export function getSettings() {
    ...
    const resolveOffsetMode = isManualRun
        ...
        : // Falls back to the field's own default rather than v1's 'immediately', so
            ...
            this.getNodeParameter('resolveOffset', 'onCompletion');
    const configuredGroupId = this.getNodeParameter('groupId');
    return {
        topic: this.getNodeParameter('topic'),
        isManualRun,
        configuredGroupId,
        ...
        isManualRun ? { ...options, fromBeginning: false } : options, manualRunGroupId(configuredGroupId, isManualRun), executionTimeout, this.logger),
        emitter: toEmitterOptions(options, resolveOffsetMode, this.getNodeParameter('allowedStatuses', []), executionTimeout),
        parser: options,
 ...
 */
export function manualRunGroupId(configured, isManualRun) {
...
const MANUAL_RUN_PREFIX = (configured) => `${configured}-n8n-manual-`;
 ...
 * @param isManualRun - Whether this is an editor test run
 */
export function explainManualRunGroupDenial(error, configuredGroupId, isManualRun) {
    if (!isManualRun || !GROUP_AUTHORIZATION_FAILED.test(error.message))
        return error;
 ...
 */
export function toConsumerOptions(options, groupId, executionTimeoutSeconds, logger) {
    ...
    const fromWorkflow = typeof executionTimeoutSeconds === 'number' &&
        Number.isFinite(executionTimeoutSeconds) &&
        executionTimeoutSeconds > 0;
    const configured = options.rebalanceTimeout;
    const usableOption = typeof configured === 'number' && Number.isFinite(configured) && configured > 0;
    ...
    if (floored !== deadlineMs) {
        logger?.warn('Kafka processing deadline raised to the Session Timeout, the shortest the library allows', { requestedMs: deadlineMs, appliedMs: floored, sessionTimeout });
    }
    ...
    return {
        groupId,
        sessionTimeout,
        heartbeatInterval: heartbeatWithinSession(options.heartbeatInterval ?? DEFAULT_HEARTBEAT_INTERVAL_MS, sessionTimeout, logger),
        rebalanceTimeout,
        ...
        autoCommitInterval: millisecondsInRange(options.autoCommitInterval, MAX_AUTO_COMMIT_INTERVAL_MS, 'Auto Commit Interval', logger),
    };
}
/**
 ...
 * @param executionTimeoutSeconds - `getWorkflowSettings().executionTimeout`, raw
 */
export function toEmitterOptions(options, resolveOffsetMode, allowedStatuses, executionTimeoutSeconds) {
    return {
        resolveOffsetMode,
        allowedStatuses,
...
//# sourceMappingURL=TriggerSettings.js.map