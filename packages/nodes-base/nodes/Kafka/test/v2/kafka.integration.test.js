...
const logger = mock();
const prepareBinaryData = (async () => mock());
let stack;
let credentials;
...
beforeAll(async () => {
    ...
    const kafka = stack.serviceResults.kafka;
    credentials = {
    ...
    inBroker = async (command) => {
        ...
        if (exitCode !== 0)
            throw new Error(`in-broker command failed (${exitCode}): ${output}`);
        return output;
    };
});
afterAll(async () => {
...
const uniqueTopic = (prefix) => `n8n-v2-${prefix}-${Date.now()}`;
async function createTopic(topic) {
    await inBroker('kafka-topics --bootstrap-server localhost:9092 --create --if-not-exists ' +
 ...
 */
async function produce(topic, message, codec) {
    await inBroker(`echo '${message}' | kafka-console-producer --bootstrap-server localhost:9092 ` +
        `--topic ${topic}${codec ? ` --compression-codec ${codec}` : ''}`);
}
/** Writes several messages in one producer run, one per line. */
async function produceMany(topic, messages) {
    await inBroker(`printf '${messages.join('\\n')}\\n' | kafka-console-producer ` +
 ...
 */
async function startConsumer(topic, groupSuffix = 'group') {
    const consumer = await createKafkaConsumer(credentials, {
        groupId: `${topic}-${groupSuffix}`,
        fromBeginning: true,
    });
    let deliver;
    const firstItem = new Promise((resolve) => (deliver = resolve));
...
describe('v2 consumer against a real broker', () => {
    ...
    it.each(['gzip', 'snappy', 'lz4', 'zstd'])('receives and parses a %s-compressed message with no compression configuration', async (codec) => {
...
describe('delivery guarantees against a real broker', () => {
    it('chunks a real batch by batch size, so each chunk is one execution', async () => {
        ...
        try {
            ...
            while (seen() < 7 && Date.now() < deadline)
                await sleep(250);
    ...
    it('re-delivers a message the workflow refused, rather than committing it', async () => {
        ...
        const consumeOnce = async (mayAdvance) => {
...
describe('a group the consumer can never join fails startup (ENT-340)', () => {
    ...
    it('rejects with the broker refusal instead of reporting a successful start', async () => {
        ...
        kafkajsConsumer = new Kafka({ clientId: 'ent340-v1', brokers: [credentials.brokers] }).consumer({ groupId });
        await kafkajsConsumer.connect();
        await kafkajsConsumer.subscribe({ topic });
        await kafkajsConsumer.run({ eachMessage: async () => { } });
        await withDeadline((async () => {
            while (!(await inBroker(`kafka-consumer-groups --bootstrap-server localhost:9092 --describe --group ${groupId} --state`)).includes('Stable')) {
                await sleep(500);
            }
        })(), 30_000, 'the kafkajs consumer to hold the group');
        ...
        const consumer = await createKafkaConsumer(credentials, { groupId }, { logger, onFatalError: (error) => failStartup(error) });
        ...
        await expect(withDeadline(startup, 30_000, 'startup to settle')).rejects.toThrow(/inconsistent group protocol/i);
        // The refused consumer must not linger in the group.
        const members = await inBroker(`kafka-consumer-groups --bootstrap-server localhost:9092 --describe --group ${groupId} --state`);
...
describe('library logging against a real broker', () => {
    ...
    const recordingLogger = (nodeLogger, onFatalError) => {
...
describe('a missing topic against a real broker', () => {
    it('refuses activation, naming the topic', async () => {
        ...
        await expect(assertTopicExists(credentials, topic, logger)).rejects.toThrow(`Kafka topic "${topic}" does not exist`);
    });
...
describe('version 1 and version 2 item parity', () => {
    ...
    it('delivers the same item content for one message, on separate consumer groups', async () => {
        ...
        try {
            ...
            const deadline = Date.now() + 60_000;
            while ((!v1.emit.mock.calls[0] || !v2Item) && Date.now() < deadline)
                await sleep(250);
            const v1Item = v1.emit.mock.calls[0]?.[0]?.[0]?.[0];
