...
import { confluentKafkaModuleMock, failNextAdminDisconnect, failNextTopicMetadata, getFakeAdmins, getFakeClientConfigs, invalidTopicNameError, resetConfluentKafkaRecordings, unknownTopicError, } from '../../mocks/confluent-kafka';
...
const lastAdmin = () => {
    ...
    if (!admin)
        throw new Error('the fake recorded no admin client');
    return admin;
};
describe('assertTopicExists', () => {
    ...
    it('puts the fix in the message, which is the only part a failed publish shows', async () => {
        ...
        failNextTopicMetadata(unknownTopicError());
        failNextTopicMetadata(unknownTopicError());
        await expect(assertTopicExists(credentials, 'missing-topic', logger)).rejects.toThrow('Create the topic on the broker, or correct the Topic field, then publish the workflow again');
    });
    it('proceeds on an inconclusive check rather than blocking activation', async () => {
        ...
        await expect(assertTopicExists(credentials, 'my-topic', logger)).resolves.toBeUndefined();
        expect(logger.warn).toHaveBeenCalledWith('Kafka topic could not be verified before starting the consumer', expect.objectContaining({ topic: 'my-topic' }));
    });
    ...
    it('does not treat some other error code as a missing topic', async () => {
        failNextTopicMetadata(Object.assign(new Error('Broker: Group authorization failed'), { code: 30 }));
        await expect(assertTopicExists(credentials, 'my-topic', logger)).resolves.toBeUndefined();
    });
    it('disconnects the admin client on every path', async () => {
        ...
        failNextTopicMetadata(unknownTopicError());
        failNextTopicMetadata(unknownTopicError());
        await expect(assertTopicExists(credentials, 'missing-topic', logger)).rejects.toThrow(UserError);
        expect(lastAdmin().disconnect).toHaveBeenCalledTimes(1);
    });
    it('reports the missing topic even if the admin disconnect then fails', async () => {
        ...
        await expect(assertTopicExists(credentials, 'missing-topic', logger)).rejects.toThrow('Kafka topic "missing-topic" does not exist');
    });
