...
describe('toKafkaJSConfig', () => {
    ...
    it.each(['plain', 'scram-sha-256', 'scram-sha-512'])('adds kafkaJS.sasl for SASL mechanism %s, with SSL off', (saslMechanism) => {
        expect(toKafkaJSConfig(creds({ authentication: true, saslMechanism, username: 'user', password: 'pass' }))).toStrictEqual({
            kafkaJS: {
                brokers: ['localhost:9092'],
                clientId: 'test',
                ssl: false,
    ...
    it.each(['plain', 'scram-sha-256', 'scram-sha-512'])('adds kafkaJS.sasl for SASL mechanism %s, with SSL on and no cert material', (saslMechanism) => {
        expect(toKafkaJSConfig(creds({
            ssl: true,
            authentication: true,
            saslMechanism,
            username: 'user',
    ...
    it('defaults kafkaJS.sasl.mechanism to plain when saslMechanism is omitted', () => {
        const result = toKafkaJSConfig(creds({ authentication: true, username: 'user', password: 'pass' }));
    ...
    it('throws when a client certificate is provided without a private key', () => {
        ...
        expect(() => toKafkaJSConfig(creds({ ssl: true, cert: CERT_PEM }))).toThrow('Kafka mTLS needs both a client certificate and a client private key');
    });
    it('throws when a client private key is provided without a certificate', () => {
        ...
        expect(() => toKafkaJSConfig(creds({ ssl: true, key: KEY_PEM }))).toThrow('Kafka mTLS needs both a client certificate and a client private key');
    });
    ...
    it('throws when a PEM value is malformed', () => {
        expect(() => toKafkaJSConfig(creds({ ssl: true, ca: 'not-a-pem-block' }))).toThrow('The Kafka CA certificate is not a valid PEM block');
    });
