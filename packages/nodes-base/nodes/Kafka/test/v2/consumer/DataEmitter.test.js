...
import { createDataEmitter, } from '../../../v2/consumer/DataEmitter';
vi.mock('@n8n/utils/sleep', () => ({ sleep: vi.fn(async () => { }) }));
const ITEMS = [{ json: { message: 'hello' } }];
const mockedSleep = vi.mocked(sleep);
const run = (status) => mock({ status: status });
let ctx;
let emitSpy;
let logger;
beforeEach(() => {
...
describe('createDataEmitter', () => {
    ...
    describe('onStatus', () => {
        ...
        it.each(['success', 'canceled'])('allows the offset after an allowed %s run', async (status) => {
        ...
        it.each([undefined, []])('fails fast when the status list is %s', (allowedStatuses) => {
            expect(() => build({ resolveOffsetMode: 'onStatus', allowedStatuses })).toThrow(NodeOperationError);
        });
    });
    describe('waiting', () => {
        it('gives up when the execution outlives the workflow timeout', async () => {
            ...
            try {
                ...
                expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('longer than the configured workflow timeout of 10 seconds'), expect.anything());
            }
            finally {
                vi.useRealTimers();
            }
        });
        it.each([-1, 0])('never times out when the workflow timeout is %s, meaning unbounded', async (executionTimeoutSeconds) => {
            vi.useFakeTimers();
            try {
    ...
    describe('retry delay', () => {
        ...
        // broker allows.
        it.each([undefined, NaN, -1, Infinity, 2_147_483_648])('falls back to the default delay when the delay is %s', async (errorRetryDelay) => {
...
//# sourceMappingURL=DataEmitter.test.js.map