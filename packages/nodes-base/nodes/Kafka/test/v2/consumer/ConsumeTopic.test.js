...
import { confluentKafkaModuleMock, getFakeConsumers, resetConfluentKafkaRecordings, } from '../../mocks/confluent-kafka';
...
const echoMessage = async (message, topic) => ({ json: { message: message.value?.toString(), topic } });
const parseMessage = vi.fn(echoMessage);
const emit = vi.fn(async (_items) => ({ mayAdvance: true }));
let logger;
beforeEach(() => {
...
const newConsumer = async () => {
    ...
    if (!consumer)
        throw new Error('the fake recorded no consumer');
    return consumer;
};
...
const messages = (...values) => values.map((value) => ({ value: Buffer.from(value) }));
describe('consumeTopic', () => {
    describe('startup', () => {
        ...
        it.each([
            ['connect', (consumer) => consumer.connect],
            ['subscribe', (consumer) => consumer.subscribe],
            ['run', (consumer) => consumer.run],
            ...
            await expect(consumeTopic(consumer, { topic: 'orders', parseMessage, emit, logger })).rejects.toThrow('start failed');
    ...
    describe('chunking', () => {
        ...
        it('parses every message with the batch topic', async () => {
            ...
            expect(parseMessage).toHaveBeenNthCalledWith(1, expect.objectContaining({ value: Buffer.from('a') }), 'orders');
        });
    });
    ...
    describe('interruptions', () => {
        ...
        it('lets an emit rejection propagate, leaving the chunk unresolved', async () => {
            ...
            await expect(consumer.deliverBatch({ messages: messages('a') })).rejects.toThrow('emit exploded');
            expect(consumer.payloadSpies.resolveOffset).not.toHaveBeenCalled();
        });
        ...
        it.each([0, -3, 1.7, NaN, Infinity])('treats a batch size of %s as usable, rather than skipping or looping', async (batchSize) => {
        ...
        it.each([0, -3, NaN])('treats a partition concurrency of %s as one', async (partitionsConsumedConcurrently) => {
        ...
        // can serve it.
        it.each([NaN, -1])('falls back to the default delay when the retry delay is %s', async (errorRetryDelay) => {
            vi.useFakeTimers();
            try {
    ...
    describe('close', () => {
        ...
        it('gives up on a disconnect that never settles, so teardown cannot hang', async () => {
            ...
            try {
                ...
                const closing = expect(handle.close()).rejects.toThrow('Kafka consumer did not disconnect in time');
...
//# sourceMappingURL=ConsumeTopic.test.js.map