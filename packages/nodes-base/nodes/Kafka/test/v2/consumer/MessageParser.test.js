...
const BINARY = { data: 'ZGF0YQ==', mimeType: 'application/octet-stream' };
...
let logger;
let prepareBinaryData;
beforeEach(() => {
    logger = mock();
    prepareBinaryData = vi.fn(async () => BINARY);
});
const parse = async (options, kafkaMessage = message(), registry) => await createMessageParser(options, logger, registry, prepareBinaryData)(kafkaMessage, TOPIC);
describe('createMessageParser', () => {
    describe('item shape, per option combination', () => {
        ...
        it('adds the headers when return-headers is on', async () => {
            const parsed = await parse({ returnHeaders: true }, message({ headers: { 'x-trace': Buffer.from('abc') } }));
        ...
        it('attaches the raw value as binary data when keep-binary-data is on', async () => {
            ...
            expect(prepareBinaryData).toHaveBeenCalledWith(Buffer.from('{"a":1}'), 'message', 'application/octet-stream');
        });
    ...
    describe('version 1 parity', () => {
        ...
        it('renders array and string header values the way v1 did', async () => {
            const parsed = await parse({ returnHeaders: true }, message({
                headers: {
    ...
    describe('JSON parsing failures', () => {
        it('keeps the string value and warns when the value is not JSON', async () => {
            ...
            expect(logger.warn).toHaveBeenCalledWith('Could not parse message to JSON, returning as string', expect.objectContaining({ error: expect.any(Error) }));
        });
    });
    describe('Schema Registry decoding', () => {
        ...
        it('fails the message rather than handing the workflow undecoded bytes', async () => {
            ...
            await expect(parse({}, message(), registry)).rejects.toThrow('Request failed with status code 404');
            expect(logger.error).toHaveBeenCalledWith('Could not decode message with Schema Registry, leaving it unread', 
        ...
        it('throws a sanitized error, since the consume loop logs whatever it is handed', async () => {
            ...
            const thrown = await parse({}, message(), registry).catch((error) => error);
        ...
        it('does not call the registry for an empty value', async () => {
            const registry = mock({ decode: vi.fn() });
