...
function optionEntries(description) {
    const collection = description.properties.find((property) => property.name === 'options');
    if (!collection?.options)
        throw new Error('the description declares no Options collection');
 ...
 */
function visibleOptionNames(description, typeVersion) {
    const values = { jsonParseMessage: true };
    const root = { ...values, resolveOffset: 'onCompletion' };
    return optionEntries(description)
        .filter((option) => NodeHelpers.displayParameter(values, option, { typeVersion }, description, root))
...
describe('KafkaTriggerV2 description', () => {
    ...
    it('differs from v1.3 by exactly the documented options, and nothing else', () => {
        ...
        expect(v13Visible.filter((name) => !v2Visible.includes(name)).sort()).toStrictEqual([...DROPPED_IN_V2].sort());
        expect(v2Visible.filter((name) => !v13Visible.includes(name)).sort()).toStrictEqual([...ADDED_IN_V2].sort());
    });
...
//# sourceMappingURL=KafkaTriggerV2.node.description.test.js.map