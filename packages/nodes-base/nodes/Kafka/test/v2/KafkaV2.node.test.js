...
const { kafkajsLoadCount, mockProducerConnect, mockProducerSendBatch, mockProducerDisconnect, mockProducerFactory, mockRegistryEncode, mockRegistryGetLatestSchemaId, } = vi.hoisted(() => {
    const kafkajsLoadCount = { value: 0 };
    const mockProducerConnect = vi.fn(async () => { });
    const mockProducerSendBatch = vi.fn(async () => []);
...
function createExecuteFunctions(params, items, options = {}) {
    ...
    return mock({
        getInputData: () => items,
        getNode: () => node,
        logger: mock({ warn: mockLoggerWarn }),
...
describe('KafkaV2 Node', () => {
    beforeEach(async () => {
        ...
        vi.mocked(Kafka).mockImplementation(function (config) {
            return {
                config,
                connect: vi.fn(),
                disconnect: vi.fn(),
    ...
    test.each(['none', 'gzip', 'snappy', 'lz4', 'zstd'])('passes the %s compression codec to the producer', async (compression) => {
        const params = {
            options: { compression },
            sendInputData: false,
            useSchemaRegistry: false,
        ...
        await new KafkaV2(baseDescription).execute.call(createExecuteFunctions(params, [{ json: {} }]));
        expect(mockProducerFactory).toHaveBeenCalledWith({
    ...
    test('reports success when the broker returns no record metadata', async () => {
        ...
        const items = [{ json: { name: 'item' } }];
        const result = await new KafkaV2(baseDescription).execute.call(createExecuteFunctions(params, items));
    ...
    test('disconnects the producer even when sendBatch rejects', async () => {
        ...
        await expect(new KafkaV2(baseDescription).execute.call(createExecuteFunctions(sendParams, [{ json: {} }]))).rejects.toThrow('broker unreachable');
        expect(mockProducerDisconnect).toHaveBeenCalledTimes(1);
    });
    ...
    test('still returns the send result when only the disconnect rejects', async () => {
        ...
        mockProducerDisconnect.mockRejectedValueOnce(new Error('disconnect failed'));
        const result = await new KafkaV2(baseDescription).execute.call(createExecuteFunctions(sendParams, [{ json: {} }]));
    ...
    test('rejects a non-string header value before the producer is created', async () => {
        ...
        await expect(new KafkaV2(baseDescription).execute.call(createExecuteFunctions(params, [{ json: {} }]))).rejects.toThrow('Header "retries" must be a string');
    ...
    test('disconnects and propagates the connect error when connect rejects', async () => {
        ...
        await expect(new KafkaV2(baseDescription).execute.call(createExecuteFunctions(sendParams, [{ json: {} }]))).rejects.toThrow('connection refused');
        expect(mockProducerSendBatch).not.toHaveBeenCalled();
        expect(mockProducerDisconnect).toHaveBeenCalledTimes(1);
    });
    test('returns the send error as item data when the node continues on fail', async () => {
        mockProducerSendBatch.mockRejectedValueOnce(new Error('broker unreachable'));
        const result = await new KafkaV2(baseDescription).execute.call(createExecuteFunctions(sendParams, [{ json: {} }], { continueOnFail: true }));
    ...
    test('fails before the producer is built when the message is not valid JSON', async () => {
        ...
        await expect(new KafkaV2(baseDescription).execute.call(createExecuteFunctions(params, [{ json: {} }]))).rejects.toThrow('Message is not valid JSON');
        expect(mockProducerFactory).not.toHaveBeenCalled();
    });
    test('fails before the producer is built when the JSON headers are not valid JSON', async () => {
        ...
        await expect(new KafkaV2(baseDescription).execute.call(createExecuteFunctions(params, [{ json: {} }]))).rejects.toThrow('Headers must be valid JSON');
        expect(mockProducerFactory).not.toHaveBeenCalled();
    });
    test('should configure the schema registry from the selected credential', async () => {
        ...
        const items = [{ json: { success: true } }];
        await new KafkaV2(baseDescription).execute.call(createExecuteFunctions(params, items, { schemaRegistryCredential }));
    ...
    test('should fail with the generic message when the schema lookup fails', async () => {
        ...
        const items = [{ json: {} }];
        await expect(new KafkaV2(baseDescription).execute.call(createExecuteFunctions(params, items, { schemaRegistryCredential }))).rejects.toThrow('Verify your Schema Registry configuration');
        expect(mockProducerFactory).not.toHaveBeenCalled();
    });
...
//# sourceMappingURL=KafkaV2.node.test.js.map