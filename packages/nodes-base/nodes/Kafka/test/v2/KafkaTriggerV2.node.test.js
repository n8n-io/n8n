...
import { explainManualRunGroupDenial, manualRunGroupId, toConsumerOptions, toEmitterOptions, } from '../../v2/TriggerSettings';
import { confluentKafkaModuleMock, failNextTopicMetadata, getFakeAdmins, getFakeConsumers, resetConfluentKafkaRecordings, setFakeConsumerAssignment, unknownTopicError, } from '../mocks/confluent-kafka';
...
async function lastFakeConsumer() {
 ...
 */
async function startTrigger(groupId, parameters = {}, overrides = {}) {
...
describe('toConsumerOptions', () => {
    ...
    describe('the processing deadline stays inside what the library accepts', () => {
        ...
        it.each([
            ...
            expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('capped'), expect.objectContaining({ appliedMs: LIBRDKAFKA_MAX_MS }));
        });
        it('caps an oversized Rebalance Timeout option too', () => {
            const { rebalanceTimeout } = toConsumerOptions({ rebalanceTimeout: 30 * 24 * 3600 * 1000 }, 'my-group', undefined);
        ...
        it.each([NaN, Infinity])('falls back to the emitter default when Rebalance Timeout is %s', (rebalanceTimeout) => {
        ...
        describe('and stays above the Session Timeout, which the library also requires', () => {
            ...
            it.each([
                ...
                const logger = mock();
                const { rebalanceTimeout } = toConsumerOptions(session === undefined ? {} : { sessionTimeout: session }, 'g', seconds, logger);
                expect((rebalanceTimeout ?? 0) * 2).toBeGreaterThanOrEqual(expectedFloor);
                expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('raised to the Session Timeout'), expect.objectContaining({ appliedMs: expectedFloor }));
            });
            ...
            it('respects a lowered Session Timeout instead of forcing the 30s default', () => {
                const logger = mock();
                const { rebalanceTimeout, sessionTimeout } = toConsumerOptions({ sessionTimeout: 8_000 }, 'g', 5, logger);
        ...
        it('lets an explicitly set Rebalance Timeout win when there is no workflow timeout', () => {
            const { rebalanceTimeout } = toConsumerOptions({ rebalanceTimeout: 120_000 }, 'my-group', undefined);
    ...
    describe('Auto Commit Interval', () => {
        ...
        it('passes a user-set interval through, overriding the pinned default', () => {
            expect(toConsumerOptions({ autoCommitInterval: 1_000 }, 'g', undefined).autoCommitInterval).toBe(1_000);
        });
        it('keeps a zero, which turns interval commits off rather than meaning unset', () => {
            ...
            expect(toConsumerOptions({ autoCommitInterval: 0 }, 'g', undefined).autoCommitInterval).toBe(0);
        });
        ...
        it.each([NaN, -1, 86_400_001, Infinity])('drops %s, which the library would reject', (value) => {
            const logger = mock();
            const result = toConsumerOptions({ autoCommitInterval: value }, 'g', undefined, logger);
            expect(result.autoCommitInterval).toBeUndefined();
            expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('Auto Commit Interval'), expect.objectContaining({ supplied: value }));
        });
    });
    describe('Heartbeat Interval stays under a third of the Session Timeout', () => {
        ...
        it('lowers the default heartbeat when the user shortens only the session timeout', () => {
            ...
            expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('Heartbeat Interval'), expect.objectContaining({ supplied: 10_000, applied: 3_333, sessionTimeout: 10_000 }));
        });
        it.each([
            ...
            const logger = mock();
            const result = toConsumerOptions({ sessionTimeout: session, heartbeatInterval: beat }, 'g', undefined, logger);
        ...
        it.each([
            ...
            const result = toConsumerOptions({ sessionTimeout: session, heartbeatInterval: beat }, 'g', undefined);
            expect(result.heartbeatInterval).toBe(expected);
        });
        ...
        it.each([0, -1, NaN])('leaves the heartbeat alone when the session timeout is %s, so librdkafka names the bad value', (session) => {
            const logger = mock();
            const result = toConsumerOptions({ sessionTimeout: session, heartbeatInterval: 10_000 }, 'g', undefined, logger);
...
describe('explainManualRunGroupDenial', () => {
    ...
    it('names the prefix to grant, so the fix does not need guessing', () => {
        ...
        expect(result.description).toContain('orders-consumer-n8n-manual-');
        // The original stays reachable rather than being replaced outright.
        expect(result.cause).toBe(denial);
    });
...
describe('toEmitterOptions', () => {
    it('passes the execution timeout through raw, so unbounded stays unbounded', () => {
        ...
        expect(toEmitterOptions({}, 'onCompletion', [], undefined).executionTimeoutSeconds).toBeUndefined();
...
describe('KafkaTriggerV2 Node', () => {
    ...
    describe('a topic that does not exist', () => {
        it('fails activation instead of publishing a workflow that consumes nothing', async () => {
            ...
            failNextTopicMetadata(unknownTopicError());
            await expect(startTrigger('v2-missing-topic')).rejects.toThrow('Kafka topic "test-topic" does not exist');
        });
        ...
        it('fails a manual test run too, rather than listening forever', async () => {
            ...
            failNextTopicMetadata(unknownTopicError());
            failNextTopicMetadata(unknownTopicError());
            await expect(manualTriggerFunction?.()).rejects.toThrow('Kafka topic "test-topic" does not exist');
            expect(getFakeConsumers()).toHaveLength(0);
        });
    ...
    describe('consumer options reach the library', () => {
        ...
        it('passes Retry Delay on Error to the loop', async () => {
            ...
            expect(consumeTopicSpy).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ errorRetryDelay: 12345 }));
        });
    });
    describe('fatal consumer errors', () => {
        /** The logger the node handed the library, which is where fatal errors surface. */
        function libraryLogger(consumer) {
            const logger = consumer.config.kafkaJS?.logger;
            if (!logger)
                throw new Error('the node gave the library no logger');
            return logger;
        }
        ...
        describe('while startup is still waiting on the group join', () => {
            ...
            it('keeps the manual-run ACL explanation when the denial fails the join wait', async () => {
                ...
                await expect(starting).rejects.toMatchObject({
                    message: expect.stringContaining('Kafka refused the consumer group used for a test run'),
                    description: expect.stringContaining('orders-consumer-n8n-manual-'),
                });
    ...
    describe('Schema Registry', () => {
        it('decodes through the registry when enabled', async () => {
            ...
            SchemaRegistry.mockImplementation(function () {
            ...
            expect(SchemaRegistry).toHaveBeenCalledWith(expect.objectContaining({ host: 'http://localhost:8081/' }));
        ...
        it('fails activation when the registry credential is misconfigured, same as v1', async () => {
            await expect(testTriggerNode(new KafkaTriggerV2(baseDescription), {
                mode: 'trigger',
                node: {
                    credentials: {
    ...
    describe('offset resolution', () => {
        it('waits for the execution before resolving the offset on onCompletion', async () => {
            ...
            deferred?.resolve(mock({ status: 'success' }));
            await delivered;
        ...
        it('leaves the offset unresolved when the execution status is not allowed', async () => {
            ...
            await new Promise((resolve) => setImmediate(resolve));
            emit.mock.calls[0][2]?.resolve(mock({ status: 'error' }));
            await delivered;
            expect(consumer.payloadSpies.resolveOffset).not.toHaveBeenCalled();
        });
    });
    describe('manual test run isolation from production', () => {
        async function startManualRun(parameters = {}) {
            const started = await testTriggerNode(new KafkaTriggerV2(baseDescription), {
                mode: 'manual',
                node: {
                    parameters: {
        ...
        it('tells the user which group ACL prefix to grant when the broker refuses it', async () => {
            ...
            expect(emitError).toHaveBeenCalledTimes(1);
            expect(emitError.mock.calls[0][0].description).toContain('orders-consumer-n8n-manual-');
        });
    ...
    describe('manual test run', () => {
        it('starts the loop only when the test event is requested, and never waits', async () => {
            ...
            const { emit, manualTriggerFunction } = await testTriggerNode(new KafkaTriggerV2(baseDescription), {
                mode: 'manual',
                node: {
                    parameters: {
                        topic: 'test-topic',
    ...
    describe('close', () => {
        ...
        it('reports a failed teardown as a TriggerCloseError, as v1 does', async () => {
            ...
            const error = await close().then(() => null, (e) => e);
            expect(error).toBeInstanceOf(TriggerCloseError);
            expect(error.cause).toBe(teardownError);
            expect(error.level).toBe('warning');
        });
    });
});
//# sourceMappingURL=KafkaTriggerV2.node.test.js.map