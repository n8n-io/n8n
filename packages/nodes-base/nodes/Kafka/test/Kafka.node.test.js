...
const { mockProducerConnect, mockProducerSend, mockProducerDisconnect, mockRegistryEncode, mockRegistryGetLatestSchemaId, } = vi.hoisted(() => {
    const mockProducerConnect = vi.fn(async () => { });
    const mockProducerSend = vi.fn(async () => []);
    ...
    const mockRegistryEncode = vi.fn(async (_id, input) => Buffer.from(JSON.stringify(input)));
    const mockRegistryGetLatestSchemaId = vi.fn(async (eventName) => {
...
vi.mock('kafkajs', async () => {
    const actual = await vi.importActual('kafkajs');
    return {
        ...actual,
...
const defaultKafkaCredentials = {
    brokers: 'localhost:9092',
    clientId: 'test-client',
    ssl: false,
    authentication: false,
};
function createExecuteFunctions(params, items, options = {}) {
    ...
    return mock({
        ...
        getCredentials: (async (type) => type === 'schemaRegistryApi'
            ? schemaRegistryCredential
            : defaultKafkaCredentials),
...
describe('Kafka Node', () => {
    ...
    test('publishes schema-registry-encoded messages with json headers', async () => {
        ...
        const items = [{ json: { success: true } }, { json: { success: true } }];
        await new KafkaV1(baseDescription).execute.call(createExecuteFunctions(params, items));
        // The legacy URL-parameter path stays unauthenticated
        expect(SchemaRegistry).toHaveBeenCalledWith(expect.objectContaining({ host: 'https://test-kafka-registry.local/' }));
    ...
    test('should configure the schema registry from the selected credential', async () => {
        ...
        const items = [{ json: { success: true } }];
        await new KafkaV1(baseDescription).execute.call(createExecuteFunctions(params, items, { schemaRegistryCredential }));
        expect(SchemaRegistry).toHaveBeenCalledWith(expect.objectContaining({
    ...
    test('should fail with the misconfiguration message when the credential is missing the password', async () => {
        ...
        const items = [{ json: {} }];
        await expect(new KafkaV1(baseDescription).execute.call(createExecuteFunctions(params, items, {
    ...
    test('should fail with the generic message when the schema lookup fails', async () => {
        ...
        const items = [{ json: {} }];
        await expect(new KafkaV1(baseDescription).execute.call(createExecuteFunctions(params, items, { schemaRegistryCredential }))).rejects.toThrow('Verify your Schema Registry configuration');
        expect(mockProducerConnect).not.toHaveBeenCalled();
    });
    test('should report a malformed message distinctly from a registry config error', async () => {
        ...
        const items = [{ json: {} }];
        await expect(new KafkaV1(baseDescription).execute.call(createExecuteFunctions(params, items, { schemaRegistryCredential }))).rejects.toThrow('Message is not valid JSON');
    ...
    test('should return the error as item data when the node continues on fail', async () => {
        ...
        const items = [{ json: {} }];
        const result = await new KafkaV1(baseDescription).execute.call(createExecuteFunctions(params, items, {
            schemaRegistryCredential: { ...schemaRegistryCredential, password: '' },
            continueOnFail: true,
        }));
...
//# sourceMappingURL=Kafka.node.test.js.map