...
import { NodeOperationError, TriggerCloseError, UnexpectedError, } from 'n8n-workflow';
...
describe('KafkaTrigger Node', () => {
    const expectedRetryConfig = {
        restartOnFailure: expect.any(Function),
    };
    let mockKafka;
    let mockRegistry;
    let mockConsumerConnect;
    let mockConsumerSubscribe;
    let mockConsumerRun;
    let mockConsumerDisconnect;
    let mockConsumerStop;
    let mockConsumerCreate;
    let mockRegistryDecode;
    let publishMessage;
    ...
    beforeEach(() => {
        ...
        mockConsumerRun = vi.fn(({ eachMessage, eachBatch }) => {
            if (eachMessage) {
        ...
        global.publishBatch = publishBatch;
        mockKafka = mock({
        ...
        Kafka.mockImplementation(function () {
            return mockKafka;
        });
    ...
    describe('surfaces async consumer failures instead of hanging', () => {
        const fireCrash = async (restart, message = 'KafkaJSNotImplemented: LZ4 compression not implemented') => {
        ...
        const startTrigger = async () => await testTriggerNode(new KafkaTriggerV1(baseDescription), {
            mode: 'trigger',
            node: {
                typeVersion: 1.3,
                parameters: {
    ...
    it('should throw an error if authentication is enabled but credentials are missing', async () => {
        await expect(testTriggerNode(new KafkaTriggerV1(baseDescription), {
            mode: 'trigger',
            node: {
                parameters: {
                    topic: 'test-topic',
    ...
    it('should use the schema registry credential without auth when authentication is none', async () => {
        ...
        expect(SchemaRegistry).toHaveBeenCalledWith(expect.objectContaining({ host: 'https://unauthenticated-registry.local:8081/' }));
    });
    it('should emit the original message and log a sanitized warning when decoding fails', async () => {
        mockRegistryDecode.mockRejectedValue(Object.assign(new Error('request to https://registry-user:registry-password@schema-registry.local:8081/schemas/ids/1 failed'), { status: 404 }));
        ...
        expect(logger.warn).toHaveBeenCalledWith('Could not decode message with Schema Registry, returning original message', {
    ...
    it('should handle manual trigger mode', async () => {
        const { emit, manualTriggerFunction } = await testTriggerNode(new KafkaTriggerV1(baseDescription), {
            mode: 'manual',
            node: {
                typeVersion: 1,
                parameters: {
    ...
    it('should register event listeners on consumer', async () => {
        ...
        mockConsumerCreate.mockReturnValueOnce(mock({
            connect: mockConsumerConnect,
            subscribe: mockConsumerSubscribe,
            run: mockConsumerRun,
            disconnect: mockConsumerDisconnect,
        ...
        expect(mockOn).toHaveBeenCalledWith('consumer.received_unsubscribed_topics', expect.any(Function));
    ...
    it('should configure a restartOnFailure callback that vetoes consumer restarts only after close', async () => {
        ...
        expect(mockConsumerCreate).toHaveBeenCalledWith(expect.objectContaining({
            groupId: 'test-group',
            retry: expectedRetryConfig,
        }));
        const { retry } = mockConsumerCreate.mock.calls[0][0];
    ...
    it('should crash the consumer non-retriably when a batch arrives after close', async () => {
        ...
        await close();
        const error = await publishMessage({ value: Buffer.from('late-message') }).then(() => null, (e) => e);
    ...
    it('should not block on disconnect when stop() times out, then disconnect once stop settles', async () => {
        ...
        try {
            let resolveStop;
            mockConsumerStop.mockImplementationOnce(async () => await new Promise((resolve) => {
                resolveStop = resolve;
            }));
            const { close } = await testTriggerNode(new KafkaTriggerV1(baseDescription), {
                mode: 'trigger',
            ...
            const closeResult = close().then(() => null, (e) => e);
    ...
    it('should handle batch processing when batchSize > 1', async () => {
        ...
        const publishBatch = global.publishBatch;
        await publishBatch([
    ...
    it('should use partitionsConsumedConcurrently when provided', async () => {
        ...
        expect(mockConsumerRun).toHaveBeenCalledWith(expect.objectContaining({
            partitionsConsumedConcurrently: 5,
        }));
    });
    ...
    describe('version 1.3', () => {
        ...
        it('should use resolveOffset "onSuccess" and wait for successful execution', async () => {
            ...
            const deferredPromise = emit.mock.calls[0][2];
            expect(deferredPromise).toBeDefined();
            deferredPromise?.resolve(mock({ status: 'success' }));
            await publishPromise;
        });
        it('should return success false when resolveOffset is "onSuccess" and execution fails', async () => {
            ...
            const deferredPromise = emit.mock.calls[0][2];
            expect(deferredPromise).toBeDefined();
            deferredPromise?.resolve(mock({ status: 'error' }));
        ...
        it('should stop processing batch when isRunning returns false (consumer stopping)', async () => {
            let eachBatchHandler;
            mockConsumerRun.mockImplementation(({ eachBatch }) => {
                if (eachBatch) {
...
describe('KafkaTrigger (versioned entry point)', () => {
    ...
    it('maps exactly versions 1, 1.1, 1.2, 1.3 to KafkaTriggerV1 and 2 to KafkaTriggerV2', () => {
        expect(Object.keys(kafkaTrigger.nodeVersions)
            .map(Number)
            .sort((a, b) => a - b)).toEqual(allVersions);
...
//# sourceMappingURL=KafkaTrigger.node.test.js.map