...
import { createSchemaRegistry, getAutoCommitSettings, configureDataEmitter, getSchemaRegistryOptions, resolveKafkaSsl, resolveRetryDelay, sanitizeRegistryError, setSchemaRegistry, stopAndDisconnectConsumer, withTimeout, } from '../utils';
...
describe('Kafka Utils', () => {
    describe('sanitizeRegistryError', () => {
        ...
        describe('credentials in the URL', () => {
            it('redacts userinfo', () => {
                expect(sanitizeRegistryError(new Error('request to https://user:pw@registry.local failed'))).toStrictEqual({ message: 'request to https://***@registry.local failed' });
            });
            it('redacts up to the last @ when the password contains an unencoded one', () => {
                ...
                const { message } = sanitizeRegistryError(new Error('request to https://user:p@ssw0rd@registry.local failed'));
            ...
            it('leaves a URL without credentials alone', () => {
                expect(sanitizeRegistryError(new Error('request to https://registry.local failed'))).toStrictEqual({ message: 'request to https://registry.local failed' });
            });
        });
        ...
        describe('shape', () => {
            it('includes the status when the error carries one', () => {
                expect(sanitizeRegistryError(Object.assign(new Error('nope'), { status: 404 }))).toStrictEqual({ message: 'nope', status: 404 });
            });
    ...
    describe('resolveKafkaSsl', () => {
        ...
        it('builds cert + key buffers when a client certificate and key are provided', () => {
            const ssl = resolveKafkaSsl(creds({ cert: CERT_PEM, key: KEY_PEM }));
            expect(Buffer.isBuffer(ssl.cert)).toBe(true);
            expect(Buffer.isBuffer(ssl.key)).toBe(true);
            expect(ssl.cert.toString()).toContain('BEGIN CERTIFICATE');
            expect(ssl.key.toString()).toContain('BEGIN PRIVATE KEY');
            expect(ssl.ca).toBeUndefined();
            expect(ssl.rejectUnauthorized).toBeUndefined();
        });
        it('wraps a CA certificate in an array', () => {
            const ssl = resolveKafkaSsl(creds({ ca: CA_PEM }));
            expect(Array.isArray(ssl.ca)).toBe(true);
            expect(Buffer.isBuffer(ssl.ca[0])).toBe(true);
            expect(ssl.cert).toBeUndefined();
            expect(ssl.key).toBeUndefined();
        });
        it('maps cert, key, CA and the insecure toggle together', () => {
            const ssl = resolveKafkaSsl(creds({ cert: CERT_PEM, key: KEY_PEM, ca: CA_PEM, allowUnauthorizedCerts: true }));
            expect(Buffer.isBuffer(ssl.cert)).toBe(true);
            expect(Buffer.isBuffer(ssl.key)).toBe(true);
            expect(Array.isArray(ssl.ca)).toBe(true);
        ...
        it('throws on a truncated PEM that has BEGIN but no matching END', () => {
            ...
            expect(() => resolveKafkaSsl(creds({ cert: truncated, key: KEY_PEM }))).toThrow('not a valid PEM block');
        });
    });
    describe('resolveRetryDelay', () => {
        ...
        it.each([Number.NaN, -1, 2_147_483_648])('warns that a delay of %s was replaced, so the misconfiguration is visible', (value) => {
            const logger = mock();
    ...
    describe('configureDataEmitter', () => {
        ...
        const createDeferredPromise = () => {
            ...
            return {
                ...
                resolve: () => { },
                reject: () => { },
                resolveWith: (value) => resolveFunc(value),
                rejectWith: (error) => rejectFunc(error),
            };
        };
        const createMockContext = (params = {}, mode = 'trigger', deferredPromise) => {
            const ctx = mock();
            const mockLogger = mock();
            ctx.getNodeParameter.mockImplementation((name, fallback) => (params[name] ?? fallback));
        ...
        describe('immediate emit mode', () => {
            ...
            it('should emit immediately for version 1.1 with parallelProcessing enabled', async () => {
                const ctx = createMockContext({}, 'trigger');
                const options = { parallelProcessing: true };
        ...
        describe('deferred emit mode - onCompletion', () => {
            it('should wait for execution completion and return success', async () => {
                ...
                const ctx = createMockContext({ resolveOffset: 'onCompletion' }, 'trigger', deferredPromise);
                const options = {};
                const emitter = configureDataEmitter(ctx, options, 1.3, new AbortController().signal);
                const testData = [{ json: { message: 'test' } }];
                const resultPromise = emitter(testData);
                // Simulate successful execution completion
                deferredPromise.resolveWith({ status: 'success' });
            ...
            it('should return success for any status in onCompletion mode', async () => {
                ...
                deferredPromise.resolveWith({ status: 'error' });
                const result = await resultPromise;
                expect(result).toEqual({ success: true });
            });
            it('should use version 1.1 onCompletion mode when parallelProcessing is false', async () => {
                ...
                const options = { parallelProcessing: false };
                const emitter = configureDataEmitter(ctx, options, 1.1, new AbortController().signal);
                const testData = [{ json: { message: 'test' } }];
                const resultPromise = emitter(testData);
                deferredPromise.resolveWith({ status: 'success' });
        ...
        describe('deferred emit mode - onSuccess', () => {
            ...
            it('should use custom errorRetryDelay when provided', async () => {
                const deferredPromise = createDeferredPromise();
                const ctx = createMockContext({ resolveOffset: 'onSuccess' }, 'trigger', deferredPromise);
                const options = { errorRetryDelay: 10000 };
                const emitter = configureDataEmitter(ctx, options, 1.3, new AbortController().signal);
                const testData = [{ json: { message: 'test' } }];
                const resultPromise = emitter(testData);
                deferredPromise.resolveWith({ status: 'error' });
        ...
        describe('deferred emit mode - onStatus', () => {
            it('should throw error when no statuses are selected', () => {
                const ctx = createMockContext({ resolveOffset: 'onStatus', allowedStatuses: [] }, 'trigger');
                const options = {};
                expect(() => configureDataEmitter(ctx, options, 1.3, new AbortController().signal)).toThrow(NodeOperationError);
            });
            it('should return success when execution status matches allowed statuses', async () => {
                const deferredPromise = createDeferredPromise();
                const ctx = createMockContext({ resolveOffset: 'onStatus', allowedStatuses: ['success', 'warning'] }, 'trigger', deferredPromise);
                const options = {};
                const emitter = configureDataEmitter(ctx, options, 1.3, new AbortController().signal);
                const testData = [{ json: { message: 'test' } }];
                const resultPromise = emitter(testData);
                deferredPromise.resolveWith({ status: 'warning' });
                const result = await resultPromise;
                expect(result).toEqual({ success: true });
            });
            it('should return failure when execution status does not match allowed statuses', async () => {
                const deferredPromise = createDeferredPromise();
                const ctx = createMockContext({ resolveOffset: 'onStatus', allowedStatuses: ['success'] }, 'trigger', deferredPromise);
                const options = {};
                const emitter = configureDataEmitter(ctx, options, 1.3, new AbortController().signal);
                const testData = [{ json: { message: 'test' } }];
                const resultPromise = emitter(testData);
        ...
        describe('error handling', () => {
            ...
            it('should handle non-Error objects in catch block', async () => {
                ...
                deferredPromise.rejectWith('String error');
                const result = await resultPromise;
                expect(mockedSleep).toHaveBeenCalled();
                expect(result).toEqual({ success: false });
            });
        ...
        describe('close signal', () => {
            ...
            it('should cut the error retry backoff short when the close signal aborts mid-backoff', async () => {
                ...
                mockedSleep.mockReturnValueOnce(new Promise(() => { }));
                const emitter = configureDataEmitter(ctx, {}, 1.3, closeController.signal);
                const resultPromise = emitter([{ json: { message: 'test' } }]);
                deferredPromise.resolveWith({ status: 'error' });
    ...
    describe('stopAndDisconnectConsumer', () => {
        ...
        it('should not await disconnect while stop() is pending, then disconnect once it settles', async () => {
            ...
            try {
                let resolveStop;
                const consumer = mock({
                    stop: vi.fn(async () => await new Promise((resolve) => {
                        resolveStop = resolve;
                    })),
                    disconnect: vi.fn(async () => { }),
                });
                ...
                expect(error.message).toBe('Kafka consumer did not stop in time');
    ...
    describe('withTimeout', () => {
        ...
        it('should reject with an OperationalError when the promise does not settle in time', async () => {
            const error = await withTimeout(new Promise(() => { }), 20, 'too slow').then(() => null, (e) => e);
            expect(error).toBeInstanceOf(OperationalError);
    ...
    describe('schema registry helpers', () => {
        ...
        const createRegistryContext = ({ params = {}, nodeCredentials, credentialData, secureEgressFilter, } = {}) => {
            const ctx = mock();
        ...
        const createEgressFilter = (result) => {
            const lookup = vi.fn();
        ...
        const blockedFilter = (message = 'The request was blocked') => createEgressFilter(createResultError(new Error(message)));
        // `options` is set on the agent at runtime but not part of the public type.
        const agentLookup = (agent) => agent?.options?.lookup;
        beforeEach(() => {
            vi.clearAllMocks();
        });
        describe('getSchemaRegistryOptions', () => {
            ...
            it('should throw when basicAuth credential is missing the password', async () => {
                ...
                await expect(getSchemaRegistryOptions(ctx, '')).rejects.toThrow('Username and password are required for Schema Registry Basic Auth');
            });
            it('should throw when no credential is selected and the fallback URL is blank', async () => {
                const ctx = createRegistryContext();
                await expect(getSchemaRegistryOptions(ctx, '  ')).rejects.toThrow(NodeOperationError);
                await expect(getSchemaRegistryOptions(ctx, '  ')).rejects.toThrow('Select a Schema Registry credential or enter a Schema Registry URL');
                expect(ctx.getCredentials).not.toHaveBeenCalled();
            });
        ...
        describe('createSchemaRegistry', () => {
            const lastConstructorArg = () => {
                const calls = SchemaRegistry.mock.calls;
            ...
            it('rejects a space-injected host when egress filtering is enabled', async () => {
                ...
                const error = await createSchemaRegistry(ctx, 'http://169.254.169.254:80 /v').catch((e) => e);
                expect(error).toBeInstanceOf(NodeOperationError);
                expect(error.description).toBe('The Schema Registry URL is not a valid URL');
            ...
            it('rejects a non-http(s) scheme when egress filtering is enabled', async () => {
                const { filter, validateUrl } = okFilter();
                const ctx = createRegistryContext({ secureEgressFilter: filter });
                await expect(createSchemaRegistry(ctx, 'ftp://internal/')).rejects.toThrow(NodeOperationError);
                expect(SchemaRegistry).not.toHaveBeenCalled();
                expect(validateUrl).not.toHaveBeenCalled();
            });
            it('rejects an IPv6-literal host when its addresses are blocked', async () => {
                ...
                const ctx = createRegistryContext({ secureEgressFilter: filter });
                await expect(createSchemaRegistry(ctx, 'http://[::1]:8081')).rejects.toThrow(NodeOperationError);
                expect(SchemaRegistry).not.toHaveBeenCalled();
            });
            it('rejects a direct-IP registry host when egress filtering is enabled', async () => {
                const { filter, validateUrl } = blockedFilter();
                const ctx = createRegistryContext({ secureEgressFilter: filter });
                await expect(createSchemaRegistry(ctx, 'http://169.254.169.254')).rejects.toThrow(NodeOperationError);
            ...
            it('throws a NodeOperationError surfaced as the continueOnFail item message', async () => {
                ...
                const ctx = createRegistryContext({ secureEgressFilter: filter });
                const error = await createSchemaRegistry(ctx, 'http://169.254.169.254').catch((e) => e);
                expect(error).toBeInstanceOf(NodeOperationError);
                expect(error.message).toBe('Verify your Schema Registry configuration');
            });
        });
        describe('setSchemaRegistry', () => {
            ...
            it('should construct the registry with credential options', async () => {
                ...
                const result = await setSchemaRegistry(ctx);
                const arg = SchemaRegistry.mock.calls[0][0];
            ...
            it('should warn with a sanitized payload and continue on connection-type errors', async () => {
                ...
                SchemaRegistry.mockImplementationOnce(function () {
            ...
            it('should redact URL userinfo and omit status in the warn payload when absent', async () => {
                ...
                SchemaRegistry.mockImplementationOnce(function () {
                    throw new Error('request to https://registry-user:registry-password@fallback-registry.local/subjects failed');
                });
                const result = await setSchemaRegistry(ctx);
                expect(result).toBeUndefined();
            ...
            it('should redact userinfo up to the last @ when the password contains an unencoded @', async () => {
                ...
                SchemaRegistry.mockImplementationOnce(function () {
                    throw new Error('request to https://registry-user:p@ssw0rd@fallback-registry.local/subjects failed');
                });
                const result = await setSchemaRegistry(ctx);
                expect(result).toBeUndefined();
            ...
            it('should rethrow misconfiguration errors instead of warning', async () => {
                ...
                await expect(setSchemaRegistry(ctx)).rejects.toThrow('Select a Schema Registry credential or enter a Schema Registry URL');
