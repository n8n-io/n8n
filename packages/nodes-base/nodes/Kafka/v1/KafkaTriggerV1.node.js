...
import { NodeConnectionTypes, NodeOperationError, TriggerCloseError, UnexpectedError, } from 'n8n-workflow';
import { connectEventListeners, disconnectEventListeners, setSchemaRegistry, configureMessageParser, createConfig, createConsumerConfig, configureDataEmitter, getAutoCommitSettings, runWithHeartbeat, stopAndDisconnectConsumer, toUserFacingConsumerError, } from '../utils';
...
export class KafkaTriggerV1 {
    ...
    async trigger() {
        ...
        const processMessage = configureMessageParser(options, this.logger, registry, this.helpers.prepareBinaryData);
                ...
                await consumer.run({
                    ...
                    eachBatch: async ({ batch, resolveOffset, heartbeat, isStale, isRunning, commitOffsetsIfNecessary, }) => {
                        ...
                        if (closeSignal.aborted) {
                            throw Object.assign(new UnexpectedError('Kafka trigger consumer received messages after close'), { retriable: false });
                        }
                        ...
                        for (let i = 0; i < messages.length; i += batchSize) {
                            ...
                            const result = await runWithHeartbeat(dataEmitter(processedData), heartbeat, consumerConfig.heartbeatInterval);
                            if (!result.success) {
        ...
        const handleConsumerError = (error) => {
        ...
        const closeFunction = async () => {
            ...
            const teardownError = await stopAndDisconnectConsumer(consumer, this.logger, CLOSE_TIMEOUT_MS);
            if (teardownError) {
...
//# sourceMappingURL=KafkaTriggerV1.node.js.map