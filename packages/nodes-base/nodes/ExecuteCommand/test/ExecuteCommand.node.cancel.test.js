...
describe('ExecuteCommand cancellation', () => {
    ...
    const createContext = (options = {}) => {
        ...
        context.getNodeParameter.mockImplementation(((name) => name === 'executeOnce' ? true : 'test-command'));
    ...
    it('does not swallow the cancellation when Continue On Fail is enabled', async () => {
        const controller = new AbortController();
        const promise = node.execute.call(createContext({ signal: controller.signal, continueOnFail: true }));
        await Promise.resolve();
        controller.abort();
        await expect(promise).rejects.toBeInstanceOf(ManualExecutionCancelledError);
    });
    it('rejects without spawning a child when already cancelled', async () => {
        const controller = new AbortController();
        controller.abort();
        await expect(node.execute.call(createContext({ signal: controller.signal }))).rejects.toBeInstanceOf(ManualExecutionCancelledError);
...
//# sourceMappingURL=ExecuteCommand.node.cancel.test.js.map