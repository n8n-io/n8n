...
describe('Elasticsearch -> elasticsearchApiRequest', () => {
    ...
    it('should make a successful GET API request', async () => {
        ...
        const result = await elasticsearchApiRequest.call(mockExecuteFunctions, 'GET', '/test-endpoint');
        expect(result).toEqual(response);
        expect(mockExecuteFunctions.helpers.httpRequestWithAuthentication).toHaveBeenCalledWith('elasticsearchApi', expect.objectContaining({
            method: 'GET',
    ...
    it('should make a successful POST API request', async () => {
        ...
        mockHttpRequestWithAuthentication.mockResolvedValue(response);
        const result = await elasticsearchApiRequest.call(mockExecuteFunctions, 'POST', '/test-endpoint', body);
        expect(result).toEqual(response);
        expect(mockExecuteFunctions.helpers.httpRequestWithAuthentication).toHaveBeenCalledWith('elasticsearchApi', expect.objectContaining({
            body,
            method: 'POST',
    ...
    it('should handle API request errors', async () => {
        ...
        await expect(elasticsearchApiRequest.call(mockExecuteFunctions, 'GET', '/test-endpoint')).rejects.toThrow(NodeApiError);
        expect(mockExecuteFunctions.helpers.httpRequestWithAuthentication).toHaveBeenCalledWith('elasticsearchApi', expect.objectContaining({
            method: 'GET',
            url: 'https://example.com/test-endpoint',
            json: true,
...
describe('Elasticsearch -> elasticsearchApiRequestAllItems', () => {
    ...
    it('should reject an invalid index id before making any request', async () => {
        await expect(elasticsearchApiRequestAllItems.call(mockExecuteFunctions, '..', {}, {})).rejects.toThrow(UserError);
