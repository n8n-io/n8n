...
import { checkInput, checkInputAndThrowError, checkMatchFieldsInput, findMatches, } from './GenericFunctions';
export class CompareDatasets {
    description = {
    ...
    async execute() {
        ...
        if (options.nodeVersion < 2.2) {
            input1 = checkInputAndThrowError(input1, matchFields.map((pair) => pair.field1), options.disableDotNotation || false, 'Input A');
            input2 = checkInputAndThrowError(input2, matchFields.map((pair) => pair.field2), options.disableDotNotation || false, 'Input B');
        }
        else {
