...
describe('SlackTrigger Node', () => {
    ...
    beforeEach(() => {
        ...
        // Default mock setup
        mockWebhookFunctions.getNodeParameter.mockImplementation((paramName, defaultValue) => {
            switch (paramName) {
                case 'trigger':
                    return ['file_share'];
                case 'watchWorkspace':
    ...
    describe('webhook method - user ignore list', () => {
        const getMessageEvent = (overrides = {}) => ({
            body: {
                type: 'event_callback',
                event: {
                    type: 'message',
        ...
        it('should ignore event when event.user matches an ignored user', async () => {
            mockWebhookFunctions.getRequestObject.mockReturnValue(getMessageEvent({ user: 'U_IGNORED' }));
            const result = await slackTrigger.webhook.call(mockWebhookFunctions);
            expect(result).toEqual({});
        });
        it('should ignore event when event.user is undefined but event.message.user matches an ignored user', async () => {
            mockWebhookFunctions.getRequestObject.mockReturnValue(getMessageEvent({ user: undefined, message: { user: 'U_IGNORED' } }));
            const result = await slackTrigger.webhook.call(mockWebhookFunctions);
            expect(result).toEqual({});
        });
        it('should not ignore event when event.user does not match any ignored user', async () => {
            mockWebhookFunctions.getRequestObject.mockReturnValue(getMessageEvent({ user: 'U_SOMEONE_ELSE' }));
            const result = await slackTrigger.webhook.call(mockWebhookFunctions);
            expect(result.workflowData).toBeDefined();
        });
        it('should not ignore event when both event.user and event.message.user are absent from the ignore list', async () => {
            mockWebhookFunctions.getRequestObject.mockReturnValue(getMessageEvent({ user: undefined, message: { user: 'U_SOMEONE_ELSE' } }));
            const result = await slackTrigger.webhook.call(mockWebhookFunctions);
            expect(result.workflowData).toBeDefined();
        });
        it('should not ignore event when neither event.user nor event.message.user exist', async () => {
            mockWebhookFunctions.getRequestObject.mockReturnValue(getMessageEvent({ user: undefined }));
            const result = await slackTrigger.webhook.call(mockWebhookFunctions);
            expect(result.workflowData).toBeDefined();
        });
        it('should use event.user over event.message.user when both are present', async () => {
            mockWebhookFunctions.getRequestObject.mockReturnValue(getMessageEvent({ user: 'U_SOMEONE_ELSE', message: { user: 'U_IGNORED' } }));
            const result = await slackTrigger.webhook.call(mockWebhookFunctions);
            expect(result.workflowData).toBeDefined();
        });
    });
    describe('webhook method - reaction emoji filter', () => {
        const reactionRequest = (reaction) => ({
            body: {
                type: 'event_callback',
                event: {
                    type: 'reaction_added',
        ...
        it('should trigger when no emoji filter is set', async () => {
            mockWebhookFunctions.getRequestObject.mockReturnValue(reactionRequest('thumbsup'));
            const result = await slackTrigger.webhook.call(mockWebhookFunctions);
            expect(result.workflowData).toBeDefined();
        });
        it('should trigger when reaction matches the filter', async () => {
            ...
            mockWebhookFunctions.getNodeParameter.mockImplementation((paramName, defaultValue) => {
                if (paramName === 'options')
                    return { reactionEmojis: 'thumbsup' };
                if (paramName === 'trigger')
                    return ['reaction_added'];
                if (paramName === 'watchWorkspace')
                    return false;
        ...
        it('should not trigger when reaction does not match the filter', async () => {
            mockWebhookFunctions.getRequestObject.mockReturnValue(reactionRequest('eyes'));
            mockWebhookFunctions.getNodeParameter.mockImplementation((paramName, defaultValue) => {
                if (paramName === 'options')
                    return { reactionEmojis: 'thumbsup' };
                if (paramName === 'trigger')
        ...
        it('should support multiple comma-separated emoji names', async () => {
            ...
            mockWebhookFunctions.getNodeParameter.mockImplementation((paramName, defaultValue) => {
                if (paramName === 'options')
                    return { reactionEmojis: 'thumbsup, eyes' };
                if (paramName === 'trigger')
                    return ['reaction_added'];
                if (paramName === 'watchWorkspace')
                    return false;
        ...
        it('should not trigger when filter has entries but reaction is an empty string', async () => {
            mockWebhookFunctions.getRequestObject.mockReturnValue(reactionRequest(''));
            mockWebhookFunctions.getNodeParameter.mockImplementation((paramName, defaultValue) => {
                if (paramName === 'options')
                    return { reactionEmojis: 'thumbsup' };
                if (paramName === 'trigger')
    ...
    describe('webhook method - other event scenarios', () => {
        it('should handle team_join event (no channel extraction needed)', async () => {
            ...
            const out = resultResolve.workflowData[0][0].json;
    ...
    describe('loadOptions - getUsers', () => {
        it('should label users with real name and handle, falling back to the handle alone', async () => {
            const mockLoadOptionsFunctions = mock();
            ...
            const result = await slackTrigger.methods.loadOptions.getUsers.call(mockLoadOptionsFunctions);
...
//# sourceMappingURL=SlackTrigger.test.js.map