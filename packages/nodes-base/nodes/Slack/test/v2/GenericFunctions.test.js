...
import { slackApiRequest, slackApiRequestAllItems, slackApiRequestAllItemsWithRateLimit, searchContextItems, formatUserLabel, processThreadOptions, getMessageContent, } from '../../V2/GenericFunctions';
...
describe('Slack V2 > GenericFunctions', () => {
    ...
    describe('slackApiRequest', () => {
        ...
        it('should not handle paid_teams_only error if simple:false', async () => {
            ...
            mockExecuteFunctions.helpers.requestWithAuthentication = vi
                .fn()
                .mockResolvedValue(mockResponse);
            await expect(slackApiRequest.call(mockExecuteFunctions, 'GET', '/test', {}, {}, {}, { simple: false })).resolves.toEqual(mockResponse);
        });
        it('should handle missing_scope error with needed scopes', async () => {
            ...
            await expect(slackApiRequest.call(mockExecuteFunctions, 'GET', '/test')).rejects.toThrow(NodeOperationError);
            await expect(slackApiRequest.call(mockExecuteFunctions, 'GET', '/test')).rejects.toThrow('Your Slack credential is missing required Oauth Scopes');
            // Test specific error properties by catching the thrown error
            await expect(slackApiRequest.call(mockExecuteFunctions, 'GET', '/test')).rejects.toMatchObject({
        ...
        it.each(['ratelimited', 'rate_limited'])('should handle %s error', async (error) => {
            ...
            await expect(slackApiRequest.call(mockExecuteFunctions, 'POST', '/assistant.search.context')).rejects.toMatchObject({
                message: `Slack error response: "${error}"`,
                description: expect.stringContaining('web-api/rate-limits'),
                level: 'warning',
            });
        });
        it.each(['not_allowed_token_type', 'invalid_action_token'])('should handle %s error', async (error) => {
            mockExecuteFunctions.helpers.requestWithAuthentication = vi
                .fn()
                .mockResolvedValue({ ok: false, error });
            await expect(slackApiRequest.call(mockExecuteFunctions, 'POST', '/assistant.search.context')).rejects.toMatchObject({
        ...
        it('should handle not_admin error', async () => {
            ...
            try {
                ...
                expect(error).toBeInstanceOf(NodeOperationError);
                expect(error.message).toBe('Need higher Role Level for this Operation (e.g. Owner or Admin Rights)');
                expect(error.description).toBe('Hint: Check the Role of your Slack App Integration. For more information see the Slack Documentation - https://slack.com/help/articles/360018112273-Types-of-roles-in-Slack');
                expect(error.level).toBe('warning');
            }
        });
        ...
        it('should surface transport/auth failures as NodeOperationError', async () => {
            ...
            await expect(slackApiRequest.call(mockExecuteFunctions, 'GET', '/test')).rejects.toThrow(NodeOperationError);
            await expect(slackApiRequest.call(mockExecuteFunctions, 'GET', '/test')).rejects.toThrow('fetch failed');
        });
        it('should not re-wrap an error that is already a NodeOperationError', async () => {
            ...
            await expect(slackApiRequest.call(mockExecuteFunctions, 'GET', '/test')).rejects.toBe(opError);
        });
    ...
    describe('slackApiRequestAllItems', () => {
        ...
        it('should use provided limit value when specified', async () => {
            ...
            mockExecuteFunctions.helpers.requestWithAuthentication = vi
                .fn()
                .mockResolvedValue(mockResponse);
            await slackApiRequestAllItems.call(mockExecuteFunctions, 'channels', 'GET', 'conversations.list', {}, { limit: 50 });
            expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('slackApi', expect.objectContaining({
                headers: expect.any(Object),
                qs: {
                    limit: 50,
        ...
        it('should handle files.list pagination', async () => {
            ...
            const result = await slackApiRequestAllItems.call(mockExecuteFunctions, 'files', 'GET', 'files.list');
        ...
        it('should handle matches in response', async () => {
            ...
            const result = await slackApiRequestAllItems.call(mockExecuteFunctions, 'messages', 'GET', 'search.messages');
    ...
    describe('slackApiRequestAllItemsWithRateLimit', () => {
        ...
        it('should paginate successfully without rate limits', async () => {
            ...
            mockExecuteFunctions.helpers.requestWithAuthentication = vi
                ...
                .mockImplementationOnce(() => responses[0])
                .mockImplementationOnce(() => responses[1]);
            const result = await slackApiRequestAllItemsWithRateLimit(mockExecuteFunctions, 'channels', 'GET', 'conversations.list');
        ...
        it('should use fallbackDelay when Retry-After header is missing', async () => {
            ...
            mockExecuteFunctions.helpers.requestWithAuthentication = vi
                ...
                .mockResolvedValueOnce(rateLimitError)
                .mockResolvedValueOnce(successResponse);
            const result = await slackApiRequestAllItemsWithRateLimit(mockExecuteFunctions, 'channels', 'GET', 'conversations.list', {}, {}, { fallbackDelay: 10000 });
            expect(result.data).toEqual([{ id: 'ch1' }]);
        ...
        it('should retry multiple times on consecutive 429 errors', async () => {
            ...
            mockExecuteFunctions.helpers.requestWithAuthentication = vi
                ...
                .mockResolvedValueOnce(rateLimitError)
                .mockResolvedValueOnce(successResponse);
            const result = await slackApiRequestAllItemsWithRateLimit(mockExecuteFunctions, 'channels', 'GET', 'conversations.list', {}, {}, { maxRetries: 3 });
            expect(result.data).toEqual([{ id: 'ch1' }]);
        ...
        it('should throw error when maxRetries exceeded with onFail: throw (default)', async () => {
            ...
            await expect(slackApiRequestAllItemsWithRateLimit(mockExecuteFunctions, 'channels', 'GET', 'conversations.list', {}, {}, { maxRetries: 2 })).rejects.toThrow(/Slack error response: "rate_limit_exceeded"/);
        ...
        it('should return partial data when maxRetries exceeded with onFail: stop', async () => {
            ...
            const result = await slackApiRequestAllItemsWithRateLimit(mockExecuteFunctions, 'channels', 'GET', 'conversations.list', {}, {}, { maxRetries: 2, onFail: 'stop' });
            expect(result.data).toEqual([{ id: 'ch1' }]);
        ...
        it('should return nextPage when using legacy pagination with onFail: stop', async () => {
            ...
            mockExecuteFunctions.helpers.requestWithAuthentication = vi
                ...
                .mockResolvedValue(rateLimitError)
                .mockResolvedValueOnce(firstPageResponse);
            const result = await slackApiRequestAllItemsWithRateLimit(mockExecuteFunctions, 'channels', 'GET', 'conversations.list', {}, {}, { maxRetries: 1, onFail: 'stop' });
            expect(result.data).toEqual([{ id: 'ch1' }]);
        ...
        it('should return nextPage from property paging when using onFail: stop', async () => {
            ...
            mockExecuteFunctions.helpers.requestWithAuthentication = vi
                ...
                .mockResolvedValue(rateLimitError)
                .mockResolvedValueOnce(firstPageResponse);
            const result = await slackApiRequestAllItemsWithRateLimit(mockExecuteFunctions, 'messages', 'GET', 'search.messages', {}, {}, { maxRetries: 1, onFail: 'stop' });
        ...
        it('should handle non-429 errors by throwing immediately', async () => {
            ...
            await expect(slackApiRequestAllItemsWithRateLimit(mockExecuteFunctions, 'channels', 'GET', 'conversations.list')).rejects.toThrow(/Slack error response: "Internal Server Error"/);
            expect(sleep).not.toHaveBeenCalled();
        });
        it('should use files.list endpoint with count parameter', async () => {
            ...
            mockExecuteFunctions.helpers.requestWithAuthentication = vi.fn().mockResolvedValue(response);
            await slackApiRequestAllItemsWithRateLimit(mockExecuteFunctions, 'files', 'GET', 'files.list');
            expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('slackApi', expect.objectContaining({
        ...
        it('should handle matches in response', async () => {
            ...
            mockExecuteFunctions.helpers.requestWithAuthentication = vi.fn().mockResolvedValue(response);
            const result = await slackApiRequestAllItemsWithRateLimit(mockExecuteFunctions, 'messages', 'GET', 'search.messages');
        ...
        it('should use default options when not provided', async () => {
            ...
            mockExecuteFunctions.helpers.requestWithAuthentication = vi
                ...
                .mockResolvedValueOnce(rateLimitError)
                .mockResolvedValueOnce(successResponse);
            await slackApiRequestAllItemsWithRateLimit(mockExecuteFunctions, 'channels', 'GET', 'conversations.list');
    ...
    describe('searchContextItems', () => {
        const page = (messages, nextCursor) => ({
            ok: true,
            results: { messages },
        ...
        it('should follow the cursor until it is empty', async () => {
            ...
            expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledTimes(2);
            expect(mockExecuteFunctions.helpers.requestWithAuthentication.mock.calls[0][1].body).toEqual({ query: 'test', limit: 20 });
            expect(mockExecuteFunctions.helpers.requestWithAuthentication.mock.calls[1][1].body).toEqual({ query: 'test', limit: 20, cursor: 'cursor-2' });
        });
        it('should cap the page size at 20 and stop once maxResults is reached', async () => {
            ...
            expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledTimes(2);
            expect(mockExecuteFunctions.helpers.requestWithAuthentication.mock.calls[0][1].body).toEqual({ query: 'test', limit: 20 });
            expect(mockExecuteFunctions.helpers.requestWithAuthentication.mock.calls[1][1].body).toEqual({ query: 'test', limit: 2, cursor: 'cursor-2' });
        });
        ...
        it('should still throw on any other error mid-pagination', async () => {
            ...
            await expect(searchContextItems.call(mockExecuteFunctions, { query: 'test' }, 50)).rejects.toMatchObject({
    ...
    describe('processThreadOptions', () => {
        ...
        it('should return empty object when threadOptions is null', () => {
            const result = processThreadOptions(null);
            expect(result).toEqual({});
        });
    ...
    describe('getMessageContent', () => {
        ...
        describe('block message type', () => {
            ...
            it('should handle block message type with includeLinkToWorkflow true', () => {
                ...
                mockExecuteFunctions.getNodeParameter.mockImplementation((param, _index) => {
                    if (param === 'messageType')
                        return 'block';
                    if (param === 'otherOptions.includeLinkToWorkflow')
                        return true;
                    if (param === 'text')
            ...
            it('should handle block message type without instanceId', () => {
                ...
                expect(result.blocks).toHaveLength(2);
                expect(result.blocks[1].text.text).toBe('_Automated with this <https://test.n8n.io/workflow/workflow-123?utm_source=n8n-internal&utm_medium=powered_by&utm_campaign=n8n-nodes-base.slack|n8n workflow>_');
            });
        ...
        describe('attachment message type', () => {
            ...
            it('should handle attachment with non-array attachments', () => {
                ...
                expect(result.attachments).toHaveLength(15);
                expect(result.attachments[result.attachments.length - 1]).toEqual({
                    text: '_Automated with this <https://test.n8n.io/workflow/workflow-123?utm_source=n8n-internal&utm_medium=powered_by&utm_campaign=n8n-nodes-base.slack_instance-123|n8n workflow>_',
                });
            });
        });
    ...
    describe('formatUserLabel', () => {
        it('should append the handle to the real name', () => {
            expect(formatUserLabel({ name: 'john.doe', real_name: 'John Doe' })).toBe('John Doe (@john.doe)');
        });
