...
describe('MessageDescription', () => {
    describe('Custom Bot Profile Photo', () => {
        const postOptions = messageFields.find((field) => field.name === 'otherOptions' &&
            field.displayOptions?.show?.operation?.includes('post'));
        const botProfileVariants = (postOptions?.options ?? []).filter((option) => 'name' in option && option.name === 'botProfile');
        const visibleVariants = (typeVersion, authentication) => botProfileVariants.filter((variant) => displayParameter({}, variant, { typeVersion }, null, { authentication }));
...
//# sourceMappingURL=MessageDescription.test.js.map