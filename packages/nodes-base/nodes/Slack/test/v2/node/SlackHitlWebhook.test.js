...
function createContext(opts) {
    ...
    ctx.getNodeParameter.mockImplementation((name, fallback) => {
        if (name === 'approvers')
            return (opts.approvers ?? []);
        if (name === 'authentication')
            return (opts.authentication ?? fallback);
        if (name === 'unauthorizedReplyText')
            return (opts.unauthorizedReplyText ?? fallback);
        if (name === 'postDecisionBehavior')
            return (opts.postDecisionBehavior ?? fallback);
        return fallback;
    });
...
describe('slackSendAndWaitWebhook', () => {
    ...
    it('resumes with the responder, channel and message id, and locks the message via response_url', async () => {
        ...
        const lockBody = httpRequest.mock.calls[0][0].body;
    ...
    it('ignores a click from someone not on the approver list and notifies them privately', async () => {
        ...
        expect(slackApiRequest).toHaveBeenCalledWith('POST', '/chat.postEphemeral', expect.objectContaining({ channel: 'C1', user: 'U_OTHER' }));
    ...
    it('sends the configured unauthorized reply text to an ignored responder', async () => {
        ...
        slackApiRequest.mockResolvedValue({ ok: true });
        await slackSendAndWaitWebhook.call(ctx);
        expect(slackApiRequest).toHaveBeenCalledWith('POST', '/chat.postEphemeral', expect.objectContaining({
            channel: 'C1',
            user: 'U_OTHER',
    ...
    it('falls back to chat.update when response_url fails', async () => {
        ...
        slackApiRequest.mockImplementation(async (_method, resource) => {
            if (resource === '/users.info')
                return { user: { profile: {} } };
            return { ok: true };
        });
        httpRequest.mockRejectedValue(new Error('response_url expired'));
        await slackSendAndWaitWebhook.call(ctx);
        expect(slackApiRequest).toHaveBeenCalledWith('POST', '/chat.update', expect.objectContaining({ channel: 'C1', ts: '111.222' }));
    });
...
//# sourceMappingURL=SlackHitlWebhook.test.js.map