...
import { getTarget, createSendAndWaitMessageBody, toMultiOptionsCsv, } from '../../V2/GenericFunctions';
describe('Slack Utility Functions', () => {
    let mockExecuteFunctions;
    beforeEach(() => {
        mockExecuteFunctions = mock();
        mockExecuteFunctions.getNode.mockReturnValue({ name: 'Slack', typeVersion: 1 });
        mockExecuteFunctions.getSignedResumeUrl.mockReturnValueOnce('http://localhost/waiting-webhook/nodeID?approved=true&signature=abc');
        mockExecuteFunctions.getSignedResumeUrl.mockReturnValueOnce('http://localhost/waiting-webhook/nodeID?approved=false&signature=abc');
        vi.clearAllMocks();
    });
    ...
    describe('createSendAndWaitMessageBody', () => {
        ...
        it('should create message with single button - 2.3+ mrkdwn', () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockReturnValueOnce('subject');
            mockExecuteFunctions.getNodeParameter.mockReturnValueOnce({});
            mockExecuteFunctions.getNode.mockReturnValue({ name: 'Slack', typeVersion: 2.3 });
            expect(createSendAndWaitMessageBody(mockExecuteFunctions)).toEqual({
                blocks: [
                    {
                        type: 'divider',
