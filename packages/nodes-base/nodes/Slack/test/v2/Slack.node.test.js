...
describe('SlackV2', () => {
    ...
    let slackApiRequestSpy;
    let slackApiRequestAllItemsSpy;
    const mockNode = {
        id: 'test-node-id',
        name: 'Slack Test',
    ...
    beforeEach(() => {
        ...
        mockExecuteFunctions.helpers.constructExecutionMetaData.mockImplementation((data, options) => {
    ...
    describe('Channel Operations - Kick and Join', () => {
        ...
        it('should kick user from channel successfully', async () => {
            ...
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/conversations.kick', {
                channel: 'C123456789',
                user: 'U987654321',
            }, {});
        ...
        it('should join channel successfully', async () => {
            ...
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/conversations.join', {
                channel: 'C123456789',
            }, {});
    ...
    describe('Channel List Operations - multiOptions types filter', () => {
        it('should send types as CSV when filters.types is an array', async () => {
            ...
            expect(slackApiRequestSpy).toHaveBeenCalledWith('GET', '/conversations.list', {}, expect.objectContaining({ types: 'public_channel,private_channel' }));
        });
    ...
    describe('Channel Operations - History, Invite, Leave, Members, etc.', () => {
        it('should get channel history with pagination', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue(mockResponse);
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('GET', '/conversations.history', {}, {
                channel: 'C123456789',
                limit: 50,
            });
            expect(result).toEqual([
        ...
        it('should get all channel history', async () => {
            ...
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestAllItemsSpy).toHaveBeenCalledWith('messages', 'GET', '/conversations.history', {}, {
                channel: 'C123456789',
                inclusive: true,
        ...
        it('should invite users to channel', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue(mockResponse);
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/conversations.invite', {
                channel: 'C123456789',
        ...
        it('should leave channel', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue({ ok: true });
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/conversations.leave', {
                channel: 'C123456789',
            }, {});
            expect(result).toEqual([[{ json: { ok: true }, pairedItem: { item: 0 } }]]);
        });
        it('should get channel members with resolve data', async () => {
            ...
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestAllItemsSpy).toHaveBeenCalledWith('members', 'GET', '/conversations.members', {}, {
                channel: 'C123456789',
            });
        ...
        it('should open conversation', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue(mockResponse);
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/conversations.open', {
                users: 'U111111111,U222222222',
                return_im: true,
            }, {});
            expect(result).toEqual([
        ...
        it('should rename channel', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue(mockResponse);
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/conversations.rename', {
                channel: 'C123456789',
                name: 'new-channel-name',
            }, {});
            expect(result).toEqual([
        ...
        it('should get conversation replies', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue(mockResponse);
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('GET', '/conversations.replies', {}, {
                channel: 'C123456789',
                ts: '1234567890.123456',
                limit: 100,
                inclusive: true,
        ...
        it('should set channel purpose', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue(mockResponse);
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/conversations.setPurpose', {
                channel: 'C123456789',
                purpose: 'New channel purpose',
            }, {});
            expect(result).toEqual([
        ...
        it('should set channel topic', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue(mockResponse);
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/conversations.setTopic', {
                channel: 'C123456789',
                topic: 'New channel topic',
            }, {});
            expect(result).toEqual([
        ...
        it('should unarchive channel', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue({ ok: true });
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/conversations.unarchive', {
                channel: 'C123456789',
            }, {});
            expect(result).toEqual([[{ json: { ok: true }, pairedItem: { item: 0 } }]]);
        });
    ...
    describe('Message Operations - Ephemeral Messages', () => {
        ...
        it('should send ephemeral message to channel', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue(mockResponse);
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/chat.postEphemeral', expect.objectContaining({
                channel: 'C123456789',
                text: 'Test ephemeral message',
                user: 'U123456789',
            }), {});
    ...
    describe('Reaction and Star Operations', () => {
        describe('Reaction Operations', () => {
            it('should add reaction to message', async () => {
                ...
                slackApiRequestSpy.mockResolvedValue({ ok: true });
                const result = await node.execute.call(mockExecuteFunctions);
                expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/reactions.add', {
                    channel: 'C123456789',
                    name: 'thumbs_up',
                    timestamp: '1234567890.123456',
                }, {});
            ...
            it('should remove reaction from message', async () => {
                ...
                slackApiRequestSpy.mockResolvedValue({ ok: true });
                const result = await node.execute.call(mockExecuteFunctions);
                expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/reactions.remove', {
                    channel: 'C123456789',
                    name: 'thumbs_up',
                    timestamp: '1234567890.123456',
                }, {});
            ...
            it('should get reactions for message', async () => {
                ...
                slackApiRequestSpy.mockResolvedValue(mockResponse);
                const result = await node.execute.call(mockExecuteFunctions);
                expect(slackApiRequestSpy).toHaveBeenCalledWith('GET', '/reactions.get', {}, {
                    channel: 'C123456789',
                    timestamp: '1234567890.123456',
                });
                expect(result).toEqual([
        ...
        describe('Star Operations', () => {
            it('should add star to message', async () => {
                ...
                slackApiRequestSpy.mockResolvedValue({ ok: true });
                const result = await node.execute.call(mockExecuteFunctions);
                expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/stars.add', {
                    channel: 'C123456789',
                    timestamp: '1234567890.123456',
                }, {});
                expect(result).toEqual([[{ json: { ok: true }, pairedItem: { item: 0 } }]]);
            ...
            it('should remove star', async () => {
                ...
                slackApiRequestSpy.mockResolvedValue({ ok: true });
                await node.execute.call(mockExecuteFunctions);
                expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/stars.remove', {
                    channel: 'C123456789',
                    file: 'F123456789',
                    file_comment: 'FC123456789',
                    timestamp: '1234567890.123456',
            ...
            it('should get all stars', async () => {
                ...
                const result = await node.execute.call(mockExecuteFunctions);
                expect(slackApiRequestAllItemsSpy).toHaveBeenCalledWith('items', 'GET', '/stars.list', {}, {});
                expect(result).toEqual([
                    [
                        {
                            json: mockStars,
            ...
            it('should get stars with limit', async () => {
                ...
                slackApiRequestSpy.mockResolvedValue(mockResponse);
                const result = await node.execute.call(mockExecuteFunctions);
                expect(slackApiRequestSpy).toHaveBeenCalledWith('GET', '/stars.list', {}, {
                    limit: 10,
                });
                expect(result).toEqual([
                    [
    ...
    describe('File Upload Operations - Binary Data Handling', () => {
        ...
        it('should upload file with binary data (node version <= 2.1)', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue(mockResponse);
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/files.upload', {}, {}, { 'Content-Type': 'multipart/form-data' }, {
        ...
        it('should upload file with external upload (node version > 2.1)', async () => {
            ...
            mockExecuteFunctions.helpers.getBinaryMetadata.mockResolvedValue(mockMetadata);
            ...
            expect(slackApiRequestSpy).toHaveBeenNthCalledWith(1, 'GET', '/files.getUploadURLExternal', {}, {
                filename: 'test.txt',
                length: 12,
            });
            // Check external upload call
            expect(slackApiRequestSpy).toHaveBeenNthCalledWith(2, 'POST', 'https://files.slack.com/upload/v1', {}, {}, { 'Content-Type': 'multipart/form-data' }, {
                formData: expect.objectContaining({
                    file: expect.objectContaining({
                        value: mockStream,
                    }),
            ...
            expect(slackApiRequestSpy).toHaveBeenNthCalledWith(3, 'POST', '/files.completeUploadExternal', {
                channels: 'C123456789',
                initial_comment: 'Test upload',
                files: [
                    {
        ...
        it('should upload file content as text', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue(mockResponse);
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/files.upload', {
                channels: 'C123456789',
        ...
        it('should use custom filename when provided', async () => {
            ...
            mockExecuteFunctions.helpers.assertBinaryData.mockReturnValue(mockBinaryData);
            mockExecuteFunctions.helpers.getBinaryStream.mockResolvedValue(Buffer.from('test'));
            mockExecuteFunctions.helpers.getBinaryMetadata.mockResolvedValue({
                fileSize: 4,
            });
            slackApiRequestSpy
    ...
    describe('File List Operations - multiOptions types filter', () => {
        it('should send types as CSV when filters.types is an array', async () => {
            ...
            await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('GET', '/files.list', {}, expect.objectContaining({ types: 'images,pdfs' }));
        });
    ...
    describe('Edge Cases and Error Scenarios', () => {
        ...
        it('should throw error for ephemeral message with username when select is user', async () => {
            ...
            await expect(node.execute.call(mockExecuteFunctions)).rejects.toThrow('You cannot send ephemeral messages using User type "By username". Please use "From List" or "By ID".');
        });
    });
    describe('User Group Operations', () => {
        describe('Add Users to User Group', () => {
            it('should add users to user group successfully', async () => {
                ...
                expect(slackApiRequestSpy).toHaveBeenNthCalledWith(1, 'GET', '/usergroups.list', {}, {
                    include_users: true,
                    usergroup: 'S123456789',
                });
                // Verify POST request to update users (should merge existing and new)
                expect(slackApiRequestSpy).toHaveBeenNthCalledWith(2, 'POST', '/usergroups.users.update', {
                    usergroup: 'S123456789',
                    users: 'U000000000,U111111111,U222222222',
                    include_count: true,
                }, {});
            ...
            it('should handle adding duplicate users (deduplication)', async () => {
                ...
                expect(slackApiRequestSpy).toHaveBeenNthCalledWith(2, 'POST', '/usergroups.users.update', expect.objectContaining({
        ...
        describe('Get Users from User Group', () => {
            it('should get users from user group with resolve data', async () => {
                ...
                expect(slackApiRequestSpy).toHaveBeenNthCalledWith(2, 'GET', '/users.info', {}, { user: 'U111111111' });
                expect(slackApiRequestSpy).toHaveBeenNthCalledWith(3, 'GET', '/users.info', {}, { user: 'U222222222' });
                expect(result).toEqual([
                    [
                        {
    ...
    describe('User Operations - Look Up by Email', () => {
        it('should look up a user by email and return the user object', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue(mockResponse);
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('GET', '/users.lookupByEmail', {}, { email: 'jane@example.com' });
            expect(result).toEqual([
                [
                    {
    ...
    describe('Message Operations - Schedule', () => {
        ...
        it('should schedule a message to a channel with post_at as unix seconds', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue(mockResponse);
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/chat.scheduleMessage', {
                channel: 'C123456789',
                post_at: 1893456000,
                text: 'Scheduled hello',
            }, {});
    ...
    describe('Message Operations - Delete Scheduled', () => {
        it('should delete a scheduled message by id', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue({ ok: true });
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/chat.deleteScheduledMessage', {
                channel: 'C123456789',
                scheduled_message_id: 'Q1298393284',
            }, {});
            expect(result).toEqual([[{ json: { ok: true }, pairedItem: { item: 0 } }]]);
    ...
    describe('Message Operations - Get Many Scheduled', () => {
        it('should list scheduled messages with a limit', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue(mockResponse);
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('GET', '/chat.scheduledMessages.list', {}, { limit: 10 });
            expect(result).toEqual([
                [
                    {
        ...
        it('should apply channel + latest + oldest filters as unix seconds', async () => {
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName, _i, fallback, _options) => {
                const params = {
                    resource: 'message',
                    operation: 'getManyScheduled',
                    returnAll: false,
            ...
            await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('GET', '/chat.scheduledMessages.list', {}, {
                channel: 'C123456789',
                latest: 1893542400,
                oldest: 1893456000,
                limit: 50,
        ...
        it('should return all scheduled messages when returnAll is true', async () => {
            ...
            slackApiRequestAllItemsSpy.mockResolvedValue(mockMessages);
            const result = await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestAllItemsSpy).toHaveBeenCalledWith('scheduled_messages', 'GET', '/chat.scheduledMessages.list', {}, {});
            expect(result).toEqual([
                [
                    {
                        json: mockMessages,
    ...
    describe('getUsers', () => {
        ...
        const asTuples = (options) => options.map((o) => [o.name, o.value]);
        beforeEach(() => {
        ...
        describe('listSearch', () => {
            ...
            it('should keep users sharing a real name distinguishable', async () => {
                const result = await node.methods.listSearch.getUsers.call(mockLoadOptionsFunctions, 'John Doe');
            ...
            it('should filter by real name', async () => {
                const result = await node.methods.listSearch.getUsers.call(mockLoadOptionsFunctions, 'jane s');
            ...
            it('should filter by user ID', async () => {
                const result = await node.methods.listSearch.getUsers.call(mockLoadOptionsFunctions, 'U222222222');
                expect(asTuples(result.results)).toEqual([['Jane Smith (@jane.smith)', 'U222222222']]);
            });
            ...
            it('should pass the pagination token through and return the next cursor', async () => {
                ...
                const result = await node.methods.listSearch.getUsers.call(mockLoadOptionsFunctions, undefined, 'page-2');
                expect(withRateLimitSpy).toHaveBeenCalledWith(mockLoadOptionsFunctions, 'members', 'GET', '/users.list', {}, { limit: 200, cursor: 'page-2' }, expect.objectContaining({ onFail: 'stop' }));
    ...
    describe('Message Operations - Search parameter visibility', () => {
        const searchProps = (typeVersion) => {
            ...
            const isShown = (property, nodeValues) => displayParameter(nodeValues, property, { typeVersion }, description);
            ...
            const showsReturnAll = description.properties.some((property) => property.name === 'returnAll' && isShown(property, base));
...
//# sourceMappingURL=Slack.node.test.js.map