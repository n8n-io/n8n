...
describe('SlackV1 — multiOptions parameter normalization', () => {
    ...
    describe('channel: invite — userIds', () => {
        it('joins an array of user IDs', async () => {
            ...
            await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/conversations.invite', { channel: 'C123456789', users: 'U111111111,U222222222' }, {});
        });
    ...
    describe('channel: open — options.users', () => {
        it('joins an array of users', async () => {
            ...
            await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/conversations.open', { users: 'U111111111,U222222222', return_im: true }, {});
        });
        it('normalizes a comma-joined string for users option', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue({ channel: { id: 'D1' } });
            await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/conversations.open', { users: 'U111111111,U222222222' }, {});
        });
    });
    describe('file: upload — options.channelIds', () => {
        it('joins an array of channel IDs', async () => {
            ...
            await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/files.upload', expect.objectContaining({ channels: 'C111111111,C222222222' }), {}, { 'Content-Type': 'application/x-www-form-urlencoded' }, expect.objectContaining({
        ...
        it('normalizes a comma-joined string for channelIds option', async () => {
            ...
            slackApiRequestSpy.mockResolvedValue({ file: { id: 'F1' } });
            await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestSpy).toHaveBeenCalledWith('POST', '/files.upload', expect.objectContaining({ channels: 'C111111111,C222222222' }), {}, { 'Content-Type': 'application/x-www-form-urlencoded' }, expect.anything());
        });
    });
    describe('file: getAll — filters.types', () => {
        it('joins an array of file types', async () => {
            ...
            await node.execute.call(mockExecuteFunctions);
            expect(slackApiRequestAllItemsSpy).toHaveBeenCalledWith('files', 'GET', '/files.list', {}, expect.objectContaining({ types: 'images,pdfs' }));
        });
...
//# sourceMappingURL=SlackV1.node.test.js.map