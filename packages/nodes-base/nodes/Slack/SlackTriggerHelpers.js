...
export async function getUserInfo(userId) {
    const user = await slackApiRequest.call(this, 'GET', '/users.info', {}, {
        user: userId,
    });
    return user.user.name;
}
export async function getChannelInfo(channelId) {
    const channel = await slackApiRequest.call(this, 'GET', '/conversations.info', {}, {
        channel: channelId,
    });
...
export async function downloadFile(url) {
    ...
    if (response.ok === false) {
        ...
        throw new NodeOperationError(this.getNode(), 'Slack error response: ' + JSON.stringify(response.error));
    }
    return response;
}
export async function verifySignature(credentialType = 'slackApi') {
