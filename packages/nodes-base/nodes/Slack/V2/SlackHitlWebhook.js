...
import { HITL_APPROVE_ACTION_ID } from './MessageInterface';
 ...
 */
async function extractSlackResponder(context, payload) {
    ...
    if (user.id) {
        try {
            const info = (await slackApiRequest.call(context, 'GET', '/users.info', {}, { user: user.id }));
            if (info?.user?.profile?.email)
                responder.email = info.user.profile.email;
        }
        catch {
 ...
 */
async function notifyNotAuthorized(context, payload, text) {
...
function buildDecisionBlocks(approved, responder) {
 ...
 */
async function lockSlackMessage(context, payload, blocks, text) {
    if (payload.response_url) {
        try {
            ...
            context.logger.warn(`Slack HITL: response_url update failed (${error instanceof Error ? error.message : String(error)}); trying chat.update`);
        }
    }
    const channel = payload.channel?.id;
    ...
    if (channel && ts) {
        try {
            ...
            context.logger.warn(`Slack HITL: chat.update failed (${error instanceof Error ? error.message : String(error)}); message left as-is`);
        }
    }
    else {
 ...
 */
export async function slackSendAndWaitWebhook() {
    ...
    const authentication = this.getNodeParameter('authentication', 'accessToken');
    ...
    const signingSecret = typeof credential.signatureSecret === 'string' ? credential.signatureSecret : '';
    ...
    const payload = typeof body.payload === 'string'
        ? jsonParse(body.payload, { fallbackValue: {} })
        : body;
    ...
    const approvers = this.getNodeParameter('approvers', []);
    if (approvers.length > 0 && !approvers.includes(payload.user?.id ?? '')) {
        ...
        await notifyNotAuthorized(this, payload, this.getNodeParameter('unauthorizedReplyText', 'You are not authorized to respond to this request.'));
        return { noWebhookResponse: true };
    }
    ...
    const postDecisionBehavior = this.getNodeParameter('postDecisionBehavior', 'showOutcome');
    if (postDecisionBehavior !== 'keepMessage') {
        ...
        const keptBlocks = (payload.message?.blocks ?? []).filter((block) => block.type !== 'actions');
        const lockedBlocks = (postDecisionBehavior === 'showOutcome'
            ? [...keptBlocks, ...buildDecisionBlocks(approved, responder)]
            : keptBlocks);
        await lockSlackMessage(this, payload, lockedBlocks, approved ? 'Request approved' : 'Request declined');
    }
    return {
        webhookResponse: '',
        workflowData: [
...
//# sourceMappingURL=SlackHitlWebhook.js.map