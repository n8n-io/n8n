...
import { BINARY_ENCODING, NodeConnectionTypes, NodeOperationError, SEND_AND_WAIT_OPERATION, } from 'n8n-workflow';
import { channelFields, channelOperations } from './ChannelDescription';
import { fileFields, fileOperations } from './FileDescription';
import { slackApiRequest, slackApiRequestAllItems, formatUserLabel, getMessageContent, getTarget, createSendAndWaitMessageBody, processThreadOptions, slackApiRequestAllItemsWithRateLimit, toMultiOptionsCsv, searchContextItems, } from './GenericFunctions';
import { advancedInteractivityNotice, approversField, captureResponderField, channelRLC, messageFields, messageOperations, postDecisionBehaviorField, replyToMessageField, sendToSelector, unauthorizedReplyField, userRLC, } from './MessageDescription';
...
import { getSendAndWaitProperties, SEND_AND_WAIT_WAITING_TOOLTIP, } from '../../../utils/sendAndWait/utils';
export class SlackV2 {
    description;
    constructor(baseDescription) {
        this.description = {
            ...
            properties: [
                ...
                ], { extraOptions: [replyToMessageField] }).filter((p) => p.name !== 'subject'),
                ...starOperations,
                ...starFields,
                ...fileOperations,
                ...fileFields,
    ...
    methods = {
        listSearch: {
            async getChannels(filter, paginationToken) {
                const qs = {
                ...
                const { data: channels, cursor } = await slackApiRequestAllItemsWithRateLimit(this, 'channels', 'GET', '/conversations.list', {}, qs, {
                    onFail: 'stop',
                    maxRetries: 2,
            ...
            async getUsers(filter, paginationToken) {
                ...
                const { data: users, cursor } = await slackApiRequestAllItemsWithRateLimit(this, 'members', 'GET', '/users.list', {}, qs, {
                    onFail: 'stop',
                    maxRetries: 2,
                    fallbackDelay: 30_000,
                });
        ...
        loadOptions: {
            ...
            async getUsers() {
                const returnData = [];
                const { data: users } = await slackApiRequestAllItemsWithRateLimit(this, 'members', 'GET', '/users.list', {}, { limit: 200 }, { onFail: 'stop' });
    ...
    async execute() {
        ...
        if (resource === 'message' && operation === SEND_AND_WAIT_OPERATION) {
            try {
                await slackApiRequest.call(this, 'POST', '/chat.postMessage', createSendAndWaitMessageBody(this));
            }
            catch (error) {
            ...
            try {
                ...
                if (resource === 'channel') {
                    ...
                    if (operation === 'history') {
                        ...
                        if (nodeVersion >= 2.4) {
                            responseData.sort((a, b) => +(b.ts ?? 0) - +(a.ts ?? 0));
                        }
                    }
                ...
                if (resource === 'message') {
                    ...
                    if (operation === 'post') {
                        ...
                        if (select === 'user' &&
                            ...
                            this.getNodeParameter('user', i)?.mode ===
                                'username') {
                            throw new NodeOperationError(this.getNode(), 'You cannot send ephemeral messages using User type "By username". Please use "From List" or "By ID".');
                        }
                        else {
                    ...
                    if (operation === 'schedule') {
                        ...
                        const target = getTarget(this, i, select);
                        const { sendAsUser } = this.getNodeParameter('otherOptions', i);
                        const postAt = this.getNodeParameter('postAt', i);
                        const content = getMessageContent.call(this, i, nodeVersion, instanceId);
                        const body = {
                            channel: target,
                        ...
                        delete otherOptions.botProfile;
                        Object.assign(body, otherOptions);
                        responseData = await slackApiRequest.call(this, 'POST', '/chat.scheduleMessage', body, qs);
                    }
                    ...
                    if (operation === 'deleteScheduled') {
                        ...
                        responseData = await slackApiRequest.call(this, 'POST', '/chat.deleteScheduledMessage', body, qs);
                    }
                    ...
                    if (operation === 'getManyScheduled') {
                        ...
                        if (filters.latest) {
                            qs.latest = Math.floor(new Date(filters.latest).getTime() / 1000);
                        }
                        if (filters.oldest) {
                            qs.oldest = Math.floor(new Date(filters.oldest).getTime() / 1000);
                        }
                        if (returnAll) {
                            responseData = await slackApiRequestAllItems.call(this, 'scheduled_messages', 'GET', '/chat.scheduledMessages.list', {}, qs);
                        }
                        else {
                            qs.limit = this.getNodeParameter('limit', i);
                            responseData = await slackApiRequest.call(this, 'GET', '/chat.scheduledMessages.list', {}, qs);
                    ...
                    if (operation === 'search') {
                        ...
                        if (nodeVersion >= 2.7) {
                            ...
                            if (options.after) {
                                body.after = moment.tz(options.after, timezone).unix();
                            }
                            if (options.before) {
                                body.before = moment.tz(options.before, timezone).unix();
                            }
                            ...
                            responseData = await searchContextItems.call(this, body, this.getNodeParameter('limit', i));
                        }
                        else {
                            ...
                            if (returnAll) {
                                responseData = await slackApiRequestAllItems.call(this, 'messages', 'GET', '/search.messages', {}, qs);
                            }
                            else {
                    ...
                    if (operation === 'upload') {
                        ...
                        if (this.getNodeParameter('binaryData', i, false) || nodeVersion > 2.1) {
                            ...
                            if (nodeVersion <= 2.1) {
                                ...
                                const uploadUrl = await slackApiRequest.call(this, 'GET', '/files.getUploadURLExternal', {}, {
                                    filename: options.fileName ? options.fileName : binaryData.fileName,
                                    length: fileSize,
                                });
                                await slackApiRequest.call(this, 'POST', uploadUrl.upload_url, {}, qs, { 'Content-Type': 'multipart/form-data' }, { formData: fileBody });
                                body.files = [
                                    {
                                ...
                                responseData = await slackApiRequest.call(this, 'POST', '/files.completeUploadExternal', body);
                ...
                if (resource === 'user') {
                    ...
                    if (operation === 'updateProfile') {
                        ...
                        responseData = await slackApiRequest.call(this, 'POST', '/users.profile.set', requestBody, qs);
                        responseData = responseData.profile;
                    }
                }
                if (resource === 'userGroup') {
                    ...
                    if (operation === 'updateUsers') {
                        const userGroupId = this.getNodeParameter('userGroupId', i);
                        const usersToAdd = this.getNodeParameter('users', i);
                        const options = this.getNodeParameter('options', i);
                        // First, get the current list of users in the group
                        const currentGroupData = await slackApiRequest.call(this, 'GET', '/usergroups.list', {}, { ...qs, include_users: true, usergroup: userGroupId });
                        // Find the specific user group from the list
                        const currentGroup = currentGroupData.usergroups.find((group) => group.id === userGroupId);
                        ...
                        Object.assign(body, options);
                        responseData = await slackApiRequest.call(this, 'POST', '/usergroups.users.update', body, qs);
                        responseData = responseData.usergroup;
                    }
                    //https://api.slack.com/methods/usergroups.list
                    if (operation === 'getUsers') {
                        ...
                        const groupData = await slackApiRequest.call(this, 'GET', '/usergroups.list', {}, { ...qs, include_users: true, usergroup: userGroupId });
                        // Find the specific user group
                        const userGroup = groupData.usergroups.find((group) => group.id === userGroupId);
                        if (!userGroup) {
                            throw new NodeOperationError(this.getNode(), `User group with ID "${userGroupId}" not found`);
                        }
                        const userIds = userGroup.users || [];
                        if (resolveData && userIds.length > 0) {
                            // Fetch full user details for each user
                            const users = [];
                            for (const userId of userIds) {
                                const { user } = await slackApiRequest.call(this, 'GET', '/users.info', {}, { user: userId });
                                users.push(user);
                            }
                            responseData = users;
                        }
                            ...
                            responseData = userIds.map((userId) => ({ id: userId }));
                        }
                    }
                }
