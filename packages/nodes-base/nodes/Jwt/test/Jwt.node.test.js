...
describe('JWT Node - custom header claims', () => {
    ...
    const setupExecuteFunctions = (params, creds) => {
        const ctx = mockDeep();
        ...
        ctx.getNodeParameter.mockImplementation((name, _itemIndex, fallback) => (params[name] ?? fallback));
        return ctx;
    };
    const signToken = async (params, creds) => {
        const ctx = setupExecuteFunctions(params, creds);
        const result = await new Jwt().execute.call(ctx);
        return result[0][0].json.token;
    };
...
//# sourceMappingURL=Jwt.node.test.js.map