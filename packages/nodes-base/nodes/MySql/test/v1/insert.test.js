...
describe('Test MySqlV1, insert - identifier escaping', () => {
    new NodeTestHarness().setupTests({
        ...
        customAssertions() {
            ...
            expect(mockConnection.query).toHaveBeenCalledWith(expect.stringContaining('`users); groups`'), expect.anything());
            expect(mockConnection.query).toHaveBeenCalledWith(expect.stringContaining('`name`'), expect.anything());
        },
    });
});
describe('Test MySqlV1, insert - table name with backtick', () => {
    new NodeTestHarness().setupTests({
        ...
        customAssertions() {
            ...
            expect(mockConnection.query).toHaveBeenCalledWith(expect.stringContaining('`user`.`; groups`'), expect.anything());
        },
    });
});
