...
import { prepareQueryAndReplacements, prepareSafeQuery, wrapData, addWhereClauses, addSortRules, replaceEmptyStringsByNulls, escapeSqlIdentifier, splitQueryToStatements, } from '../../v2/helpers/utils';
const mySqlMockNode = {
    id: '1',
    name: 'MySQL node',
    typeVersion: 2,
...
describe('Test MySql V2, prepareQueryAndReplacements', () => {
    it('should transform query and values', () => {
        const preparedQuery = prepareQueryAndReplacements('SELECT * FROM $1:name WHERE id = $2 AND name = $4 AND $3:name = 28', 2.5, ['table', 15, 'age', 'Name']);
        expect(preparedQuery).toBeDefined();
        expect(preparedQuery.query).toEqual('SELECT * FROM `table` WHERE id = ? AND name = ? AND `age` = 28');
    ...
    it('should not replace dollar amounts inside quoted strings', () => {
        const preparedQuery = prepareQueryAndReplacements("INSERT INTO test_table(content) VALUES('This is for testing $60')", 2.5, []);
        expect(preparedQuery).toBeDefined();
        expect(preparedQuery.query).toEqual("INSERT INTO test_table(content) VALUES('This is for testing $60')");
        expect(preparedQuery.values.length).toEqual(0);
    });
    it('should handle mixed parameters and dollar amounts in quotes', () => {
        const preparedQuery = prepareQueryAndReplacements("INSERT INTO $1:name(content, price) VALUES('Product costs $60', $2)", 2.5, ['products', 59.99]);
        expect(preparedQuery).toBeDefined();
        expect(preparedQuery.query).toEqual("INSERT INTO `products`(content, price) VALUES('Product costs $60', ?)");
    ...
    it('should handle parameters in double quotes', () => {
        const preparedQuery = prepareQueryAndReplacements('INSERT INTO test_table(content) VALUES("Price is $100 and $200")', 2.5, []);
        expect(preparedQuery).toBeDefined();
        expect(preparedQuery.query).toEqual('INSERT INTO test_table(content) VALUES("Price is $100 and $200")');
        expect(preparedQuery.values.length).toEqual(0);
    });
    it('should process parameters in correct order despite reverse processing', () => {
        const preparedQuery = prepareQueryAndReplacements('SELECT * FROM table WHERE col1 = $1 AND col2 = $2 AND col3 = $3 AND col4 = $4 AND col5 = $5', 2.5, ['value1', 'value2', 'value3', 'value4', 'value5']);
        expect(preparedQuery).toBeDefined();
        expect(preparedQuery.query).toEqual('SELECT * FROM table WHERE col1 = ? AND col2 = ? AND col3 = ? AND col4 = ? AND col5 = ?');
    ...
    it('should handle escaped single quotes correctly', () => {
        const preparedQuery = prepareQueryAndReplacements("INSERT INTO test_table(content) VALUES('Don''t replace $1 here')", 2.5, ['should_not_appear', 123]);
        expect(preparedQuery).toBeDefined();
        expect(preparedQuery.query).toEqual("INSERT INTO test_table(content) VALUES('Don''t replace $1 here')");
    });
    ...
    it('should use legacy processing for versions < 2.5', () => {
        expect(() => prepareQueryAndReplacements('SELECT * FROM users WHERE id = $4', 2.4, ['value1', 'value2'])).not.toThrow();
    });
    it('should use new processing for versions >= 2.5', () => {
        expect(() => prepareQueryAndReplacements('SELECT * FROM users WHERE id = $4', 2.5, ['value1', 'value2'])).toThrow('Parameter $4 referenced in query but no replacement value provided at index 4');
    });
    ...
    it('should throw error when multiple parameters are missing replacement values', () => {
        expect(() => {
            prepareQueryAndReplacements('SELECT * FROM users WHERE id = $3 AND name = $5', 2.5, ['value1']);
    ...
    it('should not throw error when all referenced parameters have replacement values', () => {
        expect(() => {
            prepareQueryAndReplacements('SELECT * FROM users WHERE id = $1 AND name = $2', 2.5, ['123', 'John']);
        }).not.toThrow();
    });
});
describe('Test MySql, prepareSafeQuery', () => {
    ...
    it('should throw when a referenced parameter has no replacement value', () => {
        expect(() => prepareSafeQuery('select * from users where id = $1', [])).toThrow('Parameter $1 referenced in query but no replacement value provided at index 1');
    });
});
...
describe('Test MySql V2, addWhereClauses', () => {
    it('add where clauses to query', () => {
        ...
        const [query, values] = addWhereClauses(mySqlMockNode, 0, 'SELECT * FROM `pet`', whereClauses, []);
    ...
    it('add where clauses to query combined by OR', () => {
        ...
        const [query, values] = addWhereClauses(mySqlMockNode, 0, 'SELECT * FROM `pet`', whereClauses, [], 'OR');
...
describe('Test MySql V2, splitQueryToStatements', () => {
    it('should split query into statements', () => {
        const query = "insert into models (`created_at`, custom_ship_time, id) values ('2023-09-07 10:26:20', 'some random; data with a semicolon', 1); insert into models (`created_at`, custom_ship_time, id) values ('2023-09-07 10:27:55', 'random data without semicolon\n', 2);";
    ...
    it('should not split by ; inside string literal', () => {
        const query = "SELECT custom_ship_time FROM models WHERE models.custom_ship_time LIKE CONCAT('%', ';', '%') LIMIT 10";
        const statements = splitQueryToStatements(query);
        expect(statements).toBeDefined();
        expect(statements).toEqual([
    ...
    describe('where clause handling', () => {
        ...
        test.each(invalidOperations)('getWhereClauses throws an exception for "%s" operation', (operation) => {
            ...
            const ctx = mock({ getNodeParameter: getNodeParameterMock });
            expect(() => utils.getWhereClauses(ctx, 0)).toThrow();
        });
        test.each(validOperations)('getWhereClauses returns valid clauses for "%s" operation', (operation) => {
            const clauses = [
                {
                    column: 'name',
                    condition: 'LIKE',
