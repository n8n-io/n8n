...
describe('Test MySql V2, runQueries', () => {
    ...
    describe('in single query batch mode', () => {
        it('should set paired items correctly', async () => {
            ...
            const result = await configureQueryRunner.call(mockExecuteFns, nodeOptions, pool)([
    ...
    it('should execute in "independently" mode, should return success true', async () => {
        const nodeOptions = { queryBatching: BATCH_MODE.INDEPENDENTLY, nodeVersion: 2 };
        const pool = createFakePool(fakeConnection);
        const fakeExecuteFunction = createMockExecuteFunction({}, mySqlMockNode);
        const runQueries = configureQueryRunner.call(fakeExecuteFunction, nodeOptions, pool);
        const poolGetConnectionSpy = vi.spyOn(pool, 'getConnection');
        ...
        expect(connectionFormatSpy).toBeCalledTimes(1);
        expect(connectionFormatSpy).toBeCalledWith('SELECT * FROM my_table WHERE id = ?; SELECT * FROM my_table WHERE id = ?', [55, 42]);
        expect(connectionReleaseSpy).toBeCalledTimes(1);
    });
    it('should execute in "transaction" mode, should return success true', async () => {
        const nodeOptions = { queryBatching: BATCH_MODE.TRANSACTION, nodeVersion: 2 };
        const pool = createFakePool(fakeConnection);
        const fakeExecuteFunction = createMockExecuteFunction({}, mySqlMockNode);
        const runQueries = configureQueryRunner.call(fakeExecuteFunction, nodeOptions, pool);
        const poolGetConnectionSpy = vi.spyOn(pool, 'getConnection');
    ...
    it('should return error item with continueOnFail = true for connection error', async () => {
        ...
        const result = await configureQueryRunner.call(fakeExecuteFunction, nodeOptions, pool)([{ query: 'SELECT * FROM my_table WHERE id = ?', values: [55] }]);
    ...
    it('should throw error when continueOnFail = false for connection error', async () => {
        ...
        await expect(configureQueryRunner.call(fakeExecuteFunction, nodeOptions, pool)([{ query: 'SELECT * FROM my_table WHERE id = ?', values: [55] }])).rejects.toThrow('Connection refused');
    });
});
