...
describe('Test MySql V2, operations', () => {
    ...
    it('deleteTable: delete, should call runQueries with', async () => {
        ...
        expect(result).toEqual([{ json: { success: true }, pairedItem: [{ item: 0 }] }]);
        expect(poolQuerySpy).toBeCalledTimes(1);
        expect(poolQuerySpy).toBeCalledWith("DELETE FROM `test_table` WHERE `id` = '1' AND `name` LIKE 'some%'");
    });
    ...
    it('deleteTable: delete, should route prep error to error output when continueOnFail is true', async () => {
        ...
        const result = await deleteTable.execute.call(fakeExecuteFunction, emptyInputItems, mockRunQueries);
    ...
    it('executeQuery, should call runQueries with', async () => {
        ...
        fakeConnectionCopy.query = vi.fn(async (query) => {
            const result = [];
        ...
        const fakeExecuteFunction = createMockExecuteFunction(nodeParameters, mySqlMockNode);
        const runQueries = configureQueryRunner.call(fakeExecuteFunction, nodeOptions, pool);
        const result = await executeQuery.execute.call(fakeExecuteFunction, emptyInputItems, runQueries, nodeOptions);
        expect(result).toBeDefined();
        ...
        expect(connectionQuerySpy).toBeCalledWith("insert into `test_table` (id, name) values (1, 'test 1')");
    ...
    it('executeQuery, should route parameter validation error to error output when continueOnFail is true', async () => {
        ...
        const nodeOptions = { nodeVersion: 2.5 };
        const mockRunQueries = vi.fn(async () => []);
        const fakeExecuteFunction = createMockExecuteFunction(nodeParameters, mySqlMockNode, true);
        const result = await executeQuery.execute.call(fakeExecuteFunction, emptyInputItems, mockRunQueries, nodeOptions);
        expect(result).toHaveLength(1);
    ...
    it('executeQuery, should throw parameter validation error when continueOnFail is false', async () => {
        ...
        const promise = executeQuery.execute.call(fakeExecuteFunction, emptyInputItems, mockRunQueries, nodeOptions);
        await expect(promise).rejects.toThrow('Parameter $1 referenced in query but no replacement value provided');
        expect(mockRunQueries).not.toHaveBeenCalled();
    });
    it('executeQuery, should route error item before success items when a preceding item fails query preparation', async () => {
        ...
        const successResult = { json: { success: true }, pairedItem: { item: 1 } };
        ...
        const result = await executeQuery.execute.call(fakeExecuteFunction, inputItems, mockRunQueries, nodeOptions);
    ...
    it('select, should call runQueries with', async () => {
        ...
        const fakeExecuteFunction = createMockExecuteFunction(nodeParameters, mySqlMockNode);
        const runQueries = configureQueryRunner.call(fakeExecuteFunction, { ...nodeOptions, nodeVersion: 2 }, pool);
        ...
        expect(connectionQuerySpy).toBeCalledWith("SELECT * FROM `test_table` WHERE `id` > 1 OR `name` = 'test' ORDER BY `id` DESC LIMIT 2");
    ...
    it('select, should route prep error to error output when continueOnFail is true', async () => {
        ...
        const mockRunQueries = vi.fn(async () => []);
        const fakeExecuteFunction = createMockExecuteFunction(nodeParameters, mySqlMockNode, true);
        const result = await select.execute.call(fakeExecuteFunction, emptyInputItems, mockRunQueries);
        expect(result).toHaveLength(1);
        expect(result[0].error).toBeDefined();
        expect(result[0].pairedItem).toEqual({ item: 0 });
        expect(mockRunQueries).not.toHaveBeenCalled();
    ...
    it('insert, should call runQueries with', async () => {
        ...
        const fakeExecuteFunction = createMockExecuteFunction(nodeParameters, mySqlMockNode);
        const runQueries = configureQueryRunner.call(fakeExecuteFunction, nodeOptions, pool);
        const result = await insert.execute.call(fakeExecuteFunction, emptyInputItems, runQueries, nodeOptions);
        expect(result).toBeDefined();
        expect(result).toEqual([{ json: { success: true }, pairedItem: { item: 0 } }]);
        expect(connectionQuerySpy).toBeCalledTimes(1);
        expect(connectionQuerySpy).toBeCalledWith("INSERT HIGH_PRIORITY IGNORE INTO `test_table` (`id`, `name`) VALUES ('2','name 2')");
    });
    it('insert, should route prep error to error output when continueOnFail is true', async () => {
        ...
        const mockRunQueries = vi.fn(async () => []);
        const fakeExecuteFunction = createMockExecuteFunction(nodeParameters, mySqlMockNode, true);
        const result = await insert.execute.call(fakeExecuteFunction, emptyInputItems, mockRunQueries, nodeOptions);
        expect(result).toHaveLength(1);
        expect(result[0].error).toBeDefined();
        expect(result[0].pairedItem).toEqual({ item: 0 });
        expect(mockRunQueries).not.toHaveBeenCalled();
    ...
    it('update, should call runQueries with', async () => {
        ...
        const result = await update.execute.call(fakeExecuteFunction, inputItems, runQueries, nodeOptions);
        expect(result).toBeDefined();
        expect(result).toEqual([
        ...
        expect(connectionQuerySpy).toBeCalledWith("UPDATE `test_table` SET `name` = 'test 4' WHERE `id` = 42");
        expect(connectionQuerySpy).toBeCalledWith("UPDATE `test_table` SET `name` = 'test 88' WHERE `id` = 88");
    });
    it('update, should route prep error to error output when continueOnFail is true', async () => {
        ...
        const mockRunQueries = vi.fn(async () => []);
        const fakeExecuteFunction = createMockExecuteFunction(nodeParameters, mySqlMockNode, true);
        const result = await update.execute.call(fakeExecuteFunction, emptyInputItems, mockRunQueries, nodeOptions);
        expect(result).toHaveLength(1);
        expect(result[0].error).toBeDefined();
        expect(result[0].pairedItem).toEqual({ item: 0 });
        expect(mockRunQueries).not.toHaveBeenCalled();
    ...
    it('upsert, should call runQueries with', async () => {
        ...
        const result = await upsert.execute.call(fakeExecuteFunction, inputItems, runQueries, nodeOptions);
        expect(result).toBeDefined();
        expect(result).toEqual([{ json: { success: true }, pairedItem: [{ item: 0 }, { item: 1 }] }]);
        expect(poolQuerySpy).toBeCalledTimes(1);
        expect(poolQuerySpy).toBeCalledWith("INSERT INTO `test_table`(`id`, `name`) VALUES(42,'test 4') ON DUPLICATE KEY UPDATE `name` = 'test 4';INSERT INTO `test_table`(`id`, `name`) VALUES(88,'test 88') ON DUPLICATE KEY UPDATE `name` = 'test 88'");
    });
    it('upsert, should route prep error to error output when continueOnFail is true', async () => {
        ...
        const mockRunQueries = vi.fn(async () => []);
        const fakeExecuteFunction = createMockExecuteFunction(nodeParameters, mySqlMockNode, true);
        const result = await upsert.execute.call(fakeExecuteFunction, emptyInputItems, mockRunQueries, nodeOptions);
        expect(result).toHaveLength(1);
        expect(result[0].error).toBeDefined();
        expect(result[0].pairedItem).toEqual({ item: 0 });
        expect(mockRunQueries).not.toHaveBeenCalled();
    ...
    it('executeQuery, should map pairedItem to original item index when a preceding item fails query preparation', async () => {
        const nodeOptions = { nodeVersion: 2.5, queryBatching: 'independently' };
        const pool = createFakePool(fakeConnection);
        const fakeExecuteFunction = {
            getNodeParameter(parameterName, itemIndex, fallbackValue) {
                if (parameterName === 'query') {
            ...
            helpers: {
                constructExecutionMetaData: (data, meta) => data.map((d) => ({ ...d, ...meta })),
            },
        };
        const runQueries = configureQueryRunner.call(fakeExecuteFunction, nodeOptions, pool);
        const inputItems = [
        ...
        const result = await executeQuery.execute.call(fakeExecuteFunction, inputItems, runQueries, nodeOptions);
