    ...
    async execute() {
        ...
        if (operation === 'executeQuery') {
            ...
            try {
                const queryQueue = items.map(async (_, index) => {
                    ...
                    if (!Array.isArray(queryReplacement)) {
                        throw new NodeOperationError(this.getNode(), 'Query Parameters must be a string of comma-separated values, or an array of values', { itemIndex: index });
                    }
                    const { query, values } = prepareSafeQuery(rawQuery, queryReplacement);
            ...
            try {
                ...
                const insertSQL = `INSERT ${insertPriority || ''} ${insertIgnore ? 'IGNORE' : ''} INTO ${escapeSqlIdentifier(table)}(${columns.map(escapeSqlIdentifier).join(',')}) VALUES ${items
