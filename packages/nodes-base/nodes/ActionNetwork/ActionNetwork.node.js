...
import { attendanceFields, attendanceOperations, eventFields, eventOperations, personFields, personOperations, personTagFields, personTagOperations, petitionFields, petitionOperations, signatureFields, signatureOperations, tagFields, tagOperations, } from './descriptions';
import { actionNetworkApiRequest, adjustEventPayload, adjustPersonPayload, adjustPetitionPayload, handleListing, makeOsdiLink, resourceLoaders, simplifyResponse, } from './GenericFunctions';
export class ActionNetwork {
    description = {
            ...
            try {
                if (resource === 'attendance') {
                    ...
                    if (operation === 'create') {
                        ...
                        const body = makeOsdiLink(personId);
                    ...
                    if (operation === 'create') {
                        ...
                        // ----------------------------------------
                        const personId = this.getNodeParameter('personId', i);
                        response = (await actionNetworkApiRequest.call(this, 'GET', `/people/${personId}`));
                    }
                    else if (operation === 'getAll') {
                        // ----------------------------------------
                        //              person: getAll
                        // ----------------------------------------
                        response = (await handleListing.call(this, 'GET', '/people'));
                    }
                ...
                if (simplify) {
                    response =
                        operation === 'getAll'
                            ? response.map((entry) => simplifyResponse(entry, resource))
