import { TriggerCloseError, NodeConnectionTypes, } from 'n8n-workflow';
import { pgTriggerFunction, initDB, searchSchema, searchTables, prepareNames, validatePostgresIdentifier, } from './PostgresTrigger.functions';
export class PostgresTrigger {
    description = {
    ...
    async trigger() {
        ...
        try {
            ...
            if (triggerMode === 'createTrigger') {
                await pgTriggerFunction.call(this, db, additionalFields, pgNames.functionName, pgNames.triggerName, pgNames.channelName);
            }
            // listen to channel
        ...
        const cleanUpDb = async () => {
            try {
                ...
                try {
                    ...
                    throw new TriggerCloseError(this.getNode(), { cause: error, level: 'error' });
                }
            }
            finally {
