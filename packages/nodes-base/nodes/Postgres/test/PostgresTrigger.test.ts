...
jest.mock('../transport', () => ({ configurePostgres: jest.fn() }));
...
describe('pgTriggerFunction', () => {
	const createContext = (params: IDataObject) => {
		const getNodeParameter = jest.fn((name: string, _i?: unknown, fallback?: unknown) =>
...
describe('PostgresTrigger.trigger (Table Row Change Events mode)', () => {
	const setup = (additionalFields: IDataObject = {}, firesOn = 'INSERT') => {
		const connection = {
			...
			client: { on: jest.fn(), removeListener: jest.fn() },
		};
		const db = {
			connect: jest.fn().mockResolvedValue(connection),
			any: jest.fn().mockResolvedValue([]),
		};
		jest.mocked(configurePostgres).mockResolvedValue({ db, pgp: {} } as never);
		...
		const fns = {
			getNodeParameter: jest.fn((name: string, _i?: unknown, fallback?: unknown) =>
				name in params ? params[name] : fallback,
			),
			...
			emit: jest.fn(),
			helpers: { returnJsonArray: jest.fn((d: unknown) => d) },
...
describe('PostgresTrigger.trigger (Advanced mode)', () => {
	const setup = (channelName: string) => {
		...
		const db = { connect: jest.fn().mockResolvedValue(connection) };
		jest.mocked(configurePostgres).mockResolvedValue({ db, pgp: {} } as never);
