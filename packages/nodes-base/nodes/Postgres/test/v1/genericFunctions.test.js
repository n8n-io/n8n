...
describe('pgUpdate', () => {
    ...
    it('rejects column entries whose cast is not a valid PostgreSQL type', async () => {
        ...
        await expect(PostgresFun.pgUpdate(getNodeParam, pgp, db, items)).rejects.toThrow(/Invalid column type/);
...
describe('pgInsert', () => {
    ...
    it('accepts type casts with precision and array suffixes', async () => {
        ...
        await PostgresFun.pgInsert(getNodeParam, pgp, db, items, false);
        expect(db.any).toHaveBeenCalledWith('insert into "myschema"."mytable"("code","scores") values(\'abc\'::varchar(10),\'{1,2}\'::int[]) RETURNING *');
    });
    it('rejects column entries whose cast is not a valid PostgreSQL type', async () => {
        const nodeParams = {
            table: 'mytable',
        ...
        const getNodeParam = (key) => nodeParams[key];
        const items = [{ json: { name: 'test' } }];
        await expect(PostgresFun.pgInsert(getNodeParam, pgp, db, items, false)).rejects.toThrow(/Invalid column type/);
        expect(db.any).not.toHaveBeenCalled();
    });
    ...
    it('accepts ARRAY type casts with parameterized element type', async () => {
        ...
        const items = [{ json: insertItem }];
        await PostgresFun.pgInsert(getNodeParam, pgp, db, items, false);
        expect(db.any).toHaveBeenCalledWith('insert into "myschema"."mytable"("tags") values(\'{1,2,3}\'::ARRAY(INTEGER)) RETURNING *');
    });
    it('accepts OBJECT type casts with column policy', async () => {
        ...
        const items = [{ json: insertItem }];
        await PostgresFun.pgInsert(getNodeParam, pgp, db, items, false);
        expect(db.any).toHaveBeenCalledWith('insert into "myschema"."mytable"("meta") values(\'x\'::OBJECT(DYNAMIC)) RETURNING *');
    });
    it('accepts GEOHASH type casts with precision', async () => {
        ...
        const items = [{ json: insertItem }];
        await PostgresFun.pgInsert(getNodeParam, pgp, db, items, false);
        expect(db.any).toHaveBeenCalledWith('insert into "myschema"."mytable"("location") values(\'u4pruydqqvj\'::geohash(8c)) RETURNING *');
    });
    it('accepts schema-qualified type casts', async () => {
        ...
        const items = [{ json: insertItem }];
        await PostgresFun.pgInsert(getNodeParam, pgp, db, items, false);
        expect(db.any).toHaveBeenCalledWith('insert into "myschema"."mytable"("email") values(\'a@b.com\'::public.citext) RETURNING *');
    });
    it('accepts type casts with precision and a trailing modifier', async () => {
        ...
        const items = [{ json: insertItem }];
        await PostgresFun.pgInsert(getNodeParam, pgp, db, items, false);
        expect(db.any).toHaveBeenCalledWith('insert into "myschema"."mytable"("t") values(\'2026-01-01T00:00:00Z\'::timestamp(3) with time zone) RETURNING *');
    });
});
