...
import { pgTriggerFunction, prepareNames, validatePostgresIdentifier, } from '../PostgresTrigger.functions';
...
describe('validatePostgresIdentifier', () => {
    it.each(['n8n_channel', '_fn1', 'MyTrigger', 'a', 'x_1_y'])('accepts the valid identifier %s', (name) => {
...
describe('prepareNames', () => {
    ...
    it.each([
        ...
        expect(() => prepareNames('id1', 'trigger', additionalFields)).toThrow();
    });
});
describe('pgTriggerFunction', () => {
    const createContext = (params) => {
        const getNodeParameter = vi.fn((name, _i, fallback) => name in params ? params[name] : fallback);
        return { getNodeParameter };
    };
    const invoke = (firesOn, additionalFields = { replaceIfExists: true }) => {
        const db = { any: vi.fn().mockResolvedValue([]) };
        const ctx = createContext({ schema: 'public', tableName: 'users', firesOn });
        const run = async () => await pgTriggerFunction.call(ctx, db, additionalFields, 'my_fn', 'my_trigger', 'my_channel');
        const sqls = () => db.any.mock.calls.map(([sql]) => sql);
        return { db, run, sqls };
    };
    ...
    it('produces templates pg-promise can format without error', async () => {
        ...
        for (const [sql, params] of db.any.mock.calls) {
            expect(() => pgp.as.format(sql, params)).not.toThrow();
        }
    });
    it('passes schema and table as separate identifier parameters', async () => {
        const { db, run } = invoke('INSERT');
        await run();
        const dropCall = db.any.mock.calls.find(([sql]) => sql.includes('DROP TRIGGER'));
...
describe('PostgresTrigger.trigger (Table Row Change Events mode)', () => {
    const setup = (additionalFields = {}, firesOn = 'INSERT') => {
        const connection = {
        ...
        vi.mocked(configurePostgres).mockResolvedValue({ db, pgp: {} });
        const params = {
        ...
        const fns = {
            getNodeParameter: vi.fn((name, _i, fallback) => name in params ? params[name] : fallback),
            getCredentials: vi.fn().mockResolvedValue({}),
            getNode: vi.fn().mockReturnValue({ id: 'node-1' }),
            getMode: vi.fn().mockReturnValue('trigger'),
            emit: vi.fn(),
            helpers: { returnJsonArray: vi.fn((d) => d) },
        };
    ...
    it('drops the created objects with matching identifiers on close', async () => {
        ...
        expect(connection.any).toHaveBeenCalledWith('DROP TRIGGER IF EXISTS $1:name ON $2:name.$3:name CASCADE', ['n8n_trigger_node_1', 'public', 'users']);
    });
...
//# sourceMappingURL=PostgresTrigger.test.js.map