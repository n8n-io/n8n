...
describe('Test PostgresV2, prepareErrorItem', () => {
    it('should return error info item', () => {
        ...
        expect(item.pairedItem.item).toEqual(1);
...
describe('Test PostgresV2, addWhereClauses', () => {
    it('should add where clauses to query', () => {
        ...
        const [updatedQuery, updatedValues] = addWhereClauses(node, 0, query, whereClauses, values, 'AND');
    ...
    it('should combine where clauses by OR', () => {
        ...
        const [updatedQuery, updatedValues] = addWhereClauses(node, 0, query, whereClauses, values, 'OR');
        expect(updatedQuery).toEqual('SELECT * FROM $1:name.$2:name WHERE $3:name = $4 OR $5:name = $6');
    ...
    it('should ignore incorrect combine condition ad use AND', () => {
        ...
        const [updatedQuery, updatedValues] = addWhereClauses(node, 0, query, whereClauses, values, 'SELECT * FROM my_table');
        expect(updatedQuery).toEqual('SELECT * FROM $1:name.$2:name WHERE $3:name = $4 AND $5:name = $6');
        expect(updatedValues).toEqual(['public', 'my_table', 'id', '1', 'foo', 'select 2']);
    });
...
describe('Test PostgresV2, addSortRules', () => {
    ...
    it('should add multiple sort rules', () => {
        ...
        const [updatedQuery, updatedValues] = addSortRules(query, sortRules, values);
        expect(updatedQuery).toEqual('SELECT * FROM $1:name.$2:name ORDER BY $3:name ASC, $4:name DESC');
...
describe('Test PostgresV2, addReturning', () => {
    it('should add RETURNING', () => {
        ...
        expect(updatedQuery).toEqual('UPDATE $1:name.$2:name SET $5:name = $6 WHERE $3:name = $4 RETURNING $7:name');
        expect(updatedValues).toEqual([
            'public',
            'my_table',
            'id',
    ...
    it('should add RETURNING *', () => {
        ...
        const outputColumns = ['id', 'foo', '*'];
        const [updatedQuery, updatedValues] = addReturning(query, outputColumns, values);
        expect(updatedQuery).toEqual('UPDATE $1:name.$2:name SET $5:name = $6 WHERE $3:name = $4 RETURNING *');
...
describe('Test PostgresV2, hasJsonDataType', () => {
    ...
    it('returns false if there are columns which are of type json', () => {
        const schema = [{ column_name: 'id', data_type: 'integer', is_nullable: 'NO' }];
...
describe('Test PostgresV2, runQueriesAndHandleErrors', () => {
    it.each([['single'], ['transaction']])('should return errors without running queries when batching is %s', async (batching) => {
        const runQueries = vi.fn().mockResolvedValue([]);
        const queries = [
            { query: 'INSERT INTO my_table (id) VALUES (1)', values: [] },
        ];
        const nodeOptions = { queryBatching: batching };
    ...
    it('should run queries and return errors when batching is independently', async () => {
        ...
        const nodeOptions = { queryBatching: 'independently' };
        const errorItemsMap = new Map();
        errorItemsMap.set(1, { json: { error: new Error('Test error') }, pairedItem: { item: 1 } });
        const result = await runQueriesAndHandleErrors(runQueries, queries, nodeOptions, errorItemsMap);
        expect(result).toEqual([
    ...
    it.each([['single'], ['transaction'], ['independently']])('should run queries when batching is %s and there are no errors', async (batching) => {
        const runQueries = vi.fn().mockResolvedValue([
            { json: { id: 1 }, pairedItem: { item: 0 } },
