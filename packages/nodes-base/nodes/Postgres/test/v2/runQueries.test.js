...
describe('Test PostgresV2, runQueries', () => {
    ...
    describe('empty result fallback (nodeVersion >= 2.3)', () => {
        ...
        const fakeConstructExecutionMetaData = (inputData, options) => inputData.map((item) => ({
            ...item,
            pairedItem: options.itemData,
        }));
        const runWithEmptyResult = async (query, queryBatching) => {
            const pgp = pgPromise();
            const db = createMockDb([]);
            const thisArg = mockDeep();
        ...
        describe.each(['single', 'transaction', 'independently'])('mode: %s', (mode) => {
            ...
            it('returns [] for a CTE (WITH ...) with no rows', async () => {
                const result = await runWithEmptyResult('WITH t AS (SELECT 1 AS x) SELECT * FROM t WHERE x = 999', mode);
                expect(result).toEqual([]);
            });
            it('returns [] for a SELECT preceded by a line comment', async () => {
                const result = await runWithEmptyResult('-- fetch some rows\nSELECT * FROM users WHERE id = 999', mode);
                expect(result).toEqual([]);
            });
            it('returns [] for a SELECT preceded by a multi-line block comment', async () => {
                const result = await runWithEmptyResult('/* fetch\n   some rows */\nSELECT * FROM users WHERE id = 999', mode);
                expect(result).toEqual([]);
            });
            it('returns [] for a SELECT with an inline trailing line comment', async () => {
                const result = await runWithEmptyResult('SELECT * FROM users WHERE id = 999 -- not found', mode);
                expect(result).toEqual([]);
            });
            ...
            it('returns {success:true} for a CTE-prefixed UPDATE with no rows', async () => {
                const result = await runWithEmptyResult("WITH payload AS (SELECT 1) UPDATE users SET name = 'x' WHERE id = 999", mode);
                expect(result).toHaveLength(1);
                expect(result[0].json).toEqual({ success: true });
            });
            it('returns {success:true} when a string literal hides a trailing UPDATE', async () => {
                const result = await runWithEmptyResult("SELECT '-- literal' WHERE false; UPDATE users SET name = 'x' WHERE id = 999", mode);
                expect(result).toHaveLength(1);
                expect(result[0].json).toEqual({ success: true });
            });
        });
...
//# sourceMappingURL=runQueries.test.js.map