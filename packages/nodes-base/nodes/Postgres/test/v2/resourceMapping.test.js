...
describe('Postgres, resourceMapping', () => {
    ...
    const createColumnData = (columnName, dataType, overrides = {}) => ({
    ...
    test.each([
    ...
    ])('$name', async ({ columnData, expectedType, expectedRequired, expectedDefaultMatch, isEnum, expectedOptions, }) => {
        ...
        if (isEnum && columnData.udt_name) {
            vi.mocked(getEnums).mockResolvedValueOnce(new Map([[columnData.udt_name, ['active', 'inactive']]]));
