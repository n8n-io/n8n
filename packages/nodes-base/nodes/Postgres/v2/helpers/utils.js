...
export function parsePostgresError(node, error, queries, itemIndex) {
...
export function addWhereClauses(_node, _itemIndex, query, clauses, replacements, combineConditions) {
    if (clauses.length === 0)
...
export function addReturning(query, outputColumns, replacements) {
...
export function configureQueryRunner(node, continueOnFail, pgp, db) {
...
export function prepareItem(values) {
...
export function hasJsonDataTypeInSchema(schema) {
    return schema.some(({ data_type }) => data_type === 'json');
}
export function convertValuesToJsonWithPgp(pgp, schema, values) {
    schema
...
export async function columnFeatureSupport(db) {
...
export async function getTableSchema(db, schema, table, options) {
...
export async function uniqueColumns(db, table, schema = 'public') {
...
export async function getEnums(db) {
...
export function getEnumValues(enumInfo, enumName) {
    const values = enumInfo.get(enumName) ?? [];
    return values.map((value) => ({ name: value, value }));
}
export async function doesRowExist(db, schema, table, values) {
    const where = [];
...
export function checkItemAgainstSchema(node, item, columnsInfo, index) {
...
export const configureTableSchemaUpdater = (initialSchema, initialTable) => {
 ...
 */
export const convertArraysToPostgresFormat = (data, schema, node, itemIndex = 0) => {
        ...
        if (columnInfo.data_type.toUpperCase() === 'ARRAY') {
            ...
            if (Array.isArray(columnValue)) {
                ...
                if (columnInfo.is_nullable === 'NO') {
                    throw new NodeOperationError(node, `Column '${columnInfo.column_name}' has to be an array`, {
                        itemIndex,
                    });
                }
            }
...
export const runQueriesAndHandleErrors = async (runQueries, queries, nodeOptions, errorItemsMap) => {
