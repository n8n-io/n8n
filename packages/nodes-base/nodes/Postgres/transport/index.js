...
export function createReceiveHandler(largeNumbersOutput) {
    return (e) => {
        if (largeNumbersOutput !== 'numbers')
            return;
        applyLargeNumbersReceive(e);
    };
}
...
export async function configurePostgres(credentials, options = {}) {
    ...
    const fallBackHandler = async (abortController) => {
        ...
        if (!credentials.sshTunnel) {
            ...
            proxy.on('connection', (localSocket) => {
                sshClient.forwardOut(LOCALHOST, localSocket.remotePort, credentials.host, credentials.port, (error, clientChannel) => {
                    if (error) {
