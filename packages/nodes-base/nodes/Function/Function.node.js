...
import { NodeConnectionTypes, deepCopy, NodeOperationError, CONSOLE_OUTPUT_REDACTED_MESSAGE, } from 'n8n-workflow';
    ...
    async execute() {
        ...
        const sandbox = {
            ...
            setBinaryDataAsync: async (item, data) => {
                ...
                if (!item) {
                    throw new NodeOperationError(this.getNode(), 'No item was provided to setBinaryDataAsync (item: INodeExecutionData, data: IBinaryKeyData).');
                }
                // Ensure data is provided, else return a friendly error.
                if (!data) {
                    throw new NodeOperationError(this.getNode(), 'No data was provided to setBinaryDataAsync (item: INodeExecutionData, data: IBinaryKeyData).');
                }
                // Set Binary Data
