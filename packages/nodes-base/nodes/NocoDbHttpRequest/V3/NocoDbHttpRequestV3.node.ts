...
export class NocoDbHttpRequestV3 implements INodeType {
		...
		this.description = {
			...
			properties: [
				...
				},
				{
					...
					description: 'The route for the request',
					required: true,
				},
				{
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			...
			const route = this.getNodeParameter('route', itemIndex) as string;

			if (!route) {
				throw new NodeOperationError(this.getNode(), 'No route configured!', { itemIndex });
			}
