...
import { CORRECTNESS_PROMPT, CORRECTNESS_INPUT_PROMPT, HELPFULNESS_PROMPT, HELPFULNESS_INPUT_PROMPT, } from '../Evaluation/CannedMetricPrompts.ee';
export const metricHandlers = {
    async customMetrics(i) {
        ...
        return Object.fromEntries((dataToSave?.assignments ?? []).map((assignment) => {
            ...
            if (isNaN(assignmentValue)) {
                throw new NodeOperationError(this.getNode(), `Value for '${assignment.name}' isn't a number`, {
            ...
            const { name, value } = validateEntry(assignment.name, assignment.type, assignmentValue, this.getNode(), i, false, 1);
            return [name, value];
        }));
    },
    async toolsUsed(i) {
        ...
        const normalizedExpectedTools = expectedTools.map((tool) => nodeNameToToolName(tool).toLowerCase());
    ...
    async helpfulness(i) {
        ...
        const systemPrompt = this.getNodeParameter('prompt', i, HELPFULNESS_PROMPT);
        const inputPromptTemplate = this.getNodeParameter('options.inputPrompt', i, HELPFULNESS_INPUT_PROMPT[0]);
        ...
        if (!llm.withStructuredOutput) {
            throw new NodeOperationError(this.getNode(), 'Language model does not support structured output', {
    ...
    async correctness(i) {
        ...
        // Get the system prompt and input prompt template, using defaults if not provided
        const systemPrompt = this.getNodeParameter('prompt', i, CORRECTNESS_PROMPT);
        const inputPromptTemplate = this.getNodeParameter('options.inputPrompt', i, CORRECTNESS_INPUT_PROMPT[0]);
        // Define the expected response schema
        const responseSchema = z.object({
            extended_reasoning: z
                .string()
