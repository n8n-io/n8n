import { UserError, NodeOperationError, EVALUATION_TRIGGER_NODE_TYPE, jsonStringify, } from 'n8n-workflow';
...
export function toDataTableValue(value) {
...
export async function setOutputs() {
    ...
    assert(isOutputsArray(outputFields), 'Invalid output fields format. Expected an array of objects with outputName and outputValue properties.');
...
export function setInputs() {
    ...
    assert(isInputsArray(inputFields), 'Invalid input fields format. Expected an array of objects with inputName and inputValue properties.');
...
export async function setMetrics() {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        const returnItem = composeReturnItem.call(this, i, newItem, newData, { dotNotation: false, include: 'none' }, 1);
        metrics.push(returnItem);
    }
    return [metrics];
}
export async function checkIfEvaluating() {
...
export function getOutputConnectionTypes(parameters) {
...
export function getInputConnectionTypes(parameters, metricRequiresModelConnectionFn) {
