...
import { getSelectFields as dataTableFilters, getSelectFilter as getDataTableFilter, } from '../../DataTable/common/selectMany';
...
import { getGoogleSheet, getResults, getRowsLeft, getNumberOfRowsLeftFiltered, getSheet, } from '../utils/evaluationTriggerUtils';
export const DEFAULT_STARTING_ROW = 2;
const MAX_ROWS = 1000;
export class EvaluationTrigger {
    description = {
    ...
    async execute() {
        ...
        if (source === 'dataTable') {
            ...
            if (this.helpers.getDataTableProxy === undefined) {
                throw new NodeOperationError(this.getNode(), 'Attempted to use Data table node but the module is disabled');
            }
            ...
            if (hasFilter.length > 0) {
                ...
                const rowsLeft = await getNumberOfRowsLeftFiltered.call(this, googleSheetInstance, googleSheet.title, currentRowNumber + 1, maxRows);
                currentRow.json._rowsLeft = rowsLeft;
                return [[currentRow]];
            }
            else {
                const currentRow = allRows.find((row) => row?.json?.row_number === firstDataRow);
                const rowsLeft = await getRowsLeft.call(this, googleSheetInstance, googleSheet.title, `${googleSheet.title}!${firstDataRow}:${maxRows}`);
                if (currentRow === undefined) {
                    throw new NodeOperationError(this.getNode(), 'No row found');
                }
                currentRow.json._rowsLeft = rowsLeft;
