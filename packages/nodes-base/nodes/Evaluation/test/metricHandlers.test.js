...
vi.mock('../../Set/v2/helpers/utils', () => ({
    validateEntry: vi.fn((name, _type, value) => ({
        name,
        value,
    })),
}));
describe('metricHandlers', () => {
    ...
    describe('customMetrics', () => {
        ...
        it('should throw error for non-numeric values', async () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockReturnValue(metricsData);
            await expect(metricHandlers.customMetrics.call(mockExecuteFunctions, 0)).rejects.toThrow(NodeOperationError);
        });
    ...
    describe('toolsUsed', () => {
        it('should return correct tool usage metrics', async () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'expectedTools')
                    return expectedTools;
                if (paramName === 'intermediateSteps')
                    return intermediateSteps;
                if (paramName === 'options.metricName')
                    return 'Tools Used';
                return undefined;
            });
        ...
        it('should throw error for missing expected tools', async () => {
            ...
            await expect(metricHandlers.toolsUsed.call(mockExecuteFunctions, 0)).rejects.toThrow(NodeOperationError);
        });
    ...
    describe('categorization', () => {
        it('should return 1 for exact match', async () => {
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'expectedAnswer')
                    return 'expected answer';
                if (paramName === 'actualAnswer')
                    return 'expected answer';
                if (paramName === 'options.metricName')
        ...
        it('should throw error for missing expected answer', async () => {
            ...
            await expect(metricHandlers.categorization.call(mockExecuteFunctions, 0)).rejects.toThrow(NodeOperationError);
        });
    ...
    describe('stringSimilarity', () => {
        ...
        it('should throw error for missing expected answer', async () => {
            ...
            await expect(metricHandlers.stringSimilarity.call(mockExecuteFunctions, 0)).rejects.toThrow(NodeOperationError);
        });
        it('should throw error for missing actual answer', async () => {
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'expectedAnswer')
    ...
    describe('helpfulness', () => {
        ...
        it('should return helpfulness score from LLM', async () => {
            ...
            const mockLLMWithStructuredOutput = mock();
            ...
            const mockChatPromptTemplate = mock();
        ...
        it('should throw error for missing user query', async () => {
            ...
            await expect(metricHandlers.helpfulness.call(mockExecuteFunctions, 0)).rejects.toThrow(NodeOperationError);
        });
        it('should throw error for missing actual answer', async () => {
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'userQuery')
        ...
        it('should handle LLM errors gracefully', async () => {
            ...
            const mockFinalChain = mock();
            mockFinalChain.invoke.mockRejectedValue(mockError);
            const mockMiddleChain = mock();
    ...
    describe('correctness', () => {
        ...
        it('should throw error for missing expected answer', async () => {
            ...
            await expect(metricHandlers.correctness.call(mockExecuteFunctions, 0)).rejects.toThrow(NodeOperationError);
        });
        it('should throw error for missing actual answer', async () => {
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'expectedAnswer')
...
//# sourceMappingURL=metricHandlers.test.js.map