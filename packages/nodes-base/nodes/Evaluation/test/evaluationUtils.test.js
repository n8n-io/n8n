...
describe('setInputs', () => {
    const mockThis = (options = {}) => mockDeep({
    ...
    it('should add execution hints and return input data if not started from evaluation trigger', () => {
        ...
        const result = setInputs.call(context);
        expect(context.addExecutionHints).toHaveBeenCalledWith(expect.objectContaining({
...
describe('setOutputs', () => {
    describe('common', () => {
        const mockThis = (options = {}) => mockDeep({
            ...
            evaluateExpression: vi.fn().mockImplementation((expr) => {
                if (expr.includes('isExecuted'))
                    return true;
                if (expr.includes('first().json'))
                    return { row_id: 1, inputField: 'inputValue' };
                return true;
            }),
            ...
            const context = mockThis({
                evaluateExpression: vi.fn().mockImplementation((expr) => {
                    if (expr.includes('isExecuted'))
                        return true;
                    if (expr.includes('first().json'))
                        return { row_id: 3, inputField: 'inputValue' };
                    return true;
                }),
            });
            await setOutputs.call(context);
            expect(mockDataTable.updateRows).toHaveBeenCalledWith(expect.objectContaining({
                filter: {
                    type: 'and',
                    filters: [
                        {
    ...
    describe('Google Sheets', () => {
        ...
        const mockThis = (options = {}) => mockDeep({
            ...
            evaluateExpression: vi.fn().mockImplementation((expr) => {
                if (expr.includes('isExecuted'))
                    return true;
                if (expr.includes('first().json'))
                    return { row_number: 2, inputField: 'inputValue' };
                return true;
            }),
            getNodeParameter: vi.fn().mockImplementation((param) => {
                if (param === 'outputs.values') {
        ...
        beforeEach(() => {
            vi.clearAllMocks();
            getGoogleSheet.mockReturnValue(mockGoogleSheetInstance);
            getSheet.mockResolvedValue(mockSheet);
        });
        it('should set outputs to Google Sheet and return evaluation data', async () => {
            ...
            expect(mockGoogleSheetInstance.updateRows).toHaveBeenCalledWith('Sheet1', [['inputField', 'result', 'score']], 'RAW', 1);
            expect(mockGoogleSheetInstance.prepareDataForUpdatingByRowNumber).toHaveBeenCalledWith([{ row_number: 2, result: 'success', score: '95' }], 'Sheet1!A:Z', [['inputField', 'result', 'score']]);
            expect(mockGoogleSheetInstance.batchUpdate).toHaveBeenCalledWith([{ range: 'Sheet1!A2:C2', values: [['foo', 'bar']] }], 'RAW');
            expect(result).toHaveLength(1);
        ...
        it('should handle row_number as string "row_number" by using 1', async () => {
            ...
            const result = await setOutputs.call(context);
            expect(mockGoogleSheetInstance.prepareDataForUpdatingByRowNumber).toHaveBeenCalledWith([{ row_number: 1, result: 'success', score: '95' }], 'Sheet1!A:Z', [['inputField', 'result', 'score']]);
            expect(result).toHaveLength(1);
        });
        it('should add new column names that are not in existing columns', async () => {
            const context = mockThis({
                evaluateExpression: vi.fn().mockImplementation((expr) => {
                    if (expr.includes('isExecuted'))
                        return true;
                    if (expr.includes('first().json'))
                        return { row_number: 2, existingCol: 'value' };
                    return true;
                }),
                getNodeParameter: vi.fn().mockImplementation((param) => {
                    if (param === 'outputs.values') {
            ...
            const result = await setOutputs.call(context);
            expect(mockGoogleSheetInstance.updateRows).toHaveBeenCalledWith('Sheet1', [['existingCol', 'newCol']], 'RAW', 1);
            expect(result).toHaveLength(1);
        });
    });
});
//# sourceMappingURL=evaluationUtils.test.js.map