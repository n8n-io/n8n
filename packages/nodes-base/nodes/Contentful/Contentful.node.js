            ...
            try {
                ...
                if (resource === 'entry') {
                    if (operation === 'get') {
                        ...
                        if (returnAll) {
                            responseData = await contentfulApiRequestAllItems.call(this, 'items', 'GET', `/spaces/${credentials?.spaceId}/environments/${env}/entries`, {}, qs);
                            if (!rawData) {
                                const assets = [];
                ...
                if (resource === 'asset') {
                    if (operation === 'get') {
                        ...
                        if (returnAll) {
                            responseData = await contentfulApiRequestAllItems.call(this, 'items', 'GET', `/spaces/${credentials?.spaceId}/environments/${env}/assets`, {}, qs);
                            if (!rawData) {
                                const assets = [];
                                responseData.map((asset) => {
                                    assets.push(asset.fields);
                ...
                if (resource === 'locale') {
                    if (operation === 'getAll') {
                        ...
                        const env = this.getNodeParameter('environmentId', i);
                        if (returnAll) {
                            responseData = await contentfulApiRequestAllItems.call(this, 'items', 'GET', `/spaces/${credentials?.spaceId}/environments/${env}/locales`, {}, qs);
                        }
                        else {
                            const limit = this.getNodeParameter('limit', 0);
                            qs.limit = limit;
