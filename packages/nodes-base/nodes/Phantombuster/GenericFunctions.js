...
export async function phantombusterApiRequest(method, path, body = {}, qs = {}, _option = {}) {
    const options = {
        headers: {},
        method,
        body,
...
export function validateJSON(self, json, name) {
    let result;
    try {
 ...
 */
export async function phantombusterStreamingRequest(options, onMessage) {
    let stream;
    try {
    ...
    try {
        for await (const line of stream) {
            ...
            if (message.type === 'error') {
                ...
                throw new NodeApiError(this.getNode(), { message: reason }, { message: reason, description: 'Phantombuster agent reported an error' });
            }
