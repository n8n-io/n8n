...
function buildWorkflow({ parameters, expectedResult, }) {
    const test = {
...
function mockExecuteFunctions(typeVersion, operation) {
    ...
    executeFunctions.getNode.mockReturnValue({ typeVersion });
    ...
    executeFunctions.helpers.constructExecutionMetaData.mockImplementation(constructExecutionMetaData);
    executeFunctions.getNodeParameter.mockImplementation((parameterName, itemIndex = 0, fallbackValue) => {
...
function mockQueryOperation(operation, options = {}) {
    const executeFunctions = mockExecuteFunctions(1.3, operation);
    executeFunctions.getInputData.mockReturnValue([inputItems[0]]);
    executeFunctions.getNodeParameter.mockImplementation((parameterName, _itemIndex = 0, fallbackValue) => {
        switch (parameterName) {
            case 'operation':
                return operation;
            case 'collection':
...
function mockFindCursor() {
    ...
    vi.spyOn(Collection.prototype, 'find').mockReturnValue(cursor);
    return applied;
}
function collectionNames(collectionSpy) {
    return collectionSpy.mock.calls.reduce((names, call) => {
        const [collectionName] = call;
...
function searchIndexOperationResult(indexName) {
...
describe('MongoDB CRUD Node', () => {
    ...
    describe('document operations in version 1.5', () => {
        ...
        function bulkWriteError(writeErrors, message = 'bulk write failed') {
            const error = Object.create(MongoBulkWriteError.prototype);
            Object.assign(error, { message, writeErrors });
            return error;
        }
        function mockBulkExecuteFunctions(operation, { continueOnFail = false, params = {}, } = {}) {
        ...
        it.each(['update', 'findOneAndUpdate'])('batches %s items into a single ordered bulkWrite per collection', async (operation) => {
        ...
        it('restores input order when grouping interleaves collections', async () => {
            ...
            const executeFunctions = mockBulkExecuteFunctions('update', {
                params: { collection: (itemIndex) => ['a', 'b', 'a'][itemIndex] },
            });
        ...
        it.each([true, 'true'])('sends upsert per operation when the parameter is truthy (%j)', async (upsert) => {
            const bulkWriteSpy = vi
                .spyOn(Collection.prototype, 'bulkWrite')
                .mockResolvedValue({});
        ...
        it('uses an unordered bulkWrite and maps write errors to items when continue-on-fail is on', async () => {
            ...
            const [items] = await node.execute.call(mockBulkExecuteFunctions('update', { continueOnFail: true }));
        ...
        ])('keeps per-item %s calls in version 1.4', async (operation, driverMethod) => {
            const driverSpy = vi.spyOn(Collection.prototype, driverMethod).mockResolvedValue({});
    ...
    describe('document operations in version 1.3', () => {
        ...
        describe('query parameters', () => {
            ...
            it('resolves sort parameters into the sort field name', async () => {
                ...
                await node.execute.call(mockQueryOperation('find', { sort: '{ "$1": -1 }', sortParameters: ['name'] }));
            ...
            it.each(['sort', 'projection'])('rejects a %s parameter that collides with a configured field', async (field) => {
                mockFindCursor();
                await expect(node.execute.call(mockQueryOperation('find', {
                    [field]: '{ "_id": 0, "$1": 1 }',
        ...
        it('uses a single insertMany when all items share the same collection', async () => {
            ...
            insertManySpy.mockResolvedValue({
                acknowledged: true,
                insertedCount: 3,
                insertedIds: Object.fromEntries([new ObjectId(), new ObjectId(), new ObjectId()].map((id, index) => [index, id])),
            });
            const sameCollectionMock = mockExecuteFunctions(1.3, 'insert');
            sameCollectionMock.getNodeParameter.mockImplementation((parameterName, _itemIndex = 0, fallbackValue) => {
                switch (parameterName) {
                    case 'operation':
                        return 'insert';
                    case 'collection':
        ...
        it('preserves input order when insert items span multiple collections', async () => {
            ...
            mock.getNodeParameter.mockImplementation((parameterName, itemIndex = 0, fallbackValue) => {
                switch (parameterName) {
                    case 'operation':
                        return 'insert';
                    case 'collection':
        ...
        describe.each(['findOneAndReplace', 'findOneAndUpdate', 'update'])('%s: non-scalar updateKey value', (operation) => {
            const itemsWithObjectKey = [
                { json: { id: { $regex: '^a' }, value: 'x', collection: 'col1' } },
            ];
            function mockObjectKey(continueOnFail) {
                ...
                mock.getNodeParameter.mockImplementation((parameterName, _itemIndex = 0, fallbackValue) => {
                    switch (parameterName) {
                        case 'operation':
                            return operation;
                        case 'collection':
            ...
            it('throws NodeOperationError when continueOnFail is off', async () => {
                await expect(node.execute.call(mockObjectKey(false))).rejects.toThrow(/must be a string, number, boolean, or date/);
            });
        ...
        describe.each(['findOneAndReplace', 'findOneAndUpdate', 'update'])('%s: item missing the updateKey field', (operation) => {
            const itemsMissingKey = [{ json: { value: 'no-id-field', collection: 'col1' } }];
            function mockMissingKey(continueOnFail) {
                const mock = mockExecuteFunctions(1.3, operation);
            ...
            it('throws NodeOperationError when continueOnFail is off', async () => {
                await expect(node.execute.call(mockMissingKey(false))).rejects.toThrow('Item is missing the updateKey field');
            });
            it('pushes error item with pairedItem when continueOnFail is on', async () => {
    ...
    describe('createSearchIndex operation', () => {
        ...
        beforeAll(() => {
            Collection.prototype.createSearchIndex = async function (...args) {
                calls.push(args);
    ...
    describe('listSearchIndexes operation', () => {
        describe('no index name provided', function () {
            ...
            beforeAll(() => {
                Collection.prototype.listSearchIndexes = function (...args) {
                    calls.push(args);
                    return { toArray: async () => await Promise.resolve([]) };
                };
            });
            afterAll(() => {
        ...
        describe('return values are transformed into the expected return type', function () {
            ...
            beforeAll(() => {
                Collection.prototype.listSearchIndexes = function () {
                    return {
                        toArray: async () => await Promise.resolve([{ name: searchIndexName }, { name: 'my-index-2' }]),
                    };
                };
            });
            afterAll(() => {
    ...
    describe('dropSearchIndex operation', () => {
        ...
        beforeAll(() => {
            Collection.prototype.dropSearchIndex = async function (...args) {
                calls.push(args);
                return undefined;
            };
        });
    ...
    describe('updateSearchIndex operation', () => {
        ...
        beforeAll(() => {
            Collection.prototype.updateSearchIndex = async function (...args) {
                calls.push(args);
                return undefined;
            };
        });
