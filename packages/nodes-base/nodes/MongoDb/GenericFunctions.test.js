...
import { buildParameterizedConnString, prepareItems, sanitizeMongoUriInMessage, serializeMongoItems, } from './GenericFunctions';
const mockNode = { name: 'MongoDB', type: 'n8n-nodes-base.mongoDb' };
    ...
    describe('sanitizeMongoUriInMessage', () => {
        describe('when replacing the supplied connection string', () => {
            it('redacts repeated occurrences', () => {
                ...
                expect(sanitizeMongoUriInMessage(message, connectionString)).toBe('tried mongodb://[REDACTED] then retried mongodb://[REDACTED]');
            });
            it('redacts the connection string when the regex does not match', () => {
                ...
                expect(sanitizeMongoUriInMessage(message, connectionString)).toBe('Invalid URL: mongodb://[REDACTED]');
            });
            it('is idempotent', () => {
                const connectionString = 'mongodb://user:password@host:27017/db';
                const message = `Invalid URL: ${connectionString}`;
        ...
        describe('when matching MongoDB URIs with the regex', () => {
            ...
            it('redacts multiple different URIs', () => {
                ...
                expect(sanitizeMongoUriInMessage(message, '')).toBe('tried mongodb://[REDACTED] then mongodb+srv://[REDACTED] both failed');
            });
            it('redacts repeated occurrences of the same URI', () => {
                const connectionString = 'mongodb://user:password@host:27017/db';
                const message = `tried ${connectionString} then retried ${connectionString}`;
                expect(sanitizeMongoUriInMessage(message, '')).toBe('tried mongodb://[REDACTED] then retried mongodb://[REDACTED]');
            });
            it.each([
