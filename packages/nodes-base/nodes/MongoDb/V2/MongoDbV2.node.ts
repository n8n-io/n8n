...
export class MongoDbV2 implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		try {
			...
			const fallbackPairedItems: IPairedItemData[] | null = generatePairedItemData(items.length);
			...
			for (let i = 0; i < items.length; i++) {
				...
				if (operation === 'insert') {
					try {
						...
						for (const insertedId of Object.keys(insertedIds)) {
							returnData.push({
								json: {
									...insertItems[parseInt(insertedId, 10)],
									id: insertedIds[parseInt(insertedId, 10)] as unknown as string,
								},
								pairedItem: { item: i },
							});
						}
