...
export class Digitaliso implements INodeType {
    ...
    async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
        ...
        if (resource === 'item') {
            ...
            let itms = this.getNodeParameter('items', 0) as string;
                ...
                try {
                    ...
                    if (!fullResponse) {
                        ...
                        responseFullData = { [itms]: responseData.body, 
                            meta: {
                                TotalRecord: parseInt(responseData.headers['x-pagination-total-count']),
                                TotalPages: parseInt(responseData.headers['x-pagination-page-count']),
                                CurrentPage: parseInt(responseData.headers['x-pagination-current-page'])
                            }
                        };
