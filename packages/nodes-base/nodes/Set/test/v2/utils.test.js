...
import { composeReturnItem, parseJsonParameter, validateEntry, resolveRawData, } from '../../v2/helpers/utils';
export const node = {
    id: '11',
    name: 'Edit Fields',
...
describe('test Set2, composeReturnItem', () => {
    ...
    it('should include binary when expected in version <3.4', () => {
        ...
        const resultWithIncludeBinary = composeReturnItem.call(fakeExecuteFunction, 0, inputItem, newData, {
            include: 'all',
            includeBinary: true,
        }, 3.3);
        expect(resultWithIncludeBinary.binary).toEqual(inputItem.binary);
        const resultWithoutIncludeBinary = composeReturnItem.call(fakeExecuteFunction, 0, inputItem, newData, {
            include: 'all',
        }, 3.3);
    ...
    it('should include binary when expected in version >=3.4', () => {
        ...
        const resultWithStripBinary = composeReturnItem.call(fakeExecuteFunction, 0, inputItem, newData, {
            include: 'all',
            stripBinary: true,
        }, 3.4);
        expect(resultWithStripBinary.binary).toBeUndefined();
        const resultWithoutStripBinary = composeReturnItem.call(fakeExecuteFunction, 0, inputItem, newData, {
            include: 'all',
        }, 3.4);
...
describe('test Set2, validateEntry', () => {
    it('should convert number to string', () => {
        const result = validateEntry('foo', 'string', 42, node, 0);
        expect(result).toEqual({
            name: 'foo',
            value: '42',
        });
    ...
    it('should convert array to string', () => {
        const result = validateEntry('foo', 'string', [1, 2, 3], node, 0);
        expect(result).toEqual({
            name: 'foo',
            value: '[1,2,3]',
        });
    ...
    it('should convert object to string', () => {
        const result = validateEntry('foo', 'string', { foo: 'bar' }, node, 0);
        expect(result).toEqual({
            name: 'foo',
            value: '{"foo":"bar"}',
        });
    ...
    it('should convert boolean to string', () => {
        const result = validateEntry('foo', 'string', true, node, 0);
        expect(result).toEqual({
            name: 'foo',
            value: 'true',
        });
    ...
    it('should convert undefined to string', () => {
        const result = validateEntry('foo', 'string', undefined, node, 0);
        expect(result).toEqual({
            name: 'foo',
            value: 'undefined',
        });
...
describe('test Set2, resolveRawData', () => {
    const createMockExecuteFunctionForResolve = (returnValue) => {
        return {
