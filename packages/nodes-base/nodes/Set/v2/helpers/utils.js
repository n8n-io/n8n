...
import { NodeOperationError, UserError, deepCopy, getValueDescription, jsonParse, validateFieldType, isBinaryValue, BINARY_MODE_COMBINED, } from 'n8n-workflow';
...
export function composeReturnItem(itemIndex, inputItem, newFields, options, nodeVersion) {
    const newItem = {
        json: {},
...
export const parseJsonParameter = (jsonData, node, i, entryName) => {
    let returnData;
    const location = entryName ? `entry "${entryName}" inside 'Fields to Set'` : "'JSON Output'";
    if (typeof jsonData === 'string') {
        try {
            returnData = jsonParse(jsonData, { repairJSON: true });
        }
        catch (error) {
    ...
    if (returnData === undefined || typeof returnData !== 'object' || Array.isArray(returnData)) {
        throw new NodeOperationError(node, `The ${location} in item ${i} does not contain a valid JSON object`);
    }
    return returnData;
};
export const validateEntry = (name, type, value, node, itemIndex, ignoreErrors = false, nodeVersion) => {
    ...
    if (type === 'string') {
        if (nodeVersion && nodeVersion > 3 && (value === undefined || value === null)) {
            if (ignoreErrors) {
                ...
                throw new NodeOperationError(node, `'${name}' expects a ${type} but we got ${getValueDescription(value)} [item ${itemIndex}]`, { description });
            }
        }
...
export function resolveRawData(rawData, i) {
...
export function prepareReturnItem(context, value, itemIndex, item, node, options) {
    const jsonValues = [];
    const binaryValues = [];
    ...
    const newData = Object.fromEntries(jsonValues.map((assignment) => {
        const { name, value } = validateEntry(assignment.name, assignment.type, assignment.value, node, itemIndex, options.ignoreConversionErrors, node.typeVersion);
        return [name, value];
    }));
    ...
    if (binaryValues.length) {
        ...
        for (const assignment of binaryValues) {
            ...
            if (!isBinaryValue(binaryData)) {
                throw new NodeOperationError(node, `Could not find binary data specified in field ${name}`, { itemIndex });
            }
