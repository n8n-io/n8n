...
describe('prepareReturnItem', () => {
    ...
    describe('when an assignment name is an inherited object member', () => {
        ...
        it('keeps built-in object prototypes intact', () => {
            ...
            expect(result.json.toString?.call).toBe('x');
        });
    });
    ...
    describe('binary assignments - mode handling', () => {
        describe('non-combined binary mode', () => {
            it('should push binary assignments to binaryValues when binaryMode is not combined', () => {
                ...
                mockContext.getWorkflowSettings.mockReturnValue({ binaryMode: 'default' });
                const value = {
                    assignments: [
                        {
                            id: '1',
            ...
            it('should handle binary assignments in separate mode', () => {
                ...
                mockContext.helpers.assertBinaryData.mockReturnValue(mockBinaryData);
                mockContext.getWorkflowSettings.mockReturnValue({ binaryMode: 'separate' });
                const value = {
                    assignments: [
                        {
                            id: '1',
        ...
        describe('combined binary mode', () => {
            ...
            it('should place binary in json when id is present', () => {
                const mockBinaryDataWithId = {
                    data: 'combineddata',
                    mimeType: 'image/jpeg',
                    fileName: 'photo.jpg',
                    id: 'binary-id-123',
                };
                mockContext.helpers.assertBinaryData.mockReturnValue(mockBinaryDataWithId);
                mockContext.getWorkflowSettings.mockReturnValue({
                    binaryMode: BINARY_MODE_COMBINED,
                });
                const value = {
            ...
            it('should treat empty string id as missing id (place only in binary)', () => {
                const mockBinaryDataEmptyId = {
                    id: '',
                    data: 'emptyiddata',
                    mimeType: 'text/plain',
                    fileName: 'file.txt',
                };
                mockContext.helpers.assertBinaryData.mockReturnValue(mockBinaryDataEmptyId);
                mockContext.getWorkflowSettings.mockReturnValue({
                    binaryMode: BINARY_MODE_COMBINED,
                });
                const value = {
    ...
    describe('binary assignments', () => {
        ...
        it('should create binary object when item has no existing binary data', () => {
            ...
            const result = prepareReturnItem(mockContext, value, 0, itemWithoutBinary, mockNode, baseOptions);
            expect(result.binary).toEqual({
                newBinary: mockBinaryData,
            });
        });
        it('should throw error when binary data is not found', () => {
            mockContext.helpers.assertBinaryData.mockReturnValue('not-binary-data');
            const value = {
                assignments: [
                    {
                        id: '1',
            ...
            expect(() => prepareReturnItem(mockContext, value, 0, baseItem, mockNode, baseOptions)).toThrow(NodeOperationError);
            expect(() => prepareReturnItem(mockContext, value, 0, baseItem, mockNode, baseOptions)).toThrow('Could not find binary data specified in field invalidBinary');
        });
    });
    ...
    describe('Edge cases', () => {
        ...
        it('should handle null assignments value', () => {
            const result = prepareReturnItem(mockContext, null, 0, baseItem, mockNode, baseOptions);
            expect(result.json).toEqual({
                existingField: 'existingValue',
            });
        });
...
describe('parseJsonParameter', () => {
    ...
    describe('Error cases', () => {
        it('should throw error for invalid JSON that cannot be recovered', () => {
            expect(() => parseJsonParameter('{key1: "value",, key2: "value2"}', mockNode, 0)).toThrow("The 'JSON Output' in item 0 contains invalid JSON");
        });
    });
});
