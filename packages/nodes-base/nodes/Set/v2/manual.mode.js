...
import { parseJsonParameter, validateEntry, composeReturnItem, resolveRawData, prepareReturnItem, } from './helpers/utils';
...
export async function execute(item, i, options, rawFieldsData, node) {
    try {
        if (node.typeVersion < 3.3) {
            ...
            for (const entry of fields) {
                if (entry.type === 'objectValue' &&
                    ...
                    entry.objectValue = parseJsonParameter(resolveRawData.call(this, rawFieldsData[entry.name], i), node, i, entry.name);
                }
                const { name, value } = validateEntry(entry.name, entry.type.replace('Value', ''), entry[entry.type], node, i, options.ignoreConversionErrors, node.typeVersion);
                newData[name] = value;
            }
