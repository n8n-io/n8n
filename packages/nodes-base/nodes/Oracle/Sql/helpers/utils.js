...
export function mapDbType(dbType) {
 ...
 */
export function getCompatibleValue(type, value) {
...
export function addSortRules(query, rules) {
    if (rules.length === 0)
        return query;
...
export async function getColumnMetaData(node, pool, schema, table, index = 0) {
    let conn;
    try {
...
WHERE atc.owner = :schema_name
  AND atc.table_name = :table_name
ORDER BY atc.COLUMN_NAME`;
        const result = await conn.execute(sql, { table_name: table, schema_name: schema }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
...
export function prepareErrorItem(error, index) {
    return {
    ...
    if (results.outBinds) {
        ...
        for (let j = 0; j < normalizedRows.length; j++) {
            ...
            if (serializeDates) {
                ...
                const routed = await routeBinaryProperties.call(this, row);
                row = routed.json;
...
export function configureQueryRunner(node, continueOnFail, pool) {
...
export const configureTableSchemaUpdater = (node, initialSchema, initialTable) => {
...
export function getColumnMap(tableSchema) {
...
export function addWhereClauses(query, clauses, combineConditions, schema, isExecuteMany = false) {
    if (clauses.length === 0)
        return [query, []];
...
*/
export function checkItemAgainstSchema(node, columns, schema, item, index = 0) {
...
function generateBindVariablesList(item, bindParameters, query) {
    if (item.datatype !== 'string') {
        throw new UserError(`Unsupported datatype '${item.datatype}' for IN clause expansion. Only 'string' of comma-separated values is allowed. `);
    }
...
export function getBindParameters(query, parameterList, options = {}) {
    const bindParameters = {};
        ...
        if (!bindItem.parseInStatement) {
            ...
            switch (type) {
                ...
                case 'blob':
                    ...
                    throw new UserError('BLOB data must be a valid Buffer or \'{ type: "Buffer", data: [...] }\'');
                case 'date': {
                ...
                case 'sparse': {
                    ...
                    if (typeof indices === 'string') {
                        try {
                            ...
                            throw new UserError(`Indices field must be a valid JSON array or use an expression like {{ [1, 2, 3] }}, Err:${caughtError}`);
                        }
                    }
                    if (typeof values === 'string') {
                        try {
                            ...
                            throw new UserError(`Values field must be a valid JSON array or use an expression like {{ [1, 2, 3] }}, Err:${caughtError} `);
                        }
                    }
                ...
                case 'vector':
                    {
                        ...
                        if (typeof val === 'string') {
                            try {
                                ...
                                throw new UserError(`Value field must be a valid JSON array or use an expression like {{ [1, 2, 3] }}, Err: ${caughtError}`);
                            }
                        }
                    }
                    break;
...
export function getInBindParametersForSourceSelect(inputColumns, schema, valData, bindParameters, bindDefs, posIndex = 0) {
...
export function getOnClauseFromColumns(columns) {
    return columns
        .map((col) => {
...
export function getUpdateSetClause(updateCols, schema, valData, bindParameters, bindDefs, posIndex) {
    const setClauses = [];
    let index = posIndex;
...
export function getInsertClauseAndBinds(insertCols, schema, valData, bindParameters, bindDefs, posIndex) {
    const quotedCols = [];
    const placeholders = [];
    let index = posIndex;
...
export function getInBindParametersForExecute(inputColumns, schema, valData, operation, bindParameters, posIndex = 0) {
    const quotedColsArray = [];
...
export function getOutBindDefsForExecute(query, columnMetaDataObject, outputColumns, bindInfo, bindIndex = 0) {
...
export function getBindDefsForExecuteMany(node, query, columnMetaDataObject, inputColumns, outputColumns, item, options, operation = 'insert', columnsToMatchOn = []) {
...
export function formatItemValues(item, col) {
    const result = [];
