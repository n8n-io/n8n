...
const createFakePool = (connection) => {
    return {
        getConnection() {
            return connection;
        },
    ...
    const fakeExecuteFunction = {
        getNodeParameter(parameterName, _itemIndex, fallbackValue, options) {
            if (parameterName === 'columns.value') {
                const columns = nodeParameters.columns;
        ...
        getNode() {
            node.parameters = { ...node.parameters, ...nodeParameters };
            return node;
        },
        getCredentials: () => {
            return CONFIG;
...
export function getRunQueriesFn(mockThis, pool) {
    if (integratedTests) {
        // Create the real query runner using the node context
        const realRunQueries = configureQueryRunner.call(mockThis, mockThis.getNode(), mockThis.continueOnFail(), pool);
...
VALUES (
  ...
  VECTOR('[0.5, 0.5, 0.5]'), UTL_RAW.CAST_TO_RAW('binary_data_here for BLOB3'), UTL_RAW.CAST_TO_RAW('binary_data_here for RAW3')
    ...
    async function populateRows(conn) {
        if (conn) {
    ...
    const getInputItems = (items) => items.map((v) => ({ json: v }));
    const patchMockParams = (items, nodePars, mappingMode = 'defineBelow') => {
        nodePars.columns.value = structuredClone(items);
        nodePars.columns.mappingMode = mappingMode;
        return createMockExecuteFunction(nodePars);
    };
    function verifyOutPutColumnsOptions(runQueries, nodeOptions, inputItems, result) {
        if (integratedTests &&
            Array.isArray(nodeOptions.outputColumns) &&
            (nodeOptions.outputColumns.includes('EMBEDDING') ||
                nodeOptions.outputColumns.includes('*'))) {
            ...
            if (nodeOptions.outputColumns.includes('*')) {
                expect(result.map(({ json }) => {
                    const { PICTURE, ...rest } = json; // skip comparing PICTURE column which is LOB
                    return {
                        ...rest,
                        EMBEDDING: new Float32Array(rest.EMBEDDING),
                        PROFILE: Buffer.from(rest.PROFILE),
                    };
                })).toEqual(inputItems.map(({ json }) => {
                ...
                for (let i = 0, j = 0; i < result.length;) {
                    ...
                    expect(actualEmbedding).toEqual(new Float32Array(expectedEmbedding));
    ...
    const runOperation = async ({ operation = insert, items, nodePars, nodeOptions, continueOnFailValue, mappingMode = 'defineBelow', failedIndex = 1, errorPattern, }) => {
        ...
        if (errorPattern && !continueOnFailValue && integratedTests) {
            await expect(operation.execute.call(mock, runQueries, inputItems, nodeOptions, pool)).rejects.toThrow(errorPattern);
            return;
        }
    ...
    beforeAll(async function () {
        if (integratedTests) {
            const credentials = await mockThisDef.getCredentials('oracleDBApi');
    ...
    describe('Test delete operation', () => {
        ...
        it.each(testCases)('should handle condition: %s', async ({ condition, value, expectedQuery, bindVal, executeManyValues, options = {
            ...
            const nodeOptions = nodeParameters.options;
    ...
    describe('Test execute operation', () => {
        ...
        it('should throw Oracle driver timeout errors when continueOnFail is false', async () => {
            ...
            const timeoutError = Object.assign(new Error('NJS-510: connection timed out during connect() call'), { errorNum: 510 });
            const timeoutPool = {
            ...
            const mockThis = createMockExecuteFunction(nodeParameters);
            const runQueries = configureQueryRunner.call(mockThis, mockThis.getNode(), mockThis.continueOnFail(), timeoutPool);
            const items = [{ json: {}, pairedItem: { item: 0, input: undefined } }];
            const nodeOptions = nodeParameters.options;
            await expect(executeSQL.execute.call(mockThis, runQueries, items, nodeOptions, timeoutPool)).rejects.toThrow(/NJS-510/i);
        ...
        it('should call runQueries with binds passed using bindInfo and single item', async () => {
            ...
            expect(runQueries).toHaveBeenCalledTimes(1);
            const callArgs = runQueries.mock.calls[0];
        ...
        it.each([
            {
                ...
                queryTemplate: (deptTable) => `SELECT * FROM ${deptTable} WHERE deptno = :Dno AND empname IN :Names`,
                bindValues: [
                ...
                expectedRegex: /^SELECT \* FROM N8N_TEST_DEPT WHERE deptno = :Dno AND empname IN \(:Names[\w_]+(,:Names[\w_]+)*\)$/,
            },
            {
                title: 'should call runQueries with binds passed using bindInfo with parseIn as true with _ in bindname',
                queryTemplate: (deptTable) => `SELECT * FROM ${deptTable} WHERE deptno = :Dno AND empname IN :Names_`,
                bindValues: [
                    { name: 'Dno', valueNumber: 10, datatype: 'number', parseInStatement: false },
                ...
                expectedRegex: /^SELECT \* FROM N8N_TEST_DEPT WHERE deptno = :Dno AND empname IN \(:Names_[\w_]+(,:Names_[\w_]+)*\)$/,
            },
            {
                title: 'should call runQueries with binds passed using bindInfo with parseIn as true with $ in bindname',
                queryTemplate: (deptTable) => `SELECT * FROM ${deptTable} WHERE deptno = :Dno AND empname IN :Names_ex2$`,
                bindValues: [
                    { name: 'Dno', valueNumber: 10, datatype: 'number', parseInStatement: false },
                    {
                        name: 'Names_ex2$',
                ...
                expectedRegex: /^SELECT \* FROM N8N_TEST_DEPT WHERE deptno = :Dno AND empname IN \(:Names_ex2\$[\w_]+(,:Names_ex2\$[\w_]+)*\)$/,
            },
            {
                title: 'should call runQueries with binds passed using partial matching bindInfo with parseIn as true',
                queryTemplate: (deptTable) => `SELECT * FROM ${deptTable} WHERE deptno = :Names1 AND empname IN :Names`,
                bindValues: [
                ...
                expectedRegex: /^SELECT \* FROM N8N_TEST_DEPT WHERE deptno = :Names1 AND empname IN \(:Names[\w_]+(,:Names[\w_]+)*\)$/,
            },
            {
                title: 'should correctly expand IN clause for single scalar bind value (SELECT :param1 ...)',
                ...
                expectedRegex: /^SELECT\s*\(?:param1[\w_]*\)?\s*FROM\s*DUAL\s*WHERE\s*DUMMY\s*IN\s*\(:param1[\w_]*\)$/,
            },
            {
        ...
        it('should call runQueries with out binds in plsql using bindInfo and multiple items', async () => {
            ...
            expect(runQueries).toHaveBeenCalledTimes(1);
            const [calls] = runQueries.mock.calls[0];
		...
		:ID, :NAME, :AGE, :SALARY, :JOIN_DATE, :IS_ACTIVE, :META_DATA, :EMBEDDING, :PICTURE)`;
            ...
            for (const input of inputs) {
                ...
                const runQueries = getRunQueriesFn(mockThis, pool);
                const nodeOptions = nodeParameters.options;
                const result = await executeSQL.execute.call(mockThis, runQueries, input.items, nodeOptions, pool);
                expect(runQueries).toHaveBeenCalled();
                const [expectedArgs] = runQueries.mock.calls.pop();
                expect(expectedArgs).toHaveLength(1);
                expect(expectedArgs[0].query).toBe(expectedQuery);
                ...
                if (integratedTests) {
                    ...
                    for (const [key, val] of Object.entries(row)) {
                        ...
                        if (original === null || original === undefined) {
                            ...
                            expect(val.toISOString()).toBe(DOJ);
                        }
                        else if (key === 'META_DATA') {
                            continue;
                        }
                        else if (key === 'EMBEDDING') {
                            expect(Array.from(val)).toEqual(original);
                        }
                        else {
    ...
    describe('Test select operation', () => {
        ...
        const getWhereClause = (condition1 = 'equal', val1 = 2, condition2 = 'equal', val2 = { department: 'Engineering', level: 3 }) => {
            const retObj = { values: [] };
            let inval1 = val1;
        ...
        it.each(testCases)('should handle condition: %s', async ({ whereClause, expectedQuery, bindVal, inOptions, expectedResultMetadata }) => {
            const nodeParameters = {
                operation: 'select',
                schema: {
                    __rl: true,
        ...
        it.each(limitSanitizationCases)('should sanitize limit value: $label', async ({ limit, expectedLimit }) => {
            const nodeParameters = {
                operation: 'select',
    ...
    describe('Test insert operation', () => {
        ...
        let originalColumnsValue;
        beforeEach(() => {
            originalColumnsValue = structuredClone(nodeParameters.columns?.value);
        });
        afterEach(() => {
            vi.clearAllMocks();
            continueOnFail = false;
            if (originalColumnsValue) {
                nodeParameters.columns.value = originalColumnsValue;
            }
            queriesIndependent = [
                {
                    query: expectedQuery,
        ...
        it('should insert valid data (defineBelow) with select all outputcolumns independently batch mode', async () => {
            ...
            const selectAlloutputsNodeParams = {
                ...nodeParameters,
        ...
        it('should insert valid data (defineBelow)', async () => {
            await runOperation({
                ...
                nodeOptions: nodeParameters.options,
            });
        });
    ...
    describe('Test upsert operation', () => {
        ...
        const expectedNonExistentBindValuesForIndependent = expectedBindValuesForIndependent.map((bindValues) => {
