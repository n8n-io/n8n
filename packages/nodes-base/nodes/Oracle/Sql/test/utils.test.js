...
import { addSortRules, configureQueryRunner, getBindParameters, getCompatibleValue, getOutBindDefsForExecute, } from '../helpers/utils';
...
describe('Test returning Clause', () => {
    it('should add RETURNING clause', () => {
        const query = 'INSERT INTO "VECTOR"."FRUITS" ("FRUIT_ID","PRICE_PER_KG","FRUIT_NAME") VALUES (:0,:1,:2)';
        const metaData = {
            COLOR: {
                type: 'VARCHAR2',
                nullable: true,
        ...
        const updatedQuery = getOutBindDefsForExecute(query, metaData, outputColumns, bindInfo, bindIndex);
...
describe('Test configureQueryRunner', () => {
    it('should return object out bind values from execute operations', async () => {
        ...
        expect(execute).toHaveBeenCalledWith('BEGIN demo(:ret); END;', {
            ret: {
                type: oracleDBTypes.STRING,
                dir: oracleDBTypes.BIND_OUT,
                maxSize: 4000,
    ...
    it('should return RETURNING date out binds as ISO strings on version 1.1', async () => {
        ...
        const node = { typeVersion: 1.1 };
        const queryRunner = configureQueryRunner.call(context, node, false, pool);
        const result = await queryRunner([{ query: 'INSERT ... RETURNING created INTO :created', values: {} }], [], { operation: 'execute', stmtBatching: 'independently' });
    ...
    it('should move RETURNING binary out binds to the binary output on version 1.1', async () => {
        ...
        const node = { typeVersion: 1.1 };
        const queryRunner = configureQueryRunner.call(context, node, false, pool);
        const result = await queryRunner([{ query: 'INSERT ... RETURNING doc INTO :doc', values: {} }], [], { operation: 'execute', stmtBatching: 'independently' });
    ...
    it('should append out-bind execution data one item at a time without spread push', async () => {
        ...
        const constructExecutionMetaData = vi.fn().mockImplementation((data) => {
            const item = data[0];
            if (item)
                expectedEntries[expectedEntries.length] = item;
        ...
        try {
            ...
            executionDataPushCalls = pushCalls.filter(([entry]) => entry && expectedEntries.includes(entry));
        }
        finally {
...
describe.skip('configureQueryRunner stack overflow regression', () => {
    it('should handle large out bind datasets without stack overflow', async () => {
        ...
        const constructExecutionMetaData = vi
            .fn()
            .mockImplementation((data) => Array.from({ length: chunkSize }, () => data[0]));
        const context = {
            helpers: {
                constructExecutionMetaData,
            },
        ...
        const result = await queryRunner(queries, [], {
            operation: 'insert',
            stmtBatching: 'single',
        });
