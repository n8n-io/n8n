...
describe('DocumentDB Node: Generic Functions', () => {
	...
	describe('sanitizeDocumentDatabaseUriInMessage', () => {
		describe('when replacing the supplied connection string', () => {
			it('redacts repeated occurrences', () => {
				...
				expect(sanitizeDocumentDatabaseUriInMessage(message, connectionString)).toBe(
			...
			it('is idempotent', () => {
				const connectionString = 'mongodb://user:password@host:27017/db';
				const message = `Invalid URL: ${connectionString}`;
				const sanitized = sanitizeDocumentDatabaseUriInMessage(message, connectionString);

				expect(sanitizeDocumentDatabaseUriInMessage(sanitized, connectionString)).toBe(sanitized);
			});
		});

		describe('when matching DocumentDB URIs with the regex', () => {
			it.each([
				[
				...
				expect(sanitizeDocumentDatabaseUriInMessage(message, '')).toBe(expected);
			});

			it('redacts multiple different URIs', () => {
				const message = 'tried mongodb://a:b@host1, then mongodb+srv://c:d@cluster, both failed';

				expect(sanitizeDocumentDatabaseUriInMessage(message, '')).toBe(
			...
			it.each([
				...
				expect(sanitizeDocumentDatabaseUriInMessage(message, '')).toBe(message);
			});
		});
	});
	...
	describe('serializeDocumentDatabaseItems', () => {
		...
		it('should stringify nested ObjectIds and Dates to JSON-safe values', async () => {
			...
			const result = await serializeDocumentDatabaseItems.call(thisArg, items);
