...
import { DocumentDb } from '../DocumentDb.node';
import type { MockInstance } from 'vitest';

const manualTriggerName = 'When clicking "Execute Workflow"';
const vectorIndexName = 'my-vector-index';
...
MongoClient.connect = async function () {
	...
	const client = new MongoClient('mongodb://localhost:10260', { driverInfo });
			...
			workflowData: {
				nodes: [
					...
					},
					{
						...
						name: 'documentDb',
						type: 'n8n-nodes-base.documentDb',
						typeVersion: 1.2,
						position: [1260, 360],
						credentials: {
							documentDb: {
...
function mockExecuteFunctions(typeVersion: number, operation: string) {
	...
	executeFunctions.getCredentials.mockResolvedValue({
		...
		connectionString: 'mongodb://localhost:10260',
		database: 'test',
	});
...
describe('DocumentDB CRUD Node', () => {
	...
	describe('document operations in version 1.3', () => {
		...
		it('groups insert items by collection and uses insertMany per group', async () => {
			...
			// Each item goes to a different collection ΓåÆ 3 groups ΓåÆ 3 insertMany calls
		...
		it('preserves input order when insert items span multiple collections', async () => {
			// items[0] ΓåÆ col1, items[1] ΓåÆ col2, items[2] ΓåÆ col1
			// groups: {col1: [0,2], col2: [1]} ΓÇö without sort output would be [0,2,1]
	...
	describe('createVectorIndex operation', () => {
		...
		beforeAll(() => {
			Db.prototype.command = async function (...args: unknown[]) {
				calls.push(args);
				return { ok: 1 };
			} as typeof Db.prototype.command;
		});
		afterAll(() => {
			Db.prototype.command = original;
		});
		...
		testHarness.setupTest(
			buildWorkflow({
				parameters: {
					...
					collection: 'foo',
					indexNameRequired: vectorIndexName,
				...
				expectedResult: [{ json: { indexName: vectorIndexName } }],
			}),
		);
	...
	describe('listVectorIndexes operation', () => {
		const original = Collection.prototype.listIndexes;
		beforeAll(() => {
			...
			} as typeof Collection.prototype.listIndexes;
		});
		afterAll(() => {
			Collection.prototype.listIndexes = original;
		});
	...
	describe('dropVectorIndex operation', () => {
		const calls: unknown[][] = [];
		const original = Collection.prototype.dropIndex;
		beforeAll(() => {
			Collection.prototype.dropIndex = async function (...args: unknown[]) {
				calls.push(args);
				return { ok: 1 } as never;
			} as typeof Collection.prototype.dropIndex;
		});
		afterAll(() => {
			Collection.prototype.dropIndex = original;
		});
		...
		testHarness.setupTest(
			buildWorkflow({
				...
				expectedResult: [{ json: { [vectorIndexName]: true } }],
			}),
		);

		it('calls the spy with the expected arguments', function () {
			expect(calls[0]).toEqual([vectorIndexName]);
		});
	});
});
