...
describe('AMQP Trigger Node', () => {
    ...
    it('should throw if no sink provided', async () => {
        await expect(testTriggerNode(AmqpTrigger, {
            mode: 'trigger',
    ...
    it('should reject in manual mode after 15s with no message', async () => {
        ...
        await expect(manualTriggerFunction?.()).rejects.toThrow('Aborted because no message was received within 15 seconds');
    ...
    it('should call saveFailedExecution when handleMessage throws an error in trigger mode', async () => {
        ...
        const triggerFunctions = mockDeep();
        ...
        triggerFunctions.getCredentials.mockResolvedValue({ hostname: 'localhost', port: 5672 });
        triggerFunctions.getMode.mockReturnValue('trigger');
        triggerFunctions.getNodeParameter.mockImplementation((param) => {
            if (param === 'sink')
                return 'queue://test';
            if (param === 'options')
                return { jsonParseBody: true };
            if (param === 'options.parallelProcessing')
                return true;
            if (param === 'options.jsonParseBody')
                return true;
            return undefined;
        });
    ...
    it('should handle duplicate messages correctly', async () => {
        ...
        await Promise.resolve(eventHandlers['message']({
    ...
    it('should release 1 credit per completed message when multiple are in flight', async () => {
        ...
        await Promise.all([1, 2, 3].map(async (id) => {
            await Promise.resolve(eventHandlers['message']({ message: { body: 'hello', message_id: id }, receiver }));
        }));
        vi.advanceTimersByTime(15);
        vi.useRealTimers();
        const totalCreditsGranted = addCreditSpy.mock.calls.reduce((sum, [credits]) => sum + credits, 0);
    ...
    it('should grant only the free slots when the receiver reopens after a reconnect', async () => {
        ...
        vi.useFakeTimers();
        await Promise.resolve(eventHandlers['message']({ message: { body: 'a', message_id: 1 }, receiver }));
        await Promise.resolve(eventHandlers['message']({ message: { body: 'b', message_id: 2 }, receiver }));
        eventHandlers['receiver_open']({ receiver });
    ...
    describe('when the broker detaches the receiver link', () => {
        ...
        let closeTrigger;
        const startTrigger = async (options = {}) => {
        ...
        const forceDetach = (context = detachForced) => {
            expect(eventHandlers['receiver_close'], 'no receiver_close handler registered').toBeTypeOf('function');
        ...
        it('should reopen the receiver with the original options', async () => {
            ...
            expect(mockOpenReceiver).toHaveBeenLastCalledWith(expect.objectContaining({
        ...
        it('should log the detach with the peer condition and description', async () => {
            ...
            forceDetach();
            expect(logger.info).toHaveBeenCalledWith(expect.stringContaining('detached'), expect.objectContaining({
                condition: 'amqp:link:detach-forced',
                description: 'Idle timeout: 00:10:00',
            }));
        });
...
//# sourceMappingURL=AmqpTrigger.node.test.js.map