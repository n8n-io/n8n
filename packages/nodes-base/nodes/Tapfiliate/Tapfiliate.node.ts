	...
	methods = {
		loadOptions: {
			...
			async getPrograms(
				...
				const programs = await tapfiliateApiRequestAllItems.call(
					this,
					'GET',
					'/programs/',
				);
						...
						try {
								...
								if (operation === 'create') {
									...
									if (body.addressUi) {
										...
										if ((body.address as IDataObject).country) {
											(body.address as IDataObject).country = { code: (body.address as IDataObject).country };
										}
									}

									if (body.companyName) {
										body.company = { name: body.companyName };
							...
							if (resource === 'programAffiliate') {
								if (operation === 'add') {
									...
									const affiliateId = this.getNodeParameter('affiliateId', i) as string;
									const additionalFields = this.getNodeParameter('additionalFields', i) as IDataObject;
									const body: IDataObject = { affiliate: { id: affiliateId } };
									Object.assign(body, additionalFields);
