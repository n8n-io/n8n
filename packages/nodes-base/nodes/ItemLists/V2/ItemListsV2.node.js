                ...
                for (let i = 0; i < length; i++) {
                    ...
                    for (const [entryIndex, fieldToSplitOut] of fieldsToSplitOut.entries()) {
                        ...
                        if (typeof arrayToSplit !== 'object' || arrayToSplit === null) {
                            if (nodeVersion < 2.2) {
                                throw new NodeOperationError(this.getNode(), `The provided field '${fieldToSplitOut}' is not an array or object`, { itemIndex: i });
                            }
                            else {
                ...
                if (type === 'simple') {
                    ...
                    newItems.sort((a, b) => {
                        ...
                        for (const field of sortFieldsWithDirection) {
                            let equal;
                            if (!disableDotNotation) {
                                const _a = typeof get(a.json, field.name) === 'string'
                                    ? get(a.json, field.name).toLowerCase()
                                    : get(a.json, field.name);
                                const _b = typeof get(b.json, field.name) === 'string'
                                    ? get(b.json, field.name).toLowerCase()
