...
describe('NocoDB List Search Methods', () => {
    ...
    describe('getWorkspaces', () => {
        it('should return a list of workspaces', async () => {
            mockThis.getCredentials.mockResolvedValue({ host: 'http://localhost:8080' });
    ...
    describe('getBases', () => {
        it('should return a list of bases for a given workspace', async () => {
            mockThis.getCredentials.mockResolvedValue({ host: 'http://localhost:8080' });
            mockThis.getNodeParameter.mockImplementation((name) => {
                if (name === 'workspaceId')
                    return 'ws1';
                return undefined;
            });
            ...
            expect(apiRequestSpy).toHaveBeenCalledWith('GET', '/api/v3/meta/workspaces/ws1/bases', {}, {});
            expect(result).toEqual({
                results: [
    ...
    describe('getRelatedTableFields', () => {
        ...
        it('should return related table fields successfully (version 4)', async () => {
            ...
            expect(apiRequestSpy).toHaveBeenCalledWith('GET', '/api/v3/meta/bases/base1/tables/table1/fields/linkField1', {}, {});
            expect(apiRequestSpy).toHaveBeenCalledWith('GET', '/api/v3/meta/bases/base1/tables/relatedTable1', {}, {});
            expect(result).toEqual({
                results: [
        ...
        it('should throw NodeOperationError on first API error', async () => {
            ...
            await expect(getRelatedTableFields.call(mockThis)).rejects.toThrow('Error while fetching fields: First API Error');
        });
        it('should throw NodeOperationError on second API error', async () => {
            ...
            await expect(getRelatedTableFields.call(mockThis)).rejects.toThrow('Error while fetching fields: Second API Error');
        });
    });
});
