...
describe('NocoDB Linkrows Link Node', () => {
    ...
    beforeEach(() => {
        ...
        mockThis = {
            ...
            getNodeParameter: (name, _index, defaultValue, options) => {
                if (name === 'projectId') {
                    return 'base1';
                }
                if (name === 'table') {
            ...
            getNode: () => ({ name: 'NocoDB', type: 'nocoDb', id: '1', parameters: {} }),
        };
    });
    ...
    it('should successfully link a single row', async () => {
        ...
        expect(mockApiRequest.call).toHaveBeenCalledWith(mockThis, 'POST', '/api/v3/data/base1/table1/links/linkField1/row1', [{ id: 'linkedRow1' }], {});
        expect(result).toEqual([[{ success: true }]]);
    });
    // Test Case 2: Successful linking of multiple rows
    it('should successfully link multiple rows', async () => {
        mockThis.getNodeParameter = (name, _index, defaultValue, options) => {
            if (name === 'projectId')
                return 'base1';
            if (name === 'table')
                return 'table1';
        ...
        mockApiRequest.call.mockResolvedValueOnce({ success: true });
        const result = await execute.call(mockThis);
        expect(mockApiRequest.call).toHaveBeenCalledWith(mockThis, 'POST', '/api/v3/data/base1/table1/links/linkField1/row1', [{ id: 'linkedRow1' }, { id: 'linkedRow2' }], {});
        expect(result).toEqual([[{ success: true }]]);
    });
    ...
    it('should correctly process linkId as an array of objects from expression', async () => {
        mockThis.getNodeParameter = (name, _index, defaultValue, _options) => {
            if (name === 'projectId')
                return 'base1';
            if (name === 'table')
                return 'table1';
        ...
        mockApiRequest.call.mockResolvedValueOnce({ success: true });
        const result = await execute.call(mockThis);
        expect(mockApiRequest.call).toHaveBeenCalledWith(mockThis, 'POST', '/api/v3/data/base1/table1/links/linkField1/row1', [{ id: 'exprRow1' }, { id: 'exprRow2' }], {});
        expect(result).toEqual([[{ success: true }]]);
    });
    ...
    it('should correctly process linkId as an array of strings/numbers from expression', async () => {
        ...
        mockApiRequest.call.mockResolvedValueOnce({ success: true });
        const result = await execute.call(mockThis);
        expect(mockApiRequest.call).toHaveBeenCalledWith(mockThis, 'POST', '/api/v3/data/base1/table1/links/linkField1/row1', [{ id: 'exprRowA' }, { id: 123 }], {});
        expect(result).toEqual([[{ success: true }]]);
    });
});
