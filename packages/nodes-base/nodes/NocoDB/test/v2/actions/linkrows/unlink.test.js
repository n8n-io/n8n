...
describe('NocoDB Linkrows Unlink Node', () => {
    ...
    it('should successfully link a single row', async () => {
        ...
        expect(mockApiRequest.call).toHaveBeenCalledWith(mockThis, 'DELETE', '/api/v3/data/base1/table1/links/linkField1/row1', [{ id: 'linkedRow1' }], {});
    ...
    it('should successfully link multiple rows', async () => {
        ...
        mockApiRequest.call.mockResolvedValueOnce({ success: true });
        const result = await execute.call(mockThis);
        expect(mockApiRequest.call).toHaveBeenCalledWith(mockThis, 'DELETE', '/api/v3/data/base1/table1/links/linkField1/row1', [{ id: 'linkedRow1' }, { id: 'linkedRow2' }], {});
        expect(result).toEqual([[{ success: true }]]);
    });
    ...
    it('should correctly process linkId as an array of objects from expression', async () => {
        ...
        mockApiRequest.call.mockResolvedValueOnce({ success: true });
        const result = await execute.call(mockThis);
        expect(mockApiRequest.call).toHaveBeenCalledWith(mockThis, 'DELETE', '/api/v3/data/base1/table1/links/linkField1/row1', [{ id: 'exprRow1' }, { id: 'exprRow2' }], {});
        expect(result).toEqual([[{ success: true }]]);
    });
    ...
    it('should correctly process linkId as an array of strings/numbers from expression', async () => {
        ...
        mockApiRequest.call.mockResolvedValueOnce({ success: true });
        const result = await execute.call(mockThis);
        expect(mockApiRequest.call).toHaveBeenCalledWith(mockThis, 'DELETE', '/api/v3/data/base1/table1/links/linkField1/row1', [{ id: 'exprRowA' }, { id: 123 }], {});
        expect(result).toEqual([[{ success: true }]]);
    });
});
