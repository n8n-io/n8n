...
describe('NocoDB Rows Get Action', () => {
    ...
    beforeEach(() => {
        ...
        downloadRecordAttachments.call.mockClear();
    });
    describe('execute', () => {
        it('should return data for a single row without attachments', async () => {
            mockExecuteFunctions.getNodeParameter.mockImplementation((name) => {
                ...
                if (name === 'id')
                    return 'row1';
                if (name === 'downloadAttachments')
                    return false;
                return undefined;
            });
            apiRequest.call.mockResolvedValue({ id: 'row1', name: 'Test Row' });
            const result = await execute.call(mockExecuteFunctions);
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'GET', '/api/v3/data/base1/table1/records/row1', {}, {});
        ...
        it('should return data for a single row with attachments', async () => {
            mockExecuteFunctions.getNodeParameter.mockImplementation((name) => {
                ...
                if (name === 'downloadAttachments')
                    return true;
                if (name === 'downloadFieldNames')
                    return ['attachmentField'];
                return undefined;
            });
            apiRequest.call.mockResolvedValue({
                id: 'row1',
                name: 'Test Row',
                attachmentField: 'url',
            });
            downloadRecordAttachments.call.mockResolvedValue([
                { binary: { attachmentField: { data: 'binaryData' } } },
            ]);
            const result = await execute.call(mockExecuteFunctions);
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'GET', '/api/v3/data/base1/table1/records/row1', {}, {});
            expect(downloadRecordAttachments.call).toHaveBeenCalledWith(mockExecuteFunctions, [{ id: 'row1', name: 'Test Row', attachmentField: 'url' }], ['attachmentField'], [{ item: 0 }]);
        ...
        it('should handle errors and continue on fail', async () => {
            ...
            apiRequest.call.mockRejectedValue(new Error('API Error'));
