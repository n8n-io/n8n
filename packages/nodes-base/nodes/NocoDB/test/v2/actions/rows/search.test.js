...
describe('NocoDB Rows Search Action', () => {
    ...
    describe('v4', () => {
        it('should make a basic search request with where and fields', async () => {
            ...
            mockExecuteFunctions.getNodeParameter = vi.fn().mockImplementation((name) => {
                if (name === 'version')
                    return 4;
                if (name === 'projectId')
                    return 'base1';
            ...
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'GET', '/api/v3/data/base1/table1/records', {}, {
                where: '(name,eq,test)',
                fields: 'fieldA,fieldB',
                limit: 50,
            });
    ...
    describe('returnAll', () => {
        it('should make a search request with returnAll set to true', async () => {
            ...
            apiRequestAllItems.call.mockResolvedValue([
                { id: 1, name: 'test1' },
                { id: 2, name: 'test2' },
            ]);
            ...
            expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('returnAll', 0);
            expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('options', 0, {});
            expect(apiRequestAllItems.call).toHaveBeenCalledWith(mockExecuteFunctions, 'GET', '/api/v3/data/base1/table1/records', {}, {});
            expect(result).toEqual([expectedResponse]);
        });
    });
});
