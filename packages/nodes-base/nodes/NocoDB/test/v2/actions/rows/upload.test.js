...
describe('NocoDB Rows Upload Action', () => {
    ...
    describe('base64 upload mode', () => {
        it('should upload a file successfully in base64 mode', async () => {
            ...
            // Mock API response
            apiRequest.call.mockResolvedValueOnce(mockReturnValue);
            const result = await execute.call(mockExecuteFunctions);
            // Assertions
            expect(mockExecuteFunctions.getInputData).toHaveBeenCalled();
            expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('projectId', 0, undefined, { extractValue: true });
            ...
            expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('uploadFieldName', 0, undefined, { extractValue: true });
            expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('uploadMode', 0);
            expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('contentType', 0, undefined, { extractValue: true });
            expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('filename', 0, undefined, {
                extractValue: true,
            });
            expect(mockExecuteFunctions.getNodeParameter).toHaveBeenCalledWith('base64value', 0, undefined, { extractValue: true });
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'POST', '/api/v3/data/testProjectId/testTable/records/testRowId/fields/testFieldName/upload', {
    ...
    describe('url upload mode', () => {
        it('should upload a file successfully in url mode when existing field is empty', async () => {
            ...
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'GET', '/api/v3/data/testProjectId/testTable/records/testRowId', {}, {});
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'PATCH', '/api/v3/data/testProjectId/testTable/records', [
                {
                    id: 'testRowId',
                    fields: {
        ...
        it('should upload a file successfully in url mode when existing field is an array', async () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((name) => {
                ...
                if (name === 'uploadMode')
                    return 'url';
                if (name === 'url')
                    return 'http://example.com/new-file.png';
                return undefined;
            });
            // Mock API responses
            apiRequest.call
    ...
    describe('error handling', () => {
        it('should return an error object when API request fails and continueOnFail is true', async () => {
            ...
            apiRequest.call.mockRejectedValueOnce(apiError);
            const result = await execute.call(mockExecuteFunctions);
            // Assertions
