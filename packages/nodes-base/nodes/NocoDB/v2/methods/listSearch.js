...
export async function getWorkspaces(filter) {
    try {
        const baseUrl = await getBaseUrl.call(this);
        const constructUrl = (workspaceId) => {
        ...
        const results = responseData.list.map((i) => ({
            name: i.title,
            value: i.id,
            url: constructUrl(i.id),
        }));
        return {
            results: filter && filter !== '' ? results.filter((flt) => flt.name.includes(filter)) : results,
        };
    }
    catch (e) {
...
export async function getBases(filter) {
    ...
    try {
        ...
        const responseData = await apiRequest.call(this, requestMethod, endpoint, {}, {});
        let results = responseData.list.map((i) => ({
            name: i.title,
            value: i.id,
            url: constructUrl(i.id),
        }));
        ...
        throw new NodeOperationError(this.getNode(), new Error('Error while fetching bases!', {
            cause: e,
        }), {
            level: 'warning',
        });
...
export async function getViews(filter) {
    const baseId = this.getNodeParameter('projectId', 0, {
        extractValue: true,
    });
    ...
    if (tableId) {
        try {
            ...
            const responseData = await apiRequest.call(this, requestMethod, endpoint, {}, {});
            const results = responseData.views.map((i) => {
                return {
                    name: i.title,
                    value: i.id,
                    url: constructUrl(i.id),
            ...
            return {
                results: filter && filter !== ''
                    ? results.filter((flt) => flt.name.includes(filter))
                    : results,
            };
        }
        catch (e) {
...
export async function getFields(filter) {
    ...
    if (tableId) {
        try {
            ...
            const endpoint = `/api/v3/meta/bases/${baseId}/tables/${tableId}`;
            const responseData = await apiRequest.call(this, requestMethod, endpoint, {}, {});
            const results = responseData.fields.map((i) => {
                return {
                    name: i.title,
                    value: i.id,
                };
...
export async function getTriggerFields(filter) {
    const baseId = this.getNodeParameter('projectId', 0, {
        extractValue: true,
    });
    const tableId = this.getNodeParameter('table', 0, {
...
export async function getDownloadFields(filter) {
    const { results } = await getDownloadFieldsId.call(this, filter);
    return {
        results: results.map((field) => ({ name: field.name, value: field.name })),
    };
}
export async function getDownloadFieldsId(filter) {
    try {
        const fetcher = new ColumnsFetcher(this);
        const fields = await fetcher.fetchFromDefinedParam();
        return {
            results: fields
                .filter((field) => field.type === 'Attachment' && (!filter || field.title.includes(filter)))
                .map((field) => {
                return {
                    name: field.title,
                    value: field.id,
...
export async function getLinkFieldsId(filter) {
    try {
        const fetcher = new ColumnsFetcher(this);
        const fields = await fetcher.fetchFromDefinedParam();
        return {
            results: fields
                .filter((field) => ['Link', 'LinkToAnotherRecord'].includes(field.type) &&
...
export async function getRelatedTableFields() {
    ...
    if (linkFieldName) {
        try {
            ...
            return {
                results: responseData.fields.map((i) => {
                    return {
                        name: i.title,
                        value: i.id,
                    };
