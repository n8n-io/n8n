    ...
    options: [
        {
            ...
            description: 'The view to operate on. Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/">expression</a>.',
            typeOptions: {
    ...
    for (let i = 0; i < items.length; i++) {
        try {
            ...
            if (qs.sort) {
                ...
                qs.sort = JSON.stringify(properties.map((prop) => {
                    return {
            ...
            if (qs.viewId && qs.viewId.value) {
                qs.viewId = qs.viewId.value;
            }
            if (qs.shuffle) {
                qs.shuffle = 1;
            }
            ...
            if (downloadAttachments) {
                ...
                const response = await downloadRecordAttachments.call(this, responseData, downloadFieldNames, [{ item: i }]);
