...
describe('Splunk, user resource', () => {
    ...
    test('create operation', async () => {
        ...
        expect(transport.splunkApiRequest).toHaveBeenCalledWith('POST', '/services/authentication/users', { name: 'John Doe', password: 'password', roles: ['role1', 'role2'] });
    ...
    test('deleteUser operation', async () => {
        ...
        expect(transport.splunkApiRequest).toHaveBeenCalledWith('DELETE', '/services/authentication/users/12345');
    ...
    test('get operation', async () => {
        ...
        expect(transport.splunkApiJsonRequest).toHaveBeenCalledWith('GET', '/services/authentication/users/12345');
    ...
    test('getAll operation', async () => {
        ...
        expect(transport.splunkApiJsonRequest).toHaveBeenCalledWith('GET', '/services/authentication/users', {}, { count: 0 });
        expect(responseData).toEqual([{ test: 'test' }]);
    });
    test('update operation', async () => {
        ...
        executeFunctions.getNodeParameter.calledWith('userId', 0).mockReturnValue('12345');
        transport.splunkApiRequest.mockReturnValue(Promise.resolve({
            feed: {
                entry: [
                    {
                        id: '1',
        ...
        expect(transport.splunkApiRequest).toHaveBeenCalledWith('POST', '/services/authentication/users/12345', { email: 'testW@example.com', roles: ['role1', 'role2'] });
        expect(responseData).toEqual([
            {
                id: '1',
                test: 'test1',
