...
describe('Splunk, report resource', () => {
    ...
    test('create operation', async () => {
        ...
        transport.splunkApiJsonRequest.mockReturnValue([
            {
                id: '12345',
                cronSchedule: '*/5 * * * *',
        ...
        transport.splunkApiRequest.mockReturnValue(Promise.resolve(response));
        const responseData = await report.create.execute.call(executeFunctions, 0);
        expect(transport.splunkApiJsonRequest).toHaveBeenCalledWith('GET', '/services/search/jobs/12345');
    ...
    test('deleteReport operation', async () => {
        ...
        transport.splunkApiRequest.mockReturnValue({});
        const responseData = await report.deleteReport.execute.call(executeFunctions, 0);
        expect(transport.splunkApiRequest).toHaveBeenCalledWith('DELETE', '/services/saved/searches/12345');
        expect(responseData).toEqual({ success: true });
    });
    test('get operation', async () => {
        const executeFunctions = mock();
        executeFunctions.getNodeParameter.calledWith('reportId', 0).mockReturnValue('12345');
        transport.splunkApiJsonRequest.mockReturnValue([{ test: 'test' }]);
        const responseData = await report.get.execute.call(executeFunctions, 0);
        expect(transport.splunkApiJsonRequest).toHaveBeenCalledWith('GET', '/services/saved/searches/12345');
    ...
    test('getAll operation', async () => {
        ...
        expect(transport.splunkApiJsonRequest).toHaveBeenCalledWith('GET', '/services/saved/searches', {}, { count: 0 });
        expect(responseData).toEqual([{ test: 'test' }]);
    });
});
