...
describe('Splunk, search resource', () => {
    ...
    test('create operation', async () => {
        ...
        transport.splunkApiRequest.mockReturnValue({ response: { sid: '12345' } });
    ...
    test('deleteJob operation', async () => {
        ...
        expect(transport.splunkApiRequest).toHaveBeenCalledWith('DELETE', '/services/search/jobs/12345');
    ...
    test('getAll operation', async () => {
        ...
        expect(transport.splunkApiJsonRequest).toHaveBeenCalledWith('GET', '/services/search/jobs', {}, { count: 0 });
        expect(responseData).toEqual([{ test: 'test' }]);
    });
    test('getResult operation', async () => {
        ...
        expect(transport.splunkApiJsonRequest).toHaveBeenCalledWith('GET', '/services/search/jobs/12345/results', {}, { count: 10, search: 'search key1=test1' });
        expect(responseData).toEqual([{ test: 'test' }]);
    });
});
