...
import { appWebhookSubscriptionCreate, appWebhookSubscriptionDelete, appWebhookSubscriptionList, } from './GenericFunctions';
export const filterStatuses = (events, allowedStatuses) => {
    if (!allowedStatuses)
        return events;
        ...
        default: {
            async checkExists() {
                ...
                if (subscription.callback_url !== webhookUrl) {
                    throw new NodeOperationError(this.getNode(), `The WhatsApp App ID ${appId} already has a webhook subscription. Delete it or use another App before executing the trigger. Due to WhatsApp API limitations, you can have just one trigger per App.`, { level: 'warning' });
                }
