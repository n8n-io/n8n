...
import { getSendAndWaitConfig, getSendAndWaitProperties, SEND_AND_WAIT_WAITING_TOOLTIP, sendAndWaitWebhook, } from '../../utils/sendAndWait/utils';
...
export class WhatsApp {
    ...
    customOperations = {
        message: {
            async [SEND_AND_WAIT_OPERATION]() {
                const phoneNumberId = this.getNodeParameter('phoneNumberId', 0);
                const recipientPhoneNumber = sanitizePhoneNumber(this.getNodeParameter('recipientPhoneNumber', 0));
                const config = getSendAndWaitConfig(this);
                const instanceId = this.getInstanceId();
                try {
                    await this.helpers.httpRequestWithAuthentication.call(this, WHATSAPP_CREDENTIALS_TYPE, createMessage(config, phoneNumberId, recipientPhoneNumber, instanceId));
                }
                catch (error) {
