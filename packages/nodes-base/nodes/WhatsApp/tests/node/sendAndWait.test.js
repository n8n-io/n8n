...
describe('Test WhatsApp Business Cloud, sendAndWait operation', () => {
    ...
    it('should send message and put execution to wait', async () => {
        ...
        mockExecuteFunctions.getNodeParameter.mockImplementation((key) => {
            ...
            if (key === 'channelId')
                return 'channelID';
            if (key === 'options.limitWaitTime.values')
                return {};
        });
        ...
        expect(mockExecuteFunctions.helpers.httpRequestWithAuthentication).toHaveBeenCalledWith('whatsAppApi', {
    ...
    it('should route API errors to error output when continueOnFail is true', async () => {
        ...
        mockExecuteFunctions.helpers.httpRequestWithAuthentication.mockRejectedValueOnce(new Error('invalid_recipient'));
        const result = await whatsApp.customOperations.message.sendAndWait.call(mockExecuteFunctions);
    ...
    it('should throw NodeOperationError when continueOnFail is false', async () => {
        ...
        mockExecuteFunctions.helpers.httpRequestWithAuthentication.mockRejectedValueOnce(new Error('invalid_recipient'));
        await expect(whatsApp.customOperations.message.sendAndWait.call(mockExecuteFunctions)).rejects.toThrow('invalid_recipient');
        expect(mockExecuteFunctions.putExecutionToWait).not.toHaveBeenCalled();
    });
});
