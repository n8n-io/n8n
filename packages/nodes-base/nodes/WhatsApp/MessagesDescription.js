...
import { cleanPhoneNumber, componentsRequest, mediaUploadFromItem, sendErrorPostReceive, setType, templateInfo, } from './MessageFunctions';
export const mediaTypes = ['image', 'video', 'audio', 'sticker', 'document'];
const currencies = countryCodes.data.map(({ code, currency }) => ({
    name: `${code} - ${currency}`,
    value: code,
}));
...
export const messageTypeFields = [
    // ----------------------------------
    //         type: contact
    // ----------------------------------
    {
