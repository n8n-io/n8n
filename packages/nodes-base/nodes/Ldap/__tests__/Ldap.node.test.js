...
describe('Ldap', () => {
    ...
    describe('search', () => {
        ...
        beforeEach(() => {
            ...
            vi.spyOn(Helpers, 'createLdapClient').mockResolvedValue(mockClient);
        ...
        function mockParameters(overrides = {}) {
        ...
        it('should escape a wildcard (*) in searchText resolved from an expression', async () => {
            ...
            await new Ldap().execute.call(executeFunctions);
            expect(mockSearch).toHaveBeenCalledWith('dc=example,dc=com', expect.objectContaining({
        ...
        describe('custom filter', () => {
            ...
            function mockCustomFilter(customFilter, resolvedValue) {
                mockParameters({ searchFor: 'custom', customFilter });
                executeFunctions.evaluateExpression.mockImplementation((expr) => {
                    if (expr === '{{ $json.body.username }}')
            ...
            it('should pass a static custom filter through unescaped', async () => {
                ...
                await new Ldap().execute.call(executeFunctions);
                expect(mockSearch).toHaveBeenCalledWith('dc=example,dc=com', expect.objectContaining({ filter: '(objectclass=*)' }));
            });
            it('should keep converting hand-escaped special characters in a static custom filter', async () => {
                mockCustomFilter('(cn=john\\*doe)');
                await new Ldap().execute.call(executeFunctions);
                expect(mockSearch).toHaveBeenCalledWith('dc=example,dc=com', expect.objectContaining({ filter: '(cn=john\\2adoe)' }));
            });
        ...
        describe('raw parameter reads', () => {
            ...
            it('should read the custom filter before it is interpolated', async () => {
                ...
                await new Ldap().execute.call(executeFunctions);
                expect(executeFunctions.getNodeParameter).toHaveBeenCalledWith('customFilter', 0, undefined, { rawExpressions: true });
            });
        });
    });
    describe('rename', () => {
...
//# sourceMappingURL=Ldap.node.test.js.map