...
export class SplitOut {
    ...
    async execute() {
        ...
        for (let i = 0; i < items.length; i++) {
            const fieldsToSplitOut = prepareFieldsArray(this.getNodeParameter('fieldToSplitOut', i), 'Fields To Split Out').map((field) => String(field).replace(/^\$json\./, ''));
            ...
            if (destinationFields.length && destinationFields.length !== fieldsToSplitOut.length) {
                throw new NodeOperationError(this.getNode(), 'If multiple fields to split out are given, the same number of destination fields must be given');
            }
            ...
            for (const [entryIndex, fieldToSplitOut] of fieldsToSplitOut.entries()) {
                ...
                if (fieldToSplitOut === '$binary') {
                    ...
                    if (!disableDotNotation) {
                        entityToSplit = get(item, fieldToSplitOut);
                    }
                    else {
            ...
            for (const splitEntry of splited) {
                ...
                if (include === 'selectedOtherFields') {
                    const fieldsToInclude = prepareFieldsArray(this.getNodeParameter('fieldsToInclude', i, ''), 'Fields To Include');
