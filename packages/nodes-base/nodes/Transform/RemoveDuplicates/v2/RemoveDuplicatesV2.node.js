...
export class RemoveDuplicatesV2 {
    description;
                ...
                case 'removeItemsSeenInPreviousExecutions': {
                    ...
                    if (logic === 'removeItemsWithAlreadySeenKeyValues') {
                        if (!['node', 'workflow'].includes(scope)) {
                            throw new NodeOperationError(this.getNode(), `The scope '${scope}' is not supported. Please select either "node" or "workflow".`);
                        }
                        let checkValue;
                        const itemMapping = {};
                        ...
                        if (currentProcessedDataCount + items.length > maxEntriesNum) {
                            throw new NodeOperationError(this.getNode(), 'The number of items to be processed exceeds the maximum history size. Please increase the history size or reduce the number of items to be processed.');
                        }
                        ...
                        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
                            ...
                            if (!incrementalKey?.toString()) {
                                throw new NodeOperationError(this.getNode(), 'The `Value to Dedupe` On is empty. Please provide a value.');
                            }
                            parsedIncrementalKey = Number(incrementalKey);
                            if (isNaN(parsedIncrementalKey)) {
                                throw new NodeOperationError(this.getNode(), `The value '${incrementalKey}' is not a number. Please provide a number.`);
                            }
                        ...
                        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
                            ...
                            try {
                                ...
                                throw new NodeOperationError(this.getNode(), `The value '${checkValue}' is not a valid date. Please provide a valid date.`);
                            }
                            if (itemMapping[checkValue]) {
                                itemMapping[checkValue].push(items[itemIndex]);
                            }
