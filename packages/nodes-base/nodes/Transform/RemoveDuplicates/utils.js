...
export const validateInputData = (node, items, keysToCompare, disableDotNotation) => {
...
export function removeDuplicateInputItems(context, items) {
    ...
    if (compare === 'allFieldsExcept') {
        const fieldsToExclude = prepareFieldsArray(context.getNodeParameter('fieldsToExclude', 0, ''), 'Fields To Exclude');
        if (!fieldsToExclude.length) {
            throw new NodeOperationError(context.getNode(), 'No fields specified. Please add a field to exclude from comparison');
        }
    ...
    if (compare === 'selectedFields') {
        const fieldsToCompare = prepareFieldsArray(context.getNodeParameter('fieldsToCompare', 0, ''), 'Fields To Compare');
        if (!fieldsToCompare.length) {
            throw new NodeOperationError(context.getNode(), 'No fields specified. Please add a field to compare on');
        }
        if (!disableDotNotation) {
            keys = Object.keys(flattenKeys(items[0].json));
        }
