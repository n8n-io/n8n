...
export class WooCommerceTrigger {
    description = {
                ...
                for (const webhook of webhooks) {
                    if (webhook.status === 'active' &&
                        ...
                        if (!webhookData.secret) {
                            ...
                            try {
                                await woocommerceApiRequest.call(this, 'DELETE', `/webhooks/${webhook.id}`, {}, { force: true });
                            }
                            catch (error) {
    ...
    async webhook() {
        ...
        if (!secret) {
            throw new NodeOperationError(this.getNode(), 'WooCommerce webhook secret is missing', {
                description: 'The stored webhook secret could not be found. Deactivate and re-activate the workflow so n8n can re-register the webhook with WooCommerce.',
            });
        }
        const isValid = verifySignature({
            getExpectedSignature: () => createHmac('sha256', secret).update(req.rawBody).digest('base64'),
            getActualSignature: () => headerData['x-wc-webhook-signature'] ?? null,
        });
        if (!isValid) {
