	...
	methods = {
		loadOptions: {
			...
			async getTags(
				...
				const returnData: INodePropertyOptions[] = [];
				const tags = await woocommerceApiRequestAllItems.call(
					this,
					'GET',
					'/products/tags',
					{},
					...
					for (let i = 0; i < length; i++) {
						if (resource === 'customer') {
							...
							if (operation === 'create') {
								...
								if (additionalFields.categories) {
									body.categories = ((additionalFields.categories as string[]).map(
										category => ({ id: parseInt(category, 10) }),
									) as unknown) as IDataObject[];
								}

								const images = (this.getNodeParameter(
									'imagesUi',
									i,
								) as IDataObject).imagesValues as IImage[];
								if (images) {
									body.images = images;
								}
								const dimension = (this.getNodeParameter(
									'dimensionsUi',
									i,
								) as IDataObject).dimensionsValues as IDimension;
								if (dimension) {
								...
								const metadata = (this.getNodeParameter(
									'metadataUi',
									i,
								) as IDataObject).metadataValues as IDataObject[];
								if (metadata) {
						...
						if (resource === 'order') {
							...
							if (operation === 'create') {
								...
								const billing = (this.getNodeParameter(
									'billingUi',
									i,
								) as IDataObject).billingValues as IAddress;
								if (billing !== undefined) {
									body.billing = billing;
									toSnakeCase(billing as IDataObject);
								}
								const shipping = (this.getNodeParameter(
									'shippingUi',
									i,
								) as IDataObject).shippingValues as IAddress;
								if (shipping !== undefined) {
									body.shipping = shipping;
									toSnakeCase(shipping as IDataObject);
								}
								const couponLines = (this.getNodeParameter(
									'couponLinesUi',
									i,
								) as IDataObject).couponLinesValues as ICouponLine[];
								if (couponLines) {
									body.coupon_lines = couponLines;
									setMetadata(couponLines);
									toSnakeCase(couponLines);
								}
								const feeLines = (this.getNodeParameter(
									'feeLinesUi',
									i,
								) as IDataObject).feeLinesValues as IFeeLine[];
								if (feeLines) {
								...
								const lineItems = (this.getNodeParameter(
									'lineItemsUi',
									i,
								) as IDataObject).lineItemsValues as ILineItem[];
								if (lineItems) {
								...
								const shippingLines = (this.getNodeParameter(
									'shippingLinesUi',
									i,
								) as IDataObject).shippingLinesValues as IShoppingLine[];
								if (shippingLines) {
							...
							if (operation === 'update') {
								...
								if (updateFields.customerId) {
									body.customer_id = parseInt(
										updateFields.customerId as string,
										10,
									);
								}
								...
								if (updateFields.parentId) {
									body.parent_id = parseInt(
										updateFields.parentId as string,
										10,
									);
								}
