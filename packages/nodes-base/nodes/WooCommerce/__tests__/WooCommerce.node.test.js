...
const woocommerceApiRequestMock = GenericFunctions.woocommerceApiRequest;
/** Builds a mocked IExecuteFunctions for the given resource/operation and per-item params. */
function createMockExecuteFunctions(resource, operation, items) {
    return {
        getInputData: vi.fn().mockReturnValue(items.map((json) => ({ json }))),
        getNodeParameter: vi
            .fn()
            .mockImplementation((name, itemIndex, fallback) => {
            if (name === 'resource')
                return resource;
            if (name === 'operation')
                return operation;
        ...
        helpers: {
            returnJsonArray: vi
                .fn()
                .mockImplementation((data) => (Array.isArray(data) ? data : [data]).map((json) => ({ json }))),
...
describe('WooCommerce Node', () => {
    ...
    describe('Order: Get operation', () => {
        it('returns exactly one item per input item when each Order ID is valid', async () => {
            woocommerceApiRequestMock.mockImplementation(async (_method, endpoint) => ({
        ...
        it('throws when the Order ID is empty instead of falling back to the list endpoint', async () => {
            ...
            woocommerceApiRequestMock.mockResolvedValue(Array.from({ length: 20 }, (_v, i) => ({ id: i + 1 })));
            const mockThis = createMockExecuteFunctions('order', 'get', [{ orderId: '' }]);
            await expect(new WooCommerce().execute.call(mockThis)).rejects.toBeInstanceOf(NodeOperationError);
    ...
    describe.each([
        ...
        it.each(['get', 'update', 'delete'])('throws on an empty ID for the %s operation', async (operation) => {
    ...
    describe('Order: Get All operation', () => {
        it('does not leak query parameters between input items', async () => {
            ...
            await new WooCommerce().execute.call(mockThis);
            const secondCallQs = woocommerceApiRequestMock.mock.calls[1][3];
            expect(secondCallQs.status).toBeUndefined();
            expect(secondCallQs).toEqual({ per_page: 50 });
        });
    });
});
//# sourceMappingURL=WooCommerce.node.test.js.map