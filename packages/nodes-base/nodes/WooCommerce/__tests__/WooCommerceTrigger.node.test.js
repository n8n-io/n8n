...
describe('WooCommerceTrigger Node', () => {
    describe('webhook method', () => {
        ...
        it('throws NodeOperationError when stored secret is missing', async () => {
            ...
            await expect(trigger.webhook.call(mockThis)).rejects.toThrow('WooCommerce webhook secret is missing');
        });
    ...
    describe('checkExists method', () => {
        ...
        it('returns true and stores webhookId when matching webhook exists and secret is stored', async () => {
            ...
            expect(woocommerceApiRequestMock).toHaveBeenCalledWith('GET', '/webhooks', {}, { status: 'active', per_page: 100 });
        });
        it('deletes the orphaned webhook and returns false when secret is missing from static data', async () => {
            ...
            expect(woocommerceApiRequestMock).toHaveBeenNthCalledWith(2, 'DELETE', '/webhooks/99', {}, { force: true });
        ...
        it('returns false without throwing when deleting the orphaned webhook fails', async () => {
            ...
            expect(result).toBe(false);
            expect(webhookData.webhookId).toBeUndefined();
            expect(logger.warn).toHaveBeenCalledWith('Failed to delete orphaned webhook during checkExists', { webhookId: 99, error: deleteError });
        });
...
//# sourceMappingURL=WooCommerceTrigger.node.test.js.map