...
export function throwOnErrorStatus(responseData) {
    ...
    if (errorRecord?.status === 'error') {
        ...
        throw new NodeOperationError(this.getNode(), errorRecord, {
            message: errorRecord.message,
            description: JSON.stringify(errorRecord, null, 2),
        });
    }
}
export async function zohoApiRequest(method, endpoint, body = {}, qs = {}, uri) {
    ...
    try {
        ...
        const args = error.cause?.data
            ...
            : undefined;
        throw new NodeApiError(this.getNode(), error, args);
    }
}
/**
 * Make an authenticated API request to Zoho CRM API and return all items.
 */
export async function zohoApiRequestAllItems(method, endpoint, body = {}, qs = {}) {
    const returnData = [];
    let responseData;
    qs.per_page = 200;
    qs.page = 1;
...
export function throwOnMissingProducts(resource, productDetails) {
    if (!productDetails.length) {
        throw new NodeOperationError(this.getNode(), `Please enter at least one product for the ${resource}.`);
    }
}
// ----------------------------------------
 ...
 */
export const adjustProductDetails = (productDetails, operation) => {
 ...
 */
export const adjustProductDetailsOnUpdate = (allFields) => {
...
//           payload adjusters
// ----------------------------------------
export const adjustAccountPayload = flow(adjustBillingAddressFields, adjustShippingAddressFields, adjustCustomFields);
export const adjustContactPayload = flow(adjustMailingAddressFields, adjustOtherAddressFields, adjustDateOfBirthField, adjustCustomFields);
export const adjustDealPayload = flow(adjustClosingDateField, adjustCustomFields);
export const adjustInvoicePayload = flow(adjustBillingAddressFields, adjustShippingAddressFields, adjustInvoiceDateField, adjustDueDateField, adjustAccountIdField, adjustCustomFields);
export const adjustInvoicePayloadOnUpdate = flow(adjustInvoicePayload, adjustProductDetailsOnUpdate);
export const adjustLeadPayload = flow(adjustAddressFields, adjustCustomFields);
export const adjustPurchaseOrderPayload = flow(adjustBillingAddressFields, adjustShippingAddressFields, adjustDueDateField, adjustPurchaseOrderDateField, adjustCustomFields);
export const adjustQuotePayload = flow(adjustBillingAddressFields, adjustShippingAddressFields, adjustValidTillField, adjustCustomFields);
export const adjustSalesOrderPayload = flow(adjustBillingAddressFields, adjustShippingAddressFields, adjustDueDateField, adjustAccountIdField, adjustContactIdField, adjustDealIdField, adjustCustomFields);
 ...
 */
export const toLoadOptions = (items, nameProperty) => items.map((item) => ({ name: item[nameProperty], value: item.id }));
export function getModuleName(resource) {
    const map = {
        account: 'Accounts',
        contact: 'Contacts',
        deal: 'Deals',
 ...
 */
export async function getFields(resource, { onlyCustom } = { onlyCustom: false }) {
...
export const capitalizeInitial = (str) => str[0].toUpperCase() + str.slice(1);
...
export async function getPicklistOptions(resource, targetField) {
    const qs = { module: getModuleName(resource) };
 ...
 */
export const addGetAllFilterOptions = (qs, options) => {
