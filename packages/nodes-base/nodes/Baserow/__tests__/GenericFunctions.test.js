...
import { baserowApiRequest, baserowApiRequestAllItems, formatBaserowFilterValue, getFieldNamesAndIds, normalizeFilterFieldId, toOptions, TableFieldMapper, } from '../GenericFunctions';
describe('Baserow > GenericFunctions', () => {
    ...
    describe('baserowApiRequest', () => {
        it('should return data on success', async () => {
            ...
            const result = await baserowApiRequest.call(mockExecuteFunctions, 'GET', '/endpoint', 'baserowApi');
            expect(result).toEqual({ success: true });
            expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('baserowApi', expect.objectContaining({
                method: 'GET',
        ...
        it('should throw NodeApiError on failure', async () => {
            ...
            await expect(baserowApiRequest.call(mockExecuteFunctions, 'GET', '/endpoint', 'baserowApi')).rejects.toThrow(NodeApiError);
        });
    });
    describe('baserowApiRequestAllItems', () => {
        it('should accumulate all pages', async () => {
            ...
            const result = await baserowApiRequestAllItems.call(mockExecuteFunctions, 'GET', '/endpoint', 'baserowApi', {}, {});
    ...
    describe('formatBaserowFilterValue', () => {
        it('should format plain ISO date for date_is_after', () => {
            expect(formatBaserowFilterValue('date_is_after', '2026-06-17')).toBe('UTC?2026-06-17?exact_date');
        });
        it('should use custom timezone for plain ISO date', () => {
            expect(formatBaserowFilterValue('date_is_before', '2026-06-17', 'Europe/Berlin')).toBe('Europe/Berlin?2026-06-17?exact_date');
        });
        ...
        it('should fix malformed UTC??YYYY-MM-DD values', () => {
            expect(formatBaserowFilterValue('date_is_after', 'UTC??2026-06-17')).toBe('UTC?2026-06-17?exact_date');
        });
        ...
        it('should format deprecated date_within_days as timezone?number', () => {
            expect(formatBaserowFilterValue('date_within_days', '1', 'Asia/Calcutta')).toBe('Asia/Calcutta?1');
        });
