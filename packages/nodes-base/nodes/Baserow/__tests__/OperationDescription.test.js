...
const firstFilter = (values, returnDefaults = true) => {
    const resolved = getNodeParameters(properties, values, returnDefaults, false, null, null);
    return (resolved?.additionalOptions).filters
        .fields[0];
};
describe('Baserow filter description', () => {
    ...
    it('hides Timezone for a non-date operator', () => {
        expect(firstFilter(withFilter({ operator: 'equal', value: 'abc' }))).not.toHaveProperty('timezone');
    });
...
//# sourceMappingURL=OperationDescription.test.js.map