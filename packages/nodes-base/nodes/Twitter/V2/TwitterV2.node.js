...
import { returnId, returnIdFromUsername, twitterApiRequest, twitterApiRequestAllItems, } from './GenericFunctions';
import { listFields, listOperations } from './ListDescription';
import { tweetFields, tweetOperations } from './TweetDescription';
import { userFields, userOperations } from './UserDescription';
export class TwitterV2 {
    description;
            ...
            try {
                        ...
                        if (me) {
                            ...
                            if (userRlc.mode === 'username') {
                                userRlc.value = userRlc.value.includes('@')
                                    ...
                                    : userRlc.value;
                                responseData = await twitterApiRequest.call(this, 'GET', `/users/by/username/${userRlc.value}`, {});
                            }
                            else if (userRlc.mode === 'id') {
                                responseData = await twitterApiRequest.call(this, 'GET', `/users/${userRlc.value}`, {});
                            }
                        }
                    }
                }
                if (resource === 'tweet') {
                    if (operation === 'search') {
                        ...
                        if (returnAll) {
                            responseData = await twitterApiRequestAllItems.call(this, 'data', 'GET', '/tweets/search/recent', {}, qs);
                        }
                        else {
                            const limit = this.getNodeParameter('limit', i);
                            qs.max_results = limit;
                            responseData = await twitterApiRequest.call(this, 'GET', '/tweets/search/recent', {}, qs);
                        }
                    }
                    if (operation === 'create') {
                    ...
                    if (operation === 'like') {
                        ...
                        responseData = await twitterApiRequest.call(this, 'POST', `/users/${user.id}/likes`, body);
                    }
                    if (operation === 'retweet') {
                        ...
                        const user = (await twitterApiRequest.call(this, 'GET', '/users/me', {}));
                        responseData = await twitterApiRequest.call(this, 'POST', `/users/${user.id}/retweets`, body);
                    }
                }
                if (resource === 'list') {
                    if (operation === 'add') {
                ...
                if (resource === 'directMessage') {
                    if (operation === 'create') {
                        ...
                        responseData = await twitterApiRequest.call(this, 'POST', `/dm_conversations/with/${user}/messages`, body);
                    }
                }
