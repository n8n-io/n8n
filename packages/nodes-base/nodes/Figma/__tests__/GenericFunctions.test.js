...
describe('Figma > GenericFunctions', () => {
    ...
    describe('with access token authentication', () => {
        it('should send X-FIGMA-TOKEN header and use the figmaApi credential', async () => {
            ...
            expect(mockFunctions.helpers.request).toHaveBeenCalledWith(expect.objectContaining({
                method: 'GET',
        ...
        it('should throw NodeApiError on failure', async () => {
            ...
            await expect(figmaApiRequest.call(mockFunctions, 'GET', '/v2/teams/123/webhooks')).rejects.toThrow(NodeApiError);
        });
    });
    describe('with OAuth2 authentication', () => {
        ...
        it('should route through requestWithAuthentication using the figmaOAuth2Api credential', async () => {
            ...
            expect(result).toEqual({ ok: true });
            expect(mockFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('figmaOAuth2Api', expect.objectContaining({
                method: 'POST',
