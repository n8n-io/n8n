...
export async function twakeApiRequest(
	...
	try {
		...
		if (((error as JsonObject).error as JsonObject)?.code === 'ECONNREFUSED') {
			throw new NodeApiError(this.getNode(), error as JsonObject, { message: 'Twake host is not accessible!' });
		}
