...
import { appWebhookSubscriptionCreate, appWebhookSubscriptionDelete, appWebhookSubscriptionList, facebookEntityDetail, installAppOnPage, } from './GenericFunctions';
import { listSearch } from './methods';
export class FacebookLeadAdsTrigger {
    description = {
        ...
        default: {
            async checkExists() {
                ...
                if (subscription.callback_url !== webhookUrl) {
                    throw new NodeOperationError(this.getNode(), `The Facebook App ID ${appId} already has a webhook subscription. Delete it or use another App before executing the trigger. Due to Facebook API limitations, you can have just one trigger per App.`, { level: 'warning' });
                }
                return true;
            },
            async create() {
    ...
    async webhook() {
        ...
        const events = await Promise.all(bodyData.entry
            ...
            const [lead, form] = await Promise.all([
                facebookEntityDetail.call(this, event.leadgen_id, 'field_data,created_time,ad_id,ad_name,adset_id,adset_name,form_id'),
                facebookEntityDetail.call(this, event.form_id, 'id,name,locale,status,page,questions'),
            ]);
