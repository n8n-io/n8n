...
const lookupOperation = contactOperations[0].options.find((option) => option.value === 'lookup');
const findLookupField = (name) => {
    const field = contactFields.find((candidate) => candidate.name === name &&
        (candidate.displayOptions?.show?.operation ?? []).includes('lookup'));
    if (!field) {
...
//# sourceMappingURL=ContactDescription.test.js.map