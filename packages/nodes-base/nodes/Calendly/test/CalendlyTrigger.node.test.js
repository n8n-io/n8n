...
describe('CalendlyTrigger', () => {
    ...
    describe('webhookMethods.default.create', () => {
        it('should create a user-scoped webhook subscription', async () => {
            ...
            expect(requestWithAuthentication).toHaveBeenLastCalledWith('calendlyApi', expect.objectContaining({
                method: 'POST',
        ...
        it('should create an organization-scoped webhook subscription', async () => {
            mockHookFunctions.getNodeParameter.mockImplementation((name) => {
                if (name === 'authentication')
                    return 'apiKey';
                if (name === 'events')
                    return ['invitee.created', 'invitee.canceled'];
                if (name === 'scope')
                    return 'organization';
                return undefined;
            });
        ...
        it('should reject non-HTTPS webhook URLs before calling Calendly', async () => {
            ...
            await expect(trigger.webhookMethods.default.create.call(mockHookFunctions)).rejects.toThrow('Calendly requires a public HTTPS webhook URL');
    ...
    describe('webhookMethods.default.delete', () => {
        it('should delete webhook subscription by stored URI', async () => {
            ...
            expect(result).toBe(true);
            expect(requestWithAuthentication).toHaveBeenCalledWith('calendlyApi', expect.objectContaining({
                method: 'DELETE',
                uri: webhookUri,
            }));
            expect(webhookData.webhookURI).toBeUndefined();
...
//# sourceMappingURL=CalendlyTrigger.node.test.js.map