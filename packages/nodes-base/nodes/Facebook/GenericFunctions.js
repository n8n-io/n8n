...
export async function facebookApiRequest(method, resource, body = {}, qs = {}, uri, _option = {}) {
    ...
    if (!isOAuth2 && credentials.appSecret) {
        ...
        qs.appsecret_proof = createHmac('sha256', credentials.appSecret)
...
export function getFields(object) {
    ...
    return [{ name: '*', value: '*' }].concat(data[object] || []).map((fieldObject) => ({
        ...fieldObject,
        name: fieldObject.value !== '*' ? capitalCase(fieldObject.value) : fieldObject.value,
    }));
}
export function getAllFields(object) {
