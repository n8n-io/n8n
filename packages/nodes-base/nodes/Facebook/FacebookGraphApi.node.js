...
export class FacebookGraphApi {
    ...
    async execute() {
        ...
        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
            const authType = this.getNodeParameter('authType', itemIndex, 'accessToken');
            const qs = {};
            let graphApiAccessToken;
            if (authType === 'accessToken') {
                const graphApiCredentials = await this.getCredentials('facebookGraphApi');
                graphApiAccessToken = graphApiCredentials.accessToken;
            ...
            try {
                ...
                if (authType === 'oAuth2') {
                    response = await this.helpers.requestWithAuthentication.call(this, 'facebookGraphApiOAuth2Api', requestOptions);
                }
                else {
                ...
                const apiError = new NodeApiError(this.getNode(), error);
            ...
            if (typeof response === 'string') {
                const invalidJsonError = new NodeOperationError(this.getNode(), 'Response body is not valid JSON.', { itemIndex });
                if (!this.continueOnFail()) {
