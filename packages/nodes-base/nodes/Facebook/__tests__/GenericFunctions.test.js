...
vi.mock('n8n-workflow', async () => {
    ...
    return {
        ...original,
        NodeApiError: vi.fn().mockImplementation(function (node, error) {
...
describe('Facebook GenericFunctions', () => {
    ...
    describe('facebookApiRequest', () => {
        it('should use correct credentials for regular nodes', async () => {
            ...
            await utils.facebookApiRequest.call(mockExecuteFunctions, 'GET', '/me', {}, { fields: 'id,name' });
        ...
        it('should allow custom URI', async () => {
            ...
            await utils.facebookApiRequest.call(mockExecuteFunctions, 'POST', '', { message: 'Hello' }, {}, customUri);
            expect(mockExecuteFunctions.helpers.request).toHaveBeenCalledWith(expect.objectContaining({ uri: customUri }));
        });
        describe('OAuth2 authentication', () => {
            ...
            it('should use OAuth2 credential for regular nodes', async () => {
                await utils.facebookApiRequest.call(mockExecuteFunctions, 'GET', '/me', {}, {});
                expect(mockExecuteFunctions.getCredentials).toHaveBeenCalledWith('facebookGraphApiOAuth2Api');
                expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('facebookGraphApiOAuth2Api', expect.objectContaining({
                    method: 'GET',
                    uri: 'https://graph.facebook.com/v23.0/me',
                }));
            ...
            it('should use OAuth2 app credential for trigger nodes', async () => {
                mockExecuteFunctions.getNode.mockReturnValue({ name: 'Facebook Trigger' });
                await utils.facebookApiRequest.call(mockExecuteFunctions, 'GET', '/app', {}, {});
                expect(mockExecuteFunctions.getCredentials).toHaveBeenCalledWith('facebookGraphAppOAuth2Api');
                expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('facebookGraphAppOAuth2Api', expect.objectContaining({ method: 'GET' }));
            });
            ...
            it('should not compute appsecret_proof when using OAuth2', async () => {
                ...
                await utils.facebookApiRequest.call(mockExecuteFunctions, 'GET', '/app', {}, {});
                expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('facebookGraphAppOAuth2Api', expect.objectContaining({
