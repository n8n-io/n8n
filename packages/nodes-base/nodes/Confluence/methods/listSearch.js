 ...
 */
async function searchByName(endpoint, baseQs, toDisplayName, filter, paginationToken) {
    const filterLower = (filter ?? '').trim().toLowerCase();
    const results = [];
    let cursor = paginationToken;
    for (let fetched = 0; fetched < MAX_FILTERED_SEARCH_PAGES; fetched++) {
        const qs = { ...baseQs, limit: SEARCH_PAGE_SIZE };
        if (cursor !== undefined)
            qs.cursor = cursor;
        ...
        for (const entry of entries) {
            ...
            if (typeof entry.id !== 'string' && typeof entry.id !== 'number')
                continue;
            if (filterLower !== '' && !lastName.includes(filterLower))
                continue;
            if (lastName === filterLower)
                exactFound = true;
            results.push({ name: toDisplayName(entry.name, entry), value: String(entry.id) });
        }
        cursor = extractNextCursor(response);
        if (cursor === undefined || filterLower === '' || exactFound)
            break;
        ...
        if (results.length > 0 && !exactMayLieAhead)
            break;
    }
...
export async function searchSpaces(filter, paginationToken) {
    return await searchByName.call(this, '/wiki/api/v2/spaces', { sort: 'name', status: 'current' }, (name, space) => {
        const key = typeof space.key === 'string' && space.key !== '' ? ` (${space.key})` : '';
        return `${name}${key}`;
    }, filter, paginationToken);
}
export async function getLabels(filter, paginationToken) {
    return await searchByName.call(this, '/wiki/api/v2/labels', { sort: 'name' }, (name, label) => {
        // Non-global labels (my/team/system) share names with global ones; the prefix disambiguates
        const prefix = typeof label.prefix === 'string' && label.prefix !== '' && label.prefix !== 'global'
            ? ` (${label.prefix})`
            : '';
        return `${name}${prefix}`;
    }, filter, paginationToken);
}
export async function searchSpacesWithAll(filter, paginationToken) {
...
function toPageItems(entries, base, withSpaceLabel) {
    ...
    for (const entry of entries) {
        ...
        const container = entry.resultGlobalContainer;
        const space = withSpaceLabel && typeof container?.title === 'string' && container.title !== ''
            ? ` (${container.title})`
            : '';
...
function nextStartToken(next, start, count) {
    if (next === undefined)
        return undefined;
    let parsed = null;
    try {
...
export async function getPages(filter, paginationToken) {
    ...
    if (spaceId !== '') {
        ...
        if (spaceKey !== undefined)
            spaceClause = ` AND space = "${spaceKey}"`;
    }
    ...
    const exact = escaped !== '' && paginationToken === undefined
    ...
    return {
        results: toPageItems([...exact.entries, ...page.entries], page.base || exact.base, spaceId === ''),
