...
const properties = [
    {
        ...
        description: 'The CQL query to run. See <a href="https://developer.atlassian.com/cloud/confluence/advanced-searching-using-cql/" target="_blank">Atlassian\'s CQL reference</a> for the syntax.',
    },
    ...returnAllOrLimit,
    {
        displayName: 'Options',
        ...
        options: [
            ...
            },
            {
                ...
                type: 'boolean',
                default: false,
                description: 'Whether each result carries its full storage-format body (content.body.storage), fetched on the same request. Only applies to content results such as pages; space and user results have no body.',
            },
        ],
    },
];
...
function buildExpand(options) {
    const fields = (options.additionalExpandFields ?? '')
        .split(',')
        .map((field) => field.trim())
        .filter((field) => field !== '');
    if (options.fetchFullPageContent === true)
        fields.unshift('content.body.storage');
...
export const execute = async function (itemIndex) {
    ...
    for (;;) {
        ...
        if (pageParam !== undefined)
            pageQs[pageParam.key] = pageParam.value;
        const response = await confluenceApiRequest.call(this, 'GET', '/wiki/rest/api/search', {}, pageQs);
        ...
        if (next === undefined || seenPageParams.has(`${next.key}:${next.value}`))
            break;
