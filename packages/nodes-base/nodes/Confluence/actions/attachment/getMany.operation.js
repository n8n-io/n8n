...
import { fetchPaginatedResults, optionalSpaceRLC, pageRLC, parsePositiveInt, resolvePageId, } from '../common';
const properties = [
    optionalSpaceRLC,
    {
        ...pageRLC,
    ...
    ...returnAllOrLimit,
    {
        ...
        type: 'boolean',
        default: false,
        description: "Whether to also download each attachment's file and attach it to the item's binary output",
    },
    {
...
export const execute = async function (itemIndex) {
    ...
    const items = attachments.map((attachment) => ({ json: attachment }));
    if (download) {
        ...
        for (const item of items) {
            ...
            if (typeof downloadLink !== 'string' || !downloadLink.startsWith('/')) {
                throw new NodeOperationError(this.getNode(), `Attachment "${asOptionalString(item.json.title) ?? asOptionalString(item.json.id) ?? 'unknown'}" has no usable download link`, { itemIndex });
            }
            ...
            item.binary = {
                [binaryPropertyName]: await this.helpers.prepareBinaryData(buffer, asOptionalString(item.json.title), asOptionalString(item.json.mediaType)),
            };
        }
    }
    return items;
