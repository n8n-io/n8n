...
export const description = [
    {
        ...
        description: 'The ID of the attachment to delete. Attachment IDs come from the Get Many operation.',
    ...
    },
    {
        ...
        type: 'boolean',
        default: false,
        description: "Whether to permanently delete the attachment instead of moving it to trash. This cannot be undone and requires admin permission in the attachment's space.",
        displayOptions: { show: showOnDelete },
    },
];
/**
 ...
 */
async function trashAttachment(endpoint, purge, itemIndex) {
    try {
        ...
        throw new NodeOperationError(this.getNode(), 'Confluence could not delete the attachment', {
            itemIndex,
            description: 'The attachment may not exist or may already be in the trash, or the connected user may lack permission to view its page or to delete attachments in the space (Confluence reports permission failures as "not found").',
        });
    }
}
...
async function purgeAttachment(endpoint, itemIndex, trashConfirmed) {
    try {
        ...
        if (trashConfirmed) {
            throw new NodeOperationError(this.getNode(), 'The attachment was moved to trash, but could not be purged', {
                itemIndex,
        ...
        throw new NodeOperationError(this.getNode(), 'Confluence could not permanently delete the attachment', {
            itemIndex,
...
export const execute = async function (itemIndex) {
    ...
    if (purge)
        await purgeAttachment.call(this, endpoint, itemIndex, trashConfirmed);
