...
export const execute = async function (itemIndex) {
    ...
    if (label.includes(',')) {
        throw new NodeOperationError(this.getNode(), "The 'Label' parameter accepts one label name; commas are not allowed", { itemIndex });
    }
    // A label can never hold a space, so this would 204 as a no-op and report success too
    if (/\s/.test(label)) {
        throw new NodeOperationError(this.getNode(), `The label "${label}" contains a space; Confluence labels use an underscore or hyphen instead`, { itemIndex });
    }
    ...
    await confluenceApiRequest.call(this, 'DELETE', `/wiki/rest/api/content/${encodeURIComponent(pageId)}/label`, {}, { name: label });
    return { removed: true, pageId, label };
};
//# sourceMappingURL=removeLabel.operation.js.map