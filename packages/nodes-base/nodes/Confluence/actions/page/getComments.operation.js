...
import { bodyFormatOption, fetchPaginatedResults, optionalSpaceRLC, pageRLC, parsePositiveInt, resolvePageId, shapeBody, sortDirectionOption, sortQs, } from '../common';
const properties = [
    optionalSpaceRLC,
    {
        ...pageRLC,
...
export const execute = async function (itemIndex) {
    ...
    const bodyFormat = rawBodyFormat === 'atlas_doc_format' || rawBodyFormat === 'plainText'
        ? rawBodyFormat
        : 'storage';
    ...
    const total = returnAll
        ? Infinity
        : parsePositiveInt.call(this, this.getNodeParameter('limit', itemIndex, 50), 'Limit', itemIndex);
    const pageId = await resolvePageId.call(this, itemIndex);
    const qs = { 'body-format': requestedFormat, ...sortQs(options) };
    const comments = await fetchPaginatedResults.call(this, `/wiki/api/v2/pages/${encodeURIComponent(pageId)}/footer-comments`, total, qs);
    return comments.map((comment) => shapeBody(comment, bodyFormat));
};
//# sourceMappingURL=getComments.operation.js.map