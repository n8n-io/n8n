...
import { PAGE_LIMIT, bodyFormatOption, nextUnseenCursor, optionalSpaceRLC, pageRLC, parsePositiveInt, resolvePageId, shapeBody, } from '../common';
 ...
 */
async function collectDescendantPageIds(rootId, maxCount) {
    const pageIds = [];
    ...
    while (frontier.length > 0 && pageIds.length < maxCount) {
        ...
        for (const nodeId of frontier) {
            ...
            do {
                const qs = { depth: MAX_DEPTH, limit: PAGE_LIMIT };
                if (cursor !== undefined)
                    qs.cursor = cursor;
                const response = await confluenceApiRequest.call(this, 'GET', `/wiki/api/v2/pages/${encodeURIComponent(nodeId)}/descendants`, {}, qs);
 ...
 * which is intended.
 */
async function fetchPagesByIds(ids, requestedFormat) {
    const pages = [];
    for (let start = 0; start < ids.length; start += PAGE_LIMIT) {
        const chunk = ids.slice(start, start + PAGE_LIMIT);
        const response = await confluenceApiRequest.call(this, 'GET', '/wiki/api/v2/pages', {}, { id: chunk.join(','), 'body-format': requestedFormat, limit: PAGE_LIMIT });
...
export const execute = async function (itemIndex) {
    ...
    const includeDescendants = this.getNodeParameter('includeDescendants', itemIndex, false);
    ...
    if (!includeDescendants) {
        const page = await confluenceApiRequest.call(this, 'GET', `/wiki/api/v2/pages/${encodeURIComponent(pageId)}`, {}, { 'body-format': requestedFormat });
        return shapeBody(page, bodyFormat);
    }
    const maxPages = parsePositiveInt.call(this, this.getNodeParameter('maxPages', itemIndex, 100), 'Max Pages', itemIndex);
    const descendantIds = await collectDescendantPageIds.call(this, pageId, Math.max(maxPages - 1, 0));
