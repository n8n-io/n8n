...
export const execute = async function (itemIndex) {
    ...
    try {
        await confluenceApiRequest.call(this, 'DELETE', `/wiki/api/v2/footer-comments/${encodeURIComponent(commentId)}`);
    }
    catch (error) {
        ...
        if (error instanceof NodeApiError && error.httpCode === '404') {
            throw new NodeOperationError(this.getNode(), 'Confluence could not delete the comment', {
                itemIndex,
                description: 'The comment may not exist or may already be deleted, or the connected user may lack view or delete permission on its page (Confluence reports permission failures as "not found").',
            });
        }
        throw error;
    }
