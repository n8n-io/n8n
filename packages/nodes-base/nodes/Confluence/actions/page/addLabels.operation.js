...
const properties = [
    ...
    },
    {
        ...
        description: 'The label names to add, comma-separated for several. Label names cannot contain spaces — use an underscore or hyphen instead. Existing labels are kept.',
    },
];
...
export const execute = async function (itemIndex) {
    ...
    if (spaced !== undefined) {
        throw new NodeOperationError(this.getNode(), `The label "${spaced}" contains a space; use an underscore or hyphen instead`, { itemIndex });
    }
    ...
    return await confluenceApiRequest.call(this, 'POST', `/wiki/rest/api/content/${encodeURIComponent(pageId)}/label`, names.map((name) => ({ prefix: 'global', name })));
};
