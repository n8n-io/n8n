...
export function bodyProperties(operations, bodyHint, contentNoun = 'Page content') {
...
export function buildBodyEnvelope(format, content) {
    switch (format) {
        case 'plainText': {
            if (typeof content === 'object' && content !== null) {
                throw new Error('Plain text body must be text, got an object. Use the ADF JSON format for document objects.');
            }
        ...
        default:
            throw new Error(`Unsupported body format "${format}"`);
    }
}
...
function adfNodeHasContent(node) {
    if (node === null || typeof node !== 'object' || Array.isArray(node))
        return false;
    const { type, text, content, attrs } = node;
    if (typeof text === 'string' && text.trim() !== '')
        return true;
    if (typeof type === 'string' && !STRUCTURAL_ADF_TYPES.has(type))
        return true;
    // Among structural types, expand/nestedExpand render their attrs.title as visible text
    const title = attrs !== null && typeof attrs === 'object' ? attrs.title : undefined;
...
function storageHasContent(markup) {
    ...
    for (const [, tagName] of markup.matchAll(/<\/?([a-zA-Z][\w:-]*)/g)) {
        if (!STRUCTURAL_STORAGE_TAGS.has(tagName.toLowerCase()))
            return true;
    }
    return false;
}
...
export function envelopeHasContent(envelope) {
    if (envelope.representation !== 'atlas_doc_format')
        return storageHasContent(envelope.value);
    try {
...
export function readBodyEnvelope(ctx, itemIndex) {
    const format = ctx.getNodeParameter('bodyFormat', itemIndex, 'plainText');
...
export function readBodyEnvelopeIfProvided(ctx, itemIndex) {
    const format = ctx.getNodeParameter('bodyFormat', itemIndex, 'plainText');
    const content = ctx.getNodeParameter(fieldByFormat[format], itemIndex, '');
...
//# sourceMappingURL=bodyEnvelope.js.map