...
import { confluenceApiRequest, confluenceApiRequestBinary, confluenceApiRequestUpload, } from '../../transport';
const accessibleResources = [
    { id: 'cloud-1', url: 'https://example.atlassian.net', name: 'example' },
    { id: 'cloud-2', url: 'https://Other.Atlassian.NET' },
];
const siteByUrl = (url) => ({ __rl: true, mode: 'url', value: url });
...
describe('confluenceApiRequest', () => {
    ...
    async function captureRejection(endpoint) {
    ...
    function failNextRequestWrapped(payload) {
    ...
    beforeEach(() => {
        ...
        ctx.getNodeParameter.mockReturnValue(siteByUrl('https://example.atlassian.net/wiki'));
    });
    it('routes requests to https://api.atlassian.com/ex/confluence/{cloudId}', async () => {
        ...
        expect(mockHttpRequestWithAuthentication).toHaveBeenNthCalledWith(1, 'confluenceCloudOAuth2Api', expect.objectContaining({
            url: 'https://api.atlassian.com/oauth/token/accessible-resources',
        }));
        expect(mockHttpRequestWithAuthentication).toHaveBeenNthCalledWith(2, 'confluenceCloudOAuth2Api', expect.objectContaining({
    ...
    it('passes method, body and qs through', async () => {
        ...
        expect(mockHttpRequestWithAuthentication).toHaveBeenNthCalledWith(2, 'confluenceCloudOAuth2Api', expect.objectContaining({ method: 'POST', body, qs, json: true }));
    });
    it('sends an array body through as an array', async () => {
        ...
        expect(mockHttpRequestWithAuthentication).toHaveBeenNthCalledWith(2, 'confluenceCloudOAuth2Api', expect.objectContaining({ body: [{ prefix: 'global', name: 'a' }] }));
    });
    it('defaults body and qs to empty objects', async () => {
        await confluenceApiRequest.call(ctx, 'GET', '/wiki/api/v2/pages');
        expect(mockHttpRequestWithAuthentication).toHaveBeenNthCalledWith(2, 'confluenceCloudOAuth2Api', expect.objectContaining({ body: {}, qs: {}, json: true }));
    });
    ...
    it("surfaces Atlassian's v2 envelope from a wrapped NodeApiError", async () => {
        const wrapped = failNextRequestWrapped(pageNotFoundResponse);
        const error = await captureRejection('/wiki/api/v2/pages/1');
        expect(error).toBeInstanceOf(NodeApiError);
        expect(error).not.toBe(wrapped);
        expect(error?.message).toBe('Page not found');
    ...
    it('rethrows a wrapped NodeApiError unchanged when there is no response body', async () => {
        const wrapped = failNextRequestWrapped({ message: 'socket hang up' });
        const error = await captureRejection('/wiki/api/v2/pages');
        expect(error).toBe(wrapped);
    });
    it('surfaces the cloudId lookup error when no site matches the By URL value', async () => {
        ctx.getNodeParameter.mockReturnValue(siteByUrl('https://missing.atlassian.net'));
        await expect(confluenceApiRequest.call(ctx, 'GET', '/wiki/api/v2/pages')).rejects.toThrow('No Confluence site matched "https://missing.atlassian.net"');
    });
    it('uses a From List selection as the cloudId directly, without the resources lookup', async () => {
        ctx.getNodeParameter.mockReturnValue({ __rl: true, mode: 'list', value: 'cloud-2' });
        ...
        expect(mockHttpRequestWithAuthentication).toHaveBeenCalledWith('confluenceCloudOAuth2Api', expect.objectContaining({
    ...
    it('auto-resolves an empty Site parameter when the connection reaches one site', async () => {
        ctx.getNodeParameter.mockReturnValue({ __rl: true, mode: 'list', value: '' });
        mockHttpRequestWithAuthentication
    ...
    it('reads the Site parameter through getCurrentNodeParameter in a load-options context', async () => {
        ...
        expect(mockHttpRequestWithAuthentication).toHaveBeenLastCalledWith('confluenceCloudOAuth2Api', expect.objectContaining({
...
describe('confluenceApiRequestBinary', () => {
    ...
    it('rejects an unusable binary response with a NodeOperationError', async () => {
        ...
        await expect(confluenceApiRequestBinary.call(ctx, '/wiki/download/attachments/9/a.txt')).rejects.toThrow('Confluence returned an unexpected binary response');
    });
...
describe('confluenceApiRequestUpload', () => {
    ...
    it('PUTs the multipart body with the XSRF-bypass header, no json flag', async () => {
        ...
        const data = await confluenceApiRequestUpload.call(ctx, '/wiki/rest/api/content/9/child/attachment', formData);
        expect(mockHttpRequestWithAuthentication).toHaveBeenNthCalledWith(2, 'confluenceCloudOAuth2Api', expect.objectContaining({
    ...
    it('asks to pick a site when the Site parameter is empty and several sites are reachable', async () => {
        ctx.getNodeParameter.mockReturnValue({ __rl: true, mode: 'list', value: '' });
        const promise = confluenceApiRequestUpload.call(ctx, '/wiki/rest/api/content/9/child/attachment', new FormData());
        await expect(promise).rejects.toThrow(NodeOperationError);
...
describe('credential routing (authentication selector)', () => {
    ...
    const setup = (authentication) => {
        ...
        ctx.getNodeParameter.mockImplementation(((name, _itemIndex, fallback) => {
            if (name === 'authentication')
                return authentication ?? fallback;
            if (name === 'site')
    ...
    it('uploads through the Service Account credential when selected', async () => {
        setup('serviceAccount');
        await confluenceApiRequestUpload.call(ctx, '/wiki/rest/api/content/9/child/attachment', new FormData());
        expect(mockHttpRequestWithAuthentication.mock.calls.length).toBeGreaterThanOrEqual(2);
        for (const call of mockHttpRequestWithAuthentication.mock.calls) {
            expect(call[0]).toBe('atlassianServiceAccountApi');
        }
    ...
    it('resolves the Service Account credential through getCurrentNodeParameter in a load-options context', async () => {
        ...
        loadCtx.getCurrentNodeParameter.mockImplementation(((name) => {
            if (name === 'authentication')
