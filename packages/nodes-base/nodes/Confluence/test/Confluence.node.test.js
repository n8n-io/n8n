...
describe('Confluence Node', () => {
    ...
    const operationProperty = (resource) => node.description.properties.find((p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes(resource));
    const operationOptions = (resource) => operationProperty(resource)?.options;
    ...
    it('carries the top-level Site selector on every resource', () => {
        for (const resource of ['attachment', 'page', 'search', 'space']) {
            const site = node.description.properties.find((p) => p.name === 'site' && p.displayOptions?.show?.resource?.includes(resource));
    ...
    it('exposes the getLabels fields on the node description', () => {
        const forGetLabels = node.description.properties.filter((p) => p.displayOptions?.show?.operation?.includes('getLabels'));
        expect(forGetLabels.map((p) => p.name)).toEqual(expect.arrayContaining(['page', 'returnAll', 'limit', 'options']));
    ...
    it('should render the label operations own fields', () => {
        const fieldsFor = (operation) => node.description.properties
...
//# sourceMappingURL=Confluence.node.test.js.map