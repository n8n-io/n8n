...
import { getLabels, getPages, getSites, searchSpaces, searchSpacesWithAll, } from '../../methods/listSearch';
...
describe('Confluence listSearch.getPages', () => {
    ...
    it('lists recently modified pages when no filter is given', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/rest/api/search', {}, { cql: 'type=page ORDER BY lastmodified DESC', limit: 50, start: 0 });
    ...
    it('scopes the search to the selected space and drops the space label', async () => {
        ...
        apiRequest.mockImplementation(async (_method, endpoint, _body, qs) => {
            if (endpoint === '/wiki/api/v2/spaces/999')
                return { id: 999, key: 'DOCS' };
            if (endpoint === '/wiki/rest/api/search') {
                const cql = qs.cql;
                if (cql === 'type=page AND space = "DOCS" AND title = "plan"')
                    return { results: [] };
                expect(cql).toBe('type=page AND space = "DOCS" AND title ~ "plan*" ORDER BY lastmodified DESC');
                return {
                    results: [
                        {
        ...
        const spaceLookups = apiRequest.mock.calls.filter(([, endpoint]) => endpoint === '/wiki/api/v2/spaces/999');
    ...
    it('does not reuse cached space keys across credentials', async () => {
        ...
        const createCtx = (credentialId) => {
            const scopedCtx = mockDeep();
    ...
    it('does not reuse cached space keys across sites of one credential', async () => {
        ...
        const keyPerSite = { 'cloud-1': 'DOCS', 'cloud-2': 'ENG' };
        const cqls = [];
        cloudId.mockImplementation(async () => currentSite);
        apiRequest.mockImplementation(async (_method, endpoint, _body, qs) => {
            if (endpoint === '/wiki/api/v2/spaces/999')
                return { id: 999, key: keyPerSite[currentSite] };
            if (endpoint === '/wiki/rest/api/search') {
                cqls.push(qs.cql);
                return { results: [] };
            }
    ...
    it('escapes quotes and backslashes in the CQL title filter', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/rest/api/search', {}, expect.objectContaining({
    ...
    it('skips the exact-title query on later pages', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/rest/api/search', {}, expect.objectContaining({ start: 50 }));
    });
...
describe('Confluence listSearch.searchSpaces', () => {
    ...
    it('lists current spaces sorted by name, labeled with their key', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/spaces', {}, { limit: 50, sort: 'name', status: 'current' });
        expect(result).toEqual({
            results: [
    ...
    it('keeps fetching pages while a typed filter has no match yet', async () => {
        ...
        expect(apiRequest).toHaveBeenNthCalledWith(2, 'GET', '/wiki/api/v2/spaces', {}, expect.objectContaining({ cursor: 'c2' }));
        expect(result).toEqual({
    ...
    it('resumes from the pagination cursor and returns the next one', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/spaces', {}, expect.objectContaining({ cursor: 'abc==' }));
...
describe('Confluence listSearch.getLabels', () => {
    ...
    it('lists labels sorted by name, marking non-global prefixes', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/labels', {}, { limit: 50, sort: 'name' });
    ...
    it('keeps fetching pages while a typed filter has no match yet', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledTimes(2);
        expect(apiRequest).toHaveBeenNthCalledWith(2, 'GET', '/wiki/api/v2/labels', {}, expect.objectContaining({ cursor: 'c2' }));
        expect(result).toEqual({
    ...
    it('resumes from the pagination cursor and returns the next one', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/labels', {}, expect.objectContaining({ cursor: 'abc==' }));
        expect(result.paginationToken).toBe('xyz==');
    });
});
describe('Confluence listSearch.getSites', () => {
    ...
    it('lists accessible sites sorted by name, cloudId as the value', async () => {
        ...
        expect(httpRequestWithAuthentication).toHaveBeenCalledWith('resolvedConfluenceCredential', expect.objectContaining({
