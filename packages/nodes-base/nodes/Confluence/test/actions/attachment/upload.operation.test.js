...
async function runUpload(overrides = {}, binary) {
...
describe('attachment:upload', () => {
    ...
    it.each([
        ...
        ['true', 'true'],
        ['false', 'false'],
    ])('sends minorEdit=%j as the string %j under its own form field', async (minorEdit, expected) => {
    ...
    it('propagates the error when the configured binary property is missing', async () => {
        ...
        const notFound = new NodeOperationError(testNode, "No binary data property 'missing' exists on item");
    ...
    it('falls back to a generic Content-Type when the binary data has no mimeType or usable filename', async () => {
        const result = await runUpload({}, { mimeType: undefined, fileName: undefined });
        expect(apiRequestUpload).toHaveBeenCalledTimes(1);
        const [, formData] = apiRequestUpload.mock.calls[0];
...
//# sourceMappingURL=upload.operation.test.js.map