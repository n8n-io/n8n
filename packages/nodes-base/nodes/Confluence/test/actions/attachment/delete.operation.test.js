...
function apiError(httpCode) {
    return new NodeApiError(testNode, { message: 'boom' }, { httpCode });
}
async function runDelete(overrides = {}) {
...
describe('attachment:delete', () => {
    ...
    describe('when purging', () => {
        ...
        it('still purges an attachment that is already in the trash', async () => {
            ...
            apiRequest.mockRejectedValueOnce(apiError('404')).mockResolvedValueOnce('');
            const result = await runDelete({ purge: true });
            expect(apiRequest).toHaveBeenNthCalledWith(2, 'DELETE', ENDPOINT, {}, { purge: true });
            expect(result).toEqual({ deleted: true, attachmentId: 'att123', purged: true });
        });
        ...
        it.each(['403', '404', '500'])('reports the trashed-but-not-purged state when the purge fails with %s, given a confirmed trash', async (httpCode) => {
            apiRequest.mockResolvedValueOnce('').mockRejectedValueOnce(apiError(httpCode));
            const promise = runDelete({ purge: true });
            await expect(promise).rejects.toThrow(NodeOperationError);
            await expect(promise).rejects.toThrow('The attachment was moved to trash, but could not be purged');
        });
        ...
        it('does not claim the attachment is trashed when the trash step itself was only assumed', async () => {
            ...
            const promise = runDelete({ purge: true });
            await expect(promise).rejects.toThrow(NodeOperationError);
            await expect(promise).rejects.toThrow('Confluence could not permanently delete the attachment');
