...
function attachmentPage(records, next) {
    return {
        results: records,
        ...(next === undefined ? {} : { _links: { next } }),
    };
}
async function runGetMany(overrides = {}) {
    const ctx = mockExecuteCtx({ ...baseParams, ...overrides });
    ctx.helpers.prepareBinaryData.mockImplementation(async (buffer, fileName, mimeType) => ({
...
describe('attachment:getMany', () => {
    ...
    it('paginates with the next cursor until exhausted', async () => {
        ...
        expect(apiRequest).toHaveBeenNthCalledWith(2, 'GET', ENDPOINT, {}, { limit: 250, cursor: 'c1' });
        expect(result).toEqual([{ json: { id: 'a1' } }, { json: { id: 'a2' } }]);
    });
    it('stops fetching once the limit is met and truncates', async () => {
        apiRequest.mockResolvedValue(attachmentPage([{ id: 'a1' }, { id: 'a2' }], `${ENDPOINT}?cursor=c1`));
    ...
    it('downloads each attachment into the binary field', async () => {
        ...
        expect(binaryRequest).toHaveBeenCalledWith('/wiki/download/attachments/9/notes.txt?version=1&api=v2');
        expect(result).toEqual([
            {
    ...
    it('percent-encodes raw filename characters while keeping the query string intact', async () => {
        ...
        expect(binaryRequest).toHaveBeenCalledWith('/wiki/download/attachments/9/report%20%233.pdf?version=1&api=v2');
    });
    ...
    it('keeps decrementing the page limit while following cursors under a finite limit', async () => {
        ...
        apiRequest
            ...
            .mockResolvedValueOnce(attachmentPage(Array.from({ length: 50 }, (_, i) => ({ id: `s${i}` }))));
    ...
    it('puts the file in a custom binary field', async () => {
        apiRequest.mockResolvedValue(attachmentPage([{ id: 'a1', downloadLink: '/download/attachments/9/a' }]));
...
//# sourceMappingURL=getMany.operation.test.js.map