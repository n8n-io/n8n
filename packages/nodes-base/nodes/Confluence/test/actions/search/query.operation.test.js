...
const runSearch = async (overrides = {}) => await execute.call(mockExecuteCtx({ ...baseParams, ...overrides }), 0);
const expectSearchRequest = (qs) => expect(apiRequest).toHaveBeenCalledWith('GET', SEARCH_ENDPOINT, {}, qs);
const expectNthSearchRequest = (nth, qs) => expect(apiRequest).toHaveBeenNthCalledWith(nth, 'GET', SEARCH_ENDPOINT, {}, qs);
function searchPage(ids, next) {
    return {
...
describe('search:query', () => {
    ...
    it('returns all pages by following the next cursor with the same query', async () => {
        apiRequest
            .mockResolvedValueOnce(searchPage(['1', '2'], '/rest/api/search?cql=type%3Dpage&cursor=abc&limit=50'))
    ...
    it('falls back to the start offset when the next link carries no cursor', async () => {
        apiRequest
            .mockResolvedValueOnce(searchPage(['1'], '/rest/api/search?cql=type%3Dpage&start=25&limit=25'))
    ...
    it('sends cqlcontext when content statuses are selected', async () => {
        ...
        expectSearchRequest(expect.objectContaining({ cqlcontext: '{"contentStatuses":["draft","archived"]}' }));
    });
    it('omits expand and cqlcontext when no options are set', async () => {
        await runSearch();
        const qs = apiRequest.mock.calls[0][3];
        expect(qs).not.toHaveProperty('expand');
        expect(qs).not.toHaveProperty('cqlcontext');
    });
});
//# sourceMappingURL=query.operation.test.js.map