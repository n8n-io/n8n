...
const apiRequest = confluenceApiRequest;
const apiRequestUpload = confluenceApiRequestUpload;
const createParams = {
    resource: 'page',
    operation: 'create',
...
describe('Confluence router', () => {
    ...
    it('dispatches attachment:delete and returns the deletion report', async () => {
        ...
        const result = await router.call(mockExecuteCtx({
            resource: 'attachment',
            operation: 'delete',
            attachmentId: 'att123',
        }));
    ...
    it('dispatches attachment:getMany and pairs the emitted items', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/pages/9/attachments', {}, { limit: 250 });
    ...
    it('dispatches attachment:upload and returns the created attachment', async () => {
        ...
        expect(apiRequestUpload).toHaveBeenCalledWith('/wiki/rest/api/content/9/child/attachment', expect.anything());
        expect(result).toEqual([
    ...
    it('dispatches page:removeLabel and returns the removal report', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('DELETE', '/wiki/rest/api/content/1/label', {}, { name: 'runbook' });
        expect(result).toEqual([
    ...
    it('dispatches page:get and returns the fetched page', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/pages/1', {}, { 'body-format': 'storage' });
    ...
    it('dispatches page:getManyByLabel and fans pages out into one item each', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/labels/777/pages', {}, { 'body-format': 'storage', limit: 50 });
        expect(result).toEqual([
            [
    ...
    it('dispatches page:getComments and fans the comments out into one item each', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/pages/1/footer-comments', {}, { 'body-format': 'storage', limit: 50 });
        expect(result).toEqual([
            [
    ...
    it('fans an array response out into one item per page', async () => {
        apiRequest.mockImplementation(async (_method, url) => url.endsWith('/descendants')
            ? { results: [{ id: '2', type: 'page', depth: 1 }] }
            : { results: [{ id: '1' }, { id: '2' }] });
        const result = await router.call(mockExecuteCtx({ ...getParams, includeDescendants: true, maxPages: 100 }));
        expect(result).toEqual([
            [
                { json: { id: '1' }, pairedItem: { item: 0 } },
                { json: { id: '2' }, pairedItem: { item: 0 } },
    ...
    describe('page:getLabels', () => {
        ...
        it('dispatches page:getLabels and fans the labels out into one item per label', async () => {
            ...
            expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/pages/1/labels', {}, { limit: 50 });
            expect(result).toEqual([
                [
    ...
    it('dispatches space:getMany and fans the spaces out into one item each', async () => {
        apiRequest.mockResolvedValue({ results: [{ id: '1' }, { id: '2' }] });
        const result = await router.call(mockExecuteCtx({ resource: 'space', operation: 'getMany', returnAll: false, limit: 50 }));
    ...
    it('emits an error item for a failing space operation when continue-on-fail is on', async () => {
        const ctx = mockExecuteCtx({ resource: 'space', operation: 'get', space: { mode: 'id', value: '1' } }, 2);
        ctx.continueOnFail.mockReturnValue(true);
    ...
    it.each(['update', 'append'])('dispatches page:%s', async (operation) => {
        ...
        const result = await router.call(mockExecuteCtx({ ...createParams, operation, page: { mode: 'id', value: '222' }, title: '' }));
        expect(apiRequest).toHaveBeenCalledTimes(2);
    ...
    it('dispatches search:query and fans the results out into items', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/rest/api/search', {}, { cql: 'type = page', limit: 50 });
        expect(result).toEqual([
            [
    ...
    it.each([
        ...
        await expect(router.call(ctx)).rejects.toThrow(`The operation "${resource}:${operation}" is not supported`);
