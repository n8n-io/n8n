...
describe('page:addComment', () => {
    ...
    it('hides the space and page pickers when a parent comment ID is set', () => {
        ...
        const byName = (name) => description.find((property) => property.name === name);
    ...
    it('posts an ADF comment whose only content is a non-text node', async () => {
        ...
        await execute.call(mockExecuteCtx({ ...baseParams, bodyFormat: 'atlas_doc_format', bodyAdf }), 0);
        expect(apiRequest).toHaveBeenCalledWith('POST', '/wiki/api/v2/footer-comments', {
            pageId: '123',
    ...
    it('rejects an invalid ADF body before calling the API', async () => {
        const promise = execute.call(mockExecuteCtx({ ...baseParams, bodyFormat: 'atlas_doc_format', bodyAdf: 'not-json' }), 0);
        await expect(promise).rejects.toThrow(NodeOperationError);
...
//# sourceMappingURL=addComment.operation.test.js.map