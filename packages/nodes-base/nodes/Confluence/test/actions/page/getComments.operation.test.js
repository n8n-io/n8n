...
describe('Confluence page:getComments operation', () => {
    ...
    it('fetches the footer comments of a page with the storage format by default', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/pages/123/footer-comments', {}, { 'body-format': 'storage', limit: 50 });
    ...
    it('resolves the page through the shared picker pair (by URL)', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/pages/456/footer-comments', {}, { 'body-format': 'storage', limit: 50 });
    });
    it('composes the sort option from Sort By and Sort Direction', async () => {
        ...
        await execute.call(ctx, 0);
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/pages/123/footer-comments', {}, { 'body-format': 'storage', limit: 50, sort: '-created-date' });
    });
    describe('pagination', () => {
        it('follows the cursor across pages when Return All is on', async () => {
            ...
            expect(apiRequest).toHaveBeenNthCalledWith(1, 'GET', '/wiki/api/v2/pages/123/footer-comments', {}, { 'body-format': 'storage', limit: 250 });
            expect(apiRequest).toHaveBeenNthCalledWith(2, 'GET', '/wiki/api/v2/pages/123/footer-comments', {}, { 'body-format': 'storage', limit: 250, cursor: 'abc==' });
    ...
    describe('body format', () => {
        it('passes atlas_doc_format through untouched', async () => {
            ...
            const result = await execute.call(ctx, 0);
            expect(apiRequest.mock.calls[0][3]['body-format']).toBe('atlas_doc_format');
            expect(result).toEqual(comments);
        });
...
//# sourceMappingURL=getComments.operation.test.js.map