...
describe('Confluence page:get operation', () => {
    ...
    describe('single page', () => {
        it('fetches a page by ID with the storage body format', async () => {
            ...
            expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/pages/123', {}, { 'body-format': 'storage' });
        ...
        it('fetches a page by URL through the extracted ID', async () => {
            ...
            expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/pages/456', {}, { 'body-format': 'storage' });
        });
        it('passes atlas_doc_format through untouched', async () => {
            ...
            const result = await execute.call(ctx, 0);
            expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/pages/123', {}, { 'body-format': 'atlas_doc_format' });
            expect(result).toEqual(page);
        });
    ...
    describe('get by title', () => {
        it('resolves a unique site-wide match without a space', async () => {
            apiRequest.mockImplementation(async (_method, endpoint, _body, qs) => {
                ...
                if (endpoint === '/wiki/api/v2/pages/77')
                    return { id: '77', title: 'Project plan' };
        ...
        it('names the space in the no-match error when one was given', async () => {
            ...
            await expect(execute.call(ctx, 0)).rejects.toThrow('No page titled "Missing page" found in space "Docs Space"');
        });
        it('falls back to the space ID in the no-match error without a cached name', async () => {
            ...
            await expect(execute.call(ctx, 0)).rejects.toThrow('No page titled "Missing page" found in space "42"');
        });
    ...
    describe('sub-tree (includeDescendants)', () => {
        function mockTree(descendantsByNode) {
            apiRequest.mockImplementation(async (_method, endpoint, _body, qs) => {
                const match = /^\/wiki\/api\/v2\/pages\/(\d+)\/descendants$/.exec(endpoint);
                if (match)
                    return { results: descendantsByNode[match[1]] ?? [] };
                if (endpoint === '/wiki/api/v2/pages') {
                    const ids = String(qs.id).split(',');
        ...
        it('discovers descendants and hydrates them in one batched request, root included', async () => {
            ...
            const result = (await execute.call(ctx, 0));
            expect(apiRequest).toHaveBeenCalledTimes(2);
            expect(apiRequest).toHaveBeenNthCalledWith(1, 'GET', '/wiki/api/v2/pages/100/descendants', {}, { depth: 10, limit: 250 });
            expect(apiRequest).toHaveBeenNthCalledWith(2, 'GET', '/wiki/api/v2/pages', {}, { id: '100,101,102', 'body-format': 'storage', limit: 250 });
        ...
        it('stops discovery when the server echoes a cursor it already returned', async () => {
            ...
            const result = (await execute.call(ctx, 0));
            const descendantCalls = apiRequest.mock.calls.filter(([, endpoint]) => endpoint.endsWith('/descendants'));
        ...
        it('rejects a Max Pages below 1', async () => {
            ...
            await expect(execute.call(ctx, 0)).rejects.toThrow('Max Pages must be a finite number of at least 1');
        });
        it('skips discovery entirely when Max Pages is 1', async () => {
            ...
            const result = (await execute.call(ctx, 0));
            expect(apiRequest).toHaveBeenCalledTimes(1);
            expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/pages', {}, { id: '100', 'body-format': 'storage', limit: 250 });
        ...
        it('chunks hydration into batches of 250 IDs', async () => {
            ...
            const result = (await execute.call(ctx, 0));
            const bulkCalls = apiRequest.mock.calls.filter(([, endpoint]) => endpoint === '/wiki/api/v2/pages');
            expect(bulkCalls).toHaveLength(2);
            expect(String(bulkCalls[0][3].id).split(',')).toHaveLength(250);
            expect(String(bulkCalls[1][3].id).split(',')).toHaveLength(11);
        ...
        it('honors the plain text body format on every sub-tree item', async () => {
            ...
            apiRequest.mockImplementation(async (_method, endpoint, _body, qs) => {
                ...
                if (endpoint === '/wiki/api/v2/pages') {
                    expect(qs['body-format']).toBe('atlas_doc_format');
                    const ids = String(qs.id).split(',');
                    return {
