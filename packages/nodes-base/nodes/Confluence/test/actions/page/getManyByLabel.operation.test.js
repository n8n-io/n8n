...
describe('Confluence page:getManyByLabel operation', () => {
    ...
    it('fetches pages for a label by ID with the storage body format', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/labels/777/pages', {}, { 'body-format': 'storage', limit: 100 });
    ...
    it('resolves a label picked from the list through extractValue', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/labels/777/pages', {}, expect.objectContaining({ limit: 100 }));
    });
    ...
    it('scopes the request to the selected space', async () => {
        ...
        await execute.call(ctx, 0);
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/labels/777/pages', {}, expect.objectContaining({ 'space-id': '999' }));
    });
    ...
    describe('pagination', () => {
        it('follows the cursor across pages when Return All is on', async () => {
            ...
            expect(apiRequest).toHaveBeenNthCalledWith(1, 'GET', '/wiki/api/v2/labels/777/pages', {}, { 'body-format': 'storage', limit: 250 });
            expect(apiRequest).toHaveBeenNthCalledWith(2, 'GET', '/wiki/api/v2/labels/777/pages', {}, { 'body-format': 'storage', limit: 250, cursor: 'abc==' });
        ...
        it('stops at the limit and only requests the remainder on follow-up pages', async () => {
            ...
            const result = (await execute.call(ctx, 0));
            expect(apiRequest).toHaveBeenCalledTimes(2);
            expect(apiRequest.mock.calls[0][3].limit).toBe(3);
            expect(apiRequest.mock.calls[1][3].limit).toBe(1);
            expect(result.map((page) => page.id)).toEqual(['1', '2', '3']);
        });
        ...
        it('caps the per-request page size at 250', async () => {
            ...
            await execute.call(ctx, 0);
            expect(apiRequest.mock.calls[0][3].limit).toBe(250);
        });
    ...
    describe('body formats', () => {
        it('passes atlas_doc_format through untouched', async () => {
            ...
            const result = await execute.call(ctx, 0);
            expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/labels/777/pages', {}, expect.objectContaining({ 'body-format': 'atlas_doc_format' }));
            expect(result).toEqual(pages);
        });
...
//# sourceMappingURL=getManyByLabel.operation.test.js.map