...
describe('page:create', () => {
    ...
    it('posts the assembled envelope to the v2 pages endpoint', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('POST', '/wiki/api/v2/pages', {
            spaceId: '111',
            status: 'current',
            title: 'My Page',
    ...
    it('maps a 404 with Private on to an actionable error', async () => {
        apiRequest.mockRejectedValue(new NodeApiError(testNode, { message: 'not found' }, { httpCode: '404' }));
    ...
    it('treats an empty By URL parent as "no parent" instead of failing extraction', async () => {
        await execute.call(mockExecuteCtx({ ...baseParams, parentPage: { mode: 'url', value: '' } }), 0);
        const [, , body] = apiRequest.mock.calls[0];
        expect(body).not.toHaveProperty('parentId');
    });
});
//# sourceMappingURL=create.operation.test.js.map