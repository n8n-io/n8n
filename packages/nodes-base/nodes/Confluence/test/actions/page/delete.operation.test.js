...
function createContext(params) {
    ...
    ctx.getNodeParameter.mockImplementation((name, _itemIndex, fallback, options) => {
        if (name === 'page' && options?.extractValue === true) {
            ...
            return (params.pageExtracted ?? page.value);
        }
...
describe('Confluence page:delete operation', () => {
    ...
    it('purges through the trash-then-purge two-step', async () => {
        ...
        expect(apiRequest).toHaveBeenNthCalledWith(2, 'DELETE', '/wiki/api/v2/pages/123', {}, { purge: true });
    ...
    it('resolves a By Title selection to its page ID before deleting', async () => {
        ...
        const result = await execute.call(ctx, 0);
        expect(apiRequest).toHaveBeenNthCalledWith(1, 'GET', '/wiki/api/v2/pages', {}, { title: 'Doc', limit: 250 });
    ...
    it('still purges a page that is already in the trash', async () => {
        apiRequest
            .mockRejectedValueOnce(new NodeApiError(mockNode, { message: 'Not found' }, { httpCode: '404' }))
    ...
    it('surfaces the purge-step error when a purged page does not exist at all', async () => {
        const notFound = () => new NodeApiError(mockNode, { message: 'Not found' }, { httpCode: '404' });
    ...
    it('lists the masked-permission causes when a plain delete is not found', async () => {
        apiRequest.mockRejectedValueOnce(new NodeApiError(mockNode, { message: 'Not found' }, { httpCode: '404' }));
        const ctx = createContext({ page: { mode: 'id', value: '123' }, purge: false });
...
//# sourceMappingURL=delete.operation.test.js.map