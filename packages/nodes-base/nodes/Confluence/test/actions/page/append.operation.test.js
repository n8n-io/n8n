...
describe('page:append', () => {
    ...
    it('concatenates a storage input onto the fetched storage body', async () => {
        apiRequest.mockResolvedValueOnce(storagePage).mockResolvedValueOnce({ id: '123' });
        await execute.call(mockExecuteCtx({ ...baseParams, bodyFormat: 'storage', bodyStorage: '<h2>New</h2>' }), 0);
    ...
    it('fails without saving when the response lacks the requested representation', async () => {
        ...
        await expect(execute.call(mockExecuteCtx(baseParams), 0)).rejects.toThrow('Could not read the current content of the page');
    ...
    it('merges an ADF input by concatenating the documents content arrays', async () => {
        ...
        await execute.call(mockExecuteCtx({
            ...baseParams,
            bodyFormat: 'atlas_doc_format',
            bodyAdf: JSON.stringify(adfDoc([{ type: 'heading' }])),
        }), 0);
        expect(apiRequest).toHaveBeenNthCalledWith(1, 'GET', '/wiki/api/v2/pages/123', {}, { 'body-format': 'atlas_doc_format' });
        expect(apiRequest).toHaveBeenNthCalledWith(2, 'PUT', '/wiki/api/v2/pages/123', {
            id: '123',
            status: 'current',
            title: 'Doc',
    ...
    it('rejects an ADF input without a content array before calling the API', async () => {
        await expect(execute.call(mockExecuteCtx({
            ...baseParams,
            bodyFormat: 'atlas_doc_format',
            bodyAdf: '{ "type": "doc", "version": 1 }',
        }), 0)).rejects.toThrow('ADF JSON body must be a document with a "content" array');
    ...
    it('fails clearly when the current ADF body cannot be parsed, without saving', async () => {
        ...
        }), 0)).rejects.toThrow('The current body of the page could not be read as an ADF document');
        expect(apiRequest).toHaveBeenCalledTimes(1);
    });
    ...
    it('refuses to append to a trashed page instead of restoring it', async () => {
        ...
        await expect(execute.call(mockExecuteCtx(baseParams), 0)).rejects.toThrow('The page cannot be changed because its status is "trashed"');
        expect(apiRequest).toHaveBeenCalledTimes(1);
    });
    ...
    describe('mergeAdfDocuments', () => {
        it('preserves the current document envelope and appends the content', () => {
            const merged = jsonParse(mergeAdfDocuments(JSON.stringify({ ...adfDoc([{ type: 'paragraph' }]), extra: 'kept' }), adfDoc([{ type: 'rule' }])));
        ...
        it('treats a current document without content as empty', () => {
            const merged = jsonParse(mergeAdfDocuments(JSON.stringify({ type: 'doc', version: 1 }), adfDoc([{ type: 'rule' }])));
