...
describe('page:update', () => {
    ...
    it('publishes a draft while keeping its content and single version', async () => {
        ...
        apiRequest
            ...
            .mockResolvedValueOnce({ id: '123' });
        await execute.call(mockExecuteCtx({ ...baseParams, title: '', bodyPlainText: '', status: 'current' }), 0);
        expect(apiRequest).toHaveBeenNthCalledWith(1, 'GET', '/wiki/api/v2/pages/123', {}, { 'body-format': 'storage' });
        expect(apiRequest).toHaveBeenNthCalledWith(2, 'PUT', '/wiki/api/v2/pages/123', {
            id: '123',
            status: 'current',
            title: 'Draft Title',
    ...
    it.each(['archived', 'trashed'])('refuses to write to a page whose status is %s', async (status) => {
        apiRequest.mockReset();
        apiRequest.mockResolvedValueOnce({ ...currentPage, status });
        await expect(execute.call(mockExecuteCtx(baseParams), 0)).rejects.toThrow(`The page cannot be changed because its status is "${status}"`);
    ...
    it('surfaces a stale-version 409 as a clear concurrent-edit error', async () => {
        ...
        apiRequest
            ...
            .mockRejectedValueOnce(new NodeApiError(testNode, { message: 'conflict' }, { httpCode: '409' }));
    ...
    it('resolves a By Title page reference before writing', async () => {
        ...
        apiRequest
            ...
            .mockResolvedValueOnce(currentPage)
            .mockResolvedValueOnce({ id: '123' });
        await execute.call(mockExecuteCtx({ ...baseParams, page: { mode: 'title', value: 'Old Title' } }), 0);
        expect(apiRequest).toHaveBeenNthCalledWith(1, 'GET', '/wiki/api/v2/pages', {}, { title: 'Old Title', limit: 250 });
    ...
    it('rejects a missing page without calling the API', async () => {
        await expect(execute.call(mockExecuteCtx({ ...baseParams, page: { mode: 'list', value: '' } }), 0)).rejects.toThrow("The 'Page' parameter is empty");
        expect(apiRequest).not.toHaveBeenCalled();
    });
    it('rejects an object title instead of silently keeping the current one', async () => {
        await expect(execute.call(mockExecuteCtx({ ...baseParams, title: { some: 'object' } }), 0)).rejects.toThrow('New Title must be text');
        expect(apiRequest).not.toHaveBeenCalled();
    });
    it('fails clearly when the page version cannot be read', async () => {
        ...
        await expect(execute.call(mockExecuteCtx(baseParams), 0)).rejects.toThrow('Could not read the current version of the page');
        expect(apiRequest).toHaveBeenCalledTimes(1);
    });
});
//# sourceMappingURL=update.operation.test.js.map