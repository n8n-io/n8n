...
describe('Confluence space:getMany operation', () => {
    ...
    it('follows cursors until the limit is reached and trims the overshoot', async () => {
        ...
        expect(apiRequest).toHaveBeenNthCalledWith(2, 'GET', '/wiki/api/v2/spaces', {}, { limit: 1, cursor: 'abc' });
    ...
    it('returns all spaces across pages when Return All is on', async () => {
        ...
        expect(apiRequest).toHaveBeenNthCalledWith(2, 'GET', '/wiki/api/v2/spaces', {}, { limit: 250, cursor: 'abc' });
        expect(result).toEqual(spaces(1, 3));
    });
    ...
    it('passes the description format through when the option is set', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/wiki/api/v2/spaces', {}, { 'description-format': 'view', limit: 50 });
    });
    ...
    it('rejects a non-positive limit', async () => {
        ...
        await expect(execute.call(ctx, 0)).rejects.toThrow('Limit must be a finite number of at least 1');
