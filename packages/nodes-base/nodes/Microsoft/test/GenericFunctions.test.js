...
const MAILBOX_MESSAGES = {
    required: {
        ...
        description: 'Set the Mailbox — app-only Microsoft Graph has no personal mailbox to default to.',
    },
    dotsOnly: {
...
describe('Microsoft shared validateUserTargetId', () => {
    ...
    describe('default messages (To Do "target ID" copy) at each throw site', () => {
        it('required: empty id', () => {
            expect(() => validateUserTargetId('', node)).toThrow('A target ID is required for the Service Principal');
        });
        ...
        it('invalid: bad-shape id', () => {
            ...
            expect(caught?.message).toBe('The target ID is not valid');
            expect(caught?.description).toBe('Remove any slashes, backslashes, colons, commas, or spaces and try again.');
        });
    });
