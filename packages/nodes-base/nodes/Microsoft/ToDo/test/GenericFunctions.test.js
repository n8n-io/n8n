...
import { getServicePrincipalResourceRoot, getToDoCredentialType, microsoftApiRequest, microsoftApiRequestAllItems, resolveScopeRoot, } from '../GenericFunctions';
...
describe('Microsoft ToDo GenericFunctions', () => {
    ...
    describe('microsoftApiRequest', () => {
        describe('graphApiBaseUrl from credentials', () => {
            it('should use base URL from credentials', async () => {
                ...
                expect(mockRequestOAuth2).toHaveBeenCalledWith('microsoftToDoOAuth2Api', expect.objectContaining({
                    method: 'GET',
    ...
    describe('getToDoCredentialType', () => {
        ...
        it('should return microsoftEntraServicePrincipalApi when the Service Principal is selected', () => {
            ...
            expect(getToDoCredentialType.call(mockExecuteFunctions)).toBe('microsoftEntraServicePrincipalApi');
        });
    });
    describe('Service Principal (app-only) routing', () => {
        ...
        it('rebases /me onto /users/{id} for the user target', async () => {
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/todo/lists', 0);
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.objectContaining({ uri: `${baseUrl}/v1.0/users/jane%40contoso.com/todo/lists` }));
        });
        ...
        it('uses an absolute @odata.nextLink uri verbatim (pagination bypasses scoping)', async () => {
            const nextLink = 'https://graph.microsoft.com/v1.0/users/jane%40contoso.com/todo/lists?$skiptoken=abc';
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/todo/lists', 0, {}, {}, nextLink);
        ...
        it('throws a clear error when the user target is missing', async () => {
            ...
            await expect(microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/todo/lists', 0)).rejects.toThrow('A target ID is required for the Service Principal');
        ...
        it('wraps a transport failure as NodeApiError', async () => {
            ...
            await expect(microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/todo/lists', 2)).rejects.toThrow(NodeApiError);
        });
        it('resolves the user target at the passed itemIndex', async () => {
            ...
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.objectContaining({ uri: `${baseUrl}/v1.0/users/john%40contoso.com/todo/lists` }));
        });
        it('forwards the itemIndex through microsoftApiRequestAllItems to the request', async () => {
            ...
            await microsoftApiRequestAllItems.call(mockExecuteFunctions, 'value', 'GET', '/todo/lists', 1);
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.objectContaining({ uri: `${baseUrl}/v1.0/users/john%40contoso.com/todo/lists` }));
        });
        it('paginates at the itemIndex root: page 1 scoped, page 2 via nextLink verbatim', async () => {
            ...
            const result = await microsoftApiRequestAllItems.call(mockExecuteFunctions, 'value', 'GET', '/todo/lists', 1);
            const uris = mockRequestWithAuthentication.mock.calls.map((call) => call[1].uri);
    ...
    describe('getServicePrincipalResourceRoot / validateUserTargetId', () => {
        it('builds the user root with an encoded UPN', () => {
            expect(getServicePrincipalResourceRoot('jane@contoso.com', mockNode)).toBe('/users/jane%40contoso.com');
        });
        it('accepts a GUID object id', () => {
            expect(getServicePrincipalResourceRoot('00000000-0000-0000-0000-000000000000', mockNode)).toBe('/users/00000000-0000-0000-0000-000000000000');
        });
        ...
        it("accepts a UPN containing an apostrophe (e.g. o'connor)", () => {
            expect(getServicePrincipalResourceRoot("o'connor@contoso.com", mockNode)).toBe("/users/o'connor%40contoso.com");
        });
        ...
        it('accepts a B2B guest UPN (#EXT#) and encodes it path-safely', () => {
            expect(getServicePrincipalResourceRoot('user_contoso.com#EXT#@tenant.onmicrosoft.com', mockNode)).toBe('/users/user_contoso.com%23EXT%23%40tenant.onmicrosoft.com');
        });
        ...
        it('rejects a bare host/domain (not a valid /users/{id})', () => {
            expect(() => validateUserTargetId('contoso.com', mockNode)).toThrow('The target ID is not valid');
    ...
    describe('loadOptions credential routing', () => {
        ...
        it('should default getTaskLists to the To Do credential', async () => {
            ...
            expect(loadOptionsRequestOAuth2).toHaveBeenCalledWith('microsoftToDoOAuth2Api', expect.anything());
        });
        it('routes getTaskLists through requestWithAuthentication under the Service Principal', async () => {
            ...
            mockLoadOptions.getNodeParameter.mockImplementation((name, fallback) => (name in params ? params[name] : fallback));
            await new MicrosoftToDo().methods.loadOptions.getTaskLists.call(mockLoadOptions);
            expect(mockLoadOptions.getCredentials).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi');
            expect(loadOptionsRequestWithAuth).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.objectContaining({
