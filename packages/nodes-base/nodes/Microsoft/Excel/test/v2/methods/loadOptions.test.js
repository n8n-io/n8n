...
describe('Microsoft Excel V2 - loadOptions', () => {
    let mockContext;
    const mockMicrosoftApiRequest = microsoftApiRequest;
    ...
    describe('getWorksheetColumnRow', () => {
        it('should get columns from usedRange when range is empty', async () => {
            ...
            expect(mockMicrosoftApiRequest).toHaveBeenCalledWith('GET', `/drive/items/${workbookId}/workbook/worksheets/${worksheetId}/usedRange`, undefined, { select: 'values' }, undefined, undefined, 0);
        ...
        it('should get columns from specific range when range is provided', async () => {
            ...
            mockMicrosoftApiRequest.mockResolvedValue(mockResponse);
            const result = await getWorksheetColumnRow.call(mockContext);
            expect(mockMicrosoftApiRequest).toHaveBeenCalledWith('PATCH', `/drive/items/${workbookId}/workbook/worksheets/${worksheetId}/range(address='${range}')`, { select: 'values' }, undefined, undefined, undefined, 0);
            expect(result).toEqual([
