...
import { driveEndpoint, getExcelCredentialType, getServicePrincipalResourceRoot, microsoftApiRequest, microsoftApiRequestAllItemsSkip, resolveScopeRoot, validateResourceTargetId, } from '../../transport/index';
describe('Microsoft Excel Transport', () => {
    ...
    describe('microsoftApiRequest', () => {
        describe('graphApiBaseUrl from credentials', () => {
            it('should use base URL from credentials', async () => {
                ...
                await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/workbooks', undefined, undefined, undefined, undefined, 0);
                expect(mockRequestOAuth2).toHaveBeenCalledWith('microsoftExcelOAuth2Api', expect.objectContaining({
                    method: 'GET',
        ...
        describe('authentication credential resolution', () => {
            ...
            it('should use microsoftExcelOAuth2Api when authentication is not set (backward compatibility)', async () => {
                ...
                expect(mockRequestOAuth2).toHaveBeenCalledWith('microsoftExcelOAuth2Api', expect.anything());
            });
            ...
            it('should honor graphApiBaseUrl from the generic credential (sovereign cloud)', async () => {
                ...
                await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/workbooks', undefined, undefined, undefined, undefined, 0);
                expect(mockExecuteFunctions.getCredentials).toHaveBeenCalledWith('microsoftOAuth2Api');
                expect(mockRequestOAuth2).toHaveBeenCalledWith('microsoftOAuth2Api', expect.objectContaining({
                    uri: 'https://graph.microsoft.us/v1.0/me/workbooks',
                }));
            });
        });
    ...
    describe('listSearch credential routing', () => {
        let mockLoadOptions;
        let loadOptionsRequestOAuth2;
        beforeEach(() => {
            mockLoadOptions = mockDeep();
        ...
        it('should route list-search requests through the selected generic credential', async () => {
            ...
            expect(loadOptionsRequestOAuth2).toHaveBeenCalledWith('microsoftOAuth2Api', expect.anything());
        });
        it('should default list-search requests to the Excel credential', async () => {
            ...
            expect(loadOptionsRequestOAuth2).toHaveBeenCalledWith('microsoftExcelOAuth2Api', expect.anything());
        });
        it('blocks the Service Principal list-search (drive search is unsupported app-only)', async () => {
            ...
            await expect(searchWorkbooks.call(mockLoadOptions)).rejects.toThrow('Search is not supported with the Service Principal credential');
    ...
    describe('Service Principal (app-only) routing', () => {
        ...
        const setSpParams = (overrides = {}) => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((name, _itemIndex, fallback) => (name in params ? params[name] : fallback));
        };
        beforeEach(() => {
        ...
        it('routes through requestWithAuthentication, never requestOAuth2', async () => {
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', SCOPED_RESOURCE, undefined, undefined, undefined, undefined, 0);
            expect(mockExecuteFunctions.getCredentials).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi');
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.anything());
        ...
        it('uses an absolute @odata.nextLink uri verbatim (pagination bypasses scoping)', async () => {
            const nextLink = 'https://graph.microsoft.com/v1.0/users/jane%40contoso.com/drive/root/children?$skiptoken=abc';
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/drive/root/children', {}, {}, nextLink, undefined, 0);
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.objectContaining({ uri: nextLink }));
        });
        ...
        const setSpParamsPerItem = () => {
            mockExecuteFunctions.getNodeParameter.mockImplementation((name, itemIndex, fallback) => {
                ...
                if (name === 'resourceTarget')
                    return 'user';
                if (name === 'userTarget')
                    return { value: itemIndex === 1 ? 'john@contoso.com' : 'jane@contoso.com' };
                return fallback;
            });
        };
        it('resolves the target at the passed itemIndex', async () => {
            setSpParamsPerItem();
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', SCOPED_RESOURCE, undefined, undefined, undefined, undefined, 0);
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', SCOPED_RESOURCE, undefined, undefined, undefined, undefined, 1);
            expect(mockRequestWithAuthentication).toHaveBeenNthCalledWith(1, 'microsoftEntraServicePrincipalApi', expect.objectContaining({
                uri: `${baseUrl}/v1.0/users/jane%40contoso.com/drive/items/WB/workbook/worksheets/WS/usedRange`,
            }));
            expect(mockRequestWithAuthentication).toHaveBeenNthCalledWith(2, 'microsoftEntraServicePrincipalApi', expect.objectContaining({
        ...
        it('keeps the passed itemIndex root on every $skip page (AllItemsSkip)', async () => {
            ...
            await microsoftApiRequestAllItemsSkip.call(mockExecuteFunctions, 'value', 'GET', SCOPED_RESOURCE, {}, {}, 1);
            const johnUri = `${baseUrl}/v1.0/users/john%40contoso.com/drive/items/WB/workbook/worksheets/WS/usedRange`;
            expect(mockRequestWithAuthentication).toHaveBeenNthCalledWith(1, 'microsoftEntraServicePrincipalApi', expect.objectContaining({ uri: johnUri }));
            expect(mockRequestWithAuthentication).toHaveBeenNthCalledWith(2, 'microsoftEntraServicePrincipalApi', expect.objectContaining({ uri: johnUri }));
        });
        it('throws OperationalError when a scoped resource does not start with /drive', async () => {
            await expect(microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/workbooks', undefined, undefined, undefined, undefined, 0)).rejects.toThrow(OperationalError);
        ...
        it('wraps a transport failure as NodeApiError', async () => {
            ...
            await expect(microsoftApiRequest.call(mockExecuteFunctions, 'GET', SCOPED_RESOURCE, undefined, undefined, undefined, undefined, 0)).rejects.toThrow(NodeApiError);
        });
        it('preserves method and body for a PATCH write, rebased onto the target drive', async () => {
            const body = { values: [['updated']] };
            await microsoftApiRequest.call(mockExecuteFunctions, 'PATCH', "/drive/items/WB/workbook/worksheets/WS/range(address='A1:A1')", body, undefined, undefined, undefined, 0);
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.objectContaining({
                method: 'PATCH',
                body,
        ...
        it('preserves method and body for a POST write (e.g. add worksheet / append row)', async () => {
            ...
            await microsoftApiRequest.call(mockExecuteFunctions, 'POST', '/drive/items/WB/workbook/worksheets/add', body, undefined, undefined, undefined, 0);
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.objectContaining({
                method: 'POST',
                body,
    ...
    describe('getServicePrincipalResourceRoot / validateResourceTargetId', () => {
        it('builds the user root with an encoded UPN', () => {
            expect(getServicePrincipalResourceRoot('user', 'jane@contoso.com', mockNode)).toBe('/users/jane%40contoso.com');
        });
        it('builds the drive root', () => {
            expect(getServicePrincipalResourceRoot('drive', 'b!abc-123_XYZ', mockNode)).toBe('/drives/b!abc-123_XYZ');
        });
        it('rejects an empty id', () => {
            expect(() => validateResourceTargetId('user', '', mockNode)).toThrow(NodeOperationError);
        });
        it('rejects a dots-only id', () => {
            expect(() => validateResourceTargetId('user', '..', mockNode)).toThrow('The target ID is not valid');
        });
        it('rejects a user id with a slash (path injection)', () => {
            expect(() => validateResourceTargetId('user', 'jane/../bob', mockNode)).toThrow(NodeOperationError);
        });
    ...
    describe('execute error attribution', () => {
        it('stamps context.itemIndex on a NodeError from the failing item', async () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((name, itemIndex, fallback) => {
                ...
                if (name === 'userTarget')
                    return { value: itemIndex === 1 ? 'not a user' : 'jane@contoso.com' };
