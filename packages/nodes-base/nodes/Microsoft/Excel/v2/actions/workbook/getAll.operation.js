...
import { getExcelCredentialType, microsoftApiRequest, microsoftApiRequestAllItems, } from '../../transport';
const properties = [
    {
...
export async function execute(items) {
    ...
    for (let i = 0; i < items.length; i++) {
        try {
            ...
            let responseData;
            if (returnAll) {
                responseData = await microsoftApiRequestAllItems.call(this, 'value', 'GET', "/drive/root/search(q='.xlsx')", {}, qs, i);
            }
            else {
                qs.$top = this.getNodeParameter('limit', i);
                responseData = await microsoftApiRequest.call(this, 'GET', "/drive/root/search(q='.xlsx')", {}, qs, undefined, undefined, i);
