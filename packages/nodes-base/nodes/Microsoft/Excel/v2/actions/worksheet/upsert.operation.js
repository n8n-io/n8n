...
import { checkRange, parseAddress, prepareOutput, updateByAutoMaping, updateByDefinedValues, } from '../../helpers/utils';
...
export async function execute(items) {
    ...
    try {
        ...
        if (range && dataMode !== 'raw') {
            worksheetData = await microsoftApiRequest.call(this, 'PATCH', `/drive/items/${workbookId}/workbook/worksheets/${worksheetId}/range(address='${range}')`, undefined, undefined, undefined, undefined, 0);
        }
        ...
        if (range === '') {
            ...
            worksheetData = await microsoftApiRequest.call(this, 'GET', `/drive/items/${workbookId}/workbook/worksheets/${worksheetId}/usedRange`, undefined, query, undefined, undefined, 0);
            range = worksheetData.address.split('!')[1];
        }
        let responseData;
        if (dataMode === 'raw') {
            const data = this.getNodeParameter('data', 0);
            const values = processJsonInput(data, 'Data');
            responseData = await microsoftApiRequest.call(this, 'PATCH', `/drive/items/${workbookId}/workbook/worksheets/${worksheetId}/range(address='${range}')`, { values }, undefined, undefined, undefined, 0);
        }
        if (dataMode !== 'raw' &&
            (worksheetData.values === undefined || worksheetData.values.length < 1)) {
            throw new NodeOperationError(this.getNode(), 'No data found in the specified range, mapping not possible, you can use raw mode instead to update selected range');
        }
        ...
        if (dataMode === 'define') {
            updateSummary = updateByDefinedValues.call(this, items.length, worksheetData.values, updateAll);
        }
        if (dataMode === 'autoMap') {
            const columnToMatchOn = this.getNodeParameter('columnToMatchOn', 0);
            if (!items.some(({ json }) => json[columnToMatchOn] !== undefined)) {
                throw new NodeOperationError(this.getNode(), `Any item in input data contains column '${columnToMatchOn}', that is selected to match on`);
            }
            updateSummary = updateByAutoMaping(items, worksheetData.values, columnToMatchOn, updateAll);
        }
        ...
        if (updateSummary.appendData.length) {
            ...
            if (nodeVersion > 2 && !appendAfterSelectedRange) {
                const { address } = await microsoftApiRequest.call(this, 'GET', `/drive/items/${workbookId}/workbook/worksheets/${worksheetId}/usedRange`, undefined, { select: 'address' }, undefined, undefined, 0);
                const usedRange = parseAddress(address);
                lastRow = usedRange.cellTo.row;
            }
            range = `${cellFrom.value}:${cellTo.column}${Number(lastRow) + appendValues.length}`;
        }
        responseData = await microsoftApiRequest.call(this, 'PATCH', `/drive/items/${workbookId}/workbook/worksheets/${worksheetId}/range(address='${range}')`, { values: updateSummary.updatedData }, undefined, undefined, undefined, 0);
        ...
        returnData.push(...prepareOutput.call(this, this.getNode(), responseData, {
            updatedRows,
            rawData,
            dataProperty,
        }));
