...
import { getPartitionKey, simplifyData, validateQueryParameters, processJsonInput, validatePartitionKey, validateCustomProperties, } from '../../helpers/utils';
const mockExecuteSingleFunctions = mock();
beforeEach(() => {
    vi.resetAllMocks();
    mockExecuteSingleFunctions.getNode.mockReturnValue({ name: 'MockNode' });
});
describe('getPartitionKey', () => {
    ...
    test('should throw NodeApiError for 404 error', async () => {
        ...
        const mockError = new NodeApiError(mockExecuteSingleFunctions.getNode(), {}, {
            httpCode: '404',
...
describe('simplifyData', () => {
    test('should return the same data when "simple" parameter is false', async () => {
        ...
        const result = await simplifyData.call(mockExecuteSingleFunctions, items, {});
    ...
    test('should simplify the data when "simple" parameter is true', async () => {
        ...
        const items = [{ json: { _internalKey: 'value', foo: 'bar' } }];
        const result = await simplifyData.call(mockExecuteSingleFunctions, items, {});
...
describe('validateQueryParameters', () => {
    ...
    test('should successfully map parameters when they match', async () => {
        ...
        if (result.body && result.body.parameters) {
...
describe('validateCustomProperties', () => {
    ...
    test('should throw NodeOperationError when customProperties are empty, undefined, null, or contain only invalid values', async () => {
        ...
        const emptyExecution = validateCustomProperties.call(mockExecuteSingleFunctions, requestOptions);
        ...
        const invalidExecution = validateCustomProperties.call(mockExecuteSingleFunctions, requestOptions);
