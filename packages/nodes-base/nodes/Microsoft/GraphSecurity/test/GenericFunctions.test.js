...
import { msGraphSecurityApiRequest, tolerateDoubleQuotes, throwOnEmptyUpdate, } from '../GenericFunctions';
    ...
    describe('msGraphSecurityApiRequest', () => {
        ...
        describe('successful requests', () => {
            ...
            it('should make a POST request with body data', async () => {
                ...
                mockRequest.mockResolvedValue(mockResponse);
                const result = await msGraphSecurityApiRequest.call(mockExecuteFunctions, 'POST', '/alerts', requestBody);
                expect(mockRequest).toHaveBeenCalledWith({
                    headers: {
                        Authorization: 'Bearer test-access-token',
                    },
            ...
            it('should make a request with query string parameters', async () => {
                ...
                mockRequest.mockResolvedValue(mockResponse);
                const result = await msGraphSecurityApiRequest.call(mockExecuteFunctions, 'GET', '/alerts', {}, queryParams);
                expect(mockRequest).toHaveBeenCalledWith({
                    headers: {
                        Authorization: 'Bearer test-access-token',
                    },
            ...
            it('should make a request with custom headers', async () => {
                ...
                mockRequest.mockResolvedValue(mockResponse);
                const result = await msGraphSecurityApiRequest.call(mockExecuteFunctions, 'PUT', '/secureScores', { data: 'test' }, {}, customHeaders);
                expect(mockRequest).toHaveBeenCalledWith({
                    headers: {
                        Authorization: 'Bearer test-access-token',
            ...
            it('should handle all parameters together', async () => {
                ...
                mockRequest.mockResolvedValue(mockResponse);
                const result = await msGraphSecurityApiRequest.call(mockExecuteFunctions, 'PATCH', '/alerts/123', body, qs, headers);
                expect(mockRequest).toHaveBeenCalledWith({
                    headers: {
                        Authorization: 'Bearer test-access-token',
                        'If-Match': 'etag-value',
        ...
        describe('credential handling', () => {
            it('propagates credential-resolution failures without issuing a request', async () => {
                ...
                await expect(msGraphSecurityApiRequest.call(mockExecuteFunctions, 'GET', '/alerts')).rejects.toThrow('Credentials not found');
                expect(mockExecuteFunctions.getCredentials).toHaveBeenCalledTimes(1);
                expect(mockExecuteFunctions.getCredentials).toHaveBeenCalledWith('microsoftGraphSecurityOAuth2Api');
            ...
            it('throws a NodeOperationError when the credential has no access token', async () => {
                ...
                await expect(msGraphSecurityApiRequest.call(mockExecuteFunctions, 'GET', '/alerts')).rejects.toThrow(NodeOperationError);
                await expect(msGraphSecurityApiRequest.call(mockExecuteFunctions, 'GET', '/alerts')).rejects.toThrow('No access token found in the credential');
                expect(mockRequest).not.toHaveBeenCalled();
            });
        });
        describe('error handling', () => {
            it('should handle basic API errors', async () => {
                ...
                await expect(msGraphSecurityApiRequest.call(mockExecuteFunctions, 'GET', '/alerts/invalid-id')).rejects.toThrow(NodeApiError);
            });
            it('should parse JSON error messages', async () => {
                ...
                mockRequest.mockRejectedValue(apiError);
                await expect(msGraphSecurityApiRequest.call(mockExecuteFunctions, 'GET', '/alerts')).rejects.toThrow(NodeApiError);
            });
            it('should handle BadRequest errors', async () => {
                ...
                mockRequest.mockRejectedValue(apiError);
                const thrown = await msGraphSecurityApiRequest
                    .call(mockExecuteFunctions, 'GET', '/alerts')
                    .catch((error) => error);
                expect(thrown).toBeInstanceOf(NodeApiError);
                expect(thrown.message).toBe('Request failed with bad request');
            });
            ...
            it('should handle Invalid filter clause errors', async () => {
                ...
                const thrown = await msGraphSecurityApiRequest
                    ...
                    .catch((error) => error);
                expect(thrown).toBeInstanceOf(NodeApiError);
                expect(thrown.message).toContain('Please check that your query parameter syntax is correct');
            });
        ...
        describe('graphApiBaseUrl from credentials', () => {
            ...
            describe('generic OAuth2 branch', () => {
                beforeEach(() => {
                    mockExecuteFunctions.getNodeParameter.mockImplementation((name, _i, fallback) => name === 'authentication' ? 'microsoftOAuth2Api' : fallback);
                });
                it('should use US Government cloud endpoint', async () => {
                    const mockResponse = { data: 'test' };
                    mockRequest.mockResolvedValue(mockResponse);
        ...
        describe('credential resolution', () => {
            ...
            it('should use the legacy credential when authentication is microsoftGraphSecurityOAuth2Api', async () => {
                mockExecuteFunctions.getNodeParameter.mockImplementation((name, _i, fallback) => name === 'authentication' ? 'microsoftGraphSecurityOAuth2Api' : fallback);
                mockRequest.mockResolvedValue({ data: 'test' });
                await msGraphSecurityApiRequest.call(mockExecuteFunctions, 'GET', '/alerts');
                expect(mockExecuteFunctions.getCredentials).toHaveBeenCalledTimes(1);
                expect(mockExecuteFunctions.getCredentials).toHaveBeenCalledWith('microsoftGraphSecurityOAuth2Api');
            ...
            it('resolves the legacy credential for a saved node with no authentication param', async () => {
                mockRequest.mockResolvedValue({ data: 'test' });
                await expect(msGraphSecurityApiRequest.call(mockExecuteFunctions, 'GET', '/alerts')).resolves.not.toThrow();
                expect(mockExecuteFunctions.getCredentials).toHaveBeenCalledWith('microsoftGraphSecurityOAuth2Api');
            });
            it('resolves the legacy credential for any unrecognized authentication value', async () => {
                mockExecuteFunctions.getNodeParameter.mockImplementation((name, _i, fallback) => name === 'authentication' ? 'somethingElse' : fallback);
                mockRequest.mockResolvedValue({ data: 'test' });
                await msGraphSecurityApiRequest.call(mockExecuteFunctions, 'GET', '/alerts');
                expect(mockExecuteFunctions.getCredentials).toHaveBeenCalledWith('microsoftGraphSecurityOAuth2Api');
            });
            it('resolves the legacy credential when the resolved authentication value is undefined', async () => {
                mockExecuteFunctions.getNodeParameter.mockImplementation((name, _i, fallback) => name === 'authentication' ? undefined : fallback);
                mockRequest.mockResolvedValue({ data: 'test' });
                await msGraphSecurityApiRequest.call(mockExecuteFunctions, 'GET', '/alerts');
                expect(mockExecuteFunctions.getCredentials).toHaveBeenCalledWith('microsoftGraphSecurityOAuth2Api');
            });
