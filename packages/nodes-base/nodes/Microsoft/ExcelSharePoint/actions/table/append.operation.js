...
import { autoMapRow, defineRow } from '../../helpers/dataModes';
        ...
        options: [
            {
                ...
                description: "Specifies the relative position of the new row. If not defined, the addition happens at the end. Any row below the inserted row will be shifted downwards. First row index is 0. An index higher than the table's current row count will fail.",
            },
            {
...
export async function execute(items) {
    ...
    try {
        ...
        const tableId = validatePathSegment(this.getNode(), 'Table', this.getNodeParameter('table', 0, '', { extractValue: true }));
        ...
        let values;
        if (dataMode === 'raw') {
            values = processJsonInput(this.getNodeParameter('data', 0), 'Data');
        }
        else if (dataMode === 'autoMap') {
            values = items.map((item) => autoMapRow(item.json, columnsRow));
        }
        else {
            values = items.map((_, itemIndex) => defineRow(this.getNodeParameter('fieldsUi.values', itemIndex, []), columnsRow));
        }
        ...
        let responseData;
        try {
            responseData = await (withWorkbookSession).call(this, workbookRoot, async (headers) => await (microsoftApiRequest).call(this, 'POST', `${tableEndpoint}/rows/add`, body, {}, undefined, headers));
        }
        catch (error) {
