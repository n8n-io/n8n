...
describe('Microsoft Excel (SharePoint) — Table: Get Rows, Get Columns, Lookup', () => {
    ...
    const mockRowsAndColumns = (rows = ROWS) => {
        apiRequest.mockResolvedValue({ value: rows });
        apiRequestAllItems.mockImplementation(async (endpoint) => endpoint.endsWith('/columns') ? COLUMNS : rows);
    };
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    describe('getRows', () => {
        ...
        it('passes the reply through untouched under the data property in RAW mode', async () => {
            ...
            apiRequest.mockResolvedValue({ value: ROWS });
            const result = await node.execute.call(ctx);
            expect(apiRequest).toHaveBeenCalledWith('GET', `${TABLE_ENDPOINT}/rows`, {}, { $select: 'index,values', $top: 100 });
    ...
    describe('lookup', () => {
        ...
        it('names the missing column when it does not exist on the table', async () => {
            ...
            mockRowsAndColumns();
            await expect(node.execute.call(ctx)).rejects.toThrow('Column Phone does not exist on the table selected');
        });
    });
});
