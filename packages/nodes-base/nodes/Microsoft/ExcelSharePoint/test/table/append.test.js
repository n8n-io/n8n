...
describe('Microsoft Excel (SharePoint) — Table: Append', () => {
    ...
    const mockColumnsAndSession = () => {
        ...
        apiRequest.mockImplementation(async (_method, resource) => {
    ...
    it('auto-maps input onto the table columns and writes inside a session, in order', async () => {
        ...
        const calls = apiRequest.mock.calls.map((call) => String(call[1]));
        expect(calls).toEqual([
            `${WORKBOOK_ROOT}/workbook/createSession`,
            `${TABLE_ENDPOINT}/rows/add`,
            `${WORKBOOK_ROOT}/workbook/closeSession`,
        ]);
        expect(apiRequest).toHaveBeenCalledWith('POST', `${TABLE_ENDPOINT}/rows/add`, {
            values: [
                ['Frank', 'frank@example.com'],
                ['Dana', null],
            ],
        }, {}, undefined, SESSION_HEADER);
    ...
    it('maps defined fields per item onto the columns, matching the OneDrive node', async () => {
        ...
        const params = { ...baseParams, dataMode: 'define' };
        ctx.getNodeParameter.mockImplementation((name, itemIndex, fallback) => (name === 'fieldsUi.values'
    ...
    it('sends the row-position index when set', async () => {
        ...
        mockColumnsAndSession();
        await node.execute.call(ctx);
        expect(apiRequest).toHaveBeenCalledWith('POST', `${TABLE_ENDPOINT}/rows/add`, expect.objectContaining({ index: 2 }), {}, undefined, SESSION_HEADER);
    });
    it('sends index 0 so the row is inserted as the first row', async () => {
        ...
        mockColumnsAndSession();
        await node.execute.call(ctx);
        expect(apiRequest).toHaveBeenCalledWith('POST', `${TABLE_ENDPOINT}/rows/add`, expect.objectContaining({ index: 0 }), {}, undefined, SESSION_HEADER);
    });
    it('names the Index option when Graph rejects an out-of-range insert position', async () => {
        ...
        apiRequest.mockImplementation(async (_method, resource) => {
            if (resource.endsWith('/createSession'))
                return { id: 'session-1' };
            if (resource.endsWith('/rows/add')) {
                throw new NodeApiError(mock(), { message: graphMessage }, { message: graphMessage, httpCode: '400' });
            }
            return {};
        });
        await expect(node.execute.call(ctx)).rejects.toMatchObject({
            message: graphMessage,
            description: expect.stringContaining("the 'Index' option (50) is higher than the table's current row count"),
        });
    });
    ...
    it('closes the session and keeps the run alive when the write fails under continue-on-fail', async () => {
        ...
        const result = await node.execute.call(ctx);
        expect(apiRequest).toHaveBeenCalledWith('POST', `${WORKBOOK_ROOT}/workbook/closeSession`, {}, {}, undefined, SESSION_HEADER);
