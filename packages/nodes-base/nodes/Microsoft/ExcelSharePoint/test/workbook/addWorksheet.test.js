...
describe('Microsoft Excel (SharePoint) — Workbook: Add Sheet', () => {
    ...
    it('opens a session, adds the sheet with the session header, then closes the session', async () => {
        ...
        apiRequest.mockImplementation(async (_method, resource) => {
            if (resource === `${WORKBOOK_ROOT}/workbook/createSession`)
                return { id: 'session-abc' };
            if (resource === `${WORKBOOK_ROOT}/workbook/worksheets/add`)
                return { id: 'sheet-1', name: 'Sheet2' };
            if (resource === `${WORKBOOK_ROOT}/workbook/closeSession`)
                return {};
            throw new Error(`unexpected request: ${resource}`);
        });
        const result = await node.execute.call(ctx);
        expect(apiRequest).toHaveBeenCalledTimes(3);
        expect(apiRequest).toHaveBeenNthCalledWith(1, 'POST', `${WORKBOOK_ROOT}/workbook/createSession`, { persistChanges: true });
        expect(apiRequest).toHaveBeenNthCalledWith(2, 'POST', `${WORKBOOK_ROOT}/workbook/worksheets/add`, {}, {}, undefined, { 'workbook-session-id': 'session-abc' });
        expect(apiRequest).toHaveBeenNthCalledWith(3, 'POST', `${WORKBOOK_ROOT}/workbook/closeSession`, {}, {}, undefined, { 'workbook-session-id': 'session-abc' });
    ...
    it('sends the chosen name in the request body', async () => {
        ...
        expect(apiRequest).toHaveBeenNthCalledWith(2, 'POST', `${WORKBOOK_ROOT}/workbook/worksheets/add`, { name: 'Q4' }, {}, undefined, { 'workbook-session-id': 'session-abc' });
    });
    it('still closes the session when adding the sheet fails', async () => {
        ...
        apiRequest.mockImplementation(async (_method, resource) => {
            if (resource.endsWith('/createSession'))
                return { id: 'session-abc' };
            if (resource.endsWith('/worksheets/add'))
...
//# sourceMappingURL=addWorksheet.test.js.map