...
describe('Microsoft Excel (SharePoint) Error Handler', () => {
    ...
    describe('servicePrincipalApiError', () => {
        ...
        it('falls back to a generic message when the permission is not in the lookup table', () => {
            ...
            expect(error.message).toBe('The app registration is missing a consented application permission for this operation. Grant the required Microsoft Graph application permission and admin consent, then retry.');
        });
    });
    describe('delegatedApiError', () => {
        it('falls back to a generic message when the permission is not in the lookup table', () => {
            ...
            expect(error.message).toBe('Microsoft Graph refused this request. The credential may be missing a required permission, or the signed-in account may not have access to this resource');
        });
        it('replaces a nested ItemNotFound error with the not-found message', () => {
            ...
            expect(error.message).toBe('The requested resource was not found. Check the Site, Library, Workbook, and Sheet values.');
        });
        it('replaces the not-found message when the error arrives already wrapped in a NodeApiError', () => {
            ...
            const wrapped = new NodeApiError(ctx.getNode(), rawHttpClientError);
        ...
        it('passes the status code through with no custom message when neither 403 nor nested', () => {
            ...
            expect(error.message).not.toBe('The requested resource was not found. Check the Site, Library, Workbook, and Sheet values.');
        });
