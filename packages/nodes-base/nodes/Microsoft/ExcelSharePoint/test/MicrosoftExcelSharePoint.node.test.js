...
describe('MicrosoftExcelSharePoint', () => {
    ...
    it('describes every operation for a model choosing between tools', () => {
        const operationProperties = node.description.properties.filter((property) => property.name === 'operation');
        ...
        for (const property of operationProperties) {
            for (const option of property.options ?? []) {
                ...
                expect(option.action, `operation ${String(option.value)} action`).toMatch(/\w+/);
            }
        }
    });
});
//# sourceMappingURL=MicrosoftExcelSharePoint.node.test.js.map