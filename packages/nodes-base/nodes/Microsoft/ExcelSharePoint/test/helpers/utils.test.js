...
describe('Microsoft Excel (SharePoint) — helpers/utils', () => {
    describe('validatePathSegment', () => {
        ...
        it('returns the trimmed value', () => {
            expect(validatePathSegment(node, 'Site', '  contoso.sharepoint.com  ')).toBe('contoso.sharepoint.com');
        });
        it('rejects an empty value', () => {
            expect(() => validatePathSegment(node, 'Site', '')).toThrow("The 'Site' parameter is empty");
        });
        it('rejects a whitespace-only value', () => {
            expect(() => validatePathSegment(node, 'Library', '   ')).toThrow("The 'Library' parameter is empty");
        });
        it.each(['.', '..', '...'])('rejects a dots-only value (%s)', (value) => {
            expect(() => validatePathSegment(node, 'Workbook', value)).toThrow("The 'Workbook' value is not valid");
        });
    ...
    describe('resolveWorkbookRoot', () => {
        ...
        const setParams = (ctxToSet, params) => {
            ctxToSet.getNodeParameter.mockImplementation((name, _itemIndex, fallback) => (name in params ? params[name] : fallback));
        };
        beforeEach(() => {
            vi.clearAllMocks();
            ctx = mockDeep();
            ctx.getNode.mockReturnValue(mock());
        });
        describe('by ID', () => {
            it('builds the root path from site, library, and workbook IDs, URL-encoding each segment', async () => {
                ...
                expect(root).toBe('/v1.0/sites/contoso.sharepoint.com%2Cg1%2Cg2/drives/b!drive%201/items/ITEM%20123');
            ...
            it('rejects an empty Site', async () => {
                ...
                await expect(resolveWorkbookRoot.call(ctx, 0)).rejects.toThrow("The 'Site' parameter is empty");
            });
            it('rejects an empty Library', async () => {
                ...
                await expect(resolveWorkbookRoot.call(ctx, 0)).rejects.toThrow("The 'Library' parameter is empty");
            });
            it('rejects a dots-only Workbook ID', async () => {
                ...
                await expect(resolveWorkbookRoot.call(ctx, 0)).rejects.toThrow("The 'Workbook' value is not valid");
            });
            it('resolves a pasted Site address before building the root path', async () => {
                ...
                const root = await resolveWorkbookRoot.call(ctx, 0);
                expect(apiRequest).toHaveBeenCalledWith('GET', '/v1.0/sites/contoso.sharepoint.com:/sites/mysite', {}, { $select: 'id' });
                expect(root).toBe('/v1.0/sites/contoso.sharepoint.com%2Cg1%2Cg2/drives/b!drive1/items/ITEM123');
            });
        });
        describe('by URL', () => {
            it('rejects an empty pasted address', async () => {
                setParams(ctx, { workbook: { mode: 'url', value: '' } });
                await expect(resolveWorkbookRoot.call(ctx, 0)).rejects.toThrow("The 'Workbook' parameter is empty");
                expect(apiRequest).not.toHaveBeenCalled();
            });
            it('resolves the pasted address via the Graph shares endpoint', async () => {
                ...
                const root = await resolveWorkbookRoot.call(ctx, 0);
                expect(apiRequest).toHaveBeenCalledWith('GET', expect.stringMatching(/^\/v1\.0\/shares\/u!/), {}, { $select: 'id,parentReference' });
                expect(root).toBe('/v1.0/sites/contoso.sharepoint.com%2Cg1%2Cg2/drives/b!drive1/items/ITEM123');
            });
            ...
            it('rejects when the resolved item is missing a parent site or drive', async () => {
                ...
                await expect(resolveWorkbookRoot.call(ctx, 0)).rejects.toThrow('The workbook address could not be resolved');
            });
    ...
    describe('resolveSiteId', () => {
        ...
        it('resolves the root site when the address has no path', async () => {
            ...
            expect(apiRequest).toHaveBeenCalledWith('GET', '/v1.0/sites/contoso.sharepoint.com', {}, { $select: 'id' });
        });
        it('caches the resolution per execution, across items, when the caller shares its cache', async () => {
            setParams(ctx, {
                site: { mode: 'url', value: 'https://contoso.sharepoint.com/sites/mysite' },
        ...
        it('reports a not-found address as a Site problem, not the generic message', async () => {
            ...
            apiRequest.mockRejectedValue(new NodeApiError(mock(), { message: 'x' }, { httpCode: '404' }));
        ...
        it('lets other errors bubble up unchanged', async () => {
            ...
            apiRequest.mockRejectedValue(new NodeApiError(mock(), { message: 'x' }, { httpCode: '500' }));
        ...
        it('works from a load-options context, for the library dropdown', async () => {
            const loadOptionsCtx = mockDeep();
            loadOptionsCtx.getNode.mockReturnValue(mock());
            setParams(loadOptionsCtx, { site: { mode: 'id', value: 'contoso.sharepoint.com,g1,g2' } });
            await expect(resolveSiteId.call(loadOptionsCtx, 0)).resolves.toBe('contoso.sharepoint.com,g1,g2');
        });
    });
});
