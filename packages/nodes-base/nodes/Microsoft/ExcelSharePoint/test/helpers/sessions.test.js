...
describe('Microsoft Excel (SharePoint) — helpers/sessions', () => {
    ...
    it('opens a session, runs send with the session header, then closes the session', async () => {
        apiRequest.mockImplementation(async (_method, resource) => {
            if (resource === `${workbookRoot}/workbook/createSession`)
        ...
        expect(apiRequest).toHaveBeenNthCalledWith(1, 'POST', `${workbookRoot}/workbook/createSession`, { persistChanges: true });
        expect(apiRequest).toHaveBeenNthCalledWith(2, 'POST', `${workbookRoot}/workbook/closeSession`, {}, {}, undefined, { 'workbook-session-id': 'session-abc' });
    });
    ...
    it('does not attempt to close a session that never opened', async () => {
        ...
        const send = vi.fn();
        await expect(withWorkbookSession.call(ctx, workbookRoot, send)).rejects.toThrow('createSession failed');
