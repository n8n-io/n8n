...
describe('Microsoft Excel (SharePoint) — Sheet: Clear', () => {
    ...
    it('clears the whole worksheet when no range is selected', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('POST', `${WORKBOOK_ROOT}/workbook/worksheets/Sheet1/range/clear`, { applyTo: 'All' });
    ...
    it('clears a specific range when one is selected', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('POST', `${WORKBOOK_ROOT}/workbook/worksheets/Sheet1/range(address='${encodeURIComponent('A1:B2')}')/clear`, { applyTo: 'All' });
    });
    ...
    it.each(['All', 'Formats', 'Contents'])('sends the %s Apply To choice to the API', async (applyTo) => {
        setParams({ ...byIdParams, applyTo });
        apiRequest.mockResolvedValue({});
        await node.execute.call(ctx);
        expect(apiRequest).toHaveBeenCalledWith('POST', `${WORKBOOK_ROOT}/workbook/worksheets/Sheet1/range/clear`, { applyTo });
    });
