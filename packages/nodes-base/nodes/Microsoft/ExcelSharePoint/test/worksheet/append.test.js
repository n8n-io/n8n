...
describe('Microsoft Excel (SharePoint) — Sheet: Append', () => {
    ...
    it('rejects RAW data that is not an array of arrays of strings', async () => {
        ...
        await expect(node.execute.call(ctx)).rejects.toThrow('Data must be an array of arrays of strings');
    });
    ...
    it('keeps later items running when continue-on-fail is on', async () => {
        ...
        ctx.getNodeParameter.mockImplementation((name, itemIndex, fallback) => {
            if (name === 'fieldsUi.values' && itemIndex === 0)
                throw new Error('boom');
            if (name === 'fieldsUi.values')
                return [{ column: 'name', fieldValue: 'x' }];
