...
describe('Microsoft Excel (SharePoint) — Sheet: Get Rows', () => {
    ...
    it('reads the used range for site, library, and workbook IDs, parsing rows into objects', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', `${WORKBOOK_ROOT}/workbook/worksheets/Sheet1/usedRange`, {}, {});
    ...
    it('reads a specific range when one is selected', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', `${WORKBOOK_ROOT}/workbook/worksheets/Sheet1/range(address='${encodeURIComponent('A1:B2')}')`, {}, {});
    });
    ...
    it('resolves a pasted workbook address once per execution across items', async () => {
        ...
        apiRequest.mockImplementation(async (_method, resource) => resource.startsWith('/v1.0/shares/')
        ...
        expect(apiRequest).toHaveBeenNthCalledWith(1, 'GET', '/v1.0/shares/u!aHR0cHM6Ly9jb250b3NvLnNoYXJlcG9pbnQuY29tL3NpdGVzL3NpdGUxL1NoYXJlZCBEb2N1bWVudHMvUTQgfiByZXBvcnQ_Lnhsc3g/driveItem', {}, { $select: 'id,parentReference' });
    ...
    it('requests only the chosen fields for RAW output', async () => {
        ...
        apiRequest.mockResolvedValue({ ...USED_RANGE_REPLY });
        const result = await node.execute.call(ctx);
        expect(apiRequest).toHaveBeenCalledWith('GET', `${WORKBOOK_ROOT}/workbook/worksheets/Sheet1/usedRange`, {}, { $select: 'values,address' });
    ...
    it('throws a clear error for an unknown operation', async () => {
        ...
        await expect(node.execute.call(ctx)).rejects.toThrow('The operation "appendRows" is not supported!');
        expect(apiRequest).not.toHaveBeenCalled();
    });
});
