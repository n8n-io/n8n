...
import { getSheets, getTables, searchLibraries, searchSites, searchWorkbooks, } from '../../methods/listSearch';
...
describe('Microsoft Excel (SharePoint) — dropdown search methods', () => {
    ...
    describe('searchSites', () => {
        it('searches sites by name using the literal `search` parameter', async () => {
            ...
            expect(apiRequest).toHaveBeenCalledWith('GET', '/v1.0/sites', {}, { search: 'marketing', $select: 'id,displayName,webUrl' });
        ...
        it('lists all sites when no search text is given', async () => {
            ...
            expect(apiRequest).toHaveBeenCalledWith('GET', '/v1.0/sites', {}, { search: '*', $select: 'id,displayName,webUrl' });
        });
        ...
        it('keeps the permission-naming message for delegated refusals', async () => {
            ...
            apiRequest.mockRejectedValue(new NodeApiError(mock(), { message: 'refused' }, {
                httpCode: '403',
        ...
        it('lets a non-403 failure bubble up unchanged', async () => {
            setParams({ authentication: SERVICE_PRINCIPAL_AUTH });
            apiRequest.mockRejectedValue(new NodeApiError(mock(), {}, { httpCode: '500', message: 'boom' }));
    ...
    describe('searchLibraries', () => {
        it("lists the chosen site's document libraries", async () => {
            ...
            expect(apiRequest).toHaveBeenCalledWith('GET', '/v1.0/sites/contoso.sharepoint.com%2Cg1%2Cg2/drives', {}, { $select: 'id,name,webUrl' });
            expect(result.results).toEqual([
    ...
    describe('searchWorkbooks', () => {
        ...
        const searchPath = (q) => `/v1.0/sites/${encodeURIComponent(SITE_ID)}/drives/b!drive1/root/search(q='${q}')`;
        ...
        it('lists workbooks signed in as an app (Service Principal)', async () => {
            const { microsoftApiRequest: realApiRequest } = await vi.importActual('../../transport');
            ...
            const result = await searchWorkbooks.call(ctx);
            expect(ctx.helpers.httpRequestWithAuthentication).toHaveBeenCalledWith(SERVICE_PRINCIPAL_AUTH, expect.objectContaining({ url: expect.stringContaining("/root/search(q='") }));
            expect(result.results).toEqual([BOOK_ITEM]);
        });
    });
    describe('getSheets', () => {
        it('lists the sheets in the workbook', async () => {
            ...
            expect(apiRequest).toHaveBeenCalledWith('GET', `${WORKBOOK_ROOT}/workbook/worksheets`, {}, { $top: 100 });
            expect(result.results).toEqual([
        ...
        it('keeps paging on its own while filtering until a page has a match', async () => {
            ...
            const nextLink = 'https://graph.microsoft.com/v1.0/sites/s/drives/d/items/i/workbook/worksheets?$skiptoken=p2';
            setParams(byIdParams);
            apiRequest
        ...
        it('follows a pagination token verbatim, without rebuilding the request', async () => {
            const nextLink = 'https://graph.microsoft.com/v1.0/sites/s/drives/d/items/i/workbook/worksheets?$skiptoken=abc';
            setParams(byIdParams);
        ...
        it('surfaces the next @odata.nextLink as the pagination token', async () => {
            const nextLink = 'https://graph.microsoft.com/v1.0/sites/s/drives/d/items/i/workbook/worksheets?$skiptoken=def';
            setParams(byIdParams);
    ...
    describe('getTables', () => {
        ...
        it('lists the tables in the chosen sheet', async () => {
            ...
            expect(apiRequest).toHaveBeenCalledWith('GET', `${WORKBOOK_ROOT}/workbook/worksheets/Sheet1/tables`, {}, { $top: 100 });
            expect(result.results).toEqual([
