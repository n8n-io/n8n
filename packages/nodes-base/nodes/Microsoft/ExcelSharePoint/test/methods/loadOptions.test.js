...
import { getTableColumns, getWorksheetColumnRow, getWorksheetColumnRowSkipColumnToMatchOn, } from '../../methods/loadOptions';
...
describe('Microsoft Excel (SharePoint) — column loadOptions', () => {
    ...
    const setParams = (params) => {
        ctx.getNodeParameter.mockImplementation(((name, fallback, options) => {
    ...
    it("offers the used range's header row when no range is set", async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', `${SHEET_PATH}/usedRange`, {}, {
            $select: 'values',
        });
    ...
    it("reads only the given range's first row when a range is set", async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', `${SHEET_PATH}/range(address='A1:C1')`, {}, {
            $select: 'values',
        });
    });
    ...
    it("offers the table's columns for the values-to-send dropdown, extracting the table locator", async () => {
        const apiRequestAllItems = transport.microsoftApiRequestAllItems;
        ...
        expect(apiRequestAllItems).toHaveBeenCalledWith(`${WORKBOOK_ROOT}/workbook/tables/Table1/columns`, {
            $select: 'name',
        });
        expect(result).toEqual([
            { name: 'Name', value: 'Name' },
    ...
    it('is wired into the node as load-options methods', () => {
        ...
        expect(node.methods?.loadOptions?.getWorksheetColumnRowSkipColumnToMatchOn).toBe(getWorksheetColumnRowSkipColumnToMatchOn);
    });
});
