...
describe('Microsoft Excel (SharePoint) Transport', () => {
    ...
    it.each([
        ...
        expect(mockHttpRequestWithAuthentication).toHaveBeenCalledWith('microsoftOAuth2Api', expect.objectContaining({ url: `${baseUrl}/v1.0/sites/s/drives/d/items/i` }));
    });
    it('falls back to the global cloud when the credential does not say', async () => {
        ...
        expect(mockHttpRequestWithAuthentication).toHaveBeenCalledWith('microsoftOAuth2Api', expect.objectContaining({ url: 'https://graph.microsoft.com/v1.0/sites/s' }));
    });
    it('sends Service Principal requests with its own credential type', async () => {
        ...
        expect(mockHttpRequestWithAuthentication).toHaveBeenCalledWith(SERVICE_PRINCIPAL_AUTH, expect.anything());
    });
    it('uses an explicit uri verbatim', async () => {
        ...
        expect(mockHttpRequestWithAuthentication).toHaveBeenCalledWith('microsoftOAuth2Api', expect.objectContaining({ url: nextLink }));
    });
    it('names the missing delegated permission on a refusal', async () => {
        ...
        await expect(microsoftApiRequest.call(ctx, 'GET', '/v1.0/sites/s')).rejects.toThrow(/Sites\.Read\.All/);
    });
    it('names the missing application permission on a refusal under the Service Principal', async () => {
        ...
        await expect(microsoftApiRequest.call(ctx, 'GET', '/v1.0/sites/s')).rejects.toThrow(/Sites\.Read\.All \(or Sites\.Selected/);
    });
    it('reports a plain not-found under the Service Principal without blaming one field', async () => {
        ...
        await expect(microsoftApiRequest.call(ctx, 'GET', '/v1.0/sites/s')).rejects.toThrow('The requested resource was not found. Check the Site, Library, Workbook, and Sheet values.');
    });
    it('reports a network failure as unreachable, not as a rejection', async () => {
        ...
        await expect(microsoftApiRequest.call(ctx, 'GET', '/v1.0/sites/s')).rejects.toThrow('Could not reach Microsoft Graph. Check your network connection and try again.');
    });
    ...
    describe('microsoftApiRequestAllItems', () => {
        it('returns every item from a single page', async () => {
            ...
            expect(mockHttpRequestWithAuthentication).toHaveBeenCalledWith('microsoftOAuth2Api', expect.objectContaining({
        ...
        it('accumulates every page, following @odata.nextLink verbatim on later pages', async () => {
            ...
            expect(mockHttpRequestWithAuthentication).toHaveBeenNthCalledWith(1, 'microsoftOAuth2Api', expect.objectContaining({
                url: 'https://graph.microsoft.com/v1.0/sites/s/workbook/tables',
                qs: { $top: 100 },
            }));
            // The 2nd page is requested via the exact nextLink, with no rebuilt query params
            expect(mockHttpRequestWithAuthentication).toHaveBeenNthCalledWith(2, 'microsoftOAuth2Api', expect.objectContaining({ url: nextLink, qs: {} }));
        });
    });
});
