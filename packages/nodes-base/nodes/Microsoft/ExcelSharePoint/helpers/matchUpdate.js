...
import { checkRange, parseAddress, prepareOutput, updateByAutoMaping, updateByDefinedValues, } from '../../Excel/v2/helpers/utils';
 ...
 */
export async function executeMatchUpdate(items, appendUnmatched) {
    const returnData = [];
    try {
        ...
        let range = this.getNodeParameter('range', 0, '').trim();
        checkRange(this.getNode(), range);
        const dataMode = this.getNodeParameter('dataMode', 0);
        let worksheetData = {};
        if (range !== '' && dataMode !== 'raw') {
            worksheetData = await microsoftApiRequest.call(this, 'GET', `${sheetPath}/range(address='${range}')`);
        }
        if (range === '') {
            const qs = {};
            if (dataMode === 'raw') {
        ...
        if (dataMode === 'raw') {
            const rawRows = processJsonInput(this.getNodeParameter('data', 0), 'Data');
            const rawQs = {};
            if (rawData && options.fields) {
                rawQs.$select = options.fields;
            }
            const responseData = await (microsoftApiRequest).call(this, 'PATCH', `${sheetPath}/range(address='${range}')`, { values: rawRows }, rawQs);
            returnData.push(...prepareOutput.call(this, this.getNode(), responseData, { rawData, dataProperty }));
            return returnData;
        }
        const sheetValues = worksheetData.values;
        ...
        let updateSummary = { updatedData: [], updatedRows: [], appendData: [] };
        if (dataMode === 'define') {
        ...
        if (appendUnmatched && updateSummary.appendData.length) {
            ...
            for (const [index, item] of updateSummary.appendData.entries()) {
                appendValues.push(columnsRow.map((column) => item[column]));
        ...
        const responseData = await (microsoftApiRequest).call(this, 'PATCH', `${sheetPath}/range(address='${range}')`, { values: updateSummary.updatedData }, patchQs);
        ...
        returnData.push(...this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray({ error: message }), {
            itemData: generatePairedItemData(items.length),
        }));
    }
    return returnData;
}
//# sourceMappingURL=matchUpdate.js.map