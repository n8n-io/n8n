 ...
 */
export function validatePathSegment(node, label, value) {
 ...
 */
export async function resolveWorkbookRoot(itemIndex = 0, workbookRootCache = new Map(), siteIdCache = new Map()) {
    const workbook = this.getNodeParameter('workbook', itemIndex);
    ...
    if (workbook.mode === 'url') {
        ...
        const shareId = `u!${Buffer.from(workbookValue)
            ...
            .replace(/\+/g, '-')
            .replace(/\//g, '_')}`;
        const driveItem = await microsoftApiRequest.call(this, 'GET', `/v1.0/shares/${shareId}/driveItem`, {}, { $select: 'id,parentReference' });
        const itemId = String(driveItem.id ?? '');
        const parent = driveItem.parentReference;
    ...
    const driveId = validatePathSegment(node, 'Library', String(this.getNodeParameter('library', itemIndex).value ?? ''));
 ...
 */
export async function resolveSiteId(itemIndex, cache = new Map()) {
    const site = this.getNodeParameter('site', itemIndex);
    ...
    const endpoint = path === '' ? `/v1.0/sites/${parsed.hostname}` : `/v1.0/sites/${parsed.hostname}:${path}`;
    ...
    try {
        ...
        if (error instanceof NodeApiError && error.httpCode === '404') {
            throw new NodeOperationError(this.getNode(), 'Site not found', {
                description: "Check the value in the 'Site' parameter — the address must point to an existing SharePoint site.",
            });
        }
        throw error;
    }
...
export async function runPerItem(items, produce) {
    const returnData = [];
    for (let i = 0; i < items.length; i++) {
        try {
            const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(await produce(i)), { itemData: { item: i } });
...
export async function fetchCollection(itemIndex, endpoint, qs, filterPage) {
    const keep = filterPage ?? ((items) => items);
    if (this.getNodeParameter('returnAll', itemIndex)) {
        return keep(await (microsoftApiRequestAllItems).call(this, endpoint, qs));
    }
    ...
    do {
        const response = uri
            ...
            : await (microsoftApiRequest).call(this, 'GET', endpoint, {}, { ...qs, $top: limit });
