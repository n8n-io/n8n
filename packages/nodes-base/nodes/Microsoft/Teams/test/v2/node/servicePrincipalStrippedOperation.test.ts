...
describe('Microsoft Teams V2, Service Principal reads behind SP_HIDE', () => {
	...
	describe('a resource whose operation selector is hidden', () => {
		...
		it('finds the resources this suite is about', () => {
			// onlineMeeting is absent on purpose: it keeps its operation selector visible under
			// the Service Principal credential, so no operation read is stripped for it.
			expect(cases.map((c) => c.resource).sort()).toEqual(['chat', 'chatMember', 'chatMessage']);
		});
