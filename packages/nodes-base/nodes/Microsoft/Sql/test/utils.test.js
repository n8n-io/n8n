...
import { configurePool, copyInputItem, createTableStruct, deleteOperation, escapeIdentifier, escapeTableName, executeSqlQueryAndPrepareResults, insertOperation, mssqlChunk, updateOperation, } from '../GenericFunctions';
describe('MSSQL tests', () => {
    let querySpy;
    let request;
    const assertParameters = (parameters) => {
        if (Array.isArray(parameters)) {
            parameters.forEach((values, rowIndex) => {
                values.forEach((value, index) => {
                    const received = request.parameters[`r${rowIndex}v${index}`].value;
            ...
            for (const key in parameters) {
                expect(request.parameters[key].value).toEqual(parameters[key]);
            }
        }
    };
    beforeEach(() => {
    ...
    it('should perform insert operation', async () => {
        ...
        expect(querySpy).toHaveBeenCalledWith('INSERT INTO [users] ([id], [name], [age], [active]) VALUES (@r0v0, @r0v1, @r0v2, @r0v3), (@r1v0, @r1v1, @r1v2, @r1v3), (@r2v0, @r2v1, @r2v2, @r2v3);');
        assertParameters([
            [1, 'Sam', 31, false],
            [3, 'Jon', null, true],
            [4, null, 25, false],
    ...
    it('should perform insert operation with escaped identifiers', async () => {
        ...
        await insertOperation(tables, pool);
        expect(querySpy).toHaveBeenCalledTimes(1);
        expect(querySpy).toHaveBeenCalledWith("INSERT INTO [users]] set text='asdf' where id=1;] ([id], [name], [age], [active]]]) VALUES (@r0v0, @r0v1, @r0v2, @r0v3);");
    ...
    it('should perform update operation', async () => {
        ...
        expect(querySpy).toHaveBeenCalledTimes(1);
        expect(querySpy).toHaveBeenCalledWith('UPDATE [users] SET [name] = @v0, [age] = @v1, [active] = @v2 WHERE [id] = @condition;');
        assertParameters({
            v0: 'Greg',
            v1: 43,
            v2: 0,
    ...
    it('should perform update operation with escaped identifiers', async () => {
        ...
        await updateOperation(tables, pool);
        expect(querySpy).toHaveBeenCalledTimes(1);
        expect(querySpy).toHaveBeenCalledWith("UPDATE [users]] set text='asdf' where id=1;] SET [name] = @v0, [age] = @v1, [active]]] = @v2 WHERE [id]] -- -] = @condition;");
    });
    ...
    it('should perform delete operation with escaped identifiers', async () => {
        ...
        await deleteOperation(tables, pool);
        expect(querySpy).toHaveBeenCalledTimes(1);
        expect(querySpy).toHaveBeenCalledWith("DELETE FROM [users]] set text='asdf' where id=1;] WHERE [id]]] IN (@v0);");
    });
    describe('mssqlChunk', () => {
        it('should chunk insert values correctly', () => {
            const chunks = mssqlChunk(new Array(3000)
                .fill(null)
    ...
    describe('executeSqlQueryAndPrepareResults', () => {
        const prepareBinaryData = vi.fn(async (buffer, fileName) => ({
        ...
        it('should handle SELECT query with single record', async () => {
            ...
            const result = await executeSqlQueryAndPrepareResults.call(thisArg, pool, 'SELECT * FROM users', 0);
        ...
        it('should return Date values as ISO strings on version 1.2', async () => {
            ...
            querySpy.mockResolvedValueOnce({
                recordsets: [[{ id: 1, created: date }]],
                rowsAffected: [1],
                output: {},
            });
            const pool = { request: () => new Request() };
            const result = await executeSqlQueryAndPrepareResults.call(thisArg, pool, 'SELECT * FROM users', 0, [], 1.2);
            expect(result).toEqual([
                {
        ...
        it('should keep Date objects on versions before 1.2', async () => {
            ...
            const pool = { request: () => new Request() };
            const result = await executeSqlQueryAndPrepareResults.call(thisArg, pool, 'SELECT * FROM users', 0, [], 1.1);
        ...
        it('should move binary columns to the binary output and remove them from json on version 1.2', async () => {
            ...
            querySpy.mockResolvedValueOnce({
                recordsets: [[{ id: 1, name: 'Test', photo: blob }]],
                rowsAffected: [1],
                output: {},
            });
            const pool = { request: () => new Request() };
        ...
        it('should handle SELECT query with multiple records', async () => {
            ...
            const pool = { request: () => new Request() };
            const result = await executeSqlQueryAndPrepareResults.call(thisArg, pool, 'SELECT id; SELECT name', 1);
            expect(result).toEqual([
        ...
        it('should handle non-SELECT query', async () => {
            ...
            const pool = { request: () => new Request() };
            const result = await executeSqlQueryAndPrepareResults.call(thisArg, pool, 'UPDATE users SET active = 1', 2);
            expect(result).toEqual([
                {
        ...
        it('should handle query with no affected rows', async () => {
            ...
            const pool = { request: () => new Request() };
            const result = await executeSqlQueryAndPrepareResults.call(thisArg, pool, 'DELETE FROM users WHERE id = 999', 3);
            expect(result).toEqual([
                {
        ...
        it('should throw an error when query fails', async () => {
            ...
            const pool = { request: () => new Request() };
            await expect(executeSqlQueryAndPrepareResults.call(thisArg, pool, 'INVALID SQL', 4)).rejects.toThrow(errorMessage);
        });
        it('should replace $1 with @p1 and bind the value', async () => {
            const pool = { request: () => new Request() };
            await executeSqlQueryAndPrepareResults.call(thisArg, pool, 'SELECT * FROM users WHERE id = $1', 0, [42]);
        ...
        it('should replace multiple $N placeholders and bind all values', async () => {
            const pool = { request: () => new Request() };
            await executeSqlQueryAndPrepareResults.call(thisArg, pool, 'SELECT * FROM users WHERE age > $1 AND name = $2', 0, [18, 'John']);
        ...
        it('should replace the same $N placeholder used multiple times', async () => {
            const pool = { request: () => new Request() };
            await executeSqlQueryAndPrepareResults.call(thisArg, pool, 'SELECT * FROM t WHERE id = $1 OR parent_id = $1', 0, ['abc']);
        ...
        it('should not confuse $1 with $10 when replacing parameters', async () => {
            ...
            const pool = { request: () => new Request() };
            await executeSqlQueryAndPrepareResults.call(thisArg, pool, 'SELECT * FROM t WHERE c1 = $1 AND c10 = $10', 0, values);
    ...
    describe('escapeTableName', () => {
        it('should escape table name correctly', () => {
            ...
            expect(escapeTableName("test] SET mytext='1' where id=2; -- -")).toEqual("[test]] SET mytext='1' where id=2; -- -]");
            expect(escapeTableName("[[test] (id,text) values(1,'123'); DROP TABLE users; -- ]")).toEqual("[[test]] (id,text) values(1,'123'); DROP TABLE users; -- ]");
    ...
    describe('createTableStruct', () => {
        ...
        const makeGetParam = (table, columns) => (param, _index) => {
            if (param === 'table')
                return table;
            if (param === 'columns')
                return columns;
