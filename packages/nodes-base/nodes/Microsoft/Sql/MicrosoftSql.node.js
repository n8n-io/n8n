...
import { configurePool, createTableStruct, deleteOperation, executeSqlQueryAndPrepareResults, insertOperation, updateOperation, } from './GenericFunctions';
    ...
    async execute() {
        ...
        if (operation === 'executeQuery' && nodeVersion >= 1.1) {
            for (let i = 0; i < items.length; i++) {
                try {
                    ...
                    const results = await executeSqlQueryAndPrepareResults.call(this, pool, rawQuery, i, queryValues, this.getNode().typeVersion);
        ...
        try {
            ...
            if (operation === 'update') {
                ...
                const tables = createTableStruct(this.getNodeParameter, items, ['updateKey'].concat(updateKeys), 'updateKey');
