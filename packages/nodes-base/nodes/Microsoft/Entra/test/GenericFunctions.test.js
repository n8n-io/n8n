...
import { microsoftApiRequest, microsoftApiPaginateRequest, validateEntraGroupId, validateEntraUserId, validateGroupPreSend, validateUserPreSend, } from '../GenericFunctions';
describe('Microsoft Entra GenericFunctions', () => {
    let mockExecuteFunctions;
    let mockNode;
    let mockRequestWithAuthentication;
    let mockRequestWithAuthenticationPaginated;
    beforeEach(() => {
    ...
    describe('microsoftApiRequest', () => {
        describe('graphApiBaseUrl from credentials', () => {
            it('should use base URL from credentials', async () => {
                ...
                expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraOAuth2Api', expect.objectContaining({
                    method: 'GET',
    ...
    describe('microsoftApiPaginateRequest', () => {
        describe('graphApiBaseUrl from credentials', () => {
            it('should use base URL from credentials', async () => {
                ...
                expect(mockRequestWithAuthenticationPaginated).toHaveBeenCalledWith(expect.objectContaining({
                    method: 'GET',
    ...
    describe('same-origin URL override', () => {
        ...
        it('microsoftApiRequest keeps a URL override on the credential host', async () => {
            ...
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/groups', {}, undefined, undefined, 'https://graph.microsoft.com/v1.0/groups?$skiptoken=abc');
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraOAuth2Api', expect.objectContaining({
        ...
        it.each([
            ...
            await expect(microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/groups', {}, undefined, undefined, url)).rejects.toThrow('Refusing to send credentials to an unexpected host');
        ...
        it('microsoftApiRequest rejects a URL override that is not a full URL', async () => {
            await expect(microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/groups', {}, undefined, undefined, '//not-graph.example.com/v1.0/groups')).rejects.toThrow('The request URL is not a valid URL');
            expect(mockRequestWithAuthentication).not.toHaveBeenCalled();
        });
        it('microsoftApiRequest rejects a base URL with no defined origin', async () => {
            ...
            await expect(microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/groups')).rejects.toThrow('The Graph API base URL is not a valid URL');
            expect(mockRequestWithAuthentication).not.toHaveBeenCalled();
        });
        it('microsoftApiRequest keeps an explicit default port on the credential host', async () => {
            mockRequestWithAuthentication.mockResolvedValue({ value: [] });
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/groups', {}, undefined, undefined, 'https://graph.microsoft.com:443/v1.0/groups');
        ...
        it('microsoftApiPaginateRequest rejects a URL override on another host', async () => {
            await expect(microsoftApiPaginateRequest.call(mockExecuteFunctions, 'GET', '/groups', {}, undefined, undefined, 'https://not-graph.example.com/v1.0/groups')).rejects.toThrow('Refusing to send credentials to an unexpected host');
    ...
    describe('validateEntraUserId', () => {
        it('accepts an uppercase GUID', () => {
            expect(() => validateEntraUserId('87D349ED-44D7-43E1-9A83-5F2406DEE5BD', mockNode)).not.toThrow();
        });
        it.each([
        ...
        it('reports an empty value as empty', () => {
            ...
            expect(caught?.description).toBe('Select a user from the list, or set the ID. The ID should be in the format e.g. 02bd9fd6-8f93-4758-87c3-1fb73740a315, or a user principal name e.g. jane@contoso.com.');
        });
    });
    describe('validateEntraGroupId', () => {
        ...
        it('reports an empty value as empty', () => {
            ...
            expect(caught?.description).toBe('Select a group from the list, or set the ID. The ID should be in the format e.g. 02bd9fd6-8f93-4758-87c3-1fb73740a315.');
        });
        it('tells the user that a group is addressed by object ID', () => {
            ...
            expect(caught?.description).toBe('The ID should be in the format e.g. 02bd9fd6-8f93-4758-87c3-1fb73740a315. Groups are addressed by object ID, not by name or email address.');
        });
    });
    describe.each([
        ...
        const requestOptions = { url: 'https://graph.microsoft.com/v1.0/users' };
        const context = (stored, extracted) => {
            const single = mockDeep();
            single.getNode.mockReturnValue(mockNode);
            single.getNodeParameter.mockImplementation(((name, _fallbackValue, options) => {
        ...
        it('throws for an ID containing a slash', async () => {
            const single = context({ __rl: true, mode: 'id', value: `${validId}/members` }, `${validId}/members`);
        ...
        it.each([['..'], ['.']])('throws when the composed path has a "%s" segment', async (segment) => {
            const single = context({ __rl: true, mode: 'id', value: validId }, validId);
            await expect(preSend.call(single, { url: `/users/${segment}` })).rejects.toThrow(NodeOperationError);
        });
        it('throws when the stored value carries an extraction rule', async () => {
            ...
            expect(caught?.description).toBe('Remove the ID extraction rule from this field and set the ID directly.');
        });
    });
});
