...
import { handleErrorPostReceive, validateGroupPreSend, validateUserPreSend, } from '../GenericFunctions';
describe('Microsoft Entra Node', () => {
    ...
    describe('Path ID wiring', () => {
        ...
        const routingUrls = properties
            ...
            .flatMap((option) => 'routing' in option && typeof option.routing?.request?.url === 'string'
        ...
        const interpolatedIds = new Set(routingUrls.flatMap((url) => [...url.matchAll(/\$parameter\["([^"]+)"\]/g)].map(([, name]) => name)));
        ...
        it('has a resource locator for every parameter interpolated into a request URL', () => {
            for (const name of interpolatedIds) {
                expect(resourceLocators.filter((property) => property.name === name).length, name).toBeGreaterThan(0);
            }
        });
        it('guards every resource locator', () => {
            for (const property of resourceLocators) {
                expect(property.routing?.send?.preSend, property.displayName).toContain(property.name === 'user' ? validateUserPreSend : validateGroupPreSend);
            }
        });
        ...
        it('declares no extractValue on any resource locator mode', () => {
            for (const property of resourceLocators) {
                for (const mode of property.modes ?? []) {
                    expect(mode.extractValue, `${property.displayName} / ${mode.displayName}`).toBeUndefined();
                }
            }
        });
        it('still composes the @odata.id body when adding a user to a group', () => {
            const addGroupUser = properties.find((property) => property.name === 'user' &&
                property.displayOptions?.show?.operation?.includes('addGroup'));
            expect(addGroupUser?.routing?.send?.property).toBe('@odata.id');
            expect(addGroupUser?.routing?.send?.value).toContain('directoryObjects/{{ encodeURIComponent($value) }}');
    ...
    describe('HTTP status handling', () => {
        it('handles non-authentication errors in the node error handler', async () => {
            ...
            await expect(handleErrorPostReceive.call(context, [], response)).rejects.toThrow("The required user doesn't match any existing one");
        });
    });
...
//# sourceMappingURL=MicrosoftEntra.node.test.js.map