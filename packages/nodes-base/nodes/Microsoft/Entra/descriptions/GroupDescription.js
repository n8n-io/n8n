...
import { handleErrorPostReceive, microsoftApiRequest, validateGroupPreSend, } from '../GenericFunctions';
                ...
                preSend: [
                    async function (requestOptions) {
                        ...
                        if (displayName?.length > 256) {
                            throw new NodeOperationError(this.getNode(), "'Display Name' should have a maximum length of 256");
                        }
                        return requestOptions;
                    },
                ],
                ...
                preSend: [
                    async function (requestOptions) {
                        ...
                        if (mailNickname?.includes('@')) {
                            throw new NodeOperationError(this.getNode(), `'Group Email Address' should only include the local-part of the email address, without ${mailNickname.slice(mailNickname.indexOf('@'))}`);
                        }
                        if (mailNickname?.length > 64) {
                            throw new NodeOperationError(this.getNode(), "'Group Email Address' should have a maximum length of 64");
                        }
                        if (mailNickname && !/^((?![@()\[\]"\\;:<> ,])[\x00-\x7F])*$/.test(mailNickname)) {
                            throw new NodeOperationError(this.getNode(), "'Group Email Address' should only contain characters in the ASCII character set 0 - 127 except the following: @ () \\ [] \" ; : <> , SPACE");
                        }
                        return requestOptions;
                    },
                ],
                        ...
                        preSend: [
                            async function (requestOptions) {
                                ...
                                if (isAssignableToRole) {
                                    ...
                                    if (!securityEnabled) {
                                        throw new NodeOperationError(this.getNode(), "'Security Enabled' must be set to true if 'Assignable to Role' is set");
                                    }
                                    if (visibility !== 'Private') {
                                        throw new NodeOperationError(this.getNode(), "'Visibility' must be set to 'Private' if 'Assignable to Role' is set");
                                    }
                                    if (groupType === 'Unified' && !mailEnabled) {
                                        throw new NodeOperationError(this.getNode(), "'Mail Enabled' must be set to true if 'Assignable to Role' is set");
                                    }
                                }
                                return requestOptions;
                            },
                    ...
                    async function (items, _response) {
                        for (const item of items) {
                            ...
                            if (Object.keys(fields).length) {
                                ...
                                try {
                                    await microsoftApiRequest.call(this, 'PATCH', `/groups/${encodeURIComponent(groupId)}`, body);
                                    merge(item.json, body);
                                }
                                catch (error) {
                                    try {
                                        await microsoftApiRequest.call(this, 'DELETE', `/groups/${encodeURIComponent(groupId)}`);
                                    }
                                    catch { }
                                    throw error;
                                }
