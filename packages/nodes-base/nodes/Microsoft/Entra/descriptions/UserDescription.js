...
import { handleErrorPostReceive, microsoftApiRequest, validateGroupPreSend, validateUserPreSend, } from '../GenericFunctions';
    ...
    },
    {
        ...
        routing: {
            send: {
                ...
                type: 'body',
                value: '={{ ($credentials.graphApiBaseUrl || "https://graph.microsoft.com").replace(/\\/+$/, "") }}/v1.0/directoryObjects/{{ encodeURIComponent($value) }}',
            },
        },
        type: 'resourceLocator',
    },
                ...
                preSend: [
                    async function (requestOptions) {
                        ...
                        if (!/^[A-Za-z0-9'._\-!#^~@]+$/.test(userPrincipalName)) {
                            throw new NodeOperationError(this.getNode(), "Only the following characters are allowed for 'User Principal Name': A-Z, a-z, 0-9, ' . - _ ! # ^ ~");
                        }
                        ...
                        preSend: [
                            async function (requestOptions) {
                                ...
                                if (companyName?.length > 64) {
                                    throw new NodeOperationError(this.getNode(), "'Company Name' should have a maximum length of 64");
                                }
                                return requestOptions;
                            },
                        ],
                        ...
                        preSend: [
                            async function (requestOptions) {
                                ...
                                if (employeeId?.length > 16) {
                                    throw new NodeOperationError(this.getNode(), "'Employee ID' should have a maximum length of 16");
                                }
                                return requestOptions;
                            },
                        ],
                            ...
                            if (Object.keys(fields).length) {
                                ...
                                try {
                                    if (Object.keys(separateBody).length) {
                                        await microsoftApiRequest.call(this, 'PATCH', `/users/${encodeURIComponent(userId)}`, separateBody);
                                        merge(item.json, separateBody);
                                    }
                                    if (Object.keys(body).length) {
                                        await microsoftApiRequest.call(this, 'PATCH', `/users/${encodeURIComponent(userId)}`, body);
                                        merge(item.json, body);
                                    }
                                }
                                catch (error) {
                                    try {
                                        await microsoftApiRequest.call(this, 'DELETE', `/users/${encodeURIComponent(userId)}`);
                                    }
                                    catch { }
                                    throw error;
                                }
