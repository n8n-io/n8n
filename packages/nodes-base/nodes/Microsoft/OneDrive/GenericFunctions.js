 ...
 */
export function validateOneDriveFileName(node, fileName, itemIndex) {
    ...
    if (illegalChars.length > 0) {
        throw new NodeOperationError(node, `The file name "${fileName}" contains characters that OneDrive doesn't allow: ${illegalChars.join(' ')}`, {
            itemIndex,
            description: `OneDrive file names can't contain any of these characters: ${ONEDRIVE_ILLEGAL_FILE_NAME_CHARS.join(' ')}. Remove them from the file name and try again.` +
 ...
 */
export function resolveDriveScopeRoot(isPoll, itemIndex = 0) {
 ...
 */
export async function microsoftApiRequest(method, resource, body = {}, qs = {}, uri, headers = {}, option = { json: true }, driveScopeRoot) {
...
export async function microsoftApiRequestAllItems(propertyName, method, endpoint, body = {}, query = {}, driveScopeRoot) {
    const returnData = [];
    let responseData;
    let uri;
    query.$top = 100;
...
export async function microsoftApiRequestAllItemsSkip(propertyName, method, endpoint, body = {}, query = {}, driveScopeRoot) {
    const returnData = [];
    let responseData;
    query.$top = 100;
    query.$skip = 0;
...
export async function microsoftApiRequestAllItemsDelta(link, lastDate, eventType) {
    const returnData = [];
    let responseData;
    let deltaLink = '';
    let uri = link;
        ...
        for (const value of responseData.value) {
            if (value.fileSystemInfo) {
                ...
                if (eventType === 'updated') {
                    if (DateTime.fromISO(updatedTimeStamp) >= lastDate &&
...
export async function getPath(itemId) {
