...
describe('Test MicrosoftOneDrive, per-op Service Principal scope threading', () => {
    ...
    it.each([
        ['file', 'fileId'],
        ['folder', 'folderId'],
    ])('%s:delete threads the user root onto DELETE /drive/items/{id}', async (resource, idParam) => {
        mockExecuteFunctions.getNodeParameter.mockImplementation(params({ resource, operation: 'delete', [idParam]: 'item-1' }));
        await microsoftOneDrive.execute.call(mockExecuteFunctions);
        expect(mockApiRequest).toHaveBeenCalledWith('DELETE', '/drive/items/item-1', {}, {}, undefined, {}, { json: true }, USER_ROOT);
    });
    it('file:copy encodes the path-interpolated item id (.. is escaped) and threads the root', async () => {
        mockExecuteFunctions.getNodeParameter.mockImplementation(params({
            resource: 'file',
            operation: 'copy',
            fileId: '../secret',
            additionalFields: {},
        ...
        await microsoftOneDrive.execute.call(mockExecuteFunctions);
        expect(mockApiRequest).toHaveBeenCalledWith('POST', '/drive/items/..%2Fsecret/copy', expect.objectContaining({
            parentReference: expect.objectContaining({ driveId: 'b!explicit' }),
        }), {}, undefined, {}, { json: true, resolveWithFullResponse: true }, USER_ROOT);
    });
    it('rename threads the user root onto PATCH /drive/items/{id}', async () => {
        mockExecuteFunctions.getNodeParameter.mockImplementation(params({ resource: 'file', operation: 'rename', itemId: 'item-1', newName: 'new.txt' }));
        await microsoftOneDrive.execute.call(mockExecuteFunctions);
        expect(mockApiRequest).toHaveBeenCalledWith('PATCH', '/drive/items/item-1', { name: 'new.txt' }, {}, undefined, {}, { json: true }, USER_ROOT);
    });
    it('file:get threads the user root onto GET /drive/items/{id}', async () => {
        mockExecuteFunctions.getNodeParameter.mockImplementation(params({ resource: 'file', operation: 'get', fileId: 'item-1' }));
        await microsoftOneDrive.execute.call(mockExecuteFunctions);
        expect(mockApiRequest).toHaveBeenCalledWith('GET', '/drive/items/item-1', {}, {}, undefined, {}, { json: true }, USER_ROOT);
    });
    it('file:share threads the user root onto POST /drive/items/{id}/createLink', async () => {
        ...
        await microsoftOneDrive.execute.call(mockExecuteFunctions);
        expect(mockApiRequest).toHaveBeenCalledWith('POST', '/drive/items/item-1/createLink', { type: 'view', scope: 'anonymous' }, {}, undefined, {}, { json: true }, USER_ROOT);
    });
    it('folder:create threads the user root onto POST /drive/root/children', async () => {
        mockExecuteFunctions.getNodeParameter.mockImplementation(params({ resource: 'folder', operation: 'create', name: 'NewFolder', options: {} }));
        await microsoftOneDrive.execute.call(mockExecuteFunctions);
        expect(mockApiRequest).toHaveBeenCalledWith('POST', '/drive/root/children', { name: 'NewFolder', folder: {} }, {}, undefined, {}, { json: true }, USER_ROOT);
    });
    it('folder:getChildren threads the user root through the paginator', async () => {
        mockExecuteFunctions.getNodeParameter.mockImplementation(params({ resource: 'folder', operation: 'getChildren', folderId: 'folder-1' }));
        await microsoftOneDrive.execute.call(mockExecuteFunctions);
        expect(mockApiRequestAllItems).toHaveBeenCalledWith('value', 'GET', '/drive/items/folder-1/children', {}, {}, USER_ROOT);
    });
    it('file:download threads the user root onto both the item GET and the content GET', async () => {
        ...
        mockExecuteFunctions.helpers.prepareBinaryData = vi.fn(async () => ({ data: 'bin' }));
        ...
        expect(mockApiRequest).toHaveBeenCalledWith('GET', '/drive/items/item-1/content', {}, {}, undefined, {}, { encoding: null, resolveWithFullResponse: true }, USER_ROOT);
    });
    ...
    it('file:upload (text) threads the user root and preserves the :/path:/ shape', async () => {
        ...
        await microsoftOneDrive.execute.call(mockExecuteFunctions);
        expect(mockApiRequest).toHaveBeenCalledWith('PUT', '/drive/items/parent-1:/report.txt:/content', 'hello', {}, undefined, { 'Content-Type': 'text/plain' }, { json: true }, USER_ROOT);
    });
});
//# sourceMappingURL=app-only-ops.test.js.map