...
describe('Test MicrosoftOneDrive, file > upload', () => {
    ...
    it('should use filename from node parameters even when binary has a filename (version 1.1+)', async () => {
        const items = [{ json: { data: 'test' }, binary: { data: {} } }];
        ...
        expect(genericFunctions.microsoftApiRequest).toHaveBeenCalledWith('PUT', '/drive/items/parentFolderId:/customFileName.txt:/content', expect.any(Buffer), {}, undefined, { 'Content-Type': 'text/plain', 'Content-length': expect.any(Number) }, {}, undefined);
    });
    it('should use binary filename when node parameter filename is empty', async () => {
        ...
        await microsoftOneDrive.execute.call(mockExecuteFunctions);
        expect(genericFunctions.microsoftApiRequest).toHaveBeenCalledTimes(1);
        expect(genericFunctions.microsoftApiRequest).toHaveBeenCalledWith('PUT', '/drive/items/parentFolderId:/binaryFileName.txt:/content', expect.any(Buffer), {}, undefined, { 'Content-Type': 'text/plain', 'Content-length': expect.any(Number) }, {}, undefined);
    });
    it('should properly encode special characters in filename from node parameters', async () => {
        ...
        mockExecuteFunctions.getNodeParameter.mockImplementation((key) => {
            ...
            if (key === 'parentId')
                return 'parentFolderId';
            if (key === 'fileName')
                return 'file with spaces & special.txt';
            if (key === 'binaryData')
                return true;
            if (key === 'binaryPropertyName')
                return 'data';
        ...
        await microsoftOneDrive.execute.call(mockExecuteFunctions);
        expect(genericFunctions.microsoftApiRequest).toHaveBeenCalledTimes(1);
        expect(genericFunctions.microsoftApiRequest).toHaveBeenCalledWith('PUT', '/drive/items/parentFolderId:/file%20with%20spaces%20%26%20special.txt:/content', expect.any(Buffer), {}, undefined, { 'Content-Type': 'text/plain', 'Content-length': expect.any(Number) }, {}, undefined);
    });
    ...
    it('should upload via the text path when the file name is valid', async () => {
        ...
        await microsoftOneDrive.execute.call(mockExecuteFunctions);
        expect(genericFunctions.microsoftApiRequest).toHaveBeenCalledTimes(1);
        expect(genericFunctions.microsoftApiRequest).toHaveBeenCalledWith('PUT', '/drive/items/parentFolderId:/report.txt:/content', 'hello world', {}, undefined, { 'Content-Type': 'text/plain' }, { json: true }, undefined);
    });
    it('should reject a text-path file name containing a colon without issuing an upload request', async () => {
        ...
        mockExecuteFunctions.getNodeParameter.mockImplementation((key) => {
            ...
            if (key === 'parentId')
                return 'parentFolderId';
            if (key === 'fileName')
                return 'report-2026-06-10T12:34:56.789Z.txt';
            if (key === 'binaryData')
                return false;
            if (key === 'fileContent')
                return 'hello world';
    ...
    it('should upload via the binary path with the resolved binary mime type and content length', async () => {
        ...
        expect(genericFunctions.microsoftApiRequest).toHaveBeenCalledTimes(1);
        expect(genericFunctions.microsoftApiRequest).toHaveBeenCalledWith('PUT', '/drive/items/parentFolderId:/report.pdf:/content', expect.any(Buffer), {}, undefined, { 'Content-Type': 'application/pdf', 'Content-length': expect.any(Number) }, {}, undefined);
    });
    it('should upload valid items and route only the invalid one to output under Continue On Fail', async () => {
        ...
        mockExecuteFunctions.getInputData.mockReturnValue(items);
        mockExecuteFunctions.continueOnFail.mockReturnValue(true);
        mockExecuteFunctions.getNodeParameter.mockImplementation((key, itemIndex) => {
            if (key === 'resource')
                return 'file';
            if (key === 'operation')
                return 'upload';
            ...
            if (key === 'fileContent')
                return 'hello world';
            if (key === 'fileName')
                return itemIndex === 0 ? 'good.txt' : 'bad:name.txt';
        });
        const result = await microsoftOneDrive.execute.call(mockExecuteFunctions);
        expect(genericFunctions.microsoftApiRequest).toHaveBeenCalledTimes(1);
        expect(genericFunctions.microsoftApiRequest).toHaveBeenCalledWith('PUT', '/drive/items/parentFolderId:/good.txt:/content', 'hello world', {}, undefined, { 'Content-Type': 'text/plain' }, { json: true }, undefined);
