...
import { microsoftApiRequest, getPath, getOneDriveCredentialType, validateOneDriveFileName, validateResourceTargetId, getServicePrincipalResourceRoot, resolveDriveScopeRoot, driveEndpoint, } from '../GenericFunctions';
describe('Microsoft OneDrive GenericFunctions', () => {
    ...
    describe('microsoftApiRequest', () => {
        describe('graphApiBaseUrl from credentials', () => {
            it('should use base URL from credentials', async () => {
                ...
                expect(mockRequestOAuth2).toHaveBeenCalledWith('microsoftOneDriveOAuth2Api', expect.objectContaining({
                    method: 'GET',
    ...
    describe('credential type selection', () => {
        ...
        it('should use the microsoftOneDriveOAuth2Api credential when selected', async () => {
            ...
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/drive');
            expect(mockExecuteFunctions.getCredentials).toHaveBeenCalledWith('microsoftOneDriveOAuth2Api');
            expect(mockRequestOAuth2).toHaveBeenCalledWith('microsoftOneDriveOAuth2Api', expect.anything());
            expect(mockRequestWithAuthentication).not.toHaveBeenCalled();
        });
    ...
    describe('getOneDriveCredentialType context semantics', () => {
        ...
        it('resolves the default credential in EXECUTE context (node)', () => {
            ...
            const executeCtx = {
                getNodeParameter: (name, _itemIndex, fallbackValue) => get(savedParameters, name, fallbackValue),
            };
        ...
        it('resolves the default credential in POLL context (trigger)', () => {
            ...
            const pollCtx = {
                getNodeParameter: (name, fallbackValue) => get(savedParameters, name, fallbackValue),
            };
    ...
    describe('validateOneDriveFileName', () => {
        ...
        it('names the colon and suggests a colon-free timestamp format', () => {
            ...
            expect(caught?.description).toContain("$now.toFormat('yyyy-MM-dd_HH-mm-ss')");
        });
        ...
        it('omits the timestamp suggestion when the illegal name has no colon', () => {
            ...
            expect(caught?.message).toContain("contains characters that OneDrive doesn't allow");
            expect(caught?.description).not.toContain('$now.toFormat');
        });
    });
    describe('getServicePrincipalResourceRoot', () => {
        ...
        it('coerces a non-string id robustly via String(rawId ?? "") and stringifies it into the root', () => {
            ...
            expect(getServicePrincipalResourceRoot('drive', 12345, mockNode)).toBe('/drives/12345');
        });
    });
    ...
    describe('validateResourceTargetId', () => {
        it.each(['user', 'drive'])('rejects empty / whitespace / dots-only for %s (common reject runs first)', (target) => {
        ...
        it('accepts a user GUID and a user UPN', () => {
            expect(() => validateResourceTargetId('user', '11111111-1111-1111-1111-111111111111', mockNode)).not.toThrow();
            expect(() => validateResourceTargetId('user', 'jane@contoso.com', mockNode)).not.toThrow();
        });
        it('accepts a UPN containing "+" (sub-addressing)', () => {
            expect(() => validateResourceTargetId('user', 'jane+test@contoso.com', mockNode)).not.toThrow();
        });
        it('accepts the widened Entra UPN set (! ^ ~ #) and a B2B guest (#EXT#) UPN for the user target', () => {
            ...
            expect(() => validateResourceTargetId('user', 'joe^smith@contoso.com', mockNode)).not.toThrow();
            expect(() => validateResourceTargetId('user', 'jane~doe@contoso.com', mockNode)).not.toThrow();
            expect(() => validateResourceTargetId('user', 'a#b@contoso.com', mockNode)).not.toThrow();
            expect(() => validateResourceTargetId('user', 'user_contoso.com#EXT#@tenant.onmicrosoft.com', mockNode)).not.toThrow();
        });
        it('rejects a bare host / bare label / legal-but-user-invalid host for the user target (bare-host branch is gone)', () => {
            ...
            expect(() => validateResourceTargetId('user', 'contoso.com', mockNode)).toThrow('The target ID is not valid');
            expect(() => validateResourceTargetId('user', 'jane', mockNode)).toThrow('The target ID is not valid');
            expect(() => validateResourceTargetId('user', 'sub.contoso.co.uk', mockNode)).toThrow('The target ID is not valid');
        });
        it('rejects an encoded-traversal UPN for the user target (% is not a legal UPN char)', () => {
            expect(() => validateResourceTargetId('user', '..%2f..%2fx@y.com', mockNode)).toThrow('The target ID is not valid');
        });
        it('throws for an unexpected target type instead of treating it as a user', () => {
            ...
            expect(() => validateResourceTargetId('site', 'jane@contoso.com', mockNode)).toThrow('The target ID is not valid');
        });
        ...
        it('reports the shared "user"-branch empty message (delegates to the shared validator)', () => {
            ...
            expect(caught?.description).toBe('Set the User ID under "Access As" — app-only Microsoft Graph has no personal context to default to.');
        });
        it('keeps the drive-branch empty message unchanged (mentions the personal drive)', () => {
            ...
            expect(caught?.message).toBe('A target ID is required for the Service Principal');
            expect(caught?.description).toBe('Set the User or Drive ID under "Access As" — app-only Microsoft Graph has no personal drive to default to.');
        });
    ...
    describe('microsoftApiRequest with Service Principal (driveScopeRoot)', () => {
        beforeEach(() => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((name) => name === 'authentication' ? 'microsoftEntraServicePrincipalApi' : undefined);
            mockExecuteFunctions.getCredentials.mockResolvedValue({
        ...
        it('routes a user-rooted request via requestWithAuthentication (NOT OAuth2)', async () => {
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/drive/items/x', {}, {}, undefined, {}, { json: true }, '/users/jane%40contoso.com');
        ...
        it('carries a B2B guest (#EXT#) UPN end-to-end into an encoded /users/{id}/drive uri', async () => {
            const scopeRoot = getServicePrincipalResourceRoot('user', 'user_contoso.com#EXT#@tenant.onmicrosoft.com', mockNode);
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/drive/items/x', {}, {}, undefined, {}, { json: true }, scopeRoot);
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.objectContaining({
        ...
        it('roots a drive request as /drives/{id}/items/x with no double /drive', async () => {
            await microsoftApiRequest.call(mockExecuteFunctions, 'DELETE', '/drive/items/x', {}, {}, undefined, {}, { json: true }, '/drives/b!abc');
            // A /drives/{id} root IS already a drive — no extra `/drive` segment.
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.objectContaining({
                uri: 'https://graph.microsoft.com/v1.0/drives/b!abc/items/x',
            }));
            const calledUri = mockRequestWithAuthentication.mock.calls[0][1].uri;
        ...
        it('preserves the :/path:/ upload shape under a user root (no double /drive)', async () => {
            await microsoftApiRequest.call(mockExecuteFunctions, 'PUT', '/drive/items/parent:/file.txt:/content', Buffer.from('x'), {}, undefined, { 'Content-Type': 'text/plain' }, {}, '/users/jane');
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.objectContaining({
        ...
        it('preserves the :/path:/ upload shape under a drive root (no double /drive)', async () => {
            await microsoftApiRequest.call(mockExecuteFunctions, 'PUT', '/drive/items/parent:/file.txt:/content', Buffer.from('x'), {}, undefined, { 'Content-Type': 'text/plain' }, {}, '/drives/b!abc');
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.objectContaining({
        ...
        it('uses an explicit absolute uri verbatim and never prefixes the scope root', async () => {
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '', {}, {}, 'https://graph.microsoft.com/v1.0/some/absolute/deltaLink', {}, { json: true }, '/users/jane');
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.objectContaining({
        ...
        it('routes via requestWithAuthentication for an absolute uri with NO driveScopeRoot (trigger delta convention)', async () => {
            ...
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '', {}, {}, 'https://graph.microsoft.com/v1.0/users/jane%40contoso.com/drive/root/delta');
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.objectContaining({
        ...
        it('honors a sovereign graphApiBaseUrl with a non-user (drive) root', async () => {
            ...
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/drive/items/x', {}, {}, undefined, {}, { json: true }, '/drives/b!abc');
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.objectContaining({
        ...
        it('throws an internal error when a scoped resource does not start with /drive', async () => {
            await expect(microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/users/x', {}, {}, undefined, {}, { json: true }, '/users/jane')).rejects.toThrow('must start with "/drive"');
            expect(mockRequestWithAuthentication).not.toHaveBeenCalled();
        });
        it('delegates to requestWithAuthentication once with no node-side retry, surfacing a rejection as NodeApiError', async () => {
            ...
            const error = await microsoftApiRequest
                .call(mockExecuteFunctions, 'GET', '/drive/items/x', {}, {}, undefined, {}, { json: true }, '/users/jane')
                .catch((e) => e);
    ...
    describe('getPath under Service Principal', () => {
        it('routes via requestWithAuthentication with the scoped resource-form URL', async () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((name, fallback) => {
                if (name === 'authentication')
                    return 'microsoftEntraServicePrincipalApi';
                if (name === 'resourceTarget')
                    return 'user';
                if (name === 'userTarget')
                    return 'jane@contoso.com';
                return fallback;
            });
            await getPath.call(mockExecuteFunctions, 'item-id-123');
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.objectContaining({
    ...
    describe('resolveDriveScopeRoot', () => {
        it('returns undefined for OAuth2 credentials (no scope root)', () => {
            const ctx = {
                getNode: () => mockNode,
                getNodeParameter: (name, _itemIndex, fallback) => name === 'authentication' ? 'microsoftOneDriveOAuth2Api' : fallback,
            };
