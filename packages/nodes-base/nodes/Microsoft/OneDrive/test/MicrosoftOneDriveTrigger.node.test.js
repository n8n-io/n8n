...
describe('MicrosoftOneDriveTrigger', () => {
    ...
    it('should use the Service Principal credential and a scoped manual-mode delta URL', async () => {
        ...
        expect(microsoftApiRequest).toHaveBeenCalledWith('GET', '', {}, {}, 'https://graph.microsoft.com/v1.0/users/jane%40contoso.com/drive/root/delta');
    });
    ...
    it('resets a stale OAuth2 LastLink when the Service Principal scope is selected', async () => {
        ...
        expect(microsoftApiRequestAllItemsDelta).toHaveBeenCalledWith('https://graph.microsoft.com/v1.0/users/jane%40contoso.com/drive/root/delta?token=latest', expect.anything(), expect.anything());
    });
    it('reinitialises the delta link when the target id changes between polls', async () => {
        ...
        await trigger.poll.call(pollFunctions);
        expect(microsoftApiRequestAllItemsDelta).toHaveBeenCalledWith('https://graph.microsoft.com/v1.0/drives/b!NEW/root/delta?token=latest', expect.anything(), expect.anything());
    });
    it('keeps a still-valid scoped LastLink across polls', async () => {
        ...
        await trigger.poll.call(pollFunctions);
        expect(microsoftApiRequestAllItemsDelta).toHaveBeenCalledWith(stillValid, expect.anything(), expect.anything());
    });
});
//# sourceMappingURL=MicrosoftOneDriveTrigger.node.test.js.map