...
import { driveEndpoint, getOneDriveCredentialType, getPath, microsoftApiRequest, microsoftApiRequestAllItemsDelta, resolveDriveScopeRoot, } from './GenericFunctions';
import { triggerDescription } from './TriggerDescription';
export class MicrosoftOneDriveTrigger {
    description = {
    ...
    async poll() {
        ...
        const start = DateTime.fromISO(workflowData.lastTimeChecked) || now;
        const end = now;
        ...
        try {
            if (this.getMode() === 'manual') {
                ...
                const response = (await microsoftApiRequestAllItemsDelta.call(this, lastLink, start, eventType));
