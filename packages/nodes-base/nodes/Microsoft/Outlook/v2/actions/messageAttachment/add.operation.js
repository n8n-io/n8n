...
export async function execute(index, _) {
    ...
    if (!fileName) {
        throw new NodeOperationError(this.getNode(), 'File name is not set. It has either to be set via "Additional Fields" or has to be set on the binary property!', { itemIndex: index });
    }
    ...
    if (dataBuffer.length > 3e6) {
        ...
        responseData = await microsoftApiRequest.call(this, 'POST', `/messages/${messageId}/attachments/createUploadSession`, index, body);
        ...
        responseData = await microsoftApiRequest.call(this, 'POST', `/messages/${messageId}/attachments`, index, body, {});
    }
