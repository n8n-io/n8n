...
export async function execute(index) {
    ...
    const folderId = decodeOutlookId(this.getNodeParameter('updateFields.folderId', index, '', {
        extractValue: true,
    }));
    if (folderId) {
        const body = {
        ...
        responseData = await microsoftApiRequest.call(this, 'POST', `/messages/${messageId}/move`, index, body);
    ...
    responseData = await microsoftApiRequest.call(this, 'PATCH', `/messages/${messageId}`, index, body, {});
    const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(responseData), { itemData: { item: index } });
    return executionData;
}
