...
const MAILBOX_MESSAGES = {
    required: {
        ...
        description: 'Set the Mailbox (a UPN or user object ID) — app-only Microsoft Graph has no personal mailbox to default to.',
    },
    dotsOnly: {
    ...
    invalid: {
        message: 'The mailbox is not valid',
        description: 'Enter a user principal name (UPN) or object ID. Remove any slashes, backslashes, colons, commas, spaces, or encoded characters and try again.',
    },
};
...
export function simplifyOutputMessages(data) {
...
export function prepareContactFields(fields) {
    const returnData = {};
...
export function prepareEventFields(fields) {
    const result = { ...fields };
    if (result.location) {
        result.location = { displayName: result.location };
    }
    if (result.attendees) {
        const attendeeEntries = result.attendees.values ?? [];
...
export function prepareFilterString(filters) {
...
export function prepareApiError(error, itemIndex = 0) {
    const [httpCode, err, message] = error.description.split(' - ');
    const json = jsonParse(err);
    return new NodeApiError(this.getNode(), json, {
        itemIndex,
        httpCode,
 ...
 */
export function validateMailbox(id, node) {
    validateUserTargetId(id, node, MAILBOX_MESSAGES);
}
export const encodeOutlookId = (id) => {
    return id.replace(/-/g, '%2F').replace(/=/g, '%3D').replace(/\+/g, '%2B');
};
export const decodeOutlookId = (id) => {
