...
import { downloadAttachments, microsoftApiRequest, microsoftApiRequestAllItems, } from '../../v2/transport';
describe('Microsoft Outlook Trigger GenericFunctions', () => {
    ...
    describe('getPollResponse', () => {
        ...
        beforeEach(() => {
            mockPollFunctions.getNodeParameter.mockImplementation((paramName, defaultValue) => {
                const params = {
                    filters: {},
                    options: {},
                    output: 'simple',
            ...
            prepareFilterString.mockReturnValue('');
            simplifyOutputMessages.mockReturnValue(mockMessages);
        });
        describe('successful execution', () => {
            describe('manual mode', () => {
                beforeEach(() => {
                    mockPollFunctions.getMode.mockReturnValue('manual');
                    microsoftApiRequest.mockResolvedValue({ value: mockMessages });
                });
                ...
                it('should handle downloadAttachments option in manual mode', async () => {
                    ...
                    microsoftApiRequest.mockResolvedValue({ value: mockMessages });
                    downloadAttachments.mockResolvedValue(mockExecutionData);
                    const result = await getPollResponse.call(mockPollFunctions, pollStartDate, pollEndDate);
                    expect(microsoftApiRequest).toHaveBeenCalledWith('GET', '/messages', 0, undefined, {
            ...
            describe('trigger mode', () => {
                beforeEach(() => {
                    ...
                    microsoftApiRequestAllItems.mockResolvedValue(mockMessages);
                });
                it('should handle simple output format in trigger mode', async () => {
                    const result = await getPollResponse.call(mockPollFunctions, pollStartDate, pollEndDate);
                    expect(microsoftApiRequestAllItems).toHaveBeenCalledWith('value', 'GET', '/messages', 0, undefined, {
                        $select: 'id,conversationId,subject,bodyPreview,from,toRecipients,categories,hasAttachments',
                ...
                it('should combine custom filters with date filters in trigger mode', async () => {
                    ...
                    prepareFilterString.mockReturnValue(customFilter);
            ...
            describe('folder filtering', () => {
                ...
                describe('trigger mode', () => {
                    ...
                    it('should query folder endpoint instead of /messages when foldersToInclude is set', async () => {
                        ...
                        await getPollResponse.call(mockPollFunctions, pollStartDate, pollEndDate);
                        expect(microsoftApiRequestAllItems).toHaveBeenCalledWith('value', 'GET', `/mailFolders/${folderId1}/messages`, 0, undefined, {
                            $select: 'id,conversationId,subject,bodyPreview,from,toRecipients,categories,hasAttachments',
                            $filter: `receivedDateTime ge ${pollStartDate} and receivedDateTime lt ${pollEndDate}`,
                        });
                    ...
                    it('should query each folder endpoint and merge results when multiple foldersToInclude are set', async () => {
                        ...
                        expect(microsoftApiRequestAllItems).toHaveBeenCalledWith('value', 'GET', `/mailFolders/${folderId1}/messages`, 0, undefined, expect.objectContaining({ $filter: expect.any(String) }));
                        expect(microsoftApiRequestAllItems).toHaveBeenCalledWith('value', 'GET', `/mailFolders/${folderId2}/messages`, 0, undefined, expect.objectContaining({ $filter: expect.any(String) }));
                    ...
                    it('should fall back to /messages when foldersToInclude contains only empty strings', async () => {
                        ...
                        await getPollResponse.call(mockPollFunctions, pollStartDate, pollEndDate);
                        expect(microsoftApiRequestAllItems).toHaveBeenCalledWith('value', 'GET', '/messages', 0, undefined, expect.objectContaining({ $filter: expect.any(String) }));
                    });
                });
                describe('manual mode', () => {
                    beforeEach(() => {
                    ...
                    it('should query all folder endpoints with $top=1 in manual mode', async () => {
                        ...
                        expect(microsoftApiRequest).toHaveBeenCalledWith('GET', `/mailFolders/${folderId1}/messages`, 0, undefined, {
                            $select: 'id,conversationId,subject,bodyPreview,from,toRecipients,categories,hasAttachments',
                            $top: 1,
                        });
                        expect(microsoftApiRequest).toHaveBeenCalledWith('GET', `/mailFolders/${folderId2}/messages`, 0, undefined, {
                            $select: 'id,conversationId,subject,bodyPreview,from,toRecipients,categories,hasAttachments',
                            $top: 1,
                        });
            ...
            describe('edge cases', () => {
                ...
                it('should handle empty response', async () => {
                    microsoftApiRequest.mockResolvedValue({ value: [] });
                    simplifyOutputMessages.mockReturnValue([]);
                    const result = await getPollResponse.call(mockPollFunctions, pollStartDate, pollEndDate);
                    expect(result).toHaveLength(0);
                    expect(result).toEqual([]);
                });
                it('should handle null response values', async () => {
                    microsoftApiRequest.mockResolvedValue({ value: null });
                ...
                it('should include hasAttachments when downloadAttachments is enabled', async () => {
                    ...
                    microsoftApiRequest.mockResolvedValue({ value: mockMessages });
                    downloadAttachments.mockResolvedValue([]);
                    await getPollResponse.call(mockPollFunctions, pollStartDate, pollEndDate);
                    expect(microsoftApiRequest).toHaveBeenCalledWith('GET', '/messages', 0, undefined, {
        ...
        describe('error handling', () => {
            it('should throw NodeApiError when microsoftApiRequest fails', async () => {
                ...
                mockPollFunctions.getMode.mockReturnValue('manual');
                microsoftApiRequest.mockRejectedValue(originalError);
                await expect(getPollResponse.call(mockPollFunctions, pollStartDate, pollEndDate)).rejects.toThrow(NodeApiError);
            ...
            it('should throw NodeApiError when microsoftApiRequestAllItems fails', async () => {
                ...
                mockPollFunctions.getMode.mockReturnValue('trigger');
                microsoftApiRequestAllItems.mockRejectedValue(originalError);
                await expect(getPollResponse.call(mockPollFunctions, pollStartDate, pollEndDate)).rejects.toThrow(NodeApiError);
                expect(mockPollFunctions.getNode).toHaveBeenCalled();
            });
            it('should throw NodeApiError when downloadAttachments fails', async () => {
                ...
                microsoftApiRequest.mockResolvedValue({ value: mockMessages });
                downloadAttachments.mockRejectedValue(originalError);
                await expect(getPollResponse.call(mockPollFunctions, pollStartDate, pollEndDate)).rejects.toThrow(NodeApiError);
            });
            it('should throw NodeApiError when simplifyOutputMessages fails', async () => {
                ...
                mockPollFunctions.getMode.mockReturnValue('manual');
                microsoftApiRequest.mockResolvedValue({ value: mockMessages });
                simplifyOutputMessages.mockImplementation(() => {
                    throw originalError;
                });
                await expect(getPollResponse.call(mockPollFunctions, pollStartDate, pollEndDate)).rejects.toThrow(NodeApiError);
            });
            it('should throw NodeApiError when prepareFilterString fails', async () => {
                ...
                mockPollFunctions.getMode.mockReturnValue('trigger');
                prepareFilterString.mockImplementation(() => {
                    throw originalError;
                });
                await expect(getPollResponse.call(mockPollFunctions, pollStartDate, pollEndDate)).rejects.toThrow(NodeApiError);
            });
            it('should preserve error message and description in NodeApiError', async () => {
                ...
                mockPollFunctions.getMode.mockReturnValue('manual');
                microsoftApiRequest.mockRejectedValue(originalError);
                await expect(getPollResponse.call(mockPollFunctions, pollStartDate, pollEndDate)).rejects.toThrow(expect.objectContaining({
            ...
            it('should handle errors without description', async () => {
                ...
                try {
                    ...
                    expect(error.message).toContain('Error without description');
                }
            });
        });
        ...
        describe('integration scenarios', () => {
            it('should handle complex filter combinations', async () => {
                ...
                prepareFilterString.mockReturnValue(complexFilter);
                microsoftApiRequestAllItems.mockResolvedValue(mockMessages);
                const result = await getPollResponse.call(mockPollFunctions, pollStartDate, pollEndDate);
                expect(prepareFilterString).toHaveBeenCalledWith({
                    filters: { isRead: false, fromAddress: 'test@example.com' },
            ...
            it('should handle all options together', async () => {
                ...
                prepareFilterString.mockReturnValue('isRead eq false');
                microsoftApiRequestAllItems.mockResolvedValue(mockMessages);
                downloadAttachments.mockResolvedValue(mockExecutionData);
                const result = await getPollResponse.call(mockPollFunctions, pollStartDate, pollEndDate);
                expect(microsoftApiRequestAllItems).toHaveBeenCalledWith('value', 'GET', '/messages', 0, undefined, {
