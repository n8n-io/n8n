...
import { downloadAttachments, getMimeContent, getOutlookCredentialType, getSubfolders, microsoftApiRequest, } from '../../../v2/transport';
describe('MicrosoftOutlookV2 - microsoftApiRequest', () => {
    ...
    describe('graphApiBaseUrl from credentials (OAuth2 /me)', () => {
        it('should use base URL from credentials', async () => {
            ...
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftOutlookOAuth2Api', expect.objectContaining({
                method: 'GET',
    ...
    describe('credential type resolution', () => {
        it('should use microsoftOutlookOAuth2Api when authentication is not set (backward compatibility)', async () => {
            ...
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftOutlookOAuth2Api', expect.objectContaining({ uri: 'https://graph.microsoft.com/v1.0/me/messages' }));
        });
        it('should route through microsoftOAuth2Api when the generic credential is selected', async () => {
            ...
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('microsoftOAuth2Api', expect.objectContaining({ uri: 'https://graph.microsoft.com/v1.0/me/messages' }));
            // generic OAuth2 must never reach the Service Principal credential
            expect(mockRequestWithAuthentication).not.toHaveBeenCalledWith('microsoftEntraServicePrincipalApi', expect.anything());
        });
    });
    ...
    describe('Service Principal (app-only)', () => {
        ...
        it('should use an explicit nextLink uri verbatim, not re-prefixed with /users/{mailbox}', async () => {
            setupSP({ mailbox: 'user@example.com' });
            const nextLink = 'https://graph.microsoft.com/v1.0/users/user%40example.com/messages?$skip=10&$top=10';
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/messages', 0, {}, {}, nextLink);
            const callArgs = mockRequestWithAuthentication.mock.calls[0][1];
        ...
        it('should throw a static error and make no request when the mailbox is empty', async () => {
            setupSP({ mailbox: '' });
            await expect(microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/messages', 0)).rejects.toThrow(NodeOperationError);
            await expect(microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/messages', 0)).rejects.toThrow('A mailbox is required for the Service Principal');
        ...
        it('should reject a malformed (non-empty) mailbox and make no request', async () => {
            ...
            await expect(microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/messages', 0)).rejects.toThrow('The mailbox is not valid');
            expect(mockRequestWithAuthentication).not.toHaveBeenCalled();
        });
        ...
        const setupSPPerItem = () => {
            setupSP();
            mockExecuteFunctions.getNodeParameter.mockImplementation((name, itemIndex) => {
                if (name === 'authentication')
                    return 'microsoftEntraServicePrincipalApi';
                if (name === 'mailbox')
        ...
        it('should resolve the mailbox at the passed itemIndex', async () => {
            setupSPPerItem();
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/messages', 0, {}, {}, undefined, {}, { json: true });
            await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/messages', 1, {}, {}, undefined, {}, { json: true });
        ...
        it('should attribute a Graph error to the passed itemIndex (prepareApiError path)', async () => {
            ...
            mockRequestWithAuthentication.mockRejectedValue(Object.assign(new Error('bad request'), {
                description: '400 - {"error":{"code":"ErrorInvalidRequest","message":"broken"}} - broken',
            }));
            let caught;
            try {
                await microsoftApiRequest.call(mockExecuteFunctions, 'GET', '/messages', 2, {}, {}, undefined, {}, { json: true });
            }
            catch (error) {
                caught = error;
            }
            expect(caught).toBeInstanceOf(NodeApiError);
            expect(caught.context.itemIndex).toBe(2);
        });
        it('should download attachments from the mailbox at the passed itemIndex', async () => {
            ...
            await downloadAttachments.call(mockExecuteFunctions, { id: 'MSG1', hasAttachments: true }, 'attachment_', 1);
...
describe('MicrosoftOutlookV2 - getSubfolders', () => {
    ...
    it('should paginate child folder requests using nextLink', async () => {
        ...
        mockLoadOptionsFunctions.helpers.requestWithAuthentication
            ...
            '@odata.nextLink': 'https://graph.microsoft.com/v1.0/me/mailFolders/inbox/childFolders?$skip=1',
        })
            .mockResolvedValueOnce({
            value: [{ id: 'sub2', displayName: 'Projects', childFolderCount: 0 }],
        });
        const result = await getSubfolders.call(mockLoadOptionsFunctions, folders, 0);
        expect(mockLoadOptionsFunctions.helpers.requestWithAuthentication).toHaveBeenCalledTimes(2);
    ...
    it('should return bare subfolder displayNames when addPathToDisplayName is false', async () => {
        const folders = [{ id: 'inbox', displayName: 'Inbox', childFolderCount: 1 }];
        mockLoadOptionsFunctions.helpers.requestWithAuthentication.mockResolvedValueOnce({
