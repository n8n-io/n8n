...
describe('MicrosoftOutlookV2 - loadOptions methods', () => {
    ...
    describe('getCategoriesNames', () => {
        it('should get categories names successfully', async () => {
            ...
            expect(mockTransport.microsoftApiRequestAllItems).toHaveBeenCalledWith('value', 'GET', '/outlook/masterCategories', 0);
        ...
        it('should handle API errors', async () => {
            ...
            await expect(getCategoriesNames.call(mockLoadOptionsFunctions)).rejects.toThrow('Failed to fetch categories');
        });
    ...
    describe('getFolders', () => {
        it('should get folders successfully', async () => {
            ...
            expect(mockTransport.microsoftApiRequestAllItems).toHaveBeenCalledWith('value', 'GET', '/mailFolders', 0, {});
        ...
        it('should handle API errors from microsoftApiRequestAllItems', async () => {
            ...
            mockTransport.microsoftApiRequestAllItems.mockRejectedValue(apiError);
            await expect(getFolders.call(mockLoadOptionsFunctions)).rejects.toThrow('Failed to fetch mail folders');
        });
        it('should handle errors from getSubfolders', async () => {
            ...
            await expect(getFolders.call(mockLoadOptionsFunctions)).rejects.toThrow('Failed to get subfolders');
        });
    ...
    describe('getCalendarGroups', () => {
        it('should get calendar groups successfully', async () => {
            ...
            expect(mockTransport.microsoftApiRequestAllItems).toHaveBeenCalledWith('value', 'GET', '/calendarGroups', 0, {});
            expect(result).toEqual([
        ...
        it('should handle API errors', async () => {
            ...
            mockTransport.microsoftApiRequestAllItems.mockRejectedValue(apiError);
            await expect(getCalendarGroups.call(mockLoadOptionsFunctions)).rejects.toThrow('Failed to fetch calendar groups');
        });
        ...
        it('should handle timeout scenarios', async () => {
            ...
            await expect(getCalendarGroups.call(mockLoadOptionsFunctions)).rejects.toThrow('Request timeout');
        });
    });
    describe('Edge Cases and Integration', () => {
        it('should handle network connectivity issues', async () => {
            ...
            await expect(getCategoriesNames.call(mockLoadOptionsFunctions)).rejects.toThrow('Network error');
            await expect(getFolders.call(mockLoadOptionsFunctions)).rejects.toThrow('Network error');
            await expect(getCalendarGroups.call(mockLoadOptionsFunctions)).rejects.toThrow('Network error');
        });
        it('should handle malformed API responses', async () => {
            mockTransport.microsoftApiRequestAllItems.mockResolvedValue(null);
        ...
        it('should handle authentication errors', async () => {
            ...
            await expect(getCategoriesNames.call(mockLoadOptionsFunctions)).rejects.toThrow('Authentication failed');
        });
        it('should handle rate limiting errors', async () => {
            const rateLimitError = new Error('Rate limit exceeded');
            mockTransport.microsoftApiRequestAllItems.mockRejectedValue(rateLimitError);
            await expect(getCalendarGroups.call(mockLoadOptionsFunctions)).rejects.toThrow('Rate limit exceeded');
        });
    });
