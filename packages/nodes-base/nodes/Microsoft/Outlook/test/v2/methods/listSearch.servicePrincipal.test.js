...
describe('MicrosoftOutlookV2 - listSearch Service Principal mailbox rewrite', () => {
    ...
    beforeEach(() => {
        ...
        mockLoadOptionsFunctions.getNodeParameter.mockImplementation(((name) => {
    ...
    it('does not call the API and surfaces the static error when the mailbox is empty', async () => {
        ...
        await expect(searchFolders.call(mockLoadOptionsFunctions)).rejects.toThrow('A mailbox is required for the Service Principal');
        expect(mockRequestWithAuthentication).not.toHaveBeenCalled();
    });
});
//# sourceMappingURL=listSearch.servicePrincipal.test.js.map