...
import { createMessage, makeRecipient, prepareContactFields, prepareFilterString, simplifyOutputMessages, validateMailbox, } from '../../../v2/helpers/utils';
describe('Test MicrosoftOutlookV2, makeRecipient', () => {
    it('should create recipient object', () => {
        ...
        const result = replyTo.split(',').map((recipient) => {
...
describe('Test MicrosoftOutlookV2, simplifyOutputMessages', () => {
    it('should create recipient object', () => {
        const responseData = {
            '@odata.context': "https://graph.microsoft.com/v1.0/$metadata#users('')/messages(id,conversationId,subject,bodyPreview,from,toRecipients,categories,hasAttachments)/$entity",
...
describe('Test MicrosoftOutlookV2, validateMailbox', () => {
    ...
    it('should accept a B2B guest (#EXT#) UPN', () => {
        expect(() => validateMailbox('user_contoso.com#EXT#@tenant.onmicrosoft.com', node)).not.toThrow();
    });
    it('should reject a bare host/domain (not a valid mailbox identifier)', () => {
        expect(() => validateMailbox('contoso.onmicrosoft.com', node)).toThrow('The mailbox is not valid');
    ...
    it('should reject an encoded-traversal input (validate runs before encode)', () => {
        expect(() => validateMailbox('..%2f..%2fadmin@evil.com', node)).toThrow('The mailbox is not valid');
    });
