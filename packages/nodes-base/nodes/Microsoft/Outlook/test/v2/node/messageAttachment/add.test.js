...
describe('Microsoft Outlook V2 - MessageAttachment:add', () => {
    ...
    describe('Small file (under 3MB)', () => {
        beforeEach(() => {
            ...
            mockExecuteFunctions.helpers.getBinaryDataBuffer.mockResolvedValue(Buffer.from('Hello World'));
        });
        it('should add small attachment successfully', async () => {
            const mockResponse = {
                '@odata.context': "https://graph.microsoft.com/v1.0/$metadata#users('user-id')/messages('AAMkAGI2TG93AAA%3D')/attachments/$entity",
            ...
            expect(microsoftApiRequestSpy).toHaveBeenCalledWith('POST', '/messages/AAMkAGI2TG93AAA=/attachments', 0, {
                '@odata.type': '#microsoft.graph.fileAttachment',
                name: 'test-file.txt',
        ...
        it('should use custom filename when provided', async () => {
            ...
            mockExecuteFunctions.helpers.assertBinaryData.mockReturnValue(mockBinaryData);
            mockExecuteFunctions.helpers.getBinaryDataBuffer.mockResolvedValue(Buffer.from('%PDF-1.4\n'));
            ...
            expect(microsoftApiRequestSpy).toHaveBeenCalledWith('POST', '/messages/AAMkAGI2TG93AAA=/attachments', 2, {
                '@odata.type': '#microsoft.graph.fileAttachment',
        ...
        it('should throw error when fileName is not set', async () => {
            ...
            mockExecuteFunctions.helpers.assertBinaryData.mockReturnValue(mockBinaryData);
            mockExecuteFunctions.helpers.getBinaryDataBuffer.mockResolvedValue(Buffer.from('Hello World'));
            await expect(execute.call(mockExecuteFunctions, 0, [{ json: {} }])).rejects.toThrow(NodeOperationError);
            await expect(execute.call(mockExecuteFunctions, 0, [{ json: {} }])).rejects.toThrow('File name is not set');
        });
    });
    describe('Large file (over 3MB) with chunked upload', () => {
        ...
        it('should create upload session and upload large file in chunks', async () => {
            ...
            const result = await execute.call(mockExecuteFunctions, 0, [{ json: {} }]);
            expect(microsoftApiRequestSpy).toHaveBeenCalledWith('POST', '/messages/AAMkAGI2TG93BBB=/attachments/createUploadSession', 0, {
                AttachmentItem: {
                    attachmentType: 'file',
                    name: 'large-file.bin',
                    size: 4 * 1024 * 1024,
            ...
            expect(mockExecuteFunctions.helpers.request).toHaveBeenCalledWith(uploadUrl, expect.objectContaining({
                method: 'PUT',
        ...
        it('should throw error when upload session fails', async () => {
            ...
            await expect(execute.call(mockExecuteFunctions, 0, [{ json: {} }])).rejects.toThrow('Failed to get upload session');
        });
    });
});
