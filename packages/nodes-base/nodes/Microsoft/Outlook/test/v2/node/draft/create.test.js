...
describe('Test MicrosoftOutlookV2, draft => create', () => {
    ...
    describe('with attachments', () => {
        ...
        it('should properly await attachments with binary data stream', async () => {
            ...
            mockExecuteFunctions.helpers.binaryToBuffer.mockResolvedValue(Buffer.from('Hello World'));
            const mockResponse = {
                id: 'draft-id-123',
                subject: 'Test Draft with Attachment',
            };
            ...
            expect(microsoftApiRequestSpy).toHaveBeenCalledWith('POST', '/messages', 0, expect.objectContaining({
                subject: 'Test Draft with Attachment',
        ...
        it('should properly await attachments with direct binary data', async () => {
            ...
            expect(microsoftApiRequestSpy).toHaveBeenCalledWith('POST', '/messages', 2, expect.objectContaining({
                attachments: [
                    {
                        '@odata.type': '#microsoft.graph.fileAttachment',
        ...
        it('should properly await multiple attachments', async () => {
            ...
            mockExecuteFunctions.helpers.getBinaryStream.mockResolvedValue(mockStream1);
            mockExecuteFunctions.helpers.binaryToBuffer.mockResolvedValue(Buffer.from('file1'));
            microsoftApiRequestSpy.mockResolvedValue({});
