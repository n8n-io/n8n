...
import { createMessage, downloadAttachments, makeRecipient, microsoftApiRequest, microsoftApiRequestAllItems, } from './GenericFunctions';
import { messageAttachmentFields, messageAttachmentOperations, } from './MessageAttachmentDescription';
...
export class MicrosoftOutlookV1 {
    description;
            ...
            if (operation === 'get') {
                for (let i = 0; i < length; i++) {
                    try {
                        ...
                        responseData = await microsoftApiRequest.call(this, 'GET', `/messages/${messageId}`, undefined, qs);
            ...
            if (operation === 'update') {
                for (let i = 0; i < length; i++) {
                    try {
                        ...
                        const body = createMessage(updateFields);
            ...
            if (operation === 'create') {
                for (let i = 0; i < length; i++) {
                    try {
                        ...
                        const body = createMessage(additionalFields);
                        ...
                        if (additionalFields?.recipients) {
                            ...
                            if (recipients.length !== 0) {
                                await microsoftApiRequest.call(this, 'PATCH', `/messages/${messageId}`, {
                                    toRecipients: recipients.map((recipient) => makeRecipient(recipient)),
                                });
                            }
                        }
            ...
            if (operation === 'reply') {
                for (let i = 0; i < length; i++) {
                    try {
                        ...
                        responseData = await microsoftApiRequest.call(this, 'POST', `/messages/${messageId}/${action}`, body);
                        if (additionalFields.attachments) {
                            const attachments = additionalFields.attachments
                                .attachments;
                            // // Handle attachments
                            ...
                            for (const attachment of data) {
                                await microsoftApiRequest.call(this, 'POST', `/messages/${responseData.id}/attachments`, attachment, {});
                            }
                        }
                        if (send) {
            ...
            if (operation === 'getAll') {
                ...
                for (let i = 0; i < length; i++) {
                    try {
                        ...
                        if (returnAll) {
                            responseData = await microsoftApiRequestAllItems.call(this, 'value', 'GET', endpoint, undefined, qs);
                        }
                        else {
            ...
            if (operation === 'send') {
                for (let i = 0; i < length; i++) {
                    try {
                        ...
                        // Create message object from optional fields
                        const message = createMessage(additionalFields);
                        if (additionalFields.attachments) {
                            const attachments = additionalFields.attachments
                                .attachments;
                            // // Handle attachments
                ...
                for (let i = 0; i < length; i++) {
                    try {
                        ...
                        if (dataBuffer.length > 3e6) {
                            ...
                            responseData = await microsoftApiRequest.call(this, 'POST', `/messages/${messageId}/attachments/createUploadSession`, body);
                            ...
                            responseData = await microsoftApiRequest.call(this, 'POST', `/messages/${messageId}/attachments`, body, {});
                        }
                        returnData.push({ success: true });
                    }
                    catch (error) {
            ...
            if (operation === 'download') {
                for (let i = 0; i < length; i++) {
                    try {
                        ...
                        const attachmentDetails = await microsoftApiRequest.call(this, 'GET', `/messages/${messageId}/attachments/${attachmentId}`, undefined, { $select: 'id,name,contentType' });
                        let mimeType;
                        if (attachmentDetails.contentType) {
                            mimeType = attachmentDetails.contentType;
                        }
                        const fileName = attachmentDetails.name;
                        const response = await microsoftApiRequest.call(this, 'GET', `/messages/${messageId}/attachments/${attachmentId}/$value`, undefined, {}, undefined, {}, { encoding: null, resolveWithFullResponse: true });
                        const newItem = {
                            json: items[i].json,
                            binary: {},
                        };
            ...
            if (operation === 'get') {
                for (let i = 0; i < length; i++) {
                    try {
                        ...
                        responseData = await microsoftApiRequest.call(this, 'GET', `/messages/${messageId}/attachments/${attachmentId}`, undefined, qs);
                        returnData.push(responseData);
                    }
                    catch (error) {
                        if (this.continueOnFail()) {
            ...
            if (operation === 'getAll') {
                for (let i = 0; i < length; i++) {
                    try {
                        ...
                        if (returnAll) {
                            responseData = await microsoftApiRequestAllItems.call(this, 'value', 'GET', '/mailFolders', {}, qs);
                        }
                        else {
                            qs.$top = this.getNodeParameter('limit', i);
            ...
            if (operation === 'getChildren') {
                for (let i = 0; i < length; i++) {
                    try {
                        ...
                        if (returnAll) {
                            responseData = await microsoftApiRequestAllItems.call(this, 'value', 'GET', `/mailFolders/${folderId}/childFolders`, qs);
                        }
                        else {
                            qs.$top = this.getNodeParameter('limit', i);
                            responseData = await microsoftApiRequest.call(this, 'GET', `/mailFolders/${folderId}/childFolders`, undefined, qs);
                            responseData = responseData.value;
                        }
                        returnData.push.apply(returnData, responseData);
                    }
