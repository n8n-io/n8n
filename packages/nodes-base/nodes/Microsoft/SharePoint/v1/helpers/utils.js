...
export const escapeFilterValue = (value) => value.replaceAll("'", "''");
export async function simplifyItemPostReceive(items, _response) {
    if (items.length === 0) {
        return items;
    }
...
export async function simplifyListPostReceive(items, _response) {
    if (items.length === 0) {
        return items;
    }
    const simplify = this.getNodeParameter('simplify');
...
export async function downloadFilePostReceive(_items, response) {
    let fileName;
...
export async function uploadFilePreSend(requestOptions) {
...
export async function itemGetAllFieldsPreSend(requestOptions) {
...
export async function itemColumnsPreSend(requestOptions) {
    ...
    if (['upsert', 'update'].includes(operation) && mapperValue.matchingColumns?.length > 0) {
        if (!mapperValue.matchingColumns.includes('id')) {
            ...
            const response = await microsoftSharePointApiRequest.call(this, 'GET', `/sites/${site}/lists/${list}/items`, {}, {
...
export async function handleErrorPostReceive(data, response) {
    if (String(response.statusCode).startsWith('4') || String(response.statusCode).startsWith('5')) {
        ...
        if (resource === 'file' && operation === 'download' && Buffer.isBuffer(response.body)) {
            response.body = jsonParse(response.body.toString());
        }
