...
describe('Microsoft SharePoint Node', () => {
    let executeSingleFunctions;
    beforeEach(() => {
        executeSingleFunctions = mock();
    });
    afterEach(() => {
        vi.resetAllMocks();
    });
    it('should download file post receive', async () => {
        ...
        expect(mockPrepareBinaryData).toHaveBeenCalledWith(mockResponse.body, 'encoded name.pdf', 'application/pdf');
