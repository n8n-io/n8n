...
describe('Microsoft SharePoint v2 — site selection', () => {
    ...
    it('searches sites by name using the literal `search` parameter', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/v1.0/sites', {}, {
            search: 'marketing',
    ...
    it('points app-only sign-ins without search rights at the URL mode', async () => {
        ...
        apiRequest.mockRejectedValue(new NodeApiError(mock(), { message: 'refused' }, { httpCode: '403' }));
        let thrown;
        try {
            await getSites.call(ctx);
        }
    ...
    it('points delegated sign-ins without search rights at URL or ID mode', async () => {
        ...
        apiRequest.mockRejectedValue(new NodeApiError(mock(), {}, { httpCode: '403', message: 'the credential may be missing the Sites.Read.All permission' }));
        let thrown;
        try {
            await getSites.call(ctx);
        }
        ...
        expect(description.indexOf('URL or ID mode')).toBeLessThan(description.indexOf('Sites.Read.All'));
    ...
    it('passes through a non-403 delegated error unchanged', async () => {
        ctx.getNodeParameter.mockReturnValue('microsoftOAuth2Api');
        const original = new NodeApiError(mock(), { message: 'boom' }, { httpCode: '500' });
...
describe('Microsoft SharePoint v2 — resolveSiteId', () => {
    ...
    const apiRequest = transport.microsoftApiRequest;
    const setSite = (site) => {
        ctx.getNodeParameter.mockImplementation((name, _itemIndex, fallback) => (name === 'site' ? site : fallback));
    };
    beforeEach(() => {
        vi.clearAllMocks();
        ctx = mockDeep();
    ...
    it.each([
        ...
        await expect(resolveSiteId.call(ctx, 0)).resolves.toBe('contoso.sharepoint.com,2C712604-1370-44E7-A1F5-426573FDA80A,2D2244C3-251A-49EA-93A8-39E1C3A060FE');
    ...
    it("re-encodes path segments so a raw ':' can't escape the {host}:{path} shape", async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', '/v1.0/sites/contoso.sharepoint.com:/sites/a%3Ab', {}, { $select: 'id' });
    });
    it('does not double-encode an already-encoded path segment', async () => {
        ...
        await resolveSiteId.call(ctx, 0);
        expect(apiRequest).toHaveBeenCalledWith('GET', '/v1.0/sites/contoso.sharepoint.com:/sites/My%20Site', {}, { $select: 'id' });
    });
    const COMPOSITE_ID = 'contoso.sharepoint.com,2C712604-1370-44E7-A1F5-426573FDA80A,2D2244C3-251A-49EA-93A8-39E1C3A060FE';
    it.each([
    ...
    it('resolves a repeated site URL once when given a per-run cache', async () => {
        ...
        await expect(resolveSiteId.call(ctx, 0, siteIdCache)).resolves.toBe('contoso.sharepoint.com,2C712604-1370-44E7-A1F5-426573FDA80A,2D2244C3-251A-49EA-93A8-39E1C3A060FE');
        await expect(resolveSiteId.call(ctx, 1, siteIdCache)).resolves.toBe('contoso.sharepoint.com,2C712604-1370-44E7-A1F5-426573FDA80A,2D2244C3-251A-49EA-93A8-39E1C3A060FE');
