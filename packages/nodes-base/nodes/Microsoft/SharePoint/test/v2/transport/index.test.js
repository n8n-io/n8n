...
import { getSharePointCredentialType, microsoftApiRequest, microsoftApiRequestAllItems, SERVICE_PRINCIPAL_AUTH, } from '../../../v2/transport';
describe('Microsoft SharePoint v2 Transport', () => {
    let ctx;
    let mockNode;
    let mockRequestOAuth2;
    ...
    describe('graphApiBaseUrl from credentials', () => {
        it.each([
            ...
            expect(mockRequestOAuth2).toHaveBeenCalledWith('microsoftOAuth2Api', expect.objectContaining({ uri: `${baseUrl}/v1.0/sites/s/lists/l` }));
        });
        it('should fall back to the global cloud when graphApiBaseUrl is empty', async () => {
            ...
            await microsoftApiRequest.call(ctx, 'GET', '/v1.0/sites/s/lists/l');
            expect(mockRequestOAuth2).toHaveBeenCalledWith('microsoftOAuth2Api', expect.objectContaining({ uri: 'https://graph.microsoft.com/v1.0/sites/s/lists/l' }));
        });
        ...
        it('should strip trailing slashes from the base URL', async () => {
            ...
            await microsoftApiRequest.call(ctx, 'GET', '/v1.0/sites/s/lists/l');
            expect(mockRequestOAuth2).toHaveBeenCalledWith('microsoftOAuth2Api', expect.objectContaining({ uri: 'https://graph.microsoft.us/v1.0/sites/s/lists/l' }));
        });
        it('should honor a sovereign graphApiBaseUrl under the Service Principal', async () => {
            setParams({ authentication: SERVICE_PRINCIPAL_AUTH });
            ctx.getCredentials.mockResolvedValue({ graphApiBaseUrl: 'https://graph.microsoft.us' });
            await microsoftApiRequest.call(ctx, 'GET', '/v1.0/sites/s/lists/l');
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith(SERVICE_PRINCIPAL_AUTH, expect.objectContaining({ uri: 'https://graph.microsoft.us/v1.0/sites/s/lists/l' }));
        });
    });
    describe('auth routing', () => {
        ...
        it('should pass an explicit uri through verbatim', async () => {
            ...
            expect(mockRequestOAuth2).toHaveBeenCalledWith('microsoftOAuth2Api', expect.objectContaining({ uri: nextLink }));
        });
    });
    describe('microsoftApiRequestAllItems', () => {
        it('follows @odata.nextLink across pages until it runs out (Return All)', async () => {
            ...
            const result = await microsoftApiRequestAllItems.call(ctx, 'value', 'GET', '/v1.0/sites/s/lists', {}, { $select: 'id' });
        ...
        it('handles uneven ("short") page sizes across Return All pages', async () => {
            ...
            const result = await microsoftApiRequestAllItems.call(ctx, 'value', 'GET', '/v1.0/sites/s/lists');
        ...
        it('stops exactly at the limit even when the closing page still carries a nextLink (Limit mode)', async () => {
            ...
            const result = await microsoftApiRequestAllItems.call(ctx, 'value', 'GET', '/v1.0/sites/s/lists', {}, {}, 6);
            expect(result).toEqual([
                { id: '1' },
                { id: '2' },
                { id: '3' },
        ...
        it('returns everything found when the limit exceeds the total available', async () => {
            ...
            const result = await microsoftApiRequestAllItems.call(ctx, 'value', 'GET', '/v1.0/sites/s/lists', {}, {}, 50);
            expect(result).toEqual([
                { id: '1' },
                { id: '2' },
                { id: '3' },
        ...
        it('stops after the first page when it alone already satisfies the limit', async () => {
            ...
            const result = await microsoftApiRequestAllItems.call(ctx, 'value', 'GET', '/v1.0/sites/s/lists', {}, {}, 3);
        ...
        it('makes exactly one call and returns nothing when limit is 0', async () => {
            ...
            const result = await microsoftApiRequestAllItems.call(ctx, 'value', 'GET', '/v1.0/sites/s/lists', {}, {}, 0);
            expect(result).toEqual([]);
            expect(mockRequestOAuth2).toHaveBeenCalledTimes(1);
        });
        it('clamps a negative limit to an empty result instead of slicing from the end', async () => {
            ...
            const result = await microsoftApiRequestAllItems.call(ctx, 'value', 'GET', '/v1.0/sites/s/lists', {}, {}, -5);
        ...
        it('re-sends custom headers on every page, unlike qs', async () => {
            mockRequestOAuth2
                ...
                .mockResolvedValueOnce({ value: [{ id: '2' }] });
            await microsoftApiRequestAllItems.call(ctx, 'value', 'GET', '/v1.0/sites/s/lists', {}, { $select: 'id' }, undefined, { Prefer: 'HonorNonIndexedQueriesWarningMayFailRandomly' });
    ...
    describe('permission refusals', () => {
        it('should name the missing delegated permission on a 403 under OAuth2', async () => {
            ...
            await expect(microsoftApiRequest.call(ctx, 'GET', '/v1.0/sites/s/lists/l')).rejects.toThrow(/Sites\.Read\.All/);
        });
        it('should name the missing delegated permission on a 403 for list:getAll', async () => {
            ...
            await expect(microsoftApiRequest.call(ctx, 'GET', '/v1.0/sites/s/lists')).rejects.toThrow(/Sites\.Read\.All/);
        });
        it('should name the missing application permission on a 403 under the Service Principal', async () => {
            ...
            await expect(microsoftApiRequest.call(ctx, 'GET', '/v1.0/sites/s/lists/l')).rejects.toThrow(/Sites\.Read\.All \(or Sites\.Selected/);
        });
        it('should offer Files.Read.All for document-library items as an alternative on a 403 for item:get', async () => {
            ...
            await expect(microsoftApiRequest.call(ctx, 'GET', '/v1.0/sites/s/lists/l/items/1')).rejects.toThrow(/Files\.Read\.All for document-library items/);
        });
        ...
        it('should rewrite a Graph NotFound to name the node resource under OAuth2', async () => {
            ...
            await expect(microsoftApiRequest.call(ctx, 'GET', '/v1.0/sites/s/lists/l')).rejects.toThrow('List not found');
        });
        it('should rewrite a Graph NotFound to name the item resource under OAuth2', async () => {
            ...
            await expect(microsoftApiRequest.call(ctx, 'GET', '/v1.0/sites/s/lists/l/items/i')).rejects.toThrow('Item not found');
        });
        ...
        it('should keep a static sanitized message for other Service Principal errors', async () => {
            ...
            expect(thrown?.message).toBe("Microsoft Graph rejected the request (HTTP 500). Check the operation's inputs and the app registration's permissions.");
        });
        it('should let fixed safe Graph wording through the Service Principal sanitizer', async () => {
            ...
            await expect(microsoftApiRequest.call(ctx, 'POST', '/v1.0/sites/s/lists/l/items')).rejects.toMatchObject({
                message: 'Microsoft Graph rejected the request (HTTP 400): One or more fields with unique constraints already has the provided value.',
            });
        });
    ...
    describe('same-origin guard', () => {
        it('refuses a uri override pointing at a different host', async () => {
            ctx.getCredentials.mockResolvedValue({ graphApiBaseUrl: 'https://graph.microsoft.us' });
            await expect(microsoftApiRequest.call(ctx, 'GET', '', {}, {}, 'https://graph.microsoft.com/v1.0/sites?$skiptoken=abc')).rejects.toThrow('Refusing to send credentials to an unexpected host');
            expect(mockRequestOAuth2).not.toHaveBeenCalled();
        });
    ...
    describe('load-options fallback', () => {
        it('should treat the literal fallback 0 as the default credential type', () => {
            ...
            ctx.getNodeParameter.mockReturnValue(0);
            expect(getSharePointCredentialType.call(ctx)).toBe('microsoftOAuth2Api');
        });
    });
    describe('binary bodies', () => {
        it('passes a Buffer body through raw, with the caller Content-Type winning', async () => {
            ...
            await microsoftApiRequest.call(ctx, 'PUT', '/v1.0/sites/s/drive/items/f:/a.png:/content', payload, {}, undefined, { 'Content-Type': 'image/png' });
