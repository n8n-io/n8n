...
import { odataFieldEqualsClause, SHAREPOINT_ILLEGAL_FILE_NAME_CHARS, validateSharePointFileName, } from '../../../v2/helpers/utils';
describe('Microsoft SharePoint v2 — validateSharePointFileName', () => {
    ...
    it.each([undefined, '', '   '])('rejects a missing or blank name (%j)', (fileName) => {
        expect(() => validateSharePointFileName(node, fileName, 0)).toThrow('File name must be set!');
    });
    it.each(SHAREPOINT_ILLEGAL_FILE_NAME_CHARS)('rejects a name containing %s', (char) => {
        expect(() => validateSharePointFileName(node, `a${char}b.txt`, 0)).toThrow(`contains characters that SharePoint doesn't allow: ${char}`);
    });
    it('names every offending character at once', () => {
        expect(() => validateSharePointFileName(node, 'a:b*c.txt', 0)).toThrow("contains characters that SharePoint doesn't allow: * :");
    });
    it('suggests a colon-free timestamp format only when a colon is present', () => {
        try {
            ...
            expect(error.description).toContain('colon-free format');
        }
        try {
            ...
            expect(error.description).not.toContain('colon-free format');
        }
    });
});
describe('Microsoft SharePoint v2 — odataFieldEqualsClause', () => {
    ...
    it('leaves characters the URL layer owns untouched', () => {
        expect(odataFieldEqualsClause('Title', 'a & b % c + d')).toBe("fields/Title eq 'a & b % c + d'");
    });
});
