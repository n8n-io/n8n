...
describe('Microsoft SharePoint v2 — File: Upload', () => {
    ...
    it('uploads the binary bytes to the chosen folder and returns the drive item', async () => {
        ...
        expect(resource).toBe(`/v1.0/sites/${ENCODED_SITE_ID}/drive/items/${FOLDER_ID}:/report.csv:/content`);
    ...
    it('encodes the file name and folder ID into their path segments', async () => {
        ...
        expect(resource).toBe(`/v1.0/sites/${ENCODED_SITE_ID}/drive/items/weird%20folder%23id:/Q1%20report%20%23final.csv:/content`);
    });
    it('falls back to an octet-stream content type when the binary has none', async () => {
        ctx.helpers.assertBinaryData.mockReturnValue(inMemoryBinary({ mimeType: undefined }));
        await execute.call(ctx, 0);
    ...
    it('reads filesystem-mode binary data through the buffer helper after the size check', async () => {
        ctx.helpers.assertBinaryData.mockReturnValue(inMemoryBinary({ id: 'binary-data-id', data: '' }));
    ...
    describe('filename validation (before anything is sent)', () => {
        it.each(SHAREPOINT_ILLEGAL_FILE_NAME_CHARS)('rejects a file name containing %s and names the character', async (char) => {
    ...
    describe('size cap (before anything is sent)', () => {
        it('rejects an oversized file from its metadata, without reading it', async () => {
            ...
            await expect(execute.call(ctx, 0)).rejects.toThrow('The file is 251 MB, which is larger than the 250 MB limit for SharePoint uploads');
    ...
    it('resolves a pasted site URL before uploading', async () => {
        ...
        apiRequest.mockImplementation(async (_method, resource) => resource.startsWith('/v1.0/sites/contoso.sharepoint.com:')
            ? { id: SITE_ID }
            : DRIVE_ITEM_REPLY);
        const response = await execute.call(ctx, 0);
    ...
    describe('router wiring', () => {
        ...
        it('rejects an unsupported file operation', async () => {
            ...
            await expect(node.execute.call(ctx)).rejects.toThrow('The operation "delete" is not supported!');
        });
    });
});
