...
const DOWNLOAD_URL = 'https://contoso.sharepoint.com/_layouts/15/download.aspx?UniqueId=abc&tempauth=eyJ0eXAi';
...
describe('Microsoft SharePoint v2 — File: Download', () => {
    ...
    it('downloads the file bytes under the correct name and mime type', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', `/v1.0/sites/${ENCODED_SITE_ID}/drive/items/${FILE_ID}`);
        const [payload, fileName, mimeType] = ctx.helpers.prepareBinaryData.mock.calls[0];
        expect(payload.equals(FILE_BYTES)).toBe(true);
    ...
    it('reports a failed content fetch without exposing the download link', async () => {
        ...
        ctx.helpers.httpRequest.mockRejectedValue(Object.assign(new Error('Request failed with status code 403'), {
        ...
        expect(thrown?.message).toBe('Downloading the file contents failed: Request failed with status code 403');
