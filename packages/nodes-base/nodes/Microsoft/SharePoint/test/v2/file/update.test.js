...
import { MAX_SIMPLE_UPLOAD_BYTES, SHAREPOINT_ILLEGAL_FILE_NAME_CHARS, } from '../../../v2/helpers/utils';
...
describe('Microsoft SharePoint v2 — File: Update', () => {
    ...
    const inMemoryBinary = (overrides = {}) => ({
    ...
    it('renames first, then replaces contents, returning the content reply', async () => {
        ...
        apiRequest.mockImplementation(async (method) => method === 'PATCH' ? PATCH_REPLY : PUT_REPLY);
        const result = await execute.call(ctx, 0);
    ...
    it('names the already-applied rename when replacing contents fails afterwards', async () => {
        ...
        apiRequest.mockImplementation(async (method) => {
            if (method === 'PATCH')
                return PATCH_REPLY;
            throw new NodeOperationError(mock(), 'Graph rejected the upload');
        });
        let thrown;
        try {
        ...
        expect(thrown?.message).toBe('The file was renamed to "renamed.csv", but replacing its contents failed: Graph rejected the upload');
    ...
    it('rejects an oversized replacement before the rename is sent', async () => {
        ...
        await expect(execute.call(ctx, 0)).rejects.toThrow('larger than the 250 MB limit for SharePoint uploads');
        expect(apiRequest).not.toHaveBeenCalled();
    });
    it.each(SHAREPOINT_ILLEGAL_FILE_NAME_CHARS)('rejects a new name containing %s before any request', async (char) => {
        setParams({ ...baseParams, fileName: `renamed${char}v2.csv` });
        await expect(execute.call(ctx, 0)).rejects.toThrow(`contains characters that SharePoint doesn't allow: ${char}`);
        expect(apiRequest).not.toHaveBeenCalled();
    });
