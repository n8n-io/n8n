...
const contentTypesReply = (columns) => ({
...
describe('Microsoft SharePoint v2 — list columns as form fields', () => {
    ...
    const setParams = (params) => {
        ctx.getNodeParameter.mockImplementation((name, fallback) => (name in params ? params[name] : fallback));
    };
    beforeEach(() => {
        vi.clearAllMocks();
        ctx = mockDeep();
    ...
    it('maps a full contentTypes reply to the v1 baseline fields plus the documented v2 deltas', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', `/v1.0/sites/${ENCODED_SITE_ID}/lists/${LIST_ID}/contentTypes`, {}, { expand: 'columns' });
    ...
    it('loads the fields with both sign-in methods', async () => {
        ...
        const { microsoftApiRequest: realMicrosoftApiRequest } = await vi.importActual('../../../v2/transport');
        apiRequest
    ...
    it('accepts a list title and encodes it in the request path', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', `/v1.0/sites/${ENCODED_SITE_ID}/lists/My%20List%201/contentTypes`, {}, { expand: 'columns' });
    });
    ...
    it.each(['add', 'update', 'upsert'])('builds the shared columns property for the %s mode', (mode) => {
