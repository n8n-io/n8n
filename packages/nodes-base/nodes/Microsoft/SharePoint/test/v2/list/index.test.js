...
describe('Microsoft SharePoint v2 — list search', () => {
    ...
    it('requests a site’s lists without a server-side filter', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', `/v1.0/sites/${ENCODED_SITE_ID}/lists`, {}, { $select: 'id,displayName' });
    });
    ...
    it('resolves a URL-mode site the same way the action does', async () => {
        ...
        expect(apiRequest).toHaveBeenNthCalledWith(2, 'GET', `/v1.0/sites/${ENCODED_SITE_ID}/lists`, {}, { $select: 'id,displayName' });
    });
