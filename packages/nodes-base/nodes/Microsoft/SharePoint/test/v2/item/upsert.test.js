...
describe('Microsoft SharePoint v2 — Item: Create or Update', () => {
    ...
    it('updates the single matched item and returns the item envelope', async () => {
        ...
        expect(apiRequestAllItems).toHaveBeenCalledWith('value', 'GET', ITEMS_PATH, {}, { $filter: "fields/Title eq 'Title 2'" }, 2, { Prefer: 'HonorNonIndexedQueriesWarningMayFailRandomly' });
        expect(apiRequest).toHaveBeenCalledTimes(2);
        expect(apiRequest).toHaveBeenNthCalledWith(1, 'PATCH', `${ITEMS_PATH}/${ITEM_ID}/fields`, { Title: 'Title 2' }, {}, undefined, {});
        expect(apiRequest).toHaveBeenNthCalledWith(2, 'GET', `${ITEMS_PATH}/${ITEM_ID}`, {}, { $expand: 'fields' });
    ...
    it('creates a new item when nothing matches, mapping the real column values', async () => {
        ...
        expect(apiRequest).toHaveBeenNthCalledWith(1, 'POST', ITEMS_PATH, { fields: { Title: 'Title 2' } }, {}, undefined, {});
    ...
    it('throws and does neither when several items match (AC#2)', async () => {
        ...
        await expect(node.execute.call(ctx)).rejects.toThrow('Multiple items match the selected column(s)');
    ...
    it('folds split hyperlink fields into the two-part shape when updating', async () => {
        ...
        expect(apiRequest).toHaveBeenNthCalledWith(1, 'PATCH', `${ITEMS_PATH}/${ITEM_ID}/fields`, { Link: { Url: 'https://example.com', Description: 'Example' } }, {}, undefined, { Prefer: 'apiversion=2.1' });
    });
    it('registers the Create or Update operation and dispatches to it', () => {
        const operationProp = itemActions.description[0];
        expect(operationProp.options).toContainEqual(expect.objectContaining({ name: 'Create or Update', value: 'upsert' }));
