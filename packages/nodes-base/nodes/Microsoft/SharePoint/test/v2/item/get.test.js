...
const buildSimplified = () => {
    const expected = {
        ...GRAPH_ITEM_REPLY,
        fields: { ...GRAPH_ITEM_REPLY.fields },
    };
    delete expected['@odata.context'];
    delete expected['@odata.etag'];
    delete expected['fields@odata.navigationLink'];
    delete expected.fields['@odata.etag'];
    return expected;
};
const SITE_ID = 'contoso.sharepoint.com,2C712604-1370-44E7-A1F5-426573FDA80A,2D2244C3-251A-49EA-93A8-39E1C3A060FE';
...
describe('Microsoft SharePoint v2 — Item: Get', () => {
    ...
    beforeEach(() => {
        ...
        ctx.getNode.mockReturnValue(mock({ typeVersion: 2 }));
    ...
    it('narrows the request and returns v1-trimmed fields when Simplify is on', async () => {
        ...
        expect(apiRequest).toHaveBeenCalledWith('GET', `/v1.0/sites/${ENCODED_SITE_ID}/lists/${LIST_ID}/items/${ITEM_ID}`, {}, 
    ...
    it('sends ?expand=fields and keeps the raw body when Simplify is off', async () => {
        ...
        const result = await node.execute.call(ctx);
        expect(apiRequest).toHaveBeenCalledWith('GET', `/v1.0/sites/${ENCODED_SITE_ID}/lists/${LIST_ID}/items/${ITEM_ID}`, {}, { $expand: 'fields' });
    ...
    it('resolves a site URL and accepts a list title, encoding each segment', async () => {
        ...
        expect(apiRequest).toHaveBeenNthCalledWith(1, 'GET', '/v1.0/sites/contoso.sharepoint.com:/sites/site1', {}, { $select: 'id' });
        expect(apiRequest).toHaveBeenNthCalledWith(2, 'GET', `/v1.0/sites/${ENCODED_SITE_ID}/lists/My%20List%201/items/${ITEM_ID}`, {}, { $select: ITEM_SIMPLIFY_SELECT, $expand: ITEM_SIMPLIFY_EXPAND });
    });
    it('reads a pasted item ID through the resource locator', async () => {
        ...
        await node.execute.call(ctx);
        expect(apiRequest).toHaveBeenCalledWith('GET', `/v1.0/sites/${ENCODED_SITE_ID}/lists/${LIST_ID}/items/42`, {}, { $select: ITEM_SIMPLIFY_SELECT, $expand: ITEM_SIMPLIFY_EXPAND });
    });
    ...
    it.each(['.', '..'])('rejects a "%s" Item value that would change the request shape', async (bad) => {
        setParams({
            resource: 'item',
            operation: 'get',
            site: { mode: 'id', value: SITE_ID },
