...
describe('Microsoft SharePoint v2 — Item: Get Many', () => {
    ...
    it('sends the filter as $filter and opts into non-indexed queries', async () => {
        ...
        expect(ctx.helpers.requestOAuth2).toHaveBeenCalledWith('microsoftOAuth2Api', expect.objectContaining({
            uri: ITEMS_URI,
    ...
    it('selects only the chosen fields and expands fields when included', async () => {
        ...
        await node.execute.call(ctx);
        expect(ctx.helpers.requestOAuth2).toHaveBeenCalledWith('microsoftOAuth2Api', expect.objectContaining({ qs: { $select: 'id,webUrl,fields', $expand: 'fields' } }));
    });
    it('follows next-page links without re-sending query options but keeps the Prefer header', async () => {
        ...
        expect(ctx.helpers.requestOAuth2.mock.calls[1][1]).toEqual(expect.objectContaining({
            uri: NEXT_LINK,
            qs: {},
            headers: expect.objectContaining(PREFER_NON_INDEXED),
        }));
    ...
    it('keeps fetching until the limit is met when pages come back short', async () => {
        ...
        const result = await node.execute.call(ctx);
        expect(ctx.helpers.requestOAuth2).toHaveBeenCalledWith('microsoftOAuth2Api', expect.objectContaining({ qs: { $top: 3 } }));
    ...
    it('returns every item of a 10,000+ item list on Return All', async () => {
        ...
        const page = (offset) => Array.from({ length: 5000 }, (_, index) => ({ id: `${offset + index}` }));
        ctx.helpers.requestOAuth2
    ...
    it('fails naming the column when a non-indexed filter dies part-way through a big list', async () => {
        ...
        await expect(node.execute.call(ctx)).rejects.toThrow("SharePoint could not finish filtering on column(s) 'NotIndexed': large lists can only be filtered on indexed columns");
    });
