...
describe('Microsoft SharePoint v2 — Item: Update', () => {
    ...
    it("throws v1's error when no item matches the columns", async () => {
        ...
        await expect(node.execute.call(ctx)).rejects.toThrow("The column(s) don't match any existing item");
    ...
    it('escapes single quotes in the lookup filter value', async () => {
        ...
        expect(apiRequestAllItems).toHaveBeenCalledWith('value', 'GET', ITEMS_PATH, {}, { $filter: "fields/Title eq 'O''Brien'" }, 2, { Prefer: 'HonorNonIndexedQueriesWarningMayFailRandomly' });
    });
    ...
    it('reports that the update succeeded when only the read-back fails', async () => {
        ...
        await expect(node.execute.call(ctx)).rejects.toThrow('The item was updated, but reading back the updated item failed');
        expect(apiRequest).toHaveBeenCalledTimes(2);
    });
...
describe('buildItemFieldsPayload', () => {
    ...
    it('ignores prototype-polluting keys instead of writing them', () => {
        ...
        const value = jsonParse('{"__proto__": "x", "Title": "A"}');
    ...
    it('does not pollute the prototype when a hyperlink split has a __proto__ base', () => {
        ...
        const value = jsonParse('{"__proto__.Url": "polluted"}');
