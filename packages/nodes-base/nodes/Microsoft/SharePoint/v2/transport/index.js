...
function delegatedApiError(error) {
    ...
    return new NodeApiError(this.getNode(), error, errorOptions);
}
export async function microsoftApiRequest(method, resource, body = {}, qs = {}, uri, headers = {}) {
    ...
    const baseUrl = (typeof credentials.graphApiBaseUrl === 'string' && credentials.graphApiBaseUrl !== ''
        ...
        : DEFAULT_GRAPH_BASE_URL).replace(/\/+$/, '');
...
export async function microsoftApiRequestAllItems(propertyName, method, endpoint, body = {}, qs = {}, limit, headers = {}) {
    const returnData = [];
    let uri;
    do {
