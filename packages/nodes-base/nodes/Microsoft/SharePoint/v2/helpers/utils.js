...
export const LIST_SIMPLIFY_SELECT = 'id,name,displayName,description,createdDateTime,lastModifiedDateTime,webUrl';
 ...
 */
export function assertPathSegment(node, value, paramName) {
 ...
 */
export const odataFieldEqualsClause = (column, value) => `fields/${column} eq '${String(value ?? '').replaceAll("'", "''")}'`;
/** SharePoint refuses to filter on non-indexed columns without this opt-in. */
export const NON_INDEXED_QUERY_HEADERS = {
    Prefer: 'HonorNonIndexedQueriesWarningMayFailRandomly',
};
/** Graph rejects hyperlink field writes without this (see list/columns.ts). */
export const HYPERLINK_WRITE_HEADERS = { Prefer: 'apiversion=2.1' };
/**
 ...
 */
export function nonIndexedFilterThresholdError(node, error, filter) {
    ...
    return new NodeOperationError(node, `SharePoint could not finish filtering on ${subject}: large lists can only be filtered on indexed columns`, {
        description: 'Add an index to the column in SharePoint (List settings → Indexed columns) or filter on an indexed column, then try again.',
    });
}
/** Graph's unique-constraint rejection names no column; point the user at the mapper. */
export function addUniqueConstraintHint(error) {
...
export function validateSharePointFileName(node, fileName, itemIndex) {
    ...
    if (illegalChars.length > 0) {
        throw new NodeOperationError(node, `The file name "${fileName}" contains characters that SharePoint doesn't allow: ${illegalChars.join(' ')}`, {
            itemIndex,
            description: `SharePoint file names can't contain any of these characters: ${SHAREPOINT_ILLEGAL_FILE_NAME_CHARS.join(' ')}. Remove them from the file name and try again.` +
 ...
 * being read.
 */
export async function getUploadBufferWithinCap(ctx, itemIndex, binaryPropertyName) {
    ...
    if (fileSize > MAX_SIMPLE_UPLOAD_BYTES) {
        ...
        throw new NodeOperationError(ctx.getNode(), `The file is ${sizeMb} MB, which is larger than the 250 MB limit for SharePoint uploads`, {
            itemIndex,
            description: 'Files over 250 MB need to be uploaded in pieces, which this operation does not support yet.',
        });
    }
