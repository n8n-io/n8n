...
export async function execute(i, siteIdCache) {
    ...
    const metadata = (await microsoftApiRequest.call(this, 'GET', `/v1.0/sites/${encodeURIComponent(siteId)}/drive/items/${encodeURIComponent(fileId)}`));
    ...
    const binaryData = await this.helpers.prepareBinaryData(response.body, metadata.name, metadata.file.mimeType ?? response.headers['content-type']);
