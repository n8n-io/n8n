...
export async function execute(i, siteIdCache) {
    ...
    if (content) {
        try {
            response = await microsoftApiRequest.call(this, 'PUT', `${itemPath}/content`, content.body, {}, undefined, { 'Content-Type': content.contentType });
        }
        catch (error) {
            if (!wantRename)
                throw error;
            ...
            throw new NodeOperationError(this.getNode(), `The file was renamed to "${fileName}", but replacing its contents failed: ${cause}`, {
                itemIndex: i,
