...
export async function execute(i, siteIdCache) {
    ...
    const list = assertPathSegment(this.getNode(), String(this.getNodeParameter('list', i, '', { extractValue: true })), 'List');
    const item = assertPathSegment(this.getNode(), String(this.getNodeParameter('item', i, '', { extractValue: true })), 'Item');
    // 204 has no body — report the deletion ourselves, like v1.
    await microsoftApiRequest.call(this, 'DELETE', `/v1.0/sites/${encodeURIComponent(siteId)}/lists/${encodeURIComponent(list)}/items/${encodeURIComponent(item)}`);
    return { deleted: true };
}
