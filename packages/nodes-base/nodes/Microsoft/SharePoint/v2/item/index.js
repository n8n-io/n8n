...
import { searchGraphCollection } from '../helpers/graphSearch';
import { addUniqueConstraintHint, assertPathSegment, HYPERLINK_WRITE_HEADERS, NON_INDEXED_QUERY_HEADERS, nonIndexedFilterThresholdError, odataFieldEqualsClause, } from '../helpers/utils';
 ...
 */
export async function lookupItemIdByColumns(siteId, listIdOrTitle, matchingColumns, values) {
    ...
    let rows;
    try {
        rows = (await microsoftApiRequestAllItems.call(this, 'value', 'GET', `/v1.0/sites/${encodeURIComponent(siteId)}/lists/${encodeURIComponent(listIdOrTitle)}/items`, {}, { $filter: filter }, 2, NON_INDEXED_QUERY_HEADERS));
    }
    catch (error) {
 ...
 */
export async function resolveMatchedItemIds(siteId, listIdOrTitle, matchingColumns, values) {
 ...
 */
export function resolveItemMapperValues(i) {
    if (this.getNodeParameter('columns.mappingMode', i) === 'autoMapInputData') {
        ...
        return Object.fromEntries(Object.entries(this.getInputData()[i].json).filter(([key]) => knownColumns.has(key)));
    }
    return (this.getNodeParameter('columns.value', i, {}) ?? {});
}
/**
 ...
 */
export function buildItemFieldsPayload(value, schema) {
    const fields = {};
    ...
    for (const [key, fieldValue] of Object.entries(value)) {
        ...
        if (dotIndex > 0 &&
            ...
            const folded = typeof existing === 'object' && existing !== null && !Array.isArray(existing)
                ? existing
                : {};
 ...
 */
export async function updateItemFields(siteId, listIdOrTitle, itemId, value, schema) {
    ...
    try {
        await microsoftApiRequest.call(this, 'PATCH', `${itemPath}/fields`, fields, {}, undefined, hasHyperlink ? HYPERLINK_WRITE_HEADERS : {});
    }
    catch (error) {
    ...
    try {
        ...
        throw new NodeOperationError(this.getNode(), 'The item was updated, but reading back the updated item failed', {
