...
export async function getLists(filter, paginationToken) {
    ...
    let response;
    if (paginationToken) {
        response = (await microsoftApiRequest.call(this, 'GET', '', {}, {}, paginationToken));
    }
    else {
        response = (await microsoftApiRequest.call(this, 'GET', `/v1.0/sites/${encodeURIComponent(siteId)}/lists`, {}, { $select: 'id,displayName' }));
    }
    ...
    const results = (response.value ?? [])
        .filter((list) => list.id && (!filterLower || (list.displayName ?? '').toLowerCase().includes(filterLower)))
