...
import { handleErrorPostReceive, HeaderConstants, parseContainerList, parseHeaders, XMsVersion, } from '../GenericFunctions';
                    ...
                    output: {
                        postReceive: [
                            ...
                            async function (data, _response) {
                                return [
                                    {
                                        json: await parseContainerList(data[0].json),
                                    },
                                ];
                            },
                        ],
                ...
                preSend: [
                    async function (requestOptions) {
                        ...
                        if (container.length < 3 || container.length > 63) {
                            throw new NodeOperationError(this.getNode(), "'Container Name' must be from 3 through 63 characters long");
                        }
                        if (/[A-Z]/.test(container)) {
                            throw new NodeOperationError(this.getNode(), "All letters  in 'Container Name' must be lowercase");
                        }
                        if (!/^[a-z0-9-]+$/.test(container)) {
                            throw new NodeOperationError(this.getNode(), "'Container Name' can contain only letters, numbers, and the hyphen/minus (-) character");
                        }
                        if (!/^[a-z0-9].*[a-z0-9]$/.test(container)) {
                            throw new NodeOperationError(this.getNode(), "'Container Name' must start or end with a letter or number");
                        }
                        if (/--/.test(container)) {
                            throw new NodeOperationError(this.getNode(), "Consecutive hyphens are not permitted in 'Container Name'");
                        }
