...
export async function azureStorageApiRequest(method, endpoint, body = {}, qs, headers, url) {
...
export function getCanonicalizedHeadersString(requestOptions) {
    let headersArray = [];
...
export function getCanonicalizedResourceString(requestOptions, credentials) {
    const path = new URL(getUrl(requestOptions)).pathname || '/';
...
export function parseHeaders(headers) {
    ...
    const result = {};
    const metadataKeys = Object.keys(headers).filter((x) => x.startsWith(HeaderConstants.PREFIX_X_MS_META));
    for (const key in headers) {
        ...
        const newValue = parseBooleanHeaders.includes(key)
            ? parseBooleans(headers[key])
            : parseNumberHeaders.includes(key)
                ? parseNumbers(headers[key])
                : headers[key];
    ...
    if (metadataKeys.length > 0) {
        ...
        for (const key of metadataKeys) {
            result.metadata[key.replace(HeaderConstants.PREFIX_X_MS_META, '')] =
                headers[key];
        }
    }
    return result;
}
export async function parseBlobList(xml) {
    const parser = new Parser({
        explicitArray: false,
...
export async function parseContainerList(xml) {
    const parser = new Parser({
        explicitArray: false,
        tagNameProcessors: [sanitizeXmlName, firstCharLowerCase, (name) => name.replace('-', '')],
        attrNameProcessors: [sanitizeXmlName],
...
export async function getBlobs(filter, paginationToken) {
...
export async function getContainers(filter, paginationToken) {
    let response;
    const qs = {
        comp: 'list',
    };
