    ...
    for (let i = 0; i < items.length; i++) {
        ...
        try {
            ...
            if (options.downloadFields) {
                const itemWithAttachments = await downloadRecordAttachments.call(this, [responseData], options.downloadFields);
                returnData.push(...itemWithAttachments.map((item) => ({
                    ...item,
                    json: legacyFlattenOutput(item.json, nodeVersion),
                })));
                continue;
            }
            const output = legacyFlattenOutput(responseData, nodeVersion);
            ...
            error = processAirtableError(error, id, i);
