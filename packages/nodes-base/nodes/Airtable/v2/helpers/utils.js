...
import { isResourceMapperValue, UserError, } from 'n8n-workflow';
export function removeIgnored(data, ignore) {
    if (ignore) {
        let ignoreFields = [];
 ...
 */
export function valuesMatch(recordValue, inputValue) {
...
export function findMatches(data, keys, fields, updateAll) {
    if (updateAll) {
...
export function processAirtableError(error, id, itemIndex) {
...
export function legacyFlattenOutput(record, nodeVersion) {
    if (nodeVersion >= 2.2)
        return record;
 ...
 */
export function coerceArrayTypeFields(fields, columnsParam) {
    if (!isResourceMapperValue(columnsParam))
        return;
