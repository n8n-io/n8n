...
describe('Test AirtableV2, update operation', () => {
    ...
    it('should update a record by id, autoMapInputData', async () => {
        ...
        await update.execute.call(createMockExecuteFunction(nodeParameters), items, 'appYoLbase', 'tblltable');
        expect(transport.batchUpdate).toHaveBeenCalledWith('appYoLbase/tblltable', { typecast: false }, [{ fields: { bar: 'bar 1', foo: 'foo 1' }, id: 'recXXX' }]);
    });
    it('should update a record by field name, autoMapInputData', async () => {
        ...
        await update.execute.call(createMockExecuteFunction(nodeParameters), items, 'appYoLbase', 'tblltable');
        expect(transport.batchUpdate).toHaveBeenCalledWith('appYoLbase/tblltable', { typecast: false }, [{ fields: { bar: 'bar 1', foo: 'foo 1', id: 'recXXX' }, id: 'recXXX' }]);
    });
    it('should update a record by lookup field with array value, autoMapInputData', async () => {
        ...
        await update.execute.call(createMockExecuteFunction(nodeParameters), items, 'appYoLbase', 'tblltable');
        expect(transport.batchUpdate).toHaveBeenCalledWith('appYoLbase/tblltable', { typecast: false }, [{ fields: { lookupField: 'lookup_val_1', name: 'updated name' }, id: 'recAAA' }]);
    });
    it('should update all matches by lookup field with array value, autoMapInputData', async () => {
        ...
        await update.execute.call(createMockExecuteFunction(nodeParameters), items, 'appYoLbase', 'tblltable');
        expect(transport.batchUpdate).toHaveBeenCalledWith('appYoLbase/tblltable', { typecast: false }, [
