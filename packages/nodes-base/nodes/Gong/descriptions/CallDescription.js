...
import { getCursorPaginatorCalls, gongApiPaginateRequest, isValidNumberIds, handleErrorPostReceive, extractCalls, } from '../GenericFunctions';
                            ...
                            async function (items, _responseData) {
                                ...
                                if (properties.includes('transcript')) {
                                    for (const item of items) {
                                        const callTranscripts = await gongApiPaginateRequest.call(this, 'POST', '/v2/calls/transcript', { filter: { callIds: [item.json.metaData.id] } }, {}, item.index ?? 0, 'callTranscripts');
...
export const callFields = [...getFields, ...getAllFields];
