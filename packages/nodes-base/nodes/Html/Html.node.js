...
export const capitalizeHeader = (header, capitalize) => {
    if (!capitalize)
        return header;
    return header
        .split('_')
...
export class Html {
    ...
    async execute() {
        ...
        if (operation === 'convertToHtmlTable' && items.length) {
            ...
            table += '<tr>';
            table += headers
                .map((header) => '<th>' + capitalizeHeader(header, options.capitalize) + '</th>')
                .join('');
            table += '</tr>';
            table += '</thead>';
            table += '<tbody>';
                        ...
                        for (const valueData of extractionValues.values) {
                            ...
                            if (valueData.returnArray) {
                                ...
                                htmlElement.each((_, el) => {
                                    newItem.json[valueData.key].push(getValue($(el), valueData, options, nodeVersion));
                                });
                            }
                            else {
