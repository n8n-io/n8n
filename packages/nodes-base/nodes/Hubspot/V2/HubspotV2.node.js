...
import { clean, getAssociations, getCallMetadata, getAllProperties, getEmailMetadata, getMeetingMetadata, getTaskMetadata, hubspotApiRequest, hubspotApiRequestAllItems, validateCredentials, } from './GenericFunctions';
import { ticketFields, ticketOperations } from './TicketDescription';
import { generatePairedItemData } from '../../../utils/utilities';
import { parseToTimestamp } from './utils/parseToTimestamp';
export class HubspotV2 {
    description;
    ...
    methods = {
        ...
        listSearch: {
            async searchCompanies() {
                ...
                const endpoint = '/companies/v2/companies/paged';
                const searchResults = await hubspotApiRequestAllItems.call(this, 'companies', 'GET', endpoint, {}, qs);
                return {
            ...
            async searchContacts(filter) {
                ...
                const contacts = (await hubspotApiRequestAllItems.call(this, 'contacts', 'GET', endpoint, {}, qs));
            ...
            async searchTickets() {
                ...
                const tickets = await hubspotApiRequestAllItems.call(this, 'objects', 'GET', endpoint, {}, qs);
                return {
                    // tslint:disable-next-line: no-any
        ...
        if (resource === 'contactList') {
            ...
            for (let i = 0; i < length; i++) {
                try {
                        ...
                        if (operation === 'upsert') {
                            ...
                            if (additionalFields.buyingRole) {
                                const buyingRole = Array.isArray(additionalFields.buyingRole)
                                    ? additionalFields.buyingRole.join(';')
                            ...
                            if (additionalFields.latestTrafficSourceDate) {
                                body.push({
                                    ...
                                    value: new Date(additionalFields.latestTrafficSourceDate).getTime(),
                                });
                            }
                            ...
                            if (additionalFields.prospectingAgentLastEnrolled) {
                                body.push({
                                    ...
                                    value: new Date(additionalFields.prospectingAgentLastEnrolled).getTime(),
                                });
                            }
                        ...
                        if (operation === 'get') {
                            ...
                            if (additionalFields.propertiesCollection) {
                                ...
                                qs.properties = !Array.isArray(propertiesValues.properties)
                                    ? properties.split(',')
                                    : properties;
                                qs.propertyMode = snakeCase(propertiesValues.propertyMode);
                            }
                        ...
                        if (operation === 'search') {
                            ...
                            if (filtersGroupsUi?.filterGroupsValues) {
                                ...
                                if (body.filterGroups.length > 3) {
                                    throw new NodeOperationError(this.getNode(), 'You can only have 3 filter groups', { itemIndex: i });
                                }
                            }
                    ...
                    if (resource === 'engagement') {
                        ...
                        if (operation === 'create') {
                            ...
                            if (!Object.keys(metadata).length) {
                                throw new NodeOperationError(this.getNode(), 'At least one metadata field needs to set', { itemIndex: i });
                            }
                            const ownerId = associations.ownerId && typeof associations.ownerId === 'number'
                    ...
                    if (error.cause?.error?.validationResults &&
