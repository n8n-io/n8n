...
describe('MessageAnAgent Node', () => {
    ...
    function mockParams(overrides = {}) {
        executeFunctions.getNodeParameter.mockImplementation((param, _itemIndex, fallback) => {
            if (param in overrides)
                return overrides[param];
            if (param === 'agentId')
                return { mode: 'id', value: 'agent-1' };
            if (param === 'message')
                return 'Hello agent';
            if (param === 'advanced.invokeMode')
                return 'perItem';
            if (param === 'advanced')
                return fallback ?? {};
            if (param === 'advanced.session.session')
                return fallback ?? {};
            return undefined;
        });
    }
    ...
    it('rejects a sessionId longer than the persisted thread-key budget', async () => {
        ...
        await expect(node.execute.call(executeFunctions)).rejects.toThrow('Session ID must be at most 74 characters');
    ...
    describe('prompt resolution', () => {
        it('uses the message param', async () => {
            ...
            executeFunctions.executeAgent.mockResolvedValue(mockAgentResult);
            await node.execute.call(executeFunctions);
            expect(executeFunctions.executeAgent).toHaveBeenCalledWith(expect.objectContaining({ agentId: 'agent-1' }), 'Process the refund', 'exec-123', 0);
        });
        it('reads the message param on a v1 node', async () => {
            ...
            expect(executeFunctions.executeAgent).toHaveBeenCalledWith(expect.objectContaining({ agentId: 'agent-1' }), 'v1 message', 'exec-123', 0);
        });
    ...
    describe('inline agent source', () => {
        ...
        it('uses inline defaults when legacy advanced values are still saved', async () => {
            ...
            expect(executeFunctions.executeAgent).toHaveBeenCalledWith(expect.objectContaining({ inputDataScope: 'all', exposeWorkflowData: false }), 'Hello agent', 'exec-123', 0);
        });
        it('passes the inline definition and never resolves its embedded expressions', async () => {
            ...
            expect(executeFunctions.getNodeParameter).toHaveBeenCalledWith('inlineAgent', 0, {}, { rawExpressions: true });
            expect(executeFunctions.executeAgent).toHaveBeenCalledWith(expect.objectContaining({ inlineAgent }), 'Hello agent', 'exec-123', 0);
        ...
        it('passes an inline definition with embedded skills through wholesale', async () => {
            ...
            expect(executeFunctions.executeAgent).toHaveBeenCalledWith(expect.objectContaining({ inlineAgent: inlineAgentWithSkills }), 'Hello agent', 'exec-123', 0);
        });
        it('passes a session id override through for inline agents (thread memory)', async () => {
            ...
            executeFunctions.executeAgent.mockResolvedValue({ ...mockAgentResult, session: null });
            await node.execute.call(executeFunctions);
            expect(executeFunctions.executeAgent).toHaveBeenCalledWith(expect.objectContaining({ sessionId: 'my-session' }), expect.any(String), expect.any(String), expect.any(Number));
        });
        ...
        it('throws when the inline definition is a malformed JSON string', async () => {
            ...
            executeFunctions.continueOnFail.mockReturnValue(false);
            await expect(node.execute.call(executeFunctions)).rejects.toThrow('Inline agent configuration is not valid JSON');
            expect(executeFunctions.executeAgent).not.toHaveBeenCalled();
        });
        it('throws when inline mode is selected but no agent is configured', async () => {
            ...
            mockParams({ agentSource: 'inline', inlineAgent: {} });
            executeFunctions.continueOnFail.mockReturnValue(false);
            await expect(node.execute.call(executeFunctions)).rejects.toThrow('Inline agent is not configured');
        });
    });
    it('should process multiple items with different itemIndex values', async () => {
        executeFunctions.getInputData.mockReturnValue([{ json: {} }, { json: {} }]);
        executeFunctions.getNodeParameter.mockImplementation((param, itemIndex, fallback) => {
            if (param === 'agentId')
                return { mode: 'id', value: `agent-${(itemIndex ?? 0) + 1}` };
            if (param === 'message')
                return `Message ${(itemIndex ?? 0) + 1}`;
            if (param === 'advanced')
                return fallback ?? {};
            if (param === 'advanced.invokeMode')
                return 'perItem';
        ...
        const resultForItem0 = {
            ...mockAgentResult,
            response: 'Response 1',
        };
        const resultForItem1 = {
            ...mockAgentResult,
            response: 'Response 2',
        };
        ...
        expect(executeFunctions.executeAgent).toHaveBeenCalledWith({
            ...
            inputDataScope: 'item',
            exposeWorkflowData: false,
        }, 'Message 1', 'exec-123', 0);
        expect(executeFunctions.executeAgent).toHaveBeenCalledWith({
            agentId: 'agent-2',
            sessionId: undefined,
            inputDataScope: 'item',
            exposeWorkflowData: false,
        }, 'Message 2', 'exec-123', 1);
    ...
    it('should throw NodeOperationError when the output schema is not valid JSON', async () => {
        ...
        executeFunctions.continueOnFail.mockReturnValue(false);
        await expect(node.execute.call(executeFunctions)).rejects.toThrow(NodeOperationError);
        await expect(node.execute.call(executeFunctions)).rejects.toThrow('Output schema is not valid JSON');
        expect(executeFunctions.executeAgent).not.toHaveBeenCalled();
    });
    ...
    it('throws NodeOperationError when the output schema resolves to an array', async () => {
        ...
        executeFunctions.continueOnFail.mockReturnValue(false);
        await expect(node.execute.call(executeFunctions)).rejects.toThrow('Output schema must be a JSON Schema object');
        expect(executeFunctions.executeAgent).not.toHaveBeenCalled();
    });
    ...
    describe('v3 schema from example', () => {
        ...
        it('infers an all-required JSON Schema from a JSON example, without a $schema keyword', async () => {
            ...
            expect(executeFunctions.executeAgent).toHaveBeenCalledWith(expect.objectContaining({
                outputSchema: {
                    type: 'object',
                    properties: {
                        result: { type: 'string' },
        ...
        it('throws when the JSON example is not valid JSON', async () => {
            ...
            executeFunctions.continueOnFail.mockReturnValue(false);
            await expect(v3.execute.call(executeFunctions)).rejects.toThrow('JSON example is not valid JSON');
            expect(executeFunctions.executeAgent).not.toHaveBeenCalled();
        });
        it('throws when the JSON example is an array instead of an object', async () => {
            ...
            executeFunctions.continueOnFail.mockReturnValue(false);
            await expect(v3.execute.call(executeFunctions)).rejects.toThrow('JSON example must be a JSON object');
            expect(executeFunctions.executeAgent).not.toHaveBeenCalled();
        });
    });
    describe('v3.1 session resolution', () => {
        ...
        it('defaults to the sessionId from the input item without any configuration', async () => {
            ...
            mockParams();
            executeFunctions.evaluateExpression.mockImplementation((expression) => expression === '{{ $json.sessionId }}' ? 'chat-session-1' : undefined);
            executeFunctions.executeAgent.mockResolvedValue(mockAgentResult);
            await node.execute.call(executeFunctions);
            expect(executeFunctions.executeAgent).toHaveBeenCalledWith(expect.objectContaining({ sessionId: 'chat-session-1' }), 'Hello agent', 'exec-123', 0);
        });
        it('falls back to the Chat Trigger output when the input has no sessionId', async () => {
            ...
            executeFunctions.evaluateExpression.mockImplementation((expression) => expression === "{{ $('When chat message received').first().json.sessionId }}"
                ? 'chat-session-2'
                : undefined);
            executeFunctions.executeAgent.mockResolvedValue(mockAgentResult);
            await node.execute.call(executeFunctions);
            expect(executeFunctions.executeAgent).toHaveBeenCalledWith(expect.objectContaining({ sessionId: 'chat-session-2' }), 'Hello agent', 'exec-123', 0);
        });
        it('falls back to a per-execution session when nothing resolves', async () => {
            ...
            executeFunctions.executeAgent.mockResolvedValue(mockAgentResult);
            await node.execute.call(executeFunctions);
            expect(executeFunctions.executeAgent).toHaveBeenCalledWith(expect.objectContaining({ sessionId: undefined }), 'Hello agent', 'exec-123', 0);
        });
        it('ignores a disabled Chat Trigger', async () => {
            executeFunctions.getInputData.mockReturnValue([{ json: {} }]);
            mockParams();
            executeFunctions.getChatTrigger.mockReturnValue({ ...chatTriggerNode, disabled: true });
            executeFunctions.evaluateExpression.mockImplementation((expression) => expression === '{{ $json.sessionId }}' ? undefined : 'unexpected-session');
            executeFunctions.executeAgent.mockResolvedValue(mockAgentResult);
            await node.execute.call(executeFunctions);
            expect(executeFunctions.executeAgent).toHaveBeenCalledWith(expect.objectContaining({ sessionId: undefined }), 'Hello agent', 'exec-123', 0);
        });
        ...
        it('hashes a chat-derived sessionId longer than the thread-key budget', async () => {
            ...
            mockParams();
            executeFunctions.evaluateExpression.mockImplementation((expression) => expression === '{{ $json.sessionId }}' ? longSessionId : undefined);
            executeFunctions.executeAgent.mockResolvedValue(mockAgentResult);
            await node.execute.call(executeFunctions);
            expect(executeFunctions.executeAgent).toHaveBeenCalledWith(expect.objectContaining({ sessionId: expectedHash }), 'Hello agent', 'exec-123', 0);
        });
        it('uses the custom key when "Define below" is selected', async () => {
            ...
            executeFunctions.executeAgent.mockResolvedValue(mockAgentResult);
            await node.execute.call(executeFunctions);
            expect(executeFunctions.executeAgent).toHaveBeenCalledWith(expect.objectContaining({ sessionId: 'my-key' }), 'Hello agent', 'exec-123', 0);
        ...
        it('forwards the chat session for inline agents (thread memory persistence)', async () => {
            ...
            executeFunctions.evaluateExpression.mockImplementation((expression) => expression === '{{ $json.sessionId }}' ? 'chat-session-3' : undefined);
            executeFunctions.executeAgent.mockResolvedValue({ ...mockAgentResult, session: null });
            await node.execute.call(executeFunctions);
            expect(executeFunctions.executeAgent).toHaveBeenCalledWith(expect.objectContaining({ sessionId: 'chat-session-3' }), 'Hello agent', 'exec-123', 0);
        });
    });
    it('invokes the agent once with all-items scope in "Once for All Items" mode', async () => {
        ...
        executeFunctions.getNodeParameter.mockImplementation((param, _itemIndex, fallback) => {
            if (param === 'agentId')
                return { mode: 'id', value: 'agent-1' };
            if (param === 'message')
                return 'Summarize all items';
            if (param === 'advanced')
                return fallback ?? {};
            if (param === 'advanced.invokeMode')
                return 'allItems';
            if (param === 'allowOtherNodesData')
                return false;
            return fallback;
        });
        executeFunctions.executeAgent.mockResolvedValue(mockAgentResult);
        ...
        expect(executeFunctions.executeAgent).toHaveBeenCalledWith({
            ...
            inputDataScope: 'all',
            exposeWorkflowData: false,
        }, 'Summarize all items', 'exec-123', 0);
    ...
    it('defaults to once-for-all-items when Invoke Agent is not set in Advanced', async () => {
        ...
        const result = await node.execute.call(executeFunctions);
        expect(executeFunctions.executeAgent).toHaveBeenCalledTimes(1);
        expect(executeFunctions.executeAgent).toHaveBeenCalledWith(expect.objectContaining({ inputDataScope: 'all' }), 'Summarize all items', 'exec-123', 0);
        expect(result[0]).toHaveLength(1);
    });
    it('passes exposeWorkflowData when "Allow agent to access other nodes data" is on', async () => {
        ...
        executeFunctions.getNodeParameter.mockImplementation((param, _itemIndex, fallback) => {
            ...
            if (param === 'message')
                return 'Hello';
            if (param === 'advanced')
                return { allowOtherNodesData: true };
            if (param === 'advanced.invokeMode')
                return 'perItem';
            return fallback;
        });
...
describe('MessageAnAgent versioning', () => {
    it('uses Message an Agent as the display and default name', () => {
        ...
        expect(new MessageAnAgentV1(baseDescription).description.defaults.name).toBe('Message an Agent');
        expect(new MessageAnAgentV2(baseDescription).description.defaults.name).toBe('Message an Agent');
    });
    ...
    it('keeps advanced parameters when resolving v1 workflows', () => {
        const v1 = new MessageAnAgentV1(baseDescription);
        const parameters = getNodeParameters(v1.description.properties, {
        ...
        }, false, false, { typeVersion: 1 }, v1.description);
    ...
    it('resolves parameters for a newly added v2 node', () => {
        const v2 = new MessageAnAgentV2(baseDescription);
        expect(() => getNodeParameters(v2.description.properties, {
            ...
            useStructuredOutput: false,
            advanced: {},
        }, false, false, { typeVersion: 2 }, v2.description)).not.toThrow();
    });
    it('resolves parameters for a newly added v3 node', () => {
        const v3 = new MessageAnAgentV2(baseDescription);
        expect(() => getNodeParameters(v3.description.properties, {
            agentSource: 'referenced',
            agentId: { __rl: true, mode: 'list', value: '' },
            inlineAgent: {},
            message: '',
            ...
            advanced: {},
        }, false, false, { typeVersion: 3 }, v3.description)).not.toThrow();
    });
    it('resolves parameters for a newly added v3.1 node', () => {
        const v31 = new MessageAnAgentV2(baseDescription);
        expect(() => getNodeParameters(v31.description.properties, {
            agentSource: 'referenced',
            agentId: { __rl: true, mode: 'list', value: '' },
            inlineAgent: {},
            message: '',
        ...
        }, false, false, { typeVersion: 3.1 }, v31.description)).not.toThrow();
    });
...
//# sourceMappingURL=MessageAnAgent.node.test.js.map