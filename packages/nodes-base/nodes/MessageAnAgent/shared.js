...
export const sharedVersionDescription = {
    hidden: true,
    defaults: {
...
export const messageProperty = {
    ...
    placeholder: 'Process the refund for order {{ $json.order_id }} — confirm with the customer that it was approved.',
    typeOptions: {
        rows: 4,
    },
};
...
export const commonProperties = [
    ...
    {
        displayName: 'Structured output is enforced by the model provider. For best results across providers, mark every property as required. Some providers reject optional fields or advanced keywords (e.g. OpenAI and xAI), and a few do not support structured output at all (e.g. DeepSeek).',
    ...
    },
    {
        ...
        options: [
            ...
            },
            {
                ...
                type: 'boolean',
                default: false,
                description: "Whether to give the agent a tool to read other workflow nodes' execution data, beyond its own input",
                displayOptions: {
                    hide: {
                        '/agentSource': ['inline'],
                    },
...
function parseManualOutputSchema(ctx, itemIndex, rawSchema) {
    let parsed;
    if (typeof rawSchema === 'object') {
        ...
        if (!rawSchema.trim()) {
            throw new NodeOperationError(ctx.getNode(), 'Output schema is empty. Provide a JSON Schema or turn off "Require Specific Output Format".', { itemIndex });
        }
        try {
            parsed = jsonParse(rawSchema);
        }
        catch (error) {
            throw new NodeOperationError(ctx.getNode(), `Output schema is not valid JSON: ${error.message}`, { itemIndex });
        }
    }
    else {
        throw new NodeOperationError(ctx.getNode(), 'Output schema is not valid JSON. Provide a JSON Schema or turn off "Require Specific Output Format".', { itemIndex });
    }
 ...
 */
export function getStructuredOutputSchema(ctx, itemIndex) {
    const useStructuredOutput = ctx.getNodeParameter('useStructuredOutput', itemIndex, false);
    if (!useStructuredOutput)
        return undefined;
    const { typeVersion } = ctx.getNode();
    if (typeVersion >= 3) {
        const schemaType = ctx.getNodeParameter('schemaType', itemIndex, 'fromJson');
        if (schemaType === 'fromJson') {
            const rawExample = ctx.getNodeParameter('jsonSchemaExample', itemIndex, '');
            const exampleString = typeof rawExample === 'string' ? rawExample : (JSON.stringify(rawExample) ?? '');
            if (!exampleString.trim()) {
                throw new NodeOperationError(ctx.getNode(), 'JSON example is empty. Provide an example object or turn off "Require Specific Output Format".', { itemIndex });
            }
            let schema;
            try {
                ...
                throw new NodeOperationError(ctx.getNode(), `JSON example is not valid JSON: ${error.message}`, { itemIndex });
            }
            if (schema.type !== 'object') {
                throw new NodeOperationError(ctx.getNode(), 'JSON example must be a JSON object (e.g. { "result": "value" }), not an array or single value', { itemIndex });
            }
            return schema;
        }
    }
    const rawSchema = ctx.getNodeParameter('outputSchema', itemIndex, '');
 ...
 */
function getAgentSource(ctx, itemIndex) {
    const agentSource = ctx.getNodeParameter('agentSource', itemIndex, 'referenced');
    if (agentSource === 'inline') {
        ...
        const raw = ctx.getNodeParameter('inlineAgent', itemIndex, {}, {
            rawExpressions: true,
        });
        ...
        if (typeof value !== 'object' ||
            ...
            throw new NodeOperationError(ctx.getNode(), 'Inline agent is not configured. Open the node to set up the agent, or switch to a saved agent.', { itemIndex });
        }
        return { inlineAgent: value };
    }
    const agentIdRlc = ctx.getNodeParameter('agentId', itemIndex);
...
function validateSessionIdLength(ctx, sessionId, itemIndex) {
    if (sessionId.length > SESSION_ID_MAX_LENGTH) {
        throw new NodeOperationError(ctx.getNode(), `Session ID must be at most ${SESSION_ID_MAX_LENGTH} characters (got ${sessionId.length})`, { itemIndex });
    }
}
/**
 ...
 */
function getChatSessionId(ctx, itemIndex) {
    let sessionId;
    try {
    ...
    if (typeof sessionId !== 'string' || !sessionId.trim()) {
        try {
            ...
            if (chatTrigger && chatTrigger.disabled !== true) {
                ...
                sessionId = ctx.evaluateExpression(`{{ $('${triggerName}').first().json.sessionId }}`, itemIndex);
            }
        }
        catch { }
    }
 ...
 */
function resolveSessionId(ctx, itemIndex) {
    const session = ctx.getNodeParameter('advanced.session.session', itemIndex, {});
...
export async function execute() {
    ...
    const agentSource = this.getNodeParameter('agentSource', 0, 'referenced');
    ...
    const invokeMode = agentSource === 'inline'
        ? 'allItems'
        : this.getNodeParameter('advanced.invokeMode', 0, 'allItems');
    ...
    for (let i = 0; i < loopCount; i++) {
        try {
            ...
            const advanced = this.getNodeParameter('advanced', i, {});
            const allowOtherNodesData = agentSource === 'inline' ? false : (advanced.allowOtherNodesData ?? false);
            let sessionId;
            if (this.getNode().typeVersion >= 3.1) {
                ...
                if (sessionIdOverride)
                    validateSessionIdLength(this, sessionIdOverride, i);
