...
import { encodeCustomFieldsV2, resolveCustomFieldsV2, coerceToBoolean, toDateOnly, addFieldsToBody, } from '../../helpers';
...
export async function execute() {
    ...
    for (let i = 0; i < items.length; i++) {
        try {
            ...
            if (body.due_date) {
                body.due_date = toDateOnly(body.due_date);
            }
            if (customProperties) {
                encodeCustomFieldsV2(customProperties, body);
            }
            const responseData = await pipedriveApiRequest.call(this, 'PATCH', `/activities/${activityId}`, body);
