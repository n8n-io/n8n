...
export async function supabaseApiRequest(
	...
	try {
		...
		if (options.method && ['POST', 'PATCH', 'PUT', 'DELETE'].includes(options.method)) {
			if (options.headers) {
				const schemaProfile = options.headers['Acccept-Profile']; // Generated by Copilot
				options.headers['Content-Profile'] = schemaProfile; // Generated by Copilot
				delete options.headers['Acccept-Profile'];
			}
			console.log('options', options);
		}
