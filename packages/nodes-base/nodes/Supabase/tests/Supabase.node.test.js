...
describe('Test Supabase Node', () => {
    ...
    const createMockExecuteFunction = (nodeParameters, continueOnFail = false) => {
        const fakeExecuteFunction = {
            ...
            helpers: {
                ...
                constructExecutionMetaData: (_inputData, _options) => [],
                returnJsonArray: (_jsonData) => [],
            },
        };
        return fakeExecuteFunction;
    };
    // getNodeParameter is only ever asked for these two names on the loadOptions path
    const createMockLoadOptionsFunction = (schema, getCredentials = mockGetCredentials) => {
        ...
        context.getNodeParameter.mockImplementation((name) => name === 'useCustomSchema' ? schema !== undefined : schema);
        return context;
    };
    describe('filter query builders', () => {
        it('should leave plain filter components unchanged', () => {
            expect(utils.buildOrQuery({ keyName: 'active', condition: 'is', keyValue: 'null' })).toBe('active.is.null');
            expect(utils.buildOrQuery({
                keyName: 'description',
                condition: 'fullText',
                searchFunction: 'fts',
        ...
        it('should escape quotes and backslashes in quoted components', () => {
            expect(utils.buildOrQuery({ keyName: 'name', condition: 'eq', keyValue: 'a"b\\c' })).toBe('name.eq."a\\"b\\\\c"');
        });
        it('should quote query parameter names but not values', () => {
            expect(utils.buildQuery(new Map(), {
                keyName: 'profile.name',
                condition: 'eq',
                keyValue: 'Doe,Jane',
            })).toEqual(new Map([['"profile.name"', 'eq.Doe,Jane']]));
            expect(utils.buildGetQuery(new Map(), { keyName: 'profile.name', keyValue: 'Doe,Jane' })).toEqual(new Map([['"profile.name"', 'eq.Doe,Jane']]));
        });
        it.each(['&', '?', '='])('should quote query parameter names containing %s', (character) => {
            ...
            expect(utils.buildQuery(new Map(), { keyName, condition: 'eq', keyValue: 'value' })).toEqual(new Map([[expectedKey, 'eq.value']]));
            expect(utils.buildGetQuery(new Map(), { keyName, keyValue: 'value' })).toEqual(new Map([[expectedKey, 'eq.value']]));
        });
        it.each([
            {
    ...
    describe('getAll pagination', () => {
        ...
        it('should include order parameter in the request when orderBy is set', async () => {
            ...
            await node.execute.call(fakeExecuteFunction);
            expect(supabaseApiRequest).toHaveBeenCalledWith('GET', '/my_table', {}, expect.objectContaining({ order: 'id' }), undefined, {});
            supabaseApiRequest.mockRestore();
        });
    });
    it('should allow filtering on the same field multiple times', async () => {
        ...
        await node.execute.call(fakeExecuteFunction);
        expect(supabaseApiRequest).toHaveBeenCalledWith('GET', '/my_table', {}, {
    ...
    it('should not set schema headers if no custom schema is used', async () => {
        ...
        await node.execute.call(fakeExecuteFunction);
        expect(mockRequestWithAuthentication).toHaveBeenCalledWith('supabaseApi', expect.objectContaining({
            method: 'GET',
    ...
    it('should show descriptive message when error is caught', async () => {
        ...
        await expect(node.execute.call(fakeExecuteFunction)).rejects.toHaveProperty('message', 'error: Something when wrong');
    });
    describe('getSchemaHeader function', () => {
        ...
        it('should return empty object when useCustomSchema is false for execute context', () => {
            mockExecuteContext.getNodeParameter.mockReturnValueOnce(false);
        ...
        it('should return empty object when useCustomSchema is false for loadOptions context', () => {
            mockLoadOptionsContext.getNodeParameter.mockReturnValueOnce(false);
            const result = utils.getSchemaHeader(mockLoadOptionsContext, 'GET', 'loadOptions');
            expect(result).toEqual({});
            expect(mockLoadOptionsContext.getNodeParameter).toHaveBeenCalledWith('useCustomSchema', false);
        });
        it('should return Accept-Profile header for GET method when useCustomSchema is true', () => {
            mockExecuteContext.getNodeParameter
        ...
        it('should use different parameter calls for loadOptions context', () => {
            mockLoadOptionsContext.getNodeParameter
                .mockReturnValueOnce(true)
    ...
    describe('loadOptions', () => {
        ...
        describe('getTableColumns', () => {
            it('should return table columns with their types', async () => {
                ...
                const columns = await node.methods.loadOptions.getTableColumns.call(mockLoadOptionsFunctions);
            ...
            it('should fetch the schema document once for concurrent calls sharing a schema', async () => {
                const createColumnsContext = (schema) => {
                ...
                const publicSchemaColumns = await Promise.all(Array.from({ length: 25 }, async () => await node.methods.loadOptions.getTableColumns.call(createColumnsContext())));
                ...
                await Promise.all(Array.from({ length: 5 }, async () => await node.methods.loadOptions.getTableColumns.call(createColumnsContext('analytics'))));
            ...
            it('should not share a request between callers whose credentials differ', async () => {
                const createColumnsContext = (serviceRole) => {
                    const context = createMockLoadOptionsFunction(undefined, vi.fn().mockResolvedValue({ host: 'https://api.supabase.io', serviceRole }));
                    context.getCurrentNodeParameter.mockReturnValue('users');
                    return context;
                };
...
//# sourceMappingURL=Supabase.node.test.js.map