...
import { buildGetQuery, buildOrQuery, buildQuery, getApiDefinition, getSchemaHeader, mapPairedItemsFrom, supabaseApiRequest, validateCredentials, } from './GenericFunctions';
    ...
    async execute() {
        ...
        if (resource === 'row') {
            ...
            if (operation === 'create') {
                ...
                try {
                    const createdRows = await supabaseApiRequest.call(this, 'POST', endpoint, records, {}, undefined, header);
                    ...
                    if (this.continueOnFail()) {
                        const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray({ error: error.description }), { itemData: mapPairedItemsFrom(records) });
                        returnData.push(...executionData);
                    }
                    else {
                        throw error;
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    if (filterType === 'manual') {
                        ...
                        if (matchType === 'allFilters') {
                            const filters = keys.reduce(buildQuery, new Map());
                    ...
                    let rows;
                    try {
                        rows = await supabaseApiRequest.call(this, 'DELETE', endpoint, {}, qs, undefined, header);
                    }
                    catch (error) {
                        if (this.continueOnFail()) {
            ...
            if (operation === 'get') {
                ...
                for (let i = 0; i < length; i++) {
                    const keys = this.getNodeParameter('filters.conditions', i, []);
                    const filters = keys.reduce(buildGetQuery, new Map());
                    qs = Object.fromEntries(filters);
                    let rows;
                    if (!keys.length) {
                        throw new NodeOperationError(this.getNode(), 'At least one select condition must be defined', { itemIndex: i });
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    try {
                        ...
                        do {
                            ...
                            const newRows = await supabaseApiRequest.call(this, 'GET', endpoint, {}, qs, undefined, header);
            ...
            if (operation === 'update') {
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    let updatedRow;
                    try {
                        updatedRow = await supabaseApiRequest.call(this, 'PATCH', endpoint, record, qs, undefined, header);
