...
export class SplitInBatchesV4 implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][] | null> {
		...
		const children = new Set(this.getChildNodes());
		const sourceData = this.getInputSourceData();
		const isLoop = children.has(sourceData.previousNode);

		if (!isLoop || options.reset === true) {
			// Is the first time the node runs
			const parents = this.getParentNodes('main', 1);
			nodeContext.branches = parents.filter((parent) => children.has(parent)).length;

			nodeContext.currentRunIndex = 0;
			nodeContext.maxRunIndex = Math.ceil((items.length * nodeContext.branches) / batchSize);
		...
		if (returnItems.length === 0) {
			if (nodeContext.currentRunIndex >= nodeContext.maxRunIndex) {
