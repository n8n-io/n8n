 ...
 */
export const parseAndSetBodyJson = (parameterName, setAsBodyProperty) => {
    return async function (requestOptions) {
        try {
            ...
            throw new NodeOperationError(this.getNode(), new Error(`The '${parameterName}' property must be valid JSON, but cannot be parsed`, {
                cause: err,
            }));
        }
 ...
 */
export const prepareWorkflowCreateBody = async function (requestOptions) {
 ...
 * so take care to first call parseAndSetBodyJson().
 */
export const prepareWorkflowUpdateBody = async function (requestOptions) {
    const body = requestOptions.body;
    const newBody = {};
    if (body.name) {
