...
describe('Test EmailSendV2, email => sendAndWait', () => {
    ...
    it('should use the shared transport configuration and put execution to wait', async () => {
        ...
        expect(transporter.sendMail).toHaveBeenCalledWith({
            from: 'from@mail.com',
            html: expect.stringContaining('href="http://localhost/waiting-webhook/nodeID?approved=true&signature=abc"'),
            subject: 'my subject',
            to: 'to@mail.com',
        });
    });
    ...
    it('should rethrow SMTP errors when continueOnFail is false', async () => {
        ...
        transporter.sendMail.mockRejectedValueOnce(new Error('smtp_connection_refused'));
        await expect(emailSendV2.execute.call(mockExecuteFunctions)).rejects.toThrow('smtp_connection_refused');
        expect(mockExecuteFunctions.putExecutionToWait).not.toHaveBeenCalled();
    });
});
