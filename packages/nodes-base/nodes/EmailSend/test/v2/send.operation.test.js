...
vi.mock('../../v2/utils', async () => {
    const originalModule = await vi.importActual('../../v2/utils');
    return {
        ...originalModule,
...
describe('Test EmailSendV2, send operation', () => {
    ...
    describe('comma-separated attachment strings', () => {
        it('should process comma-separated attachment names with spaces', async () => {
            ...
            mockExecuteFunctions.getNode.mockReturnValue({ typeVersion: 2.0 });
    ...
    describe('file attachments (non-inline)', () => {
            ...
            const items = [
                {
                    ...
                    binary: {
                        ...
                        file2: { data: 'data2', mimeType: 'text/csv', fileName: 'data.csv' },
                    },
                },
            ];
            mockExecuteFunctions.getInputData.mockReturnValue(items);
        ...
        it('should combine inline and file attachments', async () => {
            const items = [
                {
                    json: { data: 'test' },
                    binary: {
                        logo: { data: 'data1', mimeType: 'image/png', fileName: 'logo.png' },
                        report: {
                            data: 'data2',
                            mimeType: 'application/pdf',
                            fileName: 'report.pdf',
            ...
            expect(callArg.attachments[0]).toEqual(expect.objectContaining({ filename: 'logo.png', cid: 'logo' }));
    ...
    describe('emails without attachments', () => {
        it('should send email when no attachments specified', async () => {
            ...
            transporter.sendMail.mockResolvedValue({ messageId: 'test-id' });
            await sendOperation.execute.call(mockExecuteFunctions);
            expect(transporter.sendMail).toHaveBeenCalledWith(expect.not.objectContaining({
    ...
    describe('message parameters', () => {
        it('should request normalized text and HTML values before sending', async () => {
            const items = [{ json: { data: 'test' } }];
            mockExecuteFunctions.getInputData.mockReturnValue(items);
            mockExecuteFunctions.getNode.mockReturnValue({ typeVersion: 2.1 });
            mockExecuteFunctions.getInstanceId.mockReturnValue('instanceId');
            mockExecuteFunctions.getCredentials.mockResolvedValue({
                host: 'smtp.example.com',
                port: 587,
...
//# sourceMappingURL=send.operation.test.js.map