...
export async function serviceNowApiRequest(method, resource, body = {}, qs = {}, uri, option = {}) {
    const headers = {};
...
export async function serviceNowRequestAllItems(method, resource, body = {}, query = {}) {
    const returnData = [];
    let responseData;
    const page = 100;
...
export async function serviceNowDownloadAttachment(endpoint, fileName, contentType) {
...
export const mapEndpoint = (resource, _operation) => {
...
export const sortData = (returnData) => {
    returnData.sort((a, b) => {
        if (a.name < b.name) {
            return -1;
        }
