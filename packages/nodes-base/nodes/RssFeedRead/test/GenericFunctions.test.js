...
const RELAXED_ACCEPT = 'application/rss+xml, application/rdf+xml;q=0.8, application/atom+xml;q=0.6, application/xml;q=0.4, text/xml;q=0.4';
describe('parseFeedUrl', () => {
    ...
    beforeEach(() => {
        ...
        Parser.prototype.parseString.mockResolvedValue(parsed);
    });
    ...
    it('switches to the relaxed Accept header when useRelaxedAcceptHeader is true', async () => {
        ...
        expect(helpers.httpRequest).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('passes skipSslCertificateValidation when ignoreSSL is true', async () => {
        ...
        expect(helpers.httpRequest).toHaveBeenCalledWith(expect.objectContaining({ skipSslCertificateValidation: true }));
    });
    it('splits and trims customFields before passing them to the parser', async () => {
        ...
        expect(ParserMock).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('coerces a non-string response body to a string before parsing', async () => {
        helpers.httpRequest.mockResolvedValue(Buffer.from('<rss>buf</rss>'));
        await parseFeedUrl(helpers, feedUrl);
        expect(Parser.prototype.parseString).toHaveBeenCalledWith(expect.stringContaining('<rss>buf</rss>'));
    });
    ...
    it('propagates errors thrown by the parser', async () => {
        ...
        Parser.prototype.parseString.mockRejectedValue(parseError);
