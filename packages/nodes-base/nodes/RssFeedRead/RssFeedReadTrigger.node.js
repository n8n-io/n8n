...
export class RssFeedReadTrigger {
    description = {
    ...
    async poll() {
        ...
        const dateToCheck = Date.parse(pollData.lastItemDate ?? pollData.lastTimeChecked ?? moment().utc().format());
        if (!feedUrl) {
        ...
        try {
            ...
            if (error.code === 'ECONNREFUSED') {
                throw new NodeOperationError(this.getNode(), `It was not possible to connect to the URL. Please make sure the URL "${feedUrl}" it is valid!`);
            }
