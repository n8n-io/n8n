...
export class CalTriggerV2 {
    ...
    methods = {
        loadOptions: {
            async getEventTypes() {
                const response = await (calApiRequestV2).call(this, 'GET', '/event-types', {}, {}, { headers: { 'cal-api-version': '2024-06-14' } });
                return sortOptionParameters(response.data.map(({ id, title }) => ({ name: title, value: id })));
            },
        },
    };
    webhookMethods = {
        default: {
            async checkExists() {
                ...
                for (let page = 0; page < MAX_WEBHOOK_PAGES; page++) {
                    ...
                    const response = await (calApiRequestV2).call(this, 'GET', endpoint, {}, { take: WEBHOOK_PAGE_SIZE, skip });
            ...
            async create() {
                ...
                const response = await (calApiRequestV2).call(this, 'POST', endpoint, body);
