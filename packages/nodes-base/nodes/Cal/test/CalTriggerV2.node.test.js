...
describe('CalTriggerV2', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        trigger = new CalTrigger().getNodeType(3);
        hookFunctions = mock();
        loadOptionsFunctions = mock();
        ...
        hookFunctions.getNodeParameter.mockImplementation((name) => {
            if (name === 'events')
                return events;
            if (name === 'options')
                return options;
    ...
    it('loads event types using the required API version header', async () => {
        calApiRequestV2.mockResolvedValue({
            data: [
        ...
        expect(calApiRequestV2).toHaveBeenCalledWith('GET', '/event-types', {}, {}, { headers: { 'cal-api-version': '2024-06-14' } });
    ...
    it('creates a user webhook with the v2 request shape', async () => {
        calApiRequestV2.mockResolvedValue({ data: { id: 123 } });
    ...
    it('creates an event-type webhook and stores its scope', async () => {
        ...
        calApiRequestV2.mockResolvedValue({ data: { id: 456 } });
        const result = await trigger.webhookMethods.default.create.call(hookFunctions);
        expect(result).toBe(true);
    ...
    it('stops checking after ten full pages', async () => {
        ...
        calApiRequestV2.mockResolvedValue({ data: fullPage });
        const result = await trigger.webhookMethods.default.checkExists.call(hookFunctions);
        expect(result).toBe(false);
        expect(calApiRequestV2).toHaveBeenCalledTimes(10);
        expect(calApiRequestV2).toHaveBeenLastCalledWith('GET', '/webhooks', {}, { take: 250, skip: 2250 });
    });
    ...
    it.each([
        ...
        webhookFunctions.getRequestObject.mockReturnValue({ body });
        const result = await trigger.webhook.call(webhookFunctions);
        expect(result.workflowData).toEqual([expected]);
    });
});
//# sourceMappingURL=CalTriggerV2.node.test.js.map