...
describe('Spotify -> GenericFunctions', () => {
    ...
    it('should make a request with the correct options', async () => {
        ...
        mockThis.helpers.httpRequestWithAuthentication.mockResolvedValue(response);
        const result = await spotifyApiRequest.call(mockThis, method, endpoint, body, query);
        expect(mockThis.helpers.httpRequestWithAuthentication).toHaveBeenCalledWith('spotifyOAuth2Api', {
            method,
            headers: {
    ...
    it('should throw a NodeApiError on request failure', async () => {
        ...
        mockThis.helpers.httpRequestWithAuthentication.mockRejectedValue(error);
        await expect(spotifyApiRequest.call(mockThis, method, endpoint, body, query)).rejects.toThrow(NodeApiError);
