...
describe('BrandfetchV2.execute', () => {
    ...
    function mockParams(params) {
        executeFunctions.getNodeParameter.mockImplementation((name) => params[name]);
    }
    // ---------------------------------------------------------------------------
    // operation: data
    // ---------------------------------------------------------------------------
    it('should return full brand data when operation is data', async () => {
        ...
        GenericFunctions.brandfetchApiRequest.mockResolvedValue(brandData);
        const result = await node.execute.call(executeFunctions);
        expect(GenericFunctions.brandfetchApiRequest).toHaveBeenCalledWith('GET', '/brands/domain/n8n.io');
    ...
    it('should URL-encode type and identifier in the resource path', async () => {
        ...
        GenericFunctions.brandfetchApiRequest.mockResolvedValue({});
        await node.execute.call(executeFunctions);
        expect(GenericFunctions.brandfetchApiRequest).toHaveBeenCalledWith('GET', '/brands/isin/US%200378%2F331005');
    });
    ...
    it.each([
        ...
        ['crypto', 'BTC'],
        ['isin', 'US0378331005'],
    ])('should build the correct resource path for type %s', async (type, identifier) => {
        mockParams({ operation: 'data', type, identifier });
        GenericFunctions.brandfetchApiRequest.mockResolvedValue({});
        await node.execute.call(executeFunctions);
        expect(GenericFunctions.brandfetchApiRequest).toHaveBeenCalledWith('GET', `/brands/${type}/${identifier}`);
    });
    // ---------------------------------------------------------------------------
    // operation: colors
    // ---------------------------------------------------------------------------
    ...
    it('should download all logo formats as binary data when download is true', async () => {
        ...
        const [type0, fmt0, , id0, , suffix0] = GenericFunctions.fetchAndPrepareBinaryData
            .mock.calls[0];
        ...
        const [type1, fmt1, , id1, , suffix1] = GenericFunctions.fetchAndPrepareBinaryData
            .mock.calls[1];
    ...
    it('should preserve existing binary data from the input item when downloading', async () => {
        ...
        executeFunctions.getInputData.mockReturnValue([
            { json: {}, binary: { existing: { data: 'keep' } } },
        ]);
        GenericFunctions.brandfetchApiRequest.mockResolvedValue({
            logos: [],
        });
    ...
    it('should return brand context as JSON when operation is context', async () => {
        ...
        GenericFunctions.brandfetchApiRequest.mockResolvedValue(context);
        const result = await node.execute.call(executeFunctions);
        expect(GenericFunctions.brandfetchApiRequest).toHaveBeenCalledWith('GET', '/context/n8n.io', {}, {}, undefined, { headers: { accept: 'application/json' } });
    ...
    it('should return brand context as markdown wrapped in a context key', async () => {
        ...
        GenericFunctions.brandfetchApiRequest.mockResolvedValue('# n8n Brand Context');
        const result = await node.execute.call(executeFunctions);
        expect(GenericFunctions.brandfetchApiRequest).toHaveBeenCalledWith('GET', '/context/n8n.io', {}, {}, undefined, { json: false, headers: { accept: 'text/markdown' } });
    ...
    it('should pass cachedOnly as a query parameter', async () => {
        ...
        GenericFunctions.brandfetchApiRequest.mockResolvedValue({});
        await node.execute.call(executeFunctions);
        expect(GenericFunctions.brandfetchApiRequest).toHaveBeenCalledWith('GET', '/context/n8n.io', {}, { cachedOnly: true }, undefined, { headers: { accept: 'application/json' } });
    });
    it('should return an empty item when cachedOnly finds no cached context', async () => {
        mockParams({ operation: 'context', domain: 'n8n.io', outputFormat: 'json', cachedOnly: true });
        GenericFunctions.brandfetchApiRequest.mockResolvedValue(undefined);
        const result = await node.execute.call(executeFunctions);
    ...
    it('should trim and URL-encode the domain for the context path', async () => {
        ...
        GenericFunctions.brandfetchApiRequest.mockResolvedValue({});
        await node.execute.call(executeFunctions);
        expect(GenericFunctions.brandfetchApiRequest).toHaveBeenCalledWith('GET', `/context/${encodeURIComponent('bücher.example/path')}`, {}, {}, undefined, { headers: { accept: 'application/json' } });
    });
    // ---------------------------------------------------------------------------
    // Error handling
    // ---------------------------------------------------------------------------
    it('should push an error item when continueOnFail is true', async () => {
        ...
        GenericFunctions.brandfetchApiRequest.mockRejectedValue(new Error('boom'));
        const result = await node.execute.call(executeFunctions);
    ...
    it('exposes a valid node description', () => {
        ...
        expect(operationParam?.options?.map((o) => o.value)).toEqual(expect.arrayContaining(['logo', 'colors', 'data', 'context']));
        const typeParam = description.properties.find((p) => p.name === 'type');
        expect(typeParam?.options?.map((o) => o.value)).toEqual(expect.arrayContaining(['domain', 'ticker', 'crypto', 'isin']));
    });
    it('shows context-only fields for the context operation and brand fields otherwise', () => {
        const description = node.description;
        const byName = (name) => description.properties.find((p) => p.name === name);
...
//# sourceMappingURL=BrandfetchV2.node.test.js.map