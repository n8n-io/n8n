...
describe('TimeSaved node', () => {
    function createExecuteFunctionsMock(options) {
        const { items = [{ json: {} }], mode = 'once', minutesSaved = 10, continueOnFail = false, } = options ?? {};
    ...
    it('should expose minutesSaved as a whole number with correct typeOptions', () => {
        ...
        expect(minutesSaved?.typeOptions).toEqual(expect.objectContaining({ minValue: 0, numberPrecision: 0 }));
    });
    ...
    it('should return an error if the minutes saved is negative', async () => {
        ...
        await expect(node.execute.call(executeFunctions)).rejects.toThrow('Time saved cannot be negative, got: -1');
    ...
    it('should return an error if the minutes saved is not a number', async () => {
        ...
        await expect(node.execute.call(executeFunctions)).rejects.toThrow('Parameter "minutesSaved" is not number');
        expect(setMetadata).not.toHaveBeenCalled();
    });
...
//# sourceMappingURL=TimeSaved.node.test.js.map