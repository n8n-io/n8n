...
import { dashboardFields, dashboardOperations, teamFields, teamMemberFields, teamMemberOperations, teamOperations, userFields, userOperations, } from './descriptions';
...
export class Grafana {
    ...
    methods = {
        loadOptions: {
            async getDashboards() {
                const dashboards = (await grafanaApiRequest.call(this, 'GET', '/search', {}, { qs: 'dash-db' }));
                return dashboards.map(({ id, title }) => ({ value: id, name: title }));
            },
            async getFolders() {
                const folders = (await grafanaApiRequest.call(this, 'GET', '/folders'));
                return folders.map(({ id, title }) => ({ value: id, name: title }));
            },
            async getTeams() {
                const res = (await grafanaApiRequest.call(this, 'GET', '/teams/search'));
                return res.teams.map(({ id, name }) => ({ value: id, name }));
            },
            async getUsers() {
                const users = (await grafanaApiRequest.call(this, 'GET', '/org/users'));
                ...
                if (resource === 'dashboard') {
                    ...
                    if (operation === 'create') {
                        ...
                        if (!title) {
                            ...
                            const dashboards = (await grafanaApiRequest.call(this, 'GET', '/search'));
