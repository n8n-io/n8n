	...
	methods = {
		loadOptions: {
			...
			async getItemCodes(
				...
				const {
					Items: items,
				} = await xeroApiRequest.call(this, 'GET', '/items', {
					organizationId,
				});
			...
			async getAccountCodes(
				...
				const {
					Accounts: accounts,
				} = await xeroApiRequest.call(this, 'GET', '/Accounts', {
					organizationId,
				});
			...
			async getBrandingThemes(
				...
				} = await xeroApiRequest.call(this, 'GET', '/BrandingThemes', {
					organizationId,
				});
			...
			async getCurrencies(
				...
				} = await xeroApiRequest.call(this, 'GET', '/Currencies', {
					organizationId,
				});
			...
			async getTrakingCategories(
				...
				} = await xeroApiRequest.call(this, 'GET', '/TrackingCategories', {
					organizationId,
				});
						...
						try {
							...
							if (resource === 'invoice') {
								if (operation === 'create') {
									...
									const lineItemsValues = (this.getNodeParameter(
										'lineItemsUi',
										i,
									) as IDataObject).lineItemsValues as IDataObject[];
								...
								if (operation === 'update') {
									const invoiceId = this.getNodeParameter(
										'invoiceId',
										i,
									) as string;
									const organizationId = this.getNodeParameter(
									...
									if (updateFields.Contact) {
										body.Contact = {
											ContactID: updateFields.contactId as string,
										};
									}
								...
								if (operation === 'getAll') {
									...
									if (options.orderBy) {
										qs.order = `${options.orderBy} ${options.sortOrder ===
										undefined
											? 'DESC'
											: options.sortOrder}`;
									}
