...
import { getTableColumns, getTableViews, rowExport, rowFormatColumns, rowMapKeyToName, seaTableApiRequest, setableApiRequestAllItems, split, updateAble, } from './GenericFunctions';
import { versionDescription } from './SeaTable.node';
export class SeaTableV1 {
    description;
            ...
            if (operation === 'create') {
                ...
                for (let i = 0; i < items.length; i++) {
                    try {
                        ...
                        const response = (await seaTableApiRequest.call(this, ctx, 'GET', `/dtable-server/api/v1/dtables/{{dtable_uuid}}/rows/${rowId}`, {}, { table_id: tableId, convert: true }));
                ...
                for (let i = 0; i < items.length; i++) {
                    try {
                        ...
                        const rows = responseData.map((row) => rowFormatColumns({ ...row }, tableColumns.map(({ name }) => name).concat(['_id', '_ctime', '_mtime'])));
