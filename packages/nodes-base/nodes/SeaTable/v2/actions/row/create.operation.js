...
import { seaTableApiRequest, getTableColumns, split, rowExport, updateAble, splitStringColumnsToArrays, } from '../../GenericFunctions';
...
export async function execute(index) {
    ...
    if (fieldsToSend === 'autoMapInputData') {
        ...
        const inputDataToIgnore = split(this.getNodeParameter('inputsToIgnore', index, ''));
    ...
    if (bigdata) {
        body.rows = [rowInput];
        const responseData = await seaTableApiRequest.call(this, {}, 'POST', '/api-gateway/api/v2/dtables/{{dtable_uuid}}/add-archived-rows/', body);
    ...
    // save to normal backend
        ...
        const responseData = await seaTableApiRequest.call(this, {}, 'POST', '/api-gateway/api/v2/dtables/{{dtable_uuid}}/rows/', body);
