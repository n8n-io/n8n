import { getTableColumns, seaTableApiRequest, updateAble, escapeSqlIdentifier, } from '../GenericFunctions';
export async function getTableNames() {
    const returnData = [];
    const { metadata: { tables }, } = await seaTableApiRequest.call(this, {}, 'GET', '/api-gateway/api/v2/dtables/{{dtable_uuid}}/metadata');
...
export async function getSearchableColumns() {
    ...
    if (tableName) {
        const columns = await seaTableApiRequest.call(this, {}, 'GET', '/api-gateway/api/v2/dtables/{{dtable_uuid}}/columns', {}, { table_name: tableName });
...
export async function getLinkColumns() {
    const returnData = [];
...
export async function getTableViews() {
    ...
    const tableName = this.getCurrentNodeParameter('tableName');
    if (tableName) {
        const { views } = await seaTableApiRequest.call(this, {}, 'GET', '/api-gateway/api/v2/dtables/{{dtable_uuid}}/views', {}, { table_name: tableName });
        returnData.push({
