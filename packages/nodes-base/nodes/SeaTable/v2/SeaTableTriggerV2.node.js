...
import { seaTableApiRequest, simplify_new, enrichColumns, escapeSqlIdentifier, } from './GenericFunctions';
import { loadOptions } from './methods';
export class SeaTableTriggerV2 {
    description;
    ...
    async poll() {
        ...
        // View => use getRows.
            ...
            requestMeta = await seaTableApiRequest.call(this, ctx, 'GET', '/api-gateway/api/v2/dtables/{{dtable_uuid}}/metadata/');
            requestRows = await seaTableApiRequest.call(this, ctx, 'GET', '/api-gateway/api/v2/dtables/{{dtable_uuid}}/rows/', {}, {
            ...
            const endpoint = '/api-gateway/api/v2/dtables/{{dtable_uuid}}/sql';
            const sqlQuery = `SELECT * FROM \`${escapeSqlIdentifier(tableName)}\` WHERE ${filterField} BETWEEN "${moment(startDate).format('YYYY-MM-D HH:mm:ss')}" AND "${moment(endDate).format('YYYY-MM-D HH:mm:ss')}" ORDER BY ${filterField} DESC LIMIT ${limit}`;
            sqlResult = await seaTableApiRequest.call(this, ctx, 'POST', endpoint, {
                sql: sqlQuery,
                convert_keys: options.convert ?? true,
            });
        ...
        const collaboratorsResult = await seaTableApiRequest.call(this, ctx, 'GET', '/api-gateway/api/v2/dtables/{{dtable_uuid}}/related-users/');
