vi.mock('../../v2/GenericFunctions', async () => {
    const actual = await vi.importActual('../../v2/GenericFunctions');
    return {
        ...actual,
...
const mockedApiRequest = GenericFunctions.seaTableApiRequest;
const mockedGetCollaborators = GenericFunctions.getBaseCollaborators;
const buildMockContext = (params) => {
    return {
        getNodeParameter: vi.fn().mockImplementation((...args) => {
            const name = args[0];
            return params[name] ?? {};
        }),
        helpers: {
...
describe('SeaTable > v2 > row operations', () => {
    ...
    describe('search operation', () => {
        ...
        it('should handle backticks in table and column names for case-insensitive search', async () => {
            ...
            await executeSearch.call(ctx, 0);
            expect(getCapturedSql()).toBe('SELECT * FROM `Q1``Results` WHERE lower(`product``name`) LIKE "%test%"');
        });
        it('should handle backslashes in search term', async () => {
            ...
            await executeSearch.call(ctx, 0);
            expect(getCapturedSql()).toBe('SELECT * FROM `Employees` WHERE `path` = "C:\\\\Users\\\\file"');
        });
    });
