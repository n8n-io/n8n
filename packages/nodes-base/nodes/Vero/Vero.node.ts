						...
						try {
							...
							if (resource === 'user') {
								...
								if (operation === 'create') {
									...
									if (!jsonActive) {
										const dataAttributesValues = (this.getNodeParameter(
											'dataAttributesUi',
											i,
										) as IDataObject).dataAttributesValues as IDataObject[];
										if (dataAttributesValues) {
											const dataAttributes = {};
											for (let i = 0; i < dataAttributesValues.length; i++) {
												// @ts-ignore
												dataAttributes[dataAttributesValues[i].key] =
													dataAttributesValues[i].value;
												// @ts-ignore
									...
									try {
										responseData = await veroApiRequest.call(
											this,
											'POST',
											'/users/track',
											body,
								...
								if (operation === 'addTags' || operation === 'removeTags') {
									const id = this.getNodeParameter('id', i) as string;
									const tags = (this.getNodeParameter(
										'tags',
										i,
							...
							if (resource === 'event') {
								...
								if (operation === 'track') {
									...
									if (!jsonActive) {
										...
										const extraAttributesValues = (this.getNodeParameter(
											'extraAttributesUi',
											i,
										) as IDataObject).extraAttributesValues as IDataObject[];
										if (extraAttributesValues) {
											const extraAttributes = {};
											for (let i = 0; i < extraAttributesValues.length; i++) {
												// @ts-ignore
												extraAttributes[extraAttributesValues[i].key] =
													extraAttributesValues[i].value;
												// @ts-ignore
