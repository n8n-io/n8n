...
import { s3ApiRequest, s3ApiRequestREST, s3ApiRequestSOAP, s3ApiRequestSOAPAllItems, } from '../GenericFunctions';
...
describe('S3 Node Generic Functions', () => {
    ...
    describe('s3ApiRequest', () => {
        it('should throw error if endpoint does not start with http', async () => {
            ...
            await expect(s3ApiRequest.call(mockContext, 'test-bucket', 'GET', '/')).rejects.toThrow('HTTP(S) Scheme is required');
        });
    ...
    describe('s3ApiRequestSOAP', () => {
        it('should parse XML response', async () => {
            ...
            parseString.mockImplementation((_, __, callback) => callback(null, mockParsedResponse));
        ...
        it('should handle XML parsing errors', async () => {
            ...
            parseString.mockImplementation((_, __, callback) => callback(mockError));
            const result = await s3ApiRequestSOAP.call(mockContext, 'test-bucket', 'GET', '/');
    ...
    describe('s3ApiRequestSOAPAllItems', () => {
        it('should handle pagination with continuation token', async () => {
            ...
            const result = await s3ApiRequestSOAPAllItems.call(mockContext, 'ListBucketResult.Contents', 'test-bucket', 'GET', '/');
