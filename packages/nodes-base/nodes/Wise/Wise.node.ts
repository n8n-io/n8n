	...
	methods = {
		loadOptions: {
			async getBorderlessAccounts(this: ILoadOptionsFunctions) {
				...
				const accounts = await wiseApiRequest.call(
					this,
					'GET',
			...
			async getRecipients(this: ILoadOptionsFunctions) {
				...
				>(
					(
						activeRecipients,
						{ active, id, accountHolderName, currency, country, type },
					) => {
						if (active) {
							const recipient = {
								name: `[${currency}] ${accountHolderName} - (${country !== null
									? country + ' - '
									: ''}${type})`,
								value: id,
							};
			...
			try {
				if (resource === 'account') {
					...
					if (operation === 'getBalances') {
						...
						const borderlessAccountId = this.getNodeParameter(
							'borderlessAccountId',
							i,
						);
					...
					if (operation === 'get') {
						...
						if (range !== undefined && time === undefined) {
							qs.from = moment
								.tz(range.rangeProperties.from, timezone)
								.utc()
								.format();
							qs.to = moment
								.tz(range.rangeProperties.to, timezone)
								.utc()
								.format();
						} else {
					...
					if (operation === 'create') {
						...
						const body = {
							...
							sourceCurrency: (this.getNodeParameter(
								'sourceCurrency',
								i,
							) as string).toUpperCase(),
							targetCurrency: (this.getNodeParameter(
								'targetCurrency',
								i,
							) as string).toUpperCase(),
						} as IDataObject;

						const amountType = this.getNodeParameter('amountType', i) as
							| 'source'
							| 'target';
					...
					if (operation === 'create') {
						...
						const { reference } = this.getNodeParameter(
							'additionalFields',
							i,
						) as { reference: string };
						...
						const { environment } = (await this.getCredentials(
							'wiseApi',
						)) as IDataObject;
						...
						const transferId = this.getNodeParameter('transferId', i);
						downloadReceipt = this.getNodeParameter(
							'downloadReceipt',
							i,
						) as boolean;
						...
						if (filters.range !== undefined) {
							qs.createdDateStart = moment(
								filters.range.rangeProperties.createdDateStart,
							).format();
							qs.createdDateEnd = moment(
								filters.range.rangeProperties.createdDateEnd,
							).format();
						} else {
