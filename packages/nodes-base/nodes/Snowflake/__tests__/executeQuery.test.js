...
beforeEach(() => {
    vi.spyOn(snowflake, 'configure').mockImplementation(() => ({}));
    vi.spyOn(snowflake, 'createConnection').mockReturnValue(mockConnection);
...
function executeQueryWorkflow(query, queryReplacement) {
    return {
        nodes: [
            {
                parameters: {},
...
describe('Test Snowflake, executeQuery - comma-separated query parameters', () => {
    new NodeTestHarness().setupTest({
        ...
        input: {
            workflowData: executeQueryWorkflow('SELECT * FROM users WHERE first = ? AND last = ?', "Ada, O'Brien"),
        },
        output: { nodeData: {} },
    }, {
        credentials: { snowflake: snowflakeCredentials },
...
describe('Test Snowflake, executeQuery - query parameters provided as an array', () => {
    new NodeTestHarness().setupTest({
        ...
        input: {
            workflowData: executeQueryWorkflow('SELECT * FROM users WHERE first = ? AND last = ?', "={{ ['Ada', 'Lovelace'] }}"),
        },
        output: { nodeData: {} },
    }, {
        credentials: { snowflake: snowflakeCredentials },
...
describe('Test Snowflake, executeQuery - connection cleanup', () => {
    it('destroys the connection even when a query fails', async () => {
        const lockError = new Error("Statement '01c4b15b-020a-7e8c-0001-126284fb9fd6' has locked table 'ORDERS' in " +
        ...
        mockExecute.mockImplementation(({ sqlText, complete, }) => sqlText.startsWith('ALTER SESSION')
        ...
        executeFns.getNodeParameter.mockImplementation((name, _itemIndex, fallback) => {
            ...
            if (name === 'operation')
                return 'executeQuery';
            if (name === 'query')
                return 'UPDATE "ORDERS" SET "STATUS" = 1';
            return fallback;
        });
