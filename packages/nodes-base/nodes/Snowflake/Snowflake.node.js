...
import { connect, destroy, escapeSnowflakeIdentifier, escapeSnowflakeObjectIdentifier, execute, getConnectionOptions, prepareQueryResults, } from './GenericFunctions';
export class Snowflake {
    ...
    async execute() {
        ...
        const authMethod = this.getNodeParameter('authentication', 0, 'credentials');
        let snowflakeCredential;
        if (authMethod === 'oAuth2') {
            const oauthCredentials = await this.getCredentials('snowflakeOAuth2Api');
            const tokenData = oauthCredentials.oauthTokenData;
            if (!tokenData?.access_token) {
                throw new NodeOperationError(this.getNode(), 'OAuth2 access token is missing. Please reconnect your Snowflake OAuth2 credential.');
            }
            snowflakeCredential = {
                account: oauthCredentials.account,
                database: oauthCredentials.database,
                warehouse: oauthCredentials.warehouse,
                schema: oauthCredentials.schema,
                clientSessionKeepAlive: oauthCredentials.clientSessionKeepAlive,
                authentication: 'oauth2',
                token: tokenData.access_token,
            };
        }
        else {
            snowflakeCredential = await this.getCredentials('snowflake');
        }
        ...
        try {
                ...
                for (let i = 0; i < items.length; i++) {
                    ...
                    if (rawReplacement !== undefined && rawReplacement !== '') {
                        if (typeof rawReplacement === 'string') {
                            ...
                            binds = rawReplacement;
                        }
                        else {
                            throw new NodeOperationError(this.getNode(), 'Query Parameters must be a string of comma-separated values, or an array of values', { itemIndex: i });
                        }
                    }
                    const responseData = await execute(connection, query, binds, this.getNode());
                    const executionData = await prepareQueryResults.call(this, responseData, i, nodeVersion);
            ...
            if (operation === 'insert') {
                ...
                await execute(connection, query, binds, this.getNode());
                data.forEach((d, i) => {
            ...
            if (operation === 'update') {
                ...
                for (let i = 0; i < binds.length; i++) {
                    await execute(connection, query, binds[i], this.getNode());
                }
                data.forEach((d, i) => {
                    const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(d), { itemData: { item: i } });
                    returnData = returnData.concat(executionData);
