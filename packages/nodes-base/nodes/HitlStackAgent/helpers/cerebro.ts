...
/*
 * Minimal Cerebro client for the HITL node's non-blocking (Cerebro) mode. A
 * Cerebro API key is not a bearer token: it is exchanged at
 * /config/v1/auth/api-keys/token for a short-lived JWT that then authorizes
 * /config and /ingest calls. Tokens are cached per key fingerprint until shortly
 * before they expire, so the agent lookup + trace export do one exchange, not two.
 *
...
/**
 * Ships the OTLP spans to the agent's ingest endpoint. Cerebro's pipeline
 * (ingest → router → review-service) turns them into a review case. Best effort:
 * telemetry must not fail the workflow item.
 */
