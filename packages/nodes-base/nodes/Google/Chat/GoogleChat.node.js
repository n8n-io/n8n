...
// mediaFields,
// mediaOperations,
memberFields, memberOperations, messageFields, messageOperations, spaceFields, spaceIdProperty, spaceOperations, } from './descriptions';
import { createSendAndWaitMessageBody, googleApiRequest, googleApiRequestAllItems, validateJSON, } from './GenericFunctions';
            ...
            try {
                if (resource === 'media') {
                    if (operation === 'download') {
                        ...
                        const encoding = null;
                        responseData = await googleApiRequest.call(this, 'GET', endpoint, undefined, undefined, undefined, undefined, encoding);
                        const newItem = {
                            json: items[i].json,
                            binary: {},
                        };
                    ...
                    if (operation === 'get') {
                        ...
                        if (returnAll) {
                            responseData = await googleApiRequestAllItems.call(this, 'spaces', 'GET', '/v1/spaces');
                        }
                        else {
                    ...
                    if (operation === 'get') {
                        ...
                        const returnAll = this.getNodeParameter('returnAll', 0);
                        if (returnAll) {
                            responseData = await googleApiRequestAllItems.call(this, 'memberships', 'GET', `/v1/${spaceId}/members`, undefined, qs);
                        }
                        else {
                            const limit = this.getNodeParameter('limit', i);
                            qs.pageSize = limit;
                            responseData = await googleApiRequest.call(this, 'GET', `/v1/${spaceId}/members`, undefined, qs);
                    ...
                    if (operation === 'create') {
                        ...
                        if (jsonParameters) {
                            ...
                            if (messageJson instanceof Object) {
                                ...
                                // if it is a string
                                if (validateJSON(messageJson) !== undefined) {
                        ...
                        responseData = await googleApiRequest.call(this, 'POST', `/v1/${spaceId}/messages`, body, qs);
                    }
