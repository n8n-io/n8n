...
const createOption = objectOperations[0].options[0];
const bodySend = createOption.routing.send.preSend[1];
...
describe('Google Cloud Storage - Create object resumable upload (preSend)', () => {
    ...
    describe('session initiation', () => {
        it('sends metadata as JSON with correct upload headers when content length is known', async () => {
            ...
            expect(httpRequestWithAuthentication).toHaveBeenCalledWith('googleCloudStorageOAuth2Api', expect.objectContaining({
                method: 'POST',
...
//# sourceMappingURL=ObjectDescription.resumableUpload.test.js.map