    ...
    for (let i = 0; i < length; i++) {
        try {
            ...
            if (typeof body.queryParameters === 'object') {
                const { namedParameters } = body.queryParameters;
            ...
            const response = await googleBigQueryApiRequest.call(this, 'POST', `/v2/projects/${projectId}/jobs`, {
                configuration: {
                    query: body,
                },
            });
            ...
            if (response.status?.state === 'DONE') {
                ...
                const queryResponse = await googleBigQueryApiRequestAllItems.call(this, 'GET', `/v2/projects/${projectId}/queries/${jobId}`, undefined, qs);
                if (body.returnAsNumbers === true) {
                    ...
                    const schemaDataTypes = schemaFields?.map((field) => field.type);
    ...
    outerLoop: while (jobs.length > 0) {
        ...
        for (const job of jobs) {
            ...
            try {
                ...
                //https://cloud.google.com/bigquery/docs/reference/rest/v2/jobs/getQueryResults
                const response = await googleBigQueryApiRequestAllItems.call(this, 'GET', `/v2/projects/${job.projectId}/queries/${job.jobId}`, undefined, qs);
                ...
                if (response?.errors?.length) {
                    ...
                    throw new NodeOperationError(this.getNode(), `Error(s) ocurring while executing query from item ${job.i.toString()}: ${errorMessages.join(', ')}`, { level: 'warning' });
                }
            }
            catch (error) {
                if (this.continueOnFail()) {
