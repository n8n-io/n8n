...
export async function execute(i, item) {
    ...
    const file = await googleApiRequest.call(this, 'GET', `/drive/v3/files/${fileId}`, {}, { fields: 'mimeType,name', supportsTeamDrives: true, supportsAllDrives: true });
    let response;
    if (file.mimeType?.includes('vnd.google-apps')) {
        ...
        if (type === 'document') {
            ...
            mime = this.getNodeParameter(`${parameterKey}.sheetsToFormat`, i, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        }
        else {
            mime = this.getNodeParameter(`${parameterKey}.drawingsToFormat`, i, 'image/jpeg');
        }
        response = await googleApiRequest.call(this, 'GET', `/drive/v3/files/${fileId}/export`, {}, { mimeType: mime, supportsAllDrives: true }, undefined, requestOptions);
    }
    else {
        response = await googleApiRequest.call(this, 'GET', `/drive/v3/files/${fileId}`, {}, { alt: 'media', supportsAllDrives: true }, undefined, requestOptions);
    }
