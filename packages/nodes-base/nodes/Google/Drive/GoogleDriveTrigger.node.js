...
export class GoogleDriveTrigger {
    description = {
    ...
    async poll() {
        ...
        const queryTargetsWatchedFolder = triggerOn === 'specificFolder' && event !== 'watchFolderUpdated';
        if (queryTargetsWatchedFolder) {
            const folderToWatch = extractId(this.getNodeParameter('folderToWatch', '', { extractValue: true }));
        ...
        if (triggerOn === 'specificFile' && this.getMode() !== 'manual') {
            const fileToWatch = extractId(this.getNodeParameter('fileToWatch', '', { extractValue: true }));
...
function collectGoogleErrorReasons(value, reasons = new Set(), seen = new WeakSet(), depth = 0) {
 ...
 */
function driveFailureFromError(error, queryTargetsWatchedFolder) {
