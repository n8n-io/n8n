...
describe('GoogleDriveTrigger', () => {
    ...
    beforeEach(() => {
        ...
        mockPollFunctions.helpers.returnJsonArray.mockImplementation((data) => data.map((item, index) => ({ json: item, pairedItem: { item: index } })));
    ...
    describe('Methods', () => {
        ...
        describe('getDrives', () => {
            it('should return drives with root option', async () => {
                ...
                expect(googleApiRequestAllItemsSpy).toHaveBeenCalledWith('drives', 'GET', '/drive/v3/drives');
    ...
    describe('Poll Function - Parameter Setup', () => {
        beforeEach(() => {
            mockPollFunctions.getNodeParameter.mockImplementation((paramName) => {
                const params = {
        ...
        it('should handle specific file trigger', async () => {
            ...
            expect(googleApiRequestAllItemsSpy).toHaveBeenCalledWith('files', 'GET', '/drive/v3/files', {}, expect.objectContaining({
    ...
    describe('Poll Function - Manual Mode', () => {
        ...
        it('should fetch single file in manual mode', async () => {
            ...
            const result = await trigger.poll.call(mockPollFunctions);
            expect(googleApiRequestSpy).toHaveBeenCalledWith('GET', '/drive/v3/files', {}, expect.objectContaining({
                pageSize: 1,
            }));
            expect(result).toBeDefined();
            expect(result[0]).toHaveLength(1);
        ...
        it('should throw NodeApiError when no data found in manual mode', async () => {
            ...
            await expect(trigger.poll.call(mockPollFunctions)).rejects.toThrow('No data with the current filter could be found');
        });
    });
    ...
    describe('Poll Function - Declared Failures', () => {
        const setParameters = (params) => {
            mockPollFunctions.getNodeParameter.mockImplementation((paramName) => params[paramName] ?? '');
        };
        const driveApiError = (statusCode, reason) => {
            ...
            return new NodeApiError(mockNode, requestError);
        };
        beforeEach(() => {
            setParameters({
                triggerOn: 'specificFile',
        ...
        it.each(['rateLimitExceeded', 'userRateLimitExceeded', 'sharingRateLimitExceeded'])('should declare a 403 with reason %s as rate-limited', async (reason) => {
        ...
        it('should declare a 403 with reason dailyLimitExceeded as quota-exhausted, resetting at the next Pacific midnight', async () => {
            ...
            const { resetsAtEpochMs } = apiError.failure;
        ...
        it('should declare a 404 as configuration-invalid when the query filters on the watched folder', async () => {
            ...
            expect(apiError.message).toBe('The folder this node watches no longer exists. Please update it in the workflow.');
        });
        it.each([
...
//# sourceMappingURL=GoogleDriveTrigger.node.test.js.map