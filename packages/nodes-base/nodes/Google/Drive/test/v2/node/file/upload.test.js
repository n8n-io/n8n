...
vi.mock('../../../../v2/helpers/utils', async () => {
    const originalModule = await vi.importActual('../../../../v2/helpers/utils');
    return {
        ...originalModule,
...
describe('test GoogleDriveV2: file upload', () => {
    ...
    it('should upload buffers', async () => {
        ...
        expect(transport.googleApiRequest).toHaveBeenCalledWith('POST', '/upload/drive/v3/files', expect.any(Buffer), { uploadType: 'multipart', supportsAllDrives: true }, undefined, {
            headers: {
                'Content-Length': 498,
                'Content-Type': expect.stringMatching(/^multipart\/related; boundary=(\\S)*/),
            },
        });
        expect(transport.googleApiRequest).toHaveBeenCalledWith('PATCH', '/drive/v3/files/undefined', { mimeType, name, originalFilename }, {
            addParents: parent,
    ...
    it('should stream large files in 2MB chunks', async () => {
        ...
        expect(httpRequestSpy).toHaveBeenCalledWith(expect.objectContaining({ body: expect.any(Buffer) }));
        expect(transport.googleApiRequest).toBeCalledTimes(2);
        expect(transport.googleApiRequest).toHaveBeenCalledWith('POST', '/upload/drive/v3/files', { name, parents: [parent] }, { uploadType: 'resumable', supportsAllDrives: true }, undefined, { returnFullResponse: true, headers: { 'X-Upload-Content-Type': 'image/jpg' } });
        expect(transport.googleApiRequest).toHaveBeenCalledWith('PATCH', '/drive/v3/files/undefined', { mimeType, name, originalFilename }, {
            addParents: parent,
            supportsAllDrives: true,
            corpora: 'allDrives',
