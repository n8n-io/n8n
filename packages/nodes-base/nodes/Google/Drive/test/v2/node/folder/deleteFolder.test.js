...
describe('test GoogleDriveV2: folder deleteFolder', () => {
    it('should be called with PATCH', async () => {
        ...
        expect(transport.googleApiRequest).toHaveBeenCalledWith('PATCH', '/drive/v3/files/folderIDxxxxxx', { trashed: true }, { supportsAllDrives: true });
    });
    it('should be called with DELETE', async () => {
        ...
        const fakeExecuteFunction = createMockExecuteFunction(nodeParameters, driveNode);
        await deleteFolder.execute.call(fakeExecuteFunction, 0);
        expect(transport.googleApiRequest).toHaveBeenCalledWith('DELETE', '/drive/v3/files/folderIDxxxxxx', undefined, { supportsAllDrives: true });
    });
});
