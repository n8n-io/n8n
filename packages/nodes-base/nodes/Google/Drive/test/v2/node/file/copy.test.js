...
const googleApiRequestMock = transport.googleApiRequest;
...
describe('test GoogleDriveV2: file copy', () => {
    ...
    it('uses the user-provided name in list mode', async () => {
        ...
        expect(googleApiRequestMock).toHaveBeenCalledWith('POST', '/drive/v3/files/fileIDxxxxxx/copy', {
            copyRequiresWriterPermission: true,
            description: 'image copy',
            name: 'copyImage.png',
    ...
    it('fetches original name via metadata when name is empty in id mode', async () => {
        ...
        expect(googleApiRequestMock).toHaveBeenNthCalledWith(1, 'GET', '/drive/v3/files/fileIDxxxxxx', {}, metadataQs);
        expect(googleApiRequestMock).toHaveBeenNthCalledWith(2, 'POST', '/drive/v3/files/fileIDxxxxxx/copy', {
            copyRequiresWriterPermission: false,
    ...
    it('omits name when metadata returns no name', async () => {
        ...
        await copy.execute.call(fakeExecuteFunction, 0);
        expect(googleApiRequestMock).toHaveBeenCalledTimes(2);
        const copyCall = googleApiRequestMock.mock.calls[1];
