...
describe('GoogleDriveTrigger', () => {
	...
	describe('Node Description', () => {
		it('should have correct basic properties', () => {
			expect(trigger.description.displayName).toBe('Google Drive Trigger');
			expect(trigger.description.name).toBe('googleDriveTrigger');
		...
		it('should have correct node structure', () => {
			...
			expect(trigger.description.subtitle).toBe('={{$parameter["event"]}}');
		});
		...
		it('should have correct authentication options', () => {
			const authProp = trigger.description.properties.find((p) => p.name === 'authentication');
			expect(authProp).toBeDefined();
			if (authProp && 'type' in authProp) {
				expect(authProp.type).toBe('options');
			}
			if (authProp && 'default' in authProp) {
		...
		it('should have correct file resource locator configuration', () => {
			const fileProp = trigger.description.properties.find((p) => p.name === 'fileToWatch');
			expect(fileProp).toBeDefined();
			expect(fileProp?.type).toBe('resourceLocator');
			expect(fileProp?.required).toBe(true);
			expect(fileProp?.displayOptions?.show?.triggerOn).toEqual(['specificFile']);
			expect(fileProp?.modes).toHaveLength(3);

			const listMode = fileProp?.modes?.find((m) => m.name === 'list');
			expect(listMode?.typeOptions?.searchListMethod).toBe('fileSearch');

			const urlMode = fileProp?.modes?.find((m) => m.name === 'url');
			expect(urlMode?.extractValue?.regex).toBe(GOOGLE_DRIVE_FILE_URL_REGEX);
		});

		it('should have correct folder resource locator configuration', () => {
			const folderProp = trigger.description.properties.find((p) => p.name === 'folderToWatch');
			expect(folderProp).toBeDefined();
			expect(folderProp?.type).toBe('resourceLocator');
			expect(folderProp?.required).toBe(true);
			expect(folderProp?.displayOptions?.show?.triggerOn).toEqual(['specificFolder']);
			expect(folderProp?.modes).toHaveLength(3);

			const listMode = folderProp?.modes?.find((m) => m.name === 'list');
			expect(listMode?.typeOptions?.searchListMethod).toBe('folderSearch');

			const urlMode = folderProp?.modes?.find((m) => m.name === 'url');
			expect(urlMode?.extractValue?.regex).toBe(GOOGLE_DRIVE_FOLDER_URL_REGEX);
		});

		it('should have correct event options for specific file', () => {
			const eventProps = trigger.description.properties.filter((p) => p.name === 'event');
			const fileEventProp = eventProps.find((p) =>
				p.displayOptions?.show?.triggerOn?.includes('specificFile'),
			);

			expect(fileEventProp).toBeDefined();
			expect(fileEventProp?.options).toEqual([
				{
					name: 'File Updated',
					value: 'fileUpdated',
				},
			]);
			expect(fileEventProp?.default).toBe('fileUpdated');
		});

		it('should have correct event options for specific folder', () => {
			const eventProps = trigger.description.properties.filter((p) => p.name === 'event');
			const folderEventProp = eventProps.find((p) =>
				p.displayOptions?.show?.triggerOn?.includes('specificFolder'),
			);

			expect(folderEventProp).toBeDefined();
			expect(folderEventProp?.options).toEqual([
				{
					name: 'File Created',
					value: 'fileCreated',
		...
		it('should have correct file type options', () => {
