...
describe('Google Contacts GenericFunctions', () => {
    ...
    beforeEach(() => {
        ...
        mockExecuteFunctions.helpers.requestOAuth2 = vi.fn();
        mockLoadOptionsFunctions.helpers.requestOAuth2 = vi.fn();
    });
    afterEach(() => {
        vi.resetAllMocks();
    });
    describe('googleApiRequest', () => {
        it('should make successful API request with default parameters', async () => {
            ...
            mockExecuteFunctions.helpers.requestOAuth2.mockResolvedValue(mockResponse);
            const result = await GenericFunctions.googleApiRequest.call(mockExecuteFunctions, 'GET', '/people/me');
            expect(mockExecuteFunctions.helpers.requestOAuth2).toHaveBeenCalledWith('googleContactsOAuth2Api', {
                headers: {
        ...
        it('should make API request with custom URI', async () => {
            ...
            mockExecuteFunctions.helpers.requestOAuth2.mockResolvedValue(mockResponse);
            const result = await GenericFunctions.googleApiRequest.call(mockExecuteFunctions, 'POST', '/custom', { data: 'test' }, { param: 'value' }, customUri);
            expect(mockExecuteFunctions.helpers.requestOAuth2).toHaveBeenCalledWith('googleContactsOAuth2Api', {
                headers: {
                    'Content-Type': 'application/json',
                },
        ...
        it('should include custom headers when provided', async () => {
            ...
            mockExecuteFunctions.helpers.requestOAuth2.mockResolvedValue(mockResponse);
            await GenericFunctions.googleApiRequest.call(mockExecuteFunctions, 'PUT', '/people/123', { name: 'Updated Name' }, {}, undefined, customHeaders);
            expect(mockExecuteFunctions.helpers.requestOAuth2).toHaveBeenCalledWith('googleContactsOAuth2Api', {
                headers: {
                    'Content-Type': 'application/json',
        ...
        it('should remove empty body from request options', async () => {
            ...
            expect(mockExecuteFunctions.helpers.requestOAuth2).toHaveBeenCalledWith('googleContactsOAuth2Api', expect.objectContaining({
                method: 'GET',
                qs: {},
            ...
            const callArgs = mockExecuteFunctions.helpers.requestOAuth2.mock.calls[0][1];
        ...
        it('should work with ILoadOptionsFunctions context', async () => {
            ...
            mockLoadOptionsFunctions.helpers.requestOAuth2.mockResolvedValue(mockResponse);
            const result = await GenericFunctions.googleApiRequest.call(mockLoadOptionsFunctions, 'GET', '/people/connections');
            expect(mockLoadOptionsFunctions.helpers.requestOAuth2).toHaveBeenCalledWith('googleContactsOAuth2Api', expect.objectContaining({
                method: 'GET',
        ...
        it('should throw NodeApiError on request failure', async () => {
            ...
            mockExecuteFunctions.helpers.requestOAuth2.mockRejectedValue(apiError);
            await expect(GenericFunctions.googleApiRequest.call(mockExecuteFunctions, 'GET', '/people/invalid')).rejects.toThrow(NodeApiError);
            expect(mockExecuteFunctions.getNode).toHaveBeenCalled();
        });
        it('should handle authentication errors', async () => {
            const authError = { code: 401, message: 'Unauthorized' };
            mockExecuteFunctions.helpers.requestOAuth2.mockRejectedValue(authError);
            await expect(GenericFunctions.googleApiRequest.call(mockExecuteFunctions, 'POST', '/people')).rejects.toThrow(NodeApiError);
        });
        it('should handle network errors', async () => {
            const networkError = { code: 'ECONNREFUSED', message: 'Connection refused' };
            mockExecuteFunctions.helpers.requestOAuth2.mockRejectedValue(networkError);
            await expect(GenericFunctions.googleApiRequest.call(mockExecuteFunctions, 'GET', '/people')).rejects.toThrow(NodeApiError);
        });
        it('should preserve query string parameters', async () => {
            ...
            mockExecuteFunctions.helpers.requestOAuth2.mockResolvedValue(mockResponse);
            await GenericFunctions.googleApiRequest.call(mockExecuteFunctions, 'GET', '/people/connections', {}, queryParams);
            expect(mockExecuteFunctions.helpers.requestOAuth2).toHaveBeenCalledWith('googleContactsOAuth2Api', expect.objectContaining({
                qs: queryParams,
            }));
        });
    ...
    describe('googleApiRequestAllItems', () => {
        it('should fetch all items with pagination', async () => {
            ...
            const result = await GenericFunctions.googleApiRequestAllItems.call(mockExecuteFunctions, 'connections', 'GET', '/people/connections', {}, { personFields: 'names' });
        ...
        it('should handle single page response with no pagination', async () => {
            ...
            mockExecuteFunctions.helpers.requestOAuth2.mockResolvedValue(singlePageResponse);
            const result = await GenericFunctions.googleApiRequestAllItems.call(mockExecuteFunctions, 'connections', 'GET', '/people/connections');
        ...
        it('should handle empty response', async () => {
            ...
            mockExecuteFunctions.helpers.requestOAuth2.mockResolvedValue(emptyResponse);
            const result = await GenericFunctions.googleApiRequestAllItems.call(mockExecuteFunctions, 'connections', 'GET', '/people/connections');
        ...
        it('should work with ILoadOptionsFunctions context', async () => {
            ...
            mockLoadOptionsFunctions.helpers.requestOAuth2.mockResolvedValue(mockResponse);
            const result = await GenericFunctions.googleApiRequestAllItems.call(mockLoadOptionsFunctions, 'items', 'GET', '/custom/endpoint', { filter: 'active' }, { sortBy: 'name' });
        ...
        it('should handle pagination with undefined nextPageToken', async () => {
            ...
            mockExecuteFunctions.helpers.requestOAuth2.mockResolvedValue(responseWithUndefinedToken);
            const result = await GenericFunctions.googleApiRequestAllItems.call(mockExecuteFunctions, 'connections', 'GET', '/people/connections');
            expect(result).toHaveLength(1);
            expect(mockExecuteFunctions.helpers.requestOAuth2).toHaveBeenCalledTimes(1);
        });
        it('should handle large datasets with multiple pages', async () => {
            const createResponse = (pageNum, hasNext) => ({
        ...
        it('should propagate errors from underlying API requests', async () => {
            ...
            mockExecuteFunctions.helpers.requestOAuth2.mockRejectedValue(apiError);
            await expect(GenericFunctions.googleApiRequestAllItems.call(mockExecuteFunctions, 'connections', 'GET', '/people/connections')).rejects.toThrow('Rate limit exceeded');
        });
    });
    describe('allFields', () => {
