...
import { allFields, cleanData, googleApiRequest, googleApiRequestAllItems, } from './GenericFunctions';
export class GoogleContacts {
    ...
    methods = {
        loadOptions: {
            ...
            async getGroups() {
                const returnData = [];
                const groups = await googleApiRequestAllItems.call(this, 'contactGroups', 'GET', '/contactGroups');
            ...
            try {
                if (resource === 'contact') {
                    ...
                    if (operation === 'create') {
                        ...
                        if (additionalFields.eventsUi) {
                            ...
                            for (let index = 0; index < eventsValues.length; index++) {
                                const [month, day, year] = moment(eventsValues[index].date)
                                    .format('MM/DD/YYYY')
                                    .split('/');
                                eventsValues[index] = {
                                    date: {
                        ...
                        responseData = await googleApiRequest.call(this, 'POST', '/people:createContact', body, qs);
                    ...
                    if (operation === 'delete') {
                        ...
                        responseData = await googleApiRequest.call(this, 'DELETE', `/people/${contactId}:deleteContact`, {});
                    ...
                    if (operation === 'getAll') {
                        ...
                        if (returnAll) {
                            responseData = await googleApiRequestAllItems.call(this, useQuery ? 'results' : 'connections', 'GET', `/people${endpoint}`, {}, qs);
                            if (useQuery) {
                    ...
                    if (operation === 'update') {
                        ...
                        if (updateFields.etag) {
                            ...
                            const data = await googleApiRequest.call(this, 'GET', `/people/${contactId}`, {}, { personFields: 'Names' });
                            etag = data.etag;
                        }
                        if (fields.includes('*')) {
                            qs.personFields = allFields.join(',');
                        ...
                        if (updateFields.birthday) {
                            const [month, day, year] = moment(updateFields.birthday)
                                .format('MM/DD/YYYY')
                                .split('/');
                            body.birthdays = [
                                {
                        ...
                        responseData = await googleApiRequest.call(this, 'PATCH', `/people/${contactId}:updateContact`, body, qs);
                        responseData.contactId = responseData.resourceName.split('/')[1];
                    }
                }
