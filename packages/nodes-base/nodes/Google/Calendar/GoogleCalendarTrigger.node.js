...
export class GoogleCalendarTrigger {
    description = {
    ...
    async poll() {
        ...
        const calendarId = encodeURIComponentOnce(this.getNodeParameter('calendarId', '', { extractValue: true }));
        ...
        if (this.getMode() === 'manual') {
            ...
            if (triggerOn === 'eventCreated') {
                events = events.filter((event) => moment(event.created).isBetween(startDate, endDate));
            }
            else if (triggerOn === 'eventUpdated' || triggerOn === 'eventCancelled') {
                events = events.filter((event) => !moment(moment(event.created).format('YYYY-MM-DDTHH:mm:ss')).isSame(moment(event.updated).format('YYYY-MM-DDTHH:mm:ss')));
                if (triggerOn === 'eventCancelled') {
                    events = events.filter((event) => event.status === 'cancelled');
                }
            }
            else if (triggerOn === 'eventStarted') {
                events = events.filter((event) => moment(event.start.dateTime).isBetween(startDate, endDate, null, '[]'));
            }
            else if (triggerOn === 'eventEnded') {
                events = events.filter((event) => moment(event.end.dateTime).isBetween(startDate, endDate, null, '[]'));
            }
        }
