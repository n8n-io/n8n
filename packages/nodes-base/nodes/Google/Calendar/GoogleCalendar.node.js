    ...
    methods = {
        ...
        loadOptions: {
            ...
            async getConferenceSolutions() {
                ...
                const { conferenceProperties: { allowedConferenceSolutionTypes }, } = await googleApiRequest.call(this, 'GET', `/calendar/v3/users/me/calendarList/${calendar}`);
            ...
            try {
                if (resource === 'calendar') {
                    ...
                    if (operation === 'availability') {
                        ...
                        responseData = await googleApiRequest.call(this, 'POST', '/calendar/v3/freeBusy', body, {});
                ...
                if (resource === 'event') {
                    ...
                    if (operation === 'create') {
                        const calendarId = encodeURIComponentOnce(this.getNodeParameter('calendar', i, '', { extractValue: true }));
                        ...
                        if (additionalFields.rrule) {
                            ...
                            if (additionalFields.repeatHowManyTimes && additionalFields.repeatUntil) {
                                throw new NodeOperationError(this.getNode(), "You can set either 'Repeat How Many Times' or 'Repeat Until' but not both", { itemIndex: i });
                            }
                            ...
                            if (additionalFields.repeatUntil) {
                                const repeatUntil = moment(additionalFields.repeatUntil)
                                    .utc()
                    ...
                    if (operation === 'get') {
                        ...
                        if (responseData) {
                            if (nodeVersion >= 1.3 && options.returnNextInstance && responseData.recurrence) {
                                const eventInstances = (await googleApiRequest.call(this, 'GET', `/calendar/v3/calendars/${calendarId}/events/${responseData.id}/instances`, {}, {
                    ...
                    if (operation === 'getAll') {
                        ...
                        if (options.updatedMin) {
                            qs.updatedMin = addTimezoneToDate(dateObjectToISO(options.updatedMin), tz || timezone);
                        }
                        if (options.fields) {
                        ...
                        if (responseData) {
                            if (nodeVersion >= 1.3 && options.recurringEventHandling === 'next') {
                                ...
                                for (const event of responseData) {
                                    if (event.recurrence) {
                                        const eventInstances = (await googleApiRequestWithRetries({
                                            context: this,
                                            method: 'GET',
                            ...
                            if (!qs.timeMax &&
                                ...
                                const suggestTrim = eventExtendYearIntoFuture(responseData, timezone);
                                if (suggestTrim) {
                                    hints.push({
                    ...
                    if (operation === 'update') {
                        ...
                        if (nodeVersion >= 1.3) {
                            ...
                            if (modifyTarget === 'event') {
                                const instance = (await googleApiRequest.call(this, 'GET', `/calendar/v3/calendars/${calendarId}/events/${eventId}`, {}, qs));
                        ...
                        if (updateFields.attendeesUi) {
                            ...
                            body.attendees = [];
                            if (mode === 'add') {
                                const event = await googleApiRequest.call(this, 'GET', `/calendar/v3/calendars/${calendarId}/events/${eventId}`);
                        ...
                        if (updateFields.rrule) {
                            ...
                            if (updateFields.repeatUntil) {
                                const repeatUntil = moment(updateFields.repeatUntil)
                                    .utc()
                                    .format('YYYYMMDDTHHmmss');
                                body.recurrence?.push(`UNTIL=${repeatUntil}Z`);
                            }
