...
import { addNextOccurrence, addTimezoneToDate, dateObjectToISO, encodeURIComponentOnce, eventExtendYearIntoFuture, getCalendars, getTimezones, googleApiRequest, googleApiRequestAllItems, googleApiRequestWithRetries, } from '../GenericFunctions';
...
describe('googleApiRequest', () => {
    ...
    beforeEach(() => {
        ...
        requestOAuth2Spy = mockExecuteFunctions.helpers.requestOAuth2;
    });
    it('should make a successful GET request with default parameters', async () => {
        ...
        const result = await googleApiRequest.call(mockExecuteFunctions, 'GET', '/calendar/v3/users/me/calendarList');
    ...
    it('should make a POST request with body data', async () => {
        ...
        requestOAuth2Spy.mockResolvedValue(mockResponse);
        const result = await googleApiRequest.call(mockExecuteFunctions, 'POST', '/calendar/v3/calendars', requestBody);
        expect(requestOAuth2Spy).toHaveBeenCalledWith('googleCalendarOAuth2Api', {
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    it('should include query string parameters', async () => {
        ...
        requestOAuth2Spy.mockResolvedValue(mockResponse);
        await googleApiRequest.call(mockExecuteFunctions, 'GET', '/calendar/v3/calendars/primary/events', {}, queryParams);
        expect(requestOAuth2Spy).toHaveBeenCalledWith('googleCalendarOAuth2Api', expect.objectContaining({
            qs: queryParams,
        }));
    });
    it('should merge custom headers with default headers', async () => {
        ...
        requestOAuth2Spy.mockResolvedValue(mockResponse);
        await googleApiRequest.call(mockExecuteFunctions, 'PUT', '/calendar/v3/calendars/test', { summary: 'Updated' }, {}, undefined, customHeaders);
        expect(requestOAuth2Spy).toHaveBeenCalledWith('googleCalendarOAuth2Api', expect.objectContaining({
            headers: {
                'Content-Type': 'application/json',
    ...
    it('should use custom URI when provided', async () => {
        ...
        requestOAuth2Spy.mockResolvedValue(mockResponse);
        await googleApiRequest.call(mockExecuteFunctions, 'GET', '/calendar/v3/test', {}, {}, customUri);
        expect(requestOAuth2Spy).toHaveBeenCalledWith('googleCalendarOAuth2Api', expect.objectContaining({
            uri: customUri,
        }));
    });
    ...
    it('should throw NodeApiError when request fails', async () => {
        ...
        await expect(googleApiRequest.call(mockExecuteFunctions, 'GET', '/calendar/v3/calendars')).rejects.toThrow(NodeApiError);
    });
});
describe('googleApiRequestAllItems', () => {
    ...
    it('should fetch all items across multiple pages', async () => {
        ...
        const result = await googleApiRequestAllItems.call(mockExecuteFunctions, 'items', 'GET', '/calendar/v3/users/me/calendarList');
        ...
        expect(requestOAuth2Spy).toHaveBeenNthCalledWith(1, 'googleCalendarOAuth2Api', expect.objectContaining({
    ...
    it('should pass through body and query parameters', async () => {
        ...
        requestOAuth2Spy.mockResolvedValue(mockResponse);
        await googleApiRequestAllItems.call(mockExecuteFunctions, 'items', 'GET', '/calendar/v3/calendars/primary/events', body, query);
        expect(requestOAuth2Spy).toHaveBeenCalledWith('googleCalendarOAuth2Api', expect.objectContaining({
            body,
...
describe('getCalendars', () => {
    ...
    beforeEach(() => {
        ...
        requestOAuth2Spy = mockLoadOptionsFunctions.helpers.requestOAuth2;
        vi.clearAllMocks();
    });
...
describe('googleApiRequestWithRetries', () => {
    ...
    it('should not retry on non-retryable errors', async () => {
        ...
        await expect(googleApiRequestWithRetries({
            context: mockExecuteFunctions,
            method: 'GET',
            resource: '/calendar/v3/calendars/test',
