...
describe('Google Calendar Node', () => {
    ...
    describe('Google Calendar > Event > Update', () => {
        it('should update replace attendees in version 1.1', async () => {
            ...
            expect(genericFunctions.googleApiRequest).toHaveBeenCalledWith('PATCH', '/calendar/v3/calendars/undefined/events/myEvent', { attendees: [{ email: 'email1@mail.com' }] }, {});
        });
        it('should update replace attendees', async () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockReturnValueOnce('update');
            mockExecuteFunctions.getTimezone.mockReturnValueOnce('Europe/Berlin');
            mockExecuteFunctions.getNode.mockReturnValue(mock({ typeVersion: 1.2 }));
            mockExecuteFunctions.getNodeParameter.mockReturnValueOnce('myCalendar');
            mockExecuteFunctions.getNodeParameter.mockReturnValueOnce('myEvent');
            mockExecuteFunctions.getNodeParameter.mockReturnValueOnce(true);
            mockExecuteFunctions.getNodeParameter.mockReturnValueOnce({
        ...
        it('should update add attendees', async () => {
            ...
            expect(genericFunctions.googleApiRequest).toHaveBeenCalledWith('GET', '/calendar/v3/calendars/undefined/events/myEvent');
            expect(genericFunctions.googleApiRequest).toHaveBeenCalledWith('PATCH', '/calendar/v3/calendars/undefined/events/myEvent', { attendees: [{ email: 'email2@mail.com' }, { email: 'email1@mail.com' }] }, {});
        });
        describe('all-day <-> timed conversion', () => {
            const setupUpdate = (updateFields) => {
                mockExecuteFunctions.getInputData.mockReturnValue([{ json: {} }]);
                mockExecuteFunctions.getNodeParameter.mockReturnValueOnce('event');
                mockExecuteFunctions.getNodeParameter.mockReturnValueOnce('update');
                mockExecuteFunctions.getTimezone.mockReturnValueOnce('Europe/Berlin');
            ...
            const patchedBody = () => genericFunctions.googleApiRequest.mock.calls[0][2];
...
//# sourceMappingURL=event.update.test.js.map