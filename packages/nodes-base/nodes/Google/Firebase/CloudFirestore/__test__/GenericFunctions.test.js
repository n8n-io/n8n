...
import { googleApiRequest, jsonToDocument, documentToJson, fullDocumentToJson, } from '../GenericFunctions';
// Mock the getGoogleAccessToken function
vi.mock('../../../GenericFunctions', () => ({
    getGoogleAccessToken: vi.fn(),
}));
const mockGetGoogleAccessToken = getGoogleAccessToken;
describe('GoogleFirebaseCloudFirestore > GenericFunctions', () => {
    ...
    describe('googleApiRequest', () => {
        ...
        describe('OAuth2 authentication', () => {
            ...
            it('should make successful API request with OAuth2', async () => {
                ...
                const result = await googleApiRequest.call(mockExecuteFunctions, 'GET', '/databases/test/documents/users');
                expect(result).toEqual(mockResponse);
                expect(mockRequestWithAuth).toHaveBeenCalledWith('googleFirebaseCloudFirestoreOAuth2Api', expect.objectContaining({
                    method: 'GET',
            ...
            it('should remove empty body for GET requests', async () => {
                ...
                expect(mockRequestWithAuth).toHaveBeenCalledWith('googleFirebaseCloudFirestoreOAuth2Api', expect.not.objectContaining({ body: expect.anything() }));
            });
        ...
        describe('Service Account authentication', () => {
            ...
            it('should make API request with service account', async () => {
                ...
                expect(result).toEqual(mockResponse);
                expect(mockGetGoogleAccessToken).toHaveBeenCalledWith(expect.objectContaining({
                    email: 'test@project.iam.gserviceaccount.com',
                    privateKey: 'test-key',
                }), 'firestore');
                expect(mockRequestWithAuth).toHaveBeenCalledWith('googleApi', expect.objectContaining({
                    headers: {
                        'Content-Type': 'application/json',
            ...
            it('should handle service account credential errors', async () => {
                ...
                await expect(googleApiRequest.call(mockExecuteFunctions, 'GET', '/test')).rejects.toThrow(NodeApiError);
            });
    ...
    describe('documentToJson', () => {
        it('should handle undefined input', () => {
            expect(documentToJson(undefined)).toEqual({});
        });
        it('should convert primitive document values', () => {
            ...
            expect(documentToJson({ timestampValue: '2023-01-01T00:00:00.000Z' })).toBe('2023-01-01T00:00:00.000Z');
        });
    ...
    describe('fullDocumentToJson', () => {
        it('should handle undefined input', () => {
            expect(fullDocumentToJson(undefined)).toBeUndefined();
        });
