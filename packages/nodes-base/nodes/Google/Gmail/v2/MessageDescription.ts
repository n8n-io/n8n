...
export const messageFields: INodeProperties[] = [
	...
	},
	{
		...
		builderHint: {
			message:
				'Set to false when downstream nodes need the full email body, attachments, or content fields (text, html, textAsHtml, payload). When true, only metadata-like fields are returned and references such as $json.text or $json.html will be empty.',
		},
	},
	{
		displayName: 'Simplify',
		...
		builderHint: {
			message:
				'Default is false so downstream nodes can read the full email body and attachments. Set to true only when the workflow needs the metadata-only shape — references such as $json.text or $json.html resolve to empty values when true.',
		},
	},
	{
		displayName: 'Options',
