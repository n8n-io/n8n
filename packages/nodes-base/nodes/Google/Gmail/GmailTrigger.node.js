...
import { googleApiRequest, googleApiRequestAllItems, parseRawEmail, prepareQuery, simplifyOutput, } from './GenericFunctions';
...
export class GmailTrigger {
    description = {
        ...
        builderHint: {
            searchHint: 'When downstream nodes create records (tasks, rows, tickets) per email, guarantee each email is processed exactly once: filter to unread AND mark each email read/labelled after its record is created, or track handled message ids in a Data Table. Otherwise the same email can be reprocessed into duplicates.',
            relatedNodes: [
                {
                    nodeType: 'n8n-nodes-base.gmail',
                    relationHint: 'Mark polled emails as handled after processing (message markAsRead, or addLabels when the trigger query excludes that label) so they are not picked up again',
                },
                {
...
When this trigger feeds an action that creates records (tasks, rows, tickets, messages), ensure each email is handled once: keep \`readStatus: 'unread'\` AND add a Gmail \`markAsRead\` step (\`addLabels\` works only if this trigger's \`q\` also excludes that label — a label does not mark the email read), or record handled message ids in a Data Table — look the id up before creating the record, skip ids already seen, insert it after the create succeeds. The unread filter alone changes nothing if no step ever marks the email read. Wire the mark-as-handled step AFTER the record-creating node, so a mid-run failure cannot consume an email without producing its record.
    ...
    async poll() {
        ...
        const maxResults = shouldLimitMessages
            ? this.getNodeParameter('maxResults', 10)
            : Infinity;
        let responseData = [];
        ...
        const fetchAndProcessMessage = async (messageId, fetchQs) => {
            const fullMessage = (await googleApiRequest.call(this, 'GET', `/gmail/v1/users/me/messages/${messageId}`, {}, fetchQs));
        ...
        const simplifyResponseData = async () => {
            if (!simple || responseData.length === 0)
                return;
            try {
                responseData = this.helpers.returnJsonArray(await simplifyOutput.call(this, responseData.map((item) => item.json)));
            }
            catch (error) {
                if (this.getMode() === 'manual' || !nodeStaticData.lastTimeChecked) {
                    throw error;
                }
                this.logger.error(`Gmail Trigger could not simplify the output of '${node.name}': '${error.description}'`, { node: node.name, error });
            }
        };
        ...
        try {
                ...
                for (const [id, attempts] of retryNow) {
                    ...
                    try {
                        ...
                        if (attempted >= MAX_PENDING_FETCH_ATTEMPTS) {
                            this.logger.warn(`Gmail Trigger cannot fetch message ${id} after ${attempted} attempts; skipping it`, { node: node.name });
            ...
            if (shouldLimitMessages && pendingIds.length > 0 && budget > 0) {
                ...
                for (const [index, id] of pendingIds.entries()) {
                    ...
                    // doomed requests.
                    if (budget <= 0 || newlyFailed.length >= maxResults)
                        break;
                    try {
                        await fetchAndProcessMessage(id, fetchQs);
                        budget -= 1;
            ...
            // loops).
            do {
                const messagesResponse = await googleApiRequest.call(this, 'GET', '/gmail/v1/users/me/messages', {}, pageToken ? { ...qs, pageToken } : qs);
                messages.push(...(messagesResponse.messages ?? []));
                pageToken = messagesResponse.nextPageToken;
                pagesScanned++;
            } while (shouldLimitMessages &&
                pageToken &&
            ...
            if (shouldLimitMessages) {
                ...
                if (!messages.length && !allFetchedMessages.length) {
                    ...
                    if (!windowFullyScanned) {
                        ...
                        this.logger.warn("Gmail Trigger backlog cannot progress within one poll's reach; advancing past older messages it could not scan", { node: node.name });
                        nodeStaticData.lastTimeChecked = +now;
        ...
        const lastEmailDate = allFetchedMessages.reduce((lastDate, message) => (message.date > lastDate ? message.date : lastDate), 0);
        ...
        if (shouldLimitMessages && !windowFullyScanned) {
            const trackedIds = (nodeStaticData.pendingMessageIds?.length ?? 0) +
            ...
            if (trackedIds < MAX_TRACKED_BACKLOG_IDS) {
                ...
                this.logger.warn(`Gmail Trigger backlog exceeds ${MAX_TRACKED_BACKLOG_IDS} tracked ids; advancing past older messages it could not scan`, { node: node.name });
            }
        }
