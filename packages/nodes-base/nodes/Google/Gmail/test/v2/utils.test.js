...
import { parseRawEmail, prepareEmailAttachments, prepareEmailBody, prepareEmailsInput, prepareQuery, prepareTimestamp, } from '../../GenericFunctions';
...
describe('Google Gmail v2, prepareTimestamp', () => {
    ...
    it('should throw error on invalid data', () => {
        ...
        expect(() => prepareTimestamp(node, 0, '', dateInput, 'before')).toThrow("Invalid date/time in 'Received Before' field");
        expect(() => prepareTimestamp(node, 0, '', dateInput, 'after')).toThrow("Invalid date/time in 'Received After' field");
    });
});
describe('parseRawEmail', () => {
    it('should return a date string', async () => {
        // ARRANGE
        const executionFunctions = mock();
        const rawEmail = 'Date: Wed, 28 Aug 2024 00:36:37 -0700'.replace(/\n/g, '\r\n');
        // ACT
        const { json } = await parseRawEmail.call(executionFunctions, { raw: Buffer.from(rawEmail, 'utf8').toString('base64') }, 'attachment_');
        // ASSERT
...
describe('email input preparation', () => {
    it('should convert recipient input to a string before splitting', () => {
        const executionFunctions = mock();
        const result = prepareEmailsInput.call(executionFunctions, ['alice@example.com', 'bob@example.com'], 'To', 0);
    ...
    it('should convert attachment property names to strings before splitting', async () => {
        ...
        const result = await prepareEmailAttachments.call(executionFunctions, { attachmentsBinary: [{ property: 123 }] }, 0);
...
describe('prepareQuery', () => {
    ...
    it('should append sender to existing q parameter', () => {
        const result = prepareQuery.call(executionFunctions, { q: 'subject:hello', sender: 'alice@example.com' }, 0);
    ...
    it('should combine multiple filters into a single q parameter', () => {
        const result = prepareQuery.call(executionFunctions, {
            q: 'test',
