...
describe('GmailTrigger', () => {
    ...
    it('should exclude scheduled emails from query on v1.4', async () => {
        ...
        nock(baseUrl)
            ...
            .query((q) => q.q.includes('-in:scheduled'))
    ...
    it('should update timestamp even when all emails are filtered (prevents infinite re-fetch)', async () => {
        ...
        nock(baseUrl)
            ...
            .reply(200, createMessage({ id: 'draft-1', internalDate: draftEmailTimestamp, labelIds: ['DRAFT'] }));
        const { response } = await testPollingTriggerNode(GmailTrigger, {
            node: { typeVersion: 1.3, parameters: { filters: { includeDrafts: false } } },
            workflowStaticData: {
                'Gmail Trigger': { lastTimeChecked: initialTimestamp },
    ...
    describe('v1.4 - maxResults limit', () => {
        ...
        it('should not produce duplicates across a 3-poll pending drain cycle', async () => {
            ...
            nock(baseUrl)
                .get(new RegExp('/gmail/v1/users/me/messages?.*'))
                .reply(200, { messages: [], resultSizeEstimate: 0 });
        ...
        it('should reset possibleDuplicates when lastTimeChecked advances after a merge', async () => {
            ...
            expect(workflowStaticData['Gmail Trigger'].possibleDuplicates).toEqual(expect.arrayContaining(['1', '2', '3']));
        ...
        it('should handle same-timestamp messages split across pending boundary', async () => {
            ...
            expect(workflowStaticData['Gmail Trigger'].possibleDuplicates).toEqual(expect.arrayContaining(['A', 'B']));
            ...
            expect(workflowStaticData['Gmail Trigger'].possibleDuplicates).toEqual(expect.arrayContaining(['A', 'B', 'C']));
            expect(workflowStaticData['Gmail Trigger'].lastTimeChecked).toBe(3000000000);
        });
        ...
        it('should not re-emit pending messages drained during an early-return poll', async () => {
            ...
            const pending = (workflowStaticData['Gmail Trigger'].pendingMessageIds ?? []);
    ...
    describe('v1.4 - backlog scan, drain and retry', () => {
        ...
        const listPage = (ids, nextPageToken) => ({
            messages: ids.map((id) => createListMessage({ id })),
            resultSizeEstimate: ids.length,
            ...(nextPageToken ? { nextPageToken } : {}),
        });
        const mockLabels = () => nock(baseUrl)
            .get('/gmail/v1/users/me/labels')
            .reply(200, { labels: [{ id: 'testLabelId', name: 'Test Label Name' }] });
        const mockList = (page, expectedPageToken) => nock(baseUrl)
            .get('/gmail/v1/users/me/messages')
            .query((q) => expectedPageToken === undefined
            ? q.pageToken === undefined
            : q.pageToken === expectedPageToken)
            .reply(200, page);
        const mockGet = (id, internalDateMs) => nock(baseUrl)
            .get(`/gmail/v1/users/me/messages/${id}`)
            .query(true)
            .reply(200, createMessage({ id, internalDate: String(internalDateMs) }));
        const mockGetError = (id) => nock(baseUrl)
            .get(`/gmail/v1/users/me/messages/${id}`)
            .query(true)
        ...
        it('should hold lastTimeChecked when the page cap is hit with a token remaining', async () => {
            ...
            pages.forEach((ids, page) => mockList(listPage(ids, `token-${page + 1}`), page === 0 ? undefined : `token-${page}`));
            ...
            expect(workflowStaticData['Gmail Trigger'].possibleDuplicates).toEqual(expect.arrayContaining(['p0a', 'p0b']));
        });
        ...
        it('should hold the cursor when the scan fails mid-pagination', async () => {
            ...
            expect(workflowStaticData['Gmail Trigger'].lastTimeChecked).toBe(initialTimestamp);
            expect(workflowStaticData['Gmail Trigger'].possibleDuplicates).toEqual(expect.arrayContaining(['X', 'P1']));
        });
        ...
        it('should keep unfetched new-message ids when a fetch fails after the scan', async () => {
            ...
            expect(workflowStaticData['Gmail Trigger'].possibleDuplicates).toEqual(expect.arrayContaining(['1', '3']));
        });
        it('should keep holding the cursor when a drain fetch fails during a held backlog', async () => {
            ...
            expect(workflowStaticData['Gmail Trigger'].lastTimeChecked).toBe(initialTimestamp);
            expect(workflowStaticData['Gmail Trigger'].pendingMessageIds).toEqual([]);
            expect(workflowStaticData['Gmail Trigger'].possibleDuplicates).toEqual(expect.arrayContaining(['X', 'P1', 'P3']));
        });
        ...
        it('should count set-aside ids towards the stored-id bound', async () => {
            ...
            expect(workflowStaticData['Gmail Trigger'].lastTimeChecked).toBeGreaterThan(initialTimestamp);
        });
...
//# sourceMappingURL=GmailTrigger.test.js.map