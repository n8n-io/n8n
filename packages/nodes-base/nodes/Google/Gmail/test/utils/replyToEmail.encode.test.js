...
describe('replyToEmail — real encodeEmail round-trip', () => {
    ...
    test.each([
    ...
    ])('produces a valid multi-recipient To header: $name', async ({ replyTo, expectedRecipients }) => {
        ...
        await replyToEmail.call(mockExecuteFunctions, 'message123', { replyToSenderOnly: true }, 0, 2.2);
        const sendCall = mockedGoogleApiRequest.mock.calls.find((call) => call[1] === '/gmail/v1/users/me/messages/send');
        const body = sendCall?.[2];
        ...
        expect(addressparser(toHeader, { flatten: true }).map((a) => a.address)).toEqual(expectedRecipients);
    });
});
//# sourceMappingURL=replyToEmail.encode.test.js.map