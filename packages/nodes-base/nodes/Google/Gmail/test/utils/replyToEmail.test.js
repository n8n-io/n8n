...
describe('replyToEmail', () => {
    ...
    test('should reply to email with basic options', async () => {
        ...
        expect(mockedGoogleApiRequest).toHaveBeenNthCalledWith(1, 'GET', '/gmail/v1/users/me/messages/message123', {}, { format: 'metadata' });
        expect(mockedGoogleApiRequest).toHaveBeenNthCalledWith(2, 'GET', '/gmail/v1/users/me/profile');
        expect(mockedGoogleApiRequest).toHaveBeenNthCalledWith(3, 'POST', '/gmail/v1/users/me/messages/send', expect.objectContaining({
            threadId: 'thread123',
            raw: expect.any(String),
        }), { format: 'metadata' });
        expect(mockedEncodeEmail).toHaveBeenCalledWith(expect.objectContaining({
            from: '',
    ...
    test('should handle attachments when provided', async () => {
        ...
        expect(mockedGoogleApiRequest).toHaveBeenNthCalledWith(3, 'POST', '/gmail/v1/users/me/messages/send', expect.anything(), expect.objectContaining({
            userId: 'me',
            uploadType: 'media',
            format: 'metadata',
        }));
    ...
    const buildHeaders = ({ from = 'John Doe <john@example.com>', to = 'recipient@example.com', replyTo, }) => {
        const headers = [
            { name: 'Subject', value: 'Original Subject' },
            { name: 'Message-ID', value: '<original@example.com>' },
        ...
        if (replyTo !== undefined)
            headers.push({ name: 'Reply-To', value: replyTo });
        return headers;
    };
    const runReply = async (headers, options) => {
        mockedGoogleApiRequest
...
//# sourceMappingURL=replyToEmail.test.js.map