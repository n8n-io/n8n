...
export async function getRevisionFile(exportLink) {
    const mimeType = BINARY_MIME_TYPE;
    const response = await apiRequest.call(this, 'GET', '', undefined, { mimeType }, exportLink, undefined, {
...
export function sheetBinaryToArrayOfArrays(data, sheetName, range) {
...
export function arrayOfArraysToJson(sheetData, columns) {
    const returnData = [];
...
export function compareRevisions(previous, current, keyRow, includeInOutput, columnsToWatch, dataStartIndex, event) {
    ...
    for (let i = dataStartIndex; i < dataLength; i++) {
        ...
        if (columnsToWatch?.length) {
            ...
            if (isEqual(currentRow, previousRow))
                continue;
        }
        else {
            if (isEqual(current[i], previous[i]))
                continue;
        }
