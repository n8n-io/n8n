...
export class GoogleSheet {
	...
	async clearData(range: string): Promise<object> {
		...
		const response = await apiRequest.call(this.executeFunctions, 'POST', `/v4/spreadsheets/${this.id}/values/${range}:clear`, body);
	...
	async setData(range: string, data: string[][], valueInputMode: ValueInputOption) {
		...
		const response = await apiRequest.call(this.executeFunctions, 'POST', `/v4/spreadsheets/${this.id}/values/${range}`, body);
	...
	async appendData(range: string, data: string[][], valueInputMode: ValueInputOption) {
		...
		const response = await apiRequest.call(this.executeFunctions, 'POST', `/v4/spreadsheets/${this.id}/values/${range}:append`, body, query);
