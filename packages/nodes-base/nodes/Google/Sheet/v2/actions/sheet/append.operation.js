...
import { cellFormat, columnsResourceMapperBuilderHint, handlingExtraData, useAppendOption, } from './commonDescription';
import { autoMapInputData, cellFormatDefault, checkForSchemaChanges, mapFields, untilSheetSelected, } from '../../helpers/GoogleSheets.utils';
...
export async function execute(sheet, range, sheetId) {
    ...
    if (nodeVersion >= 4.4 && dataMode !== 'autoMapInputData') {
        ...
        if (headerRow === undefined) {
            throw new NodeOperationError(this.getNode(), `Could not retrieve the column names from row ${keyRowIndex}`);
        }
        ...
        if (!Array.isArray(schema) || schema.length === 0) {
            throw new NodeOperationError(this.getNode(), '`columns.schema` is required when `columns.mappingMode` is `defineBelow`', {
    ...
    const valueInputMode = options.cellFormat || cellFormatDefault(nodeVersion);
