...
import { cellFormat, handlingExtraData, locationDefine, upsertColumnsResourceMapperBuilderHint, useAppendOption, } from './commonDescription';
import { ROW_NUMBER, } from '../../helpers/GoogleSheets.types';
import { cellFormatDefault, checkForSchemaChanges, untilSheetSelected, } from '../../helpers/GoogleSheets.utils';
...
export async function execute(sheet, sheetName, sheetId) {
    ...
    let columnsToMatchOn;
    if (nodeVersion < 4) {
        columnsToMatchOn = [this.getNodeParameter('columnToMatchOn', 0)];
    }
    else {
        ...
        if (!Array.isArray(matchingColumns) || matchingColumns.length === 0) {
            throw new NodeOperationError(this.getNode(), '`columns.matchingColumns` is required for the Append or Update Row operation', {
                description: 'Set `columns.matchingColumns` to a non-empty `string[]` of header names that uniquely identify the row to update (e.g. `["id"]` or `["email"]`). If there is no key column to match on, use the `append` operation instead.',
            });
        }
