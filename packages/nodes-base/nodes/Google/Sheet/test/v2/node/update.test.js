...
describe('Google Sheet - Update', () => {
    ...
    it('should update rows by column values with special character', async () => {
        ...
        ], 'Sheet1!A:Z', [['Real.1', '21 "', 'dfd', 'ha []', 'macarena']]);
        expect(mockGoogleSheet.batchUpdate).toHaveBeenCalledWith([
            {
                range: 'Sheet1!B3',
                values: [['** δ$% " []']],
...
describe('Google Sheet - Update 4.6', () => {
    ...
    beforeEach(() => {
        mockExecuteFunctions = mock();
        mockGoogleSheet = mock();
        mockExecuteFunctions.getNode.mockReturnValueOnce(mock({ typeVersion: 4.6 }));
        mockGoogleSheet.batchUpdate.mockResolvedValueOnce([]);
    });
    afterEach(() => {
        vi.resetAllMocks();
    ...
    describe('row_number input error', () => {
        it.each([{ rowNumber: undefined }])('displays a helpful error message when row_number is $rowNumber', async ({ rowNumber }) => {
            mockExecuteFunctions.getInputData.mockReturnValueOnce([
                {
                    json: {
            ...
            mockGoogleSheet.getColumnValues.mockResolvedValueOnce([]);
            await expect(execute.call(mockExecuteFunctions, mockGoogleSheet, 'Sheet1')).rejects.toEqual(expect.objectContaining({
    ...
    describe('non-row_number undefined', () => {
        it.each([{ nonRowNumber: undefined }])('displays a helpful error message when row_number is $rowNumber', async ({ nonRowNumber }) => {
            mockExecuteFunctions.getInputData.mockReturnValueOnce([
                {
                    json: {
                        row_number: 2,
...
describe('Google Sheet - Update v4.6 vs v4.7 Behavior', () => {
    ...
    it('v4.7: empty string in UI is preserved and sent to backend to clear cell', async () => {
        mockExecuteFunctions = mock();
        mockGoogleSheet = mock();
        mockExecuteFunctions.getNode.mockReturnValueOnce(mock({ typeVersion: 4.7 }));
        mockGoogleSheet.batchUpdate.mockResolvedValueOnce([]);
        mockExecuteFunctions.getInputData.mockReturnValueOnce([
            {
                json: {},
