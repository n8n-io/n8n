...
describe('Google Sheet - Append or Update', () => {
    ...
    it('should insert input data if sheet is empty', async () => {
        ...
        mockExecuteFunctions.getNode.mockReturnValueOnce(mock());
        ...
        expect(mockGoogleSheet.updateRows).toHaveBeenCalledWith('Sheet1', [['name', 'text']], 'USER_ENTERED', 1);
    ...
    it('should throw error when no column names can be retrieved and sheet has data', async () => {
        ...
        mockExecuteFunctions.getNode.mockReturnValue(mock({ typeVersion: 3 }));
        mockExecuteFunctions.getNodeParameter
        ...
        mockGoogleSheet.getData.mockResolvedValueOnce([
            undefined, // No header row at keyRowIndex 0
        ...
        await expect(execute.call(mockExecuteFunctions, mockGoogleSheet, 'Sheet1', '1234')).rejects.toThrow(NodeOperationError);
    });
    ...
    it('should handle autoMapInputData with ignoreIt option', async () => {
        ...
        mockGoogleSheet.appendSheetData.mockResolvedValueOnce([]);
        await execute.call(mockExecuteFunctions, mockGoogleSheet, 'Sheet1', '1234');
        expect(mockGoogleSheet.prepareDataForUpdateOrUpsert).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should handle autoMapInputData with insertInNewColumn option', async () => {
        ...
        expect(mockGoogleSheet.updateRows).toHaveBeenCalledWith('Sheet1', [['id', 'name', 'newField']], 'RAW', 1);
    });
    ...
    it('should handle newColumn type in defineBelow mode (v3)', async () => {
        ...
        expect(mockGoogleSheet.updateRows).toHaveBeenCalledWith('Sheet1', [['id', 'name', 'description']], 'RAW', 1);
    });
    it('should handle both update and append data', async () => {
        ...
        mockExecuteFunctions.getNode.mockReturnValue(mock({ typeVersion: 4.5 }));
        ...
        mockGoogleSheet.appendSheetData.mockResolvedValueOnce([]);
        await execute.call(mockExecuteFunctions, mockGoogleSheet, 'Sheet1', '1234');
        expect(mockGoogleSheet.batchUpdate).toHaveBeenCalledWith([{ range: 'A2:B2', values: [['1', 'Updated Name']] }], 'USER_ENTERED');
    ...
    it('should handle custom valueRenderMode option', async () => {
        ...
        mockGoogleSheet.appendSheetData.mockResolvedValueOnce([]);
        await execute.call(mockExecuteFunctions, mockGoogleSheet, 'Sheet1', '1234');
        expect(mockGoogleSheet.getColumnValues).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should call checkForSchemaChanges for node version >= 4.4', async () => {
        ...
        const mockNode = mock({ typeVersion: 4.5 });
        mockExecuteFunctions.getInputData.mockReturnValueOnce([
            {
                json: { id: 1, name: 'Test' },
                pairedItem: { item: 0, input: undefined },
        ...
        await execute.call(mockExecuteFunctions, mockGoogleSheet, 'Sheet1', '1234');
        expect(GoogleSheetsUtils.checkForSchemaChanges).toHaveBeenCalledWith(mockNode, // node
        ['id', 'name'], // columnNames
        mockSchema);
    });
});
describe('Google Sheet - Append or Update v4.6 vs v4.7 Behavior', () => {
    ...
    it('v4.6: empty string in UI gets filtered out, field not sent to backend', async () => {
        ...
        mockExecuteFunctions.getNode
            .mockReturnValueOnce(mock({ typeVersion: 4.6 }))
            .mockReturnValueOnce(mock({ typeVersion: 4.6 }));
        mockExecuteFunctions.getInputData.mockReturnValueOnce([
            {
                json: {},
    ...
    it('v4.7: empty string in UI is preserved and sent to backend to clear cell', async () => {
        ...
        mockGoogleSheet = mock();
        mockExecuteFunctions.getNode
            .mockReturnValueOnce(mock({ typeVersion: 4.7 }))
            .mockReturnValueOnce(mock({ typeVersion: 4.7 }));
        mockExecuteFunctions.getInputData.mockReturnValueOnce([
            {
                json: {},
...
//# sourceMappingURL=appendOrUpdate.test.js.map