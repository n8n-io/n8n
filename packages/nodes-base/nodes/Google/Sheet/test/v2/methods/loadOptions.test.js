...
import { getSheetHeaderRow, getSheetHeaderRowAndAddColumn, getSheetHeaderRowAndSkipEmpty, getSheetHeaderRowWithGeneratedColumnNames, getSheets, } from '../../../v2/methods/loadOptions';
...
describe('Google Sheets Functions', () => {
    ...
    beforeEach(() => {
        ...
        mockLoadOptionsFunctions = {
            getNodeParameter: vi.fn((paramName) => {
    ...
    describe('getSheets', () => {
        it('should return an empty array if documentId is null', async () => {
            ...
            const result = await getSheets.call(mockLoadOptionsFunctions);
        ...
        it('should throw an error if no data is returned', async () => {
            ...
            await expect(getSheets.call(mockLoadOptionsFunctions)).rejects.toThrow(NodeOperationError);
        });
    ...
    describe('getSheetHeaderRow', () => {
        it('should return an empty array if documentId is null', async () => {
            mockLoadOptionsFunctions.getNodeParameter = vi.fn().mockReturnValue(null);
            const result = await getSheetHeaderRow.call(mockLoadOptionsFunctions);
            expect(result).toEqual([]);
        });
        ...
        it('should throw an error if no data is returned', async () => {
            ...
            await expect(getSheetHeaderRow.call(mockLoadOptionsFunctions)).rejects.toThrow(NodeOperationError);
        });
    ...
    describe('getSheetHeaderRowAndAddColumn', () => {
        it('should add a new column and exclude the column to match on', async () => {
            ...
            const result = await getSheetHeaderRowAndAddColumn.call(mockLoadOptionsFunctions);
            expect(result).toEqual([
                { name: 'Header1', value: 'Header1' },
    ...
    describe('getSheetHeaderRowWithGeneratedColumnNames', () => {
        it('should generate column names for empty values', async () => {
            ...
            mockGoogleSheetInstance.getData.mockResolvedValue([['', 'Header1', '']]);
            mockGoogleSheetInstance.testFilter.mockReturnValue(['', 'Header1', '']);
            const result = await getSheetHeaderRowWithGeneratedColumnNames.call(mockLoadOptionsFunctions);
            expect(result).toEqual([
                // eslint-disable-next-line n8n-nodes-base/node-param-display-name-miscased
    ...
    describe('getSheetHeaderRowAndSkipEmpty', () => {
        it('should skip columns with empty values', async () => {
            ...
            mockGoogleSheetInstance.getData.mockResolvedValue([['', 'Header1', '']]);
            mockGoogleSheetInstance.testFilter.mockReturnValue(['', 'Header1', '']);
            const result = await getSheetHeaderRowAndSkipEmpty.call(mockLoadOptionsFunctions);
