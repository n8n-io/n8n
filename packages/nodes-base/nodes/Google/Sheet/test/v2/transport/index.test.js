...
describe('Google Sheets Transport', () => {
    ...
    describe('apiRequest', () => {
        it('should make successful request with service account authentication', async () => {
            ...
            getGoogleAccessToken.mockResolvedValue({ access_token: mockAccessToken });
            ...
            expect(mockExecuteFunction.helpers.request).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should forward the sheetV2Trigger service scope to getGoogleAccessToken when passed', async () => {
            ...
            await apiRequest.call(mockExecuteFunction, method, resource, undefined, { mimeType: 'application/test' }, 'https://example.com/export', undefined, { resolveWithFullResponse: true, encoding: null, json: false }, 'sheetV2Trigger');
            expect(getGoogleAccessToken).toHaveBeenCalledWith(expect.anything(), 'sheetV2Trigger');
        });
        it('should default the service scope to sheetV2 when none is passed', async () => {
            ...
            expect(getGoogleAccessToken).toHaveBeenCalledWith(expect.anything(), 'sheetV2');
        ...
        it('should handle custom headers and query parameters', async () => {
            ...
            mockExecuteFunction.getNodeParameter = vi.fn().mockReturnValue('serviceAccount');
            mockExecuteFunction.getCredentials = vi.fn().mockResolvedValue({});
            getGoogleAccessToken.mockResolvedValue({ access_token: 'token' });
    ...
    describe('apiRequestAllItems', () => {
        it('should fetch all pages of results', async () => {
            ...
            const result = await apiRequestAllItems.call(mockLoadOptionsFunction, propertyName, method, endpoint);
