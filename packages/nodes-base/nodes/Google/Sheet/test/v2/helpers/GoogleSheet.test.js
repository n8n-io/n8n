...
describe('GoogleSheet', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        googleSheet = new GoogleSheet(spreadsheetId, mockExecuteFunctions);
    });
    describe('clearData', () => {
        it('should make correct API call to clear data', async () => {
            const range = 'Sheet1!A1:B2';
            await googleSheet.clearData(range);
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'POST', `/v4/spreadsheets/${spreadsheetId}/values/${range}:clear`, { spreadsheetId, range });
        });
    });
    describe('getData', () => {
        it('should retrieve data with correct parameters', async () => {
            ...
            apiRequest.call.mockResolvedValue(mockResponse);
            const result = await googleSheet.getData(range, valueRenderMode);
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'GET', `/v4/spreadsheets/${spreadsheetId}/values/${range}`, {}, {
    ...
    describe('convertSheetDataArrayToObjectArray', () => {
        ...
        it('should handle empty rows when addEmpty is false', () => {
            ...
            const result = googleSheet.convertSheetDataArrayToObjectArray(data, 1, ['name', 'age'], false);
            expect(result).toEqual([
                { name: 'John', age: '30' },
        ...
        it('should handle empty columns when includeHeadersWithEmptyCells is true', () => {
            ...
            const result = googleSheet.convertSheetDataArrayToObjectArray(data, 1, ['name', 'age'], false, true);
            expect(result).toEqual([
                { name: 'John', age: '30' },
                { name: 'MARY', age: '' },
                { name: 'Jane', age: '25' },
    ...
    describe('lookupValues', () => {
        ...
        it('should throw error for invalid key row', async () => {
            const lookupValues = [{ lookupColumn: 'age', lookupValue: '30' }];
            await expect(googleSheet.lookupValues({
                inputData: [['name', 'age']],
                keyRowIndex: -1,
                dataStartRowIndex: 1,
                lookupValues,
    ...
    describe('appendSheetData', () => {
        it('should correctly prepare and append data', async () => {
            ...
            apiRequest.call.mockResolvedValue(mockAppendResponse);
        ...
        it('should use columnNamesHint instead of calling getData for column names', async () => {
            ...
            apiRequest.call.mockResolvedValue({ values: [['John', '30']] });
            ...
            expect(apiRequest.call).toHaveBeenCalledWith(expect.anything(), 'PUT', expect.stringContaining('/values/'), expect.anything(), expect.anything());
        });
    ...
    describe('appendEmptyRowsOrColumns', () => {
        it('should throw error when no rows or columns specified', async () => {
            await expect(googleSheet.appendEmptyRowsOrColumns('sheet1', 0, 0)).rejects.toThrow('Must specify at least one column or row to add');
        });
        it('should make correct API call to append rows and columns', async () => {
            ...
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'POST', `/v4/spreadsheets/${spreadsheetId}:batchUpdate`, {
                requests: [
                    {
                        appendDimension: {
                            sheetId,
    ...
    describe('spreadsheetGetSheets', () => {
        it('should retrieve spreadsheet sheets with correct parameters', async () => {
            ...
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'GET', `/v4/spreadsheets/${spreadsheetId}`, {}, { fields: 'sheets.properties' });
    ...
    describe('spreadsheetGetSheet', () => {
        ...
        it('should throw error when sheet not found by name', async () => {
            const mockNode = { type: 'test-node' };
            await expect(googleSheet.spreadsheetGetSheet(mockNode, 'name', 'NonExistentSheet')).rejects.toThrow('Sheet with name NonExistentSheet not found');
        });
        it('should throw error when sheet not found by ID', async () => {
            const mockNode = { type: 'test-node' };
            await expect(googleSheet.spreadsheetGetSheet(mockNode, 'id', '999999999')).rejects.toThrow('Sheet with ID 999999999 not found');
        });
    });
    ...
    describe('spreadsheetBatchUpdate', () => {
        it('should make correct API call for batch update', async () => {
            ...
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'POST', `/v4/spreadsheets/${spreadsheetId}:batchUpdate`, { requests });
        });
    });
    describe('batchUpdate', () => {
        it('should make correct API call for batch value update', async () => {
            ...
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'POST', `/v4/spreadsheets/${spreadsheetId}/values:batchUpdate`, {
                data: updateData,
    ...
    describe('appendData', () => {
        beforeEach(() => {
            ...
            apiRequest.call.mockImplementation(async (_, method, _url) => {
                if (method === 'GET') {
        ...
        it('should use provided last row', async () => {
            ...
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'PUT', expect.stringContaining('Sheet1!5:5'), expect.any(Object), expect.any(Object));
        });
        it('should use append mode when useAppend is true', async () => {
            const data = [['John', '30']];
            await googleSheet.appendData('Sheet1!A:B', data, 'USER_ENTERED', 5, true);
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'POST', expect.stringContaining(':append'), expect.any(Object), expect.any(Object));
        });
    });
    describe('updateRows', () => {
        it('should make PUT request when useAppend is false', async () => {
            ...
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'PUT', `/v4/spreadsheets/${spreadsheetId}/values/Sheet1!2:2`, {
                range: 'Sheet1!2:2',
                values: data,
            }, { valueInputOption: 'USER_ENTERED' });
        });
        it('should make POST request when useAppend is true', async () => {
            ...
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'POST', `/v4/spreadsheets/${spreadsheetId}/values/Sheet1!2:3:append`, {
                range: 'Sheet1!2:3',
                values: data,
            }, { valueInputOption: 'USER_ENTERED' });
        });
        it('should handle range with rowsLength', async () => {
            ...
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'PUT', `/v4/spreadsheets/${spreadsheetId}/values/Sheet1!2:3`, {
                range: 'Sheet1!2:3',
                values: data,
            }, { valueInputOption: 'USER_ENTERED' });
        });
    ...
    describe('getColumnValues', () => {
        ...
        it('should make API call when sheet data not provided', async () => {
            ...
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'GET', expect.stringContaining('/values/Sheet1!A1:A10'), {}, { valueRenderOption: 'UNFORMATTED_VALUE', dateTimeRenderOption: 'FORMATTED_STRING' });
            expect(result).toEqual(['value1', 'value2']);
        });
        it('should throw error when column data cannot be retrieved', async () => {
            apiRequest.call.mockResolvedValue({ values: undefined });
            await expect(googleSheet.getColumnValues({
                range: 'Sheet1!A:B',
                keyIndex: 0,
                dataStartRowIndex: 1,
                valueRenderMode: 'UNFORMATTED_VALUE',
    ...
    describe('prepareDataForUpdateOrUpsert', () => {
        beforeEach(() => {
            ...
            apiRequest.call.mockImplementation((_, _method, url) => {
        ...
        it('should throw error when index key not found and upsert is false', async () => {
            ...
            await expect(googleSheet.prepareDataForUpdateOrUpsert({
                inputData,
    ...
    describe('prepareDataForUpdatingByRowNumber', () => {
        it('should prepare update data using row numbers', () => {
            ...
            const result = googleSheet.prepareDataForUpdatingByRowNumber(inputData, 'Sheet1!A1:D10', columnNamesList);
        ...
        it('should stringify object values', () => {
            ...
            const result = googleSheet.prepareDataForUpdatingByRowNumber(inputData, 'Sheet1!A1:B10', columnNamesList);
    ...
    describe('private method testing via clearData (encodeRange)', () => {
        it('should encode sheet name with special characters', async () => {
            ...
            await googleSheet.clearData(range);
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'POST', `/v4/spreadsheets/${spreadsheetId}/values/Sheet%20with%20spaces!A1:B2:clear`, { spreadsheetId, range });
        });
        it('should encode range without sheet reference', async () => {
            const range = 'Sheet with spaces';
            await googleSheet.clearData(range);
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'POST', expect.stringContaining(encodeURIComponent("'Sheet with spaces'")), { spreadsheetId, range });
        });
    });
});
//# sourceMappingURL=GoogleSheet.test.js.map