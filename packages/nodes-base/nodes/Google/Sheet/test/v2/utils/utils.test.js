...
import { addRowNumber, autoMapInputData, checkForSchemaChanges, getColumnName, getColumnNumber, getExistingSheetNames, getRangeString, getSheetId, getSpreadsheetId, hexToRgb, mapFields, prepareSheetData, removeEmptyColumns, removeEmptyRows, trimLeadingEmptyRows, trimToFirstEmptyRow, } from '../../../v2/helpers/GoogleSheets.utils';
...
describe('Test Google Sheets, autoMapInputData', () => {
    ...
    it('should autoMapInputData', async () => {
        ...
        const result = await autoMapInputData.call(fakeExecuteFunction, 'foo 1', googleSheet, items, {});
...
describe('Test Google Sheets, checkForSchemaChanges', () => {
    ...
    it('should not throw when columns match exactly', () => {
        expect(() => checkForSchemaChanges(node, ['id', 'name', 'data'], [
            { id: 'id' },
            { id: 'name' },
            { id: 'data' },
        ])).not.toThrow();
    ...
    it('should not throw when columns are reordered', () => {
        expect(() => checkForSchemaChanges(node, ['data', 'id', 'name'], [
            { id: 'id' },
            { id: 'name' },
            { id: 'data' },
        ])).not.toThrow();
    ...
    it('should not throw when new columns are inserted', () => {
        expect(() => checkForSchemaChanges(node, ['id', 'owner_email', 'name', 'data'], [
            { id: 'id' },
            { id: 'name' },
            { id: 'data' },
        ])).not.toThrow();
    ...
    it('should throw and list only the missing columns', () => {
        try {
            ...
            expect(error.description).toBe("Refresh the columns list in the 'Column to Match On' parameter. Missing columns: data");
        }
    });
});
...
describe('Test Google Sheets, Google Sheets Sheet URL Regex', () => {
    ...
    it('should not match a URL that does not match the pattern', () => {
        const url = 'https://example.com/spreadsheets/d/1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms/edit#gid=0';
        expect(regex.test(url)).toBe(false);
    });
...
describe('Test Google Sheets, getExistingSheetNames', () => {
    ...
    it('should return an array of sheet names', async () => {
        ...
        const result = await getExistingSheetNames(mockGoogleSheetInstance);
...
describe('Test Google Sheets, mapFields', () => {
    ...
    it('should map fields for node version < 4', () => {
        ...
        const result = mapFields.call(fakeExecuteFunction, 2);
        expect(result).toEqual([
            { field1: 'value0', field2: 'value0' },
            { field1: 'value1', field2: 'value2' },
        ]);
        expect(fakeExecuteFunction.getNodeParameter).toHaveBeenCalledTimes(2);
        expect(fakeExecuteFunction.getNodeParameter).toHaveBeenCalledWith('fieldsUi.fieldValues', 0, []);
    });
    ...
    it('should throw an error if no values are added in version >= 4', () => {
        ...
        expect(() => mapFields.call(fakeExecuteFunction, 1)).toThrow("At least one value has to be added under 'Values to Send'");
    });
    it('should return an empty array when inputSize is 0', () => {
        const result = mapFields.call(fakeExecuteFunction, 0);
        expect(result).toEqual([]);
    });
});
