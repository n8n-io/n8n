...
vi.mock('../../../../v2/helpers/GoogleSheets.utils', async () => ({
    ...(await vi.importActual('../../../../v2/helpers/GoogleSheets.utils')),
...
describe('Google Sheets Append Operation', () => {
    ...
    beforeEach(() => {
        ...
        GoogleSheetsUtils.autoMapInputData.mockResolvedValue([
            { name: 'John', email: 'john@example.com' },
            { name: 'Jane', email: 'jane@example.com' },
        ]);
        GoogleSheetsUtils.mapFields.mockReturnValue([
            { name: 'John', email: 'john@example.com' },
            { name: 'Jane', email: 'jane@example.com' },
        ]);
        GoogleSheetsUtils.cellFormatDefault.mockReturnValue('USER_ENTERED');
    });
    afterEach(() => {
        vi.resetAllMocks();
    });
    describe('Basic Execution', () => {
        it('should execute successfully with valid parameters', async () => {
            ...
            const result = await execute.call(mockExecuteFunctions, mockSheet, 'Sheet1!A1:B2', 'sheet123');
    ...
    describe('Data Mode Handling', () => {
        it('should use autoMapInputData mode when sheet is empty', async () => {
            ...
            expect(GoogleSheetsUtils.autoMapInputData).toHaveBeenCalledWith('Sheet1!A1:B2', mockSheet, [
    ...
    describe('Node Version Handling', () => {
        ...
        it('should use columns.mappingMode parameter for node version >= 4', async () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'columns.mappingMode')
                    return 'autoMapInputData';
                return {};
            });
            mockSheet.getData.mockResolvedValue([
    ...
    describe('Options and Configuration', () => {
        ...
        it('should handle cell format option', async () => {
            ...
            mockSheet.appendSheetData.mockResolvedValue([]);
            await execute.call(mockExecuteFunctions, mockSheet, 'Sheet1!A1:B2', 'sheet123');
            expect(mockSheet.appendSheetData).toHaveBeenCalledWith(expect.objectContaining({
                valueInputMode: 'RAW',
            }));
        });
    });
    describe('Error Handling', () => {
        it('should throw error when column names cannot be retrieved (node version >= 4.4)', async () => {
            ...
            GoogleSheetsUtils.checkForSchemaChanges.mockImplementation(() => {
                throw new NodeOperationError(mockNode, 'Column names were updated');
            });
            await expect(execute.call(mockExecuteFunctions, mockSheet, 'Sheet1!A1:B2', 'sheet123')).rejects.toThrow(NodeOperationError);
        });
        it('should throw error when header row is out of bounds', async () => {
            ...
            await expect(execute.call(mockExecuteFunctions, mockSheet, 'Sheet1!A1:B2', 'sheet123')).rejects.toThrow('Could not retrieve the column names from row 5');
        });
    ...
    describe('Integration with GoogleSheets Utils', () => {
        ...
        it('should pass pre-fetched header row to autoMapInputData when sheet has data', async () => {
            ...
            mockSheet.appendSheetData.mockResolvedValue([]);
            await execute.call(mockExecuteFunctions, mockSheet, 'Sheet1!A1:B2', 'sheet123');
            expect(GoogleSheetsUtils.autoMapInputData).toHaveBeenCalledWith('Sheet1!A1:B2', mockSheet, expect.any(Array), {}, ['Name', 'Email']);
        });
        it('should not pass header row to autoMapInputData when sheet is empty', async () => {
            ...
            expect(GoogleSheetsUtils.autoMapInputData).toHaveBeenCalledWith('Sheet1!A1:B2', mockSheet, expect.any(Array), {});
        });
        ...
        it('should call checkForSchemaChanges for node version >= 4.4', async () => {
            ...
            mockSheet.appendSheetData.mockResolvedValue([]);
            await execute.call(mockExecuteFunctions, mockSheet, 'Sheet1!A1:B2', 'sheet123');
            expect(GoogleSheetsUtils.checkForSchemaChanges).toHaveBeenCalledWith(mockNode, ['Name', 'Email'], [{ id: 'name' }, { id: 'email' }]);
        });
    });
    describe('Edge Cases', () => {
        ...
        it('should handle undefined options gracefully', async () => {
            ...
            mockSheet.appendSheetData.mockResolvedValue([]);
            await expect(execute.call(mockExecuteFunctions, mockSheet, 'Sheet1!A1:B2', 'sheet123')).resolves.not.toThrow();
        });
    });
});
//# sourceMappingURL=append.operation.test.js.map