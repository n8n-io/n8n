    ...
    describe('execute', () => {
        it('should create a basic spreadsheet with title only', async () => {
            ...
            mockExecuteFunctions.getNodeParameter = vi
                ...
                .mockReturnValueOnce({})
                .mockReturnValueOnce({});
            apiRequest.call.mockResolvedValueOnce(mockResponse);
            const result = await execute.call(mockExecuteFunctions);
            expect(apiRequest.call).toHaveBeenCalledWith(mockExecuteFunctions, 'POST', '/v4/spreadsheets', {
                properties: {
                    title: mockTitle,
        ...
        it('should handle empty sheet properties', async () => {
            ...
            apiRequest.call.mockResolvedValueOnce(mockResponse);
            await execute.call(mockExecuteFunctions);
            expect(apiRequest.call).toHaveBeenCalledWith(expect.anything(), 'POST', '/v4/spreadsheets', expect.objectContaining({
                sheets: [],
            }));
        });
