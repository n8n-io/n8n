...
import { arrayOfArraysToJson, BINARY_MIME_TYPE, compareRevisions, getRevisionFile, sheetBinaryToArrayOfArrays, } from './GoogleSheetsTrigger.utils';
        ...
        try {
            ...
            if (event !== 'rowAdded') {
                ...
                do {
                    ...
                    const { revisions, nextPageToken } = await apiRequest.call(this, 'GET', '', undefined, {
            ...
            if (options.dataLocationOnSheet) {
                ...
                if (rangeDefinition === 'specifyRangeA1') {
                    if (locationDefine.range === '') {
                        throw new NodeOperationError(this.getNode(), "The field 'Range' is empty, please provide a range");
                    }
            ...
            if (event === 'rowAdded') {
                const [columns] = (await apiRequest.call(this, 'GET', `/v4/spreadsheets/${documentId}/values/${encodeURIComponent(sheetName)}!${keyRange}`)).values || [[]];
                if (!columns?.length) {
            ...
            if (event === 'anyUpdate' || event === 'rowUpdate') {
                if (sheetName.length > 31) {
                    throw new NodeOperationError(this.getNode(), 'Sheet name is too long choose a name with 31 characters or less');
                }
                ...
                const previousRevisionSheetData = sheetBinaryToArrayOfArrays(previousRevisionBinaryData, sheetName, rangeDefinition === 'specifyRangeA1' ? range : undefined) || [];
                const includeInOutput = this.getNodeParameter('includeInOutput', 'new');
                let returnData;
                if (options.columnsToWatch) {
                    returnData = compareRevisions(previousRevisionSheetData, currentData, keyRow, includeInOutput, options.columnsToWatch, dataStartIndex, event);
                }
                else {
                    returnData = compareRevisions(previousRevisionSheetData, currentData, keyRow, includeInOutput, [], dataStartIndex, event);
                }
                if (Array.isArray(returnData) && returnData.length !== 0) {
                    return [this.helpers.returnJsonArray(returnData)];
                }
            ...
            if (error?.description
                ...
                throw new NodeOperationError(this.getNode(), "Edit access to the document is required for the 'Row Update' and 'Row Added or Updated' triggers. Request edit access to the document's owner or select the 'Row Added' trigger in the 'Trigger On' dropdown.");
            }
