...
describe('Google GSuiteAdmin Node', () => {
    ...
    it('should make a successful API request with default options', async () => {
        mockContext.helpers.httpRequestWithAuthentication.mockResolvedValueOnce({
            success: true,
        });
        const result = await googleApiRequest.call(mockContext, 'GET', '/example/resource');
        expect(mockContext.helpers.httpRequestWithAuthentication).toHaveBeenCalledWith('gSuiteAdminOAuth2Api', expect.objectContaining({
            method: 'GET',
    ...
    it('should omit the body if it is empty', async () => {
        ...
        expect(mockContext.helpers.httpRequestWithAuthentication).toHaveBeenCalledWith('gSuiteAdminOAuth2Api', expect.not.objectContaining({ body: expect.anything() }));
    });
    it('should throw a NodeApiError if the request fails', async () => {
        const errorResponse = { message: 'API Error' };
        mockContext.helpers.httpRequestWithAuthentication.mockRejectedValueOnce(errorResponse);
        await expect(googleApiRequest.call(mockContext, 'GET', '/example/resource')).rejects.toThrow(NodeApiError);
    ...
    it('should return all items across multiple pages', async () => {
        ...
        const result = await googleApiRequestAllItems.call(mockContext, 'items', 'GET', '/example/resource');
        ...
        expect(mockContext.helpers.httpRequestWithAuthentication).toHaveBeenNthCalledWith(1, 'gSuiteAdminOAuth2Api', expect.objectContaining({
            method: 'GET',
        ...
        expect(mockContext.helpers.httpRequestWithAuthentication).toHaveBeenNthCalledWith(2, 'gSuiteAdminOAuth2Api', expect.objectContaining({
            method: 'GET',
            qs: { maxResults: 100, pageToken: '' },
            headers: { 'Content-Type': 'application/json' },
            url: 'https://www.googleapis.com/admin/example/resource',
        ...
        expect(mockContext.helpers.httpRequestWithAuthentication).toHaveBeenNthCalledWith(3, 'gSuiteAdminOAuth2Api', expect.objectContaining({
            method: 'GET',
            qs: { maxResults: 100, pageToken: '' },
            headers: { 'Content-Type': 'application/json' },
            url: 'https://www.googleapis.com/admin/example/resource',
    ...
    it('should throw a NodeApiError if a request fails', async () => {
        const errorResponse = { message: 'API Error' };
        mockContext.helpers.httpRequestWithAuthentication.mockRejectedValueOnce(errorResponse);
        await expect(googleApiRequestAllItems.call(mockContext, 'items', 'GET', '/example/resource')).rejects.toThrow();
        expect(mockContext.getNode).toHaveBeenCalled();
        expect(mockContext.helpers.httpRequestWithAuthentication).toHaveBeenCalledTimes(1);
    });
});
