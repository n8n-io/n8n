...
describe('searchFunctions', () => {
    ...
    describe('searchUsers', () => {
        it('should return formatted user search results', async () => {
            googleApiRequestAllItems.mockResolvedValueOnce([
                { id: '123', name: { fullName: 'John Doe' } },
                { id: '456' },
            ]);
            const result = await searchUsers.call(mockContext);
            expect(googleApiRequestAllItems).toHaveBeenCalledWith(expect.anything(), 'GET', '/directory/v1/users', {}, { customer: 'my_customer' });
    ...
    describe('searchDevices', () => {
        it('should return formatted device search results', async () => {
            googleApiRequest.mockResolvedValueOnce({
                chromeosdevices: [{ deviceId: 'dev1', serialNumber: 'SN123' }, { deviceId: 'Dev2' }],
            });
            const result = await searchDevices.call(mockContext);
            expect(googleApiRequest).toHaveBeenCalledWith('GET', '/directory/v1/customer/my_customer/devices/chromeos/', {}, { customerId: 'my_customer' });
            expect(result).toEqual({
                results: [
        ...
        it('should return empty results if no devices found', async () => {
            googleApiRequest.mockResolvedValueOnce({ chromeosdevices: [] });
            const result = await searchDevices.call(mockContext);
            expect(result).toEqual({ results: [] });
        });
    });
