...
describe('GSuiteAdmin Node - loadOptions', () => {
    ...
    describe('getOrgUnits', () => {
        ...
        it('should return only the root unit when the response has no organizational units', async () => {
            googleApiRequest.mockResolvedValue({ kind: 'admin#directory#orgUnits' });
            const result = await node.methods.loadOptions.getOrgUnits.call(mockThis);
...
describe('GSuiteAdmin Node - logic coverage', () => {
    it('should apply all filters correctly into qs', () => {
        ...
        if (filter.query) {
            ...
            if (!regex.test(query)) {
                throw new NodeOperationError(mockThis.getNode(), 'Invalid query format. Query must follow the format "displayName:<value>" or "email:<value>".');
            }
            qs.query = query;
        }
        if (filter.userId)
    ...
    it('should throw an error for invalid query format', () => {
        ...
        }).toThrow('Invalid query format. Query must follow the format "displayName:<value>" or "email:<value>".');
    });
    ...
    it('should set phones, emails, roles, and custom fields', () => {
        ...
        if (additionalFields.customFields) {
            ...
            for (const field of additionalFields.customFields.fieldValues) {
                ...
                if (!customSchemas[field.schemaName])
                    customSchemas[field.schemaName] = {};
...
describe('GSuiteAdmin Node - user:create logic', () => {
    it('should include changePasswordAtNextLogin when set to true in create operation', async () => {
        ...
        googleApiRequest.mockImplementation(mockCall);
        const mockContext = {
            getNode: () => ({ name: 'GSuiteAdmin' }),
            getNodeParameter: vi.fn((paramName, _index) => {
                switch (paramName) {
                    case 'resource':
                        return 'user';
                    case 'operation':
        ...
        expect(mockCall).toHaveBeenCalledWith('POST', '/directory/v1/users', expect.objectContaining({
            changePasswordAtNextLogin: true,
    ...
    it('should not include changePasswordAtNextLogin when undefined in create operation', async () => {
        ...
        await new GSuiteAdmin().execute.call(mockContext);
        expect(mockCall).toHaveBeenCalledWith('POST', '/directory/v1/users', {
...
describe('GSuiteAdmin Node - user:update logic', () => {
    ...
    it('should include suspended set to false in update operation', async () => {
        ...
        await new GSuiteAdmin().execute.call(mockContext);
        expect(mockCall).toHaveBeenCalledWith('PUT', '/directory/v1/users/user-id-123', {
            suspended: false,
        }, {});
    });
    it('should include password and changePasswordAtNextLogin in update operation', async () => {
        ...
        await new GSuiteAdmin().execute.call(mockContext);
        expect(mockCall).toHaveBeenCalledWith('PUT', '/directory/v1/users/user-id-456', expect.objectContaining({
            password: 'NewSecurePassword123!',
            changePasswordAtNextLogin: true,
        }), {});
    });
    it('should include changePasswordAtNextLogin set to false in update operation', async () => {
        ...
        await new GSuiteAdmin().execute.call(mockContext);
        expect(mockCall).toHaveBeenCalledWith('PUT', '/directory/v1/users/user-id-999', expect.objectContaining({
            password: 'TestPassword!',
            changePasswordAtNextLogin: false,
        }), {});
    });
    it('should throw error for invalid custom fields', async () => {
        ...
        await expect(new GSuiteAdmin().execute.call(mockContextInvalidFields)).rejects.toThrow('Invalid custom field data');
    ...
    it('should handle error when continueOnFail is true and constructExecutionMetaData is called', async () => {
        ...
        await expect(async () => {
            ...
            if (mockContext.continueOnFail()) {
                const executionErrorData = mockContext.helpers.constructExecutionMetaData(mockContext.helpers.returnJsonArray({
            ...
            throw new NodeOperationError(mockContext.getNode(), 'Operation "update" failed for resource "user".', {
...
//# sourceMappingURL=GSuiteAdmin.node.test.js.map