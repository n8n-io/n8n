...
import { NodeConnectionTypes, deepCopy, safeInternalRegex } from 'n8n-workflow';
import type {
	IExecuteFunctions,
	IDataObject,
	INodeExecutionData,
			...
			const renameObjectKeys = (obj: IDataObject, objDepth: number) => {
				for (const key in obj) {
					if (Array.isArray(obj)) {
						...
						if (safeInternalRegex.test(searchRegex as string, key, flags)) {
							const newKey = safeInternalRegex.replace(
