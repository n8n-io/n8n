...
describe('Test IF v2 Node Tests', () => {
    ...
    describe('Test IF V2 Node Unit Tests', () => {
        const node = new IfV2.IfV2(mock());
        const input = [{ json: {} }];
        const createMockExecuteFunction = (nodeParameters, continueOnFail = false) => {
            const fakeExecuteFunction = {
                getNodeParameter(parameterName, itemIndex, fallbackValue, options) {
                    ...
                    if (parameterValue?.nodeOperationError) {
        ...
        it('should return items if continue on fail is true', async () => {
            const fakeExecuteFunction = createMockExecuteFunction({ options: { nodeOperationError: true } }, true);
        ...
        it('should throw an error if continue on fail is false and if there is an error', async () => {
            const fakeExecuteFunction = createMockExecuteFunction({ options: { nodeOperationError: true } }, false);
        ...
        it('should assign a paired item if paired item is undefined', async () => {
            const fakeExecuteFunction = createMockExecuteFunction({ options: {}, conditions: true }, false);
            const output = await node.execute.call(fakeExecuteFunction);
            expect(output).toEqual([[], [{ json: {}, pairedItem: { item: 0 } }]]);
        });
    });
});
//# sourceMappingURL=IfV2.node.test.js.map