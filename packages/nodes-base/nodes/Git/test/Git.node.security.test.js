...
const gitConfig = (repo, key, value) => runGit(['-C', repo, 'config', key, value]);
// git can still be writing into `.git` when a test ends, which makes a single-pass
// recursive remove fail with ENOTEMPTY.
const removeFixture = async (dir) => await rm(dir, { recursive: true, force: true, maxRetries: 5 });
const configValue = (repo, key) => {
    try {
...
const branchesOf = (repo) => runGit(['-C', repo, 'for-each-ref', 'refs/heads']).toString();
const commitSubject = (repo, ref) => runGit(['-C', repo, 'log', '--format=%s', '-1', ref]).toString().trim();
const errorOf = (result) => result[0][0].json.error;
 ...
 */
const buildAllowedRootContext = (operation, repositoryPath, parameters = {}, allowedRoot) => {
    const ctx = mock({
        ...
        helpers: {
            returnJsonArray: vi.fn((data) => data.map((item) => ({ json: item }))),
...
describe('Git Node command-config handling', () => {
    ...
    let marker;
    let additionalDirs;
    const buildContext = (operation, repositoryPath, parameters = {}, isFilePathBlocked = () => false) => {
        const ctx = mock({
            getInputData: vi.fn(() => [{ json: {} }]),
            ...
            helpers: {
                returnJsonArray: vi.fn((data) => data.map((item) => ({ json: item }))),
                resolvePath: vi.fn(async (path) => (await realpath(path))),
    ...
    beforeEach(async () => {
        gitNode = new Git();
        Container.set(DeploymentConfig, mock({ type: 'default' }));
        Container.set(SecurityConfig, mock({
    ...
    const git = (...args) => runGit(['-C', repoDir, ...args]);
    const markerCommand = () => `node -e "require('node:fs').writeFileSync(process.argv[1], '')" ${JSON.stringify(marker)}`;
    ...
    it('still runs ssh for ssh remotes (pinning the default does not break it)', async () => {
        ...
        runGit(['-C', repoDir, 'remote', 'add', 'origin', 'ssh://git@127.0.0.1:22/x.git']);
        const result = await gitNode.execute.call(buildContext('fetch', repoDir));
        const error = result[0][0].json.error ?? '';
    ...
    it('rejects a repo-local clean filter before add', async () => {
        ...
        const result = await gitNode.execute.call(buildContext('add', repoDir, { pathsToAdd: 'payload.txt' }));
        expect(existsSync(marker)).toBe(false);
        expect(result[0][0].json.error).toBeDefined();
    });
    ...
    it('rejects a repo-local smudge filter before switching branch', async () => {
        ...
        const result = await gitNode.execute.call(buildContext('switchBranch', repoDir, {
            branchName: 'other',
    ...
    it('rejects a filesystem path where a remote name is expected', async () => {
        ...
        expect(result[0][0].json.error).toMatch(/Invalid remote name/);
    ...
    it('rejects a branch remote outside the allowed paths before fetching', async () => {
        ...
        const result = await gitNode.execute.call(buildContext('fetch', repoDir, {}, (path) => !path.startsWith(repoDir)));
        expect(result[0][0].json.error).toMatch(/source repository path is not allowed/);
...
const REPOSITORY_OUT_OF_BOUNDS = 'The git repository containing this path is outside the allowed file paths';
describe('Git Node repository reference validation', () => {
    ...
    const makeBareClone = (subject, target) => {
    ...
    const advanceOutsideTarget = (target) => {
    ...
    it('rejects a relative remote URL that resolves outside the allowed path', async () => {
        const result = await gitNode.execute.call(buildAllowedRootContext('addConfig', subDir, { key: 'remote.origin.url', value: OUTSIDE_RELATIVE_REFERENCE }, allowedRoot));
    ...
    it('rejects a relative remote URL given from the repository top level', async () => {
        const result = await gitNode.execute.call(buildAllowedRootContext('addConfig', repoDir, { key: 'remote.origin.url', value: OUTSIDE_RELATIVE_REFERENCE }, allowedRoot));
        expect(errorOf(result)).toContain(SOURCE_DENIED);
        expect(configValue(repoDir, 'remote.origin.url')).toBeUndefined();
    });
    it('accepts a relative remote URL that stays inside the allowed path', async () => {
        const result = await gitNode.execute.call(buildAllowedRootContext('addConfig', subDir, { key: 'remote.origin.url', value: '../shared-mirror' }, allowedRoot));
    ...
    it('does not pull from a configured remote outside the allowed path', async () => {
        ...
        const result = await gitNode.execute.call(buildAllowedRootContext('pull', subDir, {}, allowedRoot));
        expect(errorOf(result)).toContain(SOURCE_DENIED);
    ...
    it('does not push to a relative target repository outside the allowed path', async () => {
        ...
        const result = await gitNode.execute.call(buildAllowedRootContext('push', subDir, { options: { repository: true, targetRepository: OUTSIDE_RELATIVE_REFERENCE } }, allowedRoot));
    ...
    it('does not push to a configured push URL outside the allowed path', async () => {
        ...
        const branchesBefore = branchesOf(target);
        const result = await gitNode.execute.call(buildAllowedRootContext('push', subDir, {}, allowedRoot));
        expect(errorOf(result)).toContain(TARGET_DENIED);
        expect(branchesOf(target)).toBe(branchesBefore);
    });
    it('rejects an operation when the enclosing repository is outside the allowed path', async () => {
        initOuterRepository();
        const result = await gitNode.execute.call(buildAllowedRootContext('log', allowedRoot, {}, allowedRoot));
    ...
    it('does not list config from an enclosing repository outside the allowed path', async () => {
        ...
        const result = await gitNode.execute.call(buildAllowedRootContext('listConfig', allowedRoot, {}, allowedRoot));
        expect(errorOf(result)).toContain(REPOSITORY_OUT_OF_BOUNDS);
    ...
    it('follows symlinks when resolving a relative remote URL', async () => {
        ...
        const result = await gitNode.execute.call(buildAllowedRootContext('addConfig', repoDir, { key: 'remote.origin.url', value: 'link/../cand' }, allowedRoot));
        expect(errorOf(result)).toContain(SOURCE_DENIED);
        expect(configValue(repoDir, 'remote.origin.url')).toBeUndefined();
    });
    ...
    it('rejects an upstream remote name before creating the branch', async () => {
        const result = await gitNode.execute.call(buildAllowedRootContext('switchBranch', repoDir, {
            branchName: 'feature',
    ...
    it('accepts a plain upstream remote name', async () => {
        const result = await gitNode.execute.call(buildAllowedRootContext('switchBranch', repoDir, { branchName: 'feature', options: { setUpstream: true, remoteName: 'origin' } }, allowedRoot));
        expect(errorOf(result)).toBeUndefined();
    ...
    it('rejects a repository whose git directory is outside the allowed path', async () => {
        ...
        const result = await gitNode.execute.call(buildAllowedRootContext('log', redirectedSub, {}, allowedRoot));
        expect(errorOf(result)).toContain(REPOSITORY_OUT_OF_BOUNDS);
    ...
    it('keeps rejecting an out-of-bounds reference in a bare repository', async () => {
        ...
        const result = await gitNode.execute.call(buildAllowedRootContext('addConfig', bare, { key: 'remote.origin.url', value: OUTSIDE_RELATIVE_REFERENCE }, allowedRoot));
        expect(errorOf(result)).toContain(SOURCE_DENIED);
    ...
    it('resolves a relative remote URL from a subdirectory of a bare repository', async () => {
        ...
        const bare = join(allowedRoot, 'bare');
        runGit(['init', '-q', '--bare', '-b', 'main', bare]);
        const result = await gitNode.execute.call(buildAllowedRootContext('addConfig', join(bare, 'refs'), { key: 'remote.origin.url', value: OUTSIDE_RELATIVE_REFERENCE }, allowedRoot));
    ...
    it('rejects an operation whose work tree is outside the allowed path', async () => {
        ...
        const result = await gitNode.execute.call(buildAllowedRootContext('status', repoDir, {}, allowedRoot));
        expect(errorOf(result)).toContain(REPOSITORY_OUT_OF_BOUNDS);
    ...
    it('rejects a relative remote URL when the work tree is the filesystem root', async () => {
        ...
        const result = await gitNode.execute.call(buildAllowedRootContext('addConfig', repoDir, { key: 'remote.origin.url', value: '../shared-mirror' }, allowedRoot));
        expect(errorOf(result)).toContain(REPOSITORY_OUT_OF_BOUNDS);
        expect(configValue(repoDir, 'remote.origin.url')).toBeUndefined();
    });
    it('resolves a relative remote URL from a run directory that only shares a name prefix with the work tree', async () => {
        ...
        const result = await gitNode.execute.call(buildAllowedRootContext('addConfig', nearbySubDir, { key: 'remote.origin.url', value: '../../shared-mirror' }, allowedRoot));
        expect(errorOf(result)).toBeUndefined();
    ...
    it('rejects a relative remote URL when the git directory sits below the repository path', async () => {
        ...
        const result = await gitNode.execute.call(buildAllowedRootContext('addConfig', holder, { key: 'remote.origin.url', value: OUTSIDE_RELATIVE_REFERENCE }, allowedRoot));
        expect(errorOf(result)).toContain(SOURCE_DENIED);
    ...
    it('allows a linked worktree inside the allowed path', async () => {
        const worktree = join(allowedRoot, 'wt');
        runGit(['-C', repoDir, 'worktree', 'add', '-q', '-b', 'wtb', worktree]);
        const result = await gitNode.execute.call(buildAllowedRootContext('status', worktree, {}, allowedRoot));
        expect(errorOf(result)).toBeUndefined();
    ...
    it('allows a submodule work tree inside the allowed path', async () => {
        ...
        const result = await gitNode.execute.call(buildAllowedRootContext('status', join(repoDir, 'sub-mod'), {}, allowedRoot));
        expect(errorOf(result)).toBeUndefined();
    ...
    it('rejects a clone source the staging base puts outside the allowed path', async () => {
        ...
        initRepository(source);
        runGit(['-C', source, 'commit', '-q', '--allow-empty', '-m', 'src']);
        const result = await gitNode.execute.call(buildAllowedRootContext('clone', target, { sourceRepository: '../outside/src' }, allowedRoot));
        expect(errorOf(result)).toContain(SOURCE_DENIED);
    ...
    it('accepts a reference whose parent directories do not exist yet', async () => {
        const result = await gitNode.execute.call(buildAllowedRootContext('addConfig', subDir, { key: 'remote.origin.url', value: '../not-yet-created/mirror' }, allowedRoot));
        expect(errorOf(result)).toBeUndefined();
    ...
    it('accepts a reference that walks back out of a missing directory', async () => {
        const result = await gitNode.execute.call(buildAllowedRootContext('addConfig', repoDir, { key: 'remote.origin.url', value: 'nosuch/../shared-mirror' }, allowedRoot));
        expect(errorOf(result)).toBeUndefined();
    ...
    it('surfaces a friendly error when the repository top level cannot be resolved', async () => {
        ...
        ctx.continueOnFail = vi.fn(() => false);
        ctx.helpers.resolvePath = vi.fn(async (path) => {
            if (path === repoDir) {
...
//# sourceMappingURL=Git.node.security.test.js.map