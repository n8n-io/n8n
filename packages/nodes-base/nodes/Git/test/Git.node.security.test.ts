...
// jest runs this suite inside a vm realm, so an error raised by `node:fs` is not an
// `instanceof Error` here. Match on `code` alone, and hand the node under test an error
// built in this realm, which its own `instanceof Error` check does recognise.
...
const resolveRealPath = async (path: string): Promise<ResolvedFilePath> => {
	try {
		...
		try {
			...
			// The parent is missing too. Report it for the path asked about, so the node's
			// own missing-parent walk takes over from here.
...
const buildAllowedRootContext = (
	...
	const ctx = mock<IExecuteFunctions>({
		...
		helpers: {
			...
			resolvePath: jest.fn(resolveRealPath),
			...
			resolveStagingBaseForTarget: jest.fn(async () => allowedRoot as ResolvedFilePath),
...
describe('Git Node command-config handling', () => {
	...
	const buildContext = (
		...
		const ctx = mock<IExecuteFunctions>({
			...
			helpers: {
				returnJsonArray: jest.fn((data: any[]) => data.map((item: any) => ({ json: item }))),
				resolvePath: jest.fn(async (path: string) => (await realpath(path)) as ResolvedFilePath),
				isFilePathBlocked: jest.fn(isFilePathBlocked),
			},
		});
		ctx.getNodeParameter.mockImplementation(
			(
...
describe('Git Node repository reference validation', () => {
	...
	it('surfaces a friendly error when the repository top level cannot be resolved', async () => {
		...
		ctx.continueOnFail.mockReturnValue(false);
		ctx.helpers.resolvePath = jest.fn(async (path: PathLike) => {
			if (path === repoDir) {
