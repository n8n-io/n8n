		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			try {
				...
				if (operation === 'add') {
					...
					returnItems.push.apply(returnItems, this.helpers.returnJsonArray(log.all).map((item) => {
							return {
								...item,
					...
					returnItems.push.apply(returnItems, this.helpers.returnJsonArray(data).map((item) => {
							return {
								...item,
								pairedItem: { item: itemIndex },
							};
					...
					returnItems.push.apply(returnItems, this.helpers.returnJsonArray([status]).map((item) => {
							return {
								...item,
								pairedItem: { item: itemIndex },
							};
