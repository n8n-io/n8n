...
export function findBlacklistedKeys(config, localConfigFiles) {
 ...
 */
function assertSafeGitReference(reference, node, safeReferencePattern, allowedCharsMessage) {
    if (!safeReferencePattern.test(reference)) {
        throw new NodeOperationError(node, `Invalid reference format. Reference contains unsafe characters. Only alphanumeric characters and ${allowedCharsMessage} are allowed`);
    }
    // Prevent argument injection by blocking references starting with -
    if (reference.startsWith('-')) {
        throw new NodeOperationError(node, 'Invalid reference format. Reference cannot start with a hyphen');
    }
    ...
    if (/[\x00-\x1f\x7f]/.test(reference)) {
        throw new NodeOperationError(node, 'Invalid reference format. Reference cannot contain control characters');
    }
}
/**
 ...
 */
export function validateGitReference(reference, node) {
 ...
 */
export function validateGitTag(name, node) {
...
const isValidRemoteName = (name) => name === '.' || (name.length > 0 && !UNSAFE_REMOTE_NAME_PATTERN.test(name));
export function validateGitRemoteName(name, node) {
    if (!isValidRemoteName(name)) {
        throw new NodeOperationError(node, `Invalid remote name '${name}'. This field takes the name of a remote, not a URL or a path`);
    }
}
/**
 ...
 */
function parseRevParseOutput(output, expectedLineCount) {
 ...
 */
function assertAbsolutePaths(paths, cause) {
...
export async function getGitRepositoryLayout(git) {
    let workTreeError;
    ...
    const fallbackOutput = await git
        ...
        .catch((error) => {
        throw new OperationalError(error instanceof Error ? error.message : 'Could not read the git repository layout', { cause: workTreeError });
    });
 ...
 */
export function ownerOfGitDir(gitDirPath) {
    ...
    return (owner.length > root.length ? owner : root);
}
...
export function getRepositoryTypeForRemoteConfigKey(key) {
...
function addRemoteValue(remoteValuesByName, remoteName, value) {
    const existingValues = remoteValuesByName.get(remoteName) ?? [];
    existingValues.push(value);
    remoteValuesByName.set(remoteName, existingValues);
}
export function getConfiguredRemoteRepositories(configValues, node) {
    const sourceValidationTargets = [];
    const targetValidationTargets = [];
    const remoteOriginUrls = [];
    const remoteOriginPushUrls = [];
    const remoteUrlsByName = new Map();
    const remotePushUrlsByName = new Map();
    const configuredRemoteNames = new Set();
    const sourceRemoteReferences = [];
    const targetRemoteReferences = [];
    ...
    const unresolved = (references) => references.filter((reference) => !configuredRemoteNames.has(reference));
    ...
    targetValidationTargets.push(...unresolvedSourceReferences, ...unresolved(targetRemoteReferences));
    return {
...
export function mapGitConfigList(config) {
    const data = [];
