...
import { getConfiguredRemoteRepositories, getGitRepositoryLayout, findBlacklistedKeys, getRepositoryTypeForRemoteConfigKey, isWithinPath, mapGitConfigList, ownerOfGitDir, validateGitRemoteName, } from '../GenericFunctions';
...
describe('GenericFunctions', () => {
    ...
    describe('validateGitRemoteName', () => {
        it.each(['origin', 'my-remote', 'remote_1', 'a.b', '_private', 'gh/upstream', 'ünïcode', '.'])('should accept %s', (name) => {
            expect(() => validateGitRemoteName(name, mockDeep())).not.toThrow();
        });
        it.each([
            '/outside/repo',
            '../outside',
            ...
            expect(() => validateGitRemoteName(name, mockDeep())).toThrow('Invalid remote name');
        });
    });
    ...
    describe('getConfiguredRemoteRepositories', () => {
        it('should return every configured remote URL as a validation target', () => {
            ...
            const result = getConfiguredRemoteRepositories(config.values, mockDeep());
            expect(result).toEqual({
    ...
    describe('getGitRepositoryLayout', () => {
        ...
        it('rejects output with an unexpected number of lines', async () => {
            ...
            await expect(getGitRepositoryLayout(git)).rejects.toThrow('Could not read the git repository layout');
        });
    ...
    describe('ownerOfGitDir', () => {
        it.each([
            ...
            expect(ownerOfGitDir(gitDir)).toBe(expected);
        });
    });
});
