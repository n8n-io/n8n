...
import { NodeConnectionTypes, NodeOperationError, UnexpectedError, assertParamIsBoolean, assertParamIsString, } from 'n8n-workflow';
import { randomBytes } from 'node:crypto';
import { mkdir, rename, rm } from 'node:fs/promises';
import { basename, dirname, isAbsolute, join, resolve, sep } from 'path';
import simpleGit from 'simple-git';
import { URL, fileURLToPath } from 'url';
import { addConfigFields, addFields, ALLOWED_CONFIG_KEYS, cloneFields, commitFields, logFields, pushFields, reflogFields, switchBranchFields, tagFields, } from './descriptions';
import { getConfiguredRemoteRepositories, getGitRepositoryLayout, findBlacklistedKeys, getRepositoryTypeForRemoteConfigKey, isWithinPath, mapGitConfigList, ownerOfGitDir, validateGitReference, validateGitRemoteName, validateGitTag, } from './GenericFunctions';
...
const REPOSITORY_OUT_OF_BOUNDS_MESSAGE = 'The git repository containing this path is outside the allowed file paths';
const REPOSITORY_OUT_OF_BOUNDS_DESCRIPTION = "The working tree or the git directory of this repository is outside the allowed file paths, matched by the blocked file patterns, or inside n8n's own directories";
const WORKTREE_GIT_DIR_OUT_OF_BOUNDS_DESCRIPTION = 'The git directory of this worktree is not inside the repository it belongs to';
...
export class Git {
    ...
    async execute() {
        ...
        let gitBaseDir;
        const requireGitBaseDir = () => {
            if (gitBaseDir === undefined) {
                throw new UnexpectedError('Git base directory was not resolved before validating a repository reference');
            }
            return gitBaseDir;
        };
        ...
        const hasUrlScheme = (repositoryPath) => /^[a-zA-Z][a-zA-Z0-9+.-]*:/.test(repositoryPath);
        const isSshRepositoryPath = (repositoryPath) => {
        ...
        const assertLocalRepositoryPathAllowed = async (repositoryPath, repositoryType) => {
            ...
            const resolvedLocalRepositoryPath = await resolvePathAllowingMissingParents(absoluteLocalRepositoryPath);
            if (this.helpers.isFilePathBlocked(resolvedLocalRepositoryPath)) {
                throw new NodeOperationError(this.getNode(), `Access to the ${repositoryType} repository path is not allowed`);
            }
        };
        const assertRepositoryReferenceAllowed = async (repository, repositoryType) => {
            const trimmedRepository = repository.trim();
            if (trimmedRepository.startsWith('-')) {
                throw new NodeOperationError(this.getNode(), `${repositoryType === 'source' ? 'Source' : 'Target'} repository cannot start with a hyphen`);
            }
            ...
            if (REMOTE_HELPER_TRANSPORT.test(trimmedRepository)) {
                throw new NodeOperationError(this.getNode(), `${repositoryType === 'source' ? 'Source' : 'Target'} repository protocol is not allowed`);
            }
        ...
        const validateConfiguredRemoteRepositories = async (repositoryType, config) => {
            const remoteRepositories = getConfiguredRemoteRepositories(config.values, this.getNode());
            const validationTargets = repositoryType === 'source'
        ...
        const resolvePathAllowingMissingParents = async (path) => {
            try {
                ...
                return join(resolvedParentPath, basename(path));
            }
        };
        /**
         ...
         */
        const resolveGitBaseDir = async (git, cwd) => {
            let layout;
            let topLevel;
            try {
            ...
            const outOfBounds = (description = REPOSITORY_OUT_OF_BOUNDS_DESCRIPTION) => new NodeOperationError(this.getNode(), REPOSITORY_OUT_OF_BOUNDS_MESSAGE, { description });
        ...
        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
            try {
                ...
                const resolvedRepositoryPath = operation === 'clone'
                ...
                if (operation === 'clone') {
                    ...
                    sourceRepository = this.getNodeParameter('sourceRepository', itemIndex, '');
                    await assertRepositoryReferenceAllowed(sourceRepository, 'source');
                    sourceRepository = await prepareRepository(sourceRepository);
                    cloneStagingPath = join(cloneStagingBase, `.n8n-clone-${randomBytes(12).toString('hex')}`);
                ...
                const cleanEnv = Object.create(null);
                ...
                const git = simpleGit(gitOptions).env(cleanEnv);
                if (operation !== 'clone') {
                ...
                if (operation === 'push' && options.repository) {
                    customTargetRepository = options.targetRepository;
                    await assertRepositoryReferenceAllowed(customTargetRepository, 'target');
                }
                const validateGitConfig = async () => {
                    ...
                    if (blacklistedConfigKeys.length > 0) {
                        throw new NodeOperationError(this.getNode(), `Repository Git config key '${blacklistedConfigKeys[0]}' is not allowed`);
                    }
                    return repositoryConfig;
                };
                if (operation === 'add') {
                    ...
                    if (!enableGitNodeAllConfigKeys && !ALLOWED_CONFIG_KEYS.includes(key)) {
                        throw new NodeOperationError(this.getNode(), `The provided git config key '${key}' is not allowed`);
                    }
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (!pinnedParent) {
                                await this.helpers.ensureParentDirectoryWithoutFollowingSymlinks(resolvedRepositoryPath);
                            ...
                            if (error instanceof Error && 'code' in error && error.code === 'EXDEV') {
                                throw new NodeOperationError(this.getNode(), 'Cannot clone to a path on a different filesystem than the n8n data directory');
                            }
                            throw error;
                        }
                        finally {
                    ...
                    if (options.repository) {
                        ...
                        const authentication = this.getNodeParameter('authentication', 0);
                        const { pushTarget } = await validateConfiguredRemoteRepositories('target', repositoryConfig);
                        if (authentication === 'gitPassword') {
