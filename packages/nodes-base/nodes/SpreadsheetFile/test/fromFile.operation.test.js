...
describe('fromFile.operation - xlsx parsing logic', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        mockExecuteFunctions.getNodeParameter.mockImplementation((paramName, _itemIndex, defaultValue) => {
            switch (paramName) {
                case 'fileFormat':
                    return 'xlsx';
        ...
        xlsxRead.mockReturnValue(mockWorkbook);
        xlsxUtils.sheet_to_json.mockReturnValue(mockParsedData);
    });
    describe('Basic xlsx parsing', () => {
        it('should parse xlsx file from in-memory binary data', async () => {
            ...
            expect(xlsxRead).toHaveBeenCalledWith(Buffer.from(mockBinaryDataInMemory.data, BINARY_ENCODING), { raw: undefined });
        ...
        it('should parse xlsx file from filesystem binary data', async () => {
            ...
            const result = await execute.call(mockExecuteFunctions, items);
            expect(result).toHaveLength(2);
            expect(mockExecuteFunctions.helpers.getBinaryStream).toHaveBeenCalledWith('binary-data-id-123', 262144);
    ...
    describe('Options handling', () => {
        it('should respect rawData option', async () => {
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'options')
                    return { rawData: true };
                if (paramName === 'fileFormat')
                    return 'xlsx';
                if (paramName === 'binaryPropertyName')
                    return 'data';
                return undefined;
            });
            const items = [{ json: {} }];
        ...
        it('should handle headerRow=false option', async () => {
            ...
            xlsxUtils.sheet_to_json.mockReturnValue(mockArrayData);
            const items = [{ json: {} }];
            const result = await execute.call(mockExecuteFunctions, items);
            expect(xlsxUtils.sheet_to_json).toHaveBeenCalledWith(mockWorkbook.Sheets.Sheet1, {
                header: 1,
    ...
    describe('Error handling', () => {
        it('should throw error when workbook has no sheets', async () => {
            ...
            xlsxRead.mockReturnValue(emptyWorkbook);
            const items = [{ json: {} }];
        ...
        it('should throw error when specified sheet does not exist', async () => {
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'options')
                    return { sheetName: 'NonExistentSheet' };
                if (paramName === 'fileFormat')
                    return 'xlsx';
                if (paramName === 'binaryPropertyName')
                    return 'data';
    ...
    describe('Additional edge cases', () => {
        ...
        it('should handle custom binary property name correctly', async () => {
            ...
            const items = [{ json: {} }];
            await execute.call(mockExecuteFunctions, items);
            expect(mockExecuteFunctions.helpers.assertBinaryData).toHaveBeenCalledWith(0, 'customBinaryField');
        });
    ...
    describe('Binary string conversion', () => {
        it('should convert buffer to binary string when readAsString is true', async () => {
            ...
            await execute.call(mockExecuteFunctions, items);
            expect(xlsxRead).toHaveBeenCalledWith(expect.any(String), { raw: undefined, type: 'binary' });
            const callArgs = xlsxRead.mock.calls[0];
            const passedData = callArgs[0];
            const expectedBinaryString = Buffer.from(mockBinaryDataInMemory.data, BINARY_ENCODING).toString('binary');
            expect(passedData).toBe(expectedBinaryString);
        });
        it('should use buffer directly when readAsString is false', async () => {
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'options')
                    return { readAsString: false };
                if (paramName === 'fileFormat')
                    return 'xlsx';
                if (paramName === 'binaryPropertyName')
                    return 'data';
    ...
    describe('Special character handling', () => {
        it('should handle special characters correctly when readAsString is true', async () => {
            ...
            xlsxRead.mockReturnValue(mockWorkbookWithSpecialChars);
            xlsxUtils.sheet_to_json.mockReturnValue(mockSpecialCharData);
            const items = [{ json: {} }];
            const result = await execute.call(mockExecuteFunctions, items);
        ...
        it('should demonstrate the difference between readAsString true vs false for character encoding', async () => {
            ...
            xlsxRead.mockReturnValue(mockWorkbookEncoding);
            xlsxUtils.sheet_to_json.mockReturnValue([
        ...
        it('should handle various international characters when readAsString is enabled', async () => {
            ...
            xlsxRead.mockReturnValue(mockInternationalWorkbook);
            xlsxUtils.sheet_to_json.mockReturnValue(mockInternationalData);
            const items = [{ json: {} }];
            const result = await execute.call(mockExecuteFunctions, items);
    ...
    describe('CSV parsing with skipRecordsWithErrors', () => {
        ...
        it('should throw error when skipped records exceed maxSkippedRecords limit', async () => {
            ...
            const csvWithThreeErrors = 'id,name\n3,"John"\n1,"Alice\n2,"Bob"\n4,"Charlie\n5,"Eve\n6,"David';
            const mockBinaryDataThreeErrors = {
            ...
            mockExecuteFunctions.helpers.assertBinaryData.mockReturnValue(mockBinaryDataThreeErrors);
            await expect(execute.call(mockExecuteFunctions, items)).rejects.toThrow('Max number of skipped records exceeded');
        });
    });
    ...
    describe('CSV parsing with empty lines (stream path)', () => {
        ...
        it('should handle CSV with trailing empty lines via stream', async () => {
            ...
            expect(result[0].json).toEqual({ id: '1', name: 'Alice' });
            expect(result[1].json).toEqual({ id: '2', name: 'Bob' });
            expect(mockExecuteFunctions.helpers.getBinaryStream).toHaveBeenCalledWith('binary-data-id-csv');
        });
        ...
        it('should reject when source stream errors', async () => {
            ...
            mockExecuteFunctions.helpers.getBinaryStream.mockResolvedValue(mockStream);
            const items = [{ json: {} }];
            await expect(execute.call(mockExecuteFunctions, items)).rejects.toThrow('stream read failure');
        });
    });
});
//# sourceMappingURL=fromFile.operation.test.js.map