...
export async function koBoToolboxApiRequest(option = {}) {
...
export async function koBoToolboxRawRequest(option) {
    const credentials = await this.getCredentials('koBoToolboxApi');
...
export function formatSubmission(submission, selectMasks = [], numberMasks = []) {
    ...
    if (isArray(response.geolocation) &&
...
export function validateAttachmentUrl(url, allowedDomain) {
    let attachmentUrl;
    let allowedUrl;
    try {
...
export async function downloadAttachments(submission, options) {
