				...
				if (resource === 'alert') {
					if (operation === 'count') {
						...
						if ('customFieldsUi' in filters) {
							...
							(_countSearchQuery['_and'] as IQueryObject[]).push.apply(
								_countSearchQuery['_and'] as IQueryObject[],
								searchQueries,
							);
						}
					...
					*/
					...
					if (operation === 'getAll') {
						...
						if ('customFieldsUi' in filters) {
							const customFields = (await prepareCustomFields.call(this, filters)) as IDataObject;
							const searchQueries = buildCustomFieldSearch(customFields);
							(_searchQuery['_and'] as IQueryObject[]).push.apply(
								_searchQuery['_and'] as IQueryObject[],
								searchQueries,
							);
						}
