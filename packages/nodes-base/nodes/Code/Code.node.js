...
import { NodeConnectionTypes, UnexpectedError, UserError, } from 'n8n-workflow';
...
\`.trim()
        ...
        properties: [
            ...
            },
            {
                ...
                default: 'javaScript',
                builderHint: {
                    propertyHint: 'Default to javaScript — the only language with library access and cross-node helpers. Choose pythonNative only when the user explicitly asks for Python.',
                },
            },
            {
                displayName: 'Language',
    ...
    async execute() {
        ...
        if (isPyLang) {
            ...
            if (!runnerStatus.available) {
                throw new PythonRunnerUnavailableError(runnerStatus.reason);
            }
            const code = this.getNodeParameter(codeParameterName, 0);
