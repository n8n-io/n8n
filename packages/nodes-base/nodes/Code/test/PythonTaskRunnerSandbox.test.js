...
const createNormalizeItemsMock = () => vi.fn().mockImplementation((items) => {
...
const createMockExecuteFunctions = (inputData = []) => {
...
describe('PythonTaskRunnerSandbox', () => {
    describe('runUsingIncomingItems', () => {
        it('should call validateRunCodeAllItems for runOnceForAllItems mode', async () => {
            ...
            const sandbox = new PythonTaskRunnerSandbox(pythonCode, nodeMode, workflowMode, executeFunctions);
            ...
            expect(executeFunctions.startJob).toHaveBeenCalledWith('python', {
                code: pythonCode,
                nodeMode,
                workflowMode,
        ...
        it('should throw NodeOperationError when pythonCode is undefined', async () => {
            ...
            const workflowMode = 'manual';
            const executeFunctions = createMockExecuteFunctions([]);
            const sandbox = new PythonTaskRunnerSandbox(undefined, nodeMode, workflowMode, executeFunctions);
            await expect(sandbox.runUsingIncomingItems()).rejects.toThrow(NodeOperationError);
            await expect(sandbox.runUsingIncomingItems()).rejects.toThrow('No Python code found to execute');
    ...
    describe('runCodeForTool', () => {
        it('should pass query and empty items to the runner', async () => {
            ...
            const sandbox = new PythonTaskRunnerSandbox(pythonCode, nodeMode, workflowMode, executeFunctions, { query });
        ...
        it('should pass structured query object to the runner', async () => {
            ...
            const result = await sandbox.runCodeForTool();
            expect(executeFunctions.startJob).toHaveBeenCalledWith('python', expect.objectContaining({ query, items: [] }), 0);
        ...
        it('should return result without validation', async () => {
            ...
            const workflowMode = 'manual';
            const executeFunctions = createMockExecuteFunctions([]);
            const sandbox = new PythonTaskRunnerSandbox(pythonCode, nodeMode, workflowMode, executeFunctions, { query: 'test' });
        ...
        it('should throw NodeOperationError when pythonCode is undefined', async () => {
            ...
            const workflowMode = 'manual';
            const executeFunctions = createMockExecuteFunctions([]);
            const sandbox = new PythonTaskRunnerSandbox(undefined, nodeMode, workflowMode, executeFunctions, { query: 'test' });
...
//# sourceMappingURL=PythonTaskRunnerSandbox.test.js.map