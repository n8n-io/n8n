...
export function getTextKey(textKeys, key, options) {
...
export function validateItem({ json, binary }, itemIndex, textKeys) {
...
export function validateTopLevelKeys(item, itemIndex) {
...
export function validateRunCodeEachItem(executionResult, itemIndex, textKeys, normalizeItems) {
...
export function validateRunCodeAllItems(executionResult, textKeys, normalizeItems) {
    if (typeof executionResult !== 'object') {
        throw new ValidationError({
    ...
    if (Array.isArray(executionResult)) {
        ...
        const mustHaveTopLevelN8nKey = executionResult.some((item) => Object.keys(item).find((key) => REQUIRED_N8N_ITEM_KEYS.has(key)));
