...
const createTestBuffer = () => Buffer.from([
...
const mockGmInstance = {
    background: vi.fn(function () {
        return this;
    }),
    blur: vi.fn(function () {
        return this;
    }),
    borderColor: vi.fn(function () {
        return this;
    }),
    border: vi.fn(function () {
        return this;
    }),
    compose: vi.fn(function () {
        return this;
    }),
    geometry: vi.fn(function () {
        return this;
    }),
    composite: vi.fn(function () {
        return this;
    }),
    crop: vi.fn(function () {
        return this;
    }),
    drawCircle: vi.fn(function () {
        return this;
    }),
    drawLine: vi.fn(function () {
        return this;
    }),
    drawRectangle: vi.fn(function () {
        return this;
    }),
    fill: vi.fn(function () {
        return this;
    ...
    drawText: vi.fn(function () {
        return this;
    }),
    identify: vi.fn(function (callback) {
        callback(null, { width: 100, height: 100, format: 'PNG' });
        return this;
    }),
    quality: vi.fn(function () {
        return this;
    }),
    ...
    setFormat: vi.fn(function () {
        return this;
    }),
    shear: vi.fn(function () {
        return this;
    }),
    stream: vi.fn(function () {
        return this;
    }),
    transparent: vi.fn(function () {
        return this;
    }),
    toBuffer: vi.fn(function (callback) {
        callback(null, createTestBuffer());
        return this;
    }),
    autoOrient: vi.fn(function () {
        return this;
    }),
    out: vi.fn(function () {
        return this;
...
describe('EditImage Node', () => {
    ...
    beforeEach(() => {
        ...
        mockExecuteFunctions.helpers.assertBinaryData.mockReturnValue(undefined);
    });
        ...
        it('should handle string type for custom property', async () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'operation')
                    return 'information';
                if (paramName === 'dataPropertyName')
                    return 'customImageProperty';
                return {};
            });
            mockExecuteFunctions.helpers.getBinaryDataBuffer.mockResolvedValue(testBuffer);
            await editImageNode.execute.call(mockExecuteFunctions);
            expect(mockExecuteFunctions.helpers.assertBinaryData).toHaveBeenCalledWith(0, 'customImageProperty');
            expect(mockExecuteFunctions.helpers.getBinaryDataBuffer).toHaveBeenCalledWith(0, 'customImageProperty');
        });
    ...
    describe('destinationKey option', () => {
        it('should use destinationKey for output property', async () => {
            ...
            mockExecuteFunctions.getInputData.mockReturnValue(items);
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName, _) => {
                if (paramName === 'operation')
                    return 'blur';
                if (paramName === 'dataPropertyName')
                    return 'data';
    ...
    describe('format and quality options', () => {
        ...
        it.each([
            ...
            mockExecuteFunctions.helpers.getBinaryDataBuffer.mockResolvedValue(testBuffer);
            await expect(editImageNode.execute.call(mockExecuteFunctions)).rejects.toThrow(`Invalid image format: ${format}`);
    ...
    describe('error handling', () => {
        ...
        it('should throw error when continueOnFail is false', async () => {
            ...
            await expect(editImageNode.execute.call(mockExecuteFunctions)).rejects.toThrow('Binary data missing');
        });
    });
    describe('operations with IBinaryData', () => {
        ...
        it.each([
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                ...
                if (paramName === 'fontSize')
                    return 18;
                if (paramName === 'fontColor')
                    return '#000000';
                if (paramName === 'positionX')
                    return 10;
                if (paramName === 'positionY')
                    return 10;
                if (paramName === 'lineLength')
                    return 80;
                if (paramName === 'options')
                    return { font: arialFont };
                return {};
            });
            mockExecuteFunctions.helpers.getBinaryDataBuffer.mockResolvedValue(testBuffer);
            mockExecuteFunctions.helpers.prepareBinaryData.mockResolvedValue({
        ...
        describe('font parameter', () => {
            ...
            it.each(['text', 'multiStep'])('should reject an unavailable font for %s', async (operation) => {
                const testBuffer = createTestBuffer();
                ...
                mockExecuteFunctions.helpers.getBinaryDataBuffer.mockResolvedValue(testBuffer);
                await expect(editImageNode.execute.call(mockExecuteFunctions)).rejects.toThrow('The selected font is not available. Select a font from the options.');
            });
        });
        ...
        it('should read from both main and composite binary properties', async () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                ...
                if (paramName === 'dataPropertyName')
                    return 'mainImage';
                if (paramName === 'dataPropertyNameComposite')
                    return 'overlayImage';
    ...
    describe('all operations with string and IBinaryData', () => {
        ...
        it('should handle draw operation with string dataPropertyName', async () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                ...
                if (paramName === 'dataPropertyName')
                    return 'imageFile';
                if (paramName === 'primitive')
                    return 'rectangle';
                if (paramName === 'color')
                    return '#ff0000';
                if (paramName === 'startPositionX')
                    return 10;
                if (paramName === 'startPositionY')
                    return 10;
                if (paramName === 'endPositionX')
                    return 100;
                if (paramName === 'endPositionY')
                    return 100;
                if (paramName === 'cornerRadius')
                    return 5;
                if (paramName === 'options')
                    return {};
                return {};
        ...
        it('should handle shear operation with string dataPropertyName', async () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                ...
                if (paramName === 'dataPropertyName')
                    return 'myImage';
                if (paramName === 'degreesX')
                    return 20;
                if (paramName === 'degreesY')
                    return 10;
                if (paramName === 'options')
                    return {};
                return {};
    ...
    describe('numeric parameters', () => {
        ...
        it.each(operationTestCases.flatMap(({ operation, parameters, numericParameterNames }) => numericParameterNames.map((parameterName) => ({
            operation,
            parameterName,
            parameters: { ...parameters, [parameterName]: 'not-a-number' },
        }))))('should reject an invalid $parameterName for the $operation operation', async ({ operation, parameterName, parameters }) => {
            ...
            await expect(editImageNode.execute.call(mockExecuteFunctions)).rejects.toThrow(`The value of "${parameterName}" must be a number`);
        });
        it.each([NaN, Infinity, -Infinity, '', '   ', null])('should reject the invalid numeric value %s', async (invalidValue) => {
            mockExecuteFunctions.getInputData.mockReturnValue([{ json: {} }]);
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'operation')
                    return 'blur';
            ...
            await expect(editImageNode.execute.call(mockExecuteFunctions)).rejects.toThrow('The value of "blur" must be a number');
        });
        ...
        it('should reject an invalid quality value', async () => {
            ...
            await expect(editImageNode.execute.call(mockExecuteFunctions)).rejects.toThrow('The value of "quality" must be a number');
        });
    });
...
//# sourceMappingURL=EditImage.node.test.js.map