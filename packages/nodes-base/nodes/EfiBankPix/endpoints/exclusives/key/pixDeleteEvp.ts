...
export async function pixDeleteEvp(
  ...
  try {
    ...
    const chavePix = context.getNodeParameter('chavePix', index) as string;
    
    const resposta = await efipay.pixDeleteEvp({ chave: chavePix });
    return resposta;
  } catch (error: any) {
