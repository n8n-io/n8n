...
import { BINARY_ENCODING, WAIT_NODE_TYPE, CHAT_TRIGGER_NODE_TYPE, UserError, } from 'n8n-workflow';
import { RespondToWebhook } from '../RespondToWebhook.node';
describe('RespondToWebhook Node', () => {
    let respondToWebhook;
    ...
    describe('chatTrigger response', () => {
        it('should handle chatTrigger correctly when enabled and responseBody is an object', async () => {
            ...
            mockExecuteFunctions.getNode.mockReturnValue(mock({ typeVersion: 1.4 }));
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'respondWith')
                    return 'json';
                if (paramName === 'responseBody')
        ...
        it('should not handle chatTrigger when disabled', async () => {
            ...
            mockExecuteFunctions.getNode.mockReturnValue(mock({ typeVersion: 1.1 }));
            mockExecuteFunctions.getParentNodes.mockReturnValue([
                mock({ type: CHAT_TRIGGER_NODE_TYPE, disabled: true }),
            ]);
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'respondWith')
                    return 'json';
    ...
    describe('execute method', () => {
        it('should throw an error if no WEBHOOK_NODE_TYPES in parents', async () => {
            ...
            mockExecuteFunctions.getNode.mockReturnValue(mock({ typeVersion: 1.1 }));
            mockExecuteFunctions.getParentNodes.mockReturnValue([
                mock({ type: 'n8n-nodes-base.someNode' }),
            ]);
            await expect(respondToWebhook.execute.call(mockExecuteFunctions)).rejects.toThrow('No Webhook node found in the workflow');
        });
        it('should not throw an error if WEBHOOK_NODE_TYPES is in parents', async () => {
            mockExecuteFunctions.getInputData.mockReturnValue([]);
            mockExecuteFunctions.getNode.mockReturnValue(mock({ typeVersion: 1.1 }));
            mockExecuteFunctions.getParentNodes.mockReturnValue([
                mock({ type: WAIT_NODE_TYPE }),
            ]);
        ...
        it('should correctly return a jwt response', async () => {
            mockExecuteFunctions.getInputData.mockReturnValue([{ json: { input: true } }]);
            mockExecuteFunctions.getNode.mockReturnValue(mock({ typeVersion: 1.1 }));
            mockExecuteFunctions.getCredentials.mockResolvedValue(mock({
                keyType: 'passphrase',
        ...
        it('should correctly return binary', async () => {
            ...
            const inputItems = [{ binary: { data: binary }, json: {} }];
            mockExecuteFunctions.getInputData.mockReturnValue(inputItems);
        ...
        it('should correctly handle continue on fail', async () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'respondWith')
                    return 'notSupportedRespondWith';
                if (paramName === 'options')
                    return {};
            });
            mockExecuteFunctions.sendResponse.mockResolvedValue(undefined);
        ...
        it('should fail the node when the response cannot be dispatched', async () => {
            ...
            await expect(respondToWebhook.execute.call(mockExecuteFunctions)).rejects.toThrow('Response not relayed');
        });
    ...
    describe('streaming functionality', () => {
        it('should stream JSON response when streaming is enabled', async () => {
            mockExecuteFunctions.getInputData.mockReturnValue([{ json: { input: true } }]);
            mockExecuteFunctions.getNode.mockReturnValue(mock({ typeVersion: 1.5 }));
            mockExecuteFunctions.getParentNodes.mockReturnValue([
                mock({ type: WAIT_NODE_TYPE }),
            ]);
...
//# sourceMappingURL=RespondToWebhook.test.js.map