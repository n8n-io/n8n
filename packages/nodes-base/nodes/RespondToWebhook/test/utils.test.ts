...
describe('sanitizeResponseData', () => {
	test.each([
		{
			responseBody: 'https://example.com',
		},
		{
			responseBody:
		...
		{
			responseBody: "<div>Test with  'single quote'</div>",
		},
		{
			responseBody:
	...
	])('wraps the response body in an iframe', ({ responseBody }) => {
		const result = sandboxResponseData(responseBody);
