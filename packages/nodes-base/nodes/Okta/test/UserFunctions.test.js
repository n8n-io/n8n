import { getCursorPaginator, getUsers, oktaApiRequest, simplifyGetAllResponse, simplifyGetResponse, } from '../UserFunctions';
...
describe('getCursorPaginator', () => {
    ...
    it('should handle empty response data', async () => {
        mockContext.getNodeParameter.mockReturnValue(true);
        mockContext.makeRoutingRequest.mockResolvedValue([]);
        const paginator = getCursorPaginator().bind(mockContext);
        const result = await paginator(mockRequestOptions);
        expect(result).toEqual([]);
        expect(mockContext.getNodeParameter).toHaveBeenCalledWith('returnAll', true);
...
//# sourceMappingURL=UserFunctions.test.js.map