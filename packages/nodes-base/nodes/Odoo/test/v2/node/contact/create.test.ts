...
describe('OdooV2 — contact:create', () => {
	...
	it('creates a contact using defineBelow mapping and returns its id', async () => {
		setupParams();
		(transport.odooApiRequest as jest.Mock).mockResolvedValue([MOCK_CONTACT_ID]);
	...
	it('processes multiple input items independently', async () => {
		...
		(transport.odooApiRequest as jest.Mock).mockResolvedValueOnce([1]).mockResolvedValueOnce([2]);
