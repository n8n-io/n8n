...
describe('bodyToRpcArgs', () => {
    ...
    it('search_read: includes limit when defined', () => {
        expect(bodyToRpcArgs('search_read', {
            domain: [['name', '=', 'foo']],
            fields: ['id'],
            offset: 0,
            limit: 10,
...
describe('odooApiRequest', () => {
    ...
    beforeEach(() => {
        ctx = mock();
        ctx.getNode.mockReturnValue({ name: 'Odoo', type: 'n8n-nodes-base.odoo' });
    });
    ...
    describe('API key auth (callJson2)', () => {
        ...
        it('POSTs to /json/2/<model>/<method> with body', async () => {
            ...
            expect(ctx.helpers.httpRequestWithAuthentication).toHaveBeenCalledWith('odooApiKeyApi', expect.objectContaining({
                method: 'POST',
        ...
        it('strips trailing slash from URL', async () => {
            ...
            expect(ctx.helpers.httpRequestWithAuthentication).toHaveBeenCalledWith('odooApiKeyApi', expect.objectContaining({ url: 'https://myorg.odoo.com/json/2/res.partner/read' }));
        });
        ...
        it('throws NodeApiError on HTTP failure', async () => {
            ...
            await expect(odooApiRequest.call(ctx, 'res.partner', 'search_read', {})).rejects.toBeDefined();
        });
    });
    describe('legacy RPC auth (rpcRequest)', () => {
        ...
        it('logs in then executes via /jsonrpc', async () => {
            ...
            // First call: login
            expect(ctx.helpers.httpRequest).toHaveBeenNthCalledWith(1, expect.objectContaining({
            ...
            // Second call: execute
            expect(ctx.helpers.httpRequest).toHaveBeenNthCalledWith(2, expect.objectContaining({
                url: 'https://myorg.odoo.com/jsonrpc',
                body: expect.objectContaining({
