...
describe('Test Mailgun node', () => {
    ...
    let mockReturnJsonArray;
    let mockConstructExecutionMetaData;
    beforeEach(() => {
    ...
    describe('comma-separated attachment strings', () => {
        it('should process comma-separated attachment names with spaces', async () => {
            const items = [
                {
                    json: { data: 'test' },
                    binary: {
                        file1: { data: 'data1', mimeType: 'text/plain', fileName: 'file1.txt' },
                        file2: { data: 'data2', mimeType: 'text/plain', fileName: 'file2.txt' },
                        file3: { data: 'data3', mimeType: 'text/plain', fileName: 'file3.txt' },
                    },
                },
            ];
            mockExecuteFunctions.getInputData.mockReturnValue(items);
            mockExecuteFunctions.getNode.mockReturnValue({ type: 'n8n-nodes-base.mailgun' });
            ...
            mockExecuteFunctions.helpers.assertBinaryData.mockImplementation((itemIndex, propertyName) => {
                return items[itemIndex].binary[propertyName];
            });
            mockExecuteFunctions.helpers.getBinaryDataBuffer.mockImplementation(async (itemIndex, propertyName) => {
            ...
            mockReturnJsonArray.mockImplementation((data) => [{ json: data }]);
            mockConstructExecutionMetaData.mockImplementation((data) => data);
            await mailgunNode.execute.call(mockExecuteFunctions);
            expect(mockRequestWithAuthentication).toHaveBeenCalledWith('mailgunApi', expect.objectContaining({
    ...
    describe('prepareBinariesDataList helper function', () => {
        ...
        it('should process IBinaryData object as attachment', async () => {
            ...
            mockExecuteFunctions.helpers.getBinaryDataBuffer.mockImplementation(async (itemIndex, propertyName) => {
                const binaryData = typeof propertyName === 'object'
                    ? propertyName
...
//# sourceMappingURL=Mailgun.node.test.js.map