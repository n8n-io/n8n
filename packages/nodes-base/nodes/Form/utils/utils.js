...
import { FORM_NODE_TYPE, FORM_TRIGGER_NODE_TYPE, NodeOperationError, WAIT_NODE_TYPE, WorkflowConfigurationError, jsonParse, tryToParseUrl, BINARY_MODE_COMBINED, tryToParseJsonToFormFields, UnexpectedError, buildCredentialConnectionsRequiredResponse, } from 'n8n-workflow';
import * as a from 'node:assert';
import sanitize from 'sanitize-html';
import { getResolvables } from '../../../utils/utilities';
import { WebhookAuthorizationError } from '../../Webhook/error';
import { generateFormPostBasicAuthToken, isIpAllowed, validateWebhookAuthentication, } from '../../Webhook/utils';
...
export const getNodeReference = (nodeName) => `$(${JSON.stringify(nodeName)})`;
 ...
 */
export const handleNewlines = (text) => {
    return text.replace(/\\n|\\\\n/g, (match) => (match === '\\\\n' ? '\\n' : '\n'));
};
export const prepareFormFields = (fields) => {
 ...
 */
export function validateSafeRedirectUrl(url) {
    if (!url)
        return null;
...
export function createDescriptionMetadata(description) {
    return description === ''
        ? 'n8n form'
...
export function getN8nWebsiteLink(instanceId) {
...
export function prepareFormData({ formTitle, formDescription, formSubmittedHeader, formSubmittedText, redirectUrl, formFields, testRun, query, instanceId, useResponseData, appendAttribution = true, buttonLabel, customCss, nodeVersion, authToken, shellInner, hasAuthenticatedSubmitter, hostNavigationPath, }) {
...
export const validateResponseModeConfiguration = (context) => {
    ...
    if (!isRespondToWebhookConnected && responseMode === 'responseNode') {
        throw new NodeOperationError(context.getNode(), new Error('No Respond to Webhook node found in the workflow'), {
    ...
    if (isRespondToWebhookConnected && nodeVersion > 2.1) {
        throw new NodeOperationError(context.getNode(), new Error('The "Respond to Webhook" node is not supported in workflows initiated by the "n8n Form Trigger"'), {
            description: 'To configure your response, add an "n8n Form" node and set the "Page Type" to "Form Ending"',
        });
    }
};
export function addFormResponseDataToReturnItem(returnItem, formFields, bodyData, nodeVersion) {
    for (const [index, field] of formFields.entries()) {
...
export async function prepareFormReturnItem(context, formFields, mode, useWorkflowTimezone = false, authedUser) {
...
export function renderForm({ context, res, formTitle, formDescription, formFields, responseMode, mode, formSubmittedText, redirectUrl, appendAttribution, buttonLabel, customCss, authToken, shellInner, hasAuthenticatedSubmitter, }) {
    ...
    if (context.getNode().type === FORM_TRIGGER_NODE_TYPE) {
        ...
        const trigger = parentNodes.find((node) => node.type === FORM_TRIGGER_NODE_TYPE);
        try {
            const triggerRef = getNodeReference(trigger.name);
            const triggerQueryParameters = context.evaluateExpression(`{{ ${triggerRef}.first().json.formQueryParameters }}`);
 ...
 */
function buildAbsoluteFormUrl(req) {
 ...
 * itself.
 */
export function getHostNavigationPath(context, shellInner) {
    if (!isHostedInFormShell(context.getRequestObject(), shellInner))
        return undefined;
    return getFormWaitingBasePath(context) ?? undefined;
}
export const isFormConnected = (nodes) => {
    return nodes.some((n) => n.type === FORM_NODE_TYPE || (n.type === WAIT_NODE_TYPE && n.parameters?.resume === 'form'));
};
/**
 ...
 */
export async function respondIfCredentialsNotReady(context, res) {
    let readiness;
    try {
 ...
 */
export function generateFormUserAuthToken(node, user, binding) {
 ...
 */
export function verifyFormUserAuthToken(token, node, executionId) {
...
function setFormOAuthToken(res, req, resourceUrl, token) {
 ...
 */
export function setFormAuthCookie(context, token, binding) {
    const req = context.getRequestObject();
    ...
    if (runOAuth2Flow) {
        ...
        if (req.method === 'GET') {
            ...
            try {
                ...
                const authorizationUrl = await context.beginN8nOAuth2Flow(resourceUrl, originalQuery ? { query: originalQuery } : undefined);
                res.writeHead(302, {
 ...
 */
export async function validateFormPageAuth(context, triggerAuthentication) {
 ...
 */
export function formShellSummaryText(total, connectedCount) {
    const remaining = total - connectedCount;
    if (remaining <= 0)
        return `All ${total} accounts connected · ready to submit`;
    if (connectedCount === 0)
        return `${total} accounts needed to submit this form`;
 ...
 * dialog.
 */
export function buildFormShellViewModel(credentials, submitterEmail) {
...
export async function formWebhook(context, authProperty = FORM_TRIGGER_AUTHENTICATION_PROPERTY) {
    ...
    const authentication = context.getNodeParameter(authProperty, 'none');
    let authedUser;
    let oAuth2Token;
    ...
    if (method === 'GET') {
        ...
        const formDescription = handleNewlines(sanitizeHtml(context.getNodeParameter('formDescription', '')));
    ...
    const returnItem = await prepareFormReturnItem(context, formFields, mode, useWorkflowTimezone, userForOutput);
    return {
        webhookResponse: { status: 200 },
        workflowData: [[returnItem]],
    };
}
export function resolveRawData(context, rawData) {
    if (!rawData)
        return rawData;
...
export function parseFormFields(context, options) {
    let fields = [];
    if (options.defineForm === 'json') {
        const getJsonOutput = () => context.getNodeParameter(options.fieldsParameterName, '', {
            rawExpressions: true,
        });
 ...
 */
export function parseJsonFormFields(context, getJsonOutput, mode) {
    try {
