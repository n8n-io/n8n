...
import { generateFormUserAuthToken, getNodeReference, handleNewlines, renderForm, resolveRawData, sanitizeHtml, setFormAuthCookie, } from './utils';
export const renderFormNode = async (context, res, trigger, fields, mode, authedUser) => {
    ...
    if (!buttonLabel) {
        buttonLabel =
            context.evaluateExpression(`{{ ${triggerRef}.params.options?.buttonLabel }}`) ||
                'Submit';
 ...
 */
export function getFormTriggerNode(context) {
    const parentNodes = context.getParentNodes(context.getNode().name);
    const formTriggers = parentNodes.filter((node) => node.type === FORM_TRIGGER_NODE_TYPE);
    if (!formTriggers.length) {
        throw new NodeOperationError(context.getNode(), 'Form Trigger node must be set before this node');
    }
