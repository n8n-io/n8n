...
const INBOUND_TRIGGER_AUTHENTICATION_BUILDER_HINT = "Default to 'none'. n8n exposes inbound trigger URLs publicly by design. Only select an authentication method when the user explicitly asks to authenticate inbound traffic.";
describe('FormTrigger', () => {
    ...
    it('should tell builders to keep inbound authentication disabled unless requested', () => {
        ...
        const authParams = formTriggerV2.description.properties.filter((property) => property.name === FORM_TRIGGER_AUTHENTICATION_PROPERTY);
    ...
    it('should expose n8nUserAuth option only on typeVersion >= 2.6', () => {
        ...
        const versionCnd = (param) => param.displayOptions?.show?.['@version']?.[0]?._cnd;
    ...
    describe('requireExecuteAccess', () => {
        ...
        const requireExecuteAccess = formTriggerV2.description.properties.find((property) => property.name === 'requireExecuteAccess');
        ...
        ])('should be shown for authentication %s on typeVersion %s: %s', (authentication, typeVersion, shown) => {
            expect(requireExecuteAccess).toBeDefined();
            expect(NodeHelpers.displayParameter({ [FORM_TRIGGER_AUTHENTICATION_PROPERTY]: authentication }, requireExecuteAccess, { typeVersion }, formTriggerV2.description)).toBe(shown);
        });
    });
    ...
    describe('Respond to Webhook', () => {
        it('should throw when misconfigured', async () => {
            ...
            await expect(missingRespondNode).rejects.toThrow('No Respond to Webhook node found in the workflow');
            ...
            await expect(unusedRespondNode).rejects.toThrow('Unused Respond to Webhook node found in the workflow');
        });
    });
    ...
    it('should apply customCss property to form render', async () => {
        ...
        expect(response.render).toHaveBeenCalledWith('form-trigger', expect.objectContaining({
...
//# sourceMappingURL=FormTriggerV2.node.test.js.map