...
const missing = (n) => Array.from({ length: n }, (_, i) => credential({ credentialId: `missing-${i}`, credentialName: `Account ${i}` }));
const configured = (n) => Array.from({ length: n }, (_, i) => credential({
...
describe('buildFormShellViewModel', () => {
    ...
    describe('counts', () => {
        ...
        it('should pluralise the dialog footer counter', () => {
            ...
            expect(buildFormShellViewModel([...configured(1), ...missing(1)]).footerText).toBe('1 of 2 accounts connected');
        ...
        it('should carry the summary line for the rendered state', () => {
            expect(buildFormShellViewModel([...configured(1), ...missing(2)]).summaryText).toBe('2 more accounts needed to submit this form');
        });
    });
    describe('icons', () => {
    ...
    describe('rows', () => {
        ...
        it('should carry the per-row connect and revoke links through', () => {
            ...
            expect(vm.credentials[0].authorizationUrl).toBe('http://localhost:5678/rest/credentials/cred-1/authorize?token=t');
            expect(vm.credentials[0].revokeUrl).toBeUndefined();
            expect(vm.credentials[1].revokeUrl).toBe('http://localhost:5678/rest/credentials/cred-2/revoke?resolverId=resolver-1');
        });
    });
});
//# sourceMappingURL=formShellViewModel.test.js.map