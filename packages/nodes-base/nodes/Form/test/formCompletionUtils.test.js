vi.mock('n8n-core', () => ({
    getHtmlSandboxCSP: vi.fn(() => 'sandbox allow-downloads allow-forms allow-modals allow-orientation-lock allow-pointer-lock allow-popups allow-presentation allow-scripts allow-top-navigation-by-user-activation allow-top-navigation-to-custom-protocols'),
...
describe('formCompletionUtils', () => {
    ...
    beforeEach(() => {
        ...
        mockWebhookFunctions.getRequestObject.mockReturnValue(mock({ headers: {}, query: {} }));
    });
    afterEach(() => {
        vi.resetAllMocks();
    });
    describe('renderFormCompletion', () => {
        ...
        beforeEach(() => {
            vi.mocked(getHtmlSandboxCSP).mockReturnValue('sandbox allow-downloads allow-forms allow-modals allow-orientation-lock allow-pointer-lock allow-popups allow-presentation allow-scripts allow-top-navigation-by-user-activation allow-top-navigation-to-custom-protocols');
        ...
        it('should render the form completion', async () => {
            mockWebhookFunctions.getNodeParameter.mockImplementation((parameterName) => {
                const params = {
        ...
        it('should pass the attribution link to the completion template', async () => {
            ...
            mockWebhookFunctions.evaluateExpression.mockImplementation((expression) => expression ===
                `{{ $(${JSON.stringify(trigger.name)}).params.options?.appendAttribution === false ? false : true }}`
                ? true
                : '');
            await renderFormCompletion(mockWebhookFunctions, mockResponse, trigger);
            expect(mockResponse.render).toHaveBeenCalledWith('form-trigger-completion', expect.objectContaining({
        ...
        it('should render completionTitle and completionMessage as-is without re-evaluating them', async () => {
            ...
            mockWebhookFunctions.evaluateExpression.mockImplementation((expression) => expression === '{{ 1 + 1 }}' ? '2' : '');
            await renderFormCompletion(mockWebhookFunctions, mockResponse, trigger);
        ...
        it('should resolve expressions in the form title inherited from the trigger', async () => {
            ...
            await renderFormCompletion(mockWebhookFunctions, mockResponse, trigger);
            expect(mockResponse.render).toHaveBeenCalledWith('form-trigger-completion', expect.objectContaining({ formTitle: 'MyForm' }));
        });
        ...
        it('throw an error if no binary data with the field name is found', async () => {
            ...
            await expect(renderFormCompletion(mockWebhookFunctions, mockResponse, trigger)).rejects.toThrowError('No binary data with field inputData found.');
        });
        ...
        it('should set Content-Security-Policy header with sandbox CSP', async () => {
            ...
            await renderFormCompletion(mockWebhookFunctions, mockResponse, trigger);
            expect(mockResponse.setHeader).toHaveBeenCalledWith('Content-Security-Policy', 'sandbox allow-downloads allow-forms allow-modals allow-orientation-lock allow-pointer-lock allow-popups allow-presentation allow-scripts allow-top-navigation-by-user-activation allow-top-navigation-to-custom-protocols');
        ...
        it('embeds an x-auth-token-compatible authToken when an authed user is provided', async () => {
            ...
            const renderArgs = vi.mocked(mockResponse.render).mock.calls.at(-1)?.[1];
            expect(renderArgs.authToken).toBeTruthy();
            // Bound to the run that rendered the page, so it is only accepted for it.
            expect(verifyFormUserAuthToken(renderArgs.authToken, mockNode, 'execution-id')).toEqual(authedUser);
        ...
        describe('host-driven navigation', () => {
            ...
            it('is offered when the shell navigated its frame here', async () => {
                ...
                mockWebhookFunctions.getRequestObject.mockReturnValue(mock({
                    headers: { 'sec-fetch-dest': 'iframe', 'sec-fetch-site': 'same-origin' },
                    query: { n8nShellHosted: '1' },
                }));
                mockWebhookFunctions.evaluateExpression.mockImplementation((expression) => expression === '{{ $execution.resumeFormUrl }}'
                    ? 'http://localhost:5678/form-waiting/execution-id'
                    : '');
                await renderFormCompletion(mockWebhookFunctions, mockResponse, trigger);
                expect(mockResponse.render).toHaveBeenCalledWith('form-trigger-completion', expect.objectContaining({ hostNavigationPath: '/form-waiting' }));
            });
            it('is not offered to a top-level completion page', async () => {
                setupCompletionParams();
                await renderFormCompletion(mockWebhookFunctions, mockResponse, trigger);
                expect(mockResponse.render).toHaveBeenCalledWith('form-trigger-completion', expect.objectContaining({ hostNavigationPath: undefined }));
            });
        });
...
//# sourceMappingURL=formCompletionUtils.test.js.map