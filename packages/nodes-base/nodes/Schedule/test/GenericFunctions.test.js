...
import { intervalToRecurrence, recurrenceCheck, resetStaleRecurrence, toCronExpression, validateInterval, withIntervalDefaults, } from '../GenericFunctions';
...
function mockMomentTz(values) {
    ...
    mockedMoment.tz.mockReturnValue(tzObj);
}
...
describe('toCronExpression', () => {
    ...
    it('should keep `*/N` for month intervals that divide 12, and fire monthly otherwise', () => {
        ...
        for (const months of [1, 2, 3, 4, 6, 12]) {
            expect(toCronExpression({ field: 'months', monthsInterval: months }, TEST_SEED)).toEqual(`56 19 14 22 */${months} *`);
        }
        // Non-divisors fire every month; recurrenceCheck enforces the elapsed-months gap.
        for (const months of [5, 7, 8, 13, 24]) {
            expect(toCronExpression({ field: 'months', monthsInterval: months }, TEST_SEED)).toEqual('56 19 14 22 * *');
        }
    });
});
...
describe('validateInterval', () => {
    ...
    describe('invalid intervals', () => {
        ...
        ])('should throw error for invalid %s interval: %j', (_field, interval, expectedDescription) => {
            try {
                validateInterval(mockNode, 0, interval);
...
describe('recurrenceCheck', () => {
    ...
    it('should return true on first execution when lastExecution is undefined', () => {
        ...
        const result = recurrenceCheck({ activated: true, index: 0, intervalSize: 3, typeInterval: 'days' }, recurrenceRules, 'UTC');
        expect(result).toBe(true);
        expect(recurrenceRules[0]).toBe(100);
    });
    it('should treat a persisted null (reset slot) as a first execution', () => {
        mockMomentTz({ dayOfYear: 100 });
        const recurrenceRules = [null];
        const result = recurrenceCheck({ activated: true, index: 0, intervalSize: 3, typeInterval: 'days' }, recurrenceRules, 'UTC');
        expect(result).toBe(true);
        expect(recurrenceRules[0]).toBe(100);
    });
    it('should not trigger again on the same day', () => {
        mockMomentTz({ dayOfYear: 100 });
        const recurrenceRules = [];
        recurrenceCheck({ activated: true, index: 0, intervalSize: 2, typeInterval: 'days' }, recurrenceRules, 'UTC');
        const result = recurrenceCheck({ activated: true, index: 0, intervalSize: 2, typeInterval: 'days' }, recurrenceRules, 'UTC');
        expect(result).toBe(false);
    });
    describe('minutes', () => {
        it('should trigger when exactly on time', () => {
            ...
            const result = recurrenceCheck({ activated: true, index: 0, intervalSize: 50, typeInterval: 'minutes' }, recurrenceRules, 'UTC');
            expect(result).toBe(true);
    ...
    describe('hours', () => {
        it('should trigger when exactly on time', () => {
            ...
            const result = recurrenceCheck({ activated: true, index: 0, intervalSize: 3, typeInterval: 'hours' }, recurrenceRules, 'UTC');
            expect(result).toBe(true);
    ...
    describe('days', () => {
        ...
        it('should handle wrap-around at year boundary', () => {
            ...
            const result = recurrenceCheck({ activated: true, index: 0, intervalSize: 5, typeInterval: 'days' }, recurrenceRules, 'UTC');
            expect(result).toBe(true);
        });
        it('should not trigger before interval on wrap-around', () => {
    ...
    describe('weeks', () => {
        it('should trigger when exactly on time', () => {
            ...
            const result = recurrenceCheck({ activated: true, index: 0, intervalSize: 2, typeInterval: 'weeks' }, recurrenceRules, 'UTC');
            expect(result).toBe(true);
            expect(recurrenceRules[0]).toBe(8);
        });
        it('should not trigger before interval has elapsed', () => {
        ...
        it('should handle wrap-around at year boundary', () => {
            ...
            const result = recurrenceCheck({ activated: true, index: 0, intervalSize: 3, typeInterval: 'weeks' }, recurrenceRules, 'UTC');
            expect(result).toBe(true);
        });
    });
    describe('months', () => {
        it('should trigger when exactly on time', () => {
            ...
            const result = recurrenceCheck({ activated: true, index: 0, intervalSize: 3, typeInterval: 'months' }, recurrenceRules, 'UTC');
            expect(result).toBe(true);
            expect(recurrenceRules[0]).toBe(5);
        });
        it('should not trigger before interval has elapsed', () => {
    ...
    describe('stale value left by a type change from days to hours', () => {
        ...
        it('never fires when a day-of-year value lingers under an hours rule', () => {
            ...
            for (let h = 0; h < 24; h++) {
                mockMomentTz({ hour: h });
                if (recurrenceCheck(hoursRule, [...recurrenceRules], 'UTC'))
                    everFires = true;
            }
...
describe('resetStaleRecurrence', () => {
    ...
    it('keeps an in-range value and backfills the signature on upgrade', () => {
        ...
        const staticData = { recurrenceRules: [200], recurrenceRuleSignatures: [] };
    ...
    it('clears a months value on upgrade (absolute-month encoding is new)', () => {
        ...
        const staticData = { recurrenceRules: [5], recurrenceRuleSignatures: [] };
        resetStaleRecurrence(staticData, [
