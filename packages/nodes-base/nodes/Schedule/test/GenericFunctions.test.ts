...
describe('toCronExpression', () => {
	...
	it('should keep `*/N` for minute intervals that divide 60, and fire every minute otherwise', () => {
		// Divisors of 60 keep their calendar-anchored cron unchanged.
		for (const minutes of [1, 2, 3, 4, 5, 6, 10, 12, 15, 20, 30, 60]) {
			expect(toCronExpression({ field: 'minutes', minutesInterval: minutes }, TEST_SEED)).toEqual(
				`56 */${minutes} * * * *`,
			);
		}
		// Non-divisors fire every minute; recurrenceCheck enforces the elapsed-minutes gap.
		for (const minutes of [7, 11, 13, 17, 23, 29, 31, 37, 41, 43, 47, 49, 53, 59]) {
			expect(toCronExpression({ field: 'minutes', minutesInterval: minutes }, TEST_SEED)).toEqual(
				'56 * * * * *',
			);
		}
...
describe('recurrenceCheck', () => {
	...
	describe('minutes', () => {
		it('should trigger when exactly on time', () => {
			mockMomentTz({ hour: 0 });
			// Mock minute via a custom approach since mockMomentTz doesn't expose minute
			const tzObj = {
				hour: () => 0,
				dayOfYear: () => 1,
				week: () => 1,
			...
			(mockedMoment.tz as unknown as Mock).mockReturnValue(tzObj);
			const recurrenceRules = [40]; // lastExecution = minute 40, interval = 5
			const result = recurrenceCheck(
				{ activated: true, index: 0, intervalSize: 5, typeInterval: 'minutes' },
				recurrenceRules,
				'UTC',
			);
			expect(result).toBe(true);
		...
		it('should not trigger before interval has elapsed', () => {
			...
			(mockedMoment.tz as unknown as Mock).mockReturnValue(tzObj);
			const recurrenceRules = [40]; // only 2 minutes elapsed, need 5
			const result = recurrenceCheck(
				{ activated: true, index: 0, intervalSize: 5, typeInterval: 'minutes' },
				recurrenceRules,
				'UTC',
		...
		it('should recover after a missed execution', () => {
			...
			(mockedMoment.tz as unknown as Mock).mockReturnValue(tzObj);
			const recurrenceRules = [40]; // missed minute 45, now at minute 10 next hour
			const result = recurrenceCheck(
				{ activated: true, index: 0, intervalSize: 5, typeInterval: 'minutes' },
				recurrenceRules,
				'UTC',
		...
		it('should handle wrap-around (e.g., 55 → 2)', () => {
			const tzObj = {
				hour: () => 0,
				dayOfYear: () => 1,
				week: () => 1,
			...
			(mockedMoment.tz as unknown as Mock).mockReturnValue(tzObj);
			const recurrenceRules = [55]; // lastExecution = 55, interval = 5, elapsed = (2-55+60)%60 = 7
			const result = recurrenceCheck(
				{ activated: true, index: 0, intervalSize: 5, typeInterval: 'minutes' },
				recurrenceRules,
				'UTC',
...
describe('intervalToRecurrence', () => {
	...
	it('should not activate recurrence for 1-minute interval', () => {
		const result = intervalToRecurrence(
			{
				field: 'minutes',
				minutesInterval: 1,
	...
	it('should return recurrence rule for non-divisor minutes interval', () => {
		const result = intervalToRecurrence(
			{
				field: 'minutes',
				minutesInterval: 59,
