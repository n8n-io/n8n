...
export const toCronExpression = (interval: ScheduleInterval, nodeKey: string): CronExpression => {
	...
	if (interval.field === 'minutes') {
		...
		// `*/${minutes}` only fires at clock minutes divisible by ${minutes}: for 59
		// that is :59 — a 1-minute gap then a 59-minute gap, not a steady 59-minute
		// rhythm. Fire every minute; recurrenceCheck enforces the elapsed time.
		return `${second} * * * * *`;
	}
