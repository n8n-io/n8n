...
export async function webexApiRequest(method, resource, body = {}, qs = {}, uri, option = {}) {
    let options = {
        method,
        body,
        qs,
...
export async function webexApiRequestAllItems(propertyName, method, endpoint, body = {}, query = {}, options = {}) {
    const returnData = [];
    let responseData;
    let uri;
    query.max = 100;
...
export function mapResource(event) {
    return {
...
export function getAttachments(attachments) {
    ...
    for (const attachment of attachments) {
        const body = [];
        const actions = [];
        for (const element of (attachment?.elementsUi)
            .elementValues || []) {
        ...
        for (const action of (attachment?.actionsUi).actionValues ||
            []) {
