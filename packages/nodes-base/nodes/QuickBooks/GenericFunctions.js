 ...
 */
export async function quickBooksApiRequest(method, endpoint, qs, body, option = {}) {
 ...
 */
export async function quickBooksApiRequestAllItems(method, endpoint, qs, body, resource) {
    let responseData;
    let startPosition = 1;
 ...
 */
export async function handleListing(i, endpoint, resource) {
    let responseData;
    const qs = {
 ...
 */
export async function getSyncToken(i, companyId, resource) {
 ...
 */
export async function getRefAndSyncToken(i, companyId, resource, ref) {
    const resourceId = this.getNodeParameter(`${resource}Id`, i);
 ...
 */
export async function handleBinaryData(items, i, companyId, resource, resourceId) {
 ...
 */
export function processLines(lines, resource) {
    lines.forEach((line) => {
        if (resource === 'bill') {
 ...
 */
export function populateFields(body, fields, resource) {
...
export const toOptions = (option) => ({ name: option, value: option });
export const splitPascalCase = (word) => {
    return word.match(/($[a-z])|[A-Z][^A-Z]+/g).join(' ');
};
export const toDisplayName = ({ name, value }) => {
    return { name: splitPascalCase(name), value };
};
export function adjustTransactionDates(transactionFields) {
    const dateFieldKeys = [
        'dateRangeCustom',
        'dateRangeDueCustom',
    ...
    let adjusted = omit(transactionFields, dateFieldKeys);
...
export function simplifyTransactionReport(transactionReport) {
