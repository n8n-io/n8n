import { NodeConnectionTypes, WAIT_INDEFINITELY, FORM_TRIGGER_NODE_TYPE, tryToParseDateTime, NodeOperationError, } from 'n8n-workflow';
import { validateWaitAmount, validateWaitUnit } from './validation';
import { updateDisplayOptions } from '../../utils/utilities';
import { formDescription, formFields, respondWithOptions, formRespondMode, formTitle, appendAttributionToForm, } from '../Form/common.descriptions';
import { formWebhook } from '../Form/utils/utils';
import { authenticationProperty, credentialsProperty, defaultWebhookDescription, httpMethodsProperty, optionsProperty, responseBinaryPropertyNameProperty, responseCodeProperty, responseDataProperty, responseModeProperty, } from '../Webhook/description';
...
const waitingTooltip = (parameters, resumeUrl, formResumeUrl) => {
    ...
    description = {
        ...
        properties: [
            ...
            ...waitTimeProperties,
            {
                ...optionsProperty,
                displayOptions: displayOnWebhook,
                options: [...optionsProperty.options, webhookSuffix],
            },
            {
                displayName: 'Options',
                name: 'options',
    ...
    async execute(context) {
        ...
        if (['webhook', 'form'].includes(resume)) {
            ...
            if (resume === 'form') {
                ...
                const resumeFormUrl = context.evaluateExpression('{{ $execution.resumeFormUrl }}', 0);
        ...
        if (resume === 'timeInterval') {
            ...
            if (!validateWaitUnit(unit)) {
                throw new NodeOperationError(context.getNode(), "Invalid wait unit. Valid units are 'seconds', 'minutes', 'hours', or 'days'.");
            }
            let waitAmount = context.getNodeParameter('amount', 0);
            if (!validateWaitAmount(waitAmount)) {
                throw new NodeOperationError(context.getNode(), 'Invalid wait amount. Please enter a number that is 0 or greater.');
            }
            if (unit === 'minutes') {
                waitAmount *= 60;
            }
            ...
            try {
                ...
                throw new NodeOperationError(context.getNode(), 'Cannot put execution to wait because `dateTime` parameter is not a valid date. Please pick a specific date and time to wait until.');
            }
        }
