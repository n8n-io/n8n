...
import { FORM_TRIGGER_NODE_TYPE, NodeOperationError, UserError, } from 'n8n-workflow';
...
describe('Execute Wait Node', () => {
    ...
    ])('Test Wait Node with specificTime $value and isValid $isValid', async ({ value, isValid, expectedWaitTill }) => {
        const putExecutionToWaitSpy = vi.fn();
        const waitNode = new Wait();
        const executeFunctionsMock = mock({
    ...
    describe('Validation', () => {
        describe('Time interval', () => {
            ...
            ])('Validate wait unit: $unit, amount: $amount', async ({ unit, amount, expectedWaitTill, error, mode }) => {
                const putExecutionToWaitSpy = vi.fn();
                const waitNode = new Wait();
...
//# sourceMappingURL=Wait.node.test.js.map