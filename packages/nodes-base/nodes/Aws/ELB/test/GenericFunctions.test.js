...
import { awsApiRequest, awsApiRequestREST, awsApiRequestSOAP, awsApiRequestSOAPAllItems, } from '../GenericFunctions';
vi.mock('xml2js', () => ({
    parseString: vi.fn(),
}));
...
describe('ELB GenericFunctions', () => {
    describe('awsApiRequest', () => {
        ...
        it('should make successful API request with basic parameters', async () => {
            ...
            const result = await awsApiRequest.call(mockExecuteFunctions, 'elasticloadbalancing', 'GET', '/test-path');
        ...
        it('should handle API errors', async () => {
            ...
            await expect(awsApiRequest.call(mockExecuteFunctions, 'elasticloadbalancing', 'GET', '/test-path')).rejects.toThrow(NodeApiError);
        });
    });
    describe('awsApiRequestREST', () => {
        ...
        it('should parse valid JSON response', async () => {
            ...
            const result = await awsApiRequestREST.call(mockExecuteFunctions, 'elasticloadbalancing', 'GET', '/test-path');
    ...
    describe('awsApiRequestSOAP', () => {
        let mockExecuteFunctions;
        const mockParseXml = parseXml;
        beforeEach(() => {
            mockExecuteFunctions = mockDeep();
            vi.clearAllMocks();
        });
        it('should parse XML response correctly', async () => {
            const xmlResponse = '<ListBucketsResult><Buckets><Bucket><Name>test-bucket</Name></Bucket></Buckets></ListBucketsResult>';
            ...
            const result = await awsApiRequestSOAP.call(mockExecuteFunctions, 'elasticloadbalancing', 'GET', '/test-path');
    ...
    describe('awsApiRequestSOAPAllItems', () => {
        ...
        describe('pagination patterns', () => {
            it('should handle DescribeLoadBalancers pagination', async () => {
                ...
                mockExecuteFunctions.helpers.requestWithAuthentication.mockResolvedValue('<xml>response</xml>');
                mockParseXml.mockImplementation((_xml, _options, callback) => {
                    callback(null, xmlResponse);
                });
                const result = await awsApiRequestSOAPAllItems.call(mockExecuteFunctions, 'DescribeLoadBalancersResponse.DescribeLoadBalancersResult.LoadBalancerDescriptions', 'elasticloadbalancing', 'POST', '/describe-load-balancers');
            ...
            it('should handle DescribeTargetGroups with nested properties', async () => {
                ...
                const result = await awsApiRequestSOAPAllItems.call(mockExecuteFunctions, 'DescribeTargetGroupsResponse.DescribeTargetGroupsResult.TargetGroups', 'elasticloadbalancing', 'POST', '/describe-target-groups');
        ...
        describe('error handling', () => {
            it('should handle service error responses', async () => {
                const mockCredentials = { region: 'us-east-1' };
                const elbError = new Error('LoadBalancerNotFound: The specified load balancer does not exist');
                mockExecuteFunctions.getCredentials.mockResolvedValue(mockCredentials);
                mockExecuteFunctions.helpers.requestWithAuthentication.mockRejectedValue(elbError);
                await expect(awsApiRequestSOAPAllItems.call(mockExecuteFunctions, 'DescribeLoadBalancersResponse.DescribeLoadBalancersResult.LoadBalancerDescriptions', 'elasticloadbalancing', 'POST', '/describe-load-balancers')).rejects.toThrow('LoadBalancerNotFound: The specified load balancer does not exist');
            });
        });
    });
});
