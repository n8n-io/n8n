    ...
    describe('awsApiRequest', () => {
        describe('Successful Requests', () => {
            it('should make successful API request with basic parameters', async () => {
                ...
                const result = await awsApiRequest.call(mockExecuteFunctions, 's3', 'GET', '/test-path', undefined, { 'Content-Type': 'application/json' });
            ...
            it('should handle lambda service with raw body', async () => {
                ...
                mockExecuteFunctions.getCredentials.mockResolvedValue(mockCredentials);
                mockExecuteFunctions.helpers.requestWithAuthentication.mockResolvedValue(mockResponse);
                const result = await awsApiRequest.call(mockExecuteFunctions, 'lambda', 'POST', '/2015-03-31/functions/test/invocations', testBody);
                expect(result).toEqual(mockResponse);
                expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('aws', {
                    qs: {
                        service: 'lambda',
            ...
            it('should stringify body for non-lambda services', async () => {
                ...
                mockExecuteFunctions.getCredentials.mockResolvedValue(mockCredentials);
                mockExecuteFunctions.helpers.requestWithAuthentication.mockResolvedValue(mockResponse);
                await awsApiRequest.call(mockExecuteFunctions, 's3', 'PUT', '/bucket/object', testBody);
                expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('aws', expect.objectContaining({
            ...
            it('should work with ILoadOptionsFunctions context', async () => {
                ...
                const result = await awsApiRequest.call(mockLoadOptionsFunctions, 'ec2', 'GET', '/instances');
                expect(result).toEqual(mockResponse);
            });
            it('should work with IWebhookFunctions context', async () => {
                ...
                const result = await awsApiRequest.call(mockWebhookFunctions, 'apigateway', 'GET', '/webhooks');
                expect(result).toEqual(mockResponse);
            });
        ...
        describe('Error Handling', () => {
            it('should handle credential retrieval errors', async () => {
                ...
                await expect(awsApiRequest.call(mockExecuteFunctions, 's3', 'GET', '/test-path')).rejects.toThrow('Failed to get AWS credentials');
                expect(mockExecuteFunctions.getCredentials).toHaveBeenCalledWith('aws');
            });
            it('should wrap API errors in NodeApiError', async () => {
                ...
                await expect(awsApiRequest.call(mockExecuteFunctions, 's3', 'GET', '/test-path')).rejects.toThrow(NodeApiError);
            ...
            it('should handle authentication errors', async () => {
                ...
                mockExecuteFunctions.getCredentials.mockResolvedValue(mockCredentials);
                mockExecuteFunctions.helpers.requestWithAuthentication.mockRejectedValue(authError);
                await expect(awsApiRequest.call(mockExecuteFunctions, 's3', 'GET', '/test-path')).rejects.toThrow(NodeApiError);
            });
        });
        describe('Edge Cases', () => {
    ...
    describe('awsApiRequestREST', () => {
        describe('JSON Response Handling', () => {
            it('should parse valid JSON response', async () => {
                ...
                const result = await awsApiRequestREST.call(mockExecuteFunctions, 'lambda', 'POST', '/2015-03-31/functions/test/invocations', '{"test": "data"}');
            ...
            it('should return raw response when JSON parsing fails', async () => {
                ...
                mockExecuteFunctions.getCredentials.mockResolvedValue(mockCredentials);
                mockExecuteFunctions.helpers.requestWithAuthentication.mockResolvedValue(invalidJsonResponse);
                const result = await awsApiRequestREST.call(mockExecuteFunctions, 's3', 'GET', '/bucket/object.txt');
            ...
            it('should handle empty string response', async () => {
                ...
                const emptyResponse = '';
                mockExecuteFunctions.getCredentials.mockResolvedValue(mockCredentials);
                mockExecuteFunctions.helpers.requestWithAuthentication.mockResolvedValue(emptyResponse);
                const result = await awsApiRequestREST.call(mockExecuteFunctions, 'lambda', 'POST', '/test');
            ...
            it('should handle null response', async () => {
                ...
                mockExecuteFunctions.getCredentials.mockResolvedValue(mockCredentials);
                mockExecuteFunctions.helpers.requestWithAuthentication.mockResolvedValue(nullResponse);
                const result = await awsApiRequestREST.call(mockExecuteFunctions, 's3', 'DELETE', '/bucket');
            ...
            it('should handle number response', async () => {
                ...
                mockExecuteFunctions.getCredentials.mockResolvedValue(mockCredentials);
                mockExecuteFunctions.helpers.requestWithAuthentication.mockResolvedValue(numberResponse);
                const result = await awsApiRequestREST.call(mockExecuteFunctions, 'cloudwatch', 'GET', '/metrics');
            ...
            it('should handle boolean response', async () => {
                ...
                mockExecuteFunctions.getCredentials.mockResolvedValue(mockCredentials);
                mockExecuteFunctions.helpers.requestWithAuthentication.mockResolvedValue(booleanResponse);
                const result = await awsApiRequestREST.call(mockExecuteFunctions, 's3', 'HEAD', '/bucket/exists');
            ...
            it('should work with ILoadOptionsFunctions context', async () => {
                ...
                mockLoadOptionsFunctions.getCredentials.mockResolvedValue(mockCredentials);
                mockLoadOptionsFunctions.helpers.requestWithAuthentication.mockResolvedValue(jsonResponse);
                mockLoadOptionsFunctions.getNode.mockReturnValue({
                    id: 'load-node',
                    name: 'Load Node',
                    type: 'n8n-nodes-base.aws',
                ...
                const result = await awsApiRequestREST.call(mockLoadOptionsFunctions, 'ec2', 'GET', '/instances');
        ...
        describe('Error Propagation', () => {
            it('should propagate errors from awsApiRequest', async () => {
                ...
                await expect(awsApiRequestREST.call(mockExecuteFunctions, 's3', 'GET', '/test')).rejects.toThrow('API request failed');
            });
            it('should handle malformed JSON gracefully', async () => {
                const mockCredentials = { region: 'us-east-1' };
                const malformedJson = '{"incomplete": json';
                mockExecuteFunctions.getCredentials.mockResolvedValue(mockCredentials);
                mockExecuteFunctions.helpers.requestWithAuthentication.mockResolvedValue(malformedJson);
                const result = await awsApiRequestREST.call(mockExecuteFunctions, 'lambda', 'POST', '/test');
    ...
    describe('awsApiRequestSOAP', () => {
        describe('XML Response Handling', () => {
            it('should parse valid XML response', async () => {
                ...
                const result = await awsApiRequestSOAP.call(mockExecuteFunctions, 'ses', 'POST', '/send-email', '<email>test</email>');
            ...
            it('should return raw response when XML parsing fails', async () => {
                ...
                mockExecuteFunctions.getCredentials.mockResolvedValue(mockCredentials);
                mockExecuteFunctions.helpers.requestWithAuthentication.mockResolvedValue(invalidXmlResponse);
                const result = await awsApiRequestSOAP.call(mockExecuteFunctions, 'ses', 'POST', '/send-email');
            ...
            it('should handle complex nested XML with actual parsing', async () => {
                const mockCredentials = { region: 'us-east-1' };
                const complexXml = '<ListQueuesResult><QueueUrl>url1</QueueUrl><QueueUrl>url2</QueueUrl></ListQueuesResult>';
                mockExecuteFunctions.getCredentials.mockResolvedValue(mockCredentials);
                mockExecuteFunctions.helpers.requestWithAuthentication.mockResolvedValue(complexXml);
                const result = await awsApiRequestSOAP.call(mockExecuteFunctions, 'sqs', 'GET', '/list-queues');
                expect(result).toEqual({
                    ListQueuesResult: {
                        QueueUrl: ['url1', 'url2'],
                    },
            ...
            it('should work with different function contexts', async () => {
                ...
                mockLoadOptionsFunctions.getCredentials.mockResolvedValue(mockCredentials);
                mockLoadOptionsFunctions.helpers.requestWithAuthentication.mockResolvedValue(xmlResponse);
                mockLoadOptionsFunctions.getNode.mockReturnValue({
                    id: 'load-node',
                    name: 'Load Node',
                    type: 'n8n-nodes-base.aws',
                ...
                const result = await awsApiRequestSOAP.call(mockLoadOptionsFunctions, 'ses', 'GET', '/options');
                expect(result).toEqual({
                    options: {
                        item: ['opt1', 'opt2'],
                    },
            ...
            it('should propagate errors from awsApiRequest', async () => {
                ...
                mockExecuteFunctions.getCredentials.mockRejectedValue(apiError);
                await expect(awsApiRequestSOAP.call(mockExecuteFunctions, 'ses', 'POST', '/test')).rejects.toThrow('SOAP API request failed');
            });
        });
    });
});
