...
describe('AwsLambda', () => {
    ...
    beforeEach(() => {
        ...
        mockExecuteFunctions.helpers.constructExecutionMetaData.mockImplementation((data, meta) => {
            return [
                {
                    ...data[0],
    ...
    describe('Load Options Methods', () => {
        describe('getFunctions', () => {
            it('should load functions without pagination', async () => {
                ...
                expect(awsApiRequestRESTSpy).toHaveBeenCalledWith('lambda', 'GET', '/2015-03-31/functions/');
            });
            it('should handle paginated function list', async () => {
                ...
                expect(awsApiRequestRESTSpy).toHaveBeenNthCalledWith(2, 'lambda', 'GET', '/2015-03-31/functions/?MaxItems=50&Marker=marker123');
            });
            it('should handle multiple pages with NextMarker', async () => {
                ...
                expect(awsApiRequestRESTSpy).toHaveBeenNthCalledWith(3, 'lambda', 'GET', '/2015-03-31/functions/?MaxItems=50&Marker=marker2');
            });
    ...
    describe('Execute Method', () => {
        ...
        describe('Successful Lambda Invocation', () => {
            it('should invoke lambda function with RequestResponse type', async () => {
                ...
                expect(awsApiRequestRESTSpy).toHaveBeenCalledWith('lambda', 'POST', '/2015-03-31/functions/arn:aws:lambda:us-east-1:123456789012:function:test-function/invocations?Qualifier=$LATEST', JSON.stringify({ test: 'data' }), {
            ...
            it('should invoke lambda function with Event type', async () => {
                ...
                expect(awsApiRequestRESTSpy).toHaveBeenCalledWith('lambda', 'POST', '/2015-03-31/functions/test-function/invocations?Qualifier=PROD', JSON.stringify({ async: true }), {
            ...
            it('should handle empty payload', async () => {
                ...
                expect(awsApiRequestRESTSpy).toHaveBeenCalledWith('lambda', 'POST', '/2015-03-31/functions/test-function/invocations?Qualifier=$LATEST', '', {
                    'X-Amz-Invocation-Type': 'RequestResponse',
                    'Content-Type': 'application/x-amz-json-1.0',
                });
            });
        ...
        describe('Lambda Function Errors', () => {
            ...
            it('should throw API errors when continueOnFail is false', async () => {
                ...
                awsApiRequestRESTSpy.mockRejectedValue(apiError);
                await expect(node.execute.call(mockExecuteFunctions)).rejects.toThrow('Invalid function name');
            });
        ...
        describe('Different Qualifier Values', () => {
            it('should handle custom version qualifier', async () => {
                ...
                expect(awsApiRequestRESTSpy).toHaveBeenCalledWith('lambda', 'POST', '/2015-03-31/functions/test-function/invocations?Qualifier=1', JSON.stringify({ version: 1 }), {
                    'X-Amz-Invocation-Type': 'RequestResponse',
                    'Content-Type': 'application/x-amz-json-1.0',
                });
            });
            it('should handle alias qualifier', async () => {
                ...
                awsApiRequestRESTSpy.mockResolvedValue(mockResponse);
                await node.execute.call(mockExecuteFunctions);
                expect(awsApiRequestRESTSpy).toHaveBeenCalledWith('lambda', 'POST', '/2015-03-31/functions/test-function/invocations?Qualifier=PROD', JSON.stringify({ environment: 'production' }), {
                    'X-Amz-Invocation-Type': 'RequestResponse',
                    'Content-Type': 'application/x-amz-json-1.0',
                });
            });
        ...
        describe('Payload Handling', () => {
            it('should handle complex JSON payload', async () => {
                ...
                awsApiRequestRESTSpy.mockResolvedValue(mockResponse);
                await node.execute.call(mockExecuteFunctions);
                expect(awsApiRequestRESTSpy).toHaveBeenCalledWith('lambda', 'POST', '/2015-03-31/functions/test-function/invocations?Qualifier=$LATEST', JSON.stringify(complexPayload), {
                    'X-Amz-Invocation-Type': 'RequestResponse',
                    'Content-Type': 'application/x-amz-json-1.0',
                });
            });
            it('should handle string payload', async () => {
                ...
                awsApiRequestRESTSpy.mockResolvedValue(mockResponse);
                await node.execute.call(mockExecuteFunctions);
                expect(awsApiRequestRESTSpy).toHaveBeenCalledWith('lambda', 'POST', '/2015-03-31/functions/test-function/invocations?Qualifier=$LATEST', 'plain string payload', {
                    'X-Amz-Invocation-Type': 'RequestResponse',
                    'Content-Type': 'application/x-amz-json-1.0',
                });
            });
...
//# sourceMappingURL=AwsLambda.node.test.js.map