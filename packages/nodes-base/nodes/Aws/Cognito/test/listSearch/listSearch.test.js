...
import { searchUsers, searchGroups, searchUserPools, searchGroupsForUser, } from '../../methods/listSearch';
...
describe('AWS Cognito Functions', () => {
    describe('searchUsers', () => {
        it('should return user results when users are found', async () => {
            ...
            expect(awsApiRequest).toHaveBeenCalledWith('POST', 'ListUsers', expect.stringContaining('UserPoolId'));
        });
        it('should throw an error if UserPoolId is missing', async () => {
            ...
            await expect(searchUsers.call(mockContext)).rejects.toThrow('User Pool ID is required to search users');
        });
    });
    describe('searchGroups', () => {
        it('should return group results when groups are found', async () => {
            ...
            expect(result).toEqual(expectedResult);
            expect(awsApiRequest).toHaveBeenCalledWith('POST', 'ListGroups', expect.stringContaining('UserPoolId'));
        });
        it('should throw an error if UserPoolId is missing', async () => {
            const mockContext = {
    ...
    describe('searchUserPools', () => {
        it('should return user pool results when user pools are found', async () => {
            ...
            expect(result).toEqual(expectedResult);
            expect(awsApiRequest).toHaveBeenCalledWith('POST', 'ListUserPools', expect.stringContaining('Limit'));
        });
    });
    describe('searchGroupsForUser', () => {
        ...
        it('should handle empty groups response', async () => {
            awsApiRequestAllItems.mockResolvedValueOnce([]);
        ...
        it('should return filtered and sorted group list', async () => {
            ...
            awsApiRequestAllItems.mockResolvedValueOnce(mockGroups);
