...
describe('AWS Cognito transport - awsApiRequest', () => {
    ...
    const warn = vi.fn();
    const createContext = (authentication) => ({
    ...
    it('should use the aws credential when authentication is iam', async () => {
        ...
        expect(httpRequestWithAuthentication).toHaveBeenCalledWith('aws', expect.objectContaining({
            method: 'POST',
            body: '{}',
    ...
    it('should use the awsAssumeRole credential when authentication is assumeRole', async () => {
        ...
        expect(httpRequestWithAuthentication).toHaveBeenCalledWith('awsAssumeRole', expect.objectContaining({
            qs: { service: 'cognito-idp', _region: 'eu-central-1' },
        }));
    });
    ...
    it('should resolve _region from the active credential', async () => {
        ...
        const context = createContext('assumeRole');
        await awsApiRequest.call(context, 'POST', 'ListUsers', '{}');
        expect(httpRequestWithAuthentication).toHaveBeenCalledWith('awsAssumeRole', expect.objectContaining({ qs: { service: 'cognito-idp', _region: 'us-east-1' } }));
    });
});
