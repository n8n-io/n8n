...
import { getUserPool, validateArn, simplifyUserPool, preSendUserFields, preSendAttributes, preSendDesiredDeliveryMediums, getUsersInGroup, simplifyUser, getUserNameFromExistingUsers, } from '../../helpers/utils';
...
describe('AWS Cognito - Helpers functions', () => {
    ...
    describe('getUserPool', () => {
        ...
        it('should throw an error if user pool ID is missing', async () => {
            await expect(getUserPool.call(loadOptionsFunctions, '')).rejects.toThrowError(NodeOperationError);
        });
        it('should throw an error if user pool is not found', async () => {
            const userPoolId = 'invalid-user-pool-id';
            awsApiRequest.mockResolvedValue({});
            await expect(getUserPool.call(loadOptionsFunctions, userPoolId)).rejects.toThrowError(NodeOperationError);
        });
    });
    describe('getUsersInGroup', () => {
        it('should throw an error if UserPoolId is missing', async () => {
            ...
            await expect(getUsersInGroup.call(loadOptionsFunctions, 'groupName', '')).rejects.toThrowError(NodeOperationError);
        });
        it('should return an empty list if no users are found', async () => {
            loadOptionsFunctions.getNodeParameter.mockReturnValue('userPoolId');
            awsApiRequestAllItems.mockResolvedValue([]);
        ...
        it('should return users correctly', async () => {
            ...
            awsApiRequestAllItems.mockResolvedValue(mockUsers);
            const result = await getUsersInGroup.call(loadOptionsFunctions, 'groupName', 'userPoolId');
            expect(result).toEqual([
                {
                    Username: 'user1',
    ...
    describe('validateArn', () => {
        it('should validate the ARN format', async () => {
            loadOptionsFunctions.getNodeParameter.mockImplementation((name) => {
        ...
        it('should throw an error if the ARN format is invalid', async () => {
            ...
            await expect(validateArn.call(loadOptionsFunctions, requestOptions)).rejects.toThrowError(NodeApiError);
        });
    });
    ...
    describe('getUserNameFromExistingUsers', () => {
        ...
        it('should return the userName if email or phone is used for authentication', async () => {
            ...
            const result = await getUserNameFromExistingUsers.call(loadOptionsFunctions, userName, userPoolId, isEmailOrPhone);
        ...
        it('should return the username from ListUsers API when it is not email or phone authentication', async () => {
            ...
            awsApiRequest.mockResolvedValue(mockApiResponse);
            const result = await getUserNameFromExistingUsers.call(loadOptionsFunctions, userName, userPoolId, isEmailOrPhone);
    ...
    describe('preSendUserFields', () => {
        ...
        it('should return the request body without a username when operation is "update" and no user is found', async () => {
            ...
            loadOptionsFunctions.getNodeParameter.mockImplementationOnce((name) => {
                if (name === 'operation')
                    return 'update';
                if (name === 'user')
    ...
    describe('preSendAttributes', () => {
        ...
        it('should throw an error if no user attributes are provided (update operation)', async () => {
            loadOptionsFunctions.getNodeParameter.mockImplementation((name) => {
                if (name === 'operation')
                    return 'update';
                if (name === 'userAttributes.attributes')
                    return [];
                return undefined;
            });
    ...
    describe('preSendDesiredDeliveryMediums', () => {
        ...
        it('should not throw an error if EMAIL is selected and email is provided', async () => {
            ...
            await expect(preSendDesiredDeliveryMediums.call(loadOptionsFunctions, requestOptions)).resolves.toEqual(requestOptions);
        });
        it('should throw an error if EMAIL is selected but email is missing', async () => {
            ...
            await expect(preSendDesiredDeliveryMediums.call(loadOptionsFunctions, requestOptions)).rejects.toThrowError('Missing required "email" attribute');
        });
        ...
        it('should throw an error if SMS is selected but phone number is missing', async () => {
            ...
            await expect(preSendDesiredDeliveryMediums.call(loadOptionsFunctions, requestOptions)).rejects.toThrowError('Missing required "phone_number" attribute');
        });
