...
describe('AWS Textract Node', () => {
    ...
    beforeEach(() => {
        ...
        executeFunctionsMock.helpers.returnJsonArray.mockImplementation((data) => Array.isArray(data) ? data.map((item) => ({ json: item })) : [{ json: data }]);
    });
    afterEach(() => {
        nock.cleanAll();
    });
    describe('analyzeExpense operation', () => {
        ...
        it('should process binary image data and return simplified response', async () => {
            ...
            expect(awsApiRequestSpy).toHaveBeenCalledWith('textract', 'POST', '', JSON.stringify({
                Document: {
        ...
        it('should handle errors and continue on fail', async () => {
            ...
            executeFunctionsMock.helpers.getBinaryDataBuffer.mockRejectedValue(new Error('Binary data not found'));
            const result = await node.execute.call(executeFunctionsMock);
        ...
        it('should throw error when continueOnFail is false', async () => {
            executeFunctionsMock.continueOnFail.mockReturnValue(false);
            executeFunctionsMock.helpers.getBinaryDataBuffer.mockRejectedValue(new Error('Binary data not found'));
            await expect(node.execute.call(executeFunctionsMock)).rejects.toThrow('Binary data not found');
        });
...
//# sourceMappingURL=AwsTextract.node.test.js.map