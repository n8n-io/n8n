...
describe('AWS S3 V2 Node - File Download', () => {
    ...
    describe('successful file download', () => {
        ...
        it('should successfully download a file and return binary data', async () => {
            ...
            expect(awsApiRequestRESTSpy).toHaveBeenNthCalledWith(2, 'test-bucket.s3', 'GET', '/path/to/test.txt', '', {}, {}, { encoding: null, resolveWithFullResponse: true }, 'eu-central-1');
            expect(executeFunctionsMock.helpers.prepareBinaryData).toHaveBeenCalledWith(expect.any(Buffer), 'test.txt', 'text/plain');
        ...
        it('should handle bucket names with dots correctly', async () => {
            ...
            expect(awsApiRequestRESTSpy).toHaveBeenNthCalledWith(2, 's3', 'GET', '/test.bucket.com/path/to/test.txt', '', {}, {}, { encoding: null, resolveWithFullResponse: true }, 'eu-central-1');
        });
        it('should extract filename correctly from different file key formats', async () => {
            ...
            for (const testCase of testCases) {
                ...
                await node.execute.call(executeFunctionsMock);
                expect(executeFunctionsMock.helpers.prepareBinaryData).toHaveBeenCalledWith(expect.any(Buffer), testCase.expectedFileName, 'text/plain');
            }
        });
    });
    describe('error handling', () => {
        ...
        it('should throw error when trying to download a directory', async () => {
            ...
            await expect(node.execute.call(executeFunctionsMock)).rejects.toThrow('Downloading a whole directory is not yet supported, please provide a file key');
        });
    });
    describe('continueOnFail logic', () => {
        ...
        it('should continue execution and return error data when continueOnFail is true', async () => {
            ...
            expect(executeFunctionsMock.helpers.constructExecutionMetaData).toHaveBeenCalledWith([{ json: { error: 'AWS API Error' } }], { itemData: { item: 0 } });
        });
...
describe('AWS S3 V2 Node - Bucket Search', () => {
    const executeFunctionsMock = mockDeep();
    let awsApiRequestRESTSpy;
    let awsApiRequestRESTAllItemsSpy;
    let node;
    const mockContents = [
    ...
    beforeEach(() => {
        ...
        executeFunctionsMock.getNode.mockReturnValue({ typeVersion: 2 });
    ...
    function setupSearchParams(overrides) {
        const { returnAll = false, delimiter = '', includeCommonPrefixes = false, limit = 100, } = overrides;
    ...
    it('should call awsApiRequestRESTAllItems with CommonPrefixes path when returnAll and includeCommonPrefixes are true', async () => {
        ...
        await node.execute.call(executeFunctionsMock);
        expect(awsApiRequestRESTAllItemsSpy).toHaveBeenCalledWith('ListBucketResult.CommonPrefixes', expect.any(String), 'GET', expect.any(String), '', expect.any(Object), {}, {}, expect.any(String));
    });
    it('should call awsApiRequestRESTAllItems with Contents path when returnAll is true and includeCommonPrefixes is false', async () => {
        ...
        await node.execute.call(executeFunctionsMock);
        expect(awsApiRequestRESTAllItemsSpy).toHaveBeenCalledWith('ListBucketResult.Contents', expect.any(String), 'GET', expect.any(String), '', expect.any(Object), {}, {}, expect.any(String));
    });
...
//# sourceMappingURL=AwsS3.node.operation.test.js.map