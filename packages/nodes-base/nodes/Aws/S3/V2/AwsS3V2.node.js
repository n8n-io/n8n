            ...
            try {
                if (resource === 'bucket') {
                    ...
                    if (operation === 'create') {
                        ...
                        responseData = await awsApiRequestREST.call(this, `${name}.s3`, 'PUT', '', data, qs, headers);
                    ...
                    if (operation === 'delete') {
                        const name = this.getNodeParameter('name', i);
                        responseData = await awsApiRequestREST.call(this, `${name}.s3`, 'DELETE', '', '', {}, headers);
                        const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray({ success: true }), { itemData: { item: i } });
                        returnData.push(...executionData);
                    }
                    ...
                    if (operation === 'getAll') {
                        ...
                        if (returnAll) {
                            responseData = await awsApiRequestRESTAllItems.call(this, 'ListAllMyBucketsResult.Buckets.Bucket', 's3', 'GET', '');
                        }
                        else {
                            qs.limit = this.getNodeParameter('limit', 0);
                            responseData = await awsApiRequestRESTAllItems.call(this, 'ListAllMyBucketsResult.Buckets.Bucket', 's3', 'GET', '', '', qs);
                    ...
                    if (operation === 'search') {
                        ...
                        const includeCommonPrefixes = additionalFields.includeCommonPrefixes;
                        const listPropertyName = additionalFields.delimiter && includeCommonPrefixes
                            ? 'ListBucketResult.CommonPrefixes'
                            : 'ListBucketResult.Contents';
                        if (returnAll) {
                            responseData = await awsApiRequestRESTAllItems.call(this, listPropertyName, servicePath, 'GET', basePath, '', qs, {}, {}, region);
                        }
                        else {
                            qs['max-keys'] = this.getNodeParameter('limit', 0);
                            const rawResponse = await awsApiRequestREST.call(this, servicePath, 'GET', basePath, '', qs, {}, {}, region);
                            const extracted = additionalFields.delimiter && includeCommonPrefixes
                ...
                if (resource === 'file') {
                    ...
                    if (operation === 'upload') {
                        ...
                        if (additionalFields.storageClass) {
                            multipartHeaders['x-amz-storage-class'] = snakeCase(additionalFields.storageClass).toUpperCase();
                        }
                        if (additionalFields.acl) {
                            multipartHeaders['x-amz-acl'] = kebabCase(additionalFields.acl);
                        }
                        if (additionalFields.grantFullControl) {
                        ...
                        if (isBinaryData) {
                            ...
                            if (binaryPropertyData.id) {
                                ...
                                const createMultiPartUpload = await awsApiRequestREST.call(this, servicePath, 'POST', `${path}?uploads`, body, qs, { ...neededHeaders, ...multipartHeaders }, {}, region);
                                ...
                                for await (const chunk of uploadData) {
                                    ...
                                    try {
                                        ...
                                        try {
                                            await awsApiRequestREST.call(this, servicePath, 'DELETE', `${path}?uploadId=${uploadId}`);
                                        }
                                        catch (err) {
                            ...
                            const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(responseData ?? { success: true }), { itemData: { item: i } });
                            returnData.push(...executionData);
                        }
                        else {
