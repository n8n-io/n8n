...
vi.mock('../../../../credentials/common/aws/utils', async () => {
    const actual = await vi.importActual('../../../../credentials/common/aws/utils');
...
describe('AWS Transcribe Generic Functions', () => {
    ...
    describe('awsApiRequest region validation', () => {
        ...
        it.each(['us-east-1', 'eu-west-1', 'ap-southeast-2'])('accepts the supported region %s and proceeds to sign and send', async (region) => {
        ...
        it.each([
            ...
            const { context, helpers } = buildContext(region);
            await expect(awsApiRequest.call(context, 'transcribe', 'POST', '/')).rejects.toThrow(UserError);
            await expect(awsApiRequest.call(context, 'transcribe', 'POST', '/')).rejects.toThrow('Unsupported AWS region');
    ...
    describe('awsApiRequest authentication selection', () => {
        ...
        const buildContext = (authentication, credentials) => {
        ...
        it('signs with the static keys when authentication is iam', async () => {
            ...
            expect(mockSign).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ accessKeyId: 'AKIA-test', secretAccessKey: 'secret-test' }));
            expect(helpers.request).toHaveBeenCalledTimes(1);
        });
        ...
        it('wraps an assumeRole failure in a NodeApiError without signing or sending', async () => {
            ...
            const { context, helpers } = buildContext('assumeRole', assumeRoleCredentials);
            await expect(awsApiRequest.call(context, 'transcribe', 'POST', '/')).rejects.toThrow(NodeApiError);
            expect(mockSign).not.toHaveBeenCalled();
            expect(helpers.request).not.toHaveBeenCalled();
        });
    });
