        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                this.getNodeParameter('options.messageAttributes.string', i, []).forEach((attribute) => {
                    attributeCount++;
                    params.push(`MessageAttribute.${attributeCount}.Name=${encodeURIComponent(attribute.name)}`);
                    params.push(`MessageAttribute.${attributeCount}.Value.StringValue=${encodeURIComponent(attribute.value)}`);
                ...
                this.getNodeParameter('options.messageAttributes.binary', i, []).forEach((attribute) => {
                    ...
                    params.push(`MessageAttribute.${attributeCount}.Name=${encodeURIComponent(attribute.name)}`);
                    params.push(`MessageAttribute.${attributeCount}.Value.BinaryValue=${encodeURIComponent(binaryData.data)}`);
