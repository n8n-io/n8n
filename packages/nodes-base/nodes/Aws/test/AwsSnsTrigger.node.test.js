...
describe('AwsSnsTrigger', () => {
    const notification = {
        ...
        SigningCertURL: 'https://sns.us-east-1.amazonaws.com/SimpleNotificationService-1234567890abcdef1234567890abcdef.pem',
    };
    const verifySignatureMock = verifySignature;
    const awsApiRequestSOAPMock = awsApiRequestSOAP;
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    it('confirms the subscription after validating the request signature', async () => {
        ...
        expect(awsApiRequestSOAPMock).toHaveBeenCalledWith('sns', 'GET', '/?Action=ConfirmSubscription&TopicArn=arn:aws:sns:us-east-1:123456789012:MyTopic&Token=token&Version=2010-03-31');
        expect(result).toEqual({ noWebhookResponse: true });
    });
...
function createWebhookFunctions(body) {
    const response = {
...
//# sourceMappingURL=AwsSnsTrigger.node.test.js.map