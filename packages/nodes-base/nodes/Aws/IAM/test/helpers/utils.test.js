...
import { preprocessTags, deleteGroupMembers, validatePath, validateUserPath, removeUserFromGroups, findUsersForGroup, simplifyGetGroupsResponse, simplifyGetAllGroupsResponse, simplifyGetAllUsersResponse, validateName, validatePermissionsBoundary, encodeBodyAsFormUrlEncoded, } from '../../helpers/utils';
...
describe('AWS IAM - Helper Functions', () => {
    ...
    beforeEach(() => {
        mockNode = {
            ...
            getNode: vi.fn(),
            helpers: {
                returnJsonArray: vi.fn((input) => input.map((i) => ({ json: i }))),
            },
        };
    });
    describe('encodeBodyAsFormUrlEncoded', () => {
        it('should encode the body as application/x-www-form-urlencoded', async () => {
            ...
            expect(result.body).toBe('client_id=myClient&client_secret=mySecret&grant_type=client_credentials');
        });
    ...
    describe('findUsersForGroup', () => {
        it('should return users for a valid groupName', async () => {
            ...
            awsApiRequest.mockResolvedValue(mockResponse);
    ...
    describe('preprocessTags', () => {
        it('should preprocess tags correctly into request body', async () => {
            ...
            expect(result.body).toBe('Tags.member.1.Key=Department&Tags.member.1.Value=Engineering&Tags.member.2.Key=Role&Tags.member.2.Value=Developer');
        });
        it('should throw error if a tag is missing a key', async () => {
            ...
            const requestOptions = { body: '', headers: {}, url: '' };
            await expect(preprocessTags.call(mockNode, requestOptions)).rejects.toThrow(NodeOperationError);
            await expect(preprocessTags.call(mockNode, requestOptions)).rejects.toThrow("Tag at position 1 is missing 'Key'. Both 'Key' and 'Value' are required.");
        });
        it('should throw error if a tag is missing a value', async () => {
            ...
            const requestOptions = { body: '', headers: {}, url: '' };
            await expect(preprocessTags.call(mockNode, requestOptions)).rejects.toThrow(NodeOperationError);
            await expect(preprocessTags.call(mockNode, requestOptions)).rejects.toThrow("Tag at position 1 is missing 'Value'. Both 'Key' and 'Value' are required.");
        });
    });
    describe('deleteGroupMembers', () => {
        it('should attempt to remove users from a group', async () => {
            mockNode.getNodeParameter.mockImplementation((param) => {
                if (param === 'group') {
                    return 'groupName';
                }
                return null;
            ...
            awsApiRequest.mockResolvedValue(mockUsers);
            ...
            expect(awsApiRequest).toHaveBeenCalledWith(expect.objectContaining({
                method: 'POST',
    ...
    describe('validatePath', () => {
        it('should throw an error for invalid path length', async () => {
            mockNode.getNodeParameter.mockReturnValue('');
            await expect(validatePath.call(mockNode, { headers: {}, url: '' })).rejects.toThrowError(NodeOperationError);
        });
        it('should throw an error for invalid path format', async () => {
            mockNode.getNodeParameter.mockReturnValue('/invalidPath');
            await expect(validatePath.call(mockNode, { url: '' })).rejects.toThrowError(NodeOperationError);
        });
    ...
    describe('validateUserPath', () => {
        it('should throw an error for invalid path prefix', async () => {
            ...
            awsApiRequest.mockResolvedValue(mockResponse);
            await expect(validateUserPath.call(mockNode, { headers: {}, url: '' })).rejects.toThrowError(NodeOperationError);
        });
    ...
    describe('validateName', () => {
        ...
        it('should throw an error if userName contains invalid characters', async () => {
            ...
            const execution = validateName.call(mockNode, requestOptions);
            await expect(execution).rejects.toThrow(NodeOperationError);
            await expect(execution).rejects.toThrow('User name can have up to 64 characters. Valid characters: letters, numbers, hyphens (-), and underscores (_).');
        });
        ...
        it('should throw an error if groupName contains invalid characters', async () => {
            ...
            const execution = validateName.call(mockNode, requestOptions);
            await expect(execution).rejects.toThrow(NodeOperationError);
            await expect(execution).rejects.toThrow('Group name can have up to 128 characters. Valid characters: letters, numbers, hyphens (-), and underscores (_).');
        });
    ...
    describe('validatePermissionsBoundary', () => {
        ...
        it('should throw an error for invalid permissions boundary format', async () => {
            ...
            await expect(validatePermissionsBoundary.call(mockNode, { body: {}, url: '', headers: {} })).rejects.toThrow(NodeOperationError);
        });
    });
    ...
    describe('simplifyGetAllGroupsResponse', () => {
        ...
        it('should return groups with users if "includeUsers" is true', async () => {
            ...
            const mockUsers = [{ UserName: 'user1' }];
            awsApiRequest.mockResolvedValueOnce({
    ...
    describe('removeUserFromGroups', () => {
        it('should remove a user from all groups', async () => {
            ...
            awsApiRequest.mockResolvedValue(mockUserGroups);
            const requestOptions = { headers: {}, url: '' };
