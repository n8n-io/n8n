...
describe('GenericFunctions - handlePagination', () => {
	...
	test('should stop fetching when the limit is reached and returnAll is false', async () => {
		...
		expect(result).toEqual([{ json: { id: 1 } }, { json: { id: 2 } }]); // Expect only 2 items due to the limit
	});
	...
	test('should handle resource-specific token key', async () => {
		// Mock responses for resource-specific pagination with NextToken
		mockMakeRoutingRequest.mockResolvedValueOnce([
	...
	test('should handle a single page with a single response', async () => {
		// Mock a single response with no pagination token
		mockMakeRoutingRequest.mockResolvedValueOnce([{ json: { id: 1, PaginationToken: undefined } }]);

		// Mock parameters
		mockGetNodeParameter.mockReturnValueOnce(false); // returnAll = false
		mockGetNodeParameter.mockReturnValueOnce(5); // limit = 5
	...
	test('should throw an error when the response format is invalid', async () => {
		// Mock an invalid response (empty json object)
		mockMakeRoutingRequest.mockResolvedValueOnce([{ json: {} }]);

		// Mock parameters
		mockGetNodeParameter.mockReturnValueOnce(2); // limit = 2

		await expect(handlePagination.call(mockContext, resultOptions)).rejects.toThrowError(
