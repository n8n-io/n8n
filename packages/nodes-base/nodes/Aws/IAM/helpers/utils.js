...
export async function encodeBodyAsFormUrlEncoded(requestOptions) {
    if (requestOptions.body) {
        requestOptions.body = new URLSearchParams(requestOptions.body).toString();
    }
    return requestOptions;
}
export async function findUsersForGroup(groupName) {
    const options = {
        method: 'POST',
        url: '',
...
export async function simplifyGetGroupsResponse(_, response) {
...
export async function simplifyGetAllGroupsResponse(items, response) {
    const includeUsers = this.getNodeParameter('includeUsers', false);
    const responseBody = response.body;
...
export async function simplifyGetAllUsersResponse(_items, response) {
    if (!response.body) {
        return [];
    }
    const responseBody = response.body;
...
export async function deleteGroupMembers(requestOptions) {
...
export async function validatePath(requestOptions) {
    const path = this.getNodeParameter('additionalFields.path');
    if (path.length < 1 || path.length > 512) {
        throw new NodeOperationError(this.getNode(), 'The "Path" parameter must be between 1 and 512 characters long.');
    }
    const validPathRegex = /^\/[\u0021-\u007E]*\/$/;
    if (!validPathRegex.test(path) && path !== '/') {
        throw new NodeOperationError(this.getNode(), 'Ensure the path is structured correctly, e.g. /division_abc/subdivision_xyz/');
    }
    return requestOptions;
}
export async function validateUserPath(requestOptions) {
    ...
    if (!users || users.length === 0) {
        throw new NodeOperationError(this.getNode(), 'No users found. Please adjust the "Path" parameter and try again.');
    }
    ...
    if (!isPathValid) {
        throw new NodeOperationError(this.getNode(), `The "${formattedPrefix}" path was not found in your users. Try entering a different path.`);
    }
    return requestOptions;
}
export async function validateName(requestOptions) {
    ...
    if (/\s/.test(name)) {
        throw new NodeOperationError(this.getNode(), `${capitalizedResource} name should not contain spaces.`);
    }
    if (isInvalid) {
        throw new NodeOperationError(this.getNode(), `${capitalizedResource} name can have up to ${maxLength} characters. Valid characters: letters, numbers, hyphens (-), and underscores (_).`);
    }
    return requestOptions;
}
export async function validatePermissionsBoundary(requestOptions) {
    ...
    if (permissionsBoundary) {
        ...
        if (!arnPattern.test(permissionsBoundary)) {
            throw new NodeOperationError(this.getNode(), 'Permissions boundaries must be provided in ARN format (e.g. arn:aws:iam::123456789012:policy/ExampleBoundaryPolicy). These can be found at the top of the permissions boundary detail page in the IAM dashboard.');
        }
        if (requestOptions.body) {
...
export async function preprocessTags(requestOptions) {
    ...
    tags.forEach((tag, index) => {
        if (!tag.key || !tag.value) {
            throw new NodeOperationError(this.getNode(), `Tag at position ${index + 1} is missing '${!tag.key ? 'Key' : 'Value'}'. Both 'Key' and 'Value' are required.`);
        }
...
export async function removeUserFromGroups(requestOptions) {
