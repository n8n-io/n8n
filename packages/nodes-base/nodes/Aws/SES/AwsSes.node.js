            ...
            try {
                if (resource === 'customVerificationEmail') {
                    if (operation === 'create') {
                        ...
                        responseData = await awsApiRequestSOAP.call(this, 'email', 'POST', '', qs.stringify({
                    ...
                    if (operation === 'send') {
                        ...
                        if (additionalFields.configurationSetName) {
                            params.push(`ConfigurationSetName=${encodeURIComponent(additionalFields.configurationSetName)}`);
                        }
                        responseData = await awsApiRequestSOAP.call(this, 'email', 'POST', '', params.join('&'));
                    ...
                    if (operation === 'update') {
                        ...
                        if (updateFields.FailureRedirectionURL) {
                            params.push(`FailureRedirectionURL=${encodeURIComponent(updateFields.FailureRedirectionURL)}`);
                        }
                        if (updateFields.email) {
                            params.push(`FromEmailAddress=${encodeURIComponent(updateFields.email)}`);
                        }
                        if (updateFields.successRedirectionURL) {
                            params.push(`SuccessRedirectionURL=${encodeURIComponent(updateFields.successRedirectionURL)}`);
                        }
                        if (updateFields.templateContent) {
                            params.push(`TemplateContent=${encodeURIComponent(updateFields.templateContent)}`);
                        }
                        if (updateFields.templateSubject) {
                            params.push(`TemplateSubject=${encodeURIComponent(updateFields.templateSubject)}`);
                        }
                        responseData = await awsApiRequestSOAP.call(this, 'email', 'POST', '', params.join('&'));
                ...
                if (resource === 'email') {
                    if (operation === 'send') {
                        ...
                        if (additionalFields.returnPath) {
                            params.push(`ReturnPath=${encodeURIComponent(additionalFields.returnPath)}`);
                        }
                        if (additionalFields.returnPathArn) {
                            params.push(`ReturnPathArn=${encodeURIComponent(additionalFields.returnPathArn)}`);
                        }
                        if (additionalFields.sourceArn) {
                            params.push(`SourceArn=${encodeURIComponent(additionalFields.sourceArn)}`);
                        }
