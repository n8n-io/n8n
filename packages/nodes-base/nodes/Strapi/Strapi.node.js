...
import { getToken, strapiApiRequest, strapiApiRequestAllItems, validateJSON, } from './GenericFunctions';
                ...
                if (resource === 'entry') {
                    ...
                    if (operation === 'getAll') {
                        ...
                        if (apiVersion === 'v4') {
                            ...
                            // Filter Option
                            if (options.where) {
                                const query = validateJSON(options.where);
                                if (query !== undefined) {
                                    qs.filters = query;
                                }
                                else {
                            ...
                            // Limit Option
                            if (returnAll) {
                                responseData = await strapiApiRequestAllItems.call(this, 'GET', `/${contentType}`, {}, qs, headers, apiVersion);
                            }
                            else {
