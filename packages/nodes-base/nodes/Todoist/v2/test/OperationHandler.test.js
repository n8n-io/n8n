...
import { todoistApiRequest, todoistApiGetAllRequest, todoistSyncRequest, } from '../../GenericFunctions';
...
vi.mock('../../GenericFunctions', () => ({
    ...
    FormatDueDatetime: vi.fn((dateTime) => dateTime),
}));
// Mock uuid
vi.mock('uuid', () => ({
    v4: vi.fn(() => 'mock-uuid-123'),
}));
const mockTodoistApiRequest = todoistApiRequest;
const mockTodoistApiGetAllRequest = todoistApiGetAllRequest;
const mockTodoistSyncRequest = todoistSyncRequest;
// Mock Context interface
const createMockContext = (params = {}) => mock({
    getNodeParameter: vi.fn((key, _idx, defaultValue) => key in params ? params[key] : defaultValue),
    getNode: vi.fn(() => mock({ typeVersion: 2.1 })),
});
    ...
    describe('Task Handlers', () => {
        describe('CreateHandler', () => {
            it('should create a task successfully', async () => {
                ...
                expect(mockTodoistApiRequest).toHaveBeenCalledWith('POST', '/tasks', expect.objectContaining({
                    content: 'Test task',
                    project_id: '123456',
                    description: 'Test description',
                    priority: 3,
        ...
        describe('GetAllHandler', () => {
            it('should get all tasks successfully', async () => {
                ...
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistApiGetAllRequest).toHaveBeenCalledWith(mockCtx, '/tasks', expect.objectContaining({
                    project_id: '123456',
                    label: '789',
                }), 10);
            ...
            it('should use /tasks/filter endpoint when filter is set on api/v1 (node version >= 2.2)', async () => {
                const handler = new GetAllHandler();
                const mockCtx = mock({
                    getNodeParameter: vi.fn((key, _idx, defaultValue) => {
                        const params = {
                            returnAll: false,
                            limit: 5,
                    ...
                    getNode: vi.fn(() => mock({ typeVersion: 2.2 })),
                });
                const mockApiResponse = [{ id: '1', content: 'Task due today' }];
                mockTodoistApiGetAllRequest.mockResolvedValue([...mockApiResponse]);
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistApiGetAllRequest).toHaveBeenCalledWith(mockCtx, '/tasks/filter', expect.objectContaining({ query: 'today' }), 5);
                expect(result).toEqual({ data: mockApiResponse });
            });
            it('should use /tasks endpoint when filter is set on rest/v2 (node version < 2.2)', async () => {
                ...
                mockTodoistApiGetAllRequest.mockResolvedValue([...mockApiResponse]);
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistApiGetAllRequest).toHaveBeenCalledWith(mockCtx, '/tasks', expect.objectContaining({ filter: 'today' }), 5);
                expect(result).toEqual({ data: mockApiResponse });
            });
        });
        ...
        describe('UpdateHandler', () => {
            it('should update a task successfully', async () => {
                ...
                mockTodoistApiRequest.mockResolvedValue(undefined);
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistApiRequest).toHaveBeenCalledWith('POST', '/tasks/123456', expect.objectContaining({
                    content: 'Updated task',
                    description: 'Updated description',
                    priority: 2,
                    labels: ['updated'],
        ...
        describe('MoveHandler', () => {
            it('should move a task successfully', async () => {
                ...
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistSyncRequest).toHaveBeenCalledWith(expect.objectContaining({
                    commands: [
    ...
    describe('Project Handlers', () => {
        describe('ProjectCreateHandler', () => {
            it('should create a project successfully', async () => {
                ...
                mockTodoistApiRequest.mockResolvedValue(expectedResponse);
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistApiRequest).toHaveBeenCalledWith('POST', '/projects', expect.objectContaining({
                    name: 'Test Project',
                    color: 'blue',
                    is_favorite: true,
                    view_style: 'board',
        ...
        describe('ProjectUpdateHandler', () => {
            it('should update a project successfully', async () => {
                ...
                mockTodoistApiRequest.mockResolvedValue(undefined);
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistApiRequest).toHaveBeenCalledWith('POST', '/projects/123456', expect.objectContaining({
                    name: 'Updated Project',
                    color: 'red',
                    is_favorite: false,
                }));
        ...
        describe('ProjectGetCollaboratorsHandler', () => {
            it('should get project collaborators successfully', async () => {
                ...
                mockTodoistApiGetAllRequest.mockResolvedValue(expectedResponse);
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistApiGetAllRequest).toHaveBeenCalledWith(mockCtx, '/projects/123456/collaborators');
                expect(result).toEqual({ data: expectedResponse });
            });
        });
    });
    describe('Section Handlers', () => {
        describe('SectionCreateHandler', () => {
            it('should create a section successfully', async () => {
                ...
                mockTodoistApiRequest.mockResolvedValue(expectedResponse);
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistApiRequest).toHaveBeenCalledWith('POST', '/sections', expect.objectContaining({
                    name: 'Test Section',
                    project_id: '123456',
                    order: 1,
                }));
        ...
        describe('SectionGetAllHandler', () => {
            it('should get all sections successfully', async () => {
                ...
                mockTodoistApiGetAllRequest.mockResolvedValue(expectedResponse);
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistApiGetAllRequest).toHaveBeenCalledWith(mockCtx, '/sections', expect.objectContaining({
                    project_id: '123456',
                }));
                expect(result).toEqual({ data: expectedResponse });
            });
        ...
        describe('SectionUpdateHandler', () => {
            it('should update a section successfully', async () => {
                ...
                mockTodoistApiRequest.mockResolvedValue(undefined);
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistApiRequest).toHaveBeenCalledWith('POST', '/sections/123456', expect.objectContaining({
                    name: 'Updated Section',
                }));
                expect(result).toEqual({ success: true });
            });
    ...
    describe('Label Handlers', () => {
        describe('LabelCreateHandler', () => {
            it('should create a label successfully', async () => {
                ...
                mockTodoistApiRequest.mockResolvedValue(expectedResponse);
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistApiRequest).toHaveBeenCalledWith('POST', '/labels', expect.objectContaining({
                    name: 'Test Label',
                    color: 'red',
                    order: 1,
                    is_favorite: true,
        ...
        describe('LabelUpdateHandler', () => {
            it('should update a label successfully', async () => {
                ...
                mockTodoistApiRequest.mockResolvedValue(undefined);
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistApiRequest).toHaveBeenCalledWith('POST', '/labels/123456', expect.objectContaining({
                    name: 'Updated Label',
                    color: 'green',
                    is_favorite: false,
                }));
    ...
    describe('Comment Handlers', () => {
        describe('CommentCreateHandler', () => {
            it('should create a comment successfully', async () => {
                ...
                mockTodoistApiRequest.mockResolvedValue(expectedResponse);
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistApiRequest).toHaveBeenCalledWith('POST', '/comments', expect.objectContaining({
                    task_id: '123456',
                    content: 'Test comment',
                }));
                expect(result).toEqual({ data: expectedResponse });
        ...
        describe('CommentGetAllHandler', () => {
            it('should get all comments with task filter successfully', async () => {
                ...
                mockTodoistApiGetAllRequest.mockResolvedValue(expectedResponse);
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistApiGetAllRequest).toHaveBeenCalledWith(mockCtx, '/comments', expect.objectContaining({
                    task_id: '123456',
                }));
                expect(result).toEqual({ data: expectedResponse });
            });
        ...
        describe('CommentUpdateHandler', () => {
            it('should update a comment successfully', async () => {
                ...
                mockTodoistApiRequest.mockResolvedValue(undefined);
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistApiRequest).toHaveBeenCalledWith('POST', '/comments/123456', expect.objectContaining({
                    content: 'Updated comment',
                }));
                expect(result).toEqual({ success: true });
            });
    ...
    describe('Special Operations', () => {
        describe('QuickAddHandler', () => {
            it('should quick add a task successfully', async () => {
                ...
                const result = await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistSyncRequest).toHaveBeenCalledWith({ text: 'Buy milk tomorrow @shopping' }, {}, '/quick/add');
                expect(result).toEqual({ data: expectedResponse });
            });
            ...
            it('should handle empty optional parameters correctly', async () => {
                ...
                expect(mockTodoistSyncRequest).toHaveBeenCalledWith({ text: 'Simple task' }, {}, '/quick/add');
                expect(result).toEqual({ data: expectedResponse });
            });
            it('should use /tasks/quick endpoint for node version >= 2.2', async () => {
                ...
                mockCtx.getNode.mockReturnValue(mock({ typeVersion: 2.2 }));
                mockTodoistSyncRequest.mockResolvedValue({ id: '123' });
                await handler.handleOperation(mockCtx, 0);
                expect(mockTodoistSyncRequest).toHaveBeenCalledWith({ text: 'Buy milk' }, {}, '/tasks/quick');
            });
        });
    });
...
//# sourceMappingURL=OperationHandler.test.js.map