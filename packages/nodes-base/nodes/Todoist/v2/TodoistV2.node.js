...
import { TodoistService, isTaskOperationType, isProjectOperationType, isSectionOperationType, isCommentOperationType, isLabelOperationType, isReminderOperationType, } from './Service';
    ...
    methods = {
        listSearch: {
            async searchProjects(filter) {
                const projects = await todoistApiGetAllRequest(this, '/projects');
                return {
                    results: projects
        ...
        loadOptions: {
            ...
            async getSections() {
                ...
                const projectId = options.projectId ??
                    this.getCurrentNodeParameter('project', { extractValue: true });
                if (projectId) {
            ...
            try {
                if (resource === 'task') {
                    if (!isTaskOperationType(operation)) {
                        throw new NodeOperationError(this.getNode(), `Invalid operation '${operation}' for task resource`);
                    }
                    responseData = await service.executeTask(this, operation, i);
                }
                else if (resource === 'project') {
                    if (!isProjectOperationType(operation)) {
                        throw new NodeOperationError(this.getNode(), `Invalid operation '${operation}' for project resource`);
                    }
                    responseData = await service.executeProject(this, operation, i);
                }
                else if (resource === 'section') {
                    if (!isSectionOperationType(operation)) {
                        throw new NodeOperationError(this.getNode(), `Invalid operation '${operation}' for section resource`);
                    }
                    responseData = await service.executeSection(this, operation, i);
                }
                else if (resource === 'comment') {
                    if (!isCommentOperationType(operation)) {
                        throw new NodeOperationError(this.getNode(), `Invalid operation '${operation}' for comment resource`);
                    }
                    responseData = await service.executeComment(this, operation, i);
                }
                else if (resource === 'label') {
                    if (!isLabelOperationType(operation)) {
                        throw new NodeOperationError(this.getNode(), `Invalid operation '${operation}' for label resource`);
                    }
                    responseData = await service.executeLabel(this, operation, i);
                }
                else if (resource === 'reminder') {
                    if (!isReminderOperationType(operation)) {
                        throw new NodeOperationError(this.getNode(), `Invalid operation '${operation}' for reminder resource`);
                    }
