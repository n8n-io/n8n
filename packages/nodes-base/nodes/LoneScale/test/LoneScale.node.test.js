 ...
 */
function createExecuteFunctions(params, options = {}) {
    return {
        getInputData: vi.fn(() => [{ json: {} }]),
        continueOnFail: vi.fn(() => options.continueOnFail ?? false),
        getNodeParameter: vi.fn((name, _itemIndex, fallback) => name in params ? params[name] : fallback),
        getNode: vi.fn(() => ({ name: 'LoneScale' })),
        helpers: {
            returnJsonArray: vi.fn((data) => (Array.isArray(data) ? data : [data]).map((json) => ({ json }))),
            constructExecutionMetaData: vi.fn((executionData) => executionData),
        },
    };
}
describe('LoneScale Node', () => {
    ...
    describe('company: search', () => {
        ...
        it('builds the query string and returns the results', async () => {
            ...
            const ctx = createExecuteFunctions(baseParams);
            const result = await new LoneScale().execute.call(ctx);
            expect(mockApiRequest).toHaveBeenCalledWith('GET', '/companies/search', {}, { domain: 'stripe.com', enrich: true });
            expect(result[0]).toHaveLength(1);
        ...
        it('only sends the identifiers that are provided', async () => {
            ...
            await new LoneScale().execute.call(ctx);
            expect(mockApiRequest).toHaveBeenCalledWith('GET', '/companies/search', {}, { slug: 'stripe' });
        });
    ...
    describe('validation', () => {
        it('throws when source is given no company identifier', async () => {
            ...
            await expect(new LoneScale().execute.call(ctx)).rejects.toThrow('Provide at least one company identifier: domain, name or Linkedin URL');
        ...
        it('throws when search is given no company identifier', async () => {
            ...
            await expect(new LoneScale().execute.call(ctx)).rejects.toThrow('Provide at least one company identifier: domain, Linkedin ID, slug or name');
            expect(mockApiRequest).not.toHaveBeenCalled();
        });
    });
    describe('error handling', () => {
        it('rethrows API errors when continueOnFail is off', async () => {
            mockApiRequest.mockRejectedValue(new NodeOperationError({ name: 'LoneScale' }, 'boom'));
            const ctx = createExecuteFunctions({
                resource: 'company',
                operation: 'search',
                searchDomain: 'stripe.com',
...
//# sourceMappingURL=LoneScale.node.test.js.map