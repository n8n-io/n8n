...
describe('listAllJobRuns', () => {
	...
	it.each([
		['the given page cap', 3],
		['the default page cap', undefined],
	])('stops at %s and hands back the next token', async (_label, maxPages) => {
		const context = createPollContext();
		apiMock(context).mockResolvedValue({ runs: [run(1)], next_page_token: 'more' });

		const result = await listAllJobRuns(context, 'databricksApi', {}, maxPages);

		const pages = maxPages ?? DEFAULT_MAX_PAGES;
...
describe('listAllPipelineEvents', () => {
	...
	it('stops at the page cap, resuming from the given token, and hands back the next token', async () => {
		const context = createPollContext();
