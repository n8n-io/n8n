...
describe('databricksApiRequest', () => {
    let httpRequestWithAuthentication;
    let context;
    beforeEach(() => {
    ...
    const capturedOptions = () => httpRequestWithAuthentication.mock.calls[0][1];
    ...
    it('should track the integration version rather than the node typeVersion', async () => {
        ...
        const staleContext = mock({
            getNode: () => mock({ typeVersion: 0.1 }),
            helpers: { httpRequestWithAuthentication },
        });
