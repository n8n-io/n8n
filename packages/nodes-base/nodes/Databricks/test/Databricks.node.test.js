...
const apiErrorFromBody = (status, data) => new NodeApiError(node, new AxiosError(`Request failed with status code ${status}`, {
    status,
    data,
}));
describe('Databricks', () => {
...
describe('makePermissionErrorLegible', () => {
    it('should promote the PERMISSION_DENIED body message and add a remediation description', () => {
        ...
        expect(error.description).toBe('Grant the named permission to the signed-in user or service principal in Databricks, then retry.');
    });
    ...
    it('should parse a Buffer body from arraybuffer requests', () => {
        const error = apiErrorFromBody(403, Buffer.from(JSON.stringify({ error_code: 'PERMISSION_DENIED', message: PERMISSION_MESSAGE })));
        makePermissionErrorLegible(error);
        expect(error.message).toBe(PERMISSION_MESSAGE);
    });
...
describe('listSearch -> PERMISSION_DENIED surfaces the Databricks message', () => {
    ...
    it('should reject with the legible message from getCatalogs', async () => {
        ...
        context.helpers.httpRequestWithAuthentication.mockRejectedValue(apiErrorFromBody(403, { error_code: 'PERMISSION_DENIED', message: PERMISSION_MESSAGE }));
...
describe('Databricks SQL -> Execute Query (FAILED/CANCELED statement)', () => {
    ...
    it.each(['FAILED', 'CANCELED'])('should surface the in-band error message of a %s statement', async (state) => {
        ...
        await expect(executeQuery.call(context, 0)).rejects.toThrow(`Query ${state.toLowerCase()}: User does not have SELECT on Table 'x'.`);
    });
    it('should strip control characters and truncate the in-band error message', async () => {
        ...
        await expect(executeQuery.call(context, 0)).rejects.toThrow(`Query failed: ${`bad message${'x'.repeat(600)}`.slice(0, 500)}`);
    });
    it('should fall back to the stringified status when no error message is present', async () => {
        ...
        await expect(executeQuery.call(context, 0)).rejects.toThrow(`Query failed: ${JSON.stringify({ state: 'FAILED' })}`);
    });
});
//# sourceMappingURL=Databricks.node.test.js.map