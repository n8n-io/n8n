 ...
 */
export async function databricksApiRequest(context, credentialType, options) {
...
export function getActiveCredentialType(context, itemIndex = 0) {
    const authentication = context.getNodeParameter('authentication', itemIndex, 'accessToken');
    return authentication === 'oAuth2' ? 'databricksOAuth2Api' : 'databricksApi';
}
export async function getHost(context, credentialType) {
...
export function makePermissionErrorLegible(error) {
...
export function extractResourceLocatorValue(param) {
    if (typeof param === 'object' && param !== null) {
        return param.value || '';
    }
    return param || '';
}
function detectFormatFromProperties(properties, invocationUrl, isOneOf = false) {
...
export function detectInputFormat(openApiSchema) {
...
export function generateExampleFromSchema(schema, format) {
...
export function validateRequestBody(requestBody, detectedFormat) {
    switch (detectedFormat) {
        ...
        case 'dataframe_records':
            if (!requestBody.dataframe_records || !Array.isArray(requestBody.dataframe_records)) {
                throw new UserError('Invalid dataframe_records format: "dataframe_records" array is required');
            }
            break;
        case 'inputs':
