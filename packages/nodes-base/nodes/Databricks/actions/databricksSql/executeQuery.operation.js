...
import { databricksApiRequest, extractResourceLocatorValue, getActiveCredentialType, getHost, sanitizeApiMessage, } from '../helpers';
export async function execute(i) {
    ...
    if (status === 'FAILED' || status === 'CANCELED') {
        ...
        const reason = typeof apiMessage === 'string' && apiMessage
    ...
    if (retries >= maxRetries) {
        throw new NodeOperationError(this.getNode(), 'Query execution timeout - exceeded maximum wait time', { itemIndex: i });
    }
