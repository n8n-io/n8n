...
describe('Xml Node - options validation', () => {
    ...
    const setupGetNodeParameter = (mode, options) => {
        mockExecuteFunctions.getNodeParameter.mockImplementation((parameterName, _itemIndex, fallbackValue) => {
            if (parameterName === 'mode')
                return mode;
            if (parameterName === 'dataPropertyName')
                return 'data';
    ...
    describe('attrkey validation', () => {
        test.each(FORBIDDEN_KEYS)('should reject invalid attrkey "%s" in xmlToJson mode', async (forbiddenKey) => {
            setupGetNodeParameter('xmlToJson', { attrkey: forbiddenKey, mergeAttrs: false });
            await expect(xmlNode.execute.call(mockExecuteFunctions)).rejects.toThrow(`The "Attribute Key" option value "${forbiddenKey}" is not allowed`);
            await expect(xmlNode.execute.call(mockExecuteFunctions)).rejects.toThrow(NodeOperationError);
        });
        test.each(FORBIDDEN_KEYS)('should reject invalid attrkey "%s" in jsonToxml mode', async (forbiddenKey) => {
            setupGetNodeParameter('jsonToxml', { attrkey: forbiddenKey });
            await expect(xmlNode.execute.call(mockExecuteFunctions)).rejects.toThrow(`The "Attribute Key" option value "${forbiddenKey}" is not allowed`);
            await expect(xmlNode.execute.call(mockExecuteFunctions)).rejects.toThrow(NodeOperationError);
        });
        test.each(FORBIDDEN_KEYS)('should reject non-string attrkey that resolves to "%s"', async (forbiddenKey) => {
            setupGetNodeParameter('xmlToJson', { attrkey: [forbiddenKey], mergeAttrs: false });
            await expect(xmlNode.execute.call(mockExecuteFunctions)).rejects.toThrow(`The "Attribute Key" option value "${forbiddenKey}" is not allowed`);
            await expect(xmlNode.execute.call(mockExecuteFunctions)).rejects.toThrow(NodeOperationError);
        });
    ...
    describe('charkey validation', () => {
        test.each(FORBIDDEN_KEYS)('should reject invalid charkey "%s" in xmlToJson mode', async (forbiddenKey) => {
            setupGetNodeParameter('xmlToJson', { charkey: forbiddenKey });
            await expect(xmlNode.execute.call(mockExecuteFunctions)).rejects.toThrow(`The "Character Key" option value "${forbiddenKey}" is not allowed`);
            await expect(xmlNode.execute.call(mockExecuteFunctions)).rejects.toThrow(NodeOperationError);
        });
        test.each(FORBIDDEN_KEYS)('should reject invalid charkey "%s" in jsonToxml mode', async (forbiddenKey) => {
            setupGetNodeParameter('jsonToxml', { charkey: forbiddenKey });
            await expect(xmlNode.execute.call(mockExecuteFunctions)).rejects.toThrow(`The "Character Key" option value "${forbiddenKey}" is not allowed`);
            await expect(xmlNode.execute.call(mockExecuteFunctions)).rejects.toThrow(NodeOperationError);
        });
        test.each(FORBIDDEN_KEYS)('should reject non-string charkey that resolves to "%s"', async (forbiddenKey) => {
            setupGetNodeParameter('xmlToJson', { charkey: [forbiddenKey] });
            await expect(xmlNode.execute.call(mockExecuteFunctions)).rejects.toThrow(`The "Character Key" option value "${forbiddenKey}" is not allowed`);
            await expect(xmlNode.execute.call(mockExecuteFunctions)).rejects.toThrow(NodeOperationError);
        });
    ...
    describe('attrkey and charkey are coerced to a stable string', () => {
        ...
        test('should forward the coerced attrkey string to the builder in jsonToxml mode', async () => {
            ...
            const result = await xmlNode.execute.call(mockExecuteFunctions);
            const xml = result[0][0].json.data;
