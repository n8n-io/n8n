    ...
    async execute() {
        ...
        if (options.attrkey !== undefined) {
            ...
            if (forbiddenKeys.includes(options.attrkey)) {
                throw new NodeOperationError(this.getNode(), `The "Attribute Key" option value "${options.attrkey}" is not allowed`);
            }
        }
        if (options.charkey !== undefined) {
            options.charkey = String(options.charkey);
            if (forbiddenKeys.includes(options.charkey)) {
                throw new NodeOperationError(this.getNode(), `The "Character Key" option value "${options.charkey}" is not allowed`);
            }
        }
        let item;
        const returnData = [];
            ...
            try {
                ...
                if (mode === 'xmlToJson') {
                    ...
                    if (item.json[dataPropertyName] === undefined) {
                        throw new NodeOperationError(this.getNode(), `Item has no JSON property called "${dataPropertyName}"`, { itemIndex });
                    }
