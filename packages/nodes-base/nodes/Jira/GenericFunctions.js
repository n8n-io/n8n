...
import { getAtlassianApiBaseUrl, getAtlassianCloudId, getAtlassianSiteParameter, resolveAtlassianCloudId, } from '@utils/atlassian';
export async function jiraSoftwareCloudApiRequest(endpoint, method, body = {}, query, uri, option = {}) {
    ...
    if (jiraVersion === 'server') {
        ...
        if (typeof rawDomain !== 'string' || rawDomain === '') {
            throw new NodeOperationError(this.getNode(), 'The Jira credential is missing the Site URL field');
        }
        ...
        const cloudId = await resolveAtlassianCloudId.call(this, credentialType, getAtlassianSiteParameter(this), 'jira');
        domain = getAtlassianApiBaseUrl('jira', cloudId);
    }
    else {
...
export function handlePagination(method, body, query, paginationType, responseData) {
    if (!responseData) {
...
export async function jiraSoftwareCloudApiRequestAllItems(propertyName, endpoint, method, body = {}, query = {}, paginationType = 'offset') {
    const returnData = [];
    let responseData;
...
export function getWebhookId(webhook) {
    if (webhook.id)
...
export function filterSortSearchListItems(items, filter) {
    return items
        .filter((item) => !filter ||
...
export async function refreshJiraWebhook(endpoint, webhookId) {
