...
import { allEvents, eventExists, getWebhookId, getWebhookEndpoint, jiraSoftwareCloudApiRequest, jiraSoftwareCloudApiRequestAllItems, refreshJiraWebhook, OAUTH2_WEBHOOK_REFRESH_INTERVAL_MS, OAUTH2_WEBHOOK_EXPIRY_BUFFER_MS, OAUTH2_SUPPORTED_WEBHOOK_EVENTS, } from './GenericFunctions';
export class JiraTrigger {
            ...
            async create() {
                ...
                if (authenticateWebhook) {
                    ...
                    try {
                        ...
                        throw new NodeOperationError(this.getNode(), new Error('Could not retrieve HTTP Query Auth credentials', { cause: e }));
                    }
                ...
                if (jiraVersion === 'cloudOAuth2') {
                    ...
                    if (filteredEvents.length === 0) {
                        throw new NodeOperationError(this.getNode(), 'None of the selected events are supported with Jira Cloud OAuth2. Select at least one of: issues, comments, versions, or sprints.');
                    }
                    let jqlFilter = additionalFields.filter;
                    if (!jqlFilter) {
                        const projects = (await jiraSoftwareCloudApiRequestAllItems.call(this, 'values', '/api/2/project/search', 'GET'));
                        if (!projects.length) {
                            throw new NodeOperationError(this.getNode(), 'No accessible Jira projects found for this OAuth2 user. The webhook cannot be registered without at least one accessible project.');
                        }
                    ...
                    const result = responseData.webhookRegistrationResult?.[0];
                    if (!result?.createdWebhookId) {
                        const errorDetail = result?.errors?.join(', ') ?? 'no webhook ID returned';
                        throw new NodeOperationError(this.getNode(), `Failed to register webhook: ${errorDetail}`);
                    }
    ...
    async webhook() {
        ...
        if (jiraVersion === 'cloudOAuth2') {
            const webhookData = this.getWorkflowStaticData('node');
            const lastRefreshed = webhookData.lastRefreshed;
