...
const jiraSoftwareCloudApiRequestMock = GenericFunctions.jiraSoftwareCloudApiRequest;
const jiraSoftwareCloudApiRequestAllItems = GenericFunctions.jiraSoftwareCloudApiRequestAllItems;
describe('Jira Node', () => {
    let jiraNode;
    ...
    describe('description', () => {
        ...
        it('should reload site-scoped pickers when the Site changes', () => {
            const collectPickers = (properties) => properties.flatMap((property) => {
                const nested = [
                    ...(property.options ?? []),
                    ...(property.options ?? []).flatMap((option) => option.values ?? []),
                ];
                const isPicker = property.type === 'resourceLocator' ||
                    property.typeOptions?.loadOptionsMethod !== undefined;
                return [...(isPicker ? [property] : []), ...collectPickers(nested)];
            });
            const pickers = collectPickers(jiraNode.description.properties).filter((property) => property.name !== 'site');
    ...
    describe('issue getAll', () => {
        it('should set default fields to "*navigable" when not provided', async () => {
            executeFunctionsMock.getNodeParameter.mockImplementation((parameterName) => {
            ...
            expect(jiraSoftwareCloudApiRequestMock).toHaveBeenCalledWith('/api/2/search/jql', 'POST', expect.objectContaining({
                fields: ['*navigable'],
            }));
        });
        ...
        it('should send expand as an array on Jira Server', async () => {
            ...
            await jiraNode.execute.call(executeFunctionsMock);
            expect(jiraSoftwareCloudApiRequestMock).toHaveBeenCalledWith('/api/2/search', 'POST', expect.objectContaining({
                expand: ['changelog', 'names'],
            }));
        });
        ...
        it('should call new endpoint for the cloud version with return all = true', async () => {
            ...
            await jiraNode.execute.call(executeFunctionsMock);
            expect(jiraSoftwareCloudApiRequestAllItems).toHaveBeenCalledWith('issues', '/api/2/search/jql', 'POST', expect.anything(), {}, 'token');
        });
        it.each([['server'], ['serverPat']])('should call old endpoint for the self-hosted version with return all = false', async (jiraVersion) => {
            executeFunctionsMock.getNodeParameter.mockImplementation((parameterName) => {
                switch (parameterName) {
                    case 'resource':
                        return 'issue';
            ...
            await jiraNode.execute.call(executeFunctionsMock);
            expect(jiraSoftwareCloudApiRequestMock).toHaveBeenCalledWith('/api/2/search', 'POST', expect.anything());
        });
        it.each([['server'], ['serverPat']])('should call old endpoint for the self-hosted version with return all = true', async (jiraVersion) => {
            executeFunctionsMock.getNodeParameter.mockImplementation((parameterName) => {
                switch (parameterName) {
                    case 'resource':
                        return 'issue';
    ...
    describe('issue getAll on cloud-gateway auth versions', () => {
        it.each(['cloudOAuth2', 'cloudServiceAccount'])('should use cloud JQL endpoint when jiraVersion is "%s"', async (jiraVersion) => {
            executeFunctionsMock.getNodeParameter.mockImplementation((parameterName) => {
                switch (parameterName) {
                    case 'resource':
                        return 'issue';
    ...
    describe('issueAttachment download', () => {
        ...
        const mockParameters = (operation) => {
            executeFunctionsMock.getNodeParameter.mockImplementation((parameterName) => {
                switch (parameterName) {
                    case 'resource':
        ...
        beforeEach(() => {
            executeFunctionsMock.helpers.returnJsonArray.mockImplementation((data) => (Array.isArray(data) ? data : [data]).map((item) => 'json' in item ? item : { json: item }));
            executeFunctionsMock.helpers.constructExecutionMetaData.mockImplementation((items) => items);
        ...
        it('should download a single attachment without forwarding credentials across origins', async () => {
            ...
            await jiraNode.execute.call(executeFunctionsMock);
            expect(jiraSoftwareCloudApiRequestMock).toHaveBeenNthCalledWith(2, '', 'GET', {}, {}, attachmentMeta.content, downloadOptions);
        });
    ...
    describe('continueOnFail', () => {
        it('should return error item when API request fails and continueOnFail is true', async () => {
            ...
            executeFunctionsMock.helpers.returnJsonArray.mockImplementation((data) => [{ json: data }]);
            executeFunctionsMock.helpers.constructExecutionMetaData.mockImplementation((items) => items);
            executeFunctionsMock.getNodeParameter.mockImplementation((parameterName) => {
                switch (parameterName) {
                    case 'resource':
        ...
        it('should throw when API request fails and continueOnFail is false', async () => {
            ...
            await expect(jiraNode.execute.call(executeFunctionsMock)).rejects.toThrow('Issue does not exist');
        });
        it('should return error item with preserved paired-item metadata when attachment download fails', async () => {
            ...
            executeFunctionsMock.continueOnFail.mockReturnValue(true);
            executeFunctionsMock.helpers.returnJsonArray.mockImplementation((data) => [{ json: data }]);
            executeFunctionsMock.helpers.constructExecutionMetaData.mockImplementation((items, { itemData }) => items.map((item) => ({
                ...item,
                pairedItem: itemData,
            })));
            executeFunctionsMock.getNodeParameter.mockImplementation((parameterName) => {
...
//# sourceMappingURL=Jira.node.test.js.map