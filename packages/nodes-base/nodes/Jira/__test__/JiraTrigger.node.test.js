...
import { allEvents, OAUTH2_WEBHOOK_REFRESH_INTERVAL_MS, OAUTH2_WEBHOOK_EXPIRY_BUFFER_MS, OAUTH2_SUPPORTED_WEBHOOK_EVENTS, } from '../GenericFunctions';
...
describe('JiraTrigger', () => {
    describe('Webhook lifecycle', () => {
        ...
        const mockCloudIdLookup = () => vi.fn().mockResolvedValueOnce([{ id: OAUTH2_CLOUD_ID, url: OAUTH2_DOMAIN }]);
        // The accessible-resources cache is keyed by credential ID
        const mockJiraNode = mock({
            typeVersion: 1,
        ...
        function mockHookFunctions(mockRequest) {
        ...
        test('should register a webhook subscription on Jira 10', async () => {
            ...
            const exists = await trigger.webhookMethods.default?.checkExists.call(mockHookFunctions(mockExistsRequest));
            expect(mockExistsRequest).toHaveBeenCalledTimes(2);
            expect(mockExistsRequest).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({ uri: 'https://jira.local/rest/api/2/serverInfo' }));
            expect(mockExistsRequest).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({ uri: 'https://jira.local/rest/jira-webhook/1.0/webhooks' }));
            expect(staticData.endpoint).toBe('/jira-webhook/1.0/webhooks');
            expect(exists).toBe(false);
            const mockCreateRequest = vi.fn().mockResolvedValueOnce({ id: 1 });
            const created = await trigger.webhookMethods.default?.create.call(mockHookFunctions(mockCreateRequest));
            expect(mockCreateRequest).toHaveBeenCalledTimes(1);
            expect(mockCreateRequest).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
                method: 'POST',
            ...
            const deleted = await trigger.webhookMethods.default?.delete.call(mockHookFunctions(mockDeleteRequest));
            expect(deleted).toBe(true);
            expect(mockDeleteRequest).toHaveBeenCalledTimes(1);
            expect(mockDeleteRequest).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
                method: 'DELETE',
        ...
        test('should register a webhook subscription on Jira 9', async () => {
            ...
            expect(mockExistsRequest).toHaveBeenCalledTimes(2);
            expect(mockExistsRequest).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({ uri: 'https://jira.local/rest/api/2/serverInfo' }));
            expect(mockExistsRequest).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({ uri: 'https://jira.local/rest/webhooks/1.0/webhook' }));
        ...
        test('should register a webhook subscription on Jira Cloud (OAuth2)', async () => {
            ...
            function mockOAuth2HookFunctions(mockRequest, mockHttpRequest) {
                return mockDeep({
                    getWorkflowStaticData: () => staticData,
                    getNode: vi.fn(() => mockJiraNode),
                    getNodeWebhookUrl: vi.fn(() => 'https://n8n.local/webhook/id'),
                    getNodeParameter: vi.fn((param) => {
                        ...
                        if (param === 'jiraVersion')
                            return 'cloudOAuth2';
                        if (param === 'additionalFields')
                            return { filter: 'project = TEST' };
                        return {};
                    }),
                    getCredentials: async () => ({ domain }),
                    helpers: {
                        requestWithAuthentication: mockRequest,
            ...
            const exists = await trigger.webhookMethods.default?.checkExists.call(mockOAuth2HookFunctions(mockExistsRequest, mockCloudIdRequest));
            expect(mockCloudIdRequest).toHaveBeenCalledTimes(1);
            expect(mockCloudIdRequest).toHaveBeenCalledWith('jiraSoftwareCloudOAuth2Api', expect.objectContaining({
                url: 'https://api.atlassian.com/oauth/token/accessible-resources',
            }));
            expect(mockExistsRequest).toHaveBeenCalledTimes(1);
            expect(mockExistsRequest).toHaveBeenCalledWith('jiraSoftwareCloudOAuth2Api', expect.objectContaining({ uri: baseApiUrl, method: 'GET' }));
            ...
            const created = await trigger.webhookMethods.default?.create.call(mockOAuth2HookFunctions(mockCreateRequest));
            expect(mockCreateRequest).toHaveBeenCalledTimes(1);
            expect(mockCreateRequest).toHaveBeenCalledWith('jiraSoftwareCloudOAuth2Api', expect.objectContaining({
                method: 'POST',
                uri: baseApiUrl,
                body: {
                    url: 'https://n8n.local/webhook/id',
            ...
            const mockDeleteRequest = vi.fn().mockResolvedValueOnce({});
            const deleted = await trigger.webhookMethods.default?.delete.call(mockOAuth2HookFunctions(mockDeleteRequest));
            expect(deleted).toBe(true);
            expect(mockDeleteRequest).toHaveBeenCalledTimes(1);
            expect(mockDeleteRequest).toHaveBeenCalledWith('jiraSoftwareCloudOAuth2Api', expect.objectContaining({
                method: 'DELETE',
                uri: baseApiUrl,
        ...
        test('should refresh OAuth2 webhook in checkExists when near expiry', async () => {
            ...
            expect(mockRequest).toHaveBeenCalledWith('jiraSoftwareCloudOAuth2Api', expect.objectContaining({
                method: 'PUT',
        ...
        test('should refresh OAuth2 webhook in handler when interval has elapsed', async () => {
            ...
            webhookFns.helpers.returnJsonArray.mockImplementation((data) => [
                { json: data },
            ]);
            await trigger.webhook.call(webhookFns);
            expect(webhookFns.helpers.requestWithAuthentication).toHaveBeenCalledWith('jiraSoftwareCloudOAuth2Api', expect.objectContaining({
                method: 'PUT',
                uri: `${baseApiUrl}/refresh`,
                body: { webhookIds: [3000] },
            }));
            expect(staleData.lastRefreshed).toBeGreaterThan(Date.now() - OAUTH2_WEBHOOK_REFRESH_INTERVAL_MS);
        });
        ...
        test('should filter allEvents to supported subset when * is selected for OAuth2', async () => {
            ...
            await trigger.webhookMethods.default?.create.call(hookFns);
            const sentEvents = mockRequest.mock.calls[0][1].body.webhooks[0].events;
        ...
        test('should throw when OAuth2 user has no accessible projects', async () => {
            ...
            await expect(trigger.webhookMethods.default?.create.call(hookFns)).rejects.toThrow('No accessible Jira projects found');
        });
        ...
        test('should throw when all selected events are unsupported for OAuth2', async () => {
            ...
            const hookFns = mockDeep({
                ...
                getNodeParameter: vi.fn((param) => {
                    if (param === 'events')
                        return ['board_created', 'user_deleted'];
                    if (param === 'jiraVersion')
                        return 'cloudOAuth2';
                    if (param === 'additionalFields')
                        return {};
            ...
            await expect(trigger.webhookMethods.default?.create.call(hookFns)).rejects.toThrow('None of the selected events are supported');
        });
        test('should find existing webhook when * is selected on classic path', async () => {
            ...
            const hookFns = mockDeep({
                ...
                getCredentials: async () => ({ domain: 'https://jira.local', email: 'test@n8n.io', password: 'secret' }),
...
//# sourceMappingURL=JiraTrigger.node.test.js.map