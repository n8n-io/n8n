...
vi.mock('../GenericFunctions', async () => {
    ...
    return {
        ...originalModule,
        jiraSoftwareCloudApiRequest: vi.fn(async function (endpoint, method) {
...
describe('Jira Node, methods', () => {
    ...
    describe('listSearch.getSites', () => {
        it('should list the sites the service account can reach', async () => {
            const ctx = mockDeep();
            ctx.getNode.mockReturnValue(mock({ credentials: undefined }));
            ...
            expect(ctx.helpers.httpRequestWithAuthentication).toHaveBeenCalledWith('atlassianServiceAccountApi', expect.objectContaining({
    ...
    describe('listSearch.getCustomFields', () => {
        it('should call correct endpoint and return custom fields for server version', async () => {
            ...
            const { results } = await jira.methods.listSearch.getCustomFields.call(loadOptionsFunctions);
            expect(results).toEqual([
                {
                    name: 'Field 123',
...
//# sourceMappingURL=node.methods.test.js.map