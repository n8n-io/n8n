...
describe('Jira -> GenericFunctions', () => {
    describe('jiraSoftwareCloudApiRequestAllItems', () => {
        ...
        beforeEach(() => {
            ...
            mockExecuteFunctions.helpers.requestWithAuthentication.mockImplementation(async function (_, options) {
        ...
        it('should get all items and not pass the body when the method is GET', async () => {
            const result = await jiraSoftwareCloudApiRequestAllItems.call(mockExecuteFunctions, 'issues', '/api/2/search', 'GET');
            ...
            expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('jiraSoftwareServerApi', expect.not.objectContaining({
    ...
    describe('jiraSoftwareCloudApiRequest credential routing', () => {
        let mockExecuteFunctions;
        let jiraSoftwareCloudApiRequest;
        beforeEach(async () => {
            vi.resetModules();
            ...
            mockExecuteFunctions.getNode.mockReturnValue({ name: 'Jira' });
        });
        afterEach(() => {
            vi.clearAllMocks();
        });
        it('should use jiraSoftwareCloudApi credential for jiraVersion "cloud"', async () => {
            ...
            expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('jiraSoftwareCloudApi', expect.objectContaining({ uri: 'https://example.atlassian.net/rest/api/2/myself' }));
        });
        ...
        it('should use jiraSoftwareCloudOAuth2Api credential for jiraVersion "cloudOAuth2" and look up cloudId', async () => {
            ...
            await jiraSoftwareCloudApiRequest.call(mockExecuteFunctions, '/api/2/myself', 'GET');
            expect(mockExecuteFunctions.getCredentials).toHaveBeenCalledWith('jiraSoftwareCloudOAuth2Api');
            expect(mockExecuteFunctions.helpers.httpRequestWithAuthentication).toHaveBeenCalledWith('jiraSoftwareCloudOAuth2Api', expect.objectContaining({
                url: 'https://api.atlassian.com/oauth/token/accessible-resources',
            }));
            expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('jiraSoftwareCloudOAuth2Api', expect.objectContaining({
        ...
        describe('jiraVersion "cloudServiceAccount"', () => {
            ...
            const mockParameters = (site) => {
                mockExecuteFunctions.getNodeParameter.mockImplementation((parameterName) => parameterName === 'site' ? site : 'cloudServiceAccount');
            };
            it('should call the gateway with the cloudId chosen from the Site list', async () => {
                ...
                expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('atlassianServiceAccountApi', expect.objectContaining({
                    uri: `https://api.atlassian.com/ex/jira/${cloudId}/rest/api/2/myself`,
                }));
            });
            it('should resolve a Site URL against the accessible resources', async () => {
                ...
                mockExecuteFunctions.helpers.httpRequestWithAuthentication.mockResolvedValueOnce(accessibleResources);
                await jiraSoftwareCloudApiRequest.call(mockExecuteFunctions, '/api/2/myself', 'GET');
                expect(mockExecuteFunctions.helpers.httpRequestWithAuthentication).toHaveBeenCalledWith('atlassianServiceAccountApi', expect.objectContaining({
                    url: 'https://api.atlassian.com/oauth/token/accessible-resources',
                }));
                expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('atlassianServiceAccountApi', expect.objectContaining({
                    uri: `https://api.atlassian.com/ex/jira/${cloudId}/rest/api/2/myself`,
            ...
            it('should auto-resolve when the node carries no Site parameter at all', async () => {
                mockExecuteFunctions.getNodeParameter.mockImplementation((parameterName) => parameterName === 'site' ? null : 'cloudServiceAccount');
                mockExecuteFunctions.helpers.httpRequestWithAuthentication.mockResolvedValueOnce(accessibleResources);
                await jiraSoftwareCloudApiRequest.call(mockExecuteFunctions, '/api/2/myself', 'GET');
            ...
            it('should ask for a Site when the account reaches several sites and none is chosen', async () => {
                ...
                await expect(jiraSoftwareCloudApiRequest.call(mockExecuteFunctions, '/api/2/myself', 'GET')).rejects.toThrow("pick a site in the 'Site' parameter");
        ...
        it('should throw a NodeOperationError naming the Site URL field when the cloudOAuth2 credential lacks it', async () => {
            mockExecuteFunctions.getNodeParameter.mockReturnValue('cloudOAuth2');
            mockExecuteFunctions.getCredentials.mockResolvedValue({});
            const promise = jiraSoftwareCloudApiRequest.call(mockExecuteFunctions, '/api/2/myself', 'GET');
        ...
        it('should use jiraSoftwareServerApi credential for jiraVersion "server"', async () => {
            ...
            expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('jiraSoftwareServerApi', expect.objectContaining({ uri: 'https://jira.company.com/rest/api/2/myself' }));
        });
        it('should use jiraSoftwareServerPatApi credential for jiraVersion "serverPat"', async () => {
            ...
            expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('jiraSoftwareServerPatApi', expect.objectContaining({ uri: 'https://jira.company.com/rest/api/2/myself' }));
        });
    });
