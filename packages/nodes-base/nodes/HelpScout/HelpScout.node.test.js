...
describe('HelpScout Node', () => {
    ...
    const mockHelpscoutApiRequest = GenericFunctions.helpscoutApiRequest;
    beforeEach(() => {
    ...
    describe('Thread create operation - binary attachments', () => {
        it('should stream large binary files using getBinaryStream when binary data has an id', async () => {
            ...
            const assertBinaryData = mockExecuteFunctions.helpers.assertBinaryData;
            const getBinaryStream = mockExecuteFunctions.helpers.getBinaryStream;
            const binaryToBuffer = mockExecuteFunctions.helpers.binaryToBuffer;
            ...
            getBinaryStream.mockResolvedValue(Readable.from(mockBuffer));
            ...
            expect(mockExecuteFunctions.helpers.getBinaryStream).toHaveBeenCalledWith('binary-data-id-123', 256 * 1024);
            expect(mockExecuteFunctions.helpers.binaryToBuffer).toHaveBeenCalled();
            expect(mockHelpscoutApiRequest).toHaveBeenCalledWith('POST', '/v2/conversations/conv-123/notes', expect.objectContaining({
                text: 'Test message',
        ...
        it('should use direct base64 data when binary data does not have an id', async () => {
            ...
            expect(mockHelpscoutApiRequest).toHaveBeenCalledWith('POST', '/v2/conversations/conv-456/notes', expect.objectContaining({
        ...
        it('should process multiple binary attachments in a single request', async () => {
            ...
            expect(callArgs[2].attachments).toEqual(expect.arrayContaining([
                expect.objectContaining({
                    fileName: 'image1.png',
                    mimeType: 'image/png',
                }),
        ...
        it('should throw error when specified binary property does not exist on input', async () => {
            ...
            assertBinaryData.mockImplementation(() => {
                throw new NodeOperationError(mockExecuteFunctions.getNode(), 'Binary property nonExistentFile does not exist on input', { itemIndex: 0 });
            });
            await expect(helpScout.execute.call(mockExecuteFunctions)).rejects.toThrow('Binary property nonExistentFile does not exist on input');
        ...
        it('should use default fileName "unknown" when binary data has no fileName', async () => {
            ...
            mockHelpscoutApiRequest.mockResolvedValue({ success: true });
            await helpScout.execute.call(mockExecuteFunctions);
            expect(mockHelpscoutApiRequest).toHaveBeenCalledWith('POST', '/v2/conversations/conv-default/notes', expect.objectContaining({
                attachments: expect.arrayContaining([
                    expect.objectContaining({
                        fileName: 'unknown',
                        mimeType: 'application/octet-stream',
        ...
        it('should handle IBinaryData object passed directly as property value', async () => {
            ...
            expect(mockHelpscoutApiRequest).toHaveBeenCalledWith('POST', '/v2/conversations/conv-obj/notes', expect.objectContaining({
                attachments: expect.arrayContaining([
                    expect.objectContaining({
                        fileName: 'animation.gif',
                        mimeType: 'image/gif',
        ...
        it('should throw error when neither customer email nor customer ID is provided', async () => {
            ...
            mockExecuteFunctions.continueOnFail.mockReturnValue(false);
            await expect(helpScout.execute.call(mockExecuteFunctions)).rejects.toThrow('Either customer email or customer ID must be set');
        });
    });
});
//# sourceMappingURL=HelpScout.node.test.js.map