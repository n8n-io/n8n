...
import { constants, createCipheriv, createDecipheriv, createHash, createHmac, createSign, getHashes, privateDecrypt, publicEncrypt, randomBytes, scrypt, } from 'crypto';
        ...
        if (action === 'hmac' || action === 'sign' || action === 'encrypt' || action === 'decrypt') {
            ...
            if (action === 'hmac') {
                if (!credentials.hmacSecret) {
                    throw new NodeOperationError(this.getNode(), 'No HMAC secret set in credentials. Please add an HMAC secret to your Crypto credentials.');
                }
                hmacSecret = credentials.hmacSecret;
            }
            if (action === 'sign') {
                if (!credentials.signPrivateKey) {
                    throw new NodeOperationError(this.getNode(), 'No private key set in credentials. Please add a private key to your Crypto credentials.');
                }
            ...
            if (action === 'encrypt' || action === 'decrypt') {
                ...
                if (mode === 'symmetric') {
                    if (!credentials.encryptionPassphrase) {
                        throw new NodeOperationError(this.getNode(), 'No encryption passphrase set in credentials. Please add an Encryption Passphrase to your Crypto credentials.');
                    }
                    encryptionPassphrase = credentials.encryptionPassphrase;
                }
                if (mode === 'asymmetric' && action === 'encrypt') {
                    if (!credentials.encryptionPublicKey) {
                        throw new NodeOperationError(this.getNode(), 'No encryption public key set in credentials. Please add an Encryption Public Key to your Crypto credentials.');
                    }
                ...
                if (mode === 'asymmetric' && action === 'decrypt') {
                    if (!credentials.encryptionPrivateKey) {
                        throw new NodeOperationError(this.getNode(), 'No encryption private key set in credentials. Please add an Encryption Private Key to your Crypto credentials.');
                    }
            ...
            try {
                ...
                if (action === 'encrypt') {
                    ...
                    if (mode === 'symmetric') {
                        ...
                        const cipherInstance = createCipheriv(cipher, key, iv);
                        ...
                        try {
                            ...
                            if (opensslError.code === 'ERR_OSSL_RSA_DATA_TOO_LARGE_FOR_KEY_SIZE' ||
                                ...
                                throw new NodeOperationError(this.getNode(), 'Plaintext is too large for the RSA key. Use symmetric mode for larger data.', { itemIndex: i });
                            }
                            throw error;
                        }
                    }
                    ...
                    try {
                        if (mode === 'symmetric') {
                            ...
                            if (payload.length < headerLength) {
                                throw new NodeOperationError(this.getNode(), 'Ciphertext is malformed or truncated', { itemIndex: i });
                            }
                            const version = payload[0];
                            if (version !== SYMMETRIC_FORMAT_VERSION) {
                                throw new NodeOperationError(this.getNode(), `Unsupported ciphertext version 0x${version.toString(16).padStart(2, '0')}`, { itemIndex: i });
                            }
                        ...
                        if (error instanceof NodeOperationError)
                            throw error;
                        throw new NodeOperationError(this.getNode(), 'Decryption failed: wrong passphrase, key, cipher, or corrupted payload', { itemIndex: i });
                    }
                }
                let newItem;
