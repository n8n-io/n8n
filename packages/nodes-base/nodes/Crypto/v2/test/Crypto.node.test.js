...
describe('CryptoV2 Node', () => {
    ...
    describe('Credential Validation', () => {
        describe('HMAC action', () => {
            it('should throw error when hmacSecret is not set in credentials', async () => {
                ...
                await expect(cryptoNode.execute.call(mockExecuteFunctions)).rejects.toThrow(NodeOperationError);
                await expect(cryptoNode.execute.call(mockExecuteFunctions)).rejects.toThrow('No HMAC secret set in credentials');
            });
        ...
        describe('Sign action', () => {
            it('should throw error when signPrivateKey is not set in credentials', async () => {
                ...
                await expect(cryptoNode.execute.call(mockExecuteFunctions)).rejects.toThrow(NodeOperationError);
                await expect(cryptoNode.execute.call(mockExecuteFunctions)).rejects.toThrow('No private key set in credentials');
            });
            it('should sign data with valid private key', async () => {
                ...
                const result = await cryptoNode.execute.call(mockExecuteFunctions);
                expect(result[0][0].json.data).toBe('ZlDI7xX0XElJHwEpTw08Ykz/D+IJ+hQkcb4Cr929bUjiiLRXy8Etagc0Miuld2WnksIaznNmlqn7bom5oOpDnw==');
            });
        });
    });
    ...
    describe('Encrypt / Decrypt actions', () => {
        ...
        const mockEncryptParams = (overrides = {}) => ({
            action: 'encrypt',
            mode: 'symmetric',
            cipher: 'aes-256-gcm',
            value: plaintext,
        ...
        const mockDecryptParams = (value, overrides = {}) => ({
            action: 'decrypt',
            mode: 'symmetric',
            cipher: 'aes-256-gcm',
            value,
        ...
        describe('Credential validation', () => {
            it('throws when symmetric encrypt has no passphrase', async () => {
                ...
                mockExecuteFunctions.getNodeParameter.mockImplementation((name) => params[name]);
                mockExecuteFunctions.getCredentials.mockResolvedValue({ encryptionPassphrase: '' });
                await expect(cryptoNode.execute.call(mockExecuteFunctions)).rejects.toThrow(NodeOperationError);
                await expect(cryptoNode.execute.call(mockExecuteFunctions)).rejects.toThrow('No encryption passphrase set');
            });
            it('throws when asymmetric encrypt has no public key', async () => {
                ...
                await expect(cryptoNode.execute.call(mockExecuteFunctions)).rejects.toThrow('No encryption public key set');
            });
            ...
            it('throws when asymmetric decrypt has no private key', async () => {
                ...
                await expect(cryptoNode.execute.call(mockExecuteFunctions)).rejects.toThrow('No encryption private key set');
            });
        });
        describe('Symmetric round-trip', () => {
            it.each(symmetricCiphers)('encrypts and decrypts with %s', async (cipher) => {
                ...
                mockExecuteFunctions.getNodeParameter.mockImplementation((name) => encryptParams[name]);
                const encryptResult = await cryptoNode.execute.call(mockExecuteFunctions);
                const ciphertext = encryptResult[0][0].json.data;
                expect(typeof ciphertext).toBe('string');
                expect(ciphertext.length).toBeGreaterThan(0);
                expect(ciphertext).toMatch(/^[A-Za-z0-9+/]+=*$/);
                const decryptParams = mockDecryptParams(ciphertext, { cipher });
                mockExecuteFunctions.getNodeParameter.mockImplementation((name) => decryptParams[name]);
        ...
        describe('Asymmetric round-trip', () => {
            ...
            it('throws a clear error when RSA plaintext exceeds the key size limit', async () => {
                ...
                mockExecuteFunctions.getNodeParameter.mockImplementation((name) => encryptParams[name]);
                await expect(cryptoNode.execute.call(mockExecuteFunctions)).rejects.toThrow(NodeOperationError);
                await expect(cryptoNode.execute.call(mockExecuteFunctions)).rejects.toThrow('Plaintext is too large');
            });
        ...
        describe('Payload format validation', () => {
            it('throws a clear error when symmetric ciphertext is truncated', async () => {
                ...
                mockExecuteFunctions.getNodeParameter.mockImplementation((name) => decryptParams[name]);
                await expect(cryptoNode.execute.call(mockExecuteFunctions)).rejects.toThrow('Ciphertext is malformed or truncated');
            });
            it('throws a clear error on unsupported ciphertext version', async () => {
                ...
                mockExecuteFunctions.getNodeParameter.mockImplementation((name) => decryptParams[name]);
                await expect(cryptoNode.execute.call(mockExecuteFunctions)).rejects.toThrow('Unsupported ciphertext version 0xff');
            });
...
//# sourceMappingURL=Crypto.node.test.js.map