					...
					try {
						if (resource === 'message') {
							...
							if (operation === 'sendPrivate') {
								...
								const body: IMessage = { type: 'private', to, content };
							...
							if (operation === 'sendStream') {
								...
								const content = this.getNodeParameter('content', i) as string;
								const body: IMessage = { type: 'stream', to: stream, topic, content };
								responseData = await zulipApiRequest.call(this, 'POST', '/messages', body);
							}
							...
							if (operation === 'updateFile') {
								...
								const formData = { file: { //@ts-ignore
										value: binaryDataBuffer, options: { //@ts-ignore
											filename: items[i].binary[binaryProperty].fileName, contentType: items[i].binary[binaryProperty].mimeType } } }; //@ts-ignore
							...
							if (operation === 'create') {
								...
								if (jsonParameters) {
									...
									if (additionalFields.principals) {
										...
										//@ts-ignore
										additionalFields.principals.properties.map(
