...
describe('ExecuteWorkflowTrigger', () => {
    ...
    const mockNode = mock({ typeVersion: 1 });
    ...
    it('should filter out parent input in `Using Fields below` mode', async () => {
        ...
        const getFieldEntriesMock = getFieldEntries.mockReturnValue(mockNewParams);
        const result = await new ExecuteWorkflowTrigger().execute.call(executeFns);
        const expected = [
            [
...
//# sourceMappingURL=ExecuteWorkflowTrigger.node.test.js.map