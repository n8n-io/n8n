...
workflowInputs: {
  ...
  convertFieldsToString: true,
        ...
        properties: [
            ...
            {
                displayName: 'The "Local File" and "URL" sources are deprecated and will be removed in a future version. Import the workflow into this n8n instance and use the "Database" source, or paste its JSON into the "Parameter" source instead.',
            ...
            {
                displayName: '"Run once for each item" is deprecated and will be removed in a future version. To run the sub-workflow once per item, add a "Loop Over Items" node before this node and use "Run once with all items".',
                    ...
                    for (const [itemIndex, item] of output.entries()) {
                        if (item.pairedItem) {
                            ...
                            if (workflowRunData !== undefined) {
                                const pairedItem = findPairedItemThroughWorkflowData(workflowRunData, item, itemIndex);
