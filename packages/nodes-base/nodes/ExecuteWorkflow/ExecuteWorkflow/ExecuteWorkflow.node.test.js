...
describe('ExecuteWorkflow', () => {
    ...
    test('should document pass-through and declared workflow input mappings for the builder', () => {
        const workflowInputs = executeWorkflow.description.properties.find((property) => property.name === 'workflowInputs');
    ...
    test('should execute workflow in "each" mode and wait for sub-workflow completion', async () => {
        ...
        getWorkflowInfo.mockResolvedValue({ id: 'subWorkflowId' });
        ...
        expect(executeFunctions.executeWorkflow).toHaveBeenCalledWith({ id: 'subWorkflowId' }, [{ json: { key: 'value' }, index: 0, pairedItem: { item: 0 }, binary: undefined }], undefined, {
            parentExecution: {
    ...
    test('should handle errors and continue on fail, no items, < 1.3 version', async () => {
        ...
        getWorkflowInfo.mockRejectedValue(new Error('Test error'));
    ...
    test('should throw error if not continuing on fail', async () => {
        ...
        await expect(executeWorkflow.execute.call(executeFunctions)).rejects.toThrow('Error executing workflow with item at index 0');
    });
});
//# sourceMappingURL=ExecuteWorkflow.node.test.js.map