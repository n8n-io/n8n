...
describe('GraphQL Node', () => {
    ...
    describe('error response', () => {
        const createMockExecuteFunctions = (parameters) => {
        ...
        it('should format error response if response.errors is an array with objects', async () => {
            ...
            await expect(node.execute.call(mockExecuteFunctions)).rejects.toThrow('Bad format 1, Bad format 2');
        });
        ...
        it('should wrap request errors without copying request options', async () => {
            ...
            const node = new GraphQL();
            const error = await node.execute.call(mockExecuteFunctions).catch((error) => error);
    ...
    describe('credential allowed domains', () => {
        const createMockExecuteFunctions = (parameters, credentialsByName = {}) => {
            const mockExecuteFunctions = mockDeep();
            mockExecuteFunctions.getNodeParameter.mockImplementation((parameter, _idx, fallbackValue) => {
                return get(parameters, parameter) ?? fallbackValue;
            });
            ...
            mockExecuteFunctions.getCredentials.mockImplementation(async (type) => {
                const data = credentialsByName[type];
                if (!data)
                    throw new Error(`No credentials of type ${String(type)}`);
                return data;
            });
            mockExecuteFunctions.helpers.returnJsonArray.mockImplementation(returnJsonArray);
            mockExecuteFunctions.helpers.constructExecutionMetaData.mockImplementation(constructExecutionMetaData);
        ...
        it("throws when credential restriction is 'none'", async () => {
            ...
            const node = new GraphQL();
            await expect(node.execute.call(mockExecuteFunctions)).rejects.toThrow('configured to prevent use within an HTTP Request or GraphQL node');
        ...
        it("throws when restriction is 'domains' but no domains are configured", async () => {
            ...
            const node = new GraphQL();
            await expect(node.execute.call(mockExecuteFunctions)).rejects.toThrow('No allowed domains specified');
            expect(mockExecuteFunctions.helpers.request).not.toHaveBeenCalled();
        });
        ...
        it('calls requestWithAuthentication for a predefined credential type', async () => {
            ...
            expect(mockExecuteFunctions.helpers.requestWithAuthentication.mock.calls[0][0]).toBe('githubApi');
        ...
        it('does not set OAuth2 additional options for a predefined credential type outside the OAuth2 quirks table', async () => {
            ...
            const node = new GraphQL();
            await node.execute.call(mockExecuteFunctions);
            expect(mockExecuteFunctions.helpers.requestWithAuthentication.mock.calls[0][2]).toBeUndefined();
        });
        ...
        it('still attempts requestWithAuthentication when the initial credential fetch fails', async () => {
            ...
            mockExecuteFunctions.helpers.requestWithAuthentication.mockRejectedValue(new Error('No credentials found of type githubApi'));
            const node = new GraphQL();
...
//# sourceMappingURL=GraphQL.node.test.js.map