...
export class Freshservice {
    ...
    methods = {
        loadOptions: {
            async getAgents() {
                const { agents } = (await freshserviceApiRequest.call(this, 'GET', '/agents'));
                return toUserOptions(agents.filter((agent) => agent.active));
            },
            async getAgentGroups() {
                const { groups } = (await freshserviceApiRequest.call(this, 'GET', '/groups'));
                return toOptions(groups);
            },
            async getAgentRoles() {
                const { roles } = (await freshserviceApiRequest.call(this, 'GET', '/roles'));
                return toOptions(roles);
            },
            async getAssetTypes() {
                const { asset_types } = (await freshserviceApiRequest.call(this, 'GET', '/asset_types'));
                return toOptions(asset_types);
            },
            async getAssetTypeFields() {
                const assetType = this.getCurrentNodeParameter('assetTypeId');
                const { asset_type_fields } = (await freshserviceApiRequest.call(this, 'GET', `/asset_types/${assetType}/fields`));
                let fields = [];
                fields = fields
            ...
            async getDepartments() {
                const { departments } = (await freshserviceApiRequest.call(this, 'GET', '/departments'));
                return toOptions(departments);
            },
            async getLocations() {
                const { locations } = (await freshserviceApiRequest.call(this, 'GET', '/locations'));
                return toOptions(locations);
            },
            async getRequesters() {
                const { requesters } = (await freshserviceApiRequest.call(this, 'GET', '/requesters'));
