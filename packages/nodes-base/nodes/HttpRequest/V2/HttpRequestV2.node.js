import { NodeApiError, NodeOperationError, removeCircularRefs, NodeConnectionTypes, } from 'n8n-workflow';
import { sleep } from '@n8n/utils/sleep';
import { applyTemplatedAuth } from '@utils/templated-auth';
import { getAllowedDomains, getOAuth2AdditionalParameters, getSecrets, replaceNullValues, sanitizeUiMessage, } from '../GenericFunctions';
export class HttpRequestV2 {
    description;
    ...
    async execute() {
        ...
        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
            ...
            if (nodeCredentialType) {
                let credentialData;
                try {
                    credentialData = await this.getCredentials(nodeCredentialType, itemIndex);
                }
                catch { }
                if (credentialData) {
            ...
            if (itemIndex > 0 &&
                ...
                if (itemIndex % batchSize === 0) {
                    await sleep(options.batchInterval);
                }
            }
        ...
        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
            ...
            if (response.status !== 'fulfilled') {
                if (!this.continueOnFail()) {
                    // throw error;
                    throw new NodeApiError(this.getNode(), response, { itemIndex });
                }
                else {
