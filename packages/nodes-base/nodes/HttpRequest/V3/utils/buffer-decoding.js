const CHINESE_ENCODINGS = ['gb18030', 'gbk', 'gb2312'];
 ...
 */
function detectEncoding(contentType) {
    if (!contentType)
        return undefined;
    // Regex breakdown:
    // /charset=([^;,\s]+)/i
 ...
 */
export async function binaryToStringWithEncodingDetection(body, contentType, helpers) {
    let bufferedData;
    ...
    if (decodedString.includes(REPLACEMENT_CHAR) || HIGH_ASCII_PATTERN.test(decodedString)) {
        const detected = helpers.detectBinaryEncoding(bufferedData).toLowerCase();
        ...
        for (const chinese of CHINESE_ENCODINGS) {
            try {
                const reDecoded = await helpers.binaryToString(bufferedData, chinese);
                if (!reDecoded.includes(REPLACEMENT_CHAR) && reDecoded.length > 0)
                    return reDecoded;
            }
            catch { }
        }
...
//# sourceMappingURL=buffer-decoding.js.map