...
import { BINARY_ENCODING, NodeApiError, NodeConnectionTypes, NodeOperationError, jsonParse, removeCircularRefs, setSafeObjectProperty, } from 'n8n-workflow';
import { applyTemplatedAuth } from '@utils/templated-auth';
import { keysToLowercase } from '@utils/utilities';
import { mainProperties } from './Description';
import { binaryContentTypes, getAllowedDomains, getOAuth2AdditionalParameters, getSecrets, prepareRequestBody, reduceAsync, replaceNullValues, sanitizeUiMessage, setAgentOptions, updadeQueryParameterConfig, } from '../GenericFunctions';
...
function isPaginationRequestType(value) {
    return value === 'body' || value === 'headers' || value === 'qs';
}
export class HttpRequestV3 {
    description;
            ...
            try {
                ...
                if (authentication === 'genericCredentialType') {
                    ...
                    if (genericCredentialType === 'httpBasicAuth') {
                        ...
                        httpTemplatedCustomAuth = await this.getCredentials('httpTemplatedCustomAuth', itemIndex);
                    ...
                    let nodeCredentialData;
                    try {
                        nodeCredentialData = await this.getCredentials(nodeCredentialType, itemIndex);
                    }
                    catch { }
                ...
                if (typeof url !== 'string') {
                    ...
                    throw new NodeOperationError(this.getNode(), `URL parameter must be a string, got ${actualType}`);
                }
                url = url.trim();
                if (!url) {
                ...
                if (!url.startsWith('http://') && !url.startsWith('https://')) {
                    throw new NodeOperationError(this.getNode(), `Invalid URL: ${url}. URL must start with "http" or "https".`);
                }
                ...
                requestOptions = {
                    ...
                    sendCredentialsOnCrossOriginRedirect: sendCredentialsOnCrossOriginRedirect ?? defaultSendCredentialsOnCrossOriginRedirect,
                    allowedDomains,
                };
                ...
                if (sendBody && bodyParameters) {
                    if (specifyBody === 'keypair' || bodyContentType === 'multipart-form-data') {
                        requestOptions.body = await prepareRequestBody(bodyParameters, bodyContentType, nodeVersion, parametersToKeyValue);
                    }
                ...
                if (sendHeaders && headerParameters) {
                    ...
                    if (specifyHeaders === 'keypair') {
                        additionalHeaders = await reduceAsync(headerParameters.filter((header) => header.name), parametersToKeyValue);
                    }
                ...
                if (httpCustomAuth !== undefined) {
                    const customAuth = jsonParse(httpCustomAuth.json || '{}', { errorMessage: 'Invalid Custom Auth JSON' });
                ...
                if (pagination && pagination.paginationMode !== 'off') {
                    ...
                    if (pagination.paginationMode === 'updateAParameterInEachRequest') {
                        ...
                        if (parameters.length === 1 &&
                            parameters[0].name === '' &&
                            parameters[0].value === '') {
                            throw new NodeOperationError(this.getNode(), "At least one entry with 'Name' and 'Value' filled must be included in 'Parameters' to use 'Update a Parameter in Each Request' mode ");
                        }
                        pagination.parameters.parameters.forEach((parameter, index) => {
                            const parameterName = parameter.name;
                            if (parameterName === '') {
                                throw new NodeOperationError(this.getNode(), `Parameter name must be set for parameter [${index + 1}] in pagination settings`);
                            }
                            if (!isPaginationRequestType(parameter.type)) {
                                throw new NodeOperationError(this.getNode(), `Parameter type must be one of: body, headers, qs for parameter [${index + 1}] in pagination settings`);
                            }
                            paginationData.request[parameter.type] ??= Object.create(null);
                            const parameterValue = parameter.value;
                            if (parameterValue === '') {
                                throw new NodeOperationError(this.getNode(), `Some value must be provided for parameter [${index + 1}] in pagination settings, omitting it will result in an infinite loop`);
                            }
                            setSafeObjectProperty(paginationData.request[parameter.type], parameterName, parameterValue);
                        });
                    }
                    ...
                    const requestPromise = this.helpers.requestWithAuthenticationPaginated
                        .call(this, requestOptions, itemIndex, paginationData, nodeCredentialType ?? genericCredentialType, undefined, sanitizedRequest)
                        .catch((error) => {
            ...
            try {
                ...
                if (responseData.status !== 'fulfilled') {
                    ...
                    if (!this.continueOnFail()) {
                        ...
                        removeCircularRefs(responseData.reason);
                        ...
                        returnItems.push({
                            json: {
                                ...
                                ...(nodeVersion >= 4.5
                                    ? {
                                        details: createErrorDetails(this.getNode(), responseData.reason, itemIndex),
                                    }
                                    : {}),
                            },
                            pairedItem: {
                ...
                for (let [index, response] of Object.entries(responses)) {
                    ...
                    if (autoDetectResponseFormat) {
                        if (responseContentType.includes('application/json')) {
                            ...
                            if (!response.__bodyResolved) {
                                ...
                                const data = await binaryToStringWithEncodingDetection(response.body, responseContentType, this.helpers);
                    ...
                    if (responseFormat === 'file') {
                        ...
                        const preparedBinaryData = await this.helpers.prepareBinaryData(binaryData, undefined, mimeTypeFromResponse(responseContentType));
                        preparedBinaryData.fileName = setFilename(preparedBinaryData, 
                        // options is always set here: error items are skipped before this branch
                        requests[itemIndex].options, responseFileName);
                            ...
                            if (responseFormat === 'json' && typeof returnItem.body === 'string') {
                                if (isEmptyResponseBody(returnItem.body)) {
                                    ...
                                    try {
                                        ...
                                        throw new NodeOperationError(this.getNode(), 'Response body is not valid JSON. Change "Response Format" to "Text"', { itemIndex });
                                    }
                                }
                            }
                            returnItems.push({
