...
import { REDACTED, prepareRequestBody, sanitizeUiMessage, setAgentOptions, replaceNullValues, getSecrets, } from '../../GenericFunctions';
describe('HTTP Node Utils', () => {
    describe('prepareRequestBody', () => {
        ...
        it('should create FormData with knownLength for stream binary data', async () => {
            ...
            const mockReducer = vi.fn().mockResolvedValue({
                File: {
            ...
            const result = await prepareRequestBody(bodyParameters, 'multipart-form-data', 4.2, mockReducer);
    ...
    describe('sanitizeUiMessage', () => {
        ...
        it('should remove large Buffers', async () => {
            ...
            expect(sanitizeUiMessage(requestOptions, {}).body).toEqual('Binary data got replaced with this text. Original was a Buffer with a size of 900000 bytes.');
        });
        ...
        it.each([
            ...
            expect(sanitizeUiMessage({ method: 'POST', uri, body: upload() }, {}).body).toEqual(replacement);
        });
        it('should keep a Buffer nested in a formData field below the size limit', () => {
            ...
            expect(typeof sanitized.file.value).not.toBe('string');
        });
        ...
        it('should keep an own __proto__ key without retargeting the prototype', () => {
            ...
            const requestOptions = { method: 'POST', uri: 'https://example.com', body };
            const sanitized = sanitizeUiMessage(requestOptions, {}).body;
        ...
        it('should keep the placeholder when the body also carries auth data', () => {
            ...
            expect(sanitizeUiMessage(requestOptions, { body: ['token'] }).body).toEqual(STREAM_REPLACEMENT);
        });
        it('should remove keys that contain sensitive data and do not modify requestOptions', async () => {
            ...
            expect(sanitizeUiMessage(requestOptions, {
