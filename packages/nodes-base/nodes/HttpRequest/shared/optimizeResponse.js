...
function htmlOptimizer(ctx, itemIndex, maxLength) {
    ...
    return (response) => {
        if (typeof response !== 'string') {
            throw new NodeOperationError(ctx.getNode(), `The response type must be a string. Received: ${typeof response}`, { itemIndex });
        }
        const returnData = [];
    ...
    return (response) => {
        ...
        if (typeof response === 'string') {
            try {
                ...
                throw new NodeOperationError(ctx.getNode(), `Received invalid JSON from response '${response}'`, { itemIndex });
            }
        }
        if (typeof responseData !== 'object' || !responseData) {
            throw new NodeOperationError(ctx.getNode(), 'The response type must be an object or an array of objects', { itemIndex });
        }
        ...
        if (!Array.isArray(responseData)) {
            if (dataField) {
                if (!Object.prototype.hasOwnProperty.call(responseData, dataField)) {
                    throw new NodeOperationError(ctx.getNode(), `Target field "${dataField}" not found in response.`, {
                        itemIndex,
...
export const configureResponseOptimizer = (ctx, itemIndex) => {
...
export const optimizeResponseProperties = [
    {
