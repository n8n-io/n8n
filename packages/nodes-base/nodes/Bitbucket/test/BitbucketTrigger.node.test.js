...
describe('BitbucketTrigger', () => {
    let bitbucketTrigger;
    let bitbucketApiRequestSpy;
    let bitbucketApiRequestAllItemsSpy;
    const mockNode = {
        id: 'test-node-id',
    ...
    describe('credential test', () => {
        const mockCredentialTestFunctions = mockDeep();
        beforeEach(() => {
        ...
        it('should return success for valid credentials', async () => {
            ...
            const result = await bitbucketTrigger.methods.credentialTest.bitbucketApiTest.call(mockCredentialTestFunctions, mockCredentials);
    ...
    describe('load options methods', () => {
        ...
        describe('getWorkspaceEvents', () => {
            it('should return workspace events', async () => {
                ...
                const result = await bitbucketTrigger.methods.loadOptions.getWorkspaceEvents.call(mockLoadOptionsFunctions);
                expect(bitbucketApiRequestAllItemsSpy).toHaveBeenCalledWith('values', 'GET', '/hook_events/workspace');
                expect(result).toEqual([
                    {
                        name: 'repo:push',
                        value: 'repo:push',
        ...
        describe('getRepositoriesEvents', () => {
            it('should return repository events', async () => {
                ...
                bitbucketApiRequestAllItemsSpy.mockResolvedValue(mockEvents);
                const result = await bitbucketTrigger.methods.loadOptions.getRepositoriesEvents.call(mockLoadOptionsFunctions);
                expect(bitbucketApiRequestAllItemsSpy).toHaveBeenCalledWith('values', 'GET', '/hook_events/repository');
                expect(result).toEqual([
                    {
                        name: 'repo:push',
                        value: 'repo:push',
        ...
        describe('getRepositories', () => {
            it('should return repositories for workspace', async () => {
                ...
                const result = await bitbucketTrigger.methods.loadOptions.getRepositories.call(mockLoadOptionsFunctions);
                expect(mockLoadOptionsFunctions.getCurrentNodeParameter).toHaveBeenCalledWith('workspace');
                expect(bitbucketApiRequestAllItemsSpy).toHaveBeenCalledWith('values', 'GET', '/repositories/test-workspace');
                expect(result).toEqual([
                    {
        ...
        describe('getWorkspaces', () => {
            it('should return workspaces', async () => {
                ...
                const result = await bitbucketTrigger.methods.loadOptions.getWorkspaces.call(mockLoadOptionsFunctions);
                expect(bitbucketApiRequestAllItemsSpy).toHaveBeenCalledWith('values', 'GET', '/user/workspaces');
                expect(result).toEqual([
                    {
                        name: 'Workspace 1',
                        value: 'workspace1',
    ...
    describe('webhook methods', () => {
        ...
        describe('checkExists', () => {
            it('should return true if webhook exists for workspace', async () => {
                ...
                mockHookFunctions.getNodeParameter.mockImplementation((paramName) => {
                    if (paramName === 'resource')
                        return 'workspace';
                    if (paramName === 'workspace')
                        return 'test-workspace';
                    return undefined;
                });
                bitbucketApiRequestSpy.mockResolvedValue(mockHooks);
                const result = await bitbucketTrigger.webhookMethods.default.checkExists.call(mockHookFunctions);
                expect(bitbucketApiRequestSpy).toHaveBeenCalledWith('GET', '/workspaces/test-workspace/hooks');
            ...
            it('should return true if webhook exists for repository', async () => {
                ...
                mockHookFunctions.getNodeParameter.mockImplementation((paramName) => {
                    ...
                    if (paramName === 'workspace')
                        return 'test-workspace';
                    if (paramName === 'repository')
                        return 'test-repo';
                    return undefined;
                });
                bitbucketApiRequestSpy.mockResolvedValue(mockHooks);
                const result = await bitbucketTrigger.webhookMethods.default.checkExists.call(mockHookFunctions);
                expect(bitbucketApiRequestSpy).toHaveBeenCalledWith('GET', '/repositories/test-workspace/test-repo/hooks');
                expect(result).toBe(true);
            });
        ...
        describe('create', () => {
            it('should create webhook for workspace', async () => {
                ...
                expect(bitbucketApiRequestSpy).toHaveBeenCalledWith('POST', '/workspaces/test-workspace/hooks', {
            ...
            it('should create webhook for repository', async () => {
                ...
                bitbucketApiRequestSpy.mockResolvedValue(mockResponse);
                const result = await bitbucketTrigger.webhookMethods.default.create.call(mockHookFunctions);
                expect(bitbucketApiRequestSpy).toHaveBeenCalledWith('POST', '/repositories/test-workspace/test-repo/hooks', {
                    description: 'n8n webhook',
                    url: 'https://test.n8n.io/webhook/test',
                    active: true,
        ...
        describe('delete', () => {
            it('should delete webhook for workspace', async () => {
                ...
                expect(bitbucketApiRequestSpy).toHaveBeenCalledWith('DELETE', '/workspaces/test-workspace/hooks/12345678-1234-1234-1234-123456789012');
                expect(result).toBe(true);
            ...
            it('should delete webhook for repository', async () => {
                ...
                bitbucketApiRequestSpy.mockResolvedValue({});
                const result = await bitbucketTrigger.webhookMethods.default.delete.call(mockHookFunctions);
                expect(bitbucketApiRequestSpy).toHaveBeenCalledWith('DELETE', '/repositories/test-workspace/test-repo/hooks/12345678-1234-1234-1234-123456789012');
                expect(result).toBe(true);
                expect(staticData.webhookId).toBeUndefined();
            });
...
//# sourceMappingURL=BitbucketTrigger.node.test.js.map