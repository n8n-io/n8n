	...
	methods = {
		loadOptions: {
			...
			async getCurrencies(
				...
				const { data } = await stripeApiRequest.call(
					this,
					'GET',
					'/country_specs',
					{},
						...
						try {
							if (resource === 'balance') {
								...
								if (operation === 'add') {
									...
									const body = { source: this.getNodeParameter('token', i) } as IDataObject;
								...
								if (operation === 'create') {
									...
									const body = { customer: this.getNodeParameter('customerId', i), currency: (this.getNodeParameter('currency', i) as string).toLowerCase(), amount: this.getNodeParameter('amount', i), source: this.getNodeParameter('source', i) } as IDataObject;
								...
								if (operation === 'create') {
									...
									const body = { duration: this.getNodeParameter('duration', i) } as IDataObject;
								...
								if (operation === 'create') {
									...
									const body = { name: this.getNodeParameter('name', i) } as IDataObject;
								...
								if (operation === 'create') {
									...
									const body = { type: this.getNodeParameter('type', i), amount: this.getNodeParameter('amount', i), currency: this.getNodeParameter('currency', i) } as IDataObject;
								...
								if (operation === 'create') {
									...
									body.card = { number: this.getNodeParameter('number', i), exp_month: this.getNodeParameter('expirationMonth', i), exp_year: this.getNodeParameter('expirationYear', i), cvc: this.getNodeParameter('cvc', i) };
