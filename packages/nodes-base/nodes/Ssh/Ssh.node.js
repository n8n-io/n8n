...
import { BINARY_ENCODING, NodeConnectionTypes, NodeOperationError, sanitizeFilename, } from 'n8n-workflow';
...
async function resolveHomeDir(path, ssh, itemIndex) {
    ...
    if (path.startsWith('~')) {
        throw new NodeOperationError(this.getNode(), 'Invalid path. Replace "~" with home directory or "~/"', {
            itemIndex,
        });
    }
    return path;
                ...
                try {
                    if (!credentials.privateKey) {
                        ...
                        const options = {
                            ...
                            username: credentials.username,
                            port: credentials.port,
                            privateKey: formatPemBlock(credentials.privateKey),
                        };
