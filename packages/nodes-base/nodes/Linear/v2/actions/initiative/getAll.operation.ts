...
export async function execute(
	...
	const body = {
		query: `query Initiatives($first: Int, $after: String) {
	...
	try {
		const initiatives = await linearApiRequestAllItems.call(this, 'data.initiatives', body, limit);
		returnData.push(
			...this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(initiatives), {
				itemData: { item: 0 },
			}),
		);
	} catch (error) {
