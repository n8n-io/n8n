	...
	webhookMethods = {
		default: {
			...
			async create(this: IHookFunctions): Promise<boolean> {
				...
				const body = {
					query: `
						mutation webhookCreate($url: String!, $teamId: String, $resources: [String!]!, $allPublicTeams: Boolean) {
							webhookCreate(
								input: {
									url: $url
									teamId: $teamId
					...
					variables: {
						url: webhookUrl,
						teamId: teamId === 'all_public_teams' ? null : teamId,
						resources: resources.map(capitalizeFirstLetter),
						allPublicTeams: teamId === 'all_public_teams' ? true : false,
					},
				};
