...
import { certificateRequestFields, certificateRequestOperations, } from './CertificateRequestDescription';
import { encryptPassphrase, venafiApiRequest, venafiApiRequestAllItems } from './GenericFunctions';
export class VenafiTlsProtectCloud {
    description = {
    ...
    methods = {
        loadOptions: {
            ...
            async getCertificateIssuingTemplates() {
                ...
                const { certificateIssuingTemplateAliasIdMap } = (await venafiApiRequest.call(this, 'GET', `/outagedetection/v1/applications/${currentApplication}`));
            ...
            try {
                if (resource === 'certificateRequest') {
                    ...
                    if (operation === 'create') {
                        ...
                        responseData = await venafiApiRequest.call(this, 'POST', '/outagedetection/v1/certificaterequests', body, qs);
                    ...
                    if (operation === 'get') {
                        ...
                        responseData = await venafiApiRequest.call(this, 'GET', `/outagedetection/v1/certificaterequests/${certificateId}`, {}, qs);
                    }
                    ...
                    if (operation === 'getMany') {
                        ...
                        if (returnAll) {
                            responseData = await venafiApiRequestAllItems.call(this, 'certificateRequests', 'GET', '/outagedetection/v1/certificaterequests', {}, qs);
                        }
                        else {
                            const limit = this.getNodeParameter('limit', i);
                            responseData = await venafiApiRequest.call(this, 'GET', '/outagedetection/v1/certificaterequests', {}, qs);
                ...
                if (resource === 'certificate') {
                    ...
                    if (operation === 'delete') {
                        ...
                        responseData = await venafiApiRequest.call(this, 'POST', '/outagedetection/v1/certificates/deletion', { certificateIds: [certificateId] });
                    ...
                    if (operation === 'download') {
                        ...
                        if (downloadItem === 'certificate') {
                            ...
                            responseData = await venafiApiRequest.call(this, 'GET', `/outagedetection/v1/certificates/${certificateId}/contents`, {}, qs, { encoding: null, json: false, resolveWithFullResponse: true, cert: true });
                        }
                        else {
                            ...
                            const encryptedValues = (await encryptPassphrase.call(this, certificateId, privateKeyPassphrase, keystorePassphrase));
                            body.encryptedPrivateKeyPassphrase = encryptedValues[0];
                            if (exportFormat === 'JKS') {
                                body.encryptedKeystorePassphrase = encryptedValues[1];
                            }
                            responseData = await venafiApiRequest.call(this, 'POST', `/outagedetection/v1/certificates/${certificateId}/keystore`, body, {}, { encoding: null, json: false, resolveWithFullResponse: true });
                        }
                    ...
                    if (operation === 'get') {
                        const certificateId = this.getNodeParameter('certificateId', i);
                        responseData = await venafiApiRequest.call(this, 'GET', `/outagedetection/v1/certificates/${certificateId}`, {}, qs);
                    }
                    ...
                    if (operation === 'getMany') {
                        ...
                        if (returnAll) {
                            responseData = await venafiApiRequestAllItems.call(this, 'certificates', 'GET', '/outagedetection/v1/certificates', {}, qs);
                        }
                        else {
                            qs.limit = this.getNodeParameter('limit', i);
                            responseData = await venafiApiRequest.call(this, 'GET', '/outagedetection/v1/certificates', {}, qs);
                            responseData = responseData.certificates;
                        }
                    }
