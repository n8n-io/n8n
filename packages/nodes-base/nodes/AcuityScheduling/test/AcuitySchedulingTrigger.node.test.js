...
describe('AcuitySchedulingTrigger', () => {
    ...
    let response;
    let ctx;
    const requestBody = { action: 'appointment.scheduled', id: 123 };
    const buildContext = (body = requestBody) => {
        response = {
    ...
    describe('webhook', () => {
        it('triggers workflow when signature is valid and resolveData is false', async () => {
            ctx.getNodeParameter.mockImplementation((name) => name === 'resolveData' ? false : undefined);
...
//# sourceMappingURL=AcuitySchedulingTrigger.node.test.js.map