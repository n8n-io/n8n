...
function uuidValidateWithoutDashes(value) {
    ...
    if (value.length == 32) {
        ...
        if (uuidValidate(strWithDashes))
            return true;
    }
    throw new NodeOperationError(this.getNode(), `The relation id "${value}" is not a valid uuid with optional dashes.`);
}
const apiVersion = {
    1: '2021-05-13',
    2: '2021-08-16',
...
export async function notionApiRequest(method, resource, body = {}, qs = {}, uri, option = {}) {
    try {
        let options = {
            headers: {
...
export async function notionApiRequestAllItems(propertyName, method, endpoint, body = {}, query = {}) {
...
export async function notionApiRequestGetBlockChildrens(blocks, responseData = [], limit) {
    if (blocks.length === 0)
        return responseData;
        ...
        case 'multi_select':
            ...
            result = {
                ...
                multi_select: (Array.isArray(multiSelectValue)
                    ...
                    .map((option) => (!uuidValidate(option) ? { name: option } : { id: option })),
            };
            break;
        case 'email':
            result = {
...
export function mapProperties(properties, timezone, version = 1) {
    return properties
...
export function mapSorting(data) {
...
export function mapFilters(filtersList, timezone) {
    return filtersList.reduce((obj, value) => {
        let key = getNameAndType(value.key).type;
        let valuePropertyName = key === 'last_edited_time' ? value[camelCase(key)] : value[`${camelCase(key)}Value`];
...
export function simplifyProperties(properties) {
    const results = {};
...
export function getPropertyTitle(properties) {
...
const foldedSnakeCase = (value) => snakeCase(value.replace(/[^\x20-\x7E]/g, ' '));
...
export function simplifyObjects(objects, download = false, version = 2) {
...
export function getFormattedChildren(children) {
    const results = [];
...
export function getConditions() {
    ...
    elements.push({
        ...
        options: Object.keys(formula).map((key) => ({ name: capitalCase(key), value: key })),
        default: '',
    ...
    for (const key of Object.keys(formula)) {
        elements.push({
            ...
            options: formula[key].map((entry) => ({ name: capitalCase(entry), value: entry })),
            default: '',
            description: 'The value of the property to filter by',
        });
    }
...
export async function downloadFiles(records, pairedItem) {
    const elements = [];
...
export function getSearchFilters(resource) {
    return [
        {
            displayName: 'Filter',
            name: 'filterType',
 ...
 */
export function extractDatabaseMentionRLC(blockValues) {
...
export function simplifyBlocksOutput(blocks, rootId) {
    for (const block of blocks) {
...
export function extractBlockId(nodeVersion, itemIndex) {
    let blockId;
    if (nodeVersion < 2.2) {
...
export const prepareNotionError = (node, error, itemIndex) => {
