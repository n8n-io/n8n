...
import { downloadFiles } from '../../../shared/GenericFunctions';
import { getIconFromOptions, getPageCreateContent, getPageId, handleOperationError, jsonParse, mapProperties, mapSorting, normalizePropertyValues, simplifyObjects, validateJSON, } from '../../helpers/utils';
import { getDataSourceProperties, notionApiRequestAllItemsV3, notionApiRequestV3, } from '../../transport';
import { blockBuilder, dataSourceLocator, iconOptions, pageLocator, returnAllOrLimit, } from '../common.descriptions';
const PROPERTY_KEY_DESCRIPTION = 'Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/">expression</a>. Use the format <code>Property Name|property_type</code>, for example <code>Due Date|date</code>.';
...
function propertiesUi(operation, keyLoadOptionsMethod, keyLoadOptionsDependsOn, selectLoadOptionsMethod, selectLoadOptionsDependsOn) {
    return {
        ...
        options: [
            {
                ...
                values: [
                    ...
                    },
                    {
                        ...
                        description: 'List of users. Choose from the list, or specify IDs using an <a href="https://docs.n8n.io/code/expressions/">expression</a>.',
                    },
                    {
...
export const description = [
    ...
    propertiesUi('create', 'getDataSourcePropertiesOptions', ['dataSourceId'], 'getPropertySelectValues', ['dataSourceId', '&key']),
    propertiesUi('update', 'getDataSourcePropertiesFromPage', ['pageId'], 'getDataSourceOptionsFromPage', ['pageId', '&key']),
    {
...
async function getTitleKey(dataSourceId) {
...
export async function create(items) {
    ...
    for (let i = 0; i < items.length; i++) {
        try {
            ...
            if (!dataSourceId) {
                throw new NodeOperationError(this.getNode(), 'DataSource ID is required to create a database page', {
                    itemIndex: i,
                });
            }
            const title = this.getNodeParameter('title', i, '');
            if (!title.trim()) {
                throw new NodeOperationError(this.getNode(), 'Title is required to create a database page', {
                    itemIndex: i,
                });
            }
            ...
            const propertyValues = this.getNodeParameter('propertiesUi.propertyValues', i, []);
            if (propertyValues.length) {
                const mappedProperties = mapProperties.call(this, normalizePropertyValues(propertyValues), this.getTimezone(), 3);
...
export async function get(items) {
    ...
    for (let i = 0; i < items.length; i++) {
        try {
            let response = await notionApiRequestV3.call(this, 'GET', `/pages/${getPageId.call(this, i)}`);
            const download = this.getNodeParameter('options.downloadFiles', i, false);
            const simple = this.getNodeParameter('simple', i);
            let executionData;
            if (download) {
                executionData = await downloadFiles.call(this, [response], [{ item: i }]);
                if (simple)
                    executionData = simplifyObjects(executionData, true, 3);
            }
            else {
                if (simple)
                    response = simplifyObjects(response, false, 3);
                executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(response), { itemData: { item: i } });
            }
            returnData.push.apply(returnData, executionData);
        }
        catch (error) {
...
export async function getAll(items) {
    ...
    for (let i = 0; i < items.length; i++) {
        try {
            ...
            const response = await notionApiRequestAllItemsV3.call(this, 'results', 'POST', `/data_sources/${dataSourceId}/query`, body, limit ? { limit } : {});
            const download = this.getNodeParameter('options.downloadFiles', i, false);
            const simple = this.getNodeParameter('simple', i);
            let executionData;
            if (download) {
                ...
                executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(output), { itemData: { item: i } });
            }
            returnData.push.apply(returnData, executionData);
        }
        catch (error) {
...
export async function update(items) {
    ...
    for (let i = 0; i < items.length; i++) {
        try {
            ...
            if (icon)
                body.icon = icon;
            let response = await notionApiRequestV3.call(this, 'PATCH', `/pages/${getPageId.call(this, i)}`, body);
            if (this.getNodeParameter('simple', i))
                response = simplifyObjects(response, false, 3);
            const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(response), { itemData: { item: i } });
            returnData.push.apply(returnData, executionData);
...
//# sourceMappingURL=DatabasePage.resource.js.map