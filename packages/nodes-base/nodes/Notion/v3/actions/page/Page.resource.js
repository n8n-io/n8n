import { formatTitle, getIconFromOptions, getMarkdownUpdateBody, getPageCreateContent, getPageId, getSearchSort, handleOperationError, simplifyObjects, } from '../../helpers/utils';
import { notionApiRequestAllItemsV3, notionApiRequestV3 } from '../../transport';
import { blockBuilder, iconOptions, pageLocator, returnAllOrLimit, searchOptions, } from '../common.descriptions';
...
export async function archive(items) {
    const returnData = [];
    for (let i = 0; i < items.length; i++) {
        try {
            let response = await notionApiRequestV3.call(this, 'PATCH', `/pages/${getPageId.call(this, i)}`, { in_trash: true });
    ...
    for (let i = 0; i < items.length; i++) {
        try {
            const body = {
                ...
                properties: formatTitle(this.getNodeParameter('title', i)),
                ...getPageCreateContent.call(this, i),
            };
            const icon = getIconFromOptions.call(this, i);
            if (icon)
                body.icon = icon;
            let response = await notionApiRequestV3.call(this, 'POST', '/pages', body);
            if (this.getNodeParameter('simple', i))
                response = simplifyObjects(response, false, 3);
            const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(response), { itemData: { item: i } });
            returnData.push.apply(returnData, executionData);
...
export async function getMarkdown(items) {
    ...
    for (let i = 0; i < items.length; i++) {
        try {
            ...
            const response = await notionApiRequestV3.call(this, 'GET', `/pages/${getPageId.call(this, i)}/markdown`, {}, includeTranscript ? { include_transcript: true } : {});
            const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(response), { itemData: { item: i } });
            returnData.push.apply(returnData, executionData);
        }
        catch (error) {
...
export async function search(items) {
    ...
    for (let i = 0; i < items.length; i++) {
        try {
            const body = { filter: { property: 'object', value: 'page' } };
            ...
            let response = await notionApiRequestAllItemsV3.call(this, 'results', 'POST', '/search', body, limit ? { limit } : {});
            if (this.getNodeParameter('simple', i))
                response = simplifyObjects(response, false, 3);
            const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(response), { itemData: { item: i } });
            returnData.push.apply(returnData, executionData);
...
export async function updateMarkdown(items) {
    const returnData = [];
    for (let i = 0; i < items.length; i++) {
        try {
            const response = await notionApiRequestV3.call(this, 'PATCH', `/pages/${getPageId.call(this, i)}/markdown`, getMarkdownUpdateBody.call(this, i));
            const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(response), { itemData: { item: i } });
            returnData.push.apply(returnData, executionData);
        }
        catch (error) {
...
//# sourceMappingURL=Page.resource.js.map