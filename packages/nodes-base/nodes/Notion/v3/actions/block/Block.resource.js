import { extractBlockId, formatBlocks, handleOperationError, normalizeBlockValues, simplifyBlocksOutput, } from '../../helpers/utils';
...
async function fetchNestedBlocks(blocks, responseData = [], limit) {
    for (const block of blocks) {
        ...
        const children = await notionApiRequestAllItemsV3.call(this, 'results', 'GET', `/blocks/${blockId}/children`);
...
export const description = [
    ...
    },
    {
        ...
        default: '',
        displayOptions: { show: { resource: ['block'], operation: ['append'] } },
        description: 'ID of the block after which to insert the new blocks. Leave empty to append at the end.',
    },
...
export async function append(items) {
    const returnData = [];
    for (let i = 0; i < items.length; i++) {
        try {
            const blockIdValue = extractBlockId.call(this, i);
            const rawBlockValues = this.getNodeParameter('blockUi.blockValues', i, []);
            ...
            const afterBlockId = this.getNodeParameter('afterBlockId', i, '');
            const body = { children: formatBlocks(blockValuesData) };
            if (afterBlockId) {
                body.position = {
                    type: 'after_block',
            ...
            const response = await notionApiRequestV3.call(this, 'PATCH', `/blocks/${blockIdValue}/children`, body);
...
export async function getMarkdown(items) {
    const returnData = [];
    for (let i = 0; i < items.length; i++) {
        try {
            const blockIdValue = extractBlockId.call(this, i);
            const includeTranscript = this.getNodeParameter('includeTranscript', i);
            // uses page endpoint, but it supports block ids too
            const response = await notionApiRequestV3.call(this, 'GET', `/pages/${blockIdValue}/markdown`, {}, includeTranscript ? { include_transcript: true } : {});
            const executionData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(response), { itemData: { item: i } });
            returnData.push.apply(returnData, executionData);
        }
        catch (error) {
...
export async function getAll(items) {
    ...
    for (let i = 0; i < items.length; i++) {
        try {
            ...
            let response = await notionApiRequestAllItemsV3.call(this, 'results', 'GET', `/blocks/${blockIdValue}/children`, {}, limit ? { page_size: Math.min(limit, 100), limit } : {});
            const fetchNestedBlocksOption = this.getNodeParameter('fetchNestedBlocks', i);
...
//# sourceMappingURL=Block.resource.js.map