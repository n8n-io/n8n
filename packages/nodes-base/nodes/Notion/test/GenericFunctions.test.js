...
import { blockUrlExtractionRegexp, blockUrlValidationRegexp, databasePageUrlExtractionRegexp, databasePageUrlValidationRegexp, databaseUrlExtractionRegexp, databaseUrlValidationRegexp, } from '../shared/constants';
import { NotionTrigger } from '../NotionTrigger.node';
import { extractDatabaseMentionRLC, extractPageId, formatBlocks, getPageId, notionApiRequest, notionApiRequestAllItems, simplifyObjects, } from '../shared/GenericFunctions';
...
const collectNotionUrlExpressions = (value) => {
...
describe('Test Notion', () => {
    ...
    describe('extractPageId From URL', () => {
        ...
        test.each([
            ['www.notion.so', 'https://www.notion.so/fake-instance'],
            ['www.notion.com', 'https://www.notion.com/fake-instance'],
        ])('should return the last part after splitting by "-" when URL contains multiple "-" on %s', (_, baseUrl) => {
            for (const testId of testIds) {
        ...
        test.each([
            ['www.notion.so', 'https://www.notion.so/fake-instance'],
            ['www.notion.com', 'https://www.notion.com/fake-instance'],
        ])('should return the last part after splitting by "-" when URL contains one "-" on %s', (_, baseUrl) => {
            for (const testId of testIds) {
...
describe('Test Notion resource locator URL regexes', () => {
    ...
    ])('should validate and extract IDs from %s on %s', (_, __, validationRegexp, extractionRegexp, url) => {
...
describe('Test Notion resource locator generated URLs', () => {
    ...
    it('should generate notion.com URL expressions across all node descriptions', () => {
        ...
        expect(urlExpressions).not.toContain('=https://www.notion.(?:so|com)/{{$value.replace(/-/g, "")}}');
    });
});
...
describe('Test Notion, getPageId', () => {
    ...
    it('should throw error when page ID cannot be extracted', () => {
        ...
        mockExecuteFunctions.getNodeParameter.mockReturnValue(page);
        mockExecuteFunctions.getNode.mockReturnValue(mock({ name: 'Notion', type: 'notion' }));
        expect(() => getPageId.call(mockExecuteFunctions, 0)).toThrow(NodeOperationError);
        expect(() => getPageId.call(mockExecuteFunctions, 0)).toThrow('Could not extract page ID from URL: https://www.notion.so/invalid-url');
    });
    it('should throw error when page value is empty', () => {
        ...
        mockExecuteFunctions.getNode.mockReturnValue(mock({ name: 'Notion', type: 'notion' }));
        expect(() => getPageId.call(mockExecuteFunctions, 0)).toThrow(NodeOperationError);
        expect(() => getPageId.call(mockExecuteFunctions, 0)).toThrow('Could not extract page ID from URL: ');
    });
    it('should throw error when page value is undefined', () => {
        ...
        mockExecuteFunctions.getNode.mockReturnValue(mock({ name: 'Notion', type: 'notion' }));
        expect(() => getPageId.call(mockExecuteFunctions, 0)).toThrow(NodeOperationError);
        expect(() => getPageId.call(mockExecuteFunctions, 0)).toThrow('Could not extract page ID from URL: undefined');
    });
    it('should throw error when page value is not a string', () => {
        ...
        mockExecuteFunctions.getNode.mockReturnValue(mock({ name: 'Notion', type: 'notion' }));
        expect(() => getPageId.call(mockExecuteFunctions, 0)).toThrow(NodeOperationError);
        expect(() => getPageId.call(mockExecuteFunctions, 0)).toThrow('Could not extract page ID from URL: 123');
    });
});
describe('Test Notion, notionApiRequest', () => {
    ...
    beforeEach(() => {
        mockExecuteFunctions = mock();
        mockExecuteFunctions.getNode.mockReturnValue(mock({ typeVersion: 2 }));
    ...
    it('should use notionApi credential when authentication is apiKey', async () => {
        ...
        expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('notionApi', expect.objectContaining({ method: 'GET', uri: 'https://api.notion.com/v1/users' }));
    });
    it('should use notionOAuth2Api credential when authentication is oAuth2', async () => {
        ...
        await notionApiRequest.call(mockExecuteFunctions, 'GET', '/users');
        expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('notionOAuth2Api', expect.objectContaining({ method: 'GET', uri: 'https://api.notion.com/v1/users' }));
    });
    it('should default to notionApi credential when authentication parameter is not set', async () => {
        ...
        await notionApiRequest.call(mockExecuteFunctions, 'GET', '/users');
        expect(mockExecuteFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('notionApi', expect.objectContaining({ method: 'GET' }));
    });
});
describe('Test Notion, notionApiRequestAllItems', () => {
    ...
    it('should extract limit from query, remove it, and stop pagination early', async () => {
        ...
        const result = await notionApiRequestAllItems.call(mockExecuteFunctions, 'results', 'GET', '/blocks/test-block/children', {}, query);
    ...
    it('should paginate through all pages for non-block resources', async () => {
        ...
        mockExecuteFunctions.helpers.requestWithAuthentication
            .mockResolvedValueOnce(page1)
            .mockResolvedValueOnce(page2);
        const result = await notionApiRequestAllItems.call(mockExecuteFunctions, 'results', 'POST', '/search', {}, { limit: 5 });
    ...
    it('should return all items without slicing when no limit is provided', async () => {
        ...
        mockExecuteFunctions.helpers.requestWithAuthentication.mockResolvedValueOnce(page1);
        const result = await notionApiRequestAllItems.call(mockExecuteFunctions, 'results', 'POST', '/search', {});
        expect(result).toEqual([{ id: '1' }, { id: '2' }]);
