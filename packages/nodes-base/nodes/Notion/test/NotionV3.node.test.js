...
const mockDownloadFiles = GenericFunctions.downloadFiles;
...
function createMockExecuteFunction(nodeParameters, options = {}) {
    return {
        getInputData: () => [{ json: {} }],
...
describe('NotionV3', () => {
    ...
    it('creates database pages with a data source parent', async () => {
        ...
        expect(mockNotionApiRequest).toHaveBeenCalledWith('POST', '/pages', expect.objectContaining({
    ...
    it('does not create database pages without a title', async () => {
        ...
        await expect(node.execute.call(context)).rejects.toThrow('Title is required to create a database page');
    ...
    it('appends blocks after a specific block', async () => {
        ...
        await node.execute.call(context);
        expect(mockNotionApiRequest).toHaveBeenCalledWith('PATCH', '/blocks/parent-block-id/children', expect.objectContaining({
            position: {
                type: 'after_block',
    ...
    it('gets block markdown through the page markdown endpoint', async () => {
        ...
        await node.execute.call(context);
        expect(mockNotionApiRequest).toHaveBeenCalledWith('GET', '/pages/block-id/markdown', {}, { include_transcript: true });
    });
    it('extracts block IDs from notion.com block URL hashes', async () => {
        ...
        const context = createMockExecuteFunction({
            ...
            blockId: {
                __rl: true,
                mode: 'url',
                value: 'https://www.notion.com/Block-Test-88888ccc303e4f44847f27d24bd7ad8e?pvs=4#550e8400e29b41d4a716446655440000',
            },
            includeTranscript: false,
        });
        await node.execute.call(context);
        expect(mockNotionApiRequest).toHaveBeenCalledWith('GET', '/pages/550e8400e29b41d4a716446655440000/markdown', {}, {});
    });
    ...
    it('extracts page IDs from full URLs with page query parameters', async () => {
        ...
        const context = createMockExecuteFunction({
            ...
            pageId: {
                __rl: true,
                mode: 'url',
                value: 'https://www.notion.com/0fe2f7de558b471eab07e9d871cdf4a9?v=f2d424ba0c404733a3f500c78c881610&p=550e8400e29b41d4a716446655440000&pm=s',
            },
            markdownUpdateType: 'replace_content',
            markdown: '# New content',
        });
        await node.execute.call(context);
        expect(mockNotionApiRequest).toHaveBeenCalledWith('PATCH', '/pages/550e8400e29b41d4a716446655440000/markdown', {
            type: 'replace_content',
            replace_content: { new_str: '# New content' },
        });
    });
    ...
    it('updates database page date properties', async () => {
        ...
        await node.execute.call(context);
        expect(mockNotionApiRequest).toHaveBeenCalledWith('PATCH', '/pages/page-id', expect.objectContaining({
            properties: {
                Due: {
                    type: 'date',
                    date: {
    ...
    it('fetches nested blocks when requested', async () => {
        ...
        const result = await node.execute.call(context);
        expect(mockNotionApiRequestAllItems).toHaveBeenCalledWith('results', 'GET', '/blocks/parent-block-id/children', {}, {});
        expect(mockNotionApiRequestAllItems).toHaveBeenCalledWith('results', 'GET', '/blocks/child-block-id/children');
    ...
    it('queries data source pages with structured filters and sorting', async () => {
        ...
        await node.execute.call(context);
        expect(mockNotionApiRequestAllItems).toHaveBeenCalledWith('results', 'POST', '/data_sources/data-source-id/query', {
            filter: {
                and: [
                    {
                        property: 'Name',
...
//# sourceMappingURL=NotionV3.node.test.js.map