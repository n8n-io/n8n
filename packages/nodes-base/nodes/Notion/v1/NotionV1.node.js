...
import { extractDatabaseMentionRLC, extractPageId, extractResourceId, formatBlocks, formatTitle, getBlockTypesOptions, mapFilters, mapProperties, mapSorting, notionApiRequest, notionApiRequestAllItems, simplifyObjects, } from '../shared/GenericFunctions';
import { listSearch } from '../shared/methods';
export class NotionV1 {
    description;
    ...
    async execute() {
        ...
        if (resource === 'block') {
            if (operation === 'append') {
                for (let i = 0; i < length; i++) {
                    const blockId = extractPageId(this.getNodeParameter('blockId', i, '', { extractValue: true }));
        ...
        if (resource === 'databasePage') {
            if (operation === 'create') {
                for (let i = 0; i < length; i++) {
                    ...
                    if (properties.length !== 0) {
                        body.properties = mapProperties.call(this, properties, timezone);
                    }
                    const blockValues = this.getNodeParameter('blockUi.blockValues', i, []);
                    extractDatabaseMentionRLC(blockValues);
            ...
            if (operation === 'update') {
                for (let i = 0; i < length; i++) {
                    const pageId = extractPageId(this.getNodeParameter('pageId', i, '', { extractValue: true }));
                    const simple = this.getNodeParameter('simple', i);
                    const properties = this.getNodeParameter('propertiesUi.propertyValues', i, []);
                    const body = {
                        properties: {},
        ...
        if (resource === 'page') {
            if (operation === 'create') {
                for (let i = 0; i < length; i++) {
                    ...
                    body.parent.page_id = extractPageId(this.getNodeParameter('pageId', i, '', { extractValue: true }));
