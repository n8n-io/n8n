...
import { databaseUrlExtractionRegexp, databaseUrlValidationRegexp, idExtractionRegexp, idValidationRegexp, } from './shared/constants';
...
function createQueryDatabaseRequest(ctx) {
    const nodeVersion = ctx.getNode().typeVersion;
    if (nodeVersion >= 1.1) {
        const dataSourceId = ctx.getNodeParameter('dataSourceId', '', {
            extractValue: true,
        });
        return async (body) => (await notionApiRequestV3.call(ctx, 'POST', `/data_sources/${dataSourceId}/query`, body));
    }
    const databaseId = ctx.getNodeParameter('databaseId', '', { extractValue: true });
    return async (body) => (await notionApiRequest.call(ctx, 'POST', `/databases/${databaseId}/query`, body, {}, '', {
        headers: {
            'Notion-Version': '2022-02-22',
        },
    }));
}
export class NotionTrigger {
    description = {
    ...
    async poll() {
        ...
        const lastTimeChecked = webhookData.lastTimeChecked
            ? moment(webhookData.lastTimeChecked)
        ...
        if (Array.isArray(data) && data.length && Object.keys(data[0]).length !== 0) {
            do {
                ...
                !moment(records[records.length - 1][sortProperty]).isBefore(lastTimeChecked) &&
                hasMore);
            ...
            if (records[0]) {
                const latestTimestamp = moment(records[0][sortProperty]);
                // Save record ids with the same timestamp as the latest processed records
                webhookData.possibleDuplicates = records
                    .filter((record) => moment(record[sortProperty]).isSame(latestTimestamp))
