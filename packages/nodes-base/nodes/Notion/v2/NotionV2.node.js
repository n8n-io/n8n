...
import { downloadFiles, extractBlockId, extractDatabaseMentionRLC, extractResourceId, getPageId, formatBlocks, formatTitle, mapFilters, mapProperties, mapSorting, notionApiRequest, notionApiRequestAllItems, notionApiRequestGetBlockChildrens, prepareNotionError, simplifyBlocksOutput, simplifyObjects, validateJSON, } from '../shared/GenericFunctions';
    ...
    async execute() {
                ...
                for (let i = 0; i < itemsLength; i++) {
                    try {
                        ...
                        if (returnAll) {
                            ...
                            responseData = await notionApiRequestAllItems.call(this, 'results', 'GET', `/blocks/${blockId}/children`, {}, { page_size: Math.min(limit, 100), limit });
                            if (fetchNestedBlocks) {
                                responseData = await notionApiRequestGetBlockChildrens.call(this, responseData, [], limit);
                            }
                        }
        ...
        if (resource === 'database') {
            if (operation === 'get') {
                ...
                for (let i = 0; i < itemsLength; i++) {
                    try {
                        const databaseId = extractResourceId(this.getNodeParameter('databaseId', i, '', { extractValue: true }));
                ...
                for (let i = 0; i < itemsLength; i++) {
                    try {
                        ...
                        if (returnAll) {
                            ...
                            responseData = await notionApiRequestAllItems.call(this, 'results', 'POST', '/search', body, { limit });
                        }
                        if (simple) {
                ...
                for (let i = 0; i < itemsLength; i++) {
                    try {
                        ...
                        if (propertiesValues.length !== 0) {
                            body.properties = Object.assign(body.properties, mapProperties.call(this, propertiesValues, timezone, 2));
                        }
                        const blockValues = this.getNodeParameter('blockUi.blockValues', i, []);
                        extractDatabaseMentionRLC(blockValues);
                ...
                for (let i = 0; i < itemsLength; i++) {
                    try {
                        ...
                        if (returnAll) {
                            ...
                            const limit = this.getNodeParameter('limit', i);
                            body.page_size = Math.min(limit, 100);
                            responseData = await notionApiRequestAllItems.call(this, 'results', 'POST', `/databases/${databaseId}/query`, body, { limit });
                        }
                        if (download) {
                ...
                for (let i = 0; i < itemsLength; i++) {
                    try {
                        ...
                        if (properties.length !== 0) {
                            body.properties = mapProperties.call(this, properties, timezone, 2);
                        }
                        const options = this.getNodeParameter('options', i);
                        if (options.icon) {
                            if (options.iconType && options.iconType === 'file') {
