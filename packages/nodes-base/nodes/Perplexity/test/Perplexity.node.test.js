...
describe('Perplexity Node', () => {
    ...
    describe('PerplexityV2', () => {
        ...
        it('should keep the v2 Resource selector defaulted to chat', () => {
            const v2Resource = v2.description.properties.find((p) => p.name === 'resource' &&
                p.type === 'options' &&
        ...
        it('should keep the v1 Resource selector defaulted to chat', () => {
            const v1Resource = v2.description.properties.find((p) => p.name === 'resource' &&
                p.type === 'hidden' &&
        ...
        it('should include a Chat deprecation notice on v1 and v2', () => {
            const chatDeprecationNotice = v2.description.properties.find((p) => p.name === 'chatDeprecationNotice' && p.type === 'notice');
            ...
            expect(chatDeprecationNotice?.displayName).toMatch(/docs\.perplexity\.ai\/docs\/agent-api\/migrate-from-sonar/);
        });
    });
    describe('PerplexityV3', () => {
        ...
        it('should default the Resource selector to agent', () => {
            const resource = v3.description.properties.find((p) => p.name === 'resource' && p.type === 'options');
            expect(resource).toBeDefined();
            expect(resource?.default).toBe('agent');
        });
        it('should not include the Chat resource', () => {
            const resource = v3.description.properties.find((p) => p.name === 'resource' && p.type === 'options');
            const values = (resource?.options ?? []).map((o) => o.value);
        ...
        it('should not include a Chat deprecation notice', () => {
            const chatDeprecationNotice = v3.description.properties.find((p) => p.name === 'chatDeprecationNotice');
            expect(chatDeprecationNotice).toBeUndefined();
        });
        it('should not carry over any Chat properties', () => {
            const chatProperties = v3.description.properties.filter((p) => p.displayOptions?.show?.resource?.includes('chat'));
            expect(chatProperties).toEqual([]);
        });
    });
});
//# sourceMappingURL=Perplexity.node.test.js.map