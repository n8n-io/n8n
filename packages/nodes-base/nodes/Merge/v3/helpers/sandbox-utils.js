 ...
 */
export function isSandboxMemoryError(error) {
...
export async function createSandboxContext() {
    const memoryLimitMb = Container.get(NodesConfig).mergeSqlSandboxMemoryLimitMb;
 ...
 */
export async function withSandboxContext(fn) {
 ...
 */
export async function runAlaSqlInSandbox(tableData, query, parameters = []) {
    try {
