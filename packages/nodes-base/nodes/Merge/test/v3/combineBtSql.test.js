import { createSandboxContext, resetSandboxCache, runAlaSqlInSandbox, withSandboxContext, } from '../../v3/helpers/sandbox-utils';
describe('combineBySql sandbox (isolated-vm)', () => {
    ...
    describe('runAlaSqlInSandbox – isolation', () => {
        ...
        it('should bind values passed separately', async () => {
            ...
            ], 'SELECT name FROM input1 WHERE name = ?', ["Sam' OR name = 'Dan"]);
            expect(result).toEqual([]);
        });
        // ── Host API access ────────────────────────────────────────────────────
        it('should not expose require inside the sandbox', async () => {
            await withSandboxContext(async (context) => {
                await expect(context.eval('(function() { return typeof require; })()', { copy: true })).resolves.toBe('undefined');
            });
        });
        it('should not expose process inside the sandbox', async () => {
            await withSandboxContext(async (context) => {
                await expect(context.eval('(function() { return typeof process; })()', { copy: true })).resolves.toBe('undefined');
            });
        });
        it('should not expose __dirname inside the sandbox', async () => {
            await withSandboxContext(async (context) => {
                await expect(context.eval('(function() { return typeof __dirname; })()', { copy: true })).resolves.toBe('undefined');
            });
        });
        it('should not expose __filename inside the sandbox', async () => {
            await withSandboxContext(async (context) => {
                await expect(context.eval('(function() { return typeof __filename; })()', { copy: true })).resolves.toBe('undefined');
            });
        });
        it('should not expose global inside the sandbox', async () => {
            await withSandboxContext(async (context) => {
                await expect(context.eval('(function() { return typeof global; })()', { copy: true })).resolves.toBe('undefined');
            });
        });
        it('should not expose Buffer inside the sandbox', async () => {
            await withSandboxContext(async (context) => {
                await expect(context.eval('(function() { return typeof Buffer; })()', { copy: true })).resolves.toBe('undefined');
            });
        });
        // ── File system access ─────────────────────────────────────────────────
        it('should block FROM FILE() SQL clause (no fs in browser bundle)', async () => {
            await expect(runAlaSqlInSandbox([[{ id: 1 }]], "SELECT * FROM FILE('/etc/passwd', {headers: false})")).rejects.toThrow();
        });
        ...
        it.each(['NDJSON', 'JSON', 'CSV', 'TXT', 'XML', 'XLSX'])('should block FROM %s() deterministically with a clear error', async (source) => {
            await expect(runAlaSqlInSandbox([[{ id: 1 }]], `SELECT * FROM ${source}('data.file')`)).rejects.toThrow(/disabled/i);
        });
        it('should block ATTACH CSV FILE attempts', async () => {
            await expect(runAlaSqlInSandbox([[{ id: 1 }]], "ATTACH CSV '/etc/passwd' AS secret; SELECT * FROM secret")).rejects.toThrow();
        });
        it('should block SOURCE statement from reading files', async () => {
            await expect(runAlaSqlInSandbox([[{ id: 1 }]], "SOURCE '/etc/passwd'")).rejects.toThrow(/disabled/i);
        });
        it('should block REQUIRE statement from loading files', async () => {
            await expect(runAlaSqlInSandbox([[{ id: 1 }]], "REQUIRE '/tmp/malicious.js'")).rejects.toThrow(/disabled/i);
        });
        it('should block SELECT INTO FILE from writing files', async () => {
            await expect(runAlaSqlInSandbox([[{ id: 1 }]], "SELECT * INTO FILE('/tmp/exfil.txt') FROM input1")).rejects.toThrow();
        });
        it('should keep fetch and XMLHttpRequest stubs after alasql initialisation', async () => {
            await withSandboxContext(async (context) => {
                await expect(context.eval("fetch('file:///etc/passwd')", { copy: true })).rejects.toThrow(/disabled/i);
                await expect(context.eval('new XMLHttpRequest()', { copy: true })).rejects.toThrow(/disabled/i);
            });
        });
        ...
        it('should block CREATE FUNCTION from accessing host APIs', async () => {
            ...
            await expect(runAlaSqlInSandbox([[{ id: 1 }]], 'SELECT id FROM input1 WHERE alasql(\'CREATE FUNCTION danger() RETURNS STRING BEGIN RETURN typeof require END; SELECT danger() FROM [{"x":1}]\')')).rejects.toThrow();
        });
        it('should return "undefined" for require even when called via a SQL function', async () => {
            ...
            await expect(runAlaSqlInSandbox([[{ id: 1 }]], "SELECT alasql('CREATE FUNCTION getRequire() RETURNS STRING BEGIN RETURN typeof require END') FROM input1")).rejects.toThrow();
        });
        it('should block eval() inside the sandbox from escaping isolation', async () => {
            await withSandboxContext(async (context) => {
                ...
                const result = (await context.eval(`(function() {
        ...
        it('should block alasql arrow-operator prototype traversal from reaching host globals', async () => {
            ...
            const result = await runAlaSqlInSandbox([[{ id: 1 }]], 'SELECT {}.constructor->constructor->[call](\'\',\'return {result: process.getBuiltinModule("child_process").execSync("id").toString()}\') AS r FROM input1');
            expect(result).toEqual([{}]);
        });
        ...
        it('should keep sandbox-side global mutations contained to the sandbox', async () => {
            await withSandboxContext(async (context) => {
                ...
                expect({}.__mutated).toBeUndefined();
            });
        });
        it('should give each invocation a fresh global state independent of prior invocations', async () => {
            ...
            const mutatingFn = 'return function(){try{if(this&&this[0]&&this[0].value){Object.prototype.__marker=this[0].value;}}catch(e){} return this;}';
        ...
        it('should not allow a CREATE FUNCTION', async () => {
            await expect(runAlaSqlInSandbox([[{ x: 1 }]], 'CREATE FUNCTION double(n) RETURNS NUMBER BEGIN RETURN n * 2 END; SELECT double(x) AS v FROM input1')).rejects.toThrow();
            await expect(runAlaSqlInSandbox([[{ x: 1 }]], 'SELECT double(x) AS v FROM input1')).rejects.toThrow();
        });
        // ── Resource limits ────────────────────────────────────────────────────
        it('should enforce the CPU timeout on an infinite loop', async () => {
            await withSandboxContext(async (context) => {
                await expect(context.eval('(function() { while(true) {} })()', { timeout: 100, copy: true })).rejects.toThrow();
            });
        });
    });
    describe('runAlaSqlInSandbox – SQL execution', () => {
        ...
        it('should handle a large dataset (25k rows) without exhausting the sandbox', async () => {
            ...
            const result = await runAlaSqlInSandbox([rows], 'SELECT COUNT(*) AS n FROM input1 WHERE b > 10000');
            expect(result[0]).toEqual({ n: 19999 });
        });
    });
});
//# sourceMappingURL=combineBtSql.test.js.map