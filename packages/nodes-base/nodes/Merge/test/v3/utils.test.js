...
describe('rowToExecutionData', () => {
    ...
    test('should separate json properties and pairedItem properties', () => {
        const input = {
            key1: 'value1',
            key2: 42,
            pairedItem1: { item: 0, input: undefined },
            pairedItem2: { item: 0, input: 1 },
        };
...
describe('modifySelectQuery', () => {
    ...
    test('should append pairedItem columns when input tables exist', () => {
        ...
        expect(modifiedQuery).toBe('SELECT column1, column2, input1.pairedItem AS pairedItem1 FROM input1 WHERE input1.id = table.id');
    });
    test('should handle multiple input tables correctly', () => {
        ...
        const modifiedQuery = modifySelectQuery(query, 2);
        expect(modifiedQuery).toBe('SELECT column1, input1.pairedItem AS pairedItem1, input2.pairedItem AS pairedItem2 FROM input1 LEFT JOIN input2 ON input1.name = input2.name');
    });
