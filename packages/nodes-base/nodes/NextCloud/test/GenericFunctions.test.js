...
function buildFunctions(authentication = 'accessToken') {
    const requestWithAuthentication = vi.fn();
    const getCredentials = vi.fn(async () => ({ webDavUrl }));
    const getNodeParameter = vi.fn((parameterName) => {
...
function requestOptions(requestWithAuthentication, callIndex = 0) {
    return requestWithAuthentication.mock.calls[callIndex][1];
}
describe('NextCloud GenericFunctions', () => {
    ...
    describe('Request building', () => {
        it('uses access token credentials and builds a WebDAV request by default', async () => {
            ...
            expect(requestWithAuthentication).toHaveBeenCalledWith('nextCloudApi', expect.objectContaining({
                method: 'GET',
            ...
            expect(requestOptions(requestWithAuthentication).uri).toEqual(expect.stringContaining('/remote.php/webdav'));
        });
        it('handles non-standard WebDAV URLs gracefully', async () => {
            ...
            requestWithAuthentication.mockResolvedValue({ status: 'ok' });
            await nextCloudApiRequest.call(functions, 'GET', '/test.txt', '', undefined, undefined, undefined, true);
        ...
        it('uses OAuth2 credentials when OAuth2 authentication is selected', async () => {
            ...
            expect(requestWithAuthentication).toHaveBeenCalledWith('nextCloudOAuth2Api', expect.objectContaining({
                method: 'GET',
                uri: `${webDavUrl}//test.txt`,
            }));
        });
        it('removes the WebDAV path for OCS requests', async () => {
            ...
            await nextCloudApiRequest.call(functions, 'POST', 'ocs/v1.php/cloud/users', 'userid=alice', { 'OCS-APIRequest': true }, undefined, undefined, false);
            ...
            expect(requestOptions(requestWithAuthentication).uri).not.toEqual(expect.stringContaining('/remote.php/webdav'));
        });
        it('strips non-standard WebDAV path for OCS requests while preserving subpath', async () => {
            ...
            getCredentials.mockResolvedValue({ webDavUrl: customUrl });
            requestWithAuthentication.mockResolvedValue('<ocs />');
            await nextCloudApiRequest.call(functions, 'POST', 'ocs/v1.php/cloud/users', '', {}, undefined, undefined, false);
            expect(requestOptions(requestWithAuthentication).uri).toBe('https://custom.example.com/nextcloud/ocs/v1.php/cloud/users');
        });
        it('passes body, headers, query, and null encoding to requestWithAuthentication', async () => {
            ...
            await nextCloudApiRequest.call(functions, 'PROPFIND', '/test.txt', body, headers, null, query, true);
            expect(requestOptions(requestWithAuthentication)).toMatchObject({
                method: 'PROPFIND',
                uri: `${webDavUrl}//test.txt`,
                body,
        ...
        it('URL-encodes endpoint characters while preserving path separators', async () => {
            ...
            expect(requestOptions(requestWithAuthentication).uri).toBe(`${webDavUrl}//folder%20name/test%20file.txt`);
        });
        ...
        it('strips standard remote.php/webdav path for OCS requests', async () => {
            ...
            requestWithAuthentication.mockResolvedValue('<ocs />');
            await nextCloudApiRequest.call(functions, 'POST', 'ocs/v1.php/cloud/users', '', {}, undefined, undefined, false);
            expect(requestOptions(requestWithAuthentication).uri).toBe('https://nextcloud.example.com/ocs/v1.php/cloud/users');
        });
    ...
    describe('webDavUrl validation', () => {
        it('throws NodeOperationError when webDavUrl is empty', async () => {
            ...
            const promise = nextCloudApiRequest.call(functions, 'GET', '/test.txt', '');
            await expect(promise).rejects.toThrow(NodeOperationError);
            await expect(promise).rejects.toThrow('Invalid WebDAV URL in credentials: "". The URL must start with https:// or http://.');
        });
        it('throws NodeOperationError when webDavUrl is missing double slash', async () => {
            ...
            const promise = nextCloudApiRequest.call(functions, 'GET', '/test.txt', '');
            await expect(promise).rejects.toThrow(NodeOperationError);
            await expect(promise).rejects.toThrow('Invalid WebDAV URL in credentials: "http:/localhost:8080/remote.php/webdav". The URL must start with https:// or http://.');
        });
        it('throws NodeOperationError when webDavUrl is completely malformed', async () => {
            ...
            const promise = nextCloudApiRequest.call(functions, 'GET', '/test.txt', '');
            await expect(promise).rejects.toThrow(NodeOperationError);
            await expect(promise).rejects.toThrow('Invalid WebDAV URL in credentials: "not-a-url". The URL must start with https:// or http://.');
        });
