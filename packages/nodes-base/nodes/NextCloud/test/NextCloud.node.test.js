...
function buildExecuteFunctions({ parameters, inputData = [{ json: {} }], authentication = 'accessToken', continueOnFail = false, }) {
    ...
    const constructExecutionMetaData = vi.fn((data, metadata) => data.map((item) => ({ ...item, pairedItem: metadata?.itemData })));
    const parameterForItem = (itemIndex) => Array.isArray(parameters) ? (parameters[itemIndex] ?? parameters[0]) : parameters;
    const executeFunctions = {
        ...
        getNodeParameter: vi.fn((parameterName, itemIndex, defaultValue) => {
            if (parameterName === 'authentication')
                return authentication;
            const itemParameters = parameterForItem(itemIndex);
            if (parameterName in itemParameters)
                return itemParameters[parameterName];
            return defaultValue;
        }),
        helpers: {
            assertBinaryData,
...
describe('NextCloud Node', () => {
    ...
    describe.each(['accessToken', 'oAuth2'])('authentication: %s', (authentication) => {
        it('uses the matching credential type', async () => {
            ...
            expect(requestWithAuthentication).toHaveBeenCalledWith(authentication === 'accessToken' ? 'nextCloudApi' : 'nextCloudOAuth2Api', expect.any(Object));
        });
    });
    describe('file', () => {
        it('downloads a file as binary data', async () => {
            const downloadBuffer = Buffer.from('downloaded file');
            const { executeFunctions, prepareBinaryData, requestWithAuthentication } = buildExecuteFunctions({
                parameters: {
                    resource: 'file',
                    operation: 'download',
                    path: '/test.txt',
        ...
        it('uploads a binary file', async () => {
            const { executeFunctions, getBinaryDataBuffer, requestWithAuthentication } = buildExecuteFunctions({
                inputData: [
    ...
    describe.each([
        ...
        it.each([
            ...
            if (parameterName && shareWith)
                parameters[parameterName] = shareWith;
            const { executeFunctions, requestWithAuthentication } = buildExecuteFunctions({ parameters });
            requestWithAuthentication.mockResolvedValue(ocsSuccessResponse);
            const result = await executeNode(executeFunctions);
            const body = requestOptions(requestWithAuthentication).body;
            expect(requestOptions(requestWithAuthentication)).toMatchObject({
                method: 'POST',
            ...
            if (shareWith)
                expect(body).toContain(`shareWith=${encodeURIComponent(shareWith)}`);
            if (shareType === 3)
        ...
        it('returns an internal link from the WebDAV file id', async () => {
            const { constructExecutionMetaData, executeFunctions, requestWithAuthentication } = buildExecuteFunctions({
                parameters: {
                    resource,
                    operation: 'share',
                    path,
            ...
            expectWebDavUri(requestOptions(requestWithAuthentication).uri);
            expect(constructExecutionMetaData).toHaveBeenCalledWith([{ json: { link: `${baseUrl}/f/55555` } }], { itemData: { item: 0 } });
        ...
        it('returns an internal link from folder PROPFIND with multiple responses', async () => {
            ...
            expect(constructExecutionMetaData).toHaveBeenCalledWith([{ json: { link: `${baseUrl}/f/77777` } }], { itemData: { item: 0 } });
    ...
    describe('multi item execution', () => {
        ...
        it('does not leak OCS headers from a regular share into a subsequent Internal Link request', async () => {
            ...
            expectWebDavUri(requestOptions(requestWithAuthentication, 1).uri);
            expect(requestOptions(requestWithAuthentication, 1).headers).not.toHaveProperty('OCS-APIRequest');
...
//# sourceMappingURL=NextCloud.node.test.js.map