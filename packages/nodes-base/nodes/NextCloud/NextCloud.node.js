import { NodeApiError, NodeConnectionTypes, NodeOperationError, sanitizeXmlName, } from 'n8n-workflow';
...
<d:propfind xmlns:d="DAV:" xmlns:oc="http://owncloud.org/ns">
  <d:prop><oc:fileid/></d:prop>
                ...
                try {
                    responseData = await nextCloudApiRequest.call(this, requestMethod, endpoint, body, headers, encoding, qs, useWebDavEndpoint);
                }
                catch (error) {
                ...
                if (resource === 'file' && operation === 'download') {
                    ...
                    if (shareType === 200) {
                        ...
                        if (typeof responseData !== 'string') {
                            throw new NodeOperationError(this.getNode(), 'Could not retrieve internal link: unexpected response type from NextCloud', { itemIndex: i });
                        }
                        const propfindData = await new Promise((resolve, reject) => {
                            parseString(responseData, {
                                ...
                                if (!data || typeof data !== 'object') {
                                    return reject(new NodeOperationError(this.getNode(), 'Could not retrieve internal link: invalid XML response structure', { itemIndex: i }));
                                }
                                resolve(data);
                            });
                        });
                        ...
                        if (!multistatus) {
                            throw new NodeOperationError(this.getNode(), 'Could not retrieve internal link: malformed PROPFIND response', { itemIndex: i });
                        }
                        ...
                        if (typeof fileid !== 'string' || fileid.length === 0) {
                            throw new NodeOperationError(this.getNode(), 'Could not retrieve internal link: oc:fileid not found in PROPFIND response', { itemIndex: i });
                        }
                        const webDavBase = credentials.webDavUrl.replace(/\/remote\.php\/webdav\/?$/, '');
                        if (webDavBase === credentials.webDavUrl) {
                            throw new NodeOperationError(this.getNode(), 'WebDAV URL must end with /remote.php/webdav for generating an internal link. Please check your Nextcloud credentials.', { itemIndex: i });
                        }
                        const internalLink = `${webDavBase}/f/${fileid}`;
                        const executionData = this.helpers.constructExecutionMetaData(wrapData({ link: internalLink }), { itemData: { item: i } });
                        returnData.push(...executionData);
                    }
                    else {
                        if (typeof responseData !== 'string') {
                            throw new NodeOperationError(this.getNode(), 'Unexpected response type from NextCloud OCS share endpoint', { itemIndex: i });
                        }
                        const jsonResponseData = await new Promise((resolve, reject) => {
                            parseString(responseData, {
                                ...
                                if (!data?.ocs?.data || typeof data.ocs.data !== 'object') {
                                    return reject(new NodeApiError(this.getNode(), { error: 'Invalid OCS response structure' }));
                                }
                                resolve(data.ocs.data);
                            });
                        });
                        const executionData = this.helpers.constructExecutionMetaData(wrapData(jsonResponseData), { itemData: { item: i } });
                        returnData.push(...executionData);
                    }
                }
                else if (resource === 'user') {
