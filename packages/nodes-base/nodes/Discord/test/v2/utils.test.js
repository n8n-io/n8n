...
import { createSimplifyFunction, prepareOptions, prepareEmbeds, checkAccessToGuild, setupChannelGetter, parseDiscordError, } from '../../v2/helpers/utils';
const node = {
    id: '1',
    name: 'Discord node',
    typeVersion: 2,
...
describe('Test Discord > setupChannelGetter & checkAccessToChannel', () => {
    beforeEach(() => {
        vi.spyOn(transport, 'discordApiRequest').mockImplementation(async (method) => {
            if (method === 'GET') {
                return {
                    guild_id: '123456',
                };
    ...
    it('should setup channel getter and get channel id', async () => {
        const fakeExecuteFunction = (auth) => {
            return {
                getNodeParameter: (parameter) => {
...
describe('Test Discord > parseDiscordError', () => {
    const errorMessage = '400 - {"message":"Invalid Form Body","code":50035,"errors":{"embeds":{"0":{"description":{"_errors":[{"code":"BASE_TYPE_REQUIRED","message":"This field is required"}]}}}}}';
