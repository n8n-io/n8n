...
describe('Discord V2 Utils', () => {
    describe('prepareMultiPartForm', () => {
        ...
        it('should create multipart form with single file', async () => {
            ...
            const result = await prepareMultiPartForm.call(mockExecuteFunctions, files, jsonPayload, itemIndex);
        ...
        it('should create multipart form with multiple files', async () => {
            const files = [{ inputFieldName: 'file1' }, { inputFieldName: 'file2' }];
        ...
        it('should throw error when binary data is not found', async () => {
            ...
            mockExecuteFunctions.getNode = vi.fn().mockReturnValue({ name: 'Discord' });
            await expect(prepareMultiPartForm.call(mockExecuteFunctions, files, jsonPayload, itemIndex)).rejects.toThrow(NodeOperationError);
            await expect(prepareMultiPartForm.call(mockExecuteFunctions, files, jsonPayload, itemIndex)).rejects.toThrow('Input item [0] does not contain binary data on property file1');
        });
        ...
        it('should include attachments in payload_json with correct structure', async () => {
            const files = [{ inputFieldName: 'file1' }, { inputFieldName: 'file2' }];
            const jsonPayload = { content: 'Test with attachments' };
            const itemIndex = 0;
            const binaryData1 = {
                data: 'base64data1',
                mimeType: 'image/png',
            ...
            const payloadJsonField = result
                .getBuffer()
                .toString()
                .match(/Content-Disposition: form-data; name="payload_json"[\s\S]*?\r\n\r\n([\s\S]*?)\r\n--/);
            expect(payloadJsonField).not.toBeNull();
            const payloadJson = jsonParse(payloadJsonField[1]);
        ...
        it('should use filename as filename and mimeType as content-type in file parts', async () => {
            const files = [{ inputFieldName: 'document' }];
            const jsonPayload = { content: 'Test' };
            const itemIndex = 0;
            const binaryData = {
                data: 'base64data',
                mimeType: 'application/pdf',
        ...
        it('should handle empty files array', async () => {
            const files = [];
            const jsonPayload = { content: 'Test message no files' };
            const itemIndex = 0;
            mockExecuteFunctions.getNode = vi.fn().mockReturnValue({ name: 'Discord' });
            const result = await prepareMultiPartForm.call(mockExecuteFunctions, files, jsonPayload, itemIndex);
            expect(result).toBeDefined();
    ...
    describe('getAuditLogReasonHeaders', () => {
        ...
        const mockParams = (params) => {
            mockExecuteFunctions.getNodeParameter = vi
                .fn()
                .mockImplementation((name, _i, fallback) => name in params ? params[name] : fallback);
        };
        beforeEach(() => {
            mockExecuteFunctions = mockDeep();
        });
