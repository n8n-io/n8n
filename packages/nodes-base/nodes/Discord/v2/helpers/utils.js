...
export const createSimplifyFunction = (includedFields) => (item) => {
    const result = {};
...
export function parseDiscordError(error, itemIndex = 0) {
...
export function prepareErrorData(error, i) {
...
export function getAuditLogReasonHeaders(itemIndex) {
    const reason = this.getNodeParameter('reason', itemIndex, '');
    const text = reason === 'other'
        ? this.getNodeParameter('reasonCustom', itemIndex, '')
...
export function prepareOptions(options, guildId) {
    if (options.flags) {
...
export function prepareEmbeds(embeds) {
    return embeds
        .map((embed) => {
        let embedReturnData = {};
        if (embed.inputMethod === 'json') {
            ...
            try {
                embedReturnData = jsonParse(embed.json);
            }
            catch (error) {
...
export async function prepareMultiPartForm(files, jsonPayload, i) {
    const multiPartBody = new FormData();
    const attachments = [];
    const filesData = [];
    for (const [index, file] of files.entries()) {
        const binaryData = this.helpers.assertBinaryData(i, file.inputFieldName);
        if (!binaryData) {
            throw new NodeOperationError(this.getNode(), `Input item [${i}] does not contain binary data on property ${file.inputFieldName}`);
        }
    ...
    for (const [index, binaryData] of filesData.entries()) {
        multiPartBody.append(`files[${index}]`, binaryData.data, {
            contentType: binaryData.mime,
            filename: binaryData.name,
        });
    }
    return multiPartBody;
}
export function checkAccessToGuild(node, guildId, userGuilds, itemIndex = 0) {
    if (!userGuilds.some((guild) => guild.id === guildId)) {
        throw new NodeOperationError(node, `You do not have access to the guild with the id ${guildId}`, {
            itemIndex,
            level: 'warning',
        });
    }
}
export async function checkAccessToChannel(channelId, userGuilds, itemIndex = 0) {
    let guildId = '';
    try {
    ...
    if (!guildId) {
        throw new NodeOperationError(this.getNode(), `Could not find server for channel with the id ${channelId}`, {
            itemIndex,
        });
    }
    checkAccessToGuild(this.getNode(), guildId, userGuilds, itemIndex);
}
export async function setupChannelGetter(userGuilds) {
...
export async function sendDiscordMessage({ guildId, userGuilds, isOAuth2, body, files = [], itemIndex = 0, }) {
    ...
    if (sendTo === 'user') {
        ...
        if (isOAuth2) {
            try {
                ...
                if (error instanceof NodeApiError && error.httpCode === '404') {
                    throw new NodeOperationError(this.getNode(), `User with the id ${userId} is not a member of the selected guild`, {
                        itemIndex,
                    });
                }
        ...
        channelId = (await discordApiRequest.call(this, 'POST', '/users/@me/channels', {
            recipient_id: userId,
        })).id;
        if (!channelId) {
            throw new NodeOperationError(this.getNode(), 'Could not create a channel to send direct message to', { itemIndex });
        }
    }
    if (sendTo === 'channel') {
    ...
    if (files?.length) {
        ...
        response = await discordApiMultiPartRequest.call(this, 'POST', `/channels/${channelId}/messages`, multiPartBody);
    }
    else {
