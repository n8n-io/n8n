...
import { createSimplifyFunction, parseDiscordError, prepareErrorData, setupChannelGetter, } from '../../helpers/utils';
    ...
    for (let i = 0; i < items.length; i++) {
        try {
            ...
            if (!returnAll) {
                ...
                qs.limit = limit;
                response = await discordApiRequest.call(this, 'GET', `/channels/${channelId}/messages`, undefined, qs);
            }
            else {
                let responseData;
                qs.limit = 100;
                do {
                    responseData = await discordApiRequest.call(this, 'GET', `/channels/${channelId}/messages`, undefined, qs);
