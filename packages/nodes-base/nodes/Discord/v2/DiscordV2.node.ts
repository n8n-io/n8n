...
export class DiscordV2 implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < length; i++) {
			...
			if (options.embeds) {
				try {
					body.embeds = JSON.parse(options.embeds as string);
			...
			if (options.components) {
				try {
					body.components = JSON.parse(options.components as string);
				} catch (e) {
			...
			if (options.allowed_mentions) {
				body.allowed_mentions = jsonParse(options.allowed_mentions as string);
			}
			...
			if (options.payloadJson) {
				body.payload_json = jsonParse(options.payloadJson as string);
			}

			if (options.attachments) {
				body.attachments = jsonParse(options.attachments as string);
			}
