...
export class Nasa {
    ...
    async execute() {
            ...
            try {
                ...
                if (resource === 'astronomyPictureOfTheDay') {
                    if (operation === 'get') {
                        ...
                        qs.date =
                            moment(additionalFields.date).format('YYYY-MM-DD') ||
                ...
                if (resource === 'asteroidNeoFeed') {
                    if (operation === 'get') {
                        ...
                        qs.start_date =
                            moment(additionalFields.startDate).format('YYYY-MM-DD') || currentDate;
                        qs.end_date =
                            moment(additionalFields.endDate).format('YYYY-MM-DD') || currentDate;
                    }
                }
                ...
                if (resource.startsWith('donki')) {
                    if (additionalFields.startDate) {
                        qs.startDate = moment(additionalFields.startDate).format('YYYY-MM-DD');
                    }
                    else {
                        qs.startDate = moment().subtract(30, 'days').format('YYYY-MM-DD');
                    }
                    if (additionalFields.endDate) {
                        qs.endDate = moment(additionalFields.endDate).format('YYYY-MM-DD');
                    }
                    else {
                ...
                if (resource === 'earthImagery') {
                    if (operation === 'get') {
                        ...
                        if (additionalFields.date) {
                            qs.date = moment(additionalFields.date).format('YYYY-MM-DD');
                        }
                        else {
        ...
        else if (resource === 'astronomyPictureOfTheDay' &&
            operation === 'get' &&
            download &&
