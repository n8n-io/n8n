...
import { pptrLogin } from './login.pptr';
	...
	description: INodeTypeDescription = {
		...
		properties: [
				...
				options: [
					{
						...
						value: 'fetchLoginDetails',
						description: 'Returns login details after logging in',
						action: 'Fetch login details',
					},
				],
				default: 'fetchLoginDetails',
			},
			...
			},
			{
				...
				default: '',
				description:
					"The (time-based one-time password) TOTP token is a 6-digit code generated by a TOTP authentication app, or by n8n's TOTP node",
			},
			...
			},
			{
				...
				displayOptions: {
					hide: {
						resource: ['user', 'organization', 'login'],
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				let owner = '';
				if (fullOperation !== 'user:invite' && resource !== 'login') {
				...
				if (resource === 'login' && operation === 'fetchLoginDetails') {
				...
				if (this.continueOnFail()) {
					if (
						...
						// @TODO: Detect exact issue
						if (error.message.includes('Waiting')) {
							error.message = 'Access denied: 2FA code is incorrect';
						}
