...
export async function getUsers(filter, paginationToken) {
    ...
    try {
        responseData = await githubApiRequest.call(this, 'GET', '/search/users', {}, { q: filter, page, per_page });
    }
    catch {
...
export async function getRepositories(filter, paginationToken) {
    ...
    try {
        responseData = await githubApiRequest.call(this, 'GET', '/search/repositories', {}, { q, page, per_page });
    }
    catch {
...
export async function getWorkflows(paginationToken) {
    const owner = this.getCurrentNodeParameter('owner', { extractValue: true });
...
export async function getRefs(filter, paginationToken) {
    const owner = this.getCurrentNodeParameter('owner', { extractValue: true });
    const repository = this.getCurrentNodeParameter('repository', { extractValue: true });
    const page = paginationToken ? +paginationToken : 1;
    const per_page = 100;
    const responseData = await githubApiRequest.call(this, 'GET', `/repos/${owner}/${repository}/git/refs`, {}, { page, per_page });
    const refs = [];
