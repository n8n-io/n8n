...
/**
 * Declarative twin of `GithubTrigger`: the webhook lifecycle and event handling
 * come entirely from `description.trigger` — no `webhookMethods`, no
 * `webhook()`. Uses the same GitHub credentials as the original node, mints a
 * webhook secret on registration, and verifies each delivery's HMAC signature.
 *
 * Known differences from `GithubTrigger` (features of later phases of the
 * declarative-webhook-triggers spec): no 422 conflict-adoption and no friendly
 * per-status error messages, and the owner/repository resource locators have no
 * paste-a-URL mode.
 */
export class GithubDeclarativeTrigger implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'GitHub Declarative Trigger',
		name: 'githubDeclarativeTrigger',
		icon: { light: 'file:github.svg', dark: 'file:github.dark.svg' },
		group: ['trigger'],
		version: 1,
		subtitle:
			'={{$parameter["owner"] + "/" + $parameter["repository"] + ": " + $parameter["events"].join(", ")}}',
		description: 'Starts the workflow when GitHub events occur (declarative trigger)',
		defaults: {
			name: 'GitHub Declarative Trigger',
		},
		inputs: [],
		...
		trigger: {
			...
			lifecycle: {
				checkExists: {
					routing: {
						request: {
							method: 'GET',
							url: '={{ "/repos/" + $parameter.owner + "/" + $parameter.repository + "/hooks/" + $staticData.webhookId }}',
						},
					},
				...
				create: {
					routing: {
						request: {
							method: 'POST',
							url: '={{ "/repos/" + $parameter.owner + "/" + $parameter.repository + "/hooks" }}',
							body: {
								name: 'web',
								config: {
									url: '={{ $webhookUrl }}',
									...
									insecure_ssl: '={{ $parameter.options?.insecureSSL ? "1" : "0" }}',
								},
					...
					validateResponse: '={{ $response.id !== undefined && $response.active === true }}',
					store: {
			...
			handler: {
				...
				// GitHub's post-registration liveness check: answer, never run.
				ping: { when: '={{ $request.headers["x-github-event"] === "ping" }}', response: 'OK' },
				filter: {
					allowed: '={{ $parameter.events }}',
					actual: '={{ $request.headers["x-github-event"] }}',
					wildcard: '*',
				},
				// Same item shape as GithubTrigger
