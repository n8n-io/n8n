...
export function verifySignature() {
    ...
    if (!webhookSecret) {
        this.logger.warn(`Github Trigger "${this.getNode().name}" rejected a delivery because no webhook secret is stored. Activate the workflow again to re-register the webhook.`, { workflowId: this.getWorkflow().id });
        return false;
    }
