...
describe('Test Github Node', () => {
        ...
        describe('removeTrailingSlash Function', () => {
            ...
            beforeEach(() => {
                ...
                vi.mock('../../../../utils/utilities', async () => ({
                    ...(await vi.importActual('../../../../utils/utilities')),
                    getFileSha: vi.fn().mockResolvedValue('mockedSHA'),
                }));
            });
            it('should call remove trailing slash', async () => {
                mockExecutionContext.getNodeParameter.mockImplementation((parameterName) => {
                ...
                expect(mockExecutionContext.helpers.requestWithAuthentication).toHaveBeenCalledWith('githubOAuth2Api', {
                    body: {},
                    json: true,
                    method: 'GET',
                    qs: {},
    ...
    describe('Error Handling', () => {
        ...
        it('should throw NodeApiError for general API errors in dispatchAndWait operation', async () => {
            ...
            expect(mockExecutionContext.helpers.requestWithAuthentication).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
                method: 'POST',
                uri: expect.stringContaining(`/repos/${owner}/${repository}/actions/workflows/${workflowId}/dispatches`),
    ...
    describe('Workflow Operations', () => {
        ...
        it('should use extractValue for workflowId in disable operation', async () => {
            ...
            const repository = 'testRepository';
            const workflowId = 147025216;
            mockExecutionContext.getNodeParameter.mockImplementation((parameterName, _itemIndex, defaultValue, options) => {
                if (parameterName === 'owner') {
                    return owner;
                }
                if (parameterName === 'repository') {
    ...
    describe('Parameter Extraction', () => {
        it('should use extractValue for workflowId parameter', () => {
            ...
            const workflowOperations = description.properties.find((prop) => prop.name === 'operation' && prop.displayOptions?.show?.resource?.includes('workflow'));
            expect(workflowOperations).toBeDefined();
            expect(workflowOperations?.options).toEqual(expect.arrayContaining([
    ...
    describe('User Operations', () => {
        ...
        it('should fetch open issues by default (user:getIssues)', async () => {
            mockExecutionContext.getNodeParameter.mockImplementation((parameterName) => {
                ...
                if (parameterName === 'getUserIssuesFilters')
                    return {};
                if (parameterName === 'returnAll')
                    return true;
                if (parameterName === 'authentication')
                    return 'accessToken';
                return '';
    ...
    describe('Dispatch and Wait - Limit Wait Time', () => {
        ...
        const setLimitWaitTime = (values) => {
            mockExecutionContext.getNodeParameter.mockImplementation((parameterName, _itemIndex, defaultValue) => {
                if (parameterName === 'options.limitWaitTime.values')
        ...
        it('should wait for the configured time interval', async () => {
            ...
            await githubNode.execute.call(mockExecutionContext);
            expect(mockExecutionContext.putExecutionToWait).toHaveBeenCalledWith(new Date(now + 30 * 60 * 1000));
        });
        it('should wait until the configured date and time', async () => {
            ...
            await githubNode.execute.call(mockExecutionContext);
            expect(mockExecutionContext.putExecutionToWait).toHaveBeenCalledWith(new Date(maxDateAndTime));
        });
        it('should throw for an invalid date and time without dispatching the workflow', async () => {
            ...
            await expect(githubNode.execute.call(mockExecutionContext)).rejects.toThrow(NodeOperationError);
...
//# sourceMappingURL=Github.node.test.js.map