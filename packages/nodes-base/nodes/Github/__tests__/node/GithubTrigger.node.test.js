...
const createMockHookFunctions = (webhookData, { insecureSSL = false } = {}) => ({
...
const mockExistingWebhook = (existingWebhook = {
    id: 123,
    events: ['push'],
...
describe('GithubTrigger Node', () => {
    ...
    describe('create webhook method', () => {
        ...
        it('should send the secret to GitHub API when creating webhook', async () => {
            ...
            expect(apiRequestSpy).toHaveBeenCalledWith('POST', '/repos/some-owner/some-repo/hooks', expect.objectContaining({
        ...
        it('should store a secret and update the existing webhook when handling 422', async () => {
            ...
            expect(apiRequestSpy).toHaveBeenCalledWith('GET', '/repos/some-owner/some-repo/hooks/123', {});
            expect(apiRequestSpy).toHaveBeenLastCalledWith('PATCH', '/repos/some-owner/some-repo/hooks/123', expect.objectContaining({
        ...
        it('should send the full config, not just the secret, when updating the existing webhook', async () => {
            ...
            const trigger = new GithubTrigger();
            await trigger.webhookMethods.default.create.call(mockThis);
            expect(apiRequestSpy).toHaveBeenLastCalledWith('PATCH', '/repos/some-owner/some-repo/hooks/123', {
                config: {
        ...
        it('should not store a secret when the existing webhook cannot be updated', async () => {
            withAdoptableWebhook();
            const apiError = new NodeApiError({ name: 'Github Trigger' }, {
                message: 'Forbidden',
                httpCode: '403',
            });
            mockExistingWebhook().mockRejectedValueOnce(apiError);
            const trigger = new GithubTrigger();
            await expect(trigger.webhookMethods.default.create.call(mockThis)).rejects.toThrow(/could not be updated with a signing secret/);
            expect(webhookData.webhookSecret).toBeUndefined();
            expect(webhookData.webhookId).toBe(123);
        });
        ...
        it('should correct the events on the existing webhook', async () => {
            ...
            await trigger.webhookMethods.default.create.call(mockThis);
            expect(webhookData.webhookId).toBe('123');
            expect(apiRequestSpy).toHaveBeenLastCalledWith('PATCH', '/repos/some-owner/some-repo/hooks/123', expect.objectContaining({ events: ['push'] }));
        });
        ...
        it('should quote Githubs reason rather than the generic status text', async () => {
            const apiError = new NodeApiError({ name: 'Github Trigger' }, {
                message: '422 - {"message":"Validation Failed","errors":[{"resource":"Hook","code":"custom"}]}',
                statusCode: 422,
        ...
        it('should refuse to adopt a webhook this workflow did not register', async () => {
            ...
            const trigger = new GithubTrigger();
            await expect(trigger.webhookMethods.default.create.call(mockThis)).rejects.toThrow(/refused to create the webhook/);
            expect(webhookData.webhookSecret).toBeUndefined();
            expect(webhookData.webhookId).toBeUndefined();
        ...
        it('should rethrow a non-NodeApiError from the update untouched', async () => {
            ...
            const trigger = new GithubTrigger();
            await expect(trigger.webhookMethods.default.create.call(mockThis)).rejects.toThrow('socket hang up');
            expect(webhookData.webhookSecret).toBeUndefined();
        });
        it('should not store a secret when Github reports the update was not applied', async () => {
            ...
            const trigger = new GithubTrigger();
            await expect(trigger.webhookMethods.default.create.call(mockThis)).rejects.toThrow(/did not apply the update/);
            expect(webhookData.webhookSecret).toBeUndefined();
        });
        it('should warn when a successful create strands the previously stored webhook', async () => {
            ...
            await trigger.webhookMethods.default.create.call(mockThis);
            expect(webhookData.webhookId).toBe(789);
            expect(mockThis.logger.warn).toHaveBeenCalledWith(expect.stringMatching(/Webhook 123 may still be on the repository/), expect.objectContaining({ strandedWebhookId: 123 }));
        });
        ...
        it('should throw NodeOperationError if repo is not found (404)', async () => {
            vi.spyOn(GenericFunctions, 'githubApiRequest').mockRejectedValue({ httpCode: '404' });
            const trigger = new GithubTrigger();
            await expect(trigger.webhookMethods.default.create.call(mockThis)).rejects.toThrow(NodeOperationError);
            await expect(trigger.webhookMethods.default.create.call(mockThis)).rejects.toThrow(/Check that the repository exists/);
        });
    });
    describe('adopting a webhook registered before secrets were stored', () => {
        it('should re-register it against the same remote hook and store a secret', async () => {
            ...
            const mockThis = createMockHookFunctions(webhookData);
            const trigger = new GithubTrigger();
            ...
            expect(webhookData.webhookEvents).toEqual(['push']);
            expect(webhookData.webhookSecret).toHaveLength(64);
            expect(apiRequestSpy).toHaveBeenLastCalledWith('PATCH', '/repos/some-owner/some-repo/hooks/424242', expect.objectContaining({
...
//# sourceMappingURL=GithubTrigger.node.test.js.map