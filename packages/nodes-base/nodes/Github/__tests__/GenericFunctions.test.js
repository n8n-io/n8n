...
import { githubApiRequest, getFileSha, githubApiRequestAllItems, isBase64, validateJSON, } from '../GenericFunctions';
...
describe('GenericFunctions', () => {
    ...
    describe('githubApiRequest', () => {
        it('should make a successful API request', async () => {
            ...
            mockExecuteHookFunctions.helpers.requestWithAuthentication.mockResolvedValue(responseData);
            const result = await githubApiRequest.call(mockExecuteHookFunctions, method, endpoint, body);
            expect(result).toEqual(responseData);
            expect(mockExecuteHookFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('githubApi', {
                method: 'GET',
                body: {},
                qs: undefined,
        ...
        it('should throw a NodeApiError on API failure', async () => {
            ...
            mockExecuteHookFunctions.helpers.requestWithAuthentication.mockRejectedValue(error);
            await expect(githubApiRequest.call(mockExecuteHookFunctions, method, endpoint, body)).rejects.toThrow(NodeApiError);
        });
    });
    describe('getFileSha', () => {
        it('should return the SHA of a file', async () => {
            ...
            mockExecuteHookFunctions.helpers.requestWithAuthentication.mockResolvedValue(responseData);
            const result = await getFileSha.call(mockExecuteHookFunctions, owner, repository, filePath, branch);
        ...
        it('should throw a NodeOperationError if SHA is missing', async () => {
            ...
            const responseData = {};
            mockExecuteHookFunctions.helpers.requestWithAuthentication.mockResolvedValue(responseData);
            await expect(getFileSha.call(mockExecuteHookFunctions, owner, repository, filePath)).rejects.toThrow(NodeOperationError);
        });
    });
    describe('githubApiRequestAllItems', () => {
        it('should fetch all items with pagination', async () => {
            ...
            mockExecuteHookFunctions.helpers.requestWithAuthentication
                .mockResolvedValueOnce({ headers: { link: 'next' }, body: responseData1 })
                .mockResolvedValueOnce({ headers: {}, body: responseData2 });
            const result = await githubApiRequestAllItems.call(mockExecuteHookFunctions, method, endpoint, body, query);
