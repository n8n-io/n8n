...
describe('GitHub Node URL Pattern Tests', () => {
    ...
    const getOwnerValidationRegex = () => {
        const mode = getOwnerUrlMode();
        const validation = mode?.validation?.[0];
    ...
    const getTriggerOwnerUrlMode = () => {
        const ownerParam = githubTriggerNode.description.properties.find((prop) => prop.name === 'owner');
        return ownerParam?.modes?.find((mode) => mode.name === 'url');
    };
    const getTriggerRepositoryUrlMode = () => {
        const repoParam = githubTriggerNode.description.properties.find((prop) => prop.name === 'repository');
        return repoParam?.modes?.find((mode) => mode.name === 'url');
    };
    ...
    describe('URL Pattern Edge Cases', () => {
        it('should handle URLs with subdomains', () => {
            ...
            expect('https://github.acme.corp/engineering/backend-api'.match(repoRegex)?.[1]).toBe('backend-api');
        });
...
//# sourceMappingURL=UrlPatterns.test.js.map