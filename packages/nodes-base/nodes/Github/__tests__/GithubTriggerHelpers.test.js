...
describe('GithubTriggerHelpers', () => {
    ...
    const testBody = '{"action":"opened","pull_request":{"id":123},"repository":{"full_name":"owner/repo"}}';
    ...
    describe('verifySignature', () => {
        it('should return false when no webhook secret is stored', () => {
            ...
            expect(mockWebhookFunctions.logger.warn).toHaveBeenCalledWith(expect.stringContaining('no webhook secret is stored'), expect.objectContaining({ workflowId: 'wf-1' }));
        });
        ...
        it('should return false when computed and provided signatures have different lengths', () => {
            ...
            createHmac.mockReturnValue(mockHmacInstance);
            const result = verifySignature.call(mockWebhookFunctions);
            expect(result).toBe(false);
...
//# sourceMappingURL=GithubTriggerHelpers.test.js.map