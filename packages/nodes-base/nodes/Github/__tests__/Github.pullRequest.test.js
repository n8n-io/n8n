 ...
 */
function makeGithubError(statusCode, message) {
    return {
        statusCode,
        error: {
            message,
...
describe('Github Node - Pull Request Operations', () => {
    ...
    describe('pullRequest:create', () => {
        it('should create a PR with all fields (title, body, head, base, draft=true)', async () => {
            ...
            expect(mock.helpers.requestWithAuthentication).toHaveBeenCalledWith('githubApi', // credential name or function - ignore for test
            {
                method: 'POST',
        ...
        it('should create a PR with draft=false', async () => {
            ...
            expect(mock.helpers.requestWithAuthentication).toHaveBeenCalledWith('githubApi', expect.objectContaining({
        ...
        it('should default body to empty string when not provided', async () => {
            ...
            await github.execute.call(mock);
            const callArgs = mock.helpers.requestWithAuthentication.mock.calls[0][1];
        ...
        it('should throw NodeApiError on 422 validation failure', async () => {
            ...
            mock.helpers.requestWithAuthentication.mockRejectedValue(makeGithubError(422, 'No commits between main and feature'));
        ...
        it('should propagate the original GitHub error body inside the NodeApiError', async () => {
            ...
            await expect(github.execute.call(mock)).rejects.toBeInstanceOf(NodeApiError);
            await expect(github.execute.call(mock)).rejects.toThrow('Your request is invalid or could not be processed by the service');
        });
    ...
    describe('pullRequest:update', () => {
        ...
        it('should NOT send a `draft` field even if it leaks into editFields', async () => {
            const mock = createMockExecuteFunction({
                operation: 'update',
                pullRequestNumber: 8,
                editFields: { title: 'New title', draft: true },
            });
        ...
        it('should throw NodeApiError on 422 when setting an invalid state', async () => {
            const mock = createMockExecuteFunction({
                ...
                editFields: { state: 'merged' },
            });
            mock.helpers.requestWithAuthentication.mockRejectedValue(makeGithubError(422, 'Validation Failed'));
            await expect(github.execute.call(mock)).rejects.toBeInstanceOf(NodeApiError);
        });
    });
    // -----------------------------------------------------------
    ...
    describe('pullRequest:close', () => {
        ...
        it('should throw NodeApiError when PR is already closed (405)', async () => {
            ...
            mock.helpers.requestWithAuthentication.mockRejectedValue(makeGithubError(405, 'Method Not Allowed'));
            await expect(github.execute.call(mock)).rejects.toBeInstanceOf(NodeApiError);
        });
        it('should throw NodeApiError when PR is already merged (405)', async () => {
            ...
            mock.helpers.requestWithAuthentication.mockRejectedValue(makeGithubError(405, 'Pull Request is already merged'));
            await expect(github.execute.call(mock)).rejects.toBeInstanceOf(NodeApiError);
        });
        it('should throw NodeApiError on 404 PR not found', async () => {
            const mock = createMockExecuteFunction({
    ...
    describe('pullRequest:reopen', () => {
        ...
        it('should throw NodeApiError when PR is merged (405 – cannot reopen merged)', async () => {
            ...
            mock.helpers.requestWithAuthentication.mockRejectedValue(makeGithubError(405, 'Cannot reopen a merged pull request'));
            await expect(github.execute.call(mock)).rejects.toBeInstanceOf(NodeApiError);
        });
        it('should throw NodeApiError on 404 PR not found', async () => {
            const mock = createMockExecuteFunction({
    ...
    describe('pullRequest:createComment', () => {
        ...
        it('should use exactly the /issues/{pullRequestNumber}/comments path', async () => {
            ...
            await github.execute.call(mock);
            const callArgs = mock.helpers.requestWithAuthentication.mock.calls[0][1];
            expect(callArgs.uri).toBe('https://api.github.com/repos/test-owner/test-repo/issues/7/comments');
        });
        it('should throw NodeApiError on 404 PR not found', async () => {
            const mock = createMockExecuteFunction({
                operation: 'createComment',
    ...
    describe('pullRequest:editComment', () => {
        ...
        it('should use exactly the /issues/comments/{commentId} path', async () => {
            ...
            await github.execute.call(mock);
            const callArgs = mock.helpers.requestWithAuthentication.mock.calls[0][1];
            expect(callArgs.uri).toBe('https://api.github.com/repos/test-owner/test-repo/issues/comments/123');
        });
    ...
    describe('pullRequest:getPatch', () => {
        it('should GET the raw patch and return it as { patch }', async () => {
            ...
            const rawPatch = 'From abc123 Mon Sep 17 00:00:00 2001\n--- a/file\n+++ b/file\n@@ -1 +1,2 @@\n+new line';
    ...
    describe('pullRequest:merge', () => {
        ...
        it('should throw NodeApiError on 405 merge conflict', async () => {
            ...
            mock.helpers.requestWithAuthentication.mockRejectedValue(makeGithubError(405, 'Merge conflict'));
            await expect(github.execute.call(mock)).rejects.toBeInstanceOf(NodeApiError);
        });
        it('should throw NodeApiError when PR is already merged (405)', async () => {
            const mock = createMockExecuteFunction({
    ...
    describe('cross-cutting concerns', () => {
        it.each([
            ...
            mock.helpers.requestWithAuthentication.mockResolvedValue(op === 'getDiff' ? 'diff' : op === 'getPatch' ? 'patch' : { ok: true });
            const result = await github.execute.call(mock);
        ...
        it('should re-throw API errors as NodeApiError (not NodeOperationError)', async () => {
            ...
            mock.helpers.requestWithAuthentication.mockRejectedValue(makeGithubError(500, 'Internal Server Error'));
            let caught;
            try {
                await github.execute.call(mock);
            }
...
//# sourceMappingURL=Github.pullRequest.test.js.map