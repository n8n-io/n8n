...
describe('Github Node - File Create/Edit Operations', () => {
    ...
    describe('File Create - Binary Data', () => {
        it('should handle binary data by converting buffer to base64', async () => {
            ...
            mockExecuteFunctions.helpers.getBinaryDataBuffer.mockResolvedValue(expectedBuffer);
            GenericFunctions.githubApiRequest.mockResolvedValue({
                content: {
                    name: 'file.txt',
                    path: 'test/file.txt',
                    sha: 'abc123',
            ...
            expect(GenericFunctions.githubApiRequest).toHaveBeenCalledWith('PUT', '/repos/test-owner/test-repo/contents/test%2Ffile.txt', expect.objectContaining({
    ...
    describe('File Edit - Binary Data', () => {
        it('should get file SHA and convert buffer to base64 for edit operation', async () => {
            ...
            mockExecuteFunctions.helpers.assertBinaryData.mockReturnValue(mockBinaryData);
            mockExecuteFunctions.helpers.getBinaryDataBuffer.mockResolvedValue(expectedBuffer);
            GenericFunctions.getFileSha.mockResolvedValue('existing-sha-123');
            GenericFunctions.githubApiRequest.mockResolvedValue({
                content: {
                    name: 'file.txt',
                    path: 'test/file.txt',
            ...
            const result = await github.execute.call(mockExecuteFunctions);
            expect(GenericFunctions.getFileSha).toHaveBeenCalledWith('test-owner', 'test-repo', 'test/file.txt', undefined);
            expect(mockExecuteFunctions.helpers.getBinaryDataBuffer).toHaveBeenCalledWith(0, 'data');
            expect(GenericFunctions.githubApiRequest).toHaveBeenCalledWith('PUT', '/repos/test-owner/test-repo/contents/test%2Ffile.txt', expect.objectContaining({
                content: expectedBuffer.toString('base64'),
...
//# sourceMappingURL=Github.file.create.test.js.map