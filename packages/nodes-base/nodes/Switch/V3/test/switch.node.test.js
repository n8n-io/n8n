...
describe('SwitchV3 Node', () => {
    ...
    beforeEach(() => {
        ...
        mockLoadOptionsFunctions = mockDeep();
        vi.clearAllMocks();
    });
    describe('Version-specific behavior', () => {
        it('should have two numberOutputs parameters with different version conditions', () => {
            ...
            const numberOutputsParams = switchNode.description.properties.filter((prop) => prop.name === 'numberOutputs');
            expect(numberOutputsParams).toHaveLength(2);
        });
        it('should have noDataExpression: true for version 3.3+ numberOutputs parameter', () => {
            const switchNode = new SwitchV3(baseDescription);
            const numberOutputsParamWithNoExpression = switchNode.description.properties.find((prop) => prop.name === 'numberOutputs' && prop.noDataExpression === true);
        ...
        it('should have numberOutputs parameter without noDataExpression for older versions', () => {
            const switchNode = new SwitchV3(baseDescription);
            const numberOutputsParamWithoutNoExpression = switchNode.description.properties.find((prop) => prop.name === 'numberOutputs' && !prop.noDataExpression);
        ...
        it('should document SDK-discoverable fallback output metadata', () => {
            ...
            const optionsProperty = switchNode.description.properties.find((prop) => prop.name === 'options');
            const optionParameters = (optionsProperty?.options ?? []);
            const fallbackOutput = optionParameters.find((option) => option.name === 'fallbackOutput');
            const renameFallbackOutput = optionParameters.find((option) => option.name === 'renameFallbackOutput');
    ...
    describe('Expression Mode Execution', () => {
        ...
        it('should throw error for invalid output index', async () => {
            ...
            await expect(switchNode.execute.call(mockExecuteFunctions)).rejects.toThrow(NodeOperationError);
        });
    ...
    describe('Rules Mode Execution', () => {
        ...
        it('should route items based on matching rules', async () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName, itemIndex, defaultValue, options) => {
                if (paramName === 'mode')
                    return 'rules';
                if (paramName === 'rules.values') {
                    return [
                ...
                if (paramName.includes('conditions') && options?.extractValue) {
                    if (itemIndex === 0)
                        return true; // active matches first rule
                    if (itemIndex === 1)
                        return false; // inactive doesn't match first rule
                    return false;
                }
                return defaultValue;
            });
        ...
        it('should handle fallback output when no rules match', async () => {
            ...
            mockExecuteFunctions.getInputData.mockReturnValue(inputData);
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName, _itemIndex, defaultValue, options) => {
                if (paramName === 'mode')
                    return 'rules';
                if (paramName === 'rules.values') {
        ...
        it('should handle allMatchingOutputs option', async () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((paramName, _itemIndex, defaultValue, options) => {
                ...
                if (paramName === 'options')
                    return { allMatchingOutputs: true };
                if (paramName.includes('conditions') && options?.extractValue) {
    ...
    describe('Load Options', () => {
        it('should load fallback output options with no rules', async () => {
            ...
            const result = await switchNode.methods.loadOptions.getFallbackOutputOptions.call(mockLoadOptionsFunctions);
            expect(result).toEqual([
                {
...
//# sourceMappingURL=switch.node.test.js.map