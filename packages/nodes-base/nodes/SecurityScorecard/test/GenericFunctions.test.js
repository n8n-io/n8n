import { resolveReportDownloadUrl, SECURITY_SCORECARD_API_BASE_URL, scorecardApiRequest, } from '../GenericFunctions';
...
describe('SecurityScorecard GenericFunctions', () => {
    describe('resolveReportDownloadUrl', () => {
        it('should build API URL from a report file path', () => {
            expect(resolveReportDownloadUrl('example-report.pdf')).toBe(`${SECURITY_SCORECARD_API_BASE_URL}/reports/files/example-report.pdf`);
        });
        it('should preserve paths that already include reports/files', () => {
            expect(resolveReportDownloadUrl('reports/files/nested/report.pdf')).toBe(`${SECURITY_SCORECARD_API_BASE_URL}/reports/files/nested/report.pdf`);
        });
    ...
    describe('scorecardApiRequest', () => {
        ...
        it('should allow requests to the SecurityScorecard API host', async () => {
            await scorecardApiRequest.call(executeFunctions, 'GET', '', {}, {}, 'https://api.securityscorecard.io/reports/files/example-report.pdf');
    ...
    describe('download operation', () => {
        it('should reject external report URLs before making a request', async () => {
            ...
            await expect(node.execute.call(executeFunctions)).rejects.toThrow('Report URL must point to SecurityScorecard report files');
