...
import { ALL_CONDITIONS, ANY_CONDITION, ROWS_LIMIT_DEFAULT } from './constants';
...
export function getSelectFields(displayOptions, requireCondition = false, skipOperator = false) {
    return [
        {
...
export async function getSelectFilter(ctx, index) {
    ...
    let allColumnsWithTypes = DATA_TABLE_SYSTEM_COLUMN_TYPE_MAP;
    if (fields.length > 0) {
        ...
        if (invalidConditions.length > 0) {
            ...
            throw new NodeOperationError(node, `Filter validation failed: Column(s) "${invalidColumnNames}" do not exist in the selected table. ` +
...
export async function executeSelectMany(ctx, index, dataTableProxy, rejectEmpty = false, limit, sortBy) {
    ...
    while (true) {
        ...
        if (expectedTotal !== undefined && count !== expectedTotal) {
            throw new NodeOperationError(ctx.getNode(), 'synchronization error: result count changed during pagination');
        }
        expectedTotal = count;
