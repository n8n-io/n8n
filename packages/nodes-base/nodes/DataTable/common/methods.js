import { DATA_TABLE_SYSTEM_COLUMN_TYPE_MAP, } from 'n8n-workflow';
...
export async function tableSearch(filterString, prevPaginationToken) {
...
export async function getDataTableColumns() {
    const returnData = Object.entries(DATA_TABLE_SYSTEM_COLUMN_TYPE_MAP).map(([name, type]) => ({
        name: `${name} (${type})`,
        value: name,
        type,
    }));
...
export async function getConditionsForColumn() {
    const proxy = await getDataTableProxyLoadOptions(this);
    if (!proxy) {
        return [];
    }
    ...
    const type = DATA_TABLE_SYSTEM_COLUMN_TYPE_MAP[keyName] ??
