...
describe('DataTable Get Operation - Sort Feature', () => {
    ...
    let mockDataTableProxy;
    const node = { id: 'test', typeVersion: 1.1 };
    beforeEach(() => {
    ...
    describe('Single Column Sort', () => {
        it('should sort by column ascending', async () => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((param) => {
                if (param === DATA_TABLE_ID_FIELD)
                    return { mode: 'id', value: 'table123' };
                if (param === 'orderBy')
                    return true;
            ...
            mockDataTableProxy.getManyRowsAndCount.mockResolvedValue({
                data: [
                    { id: 1, name: 'Alice' },
                    { id: 2, name: 'Bob' },
                ],
            ...
            // ASSERT
            expect(mockDataTableProxy.getManyRowsAndCount).toHaveBeenCalledWith(expect.objectContaining({
                sortBy: ['name', 'ASC'],
            }));
        });
        ...
        it.each(['createdAt', 'updatedAt'])('should allow sorting by system column %s even though getColumns does not include it', async (column) => {
            mockExecuteFunctions.getNodeParameter.mockImplementation((param) => {
                if (param === DATA_TABLE_ID_FIELD)
                    return { mode: 'id', value: 'table123' };
                if (param === 'orderBy')
    ...
    describe('No Sort Rule', () => {
        ...
        it('should work with v1.0 (legacy version)', async () => {
            // ARRANGE
            const v10Node = { id: 'test', typeVersion: 1.0 };
            mockExecuteFunctions.getNode.mockReturnValue(v10Node);
            mockExecuteFunctions.getNodeParameter.mockImplementation((param) => {
                if (param === DATA_TABLE_ID_FIELD)
                    return { mode: 'id', value: 'table123' };
                if (param === 'orderBy')
    ...
    describe('Column Validation', () => {
        it('should throw when orderBy column does not exist in the table', async () => {
            ...
            await expect(getOperation.execute.call(mockExecuteFunctions, 0)).rejects.toThrow(NodeOperationError);
