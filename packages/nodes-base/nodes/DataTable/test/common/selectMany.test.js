...
describe('selectMany utils', () => {
    ...
    const dataTableProxy = vi.mocked({
        getManyRowsAndCount,
    });
    ...
    describe('executeSelectMany', () => {
        ...
        it('should panic if pagination gets out of sync', async () => {
            ...
            await expect(execution).rejects.toThrow('synchronization error: result count changed during pagination');
        });
        ...
        describe('sorting', () => {
            it('should pass sortBy parameter to getManyRowsAndCount with ASC direction', async () => {
                ...
                // ASSERT
                expect(getManyRowsAndCount).toHaveBeenCalledWith(expect.objectContaining({
                    sortBy: ['name', 'ASC'],
                }));
            });
            ...
            it('should work with undefined sortBy', async () => {
                ...
                // ACT
                await executeSelectMany(mockExecuteFunctions, 0, dataTableProxy, false, undefined, undefined);
                // ASSERT
                expect(getManyRowsAndCount).toHaveBeenCalledWith(expect.objectContaining({
                    sortBy: undefined,
                }));
            ...
            it('should maintain sortBy across paginated requests', async () => {
                ...
                await executeSelectMany(mockExecuteFunctions, 0, dataTableProxy, false, undefined, sortBy);
                // ASSERT
                expect(getManyRowsAndCount).toHaveBeenNthCalledWith(1, expect.objectContaining({
                    sortBy: ['id', 'ASC'],
                }));
                expect(getManyRowsAndCount).toHaveBeenNthCalledWith(2, expect.objectContaining({
                    sortBy: ['id', 'ASC'],
                }));
            });
        });
    ...
    describe('getSelectFilter', () => {
        it('should validate filter conditions against table schema', async () => {
            ...
            await expect(execution).rejects.toThrow(NodeOperationError);
            await expect(execution).rejects.toThrow('Filter validation failed: Column(s) "invalid_column" do not exist in the selected table. ' +
        ...
        it('should report multiple invalid columns in error message', async () => {
            ...
            const execution = getSelectFilter(mockExecuteFunctions, 0);
            await expect(execution).rejects.toThrow(NodeOperationError);
            await expect(execution).rejects.toThrow('Filter validation failed: Column(s) "invalid1, invalid2" do not exist in the selected table. ' +
                'This often happens when switching between tables with different schemas. ' +
                'Please update your filter conditions.');
        });
    });
});
//# sourceMappingURL=selectMany.test.js.map