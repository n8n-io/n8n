...
describe('dataObjectToApiInput', () => {
    ...
    describe('Luxon DateTime objects', () => {
        it('should convert Luxon DateTime objects to JavaScript Date', () => {
            ...
            expect(result.createdAt.toISOString()).toBe('2025-09-01T12:00:00.000Z');
        });
    });
    ...
    describe('error cases', () => {
        it('should throw error for array inputs', () => {
            ...
            expect(() => dataObjectToApiInput(input, mockNode, 0)).toThrow(NodeOperationError);
            expect(() => dataObjectToApiInput(input, mockNode, 0)).toThrow('unexpected array input \'["item1","item2"]\' in row 0');
        });
        it('should throw error for plain objects', () => {
            const input = { metadata: { key: 'value' } };
            expect(() => dataObjectToApiInput(input, mockNode, 0)).toThrow(NodeOperationError);
            expect(() => dataObjectToApiInput(input, mockNode, 0)).toThrow('unexpected object input \'{"key":"value"}\' in row 0');
        });
        ...
        test('dataObjectToApiInput throws on invalid date-like object', () => {
            ...
            const input = { createdAt: dateLikeObject, name: 'test' };
            expect(() => dataObjectToApiInput(input, mockNode, 0)).toThrow(NodeOperationError);
            expect(() => dataObjectToApiInput(input, mockNode, 0)).toThrow("unexpected object input '{}' in row 0");
        });
        it('should include correct row number in error message', () => {
            const input = { items: ['item1'] };
            expect(() => dataObjectToApiInput(input, mockNode, 5)).toThrow('unexpected array input \'["item1"]\' in row 5');
        });
    });
    describe('mixed data types', () => {
        it('should handle mixed valid data types', () => {
            ...
            expect(result.updatedAt.toISOString()).toBe('2025-09-02T10:30:00.000Z');
            expect(result.scheduledAt).toBeInstanceOf(Date);
            expect(result.scheduledAt.toISOString()).toBe('2025-09-03T08:15:00.000Z');
...
describe('buildGetManyFilter', () => {
    describe('isEmpty/isNotEmpty translation', () => {
        it('should translate isEmpty to eq with null value', () => {
            const fieldEntries = [
                { keyName: 'name', condition: 'isEmpty', keyValue: 'ignored' },
            ];
        ...
        it('should translate isNotEmpty to neq with null value', () => {
            const fieldEntries = [
                { keyName: 'email', condition: 'isNotEmpty', keyValue: 'ignored' },
            ];
        ...
        it('should handle mixed conditions including isEmpty/isNotEmpty', () => {
            const fieldEntries = [
                { keyName: 'name', condition: 'eq', keyValue: 'John' },
                { keyName: 'email', condition: 'isEmpty', keyValue: 'ignored' },
                { keyName: 'phone', condition: 'isNotEmpty', keyValue: 'ignored' },
            ];
    ...
    describe('isTrue/isFalse translation', () => {
        it('should translate isTrue to eq with true value', () => {
            const fieldEntries = [
                { keyName: 'isActive', condition: 'isTrue', keyValue: 'ignored' },
            ];
            const result = buildGetManyFilter(fieldEntries, ALL_CONDITIONS, { isActive: 'boolean' }, mockNode);
            expect(result).toEqual({
                type: 'and',
                filters: [
                    {
        ...
        it('should translate isFalse to eq with false value', () => {
            const fieldEntries = [
                { keyName: 'email', condition: 'isFalse', keyValue: 'ignored' },
            ];
            const result = buildGetManyFilter(fieldEntries, ANY_CONDITION, { email: 'boolean' }, mockNode);
            expect(result).toEqual({
                type: 'or',
                filters: [
                    {
        ...
        it('should handle mixed conditions including isTrue/isFalse', () => {
            const fieldEntries = [
                { keyName: 'name', condition: 'eq', keyValue: 'John' },
                { keyName: 'isActive', condition: 'isTrue', keyValue: 'ignored' },
                { keyName: 'isDeleted', condition: 'isFalse', keyValue: 'ignored' },
            ];
            const result = buildGetManyFilter(fieldEntries, ALL_CONDITIONS, {
                name: 'string',
                isActive: 'boolean',
    ...
    it('should handle other conditions', () => {
        const fieldEntries = [
            ...
            { keyName: 'name', condition: 'like', keyValue: '%john%' },
        ];
    ...
    describe('date handling in filters', () => {
        it('should pass Date objects through unchanged', () => {
            ...
            const result = buildGetManyFilter(fieldEntries, ALL_CONDITIONS, { createdAt: 'date' }, mockNode);
            expect(result).toEqual({
                type: 'and',
                filters: [
                    {
        ...
        it('should convert ISO date strings to Date objects', () => {
            ...
            expect(result.filters[0].value.toISOString()).toBe(dateString);
        });
        it('should throw an Error for invalid date strings', () => {
            ...
            expect(() => buildGetManyFilter(fieldEntries, ALL_CONDITIONS, { createdAt: 'date' }, mockNode)).toThrowError(`Invalid date string '${invalidDateString}' for column 'createdAt'`);
        });
    });
});
