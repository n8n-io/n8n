	...
	methods = {
		loadOptions: {
			...
			async getCategories(
				...
				const categories = await wordpressApiRequestAllItems.call(
					this,
					'GET',
					'/categories',
					{},
			...
			async getTags(
				...
				const returnData: INodePropertyOptions[] = [];
				const tags = await wordpressApiRequestAllItems.call(
					this,
					'GET',
					'/tags',
					{},
						...
						try {
							if (resource === 'post') {
								...
								if (operation === 'update') {
									...
									const body: IPost = { id: parseInt(postId, 10) };
							...
							if (resource === 'user') {
								...
								if (operation === 'create') {
									...
									const additionalFields = this.getNodeParameter('additionalFields', i) as IDataObject;
									const body: IUser = { name, username, first_name: firstName, last_name: lastName, email, password };
