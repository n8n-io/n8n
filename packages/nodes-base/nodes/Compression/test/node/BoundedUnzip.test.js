 ...
 */
function createZipOfNestedArchivesWithDataDescriptors(memberNames) {
    const innerArchive = Buffer.from(fflate.zipSync({
 ...
 */
function createZip64Archive(realSize, options) {
    const base = Buffer.from(fflate.zipSync({ 'file.txt': [new Uint8Array(realSize), { level: 6 }] }));
...
function createZipWithUnderstatedSize(realSize, declaredSize) {
...
function createZipWithUnsupportedCompression() {
...
describe('boundedUnzip', () => {
    ...
    it('should reject when total decompressed output exceeds the size limit', async () => {
        ...
        await expect(boundedUnzip(compressed, 1500, 100)).rejects.toThrow('The decompressed output exceeds the maximum allowed size of 0 MB');
    });
    it('should reject a single entry whose declared size exceeds the limit', async () => {
        ...
        await expect(boundedUnzip(compressed, 1024, 100)).rejects.toThrow('The decompressed output exceeds the maximum allowed size of 0 MB');
    });
    it('should reject when entry count exceeds the limit', async () => {
        ...
        await expect(boundedUnzip(compressed, 1024 * 1024, 3)).rejects.toThrow('The archive contains more than 3 entries');
    });
    ...
    it('should enforce the size bound against the resolved ZIP64 size, not the sentinel', async () => {
        ...
        expect(result['file.txt'].length).toBe(realSize);
        await expect(boundedUnzip(compressed, realSize - 1, 100)).rejects.toThrow('The decompressed output exceeds the maximum allowed size of 0 MB');
    });
    ...
    it('should stop a large understated entry once its real output passes the limit', async () => {
        ...
        await expect(boundedUnzip(compressed, 64 * 1024, 100)).rejects.toThrow('The decompressed output exceeds the maximum allowed size of 0 MB');
    });
    it('should keep an entry named __proto__ as a plain key on the result', async () => {
        ...
        const compressed = Buffer.from(createZipData({ [placeholder]: 8 })
            .toString('latin1')
            .replaceAll(placeholder, '__proto__'), 'latin1');
        const result = await boundedUnzip(compressed, 1024, 100);
    ...
    it('should stop reading the central directory once the entry limit is passed', async () => {
        ...
        await expect(boundedUnzip(compressed, 1024 * 1024, 2)).rejects.toThrow('The archive contains more than 2 entries');
    });
    ...
    it('should reject invalid zip data', async () => {
        await expect(boundedUnzip(Buffer.from('invalid zip data'), 1024, 100)).rejects.toThrow('invalid zip data');
    });
    it('should surface zip decompression errors', async () => {
        const compressed = createZipWithUnsupportedCompression();
        await expect(boundedUnzip(compressed, 1024, 100)).rejects.toThrow('unknown compression type 99');
    });
});
//# sourceMappingURL=BoundedUnzip.test.js.map