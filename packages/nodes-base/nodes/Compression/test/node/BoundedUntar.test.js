...
function tarHeader(name, size, typeflag = '0', linkname = '') {
    ...
    if (linkname)
        header.write(linkname, 157, 100, 'utf-8');
...
describe('boundedUntar', () => {
    ...
    it('should reject when total decompressed output exceeds the size limit', async () => {
        ...
        await expect(boundedUntar(archive, 1500, 100)).rejects.toThrow('The decompressed output exceeds the maximum allowed size of 0 MB');
    });
    it('should reject a single entry whose declared size exceeds the limit', async () => {
        ...
        await expect(boundedUntar(archive, 1024, 100)).rejects.toThrow('The decompressed output exceeds the maximum allowed size of 0 MB');
    });
    it('should reject when entry count exceeds the limit', async () => {
        ...
        await expect(boundedUntar(archive, 1024 * 1024, 3)).rejects.toThrow('The archive contains more than 3 entries');
    });
...
//# sourceMappingURL=BoundedUntar.test.js.map