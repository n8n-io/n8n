...
function createGzipData(uncompressedSize) {
...
describe('boundedGunzip', () => {
    ...
    it('should reject data exceeding the size limit', async () => {
        ...
        await expect(boundedGunzip(compressed, 1024)).rejects.toThrow('The decompressed output exceeds the maximum allowed size of 0 MB');
    });
    ...
    it('should reject invalid gzip data', async () => {
        await expect(boundedGunzip(Buffer.from('invalid gzip data'), 1024)).rejects.toThrow('invalid gzip data');
    });
});
//# sourceMappingURL=BoundedGunzip.test.js.map