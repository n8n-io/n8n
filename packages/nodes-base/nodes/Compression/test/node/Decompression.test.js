...
const mockBoundedUnzip = (data, error) => {
    if (error) {
...
const mockBoundedUntar = (data, error) => {
    if (error) {
...
const mockBoundedGunzip = (data, error) => {
    if (error) {
...
describe('Compression Node - Decompress Operation', () => {
    ...
    describe('Zip Decompression', () => {
        it('should decompress a zip file successfully', async () => {
            ...
            vi.mocked(mockExecuteFunctions.helpers.getBinaryDataBuffer).mockResolvedValue(Buffer.from('mock zip data'));
        ...
        it('should process multiple zip files from comma-separated binary properties', async () => {
            ...
            expect(vi.mocked(mockExecuteFunctions.helpers.assertBinaryData)).toHaveBeenCalledWith(0, 'data1');
            expect(vi.mocked(mockExecuteFunctions.helpers.assertBinaryData)).toHaveBeenCalledWith(0, 'data2');
        });
    });
    describe('Gzip Decompression', () => {
        it('should decompress a gzip file successfully', async () => {
            ...
            vi.mocked(mockExecuteFunctions.helpers.assertBinaryData).mockReturnValue(mockBinaryData);
            vi.mocked(mockExecuteFunctions.helpers.getBinaryDataBuffer).mockResolvedValue(Buffer.from('mock gzip data'));
        ...
        it('should determine mime type and file extension for gzip file', async () => {
            ...
            mockBoundedGunzip(mockGunzipData);
            let callCount = 0;
            vi.mocked(mockExecuteFunctions.helpers.prepareBinaryData).mockImplementation(async (buffer, fileName, mimeType) => {
                callCount++;
                if (callCount === 1) {
                    return {
                        data: buffer.toString('base64'),
    ...
    describe('Tar Decompression', () => {
        ...
        it('should decompress a tar file successfully', async () => {
            ...
            vi.mocked(mockExecuteFunctions.helpers.assertBinaryData).mockReturnValue(mockBinaryData);
            vi.mocked(mockExecuteFunctions.helpers.getBinaryDataBuffer).mockResolvedValue(Buffer.from('mock tar data'));
        ...
        it('should route a .tar.gz file to tar extraction, not gzip', async () => {
            ...
            vi.mocked(mockExecuteFunctions.helpers.assertBinaryData).mockReturnValue(mockBinaryData);
            vi.mocked(mockExecuteFunctions.helpers.getBinaryDataBuffer).mockResolvedValue(Buffer.from('mock targz data'));
            mockBoundedUntar(tarContents);
            prepareBinaryDataPassthrough();
            const result = await compression.execute.call(mockExecuteFunctions);
            expect(result[0][0].binary?.file_0).toBeDefined();
        ...
        it('should decompress a .tgz file via tar extraction', async () => {
            ...
            vi.mocked(mockExecuteFunctions.helpers.assertBinaryData).mockReturnValue(mockBinaryData);
            vi.mocked(mockExecuteFunctions.helpers.getBinaryDataBuffer).mockResolvedValue(Buffer.from('mock tgz data'));
            mockBoundedUntar(tarContents);
            prepareBinaryDataPassthrough();
            const result = await compression.execute.call(mockExecuteFunctions);
            expect(result[0][0].binary?.file_0).toBeDefined();
    ...
    describe('Error Handling', () => {
        it('should throw NodeOperationError for an unsupported archive format', async () => {
            ...
            vi.mocked(mockExecuteFunctions.helpers.assertBinaryData).mockReturnValue(mockBinaryData);
            vi.mocked(mockExecuteFunctions.helpers.getBinaryDataBuffer).mockResolvedValue(Buffer.from('mock rar data'));
            await expect(compression.execute.call(mockExecuteFunctions)).rejects.toThrow(NodeOperationError);
        });
        ...
        it('should handle decompression errors with continueOnFail', async () => {
            ...
            vi.mocked(mockExecuteFunctions.helpers.assertBinaryData).mockReturnValue(mockBinaryData);
            vi.mocked(mockExecuteFunctions.helpers.getBinaryDataBuffer).mockResolvedValue(Buffer.from('invalid zip data'));
        ...
        it('should throw error when continueOnFail is false', async () => {
            ...
            mockBoundedUnzip({}, new Error('Invalid zip file'));
            mockExecuteFunctions.continueOnFail.mockReturnValue(false);
            await expect(compression.execute.call(mockExecuteFunctions)).rejects.toThrow('Invalid zip file');
        });
        it('should handle gunzip errors with continueOnFail', async () => {
            ...
            vi.mocked(mockExecuteFunctions.helpers.assertBinaryData).mockReturnValue(mockBinaryData);
            vi.mocked(mockExecuteFunctions.helpers.getBinaryDataBuffer).mockResolvedValue(Buffer.from('invalid gzip data'));
...
//# sourceMappingURL=Decompression.test.js.map