...
function locateCentralDirectory(data, eocdOffset) {
    ...
    if (locatorOffset >= 0 && data.readUInt32LE(locatorOffset) === ZIP64_EOCD_LOCATOR_SIGNATURE) {
        ...
        if (zip64Offset + ZIP64_EOCD_SIZE <= data.length &&
 ...
 */
function resolveZip64Sentinels(extra, fields) {
    ...
    for (let offset = 0; offset + 4 <= extra.length;) {
        ...
        const body = offset + 4;
        if (body + blockSize > extra.length)
            return undefined;
        if (extra.readUInt16LE(offset) === ZIP64_EXTRA_FIELD_ID) {
            if (blockSize < sentinelCount * 8)
                return undefined;
            let taken = 0;
            return fields.map((field) => field === ZIP64_SENTINEL ? readUInt64(extra, body + 8 * taken++) : field);
        }
...
function findDataOffset(data, localHeaderOffset) {
...
//# sourceMappingURL=ZipCentralDirectory.js.map