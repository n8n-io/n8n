...
import { BINARY_ENCODING, NodeOperationError, deepCopy, jsonParse, BINARY_MODE_COMBINED, } from 'n8n-workflow';
...
export async function execute(items, operation) {
    const returnData = [];
    ...
    for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
        try {
            ...
            if (operation === 'fromIcs') {
                convertedValue = parseIcsCalendar(convertedValue);
            }
