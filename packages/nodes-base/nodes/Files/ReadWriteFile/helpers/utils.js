...
export function errorMapper(error, itemIndex, context) {
    let message;
    let description;
...
export function normalizeFileSelector(fileSelectorRaw) {
    const fileSelector = String(fileSelectorRaw);
    if (!WINDOWS_DRIVE_PREFIX.test(fileSelector))
        return fileSelector;
...
export function escapeBracketsAndParens(fileSelector) {
