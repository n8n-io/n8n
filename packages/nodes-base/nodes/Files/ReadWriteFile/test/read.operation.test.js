...
describe('Read/Write Files from Disk, read operation', () => {
    let directory;
    const write = async (relativePath) => {
    ...
    const readWith = async (fileSelector, literalBrackets, typeVersion = 1.1) => {
        const context = mockDeep();
        context.getNode.mockReturnValue(mock({ typeVersion }));
        context.getNodeParameter.mockImplementation((name) => name === 'fileSelector'
            ? fileSelector
            : literalBrackets === undefined
                ? {}
                : { literalBrackets });
        context.helpers.prepareBinaryData.mockResolvedValue(mock());
        await execute.call(context, [{ json: {} }]);
        return context.helpers.prepareBinaryData.mock.calls
            .map(([, filePath]) => path.basename(filePath))
            .sort();
    };
    const failWith = async (fileSelector, literalBrackets) => {
        const error = await readWith(fileSelector, literalBrackets).then(() => undefined, (thrown) => thrown);
    ...
    describe('with literal brackets turned off', () => {
        ...
        it('does not enable extended glob syntax', async () => {
            ...
            await expect(readWith(`${directory}/alt/@(cat|dog).txt`, false)).rejects.toThrow('No file(s) found');
        });
...
//# sourceMappingURL=read.operation.test.js.map