...
export const properties = [
    {
        displayName: 'The node can only access paths under /home/node/. Paths outside this directory (for example, /tmp/ or /data/) fail with an access error.',
        name: 'cloudNotice',
        type: 'notice',
        default: '',
        displayOptions: {
...
export async function execute(items) {
    ...
    for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
        try {
            fileSelector = normalizeFileSelector(this.getNodeParameter('fileSelector', itemIndex));
            const options = this.getNodeParameter('options', itemIndex, {});
            let dataPropertyName = 'data';
            if (options.dataPropertyName) {
                dataPropertyName = options.dataPropertyName;
            }
            const literalBrackets = (options.literalBrackets ?? true);
