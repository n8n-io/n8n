...
import { parsePublishArguments, rabbitmqConnect, rabbitmqConnectExchange, rabbitmqConnectQueue, } from './GenericFunctions';
    ...
    methods = {
        credentialTest: {
            async rabbitmqConnectionTest(credential) {
                try {
                    const connection = await rabbitmqConnect(credential.data);
    ...
    async execute() {
        ...
        try {
            ...
            if (mode === 'queue') {
                ...
                for (let i = 0; i < items.length; i++) {
                    const routingKey = this.getNodeParameter('routingKey', nodeVersion >= 1.2 ? i : 0);
                    if (sendInputData) {
                        message = JSON.stringify(items[i].json);
                    }
                    else {
