...
describe('RabbitMQ node', () => {
    ...
    const buildExecuteFunctions = (typeVersion, operation = 'sendMessage') => {
        const executeFunctions = mockDeep();
        ...
        executeFunctions.getNodeParameter.mockImplementation((parameterName, itemIndex, fallbackValue) => {
    ...
    it('includes node version 1.2 and exposes operation selector for it', () => {
        ...
        const visibleOperation = node.description.properties.find((property) => property.name === 'operation' && property.type === 'options');
    ...
    it('keeps v1.1 exchange routing key resolution on item 0', async () => {
        ...
        expect(mockChannel.publish).toHaveBeenNthCalledWith(1, 'test.exchange', 'key.alpha', Buffer.from(JSON.stringify(inputItems[0].json)), { headers: {} });
        expect(mockChannel.publish).toHaveBeenNthCalledWith(2, 'test.exchange', 'key.alpha', Buffer.from(JSON.stringify(inputItems[1].json)), { headers: {} });
        expect(mockChannel.publish).toHaveBeenNthCalledWith(3, 'test.exchange', 'key.alpha', Buffer.from(JSON.stringify(inputItems[2].json)), { headers: {} });
    ...
    it('evaluates exchange routing key per item in v1.2', async () => {
        ...
        await node.execute.call(executeFunctions);
        expect(mockChannel.publish).toHaveBeenNthCalledWith(1, 'test.exchange', 'key.alpha', Buffer.from(JSON.stringify(inputItems[0].json)), { headers: {} });
        expect(mockChannel.publish).toHaveBeenNthCalledWith(2, 'test.exchange', 'key.beta', Buffer.from(JSON.stringify(inputItems[1].json)), { headers: {} });
        expect(mockChannel.publish).toHaveBeenNthCalledWith(3, 'test.exchange', 'key.gamma', Buffer.from(JSON.stringify(inputItems[2].json)), { headers: {} });
...
//# sourceMappingURL=RabbitMQ.node.test.js.map