...
describe('RabbitMQTrigger node', () => {
    ...
    const mockTriggerFunctions = mockDeep();
    let connectSpy;
    ...
    describe('manual execution', () => {
        it('should get a message from the queue', async () => {
            ...
            mockChannel.get.mockResolvedValue(message);
            const { closeFunction, manualTriggerFunction } = await trigger.trigger.call(mockTriggerFunctions);
            await manualTriggerFunction();
            expect(mockChannel.prefetch).toHaveBeenCalledWith(1);
            expect(mockChannel.get).toHaveBeenCalledWith('testQueue');
            expect(handleMessageSpy).toHaveBeenCalledWith(message, mockChannel, expect.anything(), 'immediately', options);
...
//# sourceMappingURL=RabbitMQTrigger.node.test.js.map