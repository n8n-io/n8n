    ...
    webhookMethods = {
        default: {
            async checkExists() {
                ...
                const webhooks = await boxApiRequestAllItemsMarker.call(this, 'entries', 'GET', endpoint, {});
                for (const webhook of webhooks) {
                    // Skip full fetch if target doesn't match — target is available in the mini object
                    if (webhook.target.id !== targetId ||
                    ...
                    if (fullWebhook.address === webhookUrl) {
                        const allEventsMatch = events.every((event) => fullWebhook.triggers.includes(event));
                        if (!allEventsMatch) {
                            return false;
                        }
                        webhookData.webhookId = fullWebhook.id;
                        return true;
                    }
                }
                return false;
