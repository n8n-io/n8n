...
describe('SalesforceTrigger', () => {
	...
	describe('Node Description', () => {
		it('should have correct basic properties', () => {
			expect(trigger.description.displayName).toBe('Salesforce Trigger');
			expect(trigger.description.name).toBe('salesforceTrigger');
		...
		it('should have correct node structure', () => {
			...
			expect(trigger.description.outputs).toEqual([NodeConnectionTypes.Main]);
			expect(trigger.description.subtitle).toBe('={{($parameter["triggerOn"])}}');
		});
		...
		it('should have correct triggerOn options', () => {
			...
			expect(triggerProp?.default).toBe('');
			...
			const expectedOptions = [
				...
				{ name: 'Case Created', value: 'caseCreated', description: 'When a new case is created' },
				{
					name: 'Case Updated',
					value: 'caseUpdated',
				...
				{ name: 'Lead Created', value: 'leadCreated', description: 'When a new lead is created' },
				{
					name: 'Lead Updated',
					value: 'leadUpdated',
				...
				{ name: 'User Created', value: 'userCreated', description: 'When a new user is created' },
				{
					name: 'User Updated',
					value: 'userUpdated',
			...
			expect(triggerProp?.options).toEqual(expectedOptions);
		});

		it('should have correct custom object configuration', () => {
			const customObjectProp = trigger.description.properties.find(
				(p) => p.name === 'customObject',
			);
			expect(customObjectProp).toBeDefined();
			expect(customObjectProp?.type).toBe('options');
			expect(customObjectProp?.required).toBe(true);
			expect(customObjectProp?.default).toBe('');
			expect(customObjectProp?.displayOptions?.show?.triggerOn).toEqual([
				'customObjectUpdated',
				'customObjectCreated',
			]);
			expect(customObjectProp?.typeOptions?.loadOptionsMethod).toBe('getCustomObjects');
		});
	});
