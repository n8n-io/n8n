...
describe('SalesforceTrigger', () => {
    ...
    const salesforceApiRequestAllItemsSpy = vi.spyOn(GenericFunctions, 'salesforceApiRequestAllItems');
    ...
    const filterAndManageProcessedItemsSpy = vi.spyOn(GenericFunctions, 'filterAndManageProcessedItems');
    beforeEach(() => {
    ...
    describe('Methods', () => {
        ...
        describe('getCustomObjects', () => {
            it('should return custom objects with sorted options', async () => {
                ...
                const result = await trigger.methods.loadOptions.getCustomObjects.call(mockLoadOptionsFunctions);
    ...
    describe('Authentication', () => {
        ...
        it('should expose an authentication selector defaulting to OAuth2', () => {
            const authentication = trigger.description.properties.find((p) => p.name === 'authentication');
    ...
    describe('Poll Function - Parameter Setup', () => {
        ...
        it('should handle standard object creation trigger', async () => {
            ...
            getQuerySpy.mockReturnValue('SELECT id,name,type FROM Account WHERE CreatedDate >= 2023-01-01T00:00:00.000Z');
            ...
            expect(getQuerySpy).toHaveBeenCalledWith(expect.objectContaining({
                conditionsUi: {
            ...
            expect(salesforceApiRequestAllItemsSpy).toHaveBeenCalledWith('records', 'GET', '/query', {}, expect.objectContaining({
        ...
        it('should handle standard object update trigger', async () => {
            mockPollFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'triggerOn')
                    return 'accountUpdated';
                return '';
            });
            const mockWorkflowData = { lastTimeChecked: '2023-01-01T00:00:00.000Z' };
            mockPollFunctions.getWorkflowStaticData.mockReturnValue(mockWorkflowData);
            getPollStartDateSpy.mockReturnValue('2022-12-31T23:45:00.000Z');
            getQuerySpy.mockReturnValue('SELECT id,name,type FROM Account WHERE LastModifiedDate >= 2022-12-31T23:45:00.000Z');
            salesforceApiRequestAllItemsSpy.mockResolvedValue([
        ...
        it('should handle custom object triggers', async () => {
            mockPollFunctions.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'triggerOn')
                    return 'customObjectCreated';
                if (paramName === 'customObject')
            ...
            mockPollFunctions.getWorkflowStaticData.mockReturnValue(mockWorkflowData);
            getPollStartDateSpy.mockReturnValue('2023-01-01T00:00:00.000Z');
            getQuerySpy.mockReturnValue('SELECT id FROM CustomObject__c WHERE CreatedDate >= 2023-01-01T00:00:00.000Z');
            salesforceApiRequestAllItemsSpy.mockResolvedValue([
        ...
        it('should update lastTimeChecked after polling', async () => {
            ...
            expect(DateTime.fromISO(mockWorkflowData.lastTimeChecked).isValid).toBe(true);
        });
    });
    ...
    describe('Poll Function - Error Handling', () => {
        ...
        it('should log error and rethrow in trigger mode with lastTimeChecked', async () => {
            ...
            await expect(trigger.poll.call(mockPollFunctions)).rejects.toThrow('API Error');
            expect(mockPollFunctions.logger.error).toHaveBeenCalledWith(expect.stringContaining("There was a problem in 'Salesforce Trigger Test' node in workflow 'test-workflow'"), expect.objectContaining({
    ...
    describe('Poll Function - Edge Cases', () => {
        ...
        it('should return null when no new items after filtering', async () => {
            const mockWorkflowData = { processedIds: ['001000000000001'] };
            mockPollFunctions.getWorkflowStaticData.mockReturnValue(mockWorkflowData);
            getPollStartDateSpy.mockReturnValue('2023-01-01T00:00:00.000Z');
            getQuerySpy.mockReturnValue('SELECT id,name,type FROM Account');
            salesforceApiRequestAllItemsSpy.mockResolvedValue([
    ...
    describe('Poll Function - Date Handling', () => {
        ...
        it('should use getPollStartDate with safety margin', async () => {
            const mockWorkflowData = { lastTimeChecked: '2023-01-01T12:00:00.000Z' };
            mockPollFunctions.getWorkflowStaticData.mockReturnValue(mockWorkflowData);
        ...
        it('should set endDate to current time', async () => {
            ...
            const lastTimeChecked = DateTime.fromISO(mockWorkflowData.lastTimeChecked);
            expect(lastTimeChecked >= startTime).toBe(true);
            expect(lastTimeChecked <= endTime).toBe(true);
        });
    });
});
//# sourceMappingURL=SalesforceTrigger.node.test.js.map