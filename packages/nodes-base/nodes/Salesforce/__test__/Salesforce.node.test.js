...
const resolveParam = (params, param, options) => {
    const value = get(params, param);
    const extractValue = options?.extractValue;
...
vi.mock('../GenericFunctions', async () => {
    const actual = await vi.importActual('../GenericFunctions');
    return {
        ...actual,
        getQuery: vi.fn(),
...
describe('Salesforce', () => {
    ...
    beforeEach(() => {
        ...
        mockExecuteFunctions.helpers.constructExecutionMetaData.mockImplementation((data, meta) => data.map((item, index) => ({
            ...(typeof item === 'object' && item !== null ? item : {}),
            pairedItem: meta?.itemData?.item || index,
        })));
        mockExecuteFunctions.helpers.returnJsonArray.mockImplementation((data) => Array.isArray(data) ? data.map((item) => ({ json: item })) : [{ json: data }]);
    ...
    describe('LoadOptions Methods', () => {
        ...
        describe('getLeadStatuses', () => {
            it('should return lead statuses with sorted options', async () => {
                ...
                const result = await node.methods.loadOptions.getLeadStatuses.call(mockLoadOptionsFunctions);
                expect(salesforceApiRequestAllItemsSpy).toHaveBeenCalledWith('records', 'GET', '/query', {}, { q: 'SELECT id, MasterLabel FROM LeadStatus' });
        ...
        describe('searchUsers (listSearch)', () => {
            it('should run a filtered SOQL query with a batchSize header and map results', async () => {
                ...
                const result = await node.methods.listSearch.searchUsers.call(mockLoadOptionsFunctions, 'jo');
                expect(salesforceApiRequestAllItemsSpy).not.toHaveBeenCalled();
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('GET', '/query', {}, { q: "SELECT Id, Name FROM User WHERE Name LIKE '%jo%' ORDER BY Name" }, undefined, { headers: { 'Sforce-Query-Options': 'batchSize=200' } });
                expect(result).toEqual({
                    results: [
            ...
            it('should omit the WHERE clause when no filter is given', async () => {
                ...
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('GET', '/query', {}, { q: 'SELECT Id, Name FROM User ORDER BY Name' }, undefined, { headers: { 'Sforce-Query-Options': 'batchSize=200' } });
            ...
            it('should escape single quotes in the filter without applying a LIMIT', async () => {
                ...
                const qs = salesforceApiRequestSpy.mock.calls[0][3];
            ...
            it('should follow the pagination cursor instead of re-querying', async () => {
                ...
                const result = await node.methods.listSearch.searchUsers.call(mockLoadOptionsFunctions, '', '/services/data/v59.0/query/01g-2000');
        ...
        describe('searchCaseOwners (listSearch)', () => {
            it('should combine case queues and users with prefixed labels', async () => {
                ...
                const result = await node.methods.listSearch.searchCaseOwners.call(mockLoadOptionsFunctions, '');
                // Queues are fetched in full (legacy behaviour), users via a capped search.
                expect(salesforceApiRequestAllItemsSpy).toHaveBeenCalledWith('records', 'GET', '/query', {}, {
            ...
            it('should not prefix users when the filter matches no queues', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ records: [{ Id: 'user1', Name: 'John Doe' }] });
                const result = await node.methods.listSearch.searchCaseOwners.call(mockLoadOptionsFunctions, 'doe');
            ...
            it('should filter queues in-memory by the search term', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ records: [] });
                const result = await node.methods.listSearch.searchCaseOwners.call(mockLoadOptionsFunctions, 'support');
            ...
            it('should skip the queue query when following a pagination cursor', async () => {
                salesforceApiRequestSpy.mockResolvedValue({ records: [{ Id: 'user1', Name: 'John Doe' }] });
                const result = await node.methods.listSearch.searchCaseOwners.call(mockLoadOptionsFunctions, '', '/services/data/v59.0/query/01g-2000');
                expect(salesforceApiRequestAllItemsSpy).not.toHaveBeenCalled();
                expect(salesforceApiRequestSpy).toHaveBeenCalledTimes(1);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('GET', '/query/01g-2000');
        ...
        describe('searchLeadOwners (listSearch)', () => {
            it('should query the Lead queues and prefix users', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ records: [{ Id: 'user1', Name: 'John Doe' }] });
                const result = await node.methods.listSearch.searchLeadOwners.call(mockLoadOptionsFunctions, '');
                expect(salesforceApiRequestAllItemsSpy).toHaveBeenCalledWith('records', 'GET', '/query', {}, {
        ...
        describe('getCustomFields', () => {
            it('should return custom fields for a resource', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getCustomFields.call(mockLoadOptionsFunctions);
        ...
        describe('getRecordTypes', () => {
            ...
            it('should properly escape special characters in resource names', async () => {
                mockLoadOptionsFunctions.getNodeParameter.mockReturnValue("Account'; DROP TABLE RecordType--");
                salesforceApiRequestAllItemsSpy.mockResolvedValue([]);
                ...
                const query = callArgs[4].q;
                expect(query).toContain("\\'");
                expect(query).toContain("Account\\'");
                expect(query).toMatch(/SELECT Id, Name, SobjectType, IsActive FROM RecordType WHERE SobjectType = /);
            ...
            it('should prevent UNION', async () => {
                mockLoadOptionsFunctions.getNodeParameter.mockReturnValue("Account' UNION SELECT Id, Name FROM User--");
                salesforceApiRequestAllItemsSpy.mockResolvedValue([]);
                await node.methods.loadOptions.getRecordTypes.call(mockLoadOptionsFunctions);
                const callArgs = salesforceApiRequestAllItemsSpy.mock.calls[0];
                const query = callArgs[4].q;
        ...
        describe('getExternalIdFields', () => {
            it('should return external ID and lookup fields', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getExternalIdFields.call(mockLoadOptionsFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('GET', '/sobjects/Account/describe');
                expect(result).toEqual([
            ...
            it('should handle custom object for external ID fields', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getExternalIdFields.call(mockLoadOptionsFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('GET', '/sobjects/CustomObject__c/describe');
        ...
        describe('searchAccounts (listSearch)', () => {
            ...
            it('should narrow results with a SOQL LIKE clause when a filter is provided', async () => {
                ...
                expect(qs.q).toBe("SELECT Id, Name FROM Account WHERE Name LIKE '%acme%' ORDER BY Name");
            });
            it('should escape single quotes in the filter to prevent breaking the SOQL string', async () => {
                ...
                const [, , , qs] = salesforceApiRequestSpy.mock.calls[0];
                expect(qs.q).toBe("SELECT Id, Name FROM Account WHERE Name LIKE '%O\\'Brien%' ORDER BY Name");
            });
            ...
            it('should follow the cursor across pages and stop when nextRecordsUrl is absent', async () => {
                ...
                const page1 = await node.methods.listSearch.searchAccounts.call(mockLoadOptionsFunctions, 'acme');
                expect(page1.paginationToken).toBe('/services/data/v59.0/query/01g4o00000abcdef-200');
                const page2 = await node.methods.listSearch.searchAccounts.call(mockLoadOptionsFunctions, 'acme', page1.paginationToken);
            ...
            it('should page through a 2000-account org via the cursor and terminate exactly once exhausted', async () => {
                ...
                let pages = 0;
                do {
                    const res = await node.methods.listSearch.searchAccounts.call(mockLoadOptionsFunctions, undefined, token);
        ...
        describe('getCampaigns', () => {
            it('should return campaigns with sorted options', async () => {
                ...
                expect(salesforceApiRequestAllItemsSpy).toHaveBeenCalledWith('records', 'GET', '/query', {}, { q: 'SELECT id, Name FROM Campaign' });
                expect(result).toEqual([
        ...
        describe('getStages', () => {
            it('should return opportunity stages from picklist values', async () => {
                ...
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('GET', '/sobjects/opportunity/describe');
                expect(result).toEqual([
        ...
        describe('getAccountTypes', () => {
            it('should return account types from picklist values', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getAccountTypes.call(mockLoadOptionsFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('GET', '/sobjects/account/describe');
                expect(result).toEqual([
        ...
        describe('getAccountSources', () => {
            it('should return account sources from picklist values', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getAccountSources.call(mockLoadOptionsFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('GET', '/sobjects/account/describe');
                expect(result).toEqual([
                    { name: 'Web', value: 'Web' },
                    { name: 'Phone Inquiry', value: 'Phone Inquiry' },
        ...
        describe('getCaseStatuses', () => {
            it('should return case statuses from picklist values', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getCaseStatuses.call(mockLoadOptionsFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('GET', '/sobjects/case/describe');
                expect(result).toEqual([
                    { name: 'New', value: 'New' },
                    { name: 'Working', value: 'Working' },
        ...
        describe('getCustomObjects', () => {
            it('should return custom objects only', async () => {
                ...
                const result = await node.methods.loadOptions.getCustomObjects.call(mockLoadOptionsFunctions);
        ...
        describe('getCustomObjectFields', () => {
            it('should return all fields for a custom object', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getCustomObjectFields.call(mockLoadOptionsFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('GET', '/sobjects/CustomObject__c/describe');
                expect(result).toEqual([
                    { name: 'ID', value: 'Id' },
        ...
        describe('Field Methods', () => {
            it('should return account fields', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getAccountFields.call(mockLoadOptionsFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('GET', '/sobjects/account/describe');
                expect(result).toEqual([
            ...
            it('should return opportunity fields', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getOpportunityFields.call(mockLoadOptionsFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('GET', '/sobjects/opportunity/describe');
                expect(result).toEqual([
            ...
            it('should return contact fields', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getContactFields.call(mockLoadOptionsFunctions);
            ...
            it('should return attachment fields', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getAtachmentFields.call(mockLoadOptionsFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('GET', '/sobjects/attachment/describe');
                expect(result).toEqual([
        ...
        describe('Task Picklist Methods', () => {
            it('should return task statuses', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getTaskStatuses.call(mockLoadOptionsFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('GET', '/sobjects/task/describe');
                expect(result).toEqual([
            ...
            it('should return task subjects', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getTaskSubjects.call(mockLoadOptionsFunctions);
                expect(result).toEqual([
                    { name: 'Call', value: 'Call' },
            ...
            it('should return task call types', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getTaskCallTypes.call(mockLoadOptionsFunctions);
                expect(result).toEqual([
            ...
            it('should return task priorities', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getTaskPriorities.call(mockLoadOptionsFunctions);
                expect(result).toEqual([
                    { name: 'Low', value: 'Low' },
            ...
            it('should return task recurrence types', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getTaskRecurrenceTypes.call(mockLoadOptionsFunctions);
                expect(result).toEqual([
            ...
            it('should return task recurrence instances', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getTaskRecurrenceInstances.call(mockLoadOptionsFunctions);
                expect(result).toEqual([
        ...
        describe('Case Picklist Methods', () => {
            ...
            it('should return case priorities', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue(mockDescribe);
                const result = await node.methods.loadOptions.getCasePriorities.call(mockLoadOptionsFunctions);
                expect(result).toEqual([
                    { name: 'Low', value: 'Low' },
    ...
    describe('Execute Method - Lead Resource', () => {
        beforeEach(() => {
            ...
            mockExecuteFunctions.getNodeParameter.mockImplementation((param, _index) => {
        ...
        describe('Lead Create Operation', () => {
            it('should handle lead create with minimal fields', async () => {
                ...
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('POST', '/sobjects/lead', expect.objectContaining({
                    Company: 'Test Company',
                    LastName: 'Test Lead',
                }));
            });
            ...
            it('should resolve a resourceLocator owner value to OwnerId', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ id: 'lead456', success: true });
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('POST', '/sobjects/lead', expect.objectContaining({ OwnerId: 'user123' }));
            });
            it('should handle lead upsert operation', async () => {
                mockExecuteFunctions.getNodeParameter.mockImplementation((param, index) => {
                    const params = {
                        resource: 'lead',
                        operation: 'upsert',
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('PATCH', '/sobjects/lead/External_Id__c/EXT123', expect.objectContaining({
                    Company: 'Upsert Company',
                    LastName: 'Upsert Lead',
                    Email: 'upsert@example.com',
                }));
        ...
        describe('Lead Update Operation', () => {
            it('should handle lead update with all fields', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('PATCH', '/sobjects/lead/lead123', expect.objectContaining({
                    HasOptedOutOfEmail: false,
            ...
            it('should throw error when no update fields provided', async () => {
                ...
                await expect(node.execute.call(mockExecuteFunctions)).rejects.toThrow('You must add at least one update field');
            });
        });
        describe('Lead Other Operations', () => {
            ...
            it('should handle lead getAll operation with returnAll true', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(getQuerySpy).toHaveBeenCalledWith({ fields: 'Id,FirstName,LastName' }, 'Lead', true, 0, 1);
                expect(salesforceApiRequestAllItemsSpy).toHaveBeenCalledWith('records', 'GET', '/query', {}, { q: 'SELECT Id,FirstName,LastName FROM Lead' });
            });
            ...
            it('should handle lead addToCampaign operation', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('POST', '/sobjects/CampaignMember', expect.objectContaining({
                    LeadId: 'lead123',
                    CampaignId: 'campaign456',
                    Status: 'Sent',
                }));
            ...
            it('should handle lead addNote operation', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('POST', '/sobjects/note', expect.objectContaining({
                    Title: 'Important Note',
                    ParentId: 'lead123',
                    Body: 'This is the note body',
                    OwnerId: 'user789',
    ...
    describe('Execute Method - Case Resource', () => {
        ...
        describe('Case Create Operation', () => {
            it('should handle case create with minimal fields', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('POST', '/sobjects/case', expect.objectContaining({
                    Type: 'Problem',
                }));
            });
        ...
        describe('Case Update Operation', () => {
            it('should handle case update with all fields', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ success: true });
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('PATCH', '/sobjects/case/case123', expect.objectContaining({
                    Type: 'Updated Problem',
                    Origin: 'Email',
                    Reason: 'Performance',
                    Status: 'Working',
            ...
            it('should pass ParentId when updating a case to set a parent', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ success: true });
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('PATCH', '/sobjects/case/childCase456', expect.objectContaining({
                    ParentId: 'parentCase456',
                }));
            });
            it('should handle case update with custom fields', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ success: true });
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('PATCH', '/sobjects/case/case789', expect.objectContaining({
        ...
        describe('Case Other Operations', () => {
            ...
            it('should handle case getAll operation with returnAll true', async () => {
                ...
                expect(salesforceApiRequestAllItemsSpy).toHaveBeenCalledWith('records', 'GET', '/query', {}, { q: 'SELECT Id,Subject,Type FROM Case' });
            });
            ...
            it('should handle case addComment operation', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('POST', '/sobjects/casecomment', expect.objectContaining({
                    ParentId: 'case123',
                    CommentBody: 'This is a test comment',
                    IsPublished: true,
                }));
    ...
    describe('Execute Method - Contact Resource', () => {
        ...
        describe('Contact Create Operation', () => {
            it('should handle contact create with minimal fields', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('POST', '/sobjects/contact', expect.objectContaining({
                    LastName: 'Test Contact',
                }));
            });
            ...
            it('should map the Account resourceLocator (list mode) onto AccountId', async () => {
                mockExecuteFunctions.getNodeParameter.mockImplementation((param, _i, _d, options) => {
                    const params = {
                        resource: 'contact',
                        operation: 'create',
            ...
            it('should omit AccountId when the Account value is empty', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                const body = salesforceApiRequestSpy.mock.calls[0][2];
            ...
            it('should handle contact upsert operation', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('PATCH', '/sobjects/contact/External_Contact_Id__c/EXT_CONTACT_123', expect.objectContaining({
                    LastName: 'Upsert Contact',
                    Email: 'upsert@contact.com',
                }));
                // Ensure external ID field is removed from body
        ...
        describe('Contact Update Operation', () => {
            it('should handle contact update with all fields', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ success: true });
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('PATCH', '/sobjects/contact/contact123', expect.objectContaining({
                    LastName: 'Updated Contact',
                    Fax: '9876543210',
                    Email: 'updated@contact.com',
                    RecordTypeId: 'rt999',
            ...
            it('should accept Account as a resourceLocator object on update', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ success: true });
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('PATCH', '/sobjects/contact/contact-rl-update', expect.objectContaining({ AccountId: 'acc-updated' }));
            });
        });
        describe('Contact Other Operations', () => {
            ...
            it('should handle contact getAll operation with returnAll true', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(getQuerySpy).toHaveBeenCalledWith({ fields: 'Id,FirstName,LastName' }, 'Contact', true, 0, 1);
                expect(salesforceApiRequestAllItemsSpy).toHaveBeenCalledWith('records', 'GET', '/query', {}, { q: 'SELECT Id,FirstName,LastName FROM Contact' });
            });
            it('should handle contact delete operation', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ success: true });
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('DELETE', '/sobjects/contact/contact123');
            });
    ...
    describe('Execute Method - CustomObject Resource', () => {
        ...
        describe('CustomObject Create Operation', () => {
            it('should handle customObject create with custom fields', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('POST', '/sobjects/CustomObject__c', expect.objectContaining({
            ...
            it('should handle customObject upsert operation', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('PATCH', '/sobjects/CustomObject__c/External_Id__c/EXT_CUSTOM_123', expect.objectContaining({
        ...
        describe('CustomObject Update Operation', () => {
            it('should handle customObject update operation', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ success: true });
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('PATCH', '/sobjects/CustomObject__c/custom123', expect.objectContaining({
            ...
            it.each([
                ...
                ['undefined', undefined],
                ['null', null],
            ])('should normalize a %s 204 response into a valid success item', async (_label, emptyResponse) => {
                mockExecuteFunctions.getNodeParameter.mockImplementation((param) => {
                    const params = {
                        resource: 'customObject',
                        operation: 'update',
        ...
        describe('CustomObject Other Operations', () => {
            it('should handle customObject get operation', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('GET', '/sobjects/CustomObject__c/custom123');
            });
            it('should handle customObject getAll operation with returnAll true', async () => {
                ...
                expect(salesforceApiRequestAllItemsSpy).toHaveBeenCalledWith('records', 'GET', '/query', {}, { q: 'SELECT * FROM CustomObject__c' });
            });
            it('should handle customObject delete operation', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ success: true });
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('DELETE', '/sobjects/CustomObject__c/custom123');
            });
        });
    });
    describe('Execute Method - Document Resource', () => {
        ...
        describe('Document Upload Operation', () => {
            it('should handle document upload operation', async () => {
                ...
                mockExecuteFunctions.helpers.getBinaryDataBuffer.mockResolvedValue(mockBinaryData.data);
                ...
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('POST', '/sobjects/ContentVersion', {}, {}, undefined, expect.objectContaining({
                ...
                const callArgs = salesforceApiRequestSpy.mock.calls[0];
                const formData = callArgs[5].formData;
                const entityContent = jsonParse(formData.entity_content.value);
            ...
            it('should handle document upload with minimal fields', async () => {
                ...
                expect(callArgs[5].formData).toBeDefined();
                const formData = callArgs[5].formData;
                const entityContent = jsonParse(formData.entity_content.value);
                expect(entityContent).toEqual({
                    Title: 'Minimal Document',
    ...
    describe('Execute Method', () => {
        ...
        describe('Flow Resource', () => {
            it('should execute flow invoke operation', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ success: true });
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('POST', '/actions/custom/flow/TestFlow', {
        ...
        describe('Search Resource', () => {
            it('should execute search query operation', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestAllItemsSpy).toHaveBeenCalledWith('records', 'GET', '/query', {}, { q: 'SELECT Id, Name FROM Account' });
            });
        });
        describe('Opportunity Resource', () => {
            it('should handle opportunity create operation with all fields', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('POST', '/sobjects/opportunity', expect.objectContaining({
                    Name: 'Test Opportunity',
                    CloseDate: '2024-12-31',
                    StageName: 'Qualification',
                    Type: 'New Customer',
            ...
            it('should map the Account resourceLocator onto AccountId (create)', async () => {
                ...
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('POST', '/sobjects/opportunity', expect.objectContaining({ Name: 'RLC Opportunity', AccountId: 'acc-rlc-opp' }));
            });
            it('should handle opportunity upsert operation', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ id: 'opp123', success: true });
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('PATCH', '/sobjects/opportunity/External_Id__c/EXT123', expect.objectContaining({
                    Name: 'Upsert Opportunity',
                    CloseDate: '2024-12-31',
                    StageName: 'Negotiation',
                    Type: 'Existing Customer',
            ...
            it('should handle opportunity update operation with all fields', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ success: true });
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('PATCH', '/sobjects/opportunity/opp123', expect.objectContaining({
                    Name: 'Updated Opportunity',
                    CloseDate: '2025-01-31',
                    StageName: 'Closed Won',
                    Type: 'Existing Customer',
        ...
        describe('Error Handling', () => {
            beforeEach(() => {
                mockExecuteFunctions.helpers.returnJsonArray.mockImplementation((data) => [{ json: data }]);
                mockExecuteFunctions.helpers.constructExecutionMetaData.mockImplementation((data, { itemData }) => data.map((item) => ({ ...item, pairedItem: { item: itemData.item } })));
            });
    ...
    describe('Execute Method - Account Resource Extended Fields', () => {
        ...
        describe('Account Create Operation - Additional Fields', () => {
            ...
            it('should handle account upsert operation', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ id: 'acc123', success: true });
                await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('PATCH', '/sobjects/account/AccountNumber/ACC-001', {
                    Name: 'Test Account',
                    Fax: '555-0199',
                    Type: 'Customer - Direct',
                });
    ...
    describe('Execute Method - Task Resource Extended Fields', () => {
        ...
        describe('Task GetAll Operation - Query and Error Handling', () => {
            it('should handle task getAll with limit', async () => {
                ...
                const result = await node.execute.call(mockExecuteFunctions);
                expect(getQuerySpy).toHaveBeenCalledWith({ fields: 'Id,Subject,Status' }, 'Task', false, 10, 1);
                expect(salesforceApiRequestAllItemsSpy).toHaveBeenCalledWith('records', 'GET', '/query', {}, {
    ...
    describe('Execute Method - Attachment Resource', () => {
        describe('Attachment Create Operation', () => {
            ...
            it('should throw error when binary property does not exist', async () => {
                ...
                await expect(node.execute.call(mockExecuteFunctions)).rejects.toThrow('The property nonexistent does not exist');
            });
        });
        describe('Attachment Update Operation', () => {
            it('should handle attachment update with binary data', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ id: 'att123', success: true });
                const result = await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('PATCH', '/sobjects/attachment/att123', {
                    Name: 'Updated File Name',
            ...
            it('should throw error when binary property does not exist for update', async () => {
                ...
                await expect(node.execute.call(mockExecuteFunctions)).rejects.toThrow('The property missing does not exist');
            });
        });
        ...
        describe('Attachment GetAll Operation', () => {
            it('should handle attachment getAll with returnAll true', async () => {
                ...
                const result = await node.execute.call(mockExecuteFunctions);
                expect(getQuerySpy).toHaveBeenCalledWith({ fields: 'Id,Name,ParentId' }, 'Attachment', true, 0, 1);
                expect(salesforceApiRequestAllItemsSpy).toHaveBeenCalledWith('records', 'GET', '/query', {}, {
            ...
            it('should handle attachment getAll operation error handling', async () => {
                ...
                salesforceApiRequestAllItemsSpy.mockRejectedValue(testError);
                await expect(node.execute.call(mockExecuteFunctions)).rejects.toThrow('Query execution failed');
            });
        });
        describe('Attachment Delete Operation', () => {
            it('should handle attachment delete operation', async () => {
                ...
                salesforceApiRequestSpy.mockResolvedValue({ id: 'att123', success: true });
                const result = await node.execute.call(mockExecuteFunctions);
                expect(salesforceApiRequestSpy).toHaveBeenCalledWith('DELETE', '/sobjects/attachment/att123');
                expect(result).toEqual([[{ json: { id: 'att123', success: true }, pairedItem: 0 }]]);
            });
            it('should handle attachment delete operation error handling', async () => {
                ...
                salesforceApiRequestSpy.mockRejectedValue(testError);
                await expect(node.execute.call(mockExecuteFunctions)).rejects.toThrow('Delete operation failed');
            });
        });
    ...
    describe('Execute Method - GetAll Query Wiring', () => {
        const mockGetAll = (resource, returnAll, limit, options = {}) => {
            mockExecuteFunctions.getNodeParameter.mockImplementation((param) => {
                const params = {
                    resource,
                    operation: 'getAll',
    ...
    describe('Error output routing (onError)', () => {
        const buildApiError = () => Object.assign(new Error('Bad request - please check your parameters'), {
