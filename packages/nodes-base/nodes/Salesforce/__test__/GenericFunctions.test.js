...
import { getValue, getConditions, sortOptions, getDefaultFields, getQuery, filterAndManageProcessedItems, salesforceApiRequest, escapeSoqlString, getResourceLocatorValue, validateSoqlFieldName, validateSoqlOperator, validateSoqlObjectName, } from '../GenericFunctions';
...
const uploadFormData = (entityContent) => ({
    entity_content: {
...
describe('Salesforce -> GenericFunctions', () => {
    ...
    describe('getConditions', () => {
        ...
        it('should handle other operations with numeric types', () => {
            ...
            const result = getConditions(options);
            expect(result).toBe('WHERE field_1 > 123 AND field_2 < 456 AND field_3 >= 789 AND field_4 <= 0');
        });
    ...
    describe('getDefaultFields', () => {
        it('should return default fields', () => {
            ...
            expect(getDefaultFields('Lead')).toBe('id,company,firstname,lastname,street,postalCode,city,email,status,LastModifiedDate');
            expect(getDefaultFields('Contact')).toBe('id,firstname,lastname,email,LastModifiedDate');
            expect(getDefaultFields('Opportunity')).toBe('id,accountId,amount,probability,type,LastModifiedDate');
            expect(getDefaultFields('Case')).toBe('id,accountId,contactId,priority,status,subject,type,LastModifiedDate');
    ...
    describe('filterAndManageProcessedItems', () => {
        ...
        it('should handle large numbers of processed IDs efficiently', () => {
            ...
            expect(result.updatedProcessedIds.slice(-10)).toEqual(responseData.map((item) => item.Id));
        });
        it('should handle very large batches of new items', () => {
            const processedIds = ['existing-1', 'existing-2'];
            // Create 1005 new items
            const responseData = Array.from({ length: 1005 }, (_, i) => ({
                Id: `new-${i}`,
                Name: `New Item ${i}`,
            }));
            const result = filterAndManageProcessedItems(responseData, processedIds);
            ...
            const expectedIds = processedIds.concat(responseData.map((item) => item.Id));
        ...
        it('should trim processed IDs to MAX_IDS limit (10000)', () => {
            ...
            const expectedIds = processedIds
                .slice(-9990)
                .concat(responseData.map((item) => item.Id));
            expect(result.updatedProcessedIds).toEqual(expectedIds);
        });
        it('should trim processed IDs when exceeding MAX_IDS with large batch', () => {
            ...
            const responseData = Array.from({ length: 6000 }, (_, i) => ({
                Id: `new-${i}`,
                Name: `New Item ${i}`,
            }));
            const result = filterAndManageProcessedItems(responseData, processedIds);
    ...
    describe('salesforceApiRequest Error Context', () => {
        ...
        it('should aggregate fields from multiple SF errors onto context', async () => {
            ...
            await expect(salesforceApiRequest.call(mockExecuteFunctions, 'POST', '/sobjects/Lead', {})).rejects.toMatchObject({
                context: {
                    errorCode: 'FIELD_CUSTOM_VALIDATION_EXCEPTION',
    ...
    describe('salesforceApiRequest - OAuth2 form data', () => {
        ...
        beforeEach(() => {
            ...
            mockRequest = vi.fn();
            mockExecuteFunctions.helpers.requestOAuth2 = mockRequest;
            mockExecuteFunctions.getNodeParameter.mockImplementation((param) => param === 'authentication' ? 'oAuth2' : undefined);
            mockExecuteFunctions.getCredentials.mockResolvedValue({
        ...
        it('forwards the formData option to the OAuth2 request helper for multipart uploads', async () => {
            ...
            await salesforceApiRequest.call(mockExecuteFunctions, 'POST', '/sobjects/ContentVersion', {}, {}, undefined, { formData });
    ...
    describe('salesforceApiRequest - JWT Authentication', () => {
        ...
        describe('JWT Authentication Flow', () => {
            ...
            it('forwards body and query parameters', async () => {
                ...
                await salesforceApiRequest.call(mockExecuteFunctions, 'POST', '/test-endpoint', testBody, testQs);
                expect(mockRequest).toHaveBeenCalledWith('salesforceJwtApi', {
                    headers: {
                        'Content-Type': 'application/json',
                    },
            ...
            it('uses the custom URI when provided', async () => {
                ...
                await salesforceApiRequest.call(mockExecuteFunctions, 'GET', '/default-endpoint', {}, {}, '/custom-uri');
                expect(mockRequest).toHaveBeenCalledWith('salesforceJwtApi', expect.objectContaining({
            ...
            it('merges additional options', async () => {
                ...
                await salesforceApiRequest.call(mockExecuteFunctions, 'GET', '/test-endpoint', {}, {}, undefined, additionalOptions);
                expect(mockRequest).toHaveBeenCalledWith('salesforceJwtApi', expect.objectContaining({
                    url: '/services/data/v59.0/test-endpoint',
                    timeout: 30000,
                    returnFullResponse: true,
            ...
            it('merges extra option headers without dropping the Content-Type header', async () => {
                ...
                await salesforceApiRequest.call(mockExecuteFunctions, 'GET', '/query', {}, { q: 'SELECT Id FROM Account' }, undefined, { headers: { 'Sforce-Query-Options': 'batchSize=200' } });
            ...
            it('logs the relative URL being invoked', async () => {
                mockRequest.mockResolvedValue({});
                await salesforceApiRequest.call(mockExecuteFunctions, 'GET', '/test-endpoint', {}, {});
                expect(mockExecuteFunctions.logger.debug).toHaveBeenCalledWith('Authentication for "Salesforce" node is using "jwt". Invoking URI /services/data/v59.0/test-endpoint');
            });
            it('sends the formData option as a multipart FormData body', async () => {
                ...
                const result = await salesforceApiRequest.call(mockExecuteFunctions, 'POST', '/sobjects/ContentVersion', {}, {}, undefined, { formData });
                ...
                expect(sentOptions.body.getHeaders()['content-type']).toMatch(/^multipart\/form-data/);
                expect(result).toEqual({ id: 'cv1', success: true });
            });
            it('preserves each formData field, its per-part content type and filename', async () => {
                mockRequest.mockResolvedValue({ statusCode: 201, body: { id: 'cv1', success: true } });
                await salesforceApiRequest.call(mockExecuteFunctions, 'POST', '/sobjects/ContentVersion', {}, {}, undefined, { formData: uploadFormData({ Title: 'Doc' }) });
            ...
            it('returns the response body on a successful status', async () => {
                ...
                const result = await salesforceApiRequest.call(mockExecuteFunctions, 'GET', '/query', {}, {});
        ...
        describe('JWT Authentication Error Handling', () => {
            it('wraps request errors in NodeApiError', async () => {
                ...
                await expect(salesforceApiRequest.call(mockExecuteFunctions, 'GET', '/test-endpoint', {}, {})).rejects.toThrow(NodeApiError);
            });
        ...
        describe('getResourceLocatorValue', () => {
            ...
            it('extracts the value from a resourceLocator object', () => {
                expect(getResourceLocatorValue({ __rl: true, mode: 'list', value: '0051700000ABCDE' })).toBe('0051700000ABCDE');
                expect(getResourceLocatorValue({ __rl: true, mode: 'id', value: '0051700000ABCDE' })).toBe('0051700000ABCDE');
            });
        ...
        describe('Security Functions: escaping', () => {
            ...
            describe('validateSoqlFieldName', () => {
                ...
                it('should reject field name with spaces', () => {
                    expect(() => validateSoqlFieldName('Invalid Field')).toThrow('Invalid SOQL field name: Invalid Field');
                });
                it('should reject field name with special characters', () => {
                    expect(() => validateSoqlFieldName("Name' OR '1'='1")).toThrow("Invalid SOQL field name: Name' OR '1'='1");
                });
                it('should reject field name with semicolon', () => {
                    expect(() => validateSoqlFieldName('Name;DROP TABLE')).toThrow('Invalid SOQL field name: Name;DROP TABLE');
                });
                it('should reject field name with parentheses', () => {
                    expect(() => validateSoqlFieldName('COUNT(Id)')).toThrow('Invalid SOQL field name: COUNT(Id)');
                });
                it('should reject field name with SQL keywords injection', () => {
                    expect(() => validateSoqlFieldName('Name WHERE 1=1')).toThrow('Invalid SOQL field name: Name WHERE 1=1');
                });
                it('should reject field name starting with number', () => {
                    expect(() => validateSoqlFieldName('123Field')).toThrow('Invalid SOQL field name: 123Field');
                });
            ...
            describe('validateSoqlOperator', () => {
                ...
                it('should reject SQL injection attempt', () => {
                    expect(() => validateSoqlOperator("= '1' OR '1'='1")).toThrow("Invalid SOQL operator: = '1' OR '1'='1");
                });
                it('should reject operator with semicolon', () => {
                    expect(() => validateSoqlOperator('=;DROP TABLE')).toThrow('Invalid SOQL operator: =;DROP TABLE');
                });
            ...
            describe('validateSoqlObjectName', () => {
                ...
                it('should reject object name with spaces', () => {
                    expect(() => validateSoqlObjectName('Invalid Object')).toThrow('Invalid SOQL object name: Invalid Object');
                });
                it('should reject object name with special characters', () => {
                    expect(() => validateSoqlObjectName("Account' OR '1'='1")).toThrow("Invalid SOQL object name: Account' OR '1'='1");
                });
                it('should reject object name with semicolon', () => {
                    expect(() => validateSoqlObjectName('Account;DROP TABLE')).toThrow('Invalid SOQL object name: Account;DROP TABLE');
                });
                it('should reject object name with parentheses', () => {
                    expect(() => validateSoqlObjectName('Account()')).toThrow('Invalid SOQL object name: Account()');
                });
                it('should reject object name with SQL keywords', () => {
                    expect(() => validateSoqlObjectName('Account WHERE 1=1')).toThrow('Invalid SOQL object name: Account WHERE 1=1');
                });
                it('should reject object name starting with number', () => {
                    expect(() => validateSoqlObjectName('123Object')).toThrow('Invalid SOQL object name: 123Object');
                });
                ...
                it('should reject object name with dots', () => {
                    expect(() => validateSoqlObjectName('Account.Name')).toThrow('Invalid SOQL object name: Account.Name');
                });
            });
            describe('getValue', () => {
                ...
                it('should escape single quotes in numeric string values', () => {
                    ...
                    const result = getValue(maliciousValue);
                    expect(result).toBe("'Bob\\' OR \\'1\\'=\\'1'");
                });
                ...
                it('should throw error for non-finite numbers', () => {
                    expect(() => getValue(Infinity)).toThrow('Invalid numeric value: must be a finite number');
                    expect(() => getValue(-Infinity)).toThrow('Invalid numeric value: must be a finite number');
                ...
                it('should prevent SOQL injection in array values', () => {
                    ...
                    const result = getValue(maliciousArray);
            ...
            describe('getQuery', () => {
                it('should validate object name', () => {
                    ...
                    expect(() => getQuery(options, "Account' OR '1'='1", true)).toThrow("Invalid SOQL object name: Account' OR '1'='1");
                });
                it('should validate field names in comma-separated string', () => {
                    const options = {
                        fields: "Id,Name' OR '1'='1",
                    };
                    expect(() => getQuery(options, 'Account', true)).toThrow("Invalid SOQL field name: Name' OR '1'='1");
                });
