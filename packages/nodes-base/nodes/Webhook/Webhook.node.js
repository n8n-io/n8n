...
import { authenticationProperty, credentialsProperty, defaultWebhookDescription, httpMethodsProperty, inboundTriggerAuthenticationBuilderHint, optionsProperty, requireExecuteAccessProperty, responseBinaryPropertyNameProperty, responseCodeOption, responseCodeProperty, responseDataProperty, responseModeProperty, responseModePropertyStreaming, } from './description';
import { WebhookAuthorizationError } from './error';
import { checkResponseModeConfiguration, configuredOutputs, handleFormData, isIpAllowed, setupOutputConnection, validateWebhookAuthentication, } from './utils';
export class Webhook extends Node {
    ...
    description = {
        ...
        properties: [
            ...
            {
                ...optionsProperty,
                options: [...optionsProperty.options, responseCodeOption].sort((a, b) => {
    ...
    async webhook(context) {
        ...
        const rawOptions = node.parameters?.options;
        ...
        if (typeof rawOnlyRunIf === 'string' && rawOnlyRunIf.startsWith('=')) {
            try {
                ...
                context.logger.warn(`Webhook "Only Run If" expression failed to evaluate; allowing request through. ${error.message}`, { nodeName: node.name });
            }
        }
