...
import { REDACTED, redactedHeaders, UnexpectedError, } from 'n8n-workflow';
import { checkResponseModeConfiguration, configuredOutputs, generateBasicAuthToken, generateFormPostBasicAuthToken, getResponseCode, getResponseData, handleFormData, isIpAllowed, setupOutputConnection, validateWebhookAuthentication, } from '../utils';
...
describe('Webhook Utils', () => {
    ...
    describe('setupOutputConnection', () => {
        it('should return a function that sets the webhookUrl and executionMode in the output data', () => {
            ...
            const setupOutput = setupOutputConnection(ctx, method, additionalData);
    ...
    describe('isIpAllowed', () => {
        ...
        it('should return true if any ip in ips matches any address in the allowlist array', () => {
            expect(isIpAllowed(['192.168.1.1', '192.168.1.2'], ['192.168.1.2', '192.168.1.3'])).toBe(true);
        });
        ...
        it('should return false if ip and ips do not match any address in the allowlist array', () => {
            expect(isIpAllowed(['192.168.1.4', '192.168.1.5'], ['192.168.1.1', '192.168.1.2'], '192.168.1.3')).toBe(false);
        });
    ...
    describe('checkResponseModeConfiguration', () => {
        ...
        it('should throw if any Respond to Webhook node is not owned by a downstream Wait node', () => {
            ...
            const context = {
                ...
                getParentNodes: vi.fn().mockImplementation((nodeName) => {
    ...
    describe('validateWebhookAuthentication', () => {
        it('should return early if authentication is "none"', async () => {
            ...
            const result = await validateWebhookAuthentication(ctx, authPropertyName);
        ...
        it('should throw an error if basicAuth is enabled but no authentication data is defined on the node', async () => {
            ...
            const authPropertyName = 'authentication';
            await expect(validateWebhookAuthentication(ctx, authPropertyName)).rejects.toThrowError('No authentication data defined on node!');
        });
        it('should throw an error if basicAuth is enabled but the provided authentication data is wrong', async () => {
            ...
            const authPropertyName = 'authentication';
            await expect(validateWebhookAuthentication(ctx, authPropertyName)).rejects.toThrowError('Authorization is required!');
        });
        it('should return unauthorized if basicAuth credentials do not match', async () => {
            ...
            const authPropertyName = 'authentication';
            await expect(validateWebhookAuthentication(ctx, authPropertyName)).rejects.toMatchObject({
                responseCode: 401,
        ...
        it('should successfully pass if basicAuth is enabled and provided basic auth data is correct', async () => {
            ...
            const authPropertyName = 'authentication';
            await validateWebhookAuthentication(ctx, authPropertyName);
        });
        ...
        it('should throw an error if headerAuth is enabled but the provided authentication data is wrong', async () => {
            ...
            const authPropertyName = 'authentication';
            await expect(validateWebhookAuthentication(ctx, authPropertyName)).rejects.toThrowError('Authorization data is wrong!');
        });
        ...
        it('should throw an error if jwtAuth is enabled but no token is provided', async () => {
            ...
            const authPropertyName = 'authentication';
            await expect(validateWebhookAuthentication(ctx, authPropertyName)).rejects.toThrowError('No token provided');
        });
        it('should throw an error if jwtAuth is enabled but the provided token is invalid', async () => {
            ...
            const authPropertyName = 'authentication';
            await expect(validateWebhookAuthentication(ctx, authPropertyName)).rejects.toThrowError('jwt malformed');
        });
        ...
        describe('records the headers it authenticated with', () => {
            const setup = (authentication, headers, credentials, node) => {
            ...
            it('leaves the request alone', async () => {
                ...
                const { ctx, request } = setup('basicAuth', { authorization, 'x-tenant-id': 'acme' }, { user: 'admin', password: 'password' });
            ...
            it('records the basic auth header, keeping the rest', async () => {
                const { ctx, request } = setup('basicAuth', {
                    authorization: `Basic ${Buffer.from('admin:password').toString('base64')}`,
                    'x-tenant-id': 'acme',
            ...
            it('records the form auth token accepted in place of basic auth', async () => {
                const node = { id: 'node-789', webhookId: 'webhook-456' };
                const credentials = { user: 'admin', password: 'password' };
                const { ctx, request } = setup('basicAuth', { 'x-auth-token': generateBasicAuthToken(node, credentials) }, credentials, node);
                await validateWebhookAuthentication(ctx, 'authentication');
                expect(redactedHeaders(request)).toEqual({ 'x-auth-token': REDACTED });
            });
            it('records the bearer token header', async () => {
                const { ctx, request } = setup('bearerAuth', { authorization: 'Bearer secret-token', accept: 'application/json' }, { token: 'secret-token' });
                await validateWebhookAuthentication(ctx, 'authentication');
                expect(redactedHeaders(request)).toEqual({
                    authorization: REDACTED,
            ...
            it('records the custom header the credential names', async () => {
                const { ctx, request } = setup('headerAuth', { test: 'secret-value', 'x-tenant-id': 'acme' }, { name: 'test', value: 'secret-value' });
                await validateWebhookAuthentication(ctx, 'authentication');
            ...
            it('records the JWT header while still returning its payload', async () => {
                ...
                jwt.verify.mockReturnValue(decodedPayload);
                const { ctx, request } = setup('jwtAuth', { authorization: 'Bearer some.jwt.token' }, { keyType: 'passphrase', publicKey: '', secret: 'secret', algorithm: 'HS256' });
    ...
    describe('handleFormData', () => {
        ...
        const mockPrepareOutput = vi.fn().mockImplementation((data) => [[data]]);
        beforeEach(() => {
            vi.clearAllMocks();
        });
