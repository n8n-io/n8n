...
const nodeTypes = {
    getByName: () => webhookNode,
    getByNameAndVersion: () => webhookNode,
    getKnownTypes: () => ({}),
};
const nodeWithParameters = (parameters) => {
    ...
    describe('resolvers match their templates', () => {
        ...
        describe.each(parameterSets)('$case', ({ parameters }) => {
            ...
            test.each(Object.keys(resolvers))('%s', (field) => {
                ...
                const viaEngine = workflow.expression.getSimpleParameterValue(node, defaultWebhookDescription[field], 'internal', {});
