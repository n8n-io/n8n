...
describe('Test Webhook Node', () => {
    ...
    describe('description', () => {
        it('should tell builders to keep inbound authentication disabled unless requested', () => {
            ...
            const authParam = node.description.properties.find((property) => property.name === 'authentication');
        ...
        it('exposes the requireExecuteAccess toggle, on by default and scoped to the n8nOAuth2 mode', () => {
            const node = new Webhook();
            const requireExecuteParam = node.description.properties.find((property) => property.name === 'requireExecuteAccess');
    ...
    describe('streaming response mode', () => {
        ...
        beforeEach(() => {
            ...
            context.getNodeParameter.mockImplementation((paramName) => {
                if (paramName === 'options')
                    return {};
                if (paramName === 'responseMode')
                    return 'streaming';
                return undefined;
            });
            req.headers = {};
    ...
    describe('n8n User Auth (OAuth2) authentication', () => {
        ...
        beforeEach(() => {
            vi.clearAllMocks();
            context = mock({ nodeHelpers: mock(), logger: mock() });
            req = mock();
            res = mock();
            context.getRequestObject.mockReturnValue(req);
            context.getResponseObject.mockReturnValue(res);
        ...
        it('rejects an invalid token with 401 and does not establish identity', async () => {
            ...
            const result = await node.webhook(context);
            expect(context.validateN8nOAuth2Token).toHaveBeenCalledWith('bad-token', `${WEBHOOK_URL}?method=GET`);
        ...
        it('establishes the trigger identity for a valid token and runs the workflow', async () => {
            ...
            const result = await node.webhook(context);
            expect(context.validateN8nOAuth2Token).toHaveBeenCalledWith('good-token', `${WEBHOOK_URL}?method=GET`);
            expect(context.establishTriggerIdentity).toHaveBeenCalledWith('good-token', `${WEBHOOK_URL}?method=GET`);
    ...
    describe('onlyRunIf filter', () => {
        ...
        let req;
        let res;
        const setup = (storedOptions, runtimeOptions = storedOptions) => {
            context = mock({ nodeHelpers: mock(), logger: mock() });
            req = mock();
            res = mock();
            context.getRequestObject.mockReturnValue(req);
        ...
        it('runs the workflow when the expression evaluates truthy', async () => {
            ...
            const result = await node.webhook(context);
            expect(context.evaluateExpression).toHaveBeenCalledWith("{{ $json.body.campaign_id === 'user-research-invite' }}", 0);
            expect(result.workflowData).toBeDefined();
        });
        ...
        it('allows the request through and logs a warning when the expression throws', async () => {
            ...
            const result = await node.webhook(context);
            expect(result.workflowData).toBeDefined();
            expect(context.logger.warn).toHaveBeenCalledWith(expect.stringContaining('Only Run If'), expect.objectContaining({ nodeName: 'Webhook' }));
        });
