...
export const getResponseCode = (parameters) => {
...
export const getResponseData = (parameters) => {
...
export const configuredOutputs = (parameters) => {
...
export const setupOutputConnection = (ctx, method, additionalData) => {
...
export const isIpAllowed = (allowlist, ips, ip) => {
...
export const checkResponseModeConfiguration = (context) => {
    ...
    if (respondToWebhookNodes.length === 0 && responseMode === 'responseNode') {
        throw new WorkflowConfigurationError(context.getNode(), new Error('No Respond to Webhook node found in the workflow'), {
    ...
    if (respondToWebhookNodes.length > 0 && !['responseNode', 'streaming'].includes(responseMode)) {
        ...
        if (hasUnusedRespondNode) {
            throw new WorkflowConfigurationError(context.getNode(), new Error('Unused Respond to Webhook node found in the workflow'), {
                description: 'Set the “Respond” parameter to “Using Respond to Webhook Node” or remove the Respond to Webhook node',
            });
        }
    }
};
export async function validateWebhookAuthentication(ctx, authPropertyName) {
...
export async function handleFormData(context, prepareOutput) {
...
export async function generateFormPostBasicAuthToken(context, authPropertyName) {
...
export function generateBasicAuthToken(node, credentials) {
