import { getAgentDetails, pollAgentStatus, throwOperationErrorIf, validateAgentParameters, } from './agent.utils';
...
export const description = [
    ...
    {
        ...profileNameField,
        description: "The Airtop browser profile ID the agent should use for this run. Leave empty to use the agent's default profile.",
...
export async function execute(index) {
    ...
    // Validate timeout
    throwOperationErrorIf(timeout < AGENT_MIN_TIMEOUT_SECONDS, ERROR_MESSAGES.AGENT_TIMEOUT_INVALID, airtopNode);
    ...
    const query = profileName ? { profileId: profileName } : {};
    const invocationResponse = await apiRequest.call(this, 'POST', invokeUrl, validatedAgentParameters, query);
