...
export async function execute(index, timeout = OPERATION_TIMEOUT) {
    const { sessionId, windowId } = validateSessionAndWindowId.call(this, index);
    const formData = validateRequiredStringField.call(this, index, 'formData', 'Form Data');
    // run automation
    const asyncAutomationResponse = await apiRequest.call(this, 'POST', `/async/sessions/${sessionId}/windows/${windowId}/execute-automation`, {
        automationId: 'auto',
        parameters: {
            customData: formData,
        },
