...
vi.mock('../GenericFunctions', () => ({
    shouldCreateNewSession: vi.fn(function (index) {
...
describe('executeRequestWithSessionManagement', () => {
    ...
    describe("When 'sessionMode' is 'new'", () => {
        it("should create a new session and window when 'sessionMode' is 'new'", async () => {
            ...
            const result = await executeRequestWithSessionManagement.call(createMockExecuteFunction(nodeParameters), 0, {
                method: 'POST',
        ...
        it("should not terminate session when 'autoTerminateSession' is false", async () => {
            ...
            expect(transport.apiRequest).not.toHaveBeenCalledWith('DELETE', '/sessions/existing-session-123');
            expect(result).toEqual({
                sessionId: 'new-session-123',
                windowId: 'new-window-123',
                success: true,
        ...
        it("should terminate session when 'autoTerminateSession' is true", async () => {
            ...
            expect(transport.apiRequest).toHaveBeenNthCalledWith(2, 'DELETE', '/sessions/new-session-123');
        });
        it("should call the operation passed in the 'request' parameter", async () => {
            ...
            expect(transport.apiRequest).toHaveBeenNthCalledWith(1, 'POST', '/sessions/new-session-123/windows/new-window-123/action', {
                operation: 'test-operation',
            });
        });
    });
    describe("When 'sessionMode' is 'existing'", () => {
        it('should not create a new session and window', async () => {
            ...
            expect(transport.apiRequest).toHaveBeenCalledWith('POST', '/sessions/existing-session-123/windows/existing-window-123/action', {});
        });
        it("should call the operation passed in the 'request' parameter", async () => {
            const nodeParameters = {
                sessionMode: SESSION_MODE.EXISTING,
            ...
            expect(transport.apiRequest).toHaveBeenNthCalledWith(1, 'POST', '/sessions/existing-session-123/windows/existing-window-123/action', {
                operation: 'test-operation',
            });
        });
    });
});
//# sourceMappingURL=session-utils.test.js.map