...
describe('Test Airtop, fill form operation', () => {
    ...
    it('should execute fill operation successfully', async () => {
        ...
        expect(apiRequestMock).toHaveBeenCalledWith('POST', '/async/sessions/test-session-123/windows/win-123/execute-automation', {
            automationId: 'auto',
            parameters: {
    ...
    it("should throw error when 'formData' parameter is empty", async () => {
        ...
        await expect(fill.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.REQUIRED_PARAMETER.replace('{{field}}', 'Form Data'));
    });
    it('should throw error when operation times out after 2 sec', async () => {
        ...
        await expect(fill.execute.call(createMockExecuteFunction(nodeParameters), 0, timeout)).rejects.toThrow('The service was not able to process your request');
    });
