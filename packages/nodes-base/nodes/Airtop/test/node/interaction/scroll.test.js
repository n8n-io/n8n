...
describe('Test Airtop, scroll operation', () => {
    ...
    it('should execute automatic scroll operation successfully', async () => {
        ...
        const result = await scroll.execute.call(createMockExecuteFunction(baseAutomaticNodeParameters), 0);
        expect(apiRequestMock).toHaveBeenCalledWith('POST', '/sessions/test-session-123/windows/win-123/scroll', {
            scrollToElement: 'the bottom of the page',
            configuration: {},
        });
    ...
    it('should execute manual scroll operation successfully', async () => {
        const apiRequestMock = transport.apiRequest;
        apiRequestMock.mockResolvedValueOnce(mockResponse);
        const result = await scroll.execute.call(createMockExecuteFunction(baseManualNodeParameters), 0);
        expect(apiRequestMock).toHaveBeenCalledWith('POST', '/sessions/test-session-123/windows/win-123/scroll', {
            configuration: {},
            scrollToEdge: {
                yAxis: 'bottom',
    ...
    it("should throw error when scrollingMode is 'automatic' and 'scrollToElement' parameter is empty", async () => {
        ...
        await expect(scroll.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.REQUIRED_PARAMETER.replace('{{field}}', 'Element Description'));
    });
    it("should validate scroll amount formats when scrollingMode is 'manual'", async () => {
        ...
        await expect(scroll.execute.call(createMockExecuteFunction(invalidNodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.SCROLL_BY_AMOUNT_INVALID);
    });
    it('should throw an error when the API returns an error response', async () => {
        ...
        await expect(scroll.execute.call(createMockExecuteFunction(baseAutomaticNodeParameters), 0)).rejects.toThrow('Failed to scroll');
    });
});
//# sourceMappingURL=scroll.test.js.map