...
describe('Test Airtop, click operation', () => {
    ...
    it('should execute click with minimal parameters', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenCalledWith('POST', '/sessions/test-session-123/windows/win-123/click', {
            elementDescription: 'the login button',
            configuration: {
                clickType: 'click',
            },
    ...
    it("should throw error when 'elementDescription' parameter is empty", async () => {
        ...
        await expect(click.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(errorMessage);
    });
