...
describe('Test Airtop, hover operation', () => {
    ...
    it('should execute hover with minimal parameters', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenCalledWith('POST', '/sessions/test-session-123/windows/win-123/hover', {
            configuration: {},
            elementDescription: 'the user profile image',
        });
    ...
    it("should throw error when 'elementDescription' parameter is empty", async () => {
        ...
        const errorMessage = ERROR_MESSAGES.REQUIRED_PARAMETER.replace('{{field}}', 'Element Description');
        await expect(hover.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(errorMessage);
    });
