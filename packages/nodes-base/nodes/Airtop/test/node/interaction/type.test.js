...
describe('Test Airtop, type operation', () => {
    ...
    it('should execute type with minimal parameters', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenCalledWith('POST', '/sessions/test-session-123/windows/win-123/type', {
            configuration: {},
            text: 'Hello World',
            pressEnterKey: false,
            elementDescription: '',
    ...
    it("should throw error when 'text' parameter is empty", async () => {
        ...
        await expect(type.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.REQUIRED_PARAMETER.replace('{{field}}', 'Text'));
    });
