...
const createMockLoadOptionsFunction = (nodeParameters = {}) => {
    return {
...
describe('Test Airtop, agent run operation', () => {
    ...
    it('should list available agents', async () => {
        const apiRequestMock = transport.apiRequest;
        ...
        expect(apiRequestMock).toHaveBeenCalledWith('GET', AGENTS_ENDPOINT, {}, { createdByMe: true, limit: 50, enabled: true, published: true, name: '' });
    ...
    it('should validate required agent parameters', async () => {
        ...
        await expect(run.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow('Missing required parameters: url');
    });
    it('should return invocationId without waiting for agent completion', async () => {
        ...
        expect(apiRequestMock).toHaveBeenNthCalledWith(2, 'POST', `${AGENTS_HOOKS_ENDPOINT}/test-agent-123/webhooks/test-webhook`, { configVars: {} }, {});
        expect(result).toEqual([
            {
                json: {
                    invocationId: 'invocation-123',
    ...
    it('should wait for agent until response contains an output', async () => {
        ...
        expect(apiRequestMock).toHaveBeenNthCalledWith(3, 'GET', `${AGENTS_HOOKS_ENDPOINT}/test-agent-123/invocations/invocation-123/result`);
        expect(apiRequestMock).toHaveBeenNthCalledWith(4, 'GET', `${AGENTS_HOOKS_ENDPOINT}/test-agent-123/invocations/invocation-123/result`);
        expect(result).toEqual([
            {
                json: {
                    invocationId: 'invocation-123',
    ...
    it('should throw an error if timeout is less than 10 seconds', async () => {
        ...
        await expect(run.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.AGENT_TIMEOUT_INVALID);
    });
    ...
    it('should filter agents by name when search filter is provided', async () => {
        ...
        expect(apiRequestMock).toHaveBeenCalledWith('GET', AGENTS_ENDPOINT, {}, { createdByMe: true, limit: 50, enabled: true, published: true, name: 'Test' });
    });
    it('should wrap agent parameters in configVars when executing', async () => {
        ...
        expect(apiRequestMock).toHaveBeenNthCalledWith(2, 'POST', `${AGENTS_HOOKS_ENDPOINT}/test-agent-123/webhooks/test-webhook`, {
            configVars: {
                url: 'https://example.com',
                maxResults: 10,
            },
    ...
    it('should send the selected browser profile as a profileId query param', async () => {
        ...
        await run.execute.call(createMockExecuteFunction(nodeParameters), 0);
        expect(apiRequestMock).toHaveBeenNthCalledWith(2, 'POST', `${AGENTS_HOOKS_ENDPOINT}/test-agent-123/webhooks/test-webhook`, { configVars: {} }, { profileId: 'my-profile' });
    });
    ...
    it('should accept a UUID-style browser profile id', async () => {
        ...
        await run.execute.call(createMockExecuteFunction(nodeParameters), 0);
        expect(apiRequestMock).toHaveBeenNthCalledWith(2, 'POST', `${AGENTS_HOOKS_ENDPOINT}/test-agent-123/webhooks/test-webhook`, { configVars: {} }, { profileId: 'a13c6f73-bd89-4a76-ab32-5a6c422e8224' });
    });
    it('should throw an error referencing the item when the browser profile contains invalid characters', async () => {
        ...
        await expect(run.execute.call(createMockExecuteFunction(nodeParameters), 1)).rejects.toMatchObject({
