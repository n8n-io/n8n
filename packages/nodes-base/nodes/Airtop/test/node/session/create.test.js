...
describe('Test Airtop, session create operation', () => {
    ...
    it('should create a session with save profile enabled', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenNthCalledWith(2, 'PUT', '/sessions/test-session-123/save-profile-on-termination/test-profile');
        expect(result).toEqual([
            {
                json: {
                    sessionId: 'test-session-123',
    ...
    it('should throw error when custom proxy URL is empty', async () => {
        ...
        await expect(create.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.PROXY_URL_REQUIRED);
    });
    /**
     * Auto solve captcha
     */
