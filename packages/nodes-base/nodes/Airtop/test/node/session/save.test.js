...
describe('Test Airtop, session save operation', () => {
    ...
    it('should save a profile on session termination successfully', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenCalledWith('PUT', '/sessions/test-session-123/save-profile-on-termination/test-profile');
    ...
    it('should throw error when sessionId is empty', async () => {
        ...
        await expect(save.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.SESSION_ID_REQUIRED);
    });
    ...
    it('should throw error when profileName is empty', async () => {
        ...
        await expect(save.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow("Please fill the 'Browser Profile ID' parameter");
    });
