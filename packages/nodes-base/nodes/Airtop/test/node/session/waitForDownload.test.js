...
describe('Test Airtop, session waitForDownload operation', () => {
    ...
    it('should wait for download successfully', async () => {
        ...
        GenericFunctions.waitForSessionEvent.mockResolvedValue(mockEvent);
        ...
        expect(GenericFunctions.waitForSessionEvent).toHaveBeenCalledWith('test-session-123', expect.any(Function), 1);
    ...
    it('should throw error when sessionId is empty', async () => {
        ...
        await expect(waitForDownload.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.SESSION_ID_REQUIRED);
    });
...
//# sourceMappingURL=waitForDownload.test.js.map