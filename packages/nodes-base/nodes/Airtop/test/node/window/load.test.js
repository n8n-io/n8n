...
describe('Test Airtop, window load operation', () => {
    ...
    it('should load URL with minimal parameters', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenCalledWith('POST', '/sessions/test-session-123/windows/win-123', {
            url: 'https://example.com',
        });
    ...
    it('should throw error when URL is empty', async () => {
        ...
        await expect(load.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(errorMessage);
    });
    it('should throw error when URL is invalid', async () => {
        ...
        await expect(load.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.URL_INVALID);
    });
