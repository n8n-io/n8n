...
describe('Test Airtop, window close operation', () => {
    ...
    it('should close a window successfully', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenCalledWith('DELETE', '/sessions/test-session-123/windows/win-123');
    ...
    it('should throw error when sessionId is empty', async () => {
        ...
        await expect(close.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.SESSION_ID_REQUIRED);
    });
    it('should throw error when windowId is empty', async () => {
        ...
        await expect(close.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.WINDOW_ID_REQUIRED);
    });
});
