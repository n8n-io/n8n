...
describe('Test Airtop, take screenshot operation', () => {
    ...
    it('should take screenshot in base64 format', async () => {
        const result = await takeScreenshot.execute.call(createMockExecuteFunction({ ...baseNodeParameters }), 0);
        expect(transport.apiRequest).toHaveBeenCalledTimes(1);
        expect(transport.apiRequest).toHaveBeenCalledWith('POST', '/sessions/test-session-123/windows/win-123/screenshot');
        expect(result).toEqual([{ ...expectedJsonResult }]);
    });
    it('should take screenshot in binary format', async () => {
        const result = await takeScreenshot.execute.call(createMockExecuteFunction({
            ...baseNodeParameters,
            outputImageAsBinary: true,
        }), 0);
        expect(transport.apiRequest).toHaveBeenCalledTimes(1);
        expect(transport.apiRequest).toHaveBeenCalledWith('POST', '/sessions/test-session-123/windows/win-123/screenshot');
        expect(GenericFunctions.convertScreenshotToBinary).toHaveBeenCalledWith(mockResponse.meta.screenshots[0]);
    ...
    it('should throw error when sessionId is empty', async () => {
        ...
        await expect(takeScreenshot.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.SESSION_ID_REQUIRED);
    });
    it('should throw error when windowId is empty', async () => {
        ...
        await expect(takeScreenshot.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.WINDOW_ID_REQUIRED);
    });
});
//# sourceMappingURL=takeScreenshot.test.js.map