...
describe('Test Airtop, window create operation', () => {
    ...
    it('should create a window with minimal parameters', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenCalledWith('POST', '/sessions/test-session-123/windows', {
            url: 'https://example.com',
        });
    ...
    it('should create a window with live view', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenNthCalledWith(1, 'POST', '/sessions/test-session-123/windows', {
            url: 'https://example.com',
        });
        expect(transport.apiRequest).toHaveBeenNthCalledWith(2, 'GET', '/sessions/test-session-123/windows/win-123', undefined, {});
        expect(result).toEqual([
            {
                json: {
                    sessionId: baseNodeParameters.sessionId,
    ...
    it('should create a window with live view and disabled resize', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenNthCalledWith(2, 'GET', '/sessions/test-session-123/windows/win-123', undefined, { disableResize: true });
        expect(result).toEqual([
            {
                json: {
                    sessionId: baseNodeParameters.sessionId,
    ...
    it('should create a window with live view and navigation bar', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenNthCalledWith(2, 'GET', '/sessions/test-session-123/windows/win-123', undefined, { includeNavigationBar: true });
        expect(result).toEqual([
            {
                json: {
                    sessionId: baseNodeParameters.sessionId,
    ...
    it('should create a window with live view and screen resolution', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenNthCalledWith(2, 'GET', '/sessions/test-session-123/windows/win-123', undefined, { screenResolution: '1280x720' });
        expect(result).toEqual([
            {
                json: {
                    sessionId: baseNodeParameters.sessionId,
    ...
    it("should throw error when 'sessionId' parameter is empty", async () => {
        ...
        await expect(create.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.SESSION_ID_REQUIRED);
    });
    it('should throw error when screen resolution format is invalid', async () => {
        ...
        await expect(create.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.SCREEN_RESOLUTION_INVALID);
    });
});
