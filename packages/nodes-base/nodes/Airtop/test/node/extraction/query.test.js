...
const mockJsonSchema = '{"type":"object","properties":{"productCount":{"type":"number"},"priceRange":{"type":"object"}}}';
...
describe('Test Airtop, query page operation', () => {
    ...
    it('should query the page with minimal parameters using existing session', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenCalledWith('POST', '/sessions/test-session-123/windows/win-123/page-query', {
            prompt: 'How many products are on the page and what is their price range?',
            configuration: {
                experimental: {
    ...
    it('should query the page using a new session', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenNthCalledWith(1, 'POST', '/sessions/new-session-456/windows/new-win-456/page-query', {
            prompt: 'How many products are on the page and what is their price range?',
            configuration: {
                experimental: {
                    includeVisualAnalysis: 'disabled',
    ...
    it("should throw error when 'sessionId' is empty in 'existing' session mode", async () => {
        ...
        await expect(query.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.SESSION_ID_REQUIRED);
    });
    it("should throw error when 'windowId' is empty in 'existing' session mode", async () => {
        ...
        await expect(query.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.WINDOW_ID_REQUIRED);
    });
