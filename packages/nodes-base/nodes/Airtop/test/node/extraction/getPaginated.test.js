...
const mockResponse = {
    data: {
        modelResponse: '{"items": [{"title": "Item 1", "price": "$10.99"}, {"title": "Item 2", "price": "$20.99"}]}',
    },
};
const mockJsonSchema = '{"type":"object","properties":{"title":{"type":"string"},"price":{"type":"string"}}}';
vi.mock('../../../transport', async () => {
    const originalModule = await vi.importActual('../../../transport');
    return {
        ...originalModule,
        apiRequest: vi.fn(async (method, endpoint) => {
...
vi.mock('../../../GenericFunctions', async () => {
    ...
    return {
        ...
        shouldCreateNewSession: vi.fn().mockImplementation(function (index) {
...
describe('Test Airtop, getPaginated operation', () => {
    ...
    it('should extract data with minimal parameters', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenCalledWith('POST', '/sessions/test-session-123/windows/win-123/paginated-extraction', {
            prompt: 'Extract all product titles and prices',
            configuration: {},
        });
    ...
    it('should extract data using a new session', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenNthCalledWith(1, 'POST', '/sessions/new-session-123/windows/new-window-123/paginated-extraction', {
            prompt: 'Extract all product titles and prices',
            configuration: {},
        });
        expect(result).toEqual([
    ...
    it("should throw error when 'sessionId' is empty and session mode is 'existing'", async () => {
        ...
        await expect(getPaginated.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.SESSION_ID_REQUIRED);
    });
    it("should throw error when 'windowId' is empty and session mode is 'existing'", async () => {
        ...
        await expect(getPaginated.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.WINDOW_ID_REQUIRED);
    });
});
//# sourceMappingURL=getPaginated.test.js.map