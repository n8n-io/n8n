...
vi.mock('../../../transport', async () => {
    ...
    return {
        ...originalModule,
        apiRequest: vi.fn(async function (method, endpoint) {
...
describe('Test Airtop, scrape operation', () => {
    ...
    it('should scrape content with minimal parameters using existing session', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenCalledWith('POST', '/sessions/test-session-123/windows/win-123/scrape-content', {});
    ...
    it('should scrape content using a new session', async () => {
        ...
        expect(transport.apiRequest).toHaveBeenNthCalledWith(1, 'POST', '/sessions/new-session-456/windows/new-win-456/scrape-content', {});
        expect(result).toEqual([
            {
                json: {
                    data: mockResponse.data,
    ...
    it("should throw error when sessionId is empty in 'existing' session mode", async () => {
        ...
        await expect(scrape.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.SESSION_ID_REQUIRED);
    });
    it("should throw error when windowId is empty in 'existing' session mode", async () => {
        ...
        await expect(scrape.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.WINDOW_ID_REQUIRED);
    });
});
