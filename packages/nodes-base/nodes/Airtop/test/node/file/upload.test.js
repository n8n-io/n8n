...
describe('Test Airtop, upload file operation', () => {
    ...
    it('should upload a file from a URL and trigger the file input successfully', async () => {
        ...
        apiRequestMock
            ...
            .mockResolvedValueOnce({});
        GenericFunctions.waitForSessionEvent.mockResolvedValueOnce(mockAvailableEvent);
        ...
        expect(apiRequestMock).toHaveBeenNthCalledWith(4, 'POST', '/sessions/test-session-123/windows/window-123/file-input', {
            fileId: 'file-123',
            elementDescription: 'test desc',
            includeHiddenElements: false,
        });
    ...
    it('should not trigger the file input when triggerFileInputParameter is false', async () => {
        ...
        expect(apiRequestMock).not.toHaveBeenCalledWith('POST', '/sessions/test-session-123/windows/window-123/file-input', expect.anything());
        expect(result).toEqual([
            {
                json: {
                    sessionId: 'test-session-123',
    ...
    it('should throw error when sessionId is empty', async () => {
        ...
        await expect(upload.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.REQUIRED_PARAMETER.replace('{{field}}', 'Session ID'));
    });
    it('should throw error when windowId is empty', async () => {
        ...
        await expect(upload.execute.call(createMockExecuteFunction(nodeParameters), 0)).rejects.toThrow(ERROR_MESSAGES.REQUIRED_PARAMETER.replace('{{field}}', 'Window ID'));
    });
});
