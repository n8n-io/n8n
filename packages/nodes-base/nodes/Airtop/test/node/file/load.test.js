...
describe('Test Airtop, load file operation', () => {
    ...
    it('should load file into session successfully', async () => {
        ...
        expect(apiRequestMock).toHaveBeenNthCalledWith(2, 'POST', '/sessions/test-session-123/windows/test-window-123/file-input', {
            fileId: 'file-123',
            elementDescription: 'the file upload box',
            includeHiddenElements: true,
        });
        expect(GenericFunctions.waitForSessionEvent).toHaveBeenCalledWith('test-session-123', expect.any(Function), expect.any(Number));
    ...
    it('should throw a NodeOperationError when the upload fails', async () => {
        const apiRequestMock = transport.apiRequest;
        apiRequestMock.mockResolvedValue({});
        GenericFunctions.waitForSessionEvent.mockRejectedValueOnce(new Error('Upload failed for File ID: file-123'));
        await expect(load.execute.call(createMockExecuteFunction(baseNodeParameters), 0)).rejects.toThrow('Upload failed for File ID: file-123');
