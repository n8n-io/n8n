...
describe('Test Airtop file helpers', () => {
    afterEach(() => {
        ...
        GenericFunctions.waitForSessionEvent.mockReset();
    });
    describe('requestAllFiles', () => {
        it('should request all files with pagination', async () => {
            ...
            const result = await helpers.requestAllFiles.call(createMockExecuteFunction({}), 'session-123');
            expect(apiRequestMock).toHaveBeenCalledTimes(2);
            expect(apiRequestMock).toHaveBeenNthCalledWith(1, 'GET', '/files', {}, { offset: 0, limit: 100, sessionIds: 'session-123' });
            expect(apiRequestMock).toHaveBeenNthCalledWith(2, 'GET', '/files', {}, { offset: 100, limit: 100, sessionIds: 'session-123' });
    ...
    describe('pollFileUntilAvailable', () => {
        it('should poll until file is available', async () => {
            ...
            const pollPromise = helpers.pollFileUntilAvailable.call(createMockExecuteFunction({}), 'file-123', 1000, 0);
        ...
        it('should throw timeout error if file never becomes available', async () => {
            ...
            const promise = helpers.pollFileUntilAvailable.call(createMockExecuteFunction({}), 'file-123', 0);
    ...
    describe('createAndUploadFile', () => {
        it('should create file entry, upload file, and poll until available', async () => {
            ...
            const result = await helpers.createAndUploadFile.call(mockExecuteFunction, 'test.png', Buffer.from('test'), 'customer_upload', pollingFunctionMock);
        ...
        it('should throw error if file creation response is missing id or upload URL', async () => {
            ...
            await expect(helpers.createAndUploadFile.call(createMockExecuteFunction({}), 'test.pdf', Buffer.from('test'), 'customer_upload')).rejects.toThrow();
        });
    });
    describe('waitForFileInSession', () => {
        it('should resolve when file_upload_status event with available status is received', async () => {
            const waitForSessionEventMock = GenericFunctions.waitForSessionEvent;
            const mockEvent = {
            ...
            expect(waitForSessionEventMock).toHaveBeenCalledWith('session-123', expect.any(Function), 1000);
        });
        it('should throw error when uploading a file with invalid file format', async () => {
            ...
            waitForSessionEventMock.mockResolvedValueOnce(mockEvent);
            const mockExecuteFunction = createMockExecuteFunction({});
            await expect(helpers.waitForFileInSession.call(mockExecuteFunction, 'session-123', 'file-123', 1000)).rejects.toMatchObject({ description: 'Upload failed due to invalid file format' });
        });
        it('should throw error when upload_failed status is received', async () => {
            ...
            await expect(helpers.waitForFileInSession.call(mockExecuteFunction, 'session-123', 'file-123', 1000)).rejects.toMatchObject({ description: 'Upload failed for File ID: file-123' });
        });
        it('should timeout if no matching event is received', async () => {
            ...
            const mockExecuteFunction = createMockExecuteFunction({});
            await expect(helpers.waitForFileInSession.call(mockExecuteFunction, 'session-123', 'file-123', 100)).rejects.toThrow('Timeout reached');
        });
    });
    describe('pushFileToSession', () => {
        it('should push file to session and wait', async () => {
            ...
            // Call the function
            await helpers.pushFileToSession.call(createMockExecuteFunction({}), mockFileId, mockSessionId, waitForFileInSessionMock);
    ...
    describe('triggerFileInput', () => {
        it('should trigger file input in window', async () => {
            ...
            expect(apiRequestMock).toHaveBeenCalledWith('POST', `/sessions/${mockSessionId}/windows/${mockWindowId}/file-input`, {
                fileId: mockFileId,
                elementDescription: 'test',
                includeHiddenElements: false,
            });
    ...
    describe('createFileBuffer', () => {
        ...
        it('should create buffer from binary data', async () => {
            ...
            const result = await helpers.createFileBuffer.call(mockExecuteFunction, 'binary', mockBinaryPropertyName, 0);
        ...
        it('should throw error for unsupported source type', async () => {
            await expect(helpers.createFileBuffer.call(createMockExecuteFunction({}), 'invalid-source', 'test-value', 0)).rejects.toThrow();
        });
    });
});
