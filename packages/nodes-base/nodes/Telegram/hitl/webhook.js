...
function isAuthorizedResponder(from, approverIds) {
...
async function applyPostDecisionEdit(context, { chatId, messageId, approved, from, originalText, postDecisionBehavior }) {
    try {
...
export async function telegramSendAndWaitWebhook() {
    ...
    const chatApproval = this.getNodeParameter('chatApproval', false);
    ...
    const expectedToken = deriveHitlSecretToken(credentials.accessToken);
    ...
    const approverIds = this.getNodeParameter('approverIds', '')
        .split(',')
        .map((id) => id.trim())
        .filter(Boolean);
    if (!isAuthorizedResponder(from, approverIds)) {
        ...
        try {
            await apiRequest.call(this, 'POST', 'answerCallbackQuery', {
                ...
                text: this.getNodeParameter('unauthorizedReplyText', 'You are not authorized to respond to this request.'),
                show_alert: true,
            });
        }
        catch {
