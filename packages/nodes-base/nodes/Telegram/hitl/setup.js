 ...
 */
export async function prepareChatApproval(context) {
    ...
    let allowedUpdates;
    try {
        const webhookInfo = (await apiRequest.call(context, 'POST', 'getWebhookInfo', {}));
    ...
    if (isLikelyTriggerWebhook(currentUrl, origin, reverseProxyPrefix)) {
        ...
        throw new NodeOperationError(context.getNode(), 'The Telegram Trigger claiming this bot was activated before one-tap approval support. Re-activate that workflow (or update the trigger to the latest version) so callback buttons reach n8n.');
    }
    throw new NodeOperationError(context.getNode(), `This bot's webhook is already claimed by another system (URL: ${currentUrl}). Use a different bot for one-tap approval, or turn off "Approve Within Chat".`);
}
