...
const SIGNED_RESUME_URL = 'https://mybot.example.com/webhook-waiting/42/node-1?approved=true&signature=abc';
...
function makeContext() {
    ...
    context.getNode.mockReturnValue(mock({ id: 'node-1' }));
...
function mockWebhookInfo(result) {
    genericFunctions.apiRequest.mockImplementation((_method, endpoint) => {
...
describe('prepareChatApproval', () => {
    ...
    it('falls back to link buttons and warns when the instance is not public HTTPS', async () => {
        const context = makeContext();
        context.getNodeParameter.mockImplementation((name) => name === 'responseType' ? 'approval' : true);
        context.getSignedResumeUrl.mockReturnValue('http://localhost/webhook-waiting/42/node-1?approved=true&signature=abc');
        const effective = await prepareChatApproval(context);
        expect(effective).toBe(false);
    ...
    it('falls back to link buttons and warns for an IPv6 loopback instance', async () => {
        ...
        const context = makeContext();
        context.getNodeParameter.mockImplementation((name) => name === 'responseType' ? 'approval' : true);
        context.getSignedResumeUrl.mockReturnValue('https://[::1]/webhook-waiting/42/node-1?approved=true&signature=abc');
        const effective = await prepareChatApproval(context);
        expect(effective).toBe(false);
        expect(context.logger.warn).toHaveBeenCalled();
        expect(genericFunctions.apiRequest).not.toHaveBeenCalled();
    ...
    it('re-registers idempotently when the webhook already points at our fixed endpoint', async () => {
        ...
        const effective = await prepareChatApproval(context);
        expect(effective).toBe(true);
        expect(genericFunctions.apiRequest).toHaveBeenCalledWith('POST', 'setWebhook', expect.objectContaining({ url: FIXED_ENDPOINT_URL }));
    });
    it('coexists without calling setWebhook when a same-instance trigger forwards callback_query', async () => {
        ...
        const effective = await prepareChatApproval(context);
        expect(effective).toBe(true);
        expect(genericFunctions.apiRequest).not.toHaveBeenCalledWith('POST', 'setWebhook', expect.anything());
    });
    it('coexists with a same-instance trigger behind a reverse-proxy path prefix', async () => {
        ...
        const context = makeContext();
        context.getNodeParameter.mockImplementation((name) => name === 'responseType' ? 'approval' : true);
        context.getSignedResumeUrl.mockReturnValue('https://mybot.example.com/n8n-instance/webhook-waiting/42/node-1?approved=true&signature=abc');
        mockWebhookInfo({
    ...
    it('throws an actionable error when the webhook is claimed by a foreign system', async () => {
        ...
        await expect(prepareChatApproval(context)).rejects.toThrow(/someone-elses-system\.example\.com/);
    });
    ...
    it('falls back to link buttons and warns when getWebhookInfo fails', async () => {
        const context = makeContext();
        context.getNodeParameter.mockImplementation((name) => name === 'responseType' ? 'approval' : true);
        genericFunctions.apiRequest.mockRejectedValue(new Error('network error'));
        const effective = await prepareChatApproval(context);
        expect(effective).toBe(false);
        expect(context.logger.warn).toHaveBeenCalled();
    });
    it('falls back to link buttons and warns when setWebhook fails', async () => {
        ...
        expect(effective).toBe(false);
        expect(context.logger.warn).toHaveBeenCalled();
        expect(genericFunctions.apiRequest).toHaveBeenCalledWith('POST', 'setWebhook', expect.anything());
    });
});
//# sourceMappingURL=prepareChatApproval.test.js.map