...
vi.mock('../../../../utils/sendAndWait/utils', async () => {
    const originalModule = await vi.importActual('../../../../utils/sendAndWait/utils');
    return {
        ...originalModule,
...
function makeCallbackQueryBody(overrides = {}) {
    return {
        callback_query: {
            id: 'cbq-1',
...
describe('telegramSendAndWaitWebhook', () => {
    ...
    it('delegates without crashing for a classic link-button GET click, whose body is undefined', async () => {
        ...
        sendAndWaitWebhook.mockResolvedValue({ noWebhookResponse: true });
        webhookFns.getBodyData.mockReturnValue(undefined);
        webhookFns.getNodeParameter.mockReturnValue(false);
        const result = await telegramSendAndWaitWebhook.call(webhookFns);
        expect(sendAndWaitWebhook).toHaveBeenCalledTimes(1);
        expect(result).toEqual({ noWebhookResponse: true });
    ...
    it('delegates to the shared sendAndWaitWebhook for a non-callback request even when chatApproval is on', async () => {
        sendAndWaitWebhook.mockResolvedValue({ noWebhookResponse: true });
        webhookFns.getBodyData.mockReturnValue({});
        webhookFns.getNodeParameter.mockImplementation((name) => name === 'chatApproval' ? true : undefined);
    ...
    it('responds 400 and does not resume when the callback data is not a recognizable reference', async () => {
        ...
        const send = vi.fn();
        webhookFns.getResponseObject.mockReturnValue({ status, send });
        const result = await telegramSendAndWaitWebhook.call(webhookFns);
    ...
    it('answers with an unauthorized alert, responds 200, and does not resume when the approver list rejects the sender', async () => {
        ...
        webhookFns.getNodeParameter.mockImplementation((name, fallback) => {
            if (name === 'chatApproval')
                return true;
            if (name === 'approverIds')
                return '777, 888';
            if (name === 'unauthorizedReplyText')
                return 'Nope.';
            return fallback;
        });
        webhookFns.getHeaderData.mockReturnValue({
    ...
    it('resumes with approved: false for a decline decision', async () => {
        webhookFns.getBodyData.mockReturnValue(makeCallbackQueryBody({ data: buildHitlCallbackReference('42', 'd', TEST_HMAC_SECRET) }));
        webhookFns.getNodeParameter.mockImplementation((name, fallback) => {
            if (name === 'chatApproval')
                return true;
            if (name === 'postDecisionBehavior')
        ...
        const result = await telegramSendAndWaitWebhook.call(webhookFns);
        expect(result
            .workflowData[0][0].json.data.approved).toBe(false);
    });
...
//# sourceMappingURL=telegramHitlWebhook.test.js.map