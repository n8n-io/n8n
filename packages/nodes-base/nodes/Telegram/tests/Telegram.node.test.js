...
describe('Telegram node', () => {
    const executeFunctionsMock = mockDeep();
    let apiRequestSpy;
    const node = new Telegram();
    const legacyBinaryAccessHelper = (index, propertyName) => {
        const items = executeFunctionsMock.getInputData();
        return items[index].binary[propertyName];
    };
    beforeEach(() => {
        vi.resetAllMocks();
        ...
        executeFunctionsMock.helpers.returnJsonArray.mockImplementation((input) => input);
        executeFunctionsMock.helpers.constructExecutionMetaData.mockImplementation((input) => input);
    });
    describe('file:get', () => {
        beforeEach(() => {
        ...
        it('should determine the mime type of the file', async () => {
            ...
            expect(executeFunctionsMock.helpers.prepareBinaryData).toHaveBeenCalledWith(Buffer.from('test-file'), 'file_1.pdf', 'application/pdf');
        });
        it('should fallback to application/octet-stream if the mime type cannot be determined', async () => {
            ...
            expect(executeFunctionsMock.helpers.prepareBinaryData).toHaveBeenCalledWith(Buffer.from('test-file'), 'file_1.foo', 'application/octet-stream');
        });
        it('should use the provided mime type if it is specified', async () => {
            ...
            expect(executeFunctionsMock.helpers.prepareBinaryData).toHaveBeenCalledWith(Buffer.from('test-file'), 'file_1.pdf', 'image/jpeg');
        });
    });
    describe('assertBinaryData usage', () => {
        ...
        it('should throw error when binary data is missing', async () => {
            ...
            await expect(node.execute.call(executeFunctionsMock)).rejects.toThrow('No binary data exists on item!');
            expect(executeFunctionsMock.helpers.assertBinaryData).toHaveBeenCalledWith(0, 'data');
        });
        it('should throw error when specified binary property does not exist', async () => {
            ...
            await expect(node.execute.call(executeFunctionsMock)).rejects.toThrow("There is no binary data property 'nonExistentProperty' on item!");
            expect(executeFunctionsMock.helpers.assertBinaryData).toHaveBeenCalledWith(0, 'nonExistentProperty');
        });
        it('should use fileName from assertBinaryData result when additionalFields.fileName is not provided', async () => {
            ...
            await node.execute.call(executeFunctionsMock);
            expect(executeFunctionsMock.helpers.assertBinaryData).toHaveBeenCalledWith(0, 'data');
            expect(apiRequestSpy).toHaveBeenCalledWith('POST', 'sendPhoto', {}, {}, expect.objectContaining({
    ...
    describe('message:sendPhoto with binary data', () => {
        ...
        it('should use correct index for binaryPropertyName parameter', async () => {
            ...
            expect(executeFunctionsMock.getNodeParameter).not.toHaveBeenCalledWith('binaryPropertyName', 0, expect.anything());
            expect(executeFunctionsMock.getNodeParameter).not.toHaveBeenCalledWith('binaryPropertyName', 1, expect.anything());
        });
        it('should use correct index for additionalFields.fileName parameter', async () => {
            ...
            apiRequestSpy.mockResolvedValue([{ result: { message_id: 123 } }]);
            await node.execute.call(executeFunctionsMock);
            expect(executeFunctionsMock.getNodeParameter).toHaveBeenCalledWith('additionalFields.fileName', 0, '');
            expect(executeFunctionsMock.getNodeParameter).toHaveBeenCalledWith('additionalFields.fileName', 1, '');
        });
        it('should use correct binary data for each item based on binaryPropertyName index', async () => {
            ...
            expect(apiRequestSpy).toHaveBeenNthCalledWith(1, 'POST', 'sendPhoto', {}, {}, expect.objectContaining({
                formData: expect.objectContaining({
                    chat_id: 'chat-id-0',
                    photo: expect.objectContaining({
            ...
            expect(apiRequestSpy).toHaveBeenNthCalledWith(2, 'POST', 'sendPhoto', {}, {}, expect.objectContaining({
                formData: expect.objectContaining({
                    chat_id: 'chat-id-1',
                    photo: expect.objectContaining({
                        value: expect.any(Buffer),
        ...
        it('should fallback to binary fileName when additionalFields.fileName is empty', async () => {
            ...
            apiRequestSpy.mockResolvedValue([{ result: { message_id: 123 } }]);
            await node.execute.call(executeFunctionsMock);
            const expectFileName = (index, filename) => {
                expect(apiRequestSpy).toHaveBeenNthCalledWith(index, 'POST', 'sendPhoto', {}, {}, {
                    formData: expect.objectContaining({
                        photo: expect.objectContaining({
                            options: expect.objectContaining({
                                filename,
        ...
        it('should process different chat IDs for multiple items correctly', async () => {
            ...
            const expectChatId = (n, chatId) => {
                expect(apiRequestSpy).toHaveBeenNthCalledWith(n, 'POST', 'sendPhoto', {}, {}, {
                    formData: expect.objectContaining({
                        chat_id: chatId,
                    }),
                });
    ...
    describe('message:sendMessageDraft', () => {
        it('should send the draft with chat_id, draft_id, text and additional fields', async () => {
            ...
            await node.execute.call(executeFunctionsMock);
            expect(apiRequestSpy).toHaveBeenCalledWith('POST', 'sendMessageDraft', {
                chat_id: '123456789',
                draft_id: 42,
        ...
        it('should allow an empty text in the draft', async () => {
            ...
            apiRequestSpy.mockResolvedValue([{ ok: true, result: true }]);
            await node.execute.call(executeFunctionsMock);
            expect(apiRequestSpy).toHaveBeenCalledWith('POST', 'sendMessageDraft', { chat_id: '123456789', draft_id: 1, text: '' }, {});
        });
        it('should throw when the draft ID is zero', async () => {
            ...
            await expect(node.execute.call(executeFunctionsMock)).rejects.toThrow('Draft ID must be non-zero');
    ...
    describe('message:sendRichMessage', () => {
        it('should wrap markdown content in rich_message and merge send options', async () => {
            ...
            await node.execute.call(executeFunctionsMock);
            expect(apiRequestSpy).toHaveBeenCalledWith('POST', 'sendRichMessage', {
                chat_id: '@channel',
    ...
    describe('message:sendRichMessageDraft', () => {
        it('should stream the rich message with chat_id and draft_id', async () => {
            ...
            apiRequestSpy.mockResolvedValue([{ ok: true, result: true }]);
            await node.execute.call(executeFunctionsMock);
            expect(apiRequestSpy).toHaveBeenCalledWith('POST', 'sendRichMessageDraft', {
                chat_id: '123456789',
                draft_id: 7,
...
//# sourceMappingURL=Telegram.node.test.js.map