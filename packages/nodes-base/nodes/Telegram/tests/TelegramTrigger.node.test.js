...
vi.mock('../GenericFunctions', async () => {
    ...
    return {
        ...originalModule,
        apiRequest: vi.fn(async function (method, query) {
...
describe('TelegramTrigger', () => {
    ...
    const createOptions = ({ type, attachment, useChannelPost = false, imageSize = 'small', }) => {
        ...
        return {
            ...
            workflow: mock({ id: '1', active: true }),
            node: mock({
                id: '2',
                parameters: {
                    additionalFields: {
    ...
    describe('Webhook', () => {
        ...
        ])('should return downloaded files for %s attachments with channel_post', async ({ type, attachment }) => {
        ...
        it.each([
            ...
            { type: 'video', attachment: { file_id: 'vid666' } },
            { type: 'document', attachment: { file_id: '0909' } },
        ])('should return downloaded files for %s attachments with message', async ({ type, attachment }) => {
    ...
    describe('HITL callback forwarding', () => {
        test('forwards to the configured waiting-webhook path instead of a hardcoded one', async () => {
            Container.set(GlobalConfig, mock({ endpoints: { webhookWaiting: 'custom-waiting' } }));
            const httpRequest = vi.fn().mockResolvedValue({});
            const telegramTrigger = new TelegramTrigger();
            const webhookFunctions = mock({
                helpers: mock({ httpRequest }),
                getNode: () => mock({ id: '2', typeVersion: 1.5 }),
                getWorkflow: () => mock({ id: '1' }),
                ...
                getCredentials: async () => ({ accessToken: '123456:test-token' }),
                getNodeParameter: () => [],
            });
            await telegramTrigger.webhook.call(webhookFunctions);
            expect(httpRequest).toHaveBeenCalledWith(expect.objectContaining({ url: 'https://mybot.example.com/custom-waiting-telegram' }));
        });
        test('preserves a reverse-proxy path prefix instead of forwarding to the origin root', async () => {
            ...
            Container.set(GlobalConfig, mock({
                endpoints: { webhook: 'webhook', webhookWaiting: 'webhook-waiting' },
            }));
            const httpRequest = vi.fn().mockResolvedValue({});
            const telegramTrigger = new TelegramTrigger();
            const webhookFunctions = mock({
    ...
    describe('create', () => {
        test('should set drop_pending_updates for version 1.3', async () => {
            ...
            expect(vi.mocked(apiRequest)).toHaveBeenCalledWith('POST', 'setWebhook', expect.objectContaining({
...
//# sourceMappingURL=TelegramTrigger.node.test.js.map