...
function gristLegacyBaseUrl(credentials) {
...
export function gristBaseUrl(credentials) {
    if (credentials.url) {
        return normalizeBaseUrl(credentials.url);
    }
    return gristLegacyBaseUrl(credentials);
}
export async function gristApiRequest(method, endpoint, body = {}, qs = {}) {
    const credentials = await this.getCredentials('gristApi');
    const options = {
        headers: {
...
export function parseSortProperties(sortProperties) {
...
export function parseFilterProperties(filterProperties) {
...
export function parseDefinedFields(fieldsToSendProperties) {
    return fieldsToSendProperties.reduce((acc, cur) => {
        acc[cur.fieldId] = cur.fieldValue;
        return acc;
    }, {});
}
export function parseAutoMappedInputs(incomingKeys, inputsToIgnore, item) {
...
export function throwOnZeroDefinedFields(fields) {
    if (!fields?.length) {
        throw new NodeOperationError(this.getNode(), "No defined data found. Please specify the data to send in 'Fields to Send'.");
    }
}
