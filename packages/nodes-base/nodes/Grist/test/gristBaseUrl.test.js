...
describe('Grist gristBaseUrl', () => {
    ...
    it('keeps a host whose name merely ends in api', () => {
        expect(gristBaseUrl({ url: 'https://api.getgrist.com' })).toBe('https://api.getgrist.com');
        expect(gristBaseUrl({ url: 'https://grist-api.example.com' })).toBe('https://grist-api.example.com');
    });
    describe('legacy credentials without a url', () => {
        it('resolves a stored self-hosted URL, stripping a trailing slash', () => {
            expect(gristBaseUrl({ selfHostedUrl: 'http://localhost:8484/' })).toBe('http://localhost:8484');
        });
        it('strips a trailing /api from a stored self-hosted URL', () => {
            expect(gristBaseUrl({ selfHostedUrl: 'http://localhost:8484/api' })).toBe('http://localhost:8484');
        });
        ...
        it('prefers a self-hosted URL over a subdomain when both are present', () => {
            expect(gristBaseUrl({ selfHostedUrl: 'https://grist.example.com', customSubdomain: 'acme' })).toBe('https://grist.example.com');
        });
    });
    ...
    describe('after credential defaults are applied, as at execution time', () => {
        const resolveThroughDefaults = (stored) => {
            const decrypted = NodeHelpers.getNodeParameters(new GristApi().properties, stored, true, false, null, null);
            return gristBaseUrl(decrypted);
        };
        it('keeps a legacy self-hosted credential on its own host', () => {
            expect(resolveThroughDefaults({
                apiKey: 'k',
                planType: 'selfHosted',
        ...
        it('keeps a legacy team credential on its subdomain host', () => {
            expect(resolveThroughDefaults({ apiKey: 'k', planType: 'paid', customSubdomain: 'acme' })).toBe('https://acme.getgrist.com');
        });
        it('uses the stored url when one has been saved', () => {
            expect(resolveThroughDefaults({ apiKey: 'k', url: 'https://grist.example.com' })).toBe('https://grist.example.com');
        });
...
//# sourceMappingURL=gristBaseUrl.test.js.map