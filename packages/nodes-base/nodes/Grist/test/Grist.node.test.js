...
describe('Execute Grist Node', () => {
    ...
    const run = async (getNodeParameter) => {
        const ctx = mock();
    ...
    it('sends auto-mapped fields for all items in one batched request', async () => {
        ...
        expect(gristApiRequest).toHaveBeenCalledWith('PUT', '/docs/test-doc-id/tables/test-table-id/records', {
            records: [
                {
    ...
    it('emits one error item paired with every input item when continueOnFail is set', async () => {
        ...
        ctx.getNodeParameter.mockImplementation(((name, i) => {
            switch (name) {
                case 'operation':
                    return 'upsert';
                case 'docId':
...
describe('Grist credentialTest', () => {
    const run = async (orgs, data = { apiKey: 'k', url: 'https://api.getgrist.com' }) => {
...
//# sourceMappingURL=Grist.node.test.js.map