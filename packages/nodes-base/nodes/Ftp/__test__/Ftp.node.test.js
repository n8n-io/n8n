...
describe('Ftp', () => {
    ...
    beforeEach(() => {
        ...
        executeFunctions.getNode.mockReturnValue({ typeVersion: 1 });
        executeFunctions.helpers.constructExecutionMetaData.mockImplementation((data) => data);
    });
    ...
    const setupSftpList = (typeVersion) => {
        ...
        vi.spyOn(sftpModule, 'default').mockImplementation(function () {
            return { connect: vi.fn(), list, end: vi.fn() };
        });
        ...
        executeFunctions.helpers.returnJsonArray.mockImplementation((data) => (Array.isArray(data) ? data : [data]).map((json) => ({ json })));
        executeFunctions.getNodeParameter.mockImplementation((parameterName, _idx, defaultValue) => {
            switch (parameterName) {
                case 'operation':
                    return 'list';
...
//# sourceMappingURL=Ftp.node.test.js.map