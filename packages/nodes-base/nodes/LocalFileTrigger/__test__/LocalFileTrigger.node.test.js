...
describe('LocalFileTrigger', () => {
    ...
    it('should set up chokidar with correct options for folder + match ignore', async () => {
        ...
        expect(chokidar.watch).toHaveBeenCalledWith('/some/folder', expect.objectContaining({
            ignored: '**/*.txt',
            ignoreInitial: true,
            depth: 1,
            followSymlinks: true,
    ...
    it('should wrap ignored in function for ignoreMode=contain', async () => {
        ...
        await node.trigger.call(context);
        const call = chokidar.watch.mock.calls[0][1];
...
//# sourceMappingURL=LocalFileTrigger.node.test.js.map