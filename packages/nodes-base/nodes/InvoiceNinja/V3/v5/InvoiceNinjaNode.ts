...
import { IInvoice, IInvoiceItem } from './invoiceInterface';
...
import { IPurchaseOrder, IPurchaseOrderItem } from './PurchaseOrderInterface';
				...
				if (resource === 'purchaseOrder') {
					...
					if (operation === 'getAll') {
						...
						if (filters.clientStatus) {
							qs.client_status = filters.clientStatus.toString() as string;
						}
						const include = that.getNodeParameter('include', i) as Array<string>;
						if (include.length) {
							qs.include = include.toString() as string;
