		...
		try {
			if (operation === 'upload') {
				...
				if (items[i].binary! === undefined)
					throw new NodeOperationError(this.getNode(), 'binary data not exists on this item.');
				const binaryData: IBinaryData = items[i].binary![binaryPropertyName]!;
				if (binaryData === undefined)
					throw new NodeOperationError(
						this.getNode(),
						`binary data property "${binaryPropertyName}" does not exists on item.${
							items[i].binary && Object.keys(items[i].binary || {}).length
								? ` Did you mean "${Object.keys(items[i].binary!)[0]}"?`
								: ''
						}`,
					);
