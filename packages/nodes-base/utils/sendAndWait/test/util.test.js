...
import { getSendAndWaitProperties, getSendAndWaitConfig, createEmail, sendAndWaitWebhook, } from '../utils';
describe('Send and Wait utils tests', () => {
    ...
    beforeEach(() => {
        ...
        mockWebhookFunctions.getWorkflowSettings.mockReturnValue(mock({}));
    });
    ...
    describe('getSendAndWaitConfig', () => {
        ...
        it('should return correct config for double approval', () => {
            ...
            expect(config.options).toEqual(expect.arrayContaining([
                {
                    label: 'Reject',
                    style: 'secondary',
        ...
        describe('customForm validation', () => {
            const mockCustomFormParameters = (params) => mockExecuteFunctions.getNodeParameter.mockImplementation((parameterName) => ({
                message: 'Pick a customer',
                responseType: 'customForm',
                ...params,
            })[parameterName]);
            it('should throw when the form JSON resolves to invalid form fields', () => {
                mockExecuteFunctions.getNode.mockReturnValue({ name: 'Send Email' });
                ...
                expect(() => getSendAndWaitConfig(mockExecuteFunctions)).toThrow('Field dropdown in field 0 has an invalid option 0');
            });
            ...
            const readParameterNames = () => mockExecuteFunctions.getNodeParameter.mock.calls.map(([name]) => name);
    ...
    describe('sendAndWaitWebhook', () => {
        ...
        it('should throw when a data-driven customForm resolves to invalid form fields', async () => {
            mockWebhookFunctions.getRequestObject.mockReturnValue({ method: 'GET' });
            mockWebhookFunctions.getResponseObject.mockReturnValue({
                render: vi.fn(),
                setHeader: vi.fn(),
            });
            mockWebhookFunctions.getNode.mockReturnValue({ name: 'Dropdown' });
            // A customer row without a name resolves to `{ option: null }`
            ...
            expect(error.message).toBe('Field dropdown in field 0 has an invalid option 1');
        ...
        it('overrides a form field named respondedAt with the server timestamp', async () => {
            ...
            expect(new Date(json.data.respondedAt).toISOString()).toBe(json.data.respondedAt);
        });
        ...
        it('should return noWebhookResponse if user-agent is empty (Microsoft Preview Service)', async () => {
            ...
            mockWebhookFunctions.getNodeParameter.mockImplementation((parameterName, fallbackValue) => {
                const params = { responseType: 'approval' };
        ...
        it.each([
            ...
            'Preview Service/1.0',
        ])('should return noWebhookResponse if user-agent contains %s (Microsoft Preview Service)', async (userAgent) => {
            const send = vi.fn();
            mockWebhookFunctions.getRequestObject.mockReturnValue({
        ...
        it.each([
            ['freeText', ''],
            ['freeText', 'SkypeUriPreview Preview/1.0'],
            ['customForm', ''],
            ['customForm', 'SkypeUriPreview Preview/1.0'],
        ])('should not block Microsoft Preview Service when responseType is %s (user-agent: %s)', async (responseType, userAgent) => {
            const mockRender = vi.fn();
            const mockSetHeader = vi.fn();
            mockWebhookFunctions.getRequestObject.mockReturnValue({
                method: 'GET',
            ...
            const formFieldParams = responseType === 'customForm'
                ? {
                    defineForm: 'fields',
                    'formFields.values': [{ label: 'Field 1', fieldType: 'text', requiredField: true }],
                }
...
describe('configureWaitTillDate', () => {
    ...
    it('should throw NodeOperationError for invalid maxDateAndTime format', () => {
        ...
        expect(() => configureWaitTillDate(mockExecuteFunctions)).toThrow('Could not configure Limit Wait Time');
    });
