...
const credentialData = (template, placeholderValues = {}, placeholderDefs) => ({
...
describe('resolveTemplatedAuth', () => {
    it('should resolve placeholders in headers, query and nested body values', () => {
        const result = resolveTemplatedAuth(credentialData({
    ...
    it('should resolve multiple placeholders within one string', () => {
        const result = resolveTemplatedAuth(credentialData({ headers: { 'X-Auth': '{{user}}:{{ token }}' } }, { user: 'bot', token: 'abc' }));
        expect(result.headers).toEqual({ 'X-Auth': 'bot:abc' });
    });
    it('should leave parts without placeholders untouched', () => {
        const result = resolveTemplatedAuth(credentialData({ headers: { Accept: 'application/json' }, qs: { page: 1 } }));
        expect(result).toEqual({ headers: { Accept: 'application/json' }, qs: { page: 1 } });
    });
    it('should throw when a placeholder has no value', () => {
        expect(() => resolveTemplatedAuth(credentialData({ headers: { 'X-Key': '{{api_key}}' } }))).toThrow('No value set for placeholder {{api_key}}');
    });
    it('should throw when a placeholder value is empty', () => {
        expect(() => resolveTemplatedAuth(credentialData({ headers: { 'X-Key': '{{api_key}}' } }, { api_key: '' }))).toThrow('No value set for placeholder {{api_key}}');
    });
    it('should throw when a placeholder value is not a plain value', () => {
        expect(() => resolveTemplatedAuth(credentialData({ headers: { 'X-Key': '{{api_key}}' } }, { api_key: { nested: true } }))).toThrow('must be a plain value');
    });
    it.each(['constructor', 'toString'])('should not resolve inherited %s values', (name) => {
        expect(() => resolveTemplatedAuth(credentialData({ headers: { Authorization: `{{${name}}}` } }))).toThrow(`No value set for placeholder {{${name}}}`);
    });
    it('should throw on invalid template JSON', () => {
        expect(() => resolveTemplatedAuth({ template: 'not json', placeholderValues: '{}' })).toThrow('Invalid Simplified Custom Auth template JSON');
    });
    it.each([
        ['array', []],
        ['string', 'headers'],
    ])('should reject a template parsed as a %s', (_, template) => {
        expect(() => resolveTemplatedAuth({ template: JSON.stringify(template), placeholderValues: '{}' })).toThrow('Simplified Custom Auth template must be a JSON object');
    });
    it.each(['headers', 'body', 'qs'])('should reject non-object template %s', (partName) => {
        expect(() => resolveTemplatedAuth(credentialData({ [partName]: 'invalid' }))).toThrow(`Simplified Custom Auth template ${partName} must be a JSON object`);
    });
    it.each([
        ['array', []],
        ['string', 'secret'],
    ])('should reject placeholder values parsed as a %s', (_, placeholderValues) => {
        expect(() => resolveTemplatedAuth({
            template: '{}',
            placeholderValues: JSON.stringify(placeholderValues),
    ...
    describe('optional placeholders', () => {
        ...
        it('should substitute an optional placeholder normally when a value is set', () => {
            const result = resolveTemplatedAuth(credentialData({ headers: { Authorization: 'Key {{api_key}}', 'X-Org': '{{org}}' } }, { api_key: 'secret', org: 'acme' }, defs));
        ...
        it('should omit a mixed entry when its optional placeholder is empty, even with statics', () => {
            const result = resolveTemplatedAuth(credentialData({ headers: { Authorization: 'Key {{api_key}}', 'X-Scope': 'org:{{org}}' } }, { api_key: 'secret', org: '' }, defs));
        ...
        it('should omit an entry before resolving its other missing placeholders', () => {
            const result = resolveTemplatedAuth(credentialData({ headers: { 'X-Scope': '{{org}}:{{api_key}}' } }, {}, defs));
            expect(result.headers).toEqual({});
        });
        it('should still fail closed for empty required placeholders', () => {
            expect(() => resolveTemplatedAuth(credentialData({ headers: { 'X-Key': '{{api_key}}' } }, { api_key: '' }, defs))).toThrow('No value set for placeholder {{api_key}}');
        });
...
describe('applyTemplatedAuth', () => {
    ...
    it.each([
        ...
        expect(() => applyTemplatedAuth(credentials, { body })).toThrow('Simplified Custom Auth body templates cannot be applied to non-object request bodies');
    });
});
//# sourceMappingURL=templated-auth.test.js.map