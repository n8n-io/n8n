...
import { clearAtlassianAccessibleResourcesCache, extractAtlassianSiteHostname, fetchAtlassianAccessibleResources, getAtlassianApiBaseUrl, getAtlassianCloudId, resolveAtlassianCloudId, } from '../atlassian';
...
describe('getAtlassianApiBaseUrl', () => {
    it('should build the product-scoped API base URL', () => {
        expect(getAtlassianApiBaseUrl('confluence', 'abc-123')).toBe('https://api.atlassian.com/ex/confluence/abc-123');
        expect(getAtlassianApiBaseUrl('jira', 'abc-123')).toBe('https://api.atlassian.com/ex/jira/abc-123');
    });
    it('should URL-encode the cloudId', () => {
        expect(getAtlassianApiBaseUrl('jira', 'a/b c')).toBe('https://api.atlassian.com/ex/jira/a%2Fb%20c');
    });
});
describe('getAtlassianCloudId', () => {
    ...
    it('should resolve the cloudId via accessible-resources', async () => {
        const result = await getAtlassianCloudId.call(ctx, credentialType, 'https://example.atlassian.net', 'confluence');
        expect(result).toBe('cloud-1');
        expect(mockHttpRequestWithAuthentication).toHaveBeenCalledTimes(1);
        expect(mockHttpRequestWithAuthentication).toHaveBeenCalledWith(credentialType, expect.objectContaining({
    ...
    it('should match hostnames case-insensitively on both sides', async () => {
        const result = await getAtlassianCloudId.call(ctx, credentialType, 'OTHER.atlassian.net/wiki', 'confluence');
    ...
    it('should skip entries with malformed URLs instead of throwing', async () => {
        ...
        const result = await getAtlassianCloudId.call(ctx, credentialType, 'example.atlassian.net', 'confluence');
        expect(result).toBe('cloud-1');
    });
    it('should serve the second lookup from the cache without refetching', async () => {
        const first = await getAtlassianCloudId.call(ctx, credentialType, 'https://example.atlassian.net/wiki', 'confluence');
        const second = await getAtlassianCloudId.call(ctx, credentialType, 'example.atlassian.net', 'confluence');
    ...
    it('should refetch once when the cached list has no match, so a newly granted site resolves', async () => {
        ...
        const result = await getAtlassianCloudId.call(ctx, credentialType, 'other.atlassian.net', 'confluence');
        expect(result).toBe('cloud-2');
    ...
    it('should not refetch when the list it just fetched has no match', async () => {
        await expect(getAtlassianCloudId.call(ctx, credentialType, 'missing.atlassian.net', 'confluence')).rejects.toThrow('No Confluence site matched');
        expect(mockHttpRequestWithAuthentication).toHaveBeenCalledTimes(1);
    });
    it('should share one cache entry across products for the same credential and hostname', async () => {
        const viaJira = await getAtlassianCloudId.call(ctx, credentialType, 'example.atlassian.net', 'jira');
        const viaConfluence = await getAtlassianCloudId.call(ctx, credentialType, 'example.atlassian.net', 'confluence');
    ...
    it('should name the input and list reachable site URLs when nothing matches', async () => {
        const promise = getAtlassianCloudId.call(ctx, credentialType, 'foo.atlassian.net', 'confluence');
        await expect(promise).rejects.toThrow(NodeOperationError);
        await expect(promise).rejects.toThrow('No Confluence site matched "foo.atlassian.net". This connection can access: https://example.atlassian.net, https://Other.Atlassian.NET.');
    });
    it('should name the product in the error message', async () => {
        await expect(getAtlassianCloudId.call(ctx, 'jiraSoftwareCloudOAuth2Api', 'https://unknown.atlassian.net', 'jira')).rejects.toThrow(/No Jira site matched/);
    });
    it('should say "no sites" when the connection can access none', async () => {
        mockHttpRequestWithAuthentication.mockResolvedValueOnce([]);
        await expect(getAtlassianCloudId.call(ctx, credentialType, 'foo.atlassian.net', 'confluence')).rejects.toThrow('can access: no sites');
    });
    ...
    it('should cap the reachable-sites list at 5 entries', async () => {
        mockHttpRequestWithAuthentication.mockResolvedValueOnce(Array.from({ length: 7 }, (_, i) => ({
            id: `cloud-${i}`,
            url: `https://site-${i}.atlassian.net`,
        })));
        await expect(getAtlassianCloudId.call(ctx, credentialType, 'foo.atlassian.net', 'confluence')).rejects.toThrow('This connection can access: https://site-0.atlassian.net, https://site-1.atlassian.net, https://site-2.atlassian.net, https://site-3.atlassian.net, https://site-4.atlassian.net, and 2 more');
    });
    it('should omit malformed entries from the reachable-sites list', async () => {
        ...
        await expect(getAtlassianCloudId.call(ctx, credentialType, 'foo.atlassian.net', 'confluence')).rejects.toThrow('This connection can access: https://example.atlassian.net');
    });
    it('should throw a clear NodeOperationError on unparseable site URL', async () => {
        await expect(getAtlassianCloudId.call(ctx, credentialType, '', 'confluence')).rejects.toThrow('"" is not a valid Atlassian site URL');
    ...
    it('should pass an already-wrapped NodeApiError through unchanged', async () => {
        ...
        await expect(getAtlassianCloudId.call(ctx, credentialType, 'example.atlassian.net', 'confluence')).rejects.toBe(wrapped);
    });
});
describe('fetchAtlassianAccessibleResources', () => {
    ...
    it('should wrap a request failure in NodeApiError', async () => {
        ...
        await expect(fetchAtlassianAccessibleResources.call(ctx, credentialType)).rejects.toBeInstanceOf(NodeApiError);
    });
...
describe('resolveAtlassianCloudId', () => {
    ...
    it('should return a From List value directly without any request', async () => {
        const result = await resolveAtlassianCloudId.call(ctx, credentialType, rlc('list', 'cloud-2'), 'confluence');
        expect(result).toBe('cloud-2');
        expect(mockHttpRequestWithAuthentication).not.toHaveBeenCalled();
    });
    it('should hostname-match a By URL value against accessible resources', async () => {
        const result = await resolveAtlassianCloudId.call(ctx, credentialType, rlc('url', 'https://other.atlassian.net/wiki'), 'confluence');
        expect(result).toBe('cloud-2');
        expect(mockHttpRequestWithAuthentication).toHaveBeenCalledTimes(1);
    });
    it('should list reachable sites when a By URL value matches nothing', async () => {
        await expect(resolveAtlassianCloudId.call(ctx, credentialType, rlc('url', 'https://missing.atlassian.net'), 'confluence')).rejects.toThrow('No Confluence site matched "https://missing.atlassian.net". This connection can access: https://example.atlassian.net, https://other.atlassian.net.');
    });
    it('should auto-resolve an empty selection when the connection reaches exactly one site', async () => {
        mockHttpRequestWithAuthentication.mockResolvedValueOnce([accessibleResources[0]]);
        const result = await resolveAtlassianCloudId.call(ctx, credentialType, rlc('list', ''), 'confluence');
        expect(result).toBe('cloud-1');
    });
    ...
    it('should list the candidates and ask to pick a site when several are reachable', async () => {
        const promise = resolveAtlassianCloudId.call(ctx, credentialType, rlc('list', ''), 'confluence');
        await expect(promise).rejects.toThrow(NodeOperationError);
        await expect(promise).rejects.toThrow("This connection can access: https://example.atlassian.net, https://other.atlassian.net — pick a site in the 'Site' parameter.");
    });
    it('should cap the candidate list at 5 entries', async () => {
        mockHttpRequestWithAuthentication.mockResolvedValue(Array.from({ length: 7 }, (_, i) => ({
            id: `cloud-${i}`,
            url: `https://site-${i}.atlassian.net`,
        })));
        await expect(resolveAtlassianCloudId.call(ctx, credentialType, rlc('list', ''), 'confluence')).rejects.toThrow(', and 2 more');
    });
    it('should name the product when the connection reaches no sites', async () => {
        mockHttpRequestWithAuthentication.mockResolvedValue([]);
        await expect(resolveAtlassianCloudId.call(ctx, credentialType, rlc('list', ''), 'confluence')).rejects.toThrow('This connection cannot access any Confluence sites');
    });
    it('should ignore sites the API returned without an ID', async () => {
        ...
        await expect(resolveAtlassianCloudId.call(ctx, credentialType, rlc('list', ''), 'confluence')).resolves.toBe('cloud-2');
    });
    it('should refetch once when the cached list came back empty', async () => {
        mockHttpRequestWithAuthentication.mockResolvedValueOnce([]);
        await expect(resolveAtlassianCloudId.call(ctx, credentialType, rlc('list', ''), 'confluence')).rejects.toThrow('cannot access any Confluence sites');
        mockHttpRequestWithAuthentication.mockResolvedValueOnce([accessibleResources[0]]);
        const result = await resolveAtlassianCloudId.call(ctx, credentialType, rlc('list', ''), 'confluence');
        expect(result).toBe('cloud-1');
    });
...
//# sourceMappingURL=atlassian.test.js.map