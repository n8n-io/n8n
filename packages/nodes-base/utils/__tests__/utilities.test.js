...
import { addExecutionHints, compareItems, flattenKeys, fuzzyCompare, getResolvables, keysToLowercase, removeTrailingSlash, shuffleArray, sortItemKeysByPriorityList, wrapData, } from '@utils/utilities';
...
describe('sortItemKeysByPriorityList', () => {
    ...
    it('should sort keys not in the priority list alphabetically', () => {
        const data = [{ json: { c: 3, a: 1, b: 2, d: 4 } }];
        const priorityList = ['b', 'a'];
        const result = sortItemKeysByPriorityList(data, priorityList);
...
describe('addExecutionHints', () => {
    ...
    const selectHint = {
        location: 'outputPane',
        message: "This node ran 2 times, once for each input item. To run for the first item only, enable 'execute once' in the node settings",
    };
    it('should add batching insert hint to Postgres executeQuery operation', () => {
        addExecutionHints(executeQueryOperationContext, mock({
            type: POSTGRES_NODE_TYPE,
        }), [{ json: {} }, { json: {} }], 'executeQuery', false);
    ...
    it('should add run per item hint to Postgres select operation ', () => {
        ...
        addExecutionHints(context, mock({
            type: POSTGRES_NODE_TYPE,
        }), [{ json: {} }, { json: {} }], 'select', false);
