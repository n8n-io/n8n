...
describe('getConnection', () => {
    ...
    test('calls cleanUpHandler after TTL expires', async () => {
        ...
        const connectionType = {};
        let abortController;
        const fallBackHandler = vi.fn(async (ac) => {
            abortController = ac;
            return connectionType;
        });
    ...
    test('throws OperationsError if the fallBackHandler aborts during connection initialization', async () => {
        ...
        await expect(connectionPromise).rejects.toThrow('Could not create pool. Connection attempt was aborted.');
    });
});
describe('onShutdown', () => {
    test('calls all clean up handlers', async () => {
        // ARRANGE
        const connectionType1 = {};
        let abortController1;
        const fallBackHandler1 = vi.fn(async (ac) => {
            abortController1 = ac;
            return connectionType1;
        });
        await cpm.getConnection({
        ...
        const fallBackHandler2 = vi.fn(async (ac) => {
            abortController2 = ac;
            return connectionType2;
        });
        await cpm.getConnection({
...
//# sourceMappingURL=connection-pool-manager.test.js.map