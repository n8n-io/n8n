...
import { convertJsonToSpreadsheetBinary, extractDataFromPDF, prepareBinariesDataList, routeBinaryProperties, } from '@utils/binary';
...
describe('convertJsonToSpreadsheetBinary', () => {
    const helpers = mock();
    const executeFunctions = mock({ helpers });
    const items = [
        { json: { key1: 'value1', key2: 'value2' } },
        { json: { key1: 'value3', key2: 'value4' } },
    ];
    const mockSheet = mock();
    const workBook = {
        SheetNames: ['Sheet'],
        Sheets: {
            Sheet: mockSheet,
    ...
    const mockBuffer = mock();
    const mockBinaryData = mock({ id: 'binaryId' });
    beforeEach(() => {
        vi.clearAllMocks();
        xlsxUtils.json_to_sheet.mockReturnValue(mockSheet);
        xlsxWrite.mockReturnValue(mockBuffer);
    ...
    describe('for fileFormat xlsx', () => {
        it('should convert from JSON', async () => {
            ...
            expect(xlsxUtils.json_to_sheet).toHaveBeenCalledWith(items.map((item) => item.json), undefined);
...
describe('routeBinaryProperties', () => {
    const helpers = mock();
    const executeFunctions = mock({ helpers });
    const binaryData = mock({ id: 'binaryId' });
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    it('should keep unsafe field names as own properties without touching the prototype', async () => {
        const row = jsonParse('{"id":1,"__proto__":{"polluted":true},"file":null}');
    ...
    it('should recognize custom binary wrappers via toBuffer', async () => {
        ...
        const { json, binary } = await routeBinaryProperties.call(executeFunctions, { id: 1, blob: wrapped }, (value) => (value === wrapped ? buffer : undefined));
