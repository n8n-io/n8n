 ...
 */
export const SP_HIDE = Object.freeze({
 ...
 */
export function validateMicrosoftGraphId(id, node) {
    ...
    if (GRAPH_ID_REJECT.test(value)) {
        throw new NodeOperationError(node, 'The ID is not valid', {
            description: 'Remove any slashes, backslashes, question marks or hashes, and make sure the ID is not double-encoded.',
        });
    }
    return value;
}
 ...
 */
export function buildMicrosoftGraphPath(segments) {
...
export function rewriteNotFound(error, message, description) {
 ...
 */
export function createMicrosoftGraphTransport(config) {
    const { defaultCredentialType } = config;
    const defaultTypeName = defaultCredentialType;
    if (defaultTypeName === SERVICE_PRINCIPAL_AUTH) {
        // Fail at module load so a misconfigured facade can never ship.
        throw new UnexpectedError('createMicrosoftGraphTransport: defaultCredentialType must be a delegated OAuth2 credential, not the Service Principal credential');
    }
    /**
