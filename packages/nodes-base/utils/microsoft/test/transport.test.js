...
import { buildMicrosoftGraphPath, createMicrosoftGraphTransport, SERVICE_PRINCIPAL_AUTH, validateMicrosoftGraphId, } from '../transport';
const { getCredentialType, microsoftApiRequest, microsoftApiRequestAllItems } = createMicrosoftGraphTransport({ defaultCredentialType: 'microsoftTeamsOAuth2Api' });
describe('Microsoft Graph transport kernel', () => {
    ...
    describe('microsoftApiRequest', () => {
        describe('graphApiBaseUrl from credentials', () => {
            it.each([
                ...
                expect(mockRequestOAuth2).toHaveBeenCalledWith('microsoftTeamsOAuth2Api', expect.objectContaining({
                    method: 'GET',
                    uri: expectedUri,
                    json: true,
                }));
        ...
        describe('custom headers', () => {
            it('merges custom headers over the defaults', async () => {
                ...
                await microsoftApiRequest.call(mockExecuteFunctions, 'POST', '/teams', { displayName: 'x' }, {}, undefined, { 'If-Match': 'etag' });
                expect(mockRequestOAuth2).toHaveBeenCalledWith('microsoftTeamsOAuth2Api', expect.objectContaining({
        ...
        describe('authentication credential resolution', () => {
            ...
            it('should use microsoftTeamsOAuth2Api when authentication is not set (backward compatibility)', async () => {
                ...
                expect(mockRequestOAuth2).toHaveBeenCalledWith('microsoftTeamsOAuth2Api', expect.anything());
        ...
        describe('service principal authentication', () => {
            beforeEach(() => {
                ...
                mockExecuteFunctions.getNodeParameter.mockImplementation((name) => name === 'authentication' ? SERVICE_PRINCIPAL_AUTH : undefined);
            ...
            it('routes the request through requestWithAuthentication (NOT requestOAuth2)', async () => {
                ...
                expect(mockRequestWithAuthentication).toHaveBeenCalledWith(SERVICE_PRINCIPAL_AUTH, expect.objectContaining({
                    method: 'GET',
            ...
            describe('error suppression (no raw Graph body leaks at any status)', () => {
                ...
                const rawLeak = 'request-id: 11111111-2222-3333-4444-555555555555; client-request-id: aaaa; token=eyJ0eParrotedSecret; resource /teams/19:injected@thread.tacv2';
                const realError = (statusCode, code) => {
                    const underlying = Object.assign(new Error(`${statusCode} - {"error":{"code":"${code}","message":"${rawLeak}"}}`), {
                        statusCode,
                        status: statusCode,
                        error: { error: { code, message: rawLeak } },
                        response: { status: statusCode, statusText: code, headers: {} },
                    });
                    return new NodeApiError(mockNode, underlying);
                };
                it.each([
                    [
                        401,
                ...
                ])('maps HTTP %i to a static message and never leaks the raw body', async (statusCode, code, expectedMessage) => {
                ...
                it('maps a status-less failure (network error) to the static unknown-status message', async () => {
                    mockRequestWithAuthentication.mockRejectedValue(new Error('connect ECONNREFUSED 127.0.0.1:443'));
                    const error = (await microsoftApiRequest
                        .call(mockExecuteFunctions, 'GET', '/v1.0/teams')
                        .catch((e) => e));
                    expect(error.constructor.name).toBe('NodeApiError');
                    expect(error.message).toBe("Microsoft Graph rejected the request (HTTP unknown). Check the operation's inputs and the app registration's permissions.");
        ...
        describe('delegated OAuth2 error mapping', () => {
            const delegatedError = (statusCode, code, message) => Object.assign(new Error(`${statusCode} - {"error":{"code":"${code}"}}`), {
                statusCode,
            ...
            it('rewrites a 404 NotFound to the static "{Resource} not found" message', async () => {
                mockExecuteFunctions.getNodeParameter.mockImplementation((name) => name === 'resource' ? 'channel' : undefined);
            ...
            it('passes the Graph error message through for other delegated errors', async () => {
                mockRequestOAuth2.mockRejectedValue(delegatedError(400, 'BadRequest', 'Invalid filter clause'));
                const error = await microsoftApiRequest
                    .call(mockExecuteFunctions, 'GET', '/v1.0/teams')
                    .catch((e) => e);
                expect(error.constructor.name).toBe('NodeApiError');
    ...
    describe('validateMicrosoftGraphId', () => {
        it('accepts a GUID and a Planner-style id', () => {
            expect(() => validateMicrosoftGraphId('1111-2222-3333-4444-555566667777', mockNode)).not.toThrow();
            expect(() => validateMicrosoftGraphId('rl1HYb0cUEiHPc7zgB_KWWUAA7Of', mockNode)).not.toThrow();
        });
        ...
        it('accepts a URL-copied percent-encoded channel id and returns the decoded form', () => {
            ...
            expect(validateMicrosoftGraphId('19%3A16259efabba44a66916d91dd91862a6f%40thread.tacv2', mockNode)).toBe('19:16259efabba44a66916d91dd91862a6f@thread.tacv2');
        });
    ...
    describe('buildMicrosoftGraphPath', () => {
        ...
        it.each(['microsoftTeamsOAuth2Api', SERVICE_PRINCIPAL_AUTH])('coerces a non-string id under %s without throwing', (credential) => {
        ...
        it.each(['microsoftTeamsOAuth2Api', SERVICE_PRINCIPAL_AUTH])('trims surrounding whitespace from a valid id under %s', (credential) => {
            mockExecuteFunctions.getNodeParameter.mockReturnValue(credential);
            const path = buildMicrosoftGraphPath.call(mockExecuteFunctions, [
                '/v1.0/teams/',
                { id: '  19:abc@thread.tacv2  ' },
        ...
        it.each(['microsoftTeamsOAuth2Api', SERVICE_PRINCIPAL_AUTH].flatMap((credential) => malformedIds.map((id) => [credential, id])))('rejects an id containing separators under %s (%s)', (credential, malformedId) => {
            mockExecuteFunctions.getNodeParameter.mockReturnValue(credential);
            expect(() => buildMicrosoftGraphPath.call(mockExecuteFunctions, [
                '/v1.0/teams/',
                { id: malformedId },
                '/channels',
            ])).toThrow('The ID is not valid');
        });
        it.each(['microsoftTeamsOAuth2Api', SERVICE_PRINCIPAL_AUTH])('rejects an empty id under %s', (credential) => {
            mockExecuteFunctions.getNodeParameter.mockReturnValue(credential);
            expect(() => buildMicrosoftGraphPath.call(mockExecuteFunctions, [
                '/v1.0/teams/',
                { id: '' },
    ...
    describe('microsoftApiRequestAllItems', () => {
        const makeContext = (requestOAuth2) => ({
            getNodeParameter: vi.fn().mockReturnValue(undefined),
            getCredentials: vi.fn().mockResolvedValue({}),
            getNode: vi.fn().mockReturnValue({ name: 'Microsoft Teams' }),
            helpers: { requestOAuth2 },
        });
        const optionsOfCall = (requestOAuth2, index) => requestOAuth2.mock.calls[index][1];
        it('forwards the query and stops before the next page once the limit is satisfied', async () => {
            ...
            const result = await microsoftApiRequestAllItems.call(ctx, 'value', 'GET', '/beta/teams/1/channels/2/messages', {}, { $top: 2 }, 2);
        ...
        it('paginates until the limit is reached without re-sending the query', async () => {
            ...
            const ctx = makeContext(requestOAuth2);
            const result = await microsoftApiRequestAllItems.call(ctx, 'value', 'GET', '/beta/teams/1/channels/2/messages', {}, { $top: 100 }, 100);
        ...
        it('walks every page when no limit is set', async () => {
            ...
            const ctx = makeContext(requestOAuth2);
            const result = await microsoftApiRequestAllItems.call(ctx, 'value', 'GET', '/v1.0/teams/1/channels');
            expect(result).toEqual([{ id: '1' }, { id: '2' }]);
            expect(requestOAuth2).toHaveBeenCalledTimes(2);
        ...
        it('refuses to follow a cross-origin @odata.nextLink', async () => {
            ...
            const ctx = makeContext(requestOAuth2);
            await expect(microsoftApiRequestAllItems.call(ctx, 'value', 'GET', '/v1.0/teams/1/channels')).rejects.toThrow('Refusing to send credentials to an unexpected host');
    ...
    describe('microsoftApiRequest under a webhook hook (IHookFunctions) context', () => {
        ...
        let hookRequestOAuth2;
        let hookRequestWithAuthentication;
        beforeEach(() => {
    ...
    describe('createMicrosoftGraphTransport', () => {
        ...
        it('rejects the Service Principal credential as the default', () => {
            expect(() => createMicrosoftGraphTransport({ defaultCredentialType: SERVICE_PRINCIPAL_AUTH })).toThrow('must be a delegated OAuth2 credential');
        });
    });
});
