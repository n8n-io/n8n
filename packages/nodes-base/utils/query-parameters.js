...
function parseQueryParameters(rawParameters, node, itemIndex, label) {
    ...
    return parameters.map((parameter, index) => {
        ...
        throw new NodeOperationError(node, `${label} parameter ${index + 1} must be a scalar or an array of scalars`, {
            itemIndex,
 ...
 * the author wrote.
 */
export function parseAndResolveQueryParameters(query, rawParameters, node, itemIndex, label = 'Query') {
    const parsedQuery = jsonParse(query, {
    ...
    const takeParameter = (placeholder, parameterIndex) => {
        if (parameterIndex < 0 || parameterIndex >= parameters.length) {
            throw new NodeOperationError(node, `${label} placeholder ${placeholder} has no matching value`, {
                itemIndex,
    ...
    const resolveKey = (key) => {
        ...
        if (typeof value !== 'string' ||
            ...
            throw new NodeOperationError(node, `${label} placeholder ${key} is not a valid field name`, {
                itemIndex,
                description: 'A placeholder used as a field name must resolve to a non-empty string that does not start with "$" and does not name a reserved object property',
            });
        }
        return value;
    };
        ...
        if (value !== null && typeof value === 'object') {
            ...
            return Object.fromEntries(Object.entries(value).map(([key, entry]) => {
                ...
                if (seenKeys.has(resolvedKey)) {
                    throw new NodeOperationError(node, `${label} field name "${resolvedKey}" is used more than once`, {
                        itemIndex,
                        description: 'A parameter bound to a field name must not collide with another field name in the same object, because one clause would silently replace the other',
                    });
                }
