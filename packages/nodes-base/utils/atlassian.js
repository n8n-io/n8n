...
const accessibleResourcesCache = new Map();
const hasSiteId = (resource) => typeof resource?.id === 'string' && resource.id !== '';
 ...
 */
export function extractAtlassianSiteHostname(siteUrl) {
    const trimmed = siteUrl.trim();
    const withScheme = /^[a-z][a-z0-9+.-]*:\/\//i.test(trimmed) ? trimmed : `https://${trimmed}`;
    return new URL(withScheme).hostname.toLowerCase();
}
export function getAtlassianApiBaseUrl(product, cloudId) {
    return `https://api.atlassian.com/ex/${product}/${encodeURIComponent(cloudId)}`;
}
async function loadAccessibleResources(credentialType, bypassCache) {
    const rawCredentialId = this.getNode().credentials?.[credentialType]?.id;
    const credentialId = typeof rawCredentialId === 'string' && rawCredentialId !== '' ? rawCredentialId : undefined;
    if (!bypassCache && credentialId !== undefined) {
        const cached = accessibleResourcesCache.get(credentialId);
        if (cached)
            return { resources: cached, fromCache: true };
    }
    let resources;
    try {
 ...
 */
export async function fetchAtlassianAccessibleResources(credentialType, { bypassCache = false } = {}) {
    return (await loadAccessibleResources.call(this, credentialType, bypassCache)).resources;
}
export async function searchAtlassianSites(credentialType, filter) {
    ...
    const results = resources
        ...
        .filter((item) => filterLower === '' ||
        item.name.toLowerCase().includes(filterLower) ||
        (item.url ?? '').toLowerCase().includes(filterLower))
        .sort((a, b) => a.name.localeCompare(b.name));
    return { results };
}
const isResourceLocator = (value) => typeof value === 'object' && value !== null && 'value' in value;
export function getAtlassianSiteParameter(ctx) {
...
async function getAccessibleResourcesOrRefetch(credentialType, isSufficient) {
    const first = await loadAccessibleResources.call(this, credentialType, false);
    if (!first.fromCache || isSufficient(first.resources))
...
function formatReachableSites(resources) {
 ...
 */
export async function getAtlassianCloudId(credentialType, siteUrl, product) {
    let hostname;
    try {
        hostname = extractAtlassianSiteHostname(siteUrl);
    }
    catch {
        throw new NodeOperationError(this.getNode(), `"${siteUrl}" is not a valid Atlassian site URL. Expected something like https://your-site.atlassian.net.`);
    }
    const matchHostname = (resources) => resources.find((resource) => {
        try {
    ...
    const resources = await getAccessibleResourcesOrRefetch.call(this, credentialType, (candidates) => matchHostname(candidates) !== undefined);
    const site = matchHostname(resources);
    if (!site) {
        throw new NodeOperationError(this.getNode(), `No ${PRODUCT_NAMES[product]} site matched "${siteUrl}". This connection can access: ${formatReachableSites(resources)}.`);
    }
    return site.id;
}
/**
 ...
 */
export async function resolveAtlassianCloudId(credentialType, site, product) {
    const value = typeof site?.value === 'string' || typeof site?.value === 'number'
    ...
    const resources = await getAccessibleResourcesOrRefetch.call(this, credentialType, (candidates) => candidates.some(hasSiteId));
    const sites = resources.filter(hasSiteId);
    if (sites.length === 0) {
        throw new NodeOperationError(this.getNode(), `This connection cannot access any ${PRODUCT_NAMES[product]} sites. Reconnect the credential and grant it access to a site.`);
    }
    if (sites.length === 1)
        return sites[0].id;
    throw new NodeOperationError(this.getNode(), `This connection can access: ${formatReachableSites(sites)} — pick a site in the 'Site' parameter.`);
}
