...
describe('loadWorkflowInputMappings', () => {
    it('loads the draft subworkflow trigger (does not prefer active version)', async () => {
        const localLoadOptions = mock();
        localLoadOptions.getWorkflowNodeContext.mockResolvedValue(null);
        await loadWorkflowInputMappings.call(localLoadOptions);
        expect(localLoadOptions.getWorkflowNodeContext).toHaveBeenCalledWith(EXECUTE_WORKFLOW_TRIGGER_NODE_TYPE);
...
describe('getWorkflowInputValues', () => {
    const supplyDataFunctions = mock();
    it('should correctly map the binary property', () => {
        ...
        expect(supplyDataFunctions.getNodeParameter).toHaveBeenCalledWith('workflowInputs.value', 0, {});
        expect(supplyDataFunctions.getNodeParameter).toHaveBeenCalledWith('workflowInputs.value', 1, {});
    });
});
