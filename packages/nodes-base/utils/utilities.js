...
export const flattenKeys = (obj, prefix = []) => {
    return !isObject(obj)
        ? { [prefix.join('.')]: obj }
        : reduce(obj, (cum, next, key) => merge(cum, flattenKeys(next, [...prefix, key])), {});
};
/**
 ...
 */
export const compareItems = (obj1, obj2, keys, disableDotNotation = false) => {
    let result = true;
...
export function processJsonInput(jsonData, inputName) {
    let values;
...
export const fuzzyCompare = (useFuzzyCompare, compareVersion = 1) => {
    if (!useFuzzyCompare) {
        //Fuzzy compare is false we do strict comparison
        return (item1, item2) => isEqual(item1, item2);
    }
    return (item1, item2) => {
...
export const keysToLowercase = (headers) => {
 ...
 */
export function getResolvables(expression) {
    if (!expression)
        return [];
    const resolvables = [];
...
export function generatePairedItemData(length) {
 ...
 */
export function preparePairedItemDataArray(pairedItem) {
...
export const sanitizeDataPathKey = (item, key) => {
 ...
 */
export function sortItemKeysByPriorityList(data, priorityList) {
    return data.map((item) => {
...
export function createUtmCampaignLink(nodeType, instanceId) {
...
export function addExecutionHints(context, node, items, operation, executeOnce) {
    if ((node.type === POSTGRES_NODE_TYPE || node.type === MYSQL_NODE_TYPE) &&
        operation === 'select' &&
        items.length > 1 &&
        !executeOnce) {
    ...
    if (node.type === POSTGRES_NODE_TYPE &&
