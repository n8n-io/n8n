...
export class MicrosoftDynamicsOAuth2Api {
    ...
    properties = [
        ...
        {
            displayName: 'The default scopes needed for the node to work are already set, If you change these the node may not function correctly. Use the <code>{subdomain}</code> and <code>{region}</code> placeholders to reference the Subdomain and Region values above.',
        ...
        },
        {
            ...
            description: 'Scopes that should be enabled. Use <code>{subdomain}</code> and <code>{region}</code> as placeholders that will be replaced with the Subdomain and Region values.',
        },
        {
            displayName: 'Scope',
            name: 'scope',
            type: 'hidden',
            default: '={{(($self["customScopes"] && $self["enabledScopes"]) ? $self["enabledScopes"] : "' +
                defaultScopes.join(' ') +
