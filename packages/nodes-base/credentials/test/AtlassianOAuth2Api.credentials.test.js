...
describe('AtlassianOAuth2Api Credential', () => {
    ...
    it('should have correct credential metadata', () => {
        ...
        const accessTokenUrlProperty = atlassianOAuth2Api.properties.find((p) => p.name === 'accessTokenUrl');
        expect(accessTokenUrlProperty?.default).toBe('https://auth.atlassian.com/oauth/token');
        const authQueryParamsProperty = atlassianOAuth2Api.properties.find((p) => p.name === 'authQueryParameters');
        expect(authQueryParamsProperty?.default).toBe('audience=api.atlassian.com&prompt=consent');
        const authenticationProperty = atlassianOAuth2Api.properties.find((p) => p.name === 'authentication');
...
//# sourceMappingURL=AtlassianOAuth2Api.credentials.test.js.map