...
describe('SlackOAuth2Api Credential', () => {
    ...
    it('should not have a hardcoded bot scope field', () => {
        const scopeProperty = credential.properties.find((p) => p.name === 'scope' && p.default === 'chat:write');
    ...
    it('should use userScope in authQueryParameters when customScopes is true, otherwise use defaults', () => {
        const authQueryParamsProperty = credential.properties.find((p) => p.name === 'authQueryParameters');
        expect(authQueryParamsProperty?.default).toBe(`={{$self["customScopes"] ? "user_scope=" + $self["userScope"] : "user_scope=${userScopes.join(' ')}"}}`);
    });
});
//# sourceMappingURL=SlackOAuth2Api.credentials.test.js.map