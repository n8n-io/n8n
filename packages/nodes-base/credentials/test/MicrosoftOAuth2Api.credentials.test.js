...
describe('MicrosoftOAuth2Api Credential', () => {
    ...
    it('should have correct credential metadata', () => {
        ...
        expect(authUrlProperty?.default).toBe('https://login.microsoftonline.com/common/oauth2/v2.0/authorize');
        const accessTokenUrlProperty = microsoftOAuth2Api.properties.find((p) => p.name === 'accessTokenUrl');
        expect(accessTokenUrlProperty?.default).toBe('https://login.microsoftonline.com/common/oauth2/v2.0/token');
    });
    it('should request the Microsoft account chooser via authQueryParameters', () => {
        const authQueryParamsProperty = microsoftOAuth2Api.properties.find((p) => p.name === 'authQueryParameters');
    ...
    describe('certificate authentication', () => {
        const getProperty = (name) => microsoftOAuth2Api.properties.find((p) => p.name === name);
...
//# sourceMappingURL=MicrosoftOAuth2Api.credentials.test.js.map