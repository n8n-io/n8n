...
import { AtlassianServiceAccountApi, getAccessToken, } from '../AtlassianServiceAccountApi.credentials';
describe('AtlassianServiceAccountApi Credential', () => {
    ...
    describe('preAuthentication / getAccessToken', () => {
        ...
        it('trims whitespace from the client ID and secret', async () => {
            ...
            const body = new URLSearchParams(requestMock.mock.calls[0][0].body);
            expect(body.get('client_id')).toBe('client-id');
        });
        it.each([
        ...
        it.each([400, 401, 403])('maps a %d token response to a static credential error', async (status) => {
            requestMock.mockRejectedValue(Object.assign(new Error('Request failed'), { response: { status } }));
            const promise = getAccessToken(baseCredentials);
            await expect(promise).rejects.toBeInstanceOf(UserError);
            await expect(promise).rejects.toThrow('Atlassian rejected the service account credentials. Check the Client ID and Client Secret.');
        });
        it('rethrows non-auth token errors unchanged', async () => {
            requestMock.mockRejectedValue(Object.assign(new Error('socket hang up'), { response: { status: 502 } }));
        ...
        it('throws a static error when the response carries no access token', async () => {
            ...
            await expect(promise).rejects.toThrow('Atlassian authentication did not return an access token');
        });
    });
...
//# sourceMappingURL=AtlassianServiceAccountApi.credentials.test.js.map