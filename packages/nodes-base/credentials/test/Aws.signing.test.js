...
describe('AWS signing helpers (unit)', () => {
    ...
    describe('buildSmithyHttpRequest', () => {
        ...
        it('injects aws4-style content-type and content-length for a string body', () => {
            const req = buildSmithyHttpRequest({ ...base, headers: {}, body: 'a=1&b=2' });
        ...
        it('injects aws4-style content-type and content-length for a Buffer body', () => {
            ...
            const req = buildSmithyHttpRequest({ ...base, headers: {}, body: buf });
            expect(req.headers['content-type']).toBe('application/x-www-form-urlencoded; charset=utf-8');
...
describe('AWS signing (integration, real signer)', () => {
    ...
    function signedHeadersFrom(authorization) {
    ...
    describe('SES email endpoints', () => {
        ...
        describe('parity with the legacy aws4 signer', () => {
            ...
            it('produces the aws4 signature byte-for-byte for an SES v2 send', async () => {
                ...
                const smithySignature = signatureFrom(smithy.headers);
                expect(smithySignature).toMatch(/^[0-9a-f]{64}$/);
                expect(smithySignature).toBe(signatureFrom(legacy.headers));
            });
...
//# sourceMappingURL=Aws.signing.test.js.map