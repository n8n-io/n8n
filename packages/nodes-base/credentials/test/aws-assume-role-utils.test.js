...
function baseCredentials(overrides = {}) {
    return {
        region: 'us-east-1',
...
describe('assumeRole() — centralized validation', () => {
    it('throws when roleArn is missing', async () => {
        await expect(assumeRole(baseCredentials({ roleArn: '' }), 'us-east-1')).rejects.toThrow(UserError);
    ...
    it('omits ExternalId from params when externalId is absent', async () => {
        ...
        const sdkArg = mockFromTemporaryCredentials.mock.calls.at(-1)?.[0];
...
describe('assumeRole() — proxy-aware client', () => {
    ...
    it('wraps SDK-level errors in UserError', async () => {
        mockProvider.mockRejectedValue(new Error('AccessDenied: User is not authorized to assume role'));
        await expect(assumeRole(baseCredentials(), 'us-east-1')).rejects.toThrow(UserError);
        await expect(assumeRole(baseCredentials(), 'us-east-1')).rejects.toThrow('STS AssumeRole failed: AccessDenied: User is not authorized to assume role');
    });
});
//# sourceMappingURL=aws-assume-role-utils.test.js.map