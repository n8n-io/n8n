...
describe.each(CASES)('%s custom scopes', (name, credential, defaultScopes) => {
    const property = (propertyName) => credential.properties.find((p) => p.name === propertyName);
    ...
    it('should keep the scope hidden and default to the standard scopes when custom scopes are off', () => {
        ...
        expect(property('scope')?.default).toBe(`={{$self["customScopes"] ? $self["enabledScopes"] : "${defaultScopes}"}}`);
    });
});
//# sourceMappingURL=GoogleCustomScopes.credentials.test.js.map