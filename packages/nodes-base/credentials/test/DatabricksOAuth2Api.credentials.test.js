...
describe('DatabricksOAuth2Api Credential', () => {
    ...
    const property = (name) => databricksOAuth2Api.properties.find((p) => p.name === name);
    ...
    describe('OAuth URLs', () => {
        ...
        it('should strip the trailing slash from the credential test baseURL', () => {
            expect(databricksOAuth2Api.test.request.baseURL).toBe('={{$credentials.host.replace(/\\/$/, "")}}');
        });
    });
    describe('scope', () => {
        ...
        it('should stay hidden and force offline_access on the authorization code grant', () => {
            expect(field?.type).toBe('hidden');
            expect(field?.default).toBe('={{$self["customScopes"] ? ($self["grantType"] === "authorizationCode" ? (($self["userEnabledScopes"].trim() || "all-apis") + ($self["userEnabledScopes"].trim().split(" ").includes("offline_access") ? "" : " offline_access")) : ($self["enabledScopes"].trim() || "all-apis")) : ($self["grantType"] === "authorizationCode" ? "all-apis offline_access" : "all-apis")}}');
        });
        const evaluate = (customScopes, scopes, grantType) => {
            ...
            const expression = (field?.default).replace(/^=\{\{/, '').replace(/\}\}$/, '');
            // eslint-disable-next-line @typescript-eslint/no-implied-eval
            const evalScope = new Function('$self', `return (${expression});`);
            return evalScope({
                customScopes,
...
//# sourceMappingURL=DatabricksOAuth2Api.credentials.test.js.map