...
describe('MicrosoftEntraServicePrincipalApi Credential', () => {
    ...
    it('should have correct static properties', () => {
        ...
        expect(authentication?.options?.map((option) => ('value' in option ? option.value : ''))).toEqual(['clientSecret', 'certificate']);
        ...
        const privateKey = credential.properties.find((property) => property.name === 'privateKey');
        expect(privateKey?.displayOptions?.show?.authentication).toEqual(['certificate']);
        expect(privateKey?.typeOptions?.password).toBe(true);
        const certificate = credential.properties.find((property) => property.name === 'certificate');
        ...
        const setupNotice = credential.properties.find((property) => property.name === 'setupNotice');
        expect(setupNotice?.type).toBe('notice');
        const graphApiBaseUrl = credential.properties.find((property) => property.name === 'graphApiBaseUrl');
        expect(graphApiBaseUrl?.type).toBe('options');
        expect(graphApiBaseUrl?.options?.map((option) => ('value' in option ? option.value : ''))).toEqual([
    ...
    it('should have the expected credential test request shape', () => {
        ...
        expect(credential.test.request.baseURL).toBe('={{$credentials.graphApiBaseUrl || "https://graph.microsoft.com"}}');
    });
    describe('preAuthentication', () => {
        ...
        describe('certificate authentication', () => {
            ...
            const postedBody = () => new URLSearchParams(requestMock.mock.calls[0][0].body);
            it('signs a client_assertion instead of sending a client_secret', async () => {
                ...
                expect(requestMock.mock.calls[0][0].body).not.toContain('stale-secret-should-not-be-sent');
                ...
                const header = jsonParse(Buffer.from(headerSeg, 'base64url').toString('utf8'));
                expect(header.x5t).toBe(EXPECTED_X5T);
                const payload = jsonParse(Buffer.from(payloadSeg, 'base64url').toString('utf8'));
            ...
            it.each([
                ...
                await expect(callPreAuthentication(credentials)).rejects.toThrow('Microsoft Entra credentials are incomplete');
                expect(requestMock).not.toHaveBeenCalled();
            });
        });
        describe('sovereign login host derivation', () => {
            const expectTokenHost = async (graphApiBaseUrl, expectedTokenUrl) => {
                await callPreAuthentication({ ...baseCredentials, graphApiBaseUrl });
                expect(requestMock).toHaveBeenCalledWith(expect.objectContaining({ url: expectedTokenUrl }));
            };
            it('derives the China login host from the China Graph base', async () => {
                await expectTokenHost('https://microsoftgraph.chinacloudapi.cn', 'https://login.partner.microsoftonline.cn/tenant-id/oauth2/v2.0/token');
            });
            it('derives the US Gov login host from the US Gov Graph base', async () => {
                await expectTokenHost('https://graph.microsoft.us', 'https://login.microsoftonline.us/tenant-id/oauth2/v2.0/token');
            });
        ...
        describe('validation before any network call', () => {
            ...
            it('rejects an invalid tenantId before any token POST', async () => {
                ...
                await expect(callPreAuthentication(credentials)).rejects.toThrow(OperationalError);
                await expect(callPreAuthentication(credentials)).rejects.toThrow('Microsoft Entra tenant ID is not a valid GUID or domain');
                expect(requestMock).not.toHaveBeenCalled();
            });
            it('rejects an unrecognized graphApiBaseUrl before any token POST', async () => {
                ...
                await expect(callPreAuthentication(credentials)).rejects.toThrow(OperationalError);
                await expect(callPreAuthentication(credentials)).rejects.toThrow('Microsoft Entra Graph API base URL is not a recognized Microsoft cloud');
                expect(requestMock).not.toHaveBeenCalled();
            });
        });
        describe('malformed token response', () => {
            it.each([
                ...
                ['an empty object', {}],
            ])('rejects with an OperationalError carrying the static message for %s', async (_label, response) => {
                requestMock.mockResolvedValueOnce(response);
                const error = await callPreAuthentication(baseCredentials).catch((e) => e);
                expect(error).toBeInstanceOf(OperationalError);
                expect(error.message).toBe(STATIC_NO_TOKEN_MESSAGE);
            });
        });
    });
    ...
    describe('authenticate', () => {
        it('attaches the cached bearer token to the request', async () => {
            const result = await credential.authenticate({ accessToken: 'cached' }, { headers: {}, method: 'GET', url: '/v1.0/organization' });
        ...
        it('reuses the cached token without performing a token exchange', async () => {
            const result = await credential.authenticate({ ...baseCredentials, accessToken: 'cached' }, { headers: {}, method: 'GET', url: '/v1.0/organization' });
            expect(result.headers?.Authorization).toBe('Bearer cached');
            expect(requestMock).not.toHaveBeenCalled();
        });
...
//# sourceMappingURL=MicrosoftEntraServicePrincipalApi.credentials.test.js.map