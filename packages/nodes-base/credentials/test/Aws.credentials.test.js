...
describe('Aws Credential', () => {
    ...
    describe('authenticate', () => {
        ...
        it('should call sign with correct parameters', async () => {
            ...
            expect(MockSignatureV4).toHaveBeenCalledWith(expect.objectContaining({
                credentials: securityHeaders,
                region: 'eu-central-1',
            }));
            expect(mockSmithySignFn).toHaveBeenCalledWith(expect.objectContaining({
                hostname: 'sts.eu-central-1.amazonaws.com',
            }));
            expect(result.method).toBe('POST');
            expect(result.url).toBe('https://sts.eu-central-1.amazonaws.com/?Action=GetCallerIdentity&Version=2011-06-15');
        });
        it('should return correct options with custom endpoint', async () => {
            const customEndpoint = 'https://custom.endpoint.com';
            const result = await aws.authenticate({ ...credentials, customEndpoints: true, snsEndpoint: customEndpoint }, { ...requestOptions, url: '', baseURL: '', qs: { service: 'sns' } });
            expect(mockSmithySignFn).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should return correct options with temporary credentials', async () => {
            const result = await aws.authenticate({ ...credentials, temporaryCredentials: true, sessionToken: 'test-token' }, requestOptions);
            expect(MockSignatureV4).toHaveBeenCalledWith(expect.objectContaining({
        ...
        describe('Amazon Bedrock services', () => {
            it.each([
            ...
            ])('should sign $host requests with the Bedrock service namespace', async ({ host, path }) => {
                const result = await aws.authenticate(credentials, {
                    ...requestOptions,
                    baseURL: '',
                    url: `https://${host}${path}`,
                ...
                expect(mockSmithySignFn).toHaveBeenCalledWith(expect.objectContaining({ hostname: host, path }));
        ...
        describe('PrivateLink (vpce) endpoints', () => {
            ...
            ])('should sign vpce $host requests with the Bedrock service namespace', async ({ host, path, region }) => {
                const result = await aws.authenticate(credentials, {
                    ...requestOptions,
                    baseURL: '',
                    url: `https://${host}${path}`,
        ...
        describe('Dual-stack and FIPS endpoints', () => {
            ...
            ])('should sign $host requests with the s3 service and the host-derived region', async ({ host, path }) => {
                const result = await aws.authenticate(credentials, {
                    ...requestOptions,
                    baseURL: '',
                    url: `https://${host}${path}`,
        ...
        describe('China regions', () => {
            ...
            it('should use amazonaws.com.cn domain for cn-north-1 region', async () => {
                ...
                expect(mockSmithySignFn).toHaveBeenCalledWith(expect.objectContaining({ hostname: 's3.cn-north-1.amazonaws.com.cn' }));
                expect(MockSignatureV4).toHaveBeenCalledWith(expect.objectContaining({ region: 'cn-north-1' }));
            ...
            it('should handle custom endpoints for China regions', async () => {
                ...
                const result = await aws.authenticate({ ...chinaCredentials, customEndpoints: true, s3Endpoint: customEndpoint }, { ...requestOptions, url: '', baseURL: '', qs: { service: 's3' } });
                expect(mockSmithySignFn).toHaveBeenCalledWith(expect.objectContaining({ hostname: 'custom.china.endpoint.com.cn' }));
                expect(result.url).toBe(`${customEndpoint}/`);
            });
        ...
        describe('Regular regions (non-China)', () => {
            it('should use amazonaws.com domain for regular regions', async () => {
                ...
                expect(MockSignatureV4).toHaveBeenCalledWith(expect.objectContaining({ region: 'eu-central-1' }));
        ...
        describe('S3 vs non-S3 signer configuration', () => {
            it('configures the signer for S3 with checksum on and path escaping off', async () => {
                ...
                expect(MockSignatureV4).toHaveBeenCalledWith(expect.objectContaining({ service: 's3', applyChecksum: true, uriEscapePath: false }));
            });
            ...
            it('configures the signer for non-S3 with checksum off and path escaping on', async () => {
                ...
                expect(MockSignatureV4).toHaveBeenCalledWith(expect.objectContaining({ service: 'sqs', applyChecksum: false, uriEscapePath: true }));
            });
        });
...
//# sourceMappingURL=Aws.credentials.test.js.map