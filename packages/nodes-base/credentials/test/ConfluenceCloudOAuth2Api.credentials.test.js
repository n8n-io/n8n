...
describe('ConfluenceCloudOAuth2Api Credential', () => {
    ...
    const defaultScopes = (confluenceOAuth2Api.properties.find((p) => p.name === 'enabledScopes')?.default).split(' ');
    ...
    const resolvedDefault = (name) => resolvedProperties.find((p) => p.name === name)?.default;
    ...
    it('should resolve the extends chain', () => {
        const byName = (name) => resolvedProperties.find((p) => p.name === name);
        ...
        expect(byName('authQueryParameters')?.default).toBe('audience=api.atlassian.com&prompt=consent');
        ...
        expect(scope?.default).toBe('={{$self["customScopes"] ? $self["enabledScopes"] : "' + defaultScopes.join(' ') + '"}}');
    });
...
//# sourceMappingURL=ConfluenceCloudOAuth2Api.credentials.test.js.map