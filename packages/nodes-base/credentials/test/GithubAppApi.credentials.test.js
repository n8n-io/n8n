...
describe('GithubAppApi Credential', () => {
    ...
    describe('preAuthentication', () => {
        it('should create app JWT and return installation access token', async () => {
            ...
            expect(mockedSign).toHaveBeenCalledWith(expect.objectContaining({ iss: '12345' }), expect.any(String), expect.objectContaining({ algorithm: 'RS256' }));
        ...
        it('should throw a clear error when JWT creation fails', async () => {
            ...
            await expect(credential.preAuthentication.call(helper, baseCredentials)).rejects.toThrow('Failed to create GitHub App JWT. Verify that App ID and Private Key are valid.');
        ...
        it('should include status code and API message when token request fails', async () => {
            ...
            await expect(credential.preAuthentication.call(helper, baseCredentials)).rejects.toThrow('Failed to get GitHub App installation token (401): Bad credentials');
        });
        it('should throw when GitHub response has no token', async () => {
            ...
            await expect(credential.preAuthentication.call(helper, baseCredentials)).rejects.toThrow('GitHub did not return an installation token. Check App and Installation IDs.');
        });
    });
});
//# sourceMappingURL=GithubAppApi.credentials.test.js.map