...
describe('SnowflakeOAuth2Api Credential', () => {
    ...
    const createOAuthClient = (scopes) => new ClientOAuth2({
        clientId,
        clientSecret,
        accessTokenUri,
        authorizationUri,
    ...
    const mockTokenEndpoint = (code, responseScopes) => {
        nock(baseUrl)
    ...
    it('should have correct credential metadata', () => {
        ...
        const accessTokenUrlProperty = snowflakeOAuth2Api.properties.find((p) => p.name === 'accessTokenUrl');
        expect(accessTokenUrlProperty?.default).toContain('snowflakecomputing.com/oauth/token-request');
        const enabledScopesProperty = snowflakeOAuth2Api.properties.find((p) => p.name === 'enabledScopes');
...
//# sourceMappingURL=SnowflakeOAuth2Api.credentials.test.js.map