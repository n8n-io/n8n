...
describe('SalesforceJwtApi Credential', () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(Container, 'get').mockImplementation((token) => {
            if (token === OutboundHttp)
                return { requests: requestsMock };
    ...
    it('should have correct properties', () => {
        ...
        expect(credential.test.request.baseURL).toBe('={{$credentials?.myDomainUrl ? $credentials.myDomainUrl.replace(/\\/$/, "") : ($credentials?.environment === "sandbox" ? "https://test.salesforce.com" : "https://login.salesforce.com")}}');
    ...
    it('caches the access token in an expirable hidden field', () => {
        ...
        const instanceUrl = credential.properties.find((property) => property.name === 'instanceUrl');
    ...
    describe('resolveAuthUrl', () => {
        it('defaults to login.salesforce.com for production when My Domain URL is empty', () => {
            expect(resolveAuthUrl({ ...baseCredentials, environment: 'production', myDomainUrl: '' })).toBe('https://login.salesforce.com');
        });
        it('defaults to test.salesforce.com for sandbox when My Domain URL is empty', () => {
            expect(resolveAuthUrl({ ...baseCredentials, environment: 'sandbox', myDomainUrl: '' })).toBe('https://test.salesforce.com');
        });
        it('falls back to the default when My Domain URL is missing', () => {
            expect(resolveAuthUrl({ ...baseCredentials, environment: 'production' })).toBe('https://login.salesforce.com');
        });
        it('uses the My Domain URL override when provided', () => {
            expect(resolveAuthUrl({
                ...baseCredentials,
    ...
    describe('preAuthentication', () => {
        const callPreAuthentication = async (credentials) => await credential.preAuthentication.call(helpers, credentials);
        it('signs the JWT with the sandbox default audience when no My Domain URL is set', async () => {
            ...
            expect(mockedSign).toHaveBeenCalledWith(expect.objectContaining({ aud: 'https://test.salesforce.com' }), expect.any(String), expect.any(Object));
        ...
        it('signs the JWT with the production default audience when no My Domain URL is set', async () => {
            ...
            expect(mockedSign).toHaveBeenCalledWith(expect.objectContaining({ aud: 'https://login.salesforce.com' }), expect.any(String), expect.any(Object));
            expect(requestMock).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it("signs the JWT with the Spring '26 sandbox My Domain URL as audience (issue #28990)", async () => {
            ...
            expect(mockedSign).toHaveBeenCalledWith(expect.objectContaining({ aud: 'https://acme--sandbox.sandbox.my.salesforce.com' }), expect.any(String), expect.any(Object));
            expect(requestMock).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('signs the JWT with a production My Domain URL as audience', async () => {
            ...
            expect(mockedSign).toHaveBeenCalledWith(expect.objectContaining({ aud: 'https://acme.my.salesforce.com' }), expect.any(String), expect.any(Object));
            expect(requestMock).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('throws when the token response is missing the access token or instance URL', async () => {
            ...
            await expect(callPreAuthentication({ ...baseCredentials, environment: 'production', myDomainUrl: '' })).rejects.toThrow('Salesforce JWT authentication did not return an access token');
        });
    });
...
//# sourceMappingURL=SalesforceJwtApi.credentials.test.js.map