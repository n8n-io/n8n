...
describe('MicrosoftDynamicsOAuth2Api Credential', () => {
    ...
    it('should offer custom scopes prefilled with the defaults as placeholder tokens', () => {
        ...
        expect(property('enabledScopes')?.default).toBe('openid offline_access https://{subdomain}.{region}/.default');
    });
    it('should keep the scope hidden, substituting the tokens and falling back to the defaults when custom scopes are off or cleared', () => {
        ...
        expect(property('scope')?.default).toBe('={{(($self["customScopes"] && $self["enabledScopes"]) ? $self["enabledScopes"] : "openid offline_access https://{subdomain}.{region}/.default").replace(/\\{subdomain\\}/g, $self["subdomain"]).replace(/\\{region\\}/g, $self["region"])}}');
    });
});
//# sourceMappingURL=MicrosoftDynamicsOAuth2Api.credentials.test.js.map