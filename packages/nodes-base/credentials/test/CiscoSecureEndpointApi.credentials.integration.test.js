 ...
 */
function rewriteToLocalServer(realOutbound, server) {
    return {
        requests: (options) => {
...
describe('CiscoSecureEndpointApi Credential (integration)', () => {
    ...
    test('chains both token POSTs over a real socket and injects the final token', async () => {
        const result = await credential.authenticate({ region: 'eu.amp', clientId: 'client-id', clientSecret: 'client-secret' }, { headers: {}, method: 'GET', url: 'https://api.eu.amp.cisco.com/v3/organizations' });
        ...
        expect(new URLSearchParams(secureEndpointReq.body).get('grant_type')).toBe('client_credentials');
        expect(result.headers?.Authorization).toBe('Bearer secure-endpoint-token');
    });
});
//# sourceMappingURL=CiscoSecureEndpointApi.credentials.integration.test.js.map