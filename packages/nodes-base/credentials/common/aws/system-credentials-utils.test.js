...
const { getSystemCredentials, getRoleForServiceAccountCredentials, credentialsResolver } = systemCredentialsUtils;
const envGetter = (...args) => systemCredentialsUtils.envGetter(...args);
describe('system-credentials-utils', () => {
    let mockSecurityConfigInstance;
    ...
    describe('getSystemCredentials', () => {
        it('should throw UserError when AWS system credentials access is disabled', async () => {
            ...
            await expect(getSystemCredentials('us-east-1')).rejects.toThrow('Access to AWS system credentials disabled, contact your administrator.');
        });
        it('should return credentials from environment resolver', async () => {
            mockEnvGetter.mockImplementation((key) => {
                switch (key) {
    ...
    describe('getContainerMetadataCredentials', () => {
        ...
        it('should fetch credentials successfully with relative URI from envGetter', async () => {
            ...
            expect(global.fetch).toHaveBeenCalledWith('http://169.254.170.2/v2/credentials/test-uuid', expect.objectContaining({
                method: 'GET',
                headers: {
    ...
    describe('getPodIdentityCredentials', () => {
        ...
        it('should fetch credentials successfully with full URI from envGetter', async () => {
            ...
            expect(mockEnvGetter).toHaveBeenCalledWith('AWS_CONTAINER_CREDENTIALS_FULL_URI');
            expect(mockEnvGetter).toHaveBeenCalledWith('AWS_CONTAINER_AUTHORIZATION_TOKEN');
            expect(global.fetch).toHaveBeenCalledWith('https://eks-pod-identity.amazonaws.com/v1/credentials', expect.objectContaining({
                method: 'GET',
                headers: {
                    'User-Agent': 'n8n-aws-credential',
                },
        ...
        it('should read token from file when AWS_CONTAINER_AUTHORIZATION_TOKEN_FILE is set', async () => {
            ...
            expect(mockReadFile).toHaveBeenCalledWith('/var/run/secrets/pods.eks.amazonaws.com/serviceaccount/eks-pod-identity-token', 'utf-8');
            expect(global.fetch).toHaveBeenCalledWith('http://169.254.170.23/v1/credentials', expect.objectContaining({
                headers: {
                    'User-Agent': 'n8n-aws-credential',
    ...
    describe('getRoleForServiceAccountCredentials', () => {
        ...
        it('should fetch credentials successfully with role and token file from envGetter', async () => {
            ...
            expect(global.fetch).toHaveBeenCalledWith('https://sts.us-east-1.amazonaws.com', expect.objectContaining({
                method: 'POST',
                headers: {
                    'User-Agent': 'n8n-aws-credential',
            ...
            expect(global.fetch.mock.calls[0][1].body).toContain('RoleArn=arn%3Aaws%3Aiam%3A%3A123456789012%3Arole%2Ftest-role');
        });
        it('should return null when fetch fails', async () => {
            mockEnvGetter.mockImplementation((key) => {
                switch (key) {
        ...
        it('should use the China STS endpoint for cn- regions', async () => {
            ...
            expect(global.fetch).toHaveBeenCalledWith('https://sts.cn-north-1.amazonaws.com.cn', expect.objectContaining({ method: 'POST' }));
        });
        it('should use the GovCloud STS endpoint for us-gov- regions', async () => {
            mockSuccessfulStsResponse();
            await getRoleForServiceAccountCredentials('us-gov-west-1');
            expect(global.fetch).toHaveBeenCalledWith('https://sts.us-gov-west-1.amazonaws.com', expect.objectContaining({ method: 'POST' }));
        });
    });
});
//# sourceMappingURL=system-credentials-utils.test.js.map