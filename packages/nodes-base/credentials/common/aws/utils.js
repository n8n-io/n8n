...
import { isObjectEmpty, UserError, } from 'n8n-workflow';
import { getAwsDomain, regions } from './regions';
import { getSystemCredentials } from './system-credentials-utils';
// ── Private ──────────────────────────────────────────────────────────────────
const SUPPORTED_AWS_REGIONS = new Set(regions.map((r) => r.name));
/**
...
function buildMasterCredentials(credentials, region) {
    if (credentials.useSystemCredentialsForRole) {
        return async () => {
            ...
            if (!sys) {
                throw new UserError('System AWS credentials are required for role assumption. Please ensure AWS credentials are available via environment variables, instance metadata, or container role.');
            }
            return {
 ...
 */
export const awsCredentialsTest = {
    request: {
        baseURL: 
 ...
 */
export function assertSupportedAwsRegion(region) {
 ...
 */
export function validateBedrockEndpointOverride(override, region) {
 ...
 */
export function parseAwsUrl(url) {
...
function resolveServiceAndRegion(url, service, region) {
    ...
    if (parsed.region) {
        if (isSupportedAwsRegion(parsed.region)) {
            ...
            throw new UserError(`Unsupported AWS region "${parsed.region}" parsed from endpoint host ${url.hostname}`);
        }
    }
 ...
 */
export function uriEncodeS3Pathname(pathname) {
    return pathname
        .split('/')
        .map((segment) => {
 ...
 */
export function awsGetSignInOptionsAndUpdateRequest(requestOptions, credentials, path, method, service, region) {
    assertSupportedAwsRegion(region);
 ...
 */
export async function assumeRole(credentials, region) {
    assertSupportedAwsRegion(region);
...
export function splitPathAndQuery(pathWithSearch) {
...
export function splitHostPort(host) {
    if (host.startsWith('[')) {
...
export function buildSmithyHttpRequest(signOpts, method) {
...
export async function signOptions(requestOptions, signOpts, securityHeaders, url, method) {
