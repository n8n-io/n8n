...
function expectedSignature(canonicalRequest, region, service) {
    const stringToSign = [
        'AWS4-HMAC-SHA256',
        AMZ_DATE,
...
function getHeader(result, name) {
    ...
    if (!key)
        throw new Error(`missing expected header: ${name}`);
    return headers[key];
}
function parseAuthorization(auth) {
    ...
    if (!signature || !signedHeaders)
        throw new Error(`unparseable Authorization header: ${auth}`);
...
async function signS3Request(options) {
    ...
    const { signOpts, url } = awsGetSignInOptionsAndUpdateRequest(requestOptions, credentials, options.path, options.method, 's3', 'us-east-1');
...
describe('S3 path signing (real signers)', () => {
    ...
    it('signs the strictly UriEncoded object path — hand-computed golden vector', async () => {
        ...
        expect(auth).toContain(`Credential=${credentials.accessKeyId}/${DATE_STAMP}/us-east-1/s3/aws4_request`);
    });
    ...
    it('signs multipart request shapes identically to aws4 (query canonicalization)', async () => {
        ...
        for (const shape of shapes) {
            ...
            expect(smithyAuth.signedHeaders).toBe(legacyAuth.signedHeaders);
            expect(smithyAuth.signature).toBe(legacyAuth.signature);
            expect(smithy.url).toBe('https://s3.us-east-1.amazonaws.com/bucket/key%20%281%29.pdf?' +
                new URLSearchParams(shape.query).toString());
        }
    });
...
//# sourceMappingURL=s3-path-signing.test.js.map