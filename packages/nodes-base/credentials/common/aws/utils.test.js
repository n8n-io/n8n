...
import { assertSupportedAwsRegion, assumeRole, AWS_REGION_SHAPE_PATTERN, awsGetSignInOptionsAndUpdateRequest, parseAwsUrl, uriEncodeS3Pathname, validateBedrockEndpointOverride, } from './utils';
...
async function resolvedMaster() {
    const provider = lastCallArg().masterCredentials;
...
describe('assumeRole', () => {
    ...
    describe('with system credentials', () => {
        it('passes an async masterCredentials function (refreshable provider) to the SDK', async () => {
            ...
            vi.spyOn(systemCredentialsUtils, 'getSystemCredentials').mockResolvedValue(mockSystemCredentials);
            ...
            const masterProvider = arg.masterCredentials;
        ...
        it('omits sessionToken from masterCredentials when system creds have none', async () => {
            ...
            const masterProvider = lastCallArg().masterCredentials;
            await expect(masterProvider()).resolves.toEqual({
                accessKeyId: 'system-access-key',
                secretAccessKey: 'system-secret-key',
            });
        ...
        it('throws UserError when system credentials are not available (provider invocation)', async () => {
            ...
            await expect(masterProvider()).rejects.toThrow('System AWS credentials are required for role assumption');
        });
        it('surfaces the original UserError message without double-wrapping', async () => {
            ...
            mockProvider.mockRejectedValue(new UserError('System AWS credentials are required for role assumption.'));
            await expect(assumeRole(credentials, 'us-east-1')).rejects.toThrow(UserError);
            await expect(assumeRole(credentials, 'us-east-1')).rejects.toThrow('System AWS credentials are required for role assumption.');
            await expect(assumeRole(credentials, 'us-east-1')).rejects.not.toThrow('STS AssumeRole failed');
        });
    });
    describe('with manual STS credentials', () => {
        ...
        it('throws when STS access key ID is missing', async () => {
            ...
            await expect(assumeRole(credentials, 'us-east-1')).rejects.toThrow(UserError);
            await expect(assumeRole(credentials, 'us-east-1')).rejects.toThrow('STS Access Key ID is required when not using system credentials');
        });
        ...
        it('throws when STS secret access key is missing', async () => {
            ...
            await expect(assumeRole(credentials, 'us-east-1')).rejects.toThrow(UserError);
            await expect(assumeRole(credentials, 'us-east-1')).rejects.toThrow('STS Secret Access Key is required when not using system credentials');
        });
    ...
    describe('proxy handling', () => {
        ...
        it('builds a NodeHttpHandler with a proxy agent when a proxy is configured', async () => {
            ...
            await assumeRole(baseCredentials, 'us-east-1');
            expect(mockCreateHttpsProxyAgent).toHaveBeenCalledWith('https://sts.us-east-1.amazonaws.com', 'http://proxy.example.com:8080');
    ...
    describe('region handling', () => {
        ...
        it('uses the amazonaws.com.cn STS host for China regions', async () => {
            await assumeRole(credentialsFor('cn-north-1'), 'cn-north-1');
    ...
    describe('field validation', () => {
        ...
        it('throws when roleArn is missing', async () => {
            await expect(assumeRole({ ...baseCredentials, roleArn: '' }, 'us-east-1')).rejects.toThrow('Role ARN is required when assuming a role.');
        });
    ...
    describe('region validation', () => {
        it.each(['@example.com#', 'us-fake-1', '', 'us-east-1/foo', 'us-east-1#frag'])('rejects unsupported region value %s before calling the SDK', async (region) => {
            const credentials = {
                region: 'us-east-1',
                customEndpoints: false,
                useSystemCredentialsForRole: false,
            ...
            await expect(assumeRole(credentials, region)).rejects.toThrow(UserError);
            await expect(assumeRole(credentials, region)).rejects.toThrow('Unsupported AWS region');
...
describe('assertSupportedAwsRegion', () => {
    ...
    it.each([[undefined], [null], [0], [true], [{}], [['us-east-1']]])('rejects non-string region value %s', (region) => {
        expect(() => assertSupportedAwsRegion(region)).toThrow(UserError);
    });
});
...
describe('awsGetSignInOptionsAndUpdateRequest', () => {
    ...
    it('builds an endpoint URL for a supported region without throwing', () => {
        const { url, signOpts } = awsGetSignInOptionsAndUpdateRequest({ headers: {} }, baseCredentials, '/path', 'GET', 'lambda', 'us-east-1');
    ...
    it('uses the China domain for China regions', () => {
        const { url } = awsGetSignInOptionsAndUpdateRequest({ headers: {} }, { ...baseCredentials, region: 'cn-north-1' }, '/path', 'GET', 'lambda', 'cn-north-1');
    ...
    it.each([
        ...
        expect(() => awsGetSignInOptionsAndUpdateRequest({ headers: {} }, baseCredentials, '/path', 'GET', 'lambda', region)).toThrow(UserError);
    });
    it('signs and sends UI Query Parameters supplied via qs (not only qs.query)', () => {
        const { url, signOpts } = awsGetSignInOptionsAndUpdateRequest({
            ...
            headers: {},
        }, baseCredentials, '', 'GET', 'bedrock', 'us-east-1');
    ...
    it('preserves the STS GetCallerIdentity special case', () => {
        const { url } = awsGetSignInOptionsAndUpdateRequest({
            ...
            headers: {},
        }, baseCredentials, '', 'POST', 'sts', 'us-east-1');
        const sentUrl = new URL(url);
    ...
    describe('Bedrock control-plane endpoint override', () => {
        ...
        it('routes the request to the override host when set', () => {
            const { url, signOpts } = awsGetSignInOptionsAndUpdateRequest(modelListRequest, {
                ...baseCredentials,
        ...
        it('substitutes the {region} placeholder in the override', () => {
            const { url } = awsGetSignInOptionsAndUpdateRequest(modelListRequest, { ...baseCredentials, bedrockEndpoint: 'https://bedrock.{region}.example.internal' }, '', 'GET', '', 'us-east-1');
            expect(new URL(url).host).toBe('bedrock.us-east-1.example.internal');
        });
        it('prepends any base path on the override endpoint', () => {
            const { url } = awsGetSignInOptionsAndUpdateRequest(modelListRequest, { ...baseCredentials, bedrockEndpoint: 'https://proxy.internal/bedrock/' }, '', 'GET', '', 'us-east-1');
            const sent = new URL(url);
        ...
        it('hits the default control-plane host when no override is set', () => {
            const { url, signOpts } = awsGetSignInOptionsAndUpdateRequest(modelListRequest, baseCredentials, '', 'GET', '', 'us-east-1');
            expect(new URL(url).host).toBe('bedrock.us-east-1.amazonaws.com');
            expect(signOpts.service).toBe('bedrock');
            expect(signOpts.region).toBe('us-east-1');
        });
        it('rejects an override with a non-http scheme', () => {
            expect(() => awsGetSignInOptionsAndUpdateRequest(modelListRequest, { ...baseCredentials, bedrockEndpoint: 'ftp://bedrock.us-east-1.amazonaws.com' }, '', 'GET', '', 'us-east-1')).toThrow(UserError);
        });
    });
    describe('PrivateLink (vpce) endpoints', () => {
        ...
        it('throws when the vpce host has an unsupported region label', () => {
            expect(() => awsGetSignInOptionsAndUpdateRequest({
                baseURL: 'https://vpce-0abc123.sqs.not-a-region.vpce.amazonaws.com',
                url: '/',
                headers: {},
            }, baseCredentials, '', 'GET', '', 'us-east-1')).toThrow('not-a-region');
        });
        it('does not overwrite an explicitly supplied service with the URL-derived one', () => {
            const { signOpts } = awsGetSignInOptionsAndUpdateRequest({
                ...
                url: '/',
                headers: {},
            }, baseCredentials, '', 'GET', 'custom-service', 'us-west-2');
    ...
    describe('Dual-stack and FIPS endpoints', () => {
        ...
        it('signs a FIPS S3 host under the base s3 service name', () => {
            const { signOpts } = awsGetSignInOptionsAndUpdateRequest({
                ...
                headers: {},
            }, baseCredentials, '', 'GET', '', 'eu-central-1');
            expect(signOpts.service).toBe('s3');
            expect(signOpts.region).toBe('us-east-1');
        });
        ...
        it('strips the -fips suffix for services without a signing-name mapping', () => {
            const { signOpts } = awsGetSignInOptionsAndUpdateRequest({ uri: 'https://lambda-fips.us-east-1.amazonaws.com/', headers: {} }, baseCredentials, '', 'GET', '', 'us-east-1');
    ...
    describe('endpoints with qualifier labels left of the service', () => {
        ...
        it('signs a legacy region-less virtual-hosted S3 host as s3 with the credential region', () => {
            const { signOpts } = awsGetSignInOptionsAndUpdateRequest({ uri: 'https://mybucket.s3.amazonaws.com/key', headers: {} }, baseCredentials, '', 'GET', '', 'us-east-1');
            expect(signOpts.service).toBe('s3');
            expect(signOpts.region).toBe('us-east-1');
        });
        it('signs an S3 transfer-acceleration host under the s3 service name', () => {
            const { signOpts } = awsGetSignInOptionsAndUpdateRequest({ uri: 'https://mybucket.s3-accelerate.amazonaws.com/key', headers: {} }, baseCredentials, '', 'GET', '', 'us-east-1');
            expect(signOpts.service).toBe('s3');
        });
        it('signs a region-less legacy SQS global host as sqs with the credential region', () => {
            const { signOpts } = awsGetSignInOptionsAndUpdateRequest({ uri: 'https://queue.amazonaws.com/123456789012/my-queue', headers: {} }, baseCredentials, '', 'GET', '', 'us-east-1');
            expect(signOpts.service).toBe('sqs');
            expect(signOpts.region).toBe('us-east-1');
        });
        ...
        it('signs a caller-supplied qualified S3 access-point service under the s3 service name', () => {
            const { signOpts } = awsGetSignInOptionsAndUpdateRequest({ headers: {} }, baseCredentials, '', 'GET', 'myap-123456789012.s3-accesspoint', 'us-east-1');
            expect(signOpts.service).toBe('s3');
        });
    });
    describe('region labels that cannot be adopted on AWS endpoint hosts', () => {
        it('signs a region-shaped bucket on the legacy global S3 endpoint with the credential region', () => {
            const { signOpts } = awsGetSignInOptionsAndUpdateRequest({ uri: 'https://test-logs-3.s3.amazonaws.com/key', headers: {} }, baseCredentials, '', 'GET', '', 'us-east-1');
            expect(signOpts.service).toBe('s3');
            expect(signOpts.region).toBe('us-east-1');
        });
        it('signs a legacy dash-region S3 host with the embedded region', () => {
            const { signOpts } = awsGetSignInOptionsAndUpdateRequest({ uri: 'https://mybucket.s3-us-west-2.amazonaws.com/key', headers: {} }, baseCredentials, '', 'GET', '', 'us-east-1');
            expect(signOpts.service).toBe('s3');
            expect(signOpts.region).toBe('us-west-2');
        });
        it('throws for a mistyped region embedded in a legacy dash-region S3 label', () => {
            const call = () => awsGetSignInOptionsAndUpdateRequest({ uri: 'https://s3-us-esat-1.amazonaws.com/bucket/key', headers: {} }, baseCredentials, '', 'GET', '', 'us-east-1');
            expect(call).toThrow(UserError);
            expect(call).toThrow('us-esat-1');
        });
        it('throws for a mistyped region label', () => {
            const call = () => awsGetSignInOptionsAndUpdateRequest({ uri: 'https://lambda.us-esat-1.amazonaws.com/', headers: {} }, baseCredentials, '', 'GET', '', 'us-east-1');
            expect(call).toThrow(UserError);
            expect(call).toThrow('us-esat-1');
        });
        it('throws for a region-shaped label the region list does not include yet', () => {
            const call = () => awsGetSignInOptionsAndUpdateRequest({ uri: 'https://lambda.eu-central-9.amazonaws.com/', headers: {} }, baseCredentials, '', 'GET', '', 'us-east-1');
            expect(call).toThrow(UserError);
            expect(call).toThrow('eu-central-9');
        });
        it('throws for an unsupported region slot even when a bucket label is a supported region', () => {
            const call = () => awsGetSignInOptionsAndUpdateRequest({ uri: 'https://us-east-1.s3.eu-central-9.amazonaws.com/key', headers: {} }, baseCredentials, '', 'GET', '', 'us-east-1');
            expect(call).toThrow(UserError);
            expect(call).toThrow('eu-central-9');
        });
        it('throws for a not-yet-supported region with a four-letter partition prefix', () => {
            const call = () => awsGetSignInOptionsAndUpdateRequest({ uri: 'https://lambda.eusc-de-east-1.amazonaws.com/', headers: {} }, baseCredentials, '', 'GET', '', 'us-east-1');
            expect(call).toThrow(UserError);
            expect(call).toThrow('eusc-de-east-1');
        });
        it.each(['https://lambda.us-east1.amazonaws.com/', 'https://foo.bar.amazonaws.com/'])('keeps the credential region without throwing when no label is region-shaped (%s)', (uri) => {
            const { signOpts } = awsGetSignInOptionsAndUpdateRequest({ uri, headers: {} }, baseCredentials, '', 'GET', '', 'us-east-1');
            expect(signOpts.region).toBe('us-east-1');
        });
    });
    describe('custom (non-AWS) endpoint hosts', () => {
        it('keeps the credential region when the host label is not a region (uri branch)', () => {
            const { signOpts } = awsGetSignInOptionsAndUpdateRequest({
                ...
                headers: {},
            }, baseCredentials, '', 'GET', 'execute-api', 'us-east-1');
            expect(signOpts.region).toBe('us-east-1');
        });
...
describe('validateBedrockEndpointOverride', () => {
    it('accepts an https URL and returns it without a trailing slash', () => {
        expect(validateBedrockEndpointOverride('https://bedrock.us-east-1.amazonaws.com', 'us-east-1')).toBe('https://bedrock.us-east-1.amazonaws.com');
    });
    it('accepts an http URL (for LocalStack / on-prem gateways)', () => {
        expect(validateBedrockEndpointOverride('http://localhost:4566', 'us-east-1')).toBe('http://localhost:4566');
    });
    it('substitutes the {region} placeholder', () => {
        expect(validateBedrockEndpointOverride('https://bedrock-runtime.{region}.example.internal', 'eu-west-3')).toBe('https://bedrock-runtime.eu-west-3.example.internal');
    });
    it('strips a trailing slash but preserves a meaningful base path', () => {
        expect(validateBedrockEndpointOverride('https://proxy.internal/bedrock/', 'us-east-1')).toBe('https://proxy.internal/bedrock');
    });
    it.each(['ftp://bedrock.us-east-1.amazonaws.com', 'file:///etc/passwd', 'ws://host'])('rejects the non-http(s) scheme %s', (endpoint) => {
    ...
    it('rejects an unsupported region before substitution', () => {
        expect(() => validateBedrockEndpointOverride('https://bedrock.{region}.amazonaws.com', 'us-fake-1')).toThrow(UserError);
    });
});
describe('uriEncodeS3Pathname', () => {
    ...
    it('leaves unreserved characters and slash separators untouched', () => {
        expect(uriEncodeS3Pathname('/bucket/Report_Final-1.2~ok/file.pdf')).toBe('/bucket/Report_Final-1.2~ok/file.pdf');
    });
    ...
    it('matches the legacy aws4 canonical encoding, except + which is preserved as %2B', () => {
        ...
        const aws4Canonical = (pathname) => pathname
            .split('/')
            .map((piece) => encodeURIComponent(decodeURIComponent(piece.replace(/\+/g, ' '))).replace(/[!'()*]/g, (c) => `%${c.charCodeAt(0).toString(16).toUpperCase()}`))
            .join('/')
            .replace(/%2F/g, '/');
        const plusFreeKeys = [
...
describe('awsGetSignInOptionsAndUpdateRequest — S3 object-path canonicalization', () => {
    ...
    it('signs and sends the strictly encoded path for the default S3 endpoint', () => {
        const { signOpts, url } = awsGetSignInOptionsAndUpdateRequest({ headers: {} }, credentials, '/bucket/report (1)+final&v=2!.pdf', 'PUT', 's3', 'us-east-1');
    ...
    it('canonicalizes virtual-hosted-style requests (bucket.s3 signs under s3)', () => {
        const { signOpts, url } = awsGetSignInOptionsAndUpdateRequest({ headers: {} }, credentials, '/report:2026-07-22T10:00.pdf', 'PUT', 'mybucket.s3', 'eu-central-1');
        expect(signOpts.service).toBe('s3');
    ...
    it('encodes the path but leaves query parameters to the signer (multipart shapes)', () => {
        const { signOpts, url } = awsGetSignInOptionsAndUpdateRequest({ headers: {}, qs: { query: { uploads: '' } } }, credentials, '/bucket/key (1).pdf', 'POST', 's3', 'us-east-1');
    ...
    it('canonicalizes paths for a custom s3Endpoint (S3-compatible providers)', () => {
        const { signOpts, url } = awsGetSignInOptionsAndUpdateRequest({ headers: {} }, { ...credentials, s3Endpoint: 'https://minio.internal:9000' }, '/bucket/a(1).pdf', 'PUT', 's3', 'us-east-1');
    ...
    it('leaves non-S3 service paths untouched (Lambda ARN colons stay raw)', () => {
        ...
        const { signOpts, url } = awsGetSignInOptionsAndUpdateRequest({ headers: {} }, credentials, path, 'POST', 'lambda', 'us-east-1');
    ...
    it('does not change the signed path for keys that already worked (spaces, unicode)', () => {
        const { signOpts } = awsGetSignInOptionsAndUpdateRequest({ headers: {} }, credentials, '/bucket/my réport 中.pdf', 'GET', 's3', 'us-east-1');
        // Identical to the WHATWG pathname this function produced before the fix.
        expect(signOpts.path).toBe(new URL('https://s3.us-east-1.amazonaws.com/bucket/my réport 中.pdf').pathname);
    });
});
