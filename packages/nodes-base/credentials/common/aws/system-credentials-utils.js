...
import { getAwsDomain } from './regions';
import { resolveContainerMetadataViaSdk, resolveEnvironmentViaSdk, resolveInstanceMetadataViaSdk, resolvePodIdentityViaSdk, resolveWebIdentityViaSdk, usesSdk, } from './system-credentials-sdk';
export const envGetter = (key) => process.env[key];
 ...
 */
export async function getSystemCredentials(region) {
...
async function getEnvironmentCredentials() {
    if (usesSdk('environment'))
        return await resolveEnvironmentViaSdk();
...
async function getInstanceMetadataCredentials() {
    if (usesSdk('instanceMetadata'))
        return await resolveInstanceMetadataViaSdk();
    try {
...
async function getContainerMetadataCredentials() {
    if (usesSdk('containerMetadata'))
        return await resolveContainerMetadataViaSdk();
    try {
...
async function getPodIdentityCredentials() {
    if (usesSdk('podIdentity'))
        return await resolvePodIdentityViaSdk();
 ...
 */
export async function getRoleForServiceAccountCredentials(region) {
    if (usesSdk('roleForServiceAccount'))
        return await resolveWebIdentityViaSdk(region);
    ...
    try {
        ...
        const credentialsData = data?.AssumeRoleWithWebIdentityResponse?.AssumeRoleWithWebIdentityResult?.Credentials;
