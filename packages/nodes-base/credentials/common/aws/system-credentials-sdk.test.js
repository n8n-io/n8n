...
global.fetch = vi.fn();
const { mockContainer, MockSecurityConfig, fromEnv, fromWebToken, fromHttp, fromContainerMetadata, fromInstanceMetadata, } = vi.hoisted(() => {
...
describe('system-credentials-sdk', () => {
    ...
    describe('per-source SDK resolvers', () => {
        ...
        describe('roleForServiceAccount (IRSA)', () => {
            beforeEach(() => {
                mockEnvGetter.mockImplementation((key) => key === 'AWS_ROLE_ARN'
            ...
            it('reads and trims the token, then resolves via fromWebToken with the pinned session name', async () => {
                ...
                expect(mockReadFile).toHaveBeenCalledWith('/var/run/secrets/eks.amazonaws.com/serviceaccount/token', 'utf-8');
        ...
        describe('containerMetadata (ECS)', () => {
            it('uses fromContainerMetadata with the 2s/no-retry config', async () => {
                mockEnvGetter.mockImplementation((key) => key === 'AWS_CONTAINER_CREDENTIALS_RELATIVE_URI' ? '/v2/credentials/uuid' : undefined);
        ...
        describe('mapping and error handling', () => {
            it('omits the session token when the SDK identity has none', async () => {
                fromInstanceMetadata.mockReturnValue(vi.fn().mockResolvedValue({
                    accessKeyId: 'a',
                    secretAccessKey: 'b',
                }));
                const result = await resolveInstanceMetadataViaSdk();
            ...
            it('returns null (never throws) when the provider rejects', async () => {
                fromInstanceMetadata.mockReturnValue(vi.fn().mockRejectedValue(new Error('metadata unavailable')));
    ...
    describe('getSystemCredentials (SDK path)', () => {
        ...
        it('resolves the IRSA source via the SDK', async () => {
            mockEnvGetter.mockImplementation((key) => key === 'AWS_ROLE_ARN' || key === 'AWS_WEB_IDENTITY_TOKEN_FILE' ? 'set' : undefined);
            mockReadFile.mockResolvedValue('jwt-token\n');
            const result = await getSystemCredentials('us-east-1');
    ...
    describe('SDK/legacy parity (containerMetadata ECS auth header)', () => {
        ...
        beforeEach(() => {
            mockEnvGetter.mockImplementation((key) => {
                if (key === 'AWS_CONTAINER_CREDENTIALS_RELATIVE_URI')
                    return RELATIVE_URI;
                if (key === 'AWS_CONTAINER_AUTHORIZATION_TOKEN')
                    return AUTH_TOKEN;
                return undefined;
            });
        });
        it('legacy path sends Authorization: Bearer <token>', async () => {
            ...
            expect(global.fetch).toHaveBeenCalledWith(`http://169.254.170.2${RELATIVE_URI}`, expect.objectContaining({
    ...
    describe('SDK/legacy parity (environment source)', () => {
        ...
        ])('resolves identically via SDK and legacy (%s)', async (_label, rawAccessKeyId, rawSecretAccessKey, rawSessionToken) => {
            mockEnvGetter.mockImplementation((key) => {
                if (key === 'AWS_ACCESS_KEY_ID')
                    return rawAccessKeyId;
                if (key === 'AWS_SECRET_ACCESS_KEY')
...
//# sourceMappingURL=system-credentials-sdk.test.js.map