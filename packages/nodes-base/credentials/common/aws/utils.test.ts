...
describe('assumeRole', () => {
	...
	describe('proxy handling', () => {
		...
		it('omits requestHandler when no proxy is configured', async () => {
			mockResolveProxyUrl.mockReturnValue(undefined);
			await assumeRole(baseCredentials, 'us-east-1');

			expect(mockCreateHttpsProxyAgent).not.toHaveBeenCalled();
			expect(MockNodeHttpHandler).not.toHaveBeenCalled();
			expect(lastCallArg().clientConfig.requestHandler).toBeUndefined();
		});
	...
	describe('field validation', () => {
		...
		it('throws when roleSessionName is missing', async () => {
			await expect(
				assumeRole({ ...baseCredentials, roleSessionName: '' }, 'us-east-1'),
			).rejects.toThrow('Role Session Name is required when assuming a role.');
		});
	});
