...
const LOGIN_HOSTS_BY_GRAPH_URL = {
...
export async function getAccessToken(credentials) {
    const { authentication, tenantId, clientId, clientSecret, privateKey, certificate, graphApiBaseUrl, } = readCredentials(credentials);
    ...
    const hasAuthSecret = authentication === 'certificate' ? Boolean(privateKey && certificate) : Boolean(clientSecret);
    ...
    if (normalizedBaseUrl && !Object.hasOwn(LOGIN_HOSTS_BY_GRAPH_URL, normalizedBaseUrl)) {
        throw new OperationalError('Microsoft Entra Graph API base URL is not a recognized Microsoft cloud');
    }
    ...
    if (authentication === 'certificate') {
        ...
        body.append('client_assertion', buildClientAssertion({ clientId, accessTokenUri: tokenUrl, privateKey, certificate }));
    }
    else {
...
export class MicrosoftEntraServicePrincipalApi {
    ...
    properties = [
        ...
        {
            displayName: 'App-only access uses application permissions that an admin must consent to on the app registration. The connection test reads the organization via Microsoft Graph, so the app needs Organization.Read.All (or Directory.Read.All) for the test to pass.',
            name: 'setupNotice',
            type: 'notice',
            default: '',
        },
...
//# sourceMappingURL=MicrosoftEntraServicePrincipalApi.credentials.js.map