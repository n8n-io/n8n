...
export function formatDiscoveryConsole(report: DiscoveryReport, verbose: boolean): void {
	const { specs } = report;
	...
	const standardCount = specs.filter((s) => s.capabilities.length === 0).length;

	console.error(`\nDiscovered ${totalFiles} active spec files`);
	console.error(`  Standard: ${standardCount}`);

	if (Object.keys(capabilityCounts).length > 0) {
		console.error('  Capabilities:');
		for (const [cap, count] of Object.entries(capabilityCounts).sort()) {
			console.error(`    ${cap}: ${count}`);
		}
	}

	if (report.skipTags.length > 0) {
		console.error(`  Skip tags: ${report.skipTags.join(', ')}`);
	}

	if (verbose) {
		console.error('\nSpecs:');
		for (const spec of specs) {
			const caps = spec.capabilities.length > 0 ? ` [${spec.capabilities.join(', ')}]` : '';
			console.error(`  ${spec.path}${caps}`);
		}
	}
