...
import { fail, ok, passLine, phase, step, summary } from './harness';
...
export async function runUpgradeCycle(ctx: CycleContext, images: CycleImages): Promise<void> {
	...
	let api = await bootStack(ctx, {
		image: images.from,
		rotationFlag: false,
		label: 'the old release',
	});
	const fromVersion = await readInstanceVersion(ctx);
	step(ctx, `FROM version: ${fromVersion}`);
	await api.createOwner();
	step(ctx, 'creating credential A (the seed data)');
	const credA = await api.createCredential('encryption-cycle cred A (seeded on FROM)', SECRET_A);
	ok(ctx, `credential A = ${credA}`);
	await api.assertDecrypts(credA, SECRET_A, 'baseline on FROM');
	const seeded = await seedJourneys(ctx, api, credA, 'encryption-cycle cred A (seeded on FROM)');
	await assertJourneys(ctx, api, seeded, 'baseline on FROM');

	phase(ctx, 'P2 upgrade');
	...
	const credB = await api.createCredential(
...
export async function runRotationCycle(ctx: CycleContext, images: CycleImages): Promise<void> {
	...
	const seeded = await seedJourneys(ctx, api, credA, 'rotation-cycle cred A (initial key)');
	...
	const credB = await api.createCredential('rotation-cycle cred B (after 1st rotate)', SECRET_B);
	...
	const key3 = await api.rotateKey(key2);
	ok(ctx, `rotated: ${key2} -> ${key3}`);
	step(ctx, 'creating credential C (must use the NEWEST key id)');
	const credC = await api.createCredential('rotation-cycle cred C (after 2nd rotate)', SECRET_C);
	...
	await api.login();
	step(ctx, 'creating credential D (the restarted instance must keep writing with the active key)');
