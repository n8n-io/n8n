...
├── kubectl port-forward svc/n8n-main <port>:5678
└── Docker
...
Playwright ──── http://localhost:<port> ──→ kubectl port-forward ──→ n8n Pod
```

1. `@testcontainers/k3s` starts K3s inside a Docker container
...
4. Host `kubectl port-forward` tunnels traffic from a local port to the n8n service
5. Playwright tests connect via `N8N_BASE_URL=http://localhost:<port>`
...
If the port-forward dies, restart the stack. Check that kubectl is installed:
```bash
