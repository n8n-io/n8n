...
Accessibility-enabled runs write `test-results/a11y-report.html`. Violations are
non-blocking by default. Set `A11Y_VIOLATION_THRESHOLD` to a non-negative integer
to fail a run whose total violation count exceeds that value.
