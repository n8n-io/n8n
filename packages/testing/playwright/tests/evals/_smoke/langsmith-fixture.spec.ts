...
test('traced fixture forwards the wrapped function return value', async ({ traced }) => {
	const result = await traced('smoke-success', async () => 'ok');
	expect(result).toBe('ok');
});

test('traced fixture propagates thrown errors', async ({ traced }) => {
	await expect(
		traced('smoke-failure', async () => Promise.reject(new Error('intentional smoke failure'))),
...
test('traced fixture emits one run-id annotation per call', async ({ traced }, testInfo) => {
	...
	await traced('multi-1', async () => 'one');
	await traced('multi-2', async () => 'two');
	await traced('multi-3', async () => 'three');
