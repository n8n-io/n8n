...
test.describe('Webhook Trigger node', () => {
	...
	test('should listen for a GET request and respond with Respond to Webhook node', async ({
		...
		// Save and activate the workflow for production webhook
		await n8n.canvas.clickSaveWorkflowButton();
		await n8n.canvas.activateWorkflow();

		const response = await n8n.ndv.makeWebhookRequest(`/webhook/${webhookPath}`, {
			method: 'GET',
		});
	...
	test.skip('should listen for a GET request and respond with custom status code 201', async ({
		n8n,
	}) => {
		// TODO: This test needs to be updated to work with the current webhook node version
		// The responseCode parameter is handled differently in newer versions
		const webhookPath = nanoid();
		...
		const response = await n8n.ndv.makeWebhookRequest(`/webhook-test/${webhookPath}`);
		expect(response.status()).toBe(200); // Default status code
	});
	...
	test('should listen for a GET request with Header Authentication', async ({ n8n, api }) => {
		...
		const correctHeaderResponse = await n8n.ndv.makeWebhookRequest(`/webhook-test/${webhookPath}`, {
			headers: { test: 'test' },
		});
