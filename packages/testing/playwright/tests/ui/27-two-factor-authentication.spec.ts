...
test.describe('Two-factor authentication @auth:none @db:reset', () => {
	...
	test('Should be able to login with MFA code', async ({ n8n }) => {
		...
		await n8n.mfaSetupModal.waitForHidden();

		await n8n.mfaComposer.loginWithMfaCode(email, password, actualSecret);
		await n8n.sideBar.clickSignout();
	});
	...
	test('Should be able to login with MFA recovery code', async ({ n8n }) => {
		...
		const { secret: actualSecret, recoveryCode: extractedRecoveryCode } =
			await n8n.settings.enableMfaWithRecovery();
