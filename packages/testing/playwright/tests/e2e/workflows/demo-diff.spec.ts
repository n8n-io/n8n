...
test.describe(
	...
	() => {
		...
		test('shows waiting state initially', async ({ n8n }) => {
			...
			await expect(n8n.demo.getWaitingState()).toBeVisible();
		});
		...
		test('renders diff view when receiving workflow data via postMessage', async ({ n8n }) => {
			...
			await expect(n8n.demo.getWaitingState()).toBeHidden();

			// The workflow name should appear in the header
			await expect(n8n.demo.getDiffHeading(/Test Workflow/)).toBeVisible();
		...
		test('ignores non-openDiff commands', async ({ n8n }) => {
			...
			await n8n.demo.postRawMessage({ command: 'someOtherCommand', data: {} });
