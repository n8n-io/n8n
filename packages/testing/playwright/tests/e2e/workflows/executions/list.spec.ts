...
test.describe('Workflow Executions', () => {
	...
	test.describe('execution timing', () => {
		...
		// Skip: Wait node webhooks now require HMAC signature validation.
		// To resume a waiting execution via webhook, the URL must include a valid signature
		// generated by the instance's HMAC secret. External tests cannot generate valid signatures.
		// TODO: Update test workflow to output the signed resume URL from $execution.resumeUrl
		test.skip('should preserve execution start time after resuming from wait node', async ({
			api,
		}) => {
			const { webhookPath, workflowId, createdWorkflow } =
			...
			const resumeUrl = `/webhook-waiting/${execution.id}`;
