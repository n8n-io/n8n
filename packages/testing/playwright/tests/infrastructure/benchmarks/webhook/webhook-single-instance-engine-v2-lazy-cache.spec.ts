...
test.describe(
	'What is the single-instance webhook ingestion ceiling with execution engine v2 and lazy expression isolates?',
	{
		tag: '@bench:webhook',
		annotation: [
			...
			{
				type: 'question',
				description: 'webhook-single-instance-engine-v2-vm-lazy-cache',
			},
		],
	},
	() => {
		test(`Async webhook + 1 noop, 1KB payload, ${SINGLE_INSTANCE_WEBHOOK_CONNECTIONS} connections × ${SINGLE_INSTANCE_WEBHOOK_DURATION_SECONDS}s (1 main, no workers, engine v2, VM lazy + compile cache)`, async ({
			api,
			services,
			backendUrl,
		}, testInfo) => {
