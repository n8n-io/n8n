...
import type { BaseRule } from '../rules/base-rule';
import type { JanitorReport, RuleResult, Severity, RunOptions, RuleConfig } from './types';
...
export class RuleRunner {
	...
	private getFilesForRule(project: Project, rule: BaseRule, targetFiles?: string[]): SourceFile[] {
		if (targetFiles && targetFiles.length > 0) {
			...
			return targetFiles
				.map((f) => {
					const absolutePath = path.isAbsolute(f) ? f : path.join(PLAYWRIGHT_ROOT, f);
