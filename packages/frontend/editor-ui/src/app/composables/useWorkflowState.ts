...
export function useWorkflowState() {
	...
	function setWorkflowExecutionData(workflowResultData: IExecutionResponse | null) {
		...
		if (workflowResultData === null) {
			...
			executionStateStore.clearDisplayedExecution();
		} else if (workflowResultData.id === IN_PROGRESS_EXECUTION_ID) {
			executionStateStore.setPendingExecution(workflowResultData);
			...
			executionStateStore.trackExecutionId(workflowResultData.id);
			useExecutionDataStore(createExecutionDataId(workflowResultData.id)).setExecution(
				workflowResultData,
			);
			if (typeof executionStateStore.activeExecutionId !== 'string') {
				executionStateStore.setPendingExecution(null);
				executionStateStore.setActiveExecutionId(undefined);
				executionStateStore.setDisplayedExecutionId(workflowResultData.id);
			}
		}
	}
	...
	function markExecutionAsStopped(stopData?: IExecutionsStopData) {
		...
		if (typeof activeExecutionId === 'string') {
			...
			executionDataStore.markAsStopped(stopData);
			if (stopData) {
				executionStateStore.applyStopDataToPendingExecution(stopData);
			}
		} else {
			...
			const displayedExecutionId = executionStateStore.displayedExecutionId;
