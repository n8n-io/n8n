...
describe('useCanvasOperations', () => {
	...
	function mockConnectionMutations(connections: IConnections) {
		vi.mocked(workflowDocumentStoreInstance.addConnection).mockImplementation(({ connection }) => {
			const [source, target] = connection;
			const sourceConnections = (connections[source.node] ??= {});
			const typeConnections = (sourceConnections[source.type] ??= []);
			const outputConnections = (typeConnections[source.index] ??= []);
			outputConnections.push(target);
		});
		vi.mocked(workflowDocumentStoreInstance.removeConnection).mockImplementation(
			({ connection }) => {
				const [source, target] = connection;
				const outputConnections = connections[source.node]?.[source.type]?.[source.index];
				if (!outputConnections) return;

				connections[source.node][source.type][source.index] = outputConnections.filter(
					(candidate) =>
	...
	function mockNodeGroupMutations(group: IWorkflowGroup) {
		vi.mocked(workflowDocumentStoreInstance.getGroupById).mockImplementation((id) =>
		...
		vi.mocked(workflowDocumentStoreInstance.addNodesToGroup).mockImplementation((id, nodeIds) => {
			...
			group.nodeIds.push(...nodeIds.filter((nodeId) => !group.nodeIds.includes(nodeId)));
		});
	}
	...
	describe('createConnectionToLastInteractedWithNode - grouped edge insertion', () => {
		it('should extend the source group when inserting from its boundary edge add action', async () => {
			const uiStore = mockedStore(useUIStore);
			...
			const connections = createConnectionsBySource(workflowConnection(sourceNode, targetNode));
			const { workflowDocumentStore } = setupGroupedCanvas({
			...
			mockConnectionMutations(connections);
			mockNodeGroupMutations(group);
			const addNodesToGroupSpy = vi.spyOn(workflowDocumentStore, 'addNodesToGroup');
			...
			await addNodesAndConnections([insertedNode], [], { nodeGroupId: group.id });
	...
	describe('addNodesAndConnections - requested group extension', () => {
		function setupGroupExtension({
			emptyGroup = false,
			...
			const sourceNode = emptyGroup
				...
				: createGroupedNode('source', 'Source', [0, 0]);
			const addedNodeTypeDescription = invalidCandidate
				? mockNodeTypeDescription({
				...
				: nodeTypeDescription;
			const addedNode = createTestNode({
				id: 'added',
				name: 'Added',
				type: addedNodeTypeDescription.name,
				position: [250, 0],
			});
			const helperNode = withHelper
				? createTestNode({
						id: 'helper',
						name: 'Helper',
						type: addedNodeTypeDescription.name,
			...
			const { workflowDocumentStore } = setupGroupedCanvas({
				nodes: [sourceNode],
				lookupNodes: [sourceNode, addedNode, ...(helperNode ? [helperNode] : [])],
				connections,
				groups: [group],
				nodeTypeDescription,
			});
			...
			if (invalidCandidate) {
				const nodeTypesStore = mockedStore(useNodeTypesStore);
				nodeTypesStore.nodeTypes[addedNodeTypeDescription.name] = {
					1: addedNodeTypeDescription,
				};
				nodeTypesStore.getNodeType = vi.fn((type) =>
					type === addedNodeTypeDescription.name ? addedNodeTypeDescription : nodeTypeDescription,
				);
			}
		...
		it('adds the new node to a regular group', async () => {
			const toast = useToast();
			const { addedNode, group, sourceNode, workflowDocumentStore } = setupGroupExtension();
			const addNodesToGroupSpy = vi.spyOn(workflowDocumentStore, 'addNodesToGroup');

			const { addNodesAndConnections } = useCanvasOperations();
			await addNodesAndConnections([addedNode], [], { nodeGroupId: group.id });

			expect(addNodesToGroupSpy).toHaveBeenCalledWith(group.id, [addedNode.id]);
			expectConnectionAdded(sourceNode, addedNode);
			expect(toast.showToast).not.toHaveBeenCalledWith(
				expect.objectContaining({ title: "'Group 1' extended" }),
			);
		});

		it('adds auto-added helper nodes from the same creator result to the group', async () => {
			const { addedNode, group, helperNode, sourceNode, workflowDocumentStore } =
				setupGroupExtension({ withHelper: true });
			if (!helperNode) throw new Error('Expected a helper node');
			mockConnectionMutations(workflowDocumentStore.connectionsBySourceNode);
			vi.mocked(workflowDocumentStore.addNode).mockImplementation((node) => {
				workflowDocumentStore.allNodes.push(node);
			});
			const addNodesToGroupSpy = vi.spyOn(workflowDocumentStore, 'addNodesToGroup');

			const { addNodesAndConnections } = useCanvasOperations();
			await addNodesAndConnections(
				[addedNode, { ...helperNode, isAutoAdd: true }],
				[{ from: { nodeIndex: 0 }, to: { nodeIndex: 1 } }],
				{ nodeGroupId: group.id },
			);

			expect(addNodesToGroupSpy).toHaveBeenCalledWith(group.id, [addedNode.id, helperNode.id]);
			expectConnectionAdded(sourceNode, addedNode);
			expectConnectionAdded(addedNode, helperNode);
		});

		it('replaces an empty-group anchor with the new node', async () => {
			const historyStore = mockedStore(useHistoryStore);
			const {
				addedNode,
				group,
				...
				uiStore,
				workflowDocumentStore,
			} = setupGroupExtension({ emptyGroup: true });
			vi.mocked(uiStore.resetLastInteractedWith).mockImplementation(() => {
				uiStore.lastInteractedWithNodeId = undefined;
				uiStore.lastInteractedWithNodeHandle = null;
			});
			vi.spyOn(workflowDocumentStore, 'getGroupById').mockImplementation((groupId) =>
				groupId === group.id ? group : undefined,
			);
			vi.spyOn(workflowDocumentStore, 'replaceNodeInGroup').mockImplementation(
				(groupId, previousNodeId, newNodeId) => {
			...
			vi.mocked(workflowDocumentStore.getParentNodes).mockReturnValue([]);
			vi.mocked(workflowDocumentStore.getChildNodes).mockReturnValue([]);
			...
			expect(workflowDocumentStore.getGroupById(group.id)).toBe(group);
			const { addNodesAndConnections } = useCanvasOperations();
			await addNodesAndConnections([addedNode], [], { nodeGroupId: group.id });

			expect(workflowDocumentStore.replaceNodeInGroup).toHaveBeenCalledWith(
				group.id,
				anchor.id,
				addedNode.id,
			);
			expect(group.nodeIds).toEqual([addedNode.id]);
			expect(workflowDocumentStore.removeNodeById).toHaveBeenCalledWith(anchor.id);
			expect(workflowDocumentStore.addConnection).not.toHaveBeenCalled();

			const commands = historyStore.pushCommandToUndo.mock.calls.map(([command]) => command);
			expect(commands).toHaveLength(4);
			expect(commands[0]).toBeInstanceOf(AddNodeCommand);
			expect(commands[1]).toBeInstanceOf(UpdateNodeGroupCommand);
			expect(commands[2]).toBeInstanceOf(MoveNodeCommand);
			expect(commands[3]).toBeInstanceOf(RemoveNodeCommand);
		...
		it('replaces an empty-group anchor with the selected node and groups its helper', async () => {
			const {
				...
				uiStore,
				workflowDocumentStore,
			} = setupGroupExtension({ emptyGroup: true, withHelper: true });
			if (!helperNode) throw new Error('Expected a helper node');
			vi.mocked(uiStore.resetLastInteractedWith).mockImplementation(() => {
				uiStore.lastInteractedWithNodeId = undefined;
				uiStore.lastInteractedWithNodeHandle = null;
			...
			expect(workflowDocumentStore.addNodesToGroup).toHaveBeenCalledWith(group.id, [helperNode.id]);
			expect(group.nodeIds).toEqual([addedNode.id, helperNode.id]);
			expectConnectionAdded(addedNode, helperNode);
		});

		it('does not infer membership from the last interacted node', () => {
			const { addedNode, nodeTypeDescription, sourceNode, workflowDocumentStore } =
				setupGroupExtension();
			const addNodesToGroupSpy = vi.spyOn(workflowDocumentStore, 'addNodesToGroup');

			const { addNode } = useCanvasOperations();
			addNode(addedNode, nodeTypeDescription, { isAutoAdd: false });

			expect(addNodesToGroupSpy).not.toHaveBeenCalled();
			expectConnectionAdded(sourceNode, addedNode);
		});

		it('records the node, group, and connection changes in the caller-owned history bulk', async () => {
			const historyStore = mockedStore(useHistoryStore);
			const { addedNode, group } = setupGroupExtension();
			historyStore.currentBulkAction = new BulkCommand([]);
			const { addNodesAndConnections } = useCanvasOperations();
			await addNodesAndConnections([addedNode], [], {
				nodeGroupId: group.id,
				trackBulk: false,
				trackHistory: true,
			});
			...
			expect(commands[0]).toBeInstanceOf(AddNodeCommand);
			expect(commands[1]).toBeInstanceOf(AddConnectionCommand);
			expect(commands[2]).toBeInstanceOf(UpdateNodeGroupCommand);
			expect(historyStore.startRecordingUndo).not.toHaveBeenCalled();
			expect(historyStore.stopRecordingUndo).not.toHaveBeenCalled();
		});

		it('does not update the group when the requested extension is invalid', async () => {
			const { addedNode, group, workflowDocumentStore } = setupGroupExtension({
