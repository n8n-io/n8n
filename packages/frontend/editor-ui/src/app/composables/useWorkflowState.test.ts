...
describe('useWorkflowState', () => {
	...
	describe('markExecutionAsStopped', () => {
		...
		describe('when activeExecutionId is null (pending scaffold)', () => {
			...
			it('mirrors stopData onto the pendingExecution ref', () => {
				...
				expect(executionStateStore.pendingExecution?.status).toBe('canceled');
				expect(executionStateStore.pendingExecution?.startedAt).toEqual(
					new Date('2023-01-01T10:00:00Z'),
				);
				expect(executionStateStore.pendingExecution?.stoppedAt).toEqual(
		...
		describe('when activeExecutionId is undefined and displayedExecutionId is set', () => {
			beforeEach(() => {
				...
				executionStateStore.setActiveExecutionId('display-exec');
				executionStateStore.setActiveExecutionId(undefined);
				expect(executionStateStore.activeExecutionId).toBeUndefined();
				expect(executionStateStore.displayedExecutionId).toBe('display-exec');
	...
	describe('resetState', () => {
		...
		it('clears displayedExecutionId so workflowExecutionData reads as null', () => {
			...
			executionStateStore.setActiveExecutionId(undefined);
			expect(executionStateStore.displayedExecutionId).toBe('exec-A');
