...
describe("convertCurl", () => {
	...
	it("Should handle request methods", () => {
		...
		expect(getResult).toEqual({ method: "GET", url: "http://localhost:2000", raw_url: "http://localhost:2000"})

		const postResult = convertCurlToJson("curl --request POST http://localhost:2000");
		expect(postResult).toEqual({ method: "POST", url: "http://localhost:2000", raw_url: "http://localhost:2000"})

		const patchResult = convertCurlToJson("curl -X PATCH http://localhost:2000");
		expect(patchResult).toEqual({ method: "PATCH", url: "http://localhost:2000", raw_url: "http://localhost:2000"})
	})
	...
	it("Should respect --request over --form", () => {
		const result = convertCurlToJson('curl --request PATCH --form d1="data1" http://localhost:2000');
		expect(result).toEqual({
			method: "PATCH",
			url: "http://localhost:2000",
			raw_url: "http://localhost:2000",
	...
	it("Should handle a single header with -H", () => {
		const result = convertCurlToJson("curl -H 'Content-Type: application/json' http://localhost:2000");
		expect(result).toEqual({
			method: "GET",
			url: "http://localhost:2000",
			raw_url: "http://localhost:2000",
	...
	it("Should handle a single header with --header", () => {
		const result = convertCurlToJson("curl --header 'Authorization: Bearer token123' http://localhost:2000");
		expect(result).toEqual({
			method: "GET",
			url: "http://localhost:2000",
			raw_url: "http://localhost:2000",
	...
	it("Should handle aws-sigv4 auth", () => {
		const result = convertCurlToJson("curl --aws-sigv4 'aws:amz:us-east-1:s3' http://localhost:2000");
		expect(result).toEqual({
			method: "GET",
			url: "http://localhost:2000",
			raw_url: "http://localhost:2000",
