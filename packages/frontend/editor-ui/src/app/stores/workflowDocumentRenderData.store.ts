...
/**
 * Creates the canvas render data store for a workflow document, keyed by the
 * workflow document id. Pinia dedupes per id, so every consumer of the same
 * document (e.g. NodeView's canvas and the Instance AI workflow preview host)
 * shares a single instance instead of double-instantiating the per-node maps
 * and subscriptions below.
 *
 * Provides a single store combining:
 ...
 * lazily.
 *
 * Lifecycle: setup is side-effectful (subscribes to `onNodesChange`, creates
 * per-node scopes) and captures the document / execution-state store
 * instances of the same document id. Dispose this store via
 * `disposeWorkflowDocumentRenderDataStore` wherever the matching workflow
 * document store is disposed (see `useWorkflowInitialization.ts` and
 * `useWorkflowDiff.ts`) — a surviving render-data store paired with a
 * recreated document store would keep referencing the dead instance.
 */
	...
	return defineStore(getWorkflowDocumentRenderDataStoreId(workflowDocumentId), () => {
		...
		function createDefaultNodeRenderType(node: INodeUi): CanvasNodeDefaultRender {
			...
			const icon = getNodeIconSource(
				iconSource,
				node,
				workflowDocumentStore.getExpressionHandler(),
			);
		...
		// Teardown. Setup runs once per document id inside the store's own
		// effect scope; `disposeWorkflowDocumentRenderDataStore` stops that
		// scope, which runs this hook. We remove the `onNodesChange`
		// subscription (otherwise the document store's event hook keeps a
		// closure referencing this instance's maps alive and keeps running
		// reconciliation) and stop every per-entry scope — entries added after
		// setup (from `onNodesChange` callbacks) run in parentless scopes, so
		// stopping the store scope alone would not reach them.
		onScopeDispose(() => {
		...
		// -------------------------------------------------------------------------
		// Store passthrough projections.
		//
		// All exposed as `computed`s that re-resolve through the underlying
		// store on each evaluation. Plain object getters cannot be used here:
		// Pinia reads each setup-return property once while assembling the
		// store (`assign(store, setupStore)`), so a getter's value would be
		// snapshotted at setup time. Computeds survive as refs that Pinia
		// unwraps on access, preserving live re-resolution.
		//
		// `pinnedDataByNodeName` requires it: pin mutations *replace* the
		// underlying ref's value (immutable-update pattern), and this setup
		// runs once per document id — a direct capture would be a permanently
		// stale snapshot. The execution-state `active*` maps swap identity when
		// the active/displayed execution changes. The remaining by-id maps are
		// stable containers today, but uniform computeds keep every passthrough
		// correct if any later becomes resolver-backed. (Consumers read these
		// inside their own computeds and none destructure them.)
		// -------------------------------------------------------------------------

		// --- workflowDocument projections ---
		const nodeInputsByNodeId = computed(() => workflowDocumentStore.nodeInputsByNodeId);
		const nodeOutputsByNodeId = computed(() => workflowDocumentStore.nodeOutputsByNodeId);
		const pinnedDataByNodeName = computed(() => workflowDocumentStore.pinnedDataByNodeName);
		const pinnedDataByNodeId = computed(() => workflowDocumentStore.pinnedDataByNodeId);
		const validationErrorsByNodeId = computed(() => workflowDocumentStore.validationErrorsByNodeId);

		// --- workflowExecutionState projections ---
		const executionIssuesByNodeName = computed(
			() => executionStateStore.activeExecutionIssuesByNodeName,
		);
		const executionStatusByNodeId = computed(
			() => executionStateStore.activeExecutionStatusByNodeId,
		);
		const executionRunDataByNodeId = computed(
			() => executionStateStore.activeExecutionRunDataByNodeId,
		);
		const executionRunDataOutputMapByNodeId = computed(
			() => executionStateStore.activeExecutionRunDataOutputMapByNodeId,
		);
		const executionWaitingByNodeId = computed(
			() => executionStateStore.activeExecutionWaitingByNodeId,
		);
		const executionRunningByNodeId = computed(() => executionStateStore.executionRunningByNodeId);
		const executionWaitingForNextByNodeId = computed(
			() => executionStateStore.executionWaitingForNextByNodeId,
		);
...
/**
 * Disposes a workflow-document-render-data store instance. Mirrors
 * `disposeWorkflowDocumentStore`.
 *
 * Pairing invariant: this store captures the workflow document and
 * execution-state store instances of its document id at setup. Call this
 * wherever the matching workflow document store is disposed — a render-data
 * store that survives a document store recreation under the same id would
 * keep referencing (and stay subscribed to) the dead instance.
 */
