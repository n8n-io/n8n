...
type WorkflowDocumentExecutionSessionField =
	| 'activeExecutionId'
...
export type WorkflowDocumentExecutionSessionPayload = {
	field: WorkflowDocumentExecutionSessionField;
};

export type WorkflowDocumentExecutionSessionChangeEvent =
	ChangeEvent<WorkflowDocumentExecutionSessionPayload>;

export interface WorkflowDocumentExecutionSessionDeps {
	workflowId: string;
	workflowTriggerNodes: Readonly<ComputedRef<INodeUi[]>>;
	getNodeType: (typeName: string, version?: number) => INodeTypeDescription | null;
}

export function useWorkflowDocumentExecutionSession(deps: WorkflowDocumentExecutionSessionDeps) {
	...
	const onExecutionSessionChange = createEventHook<WorkflowDocumentExecutionSessionChangeEvent>();
	...
	const activeExecution = computed(() => {
		return (
			getActiveExecutionDataStore({ activeExecutionId: activeExecutionId.value })?.execution ?? null
		);
	});
