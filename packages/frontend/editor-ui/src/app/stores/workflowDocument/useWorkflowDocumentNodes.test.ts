...
describe('useWorkflowDocumentNodes', () => {
	...
	describe('read API delegation', () => {
		it('findNode delegates to workflowsStore.getNodeById', () => {
			const node = createNode();
			workflowsStore.getNodeById.mockReturnValue(node);

			const { findNode } = useWorkflowDocumentNodes(deps);
			const result = findNode('node-1');

			expect(result).toBe(node);
			expect(workflowsStore.getNodeById).toHaveBeenCalledWith('node-1');
		});

		it('findNodeByName delegates to workflowsStore.getNodeByName', () => {
			const node = createNode();
			workflowsStore.getNodeByName.mockReturnValue(node);

			const { findNodeByName } = useWorkflowDocumentNodes(deps);
			const result = findNodeByName('Test Node');
	...
	describe('events', () => {
		...
		it('addNode fires onNodesChange', () => {
			const hookSpy = vi.fn();
			const node = createNode();
		...
		it('removeNode fires onNodesChange', () => {
			const hookSpy = vi.fn();
			const node = createNode();
			workflowsStore.getNodeByName.mockReturnValue(node);
