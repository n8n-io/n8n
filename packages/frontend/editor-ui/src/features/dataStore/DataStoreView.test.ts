	...
	return {
		...
		useI18n: vi.fn(() => ({
			baseText: vi.fn((key: string) => {
				if (key === "dataTable.dataTables") return "Data Stores";
				if (key === "projects.menu.personal") return "Personal";
				if (key === "dataTable.empty.label") return "No data stores";
				if (key === "dataTable.empty.description")
					return "No data stores description";
		...
		i18n: {
			baseText: vi.fn((key: string) => {
				if (key === "parameterOverride.descriptionTooltip")
					return "Override tooltip";
				return key;
			}),
		},
	};
...
let sourceControlStore: ReturnType<
	typeof mockedStore<typeof useSourceControlStore>
>;
...
describe("DataStoreView", () => {
	...
	describe("initialization", () => {
		...
		it("should filter by project if not on overview sub page", async () => {
			...
			await waitAllPromises();
			expect(dataStoreStore.fetchDataStores).toHaveBeenCalledWith(
				"test-project",
				1,
				25,
			);
