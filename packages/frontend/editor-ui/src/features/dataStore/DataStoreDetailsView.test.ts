...
describe("DataTableDetailsView", () => {
	...
	describe("Loading states", () => {
		...
		it("should hide loading state after successful data fetch", async () => {
			const pinia = createTestingPinia({ stubActions: false });
			const dataStoreStore = useDataStoreStore();
			vi.spyOn(dataStoreStore, "fetchOrFindDataStore").mockResolvedValue(
				DEFAULT_DATA_STORE,
			);
			...
			await waitFor(() => {
				expect(
					queryByTestId("data-store-details-loading"),
		...
		it("should hide loading state after error", async () => {
			const pinia = createTestingPinia({ stubActions: false });
			const dataStoreStore = useDataStoreStore();
			vi.spyOn(dataStoreStore, "fetchOrFindDataStore").mockRejectedValue(
				new Error("Failed"),
			);
	...
	describe("Data rendering", () => {
		it("should render breadcrumbs and table when data is loaded", async () => {
			...
			await waitFor(() => {
				expect(
					container.querySelector("data-store-breadcrumbs-stub"),
				).toBeInTheDocument();
				expect(
					container.querySelector("data-store-table-stub"),
				).toBeInTheDocument();
			});
		});
