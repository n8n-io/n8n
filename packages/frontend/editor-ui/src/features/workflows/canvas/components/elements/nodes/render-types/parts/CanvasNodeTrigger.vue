...
const isExecuting = computed(() => workflowExecutionSessionStore.value?.isWorkflowRunning ?? false);
...
async function handleClickExecute() {
	workflowExecutionSessionStore.value?.setSelectedTriggerNodeName(name);
