...
describe('CanvasNodeGroupTitleBar', () => {
	...
	function render(
		...
		return renderComponent(CanvasNodeGroupTitleBar, {
			...
			props: {
				...
				isGroupNode: props.isGroupNode ?? false,
			},
		});
	}
	...
	describe('description', () => {
		...
		it('keeps the description at the zoom a fit-to-view lands on', () => {
			// The threshold is low on purpose, so a zoomed-out group keeps its only
			// click-to-edit target. See GROUP_DESCRIPTION_MIN_ZOOM.
			viewportRef.value = { x: 0, y: 0, zoom: 0.5 };
			const wrapper = render({ data: withDescription('x') });
			expect(wrapper.getByTestId('canvas-node-group-description')).toBeInTheDocument();
		});

		it('hides the description below the zoom threshold but keeps the header', () => {
			viewportRef.value = { x: 0, y: 0, zoom: 0.3 };
			const wrapper = render({ data: withDescription('x') });
	...
	describe('handles', () => {
		...
		it('is connectable on a collapsed card that is not read-only', () => {
		...
		// A group node owns its ports, so edges attach in every state. Collapsed
		// and expanded differ only in how the interior is drawn.
		it('is connectable in every state when the group is a real node', () => {
			for (const isCollapsed of [true, false]) {
				const wrapper = render({
					data: makeData({ isCollapsed, isEmptyGroup: false }),
					isGroupNode: true,
				});
		...
		// On the older path the group borrows a member as its boundary, so an
		// expanded group wires through its member nodes instead of the card.
		it('is not connectable on an expanded card on the nodeGroups path', () => {
			const wrapper = render({ data: makeData({ isCollapsed: false }) });
			for (const handle of handles(wrapper)) {
				expect(handle.getAttribute('data-connectable')).toBe('false');
			}
