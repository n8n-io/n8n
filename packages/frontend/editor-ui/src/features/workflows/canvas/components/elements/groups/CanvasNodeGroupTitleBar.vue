...
const props = withDefaults(
	defineProps<{
		...
		canExtract?: boolean;
		/**
		 * True when the group is a real node that owns its ports, instead of a
		 * render-only `nodeGroups` entry that borrows a member as its boundary.
		 */
		isGroupNode?: boolean;
	}>(),
	{
...
/**
 * A group node owns its ports, so its card takes a connection in every state.
 * Collapsed and expanded differ only in how the interior is drawn, never in how
 * the edges are wired.
 *
 * On the older `nodeGroups` path the group borrows a member as its boundary, so
 * only the card of a collapsed group re-anchors edges onto these handles.
 *
 * `connectable: false` on the VueFlow node is deliberate and stays: each
 * `<Handle>` drives its own connectability through `:is-connectable`.
 */
const isHandleConnectable = computed(
	() => !props.readOnly && (props.isGroupNode || isCollapsed.value),
);
// Show the dots whenever an edge can anchor on them.
const showHandles = computed(() => props.isGroupNode || isCollapsed.value);
const handleClass = computed(() => [
	$style.handle,
	{
		[$style.handleVisible]: showHandles.value,
		...
		<div :class="$style.card">
			...
			>
				<div :class="$style.toolbarItems">
					<!-- An empty group node ungroups cleanly: it just deletes the group.
					On the older path the group borrows a member, so there is nothing
					to ungroup while it is empty. -->
					<KeyboardShortcutTooltip
						v-if="isGroupNode || !isEmptyGroup"
