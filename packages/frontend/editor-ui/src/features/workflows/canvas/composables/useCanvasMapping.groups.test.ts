...
describe('mapGroupsToVueFlowNodes', () => {
	...
	// An empty group holds no nodes, so it has no member rect. Its card draws at
	// the group node's own position instead.
	const emptyGroupInputs = {
		allGroups: [{ id: 'g1', name: 'Plan', nodeIds: [] } satisfies IWorkflowGroup],
		getNodeById: nodeStore(),
		isGroupCollapsed: () => false,
		isEmptyGroup: (id: string) => id === 'g1',
		getGroupOwnPosition: () => ({ x: 0, y: 0 }),
		readOnly: false,
		getNodeExecutionSnapshot: snapshotGetter(),
	};

	it('forces an empty group collapsed and flags it', () => {
		const out = mapGroupsToVueFlowNodes(emptyGroupInputs);
	...
	// The card position is snapped to the canvas grid, so pick a position that
	// already sits on it.
	it('draws an empty group card at the group node position', () => {
		const out = mapGroupsToVueFlowNodes({
			...emptyGroupInputs,
			getGroupOwnPosition: () => ({ x: 128, y: 80 }),
		});

		expect(out[0].position).toEqual({ x: 128, y: 80 });
	});

	it('skips an empty group with no own position to place it at', () => {
		const out = mapGroupsToVueFlowNodes({
			...emptyGroupInputs,
			getGroupOwnPosition: undefined,
		});
...
describe('remapCollapsedGroupConnections', () => {
	...
	it('rewrites endpoints when one side is inside a collapsed group', () => {
		...
		expect(incoming?.targetHandle).toBe(CANVAS_NODE_GROUP_HANDLE_LEFT);
		const outgoing = result.find((c) => c.target === 'external2');
		expect(outgoing?.source).toBe('group:g1');
		expect(outgoing?.sourceHandle).toBe(CANVAS_NODE_GROUP_HANDLE_RIGHT);
	});
