 ...
 * - empty: the header plus the add-node body. An empty group always renders
 *   collapsed.
 */
...
export interface MapGroupsToVueFlowNodesInputs {
	...
	/**
	 * Top-left corner for a group that has no members to derive a rect from.
	 * Only a group node carries its own position, so this is set on the
	 * group-node path and absent on the legacy `nodeGroups` path.
	 */
	getGroupOwnPosition?: (id: string) => { x: number; y: number } | undefined;
	readOnly: boolean;
	getNodeExecutionSnapshot: (id: string) => NodeExecutionSnapshot;
}
...
export function mapGroupsToVueFlowNodes({
	...
	for (const group of allGroups) {
		...
		// at the group's own position instead.
		const ownPosition = isEmpty ? getGroupOwnPosition?.(group.id) : undefined;
		let nodesRect: NodesRect;

		if (isEmpty) {
			// Without an own position there is nothing to place the card at.
			if (ownPosition === undefined) continue;
			// computeGroupFrameRects subtracts the padding and the header, so add
			// them back to land the card exactly at ownPosition.
			nodesRect = {
				x: ownPosition.x + GROUP_PADDING_X,
				y: ownPosition.y + GROUP_PADDING_Y_TOP + GROUP_HEADER_HEIGHT,
				width: 0,
				height: 0,
			};
		} else {
			...
			if (!group.nodeIds.some((id) => getNodeById(id) !== undefined)) continue;
			nodesRect = computeNodesRectFromStore(group.nodeIds, getNodeById, getNodeDisplaySize);
		}
		// An empty group always renders as its card: there is no interior to expand.
