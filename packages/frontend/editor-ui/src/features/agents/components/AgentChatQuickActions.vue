 ...
 *   - Run now, Edit config: rendered only, click-inert (PR1 scope).
...
function noop() {
	// intentional: Run now + Edit config are rendered-only in PR1.
}

const playIcon: IconProp = 'play';
const pencilIcon: IconProp = 'pencil';
const sparklesIcon: IconProp = 'sparkles';
</script>

<template>
	<div :class="$style.row" data-testid="agent-chat-quick-actions">
		<N8nButton type="tertiary" size="mini" data-testid="agent-quick-action-run" @click="noop">
			<template #prefix><N8nIcon :icon="playIcon" size="xsmall" /></template>
			{{ i18n.baseText('agents.builder.quickActions.run') }}
		</N8nButton>
		<N8nButton type="tertiary" size="mini" data-testid="agent-quick-action-edit" @click="noop">
			<template #prefix><N8nIcon :icon="pencilIcon" size="xsmall" /></template>
		...
		<N8nButton
			...
			@click="onAddTool"
		>
			<template #prefix><N8nIcon :icon="sparklesIcon" size="xsmall" /></template>
