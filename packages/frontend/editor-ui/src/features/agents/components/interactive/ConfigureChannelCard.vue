...
/**
 * Card for the `configure_channel` builder tool. Mirrors
 * `InstanceAiChannelSetup.vue`'s flow (same `AgentChannel*Setup` components
 * and `useAgentChannelSetup` / `useAgentIntegrationStatus` /
 * `useAgentIntegrationsCatalog` composables — all already shared under
 * `features/agents`), but reports the outcome through the agents-chat resume
 * transport (`submit` emit → `POST /build/resume` with `{ approved }`)
 * instead of instance AI's own confirm endpoint.
 */
import { N8nButton, N8nIcon, N8nText } from '@n8n/design-system';
import type { IconName } from '@n8n/design-system/components/N8nIcon/icons';
import { useI18n } from '@n8n/i18n';
import type { ChannelResumeData, ChatIntegrationDescriptor } from '@n8n/api-types';
...
async function saveChannelConfig() {
	if (submitted.value || props.disabled || connectionInFlight.value) return;
...
async function setupSlackApp(appConfigurationToken: string): Promise<boolean> {
	if (submitted.value || props.disabled || connectionInFlight.value) return false;
	connectionInFlight.value = true;
	try {
...
<template>
	<div v-if="!disabled" :class="$style.card" data-testid="configure-channel-card">
		...
		<footer :class="$style.footer">
			<N8nButton
				...
				data-testid="configure-channel-skip"
				@click="skipSetup"
			>
