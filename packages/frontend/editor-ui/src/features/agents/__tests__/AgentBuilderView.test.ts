...
const isAgentsSandboxEnabledRef = ref(true);
...
vi.mock('@/app/stores/settings.store', () => ({
	useSettingsStore: () => ({
		...
		get isAgentsSandboxEnabled() {
			return isAgentsSandboxEnabledRef.value;
		},
	}),
}));
...
const uploadAgentFilesMock = vi.fn();
const deleteAgentFileMock = vi.fn();
...
describe('AgentBuilderView — preview routing', () => {
	...
	beforeEach(() => {
		...
		deleteAgentFileMock.mockReset();
		deleteAgentFileMock.mockResolvedValue(undefined);
		isAgentsSandboxEnabledRef.value = true;
