...
describe('RuleRow — assign dropdown', () => {
	...
	it('offers instance roles on an instance rule', () => {
	...
	it('emits role update with the selected role slug', async () => {
		...
		expect(emitted().update[0]).toEqual(['rule-1', { role: 'global:admin' }]);
	});

	it('does not offer Block access on a mapping rule (default condition only)', () => {
		renderRow({ rule: makeRule({ type: 'instance' }), type: 'instance' });
