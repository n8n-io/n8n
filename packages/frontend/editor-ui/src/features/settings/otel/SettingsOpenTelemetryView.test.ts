...
describe('SettingsOpenTelemetryView', () => {
	...
	it('tracks telemetry with current settings on save', async () => {
		...
		getOtelSettingsMock.mockResolvedValue(makeSettings());
		updateOtelSettingsMock.mockResolvedValue(savedSettings);
		...
		await waitFor(() => {
			expect(telemetryTrack).toHaveBeenCalledWith('User saved otel settings', {
				enabled: false,
				includeNodeSpans: false,
				productionExecutionsOnly: false,
				tracesSampleRate: 0.5,
