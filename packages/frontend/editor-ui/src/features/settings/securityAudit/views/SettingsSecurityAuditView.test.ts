...
describe('SettingsSecurityAuditView', () => {
	...
	describe('initial empty state', () => {
		it('should display empty state message before first audit', () => {
			...
			expect(screen.getByTestId('settings-security-audit')).toBeInTheDocument();
			expect(screen.getByText('No audit has been run yet')).toBeInTheDocument();
		});

		it('should display the run audit button', () => {
			renderComponent();

			expect(screen.getByTestId('run-security-audit-button')).toBeInTheDocument();
		...
		it('should not show loading or results', () => {
			...
			expect(screen.queryByTestId('security-audit-loading')).not.toBeInTheDocument();
			expect(screen.queryByTestId('security-audit-results')).not.toBeInTheDocument();
			expect(screen.queryByTestId('security-audit-error')).not.toBeInTheDocument();
		});
	});
	...
	describe('loading state', () => {
		it('should display loading indicator when audit is running', () => {
			...
			expect(screen.getByTestId('security-audit-loading')).toBeInTheDocument();
		...
		it('should hide empty state when loading', () => {
			...
			expect(screen.queryByText('No audit has been run yet')).not.toBeInTheDocument();
		});
	});
	...
	describe('error state', () => {
		it('should display error callout when audit fails', () => {
			...
			expect(screen.getByTestId('security-audit-error')).toBeInTheDocument();
	...
	describe('results state', () => {
		it('should display results when lastRunAt is set', () => {
			...
			expect(screen.getByTestId('security-audit-results')).toBeInTheDocument();
		});

		it('should display last run text when results exist', () => {
			const pinia = createTestingPinia({
				initialState: {
					securityAudit: {
						auditResult: mockAuditResponse,
	...
	describe('run audit button interaction', () => {
		it('should call runAudit when button clicked', async () => {
			...
			await user.click(screen.getByTestId('run-security-audit-button'));
		...
		it('should show button while loading', () => {
			...
			const button = screen.getByTestId('run-security-audit-button');
