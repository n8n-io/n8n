	...
	useI18n: () => ({
		baseText: (key: string, opts?: { interpolate?: Record<string, string> }) => {
			const translations: Record<string, string> = {
				...
				'instanceAi.tools.nodes.execute.imperative': 'execute a node',
			};
...
describe('InstanceAiConfirmationPanel telemetry', () => {
	...
	describe('approval confirmation', () => {
		...
		it('prompts for approval when the assistant executes a node', () => {
			injectPendingConfirmation(
				thread,
				{
					requestId: 'execute-node',
					severity: 'warning',
					message: 'Google Sheets > Document > Create',
				},
			...
			expect(getByText('Allow n8n Assistant to execute a node?')).toBeVisible();
			expect(getByText('Google Sheets > Document > Create')).toBeVisible();
		});
