...
describe('useResourceRegistry', () => {
	...
	describe('identity stability', () => {
		...
		test('keeps map identity when a rebuild yields equivalent contents', async () => {
			const { messages, producedArtifacts, resourceNameIndex } = setupWithArtifact();
			const producedBefore = producedArtifacts.value;
			const namesBefore = resourceNameIndex.value;

			// A structural event that does not change the registry: a non-artifact
			// tool call appears on the (only) tree. The deep walk re-runs, the maps
			// are rebuilt — but consumers must keep seeing the same identity.
			messages.value[0].agentTree!.toolCalls.push(
				makeToolCall({ toolCallId: 'tc-2', toolName: 'search' }),
			);
			await nextTick();

			expect(producedArtifacts.value).toBe(producedBefore);
			expect(resourceNameIndex.value).toBe(namesBefore);
		});

		test('changes map identity when the registry actually changes', async () => {
			const { messages, producedArtifacts, resourceNameIndex } = setupWithArtifact();
			const producedBefore = producedArtifacts.value;
			const namesBefore = resourceNameIndex.value;
			...
			expect(producedArtifacts.value).not.toBe(producedBefore);
			expect(producedArtifacts.value.get('wf-2')?.name).toBe('Second Pipeline');
			expect(resourceNameIndex.value).not.toBe(namesBefore);
		});

		test('a field-level change (archived flag) is not masked by the previous map', async () => {
			const messages = ref<InstanceAiMessage[]>([]);
			...
			const before = producedArtifacts.value;
			expect(before.get('wf-1')?.archived).toBeUndefined();

			archived.value = new Set(['wf-1']);
			await nextTick();

			expect(producedArtifacts.value).not.toBe(before);
			expect(producedArtifacts.value.get('wf-1')?.archived).toBe(true);
		});
	});
});
