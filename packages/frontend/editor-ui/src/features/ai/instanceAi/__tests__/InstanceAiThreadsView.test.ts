...
vi.mock('@/app/composables/useDocumentTitle', () => ({ useDocumentTitle: () => ({ set: vi.fn() }) }));
vi.mock('@vueuse/core', async (original) => ({
	...await original<typeof import('@vueuse/core')>(), useIntersectionObserver: vi.fn(),
}));

function page(ids: string[], index = 0, hasMore = false): InstanceAiThreadListResponse {
	return { threads: ids.map(id => ({ id, title: id, resourceId: 'user', createdAt: '2026-01-01', updatedAt: '2026-01-01' })), total: 60, page: index, hasMore };
}
function render() {
	return shallowMount(InstanceAiThreadsView, { global: { renderStubDefaultSlot: true, stubs: {
		N8nScrollArea: { template: '<div><slot /></div>', methods: { scrollToTop: mocks.scrollToTop } },
	} } });
}
beforeEach(() => {
	vi.useFakeTimers();
	mocks.load.mockReset();
	store.threads = ['a', 'b', 'match'].map(id => ({ id, title: id, createdAt: '2026-01-01', updatedAt: '2026-01-01' }));
});
afterEach(() => { vi.useRealTimers(); });

describe('chat history pagination', () => {
	it('loads 30 at a time, appends without duplicates, and stops at the end', async () => {
		mocks.load.mockResolvedValueOnce(page(['a'], 0, true)).mockResolvedValueOnce(page(['a', 'b'], 1));
		const wrapper = render();
		await flushPromises();
		expect(mocks.load).toHaveBeenLastCalledWith({ page: 0, limit: 30, search: '' });
		const button = wrapper.findAllComponents({ name: 'N8nButton' }).find(button => button.text() === 'Load more');
		expect(button).toBeDefined();
		button!.vm.$emit('click');
		await flushPromises();
		expect(mocks.load).toHaveBeenLastCalledWith({ page: 1, limit: 30, search: '' });
	...
	it('retries the failed page without advancing', async () => {
		...
		wrapper.findAllComponents({ name: 'N8nButton' }).find(button => button.text() === 'Retry')!.vm.$emit('click');
		await flushPromises();
		expect(mocks.load).toHaveBeenLastCalledWith({ page: 0, limit: 30, search: '' });
	...
	it('debounces server search and ignores an older response', async () => {
		...
		mocks.load.mockReturnValueOnce(new Promise<InstanceAiThreadListResponse>(resolve => { resolveOld = resolve; }))
			.mockResolvedValueOnce(page(['match']));
		const wrapper = render();
		wrapper.findComponent({ name: 'N8nInput' }).vm.$emit('update:modelValue', 'match');
		await vi.advanceTimersByTimeAsync(1000);
		await flushPromises();
		expect(mocks.load).toHaveBeenLastCalledWith({ page: 0, limit: 30, search: 'match' });
