...
export type TimelineBlock =
	...
	| { type: 'verification-verdict'; key: string; entry: VerdictEntry }
	| { type: 'activity'; key: string };

export type VerdictEntry = Extract<InstanceAiTimelineEntry, { type: 'verification-verdict' }>;
...
export function buildTimelineBlocks(
	...
	entries.forEach((entry, idx) => {
		...
		if (entry.type === 'verification-verdict') {
			// Standalone: the verdict is generated by the backend, so it must never
			// fold into a thinking block where it reads as narration.
			pushStandalone({ type: 'verification-verdict', key: `verdict-${idx}`, entry });
			return;
		}
