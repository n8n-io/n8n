...
export function createThreadRuntime(
	...
	async function loadHistoricalMessages(): Promise<HistoricalHydrationStatus> {
		...
		const promise = (async (): Promise<HistoricalHydrationStatus> => {
			try {
				...
				// The server skips pages that render nothing, so even the newest read
				// can land past page 0.
				oldestLoadedPage = result.page ?? 0;
	...
	/**
	 * Reads one page further back and prepends it.
	 *
	 * The server skips pages that render nothing, so one read is enough to reach
	 * the next page with content, and it reports the page it settled on.
	 *
	 ...
	 * rows already on screen are dropped instead of paged around. That dedup can
	 * still empty a page — the server cannot know what is on screen — which
	 * leaves the control armed for another read rather than adding messages.
	 *
	...
	async function loadEarlierMessages(): Promise<void> {
		...
		try {
			const requestedPage = oldestLoadedPage + 1;
			const result = await fetchThreadMessagesApi(
				rootStore.restApiContext,
				threadId,
				INSTANCE_AI_THREAD_HISTORY_PAGE_SIZE,
			...
			// Resume from the page the response carries, not the one asked for: the
			// server may have skipped past pages that render nothing. The fallback
			// covers a main still on a build that does not report it.
			oldestLoadedPage = result.page ?? requestedPage;
			hasMoreHistory.value = result.hasMore === true;
