 ...
 * panel takes most kinds, while the MCP connect card renders inline in the
 * timeline.
 *
...
export function createThreadRuntime(
	...
	const resolvedConfirmationIds = reactive(new Map<string, 'approved' | 'denied' | 'deferred'>());
	...
	// A run paused on user input (confirmation) isn't stalled, so
