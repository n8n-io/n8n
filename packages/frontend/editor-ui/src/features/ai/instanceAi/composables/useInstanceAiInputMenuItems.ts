...
export function useInstanceAiInputMenuItems(attachFiles: () => void) {
	...
	if (isMcpFeatureEnabled.value) void mcpStore.fetchConnections();
	...
	function createConnectionItem({
		...
		if (status !== 'connected') {
			return {
				...
				data: status === 'none' ? { action: connect } : { status, action: connect },
			};
		}
		...
		return {
			...
			children: [
				{
					id: `${id}-status`,
					label: i18n.baseText('instanceAi.connections.row.status.connected'),
					header: true,
				},
				{
					id: `${id}-disconnect`,
	...
	return computed(() => {
		...
		if (isBrowserUseFeatureEnabled.value && settingsStore.isBrowserUseEnabledByAdmin) {
			const connectionStatus =
				settingsStore.connections.find(({ type }) => type === BROWSER_USE_CONNECTION_TYPE)
					?.status ?? 'none';
			const status = connectionStatus === 'disconnected' ? 'none' : connectionStatus;
			items.push(
				createConnectionItem({
