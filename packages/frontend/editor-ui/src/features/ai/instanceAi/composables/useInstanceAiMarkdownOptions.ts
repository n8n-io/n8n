...
export function shouldOpenInstanceAiMarkdownLinkInNewTab(href: string): boolean {
...
export function useInstanceAiMarkdownOptions(
	...
	const plugins = computed(() => {
		const linksNewTabPlugin = (vueMarkdownItInstance: MarkdownIt) => {
			...
			vueMarkdownItInstance.renderer.rules.link_open = (tokens, idx, options, env, self) => {
				...
				if (href && shouldOpenInstanceAiMarkdownLinkInNewTab(href)) {
