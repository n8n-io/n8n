...
// Inspect a successful execution against an evaluation config: returns the
// data table columns, the input/output fields available on the execution, and
// an auto-suggested column → field mapping for the "Add to dataset" modal.
...
// Insert one row into the config's data table, built from the execution per the
// (user-reviewed) mapping. Returns the inserted row id(s).
