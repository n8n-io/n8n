...
export const useAssistantStore = defineStore(STORES.ASSISTANT, () => {
	...
	const codeDiff = useCodeDiff({
		...
		getNodeName: () => chatSessionError.value?.node.name,
	});
	...
	async function applyCodeDiff(workflowState: WorkflowState, index: number) {
		await codeDiff.applyCodeDiff(workflowState, chatMessages.value, index);
	}

	async function undoCodeDiff(workflowState: WorkflowState, index: number) {
		await codeDiff.undoCodeDiff(workflowState, chatMessages.value, index);
	}
