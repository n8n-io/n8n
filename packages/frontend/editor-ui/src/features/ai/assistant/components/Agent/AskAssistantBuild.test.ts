...
describe('AskAssistantBuild', () => {
	...
	describe('shouldTidyUp logic', () => {
		...
		it('should reset shouldTidyUp flag when starting a new workflow', async () => {
			renderComponent();
			...
			// Now simulate onNewWorkflow being called (e.g., via route change)
			// The onNewWorkflow function resets shouldTidyUp
			builderStore.resetBuilderChat.mockClear();

			// Trigger route change which calls onNewWorkflow
			// We can't easily test route changes, but we can verify the store is reset
