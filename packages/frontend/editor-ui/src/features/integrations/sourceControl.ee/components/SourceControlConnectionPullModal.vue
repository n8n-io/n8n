...
import { N8nBadge, N8nButton, N8nHeading, N8nSpinner, N8nText } from '@n8n/design-system';
...
async function onPull() {
	...
	try {
		await connectionsStore.pull(props.data.connectionId);
		toast.showMessage({
			title: i18n.baseText('settings.sourceControl.connections.modal.pull.success'),
			type: 'success',
		});
		close();
	} catch (error) {
		toast.showError(error, i18n.baseText('settings.sourceControl.connections.modal.pull.error'));
	}
	...
	<Modal :name="modalName" width="600px" @close="close">
		<template #header>
			<N8nHeading tag="h1" size="xlarge">
				{{ i18n.baseText('settings.sourceControl.connections.modal.pull.title') }}
			</N8nHeading>
		...
		<template #content>
			...
			<div v-else>
				...
				<div v-else>
					<N8nText size="small" color="text-light" class="mb-xs">
						{{ i18n.baseText('settings.sourceControl.connections.modal.pull.description') }}
					</N8nText>
					<div :class="$style.fileList" data-test-id="source-control-connection-pull-files">
						<div v-for="file in files" :key="file.path" :class="$style.fileRow">
							<N8nBadge theme="tertiary">{{ file.status }}</N8nBadge>
		...
		<template #footer>
			<div :class="$style.footer">
				<N8nButton variant="subtle" :disabled="isPulling" @click="close">
					{{ i18n.baseText('settings.sourceControl.connections.button.cancel') }}
				</N8nButton>
				<N8nButton
					:disabled="isLoading || isPulling"
					data-test-id="source-control-connection-pull-button"
					@click="onPull"
				>
