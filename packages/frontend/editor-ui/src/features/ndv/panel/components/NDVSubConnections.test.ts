...
describe('NDVSubConnections', () => {
	beforeAll(() => {
		...
		workflowDocumentStoreRef.value = docStore as ReturnType<typeof useWorkflowDocumentStore>;
		vi.mocked(injectWorkflowDocumentStore).mockReturnValue(shallowRef(docStore));
	});
