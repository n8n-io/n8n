...
watch(
	...
	() => {
		if (hasRemoteMethod.value && node.value) {
			// Re-fetch remote options with new credentials.
			// The value is preserved — if it's no longer valid with the new credential,
			// the user can update it from the refreshed options list.
			void loadRemoteParameterOptions();
		}
	},
	{ immediate: true },
