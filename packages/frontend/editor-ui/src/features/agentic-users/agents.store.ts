...
export const useAgenticUsersStore = defineStore('agenticUsers', () => {
	...
	const fetchAgents = async () => {
		const [agentUsers, registrations] = await Promise.all([
			makeRestApiRequest<UserResponse[]>(rootStore.restApiContext, 'GET', '/agentic-users'),
			makeRestApiRequest<ExternalAgentRegistration[]>(
				rootStore.restApiContext,
				'GET',
				'/agentic-users/external',
			),
		]);
	...
	const removeExternalAgent = async (id: string) => {
		...
		if (registrationId) {
			await makeRestApiRequest(
				rootStore.restApiContext,
				'DELETE',
				`/agentic-users/external/${registrationId}`,
			);
		}
	...
	const updateExternalAgentMapping = async (
		...
		await makeRestApiRequest(
			rootStore.restApiContext,
			'PATCH',
			`/agentic-users/external/${agent.registrationId}/mappings`,
