...
describe('agents.store – fetchAgents', () => {
	...
	it('should call GET /agentic-users (not /users)', async () => {
		mockedMakeRequest.mockResolvedValueOnce([]);

		const store = useAgenticUsersStore();
		await store.fetchAgents();

		expect(mockedMakeRequest).toHaveBeenCalledWith(expect.anything(), 'GET', '/agentic-users');
	});
