	...
	async function runWorkflow(options: {
		...
		try {
			...
			if (options.destinationNode !== undefined) {
				directParentNodes = workflowObject.getParentNodes(
			...
			if (
				...
				startNodeNames.push(
					...workflowObject.getChildNodes(options.triggerNode, NodeConnectionTypes.Main, 1),
				);
			...
			if (
				...
				const startNode = workflowObject.getStartNode(options.destinationNode);
			...
			const startNodes: StartNodeData[] = sortNodesByYPosition(startNodeNames)
				.map((name) => {
					...
					if (sourceData === null) {
						const parentNodes = workflowObject.getParentNodes(name, NodeConnectionTypes.Main, 1);
			...
			if ('destinationNode' in options) {
				...
				if (workflowObject.id && nodeId && version === 2) {
					const agentRequest = agentRequestStore.getAgentRequest(workflowObject.id, nodeId);
			...
			workflowHelpers.setDocumentTitle(workflowObject.name as string, 'EXECUTING');
