...
import {
	...
	AIView,
	AINodesView,
} from '@/features/shared/nodeCreator/views/viewsData';
import {
...
import CategorizedItemsRenderer from '@/features/shared/nodeCreator/components/Renderers/CategorizedItemsRenderer.vue';
import NoResults from '@/features/shared/nodeCreator/components/Panel/NoResults.vue';
...
function onSelected(item: INodeCreateElement) {
	...
	if (item.type === 'view') {
		...
		console.log('>>>ORIGINAL VIEW NODES : ', view?.items);
		const whitelistedNodes = view?.items?.filter((item: any) => {
			return WHITELISTED_NODES.includes(item.key);
		});
		console.log('>>>WHITELISTED VIEW NODES : ', whitelistedNodes);
