/**
 * The definition format, with nothing of Vue or of n8n in it.
 *
 * This is the half of the package that a server can read: the node validates a
 * definition with it, and the workflow SDK generates the authoring types the AI
 * writes against from it. `core/types.ts` re-exports the lot and adds the one
 * thing that needs a renderer — a component to render.
 */
...
/**
 * The n8n descriptor types the kit uses. A subset rather than all of
 * `NodePropertyTypes`, so this file stays free of `n8n-workflow`; `core/types.ts`
 * asserts the two agree.
 */
export type UiValuePropertyType = 'string' | 'number' | 'boolean' | 'options';
...
/**
 * A prop descriptor, shaped as an n8n node property so the editor's inspector
 * can be n8n's own parameter inputs pointed at a component instead of a node.
 * Declared structurally rather than as `Omit<INodeProperties, 'type'>` to keep
 * this half of the package importable without `n8n-workflow`.
 */
/**
 * What a descriptor may declare as its default. A subset of n8n's
 * `NodeParameterValueType` spelled out locally, narrow enough that the inspector
 * can hand a descriptor straight to n8n's parameter inputs. An action prop's
 * default is the empty chain, which is why arrays are here at all.
 */
export type UiPropertyDefault = string | number | boolean | null | Array<string | number | boolean>;

export interface UiProperty {
	displayName: string;
	name: string;
	type: UiPropertyType;
	default: UiPropertyDefault;
	description?: string;
	placeholder?: string;
...
/**
 * A kit entry without its component.
 *
 * Two descriptor types carry meaning for the runtime:
 *   - `action`    the value is a chain of steps, fired rather than rendered
