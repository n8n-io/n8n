...
import type { ColoringStateEffect, Plaintext, Resolvable, ResolvableState } from '../types';

let report: (error: unknown) => void = () => {};

/** Hosts with error tracking wire it in; without one a failed decoration is silent. */
export function setExpressionEditorErrorReporter(reporter: (error: unknown) => void) {
	report = reporter;
}
