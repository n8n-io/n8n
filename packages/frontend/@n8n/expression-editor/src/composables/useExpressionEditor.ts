...
import { isEventTargetContainedBy } from '@n8n/frontend-utils/htmlUtils';
...
/** Give up on a syntax tree that takes longer than this to build, and retry later. */
const PARSER_TIMEOUT = 15_000;
...
export const useExpressionEditor = ({
	...
	staticExtensions?: MaybeRefOrGetter<Extension[]>;
	...
	/** Extensions that need the view itself, applied once the state is built. */
	onEditorCreated?: (view: EditorView) => void;
	/** Extra teardown when the editor loses focus, e.g. dismissing host tooltips. */
	...
	const updateSegments = async (): Promise<void> => {
		...
		const fullTree = ensureSyntaxTree(state, state.doc.length, PARSER_TIMEOUT);
		...
		const resolvedSegments: Segment[] = await Promise.all(
			rawSegments.map(async (segment) => {
				...
				if (token === 'Resolvable') {
					const { resolved, error, fullError } = await resolve(text);
					return {
	...
	async function resolve(resolvable: string) {
		const result = await resolver.resolve(resolvable);

		if (result.resolved === undefined) {
			return {
				...result,
				resolved: isUncalledExpressionExtension(resolvable)
				...
				if (typeof s.resolved === 'string') {
					...
					if (/\[Object:\s(\{.+\}|\{\})\]/.test(resolved)) {
						...
						try {
							...
							// Not JSON after all; leave the bracketed form as it stands.
						}
					}
	...
	if (resolver.watchDebounced) {
		watch(resolver.watchDebounced, debouncedUpdateSegments);
	}

	if (resolver.watchImmediate) {
		watch(resolver.watchImmediate, updateSegments);
	}
