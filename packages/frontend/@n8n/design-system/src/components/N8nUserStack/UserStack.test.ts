...
describe('UserStack', () => {
	...
	it('should accept a plain array for a single unlabelled group', () => {
		const { container } = render(UserStack, {
			props: {
				currentUserEmail: 'hello@n8n.io',
				users: [
		...
		expect(container.querySelector('.user-stack')).toBeInTheDocument();
		expect(container.querySelectorAll('svg')).toHaveLength(2);
		// One group carries no label, so no heading is rendered.
		expect(container.querySelector('.groupName')).toBeNull();
	});

	it('should render nothing extra for an empty array', () => {
		const { container } = render(UserStack, {
			props: { currentUserEmail: 'hello@n8n.io', users: [] },
			global: {
				components: {
					'n8n-avatar': N8nAvatar,
					'n8n-user-info': N8nUserInfo,
		...
		expect(container.querySelector('.hiddenBadge')).toBeNull();
	});
});
