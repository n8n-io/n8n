...
describe('ToolMessage', () => {
	...
	describe('error handling', () => {
		it('should display error message when error update exists', () => {
			const errorMessage = {
				...baseMessage,
				status: 'error' as const,
				updates: [
			...
			expect(wrapper.text()).toContain('Tool execution failed');
		});

		it('should display error data directly when error is a string', () => {
			const errorMessage = {
				...baseMessage,
				status: 'error' as const,
				updates: [
		...
		it('should not display error section when no error update exists', () => {
			...
			expect(wrapper.html()).not.toContain('errorContent');
		});

		it('should handle multiple error updates and show the first one', () => {
			const messageWithMultipleErrors = {
				...baseMessage,
				status: 'error' as const,
				updates: [
					{
			...
			const wrapper = mountToolMessage({
				message: messageWithMultipleErrors,
				isFirstOfRole: true,
			});
