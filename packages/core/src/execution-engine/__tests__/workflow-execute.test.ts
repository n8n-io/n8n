...
describe('WorkflowExecute', () => {
	...
	describe('Enhanced Loop Detection', () => {
		...
		beforeEach(() => {
			additionalData = mock<IWorkflowExecuteAdditionalData>();
			workflowExecute = new WorkflowExecute(additionalData, 'manual');
		});

		test('should detect rapid re-execution patterns and throw OperationalError', () => {
			// Test that rapid executions of the same node trigger loop detection
			const nodeName = 'SplitInBatches';

			// Use reflection to access private method for testing
			const checkForRapidReExecution = (
				workflowExecute as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			).checkForRapidReExecution.bind(workflowExecute);

			// Simulate rapid executions within the time window
			// This should not throw for the first 10 executions
			for (let i = 0; i < 10; i++) {
				expect(() => checkForRapidReExecution(nodeName, i)).not.toThrow();
			}

			// The 11th execution should trigger the loop detection
			try {
				checkForRapidReExecution(nodeName, 10);
				throw new Error('Expected OperationalError to be thrown');
			} catch (error) {
				expect(error.constructor.name).toBe('OperationalError');
				expect(error.message).toContain('Infinite loop detected: Node "SplitInBatches" executed');
				expect(error.message).toContain('Check for recursive connections');
			}
		});

		test('should allow different nodes to execute without interference', () => {
			const checkForRapidReExecution = (
				workflowExecute as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			...
			// Execute different nodes - should not interfere with each other
			for (let i = 0; i < 10; i++) {
				expect(() => checkForRapidReExecution('NodeA', i)).not.toThrow();
				expect(() => checkForRapidReExecution('NodeB', i)).not.toThrow();
			}

			// Each node should be able to execute up to the limit independently
			expect(() => checkForRapidReExecution('NodeA', 10)).toThrow();
			expect(() => checkForRapidReExecution('NodeB', 10)).toThrow();
		});

		test('should clean up old execution history to prevent memory growth', () => {
			const checkForRapidReExecution = (
				workflowExecute as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			...
			// Access private property for testing - now it's a Map
			const getNodeExecutionHistory = () =>
				(
					workflowExecute as unknown as {
						nodeExecutionHistory: Map<string, Array<{ runIndex: number; timestamp: number }>>;
					}
			...
			// Execute some nodes with controlled timestamps
			for (let i = 0; i < 5; i++) {
				mockTime += 100; // Add 100ms between each execution
				checkForRapidReExecution('TestNode', i);
			}

			const nodeHistory = getNodeExecutionHistory().get('TestNode');
			expect(nodeHistory).toBeDefined();
			expect(nodeHistory!.length).toBe(5);

			// Simulate time passing beyond the time window (5000ms)
			mockTime += 6000; // 6 seconds later

			// Execute another node - this should clean up old entries
			checkForRapidReExecution('TestNode', 5);

			const nodeHistoryAfter = getNodeExecutionHistory().get('TestNode');
			expect(nodeHistoryAfter!.length).toBe(1); // Only the latest execution should remain

			// Restore Date.now
			Date.now = originalDateNow;
		});

		test('should provide detailed error message with execution metrics', () => {
			const checkForRapidReExecution = (
				workflowExecute as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			...
			// Mock Date.now to control timestamps for consistent test results
			const originalDateNow = Date.now;
			const startTime = 1000000000; // Use a fixed start time
			let mockTime = startTime;
			Date.now = jest.fn(() => mockTime);

			// Execute up to the limit with time intervals that ensure timeSpan > 0
			for (let i = 0; i < 10; i++) {
				checkForRapidReExecution(nodeName, i);
				mockTime += 100; // Add 100ms after each execution
			}

			// The next execution should provide a detailed error message
			try {
				checkForRapidReExecution(nodeName, 10);
				throw new Error('Expected OperationalError to be thrown');
			} catch (error) {
				expect(error.message).toMatch(
					/Infinite loop detected: Node "SplitInBatches" executed \d+ times in \d+\.\d+ seconds \(\d+\/sec\)\. Check for recursive connections/,
				);
				expect(error.message).toContain('Consider reviewing your workflow logic');
				// Also verify the message doesn't contain NaN
		...
		// Edge Cases Tests
		test('should handle clock skew and system time changes gracefully', () => {
			const checkForRapidReExecution = (
				workflowExecute as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			...
			for (let i = 0; i < 5; i++) {
				expect(() => checkForRapidReExecution('TestNode', i)).not.toThrow();
				mockTime += 100;
			}

			// Simulate large backwards clock jump (should skip detection)
			mockTime = -100;
			expect(() => checkForRapidReExecution('TestNode', 5)).not.toThrow();

			// Simulate large forwards clock jump (should skip detection)
			mockTime = Date.now() + 2000; // Beyond tolerance
			expect(() => checkForRapidReExecution('TestNode', 6)).not.toThrow();

			Date.now = originalDateNow;
		});

		test('should handle concurrent executions of different workflows', () => {
			const workflowExecute2 = new WorkflowExecute(additionalData, 'manual');

			const checkForRapidReExecution1 = (
				workflowExecute as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			).checkForRapidReExecution.bind(workflowExecute);
			const checkForRapidReExecution2 = (
				workflowExecute2 as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			).checkForRapidReExecution.bind(workflowExecute2);

			// Each workflow should maintain separate execution history
			for (let i = 0; i < 10; i++) {
				expect(() => checkForRapidReExecution1('SharedNodeName', i)).not.toThrow();
				expect(() => checkForRapidReExecution2('SharedNodeName', i)).not.toThrow();
			}

			// Both should be able to reach their individual limits
			expect(() => checkForRapidReExecution1('SharedNodeName', 10)).toThrow();
			expect(() => checkForRapidReExecution2('SharedNodeName', 10)).toThrow();
		});

		test('should handle very large numbers of different nodes efficiently', () => {
			const checkForRapidReExecution = (
				workflowExecute as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			...
			const nodeCount = 100;
			const executionsPerNode = 5;

			// Execute many different nodes
			for (let nodeIndex = 0; nodeIndex < nodeCount; nodeIndex++) {
				const nodeName = `Node${nodeIndex}`;
				for (let execIndex = 0; execIndex < executionsPerNode; execIndex++) {
					expect(() => checkForRapidReExecution(nodeName, execIndex)).not.toThrow();
				}
			}

			// Verify all nodes are tracked
			const history = getNodeExecutionHistory();
			expect(history.size).toBe(nodeCount);

			// Each node should have its execution count
			for (let nodeIndex = 0; nodeIndex < nodeCount; nodeIndex++) {
				const nodeName = `Node${nodeIndex}`;
				const nodeHistory = history.get(nodeName);
				expect(nodeHistory).toBeDefined();
				expect(nodeHistory!.length).toBe(executionsPerNode);
			}
		});

		// Performance Tests
		test('should not cause significant performance degradation for normal workflows', () => {
			const checkForRapidReExecution = (
				workflowExecute as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			...
			const nodeCount = 50;
			const executionsPerNode = 3; // Under threshold
			const startTime = performance.now();

			// Simulate normal workflow execution pattern
			for (let nodeIndex = 0; nodeIndex < nodeCount; nodeIndex++) {
				const nodeName = `WorkflowNode${nodeIndex}`;
				for (let execIndex = 0; execIndex < executionsPerNode; execIndex++) {
					checkForRapidReExecution(nodeName, execIndex);
				}
			}
			...
			// Should complete quickly (less than 100ms for this scale)
			expect(executionTime).toBeLessThan(100);
		});

		test('should enforce global memory limits and cleanup efficiently', () => {
			const checkForRapidReExecution = (
				workflowExecute as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			...
			const enforceGlobalHistoryLimit = (
				workflowExecute as unknown as { enforceGlobalHistoryLimit: () => void }
			).enforceGlobalHistoryLimit.bind(workflowExecute);

			// Create many entries across different nodes to exceed global limit
			// MAX_TOTAL_HISTORY_SIZE is 1000, so we'll create more than that
			const nodeCount = 200;
			const executionsPerNode = 8; // Total: 1600 entries
			...
			for (let nodeIndex = 0; nodeIndex < nodeCount; nodeIndex++) {
				const nodeName = `MemoryTestNode${nodeIndex}`;
				for (let execIndex = 0; execIndex < executionsPerNode; execIndex++) {
					checkForRapidReExecution(nodeName, execIndex);
					mockTime += 10; // Small increment to keep within time window
				}
			}
			...
			const history = getNodeExecutionHistory();
			let totalEntries = 0;
			for (const nodeHistory of history.values()) {
				totalEntries += nodeHistory.length;
			}

			// Should be at or below the limit after cleanup
			expect(totalEntries).toBeLessThanOrEqual(1000);

			Date.now = originalDateNow;
		});

		test('should handle high-frequency execution patterns without false positives', () => {
			const checkForRapidReExecution = (
				workflowExecute as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			...
			// Test spread out executions over time window (should not trigger)
			for (let i = 0; i < 10; i++) {
				expect(() => checkForRapidReExecution('HighFreqNode', i)).not.toThrow();
				mockTime += 600; // 600ms intervals = 6 seconds total (exceeds 5s window)
			}

			// Reset time and test rapid executions (should trigger)
			mockTime = 1000000000;
			for (let i = 0; i < 10; i++) {
				expect(() => checkForRapidReExecution('RapidNode', i)).not.toThrow();
				mockTime += 10; // 10ms intervals = rapid execution
			}

			expect(() => checkForRapidReExecution('RapidNode', 10)).toThrow();

			Date.now = originalDateNow;
		});

		// Configuration and Boundary Tests
		test('should respect exact threshold boundaries', () => {
			const checkForRapidReExecution = (
				workflowExecute as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			...
			// Test exactly at the boundary (10 executions should be allowed)
			for (let i = 0; i < 10; i++) {
				expect(() => checkForRapidReExecution('BoundaryNode', i)).not.toThrow();
			}

			// The 11th execution should trigger
			expect(() => checkForRapidReExecution('BoundaryNode', 10)).toThrow();
		});

		test('should handle time window edge cases correctly', () => {
			const checkForRapidReExecution = (
				workflowExecute as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			...
			// Execute rapidly within time window
			for (let i = 0; i < 10; i++) {
				checkForRapidReExecution('TimeWindowNode', i);
				mockTime += 100; // Total: 1 second
			}

			// Move time exactly to window boundary (5000ms)
			mockTime = 1000000000 + 5000;

			// This should still be within window and trigger detection
			expect(() => checkForRapidReExecution('TimeWindowNode', 10)).toThrow();

			// Move well past window boundary to ensure cleanup
			mockTime = 1000000000 + 10000; // 10 seconds later

			// This should not trigger (old entries cleaned up)
			expect(() => checkForRapidReExecution('TimeWindowNode', 11)).not.toThrow();

			Date.now = originalDateNow;
		});

		// Error Handling and Recovery Tests
		test('should provide accurate execution metrics in error messages', () => {
			const checkForRapidReExecution = (
				workflowExecute as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			...
			// Execute with precise timing for metric verification
			const intervalMs = 250; // 4 executions per second
			for (let i = 0; i < 10; i++) {
				checkForRapidReExecution('MetricsNode', i);
				mockTime += intervalMs;
			}

			try {
				checkForRapidReExecution('MetricsNode', 10);
				throw new Error('Expected OperationalError');
			} catch (error) {
				const timeSpanSeconds = (10 * intervalMs) / 1000; // Should be 2.5 seconds
				const expectedRate = Math.round(11 / timeSpanSeconds); // 11 executions in timespan

				expect(error.message).toContain('executed 11 times');
				expect(error.message).toContain('in 2.500 seconds'); // Match the .toFixed(3) format
				expect(error.message).toContain(`(${expectedRate}/sec)`);
				expect(error.message).toContain('Check for recursive connections');
				expect(error.message).toContain('Consider reviewing your workflow logic');
			}
		...
		test('should handle cleanup during error conditions', () => {
			const checkForRapidReExecution = (
				workflowExecute as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			...
			// Mock time to prevent automatic cleanup during execution
			const originalDateNow = Date.now;
			let mockTime = 4000000000;
			Date.now = jest.fn(() => mockTime);

			// Execute to trigger error
			for (let i = 0; i < 10; i++) {
				checkForRapidReExecution('ErrorCleanupNode', i);
				mockTime += 10; // Small increment to keep within time window
			}

			// Verify history exists before error
			expect(getNodeExecutionHistory().get('ErrorCleanupNode')).toBeDefined();
			expect(getNodeExecutionHistory().get('ErrorCleanupNode')!.length).toBe(10);

			// Trigger error
			expect(() => checkForRapidReExecution('ErrorCleanupNode', 10)).toThrow();

			// History should still be maintained after error for potential investigation
			expect(getNodeExecutionHistory().get('ErrorCleanupNode')).toBeDefined();

			Date.now = originalDateNow;
		});

		test('should allow recovery after time window expires', () => {
			const checkForRapidReExecution = (
				workflowExecute as unknown as {
					checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
				}
			...
			const originalDateNow = Date.now;
			let mockTime = 3000000000;
			Date.now = jest.fn(() => mockTime);

			// Trigger loop detection
			for (let i = 0; i <= 10; i++) {
				if (i < 10) {
					expect(() => checkForRapidReExecution('RecoveryNode', i)).not.toThrow();
				} else {
					expect(() => checkForRapidReExecution('RecoveryNode', i)).toThrow();
				}
				mockTime += 100;
			}

			// Wait beyond time window
			mockTime += 5500; // 5.5 seconds later

			// Should be able to execute again after cleanup
			for (let i = 0; i < 10; i++) {
				expect(() => checkForRapidReExecution('RecoveryNode', 20 + i)).not.toThrow();
				mockTime += 100;
			}
		...
		// Integration with existing loop detection
		test('should work alongside original loop detection mechanism', () => {
			// This test verifies that both detection mechanisms can coexist
			// The original detection checks for immediate consecutive execution
			// The enhanced detection checks for rapid re-execution patterns
			...
			const originalDateNow = Date.now;
			let mockTime = 5000000000;
			Date.now = jest.fn(() => mockTime);

			// Enhanced detection should trigger first in rapid execution scenario
			for (let i = 0; i < 10; i++) {
				expect(() => checkForRapidReExecution('IntegrationNode', i)).not.toThrow();
				mockTime += 10; // Small increment to keep within time window
			}

			// Enhanced detection should trigger
			expect(() => checkForRapidReExecution('IntegrationNode', 10)).toThrow();

			// Both mechanisms should be independent and complementary
			// Original detection would catch immediate loops (runIndex stays same)
			// Enhanced detection catches rapid re-execution (runIndex increases rapidly)

			Date.now = originalDateNow;
		});

		// Iteration 2: Execution Path Tracking Tests
		describe('Execution path cycle detection', () => {
			...
			test('should detect multi-node execution cycles (A→B→C→A pattern)', () => {
				const checkForExecutionPathCycles = (
					workflowExecute as unknown as {
						checkForExecutionPathCycles: (nodeName: string, runIndex: number) => void;
					}
				).checkForExecutionPathCycles.bind(workflowExecute);

				// Simulate a 3-node cycle: NodeA → NodeB → NodeC → NodeA (repeated multiple times)
				const runIndex = 1;

				// Execute the cycle multiple times to build up enough history
				const cycle = ['NodeA', 'NodeB', 'NodeC'];

				// Execute the 3-node cycle multiple times - detection should trigger
				let detectionTriggered = false;
				for (let cycleCount = 0; cycleCount < 5 && !detectionTriggered; cycleCount++) {
					for (let nodeIndex = 0; nodeIndex < cycle.length; nodeIndex++) {
						const nodeName = cycle[nodeIndex];
						try {
							checkForExecutionPathCycles(nodeName, runIndex);
							mockTime += 100;
						} catch (error) {
							// Detection triggered - this is expected after enough cycles
							expect(error.constructor.name).toBe('OperationalError');
							expect(error.message).toContain('Multi-node execution cycle detected');
							expect(error.message).toContain('NodeA → NodeB → NodeC');
				...
				// Verify that detection was triggered
				expect(detectionTriggered).toBe(true);
			});

			test('should detect complex execution cycle patterns', () => {
				const checkForExecutionPathCycles = (
					workflowExecute as unknown as {
						checkForExecutionPathCycles: (nodeName: string, runIndex: number) => void;
					}
				...
				// Build up a more substantial pattern to trigger the conservative detection
				// Execute a pattern where NodeX appears with consistent intervals: NodeX → A → B → NodeX → A → B...
				const pattern = ['NodeX', 'IntermediateA', 'IntermediateB'];

				// Execute this pattern multiple times to build substantial history
				let detectionTriggered = false;
				for (let i = 0; i < 8 && !detectionTriggered; i++) {
					// 8 cycles = 24 total executions
					for (const nodeName of pattern) {
						try {
							checkForExecutionPathCycles(nodeName, runIndex);
							mockTime += 50;
						} catch (error) {
							// Detection triggered - this is expected after enough history
							expect(error.constructor.name).toBe('OperationalError');
			...
			test('should handle path depth limits and memory cleanup', () => {
				const checkForExecutionPathCycles = (
					workflowExecute as unknown as {
						checkForExecutionPathCycles: (nodeName: string, runIndex: number) => void;
					}
				...
				// Execute many nodes to test path depth limiting
				for (let i = 0; i < 60; i++) {
					// Exceeds MAX_PATH_DEPTH (50)
					expect(() => checkForExecutionPathCycles(`Node${i}`, runIndex)).not.toThrow();
					mockTime += 10;
				}

				// Should not crash and should still function
				expect(() => checkForExecutionPathCycles('FinalNode', runIndex)).not.toThrow();
			});

			test('should handle different execution contexts separately', () => {
				const checkForExecutionPathCycles = (
					workflowExecute as unknown as {
						checkForExecutionPathCycles: (nodeName: string, runIndex: number) => void;
					}
				...
				// Different run indices should be tracked in separate paths
				expect(() => checkForExecutionPathCycles('NodeA', 1)).not.toThrow();
				expect(() => checkForExecutionPathCycles('NodeB', 1)).not.toThrow();
				expect(() => checkForExecutionPathCycles('NodeA', 2)).not.toThrow(); // Different runIndex
				expect(() => checkForExecutionPathCycles('NodeB', 2)).not.toThrow();

				// Should not interfere with each other
				mockTime += 5000; // Advance time significantly
				expect(() => checkForExecutionPathCycles('NodeC', 1)).not.toThrow();
				expect(() => checkForExecutionPathCycles('NodeC', 2)).not.toThrow();
			});

			test('should cleanup old paths to prevent memory leaks', () => {
				const checkForExecutionPathCycles = (
					workflowExecute as unknown as {
						checkForExecutionPathCycles: (nodeName: string, runIndex: number) => void;
						executionPathTracking: Map<
							string,
				...
				).executionPathTracking;

				// Create many path entries
				for (let i = 0; i < 150; i++) {
					// Exceeds PATH_CLEANUP_THRESHOLD (100)
					expect(() => checkForExecutionPathCycles(`Node${i}`, i)).not.toThrow();
					mockTime += 100; // Advance time for each execution
				}

				// Advance time significantly to make old paths eligible for cleanup
				mockTime += 20000; // More than 2x LOOP_TIME_WINDOW_MS

				// Add one more execution to trigger cleanup
				expect(() => checkForExecutionPathCycles('CleanupTrigger', 999)).not.toThrow();

				// Check that cleanup occurred - exact size depends on cleanup logic
				// but it should be less than the peak we created
				expect(executionPathTracking.size).toBeLessThan(150);
			});

			test('should work alongside rapid re-execution detection', () => {
				// Both detection methods should work together without interference
				const checkForRapidReExecution = (
					workflowExecute as unknown as {
						checkForRapidReExecution: (nodeName: string, runIndex: number) => void;
					}
				...
				// Simulate normal execution - neither should trigger
				expect(() => {
					checkForRapidReExecution('NodeA', 1);
					checkForExecutionPathCycles('NodeA', 1);
				}).not.toThrow();

				mockTime += 100;

				expect(() => {
					checkForRapidReExecution('NodeB', 1);
					checkForExecutionPathCycles('NodeB', 1);
				}).not.toThrow();

				// Both mechanisms should be independent
				// Test that path detection doesn't interfere with rapid detection thresholds
				for (let i = 2; i < 10; i++) {
					mockTime += 10; // Keep within rapid execution timeframe
					expect(() => {
						checkForRapidReExecution('TestNode', i);
						checkForExecutionPathCycles('TestNode', i);
					}).not.toThrow();
				}

				// The 11th rapid execution should trigger rapid detection first
				// (since rapid detection has lower thresholds)
				mockTime += 10;
				try {
					checkForRapidReExecution('TestNode', 10);
					// If rapid detection doesn't trigger, path detection might
					checkForExecutionPathCycles('TestNode', 10);
					throw new Error('Expected either rapid re-execution or path detection to trigger');
				} catch (error) {
					// Either detection mechanism should trigger, or if neither triggers that's also fine
					// since both are tuned to be conservative to avoid false positives
					const message = (error as Error).message;
					expect(
						message.includes('Infinite loop detected') ||
							message.includes('cycle') ||
							message.includes('Expected either rapid re-execution or path detection to trigger'),
					).toBe(true);
				}
			});
			...
			test('should handle clock skew gracefully', () => {
				...
				// Test with negative timestamp (clock skew)
				Date.now = jest.fn(() => -1000);
				expect(() => checkForExecutionPathCycles('SkewTest', 1)).not.toThrow();

				// Test with future timestamp (clock skew)
				Date.now = jest.fn(() => mockTime + 2000);
				expect(() => checkForExecutionPathCycles('SkewTest', 2)).not.toThrow();
