...
export abstract class NodeExecutionContext implements Omit<FunctionsBase, 'getCredentials'> {
	...
	protected async _getCredentials<T extends object = ICredentialDataDecryptedObject>(
		...
		// Eval bypass: only mock when the node is fully unconfigured, so
		...
		// `evalLlmMockHandler` is only set by eval execution services. Handler
		// presence (not mode) is the discriminator: agent-tool eval runs execute
		// in 'internal' mode and their workflow-tool sub-executions in
		// 'chat'/'trigger'/'manual'.
