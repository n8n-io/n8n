...
export async function proxyRequestToAxios(
	...
	await validateSsrfUrl(url, ssrfBridge);
...
async function validateSsrfUrl(url: string | undefined, ssrfBridge?: SsrfBridge): Promise<void> {
	if (!ssrfBridge || !url) return;
	const result = await ssrfBridge.validateUrl(url);
	if (!result.allowed) {
...
async function httpRequestImpl(
	...
	const url = requestOptions.baseURL
		? new URL(requestOptions.url ?? '', requestOptions.baseURL).href
		: requestOptions.url;
	await validateSsrfUrl(url, ssrfBridge);
...
/** Module-level export for internal/system calls (no SSRF protection). */
export async function httpRequest(
	requestOptions: IHttpRequestOptions,
): Promise<IN8nHttpFullResponse | IN8nHttpResponse> {
	return await httpRequestImpl(requestOptions);
}
...
export async function httpRequestWithAuthentication(
	...
	try {
		...
		return await httpRequestImpl(requestOptions, additionalData.ssrfBridge);
	} catch (error) {
...
export const getRequestHelperFunctions = (
	...
	return {
		httpRequest: async (requestOptions: IHttpRequestOptions) =>
			await httpRequestImpl(requestOptions, additionalData.ssrfBridge),
