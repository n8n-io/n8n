# INS-1515: Store isolated workflow suggestions for review

A suggestion is a separate proposed change for review. The workflow's saved editor draft keeps its existing name.

[Linear ticket](https://linear.app/n8n/issue/INS-1515)

Status: implemented in this worktree. The pseudocode explains the design. See [the module README](packages/cli/src/modules/workflow-suggestions/README.md) for the implemented APIs and feature gate.

Implementation details: suggestion revisions use a conditional database update. Required validation runs before storage. Configuration diagnostics and execution verification remain `not_run` until the later integration tickets. Historical identity columns have no foreign keys so receipts survive target deletion. Suggestions retain their original project when a workflow moves.
...
Suggestion operations must leave the saved workflow, history, editor canvas, and production workflow unchanged. Build and test this with a sample fix before connecting the Assistant.

This service stores fix suggestions and **Fix ready** proposals. M2 stores **Needs you** and **Could not fix** results without proposals. INS-1517 defines the shared three-kind inbox contract. M5 adds informational detail and private chat handoff. Automatic rollout waits until all three outcomes are visible.
...
| Retention | Expire abandoned preparing suggestions after 7 days of inactivity. Keep pending content. Remove closed content after 30 days. |
...
| Piece | Owns |
| --- | --- |
| `WorkflowSuggestionService` | Create, read, revise, and submit. Coordinate domain checks. |
| `WorkflowSuggestionRepository` | Suggestion and activity persistence. Conditional updates, uniqueness, and database locks. |
...
| `WorkflowSuggestionCleanupTask` | Schedule bounded cleanup through the existing system-task infrastructure. |

Keep access helpers as functions or a small service where reuse requires it. Keep activity persistence in the suggestion repository initially. Add another class only when it has a distinct responsibility.
...
Use explicit inputs. Pass a small source identity from trusted backend code. Do not pass the full `InstanceAiContext` to suggestion storage. Keep the source identity separate from the database `OperationContext`.
...
Use one suggestion record through preparation and review. Submission changes its state. It does not copy the suggestion into another proposal system.
...
- **Identity:** suggestion ID, source key, workflow ID, project ID, background user ID, and Assistant authorship.
...
Add a proposal-owned activity table. Record submission with the suggestion transition. Later actions record the human actor separately.
...
## 2. Create and read a preparing suggestion

M2 captures the expected IDs and checksum when it admits the investigation, before the Assistant starts reasoning. It passes those same values after queue waits or restarts. For the M1 sample, capture them immediately before creating the suggestion.

Resolve the target project on the server. Check the background user's current access. Compare all three expected baseline values with a consistent workflow read before storing its content. A settings edit between investigation admission and suggestion creation must reject the old baseline.

```typescript
async createSuggestion(source: SuggestionSource) {
  await access.requireBackgroundEditor(source);

  const existing = await suggestions.findBySourceKey(source.sourceKey);
  if (existing) {
    assertSameSource(existing, source);
    return existing;
  }

  const baseline = await workflows.captureEligibleSuggestionBaseline(source);

  return suggestions.createOnce({
    source,
...
`captureEligibleSuggestionBaseline` checks the expected saved ID, published ID, and `await calculateWorkflowChecksum(workflow)`. It also checks required publication state, archive state, and ownership.
...
An internal suggestion read checks the source binding and current background-user access. It returns the latest candidate and revision. Preparing suggestions stay out of the review inbox.
...
```typescript
async reviseSuggestion(source: SuggestionSource, input: ReviseSuggestionInput) {
  const suggestion = await loadSuggestionForBackgroundUser(source, input.suggestionId);
  requirePreparingContent(suggestion);
  requireRevision(suggestion, input.expectedRevision);

  const prepared = await candidates.prepare({
    actorId: suggestion.backgroundUserId,
    projectId: suggestion.projectId,
    baseline: suggestion.originalSnapshot,
    graph: input.graph,
  });
  requirePassed(prepared.validation.requiredChecks);

  return suggestions.reviseIfCurrent({
...
The repository updates only a preparing suggestion whose revision still matches. It increments the revision and attaches validation to that same revision in one write. A conflicting edit returns a revision conflict.
...
M2 validates the final result on the server before it accepts Fix ready and submits the suggestion. The model selecting Fix ready is not sufficient proof that the candidate is valid.
...
async prepare(input: CandidateInput) {
  ...
  const validation = await checkRequiredSuggestionRules(candidate, input);
...
```typescript
async submitSuggestion(source: SuggestionSource, suggestionId: string, revision: number) {
  const checkedSuggestion = await loadSuggestionForBackgroundUser(source, suggestionId);
  if (checkedSuggestion.state === 'preparing') {
    requirePreparingContent(checkedSuggestion);
    requireRevision(checkedSuggestion, revision);
    requireValidatedChangedCandidate(checkedSuggestion);
    await candidates.assertStillAllowed(checkedSuggestion);
  }

  return txRunner.run({}, async (ctx) => {
    const submission = await suggestions.loadForSubmission(suggestionId, ctx);
    const { suggestion, workflow } = submission;
    assertSameSource(suggestion, source);

    if (suggestion.state !== 'preparing') {
      return recordedResult(suggestion);
    }

    requirePreparingContent(suggestion);
    requireRevision(suggestion, revision);
    requireValidatedChangedCandidate(suggestion);

    if (!(await baselineMatches(suggestion, workflow, ctx))) {
      return suggestions.closeAsOutdated(suggestion.id, ctx);
    }

    const proposal = await suggestions.markPendingIfCurrent(suggestion.id, revision, ctx);
    await suggestions.appendSubmittedActivity(proposal.id, revision, ctx);
...
- Use a conditional state-and-revision update to freeze the suggestion.
...
async getProposal(viewer: User, proposalId: string) {
  const proposal = await suggestions.get(proposalId);
...
Return the original and proposed snapshots, explanation, validation report, error summary, status, and activity. Assemble the proposed snapshot from the stored baseline and frozen graph. Do not fetch its content from workflow history. Closed preparing suggestions with no submitted revision are not review proposals.
...
```typescript
type SuggestionLifecycleResult = {
...
async getLifecycleResult(source: SuggestionSource) {
  const suggestion = await suggestions.findBySourceKey(source.sourceKey);
...
This is an internal backend lookup. It returns no suggestion payload or private thread. M2 and M3 can recover after a restart or background-user access loss. Cleanup preserves the result. Events can prompt a refresh but are not the source of truth.
...
async cleanup(now: Date) {
  const candidates = await suggestions.findCleanupCandidates(now, { limit: 100 });

  for (const candidate of candidates) {
    await suggestions.cleanupIfStillEligible(candidate.id, {
...
The conditional cleanup operation closes abandoned preparing suggestions and removes their content. It removes closed content only after 30 days. It preserves pending content. Successful revisions refresh suggestion activity; an ordinary detail read does not keep abandoned work alive.
...
2. Implement create, read, and revise with a sample suggestion.
...
| Original baseline | Reject settings or graph changes between investigation admission and suggestion creation. Bind source retries to the same original IDs and checksum. |
...
Enable the module with `N8N_ENABLED_MODULES=workflow-suggestions`. The Assistant, review UI, and Apply actions remain in the tickets below.
...
| [INS-1479](https://linear.app/n8n/issue/INS-1479) | Assistant suggestion tools, compilation, and shared diagnostic integration. |
| [INS-1480](https://linear.app/n8n/issue/INS-1480) | Investigation tracking, workflow claims, original baseline capture, and the three investigation outcomes. Only Fix ready submits a suggestion. |
