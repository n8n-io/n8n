...
- To find which telemetry events n8n emits (names, descriptions, properties),
  run `pnpm --filter @n8n/telemetry catalog` (`--json` for structured output)
  instead of searching the code. The `@n8n/telemetry` package is the event
  registry; the `n8n:telemetry` skill covers adding or changing events
