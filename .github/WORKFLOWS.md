...
iterations, experiment-name, model, model-url, vertex-project, vertex-location), no per-PR cancellation (dispatches run in parallel, e.g. concurrent
...
| AI/Evals            | `EVALS_ANTHROPIC_KEY`, `EVALS_OPENAI_KEY`, `EVALS_OPENROUTER_KEY`, `EVALS_XAI_KEY`, `EVALS_BASETEN_KEY`, `EVALS_FIREWORKS_KEY`, `EVALS_WAFER_KEY`, `EVALS_MORPH_KEY`, `EVALS_TOGETHER_KEY`, `EVALS_DATABRICKS_KEY`, `EVALS_MODAL_KEY`, `EVALS_AZURE_FOUNDRY_KEY`, `EVALS_VERTEX_SA_JSON`, `EVALS_VERTEX_PROJECT`, `EVALS_LANGSMITH_*` |
